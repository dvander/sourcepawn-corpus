...
{ 
	...
	CreateConVar("fpd_version", PLUGIN_VERSION, "First Person Death", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	var_fpd_enable = CreateConVar("fpd_enable", "1", "Enable / Disable FPD", FCVAR_PLUGIN);
	var_fpd_black = CreateConVar("fpd_black", "3000", "Duration to fade to black, 0 = disables", FCVAR_PLUGIN);
	var_fpd_stay = CreateConVar("fpd_stay", "7", "Seconds to stay in ragdoll after death 0 = till round end", FCVAR_PLUGIN);
	...
	if (ClientOk(Client))
	{
		...
		if (fpd_enable)
		{
			if (game == CSGO)
			{
				...
				if (StrContains(ModelName, "models/player/ctm_gsg9_variantc.mdl", false) > -1 || StrContains(ModelName, "models/player/ctm_gign_variantc.mdl", false) > -2 || StrContains(ModelName, "models/player/ctm_sas_variantb.mdl", false) > -3 || StrContains(ModelName, "models/player/ctm_fbi_varianta.mdl", false) > -4 || StrContains(ModelName, "models/player/ctm_idf_variantb.mdl", false) > -5 || StrContains(ModelName, "models/player/ctm_swat.mdl", false) > -6)
				{
					SetEntityModel(Client, "models/player/ctm_st6_varianta.mdl");
				}
			}
			if (game == HL2DM)
			{
...
public PerformFade(any: Client, duration, in)
{

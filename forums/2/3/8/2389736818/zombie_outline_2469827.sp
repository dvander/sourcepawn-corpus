...
{
	SetConVarInt(h_BotJoinAfterPlayer, 0, true);
	SetConVarInt(h_BotDeferToHuman, 1, true);
	SetConVarString(h_BotChatter, "off");
	SetConVarInt(h_HudHintSound, 0, true);
}
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (clientTeam == 3)
	{
		...
		if (StrEqual(model, "models/player/kirby/admin/admin_phoenix.mdl", false))
			ZO_CombineSoldierDied(client);
		else if (StrContains(model, "admin_phoenix", false) != -1)
			ZO_CombineSoldierDied(client);

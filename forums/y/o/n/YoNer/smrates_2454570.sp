...
#define PLUGIN_VERSION "1.0y"
new Handle:hCVARShowMenu;

public Plugin:myinfo =
{
	name = "Show Players",
	author = "Jaffa, Cvar mod by YoNer",
...
{
	SetConVarString(CreateConVar("sm_rates_version", PLUGIN_VERSION, "Show Rates version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_CHEAT), PLUGIN_VERSION);
	hCVARShowMenu = CreateConVar("sm_rates_menu", "0", "Sets if the menu should be displayed on client connect ( 1-shown 0-not shown(default) )");
...
{
	new String:showmenu[1];
	
	GetConVarString(hCVARShowMenu, showmenu, sizeof(showmenu));
	if (showmenu[1] == 1){
	startmenu(client);
	}
}

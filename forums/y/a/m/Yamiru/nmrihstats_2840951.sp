...
* Changelog
* Jan 01, 2025 - v.0.5.1:
* 	[+] Added GetClientAuthId compatibility wrapper to silence deprecation warnings
* 	[*] Updated SQL callbacks for SourceMod 1.10 (no DB/schema changes)
* Dec 22, 2013 - v.0.5:
...
*	[*] New players now start at the average of all player scores
...
// Compatibility: map deprecated GetClientAuthString to GetClientAuthId (Steam2)
// This keeps old code working and removes deprecation warnings without touching DB.
#define GetClientAuthString(%1,%2,%3) GetClientAuthId(%1, AuthId_Steam2, %2, %3, true)
...
public Plugin:myinfo = 
{
	...
	author = "Stevo.TVR (edited by Yamiru)",
...
public ConnectDatabase()
{
	...
	// DB schema unchanged – this will not touch existing layout if table already exists
...
public void T_LoadPlayer(Handle:owner, Handle:hndl, const String:error[], any:client)
{
...
public void T_AddPlayer(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	if (hndl != INVALID_HANDLE)
	{
		if (SQL_FetchRow(hndl))
...
public Action:Timer_PlayerKillsNotify(Handle:timer)
{
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{
			new change = clientKillPointsSinceNotify[i];
			new kills = clientKillsSinceNotify[i];
			PrintToChat(i, "\x04[Stats]\x01 %s%d point%s (%d) for killing %d zombie%s",
				(change >= 0 ? "+" : ""), change,
				(change != -1 && change != 1 ? "s" : ""),
				clientPoints[i],
				kills,
...
public void T_UpdateTotalQuery(Handle:owner, Handle:hndl, const String:error[], any:client)
{
...
public void T_RankQuery(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	...
	PrintToChatAll("\x01Player \x04%N\x01 is rank \x04%d\x01 of \x04%d\x01 total tracked player%s with \x04%d\x01 point%s and is \x04%d\x01 point%s away from the next rank.",
		client,
		rank,
		totalPlayers, (totalPlayers != 1 ? "s" : ""),
		clientPoints[client], (clientPoints[client] != 1 ? "s" : ""),
		rankdiff, (rankdiff != 1 ? "s" : ""));
}
...
public void T_Top10Query(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	if (hndl == INVALID_HANDLE || !IsClientInGame(client))
		return;
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (attacker != 0 && attacker != client && !IsFakeClient(attacker) && IsClientAuthorized(attacker))
	{
		...
		PrintToChat(attacker, "\x04[Stats]\x01 %s%d point%s (%d) for killing a teammate!",
			(change >= 0 ? "+" : ""),
			change,
			(change != -1 && change != 1 ? "s" : ""),
			clientPoints[attacker]);
	...
	PrintToChat(client, "\x04[Stats]\x01 %s%d point%s (%d) for getting killed",
		(change >= 0 ? "+" : ""),
		change,
		(change != -1 && change != 1 ? "s" : ""),
		clientPoints[client]);
	...
	decl String:query2[1024], String:authid2[64];
	GetClientAuthString(client, authid2, sizeof(authid2));
	Format(query2, sizeof(query2), "UPDATE nmrihstats SET points = points + %d, deaths = deaths + 1 WHERE steam_id = '%s';", change, authid2);
	SQL_TQuery(hDatabase, T_FastQuery, query2);
...
public Action:Event_PlayerExtracted(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	PrintToChat(client, "\x04[Stats]\x01 %s%d point%s (%d) for getting extracted",
		(change >= 0 ? "+" : ""),
		change,
		(change != -1 && change != 1 ? "s" : ""),
		clientPoints[client]);
...
/*public Action:Event_ObjectiveComplete(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		PrintToChat(i, "\x04[Stats]\x01 %s%d point%s (%d) for completing an objective",
			(change >= 0 ? "+" : ""),
			change,
			(change != -1 && change != 1 ? "s" : ""),
			clientPoints[i]);
...
// generic async callback for SQL_TQuery
public void T_FastQuery(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	// Nothing to do
}

public void UpdatePlayerName(const String:authid[], const String:name[])
{

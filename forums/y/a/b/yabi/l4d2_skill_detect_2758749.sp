...
public OnPluginStart()
{
    ...
    g_hCvarReport = CreateConVar(           "sm_skill_report_enable" ,      "1", "是否开启聊天框显示操作.", FCVAR_NOTIFY, true, 0.0, true, 1.0 );
    
    g_hCvarAllowMelee = CreateConVar(       "sm_skill_skeet_allowmelee",    "1", "是否检测近战击杀飞扑 Hunter", FCVAR_NOTIFY, true, 0.0, true, 1.0 );
    g_hCvarAllowSniper = CreateConVar(      "sm_skill_skeet_allowsniper",   "1", "是否检测狙击爆头击杀飞扑 Hunter", FCVAR_NOTIFY, true, 0.0, true, 1.0 );
    g_hCvarAllowGLSkeet = CreateConVar(     "sm_skill_skeet_allowgl",       "1", "是否检测榴弹击杀飞扑 Hunter", FCVAR_NOTIFY, true, 0.0, true, 1.0 );
    g_hCvarDrawCrownThresh = CreateConVar(  "sm_skill_drawcrown_damage",  "500", "需要造成多少伤害才视为引秒 Witch.", FCVAR_NOTIFY, true, 0.0, false );
    ...
    g_hCvarBHopContSpeed = CreateConVar(    "sm_skill_bhopkeepspeed",     "300", "连跳最小速度，小于这个不会视为连跳.", FCVAR_NOTIFY, true, 0.0, false );
...
stock HandleLevel( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★★{olive}%N{blue}使用近戰秒殺了正在衝鋒的{olive}%N", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★★{olive}%N{blue}使用近戰秒殺了正在衝鋒的{olive}Charger", attacker );
        }
    }
...
stock HandleLevelHurt( attacker, victim, damage )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★ {olive}%N {blue}使用近戰砍死了衝鋒的{olive}%N {default}(傷害{blue}%i{default})", attacker, victim, damage );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★ {olive}%N {blue}使用近戰砍死了衝鋒的{olive}牛 {default}(傷害{blue}%i{default})", attacker, damage );
        }
    }
...
HandleTongueCut( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N{blue}砍斷了{olive}%N{default}的舌頭", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N{blue}砍斷了{olive}smoker{default}的舌頭", attacker, victim );
        }
    }
...
HandleSmokerSelfClear( attacker, victim, bool:withShove = false )
{
    ...
    if (GetConVarBool(g_hCvarReport)) 
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★ {olive}%N {default}被{olive}%N{default}的舌頭拉後自救 {blue}%s", attacker, victim, (withShove) ? "推" : "" );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★ {olive}%N {default}被{olive}Somker{default}的舌頭拉後自救 {blue}%s", attacker, (withShove) ? "推" : "" );
        }
    }
...
HandleRockSkeeted( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        ...
        */
        CPrintToChatAll( "{green}★ {olive}%N {blue}擊碎了{default}坦克石頭", attacker );
    }
...
stock HandleHunterDP( attacker, victim, actualDamage, Float:calculatedDamage, Float:height, bool:playerIncapped = false )
{
    ...
    if (GetConVarBool(g_hCvarReport)
        ...
        {
            CPrintToChatAll( "{green}★★ {olive}%N {red}高空飛撲砸到了 {olive}%N {default}({red}%i {default}傷害, 高度: {red}%i{default})", attacker,  victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            CPrintToChatAll( "{green}★★ {olive}A hunter {red}高空飛撲砸到了 {olive}%N {default}({red}%i {default}傷害, 高度: {red}%i{default})", victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
    }
...
stock HandleJockeyDP( attacker, victim, Float:height )
{
    ...
    if (    GetConVarBool(g_hCvarReport)
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(attacker) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N {red}高空飛撲砸到了 {olive}%N {default}({red}高度{default}: {red}%i{default})", attacker,  victim, RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            CPrintToChatAll( "{green}★★★ {olive}A jockey {red}高空飛撲砸到了 {olive}%N {default}({red}高度{default}: {red}%i{default})", victim, RoundFloat(height) );
        }
    }
...
stock HandleDeathCharge( attacker, victim, Float:height, Float:distance, bool:bCarried = true )
{
    ...
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(attacker) )
        {
            CPrintToChatAll( "{green}★★★★ {olive}%N {red}衝鋒秒殺了 {olive}%N{default} %s({red}高度{default}: {red}%i{default})",
                    attacker,
                    victim,
                    (bCarried) ? "攜帶" : "撞飛",
                    RoundFloat(height)
        ...
        {
            CPrintToChatAll( "{green}★★★★ {olive}A charger {red}衝鋒秒殺了 {olive}%N{default} %s({red}高度{default}: {red}%i{default})",
                    victim,
                    (bCarried) ? "攜帶" : "撞飛",
                    RoundFloat(height) 
                );
    ...
    if ( attacker != pinVictim && GetConVarBool(g_hCvarReport))
    {
        ...
        if ( fClearTime != -1.0 && fClearTime <= fMinTime )
        {
            if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
            {
                ...
                {
                    CPrintToChatAll( "{green}★ {olive}%N{default} 在{blue}%.2f {default}秒内從{olive}%N{default}(%s) 手里救出了被控的{olive} %N",
                            attacker, fClearTime, victim,
                            g_csSIClassName[zombieClass],
                            pinVictim
                        );
                } else {
                    CPrintToChatAll( "{green}★ {olive}%N{default} 在{blue}%.2f {default}秒内從{olive}%N{default}(%s) 手里救出了被控的{olive} 隊友",
                            attacker, fClearTime,
                            victim,
                            g_csSIClassName[zombieClass]
                        );
            ...
            else if ( IS_VALID_INGAME(attacker) )
            {
                if ( IS_VALID_INGAME(pinVictim) )
                {
                    CPrintToChatAll( "{green}★ {olive}%N{default} 在{blue}%.2f {default}秒内從{olive}特感{default}%s 手里救出了被控的{olive} %N",
                            attacker, fClearTime, victim,
                            g_csSIClassName[zombieClass],
                            pinVictim
                        );
                } else {
                    CPrintToChatAll( "{green}★ {olive}%N{default} 在{blue}%.2f {default}秒内從{olive}特感{default}%s 手里救出了被控的{olive} 隊友",
                            attacker, fClearTime,
                            victim,
                            g_csSIClassName[zombieClass]
                        );
...
stock HandleBHopStreak( survivor, streak, Float: maxVelocity )
{
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        CPrintToChat(survivor, "{green}★ {olive}你 {default}連跳了 {blue}%i次 {default} ({blue}最高速度: {olive}%.1f{default})",
                streak,
                maxVelocity
            );
    }

...
public OnPluginStart()
{
    ...
    g_hCvarReport = CreateConVar(           "sm_skill_report_enable" ,      "0", "是否开启聊天框显示操作.", FCVAR_NOTIFY, true, 0.0, true, 1.0 );
...
stock HandleLevel( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★★{olive}%N{blue}使用近战秒杀了正在冲锋的{olive}%N", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★★{olive}%N{blue}使用近战武器秒了正在冲锋的{olive}牛", attacker );
        }
    }
...
stock HandleLevelHurt( attacker, victim, damage )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★ {olive}%N {blue}使用近战砍死了冲锋的{olive}%N {default}(伤害{blue}%i{default})", attacker, victim, damage );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★ {olive}%N {blue}使用近战砍死了冲锋的{olive}牛 {default}(伤害{blue}%i{default})", attacker, damage );
        }
    }
...
HandleTongueCut( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N{blue}砍断了{olive}%N{default}的舌头", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N{blue}砍断了{olive}smoker{default}的舌头", attacker, victim );
        }
    }
...
HandleSmokerSelfClear( attacker, victim, bool:withShove = false )
{
    ...
    if (GetConVarBool(g_hCvarReport)) 
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★ {olive}%N {default}被{olive}%N{default}的舌头拉后自救 {blue}%s", attacker, victim, (withShove) ? "推" : "" );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★ {olive}%N {default}被{olive}舌头{default}的舌头拉后自救 {blue}%s", attacker, (withShove) ? "推" : "" );
        }
    }
...
HandleRockSkeeted( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        ...
        */
        CPrintToChatAll( "{green}★ {olive}%N {blue}击碎了{default}坦克石头", attacker );
    }
...
stock HandleHunterDP( attacker, victim, actualDamage, Float:calculatedDamage, Float:height, bool:playerIncapped = false )
{
    ...
    if (GetConVarBool(g_hCvarReport)
        ...
        {
            CPrintToChatAll( "{green}★★ {olive}%N {red}高空飞扑砸到了 {olive}%N {default}({red}%i {default}伤害, 高度: {red}%i{default})", attacker,  victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            CPrintToChatAll( "{green}★★ {olive}A hunter {red}高空飞扑砸到了 {olive}%N {default}({red}%i {default}伤害, 高度: {red}%i{default})", victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
    }
...
stock HandleJockeyDP( attacker, victim, Float:height )
{
    ...
    if (    GetConVarBool(g_hCvarReport)
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(attacker) )
        {
            CPrintToChatAll( "{green}★★★ {olive}%N {red}高空飞扑砸到了 {olive}%N {default}({red}高度{default}: {red}%i{default})", attacker,  victim, RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            CPrintToChatAll( "{green}★★★ {olive}A jockey {red}高空飞扑砸到了 {olive}%N {default}({red}高度{default}: {red}%i{default})", victim, RoundFloat(height) );
        }
    }
...
stock HandleDeathCharge( attacker, victim, Float:height, Float:distance, bool:bCarried = true )
{
    ...
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        {
            CPrintToChatAll( "{green}★★★★ {olive}A charger {red}冲锋秒杀了 {olive}%N{default} %s({red}高度{default}: {red}%i{default})",
                    victim,
                    (bCarried) ? "携带" : "撞飞",
                    RoundFloat(height) 
                );
    ...
    if ( attacker != pinVictim && GetConVarBool(g_hCvarReport))
    {
        ...
        if ( fClearTime != -1.0 && fClearTime <= fMinTime )
        {
            if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
            {
                ...
                {
                    CPrintToChatAll( "{green}★ {olive}%N{default} 在{blue}%.2f {default}秒内从{olive}%N{default}(%s) 手里救出了被控的{olive} %N",
                            attacker, fClearTime, victim,
                            g_csSIClassName[zombieClass],
                            pinVictim
                        );
                } else {
                    CPrintToChatAll( "{green}★ {olive}%N{default} 在{blue}%.2f {default}秒内从{olive}%N{default}(%s) 手里救出了被控的{olive} 队友",
                            attacker, fClearTime,
                            victim,
                            g_csSIClassName[zombieClass]
                        );
            ...
            else if ( IS_VALID_INGAME(attacker) )
            {
                if ( IS_VALID_INGAME(pinVictim) )
                {
                    CPrintToChatAll( "{green}★ {olive}%N{default} 在{blue}%.2f {default}秒内从{olive}特感{default}%s 手里救出了被控的{olive} %N",
                            attacker, fClearTime, victim,
                            g_csSIClassName[zombieClass],
                            pinVictim
                        );
                } else {
                    CPrintToChatAll( "{green}★ {olive}%N{default} 在{blue}%.2f {default}秒内从{olive}特感{default}%s 手里救出了被控的{olive} 队友",
                            attacker, fClearTime,
                            victim,
                            g_csSIClassName[zombieClass]
                        );
...
stock HandleBHopStreak( survivor, streak, Float: maxVelocity )
{
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        CPrintToChat(survivor, "{green}★ {olive}你 {default}连跳了 {blue}%i次 {default} ({blue}最高速度: {olive}%.1f{default})",
                streak,
                maxVelocity
            );
    }

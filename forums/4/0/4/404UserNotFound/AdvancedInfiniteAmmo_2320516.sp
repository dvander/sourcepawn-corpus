...
#tryinclude <friendly>   	//Give friendly players infinite ammo!
	...
	switch(args)
	{
		case 0:
		{
			if(g_bInfiniteAmmo[client])
			{
				...
				else ReplyToCommand(client, "[SM] Advanced Infinite Ammo for %N disabled", client);
			...
			else
			{
				...
				else ReplyToCommand(client, "[SM] Advanced Infinite Ammo for %N enabled", client);
			}			
		}
		case 1:
		{
			...
			// {
			// 	if(GetConVarBool(g_hChat)) ShowActivity2(client, "[SM] ","Advanced Infinite Ammo for %s has been toggled", target_name);
			// 	else ReplyToCommand(client, "[SM] Advanced Infinite Ammo for %s has been toggled", target_name);
			// }
			// else
			// {
			if(GetConVarBool(g_hChat)) ShowActivity2(client, "[SM] ","Advanced Infinite Ammo for %s %sabled", target_name, bEnabled?"en":"dis");
			else ReplyToCommand(client, "[SM] Advanced Infinite Ammo for %s %sabled", target_name, bEnabled?"en":"dis");
			// }
		}
		case 2:
		{
			...
			if(bOnOff)
			{
				...
				else ReplyToCommand(client, "[SM] Advanced Infinite Ammo for %s enabled", target_name);
			}
			else 
			{
				for(new i = 0; i < target_count; i++)
				...
				else ReplyToCommand(client, "[SM] Advanced Infinite Ammo for %s disabled", target_name);			
			}
		}
		default:
		{
...
public Action:Command_SetAIATimed(client, args)
{
	...
	else ReplyToCommand(client, "[SM] Advanced Infinite Ammo enabled for %s for %-.2f seconds", target_name, time);
...
GiveInfiniteAmmo(client, iWeapon)
{
	...
	switch(iWeaponIndex)
	{
		...
		case 9,10,11,12,13,16,17,18,19,20,22,23,24,36,45,61,127,130,160,161,
			...
			15001,15002,15003,15005,15006,15009,15011,15012,15013,15014,15015,15016,15017,
			15018,15021,15022,15024,15027,15028,15029,15030,15032,15034,15035,15036,15037,
			15038,15041,15042,15043,15044,15045,15046,15047,15048,15049,15051,15052,
			15053,15054,15056,15057,15058:
		{

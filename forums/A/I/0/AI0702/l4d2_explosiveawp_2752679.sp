/*============================================================================================
							[L4D2] Explosive Sniper Rifle bullets
----------------------------------------------------------------------------------------------
*	Author	:	Eärendil
*	Descrp	:	Sniper Rifle bullets explode on impact.
*	Version	:	1.1
...
ConVar g_hAllow, g_hGameModes, g_hDamage, g_hRadius, g_hDamageFriend, g_hCurrGameMode, g_hAllowAWP, g_hAllowScout, g_hAllowHRifle, g_hAllowMRifle;

bool g_bAllow, g_bPluginOn, g_bBlockExpl[MAXPLAYERS + 1], g_bAllowAWP, g_bAllowScout, g_bAllowHRifle, g_bAllowMRifle;
...
{
	name = "[L4D2] Explosive Sniper Rifle bullets",
	author = "Eärendil",
	description = "Sniper Rifle bullets explode on impact.",
...
{
	CreateConVar("l4d2_explosiveawp_version", PLUGIN_VERSION, "L4D2 Explosive Sniper Rifle bullets Version", FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	g_hAllowAWP			= CreateConVar("l4d2_explawp_allow_awp",		"1",		"If set to 1 AWP will cause explosions.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hAllowScout		= CreateConVar("l4d2_explawp_allow_scout",		"1",		"If set to 1 Scout will cause explosions.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hAllowHRifle		= CreateConVar("l4d2_explawp_allow_hrifle",		"1",		"If set to 1 Hunting Rifle will cause explosions.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hAllowMRifle		= CreateConVar("l4d2_explawp_allow_mrifle",		"1",		"If set to 1 Military Rifle will cause explosions.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	g_hAllowAWP.AddChangeHook(CVarChange_CVars);
	g_hAllowScout.AddChangeHook(CVarChange_CVars);
	g_hAllowHRifle.AddChangeHook(CVarChange_CVars);
	g_hAllowMRifle.AddChangeHook(CVarChange_CVars);
...
void GetCVars()
{
	...
	g_bAllowAWP = g_hAllowAWP.BoolValue;
	g_bAllowScout = g_hAllowScout.BoolValue;
	g_bAllowHRifle = g_hAllowHRifle.BoolValue;
	g_bAllowMRifle = g_hAllowMRifle.BoolValue;
}
...
public Action Event_Bullet_Impact(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (!StrEqual(sWeaponName, "weapon_sniper_awp", false) && !StrEqual(sWeaponName, "weapon_sniper_scout", false) && !StrEqual(sWeaponName, "weapon_hunting_rifle", false) && !StrEqual(sWeaponName, "weapon_sniper_military", false))
		return;
		
	if(StrEqual(sWeaponName, "weapon_sniper_awp", false) && !g_bAllowAWP)
		return;
	
	if(StrEqual(sWeaponName, "weapon_sniper_scout", false) && !g_bAllowScout)
		return;
		
	if(StrEqual(sWeaponName, "weapon_hunting_rifle", false) && !g_bAllowHRifle)
		return;
		
	if(StrEqual(sWeaponName, "weapon_sniper_military", false) && !g_bAllowMRifle)
		return;

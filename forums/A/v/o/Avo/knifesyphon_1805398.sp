...
public OnPluginStart()
{
	...
	cvarTaser = CreateConVar("sm_knifesyphon_taser", "1", "Enables/Disables the KnifeSyphon with a taser (CS:GO).", FCVAR_PLUGIN, true, 0.0, true, 1.0);

	GetGameFolderName(gameFolder, sizeof(gameFolder));
	CreateTimer(3.0, OnPluginStart_Delayed);
	
	AutoExecConfig(true, "knifesyphon");
}
...
WeaponCheck(const String:weapon[])
{
	...
	else if (strcmp(gameFolder, "csgo") == 0) // Counter-Strike: Global Offesive
	{
		if(strcmp(weapon, "knife") == 0 || strcmp(weapon, "knifegg") == 0)
		{
			return 1;
		}
		else if(GetConVarInt(cvarTaser) && strcmp(weapon, "taser") == 0)
		{
			return 1;
		}
	}

...
static const String:weapon_primary[9][27] = {
...
static const String:weapon_secondary[9][27] = {
	...
	"tf_weapon_pistol",
	"tf_weapon_medigun"};
static const String:weapon_tertiary[9][19] = {
	"tf_weapon_bat",
	"tf_weapon_shovel",
	"tf_weapon_fireaxe",
	"tf_weapon_bottle",
...
public OnPluginStart() {
	 ...
	 * SDK Calls *
	 *************/
	new Handle:conf = LoadGameConfigFile("randomizer.games");
	...
	new Handle:cv_fixspy  = CreateConVar("rnd_fix_spy", "1", "0 = don't check, 1 = force undisguise on non-melee attacks, 2 = force undisguise on all attacks", FCVAR_PLUGIN, true, 0.0, true, 2.0);
...
public player_death(Handle:event, const String:name[], bool:dontBroadcast) {
	if (!(GetEventInt(event, "death_flags") & 32)) {
		...
		if (infotarget[client]) RemoveEdict(infotarget[client]);
	}
}
...
public givePlayerWeapons(client) {
	...
	if (pri > -1) {
		...
		if (slot == -1 || !StrEqual(name, weapon_primary[pri])) {
			...
			wpn = SDKCall(GiveNamedItem, client, weapon_primary[pri], 0, 0);
		...
		if (IsClientInGame(i) && IsPlayerAlive(i)) {
			...
			if (cvar_fixammo) {
				...
				if (ammo0new > ammo0old) {
					GetEdictClassname(GetPlayerWeaponSlot(i, 0), weapon, sizeof(weapon));
				...
				if (ammo1new > ammo1old) {
					GetEdictClassname(GetPlayerWeaponSlot(i, 1), weapon, sizeof(weapon));
					if (GetTrieValue(max_ammo, weapon, max)) {
					...
					if (spy_status[i] == 1 && (cond & 12) && (GetClientButtons(i) & IN_ATTACK)) { // Attacking?
						if (cvar_fixspy == 1) {
							...
							if (StrEqual(weapon, "tf_weapon_flamethrower")
								...
								|| StrEqual(weapon, "tf_weapon_pipebomblauncher")) TF2_DisguisePlayer(i, TFTeam:GetClientTeam(i), TFClass_Spy);
						} else {
				...
				if (setclass[i] != _:TFClass_Medic) { // Not set to Medic.
					...
					if (slot > -1) { // Have a secondary weapon.
						...
						if (StrEqual(weapon, "tf_weapon_medigun")) { // It's a medigun.
							...
							if (target != oldtarget) {
								if (heal_beams[i]) {
									AcceptEntityInput(heal_beams[i], "Kill");
									heal_beams[i] = 0;
								}
								healtarget[i] = target;
								if (target != -1) {
									...
									if (IsValidEdict(particle)) {
										...
										FormatEx(controlpoint, sizeof(controlpoint), "player%i", target);
										DispatchKeyValue(target, "targetname", controlpoint);
										// set particle stuff

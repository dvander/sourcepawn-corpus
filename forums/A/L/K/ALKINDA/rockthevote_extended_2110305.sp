...
new g_RTV, g_MapCounter;		// ALKINDA's variables for denying mapchange on third map.

new bool:g_InChange = false;
new Handle:h_RTVTimesAllowed = INVALID_HANDLE;
new g_RTVTimesAllowed;
...
public OnPluginStart()
{
	...
	h_RTVTimesAllowed = CreateConVar("sm_rtv_amount", "2", "Number of times in a row you can rtv", 0, true, 1.0, true, 10.0);
	g_RTVTimesAllowed = GetConVarInt(h_RTVTimesAllowed);
...
public OnMapStart()
{
	g_MapCounter++;//counts map changes
	g_Voters = 0;
	g_Votes = 0;
	g_VotesNeeded = 0;
	g_InChange = false;
...
public OnMapEnd()
{
	g_CanRTV = false;	
	g_RTVAllowed = false;
	if (g_RTV == (g_RTVTimesAllowed) && g_MapCounter == (g_RTVTimesAllowed + 1))//if you started rtv 3 times and map has changed 4 times
	{
		g_RTV = 0;//SET TO ZERO
		g_MapCounter = 0;//SET TO ZERO
	}
}
...
AttemptRTV(client)
{
	//PrintToChatAll("[SM] g_RTV:%i",g_RTV);
	if (g_RTV == g_RTVTimesAllowed)
	{
		PrintToChat(client,"[SM] RTV Not Allowed After %i Times In a Row", g_RTVTimesAllowed );
		return;
	}
...
StartRTV()
{
	g_RTV++;//If rtv was initiated increase counter
	//PrintToChatAll("[SM] g_RTV:%i",g_RTV);

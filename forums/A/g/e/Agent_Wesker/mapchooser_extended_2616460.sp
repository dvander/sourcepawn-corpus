...
//	Call_Finish();
	...
	if (inputlist == INVALID_HANDLE)
	{
		...
		for (new i=0; i<nominationsToAdd; i++)
		{
			...
			int tempIndex;
			tempIndex = FindStringInArray(g_NextMapList, map);
			if (tempIndex != -1)
				RemoveFromArray(g_NextMapList, tempIndex);
...
CreateNextVote()
{
	...
	int tempIndex;
	tempIndex = FindStringInArray(tempMaps, map);
	if (tempIndex != -1)
		RemoveFromArray(tempMaps, tempIndex);

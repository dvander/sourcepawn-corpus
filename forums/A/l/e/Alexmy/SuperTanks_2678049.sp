...
public OnPluginStart()
{
	...
	hWave2Cvar = CreateConVar("st_wave2_tanks", "1", "Default number of tanks in the 2nd wave of finale.",FCVAR_NOTIFY,true,0.0,true,5.0);
	...
	hSpawnFireImmunity = CreateConVar("st_spawn_fire_immunity", "1", "Is Spawn Tank immune to fire?",FCVAR_NOTIFY,true,0.0,true,1.0);
	hSpawnCommonAmount = CreateConVar("st_spawn_common_amount", "10", "Number of common infected spawned by the Spawn Tank.",FCVAR_NOTIFY,true,1.0,true,50.0);
	hSpawnCommonInterval = CreateConVar("st_spawn_common_interval", "10", "Spawn Tanks common infected spawn interval.",FCVAR_NOTIFY,true,1.0,true,60.0);
	...
	hHealHealthCommons = CreateConVar("st_heal_health_commons", "10", "Heal Tanks receive this much health per second from being near a common infected.",FCVAR_NOTIFY,true,0.0,true,1000.0);
	hHealHealthSpecials = CreateConVar("st_heal_health_specials", "100", "Heal Tanks receive this much health per second from being near a special infected.",FCVAR_NOTIFY,true,0.0,true,1000.0);
	hHealHealthTanks = CreateConVar("st_heal_health_tanks", "500", "Heal Tanks receive this much health per second from being near another tank.",FCVAR_NOTIFY,true,0.0,true,1000.0);
	...
	hWitchMaxWitches = CreateConVar("st_witch_max_witches", "30", "Maximum number of witches converted from common infected by the Witch Tank.",FCVAR_NOTIFY,true,0.0,true,100.0);
	...
	if (bSuperTanksEnabled)
	{
		if (client > 0)
		{
			if (IsClientInGame(client))
			{
				if (IsTank(client))
				{
					new index = GetSuperTankByRenderColor(GetColor(client));
					if (index >= 0 && index <= 16)
					{
...
public TankController()
{
	...
	if (iNumTanks > 0)
	{
		for (new i=1; i<=MaxClients; i++)
		{
			if (IsTank(i))
			{
				new index = GetSuperTankByRenderColor(GetColor(i));
				if (index >= 0 && index <= 16)
				{
					if (index != 0 || (index == 0 && bDefaultOverride))
					{
...
{
	if (IsTank(client) && GetColor(client) == 7595105)
	{
...
{
	if (IsTank(client) && GetColor(client) == 12800)
	{
...
{
	if (IsTank(client) && GetColor(client) == 0100170)
	{
...
{
	if (IsTank(client) && GetColor(client) == 12115128)
	{
...
{
	if (IsTank(client) && GetColor(client) == 100165255)
	{
...
{
	if (IsTank(client) && GetColor(client) == 255200255)
	{
...
{
	if (IsTank(client) && GetColor(client) == 1002525)
	{
...
{
	if (IsTank(client) && GetColor(client) == 135205255 && ShieldsUp[client] == 0)
	{
		decl Float:Origin[3];
		GetClientAbsOrigin(client, Origin);
		Origin[2] -= 120.0;
...
{
	if (IsTank(client) && GetColor(client) == 135205255 && ShieldsUp[client] == 1)
	{
		new entity = -1;
		while ((entity = FindEntityByClassname(entity, "prop_dynamic")) != INVALID_ENT_REFERENCE)
		{
	...
	if (bSuperTanksEnabled)
	{
		if (damage > 0.0 && IsValidClient(victim))
		{
			...
			else if (IsTank(victim))
			{
				...
				{
					new index = GetSuperTankByRenderColor(GetColor(victim));
					if (index >= 0 && index <= 16)
					{
						if (bTankFireImmunity[index])
						{

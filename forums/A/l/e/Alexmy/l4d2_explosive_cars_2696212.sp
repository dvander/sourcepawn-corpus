...
public OnPluginStart()
{
	...
	g_cvarMaxHealth = CreateConVar("l4d2_explosive_cars_health", "5000", "Maximum health of the cars");
	g_cvarRadius = CreateConVar("l4d2_explosive_cars_radius", "420", "Maximum radius of the explosion");
	g_cvarPower = CreateConVar("l4d2_explosive_cars_power", "300", "Power of the explosion when the car explodes");
	g_cvarDamage = CreateConVar("l4d2_explosive_cars_damage", "100", "Damage made by the explosion");
	g_cvarTrace = CreateConVar("l4d2_explosive_cars_trace", "25", "Time before the fire trace left by the explosion expires");
	g_cvarPanic = CreateConVar("l4d2_explosive_cars_panic", "1", "Should the car explosion cause a panic event? (1: Yes 0: No)");
	g_cvarPanicChance = CreateConVar("l4d2_explosive_cars_panic_chance", "100", "Chance that the cars explosion might call a horde (%)");
	g_cvarInfected = CreateConVar("l4d2_explosive_cars_infected", "1", "Should infected trigger the car explosion? (1: Yes 0: No)");
	g_cvarTankDamage = CreateConVar("l4d2_explosive_cars_tank", "0", "How much damage do the tank deal to the cars? (0: Default, which is 999 from the engine)");
	g_cvarBurnTimeout = CreateConVar("l4d2_explosive_cars_burntime", "0", "Time to wait before stopping the fire on the exploded car (0: Don't stop)");
	g_cvarUnload = CreateConVar("l4d2_explosive_cars_unload", "none", "On which maps should the plugin disable itself? (Example: c5m3_cemetery, c5m5_bridge, cmdd_custom)");
	g_cvarExplosionDmg = CreateConVar("l4d2_explosive_cars_explosion_damage", "1", "Should cars get damaged by another car's explosion?");
	g_cvarFireDmgInterval = CreateConVar("l4d2_explosive_cars_trace_interval", "0.4", "How often should the fire trace left by the explosion hurt?");
	g_cvarCarFlyMax = CreateConVar("l4d2_explosive_cars_car_max_height", "2500", "Maximum power when the car is launched");
	g_cvarCarFlyMin = CreateConVar("l4d2_explosive_cars_car_min_height", "1000", "Minimum power when the car is launched");
	g_cvarSelfHurt = CreateConVar("l4d2_explosive_cars_fire_selfdamage", "0", "Should the car get damage from its own fire before it explodes? This will eventually end in a explosion");
	g_cvarInfernoNum = CreateConVar("l4d2_explosive_cars_inferno_count", "2", "How many extra infernos (fire) should be created upon explosion?");
	g_cvarFireMulti = CreateConVar("l4d2_explosive_cars_fire_multiplier", "2.42", "Multiplier of the damage received by the car from fire (Note: Normal is 8 damage per 0.5 secs)");
	g_cvarExploMulti = CreateConVar("l4d2_explosive_cars_explosion_multiplier", "45.67", "Multiplier of the damage received by the car from explosions (Pipe bombs, grenade launcher, propane tanks, etc) (Note: Normal is 15-20 damage per shot)");

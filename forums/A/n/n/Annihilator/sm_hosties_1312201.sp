...
public Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (!EST_FOUND) // normal server (no ES_Tools) :)
	{
	...
	else // stupid ES_Tools-server, no giving weapons here... :(
	{
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	decl String:weaponName[100], String:targetip[64], String:attackerip[64], String:targetname[MAX_NAME_LENGTH], String:attackername[MAX_NAME_LENGTH], String:targetid[64], String:attackerid[64], String:killdate[64];
	
	GetClientIP(target, targetip, sizeof(targetip));
	GetClientIP(attacker, attackerip, sizeof(attackerip));
	...
	&& ((LRinprogress == false) || ((LRinprogress == true) && (attacker != LRprogressplayer1) && (attacker != LRprogressplayer2))) )
	{
		...
		if (GetConVarInt(sm_hosties_announce_wpn_attack) == 1)
		{
			if (IsClientInGame(target) && IsPlayerAlive(target))
			{
				for (new s = 0; s < 2; s++)
				{
					...
					{
						hasGun = true;
						PrintToChatAll(MESS, "CT killed T gun", attacker, target);
						break;
					}
				}
			}
		...
		if (!hasGun)
		{
			PrintToChatAll(MESS, "Freekill", attacker, target);
			EmitSoundToAll(freekill_sound);
			decl String:file[PLATFORM_MAX_PATH];
			FormatTime(killdate, sizeof(killdate), "%Y%m%d");
			Format(killdate, sizeof(killdate), "logs/freekills_%s.log", killdate);
			BuildPath(Path_SM, file, sizeof(file), killdate);
			FormatTime(killdate, sizeof(killdate), "%m/%d/%Y - %H:%M:%S");
			LogToFileEx(file, "[%s] %s (%s) [%s] freekilled %s (%s) [%s] with %s ", killdate, attackername, attackerid, attackerip, targetname, targetid, targetip, weaponName);
		}
	}
	...
	if (action == MenuAction_Select)
	{
		if (!LRblock)
		{
			if (!LRinprogress)
			{
				...
				{
					if (param2 == 0 || param2 == 1 || param2 == 2 || param2 == 3 || param2 == 4 || param2 == 5 || param2 == 6 || param2 == 7)
					{

...
public Action:CheckAroundTriggers(Handle:timer)
{
	...
	if (StrContains(mapname, "c1m1_hotel", false) != -1)
	{
		...
		// they leave the elevator on their own
		
		//GetEntityAbsOrigin(FindEntityByClassname(-1, "info_changelevel"), pos1);
		//pos1[2] -= 60.0;
		//SpawnBotBait(pos1);
	}
	...
	if (StrContains(mapname, "c1m2_streets", false) != -1)
	{
		...
		if (CheckforBots(pos2, 200.0) && !WarpTrigger)
		{
			...
			PrintToChatAll("\x04[BOTFIX] \x01Bots will be warped behind the barrier 90 seconds after that");
			...
			CreateTimer(100.0, WarpAllBots, posdata2, TIMER_FLAG_NO_MAPCHANGE);
	...
	//c4m3_sugarmill_b - is fine
	//c4m4_milltown_b - is fine
	//c4m5_milltown_escape - is fine, astonishingly
	//c5m1_waterfront - is fine
	...
	if (StrContains(mapname, "c5m5_bridge", false) != -1)
	{
		...
		else
		{
			...
			{
				PrintToChatAll("\x04[BOTFIX] \x01Bot found ready for Finale. Finale launches in 20 seconds...");
				AcceptEntityInput(button, "Press");
...
stock SpawnBotBait(Float:posdata[3]) // causes Survivor Bots to run to this position in versus
{
	if (!BotBait || !IsClientInGame(BotBait))
	{
		BotBait = CreateFakeClient("BotBait");
		if (!BotBait || !IsClientInGame(BotBait)) LogError("Trigger Fix could not spawn new Botbait!");
		else
		{
			ChangeClientTeam(BotBait, 2);
		}
	}
	
	TeleportEntity(BotBait, posdata, NULL_VECTOR, NULL_VECTOR);
}

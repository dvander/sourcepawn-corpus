...
public Action:Command_NextCampgain(client, args)
{
	...
	if (args == 0 || args > 1) {
		...
		if (foundCampaigns != INVALID_HANDLE && (foundItems = GetArraySize(foundCampaigns)) > 0) {
			...
			if (strcmp(forceNextArg, "cc") == 0 || StrContains("Crash Course", forceNextArg, false) != -1 || StrContains("crash", forceNextArg, false) != -1) {
				forceNext = "crash";
...
GetNextCampaign(String:nextcampaign[64])
{
	...
	if (strcmp(forceNext, "crash") == 0) strcopy(nextcampaign, 64, "Crash Course");
}
...
DisplayCampaignVote()
{
	...
	if (strcmp(map, "crash") != 0) AddMenuItem(g_l4dVoteMenu, "crash", "Crash Course");

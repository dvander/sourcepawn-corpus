...
- version 1.3A:
includes tPonchos new Menu points, bugfixing, nerfed Martial Artists Melee Fatigue to HALF
...
public OnPluginStart()
{
	...
	//Time for exotic oral .. i mean language
...
public Event_PConnect(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	// Bind slots 0, 6-9 on the client
...
public Event_PlayerFirstSpawn (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (PerksConfirmed[client]==0 && IsFakeClient(client)==false)
	{
		...
		PrintHintText(client,"%t", "WelcomeMessageHint");
		PrintToChat(client,"\x03[SM] %t", "WelcomeMessageChat");
	}
}
	...
	for (new i=1 ; i<=18 ; i++)
	{
		...
		if (IsClientInGame(i)==true)
		{
			...
			if (PerksConfirmed[i]==0)
				CreateTimer(1.0,Timer_ShowTopMenu,i);
...
AssignRandomPerks (client)
{
	...
	//randomize a perk
	if (iPerkCount>0)
		SurvivorUpgrade1[client] = iPerkType[ GetRandomInt(1,iPerkCount) ];
	...
	//randomize a perk
	if (iPerkCount>0)
		SurvivorUpgrade2[client] = iPerkType[ GetRandomInt(1,iPerkCount) ];
	...
	//randomize a perk
	if (iPerkCount>0)
		BoomerUpgrade[client] = iPerkType[ GetRandomInt(1,iPerkCount) ];
	...
	//randomize a perk
	if (iPerkCount>0)
		SmokerUpgrade[client] = iPerkType[ GetRandomInt(1,iPerkCount) ];
	...
	//randomize a perk
	if (iPerkCount>0)
		HunterUpgrade[client] = iPerkType[ GetRandomInt(1,iPerkCount) ];
	...
	//5 double the trouble, removed
	
	//randomize a perk
	if (iPerkCount>0)
		TankUpgrade[client] = iPerkType[ GetRandomInt(1,iPerkCount) ];
	...
	SendPanelToClient(Menu_ShowChoices(client),client,Menu_DoNothing,15);
			...
			&& HunterUpgrade[attacker]==1)
			{
				...
				if (iDmgOrig<iMinBound)
				{
					...
					if (iHP>iMinBound)
					{
						...
						PrintHintText(attacker,"Body Slam: %i %t!", iMinBound-iDmgOrig, "BonusDamageText");
					...
					else
					{
						...
						if (flHPBuff>0)
						{
							...
							if (iDmgAdd>=iHP)
							{
								...
								PrintHintText(attacker,"Body Slam: %i %t!", iDmgCount+iDmgAdd, "BonusDamageText");
							...
							else
							{
								...
								SetEntProp(victim,Prop_Data,"m_iHealth", iHP-iDmgAdd );
								SetEventInt(event,"dmg_health", iDmgOrig+iDmgAdd );
								PrintHintText(attacker,"Body Slam: %i %t!", iDmgAdd, "BonusDamageText");
								return Plugin_Continue;
							}
						}
		...
		&& IsPlayerAlive(i)==true)
		{
			...
			if (SpiritCooldown[i]==1)
			{
				...
				if (IsFakeClient(i)==false)
					PrintHintText(i,"You're too tired to self-revive!", "SpiritFailedMessage");
				continue;
			}
			...
			//now to add this revive to the counter, so Survivors get B&W correctly
			...
			if (IsFakeClient(i)==false)
				PrintHintText(i,"Spirit: %t!", "SpritSuccessMessage");
	...
	for (new i=1 ; i<=iCount ; i++)
	{
		...
		&& GetClientTeam(client[i])==2)
		{
			...
			if (IsFakeClient(client[i])==false)
				PrintHintText(client[i],"Spirit: %t!", "SpritSuccessMessage");
...
public Action:Spirit_CooldownTimer (Handle:timer, any:client)
{
	...
	if (IsPlayerAlive(client)==true
		&& GetClientTeam(client)==2
	&& IsFakeClient(client)==false)
	PrintHintText(client,"%t", "SpiritTimerFinishedMessage");
...
public Event_PlayerHealed (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (SurvivorUpgrade1[client]==5)
	{
		...
		PrintHintText(client,"Unbreakable: %t", "UnbreakableHint");
	}
}
...
public Action:SoH_ShotgunEnd (Handle:timer, any:entity)
{
	...
	if (entity <= 0 || !IsValidEntity(entity)) return Plugin_Stop;
		...
		switch (GrenadeBeingTossed[client])
		{
			...
			case 0:
			{
				...
				|| StrEqual(st_wpn,"molotov",false)==true)
				{
					...
					{
						GrenadesCarried[client]=2;
						PrintHintText(client,"Grenadier: %t %i %s(s)", "GrenadierCarryHint",GrenadesCarried[client],st_wpn);
					}
					...
					else
					{
						...
						PrintHintText(client,"%t %s%t","GrenadierCantTake2Grenades_A",st_wpn,"GrenadierCantTake2Grenades_B");
					}
				}
			}
...
public Event_WeaponFire (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	&& (b_pipe || b_mol))
	{
		...
		if (GrenadesCarried[client]>0)		//do they still have grenades left?
		{
			...
			PrintHintText(client,"Grenadier: %t %i %s(s) %t","GrenadierCounter_A", GrenadesCarried[client], st_wpn, "GrenadierCounter_B");
	...
	&& PerksConfirmed[client]==1)
	{
		switch (GetEventInt(event,"ledge_hang"))
		{
			...
			case 0:
			{
				...
				PrintHintText(client,"Helping Hand: %t %s!","HelpingHandDonorHint",st_name);
				GetClientName(client,st_name,24);
				PrintHintText(iSub,"Helping Hand: %s gave you bonus temporary health!",st_name,"HelpingHandReceiverHint");
			}
		}
	}
...
MA_ResetFatigue ()
{
	...
	for (new i=1; i<=MartialCount; i++)
	{
		...
		if (!IsValidEntity(client)) continue;
		//skip this client if they're disabled
		if (SpiritState[client]==1) continue;
		
		//----DEBUG----
		//PrintToChatAll("\x03martial artist fire, client \x01%i\x03, offset \x01%i",i,g_iMeleeFatigueO);
		new halfdelay = ( GetEntData(client,g_iMeleeFatigueO) / 2 );
		SetEntData(client, g_iMeleeFatigueO, halfdelay);
	}
}
		...
		if (IsPlayerAlive(i) && GetClientTeam(i) == 3 && SmokerUpgrade[i]==1)
		{
			...
			if (StrContains(st_class,"smoker",false) != -1)
			{
				...
				if (g_iRegenerator_heal + currenthealth < GetConVarInt(FindConVar("z_gas_health")))
				{
					...
					EmitSoundToClient(i, "npc/infected/gore/bullets/bullet_gib_07.wav");
...
public Event_PlayerNowIt (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (BoomerUpgrade[attacker]==1	&& PerksConfirmed[attacker]==1 && SlimedCount==4)
	{
		...
		if (SlimedCount==4) PrintHintText(attacker,"Barf Bagged! %t", "BarfBaggedMobHint");
	}
	...
	&& TankUpgrade[client]<5)
	{
		switch (TankUpgrade[client])
		{
			...
			case 1:
			{
				...
				if (IsFakeClient(client)==false)
					PrintHintText(client,"Adrenal Glands: %t", "AdrenalGlandsHint");
				TankMode=1;
				return ;
			}
			...
			case 2:
			{
				...
				TankMode=2;
				if (IsFakeClient(client)==false)
					PrintHintText(client,"Juggernaut: %t", "JuggernautHint");
			...
			case 3:
			{
				...
				if (IsFakeClient(client)==false)
					PrintHintText(client,"Metabolic Boost: %t", "MetabolicHint");
				TankMode=1;
				return ;
			}
			...
			case 4:
			{
				...
				SetCommandFlags("z_spawn", iflags);
				if (IsFakeClient(client)==false)
					PrintHintText(client,"Storm Caller: %t", "StormCallerHint");
	...
	&& IsFakeClient(iTankid)==true)
	{
		...
		if (TankBotTicks>=3)
		{
			...
			for (new i=1 ; i<=MaxClients ; i++)
			{
				...
				PrintToChat(i, "\x03[Perk Mod] %t", "BotTankMessage");
			}
...
public Action:MenuOpen_OnSay(client, args)
{
	...
	if (StrEqual(st_chat,"!perks",false) || StrEqual(st_chat,"/perks",false))
	{
		...
		Format(st_perk, 128, "tPoncho's Perkmod: %t", "MapPerksPanel");
		SetPanelTitle(menu,st_perk);
		//show sur1 perk
		switch (SurvivorUpgrade1[client])
		{
			case 1:
			Format(st_perk,128,"Stopping Power (%t %i%%)", "BonusDamageText", RoundToNearest(g_flStopping_dmgmult*100) );
			case 2:
			...
			case 4:
			Format(st_perk,128,"Double Tap (%t, %i%%)", "DoubleTapDescriptionPanel",RoundToNearest(100 * ((1/g_flDT_rate)-1) ) ) ;
			case 5:
			Format(st_perk,128,"Unbreakable (%t +%i)", "UnbreakableHint", g_iUnbreak_hp);
			case 6:
		...
		switch (SurvivorUpgrade2[client])
		{
			...
			{
				if (g_iL4D_GameMode==0)
					Format(st_perk,128,"Martial Artist (%t, %i%%)", "MartialArtistDescriptionPanelCoop", RoundToNearest( (g_flMA_rate_coop - 1) * 100 ) );
				else
				Format(st_perk,128,"Martial Artist (%t: %i%%)", "MartialArtistDescriptionPanel", RoundToNearest( (g_flMA_rate - 1) * 100 ) );
			}
			case 5:
		...
		{
			case 1:
			Format(st_perk,128,"Barf Bagged (%t)", "BarfBaggedDescriptionPanel");
			case 2:
			Format(st_perk,128,"Acid Vomit (%t)", "AcidVomitDescriptionPanel");
			case 3:
			Format(st_perk,128,"Dead Wreckening (%t: %i%%)", "DeadWreckeningDescriptionPanel", RoundToNearest(100*g_flDead_dmgmult) );
			case 4:
			Format(st_perk,128,"Motion Sickness (%t)", "MotionSicknessDescriptionPanel");
			default: Format(st_perk,128,"%t", "NotSet");
		}
		...
		{
			case 1:
			Format(st_perk,128,"Regenerator: %t", "RegeneratorDescriptionPanel");
			case 2:
			{
				new iDmg = 3;
				...
				Format(st_perk, 128, "Squeezer (%t +%i)", "BonusDamageText", iDmg);
			}
			case 3:
			Format(st_perk,128,"Slingshot: %t", "SlingshotDescriptionPanel");
			default:
			Format(st_perk,128,"%t", "NotSet");
		}
		...
		{
			case 1:
			Format(st_perk,128,"Body Slam (%t %i)", "BodySlamDescriptionPanel", g_iBody_minbound);
			case 2:
			{
				decl iDmg;
				if (g_iL4D_Difficulty==2)
				...
				Format(st_perk,128,"Efficient Killer (%t +%i)", "EfficientKillerDescriptionPanel", iDmg);
			}
			case 3:
			Format(st_perk,128,"Grasshopper (%t, %i%%)", "GrasshopperDescriptionPanel", RoundToNearest( (g_flGrass_rate - 1) * 100 ) );
			case 4:
			{
				decl iDmg;
				if (g_iL4D_Difficulty==2)
				...
				iDmg=g_iOld_dmg;
				Format(st_perk,128,"Old School (%t +%i)", "OldSchoolDescriptionPanel", iDmg);
			}
			default:
			Format(st_perk,128,"%t", "NotSet");
		}
		...
		{
			case 1:
			Format(st_perk,128,"Adrenal Glands, %t", "AdrenalGlandsHint");
			case 2:
			Format(st_perk,128,"Juggernaut (%t %i)", "UnbreakableHint", g_iJuggernaut_hp);
			case 3:
			Format(st_perk,128,"Metabolic Boost (%t, %i%)", "MartialArtistDescriptionPanelCoop", RoundToNearest((g_flMetabolic_speedmult)*100) );
			case 4:
			Format(st_perk,128,"Storm Caller (%t)", "StormCallerDescriptionPanel");
			default: Format(st_perk,128,"%t", "NotSet");
		}
...
public Handle:Menu_Initial (client)
{
	...
	Format(panel, 128, "%t", "InitialMenuPanel1");
	DrawPanelText(menu, panel);
	
	Format(panel, 128, "%t", "InitialMenuPanel2");	
	DrawPanelText(menu, panel);
	
	Format(panel, 128, "%t", "InitialMenuPanel3");	
	DrawPanelItem(menu, panel);
	
	Format(panel, 128, "%t", "InitialMenuPanel4");	
	DrawPanelText(menu, panel);
	
	Format(panel, 128, "%t", "InitialMenuPanel5");	
	DrawPanelText(menu, panel);
	
	Format(panel, 128, "%t", "InitialMenuPanel6");	
	DrawPanelItem(menu, panel);
	
	Format(panel, 128, "%t", "InitialMenuPanel7");	
	DrawPanelText(menu, panel);	
	
	Format(panel, 128, "%t", "InitialMenuPanel8");	
	DrawPanelText(menu, panel);	
	
	Format(panel, 128, "%t", "InitialMenuPanel9");	
	DrawPanelText(menu, panel);		
	
	Format(panel, 128, "%t", "InitialMenuPanel10");	
	DrawPanelItem(menu, panel);	
	return menu;
}

public Menu_ChooseInit (Handle:topmenu, MenuAction:action, client, param2)
{
...
public Handle:Menu_Top (client)
{
	...
	decl String:panel[128];
	Format(panel, 128, "%t", "ChooseSubMenuPanel");
	DrawPanelText(menu, panel);
	DrawPanelText(menu,"(Should Keys 6 through 10 be not");
	DrawPanelText(menu,"enabled by serverside: To quickly enable");
	...
	Format(panel, 128, "%t", "DoneNagPanel1");	
	DrawPanelText(menu, panel);
	Format(panel, 128, "%t", "DoneNagPanel2");	
	DrawPanelText(menu, panel);
	Format(panel, 128, "%t", "DoneNagPanel3");	
	DrawPanelItem(menu, panel);
	return menu;
}

//choose a submenu from top perk menu
public Menu_ChooseSubMenu (Handle:topmenu, MenuAction:action, client, param2)
{
	if (topmenu!=INVALID_HANDLE) CloseHandle(topmenu);
	if (action==MenuAction_Select)
	{
		switch(param2)
		{
			...
			case 3:
			SendPanelToClient(Menu_Inf1Perk(client),client,Menu_ChooseInf1Perk,MENU_TIME_FOREVER);
			case 4:
			SendPanelToClient(Menu_Inf3Perk(client),client,Menu_ChooseInf3Perk,MENU_TIME_FOREVER);
			case 5:
			SendPanelToClient(Menu_Inf4Perk(client),client,Menu_ChooseInf4Perk,MENU_TIME_FOREVER);
			case 6:
			SendPanelToClient(Menu_Inf2Perk(client),client,Menu_ChooseInf2Perk,MENU_TIME_FOREVER);
			case 7:
...
//confirm
public Menu_ChooseConfirm (Handle:topmenu, MenuAction:action, client, param2)
{
	if (topmenu!=INVALID_HANDLE) CloseHandle(topmenu);
	if (action==MenuAction_Select)
	{
...
public Menu_DoNothing (Handle:topmenu, MenuAction:action, client, param2)
{}


//shows perk choices
public Handle:Menu_ShowChoices (client)
{
	new Handle:menu=CreatePanel();
	decl String:st_perk[128];
	Format(st_perk, 128, "%t", "YourPerksPanel");
	SetPanelTitle(menu,st_perk);
...
public Handle:Menu_Sur1Perk (client)
{
	...
	{
		Format(st_display, 64, "%t", "DisplayDisabledText");
		DrawPanelItem(menu, st_display, ITEMDRAW_NOTEXT);
	}
	else
	{
	...
	else
	{
		...
		DrawPanelItem(menu,st_display);
		Format(st_display,64," %i%% %t", RoundToNearest(100 * ((1/g_flDT_rate)-1) ), "DoubleTapDescriptionText");
		DrawPanelText(menu,st_display);
	}
...
public Menu_ChooseSur1Perk (Handle:menu, MenuAction:action, client, param2)
{
	...
	if (action==MenuAction_Select)
	{
		switch(param2)
		{
			//stopping power
			case 1:
			SurvivorUpgrade1[client]=1;
			//double tap
			case 2:
			SurvivorUpgrade1[client]=4;
			//sleight of hand
			case 3:
			SurvivorUpgrade1[client]=6;
			//pyrotechnician
			case 4:
			SurvivorUpgrade1[client]=2;
			//spirit
			case 5:
			SurvivorUpgrade1[client]=3;
			//unbreakable
			case 6:
			SurvivorUpgrade1[client]=5;
		}
	}
...
public Handle:Menu_Sur2Perk (client)
{
	...
	else
	{
		...
		if (g_iL4D_GameMode==0)
		{
			Format(st_display, 64, "Martial Artist (%t, %i%%)", "MartialArtistDescriptionPanelCoop", RoundToNearest( (g_flMA_rate_coop - 1) * 100 ) );
			DrawPanelText(menu,st_display);
		}
		else
		{
			DrawPanelText(menu,"");
			Format(st_display,64,"Martial Artist (%t: %i%%)", "MartialArtistDescriptionPanel", RoundToNearest( (g_flMA_rate - 1) * 100 ) );
			DrawPanelText(menu,st_display);
		}
	}
...
public Menu_ChooseSur2Perk (Handle:menu, MenuAction:action, client, param2)
{
	if (menu!=INVALID_HANDLE) CloseHandle(menu);
	if (action==MenuAction_Select)
	{
		switch(param2)
		{
			...
			//helping hand
			case 2:
			SurvivorUpgrade2[client]=2;
			//hard to kill
			case 3:
			SurvivorUpgrade2[client]=5;
			//pack rat
			case 4:
			SurvivorUpgrade2[client]=3;
			//martial artist
			case 5:
			SurvivorUpgrade2[client]=4;
		}
...
public Handle:Menu_Inf1Perk (client)
{
	...
	else
	{
		...
		DrawPanelItem(menu,st_display);
		DrawPanelText(menu,"");		
		Format(st_display,64,"%t","BarfBaggedDescriptionPanel");
		DrawPanelText(menu,st_display);
	}
	...
	else
	{
		...
		DrawPanelItem(menu,st_display);
		DrawPanelText(menu,"");
		Format(st_display,64,"%t (%i)", "AcidVomitDescriptionPanel", RoundFloat(GetConVarFloat(BileDamage)));
		DrawPanelText(menu,st_display);
	...
	else
	{
		...
		DrawPanelItem(menu,st_display);
		DrawPanelText(menu,"");
		Format(st_display,64,"%t (%i)", "DeadWreckeningDescriptionPanel", RoundFloat(GetConVarFloat(BileDamage)));
		DrawPanelText(menu,st_display);
	}
	...
	else
	{
		...
		DrawPanelItem(menu,st_display);
		DrawPanelText(menu,"");
		Format(st_display,64,"+%i%% %t", RoundToNearest( (g_flMotion_rate - 1) * 100 ), "MotionSicknessDescriptionPanel" );
		DrawPanelText(menu,st_display);
	}
...
public Menu_ChooseInf1Perk (Handle:menu, MenuAction:action, client, param2)
{
	if (menu!=INVALID_HANDLE) CloseHandle(menu);
	if (action==MenuAction_Select)
	{
		switch(param2)
		{
			//barf bagged
			case 1:
			BoomerUpgrade[client]=1;
			//acid vomit
			case 2:
			BoomerUpgrade[client]=2;
			//dead wreckening
			case 3:
			BoomerUpgrade[client]=3;
			//motion sickness
			case 4:
			BoomerUpgrade[client]=4;
		}
	}
...
public Handle:Menu_Inf2Perk (client)
{
	...
	else
	{
		...
		DrawPanelItem(menu,st_display);
		Format(st_display,64,"%t", "AdrenalGlandsHint");
		DrawPanelText(menu,st_display);
	}
	...
	else
	{
		...
		DrawPanelItem(menu,st_display);
		Format(st_display,64,"%t, +%i", "JuggernautHint", g_iJuggernaut_hp);
		DrawPanelText(menu,st_display);
	}
	...
	else
	{
		...
		DrawPanelItem(menu,st_display);
		Format(st_display,64,"%t, +%i%%", "MetabolicHint", RoundToNearest((g_flMetabolic_speedmult-1)*100) );
		DrawPanelText(menu,st_display);
	}
	...
	else
	{
		...
		DrawPanelItem(menu,st_display);
		Format(st_display,64,"%t",	 "StormCallerDescriptionPanel");
		DrawPanelText(menu,st_display);
	}
...
public Menu_ChooseInf2Perk (Handle:menu, MenuAction:action, client, param2)
{
	if (menu!=INVALID_HANDLE) CloseHandle(menu);
	if (action==MenuAction_Select)
	{
		switch(param2)
		{
			//adrenal glands
			case 1:
			TankUpgrade[client]=1;
			//juggernaut
			case 2:
			TankUpgrade[client]=2;
			//metabolic boost
			case 3:
			TankUpgrade[client]=3;
			//storm caller
			case 4:
			TankUpgrade[client]=4;
		}
	}
...
public Handle:Menu_Inf3Perk (client)
{
	...
	else
	{
		...
		Format(st_display,64,"%t, %i/2sec", "RegeneratorDescriptionPanel", iDmg);
		DrawPanelText(menu,st_display);
	}
	...
	else
	{
		...
		iDmg = g_iSqueezer_dmg;
		Format(st_display, 64, "Squeezer (%t +%i)", "BonusDamageText", iDmg);
		DrawPanelText(menu,st_display);
	}
	...
	else
	{
		...
		Format(st_display,64,"%t, %i power", "SlingshotDescriptionPanel", iForce);
		DrawPanelText(menu,st_display);
	}
...
public Menu_ChooseInf3Perk (Handle:menu, MenuAction:action, client, param2)
{
	if (menu!=INVALID_HANDLE) CloseHandle(menu);
	if (action==MenuAction_Select)
	{
		switch(param2)
		{
			//Regenerator
			case 1:
			SmokerUpgrade[client]=1;
			//squeezer
			case 2:
			SmokerUpgrade[client]=2;
			//slingshot stickytongue
			case 3:
			SmokerUpgrade[client]=3;
		}
	}
...
public Handle:Menu_Inf4Perk (client)
{
	...
	else
	{
		...
		iDmg=g_iEfficient_dmg;
		DrawPanelText(menu,"");
		Format(st_display,64,"%t +%i", "EfficientKillerDescriptionPanel", iDmg);
		DrawPanelText(menu,st_display);
	}
	...
	else
	{
		...
		iDmg=g_iOld_dmg;
		Format(st_display,64,"%t +%i damage", "OldSchoolDescriptionPanel", iDmg);
		DrawPanelText(menu,st_display);
	}
...
public Menu_ChooseInf4Perk (Handle:menu, MenuAction:action, client, param2)
{
	if (menu!=INVALID_HANDLE) CloseHandle(menu);
	if (action==MenuAction_Select)
	{
		switch(param2)
		{
			//body slam
			case 1:
			HunterUpgrade[client]=1;
			//efficient killer
			case 2:
			HunterUpgrade[client]=2;
			//grasshopper
			case 3:
			HunterUpgrade[client]=3;
			//old school
			case 4:
			HunterUpgrade[client]=4;
		}
	}

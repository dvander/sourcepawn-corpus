...
#define SCORE_VERSION_REQUIRED_LEFT4DOWNTOWN "0.4.4"
...
{
	name = "L4D2 Score/Team Manager Raw Edition",
...
new Handle:emptyServerTimer = INVALID_HANDLE;
...
new Handle:fVoteRematch = INVALID_HANDLE;
...
PrepareAllSDKCalls()
{
	...
	new bool:bGetTeamScore = PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "GetTeamScore");
	if(bGetTeamScore)
	{
	...
	gConfRaw = LoadGameConfigFile("l4d2scoresraw");
	...
	new bool:bRestartFuncLoaded = PrepSDKCall_SetFromConf(gConfRaw, SDKConf_Signature, "RestartScenarioFromVote");
	if(!bRestartFuncLoaded)
	{
	...
	new bool:bSwapTeamsFuncLoaded = PrepSDKCall_SetFromConf(gConfRaw, SDKConf_Signature, "SwapTeams");
	if(!bSwapTeamsFuncLoaded)
	{
	...
	StartPrepSDKCall(SDKCall_Raw);
	DebugPrintToAll("StartRematchVote Call prepped");
	new bool:bVoteRematchFuncLoaded = PrepSDKCall_SetFromConf(gConfRaw, SDKConf_Signature, "StartRematchVote");
	if(!bVoteRematchFuncLoaded)
	{
		LogError("Could not load the StartRematchVote signature");
	}
	DebugPrintToAll("StartRematchVote Signature prepped");
	fVoteRematch = EndPrepSDKCall();
	if(fVoteRematch == INVALID_HANDLE)
	{
		LogError("Could not prep the StartRematchVote function");
	}
	
	StartPrepSDKCall(SDKCall_Raw);
	DebugPrintToAll("AreTeamsFlipped Call prepped");
	new bool:bAreTeamsFlippedFuncLoaded = PrepSDKCall_SetFromConf(gConfRaw, SDKConf_Signature, "AreTeamsFlipped");
	if(!bAreTeamsFlippedFuncLoaded)
	{
...
stock L4D2_StartRematchVote()
{
	DebugPrintToAll("About to SDKCall StartRematchVote. g_pDirector=%x", g_pDirector);
	SDKCall(fVoteRematch, g_pDirector);	
}
...
{
	decl String:gamemode[64];
	new Handle:mode = FindConVar("mp_gamemode");
...
OnNewMission()
{
	...
	if (emptyServerTimer != INVALID_HANDLE)
	{
		KillTimer(emptyServerTimer);
	}
	emptyServerTimer = INVALID_HANDLE;
}
...
{		
	...
	{
		CreateTimer(SCORE_DELAY_SCORE_SWAPPED, Timer_DirectorSwapTeams);
	}
	
	emptyServerTimer = INVALID_HANDLE;
}

public Action:Timer_DirectorSwapTeams(Handle:timer)
{
	L4D2_DirectorSwapTeams();
}
...
public OnMapEnd()
{
	...
	*/
	...
	switch(swapKind)
	{
		...
		{
			swapScoreBeginningLevel = L4D2_AreTeamsFlipped() ? !pendingSwapScores : pendingSwapScores;
		}
		case SwapAlways:
		{
...
public OnClientDisconnect(client)
{
	...
	*/
	...
	emptyServerTimer = CreateTimer(SCORE_DELAY_EMPTY_SERVER, IsNobodyConnected);
}

public Action:IsNobodyConnected(Handle:timer)
{
	for (new i = 1; i <= MaxClients; i++)
	{
...
TryTeamPlacement()
{
	SetConVarInt(FindConVar("sb_all_bot_team"), 1); // necessary to avoid 0 human Survivor server bugs
...
public Action:Command_Changelevel(args)
{
	if(args > 0)
	{
		...
		if(IsMapValid(map) && !skippingLevel)
		{
			...
			DebugPrintToAll("Do not use changelevel, you should be using sm_changemap instead to change maps!");
		}
	}
	return Plugin_Continue;
}

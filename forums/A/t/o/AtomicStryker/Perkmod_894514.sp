...
- version 1.1A: AtomicStrykers derivation with bugfixes and no more Convar changing
	...
	{	
		...
		if (IsPlayerAlive(i) && GetClientTeam(i) == 3 && SmokerUpgrade[i]==1)
		{
			...
			if (StrContains(st_class,"smoker",false) != -1)
			{
				...
				if (g_iRegenerator_heal + currenthealth >= GetConVarInt(FindConVar("z_gas_health"))) SetEntProp(i, Prop_Data, "m_iHealth", GetConVarInt(FindConVar("z_gas_health")));
				if (g_iRegenerator_heal + currenthealth < GetConVarInt(FindConVar("z_gas_health"))) SetEntProp(i, Prop_Data, "m_iHealth", g_iRegenerator_heal + currenthealth);
				continue;
			}
		}
	}
...
public Action:Event_TongueGrabPre (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	SpiritState[GetClientOfUserId(GetEventInt(event,"victim"))]=1;

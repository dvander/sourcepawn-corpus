...
new propinfoghost;
new teleporttarget[MAXPLAYERS+1];
new bool:teleportdelay[MAXPLAYERS+1];


public Plugin:myinfo = 
{
	name = "Infected Ghost Everywhere",
	author = "AtomicStryker",
	description = "Instead of always teleporting to the lead Survivor, you can iterate them all",
...
public OnGameFrame()
{
    for (new i=1; i<=MaxClients; i++)
    {
		...
		if(buttons & IN_USE && teleportdelay[i] == false)
		{
			PlayerPressesUse(i);
		}
	}
...
public Action:PlayerPressesUse(client)
{
...
FindNewTarget(any:client, any:target)
{
	target++;

...
public Plugin:myinfo = {
	name = PLUGIN_NAME,
	author = "Rodrigo286, upgraded by Keith Warren(Drixevel) and Werner Maisl (Arrow768)",
...
LoadConfig(const String:sConfigName[], const String:sConfigFile[])
{
	...
	g_respawnChance = KvGetNum(kv, "respawn_chance", 0);
...
public Store_ItemUseAction:OnRespawnUse(client, itemId, bool:equipped)
{
	...
	{
		if(g_respawnChance <= GetRandomInt(0,10))
		{
			CS_RespawnPlayer(client);
			TF2_RespawnPlayer(client);
		...
		{
			CPrintToChat(client, "%tOh No, the Respawn failed.", "Store Tag Colored");
			respawn_uses[client] += 1;

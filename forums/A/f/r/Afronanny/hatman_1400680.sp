...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_bethehorseman", Cmd_Hatman, ADMFLAG_SLAY);
	RegAdminCmd("sm_boo", Cmd_Boo, ADMFLAG_SLAY);
...
public Action:Cmd_Hatman(client, args)
{
	if (g_bIsHorseman[client])
	{
		ReplyToCommand(client, "You already are a horseman!");
	}
	
	g_bIsHorseman[client] = true;
	
	SetEntityRenderMode(client, RENDER_TRANSALPHA);
	SetEntityRenderColor(client, 255, 255, 255, 0);
	
	new playermodel = Attachable_CreateAttachable(client);
	SetEntityModel(playermodel, "models/bots/headless_hatman.mdl");
	
	g_iPlayerModel[client] = playermodel;
	SDKHook(playermodel, SDKHook_SetTransmit, ShouldTransmit);
	
	
	new userid = GetClientUserId(client);
	g_hStunTimers[client] = CreateTimer(60.0, Timer_Terrify, userid, TIMER_REPEAT);

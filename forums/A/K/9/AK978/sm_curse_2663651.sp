...
public void OnPluginStart() {
	CreateConVar("sm_curse_version", PLUGIN_VERSION, "Current sm_curse version", FCVAR_DONTRECORD);
	...
	RegConsoleCmd("sm_978curse", ConCmd_Curse, "sm_curse <#userid|name> [0/1]");
...
public Action charger_pummel_end(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (IsSurvivor(client) && IsPlayerAlive(client) && !IsFakeClient(client))
	{
		if (aaa[client] == 0)
		{
			aaa[client] = 1;
			bbb[client] = 10;
			PerformCurse(client, client, 1);
			CreateTimer(1.0, who_curse, client, TIMER_FLAG_NO_MAPCHANGE);
			PrintToChat(client, "你陷入了混亂狀態");
		}
	}
}

public Action who_curse(Handle timer,int client)
{
	bbb[client]--;
	if (bbb[client] != 0 && IsSurvivor(client) && IsPlayerAlive(client) && !IsFakeClient(client))
	{
		PrintToChat(client, "混亂狀態剩餘 %d 秒", bbb[client]);
		CreateTimer(1.0, who_curse, client, TIMER_FLAG_NO_MAPCHANGE);
	}
	else
	{
		...
		PerformCurse(client, client, 0);
		PrintToChat(client, "你的狀態已恢復正常");
	}
}

...
new String:steam_id[MAXPLAYERS+1][64];
...
public OnClientPostAdminCheck(client)
{
	if (vip[client] == 1)
	{
		...
		if (StrContains(sAuth, "steam_id[client]", true) == -1)
		{
			vip[client] = 0;
		}
	}
...
public Action:Command_add_weapon(client, args){
	...
	if(id >= 0){
		char sAuth[64];
		GetClientAuthId(client, AuthId_Steam2, sAuth, sizeof(sAuth));
		strcopy(steam_id[client], 64, sAuth);
		vip[client] = 1;
		
		weapon_strengh[id] = stren;
		PrintToChat(client, "Weapon Kick: Weapon %s (%d) with force %f", weaponname, id, stren);
...
public Action:DamageEvent(Handle:event, const String:name[], bool:dontBroadcat)
{
	...
	if (vip[attacker] != 1) return Plugin_Continue;

...
public Action JoinTeam(int client, int args)
{
	...
	if(IsClientInGame(client))
	{
		...
		{	
			...
			{
				PrintHintText(client, "You already joined the Survivor team");
			}
...
public void evtPlayerReplacedBot(Event event, const char [] name, bool dontBroadcast)
{
	...
	if(!gbPlayedAsSurvivorBefore[client])
	{
		...
		BypassAndExecuteCommand(client, "give", "katana");
                BypassAndExecuteCommand(client, "give", "pipe_bomb");
...
{
	int g_bot = GetClientOfUserId(GetEventInt(event, "bot"));
	if(GetClientTeam(g_bot) == TEAM_SURVIVORS)
		CreateTimer(DELAY_KICK_NONEEDBOT, Timer_KickNoNeededBot, g_bot, TIMER_FLAG_NO_MAPCHANGE);
}
...
public Action Timer_KickNoNeededBot(Handle timer, any g_bot)
{
	if((TotalSurvivors() <= 4))
		return Plugin_Handled;
	
	if(IsClientConnected(g_bot) && IsClientInGame(g_bot))
	{
		if(GetClientTeam(g_bot) == TEAM_INFECTED)
			return Plugin_Handled;
		
		char BotName[100];
		GetClientName(g_bot, BotName, sizeof(BotName));				
		if(StrEqual(BotName, "FakeClient", true))
			return Plugin_Handled;
		
		if(!HasIdlePlayer(g_bot))
		{
			StripWeapons(g_bot);
			KickClient(g_bot, "Kicking No Needed Bot");
		}
	}	
	return Plugin_Handled;
}
...
stock int TakeOverBot(int client, bool completely)
{
	...
	if(completely)
	{
		SDKCall(hSetHumanSpec, q_bot, client);

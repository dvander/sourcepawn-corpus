...
{
	new Handle:hVersion = CreateConVar("sm_ion_version", PLUGIN_VERSION, "Ion cannon version", 0|FCVAR_NOTIFY|FCVAR_REPLICATED|FCVAR_DONTRECORD);
	...
	g_hCVAmmo = CreateConVar("sm_ion_defaultammo", "1", "How many ion cannons should a player get by default on join?", 0, true, 0.0);
	g_hCVMaxAmmo = CreateConVar("sm_ion_maxammo", "5", "How many ion cannons should a player be able to \"carry\" at a time?", 0, true, 1.0);
	g_hCVBuy = CreateConVar("sm_ion_buy_enable", "1", "Are players allowed to buy an ion cannon via \"!buyion\"?", 0, true, 0.0, true, 1.0);
	g_hCVPrice = CreateConVar("sm_ion_buy_price", "0", "How much money does the ion cannon cost?", 0, true, 0.0);
	g_hCVDeployTime = CreateConVar("sm_ion_deploytime", "1", "How long does it take to display the beams after deploying a ion cannon in seconds?", 0, true, 0.1, true, 35.0);
	g_hCVShakeTime = CreateConVar("sm_ion_shaketime", "3", "How long should the client's screen shake after explosion in seconds?", 0, true, 0.0);
	g_hCVRadius = CreateConVar("sm_ion_radius", "2000", "The radius of the explosion.", 0, true, 1001.0);
	g_hCVMinDamage = CreateConVar("sm_ion_mindamage", "1000", "How much damage does the explosion deal at min?", 0, true, 1000.0);
	g_hCVMaxDamage = CreateConVar("sm_ion_maxdamage", "2000", "How much damage does the explosion deal at max?", 0, true, 2000.0);
	g_hCVIonWeapon = CreateConVar("sm_ion_weapon", "weapon_rifle", "Which weapon should be used to deploy ion cannons?", 0);
	g_hCVGlowSprites = CreateConVar("sm_ion_glowsprites", "300", "How many glow sprites should be spit out on explosion? (Note: They are shot twice with different velocity!)", 0, true, 0.0);
	g_hCVPlaceTime = CreateConVar("sm_ion_placetime", "5", "How many seconds should player have to stand still to deploy an ion cannon?", 0, true, 0.1, true, 15.0);
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	...
	if(buttons & IN_ATTACK && !g_bIsPressingAttack[client] && g_iIonCannonAmmo[client] > 0)
	{
		...
		if(g_hFiringWeapon[client] == INVALID_HANDLE)
		{
			...
			//PrintToChat(client, "iWeapon = %s", iWeapon);		
			if(iWeapon != -1
			...
			{
				//PrintToChat(client, "sWeapon = %s", sWeapon);
				...
				{
					CPrintToChat(client, "%s需在室外You have to be outside to deploy an {red}ion cannon{green}.", PREFIX);
				}*/
				else
				{
					...
					PrintCenterText(client, "設置衛星炮Placing an ion cannon (%d seconds)", RoundToNearest(fPlaceTime));
				}
...
{		
	...
	ServerCommand("sm_give978give$ \"%N\" -500", client);
	PrintToChat(client,"衛星定位須連續揮舞近戰武器5秒.");
...
public Action:Timer_OnFireIonCannon(Handle:timer, any:client)
{
	...
	if(iExplosion != -1)
	{
		...
		// The amount of damage done by the explosion. 爆炸造成的傷害量。
	...
	if(iFire != -1)
	{
		...
		DispatchKeyValue(iFire, "damagescale", "0"); //預設100
		// delete when out
		SetVariantString("spawnflags 128");
		AcceptEntityInput(iFire,"AddOutput");

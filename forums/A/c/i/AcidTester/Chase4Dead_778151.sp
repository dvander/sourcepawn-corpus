...
public Action:Command_randclient(client, args) {
	new randclient = GetRandomClient(3);
...
public Action:Event_Player_Hurt(Handle:event, const String:name[], bool:dontBroadcast) {	
	...
	if(currenttank == userid && !finaleyet) {
		...
		new randclient = GetRandomClient(3); //I might switch it to survivor team so tank doesn't spawn out of map sometimes
		SpawnTank(randclient);
		//SetEntityHealth(client, (health + 6000));
		}
	} 

...
{
    if(client < 1 || !IsClientInGame(client) || IsFakeClient(client)  || GetClientTeam(client) != TEAM_INFECTED)
        return Plugin_Continue;

    if(GetEntProp(client, Prop_Send, "m_iPlayerState") == 0)
    {
	PrintToChat( client, "[CoSySoX] Teamswitch is forbidden for alive infected.");

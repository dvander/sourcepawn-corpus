...
{ 
	...
	{ 
		...
		*/
		OldWitchHP = GetEntProp(witch, Prop_Data, "m_iHealth");
		//PrintToChatAll("Old Witch HP = %d", OldWitchHP);
...
Float:GetRage(witch, &Float:rage, &Float:wanderRage)
{
...
SetRage(witch, Float:rage, Float:wanderRage)
{
...
FreshEnemy(enemy, Float:enemyPos[3])
{
...
LoadWitch(witchid)
{
	...
	{
		SetEntProp(witchid, Prop_Data, "m_iHealth", OldWitchHP);
		//PrintToChatAll("NEW Witch HP = %d", OldWitchHP);
...
{
	new Float:minDis=GetConVarFloat(l4d_witch_attack_range);
	new Float:ePos[3];
	new selected=0;
	for(new i=1; i<=MaxClients; i++)
	{
...
GameCheck()
{
	...
	StrEqual(GameName, "teamscavenge", false)) GameMode = 2;

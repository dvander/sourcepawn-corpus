...
Fixed mistake on line 517 thanks figa for pointing out
...
public OnPluginStart()
{
	...
	hCookie_LmcCookie = RegClientCookie("lmc_cookie", "0", CookieAccess_Protected);
...
static BeWitched(iClient, const String:sModel[])  
{  
...
public ePlayerDeath(Handle:hEvent, const String:sEventName[], bool:bDontBroadcast)
{
	...
	{
		new iModel = GetEntProp(iEntity, Prop_Data, "m_nModelIndex", 2);
		SetEntProp(iClient, Prop_Data, "m_nModelIndex", iModel, 2);
	}
...
public Action:ShowMenu(iClient, iArgs) 
{
	...
	AddMenuItem(hMenu, "7", "Random Common");
...
static ModelIndex(iClient, iCaseNum)
{
	...
	switch(iCaseNum)
	{
		...
		case 2: BeWitched(iClient, Witch_Normal);
		case 3: {
			if(!g_bTankModel)
			{
				if(!IsFakeClient(iClient))
			...
			{
				case 0: BeWitched(iClient, Infected_TankNorm);
				case 1: BeWitched(iClient, Infected_TankSac);
			}
		}
		case 4: BeWitched(iClient, Infected_Boomer);
		case 5: BeWitched(iClient, Infected_Hunter);
		case 6: BeWitched(iClient, Infected_Smoker);
		case 7:
		{

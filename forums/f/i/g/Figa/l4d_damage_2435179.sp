...
public Action:_DM_PlayerHurt_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	decl Float:multiplierWeapon, String:weaponname[64], String:Weapon[64];
	...
	if (strcmp(Weapon, "prop_mounted_machine_gun", false) != 0)
	{
	...
	{
		if (GetClientTeam(client) == 2) return Plugin_Continue;
		if (IsPlayerTank(client) ? GetTrieValue(trieModdedWeaponsTank, Weapon, multiplierWeapon) : GetTrieValue(trieModdedWeapons, Weapon, multiplierWeapon))
		{
			//PrintToChat(attacker, "Hurt_Event weapon = (%s)", Weapon);
			damagedelta = RoundToNearest((dmg_health * multiplierWeapon) - dmg_health);
...
public Action:_DM_InfectedHurt_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetEventInt(event, "type") == -2147483646 && GetEventInt(event, "amount") == 25)
	{
		if (StrEqual(entname, "infected", false)) damagedelta = RoundToNearest((dmg_health * 3.0) - dmg_health);
		else damagedelta = RoundToNearest((dmg_health * 3.0) - dmg_health);
		
		//PrintToChatAll("damagedelta = (%d), dmg_health = (%d), entname = (%s)", damagedelta, dmg_health, entname);
	...
	else if(GetTrieValue(trieModdedWeapons, weaponname, multiplierWeapon))
	{
		damagedelta = RoundToNearest((dmg_health * multiplierWeapon) - dmg_health);
...
public Action:OnTakeDamage(client, &attacker, &inflictor, &Float:damage, &damagetype)
{
	...
	{
		if(attacker != inflictor && damagetype == -2147483646)
		{
			damage = 10.0;

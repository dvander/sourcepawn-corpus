...
{
	CreateConVar("sm_executeconfigs_version", PL_VERSION, "Execute configs on certain events.", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED);
...
public OnClientPutInServer(client)
{
	...
	if ((bIncludeBots || !IsFakeClient(client)) && (bIncludeSpec || (GetClientTeam(client) != 1)))
		ExecClientsConfig(0);
}
...
ExecClientsConfig(iClients)
{
	...
	else
	{
		for(new i = 1; i <= MaxClients; i++)
		{
			...
			if ((bIncludeBots || !IsFakeClient(i)) && (bIncludeSpec || (GetClientTeam(i) != 1)))
				iClients++;
		}
	}

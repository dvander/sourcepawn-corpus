...
#define PLUGIN_VERSION "2.5.0Beta9"
...
public OnPluginStart()
{
	...
	sm_res_play_method			 = CreateConVar("sm_res_play_method",			 "0", "What method to use to play sounds? 0 - EmitSound (volume support) | 1 - Client command \"play\" (Max. no sm_res_volume support)", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	sm_res_displaysound			 = CreateConVar("sm_res_displaysound",			 "1", "Shows in the chat sound info. Artist and title of the sound", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	
	switch (iGame)
	{
		case GAME_DOD :
		{
			sm_res_dod_blocklastcry		 = CreateConVar("sm_res_dod_blocklastcry",		 "1", "Blocks last capture point cry", FCVAR_PLUGIN, true, 0.0, true, 1.0);

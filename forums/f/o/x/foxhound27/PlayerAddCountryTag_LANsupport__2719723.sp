...
#define VERSION "2.5.0-1f"

UserMsg g_SayText2;
char OriginalName[MAXPLAYERS+1][MAX_NAME_LENGTH];

Handle PlayerJoinMessage = INVALID_HANDLE;
Handle PlayerJoinMessageLayout = INVALID_HANDLE;
Handle ShowIsAdminOnMessages = INVALID_HANDLE;
Handle ShowIsAdminInScore = INVALID_HANDLE;
Handle NameLayout = INVALID_HANDLE;
Handle AdminLayout = INVALID_HANDLE;
Handle PACTLIST_Layout = INVALID_HANDLE;
Handle LANcountry = INVALID_HANDLE;
Handle ChangeName = INVALID_HANDLE;
Handle PluginVersionCVAR = INVALID_HANDLE;
		...
		if( !IsFakeClient( client ) )
		{
			...
			if( GetConVarBool(PlayerJoinMessage) )
			{
				if( GeoipCountry( IP, Country, 100 ) )
				{
					...
					if( GetUserFlagBits(client) > 1 && GetConVarBool(ShowIsAdminOnMessages) )
					{
			...
			if( GetUserFlagBits(client) > 1 && GetConVarFloat(PluginVersionCVAR) != StringToFloat(VERSION) )
			{
		...
		if( !IsFakeClient( client ) )
		{
			for( int i = 1; i<=MaxClients; i++)
			{
				if (IsClientConnected(i) && IsClientInGame(i) && !IsFakeClient(i))
				{
					...
					if( GetUserFlagBits(i) > 1 && GetConVarBool(ShowIsAdminOnMessages) )
					{
...
void getPlayerNameWithTag(int client, char[] NameWithTag, int size)
{
	...
	if( GetUserFlagBits(client) > 1 && GetConVarBool(ShowIsAdminInScore) )
...
public Action UserMessageHook(UserMsg msg_id, Handle bf, const char[] players, int playersNum, bool reliable, bool init)
{
    char message[256];

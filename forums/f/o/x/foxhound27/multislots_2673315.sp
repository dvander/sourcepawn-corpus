...
public void OnClientPostAdminCheck(int client)
{
	...
	for(int iClient=1;iClient<=MaxClients;iClient++){
		...
		if(IsClientInGame(iClient) && IsPlayerAlive(iClient) && GetClientTeam(iClient) == 2 && IsFakeClient(iClient)){

	CreateTimer(DELAY_KICK_NONEEDBOT, Timer_KickNoNeededBot, iClient);
...
public void Check_Bots(Handle event, const char[] name, bool dontBroadcast) {
    CreateTimer(35.0, Timer_SpawnTick);
}


stock int SurvivorsLeft(){
int counter = 0;
    for(int iClient = 1; iClient <= MaxClients; iClient++)
    {
...
{
	if ((SurvivorsLeft() <= 4)) return Plugin_Handled;

...
{
	if(AutoDifficulty.BoolValue) SetTankHp(GetClientOfUserId(GetEventInt(event, "userid")), FindConVar("z_tank_health").IntValue);
}
...
void SpawnCheck()
{
	...
	if (iSurvivor >= SurvivorLimit.IntValue)  return;
	for(; iSurvivor < SurvivorLimit.IntValue; iSurvivor++) SpawnFakeSurvivorClient();

...
public void OnPluginStart() {
    ...
    HookEvent("round_start", RoundStartHook2, EventHookMode_Post);
...
public void RoundStartHook2(Handle event, const char[] name, bool dontBroadcast) {
    Echo(2, "RoundStartHook2: %s", name);
    CreateTimer(40.0, KitFix);
}


public Action KitFix(Handle timer) {
    for (new client = 1; client <= MaxClients; client++) {
        if (IsClientInGame(client) && GetClientTeam(client) == 2) {

            if (GetPlayerWeaponSlot(client, 2) == -1) QuickCheat(client, "give", g_HealItem);

...
{
	if(AutoDifficulty.BoolValue && TANKHPFIX>0) SetTankHp(GetClientOfUserId(GetEventInt(event, "userid")), TANKHPFIX);
}
...
stock int SetTankHp(int client, int hp){
...
void AutoDifficultyCheck()
{
	...
	if (TankHpMulti.BoolValue){   // if not 0
			...
			TANKHPFIX = RoundFloat(4000.0*TankHp_Multi);
	}

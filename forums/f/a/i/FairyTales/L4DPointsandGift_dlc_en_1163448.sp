...
new Handle:pointshurtcount[PLAYERS + 20];
...
public OnPluginStart()
{
	...
	pointsspecial = CreateConVar("points_amount_specialinfected","3","How many points for killing a special infected.",CVAR_FLAGS, true, 0.0);
	pointsheal = CreateConVar("points_amount_heal","30","How many points for healing someone.",CVAR_FLAGS, true, 0.0);
	pointsrevive = CreateConVar("points_amount_revive","15","How many points for reviving someone.",CVAR_FLAGS, true, 0.0);
	...
	pointsadvertising = CreateConVar("points_advertising","0","Do we want the plugin to advertise itself? 1 chat box message, 2 hint text message, 0 for none.",CVAR_FLAGS, true, 0.0);
	...
	pointstankburn = CreateConVar("points_amount_tank_burn","8","How many points you get for burning a tank.",CVAR_FLAGS,true,0.0);
	pointstankkill = CreateConVar("points_amount_tank","10","How many additional points you get for killing a tank.",CVAR_FLAGS,true,0.0);
	...
	pointsheadshot = CreateConVar("points_amount_extra_headshotkills","2","How many extra kills are survivors awarded for scoring headshots? 0 = None.",CVAR_FLAGS,true, 0.0);
	pointsminigun = CreateConVar("points_amount_extra_minigunkills","3","How many extra kills are survivors awarded for scoring minigun kills? 0 = None.",CVAR_FLAGS,true, 0.0);
	...
	/*Price Convars*/
	shotpoints = CreateConVar("points_price_shotgun","15","How many points a shotgun costs.",CVAR_FLAGS, true, -1.0);
	smgpoints = CreateConVar("points_price_smg","15","How many points a smg costs.",CVAR_FLAGS, true, -1.0);
	riflepoints = CreateConVar("points_price_rifle","35","How many points a rifle costs.",CVAR_FLAGS, true, -1.0);
	huntingpoints = CreateConVar("points_price_huntingrifle","30","How many points a hunting rifle costs.",CVAR_FLAGS, true, -1.0);
	autopoints = CreateConVar("points_price_autoshotgun","40","How many points an auto-shotgun costs.",CVAR_FLAGS, true, -1.0);
	pipepoints = CreateConVar("points_price_pipebomb","10","How many points a pipe-bomb costs.",CVAR_FLAGS, true, -1.0);
	molopoints = CreateConVar("points_price_molotov","15","How many points a molotov costs.",CVAR_FLAGS, true, -1.0);
	pistolpoints = CreateConVar("points_price_pistol","10","How many points an extra pistol costs.",CVAR_FLAGS, true, -1.0);
	pillspoints = CreateConVar("points_price_painpills","15","How many points a bottle of pills costs.",CVAR_FLAGS, true, -1.0);
	medpoints = CreateConVar("points_price_medkit","60","How many points a medkit costs.",CVAR_FLAGS, true, -1.0);
	refillpoints = CreateConVar("points_price_refill","5","How many points an ammo refill costs.",CVAR_FLAGS, true, -1.0);
	healpoints = CreateConVar("points_price_heal","80","How many points a heal costs.",CVAR_FLAGS, true, -1.0);
	...
	m60points = CreateConVar("points_price_m60","60","How many points a M60 Machine Gun costs.",CVAR_FLAGS, true, -1.0);
	...
	sg552points = CreateConVar("points_price_sg552","1","How many points a SG552 rifle costs.",CVAR_FLAGS, true, -1.0);
	silencedsmgpoints = CreateConVar("points_price_silencedsmg","15","How many points a Silenced SMG costs.",CVAR_FLAGS, true, -1.0);
	mp5points = CreateConVar("points_price_mp5","1","How many points a MP5 SMG costs.",CVAR_FLAGS, true, -1.0);
	awppoints = CreateConVar("points_price_awp","1","How many points an AWP sniper rifle costs.",CVAR_FLAGS, true, -1.0);
	militarypoints = CreateConVar("points_price_military","35","How many points a military sniper rifle costs.",CVAR_FLAGS, true, -1.0);
	scoutpoints = CreateConVar("points_price_scout","1","How many points a scout sniper rifle costs.",CVAR_FLAGS, true, -1.0);
	grenadepoints = CreateConVar("points_price_grenade","50","How many points a grenade launcher costs.",CVAR_FLAGS, true, -1.0);
	fireworkpoints = CreateConVar("points_price_firework","15","How many points a fireworks crate costs.",CVAR_FLAGS, true, -1.0);
	vomitjarpoints = CreateConVar("points_price_vomitjar","15","How many points a vomitjar costs.",CVAR_FLAGS, true, -1.0);
	...
	golfpoints = CreateConVar("points_price_golf","5","How many points the golf club costs.",CVAR_FLAGS, true, -1.0);
	gascanpoints = CreateConVar("points_price_gascan","15","How many points the gascan costs.",CVAR_FLAGS, true, -1.0);
	laserpoints = CreateConVar("points_price_laser","1","How many points a laser sight costs.",CVAR_FLAGS, true, -1.0);
	katanapoints = CreateConVar("points_price_katana","5","How many points a katana costs.",CVAR_FLAGS, true, -1.0);
	machetepoints = CreateConVar("points_price_machete","5","How many points a machete costs.",CVAR_FLAGS, true, -1.0);
	
	/*Disable Weapon Categories*/
	healthcat = CreateConVar("weapon_cat_health","1","The Health category of items. -1 to disable 1 to enable",CVAR_FLAGS, true, -1.0);
	smgcat = CreateConVar("weapon_cat_smg","1","The SMGs category of items. -1 to disable 1 to enable",CVAR_FLAGS, true, -1.0);
	riflecat = CreateConVar("weapon_cat_rifle","1","The Rifles category of items. -1 to disable 1 to enable",CVAR_FLAGS, true, -1.0);
	snipercat = CreateConVar("weapon_cat_sniper","1","The Sniper Rifles category of items. -1 to disable 1 to enable",CVAR_FLAGS, true, -1.0);
	shotguncat = CreateConVar("weapon_cat_shotgun","1","The Shotguns category of items. -1 to disable 1 to enable",CVAR_FLAGS, true, -1.0);
	pistolcat = CreateConVar("weapon_cat_pistol","1","The Pistols category of items. -1 to disable 1 to enable",CVAR_FLAGS, true, -1.0);
	explosivescat = CreateConVar("weapon_cat_explosives","1","The Explosives category of items. -1 to disable 1 to enable",CVAR_FLAGS, true, -1.0);
	ammocat = CreateConVar("weapon_cat_ammo","1","The Ammo category of items. -1 to disable 1 to enable",CVAR_FLAGS, true, -1.0);
	
	/*Infected Price Convars*/
	suicidepoints = CreateConVar("points_price_infected_suicide","5","How many points it takes to end it all.",CVAR_FLAGS, true, -1.0);
	ihealpoints = CreateConVar("points_price_infected_heal","10","How many points a heal costs (for infected).",CVAR_FLAGS, true, -1.0);
	boomerpoints = CreateConVar("points_price_infected_boomer","20","How many points a boomer costs.",CVAR_FLAGS, true, -1.0);
	hunterpoints = CreateConVar("points_price_infected_hunter","10","How many points a hunter costs.",CVAR_FLAGS, true, -1.0);
	smokerpoints = CreateConVar("points_price_infected_smoker","15","How many points a smoker costs.",CVAR_FLAGS, true, -1.0);
	spitterpoints = CreateConVar("points_price_infected_spitter","15","How many points a spitter costs.",CVAR_FLAGS, true, -1.0);
	chargerpoints = CreateConVar("points_price_infected_charger","15","How many points a charger costs.",CVAR_FLAGS, true, -1.0);
	jockeypoints = CreateConVar("points_price_infected_jockey","15","How many points a jockey costs.",CVAR_FLAGS, true, -1.0);
	tankpoints = CreateConVar("points_price_infected_tank","60","How many points a tank costs.",CVAR_FLAGS, true, -1.0);
	wwitchpoints = CreateConVar("points_price_infected_witch","40","How many points a witch costs.",CVAR_FLAGS, true, -1.0);
	mobpoints = CreateConVar("points_price_infected_mob","25","How many points a mini-event/mob costs.",CVAR_FLAGS, true, -1.0);
	panicpoints = CreateConVar("points_price_infected_mob_mega","40","How many points a mega mob costs.",CVAR_FLAGS, true, -1.0);
	
	/*Special Price Convars*/
	burnpoints = CreateConVar("points_price_special_burn","40","How many points does incendiary ammo cost?",CVAR_FLAGS,true,-1.0);
	burnpackpoints = CreateConVar("points_price_special_burn_super","60","How many points does a pack of incendiary ammo cost?",CVAR_FLAGS,true,-1.0);
...
public Action:PointsMenuRifles(clientId) {
	...
	{
	AddMenuItem(menu, "option5", "M60 Machine Gun");
	}
	AddMenuItem(menu, "option6", "Back");
	SetMenuExitButton(menu, true);
	DisplayMenu(menu, clientId, MENU_TIME_FOREVER);
...
public Action:PointsMenuPistols(clientId) {
	...
	if ( GetConVarInt(golfpoints) > -1 )
	{
	AddMenuItem(menu, "option4", "Golf Club");
	}
	...
	{
	AddMenuItem(menu, "option6", "Machete");
	}
...
public Action:RepeatBuy(client, args)
{
	...
	switch(buyitem[client])
	{
		...
		case 42: //golf
		{
		   if (points[client] >= GetConVarInt(golfpoints))
			{
				//Give the player golf
				FakeClientCommand(client, "give golfclub");
				points[client] -= GetConVarInt(golfpoints);
			}
			else
			{
				PrintToChat(client,"[SM] Not enough points!");

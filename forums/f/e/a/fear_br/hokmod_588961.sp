...
public OnPluginStart()
{
	...
	CreateConVar("hok_autoshowhelp", "1", "Show HoK Rules to users automatically if auto announce is enable", _, true, 0.0, true, 1.0)
...
// Log pluging unload and adverts users
public OnPluginEnd()
{
	...
	if ( GetConVarInt(FindConVar("hok_enabled")) )
	{
		...
		if ( IsPlayerAlive(attacker) )
		{
			...
			if ( attackerId != victimId )
			{
				...
				{
					// Permited weapons in mod settings
...
// If conVar hok_enabled changes, log change and adverts users
public HoK_Event_ServercVar(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ( StrEqual(cVarName, "hok_enabled") )
	{
		if ( ! StringToInt(cVarValue) )
		{
			LogMessage("[HoK] %t", "Mod Off")
			PrintToChatAll("[HoK] %t", "Mod Off")
			PrintHintTextToAll("[HoK] %t", "Mod Off", HOKMODVERSION)
		}
...
// Clean handle to get users action on panel

...
ConVar pb_infiniteVictims;
Handle g_hClientCookieNopiggy = INVALID_HANDLE;
int g_bNoPiggy[MAXPLAYERS+1] = false;
...
public void OnPluginStart() {
	...
	pb_infiniteVictims = CreateConVar("pb_infiniteVictims", "0", "Allow infinite victims (only 1 if disabled)");
...
{
    if (CheckCommandAccess(client, "piggyback_admin_override", ADMFLAG_CUSTOM5, true))
        return true;
    return false;
}
...
public Action spamProtect(Handle hTimer) {
...
public int LoadClientCookiesFor(int client)
{
	...
	if(StrEqual(buffer,"")){
		g_bNoPiggy[client] = !GetConVarBool(pb_enabledDefault);
		g_nopiggy[client] = (GetConVarBool(pb_enabledDefault) ? client : -1);
	}
}
...
public int Piggy(int entity, int other) {
	...
	if (strcmp(classname, PLAYER) == 0 && strcmp(otherName, PLAYER) == 0 && entity != other && GetClientTeam(entity) == GetClientTeam(other) && IsPlayerAlive(entity) && IsPlayerAlive(other) && (g_piggy[entity] <= -1) && (g_piggy[other] <= -1) && (g_piggy[other] != entity)) {
		if ((TF2_GetPlayerClass(other) != TFClass_Spy)) {
			...
			if(distance <= 20000.0) 
			{
				if(g_piggy_victim[other] != -1 && !GetConVarBool(pb_infiniteVictims))
				{

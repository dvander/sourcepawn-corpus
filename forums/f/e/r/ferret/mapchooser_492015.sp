...
public OnPluginStart()
{
	...
	if(strcmp(g_szModName, "cstrike", false) == 0)
	{
...
{
	new iWinLimit, iMaxRounds;
	new iTimeLimit = GetConVarInt(g_hMpTimelimit);
	
	if(strcmp(g_szModName, "cstrike", false) == 0)
	{
		iWinLimit = GetConVarInt(g_hMpWinlimit);
	...
	if(iWinLimit)
	{
		...
		if(g_iTeamScore[2] > iLimit || g_iTeamScore[3] > iLimit)
			bIssueVote = true;		
	}
	else if(iMaxRounds)
	{
...
public Handler_MapVoteMenu(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_VoteEnd)
	{
		...
		if(strcmp(szMap, "extend", false) == 0)
		{
			...
			if(iWinLimit && iWinLimit < iRoundExtendMax)
			{

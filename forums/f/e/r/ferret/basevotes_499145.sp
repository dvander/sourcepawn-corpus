 ...
 * - New cvar: sm_vote_master: Default is 0.0. If you set this, all other vote percentage
 *     Cvars will be ignored, and this will be used instead.
...
public OnPluginStart()
{
	...
	g_Cvar_VoteMaster = CreateConVar("sm_vote_master", "0.0", "Percentage required for a vote to succeed. Overrides over cvars if not 0.", 0, true, 0.0, true, 1.0);
...
public Handler_VoteMapMenu(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_VoteEnd)
	{
		...
		if ((strcmp(display,"Yes") == 0 && FloatCompare(percent,GetVoteLimit(g_Cvar_VoteMap)) < 0 && param1 == 0) || (strcmp(display,"No") == 0 && param1 == 1))
		{
			LogMessage("Vote map failed.");
			PrintToChatAll("[SM] %T", "Map Failed", LANG_SERVER, RoundToNearest(100.0*GetVoteLimit(g_Cvar_VoteMap)), RoundToNearest(100.0*percent), totalVotes);
		}
		else
		{
...
public Handler_VoteKickMenu(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_VoteEnd)
	{
		...
		if (FloatCompare(percent, GetVoteLimit(g_Cvar_VoteKick)) >= 0 && param1 == 0)
		{
		...
		else
		{
			...
			PrintToChatAll("[SM] %T", "Kick Failed", LANG_SERVER, g_voteTarget, RoundToNearest(100.0*GetVoteLimit(g_Cvar_VoteKick)), RoundToNearest(100.0*percent), totalVotes);
		}
	}
}
...
public Handler_VoteBanMenu(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_VoteEnd)
	{
		...
		if (FloatCompare(percent,GetVoteLimit(g_Cvar_VoteBan)) >= 0 && param1 == 0)
		{
			GetMenuItem(menu, param1, userIdStr, sizeof(userIdStr));
			userId = StringToInt(userIdStr);
		...
		else
		{
			...
			PrintToChatAll("[SM] %T", "Ban Failed", LANG_SERVER, g_voteTarget, RoundToNearest(100.0*GetVoteLimit(g_Cvar_VoteBan)), RoundToNearest(100.0*percent), totalVotes);
		}
	}
}
...
Float:GetVoteLimit(Handle:cvar)
{
	new Float:master = GetConVarFloat(g_Cvar_VoteMaster);

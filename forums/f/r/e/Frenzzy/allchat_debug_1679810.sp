...
/* Globals */
#define UPDATE_URL "http://vsdir.com/sm/allchat/update_debug.txt"
...
public Action:Event_PlayerSay(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    AllChat_DebugLog("[player_say] [userid %4d] %s", GetEventInt(event, "userid"), text);
    #endif
    ...
    #if defined DEBUG
    AllChat_DebugLog("[player_say] [userid %4d] allchat: this message is duplicated for %d players.", GetEventInt(event, "userid"), playersNum);
    #endif
...
public Action:Command_Say(client, const String:command[], argc)
{
    ...
    {
        #if defined DEBUG
        AllChat_DebugLog("  [say_team] [userid %4d] %s", GetClientUserId(client), text);
        #endif
        
        g_msgIsTeammate = true;
    }
    else if (StrEqual(command, "say2", false))
    {
        #if defined DEBUG
        AllChat_DebugLog("      [say2] [userid %4d] %s", GetClientUserId(client), text);
        #endif
    ...
    {
        #if defined DEBUG
        AllChat_DebugLog("       [say] [userid %4d] %s", GetClientUserId(client), text);
        #endif
...
#if defined DEBUG
AllChat_DebugLog(const String:format[], any:...)
{
    decl String:buffer[256], String:path[PLATFORM_MAX_PATH];
    VFormat(buffer, sizeof(buffer), format, 2);
    BuildPath(Path_SM, path, sizeof(path), "logs/AllChat_Debug.log");

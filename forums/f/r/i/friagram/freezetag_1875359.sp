...
#define MAXENTS         4096  //2048                                  // Maximum entities to search through
...
new g_autoTimeRedMax;                             // Number of iterations to actually do (because it's not 1 every second)
new g_autoTimeBlue;                               // Number of seconds to automatically unfreeze frozen people
new g_autoTimeBlueMax;                            // Number of iterations to actually do (because it's not 1 every second)
...
new Float:g_fRingIval;                            // Store our ring update interval
...
new bool:g_bRespawned[MAXPLAYERS+1];              // Track players being moved to spawnroom
new g_stuck[MAXPLAYERS+1];                        // Allow players to request being unstuck
new g_iceblock[MAXPLAYERS+1];                     // Track iceblock models
new Float:g_fChargemeter[MAXPLAYERS+1];           // Track charge meters
...
new g_unfreezer[MAXPLAYERS+1];                    // Track the unfreezing client
...
static String:g_sMeter[] = "||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||";   // Progress Bar.. lol @ max len
static Float:g_fCollisionvec[3] = {24.0,24.0,62.0}; // bbox for ducking player
static Float:g_fNovelocity[3] = {0.0,0.0,0.0};      // zero out velocity for unfrozen bots so they don't crater or fly around
...
new Handle:g_hTimeleftHUD;
new Handle:g_hGametimer;
new Float:g_fMapStartTime;                        // Stores start time of map
...
public OnPluginStart()
{
	...
	if (g_cloakOffset == -1)
	{
		SetFailState("[TF2] Freeze Tag: Freeze tag could not locate m_flCloakMeter in CTFPlayer.");
        }
        g_wearableOffset = FindSendPropOffs("CTFWearable", "m_hOwnerEntity");                           // Wearables
	if (g_cloakOffset  == -1)
	{
		SetFailState("[TF2] Freeze Tag: Freeze tag could not locate m_hOwnerEntity in CTFWearable.");
        }
	g_primaryOffset = FindSendPropOffs("CBaseCombatWeapon","m_flNextPrimaryAttack");                // Weapon attack speed, thx psychonic
	if (g_cloakOffset  == -1)
	{
		SetFailState("[TF2] Freeze Tag: Freeze tag could not locate m_flNextPrimaryAttack in CBaseCombatWeapon.");
        }
        g_secondaryOffset = FindSendPropOffs("CBaseCombatWeapon","m_flNextSecondaryAttack");
	if (g_cloakOffset  == -1)
	{
		SetFailState("[TF2] Freeze Tag: Freeze tag could not locate m_flNextSecondaryAttack in CBaseCombatWeapon.");
        }
...
public OnMapStart()
{
	...
	g_activeround = 0;
        ...
        {
        	g_iceblock[i] = -1;                                                                      // Initialize Icelbock Array
       	}
}
...
addEvents()
{
	...
	AddCommandListener(Command_InterceptItemTaunt, "use_action_slot_item_server");
                ...
                for (new ent = 1; ent < MAXENTS; ent++)
		{
			if (IsValidEdict(ent))
                	{
				GetEdictClassname(ent, entname, 32);
...
public OnEntityCreated(ent, const String:strClassname[])
{
		...
		else if (g_activeround && StrEqual( strClassname, "tf_ragdoll", false ))
		{
			CreateTimer(0.0, Timer_RemoveRagdoll, ent);
		}
	}
}
...
{
        if (g_activeround && IsValidClient(client))
	{
...
{
        if (g_activeround && IsValidClient(client) && g_bFrozen[client] )
...
{
        if (!g_bDisableCaps && IsValidClient(client) && !g_bFrozen[client])
        {
                return Plugin_Continue;
...
{
        if (!g_bDisableFlags && IsValidClient(client) && !g_bFrozen[client])     // Allow capping, but don't allow frozen players to cap
        {
                return Plugin_Continue;
        }
        return Plugin_Handled;
...
public Action:event_round_start(Handle:event, const String:name[], bool:dontBroadcast)
{
        if (g_hGametimer != INVALID_HANDLE)
        {
                ...
                g_hGametimer = INVALID_HANDLE;                                                             // If a mp_restartround is issued, two timers can exist, this prevents it
        }

        if (g_rounds == 1)
        {
                g_fMapStartTime = GetGameTime();                                                           // Store the game start timer, it resets itself after waiting for players ends
...
public Action:Timer_PreRoundEnd(Handle:timer, any:extended)
{
        ...
        g_hGametimer = CreateTimer(1.0, Timer_TeamFreezeCheck, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);   // Check every 1 second to see if everyone is frozen/tick the hud
	return Plugin_Continue;
}
...
public event_post_inventory_application(Handle:hEvent, String:strName[], bool:bDontBroadcast)
{
	...
	if (IsValidClient(client) && IsPlayerAlive(client))                                               // Enabled here, activeround comes later
	{
			...
			g_futureclass[client] = TFClass_Unknown;
                ...
                if (g_bRespawned[client])                                                                    // They got moved back to spawn by trigger or stuck command and are still frozen
                {
		         	...
		         	new medigunidx = (weapon > MaxClients && IsValidEdict(weapon) ? GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex") : -1);
      	 		 	if (medigunidx != 411 && medigunidx != 998)                              // If it's not a quick fix or vaccinator, replace it with gimp kritz
					 	...
					 	weapon = TF2Items_GiveNamedItem(client, hWeapon);
                                ...
                                if (g_fChargemeter[client] != 0.0)                                          // Fix their medigun charge, if they got respawned from being stuck inside something
	                        {
	                                ...
	                                if (IsValidEntity(medigun))
	                                {
	                                        ...
	                                        {
                                                        SetEntPropFloat(medigun, Prop_Send, "m_flChargeLevel",g_fChargemeter[client]);
	                                                g_fChargemeter[client] = 0.0;
	                                        }
	                                }
	                        }
                 	}
...
public Action:Timer_UnfreezeAllClients(Handle:timer)
{
        ...
        {
                if (g_bFrozen[i] && IsValidClient(i) && IsPlayerAlive(i))
                {                                                                // unfreezeClient(client, other, bool:automatic=false, bool:sound=true, bool:particles=true, bool:respawn=false)
...
{
        if ((attacker > MaxClients || attacker < 1) && IsValidClient(client) && IsPlayerAlive(client) && g_bFrozen[client])
...
public Action:event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
        ...
        if (g_activeround && GetEventInt(event, "death_flags") & TF_DEATHFLAG_DEADRINGER) // Don't intercept deadringers
	{
...
public Action:Timer_TheIceIsaSpy(Handle:timer, Handle:pack)
{
	...
	new touchedground = ReadPackCell(pack);
        ...
        if (!iterations || !IsValidClient(client) || !IsPlayerAlive(client) || g_bFrozen[client])                 // kill it
        {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
public Action:Timer_RemoveRagdoll(Handle:timer, any:ent)
{
	if (ent > MaxClients + 1 && IsValidEntity(ent))
	{
		AcceptEntityInput(ent, "Kill");
	}
}
...
{                                                                             // Perhaps a better method would be some trick to parent the item to another item or model on the player
        if (!g_bFrozen[client] ||                                                // It appears that t2 has disabled parenting of models to players
            !IsValidEntity(g_iceblock[client]) ||
	...
	GetClientAbsOrigin(client, pos);
        TeleportEntity(g_iceblock[client], pos, NULL_VECTOR, NULL_VECTOR);      // Move it to the client, ignore the angle, it'll look stupid turning
...
{                                                                             // If respawn worked faster and didn't #&@@ with movement speeds, we would not need this.
        ...
        if (!IsValidClient(client) || !IsPlayerAlive(client))
	{
        ...
        if (!g_bFrozen[client])                                                   // If they got unfrozen by command, or really quickly, kill the timer
        {
...
giveDummyWeapons(client)
{
	...
	if (hWeapon != INVALID_HANDLE)
	{
		...
		TF2Items_SetAttribute(hWeapon, 0, 1, 0.0);
                ...
                {
                	EquipPlayerWeapon(client, weapon);
                        SetEntDataFloat(weapon, g_primaryOffset, GetGameTime()+g_roundLength);
                        SetEntDataFloat(weapon, g_secondaryOffset, GetGameTime()+g_roundLength);
                }
	}
}
...
{
        if (g_iceblock[client] > MaxClients && IsValidEntity(g_iceblock[client]))                                  // Kill the old one if it somehow exists (admin slay frozen player or somesuch nonsense)
	{
		AcceptEntityInput(g_iceblock[client],"kill");
	}
	...
	if ( ice1 != -1 )
	{
		...
		AcceptEntityInput(ice1, "TurnOn");
                ...
                g_iceblock[client] = ice1;                                      // Preserve, for later
...
unfreezeClient(client, other, bool:automatic=false, bool:sound=true, bool:particles=true, bool:respawn=false)
{
        ...
        if (IsFakeClient(client))                                            // Un#&@# bots
        {
                ...
                TeleportEntity(client, NULL_VECTOR, NULL_VECTOR, g_fNovelocity);// Bots will pop out of freeze if they have velocity
        }
...
destroyIceBlock(client=0, bool:all=false)                                     // Destroys a single client's ice block, or all - I could use 0 here instead of the bool, but that's bad practice =S
{
	if (all)
	{
                for (new i=1; i<=MaxClients; i++)
                {
                        if (g_iceblock[i] > MaxClients && IsValidEntity(g_iceblock[i]))
	                {
		                AcceptEntityInput(g_iceblock[i],"kill");
		                g_iceblock[i] = -1;
	                }
                }
        }
        ...
        {
                if (g_iceblock[client] > MaxClients && IsValidEntity(g_iceblock[client]))
	        {
		        AcceptEntityInput(g_iceblock[client],"kill");
		        g_iceblock[client] = -1;
	        }
...
{                                                            // This is horribly sloppy, ugly function, and should be put out of its misery.
	new clientteam = GetClientTeam(client);              // Most of the time it will resolve quickly, unless they are on some rubble/weapon pile, then it will elevate them, else > respawn
        ...
        for (new i=1; i<=MaxClients; i++)
        {
                if (!g_bFrozen[i] && IsValidClient(i) && IsPlayerAlive(i) && (clientteam != GetClientTeam(i)) )                 // iterate over nonfrozen clients on opposite team
		{
                                        ...
                                        {                                                                                             // Should store medic uber here
                                                ...
                                                if (TF2_GetPlayerClass(i) == TFClass_Medic)                                           // Medics will need their ubercharge recovered
	                                        {
	                                        	...
	                                        	if (IsValidEntity(medigun))
	                                        	{
	                                        		...
	                                        		if (!strcmp(s,"tf_weapon_medigun"))
	                                        		{
	                                                        	g_fChargemeter[i] = GetEntPropFloat(medigun, Prop_Send, "m_flChargeLevel");
	                                        		}
	                                        	}
	                                        }
                                                TF2_RespawnPlayer(i);
...
public bool:TraceFilterNotSelf(entity, contentsMask, any:client)
{
        if ((entity > 0) && (entity <= MaxClients))          // player index range
        {
                if(g_bFrozen[entity] && GetClientTeam(entity) != GetClientTeam(client))
                {
...
public Action:Timer_FreezeBeacon(Handle:timer, Handle:pack)
{
	...
	new Float:vClientPosition[3], Float:maxdist;
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (!g_bFrozen[i] && IsValidClient(i) && IsPlayerAlive(i))
		{
			...
			if (GetVectorDistance(vClientPosition, vec, false) < maxdist)			// Player is in range of the effect
			{
	...
	if (testProx(frozenguy, teammate))                             // Teammate was within the frozen player's proximity ring!
        {
                ...
                if(g_unfreezer[frozenguy] == teammate && (flags & FL_ONGROUND || flags & FL_INWATER))
                {
		        decl Float:vec[3];
...
{
	if (g_activeround == 1 && g_preRoundTime)
	{

...
- Rounds with only 1 player will not end. The timer will remain static, and they will be automatically unfrozen if they die.
- Fixed a death vecotr being accidentally stored twice.
- Moved the "You are now frozen" message to display immediately upon becoming frozen (it was time-delayed before).
- Updated the /freezetag menu to show information about automatic unfreeze in tips and map settings.
...
- Prevented the game timer from continuing/looping if nobody was playing. Also fixes a rare duplicate timer bug.
*/
...
#define AUTORINGHEIGHT  20.0                                         // Height of ring from ground (in addition to RINGHEIGHT)
...
#define PARTICLEZ 30          // Distance to raise the particle effect
#define NOTIME 9.0            // Time for the no-entry symbol to last on the fake spy
...
new bool:beta;                                   // Are we in tf2 beta?
...
new bool:autoUnfreezeDamage[MAXPLAYERS+1];       // Track client cams
...
new g_redscore;
new bool:waitingforplayers;                       // Sigh
...
public OnPluginStart()
{
	g_hCvarVersion = CreateConVar("sm_tf2freezetag_version", PLUGIN_VERSION, PLUGIN_NAME, FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
  	...
  	g_hCvarCam = CreateConVar("sm_freezetag_thirdperson", "0.0", "Disable 0, Force TP 1, Preserve [0-2]", FCVAR_PLUGIN, true, 0.0, true, 2.0);
        HookConVarChange(g_hCvarCam, ConVarCamChanged);

  	g_hCvarAutoTime = CreateConVar("sm_freezetag_autotime", "0", "Length of Time Before Automatically Unfreezing Players [0-1800]", FCVAR_PLUGIN, true, 0.0, true, 1800.0);
        HookConVarChange(g_hCvarAutoTime, ConVarAutoTimeChanged);

  	g_hCvarAutoHPRatio = CreateConVar("sm_freezetag_autohpratio", "0.25", "Unfreeze Health Restore Ratio When Unfrozen Automatically [0.01-1.0]", FCVAR_PLUGIN, true, 0.01, true, 1.0);
        HookConVarChange(g_hCvarAutoHPRatio, ConVarAutoHPRatioChanged);
	...
	beta = StrEqual(gamedir, "tf_beta", false);
	if (!StrEqual(gamedir, "tf", false) && !beta)                                                  // Beta may need some special handling =/
	{
	...
	g_primaryOffset = FindSendPropOffs("CBaseCombatWeapon","m_flNextPrimaryAttack");
...
{
        if (!client || !IsPlayerAlive(client)) return Plugin_Handled;                                     // Console and deaders don't need to use this
	if (!g_bEnabled)
        {
...
{                                                                               // Would need to completely re-do this for translations because of bizzarre spacing
        ...
        else if (action == MenuAction_Select)
	{
                ...
                else if (param2 == 3)
                {
	                ...
	                if (g_preRoundTime)
                        {
                                Format(buffer, 50,     "Preround Length:  %i Seconds", g_preRoundTime);                              // Round time, if set
                                DrawPanelText(cpanel, buffer);
                        }
	                Format(buffer, 50,     "Round Length:  %i Seconds", g_timeLeft);                                             // Round Timer
                        DrawPanelText(cpanel, buffer);

                        if (g_maxRounds)
                        {
                                Format(buffer, 50,     "Round Limit:  %i", g_maxRounds);                                             // Round Limit, if set
                                DrawPanelText(cpanel, buffer);
                        }

	                DrawPanelText(cpanel, (g_forceMulti*g_forceCap ? "Damage Force Push: Enabled" : "Damage Push: Disabled") );  // Force push

	                Format(buffer, 50,     "Unfreeze Duration:  %i Seconds", g_freezeDur);                                       // Unfreeze Dutation
	                DrawPanelText(cpanel, buffer);

	                Format(buffer, 50,     "Revive Health:  %i Percent", RoundFloat(g_unfreezeHPRatio*100) );                    // Unfreeze Ratio
                        DrawPanelText(cpanel, buffer);

	                if (g_autoTime)
                        {
                                Format(buffer, 50,     "Auto Unfreeze Duration:  %i Seconds", g_autoTime);                           // Auto Unfreeze Duration
                                DrawPanelText(cpanel, buffer);
	                        Format(buffer, 50,     "Auto Unfreeze Revive Health:  %i Percent", RoundFloat(g_autoHPRatio*100) );  // Auto Unfreeze Ratio
                                DrawPanelText(cpanel, buffer);
                        }
                        else
                        {
	                ...
	                Format(buffer, 50,     "Unfreeze Radius:  %i Units", RoundFloat(g_freezeDist));                              // Unfreeze Radius
                        DrawPanelText(cpanel, buffer);
	                ...
	                DrawPanelText(cpanel, (g_openAPs ? "Area Portals: Forced Open" : "Area Portals: Unchanged") );
                        ...
                        DrawPanelText(cpanel, (g_feedback ? "Feedback Messages: Enabled" : "Feedback Messages: Disabled") );
                }
                else if (param2 == 4)
                {
...
public Action:event_player_changeclass(Handle:event, const String:name[], bool:dontBroadcast)
{
        if (activeround == 2)                                                                 // If they are trying to pick a new class and a game is already in progress, save it
        {
	        ...
	        futureclass[client] = TFClassType:GetEventInt(event, "class");                // Store their desired class selection for next round
        }
...
{
	if (g_bEnabled && !activeround)
	{
		new client = GetClientOfUserId(GetEventInt(event, "userid"));
		if (futureclass[client] != TFClass_Unknown)                                   // If the player has a class selection queued, do it
		{
			switch(futureclass[client])                                           // Maintain compliance with class-limit enforcer plugins :D
			{
...
public Action:event_round_start(Handle:event, const String:name[], bool:dontBroadcast)
{
		...
		if (g_bEnabled && g_feedback)                                                             // Info Message on first round
		{
        ...
        if (g_bEnabled && g_rounds && !waitingforplayers)                                                 // Disable on first round (waiting for players)
        {
        ...
        g_rounds++;                                                                                       // Increment the round counter, so that we know what our current round is.
        waitingforplayers = false;
	return Plugin_Continue;
}

public Action:Timer_TeamWarmupFreeze(Handle:timer, Handle:pack)               // Stop them from moving when they come back alive, inventory app allows movement for some few seconds
...
public Action:Timer_PreRoundEnd(Handle:timer, any:extended)                                               // Used to start the active round after warmup has finished
{
        if(extended)                                                                                      // If the pre-round was extended, clean up player movement speeds
        {
                for (new i=1; i<=MaxClients; i++)
                {
                        if (IsValidClient(i) && IsPlayerAlive(i))
...
public Action:OnTakeDamage(client, &attacker, &inflictor, &Float:damage, &damagetype)
{
        ...
        if (frozen[client] && (GetClientTeam(client) != GetClientTeam(attacker)))               // We are only interested in corpsicles here of opposing teams
	{
...
public Action:event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
        ...
        if (activeround && GetEventInt(event, "death_flags") & TF_DEATHFLAG_DEADRINGER) // Don't intercept deadringers
	{
                ...
                if (fakespy != -1)
                {
                        fakeice = createFakeIceBlock(client,time);                      // Spawn a ice block + sound, parent it to the spy model
                        createFakeParticle(client,time);
                        if (fakeice != -1)
                        {
                                new Handle:spypack;
...
public Action:Timer_TheIceIsaSpy(Handle:timer, Handle:pack)    // Moves fake ice blocks.
{
        ResetPack(pack);
	...
	new touchedground = ReadPackCell(pack);
        ...
        if (!iterations || !IsValidClient(client) || !IsPlayerAlive(client) || frozen[client])                 // kill it
        {
                if (IsValidEntity(fakespy))
	        {
                        ...
                        EmitAmbientSound(soundfile, pos, fakespy, SNDLEVEL_NORMAL );                           // Emit Sound When Block is Destroyed.
        ...
        if(touchedground)
        {
                        ...
                        for (new i=1; i<=MaxClients; i++)
		        {
			        if (IsValidClient(i) && GetClientTeam(i) == clientsteam)                       // spectators can't see this, but who really cares?
                                {
                                        TE_SetupGlowSprite(pos, g_NoSprite, NOTIME, 0.3, 20);
                                        TE_SendToClient(i);
                                }
			}
                }
...
//                              lifecolors[2] = lifecolors[1];
                                lifecolors[2] = 0;
                                TE_SetupBeamRingPoint(vec, 90.0, 91.0, g_BeamSprite, g_HaloSprite, 0, 10, 0.2, AUTORINGWIDTH, 0.0, lifecolors, 10, 0);
                                TE_SendToAll();
...
//                              lifecolors[0] = lifecolors[1];
                                lifecolors[0] = 0;
                                lifecolors[2] = 255;
                                TE_SetupBeamRingPoint(vec, 90.0, 91.0, g_BeamSprite, g_HaloSprite, 0, 10, 1.01, AUTORINGWIDTH, 0.0, lifecolors, 10, 0);
                                TE_SendToAll();
                                vec[2] += AUTORINGHEIGHT;
                                TE_SetupBeamRingPoint(vec, 90.0, 91.0, g_BeamSprite, g_HaloSprite, 0, 10, 1.01, AUTORINGWIDTH, 0.0, lifecolors, 10, 0);
                                TE_SendToAll();
        ...
        WritePackCell(spypack, fakespy);                                                               // spy model index
        WritePackCell(spypack, iterations-1);                                                          // iteration decrement
        WritePackCell(spypack, GetEntityFlags(client) & FL_ONGROUND);                                  // test if they are touching ground on next iteration pack
...
public Action:Timer_TeamFreezeCheck(Handle:timer)                    // The basis of this timer was borrowed from freak fortress 2 :D
{
        ...
        for (new i=1; i<=MaxClients; i++)                                                               // Print out HUD
	{
        ...
        ///////////////////////////////////////// 1 Person IS PLAYING /////////////////////////// How Lonely They Must Be ///
        ...
        else if (!time)
	{
		if ((redtotal+bluetotal) == 1)
		{
                ...
                if (redratio == blueratio)
                {
                        ...
                        if(g_feedback)
                        {
                                PrintToChatAll("\x04[Freeze]:\x01 %t [\x076E89D4%i/%i\x01 %t \x07F07560%i/%i\x01 %t]","winreason_tie",bluefrozen,bluetotal,"wincompare_frozen",redfrozen,redtotal,"win_frozen");
                        }
                }
                else if(redratio > blueratio)
                {
                        ...
                        if(g_feedback)
                        {
                                PrintToChatAll("\x04[Freeze]:\x01 %t \x076E89D4%t\x01 [\x076E89D4%i/%i\x01 %t \x07F07560%i/%i\x01 %t]","winreason_time","victory_blu",bluefrozen,bluetotal,"wincompare_frozen",redfrozen,redtotal,"win_frozen");
                        }
                }
                else
                {
                        ...
                        if(g_feedback)
                        {
                                PrintToChatAll("\x04[Freeze]:\x01 %t \x07F07560%t\x01 [\x07F07560%i/%i\x01 %t \x076E89D4%i/%i\x01 %t]","winreason_time","victory_red",redfrozen,redtotal,"wincompare_frozen",bluefrozen,bluetotal,"win_frozen");
                        }
                }
		g_gametimer = INVALID_HANDLE;
                return Plugin_Stop;
...
/*
	if ((red+redfrozen == 0 && bluefrozen == 0) || (blue+bluefrozen == 0 && redfrozen == 0))            // Waiting for players to join a team, prevents elimination stalemate loop (can only end via round timelimit).
        {
...
*/
        if ((red + blue) == 0)                                    // Stalemate
        {
                ...
                if (g_feedback)
                {
                        PrintToChatAll("\x04[Freeze]:\x01 %t [\x076E89D4%i/%i\x01 %t \x07F07560%i/%i\x01 %t]","winreason_stalemate",bluefrozen,bluetotal,"wincompare_frozen",redfrozen,redtotal,"win_frozen");
                }
		g_gametimer = INVALID_HANDLE;
                return Plugin_Stop;
        }
	if (red == 0)                                             // Red is ded baby
        {
                ...
                if (g_feedback)
                {
                        PrintToChatAll("\x04[Freeze]:\x01 %t \x076E89D4Blue wins!\x01  [\x076E89D4%i/%i\x01 %t \x07F07560%i/%i\x01 %t]","winreason_elimination",bluefrozen,bluetotal,"wincompare_frozen",redfrozen,redtotal,"win_frozen");
                }
		g_gametimer = INVALID_HANDLE;
                return Plugin_Stop;
	}
	if (blue == 0)                                            // Blue lost
        {
                ...
                if (g_feedback)
                {
                        PrintToChatAll("\x04[Freeze]:\x01 %t \x07F07560Red wins!\x01 [\x07F07560%i/%i\x01 %t \x076E89D4%i/%i\x01 %t]","winreason_elimination",redfrozen,redtotal,"wincompare_frozen",bluefrozen,bluetotal,"win_frozen");
                }
		g_gametimer = INVALID_HANDLE;
                return Plugin_Stop;
	}
...
ForceTeamWin(team)                                                               // Borrowed from freak fortress 2
{
	...
	if (team == RED)                                                         // Set the scoreboard, since game win won't always do it
	{
        ...
        if(!beta)                                                                // For some reason beta can't find m_iScore, and this fails
        {
                SetTeamScore(RED, g_redscore);                                   // We will set our own scores, since payload fouls it up, this is graphical only
...
stock createFakeIceBlock(client, time)
{
	new ice1 = CreateEntityByName("prop_dynamic");
	if ( ice1 != -1 )
	{
...
destroyIceBlock(client=0, bool:all=false)                                     // Destroys a single client's ice block, or all
{
	if (all)
	{
                for (new i=1; i<=MaxClients; i++)
...
unfreezeClient(client, other, bool:automatic=false, bool:sound=true, bool:particles=true)
{
        ...
        if (IsFakeClient(client))                                            // Un#&@# bots
        {
                ...
                TeleportEntity(client, NULL_VECTOR, NULL_VECTOR, novelocity);// Bots will pop out of the cube if they have velocity
        }
//      ResetPlayerSpeed(client);                                            // Restore Movement (Regenerate does this for us)
        ...
        ResetPlayerHealth(client,automatic);                                 // Restore them with % max health by base class
...
public Action:Timer_FreezeBeacon(Handle:timer, Handle:pack)                  // Handles Proximity Detection for the Frozen Player
{
	ResetPack(pack);
	new client = ReadPackCell(pack);
	new iterations = ReadPackCell (pack);
	...
	if (g_autoTime && iterations == g_autoTimeMax)
	{
		...
		unfreezeClient(client,client,true);                 // Unfreeze them with modified HP
	...
	{
                if(autoUnfreezeDamage[client])                                 // They took some incoming force damage, sad for them
                {
                        iterations = 0;                                        // VGS
                ...
                if (clientteam == RED)
                {
                        lifecolors[0] = 255;
                        lifecolors[2] = lifecolors[1];
                        TE_SetupBeamRingPoint(vec, 90.0, 91.0, g_BeamSprite, g_HaloSprite, 0, 10, g_ringIval, AUTORINGWIDTH, 0.0, lifecolors, 10, 0);
                        TE_SendToAll();
                        vec[2] += AUTORINGHEIGHT;
                        TE_SetupBeamRingPoint(vec, 90.0, 91.0, g_BeamSprite, g_HaloSprite, 0, 10, g_ringIval, AUTORINGWIDTH, 0.0, lifecolors, 10, 0);
                        TE_SendToAll();
	...
	if (testProx(frozenguy, teammate))                             // Teammate was within the frozen player's proximity ring!
        {
                ...
                if(unfreezer[frozenguy] == teammate)
                {
                        PrintCenterText(frozenguy, buffer);            // Don't flood the client, he's got enough to worry about
                }
...
public Action:Timer_RespawnPlayer(Handle:timer, any:client)                                      // Respawns a player
{
	if (IsValidClient(client))
	{

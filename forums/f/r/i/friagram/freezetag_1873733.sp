...
addEvents()
{
	...
	AddCommandListener(Command_InterceptItemTaunt, "use_action_slot_item_server");
        ...
        {
		new ent = -1;
		while ((ent = FindEntityByClassname2(ent, "trigger_hurt")) != -1)                        // This shit will kill people, better respawn them
		{
			SDKHook(ent, SDKHook_StartTouch, OnHurtTouch);
			SDKHook(ent, SDKHook_Touch, OnHurtTouch);
		}
		ent = -1;
		while ((ent = FindEntityByClassname2(ent, "trigger_capture_area")) != -1)                // Frozen people should not be able to cap
		{
			SDKHook(ent, SDKHook_StartTouch, OnCPTouch );
			SDKHook(ent, SDKHook_Touch, OnCPTouch);
		}
		ent = -1;
		while ((ent = FindEntityByClassname2(ent, "trigger_multiple")) != -1)                    // Fix Doors From Closing
		{
			SDKHook(ent, SDKHook_StartTouch, OnDoorTouch);
			SDKHook(ent, SDKHook_Touch, OnDoorTouch);
		}
		ent = -1;
		while ((ent = FindEntityByClassname2(ent, "item_teamflag")) != -1)                       // Frozen people should not interact with flags
		{
			SDKHook(ent, SDKHook_StartTouch, OnFlagTouch);
			SDKHook(ent, SDKHook_Touch, OnFlagTouch);
		}
		ent = -1;
		while ((ent = FindEntityByClassname2(ent, "func_respawnroom")) != -1)                    // Touching respawn rooms will allow players to change class if enabled
		{
		...
		ent = -1;
		while ((ent = FindEntityByClassname2(ent, "player" )) != -1)                             // Hook players for damage prevention and force push
		{
...
{                                                                            // Perhaps not needed, since they die.
	...
	while ((flag = FindEntityByClassname2( flag, "item_teamflag")) != -1)
	{
...
public Action:Timer_TheIceIsaSpy(Handle:timer, Handle:pack)
{
	...
	new touchedground = ReadPackCell(pack);
        ...
        {
                if (fakespy > MaxClients && IsValidEntity(fakespy))
	        {
...
{
	new ent = -1;
	while ((ent = FindEntityByClassname2(ent, "tf_ragdoll")) != -1)
	{
		AcceptEntityInput(ent, "Kill");
	}
}
...
{
        if (iceblock[client] > MaxClients && IsValidEntity(iceblock[client]))                                  // Kill the old one if it somehow exists (admin slay frozen player or somesuch nonsense)
	{
...
destroyIceBlock(client=0, bool:all=false)                                     // Destroys a single client's ice block, or all - I could use 0 here instead of the bool, but that's bad practice =S
{
	if (all)
	{
                for (new i=1; i<=MaxClients; i++)
                {
                        if (iceblock[i] > MaxClients && IsValidEntity(iceblock[i]))
	                {
        ...
        {
                if (iceblock[client] > MaxClients && IsValidEntity(iceblock[client]))
	        {
		        AcceptEntityInput(iceblock[client],"kill");
		        iceblock[client] = -1;
	        }
...
AttachParticle(ent)         // Borrowed from RTD and stripped down
{
	...
	if (particle > MaxClients && IsValidEntity(particle))
	{
		...
		CreateTimer(5.0, Timer_KillParticle, particle, TIMER_FLAG_NO_MAPCHANGE);
	}
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
public Action:Timer_KillParticle(Handle:timer, any:particle)
{
	if (particle > MaxClients && IsValidEntity(particle))
	{

...
public Action:Timer_PreRoundEnd(Handle:timer)                                                             // Used to start the active round after warmup has finished
{
        ...
        for (new i=1; i<=MaxClients; i++)
        {
        	if (IsValidClient(i))                                                                     // If they are dead, the problem should resolve itself
		{
			if(IsPlayerAlive(i))
                        {
                        ...
                        {
                                TF2_RespawnPlayer(i);                                                     // This shouldn't happen, but in rare cases it can
                        }
                }
        }
...
public Action:event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
                new time = RoundToNearest(GetEngineTime());                             // Gotta have something...
                ...
                if (fakespy != -1)
                {
                        ...
                        {
                                new Handle:pack;
                                CreateDataTimer(0.1, Timer_TheIceIsaSpy, pack, TIMER_HNDL_CLOSE|TIMER_FLAG_NO_MAPCHANGE);   // Move the decoy with the spy
                                WritePackCell(pack, client);
                                WritePackCell(pack, fakespy);                                                               // spy model index mabye not needed if the parent hierarchy can be killed
                                WritePackCell(pack, 66);                                                                    // iteration count (~10 seconds at 66?)
                                WritePackCell(pack, 0);                                                                     // touching ground (assume not)
                        }
                }
...
public Action:Timer_TheIceIsaSpy(Handle:timer, Handle:pack)    // Moves fake ice blocks.
{
	...
	new touchedground = ReadPackCell(pack);
        ...
        {
                new Handle:pack2;
                CreateDataTimer(0.1, Timer_TheIceIsaSpy, pack2, TIMER_HNDL_CLOSE|TIMER_FLAG_NO_MAPCHANGE);   // fire it again!
                WritePackCell(pack2, client);
                WritePackCell(pack2, fakespy);                                                               // spy model index
                WritePackCell(pack2, iterations-1);                                                          // iteration decrement
                WritePackCell(pack2, 1);                                                                     // stop the block from moving foreeeeeeeeeever!
        ...
        WritePackCell(pack2, fakespy);                                                               // spy model index
        WritePackCell(pack2, iterations-1);                                                          // iteration decrement
        WritePackCell(pack2, GetEntityFlags(client) & FL_ONGROUND);                                  // test if they are touching ground on next iteration pack
...
public Action:Timer_RemoveAllRagdolls(Handle:timer)
{
	new ent = -1;
	while ((ent = FindEntityByClassname2(ent, "tf_ragdoll")) != -1)       // No spawn protection
	{
		AcceptEntityInput(ent, "Kill");
	}
}
...
ForceTeamWin(team)                                                               // Borrowed from freak fortress 2
{
	...
	if (team == RED)                                                         // Set the scoreboard, since game win won't always do it
	{
        ...
        Team_SetScore(RED, g_redscore);                                          // We will set our own scores, since payload and shit fouls it up
        Team_SetScore(BLU, g_bluscore);
...
ResetPlayerHealth(client)                                                    // Decides what class player is and restores appropriate health
{
	new TFClassType:class = TF2_GetPlayerClass(client);
...
stock bool:Team_SetScore(index, score)                                                        // SMlib
{

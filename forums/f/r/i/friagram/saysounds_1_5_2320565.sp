...
new Float:gf_saysound_globaldelay;
...
public OnPluginStart()
{
	...
	HookConVarChange(cvar = CreateConVar("sm_saysounds_delay", "0.0", "Globaly delay between sounds",FCVAR_PLUGIN,true,0.0), Cvar_GlobalDelayChanged);
	gf_saysound_globaldelay = GetConVarFloat(cvar);
...
{
	gf_saysound_globaldelay = StringToFloat(newValue);
}
...
public Action:AttemptSaySound(client, String:sound[])
{
	...
	for(new i; i < size; i++)
	{
		...
		if((gb_saysound_sentence && StrContains(sound, buffer, false) >= 0) || strcmp(sound, buffer, false) == 0)
		{
			...
			lastglobal = gf_saysound_globaldelay + time;
			...
			if(gf_saysound_delay[g_access[client]])
			{
				...
				{
					lastsound[client] = time + gf_saysound_delay[g_access[client]];
				}
			}

...
new bool:gb_enabled;
new g_saysound_limit[4];
new Float:gf_saysound_delay[4];
new bool:gb_preventspam[4];
...
new Float:gf_lastglobal;
new bool:gb_globaldelay;
...
public OnPluginStart()
{
	...
	HookConVarChange(cvar = CreateConVar("sm_saysounds_sound_limit","10","Maximum sounds for clients (0 for unlimited)", FCVAR_PLUGIN, true, 0.0, false, 0.0), Cvar_LimitChanged);
	...
	g_saysound_limit[3] = GetConVarInt(cvar);

	HookConVarChange(cvar = CreateConVar("sm_saysounds_sound_delay","5.0","Time between each sound trigger for normal clients, 0.0 to disable checking", FCVAR_PLUGIN, true, 0.0, false, 0.0), Cvar_DelayChanged);
	gf_saysound_delay[1] = GetConVarFloat(cvar);

	HookConVarChange(cvar = CreateConVar("sm_saysounds_donor_delay","3.0","Time between each sound trigger for donor clients, 0.0 to disable checking", FCVAR_PLUGIN, true, 0.0, false, 0.0), Cvar_DonorDelayChanged);
	gf_saysound_delay[2] = GetConVarFloat(cvar);

	HookConVarChange(cvar = CreateConVar("sm_saysounds_admin_delay","1.0","Time between each sound trigger for admin clients, 0.0 to disable checking", FCVAR_PLUGIN, true, 0.0, false, 0.0), Cvar_AdminDelayChanged);
	gf_saysound_delay[3] = GetConVarFloat(cvar);

	HookConVarChange(cvar = CreateConVar("sm_saysounds_round", "0", "Reset the sounds on a new round?", FCVAR_PLUGIN, true, 0.0, true, 1.0), Cvar_RoundChanged);
	...
	gb_preventspam[3] = GetConVarBool(cvar);
	...
	HookConVarChange(cvar = CreateConVar("sm_saysounds_volume","1.0","Volume setting for Say Sounds (0.0 - 1.0 or 2.0)",FCVAR_PLUGIN,true,0.0,true,1.0), Cvar_VolumeChanged);
	gf_saysound_volume = GetConVarFloat(cvar);

	HookConVarChange(cvar = CreateConVar("sm_saysounds_delay","0.0","Global delay enabled to prevent sound overlap",FCVAR_PLUGIN,true,0.0,true,1.0), Cvar_GlobalDelayChanged);
	gb_globaldelay = GetConVarBool(cvar);
...
{
	g_saysound_limit[3] = StringToInt(newValue);
}
...
{
	gf_saysound_delay[3] = StringToFloat(newValue);
}
...
{
	gb_preventspam[3] = bool:StringToInt(newValue);
}
...
public Cvar_GlobalDelayChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	gb_globaldelay = bool:StringToFloat(newValue);
}
...
GetAccessLevel(client)
{
	if(CheckCommandAccess(client, "saysounds_admin", ADMFLAG_CHAT, true))
	{
		return SAYSOUND_ADMIN;
	}
	else if(CheckCommandAccess(client, "saysounds_donor", ADMFLAG_ROOT, true))
	{
		return SAYSOUND_DONOR;
	}
	else if(CheckCommandAccess(client, "saysounds_client", ADMFLAG_ROOT, true))
	{
		return SAYSOUND_CLIENT;
	}

	return SAYSOUND_NONE;
}
...
public Action:Command_Say(client, const String:command[], argc)
{
	...
	if(gb_enabled)
	{
		new access = GetAccessLevel(client);
		if(access != SAYSOUND_NONE && !g_clientprefs[client][SAYSOUND_PREF_DISABLED] && !g_clientprefs[client][SAYSOUND_PREF_BANNED])		// enabled, they can emit sounds to others
		{
			if (GetCmdArgString(speech, sizeof(speech)) >= 1)
			{
				...
				return  Action:AttemptSaySound(client, speech[startidx], access);
			}
		}
	}	
	return Plugin_Continue;
...
public Action:AttemptSaySound(client, String:sound[], access)
{
	static String:buffer[PLATFORM_MAX_PATH];
	static size, flags;
	static Handle:hpath;
	static Float:lastsound[MAXPLAYERS + 1];

	if(g_saysound_limit[access])																		// is there a limit, are they at it
	{
		if(g_soundcount[client] >= g_saysound_limit[access])
		{
			return Plugin_Continue;
		}
	}
	...
	for(new i; i < size; i++)
	{
		...
		{
			// is there a global delay
			if(gb_globaldelay && gf_lastglobal > time)
			{
				if(client)
				{
					PrintToChat(client, "[SM] please wait, a sound was recently played");
				}

				return Plugin_Continue;
			}
			
			// are they experiencing delay
			if(lastsound[client] > time)
			{
				if(client)
				{
					PrintToChat(client, "[SM] please wait, you recently played a sound");
				}
			...
			flags = GetArrayCell(gh_flags, i);
			if((flags & SAYSOUND_FLAG_ADMIN) && access != SAYSOUND_ADMIN)
			{
				adminonly = true;
			...
			if(gf_saysound_delay[access])
			{
				if(flags & SAYSOUND_FLAG_CUSTOMLENGTH)
				{
					lastsound[client] = time + Float:GetArrayCell(gh_length, i);
				}
				else
				{
					lastsound[client] = time + gf_saysound_delay[access];
				}
			}
			
			gf_lastglobal = lastsound[client];
			
			g_soundcount[client]++;
			DisplayRemainingSounds(client, access);
...
DisplayRemainingSounds(client, access)
{
	if(g_saysound_limit[access])
	{
		if(client && IsClientInGame(client))
		{
			PrintToChat(client, "[SM] you have used %d/%d sounds", g_soundcount[client], g_saysound_limit[access]);
		}
	}
}
...
public Action:Command_Sound_Reset(client, args)
{
	...
	if (count > 0)
	{
		for(new x=0;x<count;x++)
		{
			...
			DisplayRemainingSounds(clients[x], GetAccessLevel(clients[x]));
		}
	}
	else
	{
...
public Action:Command_Sound_Menu(client, args)
{
	if(client && IsClientInGame(client))
	{
		switch(GetAccessLevel(client))
		{
			case SAYSOUND_ADMIN: DisplayMenu(gh_adminmenu, client, 60);
			case SAYSOUND_CLIENT, SAYSOUND_DONOR: DisplayMenu(gh_menu, client, 60);
		}
	}
...
public menu_handler(Handle:menu,MenuAction:action,client,selection)
{
	if(action==MenuAction_Select)
	{
		...
		if (GetMenuItem(menu,selection,SelectionInfo,sizeof(SelectionInfo)))
		{
			...
			{
				AttemptSaySound(client, SelectionInfo, GetAccessLevel(client));
			}
		}
	}
}

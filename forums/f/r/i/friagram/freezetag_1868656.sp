/*
- Removed unneeded validity checks on events and commands
- Allowed console to use unfreeze command
- Prevented particles from spawning on unfreeze command and humiliation unfreeze
- Altered meter text display to frozen clients
- Fixed possible string length size problem in broadcast event
- Fixed for loop in unfreezeAll() to use (MAXPLAYERS+1) (It was before but.. not quite clear/in the correct way =S)
- Removed screen overlay functions that were no longer needed
- Added ability to automatically unfreeze players after a set time, as well as graphical indicators, and convars/config key values.
- Fixed respawn ducking to actually work. Lol.
- Moved welcome message to be displayed on the first round.
- Optimized teamcheck statements in the game timer loop.
- Removed unneeded setspeed from unfreezing event (weapon switch/regenerate does this already)
- Added attack blocking for setup, except for medic primary fire, to prevent a host of movement related problems with consumables, huntsman, minigun, etc.
- Frozen players will no longer see themselves attack when they press the fire button.
- Changed the graphical look of the ring beams.
- Only enemies can push players around now (some situations/weapons would allow teammates to do it before)
- Prevented TF2 Beta from setting the scoreboard.
*/
...
{
	new String:strVersion[16]; GetConVarString(g_hCvarVersion, strVersion, 16);                     // Check existing version, and compare to our version
...
public Action:event_round_start(Handle:event, const String:name[], bool:dontBroadcast)
{
		...
		{
                	PrintToChatAll("\x04[Freeze]:\x01 Welcome to Freeze Tag \x04%s\x01 by %s. %t",g_version,PLUGIN_AUTHOR,"feedback_welcomemsg");
		}
...
public Action:Timer_PreRoundEnd(Handle:timer, any:extended)                                               // Used to start the active round after warmup has finished
{
		...
		{
			playerclass[i] = TF2_GetPlayerClass(i);                                           // Store everypony's active, non-queud player class. Presumably, it's valid.
                        if(!IsPlayerAlive(i))
                        {
        ...
        if (activeround && GetEventInt(event, "death_flags") & TF_DEATHFLAG_DEADRINGER) // Don't intercept deadringers
	{
        ...
        if (activeround == 2)                                                           // Freeze if Active Round
        {
                ...
                if(g_cam == 2)                                                          // store client's current camera setting.
	        {
                ...
                GetEntPropVector(client, Prop_Data, "m_vecVelocity", deathVel[client]);
        ...
        {
                CreateTimer(0.0, Timer_RespawnPlayer, client);                          // Delay it
...
ResetPlayerHealth(client, bool:automatic=false)                              // Decides what class player is and restores appropriate health
{
	new TFClassType:class = TF2_GetPlayerClass(client);
        ...
        {                                                                    // yes.. that 20% is hard-coded in there, perhaps a cvar later, but I've already littered the server with so many.
                switch(class)
	        {
		        case TFClass_DemoMan: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(140*ratio));
		        case TFClass_Engineer: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(100*ratio));
		        case TFClass_Heavy: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(240*ratio));
		        case TFClass_Medic: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(120*ratio));
		        case TFClass_Pyro: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(140*ratio));
		        case TFClass_Scout: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(100*ratio));
		        case TFClass_Sniper: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(100*ratio));
		        case TFClass_Soldier: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(160*ratio));
		        case TFClass_Spy: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(100*ratio));
	        }
        }
        else
        {
                switch(class)
	        {
		        case TFClass_DemoMan: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(175*ratio));
		        case TFClass_Engineer: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(125*ratio));
		        case TFClass_Heavy: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(300*ratio));
		        case TFClass_Medic: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(150*ratio));
		        case TFClass_Pyro: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(175*ratio));
		        case TFClass_Scout: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(125*ratio));
		        case TFClass_Sniper: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(125*ratio));
		        case TFClass_Soldier: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(200*ratio));
		        case TFClass_Spy: SetEntProp(client, Prop_Data, "m_iHealth", RoundToNearest(125*ratio));
	        }
        }
}
...
{
        if (activeround && unassigned[client] && !IsPlayerAlive(client))                         // Force new joiners to respawn in active round (else they'll be dead)
        {
                CreateTimer(0.0, Timer_RespawnPlayer, client);                                   // Delay it, will respawn them and store their class.
        }

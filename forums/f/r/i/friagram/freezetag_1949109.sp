...
// merged entity flag statements
// changed onentitycreated
// removed light effect
// replaced unstuck code with player unsolid
	...
	g_bEnabled = (StringToInt(newvalue) == 0 ? false : true);                                      // Let's Make Arena More Friendly :D
        ...
        else if (!g_bEnabled && g_bHooked)                                                             // Remove hooks to reduce overhead
        {
                ...
                for (new i=1; i<=MaxClients; i++)
                {
                        ...
                        {
                                SetEntProp(i, Prop_Send, "m_CollisionGroup", 5);                       // player phys all
                        }
                }
        }
}
...
public event_player_spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		SetEntProp(client, Prop_Send, "m_CollisionGroup", 13);       // unsolid phys all
	}
}
...
public Action:Timer_PreRoundEnd(Handle:timer)
{
        for (new i=1; i<=MaxClients; i++)
        {
                if (IsClientInGame(i) && IsPlayerAlive(i))
                {
                        ...
                        SetEntProp(i, Prop_Send, "m_CollisionGroup", 13);                         // unsolid phys all
                }
        }
...
{
        if (!IsClientInGame(client))     // this should never be false
        {
LogError("[TF2] Freeze Tag: DEBUG > OnTakeDamage had an invalid client (no longer in game)");
		return Plugin_Continue;
	}
...
public event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
        ...
        if(!client)       // this should never be 0
        {
LogError("[TF2] Freeze Tag: DEBUG > event_player_death reported invalid client index.");
		return;	                                                                          // I have no idea ---- check this please!
	}
...
freezeClient(client)
{
        ...
        if (g_bUseModel)
        {
		/*                                                                              // Debug - disabled also for performance reasons

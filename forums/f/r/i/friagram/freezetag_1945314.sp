// ducking trigger fix disrupting entity flags
// entities in timers/global variables now use entity references
// timers now use userid
// removal of sloppy isvalidclient everywhere =/

/* There are a lot of global variables, because it's neccessary to track players through multiple timers, don't be frightened */
...
new Float:g_fChargemeter[MAXPLAYERS+1];           // Track medigun charge meters
...
static const Float:g_fNovelocity[3] = {0.0,0.0,0.0};      // zero out velocity for unfrozen bots so they don't crater or fly around
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
DropFlag(client)                                                            // From tf2betheghost
{                                                                           // Perhaps not needed, since they die, but just in case.
...
{                                                                             // Players can switch weapons to restore movement briefly, oh well.
        if (g_activeround != 1)
        {
                return Plugin_Stop;
        }
	...
	if (client)                                                                                       // Enabled here, activeround comes later  (checking if userid is valid should be enough...)
	{
			...
			g_futureclass[client] = TFClass_Unknown;
                ...
                if (g_bRespawned[client])                                                                 // They got moved back to spawn by trigger or stuck command and are still frozen
                {
                        ...
                        {
                                colorizeWearables(client);                                                // Transparent wearables
                        }
                }
                if (!g_bFrozen[client])
                {
		         	...
		         	new medigunidx = (weapon > MaxClients && IsValidEntity(weapon) ? GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex") : -1);
      	 		 	if (medigunidx != 411 && medigunidx != 998)                               // If it's not a quick fix or vaccinator, replace it with gimp kritz
					 	...
					 	weapon = TF2Items_GiveNamedItem(client, hWeapon);
                                ...
                                if (g_fChargemeter[client] != 0.0)                                        // Fix their medigun charge, if they got respawned from being stuck inside something
	                        {
	                                ...
	                                {
	                                        decl String:s[64];
	                                        if (GetEntityClassname(medigun, s, sizeof(s)) && !strcmp(s,"tf_weapon_medigun"))
	                                        {
...
public TF2_OnConditionAdded(client, TFCond:condition)                            // Damage over time/Stun conditions will cause them to move, and must be dealt with
{                                                                                // These may have to be converted to 0.0 timers, but this catches 99% of them
       if (!g_bFrozen[client])
       {
              return;
       }
...
freezeClient(client)
{
        ...
        if (!IsFakeClient(client))                                                              // #@&# bots, they'll be stuck but it's the only way since they don't obey movement nerfs
        {
                ...
                WritePackCell(stoppack, GetClientUserId(client));
                WritePackCell(stoppack, 7);
        }

        if (g_bUseModel)
        {
                new time = RoundToNearest(GetEngineTime());                                     // Gotta have a UID... This will do, we don't need a float.
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
public Action:Timer_MoveIce(Handle:timer, any:userid)
{                                                                                // It is possible to parent a model to a light or something, and then parent the light on the next frame, but it's hacky
        new client = GetClientOfUserId(userid);
        if (!client || !IsClientInGame(client) ||
            !g_bFrozen[client] ||                                                // This moves the model with the player in a less-hacky but more predictable manner... and is cheaper than ongameframe.
...
stock createIceBlock(client, time)
{
	...
	if ( ice1 != -1 )
	{
		...
		AcceptEntityInput(ice1, "TurnOn");
                ...
                CreateTimer(0.1, Timer_MoveIce, GetClientUserId(client), TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);        // this timer is too frequent, but it looks horrible if it's lower
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
resolveFreezeSpawnOverlaps(client)                           // This is horribly sloppy, ugly function, and should be put out of its misery.
{                                                            // Most of the time it will resolve quickly, unless they are on some rubble/weapon pile, then it will elevate them, else > respawn
	new clientteam = GetClientTeam(client);
        ...
        for (new i=1; i<=MaxClients; i++)
        {
                if (!g_bFrozen[i] && IsClientInGame(i) && IsPlayerAlive(i) && (clientteam != GetClientTeam(i)) )                 // iterate over nonfrozen clients on opposite team
		{
...
public bool:TraceFilterNotSelf(entity, contentsMask, any:client)
{
        ...
        else if (IsValidEntity(entity))
        {
                ...
                GetEntityClassname(entity, ObjClassName, sizeof(ObjClassName));

...
public void OnTakeDamagePost(int victim, int attacker, int inflictor, float damage, int damagetype)
{
    ...
    if( inflictor > 0 && IsValidEntity(inflictor) && attacker > 0)
    {
        ...
        datapack.WriteCell(EntIndexToEntRef(victim));
        ...
        else if(tdamage > MaxDamageHandle * 5 && !g_bExploded[victim])
        {
            ...
            CreateTimer(GetRandomInt(0, 100) * 0.0001, Timer_ExplodeCar, datapack, TIMER_FLAG_NO_MAPCHANGE);
        }
    }
}

Action Timer_ExplodeCar(Handle timer, DataPack datapack)
{
    if(g_bDisabled) return Plugin_Continue;

    datapack.Reset(false);

    int car = EntRefToEntIndex(datapack.ReadCell()); // victim
...
void CreateExplosion(float carPos[3], int attacker)
{
    ...
    */
    ...
    if( CheckIfEntityMax(exHurt) )
    {
        ...
        while( (i = FindEntityByClassname(i, "infected")) != INVALID_ENT_REFERENCE )
        {
            ...
            // SDKHooks_TakeDamage(i, attacker, attacker, 1.0, DMG_BURN, -1, NULL_VECTOR, carPos);

            DispatchKeyValue(exHurt, "DamageTarget", "explosive_cars");
            DispatchSpawn(exHurt);
            DispatchKeyValue(exHurt, "Damage", "1");
            DispatchKeyValue(exHurt, "DamageType", "8");
            DispatchKeyValue(i, "targetname", "explosive_cars");
            DispatchKeyValue(exHurt, "DamageRadius", sRadius);
            DispatchKeyValue(exHurt, "DamageDelay", sInterval);
            TeleportEntity(exHurt, carPos, NULL_VECTOR, NULL_VECTOR);

            // Both has the same result
            AcceptEntityInput(exHurt, "Hurt", attacker, attacker);
            // AcceptEntityInput(exHurt, "Hurt", attacker);

            DispatchKeyValue(i, "targetname", "");
            AcceptEntityInput(exHurt, "TurnOn");
            PrintToChatAll("\x03killed:\x05%d", i);
        }
        CreateTimer(g_cvarTrace.FloatValue, timerTurnOff, EntIndexToEntRef(exHurt), TIMER_FLAG_NO_MAPCHANGE);
    }

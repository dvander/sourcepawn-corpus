...
new Handle:cvar_onlyincfg = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	cvar_onlyincfg = CreateConVar("sm_adminlist_onlyincfg", "0", "only show people that appear in the .txt file");
...
public GetCVars()
{
	...
	only_cfg = GetConVarBool(cvar_onlyincfg);
...
Comando_Admins(client)
{
	...
	if (count)
	{
		...
		for (new i = 0; i < count; i++)
		{
			...
			else if (!only_cfg) CPrintToChatEx(client,Adms[i],"{green}[ADMIN] {teamcolor}%N",Adms[i]);
		}

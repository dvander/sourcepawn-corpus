...
#define PLUGIN_VERSION "1.0.5 Edited by Franc1sco Franug"
...
{
	CreateConVar("sm_skinapp_version", PLUGIN_VERSION, "[SM] SkinApp Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
public OnMapStart()
{
	...
	char curmap[PLATFORM_MAX_PATH], ctfile[PLATFORM_MAX_PATH], tfile[PLATFORM_MAX_PATH];
	GetCurrentMap(curmap, sizeof(curmap));
		
	// Does current map string contains a "workshop" prefix at a start?
	if (strncmp(curmap, "workshop", 8) == 0)
	{
		BuildPath(Path_SM, ctfile, sizeof(ctfile), "configs/models_ct_%s.ini", curmap[19]);
		if (!FileExists(ctfile)) BuildPath(Path_SM, ctfile, sizeof(ctfile), "configs/models_ct.ini");
		
		BuildPath(Path_SM, tfile, sizeof(tfile), "configs/models_t_%s.ini", curmap[19]);
		if (!FileExists(tfile)) BuildPath(Path_SM, tfile, sizeof(tfile), "configs/models_t.ini");
	}
	else /* That's not a workshop map */
	{
		// Let's check that custom skin configuration file is exists for current map
		BuildPath(Path_SM, ctfile, sizeof(ctfile), "configs/models_ct_%s.ini", curmap);
		if (!FileExists(ctfile)) BuildPath(Path_SM, ctfile, sizeof(ctfile), "configs/models_ct.ini");
		
		BuildPath(Path_SM, tfile, sizeof(tfile), "configs/models_t_%s.ini", curmap);
		if (!FileExists(tfile)) BuildPath(Path_SM, tfile, sizeof(tfile), "configs/models_t.ini");
	}
	
	g_Models_Count_CT = LoadModels(g_Models_CT, ctfile);
	g_Models_Count_T  = LoadModels(g_Models_T,  tfile);
...
stock LoadModels(String:models[][], String:file[])
{

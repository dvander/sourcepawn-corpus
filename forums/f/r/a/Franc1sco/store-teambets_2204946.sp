...
#define PLUGIN_VERSION "2.6 store version"

public Plugin:myinfo = 
{
	name = "Store Team Bets",
	author = "GrimReaper - Original by ferret and edited for store by Franc1sco franug",
...
public Action:Command_Say(client, args)
{
 	...
 	if (strcmp(szParts[0],"bet",false) == 0)
	{
		...
		WritePackString(pack, szParts[0]);
		WritePackString(pack, szParts[1]);
		WritePackString(pack, szParts[2]);
...
{
	decl String:szParts[3][16];
	ResetPack(pack);
	new client = ReadPackCell(pack);
	ReadPackString(pack, szParts[0], 16);
	ReadPackString(pack, szParts[1], 16);
	ReadPackString(pack, szParts[2], 16);
	...
	if (strcmp(szParts[1],"ct",false) == 0 || strcmp(szParts[1],"t", false) == 0)
	{
		...
		Store_GiveCredits(GetSteamAccountID(client), -iAmount);

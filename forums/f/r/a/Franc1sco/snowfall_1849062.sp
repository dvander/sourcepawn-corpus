...
new bool:can_view[MAXPLAYERS+1] = {true, ...};
...
public OnMapStart()
{
	CreateConVar("sm_snowfall_version", PLUGIN_VERSION, "version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
        ...
        RegConsoleCmd("sm_snow", Snow_Command);
...
public Action:Snow_Command(client,args)
{
	if(can_view[client])
	{
		can_view[client] = false;
		PrintToChat(client,"Now you can NOT view the snowfall");
	}
	else
	{
		can_view[client] = true;
		PrintToChat(client,"Now you can view the snowfall");
	}
}
...
{	
	...
	SDKHook(ent, SDKHook_SetTransmit, OnTransmitEntity);
}

...
{
	CreateConVar("sm_cssmolotov_version", "v2.0", _, FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
public Action:Comprar(client, argc)
{
		...
		else
		{
			...
			{
				PrintToChat(client, "\x04[SM_CSSMolotov]\x05 You don't have $%i to buy a molotov", preciomolotov2);
			}
			else
			{
				if(preciomolotov2 != 0)
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype)
{
	...
	if (!strcmp(arma, "env_fire", false))
	{
		...
		new Float:tiempo = GetConVarFloat(cvar_ignite_time);

		IgniteEntity(victim, tiempo);
	}
	return Plugin_Continue;

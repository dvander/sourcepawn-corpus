	...
	for (new client = 1; client <= MaxClients; client++)
	{
		if (IsClientInGame(client) && !IsFakeClient(client) && IsClientAuthorized(client))
		{
			...
			GetClientAuthString(client, steamId, sizeof(steamId));
        		...
        		ReplaceString(nombre, sizeof(nombre), "'", "");
        		ReplaceString(nombre, sizeof(nombre), "<", "");
        		ReplaceString(nombre, sizeof(nombre), "\"", "");
			...
			Format(query, sizeof(query), "INSERT OR REPLACE INTO saverank_time2 VALUES ('%s', '%s',%d,%d,%d,%d);", steamId, nombre,save1,save2,save3,save4);
...
public InitDB()
{
	...
	g_hDB = SQLite_UseDatabase("saverank2", error, sizeof(error));
...
public SQLTopShow(Handle:owner, Handle:hndl, const String:error[], any:client){
		...
		if (SQL_HasResultSet(hndl))
		{
			while (SQL_FetchRow(hndl))
			{
				...
				PrintToChat(client, "TOP is empty!");
		}

			...
			for (new i=0;i<4;i++)
			{
				if (iSurvivorClient[i])
				{
					...
					if (IsPlayerAlive(iSurvivorClient[i]))
					{
						...
						Format(sTempString, sizeof(sTempString), "->   %dHP %s %s %s %s", sIdentifier3, sIdentifier4, sIdentifier2, sIncapState);
					}
					else
						sTempString = "->   Dead";
					DrawPanelText(hPanel, sTempString);
...
public GetSlotInfo(iClient, iSlot, String:Identifier[])
{

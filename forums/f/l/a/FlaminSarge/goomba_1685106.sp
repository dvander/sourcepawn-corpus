...
new Handle:g_Cvar_ToggleType = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_Cvar_ToggleType = CreateConVar("goomba_immun_toggletype", "0", "Changes to immunity apply on: 0-respawn only; 1-respawn+supply locker", 0, true, 0.0, true, 1.0);
	...
	g_Cookie_ClientPref = RegClientCookie("goomba_client_pref2", "", CookieAccess_Protected);
	...
	if(victim > 0 && victim <= MaxClients)
	{
		...
		if(StrEqual(edictName, "player"))
		{
			if(IsPlayerAlive(victim))
			{
				...
				GetClientCookie_Safe(client, g_Cookie_ClientPref, strCookieClient, sizeof(strCookieClient));

				decl String:strCookieVictim[16];
				GetClientCookie_Safe(victim, g_Cookie_ClientPref, strCookieVictim, sizeof(strCookieVictim));
...
{
	if (StrEqual(name, "post_inventory_application", false))
	{
		if (!GetConVarBool(g_Cvar_ToggleType)) return;
	}
	else
	{
		if (GetConVarBool(g_Cvar_ToggleType)) return;
	...
	GetClientCookie_Safe(client, g_Cookie_ClientPref, strCookie, sizeof(strCookie));
...
stock GetClientCookie_Safe(client, Handle:cookie, String:strCookie[], maxlen)
{
	if (client <= 0 || client > MaxClients || !IsClientInGame(client)) { strcopy(strCookie, maxlen, "off"); return; }
	if (!AreClientCookiesCached(client)) { strcopy(strCookie, maxlen, "off"); return; }

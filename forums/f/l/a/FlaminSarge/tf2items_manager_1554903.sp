...
#define PLUGIN_VERSION		"1.5.1"	//to match the current ext version
...
public OnPluginStart()
{
	...
	g_hCvarItemList = CreateConVar("tf2items_manager_file", "tf2items.weapons", "File used to determine weapon list");
	HookConVarChange(g_hCvarItemList, g_cvhookItemList);
...
public g_cvhookItemList(Handle:cvar, const String:oldValue[], const String:newValue[])
{
	ParseItems();
}
...
ParseItems()
{
	...
	DestroyItems();
	GetConVarString(g_hCvarItemList, strFile, sizeof(strFile));
	// Create key values object and parse file.
	Format(strPath, sizeof(strPath), "configs/tf2items/%s.txt", strFile);
	BuildPath(Path_SM, strBuffer, sizeof(strBuffer), strPath);
	if (!FileExists(strBuffer))
	{
		LogError("[TF2Items Manager] Could not find %s, using default item list", strBuffer);
		BuildPath(Path_SM, strBuffer, sizeof(strBuffer), "configs/tf2items/tf2items.weapons.txt");
	}

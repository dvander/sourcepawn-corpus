...
public OnPluginStart() {
	...
	HookConVarChange(g_arrCVars[USEHUD], UseHUDChanged);
	g_bUseHUD = GetConVarBool(g_arrCVars[USEHUD]);
...
public UseHUDChanged(Handle:hConVar, const String:szOldValue[], const String:szNewValue[]) {
	g_bUseHUD = GetConVarBool(hConVar);
}
/*
...
public Action:CheckAllTraces(Handle:hTimer) {
	...
	new bUseHUD = g_bCanUseHUD && g_bUseHUD;
	new Float:vecPos[3];
	...
	for (new client = 1; client <= MaxClients; client++) {
		...
		if (!GetPlayerEye(client, vecPos, TraceEntityFilter_OnlyWorld)) {
			if (flGameTime > flSprayTraceTime[client][0] + GetConVarFloat(g_arrCVars[HUDTIME]) - GetConVarFloat(g_arrCVars[REFRESHRATE])) {
				if (iSprayTarget[client] != -1 && g_hHUDMessage != INVALID_HANDLE) {	//wow, such repeated code
					ClearSyncHud(client, g_hHUDMessage);
				}
				iSprayTarget[client] = -1;
			}
			continue;
		...
		if (!CheckCommandAccess(client, "spraytrace_hud_access_full", ADMFLAG_GENERIC, true)) {
			bFullAccess = false;
			if (!CheckCommandAccess(client, "spraytrace_hud_access", 0, true)) {
				continue;
			}
		}
		new bool:bTraceAdmins = CheckCommandAccess(client, "spraytrace_hud_can_trace_admins", 0, true);
		new target = -1;
		...
		new bool:bTargetIsAdmin = CheckCommandAccess(target, "spraytrace_hud_is_admin", ADMFLAG_GENERIC, true);
		...
		if (bFullAccess)
			FormatEx(strMessage, sizeof(strMessage), "%T", "Sprayed", client, g_arrSprayName[target], g_arrSprayID[target]);
		else
			FormatEx(strMessage, sizeof(strMessage), "%T", "Sprayed Name", client, g_arrSprayName[target]);
		if (bUseHUD) {
			if (!bHudParamsSet) {
				bHudParamsSet = true;
			...
			if (flGameTime > flSprayTraceTime[client][1] + 14.5 || target != iSprayTarget[client]) {
				ShowSyncHudText(client, g_hHUDMessage, strMessage);
			...
			PrintHintText(client, strMessage);
		}
	}
}
/*
...
public Action:AdminSpray(client, args) {
	if (!IsValidClient(client)) {
		if (client == 0) ReplyToCommand(client, "[Spray Trace] Command is in-game only.");
		return Plugin_Handled;
	}
	...
	if (!GoSpray(client, target)) {
		ReplyToCommand(client, "%s[Spray Trace] %T", GetCmdReplySource() == SM_REPLY_TO_CHAT ? "\x04" : "", "Cannot Spray", client);
	}
	else {
		ReplyToCommand(client, "%s[Spray Trace] %T", GetCmdReplySource() == SM_REPLY_TO_CHAT ? "\x04" : "", "Admin Sprayed", client, client, target);
		LogAction(client, -1, "[Spray Trace] %T", "Admin Sprayed", LANG_SERVER, client, target);
	}
...
//		return false;
	if (!GetPlayerEye(client, vecEndPos, TraceEntityFilter_OnlyWorld)) {
		return false;
	}
...
stock bool:GetPlayerEye(client, Float:vecPos[3], TraceEntityFilter:filter) {
	if (!IsValidClient(client))
		return false;
	...
	TR_TraceRayFilter(vecOrigin, vecAngles, MASK_SHOT, RayType_Infinite, filter);

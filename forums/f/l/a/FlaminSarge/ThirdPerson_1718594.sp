...
public Action:OnPlayerSpawned(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_bThirdPersonEnabled[GetClientOfUserId(userid)])
		CreateTimer(0.2, SetViewOnSpawn, userid, TIMER_FLAG_NO_MAPCHANGE);
}

public Action:SetViewOnSpawn(Handle:timer, any:userid)
{
	new client = GetClientOfUserId(userid);
	if (client > 0 && client <= MaxClients && IsClientInGame(client) && IsPlayerAlive(client))	//Checked g_bThirdPersonEnabled in hook callback, dont need to do it here~
	{
		SetVariantInt(1);

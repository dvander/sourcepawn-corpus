...
public Action:GiveItem (client, args)
{
	if (args < 10)
	{
		ReplyToCommand(client, "Usage: sm_gi <client> <item index> <slot> <level> <quality> <preserve attribs 1/0> <hat 1/0> <force generation 1/0> <tf_weapon_classname> <\"attribute1\" \"attribute2\" etc>  !!ALL VALUES ARE REQUIRED!!");
	...
	decl String:buffer7[64];
	decl String:buffer8[128];
	...
	new ForceGen = StringToInt(buffer7);
	if (ForceGen == 1)
	{
		Flags |= FORCE_GENERATION;
		//PrintToChatAll("FORCE_GENERATION = true");
	}

	//get attrib count
	new NumAttribs = args - 9;
	for (new i = 0; i < NumAttribs; i ++)
	{
		//Get each attrib arg
		GetCmdArg(i+10, buffer8, sizeof(buffer8));
		ExplodeString(buffer8, ";", explodeStr, 2, 64);
		
		//PrintToChatAll("ATTRIBUTE %i from arg %i = %s", i, i+9, buffer8);
		//split
	...
	//set classname
	GetCmdArg(9, strclassname, sizeof(strclassname));
	...
	if (derphat != 1) //Not a hat, apply now
	{
		...
		if (GetConVarBool(hDebug))
		{
			if (client != 0)
			{
				PrintToChat(client, "--------\n Giving item with index %i to %s in Slot %i \n Level: %i  - Quality: %i - Preserve Attributes: %i - ForceGen: %i \n Total Attributes: %i \n --------", itemIndex, target_name, slot+1, level, quality, Preserve, ForceGen, NumAttribs);
			}
			...
			for (new i = 0; i < NumAttribs; i ++)
			{
				...
				if (client != 0)
				{
					PrintToChat(client, "Attribute ID Number: %i from cmd arg %i = \"%s\"", i, i+10, buffer8);
				}
				ReplyToCommand(client, "Attribute ID Number: %i from cmd arg %i = \"%s\"", i, i+10, buffer8);
			}
			if (client != 0)
			{
				PrintToChat(client, "--------");
	...
	else if (derphat == 1) //Cache to apply later
	{
		for (new i = 0; i < target_count; i ++)
		{
			if (GetConVarBool(hDebug))
			{
				if	(client != 0)
				{
					PrintToChat(client, "--------\n Giving item with index %i to %s \n Level: %i  - Quality: %i - Preserve Attributes: %i - ForceGen: %i \n Total Attributes: %i \n --------", itemIndex, target_name, level, quality, Preserve, ForceGen, NumAttribs);
				}
				ReplyToCommand(client, "--------\n Giving item with index %i to %s \n Level: %i  - Quality: %i - Preserve Attributes: %i - ForceGen: %i \n Total Attributes: %i \n --------", itemIndex, target_name, level, quality, Preserve, ForceGen, NumAttribs);
				for (new ii = 0; ii < NumAttribs; ii ++)
				{
					GetCmdArg(ii+10, buffer8, sizeof(buffer8));
					ExplodeString(buffer8, ";", explodeStr, 2, 64);
					if	(client != 0)
					{
						PrintToChat(client, "Attribute ID Number: %i from cmd arg %i = \"%s\"", ii, ii+10, buffer8);
					}
					ReplyToCommand(client, "Attribute ID Number: %i from cmd arg %i = \"%s\"", ii, ii+10, buffer8);
				}
				ReplyToCommand(client, "--------");
				if	(client != 0)
				{

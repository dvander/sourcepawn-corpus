...
#define PLUGIN_VERSION "1.39beta"
...
public OnPluginStart()
{
	...
	cvarVersion = CreateConVar("hale_version", haleversiontitles[maxversion], "VS Saxton Hale Version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_DONTRECORD);
	...
	cvarForceSpecToHale = CreateConVar("hale_spec_force_boss", "0", "1- if a spectator is up next, will force them to Hale + spectators will gain queue points, else spectators are ignored by plugin", FCVAR_PLUGIN, true, 0.0);
...
stock bool:CheckNextSpecial()
{
	...
	while (Incoming == VSHSpecial_None || (Special && Special == Incoming))
	{
		...
		else
		{
			switch (Incoming)
			{
				case 12, 13: Incoming = VSHSpecial_Vagineer;
				case 11, 10: Incoming = VSHSpecial_HHH;
				case 14, 15: Incoming = VSHSpecial_CBS;
...
public SkipHalePanelNotify(client)
{
	...
	Format(s, sizeof(s), "%t", "vsh_to0_near");
...
//			new Handle:hItemOverride = PrepareItemHandle(hItem, _, _, "116 ; 4.0", true);
		...
		case 305:
		{
			new Handle:hItemOverride = PrepareItemHandle(hItem, _, _, "17 ; 0.1 ; 2 ; 2.0"); // ; 266 ; 1.0");
			if (hItemOverride != INVALID_HANDLE)
			{
				hItem = hItemOverride;
				return Plugin_Changed;
...
//			}
			...
			switch (index)
			{
				case 305, 14, 56, 201, 230, 402, 16, 203, 58, 526, 664: addthecrit = true;
	...
	else
	{
		...
		if ((ang[0] < -45.0) && (HaleCharge > 1))
		{
			...
			else if (Special != VSHSpecial_HHH)
			{
				...
				HaleCharge=-120;
				if (Special == VSHSpecial_Hale || Special == VSHSpecial_Vagineer || Special == VSHSpecial_CBS)
				{
					decl String:s[PLATFORM_MAX_PATH];
	...
	if (HaleRage/RageDMG >= 1)
	{
		...
		switch (Special)
		{
			...
			case VSHSpecial_CBS:
			{
				...
				EmitSoundToAll(s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, pos, NULL_VECTOR, false, 0.0);
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
				SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SpawnWeapon(client, "tf_weapon_compound_bow", 56, 100, 5, "2 ; 2.1 ; 6 ; 0.5 ; 37 ; 0.0"));
...
/*				if ( IsValidEdict( weapon ) && GetEdictClassname( weapon, wepclassname, sizeof( wepclassname ) ) && strcmp( wepclassname, "tf_weapon_stickbomb", false ) == 0 )
				...
				if (weapon != 4095 && IsValidEdict(weapon) && weapon == GetPlayerWeaponSlot(attacker, TFWeaponSlot_Melee) && damage > 1000.0)	//lousy way of checking backstabs
				{
					decl String:wepclassname[32];
					if (GetEdictClassname(weapon, wepclassname, sizeof(wepclassname)) && strcmp(wepclassname, "tf_weapon_knife", false) == 0)	//more robust knife check
					{
						...
						FireEvent(stabevent);
						if (TF2_GetPlayerClass(attacker) == TFClass_Spy && (wepindex == 225 || wepindex == 574))
						{

...
public OnPluginStart() {
	...
	hCvarDR = CreateConVar("sm_pregamemayhem_deadringer", "1", "Pregame Mayhem - Respawn deadringers", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast) {
	if (GetConVarBool(hCvarDR) || !(GetEventInt(event, "death_flags") & TF_DEATHFLAG_DEADRINGER)) CreateTimer(0.1, Timer_Respawn, GetEventInt(event, "userid"));
...
public TF2_OnWaitingForPlayersStart() {
	if (FindArena()) return;
	if (GameRules_GetRoundState() == RoundState_BetweenRounds) return;
	Instaspawn = true;

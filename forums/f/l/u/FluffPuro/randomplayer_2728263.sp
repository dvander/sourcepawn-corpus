...
{ 
	...
	for(int A = 1; A <= AllPlayers; A++)
	{
		if(IsClientConnected(A) && IsClientInGame(A))
		{
			...
			ShowHudText(A, 0, "Choosing a random player...");
		}
	}
	EmitSoundToAll(WILLSPIN);
	CreateTimer(5.0, Command_SpinWheel);
	
	CPrintToChatAll("{legendary}[Wheel of Fate]{darkturquoise} Choosing a random player...");
}
...
public Action Command_GetRandom(Handle timer)
{
	...
	for(int A = 1; A <= AllPlayers; A++)
	{
		...
		{
			SetHudTextParams(-1.0, 0.3, 25.0, 255, 255, 255, 255, 0, 25.0, 4.0, 4.0);
			ShowHudText(A, 0, "Fate has chosen %N!",iRandomClient);
		}
	}

	CPrintToChatAll("{legendary}[Wheel of Fate]{darkturquoise} Fate has chosen{olive} %N!",iRandomClient);
	PrintToServer("Fate has chosen %N!",iRandomClient);
...
{
	PrintToServer("A 20 second countdown was started for the randomly chosen player.");	
...
{
	CPrintToChat(client, "{legendary}[Wheel of Fate]{darkturquoise} Fate has chosen{olive} YOU!");

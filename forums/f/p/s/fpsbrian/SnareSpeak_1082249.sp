...
public OnPluginStart()
{
	...
	hEndRoundAllTalk = CreateConVar("snarespeak_endofround_alltalk","1","Turns on all talk when the round ends",FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	hSpitterTime = CreateConVar("snarespeak_spittertime","10","Time in seconds the spitter channel is left open. Also retriggers after these many seconds if survivor is still in spit.",FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
public Event_PlayerGrabbed(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(!GetConVarBool(hAllTalk) && !IsFakeClient(victimClient) && !IsFakeClient(attackerClient))
	{
		...
		if(GetConVarBool(hPrintChannel))
		{
			...
			if(GetConVarBool(hTwoWayCommunication))
				PrintToChat(victimClient,"\x04Voice channel\x03 Enabled\x04 between you and\x03 %s", attackerName);
	...
	if(victimClient != 0)
	{
		if(IsClientInGame(victimClient) && IsClientInGame(userClient))
		{
			...
			if(GetConVarBool(hPrintChannel))
			{
				...
				if(GetConVarBool(hTwoWayCommunication))
					PrintToChat(victimClient,"\x04Voice channel\x03 Disabled\x04 between you and\x03 %s", userName);
...
public EndBoomerSpitterEffect(int:type, victimClient)
{
	...
	if((victimClient != 0) && (attackerClient != 0) && IsClientInGame(attackerClient) && IsClientInGame(victimClient))
	{
		...
		if(GetConVarBool(hPrintChannel))
		{
			...
			GetClientName(attackerClient,attackerName,sizeof(attackerName));
			if(GetConVarBool(hTwoWayCommunication))
				PrintToChat(victimClient,"\x04Voice channel\x03 Disbled\x04 between you and\x03 %s", attackerName);

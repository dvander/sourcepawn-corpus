...
public OnPluginStart()
{
	...
	HookConVarChange((hRandom = CreateConVar("sm_kdrwl_bomb", "0",
	"Contain the KRD bomb/defuse points?\n0 = No\n1 = Yes")), OnBombCountChanged);
	BombCount = GetConVarInt(hRandom);
...
*/	
	...
	// Let's check if we remove bomb/defuse points and any botkills, if there are any
	if (BombCount == 0) // kdr is without bomb/defuse points
	{
		new Float:bonus_points = float((DefuseBombPoints[client] + BombExplodePoints[client] + BotKills[client]));
		frags -= bonus_points;
	}
	else if (BombCount == 1) // kdr is with bomb/defuse points
	{
		new Float:bonus_points = float(BotKills[client]);
		frags -= bonus_points;
	}
...
public OnBombCountChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	BombCount = GetConVarInt(cvar);
}

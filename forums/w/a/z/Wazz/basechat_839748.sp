			...
			for(new i=1; i<=MaxClients; i++) {
				if(IsClientInGame(i)) {
					if(IsPlayerAlive(client)) {
						PrintToChat(i, "\x01[ADMIN]\x04[%N]: \x03%s\x01", client, message);
					} else {
						PrintToChat(i, "*DEAD* \x01[ADMIN]\x04[%N]: \x03%s\x01", client, message);
					}
				}
			}
		} else {
			for(new i=1; 1<=MaxClients; i++) {
				if(IsClientInGame(i)) {
					if(IsPlayerAlive(client)) {
						PrintToChat(i, "\x04[%N]: \x03%s\x01", client, message);
					} else {
						if(!IsPlayerAlive(i)) {
							PrintToChat(i, "*DEAD* \x04[%N]: \x03%s\x01", client, message);
						}
					}
				}
			}

...
public OnEntityCreated(entity, const String:classname[])
{
	...
	// Checking to see if its an entity with an Item Definition Index
	if (FindSendPropOffs(netclassname, "m_iItemDefinitionIndex") == -1)
		return;
	
	new defIdx = GetEntProp(entity, Prop_Send, "m_iItemDefinitionIndex");
	new idx = -1;
	...
	// The remaining code must be done 0.1 seconds later so that the entity is fully spawned
	new Handle:dp;
	CreateDataTimer(0.1, processEdict, dp, TIMER_FLAG_NO_MAPCHANGE);
	WritePackCell(dp, entity);
	WritePackCell(dp, idx);
	
	return;
}

public Action:processEdict(Handle:timer, Handle:dp)
{	
	ResetPack(dp);
	new edict = ReadPackCell(dp);
	new idx = ReadPackCell(dp);	
	
	// Check the entity is still there

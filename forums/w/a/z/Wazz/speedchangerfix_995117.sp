...
public Action:Timer_Speed(Handle:timer, any:client)
{
	...
	if(GetConVarInt(adminOnly) == 0)
	{
		switch (playerClass)
		{
			case (TFClass_Scout) : SetEntPropFloat(client, Prop_Send, "m_flMaxspeed", GetConVarFloat(scout));
			case (TFClass_Soldier) : SetEntPropFloat(client, Prop_Send, "m_flMaxspeed", GetConVarFloat(soldier));
			case (TFClass_Pyro) : SetEntPropFloat(client, Prop_Send, "m_flMaxspeed", GetConVarFloat(pyro));
			case (TFClass_DemoMan) : SetEntPropFloat(client, Prop_Send, "m_flMaxspeed", GetConVarFloat(demoman));
			case (TFClass_Heavy) : SetEntPropFloat(client, Prop_Send, "m_flMaxspeed", GetConVarFloat(heavy));
			case (TFClass_Engineer) : SetEntPropFloat(client, Prop_Send, "m_flMaxspeed", GetConVarFloat(engineer));
			case (TFClass_Medic) : SetEntPropFloat(client, Prop_Send, "m_flMaxspeed", GetConVarFloat(medic));
			case (TFClass_Sniper) : SetEntPropFloat(client, Prop_Send, "m_flMaxspeed", GetConVarFloat(sniper));
			case (TFClass_Spy) : SetEntPropFloat(client, Prop_Send, "m_flMaxspeed", GetConVarFloat(spy));
		}
	}
	else
	{

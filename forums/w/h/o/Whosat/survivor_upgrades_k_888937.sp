...
*
* Version k3
* 		- Added option to give the survivor who cr0wned a witch upgrade(s).
* 			(surup_upgrades_at_witch_cr0wned [1])
* 			(Everytime he cr0wns, not only upon getting achievement)
* 			Please delete old survivor_upgrades_k.cfg
* 		- Swapped colours for /upghelp 2 for better highlights.
* 		- Fixed primary witch killer not getting upgrade(s)
* 		- Fixed survivor_upgrades sometimes not turning on a few stages into a mission, thus causing ALOT of problems
* 			(Fixed a whole lot of problems)
*/
...
new Handle:UpgradesAtWitchCr0wned = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	UpgradesAtWitchCr0wned = CreateConVar("surup_upgrades_at_witch_cr0wned", "1", "How many random upgrades to give the survivor who cr0wned (headshot-killed) a witch", FCVAR_PLUGIN, true, 0.0, true, 15.0);
...
public ActivateHooks()
{
	if(!bHooked)
	{
		...
		HookEvent("award_earned", Event_Award);	// For cr0wned
...
public Action:ActivateSU(Handle:hTimer)
{
	if(g_InvalidGameMode) {
		...
		bIgniteTank = GetConVarBool(IgniteTank);
...
public Action:Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(!t_MissionLost) {
		...
		// Reason: ReGiveClientUpgrades will remove all then re-give upgrades.
	} else {
		// MISSION LOST! =(
...
public Action:Event_Award(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetEventInt(event, "award") == 19) {	// 19 = cr0wned
		if(GetConVarInt(UpgradesAtWitchCr0wned)) {
			if(GetConVarInt(Verbosity)>2)
			{
				PrintToChatAll("%s The cr0wner also gets: ", PLUGIN_TAG);
			}
			else if (GetConVarInt(Verbosity)>1)
			{
				PrintToChat(GetEventInt(event, "userid"), "%s For cr0wning the witch, you get:", PLUGIN_TAG);
			}
			GiveClientUpgrades(GetEventInt(event, "userid"), GetConVarInt(UpgradesAtWitchCr0wned));
		}
	}
}
...
{
	// Reason: ReGiveClientUpgrades will remove all and then give only applicable.
	//SetConVarInt(FindConVar("survivor_upgrades"), 0);
...
public Action:UserHelp(client, args)
{
	...
	if(type == 1) {
		...
		PrintToChat(client, "\x05| \x01Mission Start:\x04%d", GetConVarInt(UpgradesAtSpawn));
		PrintToChat(client, "\x05| \x01Tank Spawned:\x04%d, \x01Tank Killed:\x04%d, \x01Primary Tank Killer:\x04%d", GetConVarInt(UpgradesAtTankSpawn), GetConVarInt(UpgradesAtTankKillAll), GetConVarInt(UpgradesAtTankKillKiller));
		PrintToChat(client, "\x05| \x01Witch Killed:\x04%d, \x01Witch Killer: \x04%d \x01Witch Cr0wner: \x04%d", GetConVarInt(UpgradesAtWitchKillAll), GetConVarInt(UpgradesAtWitchKillKiller), GetConVarInt(UpgradesAtWitchCr0wned));
	}

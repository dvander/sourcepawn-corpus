...
{
	name = "SEOwned",
	author = "Aderic (edit by Maximo)",
	description = "Bans clients that connect to the server and spam SEOwned text.",
...
{
	CVAR_BanTime = 				CreateConVar("sm_seoban_time",					"0", 					"Time (in minutes) to ban the user, 0 means permanent ban.", FCVAR_NONE);
	CVAR_BanReason = 			CreateConVar("sm_seoban_reason",				"Aimbot Autoban",  		"Ban message to be displayed to the user and recorded.", FCVAR_NONE);
	CVAR_Detection = 			CreateConVar("sm_seoban_detection",			"1",  					"If 0, disabled. If 1, strict detection. If 2, detection if URL is mentioned. If 3, detection if the name is mentioned.", FCVAR_NONE, true, 0.0, true, 3.0);
	
	CVAR_Antispam_Detection =	CreateConVar("sm_seoban_antispam_detection",	"0",				"If 0, disabled. If 1, mentioning SEOwned twice at any point will result in a ban (with the first being a warning). If 2, action will be taken based on whether it is spammed.", FCVAR_NONE);
	CVAR_Antispam_Warning =		CreateConVar("sm_seoban_antispam_warning",		"Mentioning seo will result in an automatic ban. Please refrain from mentioning it.",		"The warning that appears on detection. If unset no warning will be given to users who gain a strike. This can be up to 192 characters.", FCVAR_NONE);
	CVAR_Antispam_Rate =		CreateConVar("sm_seoban_antispam_rate",		"3",				"Seconds to sample for: if strikes exceed the amount set by sm_lmaoban_antispam_strikes, the user will be banned.", FCVAR_NONE, true, 1.0);
	CVAR_Antispam_Strikes =		CreateConVar("sm_seoban_antispam_strikes",		"2",				"Maximum number of detections per sm_lmaoban_samplerate to ban.", FCVAR_NONE, true, 1.0);
	
	AutoExecConfig(true, "seoban");
}

public OnConfigsExecuted() {
	CVAR_Version = 		CreateConVar("sm_seoban_version",		PLUGIN_VERSION, 				"Current version of the plugin. Read Only", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
bool:IsClientSpamming(client) {
	...
	if (clientData[client][csStrikes] <= antispamStrikes) {
		if (antispamWarning[0] != 0)
			...
			PrintToChat(client, "SEOwned spam check (%i / %i). Now: %i        Next reset: %i.", clientData[client][csStrikes], antispamStrikes, now, clientData[client][csNextReset]);
		#endif
...
public Action:OnClientSayCommand(client, const String:command[], const String:sArgs[]) {
	...
	if (CheckCommandAccess(client, "seoban_exempt", ADMFLAG_BAN))
		return Plugin_Continue;
	...
	if (detectionMethod == 1) {
		...
		if (StrEqual(msg, 		"SEOwned - Go get yours now from unknowncheats.me!", 	false) == false && 
			StrContains(msg, 	"SEOwned - Premium cheat by spook953 and Lak3", 					false) == -1 	&& 
			StrEqual(msg, 		"SEOwned - Available for free @ unknowncheats.me!", 		false) == false && 
			StrEqual(msg, 		"unknowncheats.me", 				false) == false)
			
			return Plugin_Continue;
	}
	else if (detectionMethod == 2)  {
		// Stop execution here if the message does not contain this.
		if (StrContains(msg, "unknowncheats.me", false) == -1)
			return Plugin_Continue;
	}
	else {
		// or this.
		if (StrContains(msg, "SEOwned", false) == -1)
			return Plugin_Continue;
	}
	...
	#if defined PLUGIN_TESTMODE
		PrintToChat(client, "SEOwned triggered.");
	#endif
	...
	if (GetFeatureStatus(FeatureType_Native, "SBBanPlayer") == FeatureStatus_Available) {
		...
		LogMessage("Autobanned client %L for advertising SEOwned via SourceBans.", client);
	}
	else if (BanClient(client, banTime, BANFLAG_AUTHID, banReason, banReason, "LMAOBAN")) {
		LogMessage("Autobanned client %L for advertising SEOwned.", client);
	}

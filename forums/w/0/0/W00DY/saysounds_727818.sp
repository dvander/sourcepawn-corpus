/*
Say Sounds
 - by Hell Phoenix, Lambdacore, Naris
 - see http://forums.alliedmods.net/showthread.php?p=496226
...
Cvarlist (default value):
	...
	sm_saysounds_volume 1.0	Volume setting for Say Sounds (0.0 <= x <= 1.0),
						individual sound override through cfg file
	...
	"Sound Combinations"
		{
			"JoinSound" // Sound to play when a player Joins the server
			{
				...
				"volume" "0.5" // Override to cvar sm_saysounds_volume
			}
			"wazza"  // Word trigger
			{
...
{
	name = "Say Sounds",
	author = "Hell Phoenix, Lambdacore, Naris",
	description = "v1.19.1 modified by Woody based on v1.19",
	version = PLUGIN_VERSION,
	url = "http://forums.alliedmods.net/showthread.php?p=496226"
};
...
public OnPluginStart(){
	...
	cvarvolume = CreateConVar("sm_saysounds_volume","1.0","Volume setting for Say Sounds (0.0 <= x <= 1.0)",FCVAR_PLUGIN,true,0.0,true,1.0); // mod by Woody
...
Send_Sound(client, const String:filelocation[], const String:name[])
{
	...
	// begin mod by Woody
	new Float:volume = KvGetFloat(listfile, "volume",GetConVarFloat(cvarvolume));
	if ((FloatCompare(0.0, volume) == 1) || (FloatCompare(volume, 1.0) == 1)) {
		volume = GetConVarFloat(cvarvolume); // do this check because of possibly "stupid" values in cfg file
	}
	// end mod by Woody
	PrintToServer("Volume: %f", volume); // DEBUG
	new Handle:pack;
		...
		if (globalLastSound < thetime){
			...
			if (singleonly){
				if(SndOn[client] && IsClientInGame(client) && !IsFakeClient(client)){
					EmitSoundToClient(client, filelocation, _, _, _, _, volume); // mod by Woody
				}
			}else{
				...
				if (clientcount){
					EmitSound(clientlist, clientcount, filelocation, _, _, _, _, volume); // mod by Woody
				}

...
public OnClientAuthorized(client, const String:auth[])
{
	...
	if(auth[0] == 'B' || auth[9] == 'L' || Database1 == INVALID_HANDLE)
	{
	...
	SQL_TQuery(Database1, VerifyBan, Query, GetClientUserId(client), DBPrio_High);
}
...
public OnRebuildAdminCache(AdminCachePart:part)
{
	...
	if(Database1 == INVALID_HANDLE) {
		...
		GotDatabase(Database1,Database1,"",0);
	}
}
...
public Action:CommandBanIp(client, args)
{
	...
	SQL_TQuery(Database1, SelectBanIpCallback,  Query, dataPack, DBPrio_High);
	return Plugin_Handled;
}
...
public Action:CommandUnban(client, args)
{
	...
	SQL_TQuery(Database1, SelectUnbanCallback, query, dataPack);
	return Plugin_Handled;
}
...
public Action:CommandAddBan(client, args)
{
	...
	SQL_TQuery(Database1, SelectAddbanCallback, Query, dataPack, DBPrio_High);
	return Plugin_Handled;
}
...
public GotDatabase(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	SQL_TQuery(Database1, ErrorCheckCallback, query);
	...
	if(loadOverrides)
	{
		...
		SQL_TQuery(Database1, OverridesDone, query);
		loadOverrides = false;
	}
	...
	if(loadGroups && enableAdmins)
	{
		...
		curLoading++;
		SQL_TQuery(Database1,GroupsDone,query);
	...
	if(loadAdmins && enableAdmins)
	{
		...
		curLoading++;
		SQL_TQuery(Database1,AdminsDone,query);
...
public SelectBanIpCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	SQL_EscapeString(Database1, reason, banReason, sizeof(banReason));
	...
	SQL_TQuery(Database1, InsertBanIpCallback, Query, data, DBPrio_High);
}
...
public SelectUnbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	ReadPackString(data, arg, sizeof(arg));
	ReadPackString(data, adminAuth, sizeof(adminAuth));
	SQL_EscapeString(Database1, reason, unbanReason, sizeof(unbanReason));
	...
	if(hndl != INVALID_HANDLE && SQL_FetchRow(hndl))
	{
		...
		SQL_TQuery(Database1, InsertUnbanCallback, query, data);
	}
	return;
}
...
public SelectAddbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	SQL_TQuery(Database1, InsertAddbanCallback, Query, data, DBPrio_High);
}
...
public ProcessQueueCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	while(SQL_MoreRows(hndl))
	{
		...
		if(startTime + time * 60 > GetTime() || time == 0)
		{
			...
			ResetPack(authPack);
			SQL_TQuery(Database1, AddedFromSQLiteCallback, query, authPack);
		} else {
...
public VerifyBan(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	if(SQL_GetRowCount(hndl) > 0)
	{
		...
		SQL_EscapeString(Database1, clientName, Name, sizeof(Name));
		if( serverID == -1 )
		{
		...
		SQL_TQuery(Database1, ErrorCheckCallback, Query, client, DBPrio_High);
...
public GroupsDone(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	SQL_TQuery(Database1, LoadGroupsOverrides, query);
...
stock UTIL_InsertBan(time, const String:Name[], const String:Authid[], const String:Ip[], const String:Reason[], const String:AdminAuthid[], const String:AdminIp[], Handle:Pack)
{
	...
	decl String:banReason[256];
	decl String:Query[1024];
	SQL_EscapeString(Database1, Name, banName, sizeof(banName));
	SQL_EscapeString(Database1, Reason, banReason, sizeof(banReason));
	if( serverID == -1 )
	{
	...
	SQL_TQuery(Database1, VerifyInsert, Query, Pack, DBPrio_High);
}
...
stock InsertServerInfo()
{
	...
	if(AutoAdd != false)
	{
		...
		SQL_TQuery(Database1, ServerInfoCallback, query);
	}
}

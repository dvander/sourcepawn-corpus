...
{
	name = "Anti Rush",
	author = "Jannik 'Peace-Maker' Hartung, Wilczek",
...
{
	new Handle:hVersion = CreateConVar("sm_antirush_version", PLUGIN_VERSION, "Anti Rush Version", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_REPLICATED|FCVAR_DONTRECORD);
	...
	g_hCVEnable = CreateConVar("sm_antirush_enable", "1", "Enable Anti Rush?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_hCVNoRushTime = CreateConVar("sm_antirush_norushtime", "120", "Time in seconds rushing is not allowed after round start?", FCVAR_PLUGIN, true, 0.0, true, 530.0);
	g_hCVPunishment = CreateConVar("sm_antirush_punishment", "2", "How should the plugin handle players who enter defined zones by default? 1: Announce in chat, 2: Bounce back, 3: Slay, 4: Only knife, 5: Allow all weapons again", FCVAR_PLUGIN, true, 1.0, true, 5.0);
	g_hCVLastManRushing = CreateConVar("sm_antirush_lastmanrushing", "1", "Allow rushing if there is only one player left alive in a team.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_hCVOneOnOneRushing = CreateConVar("sm_antirush_oneononerushing", "0", "Allow rushing if there is only one player left alive in BOTH teams.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_hCVAdminImmunity = CreateConVar("sm_antirush_adminimmunity", "0", "Should admins be allowed to rush?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_hCVFenceColor = CreateConVar("sm_antirush_fencecolor", "255 0 0 255", "Which color should the fences have? RGBA: red green blue alpha", FCVAR_PLUGIN);
	g_hCVFenceColorWhenAllowed = CreateConVar("sm_antirush_fencecolor_allowed", "0 255 0 255", "Which color should the fences have when rushing is allowed? RGBA: red green blue alpha", FCVAR_PLUGIN);
	g_hCVShowZones = CreateConVar("sm_antirush_showzones", "1", "Show the zones either when rushing is not allowed?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_hCVShowFencesWhenAllowed = CreateConVar("sm_antirush_fence_allowed", "1", "Should fences be displayed when rushing is allowed?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_hCVWarmupEnable = CreateConVar("sm_antirush_enable_at_warmup", "0", "Should antirush be active during warmup", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
{
	if(!GetConVarBool(g_hCVEnable) || !GetConVarBool(g_hCVWarmupEnable) || (g_bIsRushAllowed && !g_hCVShowFencesWhenAllowed))
	{
		g_hShowFencesTimer = INVALID_HANDLE;
		return Plugin_Stop;
	}
...
public Action:Command_ChatHook(client, args)
{
	...
	else if(g_bPlayerRenamesZone[client])
	{
		...
		RemoveMapPath(inputMap, sMap, sizeof(sMap));

...
{
	name = "SM:Error Mail",
	author = "Jannik \"Peace-Maker\" Hartung, wyrda",
...
public OnMapEnd()
{
	...
	new FileType:iFileType, iLastChange, Handle:hFile, iLine;
	...
	while(ReadDirEntry(hDir, sFile, sizeof(sFile), iFileType))
	{
		...
		if(iLastChange < iLastMailSent)
			continue;
			
		
		hFile = OpenFile(g_sTouchFile, "r");
		ReadFileLine(hFile, sLine, sizeof(sLine));//skip line number
		ReadFileLine(hFile, g_sLastLogName, sizeof(g_sLastLogName));//get filename
		CloseHandle(hFile);
		
		iLine = 0;
		// This file is completely new. Send it all!
		if(strncmp(sFile, g_sLastLogName, strlen(sFile)) != 0)
		{
		...
		else if(iLastChange > iLastMailSent)
		{
			...
			PushArrayString(g_hErrorLog, "");
			Format(sLine, sizeof(sLine), "New errorlog in: %s", sFile);
			PushArrayString(g_hErrorLog, sLine);
			PushArrayString(g_hErrorLog, "");
		...
		strcopy(g_sLastLogName, sizeof(g_sLastLogName), sFile);
		g_iLastLine = iLine+1;		
	}
...
Finish(bool:bError=false)
{
	...
	if(!bError)
	{
		...
		WriteFileLine(hFile, "%s", g_sLastLogName);
		CloseHandle(hFile);
	}

...
/*
	...
	- Added "lethal_weapon" flags to specify which admin can use lethal shoot
...
public Action ReleaseTimer(Handle timer, any client)
{
	...
	if (l4d2_lw_useammo.IntValue)
	{
		...
		int iAmmoClip = GetEntProp(g_iCurWeapon[client], Prop_Send, "m_iClip1");
		int iAmmoLeft;

		/* Check weapon class and set how much ammo should set*/
		if (StrEqual(weapon, "weapon_hunting_rifle"))
		{
			iAmmoLeft = GetEntData(client, g_iAmmoOffset + HUNTING_RIFLE_OFFSET_IAMMO);
		}
		else if (StrEqual(weapon, "weapon_sniper_military") || StrEqual(weapon, "weapon_sniper_awp") || StrEqual(weapon, "weapon_sniper_scout"))
				{
					iAmmoLeft = GetEntData(client, g_iAmmoOffset + MILITARY_SNIPER_OFFSET_IAMMO);
				}

		iAmmoLeft -= iAmmoSpend;
		if (iAmmoLeft < 0) iAmmoLeft = 0;

		/* Check weapon class and modify reserve ammunition*/
		if (StrEqual(weapon, "weapon_hunting_rifle"))
		{
			SetEntData(client, g_iAmmoOffset + HUNTING_RIFLE_OFFSET_IAMMO, iAmmoLeft);
		}
		else if (StrEqual(weapon, "weapon_sniper_military") || StrEqual(weapon, "weapon_sniper_awp") || StrEqual(weapon, "weapon_sniper_scout"))
				{
					SetEntData(client, g_iAmmoOffset + MILITARY_SNIPER_OFFSET_IAMMO, iAmmoLeft);
				}

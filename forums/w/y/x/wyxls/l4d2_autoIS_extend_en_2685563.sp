...
public OnClientConnected(client)
{
	...
	CPrintToChatAll ("{green}[autoIS] {lightgreen}Player Joined, Current alive survivors: {olive}%i{lightgreen}, SI spawn amout: {olive}%i{lightgreen}", AliveSurvivors, SpawnSize);
	CPrintToChatAll ("{green}[autoIS] {lightgreen}Current SI spawn interval: {olive}[%.2f, %.2f]{lightgreen} Second", SpawnTimeMin, SpawnTimeMax);
}
...
public ConVarSpawnSIWithTank(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	if(!SpawnSIWithTank)
	{
		...
		CPrintToChatAll("{green}[autoIS] {lightgreen}stop spawning SI{olive} when{lightgreen} Tank exist");
	}
	else
	{
		StartTimers();
		//Notify all players that l4d2_ais_spawn_si_with_tank has changed to 1
		CPrintToChatAll("{green}[autoIS] {lightgreen}keep spawning SI{olive} when{lightgreen} Tank exist");
	}
}
...
public ConVarSpawnSizeOnPlayer(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	if(SpawnSizeOnPlayer)
	{
		...
		CPrintToChatAll("{green}[autoIS] {lightgreen}Enable adjust SI spawn amount base on the number of alive survivors");
	}
	else
	{
		//Notify all players that l4d2_ais_spawn_size_on_player has changed to 0
		CPrintToChatAll("{green}[autoIS] {lightgreen}Disable adjust SI spawn amount base on the number of alive survivors");
	}
}
public ConVarSpawnSizeAddAmount(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	CPrintToChatAll("{green}[autoIS] {lightgreen}Each alive survivor increase SI spawn amount: {olive}%i", SpawnSizeAddAmount);
	SurvivorCheck();
}
...
public ConVarSpawnTimeOnPlayer(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	if(SpawnTimeOnPlayer)
	{
		...
		CPrintToChatAll("{green}[autoIS] {lightgreen}Enable adjust SI spawn interval base on the number of alive survivors");
	}
	else
	{
		//Notify all players that l4d2_ais_time_on_player has changed to 0
		CPrintToChatAll("{green}[autoIS] {lightgreen}Disable adjust SI spawn interval base on the number of alive survivors");
	}
}
public ConVarSpawnTimeReduceAmount(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	CPrintToChatAll("{green}[autoIS] {lightgreen}Each alive survivor decrease SI spawn interval: {olive}%.2f{lightgreen} Second", SpawnTimeReduceAmount);
	SurvivorCheck();
}
...
{	
	...
	CPrintToChatAll ("{green}[autoIS] {lightgreen}DEBUG: Current alive survivors: {olive}%i{lightgreen}, SI spawn amout: {olive}%i{lightgreen}", AliveSurvivors, SpawnSize);
	CPrintToChatAll ("{green}[autoIS] {lightgreen}DEBUG: Current SI spawn interval: {olive}[%.2f, %.2f]{lightgreen} Second", SpawnTimeMin, SpawnTimeMax);
}
...
{	
	...
	if (!RoundStarted)
	{
		...
		SurvivorCheck();
		//Print the amount of alive player survivors , current SI spawn size and interval
		CPrintToChatAll ("{green}[autoIS] {lightgreen}Initialnizing, Current alive survivors: {olive}%i{lightgreen}, SI spawn amout: {olive}%i{lightgreen}", AliveSurvivors, SpawnSize);
		CPrintToChatAll ("{green}[autoIS] {lightgreen}Current SI spawn interval: {olive}[%.2f, %.2f]{lightgreen} Second", SpawnTimeMin, SpawnTimeMax);
		InitTimers();
	}
}
...
public evtTankSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(!SpawnSIWithTank)
	{
		if(HaveTank && IsTank(client, ZOMBIECLASS_TANK))
		{
			...
			CPrintToChatAll("{green}[autoIS] {lightgreen}Tank spawned, stop spawning SI");
		}
	}
}
...
public evtPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(IsValidClient(client))
	{
		if(SpawnSizeOnPlayer == 1)
		{
			...
			CPrintToChatAll ("{green}[autoIS] {lightgreen}Player Died, Current alive survivors: {olive}%i{lightgreen}, SI spawn amout: {olive}%i{lightgreen}", AliveSurvivors, SpawnSize);
		}
		if(SpawnTimeOnPlayer == 1)
		{
			////Notify all players that current SI spawn interval
	...
	if(!SpawnSIWithTank)
	{
		if(!HaveTank && IsTank(client, ZOMBIECLASS_TANK))
		{
			...
			CPrintToChatAll("{green}[autoIS] {lightgreen}All Tank died, keep spawning SI");
		}
	}
}
...
public evtSurvivorRescued(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(IsValidClient(client))
	{
		if(SpawnSizeOnPlayer == 1)
		{
			...
			CPrintToChatAll ("{green}[autoIS] {lightgreen}Player Rescued, Current alive survivors: {olive}%i{lightgreen}, SI spawn amout: {olive}%i{lightgreen}", AliveSurvivors, SpawnSize);
		}
		if(SpawnTimeOnPlayer == 1)
		{
			//Notify all players that current SI spawn interval

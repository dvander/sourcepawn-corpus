...
void GiveItem(int client, int target, int item, int slot, int type, int transferType)
{
	...
	if( transferType == METHOD_SWAP )
	{
		...
		RemovePlayerItem(target, ent_t); // do it firtst
		RemovePlayerItem(client, ent_c); // do it firtst
...
int CreateAndEquip(int client, int type)
{
	...
	{
//		EquipPlayerWeapon(client, entity); GivePlayerItem already does
		return entity;
	}

...
// Here we are creating SQL DB and KV DB
public InitDB()
{
	// SQL DB
	...
	SQL_FastQuery(g_hDB, "CREATE TABLE IF NOT EXISTS savescores_scores (steamid TEXT, frags SMALLINT, deaths SMALLINT, money SMALLINT, timestamp INTEGER);");
...
public OnAllPluginsLoaded()
{
	...
	{
		LogAction(-1, -1, "Note: This plugin supports updating via Plugin Autoupdater. Install it if you want to enable auto-update functionality.");
	}
}
...
public SyncDB()
{
	for (new client = 1; client <= MaxClients; client++)
	{
		if (IsClientInGame(client) && !IsFakeClient(client) && IsClientAuthorized(client))
		{
			...
			Format(query, sizeof(query), "INSERT INTO savescores_scores VALUES ('%s', %d, %d, %d, %d);", steamId, frags, deaths, cash, GetTime());
			SQL_FastQuery(g_hDB, query);
		}
	}
}
...
public OnClientDisconnect(client)
{
	...
	if (!client || isLAN || !save_scores || !IsClientInGame(client))
	{
		return;
	}
...
// Just insert player's score and all additional information into DB
...
public SetPlayerScore(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	...
	// If tracking time < time from DB and client didn't played after connection remove score

...
public Plugin:myinfo =
{
	...
	author = "shavit, Nican132, CptMoore, Lebson506th, and TheWreckingCrew6. >32 player patch by Zeraph.",
...
public OnPluginStart()
{
	...
	CreateConVar("sm_spray_version", PLUGIN_VERSION, "Super Spray Handler plugin version", FCVAR_NONE|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	CreateConVar("sm_ssh_version", PLUGIN_VERSION, "Super Spray Handler version", FCVAR_NONE|FCVAR_SPONLY|FCVAR_DONTRECORD|FCVAR_NOTIFY);

	//Spray Manager CVars
	g_arrCVars[ENABLED] = CreateConVar("sm_ssh_enabled", "1", "Enable \"Super Spray Handler\"?", FCVAR_NONE, true, 0.0, true, 1.0);
	g_arrCVars[ANTIOVERLAP] = CreateConVar("sm_ssh_overlap", "0", "Prevent spray-on-spray overlapping?\nIf enabled, specify an amount of units that another player spray's distance from the new spray needs to be it or more, recommended value is 75.", FCVAR_NONE, true, 0.0);
	g_arrCVars[AUTH] = CreateConVar("sm_ssh_auth", "1", "Which authentication identifiers should be seen in the HUD?\n- This is a \"math\" cvar, add the proper numbers for your likings. (Example: 1 + 4 = 5/Name + IP address)\n1 - Name\n2 - SteamID\n4 - IP address", FCVAR_NONE, true, 1.0);
...
public AllSprayBansCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	{
		decl String:auth[MAX_AUTHID_LENGTH];
		SQL_FetchString(hndl, 0, auth, MAX_AUTHID_LENGTH);

		decl String:auth2[MAX_AUTHID_LENGTH];
		FormatEx(auth2, MAX_AUTHID_LENGTH, auth);
		ReplaceString(auth2, MAX_AUTHID_LENGTH, "STEAM_", "", false);
...
public Action:Command_OfflineSprayban(client, args)
{
	...
	new String:auth[MAX_AUTHID_LENGTH];
	new String:authp1[MAX_AUTHID_LENGTH];
	GetCmdArg(1, authp1, MAX_AUTHID_LENGTH);
	new String:authp2[MAX_AUTHID_LENGTH];
	GetCmdArg(2, authp2, MAX_AUTHID_LENGTH);
	new String:authp3[MAX_AUTHID_LENGTH];
	GetCmdArg(3, authp3, MAX_AUTHID_LENGTH);
	new String:authp4[MAX_AUTHID_LENGTH];
	GetCmdArg(4, authp4, MAX_AUTHID_LENGTH);
	new String:authp5[MAX_AUTHID_LENGTH];
	GetCmdArg(5, authp5, MAX_AUTHID_LENGTH);
	Format(auth, MAX_AUTHID_LENGTH, "%s%s%s%s%s", authp1, authp2, authp3, authp4, authp5);
...
public Offlinebans_UnbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	new String:auth[MAX_AUTHID_LENGTH];
	ReadPackString(data, auth, MAX_AUTHID_LENGTH);
...
public any Native_IsBanned(Handle:handler, numParams)
{
	new client = GetNativeCell(1);
...
//As violently ripped from smlib:

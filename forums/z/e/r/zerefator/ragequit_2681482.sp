...
public OnPluginStart()
{
	...
	CreateConVar("sm_ragequit_version", PLUGIN_VERSION, "RageQuit Version", 0|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	g_hCvarTime = CreateConVar("sm_ragequit_max_delay", "10.0", "Time in seconds a player has to leave in to be considered a ragequitter", 0, true, 0.0, false, _);
...
{
	if (!IsFakeClient(client) && (g_fLastDeathTime[client] > 0.0 && (GetEngineTime() - g_fLastDeathTime[client]) > GetConVarFloat(g_hCvarTime)))
	{
		...
		PrintToChatAll("\x07 %t \x01 RAGEQUIT", client);
	}
}

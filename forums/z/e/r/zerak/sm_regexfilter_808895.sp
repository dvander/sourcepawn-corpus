...
public Action:Command_SayHandle(client, args)
{
	...
	new begin, end = GetArraySize(ChatREGEXList), RegexError:ret = REGEX_ERROR_NONE;
	new arr[2], Handle:CurrRegex, Handle:CurrInfo, any:val, bool:changed = false;
...
public HandleEnd(Handle:smc, bool:halted, bool:failed)
{
	...
	if (failed)
		SetFailState("REGEXFilter file failed to parse!");
}

 ...
 */
//11/04/22
// 	add StopFortnite calls for game restart events in nmrih
//	add cooldown disable all fornite during practice time to prevent client crash
...
ConVar g_cvPractice_Transtime;
bool   g_bIsPracticeTime;
...
public Plugin myinfo =
{
	...
	author = "Kodua, Franc1sco franug, TheBO$$, Foxhound, Rogue Garlicbread",
	description = "This plugin is for demonstration of some animations from Fortnite in NMRIH",
	version = "2020",
...
public void OnPluginStart() {
	...
	RegConsoleCmd("sm_stopdance", StopForniteCMD);
	...
	HookEvent("round_end", StopFornite, EventHookMode_Pre);
	HookEvent("game_restarting", StopFornite, EventHookMode_Pre);
	HookEvent("nmrih_reset_map", StopFornite, EventHookMode_Pre);
	HookEvent("nmrih_practice_ending", DisableFornitePracticeEnd, EventHookMode_Pre);


	g_CvRestartGame = FindConVar("mp_restartgame");
	HookConVarChange(g_CvRestartGame, CheckRestart);
	...
	**/
	...
	g_cvCooldown = CreateConVar("sm_emotes_cooldown", "4.0", "Cooldown for emotes in seconds. -1 or 0 = no cooldown.", CVAR_FLAGS);
	g_cvSoundVolume = CreateConVar("sm_emotes_soundvolume", "0.4", "Sound volume for the emotes.", CVAR_FLAGS);
	...
	g_cvSpeed = CreateConVar("sm_emotes_speed", "0.85", "Sets the playback speed of the animation. default (1.0)", CVAR_FLAGS);
	g_cvPractice_Transtime = FindConVar("sv_practice_transtime");

	AutoExecConfig(true, "fortnite_emotes_extended_css");
...
public void OnMapStart()
{
	...
	AddFileToDownloadsTable("models/player/custom_player/kodua_foxhound/css_emotes_beta.mdl");
	AddFileToDownloadsTable("models/player/custom_player/kodua_foxhound/css_emotes_beta.vvd");
	AddFileToDownloadsTable("models/player/custom_player/kodua_foxhound/css_emotes_beta.dx90.vtx");
	...
	// this dont touch
	PrecacheModel("models/player/custom_player/kodua_foxhound/css_emotes_beta.mdl", true);
...
public void OnClientPutInServer(int client)
{
	...
	{	
		...
		if (CooldownTimers[client] != null && g_bIsPracticeTime == false)
		{
...
public Action StopForniteCMD(int client, int args)
{
	for (int i = 1; i <= MaxClients; i++)
            if (IsValidClient(i, false) && g_bClientDancing[i]) {
				ResetCam(i);
...
public Action StopFornite(Event event, const char[] name, bool dontBroadcast)
{
	for (int i = 1; i <= MaxClients; i++)
            if (IsValidClient(i, false) && g_bClientDancing[i]) {
				ResetCam(i);
...
public Action DisableFornitePracticeEnd(Event event, const char[] name, bool dontBroadcast)
{
	PrintToServer("StopFornitePracticeEnd");
	for (int i = 1; i <= MaxClients; i++){
            if (IsValidClient(i, false) && g_bClientDancing[i]) {
				ResetCam(i);
				StopEmote(i);
		...
		//add cooldown untill after round practice
		//this is necessary as active emote during practice round end will crash the client.
		//there is no working practice round end hook therefore this timer is needed.
		//"nmrih_practice_ending" event is called when practice timer starts ticking down.
		float practicecooldown = g_cvPractice_Transtime.FloatValue + 2.0;
		CooldownTimers[i] = CreateTimer(practicecooldown, ResetCooldown, i);
		g_bIsPracticeTime = true;
	}
}
...
*/
	...
	if (IsValidEntity(EmoteEnt)) {
		...
		DispatchKeyValue(EmoteEnt, "model", "models/player/custom_player/kodua_foxhound/css_emotes_beta.mdl");
...
{
	// SetEntProp(client, Prop_Send, "m_iAddonBits", 0); removed since no exist on NMRIH
}
...
{
	CooldownTimers[client] = null;
	if (g_bIsPracticeTime){
		g_bIsPracticeTime = false;
	}
}

void EndTimer(any client)
{
	CooldownTimers[client] = null;
	if (g_bIsPracticeTime){
		g_bIsPracticeTime = false;
		...
		case MenuAction_Select:
		{
			...
			if(menu.GetItem(param2, info, sizeof(info)))
			{
				...
				switch (iParam2)
				{
                    ...
                    //CreateEmote(client, "Twist", "none", "", false);
					case 2:
...
public void CheckRestart(ConVar convar, const char[] oldValue, const char[] newValue) {

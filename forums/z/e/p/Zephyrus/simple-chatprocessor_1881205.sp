...
public Action:OnSayText2(UserMsg:msg_id, Handle:bf, const clients[], numClients, bool:reliable, bool:init)
{
	...
	*/
	new m_bProtobuf = (GetUserMessageType() == UM_Protobuf?true:false);
	decl cpSender;
	if(m_bProtobuf)
	...
	*/
	...
	WritePackCell(cpPack, ((cpRecipients == INVALID_HANDLE) ? true : false));	
...
public OnGameFrame()
{
	for (new i = 0; i < GetArraySize(g_hDPArray); i++)
	{
		...
		new Handle:bf = (ReadPackCell(pack) ? (StartMessageAll("SayText2", USERMSG_RELIABLE|USERMSG_BLOCKHOOKS)) : (StartMessage("SayText2", clients, numClientsFinish, USERMSG_RELIABLE|USERMSG_BLOCKHOOKS)));

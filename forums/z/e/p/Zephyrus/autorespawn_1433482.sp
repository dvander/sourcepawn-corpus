...
public Action:Event_Spawn( Handle:Spawn_Event, const String:Death_Name[], bool:Death_Broadcast )
{
...
public Action:Event_Death( Handle:Death_Event, const String:Death_Name[], bool:Death_Broadcast )
{
	if( GetConVarBool(sm_ar_enable) )
	{
...
public Action:RespawnClient( Handle:timer, any:client )
{
	if( GetConVarBool(sm_ar_enable) )
	{
		if ( IsValidEntity(client) && IsClientInGame(client) && IsClientObserver(client) && !IsPlayerAlive(client) && GetClientTeam(client) != 1)
		{

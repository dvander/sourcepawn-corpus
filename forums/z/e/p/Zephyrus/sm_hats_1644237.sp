...
public Handle:CreateHatsMenu(client, category)
{
	...
	else
	{
		...
		for(new i=0;i<MAX_HATS;++i)
		{
			...
			if(g_eHats[i][Flags] != 0 && g_eHats[i][Team] == team && !(GetUserFlagBits(client) & g_eHats[i][Flags]) && !(GetUserFlagBits(client) & ADMFLAG_ROOT))
				continue;
...
{
	if(IsValidEdict(g_iHats[client][slot]) && g_iHats[client][slot] > MaxClients)
	{
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3])
{
	if(!g_bRemove)
		if(GetClientHealth(victim)-damage<=0)
		{
			for(new i=0;i<MAX_CATEGORIES;++i)
			{
				if(IsValidEdict(g_iHats[victim][i]) && g_iHats[victim][i] != 0)
				{
					new String:sModel[PLATFORM_MAX_PATH];
...
public ItemExists_Hat(const String:data[])
{
	for(new i=0;i<MAX_HATS;++i)
	{
		if(strcmp(g_eHats[i][ModelPath], data)==0)
			return i;
	}
	return -1;
}
...
GetNumHats(client, category)
{
	...
	{		
		...
		{
			if(g_eHats[i][Category] == category && (g_eHats[i][Flags]==0?true:bool:(flags & g_eHats[i][Flags])) && (g_eHats[i][Team]==0?true:(team == g_eHats[i][Team])))
				count++;
		}
	}

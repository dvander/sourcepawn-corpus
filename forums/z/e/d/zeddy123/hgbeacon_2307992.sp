...
new Handle:cvarPlayers = INVALID_HANDLE;
...
public OnPluginStart()
{
        ...
        cvarPlayers = CreateConVar("sm_beacontimer_players", "5", "Wait for less or equal to this amount of player to start the beacon", _, true, 2);
...
public EventPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
        if (GetConVarInt(cvarPluginEnable))
        {
                ...
                if(1 < GetPlayerCount() <= GetConVarInt(cvarPlayers))
                {
                        if (!g_BeaconOn)
                        {
                                BeaconTimer = CreateTimer(GetConVarFloat(cvarTime), BeaconAll);

...
#define PLUGIN_AUTHOR "AtomicStryker, Shadowysn (New syntax and common kill method), add stomping animation and make some minor changes by Kaz"
...
#define STOMP_MOVE_PENALTY				0.75 
#define STOMP_SLOW_DURATION				0.5 
	...
	if (strcmp(classname, CLASSNAME_INFECTED) == 0)
	{
		...
		if ((i >= ANIM_SEQUENCES_DOWNED_BEGIN && i <= ANIM_SEQUENCES_DOWNED_END) || i == ANIM_SEQUENCE_WALLED)
		{
			if (f >= DOWNED_ANIM_MIN_CYCLE && f <= DOWNED_ANIM_MAX_CYCLE)
			{
				...
				CreateTimer(1.0, RESET, entity, TIMER_FLAG_NO_MAPCHANGE); 
				SmashInfected(other, entity);
				
				if (GetConVarBool(cvarSlowSurvivor))
				{
					float currentSpeed = GetEntPropFloat(entity, Prop_Data, "m_flLaggedMovementValue");
					SetEntPropFloat(entity, Prop_Data, "m_flLaggedMovementValue", currentSpeed * STOMP_MOVE_PENALTY);
					CreateTimer(STOMP_SLOW_DURATION, RestoreSpeed, entity, TIMER_FLAG_NO_MAPCHANGE);
				}
			}
		}
	}
...
public Action RestoreSpeed(Handle timer, int client)
{

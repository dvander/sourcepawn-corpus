...
{
	name = "SpecList", 
	author = "cra88y | edited by quikis for nojo", 
	version = "1.1.2", 
};
...
public Action Timer_UpdateHudHint(Handle timer, any client)
{
	...
	if (IsPlayerAlive(client))
	{
		for (int i = 1; i <= MaxClients; i++)
		{
			if (!IsClientInGame(i) || !IsClientObserver(i) || IsClientSourceTV(client) || IsClientReplay(client))
				continue;
			...
			if (iTarget == client)
			{
				if (IsPlayerVip(i) == true && speclist_stealth[i] == false)
				{
					...
					{
						Format(szText, sizeof(szText), "%s<font color='#4169E1'>%N.</font> ", szText, i);
					}
					else if (IsPlayerMod(i) == true)
					{
						Format(szText, sizeof(szText), "%s<font color='#800080'>%N.</font> ", szText, i);
					}
					else
					{
	...
	if (bDisplayHint)
	{
		if (speclist_enabled[client] == true)
		{
			PrintHintText(client, "<font size='12'><font color='#87CEFA'><u>Spectators:\n</u></font><font size='15'>%s</font>", szText);
			bDisplayHint = false;
		}
	}
...
{
	if (IsClientInGame(client) && CheckCommandAccess(client, "spec_admin", ADMFLAG_ROOT))
		return true;
...
{
	if (IsClientInGame(client) && CheckCommandAccess(client, "spec_vip", ADMFLAG_CUSTOM1))
		return true;
	
	return false;
}
bool IsPlayerMod(int client)
{
	if (IsClientInGame(client) && CheckCommandAccess(client, "spec_mod", ADMFLAG_GENERIC))
		return true;

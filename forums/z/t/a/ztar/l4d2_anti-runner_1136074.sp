...
public Action:TimerAnnounce(Handle:timer, any:client)
{
	if(IsClientInGame(client) && GetConVarInt(sm_ar_announce))
		PrintToChat(client, "\x04[ARS]\x03 Anti-Runner System is applied in this server.");
}
		...
		if(GetEdictClassname(Entity, entname, sizeof(entname)))
		{
			...
			if(StrEqual(entname, "prop_door_rotating_checkpoint"))
			{
				for (new i = 1; i <= GetMaxClients(); i++)
				{
					...
					if(IsValidEntity(i) && IsClientInGame(i))
					{
						if(GetEntProp(i, Prop_Send, "m_zombieClass") == TANK && GetConVarInt(sm_ar_lock_tankalive))
						{
							...
							#if JAPANESE
							PrintHintText(client, "逃がさん・・・");
							#else
							PrintHintText(client, "Tank is still alive, Kill all of them!");
							#endif
							return Plugin_Continue;
						}
					}
				...
				if(client == idKeyman)
				{
					...
					{
						#if JAPANESE
						PrintToChatAll("\x04[ARS]\x03 キーマン\x01<%s>\x03 がセーフルームに到着した", nmKeyman);
						#else
						PrintToChatAll("\x04[ARS]\x03 Keyman \x01<%s>\x03 reached safe room!", nmKeyman);
						#endif
					}
					...
					if(adProb > GetRandomInt(0, 99))
					{
						/* Spawn Assault door */
						#if JAPANESE
						PrintHintTextToAll("アサルトドアーが現れた！");
						#else
						PrintHintTextToAll("Assault door spawned!");
						#endif
				...
				else
				{
					...
					EmitSoundToAll(SoundNotice, Entity);
					#if JAPANESE
					PrintHintTextToAll("このセーフルームの扉は <%s> のみ開けることができる", nmKeyman);
					#else
					PrintHintTextToAll("Keyman is <%s>.\nOnly Keyman can open saferoom door!", nmKeyman);
					#endif
				}
			}
		}

...
ConVar cInterval, cDamage, cRange, cDistance, cSpeed, cEnable, cChance,	cMaxProjectiles;
...
int g_iMaxProjectiles;
int g_iProjectileCount = 0;

public Plugin myinfo =
{
	name = "[L4D2] Magic Witch",
	author = "BHaType (Modified by zuaL)",
	description = "Makes witch more dangerous",
	version = "0.0.5",
	url = "https://forums.alliedmods.net/showthread.php?t=317815"
}
...
public void OnPluginStart()
{
	...
	cMaxProjectiles = CreateConVar("magic_witch_max_projectiles", "10"		, "Maximum projectiles allowed globally", FCVAR_NONE);
	...
	g_iMaxProjectiles 	= cMaxProjectiles.IntValue;
	...
	cMaxProjectiles.AddChangeHook(IntsCvar);
...
{
	if (g_iProjectileCount >= g_iMaxProjectiles)
		...
		else
		{
			...
			for(int i; i < MAXENTITIES; i++)
			{
				if((entity = EntRefToEntIndex(g_iEntities[witch][i])) != INVALID_ENT_REFERENCE && IsValidEntity(entity) && entity > MaxClients)
				{
					...
					if (g_iProjectileCount > 0) g_iProjectileCount--;

...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon){
	if ((buttons & IN_ATTACK) && (GetConVarInt(cvar_tankroar) != 0)) {
		...
		if (IsValidEntity(survivor[iii]) && (survivor[iii] != 0))
		{
			if (!GetEntProp(survivor[iii], Prop_Send, "m_isIncapacitated") && !pinned[survivor[iii]])
			{
				...
				if (CheckDistance(distance))
				{
					...
					ratio[0] = distance[0] / SquareRoot(distance[1]*distance[1] + distance[0]*distance[0]);
					ratio[1] = distance[1] / SquareRoot(distance[1]*distance[1] + distance[0]*distance[0]);
					
					GetEntPropVector(survivor[iii], Prop_Data, "m_vecVelocity", svVector);
					
					addAmount[0] = (ratio[0] * Direction()) * power;
					addAmount[1] = (ratio[1] * Direction()) * power;
					addAmount[2] = power;
					
					resultant[0] = addAmount[0] + svVector[0];
					...
					Handle hShake = StartMessageOne("Shake", survivor[iii]);
					if (hShake != INVALID_HANDLE)
					{
						BfWriteByte(hShake, 0);       // command
						BfWriteFloat(hShake, 10.0);   // amplitude (strength of shake)
						BfWriteFloat(hShake, 1.0);    // frequency
						BfWriteFloat(hShake, 1.0);    // duration in seconds
						EndMessage();
					}

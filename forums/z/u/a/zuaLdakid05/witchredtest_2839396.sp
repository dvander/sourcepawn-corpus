...
{
    return (ent > 0 && ent <= GetMaxEntities() && IsValidEntity(ent));
}
...
stock void CreateParticle(int entity, const char[] particleName, float lifetime = 0.0)
{
    ...
    // if lifetime > 0, kill particle after that time
    if (lifetime > 0.0)
    {
        char cmd[32];
        Format(cmd, sizeof(cmd), "OnUser1 !self:Kill::%f:1", lifetime);

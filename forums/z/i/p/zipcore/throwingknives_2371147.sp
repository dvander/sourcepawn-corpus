...
public Action:player_death(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if(StrContains(weapon, "knife", false) != -1) // All knive kills, because csgo
	{
		...
		if(GetConVarBool(cvar_throwingknives_steal) && GetConVarInt(cvar_throwingknives_count) > 0)
		{
			new victim = GetClientOfUserId(GetEventInt(event, "userid"));
			if(g_iPlayerKniveCount[victim] > 0)
			{
				g_iPlayerKniveCount[attacker] += g_iPlayerKniveCount[victim];
				PrintHintText(attacker, "Throwing knives: %i/%i", g_iPlayerKniveCount[attacker], GetConVarInt(cvar_throwingknives_count));
			}
		}
...
public weapon_fire(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if(g_iPlayerKniveCount[client] <= 0 && GetConVarInt(cvar_throwingknives_count) != -1)
	{
		return;
	}

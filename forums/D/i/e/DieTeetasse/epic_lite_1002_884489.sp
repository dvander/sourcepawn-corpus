...
#define DEBUG_FILE "epicmodlite_debug.txt"
...
public Plugin:myinfo =
{
	...
	url = "http://brezelbude.pytalhost.de/l4d.htm"
};
...
public OnPluginStart()
{
	//+++++ Cvars +++++
	//Versionnumber
	CreateConVar("epic_version", PLUGIN_VERSION, "Epicmod version", CVAR_FLAGS)
	...
	//+++++ Commands +++++
	RegConsoleCmd("epiclite_list_changes", Command_Description, "Lists all changes of epicmod.")
	RegServerCmd("epiclite_check_corner_camping", Command_CornerCamping, "Force to check if there is cornercamping")
	RegServerCmd("epiclite_dump_stats", Command_DumpStats, "Dump the stats arrays for survivor and infected")
...
{
	PrintToConsole(client, "List changes:")
	PrintToConsole(client, "- Round statistics like killed zombies etc pp")
	PrintToConsole(client, "- Personal statistics for infected (dmg in life and dmg in round) and survivors (kills in round)")
	PrintToConsole(client, "- Anti corner camping with slap function")
	PrintToConsole(client, "- Markersystem to mark as an infected survivors for better coordination")
	PrintToConsole(client, "- Messages for connection, disconnection and friendly fire")
...
public Event_round_start(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//Targetsystem and personal statistics initialize
	for (new i = 0; i < 4; i++)
	{
		binfected[i] = false
		bsurvivor[i] = false
...
public Event_player_team(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	LogToFile(DEBUG_FILE, "Player %d changes team...", cid)
...
CreateStats(cid)
{
	...
	if (GetClientTeam(cid) == 3)
	{
		...
		if (vorhanden)
		{
			...
			LogToFile(DEBUG_FILE, "Inf %d found in %d", cid, auswahl)
		...
		auswahl = -1
		LogToFile(DEBUG_FILE, "Finding new space for Inf %d...", cid)
		...
		if (auswahl == -1)
		{
			LogToFile(DEBUG_FILE, "No space for Inf %d found...", cid)
			return
		}
	
		LogToFile(DEBUG_FILE, "%d found...", auswahl)
	...
	if (GetClientTeam(cid) == 2)
	{
		...
		if (vorhanden)
		{
			LogToFile(DEBUG_FILE, "Survivor %d found in %d", cid, auswahl)
		...
		auswahl = -1
		LogToFile(DEBUG_FILE, "Finding new space for %d...", cid)
		...
		if (auswahl == -1)
		{
			LogToFile(DEBUG_FILE, "No space for Inf %d...", cid)
			return
		}
...
ClearStatsSurvivor(client)
{
	...
	for (new i = 0; i < 4; i++)
	{
		if (StrContains(Survivor[i], authstr) != -1)
		{
			...
			LogToFile(DEBUG_FILE, "Delete survivor %d stats in array %d...", client, i)
			bsurvivor[i] = false
			Survivor[i] = ""
			break
		}
...
ClearStatsInfected(client)
{
	...
	for (new i = 0; i < 4; i++)
	{
		...
		{
			//delete his stats
			LogToFile(DEBUG_FILE, "Delete infected %d stats in array %d...", client, i)
			binfected[i] = false
			Infected[i] = ""
			break
		}
			...
			if (GetClientTeam(inf_cid) == 3)
			{
				...
				for (new i = 0; i < 4; i++)
				{
					...
					if (StrContains(Infected[i], inf_auth) != -1)
					{
						...
						LogToFile(DEBUG_FILE, "Infected %d im Array %d %d Dmg dazuaddieren...", inf_cid, i, dmg)
						break
					}
				}
			}
...
public Event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	else
	{
		...
		if (GetClientTeam(attackercid) == 2)
		{
			...
			if (!IsFakeClient(attackercid))
			{
				...
				{
					LogToFile(DEBUG_FILE, "Survivor %d not found...", attackercid)
					attackercid = -1
				}	
				else
				{
					LogToFile(DEBUG_FILE, "Survivor %d found in %d...", attackercid, survivor_arrayid)
				}
			}
			else
			{
	...
	if (GetClientTeam(cid) == 3)
	{
		...
		if (arrayid == -1)
		{
			LogToFile(DEBUG_FILE, "Infected %d not found...", cid)
			return
		}
		
		LogToFile(DEBUG_FILE, "Infected %d found in %d...", cid, arrayid)
...
public Show_Stats()
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		if (GetClientTeam(i) == 2)
		{
			...
			if (survivor_arrayid == -1)
			{
				LogToFile(DEBUG_FILE, "Survivor %d not foundn... normal display", i)
		...
		if (GetClientTeam(i) == 3)
		{
			...
			{
				LogToFile(DEBUG_FILE, "Infected %d not found... normal display", i)		
			}
...
public Action:WelcomePlayer(Handle:timer, any:client)
{
	...
	PrintToChat(client, "[%s] Welcome to this server, %s!", PLUGIN_TAG, name)
	PrintToChat(client, "[%s] Type epiclite_list_changes in console to see the changes!", PLUGIN_TAG)

...
public OnPluginStart()
{
	...
	cvarGolden = CreateConVar("knife_golden_enable", "1", "Enable or Disable Golden");
...
public Action:DID(clientId) 
{
	...
	{
	AddMenuItem(menu, "option7", "[25] Golden");
	}
...
public DIDMenuHandler(Handle:menu, MenuAction:action, client, itemNum) 
{
		...
		{
			{
			  ...
			  PrintToChat(client,"[Knife Shop] \x02Created by: \x01TummieTum (TumTum) & edited by Dk--");
			}
	...
	if (IsPlayerAlive(client))
	{
		...
		PrintToChat(client, "[Knife Shop] You bought a Golden Knife! Your credits: \x03%i \x01(-25)", g_iCredits[client]);
				   }
				   else
					{
					if (g_iCredits[client] >= 25)
			  ...
			  {
				 PrintToChat(client, "[Knife Shop] Your credits: \x03%i (Not enough credits! Need 25)", g_iCredits[client]);
			  }
			}

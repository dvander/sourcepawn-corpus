...
new Handle:cvarFalchion = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	cvarFalchion = CreateConVar("knife_falchion_enable", "1", "Enable or Disable Falchion", FCVAR_NONE, true, 0.0, true, 1.0);
...
public Action:DID(clientId) 
{
	...
	if (GetConVarBool(cvarFalchion))
	{
	AddMenuItem(menu, "option9", "Falchion");
	}
	if (GetConVarBool(cvarButterfly))
	{
	AddMenuItem(menu, "option10", "Butterfly");
	}
		   ...
		   if (IsPlayerAlive(client))
					{
						...
						PrintToChat(client, "[SM KNIFE] \x06 %T", "Falchion", client);
					}
			else if (!IsPlayerAlive(client))
					{
						weapon_choose[client] = 8;
						PrintToChat(client, "[SM KNIFE] \x06 %T", "Dead Falchion", client);
					}
		}
		else if ( strcmp(info,"option9") == 0 ) 
		{

...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_red", Command_JoinRed, 0, "Usage: sm_mvmred to join RED team if on the spectator team");
...
public OnMapStart()
{
	...
	if (IsMvM(true))
		gMvM=1;
	if (gMvM)
		CreateTimer(7.0, spec_alert, _, TIMER_FLAG_NO_MAPCHANGE|TIMER_REPEAT);
}

public OnClientPutInServer(client)
{
	if (gMvM && !IsFakeClient(client))
		CreateTimer(5.0, timer_force_red, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE|TIMER_REPEAT);
}
public Action:spec_alert(Handle:t)
{
	for (new i=1;i<=MaxClients;i++)
		if (IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) < 2)
			PrintToChat(i,"You're in spectate. Type /red or /mvmred to play!");
}

public Action:timer_force_red(Handle:t, any:userid)
{
	PrintToServer("tenmvm: trying to get userid %d onto red", userid);
...
{
	PrintToServer("tenmvm: %N jointeam", client);
	new String:arg1[32];

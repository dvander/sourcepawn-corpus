...
public void OnPluginStart()
{
	...
	lsAntiFarmDuration = CreateConVar("lockdown_system-l4d2_anti-farm_duration", "150", "Duration Of Anti-Farm", FCVAR_SPONLY|FCVAR_NOTIFY);
	lsDuration = CreateConVar("lockdown_system-l4d2_duration", "30", "Duration Of Lockdown", FCVAR_SPONLY|FCVAR_NOTIFY);
	lsMobs = CreateConVar("lockdown_system-l4d2_mobs", "2", "Number Of Mobs To Spawn", FCVAR_SPONLY|FCVAR_NOTIFY, true, 1.0, true, 5.0);
	lsMenu = CreateConVar("lockdown_system-l4d2_menu", "0", "Enable/Disable Menu", FCVAR_SPONLY|FCVAR_NOTIFY, true, 0.0, true, 1.0);
	lsTankDemolition = CreateConVar("lockdown_system-l4d2_tank_demolition", "1", "Enable/Disable Tank Demolition", FCVAR_SPONLY|FCVAR_NOTIFY, true, 0.0, true, 1.0);
	lsType = CreateConVar("lockdown_system-l4d2_type", "0", "Lockdown Type: 0=Random, 1=Improved, 2 & 3=Default", FCVAR_SPONLY|FCVAR_NOTIFY, true, 0.0, true, 3.0);
...
{
	if (!L4D_IsMissionFinalMap() && !HasSaferoomBug())
	{
...
{
	if (L4D_IsMissionFinalMap() || HasSaferoomBug() || !bTankDemolition || !bLDFinished)
	{
		return;
	}
...
{
	if ((StrEqual(name, "round_end") && StrContains(sGameMode, "versus", false) == -1) || L4D_IsMissionFinalMap() || HasSaferoomBug())
	{
		return;
	}
		...
		if (IsValidEnt(used))
		{
			...
			if (iDoorStatus != UNLOCK)
			{
				if (bMenu)
				{
					...
					mSystemChoice.DisplayVote(iHumans, iPlayers, 15);
				}
				else
				{
					if (L4D2_GetTankCount() > 0)
					{
						...
						PrintHintText(user, "[LS] %s Still Alive! Kill %s First!", (L4D2_GetTankCount() != 1) ? "Tanks Are" : "A Tank Is", (L4D2_GetTankCount() == 1) ? "It" : "Them");
...
{
	if (L4D2_GetTankCount() < 1 || hAntiFarmTime == null)
	{
		if (hAntiFarmTime != null)
		{
			KillTimer(hAntiFarmTime);

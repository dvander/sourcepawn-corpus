...
#define STATS_PREFIX "\x04[Stats] \x01"
#define PLUGIN_DESCRIPTION "Player Stats and Ranking for Left 4 Dead and Left 4 Dead 2."
#define PLUGIN_AUTHOR "©2019-2020 http://PRIMEAS.DE - Based on the Plugin from muukis modified by Foxhound"

#define URL_PRIVACY_POLICE = "http://"
...
new bool: StatsOn[MAXPLAYERS + 1]; //new
...
new Handle: cvar_Glow = INVALID_HANDLE;
...
new Handle: stats_advice[MAXPLAYERS + 1] = INVALID_HANDLE;
...
public OnPluginStart() {
  ...
  //TIMER = CreateTimer(TIME, DisplayHint,_, TIMER_REPEAT);
  ...
  cvar_HumansNeeded = CreateConVar("l4d_stats_minhumans", "2", "Minimum Human players before stats will be enabled", 0, true, 1.0, true, 4.0);
  ...
  cvar_Glow = CreateConVar("l4d_stats_glow", "0", "glow rewards - enable 1 disable - 0", 0);
...
public OnAdminMenuReady(Handle: TopMen) {
  // Block us from being called twice
  if (TopMen == RankAdminMenu) return;

  RankAdminMenu = TopMen;
...
public Action:DisplayHint(Handle:timer) {

 for(new i=1;i<MaxClients;i++){

PrintHintText(i, "Write '!stats' to open the Player Statistics Menu.");
...
public Action: AnnounceConnect(Handle: timer, any: client) {
  ...
  //PrintHintText(client, "Write '!stats' to open the Player Statistics Menu.");
}
...
public OnClientDisconnect(client) {
  ...
  L4D2_SetEntGlow(client, L4D2Glow_None, 0, 0, {
    0,
    0,
    0
  },
...
// Insert a player into the database if they do not already exist.(update)
...
public InsertPlayerDB(Handle: owner, Handle: hndl, const String: error[], any: client) {
  ...
  if (!SQL_GetRowCount(hndl)) {
    ...
    if (!StatsOn[client]) {
      ...
      if (GetClientLanguage(client) == 9) {
        Menu_Germans(client, 0);
      } else {
...
public Action: Menu_Primeas(client, args) {

  if (GetClientLanguage(client) == 9) {

    new Handle: menu = CreateMenu(Menu_Germans_Handler);

    SetMenuTitle(menu, "Left 4 Dead (2) Spieler Statistiken\n\n"); //

    AddMenuItem(menu, "Agree", "Zustimmen");
    AddMenuItem(menu, "Decline", "Ablehnen & Löschen");
    ...
    new Handle: menu = CreateMenu(Menu_Primeas_Handler);
...
public Menu_Primeas_Handler(Handle: menu, MenuAction: action, client, identity) {
  ...
  if (action == MenuAction_Select) {
    ...
    if (strcmp(info, "Agree") == 0) {

      StatsOn[client] = true;

      CheckPlayerDB(client);

      PrintToChat(client, "%sYou have Agreed the Player Statistics.", STATS_PREFIX); //Sie haben Spieler Statistiken - Datenschutz Zugestimmt.

    }

    if (strcmp(info, "Decline") == 0) {

        new Handle: security = CreateMenu(Menu_Decide);
...
public Menu_Decide(Handle: security, MenuAction: action, client, identity) {
  ...
  if (action == MenuAction_Select) {
    ...
    if (strcmp(info, "yup") == 0) {
      ...
      if (GetClientLanguage(client) == 9) {
        ...
        PrintToChat(client, "%sDu hast die Spieler Statistiken abgelehnt. Deine Statistiken wurden gelöscht.", STATS_PREFIX);
      } else {

        PrintToChat(client, "%sYou have Decline the Player Statistics. Your Player Statistics have been deleted.", STATS_PREFIX);
      }

    }

    if (strcmp(info, "nope") == 0) {
...
public Menu_Stats_Handler(Handle: menu, MenuAction: action, client, identity) {
  ...
  if (action == MenuAction_Select) {
    ...
    if (strcmp(info, "nope") == 0) {
      ...
      if (GetClientLanguage(client) == 9) {
        new Handle: security = CreateMenu(Menu_Decide);
        SetMenuTitle(security, "Left 4 Dead (2) Spieler Statistiken\nDeine Statistiken löschen?\n\n");
        AddMenuItem(security, "yup", "Ja, Ablehnen & Löschen");
        AddMenuItem(security, "nope", "Nein");
        SetMenuExitButton(security, false);
        DisplayMenu(security, client, 30);
      } else {
        new Handle: security = CreateMenu(Menu_Decide);
...
public Action: Menu_StatsOn(client, args) {

  new Handle: menu = CreateMenu(Menu_Stats_Handler);

  SetMenuTitle(menu, "Left 4 Dead (2) Player Statistics\n"); //Spieler Statistiken - Datenschutz:

  AddMenuItem(menu, "yup", "Agree"); //Zustimmen
  AddMenuItem(menu, "nope", "Decline"); //Ablehnen
...
public Action: Menu_Germans(client, args) {

  new Handle: menu = CreateMenu(Menu_Stats_Handler);

  SetMenuTitle(menu, "Left 4 Dead (2) Spieler Statistiken\n");

  AddMenuItem(menu, "yup", "Zustimmen");
  AddMenuItem(menu, "nope", "Ablehnen");
...
public Menu_Germans_Handler(Handle: menu, MenuAction: action, client, identity) {
  ...
  if (action == MenuAction_Select) {
    ...
    if (strcmp(info, "Agree") == 0) {
      ...
      PrintToChat(client, "%sSie haben den Spieler Statistiken zugestimmt.", STATS_PREFIX);
...
public Action: DeleteStats(client, args) {
  ...
  new String: query[512];
  Format(query, sizeof(query), "DELETE FROM %splayers WHERE `steamid` = '%s';", DbPrefix, SteamID);
  SendSQLUpdate(query);

  new String: query2[512];
  Format(query2, sizeof(query2), "DELETE FROM %stimedmaps WHERE `steamid` = '%s';", DbPrefix, SteamID);
...
public Action: timer_MeleeKill(Handle: timer, any: data) {
  ...
  decl String: query[512],
  String: clientID[MAX_LINE_WIDTH];
...
public Action: event_PlayerDeath(Handle: event, const String: name[], bool: dontBroadcast) {
  ...
  L4D2_SetEntGlow(Victim, L4D2Glow_None, 0, 0, {
    255,
    255,
    255
  },
  false);
  SetEntityRenderColor(Victim, 0, 0, 0, 255);
...
public Action: event_JockeyRelease(Handle: event, const String: name[], bool: dontBroadcast) {
  ...
  if (Rescuer > 0 && !IsClientBot(Rescuer) && IsClientInGame(Rescuer)) {
    decl String: query[1024],
    String: JockeyName[MAX_LINE_WIDTH],
    String: VictimName[MAX_LINE_WIDTH],
    String: RescuerName[MAX_LINE_WIDTH],
    String: RescuerID[MAX_LINE_WIDTH],
    String: UpdatePoints[32];
...
public Action: event_ChargerKilled(Handle: event, const String: name[], bool: dontBroadcast) {
  ...
  decl String: query[1024],
  String: KillerName[MAX_LINE_WIDTH],
  String: KillerID[MAX_LINE_WIDTH],
  String: UpdatePoints[32];
  new Score = 0;
  ...
  if (Mode) {
    ...
    if (IsMatador) {
      ...
      decl String: VictimName[MAX_LINE_WIDTH],
      String: ChargerName[MAX_LINE_WIDTH];
...
public Action:
event_SurvivalStart(Handle:
event, const String: name[], bool: dontBroadcast) {
  if (StatsDisabled()) return;
...
public Action: TIMER_ADVICE(Handle: timer, any: client) {
  ...
  if (GetClientLanguage(client) == 9) {
    ...
    PrintHintText(client, "Schreib '!stats' um das Menu zu öffnen, '!rank' zeigt dein Rang.");
  ...
  if (StatsOn[client]) {
    ...
    if (GetConVarInt(cvar_Glow)) {
      ...
      case 1:
        {
          if (GetClientLanguage(client) == 9) {
            PrintToChat(client, "%sDu bist jetzt Rang 1. Du hast die Belohnung für Legendäres Leuchten erhalten.", STATS_PREFIX);
          } else {
            PrintToChat(client, "%sYou are now Rank 1. You got the Legendary Glow Reward.", STATS_PREFIX);
          }
        }
      case 2:
        {
          if (GetClientLanguage(client) == 9) {
            PrintToChat(client, "%sDu bist jetzt Rang 2. Du hast die Belohnung für Episches Leuchten erhalten.", STATS_PREFIX);
          } else {
            PrintToChat(client, "%sYou are now Rank 2. You got the Epic Glow Reward.", STATS_PREFIX);
          }
        }
      case 3:
        {
          if (GetClientLanguage(client) == 9) {
            PrintToChat(client, "%sDu bist jetzt Rang 3. Du hast die Belohnung für Seltenes Leuchten erhalten.", STATS_PREFIX);
          } else {
            PrintToChat(client, "%sYou are now Rank 3. You got the Rare Glow Reward.", STATS_PREFIX);
          }
        }
      }
    }
    ...
    if (ClientRank[client] > 0) {
      for (new o = 1; o < MaxClients; o++) {
        ...
        if (GetClientLanguage(o) == 9) {

          PrintToChat(o, "%sSpieler %s ist beigetreten. Rang: %i (Punkte: %i)", STATS_PREFIX, userName, ClientRank[client], ClientPoints[client]);
        } else {

          PrintToChat(o, "%sPlayer %s joined the Game. Rank: %i (Points: %i)", STATS_PREFIX, userName, ClientRank[client], ClientPoints[client]);
        }
      ...
      for (new o = 1; o < MaxClients; o++) {
        ...
        if (GetClientLanguage(o) == 9) {
          PrintToChat(o, "%sSpieler %s ist dem Spiel beigetreten.", STATS_PREFIX, userName);
        } else {
          PrintToChat(o, "%sPlayer %s joined the Game.", STATS_PREFIX, userName);
        }
...
public Action: cmd_MapTimes(client, args) {
  ...
  String: CurrentMapName[MAX_LINE_WIDTH];
...
public ExecuteTeamShuffle(Handle: owner, Handle: hndl, const String: error[], any: data) {
  ...
  client,
  topteam;
  new SurvivorsLimit = GetConVarInt(cvar_SurvivorLimit),
...
public CreateRanksMenu(Handle: owner, Handle: hndl, const String: error[], any: client) {
  ...
  decl String: Name[32],
  String: DisplayName[MAX_LINE_WIDTH];
...
public CreateTimedMapsMenu(Handle: owner, Handle: hndl, const String: error[], any: client) {
  ...
  decl String: GamemodeTitle[32],
  String: GamemodeInfo[2]; //, MutationInfo[MAX_LINE_WIDTH];
...
public CreateTimedMapsMenu2(Handle: owner, Handle: hndl, const String: error[], any: client) {
  ...
  new Handle: menu = CreateMenu(Menu_CreateTimedMapsMenu2Hndl),
  Gamemode;
  decl String: Map[MAX_LINE_WIDTH],
  String: Info[MAX_LINE_WIDTH],
  String: CurrentMapName[MAX_LINE_WIDTH];
...
public DisplayTimedMapsMenu3FromInfo(client, String: Info[]) {
  ...
  decl String: GamemodeInfo[2],
  String: Map[MAX_LINE_WIDTH];
...
public CreateTimedMapsMenu3(Handle: owner, Handle: hndl, const String: error[], any: client) {
  ...
  new Handle: menu = CreateMenu(Menu_CreateTimedMapsMenu3Hndl),
  Float: MapTime;
  decl String: Map[MAX_LINE_WIDTH],
  String: Info[MAX_LINE_WIDTH],
  String: Value[MAX_LINE_WIDTH];
...
public PrintToConsoleAll(const String: Message[], any: ...) {
  ...
  for (new i = 1; i <= maxplayers; i++)
  if (IsClientConnected(i) && IsClientInGame(i) && !IsClientBot(i)) PrintToConsole(i, FormattedMessage);
}
...
public UpdateMapTimingStat(Handle: owner, Handle: hndl, const String: error[], any: dp) {
  ...
  String: query[512],
  String: TimeLabel[32],
  String: Mutation[MAX_LINE_WIDTH];
  new GamemodeID,
  Float: TotalTime,
  Float: OldTime,
  Client,
  ...
  if (IsClientBot(Client) || !IsClientInGame(Client)) return;
  ...
  if (SQL_GetRowCount(hndl) > 0) {
    ...
    if ((CurrentGamemodeID != GAMEMODE_SURVIVAL && OldTime <= TotalTime) || (CurrentGamemodeID == GAMEMODE_SURVIVAL && OldTime >= TotalTime)) {
      ...
      if (EnableSounds_Maptime_Improve && GetConVarBool(cvar_SoundsEnabled)) EmitSoundToClient(Client, StatsSound_MapTime_Improve);
    }
  }
  else {
    if (Mode) {
...
SetGlow(client) {
  ...
  if(IsFakeClient(client) || !StatsOn[client] || GetClientTeam(client)!=TEAM_SURVIVORS || CurrentGamemodeID == GAMEMODE_REALISM) return;

  if (IsPlayerAlive(client) && GetConVarInt(cvar_Glow)) {
    ...
    case 2:{
      L4D2_SetEntGlow(client, L4D2Glow_Constant, 0, 0, {255,0,255}, false);
      SetEntityRenderColor(client, 128,0,128, 255);
      }
    case 3:{
      L4D2_SetEntGlow(client, L4D2Glow_Constant, 0, 0, {0,191,255}, false);

...
public OnPluginStart()
{
	decl String:game[64]
        ...
        hMaxPounceDistance = 1000;
        hMinPounceDistance = 300;
	...
	AutoExecConfig(true,"l4d2_pounceannounce");
...
public Event_PlayerPounced(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//get hunter-related pounce cvars
        new max = hMaxPounceDistance;
        new min = hMinPounceDistance;
	...
	if(distance >= min && dmg >= minAnnounce)
	{
		...
		#endif
		Format(pounceLine,sizeof(pounceLine),"Hunter %s pounced %s for %.01f HP!(Max: %d)",attackerName,victimName,dmg,maxDmg + 1);

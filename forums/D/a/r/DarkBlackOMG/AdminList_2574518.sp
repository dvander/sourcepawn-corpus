...
public Action:SayHook(client, args)
{
	...
	{   
			...
			switch(GetConVarInt(AdminListMode))
			{
				case 1:
				{
					...
					for(new i = 1 ; i <= GetMaxClients();i++)
					{
						if(IsClientInGame(i))
						{
							if(CheckCommandAccess(i, "sm_checkadminflag", ADMFLAG_GENERIC)){
				...
				case 2:
				{
					decl String:AdminNames[MAX_NAME_LENGTH];
		...
		if(StrEqual(text[startidx], "!vips") || StrEqual(text[startidx], "/vips"))
		{
			switch(GetConVarInt(AdminListMode))
			{
				case 1:
				{
					...
					for(new i = 1 ; i <= GetMaxClients();i++)
					{
						if(IsClientInGame(i))
						{
							if(CheckCommandAccess(i, "sm_checkadminflag", ADMFLAG_CUSTOM6)){
								GetClientName(i, AdminNames[count], sizeof(AdminNames[]));
								count++;
							}
						} 
					...
					decl String:buffer[1024];
					ImplodeStrings(AdminNames, count, ",", buffer, sizeof(buffer));
					PrintToChatAll("\x04Vips online are: %s", buffer);
				}
				case 2:
				{
					decl String:AdminNames[MAX_NAME_LENGTH];

...
public OnPluginStart()
{
	...
	l4d_stoptk_points_ban = CreateConVar("l4d_stoptk_points_ban", "2750.0", "", FCVAR_PLUGIN|FCVAR_SPONLY);
	...
	AutoExecConfig(true, "l4d_stoptk");
}
...
//		return Plugin_Continue;
	...
	if (GetConVarInt(l4d_stoptk_showmessages) > 0)
	{
		...
		PrintToChat(client, "\x03%d (%d - voteban; %d - ban) TK points!", RoundToZero(X * Damage[client]), RoundToZero(Punishment[client]) + RoundToZero(GetConVarFloat(l4d_stoptk_points_vote)), RoundToZero(GetConVarFloat(l4d_stoptk_points_ban)));
		PrintToChat(target, "\x01%N attacked %N", client, target);
		PrintToChat(target, "\x03%d (%d - voteban; %d - ban) TK points!", RoundToZero(X * Damage[client]), RoundToZero(Punishment[client]) + RoundToZero(GetConVarFloat(l4d_stoptk_points_vote)), RoundToZero(GetConVarFloat(l4d_stoptk_points_ban)));
	}
...
public Action:Event_MedkitUsed(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetConVarInt(l4d_stoptk_showmessages) > 0)
	{	
		PrintToChat(client, "\x01You have cured %N and your TK points has gone down: \x04%f x %f = %d", target, Damage[client], X, RoundToZero(X * Damage[client])); 
//		PrintToChat(client, "\x04You healed somebody and lost some TK points. Now you have: \x01%f x %f = %d TK points!", Damage[client], X, RoundToZero(X * Damage[client]));
	}
...
public Action:Event_ReviveSuccess(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetConVarInt(l4d_stoptk_showmessages) > 0)
	{	
		PrintToChat(client, "\x01You have revived %N and your TK points has gone down: \x04%f x %f = %d", target, Damage[client], X, RoundToZero(X * Damage[client])); 
	}
	...
	if (TotalDamage > GetConVarFloat(l4d_stoptk_points_vote))
	{
		if (TotalDamage > GetConVarFloat(l4d_stoptk_points_ban))
		{
			if (Punishment[client] < GetConVarFloat(l4d_stoptk_points_ban) && GetConVarInt(l4d_stoptk_bantype) > 0)
			{
				...
				if (GetConVarInt(l4d_stoptk_showmessages) > 0)
				{
					PrintToChatAll("\x04%N (\x01%s\x04) has been banned [\x01%d TK points!\x04]", client, ClientSteamID, RoundToZero(X * Damage[client]));
		...
		else if ((TotalDamage - Punishment[client]) > GetConVarFloat(l4d_stoptk_points_vote))
		{
			...
			if (GetConVarInt(l4d_stoptk_showmessages) > 0)
			{
				PrintToChatAll("\x01Autoban vote : %N (%d TK Points)", client, RoundToZero(Punishment[client])); 
			}

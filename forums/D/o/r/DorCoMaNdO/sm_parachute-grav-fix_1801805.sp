...
new Float:GravityCheck[MAXPLAYERS+1] = 1.0;
...
public StartPara(client,bool:open)
{
	...
	if((GetConVarInt(g_enabled)== 1 && hasPara[client]) || (GetConVarInt(g_enabled) == 1 && GetConVarInt(g_cost) == 0)){
		...
		if(velocity[2] < 0.0) {
			...
			if ( GetEntityGravity( client ) != 0.11 )
			{
				GravityCheck[client] = GetEntityGravity( client );
			}
			
			SetEntityGravity( client, 0.11 );
...
public EndPara(client)
{
	if(GetConVarInt(g_enabled)== 1 ){
		if ( GetEntityGravity( client ) == 0.11 )
		{
			SetEntityGravity(client, GravityCheck[client]);
		}
		inUse[client]=false;
		CloseParachute(client);
	}
...
public Check(client){
	if(GetConVarInt(g_enabled)== 1 ){
		...
		new cl_flags = GetEntityFlags(client);

...
public Plugin:myinfo = 
{
	...
	author = "Doodil, Original source by MoggieX",
...
//	LoadTranslations("common.phrases");
	...
	RegConsoleCmd("sm_adminsay", Command_SmAsay, "sm_adminsay <message> - sends a message to all admins");
	...
	RegAdminCmd("sm_allmsay", Command_SmAllMsay, ADMFLAG_CHAT, "sm_allmsay <message> - sends message to everyone on the server");
...
public Action:Command_SmAsay(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_adminsay <message> - sends message to admins as a menu panel");
		return Plugin_Handled;	
	}
...
// All Menu - sm_allmsay
//////////////////////////////////////////////////////////////////
public Action:Command_SmAllMsay(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_allmsay <message> - sends message to everyone as a menu panel");
		return Plugin_Handled;	
	}
...
SendPanelToTargets(String:name[], target, client, String:message[], identifier)
{
	...
	if(identifier == 1)	// Private Menu Message
	{
		...
		{
			decl String:recievername[64];
			GetClientName(target,recievername,sizeof(recievername));
			Format(title, 64, "To: %s \nFrom: %s(Admin):",recievername, name);
		}
	}
	if(identifier == 2)	// Admin Only Menu Message
	{
		Format(title, 64, "To: All Admins \nFrom: %s(Admin):", name);
	}

	if(identifier == 3)	// Non Admin Menu Message
	{
		Format(title, 64, "To: All None Admins \nFrom: %s(Admin):", name);
	}
	
	if(identifier == 4)	// Everyone Menu Message
	{
		Format(title, 64, "To: Everyone \nFrom: %s(Admin):", name);
	}
	
	if(identifier == 5)	// Everyone Menu Message
	{
		Format(title, 64, "To: All Admins \nFrom: %s:", name);
	}

	ReplaceString(message, 192, "\\n", "\n");
	//ReplaceString(title, 64, "\\n", "\n");
	...
	if(identifier == 4)	// All Menu Message
	{ 
		for(new i = 1; i < MaxClients; i++)
		{
		...
		LogAction(client, -1, "%L triggered sm_allmsay (text %s)", client, message);
	}
	
	if(identifier == 5)	// Admin Only Menu Message
	{
		for(new i = 1; i < MaxClients; i++)
		{
			if(IsClientInGame(i) && !IsFakeClient(i) && CheckCommandAccess(i, "sm_chat", ADMFLAG_CHAT))
		...
		LogAction(client, -1, "%L triggered sm_adminsay (text %s)", client, message);

...
new Handle:g_mode=INVALID_HANDLE; // Added by DDRKhat
...
public OnPluginStart()
{
	...
	g_mode = CreateConVar("l4d_md_mode", "3", "Gamemode in which this plugin applies. (1=Co-Op,2=Versus,3=Both)"); // Added by DDRKhat
...
{
	if(BlockMelee()) 	// Added by DDRKhat
	{			// This line (and everything beneath was indended an extra time) by DDRKhat
		...
		GetClientWeapon(plr_index, plr_weapon[plr_index], 255);
	} // This Line (and everything above was intended an extra time) by DDRKhat
}
...
// Below is Added by DDRKhat
BlockMelee()
{
	if(GetConVarInt(g_mode)==3) return true;
	else if(GetConVarInt(g_mode)==1&&GetConVarInt(versus)==1) return true;
	else if(GetConVarInt(g_mode)==2&&GetConVarInt(versus)==0) return true;
	else return false;
}
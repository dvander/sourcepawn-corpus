...
isbotsurvivor(any:client)
{
	if(!IsFakeClient(client)) return false;
	new String:arg[128];
	GetClientModel(client,arg,128);
	if(!FindCharInString(arg,'_',true)) return false;
...
public Action:NoSurvivorBot(Handle:timer, any:client)
{
	...
	if(isbotsurvivor(client)&&disallowBot&&Survivors()>4) {KickClient(client,"fake player");}

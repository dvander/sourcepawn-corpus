/***************************************************************************
	...
	FortressRadio SourceMod Plugin
...
new String:Adverts[8][2048] = {
	"Ingame Radio by datamann10.caster.fm",
    "Plugin coded by DJ Data!",
    "If stopping doesnt work, just type !website",
    "This plugin is currently very unstable, dont expect it to work everytime.",
...
public Plugin:myinfo = {
	name = "FortressRadio SourceMod Plugin",
	author = "DJ Data, Charlie - Coder",
	description = "FortressRadio SourceMod Plugin",
	version = PLUGINVERSION,
	url = "http://datamann10.caster.fm"
}
...
public OnPluginStart() {
	...
	g_hCvarStreamHost = CreateConVar("fr_stream_host", "http://184.173.181.32:22226/listen.mp3?authnc784ed99cfc1c51e452a27d6b6f36cc9", "Sets the website hostname used to send and retrieve info.", FCVAR_PLUGIN);
	...
	g_hCvarStreamStatsPath = CreateConVar("fr_stream_stats_path", "http://184.173.181.32:22226/listen.mp3?authnc784ed99cfc1c51e452a27d6b6f36cc9", "Website path used for getting stream info.", FCVAR_PLUGIN);
	HookConVarChange(g_hCvarStreamStatsPath, Cvar_Changed);
	
	g_hCvarWebPlayerScript = CreateConVar("fr_webplayer_script", "http://184.173.181.32:22226/listen.mp3?authnc784ed99cfc1c51e452a27d6b6f36cc9", "Website script used for the web player.", FCVAR_PLUGIN);
...
public Action:WelcomeAdvert(any:client) {
	PrintToChat(client, "\x04[FortressRadio]\x01 To listen to the radio type !radio in chat!");
}

public Action:Advertise(Handle:timer) {
	if (AdsIndex == 9) {
		AdsIndex = 0;
		PrintToChatAll("\x04[FortressRadio]\x01 %s", Adverts[AdsIndex]);
		AdsIndex++;
	} else {
		PrintToChatAll("\x04[FortressRadio]\x01 %s", Adverts[AdsIndex]);
		AdsIndex++;
	...
	if (PluginEnabled) {
		...
		for (new i=1; i<= MaxClients; i++) {
			if (IsClientConnected(i) && IsClientInGame(i)) {
				if (isTuned[i]) {
					StreamPanel("Thanks for tuning into FortressRadio!", "about:blank", i);
					
					decl String:url[256];
					FormatEx(url, sizeof(url), "http://%s/%s?vol=%s", StreamHost, WebPlayerScript, tunedVol[i]);
					StreamPanel("You are tuned into FortressRadio", url, i);
					
					PrintToChat(i, "\x04[FortressRadio]\x01 The FortressRadio plugin has been enabled, you have been auto tuned in.");
				}
			}
		}
	} else {
		...
		for (new i=1; i<= MaxClients; i++) {
			if (IsClientConnected(i) && IsClientInGame(i)) {
				if (isTuned[i]) {
					StreamPanel("Thanks for tuning into FortressRadio!", "about:blank", i);
					PrintToChat(i, "\x04[FortressRadio!]\x01 FortressRadio has been disabled, you have been auto tuned out.");
				}
			}
		}
	}
...
public Cmd_Check(String:type[], client) {
	if (client > 0 && client <= MaxClients && IsClientInGame(client) && !IsFakeClient(client)) {
		if (!PluginEnabled) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 FortressRadio has been disabled.");
			return false;
		}
		
		if (StrEqual(type, "streaminfo") && !GetStreamInfo) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Stream info gathering has been disabled.");
			return false;
		} else if (StrEqual(type, "interact") && !InteractEnabled) {
			ReplyToCommand(client, "\x04[FortressRadio!\x01 Interaction commands have been disabled.");
			return false;
		} else if (StrEqual(type, "rating") && !RatingEnabled) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Rating commands have been disabled.");
			return false;
		}
		
		if (!StrEqual(type, "streaminfo") && isTuned[client] != true) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 You are not tuned in, type !radio to tune in!");
			return false;
		}
	} else {
		ReplyToCommand(client, "[FortressRadio!] That command can only be used by clients!");
		return false;
	}
...
public Action:Cmd_ShowDJ(client, args) {
	if (Cmd_Check("streaminfo", client)) {		
		if (GetStreamInfo && !StrEqual(g_sDJ, "")) {
			ReplyToCommand(client, "\x04[FortressRadio!] Current DJ:\x01 %s", g_sDJ);
		} else {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 No stream info has been received yet!");
		}
	}
...
public Action:Cmd_ShowSong(client, args) {
	if (Cmd_Check("streaminfo", client)) {		
		if (GetStreamInfo && !StrEqual(g_sSong, "")) {
			ReplyToCommand(client, "\x04[FortressRadio!] Current Song:\x01 %s", g_sSong);
		} else {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 No stream info has been received yet!");
		}
	}
	...
	if (action == MenuAction_Select) {
		...
		if (found) {
			if (StringToInt(info) == 0) {
				...
				StreamPanel("Thanks for tuning into FortressRadio!", "about:blank", client);
				PrintToChat(client, "\x04[FortressRadio!]\x01 Thanks for tuning into FortressRadio!");
			} else {
				if (isTuned[client] != true) {
					decl String:name[128];
					GetClientName(client, name, sizeof(name));
					PrintToChatAll("\x04[FortressRadio!]\x01 %s has tuned into the radio by typing !radio", name);
				}
				...
				StreamPanel("You are tuned into FortressRadio!", url, client);
			}
		}
...
public Action:Cmd_RadioMenu(client, args) {
	if (!PluginEnabled) {
		ReplyToCommand(client, "\x04[FortressRadio!]\x01 The FortressRadio Plugin has been disabled.");
		return Plugin_Handled;
	}
	...
	SetMenuTitle(menu, "datamann10.caster.fm - Menu\n \nDJ: %s\nSong: %s\n \n", g_sDJ, g_sSong);
...
public Action:Cmd_Request(client, args) {
	if (Cmd_Check("interact", client)) {	
		if (args < 1) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Usage: !request <song>");
			return Plugin_Handled;
		}
		...
		if (strlen(request) < 8) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Requests must be more than 8 characters.");
			return Plugin_Handled;
		} else if (strlen(request) > 255) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Requests must be less than 255 characters.");
			return Plugin_Handled;
		}
		
		if (GetStreamInfo && !StrEqual(g_sDJ, "")) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Your song request has been sent to %s", g_sDJ);
		} else {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Your song request has been sent!");
		}
...
public Action:Cmd_Shoutout(client, args) {
	if (Cmd_Check("interact", client)) {		
		if (args < 1) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Usage: !shoutout <message>");
			return Plugin_Handled;
		}
		...
		if (StrEqual(g_sDJ, "AutoDJ")) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Shoutouts cannot be made when no one is onair");
			return Plugin_Handled;
		}
		
		if (strlen(shoutout) < 8) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Shoutouts must be more than 8 characters.");
			return Plugin_Handled;
		} else if (strlen(shoutout) > 255) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Shoutouts must be less than 255 characters.");
			return Plugin_Handled;
		}
		
		if (GetStreamInfo && !StrEqual(g_sDJ, "")) {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Your shoutout has been sent to %s", g_sDJ);
		} else {
			ReplyToCommand(client, "\x04[FortressRadio!]\x01 Your shoutout has been sent!");
		}
...
public Action:Cmd_Choon(client, args) {
	if (Cmd_Check("rating", client)) {		
		...
		if (GetStreamInfo && !StrEqual(g_sSong, "")) {
			PrintToChatAll("\x04[FortressRadio!]\x01 %s thinks %s is AWESOME!", name, g_sSong);
		} else {
			PrintToChatAll("\x04[FortressRadio!]\x01 %s thinks the current song is AWESOME!", name);
		}
...
public Action:Cmd_Poon(client, args) {
	if (Cmd_Check("rating", client)) {		
		...
		if (GetStreamInfo && !StrEqual(g_sSong, "")) {
			PrintToChatAll("\x04[FortressRadio!]\x01 %s thinks %s is really BAD!", name, g_sSong);
		} else {
			PrintToChatAll("\x04[FortressRadio!]\x01 %s thinks the current song is really BAD!", name);
		}
...
public Action:Cmd_djFTW(client, args) {
	if (Cmd_Check("rating", client)) {		
		...
		if (GetStreamInfo && !StrEqual(g_sDJ, "")) {
			PrintToChatAll("\x04[FortressRadio!]\x01 %s thinks %s is Awesome!", name, g_sDJ);
		} else {
			PrintToChatAll("\x04[FortressRadio!]\x01 %s thinks the current DJ is Awesome!", name);
		}
...
public Action:Cmd_djFTL(client, args) {
	if (Cmd_Check("rating", client)) {		
		...
		if (GetStreamInfo && !StrEqual(g_sDJ, "")) {
			PrintToChatAll("\x04[FortressRadio!]\x01 %s thinks %s is crap!", name, g_sDJ);
		} else {
			PrintToChatAll("\x04[FortressRadio!]\x01 %s thinks the current DJ is crap!", name);
		}
...
public OnSocketDisconnected(Handle:socket, any:dp) {
	...
	if (StrEqual(type, "streaminfo")) {		
		...
		if (pos > 0) {
			...
			new String:file[512];
			BuildPath(Path_SM, file, 512, "configs/FortressRadio_stream_info.txt");
			...
			if (KvJumpToKey(Info, "FortressRadio!"))	{
				decl String:dj[512];
				...
				if(!StrEqual(dj, g_sDJ)) {
					g_sDJ = dj;
					PrintToChatAll("\x04[FortressRadio!] Now Presenting:\x01 %s", g_sDJ);
				}

				if(!StrEqual(song, g_sSong)) {
					g_sSong = song;
					PrintToChatAll("\x04[FortressRadio!] Now Playing:\x01 %s", g_sSong);
				}
			}
...
public OnSocketError(Handle:socket, const errorType, const errorNum, any:dp) {
	LogError("[FortressRadio] Socket error %d (errno %d)", errorType, errorNum);

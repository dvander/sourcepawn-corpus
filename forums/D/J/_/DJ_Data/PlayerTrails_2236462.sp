...
public OnMapStart()
{
	...
	// read through materials/sprites/store/trails/ directory and add custom models
		...
		while (ReadDirEntry(dir, buffer, PLATFORM_MAX_PATH, type))
		{
			...
			if (!StrEqual(buffer,"",false) && !StrEqual(buffer,".",false) && !StrEqual(buffer,"..",false))
			{
				if (type == FileType_File)
				{
					...
					{
						Format(pathName, PLATFORM_MAX_PATH, "materials/sprites/store/trails/%s", buffer);
						...
						Format(buffer, sizeof(buffer), "materials/sprites/store/trails/%s.vmt", trailName);
						AddFileToDownloadsTable(buffer);
						Format(buffer, sizeof(buffer), "materials/sprites/store/trails/%s.vtf", trailName);
						AddFileToDownloadsTable(buffer);
					}
				}
			}
...
public OnPluginStart()
{
	...
	gH_Cvar_TrailsEnabled = CreateConVar("sm_playertrails_enable", "1", "Enables/Disables Player Trails", FCVAR_PLUGIN, true, 0.0, true, 1.0);

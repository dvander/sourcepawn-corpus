...
new g_b_WitchControl[MAXPLAYERS+1], Float:g_PlayerGameTime[MAXPLAYERS+1], bool:g_b_OnLadder[MAXPLAYERS+1],
	...
	Handle:h_CvarWitchSpeed, Handle:h_CvarMode, Handle:h_CvarMessageType, Handle:g_h_CreateAbility, i_RefEHandleOffset, Handle:h_CvarAbilityDuration
...
public OnPluginStart()
{
	...
	if (g_h_GameConfig != INVALID_HANDLE)
	{
		...
		PrepSDKCall_SetFromConf(g_h_GameConfig, SDKConf_Signature, "CreateAbility")
		PrepSDKCall_AddParameter(SDKType_CBasePlayer, SDKPass_Pointer)
		PrepSDKCall_SetReturnInfo(SDKType_CBaseEntity, SDKPass_Pointer)
		g_h_CreateAbility = EndPrepSDKCall()
		
		i_RefEHandleOffset = GameConfGetOffset(g_h_GameConfig, "RefEHandleOffset")
	}
	...
	h_CvarAbilityDuration = CreateConVar("l4d2_witch_ability_duration", "20.0", "Ability duration", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 1.0, true, 50.0)
...
public Action:SetWitchControl(Handle:h_Timer, Handle:h_Pack)
{
	...
	SDKCall(g_h_SetClass, i_Client, CLASS_CHARGER)
...
public Action:ChangeClass(Handle:h_Timer, any:i_Client)
{
	if (IsClientInGame(i_Client))
	{
		...
		SetEntProp(i_Client, Prop_Send, "m_customAbility", GetEntData(SDKCall(g_h_CreateAbility, i_Client), i_RefEHandleOffset))
...
{
	decl i_Ability, Float:f_Duration
	
	i_Ability = MakeCompatEntRef(GetEntProp(i_Client, Prop_Send, "m_customAbility"))
	f_Duration = GetConVarFloat(h_CvarAbilityDuration)
	
	if (GetEntPropFloat(i_Ability, Prop_Send, "m_timestamp") <= GetGameTime())
	{
		SetEntPropFloat(i_Witch, Prop_Send, "m_rage", 1.0)
		SetEntPropFloat(i_Ability, Prop_Send, "m_timestamp", GetGameTime() + f_Duration)
		SetEntPropFloat(i_Ability, Prop_Send, "m_duration", f_Duration)
	}
}

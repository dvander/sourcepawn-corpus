...
public Action:smGiveRandomWeapon(client, args)
{
	...
	if (target != -1 && IsPlayerAlive(target))
	{
		...
		GivePlayerItem(target, g_weapons[random]);
		ReplyToCommand(client, "* You have given the weapon %s to %N.", g_weapons[random], target);
	}
	else
	{
		ReplyToCommand(client, "* Couldn't find target %s or target isn't alive.", starget);
	}

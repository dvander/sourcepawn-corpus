...
new Handle:cvar_timedtrial;
new Handle:cvar_firearrows;
new Handle:cvar_timedtrial_length;
...
new Handle:pEndGlow[MAXPLAYERS+1];
...
new Handle:phourlimit = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	cvar_timedtrial = CreateConVar("premium_timedtrial_enable", "1", "use the timed trial instead");
	cvar_trial_length = CreateConVar("premium_trial_length", "900", "Time in seconds for trials");
	cvar_metal = CreateConVar("premium_metal", "100", "Amount of metal to spawn with");
	cvar_timedtrial_length = CreateConVar("premium_trial_hourlimit", "24", "when users can reuse the plugin again");
	//////////////
	//O t h e r//
	////////////
	...
	phourlimit = RegClientCookie("Timelimit", "keep check of the clients time", CookieAccess_Protected);
	AutoExecConfig();
...
public OnGameFrame()
{
	...
	{
		if( IsValidEntity(i) && (GetUserFlagBits(i) & PREMIUMFLAG || pTrial[i] || GetUserFlagBits(i) & ROOTFLAG)  && IsClientInGame(i) && IsPlayerAlive(i) )
		{
...
ReloadConvars()
{
	...
	cTimedTrial = GetConVarInt(cvar_timedtrial);
	cTimedLength = (GetConVarInt(cvar_timedtrial_length) * 60 *60);
	mySQL_connect();
}

...
public OnPluginStart()
{
	...
	CreateConVar("sm_respawn_version", "1.6", "Player Respawn Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
void RespawnPlayer(client, target, bool teleport = true)
{
	if (client > 0)
	{
		if (target == 0 || !IsClientInGame(target))
		{
			PrintToChat(client, "Target is no longer available to respawn.");
			return;
		}
		
		if (!IsPlayerAlive(target))
		{
			PrintToChat(client, "%N is no longer dead.", target);
			return;
		}
	}
	...
	if (teleport && client > 0)
	{
		DataPack pack = new DataPack();
		pack.WriteCell(GetClientUserId(client));
		pack.WriteCell(GetClientUserId(target));
		
		RequestFrame(Frame_TeleportToCrosshair, pack);
	}
}

public void Frame_TeleportToCrosshair(DataPack data)
{
	data.Reset();
	...
	float vecOrigin[3];
	if (target > 0 && IsClientInGame(target) && IsPlayerAlive(target) && GetClientCrosshairOrigin(client, vecOrigin))
		TeleportEntity(target, vecOrigin, NULL_VECTOR, NULL_VECTOR);
}

bool GetClientCrosshairOrigin(int pClient, float pOrigin[3], float pDistance = -35.0)
{
	if (pClient == 0 || pClient > MaxClients || !IsClientInGame(pClient))
		return false;

	float vOrigin[3];
	GetClientEyePosition(pClient,vOrigin);

	float vAngles[3];
	GetClientEyeAngles(pClient, vAngles);
	...
	if (bReturn)
	{
		...
		pOrigin[0] = vStart[0] + (vBuffer[0] * pDistance);
		pOrigin[1] = vStart[1] + (vBuffer[1] * pDistance);
		pOrigin[2] = vStart[2] + (vBuffer[2] * pDistance);
	}

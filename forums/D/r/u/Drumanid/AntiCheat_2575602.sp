...
#define PLUGIN_AUTHOR "CodingCow (rework: Drumanid)"
...
public void OnPluginStart()
{
	...
	AutoExecConfig_SetFile("AntiCheat", "AntiCheat");
	...
	RegConsoleCmd("sm_AntiCheat_version", printVersion);
...
{
	PrintToChat(client, "[\x02AntiCheat\x01] Version: %s", PLUGIN_VERSION);
}
...
public Action getBhop(int client, int args)
{
	...
	{
		PrintToChat(client, "[\x02AntiCheat\x01] Not a valid target!");
		return Plugin_Handled;
	}
	
	PrintToChat(client, "[\x02AntiCheat\x01] See console for output.");
...
public Action Event_BombDefused(Handle event, const char[] name, bool dontBroadcast )
{
    ...
    {
    	PrintToChatAll("[\x02AntiCheat\x01] \x0E%N \x01has been detected for Instant Defuse!", client);
    	ServerCommand("sm_ban \"%N\" \"%i\" \"[AntiCheat] Instant Defuse\"", client, g_ConVar_InstantDefuseBanTime.IntValue);
    }
}
...
public Action OnPlayerRunCmd(int client, int &iButtons, int &iImpulse, float fVelocity[3], float fAngles[3], int &iWeapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
	if(!IsFakeClient(client) && IsValidClient(client) && IsPlayerAlive(client))
	{
		...
		if(g_ConVar_BhopEnable.BoolValue && !g_ConVar_AutoBhop.BoolValue)
...
public void CheckAimbot(int client, int buttons, float angles[3])
{
	...
	delete trace;
  	...
  	{
  		PrintToChatAll("[\x02AntiCheat\x01] \x0E%N \x01has been detected for Aimbot!", client);
		ServerCommand("sm_ban \"%N\" \"%i\" \"[AntiCheat] Aimbot\"", client, g_ConVar_AimbotBanTime.IntValue);
...
public void CheckBhop(int client, int buttons)
{
	...
	{
		PrintToChatAll("[\x02AntiCheat\x01] \x0E%N \x01has been detected for Bhop Assist!", client);
		ServerCommand("sm_ban \"%N\" \"%i\" \"[AntiCheat] Bhop Assist\"", client, g_ConVar_BhopBanTime.IntValue);
...
public void CheckSidemoveCount(int client)
{
	...
	{
		PrintToChatAll("[\x02AntiCheat\x01] \x0E%N \x01has been detected for Silent-Strafe!", client);
		ServerCommand("sm_ban \"%N\" \"%i\" \"[AntiCheat] Silent-Strafe\"", client, g_ConVar_SilentStrafeBanTime.IntValue);
...
public void CheckTriggerBot(int client, int buttons, float angles[3])
{
	...
	if (TR_DidHit(trace))
	{
		...
		if (target > 0 && target <= MaxClients && GetClientTeam(target) != GetClientTeam(client) && IsPlayerAlive(target) && IsPlayerAlive(client) && !g_bShootSpam[client])
		{
			...
			else if(buttons & IN_ATTACK && prev_buttons[client] & IN_ATTACK && g_iTicksOnPlayer[client] == 1)
			{
				...
				{
					char message[128];
					Format(message, sizeof(message), "[\x02AntiCheat\x01] \x0E%N \x01detected for \x10%i\x01 tick perfect shots.", client, g_iTriggerBotCount[client]);
	...
	delete trace;
  	...
  	{
  		PrintToChatAll("[\x02AntiCheat\x01] \x0E%N \x01has been detected for TriggerBot / Smooth Aimbot!", client);
		ServerCommand("sm_ban \"%N\" \"%i\" \"[AntiCheat] TriggerBot / Smooth Aimbot\"", client, g_ConVar_TriggerbotBanTime.IntValue);
...
public void CheckMacro(int client, int buttons)
{
	...
	else if(GetEntityFlags(client) & FL_ONGROUND)
	{
		...
		{
			char message[128];
			Format(message, sizeof(message), "[\x02AntiCheat\x01] \x0E%N \x01has been detected for Macro / Hyperscroll (\x04%i\x01)!", client, g_iMacroCount[client]);
			PrintToAdmins(message, "b");
			...
			{
				KickClient(client, "[AntiCheat] Turn off Bhop Assistance!");
...
{	
	if(buttons & IN_ATTACK && !(prev_buttons[client] & IN_ATTACK))
	{
		...
		else
		{
			...
			{
				char message[128];
				Format(message, sizeof(message), "[\x02AntiCheat\x01] \x0E%N \x01has been detected for AutoShoot Script (\x04%i\x01)!", client, g_iAutoShoot[client]);
				PrintToAdmins(message, "b");
			}
...
public void CheckWallTrace(int client, float angles[3])
{
	...
	{
		PrintToChatAll("[\x02AntiCheat\x01] \x0E%N \x01detected for WallTracing.", client);
		g_iWallTrace[client] = 0;
	}
}
...
public void CheckPerfectStrafe(int client, int mousedx, int buttons)
{
	if(mousedx > 0 && turnRight[client])
	{
		...
		else
		{
			...
			{
				char message[128];
				Format(message, sizeof(message), "[\x02AntiCheat\x01] \x0E%N \x01detected for \x10%i\x01 Consistant Perfect Strafes.", client, g_iStrafeCount[client]);
				PrintToAdmins(message, "b");
			}
...
public void CheckPerfCount(int client)
{
	...
	{
		PrintToChatAll("[\x02AntiCheat\x01] \x0E%N \x01has been detected for Consistant Perfect Strafes (%i)!", client, g_iStrafeCount[client]);
		ServerCommand("sm_ban \"%N\" \"%i\" \"[AntiCheat] Consistant Perfect Strafes\"", client, g_ConVar_PerfectStrafeBanTime.IntValue);
...
public void CheckAHKStrafe(int client, int mouse)
{
	...
	if(!(GetEntityFlags(client) & FL_ONGROUND))
	{
		if((mouse >= 10 || mouse <= -10) && g_fJumpPos[client] < vec[2])
		{
			...
			{
				char message[128];
				Format(message, sizeof(message), "[\x02AntiCheat\x01] \x0E%N \x01detected for AHK Strafe", client);
				PrintToAdmins(message, "b");
				g_iMousedxCount[client] = 0;
			}
		}
...
{ 
    ...
    { 
        ...
        else if (IsValidClient(x))
        {
       		PrintToCow(x, message);
      	}
    } 
}

stock void PrintToCow(int client, const char[] message) 
{
	char steamid[64];

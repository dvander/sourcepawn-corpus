...
#define PLUGIN_VERSION "1.0.0.4M"
...
new Handle:cvAdminMines = INVALID_HANDLE;

public Plugin:myinfo = {
	name = "Tripmines",
	author = "L. Duke (edited by Dr. McKay)",
...
public OnPluginStart() 
{
  ...
  cvAdminMines = CreateConVar("sm_tripmines_allowed_admin", "3");

  // commands
  RegConsoleCmd("sm_tripmine", Command_TripMine);
  RegAdminCmd("sm_setmines", Command_SetTripmines, ADMFLAG_SLAY, "Allows an admin set a player's number of tripmines");
...
public Action:PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(CheckCommandAccess(client, "ExtraTripmines", ADMFLAG_SLAY))
		gRemaining[client] = GetConVarInt(cvAdminMines);
...
public Action:Command_SetTripmines(client, args)
{
	if(args != 2)
	{
		ReplyToCommand(client, "\x04[SM] \x01Usage: sm_setmines [target] [count]");
		return Plugin_Handled;
	}
	...
	decl String:mines[32];
	GetCmdArg(2, mines, sizeof(mines));
	new iMines = StringToInt(mines);
	if(iMines < 0)
		iMines = 0;
	
	ReplyToCommand(client,"\x04[SM] \x01You set the number of tripmines on %s to %i", target_name, iMines);
	
	for(new i = 0; i < target_count; i ++)
	{
		gRemaining[target_list[i]] = iMines;
		PrintHintText(target_list[i], "An admin set your number of tripmines to %i", iMines);
	}
	return Plugin_Handled;
}

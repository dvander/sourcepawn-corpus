...
new g_bGroupsToCheck[MAXPLAYERS + 1] = { 0, ...} ;
...
public Action:OnClientPreAdminCheck(client)
{
	...
	g_bGroupsToCheck[client] = 0;
	LookupPlayerGroups(client);
	
	CreateTimer(5.0, Timer_Notify, GetClientUserId(client)); // Steam sometimes refuses to respond to requests if the user uses retry, so we create a timer to make sure NotifyPostAdminCheck is called eventually
	return Plugin_Handled;
}

public Action:Timer_Notify(Handle:timer, any:userid)
{

...
public Action:OnSayText2(UserMsg:msg_id, Handle:bf, const clients[], numClients, bool:reliable, bool:init)
{
	...
	*/
	...
	/**
	If this message was just processed (and the recipients array was modified), suppress duplicates.
	Perhaps a better approach would be to collect all identical SayText2 messages in a frame and bundle them into one forward call,
	but I'm too lazy to do that right now.
	*/
	static lastMessageAuthor, lastMessageTickCount;
	if (lastMessageAuthor == cpSender && lastMessageTickCount == GetGameTickCount())
	{
		return Plugin_Handled;
	}
	...
	*/
	...
	/**
	Check if the cpRecipients array was modified
	*/
	if (WasRecipientsModified(clients, numClients, cpRecipients))
	{
		lastMessageAuthor = cpSender;
		lastMessageTickCount = GetGameTickCount();
	}
...
public Action:OnSayText(UserMsg:msg_id, Handle:bf, const clients[], numClients, bool:reliable, bool:init)
{
	...
	*/
	...
	/**
	If this message was just processed (and the recipients array was modified), suppress duplicates.
	Perhaps a better approach would be to collect all identical SayText messages in a frame and bundle them into one forward call,
	but I'm too lazy to do that right now.
	*/
	static lastMessageAuthor, lastMessageTickCount;
	if (lastMessageAuthor == cpSender && lastMessageTickCount == GetGameTickCount())
...
bool:WasRecipientsModified(const clients[], numClients, Handle:recipients)
{
	new size = GetArraySize(recipients);
	if (numClients != GetArraySize(recipients))
	{
		return true;
	}
	...
	{
		if (GetArrayCell(recipients, i) != clients[i])
		{
			return true;
		}
	}

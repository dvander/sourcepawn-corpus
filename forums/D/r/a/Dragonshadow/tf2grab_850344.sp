...
public Plugin:myinfo = {
	name = "Grab:TF2",
	author = "Dragonshadow",
	description = "Grab engineer buildings and move them about",
...
public OnPluginStart() 
{
	...
	cvThrowSound = CreateConVar("sm_grab_throwsound", "weapons/physcannon/superphys_launch1.wav", "sound to play for fling, change takes effect on map change", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	cvThrowTime = CreateConVar("sm_grab_flingcharge", "2.0", "Time to charge fling to full (default 2.0)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	cvThrowSpeed = CreateConVar("sm_grab_flingspeed", "1000.0", "speed at which an object is flung. (default 1000)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	cvSteal = CreateConVar("sm_grab_theif", "1", "Can buildings be stolen from another player who is holding it? (1/0 = yes/no)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	cvThrowBuildings = CreateConVar("sm_grab_fling", "1", "can buildings be flung? (1/0 = yes/no)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	cvThrowSetDisabled = CreateConVar("sm_grab_flingdisable", "1", "set buildings disabled when flung? (1/0 = yes/no)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	cvThrowGravity = CreateConVar("sm_grab_flinggrav", "1.0", "gravity of flung buildings (default 1.0)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	cvMovetype = CreateConVar("sm_grab_movetype", "1", "change the movetype of grabbed/flung objects (1/0 = yes/no)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	g_hEnabled = CreateConVar("sm_grab_shotenable", "1", "Enable/Disable Engi Shotgun Alt-Fire Support ((1/0 = yes/no))", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	RegConsoleCmd("+fling", Command_Throw);
	RegConsoleCmd("-fling", Command_UnThrow);
...
public OnGameFrame() 
{
	...
	if(g_bEnabled) 
	{
		...
		{
			if(IsClientConnected(i) && IsClientInGame(i) && IsPlayerAlive(i) && _:TF2_GetPlayerClass(i)==9) 
			{
				GetClientWeapon(i, wpn, sizeof(wpn));
				if(StrEqual(wpn, "tf_weapon_shotgun_primary")) 
				{
...
public Action:TrackObject(Handle:timer, Handle:pack)
{
	...
	if (GetGrabType(ent) == gt)
	{
		...
		new bool:disable = gDisabled[ent];
		if (disable && (gt >= dispenser && gt <= sentrygun) &&
				(vecVel[0] >= negStopSpeed && vecVel[0] <= stopSpeed) &&
		...
		else
		{
			...
			if (gt >= dispenser && gt <= sentrygun)
			{
				if (disable && !GetEntProp(ent, Prop_Send, "m_bHasSapper"))
				SetEntProp(ent, Prop_Send, "m_bDisabled", 0);
			}

#if defined(CHANGE_PARENT_AND_CRASH_SERVER)
			...
			vecCheckBelow[2] = vecPos[2] - 50.0;

			TR_TraceRayFilter(vecPos, vecCheckBelow, MASK_PLAYERSOLID, RayType_EndPoint, TraceRayDontHitSelf, ent);

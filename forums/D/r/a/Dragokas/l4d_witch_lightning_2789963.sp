...
ConVar l4d_witch_lightning_enable_on_expert; 
...
/*
	...
	1.8.2
	 - Changed precache particle method
	 - Changed parenting method
...
public void OnPluginStart()
{
	...
	l4d_witch_lightning_enable_on_expert = CreateConVar("l4d_witch_enable_on_expert", "0", "enable plugin on expert difficulty?");
	...
	HookConVarChange(g_ConVarDifficulty,	ConVarDifficultyChanged);
...
public void ConVarDifficultyChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
	RefreshDifficulty();
}
...
public Action witch_spawn(Event h_Event, const char[] s_Name, bool b_DontBroadcast)
{
	...
	if( !l4d_witch_lightning_enable_on_expert.BoolValue && g_bExpert )
	{
		return;
	}
...
{   
	...
	{
		if(L4D2Version) {
			ent = CreateEntityByName("info_particle_target"); 
			DispatchSpawn(ent);  			
		...
		SetVariantString("!activator");
		AcceptEntityInput(particle, "SetParent",witch, witch, 0);   
...
{ 
	...
	for(int i=1; i<=MaxClients; i++)
	{
		if(IsClientInGame(i) && GetClientTeam(i)==2 && IsPlayerAlive(i) )
		{
			...
			dis=GetVectorDistance(playerPos, witchPos);  
...
void DoPointHurtForInfected(int victim, int attacker = 0, float FireDamage)
{
	...
	FormatEx(N, sizeof(N), "target%d", victim);
...
{
	PrecacheParticleEffect(Particle_electrical_arc_01_system);
	PrecacheParticleEffect(Particle_st_elmos_fire);

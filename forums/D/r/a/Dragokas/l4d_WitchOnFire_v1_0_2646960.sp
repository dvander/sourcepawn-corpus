...
public void OnPluginStart()
{
	...
	//HookConVarChange(g_ConVarIntervalMin,	ConVarChanged);
	//HookConVarChange(g_ConVarIntervalMax,	ConVarChanged);
}
...
void GetCvars()
{
	...
	/*
	int iMaxWitches = 1 + RoundToFloor(((g_ConVarFlameLifeTime.FloatValue - 1.0) - g_ConVarDelayMax.FloatValue) / g_ConVarIntervalMax.FloatValue);
	
	if (g_ConVarWitchCount.IntValue > iMaxWitches) { // decrease the number of witches if all of them has no time to fit in flame ignite time
		g_ConVarWitchCount.SetInt(iMaxWitches);
	}
	*/
}
...
public void OnNextInfernoFrame(int entity)
{
	...
	if (hOwner > 0) {
		char sClass[32];
		GetEntityClassname(hOwner, sClass, sizeof(sClass));
		
		PrintToChatAll("Inferno owner is: %i - %s", hOwner, sClass);
	}
}
...
public void OnTouch(int entity, int other)
{
	if (!other)
    {
		...
		GetEntPropVector(entity, Prop_Data, "m_vecOrigin", vecMolotovExplode);

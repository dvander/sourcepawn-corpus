...
1.8.1 (fork by Dragokas)
	...
	- PARTICLE_LIGHT1 and PARTICLE_LIGHT2 removed when L41 is run (these particles are absent).
...
void GetSkyBoxes()
{
	...
	/*
	StringMapSnapshot hSnap = hMapSky.Snapshot();
...
{
	char sPath[PLATFORM_MAX_PATH];
	Format(sPath, sizeof(sPath), "materials/skybox/%sbk.vmt", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
	Format(sPath, sizeof(sPath), "materials/skybox/%sbk.vtf", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
	Format(sPath, sizeof(sPath), "materials/skybox/%sdn.vmt", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
	Format(sPath, sizeof(sPath), "materials/skybox/%sdn.vtf", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
	Format(sPath, sizeof(sPath), "materials/skybox/%sft.vmt", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
	Format(sPath, sizeof(sPath), "materials/skybox/%sft.vtf", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
	Format(sPath, sizeof(sPath), "materials/skybox/%slf.vmt", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
	Format(sPath, sizeof(sPath), "materials/skybox/%slf.vtf", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
	Format(sPath, sizeof(sPath), "materials/skybox/%srt.vmt", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
	Format(sPath, sizeof(sPath), "materials/skybox/%srt.vtf", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
	Format(sPath, sizeof(sPath), "materials/skybox/%sup.vmt", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
	Format(sPath, sizeof(sPath), "materials/skybox/%sup.vtf", sSkybox);
	if (FileExists(sPath)) AddFileToDownloadsTable(sPath);
}
*/
...
void ChangeLightStyle(char[] lightstring, int client = 0)
{
	...
	{
		int time = GetTime();
		int hours = (time / 3600) % 24 + 2; // UTC+2 (Kyiv)

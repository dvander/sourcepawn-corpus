...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	...
	if (buttons & IN_USE && (GetEngineTime() - fTime[client] > 0.5))
	{
		...
		{
			PrintToChat(client, "\x04 %t", "GetOffInfected"); // "Get that Infected off you first."
			return Plugin_Continue;
		}
		...
		if (g_bForbidInReviving) {
			...
			{
				PrintToChat(client, "\x04 %t", "AlreadyReviving"); // "You're being revived already."
				return Plugin_Continue;
			}
		}
		...
		{
			PrintToChat(client, "\x04 %t", g_bLeft4dead2 ? "NoPills2" : "NoPills"); // "You aint got no Pills."
			return Plugin_Continue;
		}
		else
		{
			CPrintToChatAllExclude(client, "\x04%N\x01 %t", client, IsPills(PillSlot) ? "UsedPills" : "UsedAdrenaline"); // "%N used his pills and revived himself!"
...
void AdjustHealth(int client)
{
	...
	int count = GetEntData(client, g_iIncapCountOffset, 1);
	count++;
	...
	SetEntData(client, g_iIncapCountOffset, count, 1);
...
public Action Timer_SetHP1(Handle timer, any UserId)
{
...
public Action Timer_AdvertisePills(Handle timer, any UserId)
{
	...
	{
		PrintToChat(client, "\x01 %t", IsPills(PillSlot) ? "HintAboutPills" : "HintAboutAdrenaline"); // "You have Pills, you can now press \x04USE \x01to pop them and stand back up by yourself"
	}
}

...
{	
	...
	g_hCvarCloudEnabled = CreateConVar(			"l4d_cloud_damage_enabled", 	"1", 	"Enable/Disable the Cloud Damage plugin", CVAR_FLAGS);
	g_hCvarModes =	CreateConVar(				"l4d_cloud_modes",				"",		"Turn on the plugin in these game modes, separate by commas (no spaces). (Empty = all).", CVAR_FLAGS );
	g_hCvarModesOff = CreateConVar(				"l4d_cloud_modes_off",			"",		"Turn off the plugin in these game modes, separate by commas (no spaces). (Empty = none).", CVAR_FLAGS );
	g_hCvarModesTog = CreateConVar(				"l4d_cloud_modes_tog",			"0",	"Turn on the plugin in these game modes. 0=All, 1=Coop, 2=Survival, 4=Versus, 8=Scavenge. Add numbers together.", CVAR_FLAGS );
	g_hCvarCloudDamage = CreateConVar(			"l4d_cloud_damage_damage", 		"5.0", 	"Amount of damage the cloud deals every 1 seconds", CVAR_FLAGS);
	g_hCvarCloudDuration = CreateConVar(		"l4d_cloud_damage_time", 		"17.0", "How long the cloud damage persists", CVAR_FLAGS);
	g_hCvarCloudRadius = CreateConVar(			"l4d_cloud_damage_radius", 		"250", 	"Radius of gas cloud damage", CVAR_FLAGS);
	g_hCvarCloudMeleeSlowEnabled = CreateConVar("l4d_cloud_meleeslow_enabled", 	"1", 	"Enable/Disable the Cloud Melee Slow Effect", CVAR_FLAGS);
	g_hCvarCloudShake = CreateConVar(			"l4d_cloud_shake_enabled", 		"1", 	"Enable/Disable the Cloud Damage Shake", CVAR_FLAGS);
	g_hCvarCloudBlocksRevive = CreateConVar(	"l4d_cloud_blocks_revive", 		"0", 	"Enable/Disable the Cloud Damage Stopping Reviving", CVAR_FLAGS);
	g_hCvarCloudDamageIngame = CreateConVar(	"l4d_cloud_damage_ingame", 		"0", 	"Damage client only when smoker still in-game (usually ~ 5 sec after kill) (1 - Yes, 0 - while cloud lifetime)", CVAR_FLAGS);
...
public Action HookSound_Callback(int[] clients, int& numClients, char[] sample, int& entity, int& channel, float& volume, int& level, int& pitch, int& flags, char[] soundEntry, int& seed)
{

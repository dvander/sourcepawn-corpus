...
-----------------------------------------------------------------------------------------
	...
	// TODO:
	* Hide selected players in stat (request by Re:Creator)
	* Use appropriate (requestor) language for "Rank for other player" menu action.
	* Hint about unsupported indeces in request to GetPoints/SetPonts native.
...
#define VIP_GROUP "Black VIP"

#define CVAR_FLAGS FCVAR_NOTIFY

#define DEBUG 0
#define BASE_MENU "sm_menu"
...
public void OnPluginStart()
{
	...
	g_ConVarMenuName = CreateConVar("l4d_hxstat_menu_name", "//^.^\\\\ Bloody Witch //^.^\\\\", "Name of statistics menu in !stat command", CVAR_FLAGS);
...
{
	g_bBaseMenuAvail = CommandExists(BASE_MENU);
}
...
public Action Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (!IsFakeClient(iVictim)) // Real survivor is killed
	{
...
public Action HxTimer_Infinite18(Handle timer)
{
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i))
		{
			...
			{
				if ( (g_iTemp[i][HX_TIME] += 1) % g_iTimeToPlay == 0 && g_iTimeToPlay)
				{
	...
	switch (action)
	{
		...
		case MenuAction_Cancel:
			if (param2 == MenuCancel_ExitBack)
				FakeClientCommand(param1, BASE_MENU);
...
void ShowRank(int client, int iDisplayToCustomClient = 0)
{
	if (g_bL4D2)
	{
		FormatEx(g_sBuf1, sizeof(g_sBuf1)
		 , "%d %T (%T:%d, %T:%d, %T:%d)\n \
			...
			%T: %d (%d)\n \
			%T: %d (%d)"
				, 					 	RoundToCeil(g_iReal[client][HX_POINTS] 			+ g_iTemp[client][HX_POINTS] 	+ g_fPenaltyBonus[client]), "points", client
				,	"M", client, 					g_iReal[client][HX_POINTS_MONTH] 	+ g_iTemp[client][HX_POINTS]
				,	"W", client,					g_iReal[client][HX_POINTS_WEEK] 	+ g_iTemp[client][HX_POINTS]
				,	"R", client,		RoundToCeil(g_iTemp[client][HX_POINTS]											+ g_fPenaltyBonus[client])
			, "Place", client,		g_iReal[client][HX_PLACE]
			, "Boomer", client, 	g_iReal[client][HX_BOOMER]		+ g_iTemp[client][HX_BOOMER],   	g_iTemp[client][HX_BOOMER]
			, "Charger", client,	g_iReal[client][HX_CHARGER]		+ g_iTemp[client][HX_CHARGER],		g_iTemp[client][HX_CHARGER]
			, "Hunter", client, 	g_iReal[client][HX_HUNTER]		+ g_iTemp[client][HX_HUNTER],   	g_iTemp[client][HX_HUNTER]
			, "Infected", client, 	g_iReal[client][HX_INFECTED]	+ g_iTemp[client][HX_INFECTED], 	g_iTemp[client][HX_INFECTED]
			, "Jockey", client, 	g_iReal[client][HX_JOCKEY]		+ g_iTemp[client][HX_JOCKEY], 		g_iTemp[client][HX_JOCKEY]
			, "Smoker", client, 	g_iReal[client][HX_SMOKER]		+ g_iTemp[client][HX_SMOKER],   	g_iTemp[client][HX_SMOKER]
			, "Spitter", client, 	g_iReal[client][HX_SPITTER]		+ g_iTemp[client][HX_SPITTER],   	g_iTemp[client][HX_SPITTER]
			, "Tank", client, 		g_iReal[client][HX_TANK]		+ g_iTemp[client][HX_TANK],     	g_iTemp[client][HX_TANK]
			, "Witch", client, 		g_iReal[client][HX_WITCH]		+ g_iTemp[client][HX_WITCH],    	g_iTemp[client][HX_WITCH]
			, "Penalty", client, 	
	...
	if (client == 0)
	{
		...
		hPanel.DrawItem(Translate(client, "%t", "Close"));
		hPanel.DrawItem(Translate(client, "%t", "Back"));
		hPanel.DrawItem(Translate(client, "%t", "Other_Player_Points"));
		hPanel.Send(iDisplayToCustomClient == 0 ? client : iDisplayToCustomClient, RankPanelHandler, 20);
		delete hPanel;
	}
}
...
void ShowTop(int client, int ePeriod, bool bPrintLog = false)
{
	...
	if (client)
	{
		if (IsClientInGame(client))
		{
			...
			if (g_hDB)
			{
				FormatEx(g_sQuery3, sizeof(g_sQuery3), "SELECT `Name`, `%s` FROM `"...HX_TABLE..."` ORDER BY `%s` DESC LIMIT %i", sField, sField, GetTopPeriodLimit(ePeriod));
...
bool GiveVIP(int client, int days)
{
	...
	if ( g_bVipCoreLib )
	{
		...
		if ( VIP_IsClientVIP (client) )
		{
			...
			if (VIP_IsValidVIPGroup(VIP_GROUP))
			{
				iTimeStamp = VIP_SecondsToTime(days * 24 * 60 * 60);
				
				VIP_GiveClientVIP(0, client, iTimeStamp, VIP_GROUP, true);
			}
			else {
				LogError("Invalid vip group: %s", VIP_GROUP);
			}
		}

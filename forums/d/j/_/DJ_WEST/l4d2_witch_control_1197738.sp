...
new g_b_WitchControl[MAXPLAYERS+1], Float:g_PlayerGameTime[MAXPLAYERS+1], bool:g_b_OnLadder[MAXPLAYERS+1],
	...
	Float:g_PlayerTraceTimer[MAXPLAYERS+1], Handle:h_CvarWitchSpeed, Handle:h_CvarMode, Handle:h_CvarMessageType, Float:g_f_WitchJump[MAXPLAYERS+1],
	Handle:h_CvarAttack
...
{
	decl i_Witch, String:s_Message[192]
	...
	if (!GetConVarInt(h_CvarMessageType))
		return Plugin_Handled
...
public Action:OnPlayerRunCmd(i_Client, &i_Buttons, &i_Impulse, Float:f_PlayerVelocity[3], Float:f_PlayerAngles[3], &i_Weapon)
{
	...
	{
		if (!g_b_OnLadder[i_Client] && !g_f_WitchJump[i_Client])
		{
		...
		else
		{
			if (i_Flags & FL_DUCKING)
			{
				...
				if (i_Buttons & IN_FORWARD)
					SetEntProp(i_Witch, Prop_Data, "m_nSequence", ANIM_CROUCH)
				else
	...
	if (i_Flags & FL_DUCKING && !g_b_OnLadder[i_Client])
		SetEntProp(i_Witch, Prop_Data, "m_nSequence", ANIM_SITTING)
	...
	if (i_Buttons & IN_FORWARD)
	{
		...
		if (i_Flags & FL_DUCKING)
		{
			ScaleVector(f_Forward, GetConVarFloat(h_CvarWitchSpeed) / 1.5)
			SetEntProp(i_Witch, Prop_Data, "m_nSequence", ANIM_CROUCH)
		}
		else
		{
	...
	for (i_Client = 1; i_Client <= MaxClients; i_Client++)
	{
		...
		if (g_f_WitchJump[i_Client])
		{
			if (i_Witch && IsValidEdict(i_Witch))
			{
				...
				f_Jump = g_f_WitchJump[i_Client]
				g_f_WitchJump[i_Client] += 0.1
				...
				if (f_Jump <= 3.0)
				{
					ScaleVector(f_Forward, f_Jump)
					ScaleVector(f_Up, f_Jump * 3.0)
				}
				else if (f_Jump <= 4.0)
				{
					ScaleVector(f_Forward, f_Jump)
...
public Action:TeleportPlayer(Handle:h_Timer, any:i_Client)
{
	...
	if (!i_Witch && !IsValidEdict(i_Witch) || !IsClientInGame(i_Client) || GetClientTeam(i_Client) == TEAM_SURVIVOR)
	{
		if (g_h_TeleportTimer[i_Client] != INVALID_HANDLE)
		{
			KillTimer(g_h_TeleportTimer[i_Client])
...
public RemoveWitchControl(i_Client, i_Witch)
{
	...
	SetEntProp(i_Client, Prop_Send, "m_iGlowType", 0)
	SetEntProp(i_Client, Prop_Send, "m_glowColorOverride", 0)

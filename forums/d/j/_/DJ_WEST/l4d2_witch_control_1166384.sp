...
new g_b_WitchControl[MAXPLAYERS+1], Float:g_PlayerGameTime[MAXPLAYERS+1], g_GameInstructor[MAXPLAYERS+1], bool:g_b_OnLadder[MAXPLAYERS+1],
	Handle:g_h_SetClass, Handle:g_h_GameConfig, Float:g_WitchLadderOrigin[MAXPLAYERS+1], Handle:g_h_TeleportTimer[MAXPLAYERS+1], Float:g_PlayerTraceTimer[MAXPLAYERS+1],
	Handle:h_CvarWitchSpeed, Handle:h_CvarMode, Handle:h_CvarMessageType
...
{
	decl i_Camera, i_Client, i_Witch
	...
	CreateTimer(0.2, DelayDisplaySitHint, i_Client)
	CreateTimer(7.0, DisplayControlHint, i_Client)
}
...
public Action:DisplayControlHint(Handle:h_Timer, any:i_Client)
{
	if (GetConVarInt(h_CvarMessageType) == 3)
		ClientCommand(i_Client, "gameinstructor_enable 1")
	CreateTimer(0.2, DelayDisplayControlHint, i_Client)
}

public Action:DelayDisplaySitHint(Handle:h_Timer, any:i_Client)
{
	...
	switch (GetConVarInt(h_CvarMessageType))
	{
		case 1:
		{
			FormatEx(s_Message, sizeof(s_Message), "\x03[%t]\x01 %t.", "Information", "Sit witch")
			ReplaceString(s_Message, sizeof(s_Message), "\n", " ")
			PrintToChat(i_Client, s_Message)
		}
		case 2: PrintHintText(i_Client, "%t", "Sit witch")
		case 3:
		{
			FormatEx(s_Message, sizeof(s_Message), "%t", "Sit witch")
			DisplayInstructorHint(i_Client, s_Message, "+duck")
		}
	}
}

public Action:DelayDisplayControlHint(Handle:h_Timer, any:i_Client)
{
	decl String:s_Message[256]
	...
	switch (GetConVarInt(h_CvarMessageType))
	{
		case 1:
		{ 
			FormatEx(s_Message, sizeof(s_Message), "\x03[%t]\x01 %t.", "Information", "Lose witch")
			ReplaceString(s_Message, sizeof(s_Message), "\n", " ")
			PrintToChat(i_Client, s_Message)
		}
		case 2: PrintHintText(i_Client, "%t", "Lose witch")
		case 3:
		{
			FormatEx(s_Message, sizeof(s_Message), "%t", "Lose witch")
			DisplayInstructorHint(i_Client, s_Message, "+use")
		}
	}
}
	
public Action:DelayDisplayWitchHint(Handle:h_Timer, any:i_Client)
{
	decl String:s_Message[256]
	FormatEx(s_Message, sizeof(s_Message), "%t", "Take witch")
	DisplayInstructorHint(i_Client, s_Message, "+use")
}
	...
	for (new i_Client = 1; i_Client <= MaxClients; i_Client++)
		if (IsClientInGame(i_Client) && !IsFakeClient(i_Client) && GetClientTeam(i_Client) == TEAM_INFECTED)
		{
			...
			switch (GetConVarInt(h_CvarMessageType))
			{
				case 1: 
				{
					FormatEx(s_Message, sizeof(s_Message), "\x03[%t]\x01 %t.", "Information", "Take witch")
					ReplaceString(s_Message, sizeof(s_Message), "\n", " ")
					PrintToChat(i_Client, s_Message)
				}
				case 2: PrintHintText(i_Client, "%t", "Take witch")
				case 3:
				{
					QueryClientConVar(i_Client, "gameinstructor_enable", ConVarQueryFinished:GameInstructor, i_Client)
					ClientCommand(i_Client, "gameinstructor_enable 1")
					CreateTimer(0.2, DelayDisplayWitchHint, i_Client)
				}
			}
		}
}
...
public Action:OnPlayerRunCmd(i_Client, &i_Buttons, &i_Impulse, Float:f_PlayerVelocity[3], Float:f_PlayerAngles[3], &i_Weapon)
{
	...
	if (!g_b_WitchControl[i_Client])
	{
		...
		{
			if (GetEntProp(i_Client, Prop_Send, "m_zombieClass") != CLASS_TANK && !GetEntProp(i_Client, Prop_Send, "deadflag"))
			{
					...
					if (IsValidEdict(i_Target))
					{
						...
						if (StrEqual(s_ClassName, "func_simpleladder"))
						{
							if (GetVectorDistance(f_Origin, f_EndOrigin) <= 25.0)
							{
								...
								TeleportEntity(i_Witch, f_Origin, NULL_VECTOR, NULL_VECTOR)
								SetEntProp(i_Witch, Prop_Send, "m_fFlags", i_Flags & ~FL_FROZEN | FL_FLY)
	...
	{
		decl Float:f_WitchAngles[3]
		GetEntPropVector(i_Witch, Prop_Send, "m_angRotation", f_WitchAngles)
		f_WitchAngles[1] += 2.0
		TeleportEntity(i_Witch, NULL_VECTOR, f_WitchAngles, NULL_VECTOR)
	}	

...
* Version 1.5.1
* 		- Some minor and major fixes to avoid crashes.
**********************************************************************************************/
...
public OnPluginStart()
{
	...
	zombieHP[3] = GetConVarInt(FindConVar("z_tank_health"));
...
public Action:RoundEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsMapVS)
	{
		...
		if (!RoundEnded)
		{
			...
			// We kick all unwanted bots (from infected's team)
			KickAllBots ();
		}
	}
}
...
public Action:PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (StrContains(class, "boomer", false) != -1)
	{
		...
		CreateTimer(float(InfectedSpawnTime), ResetSpawnRestriction, 3);
	}
	else if (StrContains(class, "smoker", false) != -1)
	{
		canSpawnSmoker = false;
		CreateTimer(float(InfectedSpawnTime), ResetSpawnRestriction, 2);
	}
...
public OnMapStart()
{
	...
	if (IsMapVS)
	{
		LogMessage("[Infected bots] Map started");
		...
		AttackFlowRange = GetConVarInt(FindConVar("z_attack_flow_range"));
...
public OnMapEnd()
{
	// If game is vs, we un-hook the death event
	if (IsMapVS)
	{
		LogMessage("[Infected bots] Map ended");
		
		UnhookEvent("player_death", PlayerDeath);
		UnhookEvent("player_team", PlayerTeam);
		UnhookEvent("player_left_start_area", PlayerLeftStart);
		UnhookEvent("player_spawn", PlayerSpawn);
		
		// We restore some settings
		SetConVarInt(FindConVar("z_attack_flow_range"), AttackFlowRange);
		SetConVarInt(FindConVar("sb_all_bot_team"), 0);
	}
	
	// We reset the round counter
	RoundNumber = 0;
}	
...
{
	// If players haven't leaved safe room ...
	...
	// We check if real players are in game, otherwise, we kick all bots ...
...
BotTypeNeeded()
{
	...
	// We need a boomer??? can we spawn a boomer???
	if ((boomers < MaxBoomers) && (canSpawnBoomer))
		return 3;
	else if ((smokers < MaxSmokers) && (canSpawnSmoker)) // we need a smoker ???? can we spawn a smoker ???
		return 2;
...
KickAllBots ()
{
	LogMessage("[Infected bots] Kicking all bots");

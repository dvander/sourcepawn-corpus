...
LoadSpots ()
{
	...
	else
	{
		...
		MinimumDistance = 999999.99;
		
		// We kill the timer
		KillCheckTimer()
		
		// We start a timer ....
		CurrentInterval = 1.0; // We start off with 1 second between checks, just in case there's a restricted spot near spawn 
...
CheckTimerReset ()
{
	...
	if (MinimumDistance > 7000)
		time = 20.00;
	else if (MinimumDistance > 5000)
		time = 15.00;
	else if (MinimumDistance > 2500)
		time = 10.00;
	else if (MinimumDistance > 1000)
		time = 3.00;
	else
	time = 0.5;

...
#define PLAYER_NOTIFY_KEY	"\x04Presione %s para cambiar de infectado."
#define PLAYER_LIMITS_UP	"\x04Limites alcanzado, Seleccione la clase actual o esperar (%d/%d)."
#define PLAYER_COOLDOWN_WAIT	"\x04Waiting for %s class to become available (%d/%d)."
#define PLAYER_CLASSES_UP_ALLOW	"\x04No mas clases disponibles, Ultima clase permitida."
#define PLAYER_CLASSES_UP_DENY	"\x04No mas clases disponibles, Seleccione la clase actual o espere."
...
public OnPluginStart()
{
	...
	g_hCooldownSpitter	= CreateConVar("zcs_cooldown_spitter", "-1", "Time before spitter class is allowed after player death in (s). (-1=Use Director, 0=No delay, 1-60=Delay)", FCVAR_PLUGIN, true, -1.0, true, 60.0);
	g_hCooldownJockey	= CreateConVar("zcs_cooldown_jockey", "-1", "Time before jockey class is allowed after player death in (s). (-1=Use Director, 0=No delay, 1-60=Delay)", FCVAR_PLUGIN, true, -1.0, true, 60.0);

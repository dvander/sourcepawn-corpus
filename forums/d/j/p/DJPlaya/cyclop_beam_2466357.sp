...
#define PLUGIN_NAME		"[Any] Cyclop's beam"
...
{
	LoadTranslations("cyclop_beam.phrases");
	char menutitle[64];
	Format(menutitle, sizeof(menutitle), "%T", "Menu_Title", LANG_SERVER);
	AutoExecConfig(true, "cyclop_beam");

	CreateConVar("sm_cyclop_version", PLUGIN_VERSION, PLUGIN_NAME, FCVAR_SPONLY | FCVAR_REPLICATED | FCVAR_NOTIFY | FCVAR_DONTRECORD);
	lb_color = CreateConVar("sm_cyclop_default", "0", "Default client color preference (0 - 8)", FCVAR_NONE, true, 0.0, true, 8.0);
	lb_alpha = CreateConVar("sm_cyclop_alpha", "63", "Beam transparency (0-255)", FCVAR_NONE, true, 0.0, true, 255.0 );
	...
	RegAdminCmd("sm_cyclop", Cmd_Toggle, ADMFLAG_SLAY, "Fire a laser");
	RegAdminCmd("sm_ct", Cmd_Toggle, ADMFLAG_SLAY, "Fire a laser");
...
public void OnConVarChange(Handle hCVar, const char[] oldValue, const char[] newValue)
{
	...
	else if (hCVar == lb_alpha) iAlpha = StringToInt(newValue) ? true : false;
}
...
{
	if (IsPlayerAlive(client) && iButtons & IN_RELOAD) CreateBeam(client);
}
...
void CreateBeam(int client, bool show = false)
{
	float flPos[3], flAng[3];
	GetClientEyePosition(client, flPos);
	GetClientEyeAngles(client, flAng);

	flPos[2] -= 0.0; // - This Hight

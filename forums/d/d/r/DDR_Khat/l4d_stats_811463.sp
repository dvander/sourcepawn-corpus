...
new Handle:cvar_coop = INVALID_HANDLE;
...
//    cvar_coop = FindConVar("director_no_human_zombies");
    cvar_coop = FindConVar("mp_gamemode");
...
public SQLErrorCheckCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
    ...
    if(!StrEqual("", error))
        LogError("SQL Error: %s, Failed Query: %s", error);
}
...
public Action:event_HealPlayer(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if (GetConVarInt(cvar_MedkitMode))
        Score = 1 + ModifyScoreDifficulty(GetConVarInt(cvar_Medkit), 2, 4);
    else
...
public DisplayRank(Handle:owner, Handle:hndl, const String:error[], any:data)
{
    ...
    Format(Value, sizeof(Value), "%s's Ranking" , Name);
...
{
    new String:gmode[32];
    GetConVarString(cvar_coop,gmode,sizeof(gmode));
    if (strncmp(gmode,"coop",sizeof(gmode),false)==1)
        return true;
    else
        return false;
}

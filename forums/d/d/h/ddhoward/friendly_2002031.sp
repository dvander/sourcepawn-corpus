/* Changes since 13.0717
	All GetEdictClassname has been changed to GetEntityClassname
	Added IsValidClient() stock
	Invuln shit changed from #define's to an enum
	action_spawn cvars only apply to the player's own spawn now, recommend mckay's no enemies in spawn plugin
		In-spawn detection changed to Touch from StartTouch
	Sound Hook, command listeners, and targeting filters are now properly removed OnPluginEnd
	Blocked wrenches and the Rescue Ranger
	Increased maximum classname length from 32 to 64
	Plugin now logs an error when a convar has an invalid value set
	Friendly Stickybombs now have invulnerability and working transparency
	most boolean convar "changes" that are actually the same value no longer fire the events that a real change does
		Only applies to convars involving hooking/unhooking things, and sm_friendly_enabled
	Removed FirstCaching bullshit
	Worked around Valve's ForcePlayerSuicide bug
To-do
	Enable weapon switching while miniguns are spun up
...
*/
#define DEFAULT_BLOCKED_WEAPONS "656,447,44,58,222,305,528,997"
...
public Airblast(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (!IsValidClient(pyro) || !IsValidClient(pitcher)) {
		return;
	}
...
public cvarChange(Handle:hHandle, const String:strOldValue[], const String:strNewValue[]) {
	...
	if (hHandle == hcvar_afkinterval || hHandle == INVALID_HANDLE) {
		...
		if (cvar_afkinterval < 0.1) {
			...
			LogError("Value of sm_friendly_maxfriendlies is invalid. Plugin will behave as if value was 1.0.");
		}
		if (hHandle != INVALID_HANDLE) {
			RestartAFKTimer();
		}
			...
			for (new i=1; i <= MaxClients; i++) {
				if (IsFriendly[i]) {
					if (cvar_invuln_p == 0) {
						...
						ApplyInvuln(i, INVULNMODE_BUDDHA);
					} else {
	...
	if (hHandle == hcvar_blockweps_white || hHandle == INVALID_HANDLE) {
		...
		if (StrEqual(strWeaponsWhite, "0")) {
			...
			strWeaponsBlack = DEFAULT_WHITELISTED_WEAPONS;
		}
	...
	if (hHandle == hcvar_overlay || hHandle == INVALID_HANDLE) {
		decl String:strOverlay[256]; strOverlay[0] = '\0';
		GetConVarString(hcvar_overlay, strOverlay, sizeof(strOverlay));
		if (StrEqual(strOverlay, "0")) {
			strOverlay = "0";
		} else if (StrEqual(strOverlay, "")) {
			strOverlay = "0";
		} else if (StrEqual(strOverlay, "1")) {
			strOverlay = DEFAULT_OVERLAY_1;
		} else if (StrEqual(strOverlay, "2")) {
			strOverlay = DEFAULT_OVERLAY_2;
		} else if (StrEqual(strOverlay, "3")) {
			strOverlay = DEFAULT_OVERLAY_3;
		} else if (StrEqual(strOverlay, "4")) {
			strOverlay = DEFAULT_OVERLAY_4;
		} else if (StrEqual(strOverlay, "5")) {
			strOverlay = DEFAULT_OVERLAY_5;
		} else if (StrEqual(strOverlay, "6")) {
...
stock bool:IsValidClient(client, f_validclientlevel:threshold=VCLIENT_INGAME, bool:nobots=false, bool:noreplay=true, bool:convert=true) {
	if (client > 4096 && convert) {
		client = EntRefToEntIndex(client);
	}
	if ((client < 1 || client > MaxClients) && threshold >= VCLIENT_VALIDINDEX) {
		return false;
	}
	if (!IsClientConnected(client) && threshold >= VCLIENT_CONNECTED) {
		return false;
	}
	if (!IsClientInGame(client) && threshold >= VCLIENT_INGAME) {
		return false;
	}
	if (GetClientTeam(client) < 1 && threshold >= VCLIENT_ONATEAM) {
		return false;
	}
	if (GetClientTeam(client) < 2 && threshold >= VCLIENT_ONAREALTEAM) {
		return false;
	}
	if (!IsPlayerAlive(client) && threshold >= VCLIENT_ALIVE) {
		return false;
	}
	if (IsFakeClient(client) && nobots) {
		return false;
	}
	if ((IsClientSourceTV(client) || IsClientReplay(client)) && noreplay) {
		return false;
	}
	return true;
}
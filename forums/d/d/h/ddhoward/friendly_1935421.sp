/*
CHANGES MADE SINCE 13.0418.1
http://forums.alliedmods.net/showthread.php?t=213205
Dead Friendly players who use sm_friendly can cancel their mode change even if sm_friendly_remember = 0
Changed chat prefix to [Friendly] from [Friendly Mode]
Added prettyful color to the prefix
Added non-important command sm_friendly_ver
Added sm_friendly_advert
*/
#pragma semicolon 1
...
#define PLUGIN_VERSION "13.0418.2"
...
new Handle:ConnectAdvertTimer[MAXPLAYERS+1];
...
public OnPluginStart()
{
	...
	for(new i=1; i<MAXPLAYERS+1; i++) ClientRFETRIZ[i] = false;
...
public Action:OnToggleFriendly(client, args) {
	...
	CPrintToChat(client, "%s Not a valid client. You must be in the game to use sm_friendly.", CHAT_PREFIX);
	return Plugin_Handled;
	}
}
...
public Action:OnToggleAdmin(client, args) {
	if (ClientIsAdmin[client]) {
		...
		if (GetConVarInt(sm_friendly_invuln) < 2) {
			CPrintToChat(client, "%s You cannot bypass Friendly Mode while {lightgreen}sm_friendly_invuln{default} = 0 or 1", CHAT_PREFIX);
			return Plugin_Handled;
		} else {
...
public OnClientPutInServer(client)
{
	...
	if (GetConVarBool(sm_friendly_advert)) {
		ConnectAdvertTimer[client] = CreateTimer(30.0, ShowJoinMessage, client);
	}
}

public OnClientDisconnect(client) {
	KillAllClientVariables(client);
	if (ConnectAdvertTimer[client] != INVALID_HANDLE) {
		KillTimer(ConnectAdvertTimer[client]);
		ConnectAdvertTimer[client] = INVALID_HANDLE;
	}
}
...
KillAllClientVariables(const client) {
	ClientIsFriendly[client] = false;
	ClientRequestedChange[client] = false;
	ClientIsAdmin[client] = false;
	ClientRFETRIZ[client] = false;
...
//////////////////////////////////////////////Advert/sm_friendly_ver Code Begins Here
public Action:smFriendlyVer(client, args) {
	ShowFriendlyVer(client);
	return Plugin_Handled;
}

public Action:ShowJoinMessage(Handle:timer, any:client) {
	if (GetConVarBool(sm_friendly_advert)) {
		ShowFriendlyVer(client);
	}
}
...
ShowFriendlyVer(const client) {
	if (client != 0) {
		CPrintToChat(client, "%s Hello! This server is currently running %s v.{lightgreen}%s{default}. Type {olive}/friendly{default} to try it out!", CHAT_PREFIX, CHAT_NAME, PLUGIN_VERSION);
	} else {
		ReplyToCommand(client, "%s Hello! This server is currently running %s v.%s.", CHAT_PREFIX_NOCOLOR, CHAT_NAME_NOCOLOR, PLUGIN_VERSION);
	}
}
/////////////////////////////////////////Advert/sm_friendly_ver Code ENDS Here
...
public Action:DebugFriendly(client, args) {
	PrintToConsole(client, "%s Friendly: %i, Admin: %i, Changing: %i, Alive: %i, Remember: %i, RFETRIZ: %i, BlockWeps: %i, ActionH: %i, ActionF: %i, Invuln: %i, NoTarget: %i, NoBlock: %i, Alpha: %i, Update: %i, AdvertEnabled: %i", CHAT_PREFIX_NOCOLOR, (ClientIsFriendly[client]), (ClientIsAdmin[client]), (ClientRequestedChange[client]), (IsPlayerAlive(client)), (GetConVarInt(sm_friendly_remember)), (ClientRFETRIZ[client]), (GetConVarInt(sm_friendly_blockweps)), (GetConVarInt(sm_friendly_action_h)), (GetConVarInt(sm_friendly_action_f)), (GetConVarInt(sm_friendly_invuln)), (GetConVarInt(sm_friendly_notarget)), (GetConVarInt(sm_friendly_noblock)), (GetConVarInt(sm_friendly_alpha)), (GetConVarInt(sm_friendly_update)), (GetConVarInt(sm_friendly_advert)));
	return Plugin_Handled;
}

//////////////////////////////////The following code was borrowed from FlaminSarge's Ghost Mode plugin: forums.alliedmods.net/showthread.php?t=183266
//////////////////////////////////This code makes wearables change alpha
...
public Updater_OnPluginUpdated() {
	if (GetConVarInt(sm_friendly_update) == 2) {
		ReloadPlugin();
		CPrintToChatAll("%s Update has been installed, map change is needed. Reconnect if you are having problems with Friendly Mode.", CHAT_PREFIX);
	}
	if (GetConVarInt(sm_friendly_update) == 1) {
		CPrintToChatAll("%s An update has been downloaded, will be installed upon map change.", CHAT_PREFIX);
	}
}

...
Default flag required for _d is changed to 0, command still commented out
AutoExecConfig no longer autogenerates the file
Plugin reloads no longer require a respawn to gain the protections of Friendly
Added overlay support
Added IsValidEdict check to weapon blocking
Added the gunslinger to the list of weapons blocked
*/
#pragma semicolon 1
...
public OnPluginEnd() {
	for (new i=1; i <= MaxClients; i++) {
		...
		if (IsFriendly[i]) {
			...
			CPrintToChat(i, "%s Friendly Mode has been unloaded or restarted.", CHAT_PREFIX);
		}
	}
}
...
/* ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
Begin code relevant to weaponblocker */
...
public Action:OnWeaponSwitch(client, weapon) {
	...
	if (IsFriendly[client] && GetConVarBool(cvar_blockweps) && !IsAdmin[client]) {
		if (IsValidEdict(weapon)) {
			...
			||	StrEqual(sWeapon, "tf_weapon_robot_arm") 

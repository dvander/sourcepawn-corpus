...
Changes since 13.0602
	...
	fixed notarget resupply bug, added sm_friendly_notarget 2
	added targeting filters
To-do
	...
	fix afk detection method to mirror changes made in the beeridle plugin
*/
...
public Action:OnWeaponSwitch(client, weapon) {
	...
	if (blocked) {
		for (new i = 0; i < sizeof(cvar_blockweps_white) && blocked && !StrEqual(cvar_blockweps_white[i], "-1"); i++) {
			if (StringToInt(cvar_blockweps_white[i]) == weaponIndex) {
				blocked = false;
			}
		}
	} else {
		for (new i = 0; i < sizeof(cvar_blockweps_black) && !blocked && !StrEqual(cvar_blockweps_black[i], "-1"); i++) {
...
public Action:TauntCmd(client, const String:strCommand[], iArgs) {
	if (IsFriendly[client] && !IsAdmin[client]) {
		...
		if (IsValidEntity(weapon)) {
			new bool:blocked = false;
			new weaponIndex = GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex");
			for (new i = 0; i < sizeof(cvar_blocktaunt) && !blocked && !StrEqual(cvar_blocktaunt[i], "-1"); i++) {
...
ForceWeaponSwitches(const client) {
	new clientUserID = GetClientUserId(client);
	CreateTimer(0.2, ForceWeaponSwitches0, clientUserID);
	CreateTimer(0.4, ForceWeaponSwitches1, clientUserID);
	CreateTimer(0.6, ForceWeaponSwitches2, clientUserID);
}
public Action:ForceWeaponSwitches0(Handle:timer, any:clientUserID) {
...
public Action:ForceWeaponSwitches1(Handle:timer, any:clientUserID) {
	new client = GetClientOfUserId(clientUserID);
	if (client == 0)
		return;
...
public Action:ForceWeaponSwitches2(Handle:timer, any:clientUserID) {
	new client = GetClientOfUserId(clientUserID);
	if (client == 0)
		return;

...
public void Event_PlayerSpawn(Event hEvent, const char[] strName, bool bDontBroadcast)
{
	if (g_hCvarEnabled.BoolValue)
	{
		int iClient = GetClientOfUserId(GetEventInt(hEvent, "userid"));
		
		if(IsValidClient(iClient) && CheckCommandAccess(iClient, "wepinspect", 0))
		{
			int iWeapon[3];
...
stock bool IsFoodOrThrownItem(int iWeapon)
{
	...
	if(IsValidEdict(iWeapon))
	{
		...
		if(!strcmp(strClassname, "tf_weapon_lunchbox_drink", false)
		|| !strcmp(strClassname, "tf_weapon_lunchbox ", false)
		|| !strcmp(strClassname, "tf_weapon_jar_milk", false)
		|| !strcmp(strClassname, "tf_weapon_cleaver", false)
		|| !strcmp(strClassname, "tf_weapon_jar", false))
			return true;
	}

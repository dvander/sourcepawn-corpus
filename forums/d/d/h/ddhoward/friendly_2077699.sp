/* Changes since 13.1121.0
	Added Festive Jarate and Festive Crossbow to the list of default blocked weapons.
	Improved forced weapon switch if a blocked minigun was spinning during the forced switch
		credit to pheadxdll's "Melee Only" plugin
	changed weapon block cvars. -2 to disable all blocking, -1 to use defaults
		previous versions caused issues with stock bat and bottle
	Added cvar sm_friendly_spellbook
	Added cvar sm_friendly_usetele
	Removed sm_friendly_r command
To-do
	get rid of reminders to reset weapon blocking cvars
	fix bazaar bargain bug
	particle effect
	cvars to clear domination status, or take status into account when determining if damage nullification applies
	figure out how to make Friendly players invisible to bosses/bots/skeletons
	menu support?
	stop cloaked spies from being revealed through collision with friendly enemies (collisionhook?)
	improve method of creating command aliases
	reversable weapon blocking cvars
	attackblock
	rework AFK checker to use OnPlayerRunCmd() rather than GetClientButtons()
	combine killbuild and nobuild cvars into 1 single cvar per building, bitwise
	!!!!!!!!!!!!!! split whole fucking plugin into a ton of optional modules using forwards and natives
*/
...
#define PLUGIN_VERSION "13.1228.0"
...
new Handle:hcvar_thirdperson = INVALID_HANDLE;
new bool:cvar_thirdperson;
//new Handle:hcvar_settransmit = INVALID_HANDLE;
...
new bool:cvar_money;
new Handle:hcvar_spellbook = INVALID_HANDLE;
...
new Handle:hcvar_usetele = INVALID_HANDLE;
new cvar_usetele;
...
new _:cvar_alpha_t;
new Handle:hcvar_alpha_proj = INVALID_HANDLE;
...
new Handle:h_fwd_CanToggleFriendly;
...
public OnPluginStart() {
	...
	h_fwd_CanToggleFriendly = CreateGlobalForward("TF2Friendly_CanToggleFriendly", ET_Event, Param_Cell);
...
UseFriendlyOnSelf(const client) {
	...
	if (GetForwardFunctionCount(h_fwd_CanToggleFriendly) > 0) {
		Call_StartForward(h_fwd_CanToggleFriendly);
...
ApplyNoblock(i, bool:remove) {
	new cvarValue;
	new normalValue;
	if (IsValidClient(i, VCLIENT_VALIDINDEX)) {
	...
	if (cvarValue == 0 || remove) {
		SetEntProp(i, Prop_Send, "m_CollisionGroup", normalValue);
	} else if (cvarValue == 1) {
...
SetOverlay(client, bool:apply) {
	if (apply) {
		Client_SetScreenOverlay(client, cvar_overlay);
	} else {
...
SetThirdPerson(const client, bool:apply) {
	...
	if (apply) {
		CreateTimer(0.2, TimerThirdPerson, userid);
	} else {
		CreateTimer(0.2, TimerFirstPerson, userid);
	}
}
...
public OnPluginEnd() {
	...
	RemoveCommandListener(TauntCmd, "taunt");
	RemoveCommandListener(TauntCmd, "+taunt");
	RemoveCommandListener(OnClientSpeaks, "say");
	RemoveCommandListener(OnClientSpeaks, "say_team");
...
public Action:Object_Built(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	decl String:b_classname[64]; b_classname[0] = '\0';
	if (!GetEntityClassname(building, b_classname, sizeof(b_classname))) {
		return Plugin_Continue;
	}
	...
	if (IsFriendly[client]) {
		...
		else if (StrEqual(b_classname, "obj_dispenser")) {
		...
		else if (StrEqual(b_classname, "obj_teleporter")) {
...
public cvarChange(Handle:hHandle, const String:strOldValue[], const String:strNewValue[]) {
		...
		if (hHandle != INVALID_HANDLE) {
			...
			while ((i = FindEntityByClassname(i, "obj_sentrygun"))!=INVALID_ENT_REFERENCE) {
				if (IsValidEntity(i)) {
					...
					if (IsFriendly[engie]) {
						if (cvar_invuln_s < 2) {
							ApplyInvuln(i, INVULNMODE_MORTAL);
							RemoveActiveSapper(i, false);
						} else if (cvar_invuln_s == 2) {
							ApplyInvuln(i, INVULNMODE_GOD);
	...
	if (hHandle == hcvar_blockweps_black || hHandle == INVALID_HANDLE) {
		...
		} else if (StrEqual(strWeaponsBlack, "0")) { //DELETE THIS BLOCK NEXT UPDATE
			cvar_blockweps_black[0] = -1;
			PrintToServer("%s A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.", CHAT_PREFIX_NOCOLOR);
			LogError("A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.");
		} else {
			decl String:strWeaponsBlack2[255][8];
			if (StrEqual(strWeaponsBlack, "-1")) {
				strWeaponsBlack = DEFAULT_BLOCKED_WEAPONS;
			}
			if (StrEqual(strWeaponsBlack, "1")) { //DELETE THIS BLOCK NEXT UPDATE
				strWeaponsBlack = DEFAULT_BLOCKED_WEAPONS;
				PrintToServer("%s A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.", CHAT_PREFIX_NOCOLOR);
				LogError("A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.");
			}
	...
	if (hHandle == hcvar_blockweps_white || hHandle == INVALID_HANDLE) {
		...
		if (StrEqual(strWeaponsWhite, "-2")) {
		...
		} else if (StrEqual(strWeaponsWhite, "0")) { //DELETE THIS BLOCK NEXT UPDATE
			cvar_blockweps_white[0] = -1;
			PrintToServer("%s A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.", CHAT_PREFIX_NOCOLOR);
			LogError("A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.");
		} else {
			...
			if (StrEqual(strWeaponsWhite, "1")) { //DELETE THIS BLOCK NEXT UPDATE
				strWeaponsWhite = DEFAULT_WHITELISTED_WEAPONS;
				PrintToServer("%s A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.", CHAT_PREFIX_NOCOLOR);
				LogError("A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.");
			}
	...
	if (hHandle == hcvar_blockweps_classes || hHandle == INVALID_HANDLE) {
		...
		if (StrEqual(strWeaponsClass, "-2")) {
		...
		} else if (StrEqual(strWeaponsClass, "0")) { //DELETE THIS BLOCK NEXT UPDATE
			cvar_blockweps_classes[0] = "-1";
			PrintToServer("%s A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.", CHAT_PREFIX_NOCOLOR);
			LogError("A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.");
		} else {
			...
			if (StrEqual(strWeaponsClass, "1")) { //DELETE THIS BLOCK NEXT UPDATE
				strWeaponsClass = DEFAULT_BLOCKED_WEAPONCLASSES;
				PrintToServer("%s A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.", CHAT_PREFIX_NOCOLOR);
				LogError("A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.");
			}
	...
	if (hHandle == hcvar_blocktaunt || hHandle == INVALID_HANDLE) {
		...
		if (StrEqual(strWeaponsTaunt, "-2")) {
			cvar_blocktaunt[0] = -1;
		} else if (StrEqual(strWeaponsTaunt, "0")) { //DELETE THIS BLOCK NEXT UPDATE
			cvar_blocktaunt[0] = -1;
			PrintToServer("%s A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.", CHAT_PREFIX_NOCOLOR);
			LogError("A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.");
		} else {
			...
			if (StrEqual(strWeaponsTaunt, "1")) { //DELETE THIS BLOCK NEXT UPDATE
				strWeaponsTaunt = DEFAULT_BLOCKED_TAUNTS;
				PrintToServer("%s A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.", CHAT_PREFIX_NOCOLOR);
				LogError("A recent update to Friendly Mode has changed how the Weapon blocking cvars work! Please see the forum thread for more info.");
			}

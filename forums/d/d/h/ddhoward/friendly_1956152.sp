/*
Changes since 13.0514
	Added PrintToServer advertisement for Updater, if Updater is not already installed.
	Changed when shit is hooked on plugin load, no longer listens for round_start
	sm_friendly_funcbutton
	Entities will only be hooked if they need to be
	Fixed admins being able to toggle Friendly on themselves and others even while plugin is disabled
	Players will no longer see the advert if the plugin is disabled, or they don't have access to sm_friendly
	switch to morecolors from colors
	added sm_friendly_action_h_spawn and sm_friendly_action_f_spawn
	added targeting for sm_friendly_admin
		sm_friendly_admin_targetothers
	added aliases
		sm_friendly_a == sm_friendly_admin
		sm_friendly_ver == sm_friendly_v
		sm_friendly_reload == sm_friendly_r
	Changes to command syntax of sm_friendly, improvements to command
	Added sm_friendly_lock (sm_friendly_l)
	Changes to sm_friendly_logging
	Added sm_friendly_delay
To-do
	anti-AFK
...
#define GENERIC_NOACCESS "[SM] You do not have access to this command."
...
new Handle:hcvar_initialize = INVALID_HANDLE;
...
public OnPluginStart() {
	...
	hcvar_initialize = CreateConVar("sm_friendly_initialize", "0", _, FCVAR_DONTRECORD|FCVAR_CHEAT|FCVAR_PLUGIN);
	...
	RegAdminCmd("sm_friendly_lock", UseLockCmd, ADMFLAG_BAN, "Toggles Blocks a player from using sm_friendly on himself.");
...
public OnConfigsExecuted() {
	FirstCaching = true;
	SetConVarInt(hcvar_initialize, 0);
	HookConVarChange(hcvar_initialize, cvarChange);
	SetConVarInt(hcvar_initialize, 1);
	UnhookConVarChange(hcvar_initialize, cvarChange);
}
...
public Action:UseLockCmd(client, args) {
	...
	if (direction == -1) {
		...
		if (numtargets > 1) {
			...
			if (cvar_logging > 0 && cvar_logging < 3) {
				LogAction(client, -1, "\"%L\" toggled Friendly Admin Lock on %s.", client, target_name);
			} else if (cvar_logging >= 3) {
				LogAction(client, -1, "\"%L\" toggled Friendly Admin Lock on %s, affecting the previous %i players.", client, target_name, count);
			}
		} else if (numtargets == 1) {
...
public Action:UseAdminCmd2(client, args) {
	if (CheckCommandAccess(client, "sm_friendly_admin", ADMFLAG_BAN, true)) {
		...
		ReplyToCommand(client, "%s", GENERIC_NOACCESS);
	}
	return Plugin_Handled;
}
...
public Action:smFriendlyVer(client, args) {
	if (CheckCommandAccess(client, "sm_friendly", 0, false) && cvar_enabled) {
		CReplyToCommand(client, "%s This server is currently running %s v.{lightgreen}%s{default}. Type {olive}/friendly{default} to use.", CHAT_PREFIX, CHAT_NAME, PLUGIN_VERSION);
	} else {

...
#define PLUGIN_VERSION "0.0.1.6-sdkhooks"
...
new Float:gNextDrop[MAXPLAYERS+1];
...
{
	if (!(IsPlayerAlive(client) || CheckCommandAccess(client, "sm_jp_usedead", ADMFLAG_ROOT, true))) {
		PrintCenterText(client, "You cannot deploy a jumppad while dead.");
	...
	//	SetEntProp(entity, Prop_Data, "m_usSolidFlags", 28);
		SetEntProp(entity, Prop_Send, "m_usSolidFlags", 28);

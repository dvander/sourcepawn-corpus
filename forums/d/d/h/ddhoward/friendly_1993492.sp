/*
Changes since 13.0706
	Added cvar sm_friendly_alpha_proj
	Added more valid client checks, removing more potential errors
	Forward and 2 natives
	if sm_friendly_ammopack = 1, Friendly players won't drop ammo
	changed a bunch of shit to smlib
	minor code style changes, still trying to find a style and stick with it
	fixed a very old bug where buildings' noblock cvars didn't actually do anything
		buildings used to adopt the collision group of their engies
	fixed item_ammopack_medium never getting unhooked, double hooking
	safely changed all non-global strings to decl
	fixed the kill building cvars not being cached properly
	optimized weapon blocking some more
	added basic cvar verification
	Players no longer switch all weapons upon becoming friendly
	Friendly Players with no allowed weapons will have all weapons removed
To-do
	give stickies godmode
	particle effect
	...
	remove AFK checker, create a seperate plugin with forwards and natives
	fix bazaar bargain bug
	stop friendly engineers from healing/refilling non-friendly buildings
		wrenches AND rescue ranger
*/
...
#define INVULNMODE_GODMODE 0
#define INVULNMODE_GOD 0
#define INVULNMODE_BUDDAH 1
#define INVULNMODE_BUDDHA 1
#define INVULNMODE_MORTAL 2
...
public Airblast(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (pyro > MaxClients || pyro < 1 || pitcher > MaxClients || pitcher < 1) {
		return;
	}
	if (!IsClientInGame(pyro) || !IsClientInGame(pitcher)) {
		return;
	}
...
public OnProjectileSpawned(entity) {
	...
	if (client <= MaxClients && client > 0) {
		if (IsFriendly[client]) {
			...
			SetEntityRenderColor(entity, _, _, _, cvar_alpha_proj);
		}
	}
}
...
/*	new demoman = GetEntPropEnt(sticky, Prop_Data, "m_hOwnerEntity");
	PrintToChatAll("%i", demoman);
	if (demoman < 1 || demoman > MaxClients) {
		return Plugin_Continue;
	}
	if (!IsClientInGame(demoman)) {
		return Plugin_Continue;
	}
...
public Action:SpawnStartTouch(spawn, client) {
	if (client < 1 || client > MaxClients) {
		return Plugin_Continue;
	}
	if (IsClientInGame(client)) {
...
stock bool:IsWeaponBlocked(weapon) {
	decl String:weaponClass[32]; weaponClass[0] = '\0';
	if (!GetEdictClassname(weapon, weaponClass, sizeof(weaponClass))) {
		return false;
	}
...
ForceWeaponSwitches(const client) {
	...
	for (new i = 0; i <= 5; i++) {
		...
		decl String:classname[32]; classname[0] = '\0';
...
public bool:TargetFriendlyLocked(const String:pattern[], Handle:clients) {
	for (new i = 1; i <= MaxClients; i++) {
		if (IsClientInGame(i) && IsLocked[i]) {
			PushArrayCell(clients, i);
		}
	}
	return true;
...
public cvarChange(Handle:hHandle, const String:strOldValue[], const String:strNewValue[]) {
		...
		if (!FirstCaching) {
			...
			while ((i = FindEntityByClassname(i, "obj_sentrygun"))!=INVALID_ENT_REFERENCE) {
				if (IsValidEntity(i)) {
					...
					if (engie < 1 || engie > MaxClients) {
						continue;
					}
					if (!IsClientInGame(engie)) {
						continue;
					}
					if (IsFriendly[engie]) {
						if (cvar_invuln_s < 2) {
	...
	if (hHandle == hcvar_alpha_proj || FirstCaching) {

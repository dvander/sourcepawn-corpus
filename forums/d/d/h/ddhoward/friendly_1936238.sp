/*
CHANGES MADE SINCE 13.0418.3
A player who uses one of this plugin's commands before the advert, will not see the advert
Added (client != 0) checks to the other commands, hurr durr
Full Goomba compatibility!
goomba.inc required
Reloading the map is dumb, just respawn
sm_friendly_enabled
*/
#pragma semicolon 1
...
public OnPluginStart() {
	...
	sm_friendly_enabled = CreateConVar("sm_friendly_enabled", "1", "(-1/0/1) Enables/Disables Friendly, please see the plugin's thread for info on usage.", FCVAR_PLUGIN);
	HookConVarChange(sm_friendly_enabled, Callback_EnabledConVarChanged);
...
public Action:OnToggleFriendly(client, args) {
	if(client != 0) {
		if (GetConVarInt(sm_friendly_enabled) == 1) {
...
public Callback_EnabledConVarChanged(Handle:sm_friendly_enabled, const String:oldValue[], const String:newValue[]) {
	if (GetConVarInt(sm_friendly_enabled) == 1) {
		CPrintToChatAll("%s An admin has re-enabled Friendly Mode. Type {olive}/friendly{default} to try it out!", CHAT_PREFIX);
	}
	if (GetConVarInt(sm_friendly_enabled) == 0) {
		for(new i=1; i<MAXPLAYERS+1; i++) {
			if (ClientIsFriendly[i]) {
				MakeClientHostile(i);
			}
		}
	...
	if (GetConVarInt(sm_friendly_enabled) == -1) {
		...
		SetConVarInt(sm_friendly_enabled, 0);
	}
}
...
public Updater_OnPluginUpdated() {
	if (GetConVarInt(sm_friendly_update) == 2) {
		ReloadPlugin();
		CPrintToChatAll("%s Update has been installed, Friendly mode may not work correctly until you respawn.", CHAT_PREFIX);
	}

/* Changes since 13.0803
	fixed an exploit that would allow players to avoid the forced weapon changes upon turning on FM
	fixed an error that would occur if a engie building that was spawned in a non-normal way took damage
	fixed a typo in the error message that occurs when sm_friendly_afkinterval is set to an invalid value
	slightly optimized IsValidClient function
To-do
...
#define PLUGIN_VERSION "13.1120.1"

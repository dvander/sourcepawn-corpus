#pragma semicolon 1
#define PLUGIN_VERSION "14.0208.0"
/* Changes since 14.0203.0
	removed needless include lines (sourcemod, tf2, sdktools)
	removed redundant initialization of handles to INVALID_HANDLE
	Changed OnClientDisconnect to OnClientDisconnect_Post so plugins using friendly.inc can safely use OnClientDisconnect
	removed stuff that doesn't need to be in OnClientPutInServer if it's in OnClientDisconnect_Post
	Removed HookClient function; just put it in OnClientPutInServer
	Removed UnhookClient function, SDKHooks does that automatically OnPluginEnd or OnClientDisconnect
	changed a several instances of "i" in loops to more descriptive variable names
	removed backwards compatibility for weapon blocking cvars
	Added forward TF2Friendly_OnFriendlySpawn()
	Added forwards TF2Friendly_OnPluginLoaded() and TF2Friendly_OnPluginUnloaded()
	Added forwards TF2Friendly_OnPluginEnabled() and TF2Friendly_OnPluginDisabled()
	Added native TF2Friendly_IsPluginEnabled()
	Added optional weapon resetting code, requires friendly.txt gamedata to function
	Removed SMLIB
	Removed some probably unneeded stuff OnPluginEnd (RemoveCommandListener, RemoveNormalSoundHook)
	Removed needless re-initialzation of client variables OnPluginStart
	Removed needless "result" variable on most forward calls
	Made those in sm_friendly_admin mode immune to exiting-friendly-punishments OnPluginEnd
	Removed pointless IsValidClient checks in 3 targeting filters
To-do
	...
	figure out how to make Friendly players invisible to bosses
	menu support?

...
Added sm_friendly_nobuild_* cvars, can be bypassed by admins
...
Added cvar sm_friendly_blockrtd with default value of 1
*/
#pragma semicolon 1
...
new Handle:sm_friendly_blockrtd = INVALID_HANDLE;
new Handle:sm_friendly_enabled = INVALID_HANDLE;
new Handle:sm_friendly_version = INVALID_HANDLE;
new Handle:sm_friendly_logging = INVALID_HANDLE;
...
new Handle:sm_friendly_nobuild_s = INVALID_HANDLE;
new Handle:sm_friendly_nobuild_d = INVALID_HANDLE;
new Handle:sm_friendly_nobuild_t = INVALID_HANDLE;
...
public OnPluginStart() {
	...
	sm_friendly_action_h = CreateConVar("sm_friendly_action_h", "-2", "(Any integer, -2 or greater) What action to take on living players who want to become Hostile? See this plugin's thread for details.", FCVAR_PLUGIN);
	sm_friendly_action_f = CreateConVar("sm_friendly_action_f", "-2", "(Any integer, -2 or greater) What action to take on living players who want to become Friendly? See this plugin's thread for details.", FCVAR_PLUGIN);
	...
	sm_friendly_blockweps = CreateConVar("sm_friendly_blockweps", "1", "(0/1) If enabled, a Friendly player will be unable to take out certain weapons.", FCVAR_PLUGIN);
	...
	sm_friendly_blockrtd = CreateConVar("sm_friendly_blockrtd", "1", "(0/1) If enabled, Friendly players will be unable to activate Roll The Dice.", FCVAR_PLUGIN);
	sm_friendly_logging = CreateConVar("sm_friendly_logging", "2", "(0/1/2) 0 = No logging, 1 = Will log use of sm_friendly_admin, 2 = Will log use of sm_friendly_admin and sm_friendly.", FCVAR_PLUGIN);
	...
	sm_friendly_killsentry_f = CreateConVar("sm_friendly_killsentry_f", "1", "(0/1) When enabled, an Engineer's sentry will vanish upon becoming Friendly.", FCVAR_PLUGIN);
	sm_friendly_killdispenser_f = CreateConVar("sm_friendly_killdispenser_f", "1", "(0/1) When enabled, an Engineer's dispenser will vanish upon becoming Friendly.", FCVAR_PLUGIN);
	sm_friendly_killtele_f = CreateConVar("sm_friendly_killtele_f", "1", "(0/1) When enabled, an Engineer's teleporters will vanish upon becoming Friendly.", FCVAR_PLUGIN);

	sm_friendly_nobuild_s = CreateConVar("sm_friendly_nobuild_s", "0", "(0/1) When enabled, a Friendly engineer will not be able to build sentries.", FCVAR_PLUGIN);
	sm_friendly_nobuild_d = CreateConVar("sm_friendly_nobuild_d", "1", "(0/1) When enabled, a Friendly engineer will not be able to build dispensers.", FCVAR_PLUGIN);
	sm_friendly_nobuild_t = CreateConVar("sm_friendly_nobuild_t", "0", "(0/1) a Friendly engineer will not be able to build teleporters.", FCVAR_PLUGIN);
...
UsedCommandAlive(const client) {
	if (ClientRequestedChange[client]) {
		...
		if (ClientIsFriendly[client]) {
			if ((GetConVarInt(sm_friendly_action_h) == 0) || ClientIsAdmin[client]) {
			...
			if ((GetConVarInt(sm_friendly_action_f) == 0) || ClientIsAdmin[client]) {
			...
			} else if (GetConVarInt(sm_friendly_action_f) == -2) {
				CPrintToChat(client, "%s You will be Friendly upon respawning.", CHAT_PREFIX);
				ClientRequestedChange[client] = true;
			} else if (GetConVarInt(sm_friendly_action_f) == -1) {
				CPrintToChat(client, "%s You will be Friendly upon respawning.", CHAT_PREFIX);
				ClientRequestedChange[client] = true;
				FakeClientCommand(client, "voicemenu 0 7"); //"No"
				ForcePlayerSuicide(client);
			} else if (GetConVarInt(sm_friendly_action_f) > 0) {
				MakeClientFriendly(client);
...
public Action:OnToggleAdmin(client, args) {
	if(client != 0) {
		...
		if (ClientIsAdmin[client]) {
			...
			if (GetConVarInt(sm_friendly_logging) > 0) {
...
MakeClientHostile(const client) {
	...
	if (GetConVarInt(sm_friendly_logging) == 2) {
...
MakeClientFriendly(const client) {
	...
	if ((GetConVarBool(sm_friendly_blockweps)) && (ClientIsAdmin[client] == false)) {
...
public Action:OnCPTouch(point, client) {
	if (client < 1 || client > MaxClients) {
		...
		if (ClientIsFriendly[client] && GetConVarBool(sm_friendly_stopcap) && (ClientIsAdmin[client] == false)) {
			return Plugin_Handled;
		} else {
			return Plugin_Continue;
		}
...
public Action:OnFlagTouch(point, client) {
	if (ClientIsFriendly[client] && GetConVarBool(sm_friendly_stopintel) && (ClientIsAdmin[client] == false)) {
		return Plugin_Handled;
	} else {
		return Plugin_Continue;
	}
...
public Action:Object_Built(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (ClientIsFriendly[client]) {
		if (StrEqual(b_classname, "obj_sentrygun")) {
			if ((GetConVarBool(sm_friendly_nobuild_s)) && (ClientIsAdmin[client] == false)) {
		...
		if (StrEqual(b_classname, "obj_dispenser")) {
			if ((GetConVarBool(sm_friendly_nobuild_d)) && (ClientIsAdmin[client] == false)) {
				AcceptEntityInput(building, "Kill");
		...
		if (StrEqual(b_classname, "obj_teleporter")) {
			if ((GetConVarBool(sm_friendly_nobuild_t)) && (ClientIsAdmin[client] == false)) {
				AcceptEntityInput(building, "Kill");
...
MakeBuildingsFriendly(const client) {
	...
	while ((sentrygun = FindEntityByClassname(sentrygun, "obj_sentrygun"))!=INVALID_ENT_REFERENCE) {
		if ((IsValidEntity(sentrygun)) && (GetEntPropEnt(sentrygun, Prop_Send, "m_hBuilder") == client)) {
			if ((GetConVarBool(sm_friendly_killsentry_f)) && (ClientIsAdmin[client] == false)) {
				AcceptEntityInput(sentrygun, "Kill");
			} else {
				if (GetConVarInt(sm_friendly_invuln_s) > 0) {
					if (GetConVarInt(sm_friendly_invuln_s) == 1) {
	...
	while ((dispenser = FindEntityByClassname(dispenser, "obj_dispenser"))!=INVALID_ENT_REFERENCE) {
		if ((IsValidEntity(dispenser)) && (GetEntPropEnt(dispenser, Prop_Send, "m_hBuilder") == client)) {
			if ((GetConVarBool(sm_friendly_killdispenser_f)) && (ClientIsAdmin[client] == false)) {
				AcceptEntityInput(dispenser, "Kill");
			} else {
				if (GetConVarInt(sm_friendly_invuln_d) > 0) {
					if (GetConVarInt(sm_friendly_invuln_d) == 1) {
	...
	while ((teleporter = FindEntityByClassname(teleporter, "obj_teleporter"))!=INVALID_ENT_REFERENCE) {
		if ((IsValidEntity(teleporter)) && (GetEntPropEnt(teleporter, Prop_Send, "m_hBuilder") == client)) {
			if ((GetConVarBool(sm_friendly_killtele_f)) && (ClientIsAdmin[client] == false)) {
				AcceptEntityInput(teleporter, "Kill");
			} else {
				if (GetConVarInt(sm_friendly_invuln_t) > 0) {
					if (GetConVarInt(sm_friendly_invuln_t) == 1) {
...
MakeBuildingsHostile(const client) {
	...
	while ((sentrygun = FindEntityByClassname(sentrygun, "obj_sentrygun"))!=INVALID_ENT_REFERENCE) {
		if ((IsValidEntity(sentrygun)) && (GetEntPropEnt(sentrygun, Prop_Send, "m_hBuilder") == client)) {
			if ((GetConVarBool(sm_friendly_killsentry)) && (ClientIsAdmin[client] == false)) {
				AcceptEntityInput(sentrygun, "Kill");
			} else {
				if (GetConVarInt(sm_friendly_invuln_s) == 2) {	
	...
	while ((dispenser = FindEntityByClassname(dispenser, "obj_dispenser"))!=INVALID_ENT_REFERENCE) {
		if ((IsValidEntity(dispenser)) && (GetEntPropEnt(dispenser, Prop_Send, "m_hBuilder") == client)) {
			if ((GetConVarBool(sm_friendly_killdispenser)) && (ClientIsAdmin[client] == false)) {
				AcceptEntityInput(dispenser, "Kill");
			} else {
				if (GetConVarInt(sm_friendly_invuln_d) == 2) {	
	...
	while ((teleporter = FindEntityByClassname(teleporter, "obj_teleporter"))!=INVALID_ENT_REFERENCE) {
		if ((IsValidEntity(teleporter)) && (GetEntPropEnt(teleporter, Prop_Send, "m_hBuilder") == client)) {
			if ((GetConVarBool(sm_friendly_killtele)) && (ClientIsAdmin[client] == false)) {
				AcceptEntityInput(teleporter, "Kill");
			} else {
				if (GetConVarInt(sm_friendly_invuln_t) == 2) {	
...
public RemoveActiveSapper(building, bool:ignoreadmin) {
	...
	while ((sapper = FindEntityByClassname(sapper, "obj_attachment_sapper"))!=INVALID_ENT_REFERENCE) {
		if(IsValidEntity(sapper) && GetEntPropEnt(sapper, Prop_Send, "m_hBuiltOnEntity") == building) {
			...
			if (ignoreadmin || (ClientIsAdmin[spy] == false)) {
				AcceptEntityInput(sapper, "Kill");
			}
		}
	}	
...
public Action:RTD_CanRollDice(client) {
	if ((ClientIsFriendly[client]) && (ClientIsAdmin[client] == false) && (GetConVarBool(sm_friendly_blockrtd))) {
...
public Action:OnWeaponSwitch(client, weapon) {
	...
	if(ClientIsFriendly[client] && GetConVarBool(sm_friendly_blockweps) && (ClientIsAdmin[client] == false)) {

...
* Added sm_spect_ex command -- targeting someone with this causes you to automatically spectate them every time they spawn. "sm_spect_ex 0" to stop spectating them.
* Added the ability to disable sm_spect for non-admins (sm_spect_players 0)
...
{
	RegConsoleCmd("sm_spect", Command_spec, "sm_spect <target> - Spectates a player.");
	RegConsoleCmd("sm_spect_ex", Command_spec_ex, "sm_spect_ex <target> - Constantly spectates a player. sm_spect_ex 0 to reset.");
	...
	/* CVARS (3) */
	cvarCanPlayersUse = CreateConVar("sm_spect_players","1","If on, non-admins can use sm_spect.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
public Action:Command_spec(client, args)
{
	...
	if (args != 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_spect <target> - Spectates a player.");
		return Plugin_Handled;
	}
	if (args == 1)
	{
...
public Action:Command_spec_ex(client, args)
{
	...
	if (args != 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_spect_ex <target> - Constantly spectates a player. sm_spect_ex 0 to reset.");
		return Plugin_Handled;
	}
	if (args == 1)
	{
...
public Action:Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (specTarget[z] == client) FakeClientCommand(z, "sm_spect #%i", GetClientUserId(client));
	}
...
public SpecMenuHandler(Handle:menu, MenuAction:action, client, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		else
		{
			...
			FakeClientCommand(client, "sm_spect #%i", UID);
		}
	}
}
...
public OnAdminMenuReady(Handle:topmenu)
{
	...
	if (playerCommands != INVALID_TOPMENUOBJECT) AddToTopMenu(hTopMenu, "sm_spect", TopMenuObject_Item, AdminMenu_Spec, playerCommands, "sm_spect", ADMFLAG_SLAY);
}
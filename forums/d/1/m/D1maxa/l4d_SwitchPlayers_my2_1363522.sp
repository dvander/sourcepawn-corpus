...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_switch", SwitchCommand, "Switch yourself to another team");
...
public Action:AfkCommand(client, Args)
{
	if (timeout[client])
	{
		PrintToChat(client, "%t", "Don't use afk/switch command too often");
		return;
	}
	timeout[client] = true;
...
AfkSwitch(client)
{
	if (GetClientTeam(client) == 1)
	{
		...
		if (oldteam == 2 || oldteam == 3)
		{
			...
			{
				new otherTeam = 5 - oldteam;
				if (IsTeamFull(otherTeam))
				{
				...
				{
					PrintToChat(client, "%t", "There is no place in your previous team so");
					PerformSwitch(client,client,otherTeam,false);
				}				
			}	
			else
			{
...
public Action:SwitchCommand(client, Args)
{
	if (timeout[client])
	{
		PrintToChat(client, "%t", "Don't use afk/switch command too often");
	...
	if (team == 1)
	{
		//PrintToChat(client, "%t", "Switch is available for survivors/infecteds only");	
		AfkSwitch(client);
	}
	else
	{		
		...
		if (IsTeamFull(otherTeam))
		{
			PrintToChat(client, "%t", "There is no place in another team");			
		}		
		else
		{
			if (otherTeam == 2)
			{
				...
				for (i = 1; i <= MaxClients; i++)
				{
					if (IsClientInGame(i) && IsFakeClient(i) &&
						GetClientTeam(i) == 2 && IsPlayerAlive(i))
						break;										
				}
				if (i > MaxClients)
				{
					PrintToChat(client, "%t", "There is no Alive Survivor bot");
					return;
				}
				else				
					KillInfected(client);										
			}			
			FakeClientCommandEx(client, "jointeam %i", otherTeam);	
			//check team changing
			new Handle:pack;
			CreateDataTimer(0.1,CheckClientTeam,pack,TIMER_FLAG_NO_MAPCHANGE);
			WritePackCell(pack, client);
			WritePackCell(pack, otherTeam);			
		}
	}
}

public Action:CheckClientTeam(Handle:timer, Handle:pack)
{
...
KillInfected (client)
{
	// If player was on infected and not ghost...

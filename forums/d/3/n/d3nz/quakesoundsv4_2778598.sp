...
public Action ShowQuakeMenu(int client) {

	Menu menu = new Menu(MenuHandlerQuake)
	new String:buffer[128]
	Format(buffer,sizeof(buffer),"%T","quake_menu",client)
	menu.SetTitle(buffer)

	if(soundPreference[client] >= 0)
	{
		Format(buffer, sizeof(buffer),"%T","disable_quake_sound", client)
		menu.AddItem("off", buffer)
	}
	else
	{
		Format(buffer, sizeof(buffer),"%T","enable_quake_sound", client)
		menu.AddItem("on", buffer)
	}

	menu.Display(client,15);
}

public int MenuHandlerQuake(Menu menu, MenuAction action, int param1, int param2) {
	switch(action) {
		case MenuAction_Select:
		{
			char item[32]
			menu.GetItem(param2, item, sizeof(item))
			if (StrEqual(item, "off")) {
				soundPreference[param1] = -1
			} else if (StrEqual(item, "on")) {
...
// If Quake Prefs Menu was selected from the clientprefs menu (!settings)
...
// Plays round play sound depending on each players config
		...
		else
		{
			...
			for(new i = 1; i <= GetMaxHumanPlayers(); i++)
			{
				if(IsClientInGame(i) && !IsFakeClient(i) && soundPreference[i] >= 0)
				{
					...
					else if(firstblood && firstbloodConfig[soundPreference[i]] > 0)
					{

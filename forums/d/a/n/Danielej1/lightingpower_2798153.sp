...
{
	RegAdminCmd("sm_thunder", THUNDER, ADMFLAG_SLAY);
}

public Action:THUNDER(clients,args)
{
    HookEvent("player_death", player_death);
    HookEvent("round_freeze_end", round_freeze_end);
}
bool enableHooks[MAXPLAYERS+1];

OnCommand(int client) 
{
enableHooks[client] = !enableHooks[client];
}

OnHook(client) 
{
if(enableHooks[client]) 
	{
	RegAdminCmd("sm_thunder", THUNDER, ADMFLAG_SLAY);
	}
}

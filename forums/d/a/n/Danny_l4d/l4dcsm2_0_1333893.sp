...
static bool:g_bL4D1Campaign;
static bool:g_bPlayerEnteredCheckPoint[MAXPLAYERS+1];
...
{	
	...
	// Survivor cvar
	new Handle:SurvivorModels = CreateConVar("l4d_csm_survivor_model_access", "1","1 = change to character (you become a clone). 2 = change model (look like the new character, but still your original character). 3 = Allow both 1 and 2.",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0, true, 3.0 );
	g_iSurvivorModels = GetConVarInt(SurvivorModels);
	HookConVarChange(SurvivorModels, _ConVarChange__SurvivorModels);
	
	// Infected cvar
	new Handle:InfectedModels = CreateConVar("l4d_csm_infected_models_access", "0", "1 = Can change character only. 2 = Change model only (look like the new character, but still your original character). 3 = Anybody can change character or model.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 3.0);
	...
	{
		// L4D1 Survivors cvar
		new Handle:L4D1Survivors = CreateConVar("l4d_csm_l4d1_survivors", "0","Toggles access to L4D1 Survivors in L4D2.",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0, true, 1.0);
...
public OnMapStart()
{
	...
	if (g_bIsL4D2)
	{
		...
		// If the campaign is a L4D1 campaign, don't allow the L4D2 Survivors to be playable (because they aren't anyway).
		
		if (StrContains(CurrentMap, "c7m", true) != -1 || StrContains(CurrentMap, "c8m", true) != -1)
			g_bL4D1Campaign = true;
...
public Action:Event_PlayerEnteredCheckpoint(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_iStartCheckpointDoor == 0)
		g_iStartCheckpointDoor = door;
	else if (g_iStartCheckpointDoor != door)
	{
		g_bPlayerEnteredCheckPoint[client] = true;
		#if DEBUG
	...
	#if DEBUG
	PrintToChatAll("Start Checkpoint door Entity index: %i", g_iStartCheckpointDoor);
	#endif
}
...
public Action:Event_PlayerLeftCheckpoint(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	g_bPlayerEnteredCheckPoint[client] = false;
...
{	
	// Part of the Saved Character mechanism. 
	...
	for (new client=1;client<=MaxClients;client++)
	{
		if (IsClientInGame(client))
		{
			if (GetClientTeam(client) == TEAM_SURVIVORS)
			{
				if (g_bPlayerEnteredCheckPoint[client])
					PlayersEnteredCheckpoint++;
			}
		}
	}
	
	if (PlayersEnteredCheckpoint != NumberOfSurvivorsExcludeDead())
		return;
...
public OnMapEnd()
{
	...
	// This is reset incase Bill's timer set this to 0.
...
public Action:InitiateMenuAdmin(client, args) 
{
	...
	if (g_iSurvivorModels == 0 && g_iInfectedModels == 0)
	{
...
public Action:ShowMenu(client, args) 
{
	...
	if(g_bAdminsOnly && admin == INVALID_ADMIN_ID)
	{
...
stock SetClientClassModel(client, character) 
{
	switch(character) 
	{
		...
		case JIMMY_GIBBS_JR_MODEL:		{	SetEntityModel(client, "models/infected/common_male_jimmy.mdl.mdl");		}
	...
	for (new client=1;client<=MaxClients;client++)
	{
		...
		if (GetClientTeam(client) == TEAM_SURVIVORS)
		{
			if (GET_BILL_NETPROP)
			{
				...
				SetEntProp(client, Prop_Send, "m_survivorCharacter", 11);
				g_bBillIsChanging = false;
				billexists = true;
				g_bWasBill[client] = true;
			}
...
stock SetUpInfected(param1, Infected)
{
	...
	SetEntProp(param1, Prop_Send, "m_iHealth", 100);

...
public OnPluginStart()
{
	...
	version = CreateConVar("l4d_loadingVersion", PLUGIN_VERSION, "Version of the loading plugin", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_REPLICATED);
...
#if DEBUG_DOOR
	PrintDebugMessage(0, -1, "[DEBUG] Gamemode [ current = %s | desired = %s | contained = %i ]", gamemode, gamemodeactive, StrContains(gamemodeactive, gamemode));
#endif
...
public Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetConVarInt(autoCycle) != 0 && StrContains(map, "05") != -1 && StrContains(gamemode, "versus", false) != -1) {
		if (roundStarted == 2) {
			...
			#if DEBUG_CHMAP
			PrintDebugMessage("[DEBUG] Ending finale round #%i (%i)", roundStarted, roundCounter);
			#endif
	...
	if (GetConVarInt(autoCycle) != 0 && StrContains(map, "02") != -1 && StrContains(map, "") != -1 && StrContains(gamemode, "versus", false) != -1) {
		if (roundStarted == 2) {
			roundCounter++;
...
public Event_PlayerUse(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	PrintDebugMessage(0, -1, "Client %N used entity %i", client, ent);
	if (IsCountDownStoppedOrRunning()) PrintDebugMessage(0, -1, "Countdown running");
	if (IsCheckPointDoor(ent)) PrintDebugMessage(0, -1, "Found check point door: %i", ent);
	#endif
}
...
public Event_PlayerLeftStartArea(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	PrintDebugMessage(0, -1, "Player %L left start area [ countdown = %i | angles = %s | door = %i ]", client, countDown, checkPointDoorEntityStartAngles, checkPointDoorEntityStart);
	
	if (GetConVarBool(scrimMode) && IsCountDownStoppedOrRunning()) PrintDebugMessage(0, -1, "Event_PlayerLeftStartArea(): failure on preventing to leave start area before countdown end");
	#endif
}
...
public Event_PlayerActivate(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (!isFirstMap && IsCountDownStoppedOrRunning() && isFirstPlayer) {
		...
		PrintDebugMessage(0, -1, "[DEBUG] First player (%L) joined the game -> GetCheckPointDoorStart()", client);
#endif
		...
		if (foundCampaigns != INVALID_HANDLE && (foundItems = GetArraySize(foundCampaigns)) > 0) {
			...
			if (campaignId >= foundItems || campaignId < 0) {
				...
				if (strcmp("Dead Air", tmp[0]) != -1) {
					...
					forceNext = "garage";
				} else if (strcmp("Vienna Calling", tmp[0]) != -1) {
					forceNext = "vienna";
				}
			...
			if (strcmp(forceNextArg, "cc") == 0 || StrContains("Vienna calling", forceNextArg, false) != -1 || StrContains("vienna", forceNextArg, false) != -1) {
				forceNext = "vienna";
				PushArrayString(foundCampaigns, "Vienna Calling");
...
#if DEBUG_DOOR
		PrintDebugMessage(0, -1, "[DEBUG] Command_DumpCPD(2): [ prop = %s | type = %s ]", prop, type);
#endif
...
#if DEBUG_DOOR
		PrintDebugMessage(0, -1, "[DEBUG] Command_DumpCPD(3): [ prop = %s | type = %s | ent = %i ]", prop, type, ent);
#endif
...
#if DEBUG_DOOR
		PrintDebugMessage(0, -1, "[DEBUG] Command_DumpCPD(4): [ prop = %s | type = %s | ent = %i | size = %i ]", prop, type, ent, size);
#endif
...
#if DEBUG_CHMAP
					PrintDebugMessage("[DEBUG] Campaign vote initialized");
#endif
				}
#if DEBUG_CHMAP
				else PrintDebugMessage("[DEBUG] Campaign vote inactive or not last chapter");
#endif
			}
#if DEBUG_CHMAP
			else if (strlen(forceNext) != 0) PrintDebugMessage("[DEBUG] Next campaign is set to '%s' already (vote cast or setnext used)", forceNext);
			else if (!isFirstRound) PrintDebugMessage("[DEBUG] Campaign vote not cast on second round");
			else PrintDebugMessage("[DEBUG] Campaign vote error");
#endif
		}
		if (countDown == 1) {
			if (isFirstRound && strlen(forceNext) == 0) {
				GetCurrentMap(map, sizeof(map));
				if (StrContains(map, "02") != -1 && StrContains(map, "") != -1 && GetConVarInt(autoCycle) == 2) {
					if (strlen(forceNext) != 0) PrintToChatAll("[DEBUG] Next campaign is set to '%s' already (vote cast or setnext used)", forceNext);
					else {
						strcopy(g_l4dVoteType, sizeof(g_l4dVoteType), "campaign");
						DisplayCampaignVote();
...
#if DEBUG_SCRIM || DEBUG_DOOR
			else PrintDebugMessage(0, -1, "[DEBUG] FreezingTimer(): Skipping freeze of %L [ countdown = %i ]", client, countDown);
#endif
		}
	}
	return Plugin_Continue;
...
bool:IsRightMapForGameMode(const String:map[])
{
	...
	if (StrContains(map, "l4d_garage", false) != -1 || StrContains(map, "l4d_viennacalling", false) != -1) return true;
...
GetNextCampaign(String:nextcampaign[64])
{
	...
	if (strcmp(forceNext, "garage") == 0) strcopy(nextcampaign, 64, "Crash Course");
	if (strcmp(forceNext, "vienna") == 0) strcopy(nextcampaign, 64, "Vienna Calling");
}

PrintDebugMessage(const String:format[], any:...)
{
	decl String:buffer[192];
	
	VFormat(buffer, sizeof(buffer), format, 2);

	if (DEBUG_TYPE == dbgtype:DEBUG_LOG) LogAction(0, -1, buffer);
	else if (DEBUG_TYPE == dbgtype:DEBUG_CHAT) PrintToChatAll(buffer);
}
...
#if DEBUG_CHMAP
				PrintDebugMessage("[DEBUG] Player %L is %s", i, isIncapacitated[i] ? "down" : "on his feet");
#endif
			}
		}
	}
...
#if DEBUG_CHMAP
	PrintDebugMessage("[DEBUG] Found %i alive players (%i incapacitated) - finale %s", aliveCount, incapCount, aliveCount > 0 ? "won" : "lost");
#endif
...
#if DEBUG_SCRIM
		PrintDebugMessage("[DEBUG] %s = %s", convar, defVal);
#endif
...
#if DEBUG_DOOR
					PrintDebugMessage(0, -1, "[DEBUG] survivor [ player = %L | origin = %f %f %f | origin = %s | sum = %i ]", i, origin[0], origin[1], origin[2], vec, sum);
#endif
				}
#if DEBUG_DOOR
				else PrintDebugMessage(0, -1, "[DEBUG] survivor [ player = %L | sum = %i ]", i, sum);
#endif
			}
		}
	}
...
#if DEBUG_DOOR
			PrintDebugMessage(0, -1, "[DEBUG] Found prop_door_rotating_checkpoint [ angles = %f %f %f | origin = %f %f %f | spawnflags = %i ]", angles[i][0], angles[i][1], angles[i][2], checkPointDoorEntityOrigin[i][0], checkPointDoorEntityOrigin[i][1], checkPointDoorEntityOrigin[i][2], flags[i]);
			DumpCPDEntity("m_angAbsRotation", "vec", checkPointDoorEntityIds[i]);
#endif
		}
#if DEBUG_DOOR
		else PrintDebugMessage(0, -1, "[DEBUG] Invalid entity id %i", checkPointDoorEntityIds[i]);
#endif
	}
...
#if DEBUG_DOOR
				PrintDebugMessage(0, -1, "[DEBUG] survivor origin :  %s", svOrigin);
				PrintDebugMessage(0, -1, "[DEBUG] checkpoint entity :  %i", checkPointDoorEntityIds[i]);
				PrintDebugMessage(0, -1, "[DEBUG] checkpoint origin :  %f %f %f", checkPointDoorEntityOrigin[i][0], checkPointDoorEntityOrigin[i][1], checkPointDoorEntityOrigin[i][2]);
				PrintDebugMessage(0, -1, "[DEBUG] distance #%i :  %f", i, dist[i]);
#endif
...
#if DEBUG_DOOR
			PrintDebugMessage(0, -1, "Determined starting safe room door entity :  %i [ angles = %s ]", checkPointDoorEntityStart, checkPointDoorEntityStartAngles);
#endif
...
DisplayCampaignVote()
{
	...
	if (strcmp(map, "garage") != 0) AddMenuItem(g_l4dVoteMenu, "garage", "Crash Course");
	if (strcmp(map, "vienna") != 0) AddMenuItem(g_l4dVoteMenu, "vienna", "Vienna Calling");
	...
	if (action == MenuAction_End) {
		...
		if (strcmp(g_l4dVoteType, "scrim") == 0 && (strcmp(item, VOTE_YES) == 0 && FloatCompare(percent,limit) < 0 && param1 == 0) || (strcmp(item, VOTE_NO) == 0 && param1 == 1)) {
			...
			if (strcmp(g_l4dVoteType, "scrim") == 0) {
				...
				#if DEBUG_CHMAP
				PrintDebugMessage("[DEBUG] Vote properties [ limit = %i | percent = %i | votes = %i ]", g_scrimVoteType, RoundToNearest(100.0*limit), RoundToNearest(100.0*percent), totalVotes);
				#endif

				strcopy(forceNext, sizeof(forceNext), item);
				#if DEBUG_CHMAP
				PrintDebugMessage("[DEBUG] Next campaign changed to '%s'", display);
				#endif
			}
		}
	}

...
public OnPluginStart()
{
	...
	HookEvent("finale_win", Event_FinalWinRL);
	...
	ChDelayRL = CreateConVar("sm_l4d_fmc_chdelayrl", "0.0", "Delay before realism mission change (float in sec).");
...
public Action:Event_FinalWinRL(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GetConVarInt(DebugEvent) == 1)
	{
		PrintToChatAll("\x04[FMC DEBUG]\x03 MODE: \"%d\" EVENT: \"%s\" ", l4d_gamemode(), name);
	...
	if(GetConVarInt(Allowed) == 1 && l4d_gamemode() == 4 && StrEqual(next_mission_force, "none") != true)
		CreateTimer(GetConVarFloat(ChDelayRL), TimerChDelayRL);
}
...
public Action:TimerAnnounce(Handle:timer, any:client)
{
	if(IsClientInGame(client))
	{
		...
		{
			PrintToChat(client, "\x04[FMC]\x03 Is a finale of mission. Finish them all!");
...
public Action:TimerChDelayRL(Handle:timer)
{
	ServerCommand("changelevel %s", next_mission_force);
}
...
l4d_gamemode()
{
	// 1 - coop / 2 - versus / 3 - survival / 4 - realism / or false (thx DDR Khat for code)
	new String:gmode[32];
...
PluginInitialization()
{
	...
	{
		BuildPath(Path_SM, FMC_FileSettings, 128, "data/sm_l4drl_mapchanger.txt");
		PrintToServer("[FMC] Discovered realism gamemode. Link to sm_l4drl_mapchanger.");
	}
	else

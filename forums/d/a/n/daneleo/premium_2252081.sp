...
public Player_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(cEnable && IsValidEntity(victim) && IsValidEntity(attacker) && attacker != victim && Class == TFClass_Spy)
	{
		...
		if(IsPlayerAlive(attacker) && (GetUserFlagBits(attacker) & PREMIUMFLAG || pTrial[attacker] || GetUserFlagBits(attacker) & ROOTFLAG)  && cFakeKill)
		{
			if(weaponindex == 59)
			{			
...
public Action:OnPlayerRunCmd(iClient, &iButtons, &iImpulse, Float:fVelocity[3], Float:fAngles[3], &iWeapon)
{
	...
	if(GetUserFlagBits(iClient) & PREMIUMFLAG || pTrial[iClient] || GetUserFlagBits(iClient) & ROOTFLAG && cSpeed)
    {
		...
		case TFClass_Heavy:
			{
				SetEntPropFloat(iClient, Prop_Send, "m_flMaxspeed", HeavySpeed);
			}
		}
	}
	return Plugin_Continue;
...
public Action:Say(client, args)
{
	...
	{		
		...
		if(StrEqual(message, "!trial"))
		{
			if(pTrial[client] == true)
			{
				CPrintToChat(client, "%s Your trial is active!", TAG);
				return Plugin_Handled;
			}

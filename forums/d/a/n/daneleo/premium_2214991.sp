...
#define PLUGIN_VERSION "1.15.8.5"
...
new colorred[MAXPLAYERS+1];
new colorgreen[MAXPLAYERS+1];
new colorblue[MAXPLAYERS+1];
...
public Player_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if(GetUserFlagBits(attacker) & PREMIUMFLAG || pTrial[attacker] || GetUserFlagBits(attacker) & ROOTFLAG  && cFakeKill)
		{
			if(weaponindex == 59)
			{			
...
public OnGameFrame()
{
	for(new i = 1; i <= GetMaxClients(); i++)
	{
		...
		{
		 if(pTrial[i] || GetUserFlagBits(i) & PREMIUMFLAG || GetUserFlagBits(i) & ROOTFLAG)
		 {
...
public Action:Command_ResetTrial(client, args)
{
	...
	for (new i = 0; i <target_count; i++)
	{
		...
		ReplyToCommand(client, "[Premium] %s's Trial has been reset", target_list[i]);

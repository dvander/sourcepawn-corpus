...
public Plugin:myinfo = 
{
	...
	author = "chinagreenelvis (Based on the Self Help plugin by Pan Xiaohai) Edited by Danky_Bong",
...
new Handle:selfrevive_eachother = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	selfrevive_ledge = CreateConVar("selfrevive_ledge", "1", "Self revive for ledge grabs, 0:Disable, 1:Pill, 2:Medkit, 3:Both", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);		
	selfrevive_incap = CreateConVar("selfrevive_incap", "1", "Self revive for incapacitation, 0:Disable, 1:Pill, 2:Medkit, 3:Both", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	selfrevive_grab = CreateConVar("selfrevive_grab", "1", "Self revive for smoker grab, 0:Disable, 1:Pill, 2:Medkit, 3:Both", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	selfrevive_pounce = CreateConVar("selfrevive_pounce", "1", "Self revive for hunter pounce, 0:Disable, 1:Pill, 2:Medkit, 3:Both", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	selfrevive_ride = CreateConVar("selfrevive_ride", "1", " Self revive for jockey ride, 0:Disable, 1:Pill, 2:Medkit, 3:Both", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	selfrevive_pummel = CreateConVar("selfrevive_pummel", "1", "Self revive for charger pummel , 0:Disable, 1:Pill, 2:Medkit, 3:Both", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	selfrevive_eachother =CreateConVar("selfrevive_eachother", "1", "Can incapped survivors help eachother?, 0:Disable, 1:Enable" , FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
...
public Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		HelpOtherState[x] = HelpState[x] = STATE_OK;
		Attacker[x] = 0;
		HelpStartTime[x] = 0.0;
...
public OnClientDisconnect()
{
	...
	HelpOtherState[client] = HelpState[client] = STATE_OK;
...
public Event_TongueGrab (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	HelpOtherState[victim] = HelpState[victim] = STATE_NONE;
...
RestartTimer(client)
{
	if (client)
	{
		...
		{
			Timers[client] = CreateTimer(float(GetConVarInt(selfrevive_delay_ledge)), Timer_Delay, client);
		}
		if (IncapType[client] == INCAP)
		{
			Timers[client] = CreateTimer(float(GetConVarInt(selfrevive_delay_incap)), Timer_Delay, client);
		}
	}
}
...
{	
	...
	if(slot!=-1)
	{
		...
		if(slot ==4)
		{
			if(GetConVarInt(selfrevive_kill)>0) KillAttacker(client);

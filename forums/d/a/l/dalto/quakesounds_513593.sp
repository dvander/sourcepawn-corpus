...
Versions:
	...
	1.3.1
		...
		* Fixed array out of nounds messages in PlayQuakeSounds()
*/
...
public EventPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(attackerClient && victimClient && GetClientTeam(attackerClient) == GetClientTeam(victimClient) && attackerId != victimId)
		soundId = TEAMKILL;
...
{
	PrintToServer("I am in the panel handler");
...
{
	PrintToServer("I entered the Quake Panel function");
	...
	if(soundPreference[client] == 0)
 	...
 	PrintToServer("I am about to send the panel to client");
...
public LoadSounds()
{
	...
	for(new i = 0; i < NUM_SOUNDS; i++) {
		...
		if(StrEqual(soundsList[0][i], ""))
			PrintToServer("Failed to load %s:%s", "standard", soundNames[i]);
		KvGetString(kvQSL, "female", soundsList[1][i], MAX_FILE_LEN);
		if(StrEqual(soundsList[1][i], ""))
			PrintToServer("Failed to load %s:%s", "female", soundNames[i]);
		settingsArray[i] = KvGetNum(kvQSL, "config", 9);
	}

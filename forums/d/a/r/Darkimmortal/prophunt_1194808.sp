...
// Rush Mode - currently not working
// Default: OFF
//#define RUSH
...
#if defined RUSH
new Handle:g_PHRush = INVALID_HANDLE;
#endif
...
public OnPluginStart()
{
	...
	if(GetExtensionFileStatus("runteamlogic.ext") < 1)
		SetFailState("RunTeamLogic is not loaded.");
	...
	g_PHEnable = CreateConVar("ph_enable", "1", "Enable the PropHunt plugin");
	g_PHPropMenu = CreateConVar("ph_propmenu", "0", "Allows non-admin players to access the prop menu");
#if defined RUSH
	g_PHRush = CreateConVar("ph_rush", "1", "Enable the Prop Rush mode", FCVAR_REPLICATED|FCVAR_NOTIFY);
	HookConVarChange(g_PHRush, OnConVarChanged_PHRush);
#endif
	HookConVarChange(g_PHEnable, OnConVarChanged_PHEnable);
	...
	SetConVarInt(FindConVar("tf_show_voice_icons"), 0, true);
...
#if defined STATS
	new String:statsmotd[128];
	BuildPath(Path_SM, statsmotd, sizeof(statsmotd), "plugins/prophunt.txt");
	SetConVarString(FindConVar("motdfile"), statsmotd);
	Stats_Init();
#endif
	...
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_flamethrower", 1);
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_minigun", 5);
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_shotgun_hwg", 8);
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_shotgun_soldier", 8);
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_shotgun_pyro", 8);
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_pistol_scout", 1);
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_pistol", 8);
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_syringegun_medic", 1);
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_smg", 1);
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_flaregun", 5);
...
#if defined RUSH
public OnConVarChanged_PHRush(Handle:hCvar, const String:oldValue[], const String:newValue[])
{
	g_PropRush = StringToInt(newValue) == 1;
}
#endif

public OnConVarChanged_PHEnable(Handle:hCvar, const String:oldValue[], const String:newValue[])
{
...
#if defined RUSH
		KvGetString(fl, "alt", buffer, sizeof(buffer), "0");
		SetConVarInt(g_PHRush, strcmp(buffer, "1") == 0 ? 1:0);
#endif
...
public Action:TakeDamageHook(client, &attacker, &inflictor, &Float:damage, &damagetype)
{
	   ...
	   && IsClientInGame(attacker) && GetClientTeam(attacker) == TEAM_BLUE)
	{
		...
		if(IsValidEntity(GetEntPropEnt(attacker, Prop_Send, "m_hActiveWeapon")) )
		{
			...
			{
			case WEP_HUNTSMAN, WEP_SHOTGUNHEAVY, WEP_SHOTGUNSOLLY, WEP_SHOTGUNENGINEER: {
				damage = FloatMul(damage, 0.8);
			}
			case WEP_BACKBURNER, WEP_MINIGUN: {
				damage = FloatMul(damage, 0.6);
			}
			case WEP_SHOTGUNPYRO: {
				damage = FloatMul(damage, 0.75);
			}
			case WEP_ROCKETLAUNCHER, WEP_DIRECTHIT: {
				damage = FloatMul(damage, 0.7);
			}
			}
			return Plugin_Changed;
		}
	...
	//block prop drowning
	if(damagetype == DMG_DROWN && client > 0 && client < MaxClients && IsClientInGame(client) && GetClientTeam(client) == TEAM_RED)
	{
		damage = 0.0;
		return Plugin_Changed;
	}
...
{
	if(GetConVarInt(g_PHPropMenu) == 1 || GetAdminFlag(GetUserAdmin(client), Admin_Kick) /*GetUserFlagBits(client) & ADMFLAG_KICK*/)
	{
...
public Handler_PropMenu(Handle:menu, MenuAction:action, param1, param2)
{
	...
	case MenuAction_Select:{
			...
			{
				if(GetConVarInt(g_PHPropMenu) == 1 || GetAdminFlag(GetUserAdmin(param1), Admin_Kick))
				{
		...
		if(IsPlayerAlive(client))
		{
			...
			if(GetClientTeam(client) == TEAM_RED)
			{
				...
				if(((((buttons & IN_ATTACK) == IN_ATTACK && (buttons & IN_DUCK) != IN_DUCK) ||
				     ((buttons & IN_FORWARD) == IN_FORWARD || (buttons & IN_LEFT) == IN_LEFT || (buttons & IN_RIGHT) == IN_RIGHT ||
			...
			{
				if(IsValidEntity(GetPlayerWeaponSlot(client, 1)) && GetEntProp(GetPlayerWeaponSlot(client, 1), Prop_Send, "m_iItemDefinitionIndex") == WEP_SHOTGUNPYRO)
				{
...
#if defined RUSH
		SetupRoundTime(g_PropRush ? g_RoundTime/5:g_RoundTime);
#else
...
public AddTime(time)
{
	new roundTimerEnt = FindEntityByClassname(-1, "team_round_timer");
	PrintToServer("HERP DE DERP LOLOL %i", roundTimerEnt);

	if(roundTimerEnt > -1 && IsValidEntity(roundTimerEnt))
	{
		decl String:addtime[8];
		Format(addtime, sizeof(addtime), "%i %i", TEAM_RED, time);
		SetVariantString("999");
		AcceptEntityInput(roundTimerEnt, "SetTime");
		PrintToServer(addtime);
	}
}
#endif
...
public Action:Timer_DoEquipBlu(Handle:timer, any:client)
{
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		if(TF2_GetPlayerClass(client) == TFClass_Sniper)
		{
			...
			if(slot1 > MaxClients && IsValidEntity(slot1) && GetEntProp(slot1, Prop_Send, "m_iItemDefinitionIndex") == WEP_SMG)
			{
		...
		if(TF2_GetPlayerClass(client) == TFClass_Pyro)
		{
			if(slot1 > MaxClients && IsValidEntity(slot1) && GetEntProp(slot1, Prop_Send, "m_iItemDefinitionIndex") == WEP_SHOTGUNPYRO)
			{
				SetEntData(GetPlayerWeaponSlot(client, 1), FindSendPropInfo("CBaseCombatWeapon", "m_iClip1"), SHOTGUN_MAX_AMMO);
			}
		}
		...
		{
			if(slot1 > MaxClients && IsValidEntity(slot1) && GetEntProp(slot1, Prop_Send, "m_iItemDefinitionIndex") == WEP_ENGINIEERPISTOL)
			{
				TF2_RemoveWeaponSlot(client, 1);
			}
		}
...
public Action:Timer_DoEquip(Handle:timer, any:client)
{
	...
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		decl String:nicemodel[MAXMODELNAME];
...
#if defined RUSH
		if(playas > 2 && g_PropRush)
		{
			EmitSoundToAll(GetRandomInt(0,1) == 0 ? SOUND_TIMEADDED1:SOUND_TIMEADDED2, _, _, SNDLEVEL_AIRCRAFT);
			AddRoundTime(g_RoundTime/10);
		}
#endif
	}
	...
	if(playas == 2 && !g_RoundOver && GetClientTeam(client) == TEAM_RED)
	{
		...
		for(new client2=1; client2 <= MaxClients; client2++)
		{
			if(IsClientInGame(client2) && !IsFakeClient(client2) && IsPlayerAlive(client2))
			{
				...
				{
					TF2_AddCondition(client2, TFCond_Jarated, 900.0);
				}
			}
		}
#endif

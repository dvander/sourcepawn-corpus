...
public Action:OnPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (((GetConVarInt(kitcount) == 0) && (GetConVarInt(kitcounthealth) == 0) && (GetConVarInt(kitcountgrenade) == 0)) && (GetEntityCount() < GetMaxEntities() - 64))
	{
		...
		if (randomdeath == 11)
		{
			...
			new healthkit = CreateEntityByName("prop_physics_override")
			SetEntityModel(healthkit,g_HealthKit_Model[GetConVarInt(dropmodelhealth)])
			SetEntProp(healthkit, Prop_Send, "m_nSkin", g_HealthKit_Skin[0])
			if (DispatchSpawn(healthkit))
			{
				SetEntProp(healthkit, Prop_Send, "m_usSolidFlags",  152)
			...
			TeleportEntity(healthkit, deathorigin, NULL_VECTOR, NULL_VECTOR)
			SDKHook(healthkit, SDKHook_Touch, OnHealthKitTouchedHealth)
			HealthKitDropTimer[healthkit] = CreateTimer(GetConVarFloat(kittime), RemoveDroppedHealthKit, healthkit, TIMER_FLAG_NO_MAPCHANGE)
			if (GetConVarInt(messagedropenabled) == 1)
			{
				PrintToChatAll("\x01\x0B\x02 %t", "player_drop_health", client, GetConVarInt(kithealth))
				PrintCenterTextAll("%t", "player_drop_health", client, GetConVarInt(kithealth))
			}
			return Plugin_Continue
		}
		
		// DROP RANDOM HEGRENADE
		if (randomdeath == 7)
		{
			new client = GetClientOfUserId(GetEventInt(event, "userid"))
			new Float:deathorigin[3]
			GetClientAbsOrigin(client,deathorigin)
			deathorigin[2] += 5.0 // above
			deathorigin[1] += 0.0 // + = left from front
			deathorigin[0] -= 100.0 // + = front
			new healthkit = CreateEntityByName("prop_physics_override")
			SetEntityModel(healthkit,g_HealthKit_Model[GetConVarInt(dropmodelgrenade)])
			SetEntProp(healthkit, Prop_Send, "m_nSkin", g_HealthKit_Skin[0])
			if (DispatchSpawn(healthkit))
			{
				SetEntProp(healthkit, Prop_Send, "m_usSolidFlags",  152)
			...
			TeleportEntity(healthkit, deathorigin, NULL_VECTOR, NULL_VECTOR)
			SDKHook(healthkit, SDKHook_Touch, OnHealthKitTouchedGrenade)
			HealthKitDropTimer[healthkit] = CreateTimer(GetConVarFloat(kittime), RemoveDroppedHealthKit, healthkit, TIMER_FLAG_NO_MAPCHANGE)
			if (GetConVarInt(messagedropenabled) == 1)
			{
				PrintToChatAll("\x01\x0B\x02 %t", "player_drop_grenade", client)
				PrintCenterTextAll("%t", "player_drop_grenade", client)
			}
			return Plugin_Continue
		}
	}
	...
	// DROP HEALTH PACK
	if ((GetConVarInt(kitcounthealth) == kitcountcounterhealth) && (GetEntityCount() < GetMaxEntities() - 64))
	{
		new client = GetClientOfUserId(GetEventInt(event, "userid"))
		new Float:deathorigin[3]
		GetClientAbsOrigin(client,deathorigin)
	...
	if (GetConVarInt(kitcounthealth) == kitcountcounterhealth)
	{
		LogMessage("Healthcounter was %d ! Error - reset to 0", kitcountcounterhealth)
		kitcountcounterhealth = 0
	}
		
	// DROP HEGRENADES
	if ((GetConVarInt(kitcountgrenade) == kitcountcountergrenade) && (GetEntityCount() < GetMaxEntities() - 64))
	{
		new client = GetClientOfUserId(GetEventInt(event, "userid"))
		new Float:deathorigin[3]
		GetClientAbsOrigin(client,deathorigin)
	...
	if (GetConVarInt(kitcountgrenade) == kitcountcountergrenade)
	{
		LogMessage("HEGrenade counter was %d ! Error - reset to 0", kitcountcountergrenade)
		kitcountcountergrenade = 0
	}
	return Plugin_Continue
}
...
public Action:OnHealthKitTouchedHealth(healthkit, client)
{
	if(IsValidClient(client) && !IsFakeClient(client))
	{
		// READ SPEED OF CLIENT
		...
		if (health < healthkitmax)
		{
			...
			// GIVE SPEED BACK TO CLIENT
			SetEntPropFloat(client, Prop_Send, "m_flLaggedMovementValue", Speed)
			
			if (GetConVarInt(messagepickupenabled) == 1)
			{
				PrintToChat(client, " \x04 %t", "player_pickup_health", GetConVarInt(kithealth))
				PrintCenterText(client, "%t", "player_pickup_health", GetConVarInt(kithealth))
			}
			return Plugin_Handled
		}	
	}
...
public Action:OnHealthKitTouchedGrenade(healthkit, client)
{
	if(IsValidClient(client) && !IsFakeClient(client))
	{
		// READ SPEED OF CLIENT
		...
		if (GetConVarInt(messagepickupenabled) == 1)
		{
			PrintToChat(client, " \x04 %t", "player_pickup_grenade", GetConVarInt(kithealth))
			PrintCenterText(client, "%t", "player_pickup_grenade", GetConVarInt(kithealth))
		}
		if (GetConVarInt(UseOwnPickUpSound) == 0)
		{
			PlayPickUpSound(client)

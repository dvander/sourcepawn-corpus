...
public int Category_Handler(TopMenu topmenu, TopMenuAction action, TopMenuObject topobj_id, int param, char[] buffer, int maxlength)
{
	...
	{
		Format(buffer, maxlength, "Choose a task:");
	}
	else if(action == TopMenuAction_DisplayOption)
	{
		Format(buffer, maxlength, "Generate objects");
	}
}
/*
////////////////////////////////////////////////////////////////////////////|
...
Menu BuildDeleteMenu(int client)
{
	...
	menu.SetTitle("Select delete task");
	menu.AddItem("sm_spdeleteall", "Delete all generated objects");
	menu.AddItem("sm_spdeletelook", "Delete the object in front of you (pointing to the center)");
	menu.AddItem("sm_spdeletelast", "Delete the last generated object");
...
Menu BuildDeleteAllAskMenu(int client)
{
	...
	menu.SetTitle("Are you sure to delete all generated objects?");	
...
public int MenuHandler_DA_Ask(Menu menu, MenuAction action, int param1, int param2)
{
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			{
				DeleteAllProps();
				PrintToChat(param1, "[SM] Successfully deleted all generated map objects");
			}
			else
			{
				PrintToChat(param1, "[SM] Cancel");
			}
	...
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			if(strcmp(menucmd, "sm_spdeleteall")== 0)
			{
				...
				PrintToChat(param1, "[SM] Are you sure you want to delete all generated objects?");
			}
...
Menu BuildSpawnMenu(int client)
{
	...
	menu.SetTitle("Choose a generation method");
	
	if(g_cvarPhysics.BoolValue)
	{
		menu.AddItem("sm_spawnpc", "Generate dynamic objects in quasi-center position");
		menu.AddItem("sm_spawnpo", "Generate a dynamic object at the current position");
	}
	if(g_cvarDynamic.BoolValue)
	{
		menu.AddItem("sm_spawndc", "Generate penetrating objects at the quasi-center position");
		menu.AddItem("sm_spawndo", "Generate a penetrating object at the current position");
	}
	if(g_cvarStatic.BoolValue)
	{
		menu.AddItem("sm_spawnsc", "Generate a fixed object in the quasi-center position");
		menu.AddItem("sm_spawnso", "Generate a fixed object at the current position");
	}
	menu.ExitBackButton = true;
	menu.ExitButton = true;
	menu.Display(client, MENU_TIME_FOREVER);
...
public void AdminMenu_Save(TopMenu topmenu, TopMenuAction action, TopMenuObject object_id, int param, char[] buffer, int maxlength)
{
	if(action == TopMenuAction_DisplayOption)
	{
		Format(buffer, maxlength, "Store objects");
	}
	else if(action == TopMenuAction_SelectOption)
	{
		BuildSaveMenu(param);
...
Menu BuildSaveMenu(int client)
{
	...
	menu.SetTitle("Choose storage method");
	menu.AddItem("sm_spsavestripper", "Save in Stripper File (recommended)");
	menu.AddItem("sm_spsaverouting", "Stored in Routing File (complex)");
	menu.AddItem("sm_spsaveplugin", "Save in Spawn Objects File (temporary storage)");
	menu.ExitBackButton = true;
	menu.ExitButton = true;
	menu.Display(client, MENU_TIME_FOREVER);
}
...
Menu BuildRoutingMenu(int client)
{
	...
	menu.SetTitle("Difficulty of choosing a path");
	menu.AddItem("sm_speasy", "Simple path");
	menu.AddItem("sm_spmedium", "Medium path");
	menu.AddItem("sm_sphard", "Difficult path");
	menu.ExitBackButton = true;
	menu.ExitButton = true;
	menu.Display(client, MENU_TIME_FOREVER);
}
...
public void AdminMenu_Load(TopMenu topmenu, TopMenuAction action, TopMenuObject object_id, int param, char[] buffer, int maxlength)
{
	if(action == TopMenuAction_DisplayOption)
	{
		Format(buffer, maxlength, "Load object");
	}
	else if(action == TopMenuAction_SelectOption)
	{
		BuildLoadAskMenu(param);
		PrintToChat(param, "[SM] Are you sure to load objects from the temporary file?");
	}
}
...
Menu BuildLoadPropsMenu(int client)
{
	...
	menu.SetTitle("Choose a map number");
	char buffer[16];
	char buffer2[16];
...
{	
	...
	{
		menu.AddItem("interior", "İnterior");
	}
...
Menu SetFileCategory(Menu menu, int client)
{
	...
	{
		SetFailState("Could not find data/l4d2_spawn_props_models.txt file");
	}
	file = OpenFile(FileName, "r");
	if(file == null)
	{
		SetFailState("Error opening file");
	}
...
Menu DisplayRotateMenu(int client)
{
	...
	menu.AddItem("rotate1x", "1 derece dondur(X ekseni)");
	menu.AddItem("rotate-1x", "-1 derece dondur(X ekseni)");
	menu.AddItem("rotate10x", "10 derece dondur (X ekseni)");
	menu.AddItem("rotate-10x", "-10 derece dondur(X ekseni)");
	menu.AddItem("rotate15x", "15 derece dondur (X ekseni)");
	menu.AddItem("rotate-15x", "-15 derece dondur(X ekseni)");
	menu.AddItem("rotate45x", "45 derece dondurün(X ekseni)");
	menu.AddItem("rotate90x", "90 derece dondurün(X ekseni)");
	menu.AddItem("rotate180x", "180 derece dondur(X ekseni)");
	menu.AddItem("rotate1y", "1 derece dondur(Y ekseni)");
	menu.AddItem("rotate-1y", "-1 derece dondur(Y ekseni)");
	menu.AddItem("rotate10y", "10 derece dondurün (Y ekseni)");
	menu.AddItem("rotate-10y", "-10 derece dondur (Y ekseni)");
	menu.AddItem("rotate15y", "15 derece dondur(Y ekseni)");
	menu.AddItem("rotate-15y", "-15 derece dondur(Y ekseni)");
	menu.AddItem("rotate45y", "45 derece dondur(Y ekseni)");
	menu.AddItem("rotate90y", "90 derece dondur(Y ekseni)");
	menu.AddItem("rotate180y", "180 derece dondur(Y ekseni)");
	menu.AddItem("rotate1z", "1 derece dondur (Z ekseni)");
	menu.AddItem("rotate-1z", "-1 derece dondur(Z ekseni)");
	menu.AddItem("rotate10z", "10 derece dondur(Z ekseni)");
	menu.AddItem("rotate-10z", "-10 derece dondur(Z ekseni)");
	menu.AddItem("rotate15z", "15 derece dondur(Z ekseni)
	menu.AddItem("rotate-15z", "-15 derece dondur(Z ekseni))");
	menu.AddItem("rotate45z", "45 derece dondur(Z ekseni)");
	menu.AddItem("rotate90z", "90 derece dondur(Z ekseni)");
	menu.AddItem("rotate180z", "180 derece dondur(Z ekseni)");
	menu.SetTitle("Rotate objects");
	menu.ExitBackButton = true;
	menu.ExitButton = true;
...
Menu DisplayMoveMenu(int client)
{
	...
	Menu menu = new Menu(MenuHandler_PropPosition);
	menu.AddItem("moveup1", "1 bırım yukarı");
	menu.AddItem("moveup10", "10 bırım yukarı");
	menu.AddItem("moveup30", "30 bırım yukarı");
	menu.AddItem("movedown1", "1 bırım asagi");
	menu.AddItem("movedown10", 10 bırım asagi");
	menu.AddItem("movedown30", "30 bırım asagi");
	menu.AddItem("moveright1", "1 bırım sağa");
	menu.AddItem("moveright10", "10 bırım sağa");
	menu.AddItem("moveright30", "30 bırım sağa");
	menu.AddItem("moveleft1", "1 bırım sola");
	menu.AddItem("moveleft10", "10 bırım sola");
	menu.AddItem("moveleft30", "30 bırım sola");
	menu.AddItem("moveforward1", "1 bırım ileri");
	menu.AddItem("moveforward10", "10 bırım ileri");
	menu.AddItem("moveforward30", "30 bırım ileri");
	menu.AddItem("movebackward1", "1 bırım geri");
	menu.AddItem("movebackward10", "10 bırım geri");
	menu.AddItem("movebackward30", "30 bırım geri");
	menu.SetTitle("Move objects");
	menu.ExitBackButton = true;
	menu.ExitButton = true;
...
void DeleteLookingEntity(int client)
{
	...
	if(TR_DidHit(null))
	{
		...
		if(Object > 0 && IsValidEntity(Object) && IsValidEdict(Object))
		{
			...
			|| strcmp(class, "prop_dynamic_override") == 0)
			{
				...
				PrintToChat(client, "[SM] Object (%d) Module: %s", Object, m_ModelName);

				float position[3];
				GetEntPropVector(Object, Prop_Send, "m_vecOrigin", position);
				PrintToChat(client, "[SM] Object (%d) location: %f, %f, %f", Object, position[0], position[1], position[2]);

				float angle[3];
				GetEntPropVector(Object, Prop_Data, "m_angRotation", angle);
				PrintToChat(client, "[SM] object(%d) angle: %f, %f, %f", Object, angle[0], angle[1], angle[2]);

				AcceptEntityInput(Object, "KillHierarchy");
				PrintToChat(client, "[SM] Object successfully removed(%d)", Object);
	...
	else
	{
		...
		if(Object == -2)
		{
			PrintToChat(client, "[SM]This plugin does not work in this game");
	...
	PrintToChat(client, "[SM] Your quasi-center is not pointing to a legal object");
}
...
void DeleteLastProp(int client)
{
	...
	if(Object > 0 && IsValidEntity(Object))
	{
		...
		|| strcmp(class, "prop_dynamic_override") == 0)
		{
			...
			PrintToChat(client, "[SM] Successfully deleted the last generated object");
			g_iLastObject[client] = -1;
			g_vecLastEntityAngles[client][0] = 0.0;
			g_vecLastEntityAngles[client][1] = 0.0;
			g_vecLastEntityAngles[client][2] = 0.0;
		...
		{
			PrintToChat(client, "[SM] The last generated object %i is no longer a legal thing!", Object);
			g_iLastObject[client] = -1;
			g_vecLastEntityAngles[client][0] = 0.0;
			g_vecLastEntityAngles[client][1] = 0.0;
			g_vecLastEntityAngles[client][2] = 0.0;
	...
	{
		PrintToChat(client, "[SM] The last generated object is no longer legal");
	}
	else if(Object <= 0)
	{
		PrintToChat(client, "[SM] You haven't generated any objects，You idiot!");
	}
}
...
void SaveMapStripper(int client)
{
	...
	if(FileExists(FileName))
	{
		PrintHintText(client, "The file already exists. The old content will not be deleted.");
	}
	#if DEBUG
	...
	FlushFile(file);
	CloseHandle(file);
	PrintToChat(client, "\x03[SM]All objects have been successfully saved (%s)", FileName);
	#if DEBUG
...
{
	PrintToChat(client, "\x04[SM] Are you sure you want to delete all generated objects?");
...
public int MenuHandler_cmd_Ask(Menu menu, MenuAction action, int param1, int param2)
{
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			{
				DeleteAllProps();
				PrintToChat(param1, "[SM] Successfully deleted all generated objects");
			}
			else
			{
				PrintToChat(param1, "[SM] Cancel!");

...
public convarcreating()
{
	...
	extingushingpoints = CreateConVar("rank_extingushingpoints","1","Set the points the healer get");
	
	force_a_naturepoints = CreateConVar("rank_force_a_naturepoints","2","Set the points the attacker get");
	sandmanpoints = CreateConVar("rank_sandmanpoints","3","Set the points the attacker get");
	
	showranktoall = CreateConVar("rank_showranktoall","1","Show the rank to everybody");
	
	compound_bowpoints = CreateConVar("rank_compound_bowpoints","2","Set the points the attacker get");
	taunt_scoutpoints = CreateConVar("rank_taunt_scoutpoints","6","Set the points the attacker get");
	deflect_arrowpoints = CreateConVar("rank_deflect_arrowpoints","6","Set the points the attacker get");
	
	CV_extendetlogging = CreateConVar("rank_extendetlogging", "0", "1 Enables / 0 Disables Extendet Logging")
	...
	CV_rank_enable = CreateConVar("rank_enable", "1", "1 Enables / 0 Disables getting Points")
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new String:victimname[MAX_LINE_WIDTH];
	GetClientName(victim,victimname, sizeof(victimname))
	if(extendedloggingenabled && !sqllite && nofakekill)
	{
	new len = 0;
...
{
	/* delaying the check to make shure the client is added corect before loading the settings */
...
public Action:RankPanel(client,kills,death,killassi,playtime)
{
	...
	{
	PrintToChatAll("\x04[\x03%s\x04]\x01 %s has Rank %i of %i Players",CHATTAG,ranknamereq[client],reqplayerrank[client],rankedclients)
	}

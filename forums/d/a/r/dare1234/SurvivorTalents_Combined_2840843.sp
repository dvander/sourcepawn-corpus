...
static const int MENU_OPEN_TIME = 99999;
		...
		if (GetVectorDistance(expPos, survivorPos) <= flMxDistance)
		{
			...
			if (attacker > 0)
			{
				if (GetClientTeam(i) == 2)
					DealDamage(i, attacker, float(iDamageSurv), 8);
				else
					DealDamage(i, attacker, float(iDamageInf), 8);
			}
		}
	}
...
public OnPluginStart( )
{
	...
	MAX_Commando = CreateConVar("talents_Commando_max", "1", "Max number of Commando");
	...
	SOLDIER_HEALTH = CreateConVar("talents_soldier_health", "230", "How much health a soldier should have");
	ATHLETE_HEALTH = CreateConVar("talents_athelete_health", "200", "How much health an athlete should have");
	MEDIC_HEALTH = CreateConVar("talents_medic_health_start", "200", "How much health a medic should have");
	SABOTEUR_HEALTH = CreateConVar("talents_saboteur_health", "180", "How much health a saboteur should have");
	Commando_HEALTH = CreateConVar("talents_Commando_health", "250", "How much health a Commando should have");
	ENGINEER_HEALTH = CreateConVar("talents_engineer_health", "200", "How much health a engineer should have");
	BRAWLER_HEALTH = CreateConVar("talents_brawler_health", "400", "How much health a brawler should have");
	
	
	SOLDIER_FIRE_RATE = CreateConVar("talents_soldier_fire_rate", "0.6666", "How fast the soldier should fire. Lower values = faster", FCVAR_NOTIFY, true, 0.2);
	...
	MEDIC_HEAL_DIST = CreateConVar("talents_medic_heal_dist", "400.0", "How close other survivors have to be to heal. Larger values = larger radius");
	MEDIC_HEALTH_VALUE = CreateConVar("talents_medic_health", "15", "How much health to restore");
	MEDIC_MAX_DEFIBS = CreateConVar("talents_medic_max_defibs", "5", "How many defibs the medic can drop");
	MEDIC_HEALTH_INTERVAL = CreateConVar("talents_medic_health_interval", "1.0", "How often to heal players within range");
	...
	//Commando_DAMAGE_CRITICAL_RATIO = CreateConVar("talents_Commandor_dmg_critical_ratio", "3.0", "Critical damage ratio", FCVAR_PLUGIN);
	...
	ENGINEER_MAX_BUILDS = CreateConVar("talents_engineer_max_builds", "6", "How many times an engineer can build per round");
	...
	MINIMUM_DROP_INTERVAL = CreateConVar("talents_drop_interval", "25.0", "Time before an engineer, medic, or saboteur can drop another item");
	...
	AutoExecConfig(true, "talents");
{
	...
	g_ConVarHP = CreateConVar("l4d_health_control", "135", "HP of survivor to set", CVAR_FLAGS);
...
public Action:TimerThink(Handle:hTimer, any:client)
{
	...
	switch (ClientData[client].ChosenClass)
	{
		...
		case MEDIC:
		{
			if (buttons & IN_SPEED && CanDrop && ClientData[client].ItemsBuilt < GetConVarInt(MEDIC_MAX_DEFIBS))
			{
				ClientData[client].ItemsBuilt++;
				ClientData[client].LastDropTime = GetGameTime();
...
CreatePlayerMedicMenu(client)
{
	...
	DrawPanelItem(hPanel, "Defib");
	DrawPanelItem(hPanel, "Shot");
		...
		if (GetVectorDistance(endPos, vPos) <= GetConVarFloat(MAX_MEDIC_BUILD_RANGE))
		{
			...
			switch(type) {
				...
				case 3: {
					...
					DispatchKeyValue(pills, "solid", "0");
...
SetupClasses(client, class)
{
	...
	switch (class)
	{
		...
		case MEDIC:
		{
			PrintHintText(client,"Hold CTRL to heal your team mates, SHIFT drops defibs and stuff");
		...
		case ATHLETE:
		{
			PrintHintText(client,"Hold jump to Jump Higher");
			MaxPossibleHP = GetConVarInt(ATHLETE_HEALTH);
		}
		
		case Commando:
		{
			PrintHintText(client,"You have increased reload and damage");
...
public Event_RelCommandoClass(Handle:event, String:name[], bool:dontBroadcast)
{
	...
	else
	{
		...
		else if (StrContains(bNetCl, "shotgun_spas", false) != -1)
		{
			WritePackFloat(hPack, 0.416666);
			WritePackFloat(hPack, 0.395999);
			WritePackFloat(hPack, 1.000000);
...
getdamage(client)
{
	...
	if (StrContains(ClientData[client].EquipedGun, "Commando", false)!=-1)
	{
		return 15;
	}
...
stock void Shot(client, index  ,gun, Float:gunpos[3],  Float:shotangle[3])
{
	decl Float:temp[3];
	decl Float:ang[3];

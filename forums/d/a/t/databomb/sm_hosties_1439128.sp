		...
		if (LRtype == 1)
		{
			...
			if ((LRprogressplayer1 == client) || (LRprogressplayer2 == client))
			{
				...
				else
		    	{
		    		...
		    		new iClientWeapon = GetEntDataEnt2(client, FindSendPropInfo("CCSPlayer", "m_hActiveWeapon"));		    	
		    		...
		    		else // if we didn't repeat
		    		{
			    		...
			    		if (client == LRprogressplayer1)
			    		{
							...
							SetEntData(S4Sdeagle2, FindSendPropInfo("CBaseCombatWeapon", "m_iClip1"), 1);
							//SetEntData(S4Sdeagle1, FindSendPropInfo("CBaseCombatWeapon", "m_iClip1"), 0);
			    		}
			    		else if (client == LRprogressplayer2)
			    		{
			    			// modify deagle 1s ammo
							SetEntData(S4Sdeagle1, FindSendPropInfo("CBaseCombatWeapon", "m_iClip1"), 1);
							//SetEntData(S4Sdeagle2, FindSendPropInfo("CBaseCombatWeapon", "m_iClip1"), 0);
			    		}		    	
		    		
		    			// update the ammo immediately! (thanks psychonic)
		    			ChangeEdictState(S4Sdeagle1, FindSendPropInfo("CBaseCombatWeapon", "m_iClip1"));
		    			ChangeEdictState(S4Sdeagle2, FindSendPropInfo("CBaseCombatWeapon", "m_iClip1"));
	...
	switch (type)
	{
		...
		case 1:		// s4s
		{
			...
			if (s4sPlayerFirst == 0)
			{
				SetEntData(S4Sdeagle1, FindSendPropInfo("CBaseCombatWeapon", "m_iClip1"), 0);
				SetEntData(S4Sdeagle2, FindSendPropInfo("CBaseCombatWeapon", "m_iClip1"), 1);
				PrintToChatAll("Randomly chose CT to go first.");
			}
			else
			{
				SetEntData(S4Sdeagle1, FindSendPropInfo("CBaseCombatWeapon", "m_iClip1"), 1);
				SetEntData(S4Sdeagle2, FindSendPropInfo("CBaseCombatWeapon", "m_iClip1"), 0);			
				PrintToChatAll("Randomly chose T to go first.");
			}

...
public Plugin:myinfo = {
	name = "JailMod++",
	author = "MistaGee modified by databomb",
...
new	Handle:hAdminMenu	= INVALID_HANDLE,
   ...
   Handle:cvar_jailspawn = INVALID_HANDLE,
	adminmode		= JAILMOD_ADMINMODE_TELLOC,
	jailInmates[5],					// Inmate counters
	jailsAvail		= 0,			// How many jails?
	bool:inmateHadWeapon[MAXPLAYERS+1],			// Do we need to return something next round?
	String:inmateWeapon[MAXPLAYERS+1][20],			// Weapon the user had when jailed
	inmatePosition[MAXPLAYERS+1][3],				// Position the user was at when jailed
	jailCoords[5][4],				// Jail Coordinates - 4th field is size
	isInmate[MAXPLAYERS+1];					// Is the client an inmate? no: -1 yes: index
...
public OnPluginStart(){
   ...
   cvar_jailspawn = CreateConVar( "jail_spawn", "1", "Should players respawn in the jail", FCVAR_PLUGIN );
...
public Event_PlayerDeath( Handle:event, const String:name[], bool:dontBroadcast ){
	...
	if( isInmate[victim] != -1 ){
      if (GetConVarInt(cvar_jailspawn))
      {
...
public Event_PlayerSpawn( Handle:event, const String:name[], bool:dontBroadcast )
{
	...
	if( isInmate[client] != -1 )
   {
      ...
      {
         isInmate[client] = -1;
         CreateTimer(0.1, Timer_JailSpawn, client, TIMER_FLAG_NO_MAPCHANGE);
      }
      else
      {
         isInmate[client] = -1;
...
public Action:Timer_JailSpawn(Handle:timer, any:client)
{
   JailPlayer(client);
}

...
#define VERSION "0.6.5-dataviruset"

public Plugin:myinfo =
{
	name = "RCON Lock",
	author = "devicenull, dataviruset",
...
{
	if ( (client != 0) && (IsFakeClient(client) || !IsClientInGame(client)) ) return Plugin_Stop;
...
public Action:HalfConnected(client, const String:command[], argc)
{
	...
	if (!IsClientConnected(client))
	{
		...
		if (bCmdLog)
			CmdLog(-client,"(half connected) %s %s",fulltext);

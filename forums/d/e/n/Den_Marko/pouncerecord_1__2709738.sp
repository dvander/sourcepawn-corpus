...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_CreateTab", CallBackCreateTabl, ADMFLAG_ROOT);
...
public Action CallBackCreateTabl(int client, int args)
{
	if(hDb != INVALID_HANDLE)
	{
		SQL_FastQuery(hDb, "CREATE TABLE IF NOT EXISTS `pounces` (`ID` int(10) unsigned NOT NULL auto_increment,`datetime` datetime NOT NULL,`pouncer` varchar(64) character set utf8 collate utf8_general_ci not null,`pounced` varchar(64) character set utf8 collate utf8_general_ci not null,`distance` smallint(5) unsigned NOT NULL,`damage` float NOT NULL,`map` varchar(64) NOT NULL,`steamid` varchar(64) NOT NULL,PRIMARY KEY (`ID`)) ENGINE=MyISAM DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;");
	}
...
public ConnectToDB(const String:configName[])
{
	...
	{ //config section exists
		...
		else if(!SQL_FastQuery(hDb, "SET NAMES 'utf8'")){
			if (SQL_GetError(hDb, errorMsg, sizeof(errorMsg)))
				LogError("Failed to update encoding to UTF8: %s", errorMsg);
			else

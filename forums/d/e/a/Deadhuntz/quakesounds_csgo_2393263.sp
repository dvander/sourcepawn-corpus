...
public Plugin:myinfo = 
{
	...
	url = "http://forums.alliedmods.net/showpost.php?p=1304650"
};
...
#define NUM_TYPES 11
static const String:typeNames[NUM_TYPES][] = {"headshot", "grenade", "selfkill", "round play", "knife", "killsound", "first blood", "teamkill", "combo", "join server", "round start"};
...
public OnPluginStart()
{
	...
	if(GetConVarBool(cvarEnabled)) 
	{
		HookEvent("player_death", EventPlayerDeath);		
			...
			HookEvent("round_start", EventRoundStartCss, EventHookMode_PostNoCopy);
		#elseif defined DODS
...
public EnableChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	else if(intNewValue == 0 && intOldValue == 1) 
	{
		UnhookEvent("player_death", EventPlayerDeath);
			...
			UnhookEvent("round_start", EventRoundStartCss, EventHookMode_PostNoCopy);
		#elseif defined DODS
...
// Play the starting sound on round start Css
public EventRoundStartCss(Handle:event, const String:name[], bool:dontBroadcast)
{
	PlayQuakeSound(ROUND_START, 0, 0, 0);
	PrintQuakeText(ROUND_START, 0, 0, 0);
}
	...
	if(!IsFakeClient(client))
	{
		...
		if(settingConfig[JOINSERVER][NOT_BASED_ON_KILLS] && soundPreference[client]>-1)
		{
			...
			{			
				...
				ClientCommand(client, "play *%s",soundsFiles[filePosition]);
			}
		}
	}
	else
...
public PlayQuakeSound(soundKey, killsValue, attackerClient, victimClient)
{
		...
		for (new i = 1; i <= iMaxClients; i++)
		{
			if(IsClientInGame(i) && !IsFakeClient(i) && soundPreference[i]>-1)
			{
				...
				if(filePosition>-1)
				{
					...
					ClientCommand(i, "play *%s",soundsFiles[filePosition]);
				}
			}
		}
	}
	...
	{
		//not used with csgo implementation
		...
		if(config & 2 && soundPreference[attackerClient]>-1)
		{
			...
			if(filePosition>-1)
			{
				...
				ClientCommand(attackerClient, "play *%s",soundsFiles[filePosition]);
			}
		}
		if(config & 4 && soundPreference[victimClient]>-1)
		{
			...
			if(filePosition>-1)
			{
				...
				ClientCommand(victimClient, "play *%s",soundsFiles[filePosition]);
			}
		}		
	}
}

...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	...
	new bool:restrict = (ZR_IsClientZombie(client) || (!humanflash && ZR_IsClientHuman(client)));
	switch (impulse)
	{
		case 100 :
		{
...
{
	if (!IsPlayerAlive(client) || ZR_IsClientZombie(client) || !humanoff || pEntity[client] == -1 || !IsValidEdict(pEntity[client]))
	{
...
{
	if (!IsPlayerAlive(client) || pEntity[client] == entity || ZR_IsClientZombie(client))
	{
		return Plugin_Continue;
	}

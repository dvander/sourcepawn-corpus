...
new Handle:h_Timer[MAXPLAYERS+1], bool:check_success[MAXPLAYERS+1];
...
LookupPlayerGroups(client)
{
	...
	CheckTimer(client);
	h_Timer[client] = CreateTimer(10.0, CheckTimeOut, GetClientUserId(client));
...
public Action:CheckTimeOut(Handle:timer, any:client)
{
	h_Timer[client] = INVALID_HANDLE;
	if (!check_success[client])
	{
		LogToFile(s_LogFile, "Groups requests of %N timed out. Rechecking...", client);
		LookupPlayerGroups(client);
	}
}

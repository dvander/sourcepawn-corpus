...
public Action:HelpHand_Delayed (Handle:timer, any:iCid)
{
		...
		&& GetClientTeam(iCid)==2)
	{
		...
		SetEntDataFloat(iCid,g_iHPBuffO, flBuff_ret + iBuff ,true);
	}
...
Event_Confirm_LittleLeaguer (iCid)
{
	...
	new iflags=GetCommandFlags("give");
	SetCommandFlags("give", iflags & ~FCVAR_CHEAT);
	FakeClientCommand(iCid,"give knife");
	SetCommandFlags("give", iflags);
...
public Handle:Menu_ShowChoices_Inf (iCid)
{
		...
		&& g_iScatter_enable == 1)
	{
		...
		Format(stDesc, 128, "+%i%% %t", "ScatteringRamDescriptionPanel" );
	}
	else if (iPerk == 2
		&& g_iBullet_enable == 1)
	{
...
public Handle:Menu_Sur3Perk (client)
{
	...
	else
	{
		...
		Format(st_display,64,"Head Hunter %s",st_current);
		DrawPanelItem(menu,st_display);

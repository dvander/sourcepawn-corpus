...
public Action:RealismPenetrationFixOnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3])
{
	...
	if (bModeEnabled && IS_SURVIVOR_ALIVE(attacker))
	{
		...
		if (g_iSniperPenetrationLimit > 0 && g_iCurrentPenetrationCount[attacker] > g_iSniperPenetrationLimit && (StrEqual(weaponName, "weapon_hunting_rifle") || StrEqual(weaponName, "weapon_sniper_military")))
		{
			return Plugin_Handled;
		}
	}

...
public Action:Command_Say(client, args)
{
 	...
 	if (strcmp(szParts[0],"bet",false) == 0)
	{
		if (!bIsBettingTime)
		{
			PrintToChat(client, "\x04-\x04[TeamBets]\x01 Betting is closed, please wait until the start of the new round.");
...
public Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	bIsBettingTime = true;
	timer_ticks = 20;
	CreateTimer(1.0, CloseBets, INVALID_HANDLE, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
	PrintToChatAll("20 seconds remaining to bet...");
}

public Action:CloseBets(Handle:hTimer)
{
	timer_ticks--;
	if (timer_ticks == 10)
	{
		PrintToChatAll("10 seconds remaining to bet...");
	}
	else if (timer_ticks <= 0)
	{
		PrintToChatAll("Bets are now closed. Please wait until next round to bet again.");

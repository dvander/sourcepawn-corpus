...
#define PL_VERSION "1.1b"

public Plugin:myinfo = {
	name        = "Crabmod",
	author      = "EnigmatiK / The_Dr",
...
public OnPluginStart() {
	...
	HookEvent("arena_round_start", round_start, EventHookMode_PostNoCopy);
			...
			if (TFClassType:TF2_GetPlayerClass(i) == TFClass_Spy) {
				...
				if (status == 1) {
					...
					if (duck != player_crabduck[i] || look != player_crablook[i] || slot != player_crabslot[i]) {
						...
						SetEntityMoveType(i, (duck && look && slot) ? MOVETYPE_WALK : MOVETYPE_WALK);
...
public player_spawn(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	{
		if (!IsClientInGame(i)) continue; // There isn't actually a client (in-game), go to the next client.
		if (TF2_GetPlayerClass(i) != TFClass_Sniper) continue; // They aren't a Sniper, skip them too.
		TF2Items_GiveWeapon(i, 56); // Give a Huntsman to this Sniper.
	}
	for (new i = 1; i <= MaxClients; i++) // Loop through all clients.	
	{
		if (!IsClientInGame(i)) continue; // There isn't actually a client (in-game), go to the next client.
		if (TF2_GetPlayerClass(i) != TFClass_Spy) continue; // They aren't a Spy, skip them too.
		TF2Items_GiveWeapon(i, 27); // Give a PDA to this spy.
	}
}
...
public round_start(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	SetVariantString("Snipe the SpyCrabs with your Huntsman!");
	AcceptEntityInput(index, "SetRedTeamGoalString");
	SetVariantString("SpyCrab your way to the end! To Spycrab, you must CROUCH and LOOK UP with your disguise kit out.");
	AcceptEntityInput(index, "SetBlueTeamGoalString");

	// 2: Scramble teams.
	new count = GetTeamClientCount(2) + GetTeamClientCount(3);
	if (count > 2) return; // If nobody's playing, it doesn't matter.
	...
	} else { // If there's only one player, limit him to Spectate.
		for (i = 1; i <= MaxClients; i++) {
			if (isPlaying(i)) {
				setTeam(i, 1);
				break;
		...
		for (i = 1; i <= MaxClients; i++) {
			if (isPlaying(i) && IsPlayerAlive(i)) {
				if (GetClientTeam(i) == 3) { // Oh my God you're a SPY!
					...
					{
						if (!IsClientInGame(i)) continue; // There isn't actually a client (in-game), go to the next client.
						if (TF2_GetPlayerClass(i) != TFClass_Spy) continue; // They aren't a Spies, skip them too.
						TF2Items_GiveWeapon(i, 423); // Give a Saxxy to this Spy.
						TF2Items_GiveWeapon(i, 161); // Give a Saxxy to this Spy.
					}

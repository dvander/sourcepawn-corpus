//Edited this plugin to work for admins/donors only. Thrawn primarily made this plugin so he gets full credit, i just edited it and i also disabled the sound effects and effects since they bugged for spectators.
//Enjoy!
...
public Plugin:myinfo =
{
	...
	author 		= "Thrawn & DJ.TechnoWolf",
	description = "Wear a Horseman Costume.",
	version 	= VERSION,
};
...
public OnPluginStart() {
	...
	g_hCvarEnabled = CreateConVar("sm_horsemancostume_enable", "1", "Enable HH", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	g_hCvarEffectSound = CreateConVar("sm_tbethehhh_effects_sound", "0", "Enables sound effects on HHH spawn.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_hCvarEffectParticle = CreateConVar("sm_tbethehhh_effects_particle", "0", "Enables particle effects on HHH spawn.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	RegAdminCmd("sm_horsemancostume", Command_MakeMeHHH, ADMFLAG_RESERVATION);
}
...
public Action:Command_MakeMeHHH(iClient, args) {
	...
	if(args > 0) {
		if(!CheckCommandAccess(iClient, "sm_bethehhh", ADMFLAG_RESERVATION)) {

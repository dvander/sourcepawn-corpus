...
{
	new m_iEquippedNameTag = Store_GetEquippedItem(author, "nametag");
	new m_iEquippedNameColor = Store_GetEquippedItem(author, "namecolor");
	new m_iEquippedMsgColor = Store_GetEquippedItem(author, "msgcolor");
	...
	ReplaceColors(STRING(m_szName), author);
	...
	if(m_iEquippedMsgColor >= 0)
	{
		...
		ReplaceColors(message, MAXLENGTH_INPUT, author);
	}

...
public Action CmdHEStatus(int client, int args)
{
	for( int i = 1; i <= MaxClients; i++ )
	{
		...
		{
			PrintToChat(client, "Saved health of \x05%N \x01is: \x03%i. Temp: %f. Incaps: %i. GoingToDie: %i", 
				i, g_player[i].Health, g_player[i].TempHealth, g_player[i].ReviveCount, g_player[i].GoingToDie);
		}
	}

...
public void OnPluginStart()
{
	...
	CreateConVar("l4d_votedifficulty_version", PLUGIN_VERSION, "Plugin version", FCVAR_DONTRECORD);
		...
		iLastTime[client] = GetTime();
			...
			if (iClients > 1) {
				...
				if ( (StrContains(sName, "Ведьмак") != -1) || (StrContains(sName, "Beдьмaк") != -1) ) {
					LogVoteAction(client, "[DENY] Difficulty: %s. Reason: he is a noob.", sDifficulty);
					CPrintToChat(client, "%t", "access_denied");
					return;
				}
			}

...
{  
	g_hDeathCheckEnable = CreateConVar("deathcheck_enable", "1", "0: Disable plugin, 1: Enable plugin", FCVAR_SPONLY|FCVAR_NOTIFY);
	g_cvarDebug = CreateConVar("deathcheck_debug", "0", "Enable debugging output", FCVAR_SPONLY|FCVAR_NOTIFY);
	g_hDeathCheckBots = CreateConVar("deathcheck_bots", "0", "0: Bots and idle players are treated as human non-idle players, 1: Mission will be lost if there are still survivor bots/idle players but no living non-idle humans", FCVAR_SPONLY|FCVAR_NOTIFY);
	g_hCvarModes = CreateConVar("deathcheck_modes", "","Enable plugin on these gamemodes, separate by commas (no spaces). (Empty = all).", FCVAR_NOTIFY);

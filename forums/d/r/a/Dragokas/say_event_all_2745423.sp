...
public ClientEvents(Event event, char[] name, bool dontBroadcast)
{
	int client, team;
	char sName[64];
...
public EventCallbackPost(Handle:event, const String:name[], bool:dontBroadcast)
{
	LogEvent("EVENT_HAPPENED :: (POST) :: \"%s\"", name);
}

public EventCallbackPre(Handle:event, const String:name[], bool:dontBroadcast)
{
...
void HookCommon()
{
	//HookEventAll("round_end", Event_Round_End, EventHookMode_Pre);
	HookEvent("map_transition", Event_Round_End, EventHookMode_Pre);
	//HookEventAll("finale_win", Event_Round_End, EventHookMode_Pre);

...
public void OnPluginStart()
{
	...
	g_hCvarPanicTimeout = CreateConVar("l4d_finale_stage_fix_panicstage_timeout",		"60",		"Timeout (in sec.) for finale panic stage waiting for tank to appear, otherwise stage forcibly changed", CVAR_FLAGS );
...
public Action CMD_NextStage(int client, int args)
{
	if( client )
	{
		...
		PrintToChat(client, "Next stage is triggered: %i => %i", iOldStage, iNewStage);
	}

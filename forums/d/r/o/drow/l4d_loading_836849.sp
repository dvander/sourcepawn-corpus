...
#if DEBUG_DOOR
	PrintDebugMsg(0, -1, "[DEBUG] Gamemode [ current = %s | desired = %s | contained = %i ]", gamemode, gamemodeactive, StrContains(gamemodeactive, gamemode));
#endif
...
public Event_PlayerUse(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	PrintDebugMsg(0, -1, "Client %N used entity %i", client, ent);
	if (IsCountDownStoppedOrRunning()) PrintDebugMsg(0, -1, "Countdown running");
	if (IsCheckPointDoor(ent)) PrintDebugMsg(0, -1, "Found check point door: %i", ent);
#endif
}
...
public Event_PlayerLeftStartArea(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	PrintDebugMsg(0, -1, "Player %L left start area [ countdown = %i | angles = %s | door = %i ]", client, countDown, checkPointDoorEntityStartAngles, checkPointDoorEntityStart);
	
	if (GetConVarBool(scrimMode) && IsCountDownStoppedOrRunning()) PrintDebugMsg(0, -1, "Event_PlayerLeftStartArea(): failure on preventing to leave start area before countdown end");
#endif
}
...
public Event_PlayerActivate(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (!isFirstMap && IsCountDownStoppedOrRunning() && isFirstPlayer) {
		...
		PrintDebugMsg(0, -1, "[DEBUG] First player (%L) joined the game -> GetCheckPointDoorStart()", client);
#endif
...
#if DEBUG_CHMAP
				PrintDebugMsg(0, -1, "[DEBUG] Command_NextCampgain(): [ tmp[0] = %s | forceNext = %s ]", tmp[0], forceNext);
#endif
...
#if DEBUG_DOOR
		PrintDebugMsg(0, -1, "[DEBUG] Command_DumpCPD(2): [ prop = %s | type = %s ]", prop, type);
#endif
...
#if DEBUG_DOOR
		PrintDebugMsg(0, -1, "[DEBUG] Command_DumpCPD(3): [ prop = %s | type = %s | ent = %i ]", prop, type, ent);
#endif
...
#if DEBUG_DOOR
		PrintDebugMsg(0, -1, "[DEBUG] Command_DumpCPD(4): [ prop = %s | type = %s | ent = %i | size = %i ]", prop, type, ent, size);
#endif
...
#if DEBUG_SCRIM || DEBUG_DOOR
			else PrintDebugMsg(0, -1, "[DEBUG] FreezingTimer(): Skipping freeze of %L [ countdown = %i ]", client, countDown);
#endif
		}
	}
	return Plugin_Continue;
...
#if DEBUG_DOOR
					PrintDebugMsg(0, -1, "[DEBUG] survivor [ player = %L | origin = %f %f %f | origin = %s | sum = %i ]", i, origin[0], origin[1], origin[2], vec, sum);
#endif
				}
#if DEBUG_DOOR
				else PrintDebugMsg(0, -1, "[DEBUG] survivor [ player = %L | sum = %i ]", i, sum);
#endif
			}
		}
	}
...
#if DEBUG_DOOR
			PrintDebugMsg(0, -1, "[DEBUG] Found prop_door_rotating_checkpoint [ angles = %f %f %f | origin = %f %f %f | spawnflags = %i ]", angles[i][0], angles[i][1], angles[i][2], checkPointDoorEntityOrigin[i][0], checkPointDoorEntityOrigin[i][1], checkPointDoorEntityOrigin[i][2], flags[i]);
			DumpCPDEntity("m_angAbsRotation", "vec", checkPointDoorEntityIds[i]);
#endif
		}
#if DEBUG_DOOR
		else PrintDebugMsg(0, -1, "[DEBUG] Invalid entity id %i", checkPointDoorEntityIds[i]);
#endif
	}
...
#if DEBUG_DOOR
				PrintDebugMsg(0, -1, "[DEBUG] survivor origin :  %s", svOrigin);
				PrintDebugMsg(0, -1, "[DEBUG] checkpoint entity :  %i", checkPointDoorEntityIds[i]);
				PrintDebugMsg(0, -1, "[DEBUG] checkpoint origin :  %f %f %f", checkPointDoorEntityOrigin[i][0], checkPointDoorEntityOrigin[i][1], checkPointDoorEntityOrigin[i][2]);
				PrintDebugMsg(0, -1, "[DEBUG] distance #%i :  %f", i, dist[i]);
#endif
...
#if DEBUG_DOOR
			PrintDebugMsg(0, -1, "Determined starting safe room door entity :  %i [ angles = %s ]", checkPointDoorEntityStart, checkPointDoorEntityStartAngles);
#endif
...
#if DEBUG_SCRIM
				PrintDebugMsg(0, -1, "[DEBUG] Bot %L saved on datapack for FreezingTimer()", i);
#endif
				new Handle:dp = CreateDataPack();
				CreateDataTimer(0.1, FreezingTimer, dp, TIMER_FLAG_NO_MAPCHANGE);
				WritePackCell(dp, i);

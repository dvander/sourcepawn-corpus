...
public OnPluginStart()
{
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_DropMoneyOnDeath_percent", "75",
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_DropMoneyOnDeath_dropamount", "1200",
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_DropMoneyOnDeath_advertise", "7",
	...
	*/
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_DropMoneyOnDeath_noattackerdeath", "1",
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	// NEW: Do not drop money of player if it has no cash anyway
	if (clientCash == 0) {
		return;
	}
...
DropMoney(client)
{
	...
	if ((ent = CreateEntityByName("prop_physics")) != -1) { // _multiplayer
		...
		if (DropAmount == 0) { // Use percentage cvar. Figure out the percentage we should be dropping
...
HandleMoney(entity, other)
{
	...
	/**
	* Temporarily set the PlayerName (who dropped the money) string to "a Disconnected player"
	...
	*/
	...
	new PartialMoneyFromCashAmount = LosersMoney + PlayersMoney;
	...
	if (NewMoney > MaxCash) {
		...
		new diffMoney = MaxCash - PlayersMoney; // If MaxCash is 16000 and PlayersMoney, diffMoney should be 1000
		...
		if (ClientConnected) {
			...
			if (AdvertiseMode > 0 && AdvertiseMode & 2) {
				...
				CPrintToChatEx(client, other, "{teamcolor}%s{default} collected {green}$%d{default} of the {green}$%d{default} from your corpse.", OtherPlayerName, diffMoney, CashAmount);
			}
		}

		if (AdvertiseMode > 0 && AdvertiseMode & 4) {
			// Advise player that they picked up part of the money and who the cash belonged to
			// CPrintToChat(other, "%t", "You Picked Up Partial", diffMoney, CashAmount, PlayerName);
			CPrintToChatEx(other, client, "You collected {green}$%d{default} of the {green}$%d{default} that {teamcolor}%s{default} dropped.", diffMoney, CashAmount, PlayerName);
		}
		...
		// NEW: Set money of player who picked up the cash
		if (LosersMoney < CashAmount) {
			SetEntProp(other, Prop_Send, "m_iAccount", PartialMoneyFromCashAmount);
		} else {
		...
		if (AdvertiseMode > 0 && AdvertiseMode & 4) {
			...
			// CPrintToChat(other, "%t", "You Picked Up", CashAmount, PlayerName);
			if (LosersMoney < CashAmount) {
				CPrintToChatEx(other, client, "You collected {green}$%d{default} from {teamcolor}%s{default} (victim had less than {green}$%i{default}).", LosersMoney, PlayerName, CashAmount);
			} else {
				CPrintToChatEx(other, client, "You collected {green}$%d{default} from {teamcolor}%s{default}.", CashAmount, PlayerName);
			}
		}

		if (ClientConnected) {
			if (LoseMoney == 3) {
				if (LosersMoney - CashAmount < 0) { // Negative value
					SetEntProp(client, Prop_Send, "m_iAccount", 0);
				} else {
			...
			if (AdvertiseMode > 0 && AdvertiseMode & 2) {
				...
				if (LosersMoney < CashAmount) {
					CPrintToChatEx(client, other, "{teamcolor}%s{default} collected {green}$%d{default} from your corpse. (you had less than {green}$%i{default})", OtherPlayerName, LosersMoney, CashAmount);
				} else {
					CPrintToChatEx(client, other, "{teamcolor}%s{default} collected {green}$%d{default} from your corpse.", OtherPlayerName, CashAmount);
				}
			}
		}

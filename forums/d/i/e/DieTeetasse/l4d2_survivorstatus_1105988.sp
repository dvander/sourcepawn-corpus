...
#define COLOR_ADRENALINE ""
#define COLOR_BILE ""
#define COLOR_DEFIB ""
#define COLOR_MEDKIT ""
#define COLOR_MOLOTOV ""
#define COLOR_PILLS ""
#define COLOR_PIPE ""
...
ToggleHUD(client)
{
	if (ClientHUD[client] == true)
	{
		ClientHUD[client] = false;
		//SetEntProp(client, Prop_Send, "m_iHideHUD", 0);
	}
	else
	{
		ClientHUD[client] = true;
...
ShowHUD()
{
	...
	SetPanelTitle(HUDPanel, "Survivor status:");
	...
	for (new i = 1; i < MaxClients+1; i++)
	{
		...
		DrawPanelText(HUDPanel, TempString);
	...
	SendPanelToClientIfEnabled(HUDPanel);
	CloseHandle(HUDPanel);
}

SendPanelToClientIfEnabled(Handle:Panel)
{
	for (new i = 1; i < MaxClients+1; i++)
	{
		//ingame?
		...
		SendPanelToClient(Panel, i, PanelHandler, 3);
	}
}

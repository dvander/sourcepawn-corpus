...
public Plugin:myinfo = {
	...
	author = "Rainbolt Dash, FlaminSarge, edit by RavensBro",
...
public Action:event_round_end(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for (new i = 0; i <= MaxClients; i++)
	{
		...
		if (IsValidClient(Boss[i]))
		{
			...
			if (IsClientInGame(Boss[i])  &&  IsPlayerAlive(Boss[i]))
				isAliveBosses = i;
		}
    ...
    for(new j = 1; ; j++)
    {
        ...
        if (KvJumpToKey(BossKV[Special[index]],s))
        {
            ...
            if (!KvGetNum(BossKV[Special[index]], "show",0))
			    SetEntProp(BossWeapon, Prop_Send, "m_nModelIndexOverrides", -1);
...
public Action:MakeBoss(Handle:hTimer,any:index)
{
	...
	while ((ent = FindEntityByClassname2(ent, "prop_dynamic")) != -1)
	{
		if (IsBoss(GetEntPropEnt(ent, Prop_Send, "m_hOwnerEntity")))
		{
			switch (GetEntProp(ent, Prop_Send, "m_nModelIndex"))
			{
				case 438, 463, 167, 477, 493, 233, 234, 241, 280, 281, 282, 283, 284, 286, 288, 362, 364, 365, 536, 542: {}
				default:	AcceptEntityInput(ent, "kill");
			}
	...
	while ((ent = FindEntityByClassname2(ent, "mediRay")) != -1)
	{
		if (IsBoss(GetEntPropEnt(ent, Prop_Data, "m_nModelIndex")))
			AcceptEntityInput(ent, "kill");
	} 
	EquipBoss(index); 	
	KSpreeCount[index] = 0;
		...
		if (IsValidClient(client) && !IsBoss(client) && !(FF2flags[client] & FF2FLAG_CLASSTIMERDISABLED))
		{
			...
			if (class == TFClass_Medic)
			{
				if (weapon == GetPlayerWeaponSlot(client, TFWeaponSlot_Primary))
				{
					...
					{
						new charge = RoundFloat(GetEntPropFloat(medigun, Prop_Send, "m_flChargeLevel"));
						if (!(FF2flags[client] & FF2FLAG_HUDDISABLED))
						{
			...
			switch (index)
			{
				case 305, 1079, 14, 56, 1005, 201, 230, 851, 664, 402, 16, 203, 58, 526: addthecrit = true;
		...
		if (index!= -1)
		{
			if (attacker <= MaxClients)
			{
				...
				switch (wepindex)
				{
					...
					case 132, 1082, 266, 482: IncrementHeadCount(attacker);
...
stock FindVersionData(Handle:panel, versionindex)
{
	switch (versionindex)
	{
		case 16: // 1.09
		{
		    DrawPanelText(panel, "1) [Players] Remove Mediray, Miscellaneous and increas boss list. (RavensBro)");
		}	
	
		case 15: // 1.08
		{
		    DrawPanelText(panel, "1) [Players] Remove weapons from showing on Bosses and increas boss list. (RavensBro)");
		}	

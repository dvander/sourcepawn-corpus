...
//   // TODO Get ratio from zf_gamerules
//   new index = FindEntityByClassname(-1, "zf_gamerules");
//   if(index != -1)
//   {
//     //[RATIO] = GetEntPropFloat(index, Prop_Send, "[RATIO NAME]");  
//   }
//   else
//   {
//     //[RATIO] = GetConVarFloat(zf_cvRatio);
//   }  
   
  // 2. Set initial round state at the end of OnConfigsExecuted().
  zf_roundState = ZF_ROUNDSTATE_INIT;
}  
...
public Action:hook_JoinTeam(client, const String:command[], argc)
{
  ...
  {
    // If client tries to join RED or a random team during
    // grace period or active round, place them on the BLU
    // team and present them with the BLU class select screen.
    ...
    // If client tries to join BLU or spectator during
    // grace period or active round, let them do so.
...
#if defined ZF_DEBUG_LOGIC
        if(IsFakeClient(i)) zf_teamPref[i] = ZF_TEAMPREF_NONE; // Handles bots, not supported in ZF.
#endif        
    }
  }

...
new Handle:kte,Handle:ts,Handle:timer2,Handle:aa,Handle:rer,Handle:colorvar,Handle:glowon,Handle:GlowLoopHand,Handle:enable;
...
public Plugin:myinfo = 
{
	...
	url = "http://forums.alliedmods.net/showpost.php?p=1623173&postcount=21"
}

public OnPluginStart()
{
	enable	= CreateConVar("l4d2_mt_enable", "1", "0=Off, 1=Plugin on.", FCVAR_PLUGIN);
	kte	= CreateConVar("l4d2_mt_killsneeded", "25", "Kills needed to enable matrix time");
...
{
	PrintToChat(client,"\x05 Matrix time is ready. Press SHIFT to activate it");
}

public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	new args;
	if((buttons & IN_SPEED) == IN_SPEED && !MatrixOn && !IsFakeClient(client) && READYTOMATRIX[client])
    {
		DoMatrix(client, args);
	}
	else if((buttons & IN_SPEED) == IN_SPEED && MatrixOn && READYTOMATRIX[client])
	{
...
{
	if (GlowLoopHand != INVALID_HANDLE){
		CloseHandle(GlowLoopHand);
		GlowLoopHand = INVALID_HANDLE;
	}
	new iMaxEntities = GetMaxEntities();
	for (new iEntity = MaxClients + 1; iEntity < iMaxEntities; iEntity++)
    {

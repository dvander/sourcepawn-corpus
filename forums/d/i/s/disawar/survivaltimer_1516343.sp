...
public Plugin:myinfo = {
	name = PLUGIN_NAME,
	author = "Raoul Duke перевод raziEiL[disawar]",
...
public Action:event_TimerStart(Handle:event, const String:name[], bool:dontBroadcast){
	...
	if (!StrEqual(mode, "survival") || timerOn == true) {
		...
		tellClients("\x01[\x05SM\x01] \x05Внимание Таймер Включен!");	
		return Plugin_Handled;
	}
}
...
public Action:event_TimerEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(timerOn == true){	
		...
		currentTimer = 1;	
		tellClients("\x01[\x05SM\x01] \x05Раунд Закончен \x01-\x05 Таймер Выключен");
		timerOn = false;
...
public Action:WarnDoubleTank(Handle:timer){
	if(timerOn == true){
		showEvent("\x05ДВА ТАНКА через 10с");
		Timers[currentTimer] = INVALID_HANDLE;
		currentTimer++;
	}
	return Plugin_Handled;	

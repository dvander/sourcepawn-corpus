...
public OnPluginStart()
{
	...
	h_amount = CreateConVar("sm_lasermines_amount", "1", "The amount to give laser mines to a player each spawn (if buy mode is disabled, -1 = Infinity)", 0, true, -1.0);
	h_maxamount = CreateConVar("sm_lasermines_maxamount", "1", "The maximum amount of laser mines a player can carry. (0-Unlimited)", 0, true, 0.0);
	...
	h_use_buy_mode = CreateConVar("sm_lasermines_buymode", "0", "Enables buy mode. In this mode you will have to buy mines", 0|FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
//	HookEvent("player_death", OnPlayerDeath);
	...
	//RegConsoleCmd("sm_bm", Command_BuyMine, "Buy a laser mine");
...
public void OnLibraryAdded(const char[] sLibrary)
{
    ...
    if(!strcmp(sLibrary, "zombieplague", false))
    {
        ...
        gItem = ZP_RegisterExtraItem("lasermines", 15, 0, 0, 1);
    }
}

public Action ZP_OnClientValidateExtraItem(int client, int extraitemIndex)
{
    // Check the item's index
    if(extraitemIndex == gItem)
    {
        // Validate class
        if(ZP_IsPlayerZombie(client) || ZP_IsPlayerSurvivor(client))
        {
            return Plugin_Stop;
        }
...
public void ZP_OnClientBuyExtraItem(int client, int extraitemIndex)
{
    // Check the item's index
    if(extraitemIndex == gItem)
    {
...
//---------------------------------------------------------------------------
public Action:Command_BuyMine(client, argc)
{
	i_clients_amount[client]++;
	i_buy_limit[client]++;
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	...
	if ((buttons & IN_RELOAD) && !(iPrevButtons[client] & IN_RELOAD))
	{
		OnButtonPressed(client);
	}
	
	if ((buttons & IN_SPEED) && !(iPrevButtons[client] & IN_SPEED))
	{
		Command_PlantMine(client,0);
	}

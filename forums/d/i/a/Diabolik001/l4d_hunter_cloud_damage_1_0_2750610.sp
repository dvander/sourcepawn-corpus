...
public Plugin myinfo = 
{
	...
	// plus code snippets for cloud creation by Mortiegama
        // L4D2 - Noxious Smoker
	// https://forums.alliedmods.net/showthread.php?t=234442
        //  modded Plugin
	name = "L4D - Hunter Cloud Damage",
	author = "Finishlast",
	description = "The cloud of smoke created when a Hunter dies causes damage to the survivors",
	version = PLUGIN_VERSION,
	url = "https://forums.alliedmods.net/showthread.php?p=2710714"

}

public void OnPluginStart()
{
	CreateConVar("l4d_hunter_cloud_damage_version", PLUGIN_VERSION, " Version of L4D Hunter Cloud Damage on this server ", CVAR_FLAGS_PLUGIN_VERSION);
	
	CloudEnabled 			= CreateConVar("l4d_hunter_cloud_damage_enabled", 		"1", 									" Enable/Disable the Cloud Damage plugin ", CVAR_FLAGS);
	CloudDamage 			= CreateConVar("l4d_hunter_cloud_damage_damage", 		"2.5", 									" Amount of damage the cloud deals every half second", CVAR_FLAGS);
	CloudDuration 			= CreateConVar("l4d_hunter_cloud_damage_time", 		"10.0", 								"How long the cloud damage persists in seconds. ", CVAR_FLAGS);
	CloudRadius 			= CreateConVar("l4d_hunter_cloud_damage_radius", 		"150", 									" Radius of gas cloud damage ", CVAR_FLAGS);
	SoundPath 				= CreateConVar("l4d_hunter_cloud_damage_sound", 		"player/survivor/voice/choke_5.wav", 	"Path to the Soundfile being played on each damaging Interval", CVAR_FLAGS);
	CloudMeleeSlowEnabled 	= CreateConVar("l4d_hunter_cloud_meleeslow_enabled", 	"0", 									" Enable/Disable the Cloud Melee Slow Effect ", CVAR_FLAGS);
	DisplayDamageMessage 	= CreateConVar("l4d_hunter_cloud_message_enabled", 	"1", 									" 0 - Disabled; 1 - small HUD Hint; 2 - big HUD Hint; 3 - Chat Notification ", CVAR_FLAGS);
	CloudShake 				= CreateConVar("l4d_hunter_cloud_shake_enabled", 		"1", 									" Enable/Disable the Cloud Damage Shake ", CVAR_FLAGS);
	CloudBlocksRevive 		= CreateConVar("l4d_hunter_cloud_blocks_revive", 		"1", 									" Enable/Disable the Cloud Damage Stopping Reviving ", CVAR_FLAGS);
	
	cvarGameModeActive		= CreateConVar("l4d_hunter_cloud_gamemodesactive", 	"coop,versus,teamversus,realism", 	" Set the gamemodes for which the plugin should be activated (same usage as sv_gametypes, i.e. add all game modes where you want it active separated by comma) ", CVAR_FLAGS);
	
	HookConVarChange(FindConVar("mp_gamemode"), GameModeChanged);
	CheckGamemode();
	
	AutoExecConfig(true, "l4d_hunter_cloud_damage");
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//added code /Lines 95 to 115 and added sound on line 182
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
public Action PlayerDeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (StrContains(class, "hunter", false) != -1)
	{
		...
		{
			#if DEBUG
			PrintToChatAll("Hunterdeath caught, Plugin running");
			#endif
			...
			EmitSoundToAll("player/boomer/explode/explo_medium_09.wav", true);   //added code
		}
	}
	return Plugin_Continue;
}
...
static void CreateGasCloud(int client, float g_pos[3])
{
	...
	ShowParticle(g_pos, "weapon_pipebomb", targettime);
	ShowParticle(g_pos, "tankwall_concrete", targettime);

	CreateTimer(1.0, Point_Hurt, data, TIMER_REPEAT);
}
...
public Action Point_Hurt(Handle timer, Handle hurt)
{
	...
	for (int target = 1; target <= MaxClients; target++)
	{
		...
		{
			case 1: PrintCenterText(target, "You are taking damage from standing in a Hunter Cloud");
			
			case 2: PrintHintText(target, "You are taking damage from standing in a Hunter Cloud");
			
			case 3: PrintToChat(target, "You are taking damage from standing in a Hunter Cloud");
		}
...
public Action HookSound_Callback(Clients[64], int &NumClients, char StrSample[PLATFORM_MAX_PATH], int &Entity)
{
	...
	//agregue un retraso de 1 segundo para que esto no active cada cuadro

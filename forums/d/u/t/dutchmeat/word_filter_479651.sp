 ...
 *
 * -----
 * I have replaced the precompiled array with a cvar,
...
public OnPluginStart() 
{
	...
	CurseString = CreateConVar("wf_badwords","","Sets the words that are not allowed to say");
	CvarToArray();
}

public CvarToArray(){
	//good for +- 32 words
	new String:CString[256];
	GetConVarString(CurseString, CString, 30);
	
	if (strlen(CString) > 1){ //make sure it isn't an empty space, and if it's filled
		for(new i = 0;i < 50;i++){
			if(strlen(bad_words[i]) < 1)
				StrToken(CString,i,bad_words[i],30);
		}
	}
}

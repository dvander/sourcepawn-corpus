...
**
** Designed and written for TEAM CHEESUS (http://www.teamcheesus.com) - Slight modifications by dubbeh (https://dubbeh.net).
**
...
public OnPluginStart() {
	// Create the ConVars
	CreateConVar("jukebox_sms_version", PLUGIN_VERSION, "Version of Jukebox for Source: Stream Music System", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	cv_enabled = CreateConVar("jb_enabled", "1", "Enables Jukebox operation.", 0, true, 0.0, true, 1.0);
	cv_reserved = CreateConVar("jb_reserved", "0", "Reserves Jukebox operation for admins.", 0, true, 0.0, true, 1.0);
	...
	cv_volume_loud = CreateConVar("jb_volume_loud", "20", "Volume boost for 'loud' playback", 0, true, 0.0, true, 50.0);
	cv_volume_soft = CreateConVar("jb_volume_soft", "-20", "Volume reduction for 'soft' playback", 0, true, -50.0, true, 0.0);
	cv_volume_min = CreateConVar("jb_volume_min", "10", "The minimum permitted volume setting, as a percentage of maximum volume.", FCVAR_SPONLY, true, 1.0, true, 100.0);
	cv_popularity_decay_rate = CreateConVar("jb_popularity_decay_rate", "5", "Percentage rate of decay of popularity scores per interval time", 0, true, 0.0, true, 100.0);
	cv_popularity_decay_interval = CreateConVar("jb_popularity_decay_interval", "86400", "The interval time between popularity decay events, in seconds", 0, true, 60.0);
	cv_advertisement = CreateConVar("jb_advertisement", "0", "The number of seconds after first spawning to display the Jukebox advertisement, or set to 0 to not display the advertisement.", 0, true, 0.0, true, 300.0);
	cv_monitor_jboff_use = CreateConVar("jb_monitor_jboff_use", "3", "The number of times the user must cancel a public song to be automatically reminded and invited to adjust personal playback settings. Set to 0 to disable.", 0, true, 0.0);
	cv_playall_on_connect = CreateConVar("jb_playall_on_connect", "1", "When enabled, connecting new players will be merged into the active public stream.", 0, true, 0.0, true, 1.0);
	cv_motd_restart = CreateConVar("jb_motd_restart", "0", "When enabled, users will be reconnected to their music stream after being interrupted by an HTML-based Message of the Day display.", 0, true, 0.0, true, 1.0);
	cv_log_use = CreateConVar("jb_log_use", "0", "Enables playback logging.", 0, true, 0.0, true, 1.0);

	// WARNING - the order of these ConVars must be coordinated with the "options_names" array of strings
	cv_options_defaults[0] = CreateConVar("jb_playall_default", "1", "Default setting for if clients should be subscribed to public streams.", 0, true, 0.0, true, 1.0);
	cv_options_defaults[1] = CreateConVar("jb_volume_default", "80", "Default playback volume for new users", 0, true, 1.0, true, 100.0);
	cv_options_defaults[2] = CreateConVar("jb_queue_default", "0", "Default setting for automatic music queuing.", 0, true, 0.0, true, 1.0);
	
	cv_db_tracks = CreateConVar("jb_db_tracks", "jb_tracks", "SQL table name for the Tracks table.", FCVAR_PROTECTED|FCVAR_SPONLY|FCVAR_PRINTABLEONLY);
	cv_db_options = CreateConVar("jb_db_options", "jb_options", "SQL table name for the Options table.", FCVAR_PROTECTED|FCVAR_SPONLY|FCVAR_PRINTABLEONLY);
	cv_db_streams = CreateConVar("jb_db_streams", "jb_streams", "SQL table name for the Streams table.", FCVAR_PROTECTED|FCVAR_SPONLY|FCVAR_PRINTABLEONLY);
	cv_db_playlists = CreateConVar("jb_db_playlists", "jb_stream_tracks", "SQL table name for the Playlists table.", FCVAR_PROTECTED|FCVAR_SPONLY|FCVAR_PRINTABLEONLY);
	// cv_db_log_users = CreateConVar("jb_db_log_users", "jb_log_users", "SQL table name for the Log Users table.", FCVAR_PROTECTED|FCVAR_SPONLY|FCVAR_PRINTABLEONLY);
	cv_db_log_usernames = CreateConVar("jb_db_log_usernames", "jb_log_usernames", "SQL table name for the Log Usernames table.", FCVAR_PROTECTED|FCVAR_SPONLY|FCVAR_PRINTABLEONLY);
	cv_db_log_history = CreateConVar("jb_db_log_history", "jb_log_history", "SQL table name for the Log History table.", FCVAR_PROTECTED|FCVAR_SPONLY|FCVAR_PRINTABLEONLY);
	cv_admin_flags_playall = CreateConVar("jb_admin_flags_playall", "jz", "An admin flag string identifying the admin flags that grant Public Stream playback control permissions, or leave blank to grant access to all users.", FCVAR_PRINTABLEONLY);
	cv_admin_flags_settings = CreateConVar("jb_admin_flags_settings", "hz", "An admin flag string identifying the admin flags that grant Jukebox Admin Menu access, or leave blank to grant access to all users with general Admin Menu access rights.", FCVAR_PRINTABLEONLY);
	cv_admin_flags_reserved = CreateConVar("jb_admin_flags_reserved", "jz", "An admin flag string identifying the admin flags that grant Jukebox operation rights during the 'reserved' state.", FCVAR_PRINTABLEONLY);
...
LoadMOTDPanelHidden(client, const String:title[], const  String:msg[], type) {
	if (GetEngineVersion() == Engine_CSGO) {
		MOTDF_ShowMOTDPanel(client, title, msg, true);
	} else {
		decl String:num[3];
...
ShowHelp(client) {
	if (GetEngineVersion() == Engine_CSGO) {
		MOTDF_ShowMOTDPanel(client, "Jukebox Help", help_url, false);
	} else {
...
StopMOTD (client) {
	LoadMOTDPanelHidden (client, "Blank", "https://radio.dubbeh.net/radiooff.html", MOTDPANEL_TYPE_URL);
}

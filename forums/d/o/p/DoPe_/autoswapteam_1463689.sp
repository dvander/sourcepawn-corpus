...
new Handle:astAdvert = INVALID_HANDLE;
new Handle:astPlaySound = INVALID_HANDLE;
new Handle:astSoundFile = INVALID_HANDLE;
new Handle:astPlaySound2 = INVALID_HANDLE;
new Handle:astSoundFile2 = INVALID_HANDLE;
...
RegisterCvars()
{
	...
	astAdvert = CreateConVar("sm_autoswapteam_advert", "1", "Mensagem de aviso? [0 = NAO, 1 = SIM]", _, true, 0.0, true, 1.0);
	astPlaySound = CreateConVar("sm_autoswapteam_playsound", "1", "Tocar som? [0 = NAO, 1 = SIM]", _, true, 0.0, true, 1.0);
	astSoundFile = CreateConVar("sm_autoswapteam_sound_file", "ambient/misc/brass_bell_C.wav", "Endereco do arquivo de som");
	astPlaySound2 = CreateConVar("sm_autoswapteam_playsound2", "1", "Tocar som? [0 = NAO, 1 = SIM]", _, true, 0.0, true, 1.0);
	astSoundFile2 = CreateConVar("sm_autoswapteam_warning_sound_file", "ambient/misc/brass_bell_C.wav", "Endereco do arquivo de som");
...
public OnMapStart()
{
	...
	GetConVarString(astSoundFile, soundFile, sizeof(soundFile));
	PrecacheSound(soundFile,true);
	
	GetConVarString(astSoundFile2, soundFile2, sizeof(soundFile2));
	...
	if (GetConVarBool(astEnable))
	{
		...
		if (GetConVarInt(astType) == 5)
		{
			...
			if (nRound == maxRound)
			{
				...
				if (GetConVarBool(astPlaySound2))
				{
					PlaySound2();
				}

				if (GetConVarBool(astAdvert))
				{
				...
				if (GetConVarInt(astType) == 1)
				{
					...
					if (GetMapTimeLimit(timeLimit) && timeLimit > 0)
					{
						...
						if (GetMapTimeLeft(timeLeft) && timeLeft > 0)
						{
							...
							else if (GetConVarBool(astAdvert))
							{

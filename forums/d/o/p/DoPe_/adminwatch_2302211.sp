...
#define PLUGIN_VERSION "1.2.3 fixed by DoPe^"
...
#if _DEBUG
		LogMessage("[adminWatch DEBUG] - Total timer started in OnClientPostAdminCheck(). [%N]", client);
#endif
	}
}
...
public OnClientDisconnect(client)
{
	...
	#if _DEBUG
			LogMessage("[adminWatch DEBUG] - Checked client out in OnClientDisconnect(). [%N]", client);
	#endif
		}
...
public Action:Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (gEnabled)
	{
		for (new i = 1; i <= MaxClients; i++)
		{
			if (IsClientInGame(i) && (GetUserFlagBits(i) & gAdminFlagBits))
			{
				CheckoutClient(i);
			}
		}
#if _DEBUG
		LogMessage("[adminWatch DEBUG] - Round ended, Checkout.");
#endif		
	}
}
public Action:Event_PlayerTeam(Handle:event , const String:name[] , bool:dontBroadcast)
{
	...
	if(gEnabled && IsClientInGame(client) && (GetUserFlagBits(client) & gAdminFlagBits))
	{

		if(oldteam == 2  && newteam == 0)
		{
			if (hTimerPlayed[client] != INVALID_HANDLE)
			{
				CloseHandle(hTimerPlayed[client]);
	...
	#if _DEBUG
				LogMessage("[adminWatch DEBUG] - %N moved to team 0. timer stopped", client);
		#endif	
		}
		if(oldteam == 2  && newteam == 1)
		{
			if (hTimerPlayed[client] != INVALID_HANDLE)
			{
				CloseHandle(hTimerPlayed[client]);
		...
		#if _DEBUG
				LogMessage("[adminWatch DEBUG] - %N moved to team 1. timer stopped", client);
		#endif	
		}
		if(oldteam == 0  && newteam == 2 && hTimerPlayed[client] == INVALID_HANDLE)
		{
			if (gPrecision)
			{
				hTimerPlayed[client] = CreateTimer(60.0, TimerAddPlayed, client, TIMER_REPEAT);
			}
			else
			{
				hTimerPlayed[client] = CreateTimer(1.0, TimerAddPlayed, client, TIMER_REPEAT);
			}
		}
		if(oldteam == 1  && newteam == 2 && hTimerPlayed[client] == INVALID_HANDLE)
		{
			if (gPrecision)
			{
				hTimerPlayed[client] = CreateTimer(60.0, TimerAddPlayed, client, TIMER_REPEAT);
		...
		if(oldteam == 0  && newteam == 3 && hTimerPlayed[client] == INVALID_HANDLE)
		{
			if (gPrecision)
			{
				hTimerPlayed[client] = CreateTimer(60.0, TimerAddPlayed, client, TIMER_REPEAT);
		...
		if(oldteam == 1  && newteam == 3 && hTimerPlayed[client] == INVALID_HANDLE)
		{
			if (gPrecision)
			{
				hTimerPlayed[client] = CreateTimer(60.0, TimerAddPlayed, client, TIMER_REPEAT);
...
public Action:OnLogAction(Handle:source, Identity:ident, client, target, const String:message[])
{
	...
	Format(query, sizeof(query), DBQueriesLogs[1], bHost, authid, bName, quotedMessage, time);
...
public Action:TimerAddTotal(Handle:timer, any:client)
{
	...
	{
#if _DEBUG
		LogMessage("[adminWatch DEBUG] - Adding a minute to total tracker in TimerAddTotal(). [%N]", client);
#endif
...
public Action:TimerAddPlayed (Handle:timer, any:client)
{
	...
	{
#if _DEBUG
		LogMessage("[adminWatch DEBUG] - Adding a minute to played tracker in TimerAddPlayed(). [%N]", client);
#endif

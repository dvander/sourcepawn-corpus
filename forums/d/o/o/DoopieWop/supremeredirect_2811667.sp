...
public OnPluginStart() {
	...
	AutoExecConfig();
	CreateConVar("sm_supremeredirect_version", PLUGIN_VERSION, "Redirect Version",  FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
public LoadConfig() {
	...
	Format(sHostIP, sizeof(sHostIP), "%d.%d.%d.%d", ipaddr[0], ipaddr[1], ipaddr[2], ipaddr[3]);
...
public SMCResult:KeyValue(Handle:smc, const String:key[], const String:value[], bool:key_quotes, bool:value_quotes) {
	if (iMaxServers >= MAXSERVERS) {
		PrintToServer("[Supreme Redirect] Max server count exceeded, max: %i. If below this limit, please ensure all server entires use IPs and NOT domains!", MAXSERVERS);
		iMaxServers = 0;
	}

...
static Handle:g_hLookupBone, Handle:g_hGetBonePosition;
static bool:g_bToHead[MAXPLAYERS+1];
...
{	// Predict speed changes based on charge
	...
	{
		new Float:flCharge = GetEntPropFloat(iClient, Prop_Data, "m_fFireDuration");
...
stock GetClosestClient(iClient)
{
	...
	for(new i = 1; i <= MaxClients; i++) if(IsValidClient(i))
	{
		if(GetClientTeam(i) != iTeam && IsPlayerAlive(i) && i != iClient)
		{
			...
			{
				if(CanSeeTarget(iClient, i, iTeam, g_bAimFoV[iClient]) && IsSprite(ClientEyes[i]))
				{
...
public CopyVector(float vVec1[3], float vVec2[3])
{
	vVec2[0] = vVec1[0];
	vVec2[1] = vVec1[1];
	vVec2[2] = vVec1[2];

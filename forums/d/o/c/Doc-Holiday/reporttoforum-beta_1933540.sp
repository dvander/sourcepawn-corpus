...
new const String:PLUGIN_VERSION[] = "1.1.7-beta";
...
/* Reporter Info */
new String:g_szReporterName[MAXPLAYERS+1][MAX_NAME_LENGTH];
new String:g_szSafeReporterName[MAXPLAYERS+1][2 * MAX_NAME_LENGTH + 1];
new String:g_szReporterAuthID[MAXPLAYERS+1][32];
new String:g_szReporterIP[MAXPLAYERS+1][32];
...
new String:g_szDescription[MAXPLAYERS+1][256];
...
/* Forward Handlers */
new Handle:g_hPlayerReported = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_hPlayerReported = CreateGlobalForward("RTF_OnPlayerReported", ET_Ignore, Param_Cell, Param_Cell, Param_String, Param_String, Param_String, Param_String);
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	CreateNative("ReportPlayer", Native_ReportPlayer);
}
...
public Action:Command_SMReport(iClient, iArgs)
{
	...
	else
	{
		...
		if(iArgs >= 1)
		{
			...
			/* Get Reporter Info */
			GetClientName(iClient, g_szReporterName[iClient], sizeof(g_szReporterName[]));
			ParsePlayerName(g_szReporterName[iClient], g_szSafeReporterName[iClient], sizeof(g_szSafeReporterName[]));
			GetClientAuthString(iClient, g_szReporterAuthID[iClient], sizeof(g_szReporterAuthID[]))
			GetClientIP(iClient, g_szReporterIP[iClient], sizeof(g_szReporterIP[]));
			...
			if(iArgs >= 2 && GetConVarBool(g_Cvar_AllowCustomReports))
			{
				...
				strcopy(g_szDescription[iClient], sizeof(g_szDescription[]), szReason);
...
public DisplayTargetSelectionMenu(iClient)
{
	...
	AddPlayersToMenu(hMenu, iClient);
...
public TargetMenuHandler(Handle:hMenu, MenuAction:iAction, iClient, iItem)
{
 ...
 public SubMenuHandler(Handle:hMenu, MenuAction:iAction, iClient, iItem)
 {
	if(iAction == MenuAction_Select)
	{
		...
		strcopy(g_szDescription[iClient], sizeof(g_szDescription[]), szMenuItemsSplit[1]);
 ...
 public GenerateThreadTitle(iClient)
 {
...
public CreateReportThread(iClient)
{
	...
	switch(g_iForumSoftwareID)
	{
		...
		case FORUM_XMB:
		{
			...
			Format(szSQLQuery, sizeof(szSQLQuery), "INSERT INTO %sthreads (fid, subject, author) VALUES ('%d', '%s', '%s');", g_szTablePrefix, g_iForumID, szSafeTitle, szSafePosterName);
		}
		case FORUM_IPBOARDS:
		{
...
public CreateThreadPost(iClient)
{
	...
	switch(g_iForumSoftwareID)
	{
		...
		case FORUM_MYBB:
		{
			...
			Format(szSQLQuery, sizeof(szSQLQuery), "INSERT INTO %sposts (tid, fid, subject, uid, username, dateline, message, ipaddress, visible) VALUES ('%d', '%d', '%s', '%d', '%s', '%d', '%s', '%s', '1');", g_szTablePrefix, g_iThreadID[iClient], g_iForumID, g_szThreadTitle[iClient], g_iUserID, g_szUserName, g_iTimeStamp[iClient], szContent, g_szReporterIP[iClient]);			
		}
		case FORUM_SMF:
		{
			Format(szContent, sizeof(szContent), "%s - \n%s: %s \n%s: %s:%s \n%s: %s \n\n%s - \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n\n%s - \n%s: %s \n%s: %s \n%s: %s \n\n %s", g_szServerInfoTitle, g_szServerInfoName, g_szHostName, g_szServerInfoIP, g_szServerIP, g_szServerPort, g_szServerInfoMap, g_szMapName, g_szReportedTitle, g_szOffPlayerName, g_szSafeTargetName[iClient], g_szOffSteamID, g_szTargetAuthID[iClient], g_szOffIP, g_szTargetIP[iClient], g_szOffCat, g_szCategory[iClient], g_szOffReason, g_szReason[iClient], g_szOffDesc, g_szSafeDescription[iClient], g_szReporterTitle, g_szRepName, g_szSafeReporterName[iClient], g_szRepSteamID, g_szReporterAuthID[iClient], g_szRepIP, g_szReporterIP[iClient], g_szBranding);
			
			Format(szSQLQuery, sizeof(szSQLQuery), "INSERT INTO %smessages (id_topic, id_board, poster_time, id_member, subject, poster_name, poster_email, poster_ip, body, approved) VALUES ('%d', '%d', '%d', '%d', '%s', '%s', '%s', '%s', '%s', '1');", g_szTablePrefix, g_iThreadID[iClient], g_iForumID, g_iTimeStamp[iClient], g_iUserID, g_szThreadTitle[iClient], g_szUserName, g_szEmailAddress, g_szReporterIP[iClient], szContent);			
		}
		case FORUM_PHPBB:
		{
			Format(szContent, sizeof(szContent), "%s - \n%s: %s \n%s: %s:%s \n%s: %s \n\n%s - \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n\n%s - \n%s: %s \n%s: %s \n%s: %s \n\n %s", g_szServerInfoTitle, g_szServerInfoName, g_szHostName, g_szServerInfoIP, g_szServerIP, g_szServerPort, g_szServerInfoMap, g_szMapName, g_szReportedTitle, g_szOffPlayerName, g_szSafeTargetName[iClient], g_szOffSteamID, g_szTargetAuthID[iClient], g_szOffIP, g_szTargetIP[iClient], g_szOffCat, g_szCategory[iClient], g_szOffReason, g_szReason[iClient], g_szOffDesc, g_szSafeDescription[iClient], g_szReporterTitle, g_szRepName, g_szSafeReporterName[iClient], g_szRepSteamID, g_szReporterAuthID[iClient], g_szRepIP, g_szReporterIP[iClient], g_szBranding);
		...
		case FORUM_WBBLITE:
		{
			...
			Format(szSQLQuery, sizeof(szSQLQuery), "INSERT INTO %spost (threadID, userID, username, message, time, enableSmilies, enableBBCodes, ipAddress) VALUES ('%d', '%d', '%s', '%s', '%d', '0', '1', '%s');", g_szTablePrefix, g_iThreadID[iClient], g_iUserID, g_szUserName, szContent, g_iTimeStamp[iClient], g_szReporterIP[iClient]);
		}
		case FORUM_AEF:
		{
			Format(szContent, sizeof(szContent), "%s - \n%s: %s \n%s: %s:%s \n%s: %s \n\n%s - \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n\n%s - \n%s: %s \n%s: %s \n%s: %s \n\n %s", g_szServerInfoTitle, g_szServerInfoName, g_szHostName, g_szServerInfoIP, g_szServerIP, g_szServerPort, g_szServerInfoMap, g_szMapName, g_szReportedTitle, g_szOffPlayerName, g_szSafeTargetName[iClient], g_szOffSteamID, g_szTargetAuthID[iClient], g_szOffIP, g_szTargetIP[iClient], g_szOffCat, g_szCategory[iClient], g_szOffReason, g_szReason[iClient], g_szOffDesc, g_szSafeDescription[iClient], g_szReporterTitle, g_szRepName, g_szSafeReporterName[iClient], g_szRepSteamID, g_szReporterAuthID[iClient], g_szRepIP, g_szReporterIP[iClient], g_szBranding);
			
			Format(szSQLQuery, sizeof(szSQLQuery), "INSERT INTO %sposts (post_tid, post_fid, ptime, poster_id, poster_ip, post, use_smileys, p_approved) VALUES ('%d', '%d', '%d', '%d', '%s', '%s', '0', '1');", g_szTablePrefix, g_iThreadID[iClient], g_iForumID, g_iTimeStamp[iClient], g_iUserID, g_szReporterIP[iClient], szContent);
		}
		case FORUM_USEBB:
		{
			Format(szContent, sizeof(szContent), "%s - \n%s: %s \n%s: %s:%s \n%s: %s \n\n%s - \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n\n%s - \n%s: %s \n%s: %s \n%s: %s \n\n %s", g_szServerInfoTitle, g_szServerInfoName, g_szHostName, g_szServerInfoIP, g_szServerIP, g_szServerPort, g_szServerInfoMap, g_szMapName, g_szReportedTitle, g_szOffPlayerName, g_szSafeTargetName[iClient], g_szOffSteamID, g_szTargetAuthID[iClient], g_szOffIP, g_szTargetIP[iClient], g_szOffCat, g_szCategory[iClient], g_szOffReason, g_szReason[iClient], g_szOffDesc, g_szSafeDescription[iClient], g_szReporterTitle, g_szRepName, g_szSafeReporterName[iClient], g_szRepSteamID, g_szReporterAuthID[iClient], g_szRepIP, g_szReporterIP[iClient], g_szBranding);
			
			Format(szSQLQuery, sizeof(szSQLQuery), "INSERT INTO %sposts (topic_id, poster_id, poster_ip_addr, content, post_time, enable_smilies) VALUES ('%d', '%d', '%s', '%s', '%d', '0');", g_szTablePrefix, g_iThreadID[iClient], g_iUserID, g_szReporterIP[iClient], szContent, g_iTimeStamp[iClient]);
		}
		case FORUM_XMB:
		{
			Format(szContent, sizeof(szContent), "%s - \n%s: %s \n%s: %s:%s \n%s: %s \n\n%s - \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n%s: %s \n\n%s - \n%s: %s \n%s: %s \n%s: %s \n\n %s", g_szServerInfoTitle, g_szServerInfoName, g_szHostName, g_szServerInfoIP, g_szServerIP, g_szServerPort, g_szServerInfoMap, g_szMapName, g_szReportedTitle, g_szOffPlayerName, g_szSafeTargetName[iClient], g_szOffSteamID, g_szTargetAuthID[iClient], g_szOffIP, g_szTargetIP[iClient], g_szOffCat, g_szCategory[iClient], g_szOffReason, g_szReason[iClient], g_szOffDesc, g_szSafeDescription[iClient], g_szReporterTitle, g_szRepName, g_szSafeReporterName[iClient], g_szRepSteamID, g_szReporterAuthID[iClient], g_szRepIP, g_szReporterIP[iClient], g_szBranding);
		...
		case FORUM_IPBOARDS:
		{
			...
			Format(szSQLQuery, sizeof(szSQLQuery), "INSERT INTO %sposts (author_id, author_name, ip_address, post_date, post, topic_id, new_topic) VALUES ('%d', '%s', '%s', '%d', '%s', '%d', '1');", g_szTablePrefix, g_iUserID, g_szUserName, g_szReporterIP[iClient], g_iTimeStamp[iClient], szContent, g_iThreadID[iClient]);
		}
	}
...
public MySQL_UpdateUserPostCount(Handle:hOwner, Handle:hDatabase, const String:szError[], any:data)
{
	...
	else
	{
		...
		if(data > 0)
			...
			PrintToServer("[%s] %s has been reported: %s", g_szPrefix, g_szTargetName[data], g_szReason[data]);
		
		/* Call the Global Forward for other Plugins */
		Call_StartForward(g_hPlayerReported);
		Call_PushCell(data);
		Call_PushCell(g_iTargetClientIndex[data]);
		Call_PushString(g_szTargetAuthID[data]);
		Call_PushString(g_szCategory[data]);
		Call_PushString(g_szReason[data]);
		Call_PushString(g_szDescription[data]);
		Call_Finish();
...
stock AlertAdmins(iClient)
{
	for(new i=1; i<MaxClients;i++)
	{
		if(g_bIsUserAdmin[i] && iClient != i)
		{
			...
			PrintToChat(i, "[%s] %s has been reported: %s by %s", g_szPrefix, g_szSafeTargetName[iClient], g_szReason[iClient], g_szReporterName[iClient]);
		}
	}
}
...
public Native_ReportPlayer(Handle:hPlugin, iNumParams)
{	
	/* Reporter Info */
	new iReporterID = GetNativeCell(1);
	GetNativeString(3, g_szReporterName[iReporterID], sizeof(g_szReporterName[]));
	GetNativeString(4, g_szReporterAuthID[iReporterID], sizeof(g_szReporterAuthID[]));
	GetNativeString(5, g_szReporterIP[iReporterID], sizeof(g_szReporterIP[]));
	
	/* Make the Players name safe */
	ParsePlayerName(g_szReporterName[iReporterID], g_szSafeReporterName[iReporterID], sizeof(g_szSafeReporterName[]));
	
	/* Reported Info */
	g_iTargetClientIndex[iReporterID] = GetNativeCell(2);
	GetNativeString(6, g_szTargetName[iReporterID], sizeof(g_szTargetName[]));
	GetNativeString(7, g_szTargetAuthID[iReporterID], sizeof(g_szTargetAuthID[]));
	GetNativeString(8, g_szTargetIP[iReporterID], sizeof(g_szTargetIP[]));
	
	/* Make the Players name safe */
	ParsePlayerName(g_szTargetName[iReporterID], g_szSafeTargetName[iReporterID], sizeof(g_szSafeTargetName[]));
	
	/* Report Info */
	GetNativeString(9, g_szCategory[iReporterID], sizeof(g_szCategory[]));
	GetNativeString(10, g_szReason[iReporterID], sizeof(g_szReason[]));
	GetNativeString(11, g_szDescription[iReporterID], sizeof(g_szDescription[]));
	
	/* Make the Description SQL Safe */
	SQL_EscapeString(g_hSQLDatabase, g_szDescription[iReporterID], g_szSafeDescription[iReporterID], sizeof(g_szSafeDescription[]));
	
	if(GetConVarBool(g_Cvar_AdminAlert))
		AlertAdmins(iReporterID);
	
	if((!GetConVarBool(g_Cvar_AlertAdminNoPost) && GetConVarBool(g_Cvar_AdminAlert)) || !GetConVarBool(g_Cvar_AdminAlert))
		CreateReportThread(iReporterID);
}
...
new const String:PLUGIN_VERSION[] = "1.1.4b";
...
new Handle:g_Cvar_BlackListEnable = INVALID_HANDLE;
...
new Handle:g_hBlackList = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	/* Admin Commands */
	RegAdminCmd("sm_reload_rtf_blacklist", CMD_ReloadBlackList, ADMFLAG_GENERIC, "Reload the black list");
	RegAdminCmd("sm_reload_rtf_rules", CMD_ReloadRules, ADMFLAG_GENERIC, "Repload rules");
	...
	g_Cvar_BlackListEnable = CreateConVar("rtf_blacklist", "", "Enables the blacklist for the command. <Default: 1>");
	...
	/* MySQL Settings */
	if(!SQL_CheckConfig("ReportToForums"))
		LogError("SQL Config not found. Check your databases.cfg");	
	else
		SQL_TConnect(MySQL_ConnectionCallback, "ReportToForums");
		
	/* Create Menu and Rules Array */
	...
	/* Load the black list from File */
	if(GetConVarBool(g_Cvar_BlackListEnable))
	{
		GenerateBlackList();
	}
}
...
public OnConfigsExecuted()
{
	...
	switch(GetConVarInt(g_Cvar_ForumSupport))
	{
		...
		case 8: g_iForumSoftware = FORUM_USEBB;
		case 9: g_iForumSoftware = FORUM_XMB;
		case 10: g_iForumSoftware = FORUM_IPBOARDS;
...
public Action:CMD_ReloadRules(iClient, iArgs)
{
	ClearArray(g_aReportReason);
	CreateReasonArray();
}

public Action:CMD_ReloadBlackList(iClient, iArgs)
{
	ClearTrie(g_hBlackList);
	GenerateBlackList();
}
...
public Action:CMD_SMReport(iClient, iArgs)
{
	...
	{
		decl String:szTemp[32], String:szAuthID[32];
		GetClientAuthString(iClient, szAuthID, sizeof(szAuthID));
		if(GetConVarBool(g_Cvar_BlackListEnable) && GetTrieString(g_hBlackList, szAuthID, szTemp, sizeof(szTemp)))
		{
			PrintToChat(iClient, "[%s] You do not have permission for this command.", g_szPrefix);
...
public CreateReportThread(iClient)
{
	...
	Format(g_szThreadTitle[iClient], sizeof(g_szThreadTitle[]), "%s - %s", g_szTargetAuthId[iClient], szReason);
...
{
	decl String:szBuffer[1024], String:szTimeStamp[10], String:szDebugFile[PLATFORM_MAX_PATH];
	new iArraySize = GetArraySize(g_hDebugArray[iClient]);
	FormatTime(szTimeStamp, sizeof(szTimeStamp), "%j-%H%M", GetTime());
	Format(szDebugFile, sizeof(szDebugFile), "addons/sourcemod/logs/rtf_debug-%s.log", szTimeStamp);
...
stock GenerateBlackList()
{
	if(g_hBlackList == INVALID_HANDLE)
		g_hBlackList = CreateTrie();
	
	new Handle:hBlackList = INVALID_HANDLE;
	decl String:szBlackListFile[PLATFORM_MAX_PATH];
	Format(szBlackListFile, sizeof(szBlackListFile), "cfg/reporttoforums/blacklist.txt");
	
	if(!FileExists(szBlackListFile))
	{
		hBlackList = OpenFile(szBlackListFile, "w");
		WriteFileLine(hBlackList, "//This file contains STEAM_ID's of those who are not allowed to use the sm_report command");
		WriteFileLine(hBlackList, "//Simply add the steamid of a player in the following format.");
		WriteFileLine(hBlackList, "//STEAM_0:1:23456789  ONE STEAM ID PER LINE");
		WriteFileLine(hBlackList, "");
	...
	else
	{
		...
		hBlackList = OpenFile(szBlackListFile, "r");
		while(ReadFileLine(hBlackList, szBuffer, sizeof(szBuffer)) && !IsEndOfFile(hBlackList))
		{
			if(!ParseLineOfText(szBuffer, true))
				continue;
			
			if(StrContains(szBuffer, "STEAM"))
				SetTrieString(g_hBlackList, szBuffer, szBuffer);
		}
		CloseHandle(hBlackList);
	}
}

...
public OnPluginStart() {
	...
	RegConsoleCmd("equip",          Command_Equip, "Restrict classes in TF2.");
}
...
public Action:Command_Equip(client, args) {
	if (!g_bFull[client]) {
		g_iClass[client] = _:TF2_GetPlayerClass(client);
		
		if (IsFull(GetClientTeam(client), g_iClass[client]) && !IsImmune(client)) {
			PickClass(client);
			g_bFull[client] = true;
		}
	}
...
public Event_ChangeClass(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (IsFull(iTeam, iClass) && !IsImmune(iClient)) {
		...
		PrintCenterText(iClient, "The Class You Are Trying To Join Is Full!");
		PrintToChat(iClient, "\x04The Class You Are Trying To Join Is Full!");
		PrintToConsole(iClient, "[SM] The Class You Are Trying To Join Is Full!");
...
bool:IsImmune(iClient) {
	return GetConVarBool(g_hImmunity) && GetUserFlagBits(iClient) & (ADMFLAG_RESERVATION|ADMFLAG_ROOT);
}

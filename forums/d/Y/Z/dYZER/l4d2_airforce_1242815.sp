...
/* Sound */
#define CHARGESOUND 	"npc/soldier1/misc15.wav"
#define CHARGEDUPSOUND	"animation/overpass_jets.wav"
#define AWPSHOT			"animation/jets/jet_by_02_mono.wav"
...
{
	name = "[L4D2] call the airforce",
	author = "original ztar/  airforce test dyzer",
	description = "Sniper rifles can be charged up and fired to create a huge explosion",
	version = PLUGIN_VERSION,
	url = "http://forums.alliedmods.net/showthread.php?p=1121995 < ori  airforce requests"
}
...
public OnPluginStart()
{
	...
	l4d2_lw_lethalforce		= CreateConVar("l4d2_lw_lethalforce","5000.0", "Lethal Weapon force", CVAR_FLAGS);
	l4d2_lw_chargetime		= CreateConVar("l4d2_lw_chargetime","5", "Lethal Weapon charge time", CVAR_FLAGS);
	l4d2_lw_shootonce		= CreateConVar("l4d2_lw_shootonce","0", "Survivor can use Lethal Weapon once per round", CVAR_FLAGS);
	l4d2_lw_ff				= CreateConVar("l4d2_lw_ff","1", "Lethal Weapon can deal direct damage to other survivors", CVAR_FLAGS);
	...
	// Additional ConVars
	l4d2_lw_flash				= CreateConVar("l4d2_lw_flash", "0", "Enable screen flash");
	...
	l4d2_lw_chargeparticle		= CreateConVar("l4d2_lw_chargeparticle", "0", "Enable showing electric particles when charged");
	l4d2_lw_useammo				= CreateConVar("l4d2_lw_useammo", "0", "Enable and require use of addtional ammunition");
	l4d2_lw_shake				= CreateConVar("l4d2_lw_shake", "1", "Enable screen shake during explosion");
	l4d2_lw_shake_intensity		= CreateConVar("l4d2_lw_shake_intensity", "20.0", "Intensity of screen shake");
	...
	AutoExecConfig(true, "l4d2_airforce");
}
...
public Action:ChargeTimer(Handle:timer, any:client)
{
	...
	//!(StrEqual(weapon, "weapon_sniper_military") && GetConVarInt(l4d2_lw_g3sg1)) &&
//		!(StrEqual(weapon, "weapon_sniper_awp") && GetConVarInt(l4d2_lw_awp)) &&
	...
	new inCharge = (buttons & IN_RELOAD)
	...
	*/
	...
	if (inCharge)// && GetEntData(WeaponClass, ClipSize))
	{
		...
		else
		{
			...
			CreateTimer(0.1, ReleaseTimer, client);
			ExplodeMain(trsPos002);

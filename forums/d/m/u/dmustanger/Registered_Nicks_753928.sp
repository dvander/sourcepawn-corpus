...
public OnPluginStart()
{
	...
	BuildPath(Path_SM, AdminFile, sizeof(AdminFile), "configs/admins_simple.ini");
...
public T_RegisterNick(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	{
		GetClientIP(data, steam, sizeof(steam));
		Format(text, sizeof(text), "\"!%s\" \"a\"", steam);
	}
	else
	{
		GetClientAuthString(data, steam, sizeof(steam));
	...
	if (reserveslot)
	{
		Admin = OpenFile(AdminFile, "at+");
		WriteFileLine(Admin, text);
		CloseHandle(Admin);
	}

...
public OnPluginStart()
{
	...
	WatchlistPrint = CreateTimer(100.0, ShowWatch,
...
public T_CheckUser(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if (SQL_FetchRow(hndl)) {
		...
		// Output a message
		IRC_Broadcast(IRC_CHANNEL_BOTH, "[Watchlist] Player %s is on the watchlist for %s", name, reason);
		for(new x = 1; x < MAXPLAYERS; x++)
		{
			if(IsClientInGame(x) && GetUserAdmin(x) != INVALID_ADMIN_ID)
			PrintToChat(x, "[Watchlist] Player %s is on the watchlist for %s", name, reason);
		}
	}
}
...
AddWatch(client, target, String:steam[], String:reason[]) {
	...
	if (target > 0) {
		...
		PrintToChat(client, "[Watchlist] Added player %s to the watchlist for %s", player_name, reason);
	}

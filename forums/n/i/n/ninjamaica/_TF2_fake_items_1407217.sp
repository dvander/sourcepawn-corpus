...
#define OBTAIN_PURCHASED    3
#define OBTAIN_UNBOXED      4
#define OBTAIN_GIFT         5
#define OBTAIN_EARNED       8
#define OBTAIN_REFUND       9
#define OBTAIN_WRAPPED      10
...
public Action:CommandFakeItem(iClient, iArgs) {
    ...
    if (iArgs >= 3) {
        ...
        if (StrContains(strQuality, "gen", false) != -1) iQuality = QUALITY_GENUINE;
    }
    ...
    if (iArgs >= 4) {
        ...
        if (StrContains(strObtain, "receive", false) != -1) iObtain = OBTAIN_GIFT;
        if (StrContains(strObtain, "earn", false) != -1) iObtain = OBTAIN_EARNED;
        if (StrContains(strObtain, "purchase", false) != -1) iObtain = OBTAIN_PURCHASED;
        if (StrContains(strObtain, "refund", false) != -1) iObtain = OBTAIN_REFUND;
        if (StrContains(strObtain, "wrap", false) != -1) iObtain = OBTAIN_WRAPPED;
    }

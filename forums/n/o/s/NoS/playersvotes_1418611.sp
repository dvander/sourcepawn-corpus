	  ...
	  )
	{
		   ...
		   (nVoteLimit > g_nVoteCount[type][client]))
		{
			if(nFromLast >= GetConVarInt(g_hVotesInterval))
			{
				...
				{
					PrintToChat(client, "%t", "voting not allowed", GetConVarInt(g_hVoteDelay[type]) - nFromStart);
				}
			}
			else
			{
				PrintToChat(client, "%t", "voting not allowed again", GetConVarInt(g_hVotesInterval) - nFromLast);
			}
		}
		else
		{
			PrintToChat(client, "%t", "votes spent", nVoteLimit, sVoteName);
		}
	}
	else
	{
		PrintToChat(client, "%t", "is disabled");
	}
}
...
{
	new Handle:hVoteMenu, bool:isEmpty = true;
	...
	if (nVoteLimit > 0)
	{
		Format(sTitle, sizeof(sTitle), "%t", "votes remaining", nVoteLimit - g_nVoteCount[type][client]);
	}
	else
	{
	...
	if (isEmpty) {
		ReplyToCommand(client, "No voting targets available");
	}
}

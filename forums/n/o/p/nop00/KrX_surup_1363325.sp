...
public OnPluginStart() {
	...
	{
		// nop - 2010-dec-05 : the string signature was not working on my linux server, so i extracted this binary signature from server.so
		if (!PrepSDKCall_SetSignature(SDKLibrary_Server, "\x55\x89\xE5\x57\x56\x53***\xE8\x9C\xF8\x94\xFF", 14))
		{
			if (!PrepSDKCall_SetSignature(SDKLibrary_Server, "@_ZN13CTerrorPlayer10AddUpgradeE19SurvivorUpgradeType", 0))
        	{
            	PrintToServer("Can't locate AddUpgrade, and i won't be able to do anything without it, sorry :-/");
        	}
		}
	}
	...
	{
		// nop - 2010-dec-05 : the string signature was not working on my linux server, so i extracted this binary signature from server.so
		if (!PrepSDKCall_SetSignature(SDKLibrary_Server, "\x55\x89\xE5\x57\x56\x53***\xE8\xEC\x61\x96\xFF", 14))
		{
			if (!PrepSDKCall_SetSignature(SDKLibrary_Server, "@_ZN13CTerrorPlayer13RemoveUpgradeE19SurvivorUpgradeType", 0))
			{
            	PrintToServer("Can't locate RemoveUpgrade, and i won't be able to do anything without it, sorry :-/");
			}
		}
	}
	PrepSDKCall_AddParameter(SDKType_PlainOldData, SDKPass_ByValue);
	...
	UpgradeAllowed[8] = CreateConVar("surup_allow_large_clip", "1", "Give the Large Clip upgrade? (0=No,1=Yes,2=At Spawn)", CVAR_FLAGS, true, 0.0, true, 2.0);

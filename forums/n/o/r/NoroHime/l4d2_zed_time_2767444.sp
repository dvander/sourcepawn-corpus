...
ConVar Enable;
ConVar TeamRestrict;
ConVar ZedInfectedchance;
ConVar ZedSurvivorchance;
ConVar ZedPipeChance;
ConVar ZedExplosionChance;
ConVar ZedModes;
ConVar ZedTank;
ConVar ZedWitch;
ConVar ZedTimer;
...
ConVar ZedHunter;
ConVar ZedBoomer;
ConVar ZedMaxAliveHuman;
ConVar ZedHeadshotMultiplier;
ConVar ZedNormalKillEnable;
ConVar ZedHighlightKillChance;
ConVar ZedHighlightKillCount;
ConVar ZedHighlightKillInterval;
...
public OnPluginStart()
{
	...
	ZedAmazing = CreateConVar("l4d2_zed_extreme_shot", "75", "The distance required to increase the chance of Zed time happening on a kill.", FCVAR_NOTIFY);
	...
	ZedNormalKillEnable = CreateConVar("l4d2_zed_normal_kill_enable", "1", "trigger enable if survivor normal kill zombies without special", FCVAR_NOTIFY);
	ZedHighlightKillChance = CreateConVar("l4d2_zed_highligh_kill_chance", "1", "1 in chance of kill multiple zombies", FCVAR_NOTIFY);
	ZedHighlightKillCount = CreateConVar("l4d2_zed_highligh_kill_count", "5", "count killed to trigger highlight ZedTime ", FCVAR_NOTIFY);
...
{
	if(!IsAllowedGameMode() || !GetConVarInt(Enable)) return;
	...
	if(!isAliveHumanSurvivor(attacker)) return;  //trigger alive human survivor only

	if(victim && IsClientInGame(victim) && GetClientTeam(victim) == 2) return; //do not victim survivor trigger

	if(attacker && isAliveHumanSurvivor(attacker)){
		float time = GetEngineTime();
		CountKilled[attacker]++;
		if(TimeFirstKill[attacker] && (time - TimeFirstKill[attacker]) < GetConVarFloat(ZedHighlightKillInterval)){
			if(CountKilled[attacker] >= GetConVarInt(ZedHighlightKillCount))
				if(GetRandomInt(1, GetConVarInt(ZedHighlightKillChance)) == 1){
	...
	{	
		if(GetConVarBool(ZedNormalKillEnable) && GetRandomInt(1, GetConVarInt(ZedSurvivorchance)) == 1)
		{
			if(ZedTimeGoing) return;
			ZedTime();
			FadeClientVolume(attacker, 90.0, 0.2, GetConVarFloat(ZedTimer)-0.2, 0.2);
		...
		if(GetVectorDistance(pos1, pos2, false) > GetConVarInt(ZedAmazing)*16) //distance greater than 16*150=2400 unit(too far)
		{

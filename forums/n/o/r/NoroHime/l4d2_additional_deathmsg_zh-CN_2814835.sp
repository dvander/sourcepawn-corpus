...
void OnWitchKilled(Event event, const char[] name, bool dontBroadcast) {
	...
	if (IsClient(attacker)) {
		...
		FormatEx(output, sizeof(output), " %s 杀死了 Witch", GetEntityTranslatedName(attacker));
...
void OnPlayerDeath(Event event, const char[] name, bool dontBroadcast) {
	...
	if (IsClient(victim)) {
		...
		if (!IsClient(attacker))
			FormatEx(output, sizeof(output), " %s 杀死了 %s", GetEntityTranslatedName(attacker), GetEntityTranslatedName(victim));
		else if (attacker == victim && GetClientTeam(victim) == 2)
			FormatEx(output, sizeof(output), " %s 死于内出血", GetEntityTranslatedName(victim));
		else
			return;
...
void OnPlayerIncapacitated(Event event, const char[] name, bool dontBroadcast) {
	...
	if (IsClient(victim)) {
		...
		if (attacker == victim && GetClientTeam(attacker) == 2) {
			...
			FormatEx(output, sizeof(output), " %s 制服了自己", GetEntityTranslatedName(victim));
		...
		// entity => player
			...
			FormatEx(output, sizeof(output), " %s 击倒了 %s", GetEntityTranslatedName(attacker), GetEntityTranslatedName(victim));
		else
			return;

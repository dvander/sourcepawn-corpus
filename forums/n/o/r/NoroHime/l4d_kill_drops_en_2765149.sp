...
new Handle: l4d_kill_addhp_difficult_easy;
new Handle: l4d_kill_addhp_difficult_normal;
new Handle: l4d_kill_addhp_difficult_hard;
new Handle: l4d_kill_addhp_difficult_expert;
...
public OnPluginStart()
{
	...
	l4d_kill_addhp_difficult_easy = CreateConVar("l4d_kill_addhp_difficult_easy", "0.75", "health recover factor of easy difficult", FCVAR_PLUGIN);
	l4d_kill_addhp_difficult_normal = CreateConVar("l4d_kill_addhp_difficult_normal", "1.0", "health recover factor of normal difficult", FCVAR_PLUGIN);
	l4d_kill_addhp_difficult_hard = CreateConVar("l4d_kill_addhp_difficult_hard", "1.25", "health recover factor of hard difficult", FCVAR_PLUGIN);
	l4d_kill_addhp_difficult_expert = CreateConVar("l4d_kill_addhp_difficult_expert", "1.5", "health recover factor of expert diffult", FCVAR_PLUGIN);
...
AddHealth(client, Float:add)
{
	...
	char difficult[24];
	GetConVarString( FindConVar("z_difficulty"), difficult, sizeof(difficult));
	new Float:difficultMult;

	if(StrEqual(difficult, "Easy", false)) 
		difficultMult = GetConVarFloat(l4d_kill_addhp_difficult_easy);
	else if(StrEqual(difficult, "Normal", false)) 
		difficultMult = GetConVarFloat(l4d_kill_addhp_difficult_normal);
	else if(StrEqual(difficult, "Hard", false)) 
		difficultMult = GetConVarFloat(l4d_kill_addhp_difficult_hard);
	else if(StrEqual(difficult, "Impossible", false)) 
		difficultMult = GetConVarFloat(l4d_kill_addhp_difficult_expert);
	...
	if(GetClientTeam(client) == 2) 
	{
		...
		addhp=RoundFloat(newhp * difficultMult);

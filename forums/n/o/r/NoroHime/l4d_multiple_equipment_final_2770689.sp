...
void DropSecondaryItem(int client)
{
	...
	for(int slot = 0; slot <= 4; slot++)
	{
		if(!StrEqual(ItemName[client][slot], ""))
		{
			...
			int upgradeBit = ItemInfo[client][slot][2];
			int upammo = ItemInfo[client][slot][3];

			if(slot==0)
			{
				if(L4D2Version)
					DropPrimaryWeapon_l4d2(client,ItemName[client][slot], ammo, clip, upgradeBit, upammo);
				else

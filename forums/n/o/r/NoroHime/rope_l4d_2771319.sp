...
{ 
	...
	if(witchid>0 && attacker>0)
	{
		...
		{
			PrintToChatAll("\x04[\x05Rope\x04]\x05 %N \x03 Pickup the Rope..", attacker);
			GiveRope(attacker);
		}
	}
	return Plugin_Handled;
...
DisableRope(client)
{
	...
	StopRope(client);
	PrintCenterText(client, "Rope Dropped"); 
}
EnableRope(client)
{

	if (isInfected(client)) {
		PrintCenterText(client, "Infected Not Allow"); 
		return;
	}
	if(g_rope_enabled[client])return;
	...
	PrintCenterText(client, "Pickup the Rope"); 
	PrintToChat(client, "\x03 Tap \x05Scope\x03 to Shoot");
	PrintToChat(client, "\x03 Press \x05Shift\x03 or \x05Ctrl\x03 to Control");


}
int grabbed[MAXPLAYERS + 1];
StartRope(client)
{
...
StopRope(client)
{
	...
	new Float:pos[3];
	// PrintToChat(client, "target %d source %d particle %d", ent1, ent2, g_rope_ent[client][2]);
	...
	if(IsEnt(ent1))
	{
		...
		RequestFrame(RemoveEntityPost, ent1);
	}
	if(IsEnt(ent2))
	{
		AcceptEntityInput(ent2, "ClearParent");  
		TeleportEntity(ent2, pos, pos, NULL_VECTOR);	
		RequestFrame(RemoveEntityPost, ent2);
	}	
	
	if(IsEnt(particle))
	{
		AcceptEntityInput(particle, "stop");  
		RequestFrame(RemoveEntityPost, particle);
	}
...
void RemoveEntityPost(int entity) {
	RemoveEntity(entity);
	RemoveEdict(entity);
}
CreateDummyEnt()
...
{
	if(!g_rope_enabled[client] && !g_rope_enabled[grabbed[client]]) return Plugin_Continue; 
	
	
	new bool:start_rope= ((buttons & IN_ZOOM) && !(g_last_button[client] & IN_ZOOM));
	if(start_rope)
	{
		if (g_rope_state[client] == state_none)
			StartRope(client);
		else 
			StopRope(client);

		if (!(g_rope_state[grabbed[client]] ==state_none))
			StopRope(client);
	}	

...
public OnPluginStart()
{
	...
	hCAEnable	= CreateConVar("sm_adv_connect_announce", "0", 	"Enable connect announce (1 - humans, 2 - bots)", FCVAR_PLUGIN, true, 0.0, true, 3.0);	
	hSProt		= CreateConVar("sm_adv_spawn_protection", "0", "Spawn protection time (0 to disable)", FCVAR_PLUGIN, true, 0.0);	
	...
	hAdmVision	= CreateConVar("sm_adv_admin_vision", 	"0", 	"Enable admin vision (for: 1 - admins, 2 - fake clients, 4 - all, when sv_alltalk 1, 8 - all)", FCVAR_PLUGIN, true, 0.0, true, 15.0);
	hAdmFlags	= CreateConVar("sm_adv_admin_flags", 	"z", 	"Set of admin flags, which allows admin vision", 	FCVAR_PLUGIN);
	...
	RegAdminCmd("sm_melee", 		Command_Melee, 		ADMFLAG_ROOT, 		"Remove all weapons, except melee weapon");
	RegAdminCmd("sm_equip", 		Command_Equip, 		ADMFLAG_ROOT, 		"Remove all weapons, and give this weapon for all");
	RegAdminCmd("sm_bury", 		Command_Bury, 		ADMFLAG_ROOT, 		"Bury target");
	RegAdminCmd("sm_unbury", 	Command_Unbury, 		ADMFLAG_ROOT, 		"Unbury target");
	RegAdminCmd("sm_hp", 		Command_HP, 			ADMFLAG_ROOT, 		"Set target's health points");
	RegAdminCmd("sm_armour", 	Command_Armour, 		ADMFLAG_ROOT, 		"Set target's armour");
	RegAdminCmd("sm_give", 		Command_Give, 		ADMFLAG_BAN, 		"Give item to target");
	RegAdminCmd("sm_speed", 		Command_Speed, 		ADMFLAG_ROOT, 		"Set target's speed");
	RegAdminCmd("sm_frags", 		Command_Frags, 		ADMFLAG_ROOT, 		"Change target's frags");
	RegAdminCmd("sm_deaths", 	Command_Deaths, 		ADMFLAG_ROOT, 		"Change target's deaths");
	RegAdminCmd("sm_balance", 	Command_Balance, 	ADMFLAG_ROOT, 	"Balance teams");
	RegAdminCmd("sm_shuffle", 	Command_Shuffle, 	ADMFLAG_ROOT, 		"Shuffle players");
	RegAdminCmd("sm_exec", 		Command_Exec, 		ADMFLAG_ROOT, 		"Execute command on target");
	RegAdminCmd("sm_fexec", 		Command_FExec, 		ADMFLAG_ROOT, 		"Fake-execute command on target");
	...
	RegAdminCmd("sm_god", 		Command_God, 		ADMFLAG_ROOT, 		"Set target's godmode state");
	RegAdminCmd("sm_rr", 		Command_RR, 			ADMFLAG_BAN, 	"Restart round");
	RegAdminCmd("sm_extend", 	Command_Extend, 		ADMFLAG_BAN, 	"Extend map");
	...
	RegAdminCmd("sm_url", 		Command_Url, 		ADMFLAG_ROOT, 	"Open URL for target");
	RegAdminCmd("sm_getmodel", 	Command_GetModel, 	ADMFLAG_ROOT, 		"Get target's model name");
	RegAdminCmd("sm_drop", 		Command_Drop, 		ADMFLAG_ROOT, 		"Drop target's weapon");
	RegAdminCmd("sm_dropslot", 	Command_DropSlot, 	ADMFLAG_ROOT, 		"Drop target's weapon from slot");
	RegAdminCmd("sm_spec", 		Command_Spec, 		ADMFLAG_ROOT, 		"Move target to spectator");
	RegAdminCmd("sm_teamswap", 	Command_TeamSwap, 	ADMFLAG_ROOT, 		"Swap teams");
	RegAdminCmd("sm_team", 		Command_Team, 		ADMFLAG_ROOT, 		"Set target's team");
	RegAdminCmd("sm_swap", 		Command_Swap, 		ADMFLAG_ROOT, 		"Swap target's team");
	RegAdminCmd("sm_lswap", 		Command_LSwap, 		ADMFLAG_ROOT, 		"Swap target's team later");
	RegAdminCmd("sm_exch", 		Command_Exchange, 	ADMFLAG_ROOT, 		"Exchange targets in teams");
	RegAdminCmd("sm_lexch", 		Command_LExchange, 	ADMFLAG_ROOT, 		"Exchange targets in teams later");

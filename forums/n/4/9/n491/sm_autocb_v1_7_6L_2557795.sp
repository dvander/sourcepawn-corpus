...
*	12/05/2020 Version 1.7.6L – Fixed incorrect height speed initialization; fixed rare case, when a player could touch single zombie 2 times.
*							Method of Auto Commonboost has simplified: now player can do auto-cb from idle zombie; increased ticks delay;
*							increased difference.
*							WARNING: This simplified (light) version isn't for speedruns. DO NOT use!
*/
...
public Action:OnPlayerRunCmd(int client, int &buttons)
{
	if (GetConVarBool(g_ConVar_CB))
	{
		if (!g_bIsCB_RootKey[client])
		{
			if (buttons & IN_JUMP)
			{
				if ((GetGameTickCount() - g_iTicks[client]) <= 4)
				{
					if (GetEntPropEnt(client, Prop_Send, "m_hGroundEntity") == g_iEntity[client])
					{
						...
						if (g_fDifference[client] < FLOAT_PI/2)
						{
		...
		else
		{
			if (IsPlayerAlive(client) && !GetEntProp(client, Prop_Send, "m_isIncapacitated"))
			{
				...
				decl String:sEntName[128]; sEntName[0] = 0;
				...
				PrintToConsole(client, "Ticks        : %d", g_iTicks[client]);
				PrintToConsole(client, "m_vecVelocity: Vector(%.03f, %.03f, %.03f)", vecVel[0], vecVel[1], vecVel[2]);
				PrintToConsole(client, "Version      : %s", PLUGIN_VER);

...
public OnPluginStart() {
	...
	g_hKHForward = CreateGlobalForward("OnKnifeHit", ET_Event, Param_Cell, Param_Cell,Param_CellByRef, Param_CellByRef);
			...
			if (IsClientIndex(victim) && IsClientInGame(attacker)) {
				...
				if (GetConVarBool(g_CVarFF) || (GetClientTeam(victim) != GetClientTeam(attacker))) {
					...
					if (IsValidEntity(g_iPointHurt)) {
						...
						new damage = (headShot) ? StringToInt(g_sHSDamage) : StringToInt(g_sDamage);

...
new g_b_WitchControl[MAXPLAYERS+1], Float:g_PlayerGameTime[MAXPLAYERS+1], bool:g_b_OnLadder[MAXPLAYERS+1],
	...
	Handle:h_CvarWitchSpeed, Handle:h_CvarMode, Handle:h_CvarWitchAttackOn, Handle:h_CvarMessageType, Float:g_b_WitchJump[MAXPLAYERS+1]
...
public OnPluginStart()
{
	...
	h_CvarWitchAttackOn = CreateConVar("l4d2_witch_attack_on", "1", "Whether or not player controlling witch can use witch's attack. 0 - player cannot use witch's attack, 1 (default) - player can use witch's attack", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 1.0)
	...
	AutoExecConfig(true, "l4d2_witch_control");
...
public Action:OnPlayerRunCmd(i_Client, &i_Buttons, &i_Impulse, Float:f_PlayerVelocity[3], Float:f_PlayerAngles[3], &i_Weapon)
{
	...
	{
		if (i_Buttons & IN_USE && GetClientTeam(i_Client) == TEAM_INFECTED && !IsPlayerOnFire(i_Client))
		{
	...
	if (f_Rage)
	{
		if (GetConVarInt(h_CvarWitchAttackOn) == 1)
		{

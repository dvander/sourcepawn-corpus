...
#define HHHLaught "hhh/laugh"
#define HHHRage "hhh/hit.wav"
#define HHHRage2 "hhh/rage2.wav"
#define HHHTerror "hhh/terror1.mp3"
#define HHHAttack "hhh/grr"
#define HHHStab "hhh/stab0"
...
#define HitlerStart1 "hitler/intro1.wav"
#define HitlerStart2 "hitler/intro2.wav"
...
//	RegConsoleCmd("haleme", SkipHalePanelCmd);
	...
	RegAdminCmd("hale_special", Command_MakeNextSpecial, ADMFLAG_VOTE, "Call a special to next round.");
	RegAdminCmd("hale_addpoints", Command_Points, ADMFLAG_VOTE, "hale_addpoints <target> <points> - Add queue points to user.");
	RegAdminCmd("hale_point_enable", Command_Point_Enable, ADMFLAG_VOTE, "Enable CP. Only with hale_point_type = 0");
	RegAdminCmd("hale_point_disable", Command_Point_Disable, ADMFLAG_VOTE, "Disable CP. Only with hale_point_type = 0");
...
public AddToDownload()
{
	...
	if (bSpecials)
	{
		...
		PrecacheSound(HitlerStart1, true);
		PrecacheSound(HitlerStart2, true);
	...
	for (i = 1; i <= 5; i++)
	{
		...
		if (i <= 3)
		{
			...
			PrecacheSound(s, true);
			Format(s, PLATFORM_MAX_PATH, "sound/%s", s);
			Format(s, PLATFORM_MAX_PATH, "%s%i.wav", HHHStab, i);
			PrecacheSound(s, true);
			Format(s, PLATFORM_MAX_PATH, "sound/%s", s);
		}
		if (i <= 4)
	...
	PrecacheSound("hitler/stab1.wav", true);
	PrecacheSound("hitler/stab2.wav", true);
	PrecacheSound("hitler/stab3.wav", true);
	PrecacheSound("hitler/stab4.wav", true);
	PrecacheSound("hhh/ded1.wav", true);
	PrecacheSound("hhh/ded2.wav", true);
	PrecacheSound("hhh/terror1.mp3", true);
...
stock bool:CheckNextSpecial()
{
	...
	while (Incoming == VSHSpecial_None || (Special && Special == Incoming))
	{
		...
		if (Special != VSHSpecial_Hale && !GetRandomInt(0, 3)) Incoming = VSHSpecial_Hale;
		else
		{
			switch (Incoming)
			{
...
public Action:StartResponceTimer(Handle:hTimer)
{
	...
	switch (Special)
	{
		...
		case VSHSpecial_HHH: Format(s, PLATFORM_MAX_PATH, "saxton_hale/summon1.wav");
		...
		{
			if (!GetRandomInt(0, 1))
				strcopy(s, PLATFORM_MAX_PATH, HitlerStart1);
			else
				strcopy(s, PLATFORM_MAX_PATH, HitlerStart2);
		...
		case VSHSpecial_Hale:	Format(s, PLATFORM_MAX_PATH, "%s%i.mp3", HaleStart, GetRandomInt(1, 12));
	}
	EmitSoundToAll(s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, pos, NULL_VECTOR, false, 0.0);
	EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, pos, NULL_VECTOR, false, 0.0);
...
EquipSaxton(client)
{
	...
	switch (Special)
	{
		case VSHSpecial_Vagineer:
		{
			SaxtonWeapon = SpawnWeapon(client, "tf_weapon_wrench", 169, 100, 5, "68 ; 2.0 ; 2 ; 2.769 ; 259 ; 1.0 ; 150 ; 1.0");
			SetEntProp(SaxtonWeapon, Prop_Send, "m_iWorldModelIndex", -1);
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
		}
		case VSHSpecial_HHH:
		{
			SaxtonWeapon = SpawnWeapon(client, "tf_weapon_sword", 266, 100, 5, "68 ; 2.0 ; 2 ; 2.769 ; 259 ; 1.0 ; 252 ; 0.80");
			SetEntProp(SaxtonWeapon, Prop_Send, "m_iWorldModelIndex", -1);
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
			HaleCharge = -500;
		}
		case VSHSpecial_CBS:
		{
			SaxtonWeapon = SpawnWeapon(client, "tf_weapon_club", 171, 100, 5, "68 ; 2.0 ; 2 ; 2.769 ; 259 ; 1.0");
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
			SetEntProp(client, Prop_Send, "m_nBody", 0);
		}
		case VSHSpecial_Mario:
		{
			SaxtonWeapon = SpawnWeapon(client, "tf_weapon_wrench", 5, 100, 5, "68 ; 2.0 ; 2 ; 2.769 ; 259 ; 1.0 ; 326 ; 1.8");
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
		}
		case VSHSpecial_Hitler:
		{
			SaxtonWeapon = SpawnWeapon(client, "tf_weapon_bonesaw", 37, 100, 5, "68 ; 2.0 ; 2 ; 2.769 ; 259 ; 1.0");
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
			SetEntProp(client, Prop_Send, "m_nBody", 0);
		}
		case VSHSpecial_Kinky:
		{
			SaxtonWeapon = SpawnWeapon(client, "tf_weapon_shovel", 447, 69, 5, "68 ; 2.0 ; 2 ; 2.769 ; 259 ; 1.0");
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
			SetEntProp(client, Prop_Send, "m_nBody", 0);
		}
		case VSHSpecial_Hale:
		{
			...
			Format(attribs, sizeof(attribs), "68 ; 2.0 ; 2 ; 2.769 ; 259 ; 1.0 ; 252 ; 0.6 ; 214 ; %d", GetRandomInt(9999, 99999));
...
//			new Handle:hItemOverride = PrepareItemHandle(hItem, _, _, "116 ; 4.0", true);
		...
		case 730:
		{
			new Handle:hItemOverride = PrepareItemHandle(hItem, _, _, "76 ; 0.5 ; 6 ; 0.01 ; 241 ; 1.3 ; 424 ; 1.5 ; 411 ; 5.0 ; 207 ; 1.50 ; 206 ; 2.0", true);
			if (hItemOverride != INVALID_HANDLE)
			{
				hItem = hItemOverride;
				return Plugin_Changed;
...
//	SetEntityRenderColor(client, 255, 255, 255, 255);
	...
	if (weapon > MaxClients && IsValidEdict(weapon))
	{
		...
		switch (index)
		{
			...
			{
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
				weapon = SpawnWeapon(client, "tf_weapon_shotgun_primary", 527, 1, 10, "4 ; 1.30 ; 2 ; 1.10 ; 97 ; 0.4 ; 106 ; 0.5 ; 125 ; 25");
			}
			case 525:
			{
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
...
//			}
			...
			{
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
				weapon = SpawnWeapon(client, "tf_weapon_pipebomblauncher", 207, 5, 10, "280 ; 3 ; 6 ; 0.65 ; 289 ; 2");
			}
			case 23:
			{
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
			...
			case 39, 351, 595, 740:
			{
				...
				{
					TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
					weapon = SpawnWeapon(client, "tf_weapon_flaregun", 740, 5, 10, "25 ; 0.5 ; 207 ; 1.33 ; 144 ; 1.0 ; 58 ; 3.2");
					weapon = SpawnWeapon(client, "tf_weapon_shotgun_pyro", 12, 1, 10, "4 ; 1.33 ; 2 ; 0.66 ; 6 ; 0.80 ; 97 ; 0.5 ; 106 ; 0.5 ; 45 ; 0.6");
					SetAmmo(client, 1, 50);
				}
			}
		}
	...
	{
		weapon = SpawnWeapon(client, "tf_weapon_shotgun_soldier", 10, 1, 10, "4 ; 1.33 ; 6 ; 0.80 ; 97 ; 0.5 ; 106 ; 0.5 ; 45 ; 0.6");
		SetAmmo(client, 1, 125);
	}
	if (IsValidEntity(FindPlayerBack(client, { 405 , 608 }, 2)))
	{
		weapon = SpawnWeapon(client, "tf_weapon_shotgun_primary", 9, 10, 10, "4 ; 1.33 ; 2 ; 0.66 ; 6 ; 0.80 ; 97 ; 0.5 ; 106 ; 0.5 ; 45 ; 0.6");
		SetAmmo(client, 0, 75);
	}
	...
	if (TF2_GetPlayerClass(client) == TFClass_Medic)
	{
		...
		{
			TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
			weapon = SpawnWeapon(client, "tf_weapon_medigun", 663, 5, 10, "10 ; 1.25 ; 178 ; 0.75 ; 200 ; 1");	//200 ; 1 for area of effect healing	// ; 178 ; 0.75 ; 128 ; 1.0 Faster switch-to
...
public Action:Command_MakeNextSpecial(client, args)
{
	...
	if (args < 1)
	{
		ReplyToCommand(client, "[VSH] Usage: hale_special <hale, vagineer, hhh, christian, mario, hitler, kinky>");
		return Plugin_Handled;
	}
...
//			}
			...
			switch (index)
			{
				...
				{
					addthecrit = true;
					if (class == TFClass_Engineer && cond == TFCond_HalloweenCritCandy) cond = TFCond_Buffed;
				}
				case 21, 208, 40, 215, 594, 659, 741:
				{
					addthecrit = true;
	...
	else
	{
		...
		if ((ang[0] < -10.0) && (HaleCharge > 1))
		{
			...
			else if (Special != VSHSpecial_HHH)
			{
				...
				HaleCharge=-120;
				if (Special == VSHSpecial_Hale || Special == VSHSpecial_Vagineer || Special == VSHSpecial_CBS || Special == VSHSpecial_Mario || Special == VSHSpecial_Hitler || Special == VSHSpecial_Kinky)
				{
					decl String:s[PLATFORM_MAX_PATH];
	...
	if (HaleRage/RageDMG >= 1)
	{
		...
		switch (Special)
		{
			...
			case VSHSpecial_HHH:
			{
				...
				Format(s, PLATFORM_MAX_PATH, "%s", HHHTerror);
			...
			case VSHSpecial_CBS:
			{
				...
				EmitSoundToAll(s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, pos, NULL_VECTOR, false, 0.0);
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
				SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SpawnWeapon(client, "tf_weapon_grenadelauncher", 19, 100, 5, "2 ; 2.20 ; 97 ; 0.5 ; 103 ; 1.30 ; 37 ; 0.0"));
			...
			case VSHSpecial_Mario:
			{
				...
				EmitSoundToAll(s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, pos, NULL_VECTOR, false, 0.0);
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
				SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SpawnWeapon(client, "tf_weapon_flaregun", 740, 100, 5, "416 ; 1 ; 6 ; 0.40 ; 25 ; 0.0"));
			...
			case VSHSpecial_Hitler:
			{
				...
				EmitSoundToAll(s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, pos, NULL_VECTOR, false, 0.0);
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
				SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SpawnWeapon(client, "tf_weapon_flamethrower", 40, 100, 5, "2 ; 2.0 ; 37 ; 0.0"));
				SetAmmo(client, 0, ((RedAlivePlayers >= 100) ? 100 : RedAlivePlayers));
				CreateTimer(0.6, UseRage, dist);
			}
			case VSHSpecial_Kinky:
			{
				...
				EmitSoundToAll(s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, pos, NULL_VECTOR, false, 0.0);
				TF2_AddCondition(Hale, TFCond_Buffed, 99.0);
				UberRageCount = 0.0;
...
public Action:event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (client == Hale && VSHRoundState == 1)
	{
		switch (Special)
		{
			case VSHSpecial_HHH:
			{
				Format(s, PLATFORM_MAX_PATH, "hhh/ded%d.wav", GetRandomInt(1, 2));
				EmitSoundToAll(s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, NULL_VECTOR, NULL_VECTOR, false, 0.0);
				EmitSoundToAll("hhh/hit.wav", _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, NULL_VECTOR, NULL_VECTOR, false, 0.0);
	...
	if (attacker == Hale && VSHRoundState == 1)
	{
		...
		if (KSpreeCount == 2 && RedAlivePlayers != 1)
		{
			switch (Special)
			{
				case VSHSpecial_Hale: Format(s, PLATFORM_MAX_PATH, "%s%i.mp3", HaleKSpree, GetRandomInt(1, 7));
				case VSHSpecial_Vagineer:
				{
...
/*				if ( IsValidEdict( weapon ) && GetEdictClassname( weapon, wepclassname, sizeof( wepclassname ) ) && strcmp( wepclassname, "tf_weapon_stickbomb", false ) == 0 )
				...
				if (bIsBackstab)
				{
					...
					{
						decl String:s[PLATFORM_MAX_PATH];
						Format(s, PLATFORM_MAX_PATH, "%s%i.wav", HHHStab, GetRandomInt(1, 3));
						EmitSoundToAll(s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
						EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, SNDCHAN_ITEM, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
					}
					else if (Special == VSHSpecial_Mario)
					...
					{
						decl String:s[PLATFORM_MAX_PATH];
						Format(s, PLATFORM_MAX_PATH, "%s%i.wav", KinkyStab, GetRandomInt(1, 1));
						EmitSoundToAll(s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
						EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, SNDCHAN_ITEM, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
					}
					if (Stabbed < 5)

...
new String:name_armor1[512] = "test";
...
{ 
	...
	g_armor4 = CreateConVar("sm_price_armor4", "50", "What should the price of the 4 armor be ?");
    
	g_armorHP1 = CreateConVar("sm_hp_armor1", "50", "How much HP should have armor 1 ?");
	g_armorHP2 = CreateConVar("sm_hp_armor2", "100", "How much HP should have armor 2 ?");
	g_armorHP3 = CreateConVar("sm_hp_armor3", "200", "How much HP should have armor 3 ?");
	g_armorHP4 = CreateConVar("sm_hp_armor4", "400", "How much HP should have armor 4 ?");
	
	g_name_armor1 = CreateConVar("sm_name_armor1", "Kevlar", "What should be the name of armor 1 ?");
	g_name_armor2 = CreateConVar("sm_name_armor2", "Light Armor", "What should be the name of armor 2 ?");
	g_name_armor3 = CreateConVar("sm_name_armor3", "Medium Armor", "What should be the name of armor 3 ?");
	g_name_armor4 = CreateConVar("sm_name_armor4", "Heavy Armor", "What should be the name of armor 4 ?");
	
	g_killbeforearmor = CreateConVar("sm_kills_before_first_armor", "1", "How much kill(s) should do the player before buying is first armor ?");
	
	g_DamageResistance = CreateConVar("sm_armor_damage_resis", "80", "Amount of damage armor will absorb");
...
public Action:Armor_Menu(client, args)
{
	if (IsValidClient(client))
	{
		if(GetClientTeam(client) != 1 || GetClientTeam(client) != 0)
		{
			...
			SetMenuTitle(menu, "Select your armor type :"); 
...
{         
	if (effect == 1)
	{
		...
		{
			CPrintToChat(client, "%sYou must get %i kills before buying this armor", plugintag, totalkills[client]);
			buyed[client] = 0;
		}
	}
	if (effect == 2)
	...
	if (IsValidClient(victim) && IsValidClient(attacker))
	{
		...
		else
		{
			...
			{ 
				...
				pourcent = GetConVarFloat(g_DamageResistance) / 100;

...
#define CBSModel "models/player/saxton_hale/cbs_v5.mdl"
#define SaxtonHale "saxton_hale/saxton_hale_responce_1a.wav"
#define MarioModel "models/player/supermario/mario_v3.mdl"
#define HitlerModel "models/player/hitler/hitlerv3.mdl"
...
#define HHHModel "models/player/saxton_hale/headless_hatman.mdl"
...
#define MarioJump "supermario/mariojump" //1-2
...
public OnPluginStart()
{
	...
	cvarEnabled = CreateConVar("hale_enabled", "0", "Do you really want set it to 0?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
//	RegConsoleCmd("haleme", SkipHalePanelCmd);
	...
	RegAdminCmd("hale_select", Command_HaleSelect, ADMFLAG_SLAY, "hale_select <target> - Select a player to be next boss");
	RegAdminCmd("hale_special", Command_MakeNextSpecial, ADMFLAG_SLAY, "Call a special to next round.");
	RegAdminCmd("hale_addpoints", Command_Points, ADMFLAG_SLAY, "hale_addpoints <target> <points> - Add queue points to user.");
	RegAdminCmd("hale_point_enable", Command_Point_Enable, ADMFLAG_SLAY, "Enable CP. Only with hale_point_type = 0");
	RegAdminCmd("hale_point_disable", Command_Point_Disable, ADMFLAG_SLAY, "Disable CP. Only with hale_point_type = 0");
	RegAdminCmd("hale_stop_music", Command_StopMusic, ADMFLAG_SLAY, "Stop any currently playing Boss music.");
...
public AddToDownload()
{
	...
	for (i = 0; i < sizeof(extensions); i++)
	{
		...
		if (bSpecials)
		{
			...
			AddFileToDownloadsTable(s);
			Format(s, PLATFORM_MAX_PATH, "models/player/saxton_hale/headless_hatman%s", extensions[i]);
			AddFileToDownloadsTable(s);
			Format(s, PLATFORM_MAX_PATH, "models/player/saxton_hale/cbs_v5%s", extensions[i]);
			AddFileToDownloadsTable(s);
		}
	}
	...
	AddFileToDownloadsTable("materials/freak_fortress_2/headless_hatman/headless_hatman.vmt");
	AddFileToDownloadsTable("materials/freak_fortress_2/headless_hatman/headless_hatman.vtf");
	AddFileToDownloadsTable("materials/freak_fortress_2/headless_hatman/headless_hatman_normal.vtf");
	AddFileToDownloadsTable("materials/freak_fortress_2/headless_hatman/hhh_pumpkin.vmt");
	AddFileToDownloadsTable("materials/freak_fortress_2/headless_hatman/pumkin_hat.vtf");
	AddFileToDownloadsTable("materials/freak_fortress_2/headless_hatman/pumkin_hat_glow.vtf");
	AddFileToDownloadsTable("materials/freak_fortress_2/headless_hatman/pumkin_hat_normal.vtf");
	...
	if (bSpecials)
	{
		...
		PrecacheSound(HitlerSpree, true);
		Format(s, PLATFORM_MAX_PATH, "%s%i.wav", HitlerSpree, i);
		PrecacheSound(HitlerWin, true);
		PrecacheSound(HitlerLast, true);
		PrecacheSound(HitlerStab, true);
		Format(s, PLATFORM_MAX_PATH, "sound/%s", HitlerStab);
		PrecacheSound(HitlerFail, true);
		Format(s, PLATFORM_MAX_PATH, "sound/%s", HitlerFail);
...
public Action:StartResponceTimer(Handle:hTimer)
{
	...
	EmitSoundToAll(s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, pos, NULL_VECTOR, false, 0.0);
	EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, pos, NULL_VECTOR, false, 0.0);
	if (Special == VSHSpecial_CBS || Special == VSHSpecial_Hitler)
	{
...
EquipSaxton(client)
{
	...
	switch (Special)
	{
		...
		case VSHSpecial_HHH:
		{
			SaxtonWeapon = SpawnWeapon(client, "tf_weapon_sword", 266, 100, 5, "68 ; 2.0 ; 2 ; 2.769 ; 259 ; 1.0 ; 252 ; 0.80 ; 402 ; 1");
			SetEntProp(SaxtonWeapon, Prop_Send, "m_iWorldModelIndex", -1);
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
			HaleCharge = -500;
		}
		...
		case VSHSpecial_Mario:
		{
			SaxtonWeapon = SpawnWeapon(client, "tf_weapon_wrench", 5, 100, 5, "68 ; 2.0 ; 2 ; 2.769 ; 259 ; 1.0 ; 150 ; 1.0 ; 326 ; 1.8");
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
		}
		case VSHSpecial_Hitler:
		{
...
//	if (hItem != INVALID_HANDLE) return Plugin_Continue;
	switch (iItemDefinitionIndex)
	{
		case 39, 351:
		{
			new Handle:hItemOverride = PrepareItemHandle(hItem, _, _, "25 ; 0.5 ; 207 ; 1.33 ; 144 ; 1.0 ; 58 ; 2.2", true);
		...
		case 40:
		{
			new Handle:hItemOverride = PrepareItemHandle(hItem, _, _, "165 ; 1.0 ; 125 ; -15");
			if (hItemOverride != INVALID_HANDLE)
			{
				hItem = hItemOverride;
				return Plugin_Changed;
...
//		}
		case 43, 239:
		{
			new Handle:hItemOverride = PrepareItemHandle(hItem, _, 239, "107 ; 2.0 ; 1 ; 0.5 ; 128 ; 1 ; 191 ; -15", true);
			if (hItemOverride != INVALID_HANDLE)
			{
				hItem = hItemOverride;
				return Plugin_Changed;
		...
		case 415:
		{
			new Handle:hItemOverride = PrepareItemHandle(hItem, _, _, "265 ; 99999.0 ; 178 ; 0.6 ; 2 ; 1.33 ; 3 ; 0.5", true);
			if (hItemOverride != INVALID_HANDLE)
			{
				hItem = hItemOverride;
				return Plugin_Changed;
...
//	new Handle:hWeapon = TF2Items_CreateItem(flags);	//INVALID_HANDLE;
	...
	if (attribCount > 0)
	{
		...
		new i2 = 0;
		for (new i = 0; i < attribCount && i < 16; i += 2)
		{
...
//			}
			...
			{
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
				weapon = SpawnWeapon(client, "tf_weapon_handgun_scout_secondary", 36, 5, 10, "280 ; 5 ; 6 ; 0.80 ; 2 ; 0.66 ; 4 ; 4.167 ; 78 ; 2.084 ; 137 ; 6.0");
				SetAmmo(client, 1, 100);
			}
			case 39, 351, 595, 740:
			{
	...
	if (TF2_GetPlayerClass(client) == TFClass_Medic)
	{
		...
		{
			TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
			weapon = SpawnWeapon(client, "tf_weapon_medigun", 663, 5, 10, "10 ; 1.25 ; 178 ; 0.75 ; 200 ; 1");
...
public Action:Command_MakeNextSpecial(client, args)
{
	...
	if (args < 1)
	{
		ReplyToCommand(client, "[VSH] Usage: hale_special <hale, vagineer, hhh, christian, mario, hitler>");
		return Plugin_Handled;
	}
...
public Action:event_player_spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	else if (VSHRoundState >= 0)
	{
		if (!(VSHFlags[client] & VSHFLAG_HASONGIVED))
		{
			...
			RemovePlayerBack(client, { 57, 133, 231, 405, 608, 642 }, 6);
	...
	else
	{
		...
		if ((ang[0] < -10.0) && (HaleCharge > 1))
		{
			...
			else if (Special != VSHSpecial_HHH)
			{
				...
				HaleCharge=-120;
				if (Special == VSHSpecial_Hale || Special == VSHSpecial_Vagineer || Special == VSHSpecial_CBS || Special == VSHSpecial_Mario || Special == VSHSpecial_Hitler)
				{
					decl String:s[PLATFORM_MAX_PATH];
	...
	if (HaleRage/RageDMG >= 1)
	{
		...
		switch (Special)
		{
			...
			case VSHSpecial_CBS:
			{
				...
				EmitSoundToAll(s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, pos, NULL_VECTOR, false, 0.0);
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
				SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SpawnWeapon(client, "tf_weapon_rocketlauncher_directhit", 127, 100, 5, "2 ; 2.0 ; 100 ; 0.3 ; 103 ; 2.0 ; 97 ; 0.5 ; 37 ; 0.0"));
...
public Action:event_hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (custom == TF_CUSTOM_BACKSTAB)
		return Plugin_Continue;
	if (custom == TF_CUSTOM_TELEFRAG) damage = (IsPlayerAlive(attacker) ? 33000 : 1);
		...
		if (attacker <= MaxClients)
		{
			...
			if (inflictor == attacker || inflictor == weapon)
			{
				...
				switch (wepindex)
				{
					...
					{
						if ((wepindex == 14 || wepindex == 201 || wepindex == 664 || wepindex == 752) && VSHRoundState != 2)
						{
					...
					case 528:
					{
						if (circuitStun > 0.0)
						{
							...
							EmitSoundToAll("vo/engineer_no01.wav", client);
...
/*				if ( IsValidEdict( weapon ) && GetEdictClassname( weapon, wepclassname, sizeof( wepclassname ) ) && strcmp( wepclassname, "tf_weapon_stickbomb", false ) == 0 )
				...
				if (bIsBackstab)
				{
					...
					if (GetClientCloakIndex(attacker) == 59)
					{
						...
						SetEntPropFloat(attacker, Prop_Send, "m_flStealthNextChangeTime", GetGameTime() + 0.5);
					}
					...
					{
						EmitSoundToAll("vo/engineer_cheers03.wav", _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
						EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, "vo/engineer_cheers03.wav", _, SNDCHAN_ITEM, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
					}
					else if (Special == VSHSpecial_HHH)
					{
						decl String:s[PLATFORM_MAX_PATH];
...
public SickleClimbWalls(client)		//Credit to Mecha the Slag
{
	...
	SDKHooks_TakeDamage(client, client, client, 25.0, DMG_CLUB, GetPlayerWeaponSlot(client, TFWeaponSlot_Melee));
...
public Action:HookSound(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	if (entity == Hale && Special == VSHSpecial_HHH && strncmp(sample, "hhh", 2, false) == 0 && StrContains(sample, "halloween_boss") == -1)
	{

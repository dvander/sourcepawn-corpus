...
#define KinkyModelPrefix "models/capnkinky/capnkinky"
...
#define KinkyDeath "kinky/kinkydeath"
...
//		PrecacheModel(ReloadEggModel, true);
	...
	AddFileToDownloadsTable("materials/models/capnkinky/soldier.vmt");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier.vtf");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_blu.vmt");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_blu.vtf");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_blu_uber.vmt");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_blu_uber.vtf");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_exponent.vtf");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_head.vmt");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_head.vtf");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_head_blue.vmt");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_head_blue_invun.vmt");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_head_blue_invun.vtf");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_head_red_invun.vmt");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_head_red_invun.vtf");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_normals.vtf");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_officer.vtf");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_officer_blue_uber.vmt");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_officer_blue_uber.vtf");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_uber.vtf");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_red_uber.vmt");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_officer_red_uber.vmt");
	AddFileToDownloadsTable("materials/models/capnkinky/soldier_officer_red_uber.vtf");
	...
	for (i = 1; i <= 5; i++)
	{
		if (i <= 2)
		{
			...
			if (bSpecials)
			{
				...
				Format(s, PLATFORM_MAX_PATH, "sound/%s", s);
				AddFileToDownloadsTable(s);
				Format(s, PLATFORM_MAX_PATH, "%s%i.wav", KinkyDeath, i);
				PrecacheSound(s, true);
				Format(s, PLATFORM_MAX_PATH, "sound/%s", s);
				AddFileToDownloadsTable(s);
			}
...
stock bool:CheckNextSpecial()
{
	...
	while (Incoming == VSHSpecial_None || (Special && Special == Incoming))
	{
		...
		if (Special != VSHSpecial_Hale && !GetRandomInt(0, 6)) Incoming = VSHSpecial_Hale;
		else
		{
			switch (Incoming)
			{
...
#endif
				case 5: Incoming = VSHSpecial_Kinky;
...
public Action:event_round_end(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetEventInt(event, "team") == HaleTeam)
	{
		switch (Special)
		{
			...
			{
				Format(s, PLATFORM_MAX_PATH, "%s%i.wav", KinkyWin, GetRandomInt(1, 5));
				EmitSoundToAll(s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, _, NULL_VECTOR, false, 0.0);
				EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, _, NULL_VECTOR, false, 0.0);
			}
		}
...
public Action:StartResponceTimer(Handle:hTimer)
{
	...
	switch (Special)
	{
		...
		case VSHSpecial_Kinky:	Format(s, PLATFORM_MAX_PATH, "%s%i.wav", KinkyStart, GetRandomInt(1, 7));
		default:
		{
			if (!GetRandomInt(0, 1))
...
EquipSaxton(client)
{
	...
	switch (Special)
	{
		...
		case VSHSpecial_Kinky:
		{
			SaxtonWeapon = SpawnWeapon(client, "tf_weapon_shovel", 447, 69, 5, "68 ; 2.0 ; 2 ; 3.1 ; 259 ; 1.0");
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
		}
		default:
		{
...
public Action:MakeHale(Handle:hTimer)
{
	...
	switch (Special)
	{
		case VSHSpecial_Hale, VSHSpecial_Kinky:
...
public Action:Command_MakeNextSpecial(client, args)
{
	...
	if (args < 1)
	{
		ReplyToCommand(client, "[VSH] Usage: hale_special <hale, vagineer, hhh, christian, kinky>");
		return Plugin_Handled;
	}
	...
	else if (StrContains(arg, "kinky", false) != -1 || StrContains(arg, "capn", false) != -1)
	{
		...
		if ((ang[0] < -45.0) && (HaleCharge > 1))
		{
			...
			else if (Special != VSHSpecial_HHH)
			{
				...
				switch (Special)
				{
					...
					case VSHSpecial_Kinky:
						strcopy(s, PLATFORM_MAX_PATH, "kinky/spree4.wav");
					case VSHSpecial_Hale:
					{
...
//				CreateTimer(0.1, Timer_ChangeRagdoll, any:GetEventInt(event, "userid"));
			...
			case VSHSpecial_Kinky:
			{
				Format(s, PLATFORM_MAX_PATH, "%s%i.wav", KinkyDeath, GetRandomInt(1, 2));
				EmitSoundToAll(s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, client, NULL_VECTOR, NULL_VECTOR, false, 0.0);
				EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, SNDCHAN_ITEM, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, client, NULL_VECTOR, NULL_VECTOR, false, 0.0);
			}
		}
...
//				CreateTimer(0.1, Timer_DissolveRagdoll, any:GetEventInt(event, "userid"));
		...
		if (KSpreeCount == 3 && RedAlivePlayers != 1)
		{
			switch (Special)
			{
				...
				case VSHSpecial_Kinky:
				{
					Format(s, PLATFORM_MAX_PATH, "%s%i.wav", KinkySpree, GetRandomInt(1, 6));
					EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, NULL_VECTOR, NULL_VECTOR, false, 0.0);
				}
			}
			EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
...
/*				if ( IsValidEdict( weapon ) && GetEdictClassname( weapon, wepclassname, sizeof( wepclassname ) ) && strcmp( wepclassname, "tf_weapon_stickbomb", false ) == 0 )
				...
				if (bIsBackstab)
				{
					...
					switch (Special)
					{
						...
						case VSHSpecial_Kinky:
						{
							Format(s, PLATFORM_MAX_PATH, "%s%i.wav", KinkyStab, GetRandomInt(1, 3));
							EmitSoundToAll(s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
							EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, SNDCHAN_ITEM, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
						}
					}

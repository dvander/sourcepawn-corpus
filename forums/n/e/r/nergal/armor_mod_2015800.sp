...
{ 
	...
	DamageResistance = CreateConVar("sm_armor_damage_resis", "0.3", "Amount of damage armor will absorb");
	g_killbeforearmor = CreateConVar("sm_kills_before_first_armor", "0", "How much kill(s) should do the player before buying his/her first armor ?");
	
	AutoExecConfig(true, "armorpackV5");
...
{         
	if (effect == 1)
	{
		...
		{
			CPrintToChat(client, "%sYou need to get %i kills before buying that armor!", plugintag, totalkills[client]);
			buyed[client] = 0;
		}
	}
	if (effect == 2)
...
{     
	if(IsValidClient(victim) && IsValidClient(attacker))
	{
		...
		{
			//save = how much damage the armor absorbs, take = remaining damage applied to player who was attacked
			new damage = GetEventInt(event, "damageamount");
			intdamage = RoundFloat(damage);
			new Float:armortype = GetConVarFloat(DamageResistance);
			new save;
			new take;
			save = RoundFloat(intdamage * armortype);
			...
			if (armor[victim] >= 1)
			{
				...
				if(armor[victim] <= -1)
				{
					...
					{
						PrintHintText(victim, "Your armor is done!");
					}
					else
					{
				...
				if (victim != attacker)
				{
					if(armor[victim] >= 1)
					{
						...
						{
						PrintHintText(attacker, "This player has armor, HP of armor : %d", armor[victim]);
						}
					}
					else
					{

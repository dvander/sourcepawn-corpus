...
new Handle:cvSaxton = INVALID_HANDLE;
...
public OnPluginStart()
{
    ...
    cvSaxton = CreateConVar("sm_tf2nades_blue", "1", "Disable grenades for blue team in Saxton Hale mod", FCVAR_PLUGIN|FCVAR_NOTIFY);
    cvRed = CreateConVar("sm_tf2nades_red", "0", "disables nades for red team", FCVAR_PLUGIN|FCVAR_NOTIFY);
...
public EntityOutput_OnPlayerTouch(const String:output[], caller, activator, Float:delay)
{
	if (IsValidEntity(caller))
    {
        ...
        if (StrEqual(modelname, "models/items/ammopack_large_bday.mdl"))
		{
			new Float:pos[3];
    ...
    if (IsValidEntity(caller))
    {
        decl String:modelname[128]; modelname[0] = '\0';
        GetEntPropString(caller, Prop_Data, "m_ModelName", modelname, 128);
        if (StrEqual(modelname, "models/items/ammopack_small_bday.mdl"))
		{
			new Float:pos[3];
			GetEntPropVector(caller, Prop_Send, "m_vecOrigin", pos);
			FindPlayersInRange(pos, 64.0, 0, -1, false, -1);
    ...
    if (IsValidEntity(caller))
    {
        decl String:modelname[128]; modelname[0] = '\0';
        GetEntPropString(caller, Prop_Data, "m_ModelName", modelname, 128);
        if (StrEqual(modelname, "models/items/ammopack_small.mdl"))
		{
			new Float:pos[3];
			GetEntPropVector(caller, Prop_Send, "m_vecOrigin", pos);
			FindPlayersInRange(pos, 64.0, 0, -1, false, -1);
...
public Action:Command_Nade1(client, args) 
{
    ...
    if(GetConVarInt(cvSaxton)==1 && (GetClientTeam(client)==3))
    {
            return Plugin_Handled;

    }
    if(GetConVarInt(cvRed)==1 && (GetClientTeam(client)==2))
    {
            return Plugin_Handled;
...
SetupNade(NadeType:type, team, bool:special)
{
    ...
    if (!special || type >= FragNade)
    {
        ...
        if (team==2)
		{
			strcopy(gnParticle, sizeof(gnParticle), "scorchshot_trail_crit_red");
		}
		else
		{
			strcopy(gnParticle, sizeof(gnParticle), "scorchshot_trail_crit_blue");
		}
        return;
    }
    else

...
#define MarioModel "models/player/supermario/mario_v4.mdl"
#define MarioModelPrefix "models/player/supermario/mario_v4"
#define HitlerModel "models/player/hitler/hitlerv4.mdl"
#define HitlerModelPrefix "models/player/hitler/hitlerv4"
...
#define HitlerTheme "hitler/hitlertheme.mp3"
...
public AddToDownload()
{
	...
	if (bSpecials)
	{
		...
		PrecacheSound(HitlerTheme, true);
	...
	PrecacheSound("hhh/stab01.wav", true);
	PrecacheSound("hhh/stab02.wav", true);
	PrecacheSound("hhh/stab03.wav", true);
...
public Action:event_round_end(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetEventInt(event, "team") == HaleTeam)
	{
		switch (Special)
		{
			...
			{
				Format(s, PLATFORM_MAX_PATH, "%s%i.wav", KinkyWin, GetRandomInt(1, 2));
				EmitSoundToAll(s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, _, NULL_VECTOR, false, 0.0);
				EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, _, NULL_VECTOR, false, 0.0);
			}
		}
...
public Action:StartResponceTimer(Handle:hTimer)
{
	...
	switch (Special)
	{
		...
		case VSHSpecial_Kinky: Format(s, PLATFORM_MAX_PATH, "%s%i.wav", KinkyStart, GetRandomInt(1, 3));
		case VSHSpecial_Hale:
		{
			if (!GetRandomInt(0, 1))
...
public Action:Timer_MusicPlay(Handle:timer)
{
	...
	else
	{
		switch (Special)
		{
			...
			case VSHSpecial_Hitler:
			{
				strcopy(sound, sizeof(sound), HitlerTheme);
				time = 260.1;
			}
			case VSHSpecial_Kinky:
			{
...
EquipSaxton(client)
{
	...
	switch (Special)
	{
		...
		case VSHSpecial_HHH:
		{
			SaxtonWeapon = SpawnWeapon(client, "tf_weapon_sword", 266, 100, 5, "68 ; 2.0 ; 2 ; 2.769 ; 259 ; 1.0 ; 252 ; 0.80 ; 402 ; 1"); //402
			SetEntProp(SaxtonWeapon, Prop_Send, "m_iWorldModelIndex", -1);
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
			HaleCharge = -500;
		}
...
//			}
	...
	{
		weapon = SpawnWeapon(client, "tf_weapon_smg", 751, 5, 10, "5 ; 1.25 ; 2 ; 1.5 ; 78 ; 2.40 ; 156 ; 1 ; 96 ; 3.5");
		SetAmmo(client, 1, 125);
	}
...
stock StopHaleMusic(client)
{
	...
	StopSound(client, SNDCHAN_AUTO, HitlerTheme);
	...
	else if (RedAlivePlayers == 1 && IsValidClient(Hale) && VSHRoundState == 1)
	{
		...
		if (Special != VSHSpecial_HHH)
		{
			if (Special == VSHSpecial_CBS)
			{
				...
				Format(s, PLATFORM_MAX_PATH, "%s%i.wav", KinkyLast, GetRandomInt(1, 2));
			else
			{
...
/*				if ( IsValidEdict( weapon ) && GetEdictClassname( weapon, wepclassname, sizeof( wepclassname ) ) && strcmp( wepclassname, "tf_weapon_stickbomb", false ) == 0 )
				...
				if (bIsBackstab)
				{
					...
					{
						decl String:s[PLATFORM_MAX_PATH];
						Format(s, PLATFORM_MAX_PATH, "hhh/stab0%s%i.wav", GetRandomInt(1, 3));
						EmitSoundToAll(s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
						EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, SNDCHAN_ITEM, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
					}
					else if (Special == VSHSpecial_Mario)
					...
					{
						EmitSoundToAll("kinky/lastguy1.wav", _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
						EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, "kinky/lastguy1.wav", _, SNDCHAN_ITEM, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, NULL_VECTOR, NULL_VECTOR, false, 0.0);
					}
					if (Stabbed < 5)
						Stabbed++;
					new healers[MAXPLAYERS];

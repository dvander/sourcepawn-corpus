...
#define MSGTAG " \x01[\x0BRoG Tournament\x01]\x08"
//////////////////////////////////////////////////////////
//ConVars
...
public Plugin myinfo = {
	name = "[RoG Tournament]",
	author = "noBrain, NewEdit_",
	description = "[RoG Tournament] - Retake (2v2)",
...
public void OnPluginStart()
{
	...
	//ConVars
	i_nMaxReady = CreateConVar("sm_max_ready_num", "4", "Pocet lidi pro zapnuti zapasu!");
	i_nReadySys = CreateConVar("sm_use_ready_sys", "1", "System ready!");
	//Extra
	LoadTranslations("tournament.phrases.txt"); 
	if (!DirExists("cfg/torunament"))
	{
		CreateDirectory("cfg/tournament", 511);
	}
}
...
public void OnMapStart()
{
	...
	CMapFile(h_iMapName);
	PrintToServer("%s Map %s nastaveni vygenerovano!", MSGTAG, h_iMapName);
	IsMatchLive = false;
}
...
public Action Timer_SetupSpawn(Handle timer, any client)
{
	...
	ServerCommand("tv_msg \"%s Retake %s je na.\"", MSGTAG, h_iSideString);
	if(HasBomb(client))
	{
...
public Action RoundStart(Handle event, const char[] name, bool dontBroadcast) 
{
	...
	{
		PrintToChatAll("%s Zapas failed!", MSGTAG);
		IsMatchLive = false;
		IsMatchSetup = false;
		i_hReadyNum = 0;
		...
		SetConVarInt(g_iStartMoney, 60000);
		ServerCommand("mp_warmup_start");
		PrintToChatAll("%s Zapas byl pozastaven!", MSGTAG);
		return;
	}
	if(!IsMatchSetup)
	{
...
public Action Timer_ReadyMessage(Handle iTimer)
{
	...
	for(int i = 1;i <= MaxClients; i++)
	{
		...
		{
			PrintHintText(i, "Napis <font color='#42d7f4'>!ready</font> pokud jsi pripraven/a! \n Aktualne <font color='#bbf441'>%d/%d</font> je pripraveno k zapasu!", i_hReadyNum, ReadyNumb);
		}
		else if(IsValidClient(i) && IsClientReady[i])
		{
			PrintHintText(i, "Jsi <font color='#cc0000'>pripraven/a</font> k zapasu! \n Aktualne <font color='#bbf441'>%d/%d</font> je pripraveno k zapasu!", i_hReadyNum, ReadyNumb);
		}
	}
...
public Action Command_AddBombSpawn(int client, int args)
{
	...
	Format(StrPath, sizeof(StrPath), "cfg/tournament/%s.cfg", h_iMapName);
...
{
	char StrPath[128];
	Format(StrPath, sizeof(StrPath), "cfg/tournament/%s.cfg", StrMapName);
	...
	if(KvJumpToKey(kv, StrSide, false))
	{
		...
		if(i_hTeam == 2)
		{
			if(KvJumpToKey(kv, "T", false))
			{
				...
				{
					PrintToServer("&s Spawn nebyl nalezen!.", MSGTAG);
					return false;
				}
			}
		}
...
public Action Timer_RestartMatch(Handle timer)
{
	...
	char CfgPath[64] = "tournament/tournament.cfg";
	ServerCommand("exec %s", CfgPath);
	PrintToChatAll("%s Zapas je live!", MSGTAG);
	IsMatchLive = true;
}
...
void DisplayGunsMenu(int client)
{
	...
	BuildPath(Path_SM, StrszPath, sizeof(StrszPath), "configs/zbrane.ini");
	Handle menu = CreateMenu(Guns);
	Handle kv = CreateKeyValues("guns");
	SetMenuTitle(menu, "Vyber zbrani:");

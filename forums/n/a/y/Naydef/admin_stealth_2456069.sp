...
//To-do: Merge this admin stealth version in the main plugin and clean the awful defines!!!
#define JOIN_MESSAGE "Player %N has joined the game"
#define QUIT_MESSAGE "Player %N left the game (Disconnect by user.)"
#define JOIN_MESSAGE_CSGO " \x01\x0BPlayer \x05%N\x01 connected from \x04%s\x01"
#define QUIT_MESSAGE_CSGO " \x01\x0BPlayer \x05%N\x01 has left the game. (Disconnect)"
...
bool:PrintConDisMessg(client, bool:connect)
{
	...
	if(connect)
	{
		...
		{
			new String:buffer[45]; // Let the buffer be bigger than enough in case of strange things in this world!
			GetClientIP(client, buffer, sizeof(buffer), true);
			GeoipCountry(buffer, buffer, sizeof(buffer)); // Don't allocate more variables!
			PrintToChatAll(JOIN_MESSAGE_CSGO, client, buffer);
		}
	}
	else
	{

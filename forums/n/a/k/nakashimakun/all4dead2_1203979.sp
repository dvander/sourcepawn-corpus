...
public Action:Menu_CreateWeaponMenu(client, args) {
	...
	AddMenuItem(menu, "s7", "Spawn a SG552");
	AddMenuItem(menu, "s8", "Spawn a MP5");
	AddMenuItem(menu, "s6", "Spawn a AWP");
	AddMenuItem(menu, "s6", "Spawn a Scout");
	DisplayMenu(menu, client, MENU_DISPLAY_TIME);
	return Plugin_Handled;
}
...
public Action:Menu_CreateMeleeWeaponMenu(client, args) {
	...
	AddMenuItem(menu, "ms", "Spawn a riot shield");
	...
	if (action == MenuAction_Select) {
		switch (itempos) {
			case 0: {
				...
				Do_SpawnItem(cindex, "hunting_knife");
			} case 11: {
				Do_SpawnItem(cindex, "golfclub");
			} case 12: {
				Do_SpawnItem(cindex, "riotshield");
			}
		}

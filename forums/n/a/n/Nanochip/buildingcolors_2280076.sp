...
public OnPluginStart()
{
	...
	hCookieColor = RegClientCookie("buildingcolor", "Which building color?", CookieAccess_Private);
...
{
	decl String:strCookieColor[16];
	GetClientCookie(client, hCookieColor, strCookieColor, sizeof(strCookieColor));
	if (StrEqual(strCookieColor, "0")) gColor[client] = 0;
	else if (StrEqual(strCookieColor, "1")) gColor[client] = 1;
	else if (StrEqual(strCookieColor, "2")) gColor[client] = 2;
	else if (StrEqual(strCookieColor, "3")) gColor[client] = 3;
	else if (StrEqual(strCookieColor, "4")) gColor[client] = 4;
	else if (StrEqual(strCookieColor, "5")) gColor[client] = 5;
	else if (StrEqual(strCookieColor, "6")) gColor[client] = 6;
	else if (StrEqual(strCookieColor, "7")) gColor[client] = 7;
	else gColor[client] = 0;
}
...
public ColorMenuCallback(Handle:menu, MenuAction:action, client, param2)
{
	...
	if(action == MenuAction_Select)
	{
		...
		SetClientCookie(client, hCookieColor, info);

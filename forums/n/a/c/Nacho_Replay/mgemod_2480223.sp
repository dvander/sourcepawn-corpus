...
new lastUsedStamp[MAXPLAYERS+1];
...
public OnClientPostAdminCheck(client)
{
	if (client)
	{
		if (IsFakeClient(client))
		{
			...
			if (!g_bNoStats)
			{
				decl String:steamid_dirty[32], String:steamid[64], String:query[256];
...
ResetPlayer(client)
{
	...
	if(!IsPlayerAlive(client) || g_bArenaBBall[arena_index])
	{
		...
		CreateTimer(0.5, RipSpect,GetClientUserId(client));
	} else {
...
ShowTop5Menu(client, char[][] name, rating[])
{
	if (client<=0)
		return;
...
{ //handle commands "!ammomod" "!add" and such //building queue's menu and listing arena's
	...
	if (GetTime() - lastUsedStamp[client] < 5) {
		PrintToChat(client, "You must wait 5 seconds between attempts!");
...
stock int ForceShotgun(int client)
{
	if (IsValidClient(client))
	{
		Handle hWeapon = TF2Items_CreateItem(OVERRIDE_ALL|FORCE_GENERATION);
		TF2Items_SetClassname(hWeapon, "tf_weapon_shotgun");
		TF2Items_SetItemIndex(hWeapon, 1141);
		TF2Items_SetLevel(hWeapon, 100);
		TF2Items_SetQuality(hWeapon, 6);

		int Sshot = TF2Items_GiveNamedItem(client, hWeapon);
		CloseHandle(hWeapon);
		EquipPlayerWeapon(client, Sshot);
	}
}
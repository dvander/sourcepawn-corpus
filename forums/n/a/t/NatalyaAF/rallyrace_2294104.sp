...
#define  RALLYRACE_VERSION			"N-GO 0.49 Debug"
...
public OnPluginStart()
{
	...
	PrecacheSound("vehicles/mustang_horn.mp3", false);
	PrecacheSound("vehicles/police_siren_single.mp3", false);
...
public Action:RaceTimerFunc(Handle:timer)
{
	switch(g_Game_Stats)
	{
		...
		case READY:
		{
			...
			{
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 00");
				LogMessage("[RR Debug] 00");
				new maxClients = GetMaxClients();
				new total = 0;
				new clients[maxClients];
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 01");
				LogMessage("[RR Debug] 01");
				for (new i=1; i<=maxClients; i++)
				{
				...
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 02");
				LogMessage("[RR Debug] 02");
				...
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 03");
				LogMessage("[RR Debug] 03");
				g_Player_Race_Finish = 0;
				g_Player_Race_Count = 0;
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 04");
				LogMessage("[RR Debug] 04");
				// PrintCenterTextAll("");
				g_Game_Stats = RACING;
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 05");
				LogMessage("[RR Debug] 05");
				g_Game_CountDown = GetConVarInt(h_rallyrace_racetime) + 4;
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 06");
				LogMessage("[RR Debug] 06");
				ResetCheckPointsPass();
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 07");
				LogMessage("[RR Debug] 07");
				SpawnCarsToRace(clients,total);
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 08");
				LogMessage("[RR Debug] 08");
				new use2 = 0;
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 09");
				LogMessage("[RR Debug] 09");
				use2 = GetConVarInt(h_rallyrace_use);
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 10");
				LogMessage("[RR Debug] 10");
				if (use2 != 1)
				{
					PrintToChatAll("\x04 [Rally] Pressing 'e' or use to exit a car has been disabled for this race.");
				}
				PrintToChatAll("\x04 [RR Debug] - READY - 0 - 11");
				LogMessage("[RR Debug] 11");
			}
		}
		case RACING:
		{
...
public SpawnCarsToRace(clients[], total)
{
	...
	LogMessage("[RR Debug] 07 B");
	for(i = 1; i <= maxplayers; i++)
	{
		if (player_ready[i] == 1)
		{
			LogMessage("[RR Debug] 07 C %i", i);
			...
			LogMessage("[RR Debug] 07 C %i A", i);
			new car = CreateJeep(i, vec, ang);
			LogMessage("[RR Debug] 07 C %i B", i);
...
//				TeleportEntity(i, vec, NULL_VECTOR, NULL_VECTOR);
			...
			LogMessage("[RR Debug] 07 C %i C - Car Spawned Successfully for client %i", i, i);
		}
	}
}
...
{
	LogMessage("[RR Debug] 07 C %i A - A", client);
	new t = selected_car_type[client];
	new String:skin[4];
	Format(skin, sizeof(skin), "%i", selected_car_skin[client]);
	LogMessage("[RR Debug] 07 C %i A - B T= %i and S = %s", client, t, skin);
	new ent  = CreateEntityByName("prop_vehicle_driveable");
	LogMessage("[RR Debug] 07 C %i A - C Car Index = %i", client, ent);
	if(IsValidEntity(ent))
	{
		LogMessage("[RR Debug] 07 C %i A - D Car %i is valid.", client, ent);
		...
		LogMessage("[RR Debug] 07 C %i A - E Car %i name: %s", client, ent, Car_Name);
		AcceptEntityInput(ent, "Unlock", 0);
		LogMessage("[RR Debug] 07 C %i A - F Car %i Unlocked", client, ent);
		if (t == 0)
		{
			t = GetRandomCar();
			
			IntToString(GetRandomInt(g_Car_skin_min[t], g_Car_skin_max[t]), skin,sizeof(skin));
			LogMessage("[RR Debug] 07 C %i A - F Car %i Random Type", client, ent);
			DispatchKeyValue(ent, "vehiclescript", car_script[t]);
			LogMessage("[RR Debug] 07 C %i A - F Car %i got %s", client, ent, car_script[t]);
			DispatchKeyValue(ent, "model", car_model[t]);
			LogMessage("[RR Debug] 07 C %i A - F Car %i set to %s", client, ent, car_model[t]);
			DispatchKeyValueFloat (ent, "MaxPitch", 360.00);
			DispatchKeyValueFloat (ent, "MinPitch", -360.00);
			DispatchKeyValueFloat (ent, "MaxYaw", 90.00);
			LogMessage("[RR Debug] 07 C %i A - F Car %i PYR Okay!", client, ent);
			DispatchKeyValue(ent, "targetname", Car_Name);
			LogMessage("[RR Debug] 07 C %i A - F Car %i named %s", client, ent, Car_Name);
			...
			LogMessage("[RR Debug] 07 C %i A - F Car %i Almost There", client, ent);
			DispatchKeyValue(ent, "spawnflags", "256" );
			DispatchKeyValue(ent, "setbodygroup", "511" );
			LogMessage("[RR Debug] 07 C %i A - F Car %i SF and BG Set", client, ent);
			SetEntProp(ent, Prop_Send, "m_nSolidType", 2);
			LogMessage("[RR Debug] 07 C %i A - F Car %i Solid Type 2", client, ent);
			DispatchSpawn(ent);
			LogMessage("[RR Debug] 07 C %i A - F Car %i Spawned", client, ent);
			ActivateEntity(ent);
			LogMessage("[RR Debug] 07 C %i A - F Car %i Activated", client, ent);
		}
		if (t > 0)
		{
			LogMessage("[RR Debug] 07 C %i A - F Car %i Type is %i", client, ent, t);
			DispatchKeyValue(ent, "vehiclescript", car_script[t]);
			LogMessage("[RR Debug] 07 C %i A - F Car %i got %s", client, ent, car_script[t]);
			DispatchKeyValue(ent, "model", car_model[t]);
			LogMessage("[RR Debug] 07 C %i A - F Car %i set to %s", client, ent, car_model[t]);
		...
		LogMessage("[RR Debug] 07 C %i A - J Car %i Getting Lights Now", client, ent);

		if ((car_lights[t] == 1) || (car_lights[t] == 2))
		{
			LogMessage("[RR Debug] 07 C %i A - K Car %i Lights = %i", client, ent, car_lights[t]);
...
/*
	new car = GetEntPropEnt(client, Prop_Send, "m_hVehicle");
	PrintToChatAll("car = %i", car);
	if (car != -1)
	{
		// sometimes the vehicle won't start or think so keep trying as long as theres a driver
		AcceptEntityInput(car, "TurnOn");
		SetEntProp(car, Prop_Data, "m_nNextThinkTick", 1);
		SetEntPropFloat(car, Prop_Data, "m_flTurnOffKeepUpright", 1.0);
	} */
}
	...
	if (IsPlayerAlive(client))
	{
		...
		if(car != -1)
		{
			if (!GetEntProp(car, Prop_Data, "m_bLocked"))
			{
				...
				EmitSoundToAll("doors/default_locked.wav", client, SNDCHAN_AUTO, SNDLEVEL_NORMAL);
			}
			else
			{
				AcceptEntityInput(car, "Unlock", client);
				EmitSoundToAll("doors/latchunlocked1.wav", client, SNDCHAN_AUTO, SNDLEVEL_NORMAL);
			}
		}

...
{
    if (EntRefToEntIndex(gObj[client]) > 0)
    ...
    if (g_EnableBits) 
    {
        ...
        for (new i=1;i<=MaxClients;i++) 
        {
                ...
                IsPlayerAlive(i) && TF2_GetPlayerClass(i)==TFClass_Engineer) 
            {
                ...
                else if((g_EnableBits & (ENABLE_ALT_WRENCH | ENABLE_RELOAD_WRENCH)) &&
                        StrEqual(wpn, "tf_weapon_wrench")) 
                {
                    buttons = GetEntProp(i, Prop_Data, "m_nButtons", buttons);
                    if (g_EnableBits & ENABLE_ALT_WRENCH)
                    {
                        if (buttons & IN_ATTACK2)
                        {
                            if(!g_EngiButtonDown[i]) 
                    ...
                    if (g_EnableBits & ENABLE_RELOAD_WRENCH)
                    {
...
{
    // Skip feigned deaths.
    if (GameType == tf2 && (GetEventInt(event, "death_flags") & DF_FEIGNDEATH))
        return Plugin_Continue;
...
public Action:Command_Rotate(client, args)
{
    ...
    else if (EntRefToEntIndex(gObj[client]) > 0)
    {
    ...
    if (grab < none)
    {
        ...
        if (res == Plugin_Continue)
        {
            ...
            if (grab >= dispenser && grab <= teleporter_exit)
            {
                ...
                {
                    KillTimer(gTrackTimers[ent], true);
...
public Action:Command_Throw(client, args)
{
    ...
    else if (EntRefToEntIndex(gObj[client]) < 1)
...
public Action:TrackObject(Handle:timer, Handle:pack)
{
    ...
    if (ent > 0 && IsValidEntity(ent) && IsValidEdict(ent))
    {
        ...
        LogError("Entity reference %d references invalid entity %d", ref, ent);

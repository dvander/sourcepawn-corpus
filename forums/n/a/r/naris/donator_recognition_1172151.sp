...
public OnPluginStart()
{
	...
	if (GetGameType() == tf2)
	{
		...
		HookEventEx("arena_round_start", hook_Setup, EventHookMode_PostNoCopy);
		...
		HookEventEx("teamplay_round_win", hook_End, EventHookMode_Post);
		HookEventEx("arena_win_panel", hook_End, EventHookMode_Post);
	}
	else if (GameType == dod)
	{
		...
		HookEventEx("dod_round_win", hook_End, EventHookMode_Post);
	}
	else if (GameType == cstrike)
	{
		...
		HookEventEx("round_end", hook_End, EventHookMode_Post);
	}
...
public hook_Active(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new value = timer > 0 ? GetEntProp(timer, Prop_Send, "m_nState") : -1;
	if (timer <= 0 || value > 0)
		hook_Setup(event, name, dontBroadcast);
}

public hook_End(Handle:event, const String:name[], bool:dontBroadcast)
{	
	for(new i = 1; i <= MaxClients; i++)
	{
		if (!IsClientInGame(i) || !IsPlayerAlive(i) || IsClientObserver(i)) continue;

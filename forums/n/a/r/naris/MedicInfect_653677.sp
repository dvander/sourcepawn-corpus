...
new Handle:OnInfectionDamageHandle = INVALID_HANDLE;
...
public bool:AskPluginLoad(Handle:myself,bool:late,String:error[],err_max)
{
    ...
    CreateNative("HookInfectionDamage",Native_HookInfectionDamage);
    ...
    OnInfectionDamageHandle=CreateForward(ET_Hook,Param_Cell,Param_Cell,Param_CellByRef);
...
public OnConfigsExecuted()
{
    ...
    if (timeval > 0.0)
        MediTimer = CreateTimer(timeval, HandleMediInfection, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
    else
        MediTimer = INVALID_HANDLE;

    timeval = GetConVarFloat(Cvar_SpreadCheckTime);
    if (timeval > 0.0)
        SpreadTimer = CreateTimer(timeval, HandleSpreadInfection, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
    else
...
public Action:HandleInfection(Handle:timer)
{
    ...
    for(new client = 1; client <= maxplayers; client++)
    {
        ...
        if (NativeDamageHooked)
        {
            ...
            Call_StartForward(OnInfectionDamageHandle);
        ...
        if(hp <= 0)
        {
            EmitSoundToAll(DeathSound[GetRandomInt(0,sizeof(DeathSound)-1)], client);
        ...
        else
        {
            ...
            EmitSoundToAll(HurtSound[GetRandomInt(0,sizeof(HurtSound)-1)], client);
...
public HandleMediChange(Handle:convar, const String:oldValue[], const String:newValue[])
{
    ...
    if(val > 0.0)
        MediTimer = CreateTimer(val, HandleMediInfection, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
    else
        MediTimer = INVALID_HANDLE;
}
...
public HandleSpreadChange(Handle:convar, const String:oldValue[], const String:newValue[])
{
    ...
    new Float:val = StringToFloat(newValue);
    if(val > 0.0)
        SpreadTimer = CreateTimer(val, HandleSpreadInfection, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
    else
        SpreadTimer = INVALID_HANDLE;
}
...
public Native_HookInfectionDamage(Handle:plugin,numParams)
{
    if(numParams >= 1)
    {
        AddToForward(OnInfectionDamageHandle, plugin, Function:GetNativeCell(1));

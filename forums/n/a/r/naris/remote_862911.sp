...
new bool:clientPermissions[MAXPLAYERS+1];
...
{		
    ...
    cvarSpeed = CreateConVar("sm_remote_speed", "200.0", "Speed at which remote sentries move.");
    cvarJump = CreateConVar("sm_remote_jump", "2000.0", "Speed at which remote sentries jump.");
    ...
    if (type == unknown)
    {
        ...
        else
        {
            ...
            if (permissions & CAN_BUILD)
            {
                ...
                AddMenuItem(menu,"0","Dispenser", ITEMDRAW_DEFAULT);
                AddMenuItem(menu,"1","Teleporter Entry", ITEMDRAW_DEFAULT);
                AddMenuItem(menu,"2","Teleporter Exit", ITEMDRAW_DEFAULT);
                AddMenuItem(menu,"3","Level 1 Sentry Gun", ITEMDRAW_DEFAULT);
                AddMenuItem(menu,"4","Level 2 Sentry Gun", ITEMDRAW_DEFAULT);
                AddMenuItem(menu,"5","Level 3 Sentry Gun", ITEMDRAW_DEFAULT);
                DisplayMenu(menu,client,MENU_TIME_FOREVER);
            }
            else

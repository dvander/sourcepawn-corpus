...
static const String:soundNames[NUM_SOUNDS][] = {"headshot", "grenade", "rocket", "critrocket", "selfkill", "train",
...
public OnClientPutInServer(client)
{
	...
	{
		CheckSound(eventSoundsList[JOINSERVER]);
...
public PrepareEventSounds()
{
	...
	for(new sound = 0; sound < NUM_EVENT_SOUNDS; sound++) {
		if(!StrEqual(eventSoundsList[sound], "")) {
			//Don't precache sounds until they are actuaslly used.
...
public PlayQuakeSound(soundKey, attackerClient, assisterClient, victimClient)
{
	...
	if(settingsArray[soundKey] & 1) {
		for (new i = 1; i < playersConnected; i++) {
			if(IsClientInGame(i) && !IsFakeClient(i) && soundPreference[i] && !StrEqual(soundsList[soundPreference[i]-1][soundKey], "")) {
				CheckSound(soundsList[soundPreference[i]-1][soundKey]);
	...
	if(attackerClient && IsClientInGame(attackerClient) && !IsFakeClient(attackerClient)) {
		if(soundPreference[attackerClient] && (settingsArray[soundKey] & 2) && !StrEqual(soundsList[soundPreference[attackerClient]-1][soundKey], "")) {
			CheckSound(soundsList[soundPreference[attackerClient]-1][soundKey]);
	...
	if(assisterClient && IsClientInGame(assisterClient) && !IsFakeClient(assisterClient)) {
		if(soundPreference[assisterClient] && (settingsArray[soundKey] & 2) && !StrEqual(soundsList[soundPreference[assisterClient]-1][soundKey], "")) {
			CheckSound(soundsList[soundPreference[assisterClient]-1][soundKey]);
	...
	if(victimClient && IsClientInGame(victimClient) && !IsFakeClient(victimClient)) {
		if(soundPreference[victimClient] && (settingsArray[soundKey] & 4) && !StrEqual(soundsList[soundPreference[victimClient]-1][soundKey], "")) {
			CheckSound(soundsList[soundPreference[victimClient]-1][soundKey]);
...
public PrepareSound(sound)
{
	...
	for(new set = 0; set < numSets; set++) {
		if(!StrEqual(soundsList[set][sound], "")) {
			// Don't preache sounds until they are actually used
...
stock CheckSound(const String:sound[], bool:preload=false)
{
...
public IsRocket(const String:weapon[])
{
	...
	if(StrEqual(weapon, "bazookarocket") || StrEqual(weapon, "panzerschreckrocket"))
		return 1;

	// TF2 rockets
	if(StrEqual(weapon, "tf_projectile_rocket"))
		return 1;

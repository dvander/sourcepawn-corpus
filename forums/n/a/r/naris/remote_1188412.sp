...
#define REMOTE_CAN_STEAL		    (1 << 1)
#define REMOTE_CAN_BUILD     		(1 << 2)
#define REMOTE_CAN_BUILD_LEVEL_2    (1 << 3)
#define REMOTE_CAN_BUILD_LEVEL_3    (1 << 4)
#define REMOTE_CAN_BUILD_INSTANTLY	(1 << 5)
#define REMOTE_CAN_BUILD_FLOATING	(1 << 6)
#define REMOTE_CAN_ZOMBIE		    (1 << 7)
...
new TFObjectType:g_RemoteType[MAXPLAYERS+1];
...
new Handle:g_hLimits[4][TFObjectType];
new g_isAllowed[MAXPLAYERS+1][TFObjectType]; // how many buildings each player is allowed
...
{		
    ...
    g_hLimits[2][1] = CreateConVar("sm_buildlimit_red_teleporters", "1", "Limit for Red teleporter pairs in TF2.");
    g_hLimits[2][2] = CreateConVar("sm_buildlimit_red_sentries",    "1", "Limit for Red sentries in TF2.");
    g_hLimits[3][0] = CreateConVar("sm_buildlimit_blu_dispensers",  "1", "Limit for Blu dispensers in TF2.");
    g_hLimits[3][1] = CreateConVar("sm_buildlimit_blu_teleporters", "1", "Limit for Blu teleporter pairs in TF2.");
    g_hLimits[3][2] = CreateConVar("sm_buildlimit_blu_sentries",    "1", "Limit for Blu sentries in TF2.");
                ...
                else
                {
                    if (permissions > 0) // Hack to find native bits
                    {
                        switch (TF2_GetPlayerClass(i))
                        {
                            case TFClass_Spy:
                            {
                                new TFPlayerCond:pcond = TF2_GetPlayerCond(i);
...
{
    RemoteControl(client, TFObjectType_Unknown);
    return Plugin_Handled;
}
...
RemoteControl(client, TFObjectType:type)
{
    ...
    if (target > 0) 
    {
        type = TF2_GetObjectType(target, true);
        if (type < TFObjectType_Sapper)
        {
    ...
    if (type == TFObjectType_Unknown)
    {
        ...
        if ((permissions & REMOTE_CAN_BUILD) != 0)
        {
            ...
            AddMenuItem(menu,"0","Build a Dispenser",
                        (counts[TFObjectType_Dispenser] >= g_isAllowed[client][TFObjectType_Dispenser])
                        ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);

            AddMenuItem(menu,"3","Build a Teleporter Entry",
                        (counts[TFObjectType_TeleporterEntry] >= g_isAllowed[client][TFObjectType_TeleporterEntry])
                        ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);

            AddMenuItem(menu,"4","Build a Teleporter Exit",
                        (counts[TFObjectType_TeleporterExit] >= g_isAllowed[client][TFObjectType_TeleporterExit])
                        ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);

            new flag = (counts[TFObjectType_Sentrygun] >= g_isAllowed[client][TFObjectType_Sentrygun])
                       ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT;

            if (!(permissions & REMOTE_CAN_BUILD_UPGRADED))
                AddMenuItem(menu,"2","Build a Sentry Gun", flag);
            else
            {
                AddMenuItem(menu,"6","Build a Level 1 Sentry Gun", flag);
                AddMenuItem(menu,"7","Build a Level 2 Sentry Gun", flag);
                if ((permissions & REMOTE_CAN_BUILD_LEVEL_3))
                    AddMenuItem(menu,"8","Build a Level 3 Sentry Gun", flag);
            }

            AddMenuItem(menu,"9","Destroy a Structure", ((sum > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED));
        }
        ...
        {
            CancelMenu(menu);
            control(client, target, TF2_GetObjectType(target, true));
            return;
        }
        else if (sum == 0)
        {
    ...
    else
    {
        ...
        if (objectid <= 0 && (permissions & REMOTE_CAN_BUILD))
        {
                 ...
                 CountObjects(client,TF2_ObjectClassNames[type], TF2_ObjectModes[type]) <= 0))
            {
                ...
                {
                    object = _:TFObjectType_Teleporter;
                    mode = _:(type - TFObjectType_TeleporterEntry);
                }
                else
                {
                    object =_:type;
            ...
            else
            {
                objectid = BuildSelectedObject(client, type, 1, true,
                                               ((permissions & REMOTE_CAN_BUILD_INSTANTLY) == 0),
                                               ((permissions & REMOTE_CAN_BUILD_FLOATING) != 0));
            }
        }
        else if (objectid > 0)
...
BuildObject(client, TFObjectType:type)
{
    new permissions = GetPermissions(client);
    if (!(permissions & REMOTE_CAN_BUILD))
    {
...
BuildMenu(client, permissions, bool:control)
{
    ...
    AddMenuItem(menu,"3","Teleporter Entry",
                (counts[TFObjectType_TeleporterEntry] >= g_isAllowed[client][TFObjectType_TeleporterEntry])
                ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
                
    AddMenuItem(menu,"4","Teleporter Exit",
                (counts[TFObjectType_TeleporterExit] >= g_isAllowed[client][TFObjectType_TeleporterExit])
                ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
    ...
    {
        AddMenuItem(menu,"6","Level 1 Sentry Gun", flag);
        AddMenuItem(menu,"7","Level 2 Sentry Gun", flag);
        if ((permissions & REMOTE_CAN_BUILD_LEVEL_3))
            AddMenuItem(menu,"8","Level 3 Sentry Gun", flag);
    }

    AddMenuItem(menu,"9","Destroy Structure", ((sum > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED));
    DisplayMenu(menu,client,MENU_TIME_FOREVER);
}
...
public BuildSelected(Handle:menu,MenuAction:action,client,selection)
{
    if (action == MenuAction_Select)
    {
        ...
        else
        {
            ...
            else if (item == 5) // Amplifier
            {
                type = TFObjectType_Dispenser; // TFObjectType_Amplifier
                level = 1;
            }
            else if (item >= 6)
            {
            ...
            {
                BuildSelectedObject(client, type, level, g_RemoteBuild[client],
                                    ((permissions & REMOTE_CAN_BUILD_INSTANTLY) == 0),
                                    ((permissions & REMOTE_CAN_BUILD_FLOATING) != 0));
            }
        }
...
BuildSelectedObject(client, TFObjectType:type, iLevel=1, bool:remote=false,
                    bool:disable=true, bool:drop=true, bool:check=true,
                    iHealth=-1, iMaxHealth=-1, Float:objectPosition[3]={0.0})
{
    new objectid = -1;
    ...
    if (res == Plugin_Continue)
    {
        ...
        switch (type)
        {
            case TFObjectType_Dispenser:       objectid = BuildDispenser(client, pos, angles, disable, iLevel, iHealth, iMaxHealth);
            case TFObjectType_Sentrygun:       objectid = BuildSentry(client, pos, angles, disable, iLevel, iHealth, iMaxHealth);
            case TFObjectType_TeleporterEntry: objectid = BuildTeleporterEntry(client, pos, angles, disable, iLevel, iHealth, iMaxHealth);
            case TFObjectType_TeleporterExit:  objectid = BuildTeleporterExit(client, pos, angles, disable, iLevel, iHealth, iMaxHealth);
        }
        ...
        if (objectid > 0)
        {
            ...
            if (remote)
            {
                ...
                {
                    if (type != TFObjectType_Teleporter &&
                        type != TFObjectType_TeleporterEntry &&
                        type != TFObjectType_TeleporterExit)
                    {
            ...
            if (disable || !remote)
            {
                CreateTimer((type == TFObjectType_Sentrygun) ? 10.0 * float(iLevel) : 20.0,
                            Activate, EntIndexToEntRef(objectid), TIMER_FLAG_NO_MAPCHANGE);
            }
        }
    }
...
CountBuildings(client, counts[TFObjectType])
{
    new sum;
    for (new TFObjectType:t = TFObjectType_Dispenser;t <= TFObjectType_TeleporterExit; t++)
    {
        if (t != TFObjectType_Teleporter)
        {
            counts[t] = CountObjects(client, TF2_ObjectClassNames[t], TF2_ObjectModes[t]);
            sum += counts[t];
        }
    }
    counts[TFObjectType_Teleporter] = counts[TFObjectType_TeleporterEntry]
                                    + counts[TFObjectType_TeleporterExit];
    return sum;
}
...
AddBuildingsToMenu(Handle:menu, client, bool:all=false, counts[TFObjectType]=0, &target=0)
{
    new sum;
    for (new TFObjectType:t = TFObjectType_Dispenser;t <= TFObjectType_TeleporterExit; t++)
    {
...
DestroyBuilding(object)
{
    ...
    {
        if (TF2_GetObjectType(object) != TFObjectType_Unknown)
        {
            SetVariantInt(1000);
        ...
        if (g_RemoteType[index] == type)
        {
            if (objectid <= 0)
            {
                for (new i=MaxClients+1;i<g_iMaxEntities;i++)
                {
                    ...
                    {
                        if (TF2_GetObjectType(i,true) == type)
                        {
...
public Action:PlayerChangeTeamEvent(Handle:event,const String:name[],bool:dontBroadcast)
{
    ...
    if (client > 0 && flags != HasBuiltNothing)
    {
        if ((flags & HasBuiltDispenser) != HasBuiltNothing)
            DestroyObjects(TF2_ObjectClassNames[TFObjectType_Dispenser], client, false);

        if ((flags & HasBuiltSentry) != HasBuiltNothing)
            DestroyObjects(TF2_ObjectClassNames[TFObjectType_Sentrygun], client, false);

        if ((flags & HasBuiltTeleporterEntrance) != HasBuiltNothing ||
            (flags & HasBuiltTeleporterExit) != HasBuiltNothing)
        {
            DestroyObjects(TF2_ObjectClassNames[TFObjectType_Teleporter], client, false);
        }
...
public EventRoundOver(Handle:event,const String:name[],bool:dontBroadcast)
{
    ...
    DestroyObjects(TF2_ObjectClassNames[TFObjectType_Dispenser], -1, false);
    DestroyObjects(TF2_ObjectClassNames[TFObjectType_Teleporter], -1, false);
    DestroyObjects(TF2_ObjectClassNames[TFObjectType_Sentrygun], -1, false);
...
public Action:Activate(Handle:timer,any:ref)
{
    ...
    {
        new TFObjectType:type = TF2_GetObjectType(object, true);
        if (type != TFObjectType_Unknown)
        {
...
public ObjectSelected(Handle:menu,MenuAction:action,client,selection)
{
    if (action == MenuAction_Select)
    {
        ...
        if (objectRef >= 0 && objectRef <= 9) // 0-9 are build options
        {
            g_RemoteBuild[client] = true;
        ...
        else
        {
            ...
            if (objectid > 0 && IsValidEdict(objectid) && IsValidEntity(objectid))
                control(client, objectid, TF2_GetObjectType(objectid, true));
        }
    }
    else if (action == MenuAction_End)
        CloseHandle(menu);
...
bool:control(client, objectid, TFObjectType:type)
{
    new Action:res = Plugin_Continue;
        ...
        !IsEntLimitReached(.client=client,.message="unable to create info_observer_point"))
    {
        ...
        if (watcher > 0 && DispatchSpawn(watcher))
        {
            new Float:angles[3];
...
public Action:RemoteOff(client, args)
{
    ...
    if (objectRef != 0)
    {
        ...
        if (object > 0 && IsValidEdict(object) && IsValidEntity(object))
        {
            ...
            new TFObjectType:type = g_RemoteType[client];
            if ((type != TFObjectType_Teleporter &&
                 type != TFObjectType_TeleporterEntry &&
                 type != TFObjectType_TeleporterExit) &&
    ...
    g_RemoteObjectRef[client] = 0;
    g_RemoteType[client] = TFObjectType_Unknown;
    return Plugin_Handled;
}
...
public OnClientPutInServer(client)
{
    ...
    g_isAllowed[client][TFObjectType_Teleporter]++; // 1 entry + 1 exit
}
...
public Action:Command_Build(client, args)
{
           ...
           (GetUserFlagBits(client) & ADMFLAG_GENERIC|ADMFLAG_ROOT)))))
    {
        ...
        new TFObjectType:object = TFObjectType:StringToInt(sObject);
        new mode = StringToInt(sMode);

        new TFTeam:team = TFTeam:GetClientTeam(client);
        if (object < TFObjectType_Dispenser || object > TFObjectType_TeleporterExit || team < TFTeam_Red)
            return Plugin_Continue;
...
bool:CheckBuild(client, TFObjectType:type, mode=-1, &iCount=0)
{
    if (type >= TFObjectType_Sapper) // Don't check sappers or unknown objects
        iCount = -1;
    else
    {
        ...
        new iLimit = g_bNativeControl ? g_isAllowed[client][type]
                   : GetConVarInt(g_hLimits[GetClientTeam(client)][type]);
...
{
    decl Float:pos[3];
    new ent = BuildSelectedObject(GetNativeCell(1), TFObjectType:GetNativeCell(2),
                                  bool:GetNativeCell(3), bool:GetNativeCell(4),
                                  bool:GetNativeCell(5), bool:GetNativeCell(6),
                                  GetNativeCell(7), GetNativeCell(8),
                                  GetNativeCell(9), pos);
    SetNativeArray(10, pos, sizeof(pos));
    return ent;                                  
}
...
public Native_BuildSentry(Handle:plugin,numParams)
{
    ...
    if (!IsEntLimitReached(.client=client, .message="unable to create obj_sentrygun"))
    {
        ...
        return BuildSentry(client, fOrigin, fAngle, bool:GetNativeCell(4),
...
public Native_BuildDispenser(Handle:plugin,numParams)
{
    ...
    if (!IsEntLimitReached(.client=client, .message="unable to create obj_dispenser"))
    {
        ...
        GetNativeArray(2, fOrigin, sizeof(fOrigin));
        GetNativeArray(3, fAngle, sizeof(fAngle));
        return BuildDispenser(client, fOrigin, fAngle, bool:GetNativeCell(4),
...
public Native_BuildTeleporterEntry(Handle:plugin,numParams)
{
    ...
    if (!IsEntLimitReached(.client=client, .message="unable to create obj_teleporter entrance"))
    {
        ...
        GetNativeArray(2, fOrigin, sizeof(fOrigin));
        GetNativeArray(3, fAngle, sizeof(fAngle));
        return BuildTeleporterEntry(client, fOrigin, fAngle, bool:GetNativeCell(4),
                                    GetNativeCell(5), GetNativeCell(6), GetNativeCell(7));
    }
    else
        return -1;
...
public Native_BuildTeleporterExit(Handle:plugin,numParams)
{
    ...
    if (!IsEntLimitReached(.client=client, .message="unable to create obj_teleporter exit"))
    {
        ...
        GetNativeArray(2, fOrigin, sizeof(fOrigin));
        GetNativeArray(3, fAngle, sizeof(fAngle));
        return BuildTeleporterExit(client, fOrigin, fAngle, bool:GetNativeCell(4),
                                   GetNativeCell(5), GetNativeCell(6), GetNativeCell(7));
    }
    else
        return -1;
...
public Native_DestroyBuildings(Handle:plugin,numParams)
{
    ...
    if (all || (flags & HasBuiltDispenser) != HasBuiltNothing)
        count += DestroyObjects(TF2_ObjectClassNames[TFObjectType_Dispenser], client, all);

    if (all || (flags & HasBuiltSentry) != HasBuiltNothing)
        count += DestroyObjects(TF2_ObjectClassNames[TFObjectType_Sentrygun], client, all);

    if (all || (flags & HasBuiltTeleporterEntrance) != HasBuiltNothing ||
               (flags & HasBuiltTeleporterExit) != HasBuiltNothing)
    {
        count += DestroyObjects(TF2_ObjectClassNames[TFObjectType_Teleporter], client, all);
    }
...
{
    new client = GetNativeCell(1);
    g_isAllowed[client][TFObjectType_Dispenser] = GetNativeCell(3);
    g_isAllowed[client][TFObjectType_Sentrygun] = GetNativeCell(2);
    g_isAllowed[client][TFObjectType_TeleporterEntry] = GetNativeCell(4);
    g_isAllowed[client][TFObjectType_TeleporterExit] = GetNativeCell(5);

    g_isAllowed[client][TFObjectType_Teleporter] = g_isAllowed[client][TFObjectType_TeleporterEntry]
                                                 + g_isAllowed[client][TFObjectType_TeleporterExit];
}
...
{
    new iCount;
    new bool:result = CheckBuild(GetNativeCell(1), TFObjectType:GetNativeCell(2),
...
#tryinclude <tf2_build>
    ...
    stock BuildSentry(iBuilder, const Float:fOrigin[3], const Float:fAngle[3],
                      bool:iDisabled=false, iLevel=1, iHealth=-1, iMaxHealth=-1,
                      iShells=-1, iRockets=-1)
    {
        ...
        if (iSentry)
        {
            ...
            g_HasBuilt[iBuilder] |= HasBuiltSentry;
        }
        return iSentry;
    }

    stock BuildDispenser(iBuilder, const Float:fOrigin[3], const Float:fAngle[3],
                         bool:iDisabled=false, iLevel=1, iHealth=-1, iMaxHealth=-1,
                         iMetal=-1)
    {
        static const Float:fBuildMaxs[3] = { 24.0, 24.0, 66.0 };
        ...
        new iDispenser = CreateEntityByName(TF2_ObjectClassNames[TFObjectType_Dispenser]);
        if (iDispenser)
        {
            ...
            SetEntProp(iDispenser, Prop_Send, "m_iObjectType", 			            _:TFObjectType_Dispenser, 1);
            ...
            {
                SetEventInt(event, "userid", GetClientUserId(iBuilder));
                SetEventInt(event, "object", _:TFObjectType_Dispenser);
            ...
            g_HasBuilt[iBuilder] |= HasBuiltDispenser;
        }
        return iDispenser;
    }

    stock BuildTeleporterEntry(iBuilder, const Float:fOrigin[3], const Float:fAngle[3],
                               bool:iDisabled=false, iLevel=1, iHealth=-1, iMaxHealth=-1)
    {
        ...
        new iTeleporter = CreateEntityByName(TF2_ObjectClassNames[TFObjectType_Teleporter]);
        if (iTeleporter)
        {
            ...
            SetEntProp(iTeleporter, Prop_Send, "m_iObjectType", 				    _:TFObjectType_Teleporter, 1);
            SetEntProp(iTeleporter, Prop_Send, "m_nSkin", 					        (iTeam-2), 1);
            SetEntProp(iTeleporter, Prop_Send, "m_iUpgradeLevel", 			        iLevel, 4);
            SetEntProp(iTeleporter, Prop_Send, "m_iObjectMode", 				    0, TF2_ObjectModes[TFObjectType_TeleporterEntry]);
            ...
            {
                SetEventInt(event, "userid", GetClientUserId(iBuilder));
                SetEventInt(event, "object", _:TFObjectType_TeleporterEntry);
            ...
            g_HasBuilt[iBuilder] |= HasBuiltTeleporterEntrance;
        }
        return iTeleporter;
    }

    stock BuildTeleporterExit(iBuilder, const Float:fOrigin[3], const Float:fAngle[3],
                              bool:iDisabled=false, iLevel=1, iHealth=-1, iMaxHealth=-1)
    {
        static const Float:fBuildMaxs[3] = { 28.0, 28.0, 66.0 };
        ...
        if (iTeleporter)
        {
            ...
            SetEntProp(iTeleporter, Prop_Send, "m_nSkin", 					        (iTeam-2), 1);
            SetEntProp(iTeleporter, Prop_Send, "m_iUpgradeLevel", 			        iLevel, 4);
            SetEntProp(iTeleporter, Prop_Send, "m_iObjectMode", 				    1, TF2_ObjectModes[TFObjectType_TeleporterExit]);
            SetEntProp(iTeleporter, Prop_Send, "m_bBuilding", 				        0, 2);
            SetEntProp(iTeleporter, Prop_Send, "m_bPlacing", 				        0, 2);
            SetEntProp(iTeleporter, Prop_Send, "m_bHasSapper", 				        0, 2);
            SetEntProp(iTeleporter, Prop_Send, "m_nNewSequenceParity", 		        4, 4 );
            ...
            {
                SetEventInt(event, "userid", GetClientUserId(iBuilder));
                SetEventInt(event, "object", _:TFObjectType_TeleporterExit);
                SetEventInt(event, "index", iTeleporter);
                SetEventBool(event, "sourcemod", true);
                FireEvent(event);
            }
            ...
            g_WasBuilt[iTeleporter] = true;
            g_HasBuilt[iBuilder] |= HasBuiltTeleporterExit;
        }
        return iTeleporter;
    }

...
Cvarlist (default value):
    ...
    sm_saysoundhe_update_clientprefs 0      If set, updates the clientprefs DB when clients leave. 
...
public Action:OnAudioBroadcast(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if(GetGameType() == tf2)
    {
        ...
        if(StrEqual(sound, "Game.Stalemate") && runSoundEvent(event,"round","Stalemate",0,0,-1))
            ...
            return Plugin_Handled;
        /*
        if(suddendeath && runSoundEvent(event,"suddendeathend","suddendeathend", _, _,g_BroadcastAudioTeam))
        {
            //runSoundEvent(event,"suddendeathend","suddendeathend",0,0);
            suddendeath = false;
            return Plugin_Continue;
        }
        runSoundEvent(event,"roundend","roundend", _, _,g_BroadcastAudioTeam);
        return Plugin_Continue;
        */
    }
    else if(GetGameType() == dod)
...
// ####	TF2 & DoD Event ###
	if(GetGameType() == tf2){
		g_BroadcastAudioTeam = GetEventInt(event, "team");
		if(suddendeath && runSoundEvent(event,"suddendeathend","suddendeathend", _, _,g_BroadcastAudioTeam)) {
			//runSoundEvent(event,"suddendeathend","suddendeathend",0,0);
			suddendeath = false;
			return Plugin_Continue;
...
public CheckJoin(client, const String:auth[])
{
    ...
    if(GetConVarBool(cvarspecificjoinexit))
    {
        decl String:filelocation[PLATFORM_MAX_PATH+1];
        KvRewind(listfile);
        if (KvJumpToKey(listfile, auth) && !greeted[client])
        {
            filelocation[0] = '\0';
    ...
    {
        KvRewind(listfile);
        if (KvJumpToKey(listfile, "JoinSound") && !greeted[client])
        {

...
public OnPluginStart()
{
    ...
    ConfigFileName = CreateConVar("sm_maxclass_config","MaxClass.txt","config file location");
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            if (c == TFClass_Engineer)	// Wrap back to 1st class
                c = TFClass_Scout;
            else if (c == pick) // if we hit the initial class, all classes must be full
                break;
        }
    }
...
RecountClasses()
{
    ...
    for (new i=1; i<=maxplayers; i++)
    {
        if (IsClientInGame(i))
        {
            if (GetConVarBool(CheckAdmins) && !GetConVarBool(CountAdmins))
            {
                if (CheckForAdmin( i ))
                    continue;
            }

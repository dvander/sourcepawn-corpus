...
ThrowNade(client, bool:special=true, bool:Setup)
{
	...
	// get nade type
	new NadeType:type = (gNativeOverride) ? gSpecialType[client] : DefaultNade;
	...
	SetupNade(type, team, special ? 1 : 0);
...
SetupNade(NadeType:type, team, special)
{
	...
	if (numParams >= 1)
	{
		...
		if (IsPlayerAlive(client))
		{
			if (gSpecialType[client] != old_type)
			{
				...
				SetupNade(type, GetClientTeam(client), 1);
			}

            ...
            if (TF_IsUberCharge(client) == 0)
            {
                    ...
                    g_MedicUberCharge[client] < GetConVarInt(g_MedipacksDeploy))
                {
                    ...
                    if (weaponent > 0 && GetEntProp(weaponent, Prop_Send, "m_iItemDefinitionIndex") != 35) // Kritzkrieg
                        TF_DropMedipack(client, true, (g_Holiday == TFHoliday_Halloween));
                }
...
public Action:Command_Spawn(client,args)
{
    ...
    new bool:halloween = (g_Holiday == TFHoliday_Halloween) ||
                          StrEqual(command, "sm_halloweenhealth");
    ...
    if (StrEqual(buffer, "full", false))
    {
        ...
        TF_SpawnMedipack(client, "item_healthkit_full", true, halloween);
        return Plugin_Handled;
    }
    else if (StrEqual(buffer, "medium", false))
    {
        ...
        TF_SpawnMedipack(client, "item_healthkit_medium", true, halloween);
        return Plugin_Handled;
    }
    else if (StrEqual(buffer, "small", false))
    {
        ...
        TF_SpawnMedipack(client, "item_healthkit_small", true, halloween);
        return Plugin_Handled;
    }
    ...
    if (MedipacksKeep > 0 || MedipacksLimit > 0)
    {
        ...
        for (new c = MaxClients; c < maxents; c++)
        {
            ...
            if (time > 0)
            {
                ...
                {
                    g_MedipacksCount--;
                    SetArrayCell(g_MedipacksRef, c, 0);
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    TF_DropMedipack(client, false, (g_Holiday == TFHoliday_Halloween));
}
...
public Action:Entity_OnPlayerTouch(const String:output[], caller, activator, Float:delay)
{
    if (activator > 0 && caller > 0)
    {
        ...
        if (time > 0)
        {
            SetArrayCell(g_MedipacksRef, caller, 0);
...
stock TF_SpawnMedipack(client, String:name[], bool:cmd, bool:halloween)
{
...
stock bool:TF_DropMedipack(client, bool:cmd, bool:halloween)
{
    new charge;
    if (cmd)
...
public Native_DropMedipack(Handle:plugin, numParams)
{
    ...
    new bool:halloween = (type >= 2 ||
                          type == 0 && g_Holiday == TFHoliday_Halloween);

    if (charge >= 0)
    {
        g_MedicUberCharge[client] = charge;
        return TF_DropMedipack(client, false, halloween);
    }
    else
        return TF_DropMedipack(client, true, halloween);
}

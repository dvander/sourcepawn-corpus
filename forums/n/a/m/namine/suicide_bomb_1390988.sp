...
new Handle:g_Cvar_BuyTime		= INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_Cvar_Bomb1Radius	= CreateConVar("bomb_1_radius", "150", "Set the radius for bomb 1 in inches.", FCVAR_PLUGIN);
	...
	g_Cvar_Bomb2Radius	= CreateConVar("bomb_2_radius", "250", "Set the radius for bomb 2 in inches.", FCVAR_PLUGIN);
	...
	g_Cvar_Bomb3Radius	= CreateConVar("bomb_3_radius", "400", "Set the radius for bomb 3 in inches.", FCVAR_PLUGIN);
	g_Cvar_DamageExpl	= CreateConVar("bomb_damage_explode", "0", "If 1, you detonate if shot while you have a bomb.", FCVAR_PLUGIN);
	g_Cvar_BuyTime		= CreateConVar("bomb_buy_time", "60.0", "The time in seconds that you are allowed to buy a bomb.", FCVAR_PLUGIN);
...
public OnRoundStart()
{
	new Float:buy_time;
	bomb_buyable = true;
	buy_time = GetConVarFloat(g_Cvar_BuyTime);
	CreateTimer(buy_time, Buy_Time);
}

public Action:Buy_Time(Handle:timer, any:client)
{
	bomb_buyable = false;
}
...
public Action:Bomb_Menu(client, args)
{
	if (GetConVarInt(g_Cvar_Enable))
	{
		...
		if (bomb_buyable)
		{
			if (player_team < 2)
			{
				PrintToChat(client, "\x03[xG] \x01%T", "Youre_Dead", client);
			...
			if (player_team == 2)
			{
				if(GetConVarInt(g_Cvar_Enabled_T))
				{
					...
					else PrintToChat(client, "\x03[xG] \x01%T", "Have_Bomb_Already", client);
					return Plugin_Handled;
				}
				else PrintToChat(client, "\x03[xG] \x01%T", "Disabled_Team", client, "Terrorists");
				return Plugin_Handled;
			}
			if (player_team == 3)
			{
				...
				else PrintToChat(client, "\x03[xG] \x01%T", "Disabled_Team", client, "Counter-Terrorists");
				return Plugin_Handled;
			}
		}
		else PrintToChat(client, "\x03[xG] \x01%T", "Overtime", client);
		return Plugin_Handled;
	}
	else PrintToChat(client, "\x03[xG] \x01%T", "Disabled", client);
	return Plugin_Handled;
}
		...
		if (player_bomb_type[param1] == 0)
		{
			if (param2 == 0)
			{
				...
				if (money >= price1)
				{
					...
					PrintToChat(param1, "\x03[xG] \x01%T", "Bought_C", param1, "Small Bomb");
					return;
				}
				else PrintToChat(param1, "\x03[xG] \x01%T", "Expensive", param1, price1);
				return;
			}
			if (param2 == 1)
			{
				...
				if (money >= price2)
				{
					...
					PrintToChat(param1, "\x03[xG] \x01%T", "Bought_C", param1, "Medium Bomb");
					return;
				}
				else PrintToChat(param1, "\x03[xG] \x01%T", "Expensive", param1, price2);
				return;
			}
			if (param2 == 2)
			{
				...
				if (money >= price3)
				{
					...
					PrintToChat(param1, "\x03[xG] \x01%T", "Bought_C", param1, "Large Bomb");
					return;
				}
				else PrintToChat(param1, "\x03[xG] \x01%T", "Expensive", param1, price3);
				return;
			}
		}
		else PrintToChat(param1, "\x03[xG] \x01%T", "Have_Bomb_Already", param1);
		return;
	}
	return;
}
public Action:Command_Detonate(client, args)
{
	if (GetConVarInt(g_Cvar_Enable))
	{
		if (player_bomb_type[client] > 0)
		{
			...
			else
			{
				if (player_bomb_type[client] == 1)
				{
					...
					{
						CreateTimer(0.0, Bomb_Time, client);
						EmitSoundToAll("ambient/explosions/explode_1.wav", client, SNDCHAN_AUTO, SNDLEVEL_MINIBIKE);						
					}
					else
					{
						explode(client);
		...
		else PrintToChat(client, "\x03[xG] \x01%T", "No_Bomb", client);
	}
	else PrintToChat(client, "\x03[xG] \x01%T", "Disabled", client);
	return Plugin_Handled;
}
...
stock explode(client)
{
	...
	if (explosion != -1)
	{
		...
		EmitSoundToAll("ambient/explosions/explode_1.wav", explosion, 1, 90);						

...
public Action:TestTrace(client, args)
{
	...
	if (GetPlayerEye(client, vecPos))
	{
		for (new i = 1; i<= MaxClients; i++)
		{
			if (GetVectorDistance(vecPos, g_arrSprayTrace[i]) <= GetConVarFloat(g_arrCVars[MAXDIS]))
			{
				...
				PrintToChat(client, "\x03[Spray]\x01 %T", "Spray By", client, g_arrSprayName[i], g_arrSprayID[i], time);
	...
	PrintToChat(client, "\x03[Spray]\x01 %T", "No Spray", client);
	return Plugin_Handled;
}
	...
	if (GetPlayerEye(client, vecPos))
	{
		...
		for (new i = 1; i<= MaxClients; i++)
		{
			if (GetVectorDistance(vecPos, g_arrSprayTrace[i]) <= GetConVarFloat(g_arrCVars[MAXDIS]))
			{
				...
				PrintToChat(client, "\x03[Spray]\x01 %T", "Spray By", client, g_arrSprayName[i], g_arrSprayID[i], RoundFloat(GetGameTime()) - g_arrSprayTime[i]);
				SprayDecal(i, 0, vecEndPos);
				g_arrSprayTrace[i][0] = 0.0;
				g_arrSprayTrace[i][1] = 0.0;
				g_arrSprayTrace[i][2] = 0.0;
				PrintToChat(client, "\x03[Spray]\x01 %T", "Spray Removed", client, g_arrSprayName[i], g_arrSprayID[i], szAdminName);
				LogAction(client, -1, "[SprayTracer] %T", "Spray Removed", LANG_SERVER, g_arrSprayName[i], g_arrSprayID[i], szAdminName);
				AdminMenu(client, i);
				return Plugin_Handled;
			}
		}
...
public Action:AdminSpray(client, args)
{
	...
	if (args == 1)
	{
		...
		{
			ReplyToCommand(client, "\x03[Spray]\x01 %T", "Could Not Find Name", client, arg);
			return Plugin_Handled;
		}
	}
	else
...
public GoSpray(client, target)
{
	...
	new ent = GetPlayerEyeEnt(client, vecEndPos);

	if (ent != -1)
	{
		decl String:targetName[32];
		decl String:szAdminName[32];
		GetClientName(target, targetName, 31);
		GetClientName(client, szAdminName, 31);
		SprayDecal(target, ent, vecEndPos);
		EmitSoundToAll("misc/sprayer.wav", client, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, 0.6);
		PrintToChat(client, " \x03[Spray]\x01 %T", "Admin Sprayed", client, szAdminName, targetName);
		LogAction(client, -1, "[SprayTracer] %T", "Admin Sprayed", LANG_SERVER, szAdminName, targetName);
	}
	else
	{
		PrintToChat(client, " \x03[Spray]\x01 %T", "Cannot Spray", client);
	}
}
stock GetPlayerEyeEnt(client, Float:vecPos[3])
{
	if (!IsValidClient(client))
		return false;
...
public MenuHandler_AdminSpray(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		{
			PrintToChat(param1, "\x03[Spray]\x01 %T", "Could Not Find", param1);
		}
		else
		{
...
public AdminMenuHandler(Handle:hMenu, MenuAction:action, client, itemNum)
{
	if ( action == MenuAction_Select )
	{
		...
		if ((strcmp(szInfo, "sban") == 0) || (strcmp(szInfo, "pban") == 0))
		{
			if (sprayer)
			{
				...
				if ( g_hExternalBan != INVALID_HANDLE )
				{
					...
					if (iTime == 0)
					{
						LogAction(client, -1, "[SprayTracer] %T", "EPBanned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID, "SourceBans");
					}
					else
					{
						LogAction(client, -1, "[SprayTracer] %T", "EBanned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID, iTime, "SourceBans");
					}
				...
				else
				{
					...
					if ( g_hExternalBan != INVALID_HANDLE )
					{
						...
						if (iTime == 0)
						{
							LogAction(client, -1, "[SprayTracer] %T", "EPBanned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID, "MySQL Bans");
						}
						else
						{
							LogAction(client, -1, "[SprayTracer] %T", "EBanned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID, iTime, "MySQL Bans");
						}
					...
					else
					{
						...
						if (iTime == 0)
						{
							LogAction(client, -1, "[SprayTracer] %T", "PBanned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
						}
						else
						{
							LogAction(client, -1, "[SprayTracer] %T", "Banned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID, iTime);
						}
					}
				}
				if (iTime == 0)
				{
					PrintToChatAll("\x03[Spray]\x01 %T", "PBanned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
				}
				else
				{
					PrintToChatAll("\x03[Spray]\x01 %T", "Banned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID, iTime);
				}
			}
			else
			{
				PrintToChat(client, " \x03[Spray]\x01 %T", "Could Not Find Name ID", client, szSprayerName, szSprayerID);
				LogAction(client, -1, "[SprayTracer] %T", "Could Not Find Name ID", LANG_SERVER, szSprayerName, szSprayerID);
			}
		}
		else if ( sprayer && IsClientInGame(sprayer) )
		{
			...
			{
				PrintToChat(client, " \x03[Spray]\x01 %T", "Admin Immune", client, szSprayerName);
				LogAction(client, -1, "[SprayTracer] %T", "Admin Immune Log", LANG_SERVER, szAdminName, szSprayerName);
				AdminMenu(client, sprayer);
			}
			else if ( strcmp(szInfo, "warn") == 0 )
			{
				PrintToChat(sprayer, "\x03[Spray]\x01 %T", "Please change", sprayer);
				PrintToChat(client, " \x03[Spray]\x01 %T", "Warned", client, szSprayerName, szSprayerID);
				LogAction(client, -1, "[SprayTracer] %T", "Log Warned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
				AdminMenu(client, sprayer);
			}
			else if ( strcmp(szInfo, "slap") == 0 )
			{
				PrintToChat(sprayer, "\x03[Spray]\x01 %T", "Please change", sprayer);
				PrintToChat(client, " \x03[Spray]\x01 %T", "Slapped And Warned", client, szSprayerName, szSprayerID, GetConVarInt(g_arrCVars[SLAPDMG]));
				LogAction(client, -1, "[SprayTracer] %T", "Log Slapped And Warned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID, GetConVarInt(g_arrCVars[SLAPDMG]));
			...
			{
				PrintToChat(sprayer, "\x03[Spray]\x01 %T", "Please change", sprayer);
				PrintToChat(client, " \x03[Spray]\x01 %T", "Slayed And Warned", client, szSprayerName, szSprayerID);
				LogAction(client, -1, "[SprayTracer] %T", "Log Slayed And Warned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
			...
			{
				PrintToChat(sprayer, "\x03[Spray]\x01 %T", "Please change", sprayer);
				PrintToChat(client, " \x03[Spray]\x01 %T", "Burnt And Warned", client, szSprayerName, szSprayerID);
				LogAction(client, -1, "[SprayTracer] %T", "Log Burnt And Warned", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
			...
			{
				PrintToChat(sprayer, "\x03[Spray]\x01 %T", "Please change", sprayer);
				PrintToChat(client, " \x03[Spray]\x01 %T", "Froze", client, szSprayerName, szSprayerID);
				LogAction(client, -1, "[SprayTracer] %T", "Log Froze", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
			...
			{
				PrintToChat(sprayer, "\x03[Spray]\x01 %T", "Please change", sprayer);
				PrintToChat(client, " \x03[Spray]\x01 %T", "Beaconed", client, szSprayerName, szSprayerID);
				LogAction(client, -1, "[SprayTracer] %T", "Log Beaconed", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
			...
			{
				PrintToChat(sprayer, "\x03[Spray]\x01 %T", "Please change", sprayer);
				PrintToChat(client, " \x03[Spray]\x01 %T", "FreezeBombed", client, szSprayerName, szSprayerID);
				LogAction(client, -1, "[SprayTracer] %T", "Log FreezeBombed", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
			...
			{
				PrintToChat(sprayer, "\x03[Spray]\x01 %T", "Please change", sprayer);
				PrintToChat(client, " \x03[Spray]\x01 %T", "FireBombed", client, szSprayerName, szSprayerID);
				LogAction(client, -1, "[SprayTracer] %T", "Log FireBombed", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
			...
			{
				PrintToChat(sprayer, "\x03[Spray]\x01 %T", "Please change", sprayer);
				PrintToChat(client, " \x03[Spray]\x01 %T", "TimeBombed", client, szSprayerName, szSprayerID);
				LogAction(client, -1, "[SprayTracer] %T", "Log TimeBombed", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
			...
			{
				PrintToChat(sprayer, "\x03[Spray]\x01 %T", "Please change", sprayer);
				PrintToChat(client, " \x03[Spray]\x01 %T", "Drugged", client, szSprayerName, szSprayerID);
				LogAction(client, -1, "[SprayTracer] %T", "Log Drugged", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
			...
			else if ( strcmp(szInfo, "kick") == 0 )
			{
				...
				PrintToChatAll("\x03[Spray]\x01 %T", "Kicked", LANG_SERVER, szSprayerName, szSprayerID);
				LogAction(client, -1, "[SprayTracer] %T", "Log Kicked", LANG_SERVER, szAdminName, szSprayerName, szSprayerID);
			}
		}
		else
		{
		...
		if (GetConVarBool(g_arrCVars[AUTOREMOVE]))
		{
			...
			PrintToChat(client, "\x03[Spray]\x01 %T", "Spray Removed", client, szSprayerName, szSprayerID, szAdminName);
			LogAction(client, -1, "[SprayTracer] %T", "Spray Removed", LANG_SERVER, szSprayerName, szSprayerID, szAdminName);
		}
	}

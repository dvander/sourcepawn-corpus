...
char g_SpecialPaths[27][] = {  // sv_precacheinfo
...
public Action ADTimer(Handle timer) {
	...
	if (g_ADFreeze) {
		for (int i = 1 ; i <= MaxClients ; i++) {
			...
			// do once :)
			else if (g_ADInterval == 0 && CountTeamMates(2) >= 1) {
				MkBots(g_ExtraPlayers, 2);
				g_ADInterval--;
			}
		}
...
PrecacheModels() {
	...
	for (int i = 0 ; i < sizeof(g_SpecialPaths) ; i++) {
		Format(g_sB, sizeof(g_sB), "%s", g_SpecialPaths[i]);
		//if (!IsModelPrecached(g_sB)) {  // doesn't work for SI?
		int retcode = PrecacheModel(g_sB);

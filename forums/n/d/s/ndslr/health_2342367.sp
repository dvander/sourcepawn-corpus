...
public void OnPluginStart()
{
	...
	cvar_sm_hp_heal = CreateConVar("sm_hp_heal", "25", "Amount of Heal");
	...
	HookConVarChange(cvar_sm_hp_heal, Event_CvarChange);
}
...
public Action Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
    if (enabled)
    {
        ...
        {
            maxHP = GetConVarInt(cvar_sm_hp_max);
			healHP = GetConVarInt(cvar_sm_hp_heal);
            new attackerHP = GetClientHealth(attackerIndex);

            if (attackerHP < maxHP)
            {
				newHP = attackerHP + healHP;
				if (newHP < maxHP) {
...
public Event_CvarChange(Handle:cvar, const String:oldValue[], const String:newValue[])
{
	...
	healHP = GetConVarBool(cvar_sm_hp_heal);
}
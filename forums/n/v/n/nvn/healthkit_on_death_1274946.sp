...
new Handle:hod_min_heal		= INVALID_HANDLE;
new Handle:hod_max_heal		= INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	hod_min_heal	= CreateConVar("hod_min_heal", "10", "", FCVAR_PLUGIN);
	hod_max_heal	= CreateConVar("hod_max_heal", "40", "", FCVAR_PLUGIN);
...
public StartTouch(iEntity, iOther)
{
	...
	if (iClientHealth < iMaxHealth) {
		iClientHealth += GetRandomInt(GetConVarInt(hod_min_heal),GetConVarInt(hod_max_heal));
		if (iClientHealth > iMaxHealth) {

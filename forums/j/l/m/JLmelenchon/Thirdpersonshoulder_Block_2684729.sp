...
public QueryClientConVarCallback(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName[], const String:cvarValue[])
{
	if (IsClientInGame(client) && !IsClientInKickQueue(client))
	{
											 ...
											 */
		{
			...
			new String:sName[MAX_NAME_LENGTH];
			GetClientName(client, sName, sizeof(sName));
			LogAction(0, client, "\"%L\" received a warning for potentially using thirdpersonshoulder mode", client);
			...
			if (iWarnings[client] >= iTotalWarnings)
			{
				...
				LogAction(0, client, "Kicked \"%L\" for refusing to disable third person camera.", client);
				PrintToChatAll("\x04 %s kicked for refusing to disable third person camera after many warnings.\x01", sName);
			}
		}
	}
}
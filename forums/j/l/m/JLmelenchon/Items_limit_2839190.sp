...
{
    char mode[32];
    Handle h = FindConVar("mp_gamemode");
    if (h == INVALID_HANDLE) return false;
    GetConVarString(h, mode, sizeof(mode));
...
{
    bool dbg = (GetConVarInt(gCvarDebug) != 0);

    for (int i = 0; i < view_as<int>(IT_MAX); i++)
    {
        int limit = GetConVarInt(g_ItemCvars[i]);
        if (limit < 0)
        {
        ...
        for (int idx = total - 1; idx >= 0 && removed < toRemove; idx--)
        {
...
Action Timer_PhaseDelete(Handle timer)
{
    ...
    float dPhase = GetConVarFloat(gCvarRestoreDel1);
    CreateTimer(dPhase, Timer_PhaseCreate, _, TIMER_FLAG_NO_MAPCHANGE);
    return Plugin_Stop;
}
...
{
    bool dbg = (GetConVarInt(gCvarDebug) != 0);
    int preferSpawn = GetConVarInt(gCvarUseSpawn);
...
public void OnPluginStart()
{
    ...
    gCvarRestoreDel2   = CreateConVar("il_restore_start_delay", "3.0",  "Délai initial avant delete (2e manche)");
...
public void OnRoundIsLive()
{
    ...
    else
    {
        ...
        if (!GetConVarInt(gCvarMirror) || !g_HasCache) return;
        ...
        float dStart = GetConVarFloat(gCvarRestoreDel2);
        ...
        if (GetConVarInt(gCvarDebug))
            LogMessage("[IL] 2e manche: restauration planifiée (start=%.2fs, phase=%.2fs).",
                        dStart, GetConVarFloat(gCvarRestoreDel1));
    }
}
...
void Event_RoundEnd(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (IsVersusMode() && InSecondHalf())
    {
        CacheClearAll();
        g_FirstHalfAppliedThisMap = false;
        if (GetConVarInt(gCvarDebug)) LogMessage("[IL] Fin 2e manche: cache purgé.");
    }
}

public void OnMapStart()
{
    if (L4D_IsFirstMapInScenario() == true && !InSecondHalf())
    {
        // Changement de map -> purge
        CacheClearAll();
        g_FirstHalfAppliedThisMap = false;
        if (GetConVarInt(gCvarDebug)) LogMessage("[IL] nouvelle map : cache purgé.");
    }
}

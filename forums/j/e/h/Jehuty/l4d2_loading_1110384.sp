...
#define PLUGIN_VERSION "1.0.4" //-Added fix against bots running out of saferoom before countdown has finished
                                                        //-Plugin will now spawn a locked door 2 seconds after round start instead of 5.
                                                        //This will prevent people from opening the door before count down has finished.
...
public OnPluginStart()
{
	...
	freezeOn1st = CreateConVar("l4d_freezeOn1st", "0", "Freeze survivors on first chapter (0 = no, 1 = freeze survivors until all players have loaded; 2 = use countdown like on the other chapters)");
	...
	waitOnTimeOut = CreateConVar("l4d2_timeout", "50", "Wait this many seconds after a map starts before giving up on waiting for a client (timeout)");
	...
	awaitSpawn = CreateConVar("l4d2_infectedSpawn", "0", "Wait for infected to be ready to spawn before starting countdown");
...
public Action:LiveTimer(Handle:timer)
{
	...
	{
		PrintTextAll("%i seconds remaining untill starting round", GetConVarInt(prepareTimeScrim) - countDown);
	}
...
public Action:StartTimer(Handle:timer)
{
	...
	{
		PrintTextAll("%i seconds remaining untill starting round", GetConVarInt(isFirstRound ? prepareTime1st : prepareTime2nd) - countDown);
	}
...
GetCheckPointDoorStart()
{
	...
	if (foundAnOrigin)
	{
		...
		if (checkPointDoorEntityStart != 0)
		{
			...
			SpawnFakeDoor(checkPointDoorEntityStart, true, 2.0);
		}

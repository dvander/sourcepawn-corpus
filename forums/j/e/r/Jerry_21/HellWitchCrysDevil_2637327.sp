...
public OnMapStart()
{
	...
	PrecacheMaterial("materials/sprites/glow01.vmt");     //RUTA CORRECTA MODIFICADA POR ESTAR MAL
...
public Action:Tail(Handle:hTimer, any:iEntRef)
{
	...
	if(!g_bDevilWitch)//should be false
	{
		...
		{
			switch(GetRandomInt(1, 10))        //CAMBIE ESTO PARA QUE HAYA 20% DE PROBABILIDAD DE QUE SALGA LA DEVIL WITCH VALOR POR DEFECTO 10
			{
				case 1:
				{
					g_bDevilWitch = true;
					...
					Format(sValues, sizeof(sValues), "Witch Demoniaca Manchada!");
...
public Action:eWitchStartled(Handle:hEvent, const String:strName[], bool:bDontBroadcast)
{
	...
	for(new s = 1; s <= 15; s++)
	{
		...
		DispatchKeyValue(iEnt, "model", "materials/sprites/glow01.vmt");
	...
	if(g_bHW)
	{
		if(!GetEventBool(hEvent, "oneshot"))
		{
			...
			if(iWitchKiller > 0 && iWitchKiller <= MaxClients && IsClientInGame(iWitchKiller) && GetClientTeam(iWitchKiller) == 2)
			{
				...
				{
					CreateTimer(10.0, WitchMob, iWitchKiller, TIMER_FLAG_NO_MAPCHANGE);

...
#if !defined DMG_GENERIC  //CHECAR ESA !
...
public OnPluginStart()
{
	...
	hCvar_TankMobCount =    CreateConVar("HW_TankCount", "0", "Amount In TankMob 0 = Disable", FCVAR_PLUGIN, true, 0.0, true, 31.0);
	hCvar_HunterMobCount =  CreateConVar("HW_HunterCount", "3", "Amount In HunterMob 0 = Disable", FCVAR_PLUGIN, true, 0.0, true, 31.0);
	hCvar_ChargerMobCount = CreateConVar("HW_ChargerCount", "3", "Amount In ChangerMob 0 = Disable", FCVAR_PLUGIN, true, 0.0, true, 31.0);
	hCvar_BoomerMobCount =  CreateConVar("HW_BoomerCount", "3", "Amount In BoomerMob 0 = Disable", FCVAR_PLUGIN, true, 0.0, true, 31.0);
	hCvar_MobCall       =   CreateConVar("HW_MobCall", "2", "Should We Enable Mobs?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	hCvar_DirHint       =   CreateConVar("HW_DirectorHint", "1", "Should We Enable Director Hints?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	hCvar_TankRush      =   CreateConVar("HW_TankRush", "0", "Should We Enable TankRush Globally?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	
	//nuevos cvars para smoker'jockey y spitters
	hCvar_SmokerMobCount =    CreateConVar("HW_SmokerCount", "3", "Amount In SmokerMob 0 = Disable", FCVAR_PLUGIN, true, 0.0, true, 31.0);
	hCvar_JockeyMobCount =    CreateConVar("HW_JockeyCount", "3", "Amount In JockeyMob 0 = Disable", FCVAR_PLUGIN, true, 0.0, true, 31.0);
	hCvar_SpitterMobCount =    CreateConVar("HW_SpitterCount", "3", "Amount In SpitterMob 0 = Disable", FCVAR_PLUGIN, true, 0.0, true, 31.0);
...
public Action:Tail(Handle:hTimer, any:iEntRef)
{
	...
	if(!g_bDevilWitch)//should be false
	{
		...
		{
			switch(GetRandomInt(1, 4))        //CAMBIE ESTO PARA QUE HAYA 20% DE PROBABILIDAD DE QUE SALGA LA DEVIL WITCH VALOR POR DEFECTO 10
			{
				case 1:
				{
					g_bDevilWitch = true;
...
public Action:eWitchStartled(Handle:hEvent, const String:strName[], bool:bDontBroadcast)
{
	...
	if(IsClientInGame(iWitchiClient) && GetClientTeam(iWitchiClient) == 2 && IsPlayerAlive(iWitchiClient))
	{
		...
		Format(sValues, sizeof(sValues), "La Witch Demoniaca Va Contra Tí %N", iWitchiClient);
		DispatchKeyValue(iEntity, "hint_caption", sValues);
...
public Action:ShockWave(Handle:hTimer, any:iWitch)
{
		...
		if (IsClientInGame(iClient) || IsFakeClient(iClient) && GetClientTeam(iClient) == 2 && IsPlayerAlive(iClient))
	{
		...
		PrintToChatAll("Control De Rango");
		...
		if (distance < range)
		{
			...
			PrintToChatAll("Terminado %N", iClient);
	...
	if(g_bHW)
	{
		if(!GetEventBool(hEvent, "oneshot"))
		{
			...
			if(iWitchKiller > 0 && iWitchKiller <= MaxClients && IsClientInGame(iWitchKiller) && GetClientTeam(iWitchKiller) == 2)
			{
				...
				{
					CreateTimer(10.0, WitchMob, iWitchKiller, TIMER_FLAG_NO_MAPCHANGE);  //parte donde empieza a llamar a las demas witch  poner printTochat y quitar lineas y complementada por accion de witchmob
					...
					Format(sValues, sizeof(sValues), "Ha Invocado A Witchs!");
...
public Action:MobCall(Handle:hTimer, any:iWitchKiller)
{
	...
	switch(GetRandomInt(1, 10))   //aumentado de 7  a 10 por infectados agregados
	{
		case 1:
		{
			if(g_bMobCall)
...
public Action:WitchMob(Handle:hTimer, any:iWitchKiller)
{
	...
	for(new i = 1; i <= 6; i++)                                          //CANTIDAD DE WITCH QUE SALDRAN POR INVOCACION DE LA DEVIL WITCH

...
public OnPluginStart()
{
	...
	*/
	...
	RegAdminCmd("sm_swap", Command_Swap, ADMFLAG_KICK, "sm_swap <player1> [player2] ... [playerN] - swap all listed players to opposite teams");
	RegAdminCmd("sm_swapto", Command_SwapTo, ADMFLAG_KICK, "sm_swapto <player1> [player2] ... [playerN] <teamnum> - swap all listed players to <teamnum> (1,2, or 3)");
...
public Action:Command_GetTeamScore(client, args)
{
	...
	if(fGetTeamScore == INVALID_HANDLE)
	{
		DebugPrintToAll("Could not load GetTeamScore function, GetConf = %b", fGetTeamScore);
		return Plugin_Handled;
	}
...
{	
	if(fClearTeamScores == INVALID_HANDLE)
	{
		DebugPrintToAll("Could not load ClearTeamScores function, GetConf = %b", fClearTeamScores);
		return Plugin_Handled;
	}

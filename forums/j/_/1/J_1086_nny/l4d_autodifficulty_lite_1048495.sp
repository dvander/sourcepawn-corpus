...
new Handle:l4d_autodifficulty_lite;
...
{
	l4d_autodifficulty_lite = CreateConVar("l4d_autodifficulty_lite", "1", "Is the plugin enabled.", FCVAR_PLUGIN);
	HookConVarChange(l4d_autodifficulty_lite, Autodifficulty_EnableDisable);
...
public OnMapStart()
{
    if (GetConVarInt(l4d_autodifficulty_lite) == 1) 
	{
...
public Autodifficulty(playerscount)
{
	...
	if (playerscount > 4)
	{
		...
		SetConVarInt(z_hunter_health, RoundToZero(150.0 * playerscount), false, false);
		SetConVarInt(z_smoker_health, RoundToZero(150.0 * playerscount), false, false);
		SetConVarInt(z_boomer_health, RoundToZero(50.0 * playerscount), false, false);

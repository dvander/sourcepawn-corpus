...
//      PrintToChat(target, "\x01%N [\x05%f x %f = %d (%d) TK points!\x01] attacked %N", client, X, Damage[client], RoundToZero(X * Damage[client]), RoundToZero(Punishment[client]) + 750, target);
	...
	PrintToChat(client, "\x03%f x %f = %d (%d) TK points!", X, Damage[client], RoundToZero(X * Damage[client]), RoundToZero(Punishment[client]) + 750);
	PrintToChat(target, "\x01%N attacked %N", client, target);
	PrintToChat(target, "\x03%f x %f = %d (%d) TK points!", X, Damage[client], RoundToZero(X * Damage[client]), RoundToZero(Punishment[client]) + 750);

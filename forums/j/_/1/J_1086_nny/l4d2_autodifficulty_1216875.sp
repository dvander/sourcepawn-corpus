...
stock CheckCvarMax(const Cvar_Value, Cvar_Value_Max)
{
		...
		if (IsClientConnected(i))
		{
			...
			if (StrEqual(ClientSteamID, "BOT", false))
				if (IsFakeClient(i))
				{
					...
					{
//						PrintToChat(client, "\x04%s", ClientName);
...
public Action:Command_Say(client, args)
{
	...
	else if (strcmp(text[startidx], "!easyâûàûâàûâ", false) == 0)
	{
	...
	else if (strcmp(text[startidx], "!normalûâàûâàûâà", false) == 0)
	{
	...
	else if (strcmp(text[startidx], "!hardûâàûâàûâàûà", false) == 0)
	{

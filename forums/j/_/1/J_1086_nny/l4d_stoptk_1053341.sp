...
new String:LastBannedClient[32];
...
public OnPluginStart()
{
	...
	CreateConVar("l4d_tk_deadline", "3500", "TK Points needed to get banned.", FCVAR_PLUGIN);
	CreateConVar("l4d_tk_startvote_points", "750", "TK Points needed to get voteban.", FCVAR_PLUGIN);
}
...
//	PrintToChat(client, "\x01%N [\x05%f x %f = %d (%d) TK points!\x01] attacked %N", client, X, Damage[client], RoundToZero(X * Damage[client]), RoundToZero(Punishment[client]) + 750, target);
//      PrintToChat(target, "\x01%N [\x05%f x %f = %d (%d) TK points!\x01] attacked %N", client, X, Damage[client], RoundToZero(X * Damage[client]), RoundToZero(Punishment[client]) + 750, target);

	PrintToChat(client, "\x01%N attacked %N", client, target);
	PrintToChat(client, "[%N] \x03%f x %f = %d (%d) TK points!", client, X, Damage[client], RoundToZero(X * Damage[client]), RoundToZero(Punishment[client]) + 750);
	PrintToChat(target, "\x01%N attacked %N", client, target);
	PrintToChat(target, "[%N] \x03%f x %f = %d (%d) TK points!", client, X, Damage[client], RoundToZero(X * Damage[client]), RoundToZero(Punishment[client]) + 750);
...
public Action:Event_PlayerIncapacitated(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	PrintToChat(client, "\x04%N [\x05%d (%d) TK points!\x04] attacked %N", client, RoundToZero(X * Damage[client]), RoundToZero(Punishment[client]) + 750, target);
        PrintToChat(target, "\x04%N [\x05%d (%d) TK points!\x04] attacked %N", client, RoundToZero(X * Damage[client]), RoundToZero(Punishment[client]) + 750, target);
	CheckPunishmentPoints(client);
...
public CheckPunishmentPoints(client)
{
	...
	{
		if ((TotalDamage) > 3500.0)
		{
			decl String:ClientSteamID[32];
			GetClientAuthString(client, ClientSteamID, sizeof(ClientSteamID));
			if (strcmp(ClientSteamID, LastBannedClient, false) != 0)
			{
				...
				LastBannedClient = ClientSteamID;
			}
		}
		else if ((TotalDamage - Punishment[client]) > 750.0)
		{

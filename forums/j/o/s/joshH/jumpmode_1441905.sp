...
new g_iClip1Offset, g_iAmmoOffset;
...
public OnPluginStart()
{
	...
	g_iClip1Offset = FindSendPropInfo("CTFWeaponBase", "m_iClip1");
...
public Action:eventPlayerChangeClass(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(g_bPluginEnabled && client != 0 && IsClientInGame(client))
	{
		ZeroLocs(client);
	...
	for (new i = 0; i < 3; i++)
	{
		if(!(iClass == TFClass_Heavy && i == 1))
		{
			if (TFClass_MaxAmmo[iClass][i] != -1)
				SetEntData(client, g_iAmmoOffset + ((i+1)*4), TFClass_MaxAmmo[iClass][i]);
			if (i != 2 && TFClass_MaxClip[iClass][i] != -1) {
				new slot = GetPlayerWeaponSlot(client, i);
				if( slot != -1 )
					SetEntData(slot, g_iClip1Offset, TFClass_MaxClip[iClass][i]);
			}
		}
	}
}

...
* Version 1.5.0         支持l4d2的写实模式和valve's的突变模式。
...
public Plugin:myinfo = 
{
	...
	version = Version,
	url = "https://forums.alliedmods.net/showpost.php?p=1541805&postcount=605 skiner@inbox.ru"
};
...
public OnPluginStart()
{
	...
	CreateConVar("sm_l4d_fmc_version", Version, "Version of L4D/L4D2 Force Mission Changer plugin.", FCVAR_NOTIFY);
	...
	CheckRoundCounter = CreateConVar("sm_l4d_fmc_crec", "4", "round_end的事件数量级别和变化对比: 4 for l4d <> 1.0.1.2");
	ChDelayVS = CreateConVar("sm_l4d_fmc_chdelayvs", "0.0", "对抗模式换图延迟（秒）");
	ChDelayCOOP = CreateConVar("sm_l4d_fmc_chdelaycoop", "0.0", "战役模式换图延迟（秒）");
	TimerRoundEndBlockVS = CreateConVar("sm_l4d_fmc_re_timer_block", "0.5", "不考虑当前事件round_end的时间（以秒为单位）.");
	cvarAnnounce = CreateConVar("sm_l4d_fmc_announce", "1", "对所有玩家提示换图信息 0 = 关  1 = 开");
	
	//For custom crec
	RegServerCmd("sm_l4d_fmc_crec_add", Command_CrecAdd, "为指定的地图添加自定义值sm_l4d_fmc_crec和sm_l4d_fmc_re_timer_block。 最大50.");
	RegServerCmd("sm_l4d_fmc_crec_clear", Command_CrecClear, "清除所有自定义值sm_l4d_fmc_crec和sm_l4d_fmc_re_timer_block.");
	RegServerCmd("sm_l4d_fmc_crec_list", Command_CrecList, "显示所有自定义值sm_l4d_fmc_crec和sm_l4d_fmc_re_timer_block.");
...
{
	//If game mode actually changed //如果游戏模式真的改变了
...
public Action:TimerAnnounce(Handle:timer, any:client)
{
	if(IsClientInGame(client))
	{
		...
		{
			PrintToChat(client, "\x04[地图信息]\x03 已到最后一关，祝各位完成救援！");
			PrintToChat(client, "\x04[地图信息]\x03 下一关地图: \x04%s", announce_map);
		}
		else
...
l4d_gamemode()
{
	// 1 - coop - realism - coop mutations // 1 - 合作 - 现实主义 - 合作社突变
	// 2 - versus -versus mutations  // 2 - 对抗 - 突变
	// 3 - survival modes(thx DDR Khat for code) // 3 - 生存模式（thx DDR Khat代码）
...
PluginInitialization()
{
	...
	{
		KvGetString(hKVSettings, "下一个任务地图", next_mission_force, 64, next_mission_def);
		KvGetString(hKVSettings, "下一个任务名称", force_mission_name, 64, "none");
	}
	KvRewind(hKVSettings);

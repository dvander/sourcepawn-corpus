...
{
	CreateConVar("L4D2_TANK_HP_version", "L4D2坦克生命值设定", "注意:以下数值相加后是普通难度Tank血量,专家的要再*2", 8512, false, 0, false, 0);
	hBasicTankHP = CreateConVar("l4d2_basic_hp", "4000", "Tank基本生命值(<=4个玩家时)", 0, true, 1, true, 100000);
	hAddTankHP = CreateConVar("l4d2_add_hp", "1000", ">4个玩家时,每多一名玩家Tank增加生命值(例.普通:7人就是7-4=3*1k+4k=7k ;专家:数值再*2 7k*2=1.4w)", 0, true, 1, true, 100000);
	hTankHPSH = CreateConVar("l4d2_show_hp", "2", "提示Tank生命[0关,1回合开始时,2玩家加入时,3=1+2]", 0, true, 0, true, 3);
...
public Action TankhpshowDelays(Handle timer)
{
	...
	PrintToChatAll("\x04[!提示!]\x05 幸存者人数改变了,\x03 %s \x05难度 Tank血量现在是\x03 %d", GameDIFF, DisTankHP);
	return Action 0;
}

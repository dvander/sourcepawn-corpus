...
- version 1.1:
		Replaced 速度恶魔 with Grasshopper.
...
- version 1.4.1:
		...
		Merged 速度恶魔 with Old School. Reduced 速度恶魔 to 1.4x and
...
- version 2.0.1a:
		...
		杀手代号47 (remake): now deals a flat +50% damage bonus to all damage.
...
- version 2.0.2a:
		...
		双喷发射 now activates properly for the first attack after spawning.
...
- version 2.0.7a:
		...
		Revised 双喷发射 to be more consistent in giving two shots.
...
- version 2.0.9:
		Added 骑士技能 for Jockeys: gives 60% more health.
		...
		Lowered 饿虎扑食 minimum pounce damage from 11 to 10
		Lowered 杀手代号47 damage bonus from 0.3 to 0.2
		Lowered Grasshopper pounce speed multiplier from 1.3 to 1.2
- version 2.0.10:
		Added 青蛙技能 for Jockeys: 20% more damage, 30% more leap distance.
		Added 恶灵骑士技能 for Jockeys: near invisibility.
		Added 高速子弹技能 for Chargers: charges are 30% faster and longer.
		Modified 散射Ram to also give 30% more health.
...
- version 2.1.0:
		双喷发射, Adrenal Glands and other cooldown-reducing perks should now
...
- version 2.1.1:
		Added 粘鼠胶技能 to spitters: slows survivor speed by 50% for up to two seconds
			after leaving spit.
		Added 烟鬼!技能 to smokers: can walk while smoking (thanks to Olj for this perk!)
...
- version 2.1.3:
		...
		Updated offsets for ability and attack timers, cooldown-reducing perks (ie. 双喷发射)
			and attack speed increase perks (ie. Adrenal Glands) should now work properly.
- version 2.1.4:
		Increased 双喷发射 delay in-between spits from 2.5s -> 6s.
		Increased Mega-Adhesive slow from 50% -> 60%.
		Updated Pack Rat further, should be less buggy... with any luck.
- version 2.1.5:
		Rewrote some code, 粘鼠胶 should no longer interfere with other movement-changing
			infected perks.
...
- version 2.2.0:
		...
		Updated offsets for cooldown-reducing infected perks, should be working now (双喷发射, 拖拽掉落).
		Added cvars to enable or disable all infected or survivor
...
public Plugin:myinfo=
{
	...
	url="https://forums.alliedmods.net/showpost.php?p=932386&postcount=200?p=932386&postcount=200"
}
...
//init player perk vars //初始玩家技能
new g_iSur1[MAXPLAYERS+1];	//survivors, primary //幸存者，初级
new g_iSur2[MAXPLAYERS+1];	//survivors, secondary //幸存者，二级
new g_iSur3[MAXPLAYERS+1];	//survivors, tertiary //幸存者，三级
new g_iInf1[MAXPLAYERS+1];	//boomer
new g_iInf2[MAXPLAYERS+1];	//tank
new g_iConfirm[MAXPLAYERS+1];	//check if perks are confirmed, to prevent mid-game changing abuses //检查技能是否得到确认，以防止游戏中出现的滥用行为
...
//timer perks handle //定时器特殊技能
new Handle:g_hTimerPerks = INVALID_HANDLE;

//烟火师技能 //烟火专家技能
//track how many grenades are carried for 烟火师技能 //追踪有多少手榴弹是为了烟火专家而进行的
new g_iGren[MAXPLAYERS+1];
//used so functions don't confuse legitimate grenade pickups //使用了这样的技能，不会混淆合法的手榴弹拾取。
//with acquisitions from grenadier perk //和来自grenadier掷弹兵技能的收购
new g_iGrenThrow[MAXPLAYERS+1];
//used to track which type of grenade was used; //用于跟踪所使用的手榴弹类型;
//1 = pipe, 2 = molotov //1 =土制雷管，2 =燃烧瓶
new g_iGrenType[MAXPLAYERS+1];
//used to track how many "ticks" have passed //用来追踪有多少“ticks”通过。
//since we want to give pipe bombs after a given //因为我们想在给定的情况下给管道炸弹。
//number of ticks //ticks的数量。
new g_iPyroTicks[MAXPLAYERS+1];
new g_iPyroRegisterIndex[MAXPLAYERS+1] = -1;
//and this tracks how many have DT //tracks有多少DT
new g_iPyroRegisterCount = 0;

//Spirit 技能 //SPIRIT 技能
//0 = not incapped //0 = 不会倒地
//1 = incapped //1 = 倒地
new g_iPIncap[MAXPLAYERS+1];
//used to keep track of whether cooldown is in effect //用于跟踪的冷却时间是否有效
new g_iSpiritCooldown[MAXPLAYERS+1];
//used to track the timers themselves //用来跟踪计时器。
new Handle:g_iSpiritTimer[MAXPLAYERS+1];

//双发快射 / /双发快射技能
//used to track who has the 双发快射. //用于跟踪拥有双重tap奖励的人员。
//The index goes up to 18, but each index has //指数达到18，但每个指数都有
//a value indicating a client index with DT //一个指示客户端索引与DT的值。
//so the plugin doesn't have to cycle a full //这样插件就不用循环一次了。
//18 times per game frame just for double tap. //每个游戏帧每秒18次，双击即可。
new g_iDTRegisterIndex[MAXPLAYERS+1] = -1;
//and this tracks how many have DT /这追踪有多少DT
...
//武术大师技能 //武术家技能
//similar to Double Tap 类似于双击
new g_iMARegisterIndex[MAXPLAYERS+1] = -1;
//and this tracks how many have MA 这种跟踪有多少MA
new g_iMARegisterCount = 0;
//these are similar to those used by Double Tap 这些与Double Tap使用的类似
...
//VARIOUS INFECTED PERKS //多面感染的技能
...
//VARS TO STORE CONVAR VALUES //用于存储变量值的VARS
...
//青蛙, dmg multiplier, leap multiplier
...
//INF6 (SPITTER) PERKS
//双喷发射, time delay between two shots
//one-size-fits-all
...
//INF7 (CHARGER) PERKS
//散射Ram, charge force multiplier and maximum cooldown
//one-size-fits-all
...
CreateConvars()
{
	...
	g_hStopping_dmgmult = CreateConVar(
		...
		"0.2" ,
		"停止力量特性：奖励伤害倍增，加成到基础伤害（夹在0.05 <1.0之间）" ,
	...
	g_hPyro_maxticks = CreateConVar(
		...
		"60" ,
		"烟火师技能: 在给幸存者一个管道炸弹之前，tick嘀嗒声的数量(tick嘀嗒声是2个)。60个节拍= 120秒。在0到300之间，其中0将禁用该特性。" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hPyro_enable = CreateConVar(
		...
		"1" ,
		"烟火师技能: 是否让比赛团队的玩家选择技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hPyro_enable_sur = CreateConVar(
		...
		"1" ,
		"烟火师技能: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSpirit_buff = CreateConVar(
		...
		"10" ,
		"Spirit 技能: 奖励自我缓冲的健康缓冲（在0＜170之间）" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSpirit_cd = CreateConVar(
		...
		"60" ,
		"Spirit 技能: 在几秒钟内自我恢复的冷却时间， campaign比赛（夹在1 <1800）" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSpirit_cd_sur = CreateConVar(
		...
		"60" ,
		"Spirit 技能: 在几秒钟内自我恢复的冷却时间，生存（夹在1 <1800）" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSpirit_cd_vs = CreateConVar(
		...
		"60" ,
		"Spirit 技能: 几秒钟内自我恢复的冷却时间，versus对抗（1 <1800）" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSpirit_enable = CreateConVar(
		...
		"1" ,
		"Spirit 技能: 是否让比赛团队的玩家选择技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSpirit_enable_sur = CreateConVar(
		...
		"1" ,
		"Spirit 技能: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSpirit_enable_vs = CreateConVar(
		...
		"1" ,
		"Spirit 技能: 许玩家在对抗中选择该技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hDT_rate = CreateConVar(
		...
		"0.6667" ,
		"双发快射: 子弹发射的间隔乘以这个值。注意:足够短的间隔将枪开火只有正常速度!(夹紧0.2 < 0.9)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hDT_enable_sur = CreateConVar(
		...
		"1" ,
		"双发快射: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSoH_rate = CreateConVar(
		...
		"0.5714" ,
		"眼疾手快技能: 重新加载所引起的间隔乘以这个值(夹在0.2 < 0.9之间)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSoH_enable = CreateConVar(
		...
		"1" ,
		"眼疾手快技能: 是否让比赛团队的玩家选择技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSoH_enable_sur = CreateConVar(
		...
		"1" ,
		"眼疾手快技能: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hUnbreak_hp = CreateConVar(
		...
		"20" ,
		"牢不可破技能: 为坚不可摧的身体提供的额外健康;这个值也作为恢复的额外健康缓冲(在1到100之间夹紧)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hUnbreak_enable = CreateConVar(
		...
		"1" ,
		"牢不可破技能: 是否让比赛团队的玩家选择技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hUnbreak_enable_sur = CreateConVar(
		...
		"1" ,
		"牢不可破技能: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hChem_enable = CreateConVar(
		...
		"1" ,
		"瘾君子技能: 是否让比赛团队的玩家选择技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hChem_enable_sur = CreateConVar(
		...
		"1" ,
		"瘾君子技能: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hHelpHand_timemult = CreateConVar(
		...
		"0.6" ,
		"东郭先生技能: 用援助之手来恢复他人的时间乘数（夹在0.01 <1.0之间）" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hHelpHand_buff = CreateConVar(
		...
		"15" ,
		"东郭先生技能: 在复活之后给予盟友的奖励健康缓冲，战役/生存（夹在0 <170）" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hHelpHand_buff_vs = CreateConVar(
		...
		"10" ,
		"东郭先生技能: 在复活之后给予盟友的奖励健康缓冲区，与（夹在0 <170之间）" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hHelpHand_enable = CreateConVar(
		...
		"1" ,
		"东郭先生技能: 是否让比赛团队的玩家选择技能, 0 =禁用，1 =启用 (注意：这个技能通常会调整survivor_revive_duration ConVar; 禁用此特技能阻止插件调整此ConVar）" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hHelpHand_enable_sur = CreateConVar(
		...
		"1" ,
		"东郭先生技能: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用 (注意：这个技能通常会调整survivor_revive_duration ConVar; 禁用此技能将会阻止插件调整此ConVar)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hHelpHand_enable_vs = CreateConVar(
		...
		"1" ,
		"东郭先生技能: 许玩家在对抗中选择该技能, 0 =禁用，1 =启用 (注意：这个技能通常会调整survivor_revive_duration ConVar; 禁用此技能将会阻止插件调整此ConVar)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hHelpHand_convar = CreateConVar(
		...
		"1" ,
		"东郭先生技能: 这个技能通常会调整survivor_revive_duration ConVar; 将其设置为0将停止插件调整此ConVar" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hPack_ammomult = CreateConVar(
		...
		"0.2" ,
		"收破烂王技能: 奖励弹药容量，增加到基本容量（夹在0.01 <1.0之间）" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hPack_enable = CreateConVar(
		...
		"1" ,
		"收破烂王技能: 是否让比赛团队的玩家选择技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hPack_enable_sur = CreateConVar(
		...
		"1" ,
		"收破烂王技能: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hHard_hpmult = CreateConVar(
		...
		"0.5" ,
		"九尾狐技能: 奖金倒地健康倍增，产品添加到基础incap健康(夹在0.01 < 3.0)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hHard_enable = CreateConVar(
		...
		"1" ,
		"九尾狐技能: 是否让比赛团队的玩家选择技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hHard_enable_sur = CreateConVar(
		...
		"1" ,
		"九尾狐技能: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hMA_maxpenalty = CreateConVar(
		...
		"4" ,
		"武术大师技能: 最大的惩罚适用于幸存者。它是Valve的编码，所以我不知道每个值的确切含义是什么，但是6是最大的推开惩罚(~1.5s)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hMA_enable = CreateConVar(
		...
		"1" ,
		"武术大师技能: 是否让比赛团队的玩家选择技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hMA_enable_sur = CreateConVar(
		...
		"1" ,
		"武术大师技能: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hExtreme_enable = CreateConVar(
		...
		"1" ,
		"惊弓之鸟技能: 是否让比赛团队的玩家选择技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hExtreme_enable_sur = CreateConVar(
		...
		"1" ,
		"惊弓之鸟技能: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hLittle_enable = CreateConVar(
		...
		"1" ,
		"Little Leaguer perk: 是否让比赛团队的玩家选择技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hLittle_enable_sur = CreateConVar(
		...
		"1" ,
		"Little Leaguer perk: 允许玩家在生存模式中选择额外的技能。, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hLittle_enable_vs = CreateConVar(
		...
		"1" ,
		"Little Leaguer perk: 许玩家在对抗中选择该技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hBlind_cdmult = CreateConVar(
		...
		"0.5" ,
		"倒霉鬼技能: 冷却(默认为30秒)乘以这个值(夹在0.01 < 1.0之间)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hBlind_enable = CreateConVar(
		...
		"1" ,
		"倒霉鬼技能: 允许玩家选择该技能, 0 =禁用，1 =启用 (注意:这个技能通常调整z_vomit_interval ConVar;禁用该技能将停止从调整这个ConVar插件)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hDead_dmgmult = CreateConVar(
		...
		"0.5" ,
		"遇难沉船技能: 普通受感染的伤害乘以这个值并增加到其基础伤害(夹在0.01到4.0之间)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hMotion_rate = CreateConVar(
		...
		"1.25" ,
		"晕车技能: Boomer运动的速度乘以这个值(夹在1.0 < 4.0)之间)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hMotion_enable = CreateConVar(
		...
		"1" ,
		"晕车技能: 允许玩家选择该技能, 0 =禁用，1 =启用 (注意:此技能通常调整z_vomit_疲劳的ConVar;禁用该技能将停止从调整这个ConVar插件)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hTongue_speedmult = CreateConVar(
		...
		"1.5" ,
		"巧舌如簧技能: 在抓住幸存者之前的舌头行进速度;乘以这个值(夹在1.0 < 5.0之间)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hTongue_pullmult = CreateConVar(
		...
		"1.5" ,
		"巧舌如簧技能: 抓住幸存者后的舌头拉速;乘以这个值(夹在1.0 < 5.0之间)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hTongue_enable = CreateConVar(
		...
		"1" ,
		"巧舌如簧技能: 允许玩家选择该技能, 0 =禁用，1 =启用 (注意:这个技能的功能通常会调整舌头的范围，舌头的伤害和舌头的速度。禁用此技能将阻止插件调整这些对流。)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSqueezer_dmgmult = CreateConVar(
		...
		"0.5" ,
		"榨油机技能: 所有基础伤害乘以这个值，然后添加到基础损害(夹在0.01到4.0之间)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hDrag_enable = CreateConVar(
		...
		"1" ,
		"拖拽解除武装技能: 允许玩家选择该技能, 0 =禁用，1 =启用 (注意：这个技能通常会调整 tongue_hit_delay 和 tongue_player_dropping_to_ground_time ConVars; 禁用此特权将会阻止插件调整这些ConVars)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSmokeItMaxRange = CreateConVar(
		...
		"5000" ,
		"Smoker!技能: Smoker的最大舌头伸展，舌头将会被释放，如果超过这个(默认950)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hBody_minbound = CreateConVar(
		...
		"10" ,
		"饿虎扑食技能: 定义饿虎扑食处理的最小初始伤害(夹在2 < 100)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hEfficient_dmgmult = CreateConVar(
		...
		"2.2" ,
		"杀手代号47技能: 所有基础伤害乘以这个值，然后添加到基础损害(夹在0.01到4.0之间)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hAdrenal_throwcdmult = CreateConVar(
		...
		"0.4" ,
		"肾上腺技能: 扔石头的冷却时间乘以这个值(夹在0.01到1.0之间)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hAdrenal_enable = CreateConVar(
		...
		"1" ,
		"肾上腺技能: 允许玩家选择该技能, 0 =禁用，1 =启用 (注意：这个技能通常会调整 tank_swing_interval, tank_swing_miss_interval, z_tank_attack_interval, z_tank_throw_interval, and z_tank_throw_force ConVars; 禁用此特权将会阻止插件调整这些ConVars)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hJuggernaut_hp = CreateConVar(
		...
		"3000" ,
		"重型卡车技能: 给予坦克的额外健康值；绝对值（在1＜99999之间）" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hDouble_hpmult = CreateConVar(
		...
		"0.35" ,
		"双倍麻烦技能: 所有坦克产生的健康乘数（夹在0.1 <2.0之间）" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hCavalier_hpmult = CreateConVar(
		...
		"0.6" ,
		"骑士技能:奖金健康倍增器，产品添加到基础健康(夹紧在0.01 < 3.0)之间" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hFrogger_dmgmult = CreateConVar(
		...
		"0.35" ,
		"青蛙技能: 所有基础伤害乘以这个值，然后加到基础伤害。 (夹在 0.01 < 4.0)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hGhost_alpha = CreateConVar(
		...
		"25" ,
		"恶灵骑士技能: 设置alpha级别 (夹在 0 总invis < 255 不透明)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hTwinSF_delay = CreateConVar(
		...
		"6" ,
		"Spitfire双喷技能: 两枪之间的延迟，以秒为单位 (夹在 0.5 < 20.0)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hTwinSF_enable = CreateConVar(
		...
		"1" ,
		"Spitfire双喷技能: 允许玩家选择该技能, 0 =禁用，1 =启用" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hMegaAd_slow = CreateConVar(
		...
		"0.6" ,
		"粘鼠胶技能: 幸存者运行速度直接乘以这个值- 0.6意味着它们以60%的速度运行。 (夹在 0 < 1.0)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hScatter_force = CreateConVar(
		...
		"1.6" ,
		"散射Ram技能: 直接产生的倍增器能让幸存者在充电中使用 (夹在 1.0 < 3.0)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hScatter_hpmult = CreateConVar(
		...
		"0.3" ,
		"散射Ram技能: 奖金健康倍增器，产品增加基础健康。 (夹在 0.01 < 3.0)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hBullet_rate = CreateConVar(
		...
		"1.5" ,
		"高速子弹技能: Time rate while charging (夹在 1.0 < 10.0)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hBot_Sur1 = CreateConVar(
		...
		"1,2,3" ,
		"幸存者1技能的Bot偏好: 1 = 停止权力, 2 = 双发快射, 3 = 手法敏捷" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );

	g_hBot_Sur2 = CreateConVar(
		"l4d_perkmod_bot_survivor2" ,
		"1,2,3" ,
		"幸存者2技能的Bot偏好: 1 = 牢不可破, 2 = spirit, 3 = 一臂之力, 4 = 武术家" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );

	g_hBot_Sur3 = CreateConVar(
		"l4d_perkmod_bot_survivor3" ,
		"1,2" ,
		"幸存者2技能的Bot偏好: 1 = 收破烂, 2 = 很难杀死" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );

	g_hBot_Inf1 = CreateConVar(
		"l4d_perkmod_bot_boomer" ,
		"1,2,3" ,
		"boomer技能的Bot偏好: 1 = 呕吐袋, 2 = 撞大运, 3 = 死亡的沉船 (注意:您可以选择多个使用格式“1、3、4”，游戏将在您的选择之间随机化。)" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );

	g_hBot_Inf3 = CreateConVar(
		"l4d_perkmod_bot_smoker" ,
		"2,3" ,
		"Bot技能偏好smoker 1 = 巧舌, 2 = 榨汁机, 3 = 拖拽掉落 (注意:您可以选择多个使用格式“1、3、4”，游戏将在您的选择之间随机化。" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );

	g_hBot_Inf4 = CreateConVar(
		"l4d_perkmod_bot_hunter" ,
		"1" ,
		"Bot技能偏好hunter 1 = 杀手代号47, 2 = 速度恶魔 (注意:您可以选择多个使用格式“1、3、4”，游戏将在您的选择之间随机化。" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );

	g_hBot_Inf2 = CreateConVar(
		"l4d_perkmod_bot_tank" ,
		"1,2,3,4,5" ,
		"Bot技能偏好tank 1 = 肾上腺素, 2 = 重型卡车, 3 = 促进代谢, 4 = 风暴巫术, 5 = 加倍麻烦 (注意:您可以选择多个使用格式“1、3、4”，游戏将在您的选择之间随机化。" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );

	g_hBot_Inf5 = CreateConVar(
		"l4d_perkmod_bot_jockey" ,
		"1,2,3,4" ,
		"Bot技能偏好jockey 1 = 乘风破浪, 2 = 骑士, 3 = 青蛙, 4 = 幽灵骑士 (注意:您可以选择多个使用格式“1、3、4”，游戏将在您的选择之间随机化。" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );

	g_hBot_Inf6 = CreateConVar(
		"l4d_perkmod_bot_spitter" ,
		"1,2" ,
		"Bot技能偏好spitter 1 = 双喷, 2 = 粘鼠胶 (注意:您可以选择多个使用格式“1、3、4”，游戏将在您的选择之间随机化。" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );

	g_hBot_Inf7 = CreateConVar(
		"l4d_perkmod_bot_charger" ,
		"1,2" ,
		"Bot技能偏好charger 1 = 散射ram, 2 = 超速子弹 (注意:您可以选择多个使用格式“1、3、4”，游戏将在您的选择之间随机化。" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSur1_default = CreateConVar(
		...
		"4" ,
		"默认幸存者的技能，初级: 1 = 阻止技能, 2 = 双发快射, 3 = 手法熟练, 4 = 烟火专家" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSur2_default = CreateConVar(
		...
		"1" ,
		"默认选择技能为幸存者，2级: 1 = 牢不可破的, 2 = spirit, 3 = 援助之手, 4 = 武术家" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSur3_default = CreateConVar(
		...
		"1" ,
		"默认选择技能为幸存者，次要: 1 = 收藏瘾, 2 = 瘾君子, 3 = 很难杀死" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hInf1_default = CreateConVar(
		...
		"1" ,
		"默认选择的技能Boomer: 1 = 呕吐袋, 2 = 撞大运, 3 = 死亡沉船" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hInf2_default = CreateConVar(
		...
		"2" ,
		"默认选择的技能Tank: 1 = 肾上腺素, 2 = 重型卡车, 3 = 代谢促进, 4 = 风暴巫术, 5 = 加倍麻烦" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hInf3_default = CreateConVar(
		...
		"1" ,
		"默认选择的技能Smoker: 1 = 巧舌, 2 = 榨汁机, 3 = 拖拽掉落" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hInf4_default = CreateConVar(
		...
		"1" ,
		"默认选择的技能Hunter: 1 = 饿虎扑食, 2 = 高效的杀手, 3 = 蚱蜢跳跃, 4 = 守旧派, 5 = 速度恶魔" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	//perk hierarchy 技能等级制度
	//--------------
	g_hInfAll_enable = CreateConVar(
		"l4d_perkmod_perktree_infected_enable" ,
		"1" ,
		"如果设置为1，玩家将被允许选择特殊感染的额外技能(影响SI的所有技能)." ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hSurAll_enable = CreateConVar(
		"l4d_perkmod_perktree_survivor_enable" ,
		"1" ,
		"如果设置为1, 玩家将被允许作为幸存者选择额外技能(影响幸存者的所有额外技能)." ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hForceRandom = CreateConVar(
		...
		"0" ,
		"如果设置为1, 玩家将被分配随机的额外津贴，他们不能编辑他们的津贴。." ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hDbEnabled = CreateConVar(
		...
		"1" ,
		"如果设置为1, 玩家自定义选择将被存储到数据库中，并在下次使用作为默认值." ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );

	g_hDbConfName = CreateConVar(
		"l4d_perkmod_db_conf_name" ,
		"default" ,
		"DB连接配置从databas.cfg读取。" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
}
//=============================
// ConVar Changes
...
//changes in perkmod convars 改变perkmod convars
//---------------------------

//stopping power 停止能力
//the enable/disable functions also call 启用/禁用函数也调用。
//the checks-pre-calculate function 检查-预计算函数。
...
//=============================
// Events Directly related to perks 与技能直接相关的事项
//=============================

//this trigger only runs on players, not common infected 此触发仅在玩家身上运行，而不是常见的感染
...
public Event_PlayerRescued (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//reset vars related to Spirit 技能
	g_iMyDisabler[iCid] = -1;
	g_iPIncap[iCid]=0;
	g_iSpiritCooldown[iCid]=0;
	//reset var related to 倒霉鬼技能
...
public Action:Event_TongueGrabPre (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//Spirit 技能, tell plugin player is disabled by smoker
...
public Event_JockeyRideEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//since 乘风破浪 changes the survivor's speeds,
...
//call menu on first spawn, otherwise set default values for bots 在第一个衍生工具上调用菜单，否则为机器人设置默认值
public Event_PlayerFirstSpawn (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		CreateTimer(3.0,Timer_ShowTopMenu,iCid);
		PrintHintText(iCid,"%t", "欢迎信息提示");
		PrintToChat(iCid,"\x03[SM] %t", "欢迎信息聊天");
	}
}

//checks to show perks menu on roundstart 检查在roundstart显示额外技能菜单。
//and resets various vars to default 并将不同的vars重新设置为默认值
...
//for 饿虎扑食. Simpler code I wrote for the other infected-to-survivor perks kept
//inadvertently killing the survivors when they weren't black-and-white... but since
//饿虎扑食 never had that problem, I decided to use 饿虎扑食's code to avoid that
//problem altogether... hence this giant function. However, since 饿虎扑食 doesn't fire
...
Unbreakable_OnHeal (iCid)
{
	...
	if (g_iSur2[iCid]==1)
	{
		...
		PrintHintText(iCid,"坚不可摧: %t!", "UnbreakableHint");
	}
}
...
Unbreakable_OnRevive (iSub, iLedge)
{
		...
		&& iLedge == 0)
	{
			...
			|| g_iUnbreak_enable_vs==1	&&	g_iL4D_GameMode==2))
		{
			...
			PrintHintText(iSub,"坚不可摧: %t!", "UnbreakableHint");
		}
	}
}
...
Spirit_Timer ()
{
	...
	//the Spirit 技能, this function will return
	for (new iI=1 ; iI<=MaxClients ; iI++)
	{
		if (g_iSur2[iI]==2)
		{
	...
	for (new iI=1 ; iI<=iCount ; iI++)
	{
			...
			&& GetClientTeam(iCid[iI])==2)
		{
			...
			//the Spirit 技能 who's also incapped, so we
		...
		case 0:
			{
				...
				PrintHintText(iCid,"东郭先生: %t %s!", "HelpingHandDonorHint", st_name);
				GetClientName(iCid,st_name,24);
				PrintHintText(iSub,"东郭先生: %s %t",st_name, "HelpingHandReceiverHint");
			}
		}
	}
...
BarfBagged_OnIt (iAtt)
{
		...
		&& GetClientTeam(iAtt) == 3)
	{
		...
		if (g_iSlimed==4) PrintHintText(iAtt,"呕吐袋! %t", "BarfBaggedMobHint");
	}
	return 0;
}
...
{
	//----DEBUG----
	//PrintToChatAll("\x03拖拽掉落 running checks");

	//stop if 拖拽掉落 is disabled
	if (g_iInf3_enable==0
		|| g_iDrag_enable==0)
		return;
		...
		&& g_iInf4[iAtt]==1)
	{
		...
		if (iDmgOrig<iMinBound)
		{
			...
			if (iHP>iMinBound)
			{
				...
				PrintHintText(iAtt,"饿虎扑食: %i %t!", iMinBound-iDmgOrig, "BonusDamageText");
			...
			else
			{
				...
				if (flHPBuff>0)
				{
					...
					if (iDmgAdd>=iHP)
					{
						...
						if (iHPBuff<iDmgAdd) iDmgAdd=iHPBuff;
						SetEntDataFloat(iVic,g_iHPBuffO, flHPBuff-iDmgAdd ,true);
						PrintHintText(iAtt,"饿虎扑食: %i 额外的伤害!", iDmgCount+iDmgAdd);
					...
					else
					{
						...
						SetEntProp(iVic,Prop_Data,"m_iHealth", iHP-iDmgAdd );
						PrintHintText(iAtt,"饿虎扑食: %i 额外的伤害!", iDmgAdd);
						return 1;
					}
				}
...
SpeedDemon_DamageAdd (iAtt,iVic,iTA,iType,String:stWpn[],iDmgOrig)
{
		...
		&& g_iMyDisableTarget[iAtt] == -1)
	{
		...
		//----DEBUG----
		//PrintToChatAll("\x05速度恶魔\x03 damage bonus \x01%i",iDmgAdd);
...
Tank_ApplyPerk (any:iCid)
{
		...
		case 1:
			{
				...
				if (IsFakeClient(iCid)==false)
					PrintHintText(iCid,"肾上腺: %t", "AdrenalGlandsHint");
				return ;
			}
		//check for 重型卡车技能
		case 2:
			{
				...
				if (IsFakeClient(iCid)==false)
					PrintHintText(iCid,"重型卡车: %t", "JuggernautHint");
		...
		case 3:
			{
				...
				if (IsFakeClient(iCid)==false)
					PrintHintText(iCid,"新陈代谢加速: %t", "MetabolicHint");
				return ;
			}
		...
		case 4:
			{
				...
				SetCommandFlags("z_spawn", iflags);
				if (IsFakeClient(iCid)==false)
					PrintHintText(iCid,"风暴巫术: %t", "StormCallerHint");
		...
		&& g_iConfirm[iCid]==1)
	{
		...
		if (IsFakeClient(iCid)==false)
			PrintHintText(iCid,"双重麻烦: %t", "DoubleTroubleHint1");
		return ;
	}
...
//======================================
//	CHAT CHECK, TOP MENU, SELECT SUBMENU 聊天检查，顶部菜单，选择子菜单。
//======================================
...
//build initial menu  构建初始菜单
public Handle:Menu_Initial (iCid)
{
	new Handle:menu = CreatePanel();
	decl String:stPanel[128];
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052");

	//"This server is using Perkmod" 此服务器使用Perkmod
	Format(stPanel, 128, "%t", "InitialMenuPanel1");
	DrawPanelText(menu, stPanel);
	//"Select option 1 to customize your perks" 选择选项1自定义您的津贴。
	Format(stPanel, 128, "%t", "InitialMenuPanel2");
	DrawPanelText(menu, stPanel);
	//"Customize Perks" 定制津贴
	Format(stPanel, 128, "%t", "InitialMenuPanel3");
	DrawPanelItem(menu, stPanel);

	//random perks, enable only if cvar is set 随机津贴，只有当cvar被设置时才启用。
	...
	{
		//"You can opt to randomize your perks" 你可以选择随机分配你的津贴
		Format(stPanel, 128, "%t", "InitialMenuPanel4");
		DrawPanelText(menu, stPanel);
		//"but you can't change them afterwards" 但你不能在事后改变它们
		Format(stPanel, 128, "%t", "InitialMenuPanel5");
		DrawPanelText(menu, stPanel);
		//"Randomize Perks" 随机津贴
		Format(stPanel, 128, "%t", "InitialMenuPanel6");
		DrawPanelItem(menu, stPanel);
	}

	//"Otherwise, you can use whatever" 否则，你可以使用任何东西。
	Format(stPanel, 128, "%t", "InitialMenuPanel7");
	DrawPanelText(menu, stPanel);
	//"perks you've selected already" 津贴你已经选定了
	Format(stPanel, 128, "%t", "InitialMenuPanel8");
	DrawPanelText(menu, stPanel);
	//"by using option 3" 通过使用选项3
...
//build top menu 建立顶部菜单
public Handle:Menu_Top (iCid)
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - 主菜单");
	DrawPanelText(menu,"子菜单选择一个技能");
	decl String:st_perk[32];
	decl String:st_display[MAXPLAYERS+1];

	//set name for sur1 perk 设置sur1 技能的名称。
	if (g_iSur1[iCid]==1
		&& (g_iStopping_enable==1		&&	g_iL4D_GameMode==0
		|| g_iStopping_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iStopping_enable_vs==1		&&	g_iL4D_GameMode==2))
...
//		st_perk="不设置";
	...
	Format(st_display,64,"幸存者 - 初选 (%s)",st_perk);
	if (g_iSur1_enable==1)
...
//		DrawPanelItem(menu,"关闭", ITEMDRAW_NOTEXT);
	...
	//set name for sur2 perk 设置sur2 技能的名称
	if (g_iSur2[iCid]==1
		&& (g_iUnbreak_enable==1		&&	g_iL4D_GameMode==0
		|| g_iUnbreak_enable_sur==1		&&	g_iL4D_GameMode==1
		|| g_iUnbreak_enable_vs==1		&&	g_iL4D_GameMode==2))
...
//	Format(st_display,64,"Survivor - Secondary (%s)", st_perk);
	Format(st_display,64,"幸存者 - 二级 (%s)", st_perk);
	if (g_iSur2_enable==1)
		DrawPanelItem(menu,st_display);
	else
//		DrawPanelItem(menu,"关闭", ITEMDRAW_NOTEXT);
	...
	//set name for sur3 perk 设置sur3 perk的名称。
	if (g_iSur3[iCid]==1
		&& (g_iPack_enable==1		&&	g_iL4D_GameMode==0
		|| g_iPack_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iPack_enable_vs==1		&&	g_iL4D_GameMode==2))
	...
	Format(st_display,64,"幸存者 - 三级 (%s)", st_perk);
	if (g_iSur3_enable==1)
		DrawPanelItem(menu,st_display);
	else
		DrawPanelItem(menu,"关闭", ITEMDRAW_NOTEXT);
...
//choose a submenu from top perk menu 从顶部选项菜单中选择一个子菜单
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - Main Menu");
	DrawPanelText(menu,"选择子菜单来确定技能");
	decl String:st_perk[32];
	decl String:st_display[MAXPLAYERS+1];
	decl iPerk;

	//set name for inf1 perk 为inf1 perk设置名称。
	iPerk = g_iInf1[iCid];
	if (iPerk==1
		&& g_iBarf_enable==1)
		st_perk="呕吐袋";
	...
	DrawPanelText(menu,"为了让你的技能发挥作用");
	DrawPanelText(menu,"你必须点击“完成”");
	DrawPanelItem(menu,"完成");
	return menu;
}
...
//menu for confirming perk choices 确认perk选项的菜单。
...
public Handle:Menu_ShowChoices (iCid)
{
	...
	if (iPerk == 1
		...
		|| g_iStopping_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iStopping_enable_vs==1		&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"阻止技能 (+%i%% %t)", RoundToNearest(g_flStopping_dmgmult*100), "BonusDamageText" );
	else if (iPerk == 2
		&& (g_iDT_enable==1		&&	g_iL4D_GameMode==0
		|| g_iDT_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iDT_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"双发快射 (%t, %t, %t)", "DoubleTapDescriptionPanel", "SleighOfHandDescriptionPanel", "DoubleTapRestrictionWarning" ) ;
	else if (iPerk == 3
		&& (g_iSoH_enable==1		&&	g_iL4D_GameMode==0
		|| g_iSoH_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iSoH_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"熟练手法 (%t +%i%%)", "SleighOfHandDescriptionPanel", RoundToNearest(100 * ((1/g_flSoH_rate)-1) ) ) ;
	else if (iPerk == 4
		&& (g_iPyro_enable==1		&&	g_iL4D_GameMode==0
		|| g_iPyro_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iPyro_enable_vs==1		&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"烟火专家 (%t)", "PyroDescriptionPanel");
	else
		Format(st_perk,128,"%t", "不设置");

	if (g_iSur1_enable==1)
	{
		DrawPanelItem(menu,"幸存者, 初级:");
	...
	if (iPerk == 1
		...
		|| g_iUnbreak_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iUnbreak_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"九尾狐之命 (+%i %t)", g_iUnbreak_hp, "UnbreakableHint");
	else if (iPerk == 2
		&& (g_iSpirit_enable==1		&&	g_iL4D_GameMode==0
		|| g_iSpirit_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iSpirit_enable_vs==1	&&	g_iL4D_GameMode==2))
		...
		|| g_iHelpHand_enable_vs==1		&&	g_iL4D_GameMode==2))
	{
		...
		if (g_iL4D_GameMode==2)
			...
			iBuff=g_iHelpHand_buff;
		if (g_iHelpHand_convar==1)
			Format(st_perk,128,"东郭先生 (%t +%i)", "HelpingHandDescriptionPanel2", iBuff);
		else
			Format(st_perk,128,"东郭先生 (%t +%i)", "HelpingHandDescriptionPanel", iBuff);
	}
	else if (iPerk == 4
		&& (g_iMA_enable==1		&&	g_iL4D_GameMode==0
		|| g_iMA_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iMA_enable_vs==1	&&	g_iL4D_GameMode==2))
	{
		if (g_iMA_maxpenalty < 6)
			Format(st_perk,128,"武术家 (%t)", "MartialArtistDescriptionPanel");
		else
			Format(st_perk,128,"武术家 (%t)", "MartialArtistDescriptionPanel_noreduc");
	}
	else
		Format(st_perk,128,"%t", "不设置");
	...
	if (iPerk == 1
		...
		|| g_iPack_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iPack_enable_vs==1		&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"收藏瘾 (%t +%i%%)", "PackRatDescriptionPanel", RoundToNearest(g_flPack_ammomult*100) );
	else if (iPerk == 2
		&& (g_iChem_enable==1		&&	g_iL4D_GameMode==0
		|| g_iChem_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iChem_enable_vs==1		&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"瘾君子 (%t +%i)", "ChemReliantDescriptionPanel", g_iChem_buff);
	else if (iPerk == 3
		&& (g_iHard_enable==1			&&	g_iL4D_GameMode==0
		|| g_iHard_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iHard_enable_vs==1		&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"九尾狐之命 (+%i%% %t)", RoundToNearest(g_flHard_hpmult*100), "HardToKillDescriptionPanel" );
	else if (iPerk == 4
		&& (g_iExtreme_enable==1		&&	g_iL4D_GameMode==0
		|| g_iExtreme_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iExtreme_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"惊弓之鸟 (+%i%% %t)", RoundToNearest(g_flExtreme_rate*100-100), "MartialArtistDescriptionPanelCoop" );
	else if (iPerk == 5
		&& (g_iLittle_enable==1		&&	g_iL4D_GameMode==0
		|| g_iLittle_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iLittle_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"少年棒球队 (%t)", "LittleLeaguerDescriptionPanel" );
	else
		Format(st_perk,128,"%t", "不设置");

	if (g_iSur2_enable==1)
	{
		DrawPanelItem(menu,"幸存者, 三级:");
		DrawPanelText(menu,st_perk);
	}
...
{
	new Handle:menu=CreatePanel();
	SetPanelTitle(menu,"技能系统 - v2.2 汉化joyist - QQ群706789052: Your perks for this round");
	decl String:st_perk[128];
	...
	{
		Format(st_perk,128,"Boomer: %t", "不设置");
		stDesc = "";
	}
	...
	{
		Format(st_perk,128,"Smoker: %t", "不设置");
		stDesc = "";
	}
	...
	{
		Format(st_perk,128,"Hunter: %t", "不设置");
		stDesc = "";
	}
	if (g_iInf4_enable==1)
	{
	...
	{
		Format(st_perk,128,"Jockey: %t", "不设置");
		stDesc = "";
	}
	...
	{
		Format(st_perk,128,"Spitter: %t", "不设置");
		stDesc="";
	}
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - Survivor: Primary");
	decl String:st_display[MAXPLAYERS+1];
	...
	else
	{
		...
		{
			case 1: st_current="(当前的)";
			default: st_current="";
		}
		Format(st_display,64,"阻止本领 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		switch (g_iSur1[client])
		{
			case 2: st_current="(当前的)";
			default: st_current="";
		}
		Format(st_display,64,"双发快射 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		switch (g_iSur1[client])
		{
			case 3: st_current="(当前的)";
			default: st_current="";
		}
		Format(st_display,64,"熟练手法 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		switch (g_iSur1[client])
		{
			case 4: st_current="(当前的)";
			default: st_current="";
		}
		Format(st_display,64,"烟火专家 %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - Survivor: Secondary");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
	...
	else
	{
		...
		Format(st_display,64,"牢不可破 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"东郭先生 %s",st_current);
		DrawPanelItem(menu,st_display);
		...
		else
		{
			...
			Format(st_display,64,"武术家 %s",st_current);
			DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - Survivor: Tertiary");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
	...
	else
	{
		...
		Format(st_display,64,"耗子偷窃 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"瘾君子 %s",st_current);
		DrawPanelItem(menu,st_display);
		if (g_iChem_buff > 0)
		{
	...
	else
	{
		...
		Format(st_display,64,"九尾狐之命 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"惊弓之鸟 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"少年棒球队 %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - Boomer");
	...
	else
	{
		...
		Format(st_display,64,"呕吐袋 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"晕车 %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - Tank");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
	...
	else
	{
		...
		Format(st_display,64,"肾上腺 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"重型卡车 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"代谢加速 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"风暴巫术 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		switch (g_iInf2[client])
		{
			case 5: st_current="(当前的)";
			default: st_current="";
		}
		Format(st_display,64,"加倍麻烦 %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - Smoker");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
	...
	else
	{
		...
		Format(st_display,64,"巧舌如簧 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"榨汁机 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"拖拽掉落 %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - Hunter");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
	...
	else
	{
		...
		Format(st_display,64,"饿虎扑食 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"杀手代号47 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"蚱蜢跳跃 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"速度恶魔 %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - Jockey");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
	...
	else
	{
		...
		Format(st_display,64,"乘风破浪 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"青蛙 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"幽灵骑士 %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - Spitter");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
	...
	else
	{
		...
		Format(st_display,64,"双喷发射 %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"粘鼠胶 %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "技能系统 - v2.2 汉化joyist - QQ群706789052 - Charger");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
	...
	else
	{
		...
		Format(st_display,64,"散射Ram %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"高速子弹 %s",st_current);
		DrawPanelItem(menu,st_display);
...
//create table if necessary 如果需要的话，创建一个表格
public InitDB()
{

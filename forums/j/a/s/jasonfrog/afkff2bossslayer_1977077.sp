...
public Action:Timer_SlayAFK(Handle:timer)
{
	if (g_boss_is_afk)
	{
		if (IsPlayerAlive(g_boss) && IsClientConnected(g_boss) && GetClientTeam(g_boss) == FF2_GetBossTeam())
		{
	...
	{
		if (IsPlayerAlive(g_boss_companion) && IsClientConnected(g_boss_companion) && GetClientTeam(g_boss_companion) == FF2_GetBossTeam())
		{

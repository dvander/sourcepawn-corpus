...
#define HHH             "models/bots/soldier_boss/bot_soldier_boss.mdl"
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_berapidsol", Command_Deflector, ADMFLAG_ROOT, "It's a good time to run");
...
stock GiveDeflector(client)
{
	...
	TF2Attrib_SetByName(weapon, "max health additive bonus", 3600.0);
	TF2Attrib_SetByName(weapon, "cannot be backstabbed", 1.0);
	TF2Attrib_SetByName(weapon, "ammo regen", 100.0);
   
	TF2Attrib_SetByName(weapon, "damage bonus", 0.75);
	...
	TF2Attrib_SetByName(weapon, "clip size upgrade atomic", 5.0);
	TF2Attrib_SetByName(weapon, "faster reload rate", 0.2);
	TF2Attrib_SetByName(weapon, "blast radius decreased", 1.2);
	TF2Attrib_SetByName(weapon, "projectile spread angle penalty", 4.0);
}

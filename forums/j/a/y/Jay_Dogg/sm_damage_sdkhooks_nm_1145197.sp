...
{
	name = "SM Damage - blanks",
	author = "SWAT_88, sdkhooks port by AtomicStryker, blanks by nitemare",
	description = "Individual damage settings for each Weapon. Plus BLANKS",
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_blanks", CmdDamageSet0, ADMFLAG_CHEATS);	
	RegAdminCmd("sm_unblank", CmdDamageSet1, ADMFLAG_CHEATS);	
	RegAdminCmd("sm_bfg", CmdDamageSet2, ADMFLAG_CHEATS);
	RegAdminCmd("sm_rubberbullets", CmdDamageSet3, ADMFLAG_CHEATS);
...
public Action:CmdDamageSet0(client, args)
{
	decl String:player[256];
	decl String:multiplier[20];
	...
	if (args == 1)
	{
		...
		FloatToString(0.0001, multiplier, sizeof(multiplier));
	...
	{
		ReplyToCommand(client, "Usage: sm_blanks <player>");
	}
	
	return Plugin_Handled;
}

public Action:CmdDamageSet1(client, args)
{
	decl String:player[256];
	decl String:multiplier[20];
	...
	if (args == 1)
	{
		...
		FloatToString(1.0, multiplier, sizeof(multiplier));
	...
	{
		ReplyToCommand(client, "Usage: sm_unblank <player>");
	}
	
	return Plugin_Handled;
}
public Action:CmdDamageSet2(client, args)
{
	decl String:player[256];
	decl String:multiplier[20];
	...
	if (args == 1)
	{
		...
		FloatToString(3.0, multiplier, sizeof(multiplier));
	...
	{
		ReplyToCommand(client, "Usage: sm_bfg <player>");
	}
	
	return Plugin_Handled;
}
public Action:CmdDamageSet3(client, args)
{
	decl String:player[256];
	decl String:multiplier[20];
	...
	if (args == 1)
	{
		...
		FloatToString(0.3, multiplier, sizeof(multiplier));
	...
	{
		ReplyToCommand(client, "Usage: sm_rubberbullets <player>");
	}

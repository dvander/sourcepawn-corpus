...
#define RTV 1 // Set to 0 to remove the rtv code, slight speed increase
...
public OnConfigsExecuted()
{
	...
	b_PickMapMed = true;
	b_PickMapLow = true;
	if (GetConVarInt(cvar_DisableHighTier) != 0 && GetConVarInt(cvar_DisableHighTier) == g_HighCount)
	{
		if (p_High == p_Low+1)
		{
			...
			SetConVarInt(cvar_HighPlayerCount, MAXPLAYERS+1);
			SetConVarInt(cvar_LowPlayerCount, 1);
		}
		g_HighCount = 0;
	}
...
public OnMapEnd()
{
	...
	#endif
	if (g_CycleRun == 3 && GetConVarInt(cvar_DisableHighTier) != 0)
	{
		g_HighCount++;
	}
...
RandomCycleCheck()
{
	...
	if (GetConVarFloat(cvar_RTV) > 0 && !b_rtv_Enable)
	{
		b_rtv_Enable = true;
	}
	#endif
	...
	if 	((p_Low == 0 && g_Players < p_High && p_High > p_Low && g_CycleRun != 4) || (p_High == 0 && g_Players > p_Low && p_Low > p_High && g_CycleRun != 4) || (p_High == 0 && p_Low == 0 && g_CycleRun != 4))
	{

...
public OnPluginStart()
{
	...
	Format(desc, sizeof(desc), "%t", "srvcln_logging_mode");
	cvar_logging = CreateConVar("sm_srvcln_logging_mode", "0", desc, FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	Format(desc, sizeof(desc), "%t", "srvcln_replays");
	cvar_type[RPY] = CreateConVar("sm_srvcln_replays", "0", desc, FCVAR_PLUGIN, true, 0.0, true, 1.0);
	
	Format(desc, sizeof(desc), "%t", "srvcln_roundbackups");
	cvar_type[RND] = CreateConVar("sm_srvcln_roundbackups", "0", desc, FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	cvar_time[LOG] = CreateConVar("sm_srvcln_logs_time", "168", desc, FCVAR_PLUGIN, true, -1.0);
	
	Format(desc, sizeof(desc), "%t", "srvcln_sprays_time");
	cvar_time[SPR] = CreateConVar("sm_srvcln_sprays_time", "168", desc, FCVAR_PLUGIN, true, -1.0);
	
	Format(desc, sizeof(desc), "%t", "srvcln_smlogs_time");
	cvar_time[SML] = CreateConVar("sm_srvcln_smlogs_time", "168", desc, FCVAR_PLUGIN, true, -1.0);
	
	Format(desc, sizeof(desc), "%t", "srvcln_demos_time");
	cvar_time[DEM] = CreateConVar("sm_srvcln_demos_time", "168", desc, FCVAR_PLUGIN, true, -1.0);
	
	Format(desc, sizeof(desc), "%t", "srvcln_replays_time");
	cvar_time[RPY] = CreateConVar("sm_srvcln_replays_time", "168", desc, FCVAR_PLUGIN, true, -1.0);
	
	Format(desc, sizeof(desc), "%t", "srvcln_roundbackups_time");
	cvar_time[RND] = CreateConVar("sm_srvcln_roundbackups_time", "168", desc, FCVAR_PLUGIN, true, -1.0);
...
{	
	...
	{
		GetConVarString(g_backuproundprefix, s_backuproundprefix, sizeof(s_backuproundprefix));
	}
	else if (type == RND)
	{
		filename = "\0";
	...
	if (type != RPY)
	{
		while (ReadDirEntry(h_dir, filename, sizeof(filename)))
		{
			...
			{
				if (StrContains(filename, s_backuproundprefix, false) == 0 && StrContains(filename, ".txt", false) == strLength-4)
				{
					CanDelete(Time32, TimeType, dir, filename, type, logging);
					continue;
				}
	...
	else
	{
		for (new i=0;i<=2;i++)
		{
			...
			{
				case 0:
					Format(dir, sizeof(dir), "replay/server/blocks");
				case 1:
					Format(dir, sizeof(dir), "replay/server/sessions");
				case 2:
					Format(dir, sizeof(dir), "replay/server/tmp");
			}
			...
			while (ReadDirEntry(h_dir, filename, sizeof(filename)))
			{
				...
				if (StrContains(filename, ".bz2", false) == strLength-4)
				{
					CanDelete(Time32, TimeType, dir, filename, type, logging);
					continue;
				}
...
CanDelete(const Time32, const TimeType, const String:dir[], const String:filename[], const type, const logging)
{
	#if DEBUG

...
public Plugin:myinfo = {
	name = "Custom Votes",
	author = "chundo, votemenu stripped by Wazz, bugfix by jameless",
...
public OnPluginStart() {
	...
	RegAdminCmd("sm_cvote", Command_CustomVote, ADMFLAG_GENERIC, "Initiate a vote", "customvotes", FCVAR_PLUGIN);
...
public Action:Command_CustomVote(client, args) {
	...
	//Check for lack of players to prevent crash	
	new players = 0;
	...
	if (players<1) {ReplyToCommand(client,"You cannot start a vote with no players in game"); return Plugin_Handled;}
	...
	for (new i = 0; i < s; ++i) {
		...
		if (CanInitiateVote(client, tvote[admin])) {
			if (IsVoteAllowed(client, i, false, errormsg, sizeof(errormsg)))
				...
				AddMenuItem(menu, tvote[name], votetitle, ITEMDRAW_DISABLED);

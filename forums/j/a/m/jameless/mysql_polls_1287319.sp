...
{
	name = "MySQL Polls",
	author = "Cep>|< - Russian BugTrack Group, crazydog, and Jameless",
...
new Handle:g_Connected = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_Connected = CreateConVar("sm_mysql_connected", "1800.0", "How long a player must be connected before being asked");
    }
...
{	
	if (action == MenuAction_Select)
	{
        ...
        if(IsFakeClient(client)) return;
        if(!GetClientAuthString(client, AuthStr, 32)) return;
        new String:info[32],String:name[128],String:ip[30],String:query[256];
        GetClientIP(client, ip, sizeof(ip));
        GetClientName(client, name, sizeof(name));
        GetMenuItem(menu, param2, info, sizeof(info));
	if(strcmp(info, "-1") != 0){
        	Format(query, sizeof(query), "INSERT INTO %s_votes (poll_id,variant,steam,ip,name,time) VALUES ('%i','%s','%s','%s','%s','%i')", sm_mysql_polls_prefix, client_current_poll[client], info, AuthStr, ip, name, GetTime());
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new connected = GetConVarInt(g_Connected);
	if(ff == 1) return true;
	...
	if (GetClientTime(client) < connected) return true;
	if(asked[client] < 2){

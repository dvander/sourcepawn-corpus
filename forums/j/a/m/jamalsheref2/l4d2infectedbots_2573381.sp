...
public OnPluginStart()
{
	...
	h_InfectedSpawnTime = CreateConVar("l4d2_infectedbots_spawn_time", "30.0", "Sets spawn time for special infected spawned by the plugin in seconds", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY);
	...
	h_JoinableTeams = CreateConVar("l4d2_infectedbots_infected_team_joinable", "0", "If 1, players can join the infected team in coop/survival", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY, true, 0.0, true, 1.0);
	...
	h_JoinableTeamsAnnounce = CreateConVar("l4d2_infectedbots_jointeams_announce", "0", "If 1, clients will be announced to about joining the infected team", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY, true, 0.0, true, 1.0);
	...
	h_idletime_b4slay = CreateConVar("l4d2_infectedbots_idle_time_before_slay", "999", "Amount of seconds before a special infected is slain", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY);
	h_timerafterhurt_b4slay = CreateConVar("l4d2_infectedbots_timer_hurt_before_slay", "999", "Amount of seconds a new timer will start for specials that get hurt before they are slain", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY);
...
public Action:evtRoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GameMode != 0)
	{
		...
		if (!b_HasRoundStarted)
		{
			...
			if (GameMode != 3)
			{
				CreateTimer(9.0, PlayerLeftStart); //?
			}
		}
	}
}
	...
	if (client)
	{
		if (GetClientTeam(client) == TEAM_INFECTED)
		{
			...
			if(!IsPlayerTank(client))
				...
				if(StrContains(ModelName, "hulk.mdl", true) != -1 ||

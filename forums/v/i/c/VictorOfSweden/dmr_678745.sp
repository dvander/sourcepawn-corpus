 ...
 * 
 * Minor modifications done by [SpA]VictorOfSweden 2008-08-30
 * =============================================================================
 * 
...
public Plugin:myinfo = {
    ...
    author = "Ryan \"FLOOR_MASTER\" Mannion and [SpA]VictorOfSweden",
...
public OnPluginStart() {
	...
	HookConVarChange(g_cvars[CVAR_FORCE_NEXTMAP], OnForceNextmapChange);
...
public Action:Command_ReloadDMR(client, args) {
    ...
    if (DetermineNextMap(dmr_map_key, nextmap_key, sizeof(nextmap_key), nextmap, sizeof(nextmap))) {
		Format(response, sizeof(response), "Reloaded DMR: %s. dmr_map_key is %s, nextmap_key is %s, nextmap is: %s.",
	    config, dmr_map_key, nextmap_key, nextmap);
    } else {
...
stock Handle:GetNextMaps(count=0) {
    ...
    if (HasForcedNextMap() && nextmaps_size) {
...
public OnForceNextmapChange(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	decl String:nextmap[MAX_KEY_LENGTH];
    decl String:start_key[MAX_KEY_LENGTH];
    decl String:nextmap_key[MAX_KEY_LENGTH];
...
stock bool:HasForcedNextMap(){
	decl String:forcenextmap[64];
	GetConVarString(g_cvars[CVAR_FORCE_NEXTMAP], forcenextmap, sizeof(forcenextmap));
	return (0 < strlen(forcenextmap));
}

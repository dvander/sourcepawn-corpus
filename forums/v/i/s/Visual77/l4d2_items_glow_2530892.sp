...
{ 
	...
	if (StrContains(gamemode, "scavenge", false) != -1)
	{
		...
		CreateTimer(10.0, delayScavengeStart);
	}
...
public Action hasEveryoneSpawned(Handle timer)
{
	...
	if (delayScavenge) return Plugin_Continue;
	...
	if (GetTotalSurvivors() >= survivorLimit.IntValue)
	{
		...
		SetGlowOnProp(footLocker, sizeof(footLocker), "prop_static", cvarGlowColor_footlocker, 3);
		SetGlowOnProp(medcabinet, sizeof(medcabinet), "prop_dynamic", cvarGlowColor_medcabinet, 2);
...
stock bool IsScavengeGascan(int iEntity)
{
	char sTempString[33];
	GetEntityClassname(iEntity, sTempString, sizeof(sTempString));
	if(sTempString[0] != 'w' && sTempString[0] != 'p')
        	return false;
    
    	if(!StrEqual(sTempString, "weapon_gascan") && !StrEqual(sTempString, "prop_physics"))
        	return false;

    	GetEntPropString(iEntity, Prop_Data, "m_ModelName", sTempString, sizeof(sTempString));
    	if(!StrEqual(sTempString, "models/props_junk/gascan001a.mdl", false))
        	return false;

    	switch(GetEntProp(iEntity, Prop_Send, "m_nSkin")) 
    	{
        	case 0: 
        	{
            		return false;

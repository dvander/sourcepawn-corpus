...
* Version 1.3.1: Ready for L4D1 version 1.0.1.2
*/
...
public OnCVGameModeChange(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	if (strcmp(oldValue, newValue) != 0 && (l4d_gamemode() == 1 || l4d_gamemode() == 2 || l4d_gamemode() == 3 || l4d_gamemode() == 4))
	{
		HookEvent("round_end", Event_RoundEnd);
		HookEvent("finale_win", Event_FinalWin);
		HookEvent("mission_lost", Event_FinalLost);
...
l4d_gamemode()
{
	// 1 - coop / 2 - VS / 3 - Mutation VS / 4 - Survival/ or false (thx DDR Khat for code)
	new String:gmode[32];

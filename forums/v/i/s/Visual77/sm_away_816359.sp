...
new Handle:survival = INVALID_HANDLE;
new Handle:coop = INVALID_HANDLE;
...
{
	name = "L4D Away",
	author = "Ivailosp, visual77 (based on the work of Ivailosp and djromero)",
	description = "Allows a player to take a break in survival/coop mode and spectate in versus",
	version = "1.0.0",
	url = "N/A"
};
...
public OnPluginStart()
{
	...
	h_AfkWarnSpecTime = CreateConVar("l4d_specafk_warnspectime", "7200", "Warn time before spec", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY, false, 0.0, false, 0.0);
	h_AfkSpecTime = CreateConVar("l4d_specafk_spectime", "7200", "time before spec (after warn)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY, false, 0.0, false, 0.0);
	h_AfkWarnKickTime = CreateConVar("l4d_specafk_warnkicktime", "7200", "Warn time before kick (while already on spec)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY, false, 0.0, false, 0.0);
	h_AfkKickTime = CreateConVar("l4d_specafk_kicktime", "7200", "time before kick (while already on spec after warn)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY, false, 0.0, false, 0.0);
	...
	versus = FindConVar("mp_gamemode");
	
        //Finding base game mode cvar
	survival = FindConVar("mp_gamemode");

        //Finding base game mode cvar
	coop = FindConVar("mp_gamemode");
...
public Action:cmd_spectate(client, args)
{
	...
	GetConVarString(versus, gamemode, sizeof(gamemode));
	if(StrEqual(gamemode, "versus", true)&&IsValidPlayer(client))
	{
	...
	if (client)
	{
		if (GetConVarBool(cvarAnnounce))
			CreateTimer(30.0, TimerAnnounce, client);
                ...
                if (GetConVarBool(cvarAnnounce))
			CreateTimer(30.0, TimerrAnnounce, client);

                if (GetConVarBool(cvarAnnounce))
			CreateTimer(30.0, CoopAnnounce, client);
						...
						if (!afkPlayerTrapped[i])
						{
							...
							if (afkPlayerTimeLeftWarn[i] > 0) // warn time ...
							{
								...
								if (afkPlayerTimeLeftWarn[i] <= 0)
								{
									...
									// We warn the player ....
									PrintToChat(i, "", afkPlayerTimeLeftAction[i]);
								}
							}
...
public Action:TimerAnnounce(Handle:timer, any:client)
{
	...
	//spectate works only in versus, therefore we announce
	decl String:gamemode[16];
	GetConVarString(versus, gamemode, sizeof(gamemode));
	if(StrEqual(gamemode, "versus", true)&&IsValidPlayer(client))	
	{
	if (IsClientInGame(client))
		...
		PrintToChat(client, "\x04[SM]\x03 Type !spectate if you need to go AFK");
                
	}
}

public Action:TimerrAnnounce(Handle:timer, any:client)
{

	//away works in survival mode so therefore we announce 
	decl String:gamemode[16];
	GetConVarString(survival, gamemode, sizeof(gamemode));
	if(StrEqual(gamemode, "survival", true)&&IsValidPlayer(client))	
	{
	if (IsClientInGame(client))
	
		PrintToChat(client, "\x04[SM]\x03 Type !away if you need to take a break");
                
	}
}

public Action:CoopAnnounce(Handle:timer, any:client)
{

	//away works in coop mode, so therefore we announce
	decl String:gamemode[16];
	GetConVarString(coop, gamemode, sizeof(gamemode));
	if(StrEqual(gamemode, "coop", true)&&IsValidPlayer(client))	
	{
	if (IsClientInGame(client))

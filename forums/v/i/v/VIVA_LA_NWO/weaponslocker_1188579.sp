...
public Plugin:myinfo = {
	name = "Weapons Locker",
	author = "Joshua \"VIVA LA NWO\"",
	description = "Provides ammo, health, metal, and uber commands. Based off of the_reverend's version, available here: http://forums.alliedmods.net/showthread.php?t=77261",
	version = PL_VERSION,
...
public Action:Command_uber(client, args) {
	if (GetConVarInt(cvarEnable))
	{
		...
		if( class != TFClass_Medic ) {
			ReplyToCommand( client, "[SM] Target is not a medic." );
...
public Action:Command_GiveAmmo(client, args) {
	...
	{
		ReplyToCommand( client, "[SM] This command only works on alive players." );
...
public Action:Command_GiveHealth(client, args)
{
	...
	if (args > 1)
	{
		...
		{
			ReplyToCommand( client, "[SM] Please enter a valid amount (0 - 500)" );
	...
	LogAction(client, target, "\"%L\" set health level on \"%L\" to %d", client, target, health);
...
public Action:Command_GiveMetal(client, args)
{
	...
	{
		ReplyToCommand( client, "[SM] Target is not an engineer." );
	...
	if (args > 1)
	{
		...
		{
			ReplyToCommand( client, "[SM] Please enter a valid amount (0 - 200)" );
	...
	LogAction(client, target, "\"%L\" set metal level on \"%L\" to %d", client, target, metal);
...
public Action:Command_GiveUber(client, args)
{
	...
	LogAction(client, target, "\"%L\" set uber level on \"%L\" to %d", client, target, uber);

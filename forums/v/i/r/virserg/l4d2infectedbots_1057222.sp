...
public Action:TankSpawner(Handle:timer, any:client)
{
	...
	if (GetConVarBool(h_CoopPlayableTank) && IndexCount != 0 )
	{
		...
		if (IsPlayerJockey(Index[tank]))
		{
			...
			new admindata = GetUserFlagBits(Index[tank]);
			
			if (FindConVar("sm_admin_cheats_version") != INVALID_HANDLE)
			{
				SetUserFlagBits(Index[tank], ADMFLAG_ROOT);
			}
			
			// enable the z_spawn command without sv_cheats
			new flags = GetCommandFlags("dismount");
			SetCommandFlags("dismount", flags & ~FCVAR_CHEAT);
			
			FakeClientCommand(Index[tank], "dismount");
			
			// restore z_spawn and user flags
			SetCommandFlags("dismount", flags|FCVAR_CHEAT);
			if (FindConVar("sm_admin_cheats_version") != INVALID_HANDLE)
			{
				SetUserFlagBits(Index[tank], admindata);
			}
		}

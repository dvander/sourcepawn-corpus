...
// Для Random Common - отдельный счетчик для каждого игрока
static int iCommonChoice[MAXPLAYERS+1] = {0, ...};
static int iLastValidModel[MAXPLAYERS+1] = {0, ...};
...
public void OnMapStart()
{
	...
	Handle hPrecacheCvar = FindConVar(sSharedCvarNames[6]);
	if(hPrecacheCvar != INVALID_HANDLE)
	{
		char sCvarString[4096];
		char sMap[PLATFORM_MAX_PATH];
		GetConVarString(hPrecacheCvar, sCvarString, sizeof(sCvarString));
...
void ePlayerBotReplace(Handle hEvent, const char[] sEventName, bool bDontBroadcast)
{
	...
	// Проверяем обоих клиентов
	if(iClient < 1 || iClient > MaxClients || iBot < 1 || iBot > MaxClients)
		return;

	if(!IsClientInGame(iClient) || !IsClientInGame(iBot))
		return;
	...
	iCommonChoice[iBot] = iCommonChoice[iClient];
	iLastValidModel[iBot] = iLastValidModel[iClient];
...
void ModelIndex(int iClient, int iCaseNum, bool bUsingMenu=false)
{
	...
	switch(iCaseNum)
	{
		...
		case 2:
		{
			// Используем индивидуальные счетчики для каждого игрока
			if(iCommonChoice[iClient] == 0)
				iCommonChoice[iClient] = GetRandomInt(0, COMMON_MODEL_PATH_SIZE-1);
			
			if(!IsModelValid(iClient, LMCModelSectionType_Common, iCommonChoice[iClient]))
			{
				if(IsModelValid(iClient, LMCModelSectionType_Common, iLastValidModel[iClient]))
					LMC_L4D1_SetTransmit(iClient, LMC_SetClientOverlayModel(iClient, sCommonPaths[iLastValidModel[iClient]]));
			}
			else
			{
				LMC_L4D1_SetTransmit(iClient, LMC_SetClientOverlayModel(iClient, sCommonPaths[iCommonChoice[iClient]]));
				iLastValidModel[iClient] = iCommonChoice[iClient];
			}

			// Переход к следующей модели для этого игрока
			iCommonChoice[iClient]++;
			if(iCommonChoice[iClient] >= COMMON_MODEL_PATH_SIZE)
				iCommonChoice[iClient] = 0;
...
public void OnClientDisconnect(int iClient)
{
	...
	// Сбрасываем счетчики для Random Common
	iCommonChoice[iClient] = 0;
	iLastValidModel[iClient] = 0;

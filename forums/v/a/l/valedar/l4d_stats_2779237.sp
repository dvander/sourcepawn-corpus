...
public OnPluginStart()
{
    ...
    CreateConVar("sm_l4dstats_version", PLUGIN_VERSION, "L4D Stats Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
    ...
    cvar_HumansNeeded = CreateConVar("sm_l4dstats_minhumans", "2", "Minimum Human players before stats will be enabled");
    cvar_UpdateRate = CreateConVar("sm_l4dstats_updaterate", "90", "Number of seconds between Common Infected point earn announcement/update");
    cvar_AnnounceMode = CreateConVar("sm_l4dstats_announcemode", "2", "Chat announcment mode. 0 = Off, 1 = Player Only, 2 = Player Only w/ Public Headshots, 3 = All Public");
    cvar_MedkitMode = CreateConVar("sm_l4dstats_medkitmode", "0", "Medkit point award mode. 0 = Based on amount healed, 1 = Static amount");
    cvar_SiteURL = CreateConVar("sm_l4dstats_siteurl", "", "Community site URL, for rank panel display");
    cvar_RankOnJoin = CreateConVar("sm_l4dstats_rankonjoin", "1", "Display player's rank when they connect. 0 = Disable, 1 = Enable");
    cvar_SilenceChat = CreateConVar("sm_l4dstats_silencechat", "0", "Silence chat triggers. 0 = Show chat triggers, 1 = Silence chat triggers");
    cvar_DisabledMessages = CreateConVar("sm_l4dstats_disabledmessages", "1", "Show 'Stats Disabled' messages, allow chat commands to work when stats disabled. 0 = Hide messages/disable chat, 1 = Show messages/allow chat");

    // Game mode Cvars
    cvar_EnableCoop = CreateConVar("sm_l4dstats_enablecoop", "1", "Enable/Disable coop stat tracking");

    // Infected point Cvars
    cvar_Infected = CreateConVar("sm_l4dstats_infected", "1", "Base score for killing a Common Infected");
    cvar_Hunter = CreateConVar("sm_l4dstats_hunter", "2", "Base score for killing a Hunter");
    cvar_Smoker = CreateConVar("sm_l4dstats_smoker", "3", "Base score for killing a Smoker");
    cvar_Boomer = CreateConVar("sm_l4dstats_boomer", "5", "Base score for killing a Boomer");

    // Misc personal gain Cvars
    cvar_Pills = CreateConVar("sm_l4dstats_pills", "15", "Base score for giving Pills to a friendly");
    cvar_Medkit = CreateConVar("sm_l4dstats_medkit", "20", "Base score for using a Medkit on a friendly");
    cvar_SmokerDrag = CreateConVar("sm_l4dstats_smokerdrag", "5", "Base score for saving a friendly from a Smoker Tongue Drag");
    cvar_ChokePounce = CreateConVar("sm_l4dstats_chokepounce", "10", "Base score for saving a friendly from a Hunter Pounce / Smoker Choke");
    cvar_Revive = CreateConVar("sm_l4dstats_revive", "15", "Base score for Revive a friendly from Incapacitated state");
    cvar_Rescue = CreateConVar("sm_l4dstats_rescue", "10", "Base score for Rescue a friendly from a closet");
    cvar_Protect = CreateConVar("sm_l4dstats_protect", "3", "Base score for Protect a friendly in combat");

    // Team gain Cvars
    cvar_Tank = CreateConVar("sm_l4dstats_tank", "25", "Base team score for killing a Tank");
    cvar_Panic = CreateConVar("sm_l4dstats_panic", "25", "Base team score for surviving a Panic Event with no Incapacitations");
    cvar_BoomerMob = CreateConVar("sm_l4dstats_boomermob", "10", "Base team score for surviving a Boomer Mob with no Incapacitations");
    cvar_SafeHouse = CreateConVar("sm_l4dstats_safehouse", "10", "Base score for reaching a Safe House");
    cvar_Witch = CreateConVar("sm_l4dstats_witch", "10", "Base score for Not Disturbing a Witch");
    cvar_Campaign = CreateConVar("sm_l4dstats_campaign", "5", "Base score for Completing a Campaign");

    // Point loss Cvars
    cvar_FFire = CreateConVar("sm_l4dstats_ffire", "25", "Base score for Friendly Fire");
    cvar_FIncap = CreateConVar("sm_l4dstats_fincap", "75", "Base score for a Friendly Incap");
    cvar_FKill = CreateConVar("sm_l4dstats_fkill", "250", "Base score for a Friendly Kill");
    cvar_InSafeRoom = CreateConVar("sm_l4dstats_insaferoom", "5", "Base score for letting Infected in the Safe Room");
    cvar_Restart = CreateConVar("sm_l4dstats_restart", "100", "Base score for a Round Restart");
...
public Action:event_TankKilled(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    for (new i = 1; i <= maxplayers; i++)
    {
        if (IsClientConnected(i) && IsClientInGame(i) && !IsClientBot(i))
        {
            GetClientAuthId(i, AuthId_Steam2, iID, sizeof(iID));
...
public Action:event_GivePills(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    GetClientAuthId(Recepient, AuthId_Steam2, RecepientID, sizeof(RecepientID));

    decl String:GiverName[MAX_LINE_WIDTH];
    GetClientName(Giver, GiverName, sizeof(GiverName));
    decl String:GiverID[MAX_LINE_WIDTH];
    GetClientAuthId(Giver, AuthId_Steam2, GiverID, sizeof(GiverID));
...
public Action:event_RevivePlayer(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    GetClientAuthId(Savior, AuthId_Steam2, SaviorID, sizeof(SaviorID));
...
public Action:event_Award(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    GetClientAuthId(User, AuthId_Steam2, UserID, sizeof(UserID));
...
public DisplayRank(Handle:owner, Handle:hndl, const String:error[], any:data)
{
    ...
    new Float:HeadshotRatio = Headshots == 0 ? 0.00 : (float(Headshots) / float(Kills))*100;

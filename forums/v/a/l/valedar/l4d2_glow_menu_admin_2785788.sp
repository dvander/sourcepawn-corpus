...
public void Event_Player_Spawn(Event event, const char[] name, bool dontBroadcast)
{
    ...
    else if (GetClientTeam(client) == 2 && CheckFlag(client))
    {
        ReadCookies(client);
    }
}
...
public Action GetAura(int client, int id)
{
	if(!CheckFlag(client)) return Plugin_Stop;
	switch (id)
	{
		case 0:
		{
...
stock bool CheckFlag(int client)
{
   return (GetUserFlagBits(client) & ADMFLAG_CUSTOM1 || GetUserFlagBits(client) & ADMFLAG_ROOT || GetUserFlagBits(client) & ADMFLAG_KICK);
}
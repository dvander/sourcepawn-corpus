...
{
	name = "Self Help ",
	author = "Pan Xiaohai edit XXX 18+, medicines now have permanent health.",
	description = " ",
...
public OnPluginStart()
{
	...
	l4d_selfhelp_durtaion = CreateConVar("l4d_selfhelp_durtaion", "3.0", "self help duration");
...
ReviveClientWithKid(client)
{
	...
	new temphpoffset = FindSendPropInfo("CTerrorPlayer","m_healthBuffer");
	SetEntDataFloat(client, temphpoffset, GetConVarFloat(revivehealth)*0.0, true);
}
ReviveClientWithPills(client)
{
 
	//new propincapcounter = FindSendPropInfo("CTerrorPlayer", "m_currentReviveCount");
	//new count = GetEntData(client, propincapcounter, 2);
	//count++;
	//if(count>2)count=2;
	CheatCommand(client, "give", "health", "1");

//removed server printing, the game does it already
	//if you love it its in comments
//added timer on ff, updates only if a new person is shot
//broke up cvars for printing options
//changed the display of ff to reduce spam see cvars
...
new Handle:kill_timers[MAX_PLAYERS+1][3];
new kill_counts[MAX_PLAYERS+1][3];
...
public Action:Event_Player_Hurt(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	new userid = kill_counts[attacker_userid][2];
	kill_timers[attacker_userid][2] = CreateTimer(5.0, KillCountTimer, ((attacker_userid*10)+2));
	kill_counts[attacker_userid][2] = client_userid;
	...
	if ((ff_attack == 1 || ff_attack == 2) && (userid != client_userid))
	{

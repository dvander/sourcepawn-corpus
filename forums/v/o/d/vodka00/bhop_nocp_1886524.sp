...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_bhophelp", Cmd_help, "Command to open the help menu with all the commands");
	RegConsoleCmd("sm_bhop", Cmd_bhop, "Command to open the menu with all the commands");
...
public Action:Event_player_spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		nbr_scout[client] = 0;
		CPrintToChat(client, "\x04[%s]: \x01%t", PREFIX, "help_msg");
...
SendMenuBhop(client)
{
	if (IsPlayerAlive(client))
	{
		decl String:bhopmenu_scout[255];
		decl String:bhopmenu_lowgrav[255];
		decl String:bhopmenu_normal[255];
		decl String:bhopmenu_autobhop[255];
		decl String:bhopmenu_help[255];
		
		new Handle:menu = CreateMenu(MenuBhop);
		SetMenuTitle(menu, "  - Bhop Menu -  \n ");
		Format(bhopmenu_scout, sizeof(bhopmenu_scout), "%T", "bhopmenu_scout", client);
		AddMenuItem(menu, "scout", bhopmenu_scout);
		if (bhop_gravity == 1)
		{
			Format(bhopmenu_lowgrav, sizeof(bhopmenu_lowgrav), "%T", "bhopmenu_lowgrav", client);
			AddMenuItem(menu, "lowgrav", bhopmenu_lowgrav);
			Format(bhopmenu_normal, sizeof(bhopmenu_normal), "%T", "bhopmenu_normal", client);
			AddMenuItem(menu, "normal", bhopmenu_normal);
		}
		if (bhop_autobhop == 1)
		{
			Format(bhopmenu_autobhop, sizeof(bhopmenu_autobhop), "%T", "bhopmenu_autobhop", client);
			AddMenuItem(menu, "autobhop", bhopmenu_autobhop);
		}
		Format(bhopmenu_help, sizeof(bhopmenu_help), "%T", "bhopmenu_help", client);
		AddMenuItem(menu, "help", bhopmenu_help);
		SetMenuExitButton(menu, true);
		DisplayMenu(menu, client, 0);
	}
	else
...
public MenuBhop(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		if (param2 == 0)
		{
			...
			{
				if (nbr_scout[param1] < bhop_max_scout)
				{
					GivePlayerItem(param1, "weapon_scout");
					FakeClientCommand(param1, "use weapon_scout");
					nbr_scout[param1]++;
					CPrintToChat(param1, "\x04[%s]: \x01%t", PREFIX, "receive_scout");
				}
				else
				{
					CPrintToChat(param1, "\x04[%s]: \x01%t", PREFIX, "too_much_scout");
				}
				SendMenuBhop(param1);
			}
			else
			{
				CPrintToChat(param1, "\x04[%s]: \x01%t", PREFIX, "mustbe_alive");
			}
		}
		...
		if (bhop_gravity == 1 && bhop_autobhop == 0)
		{
			if (param2 == 3)
			{
				if (IsPlayerAlive(param1))
				{
					SetEntityGravity(param1, bhop_gravity_value);
					CPrintToChat(param1, "\x04[%s]: \x01%t", PREFIX, "lowgrav");
					SendMenuBhop(param1);
				}
				else
				{
			...
			else if (param2 == 4)
			{
				...
				CPrintToChat(param1, "\x04[%s]: \x01%t", PREFIX, "normalgrav");
				SendMenuBhop(param1);
			}
			else if (param2 == 5)
			{
		...
		else if (bhop_gravity == 0 && bhop_autobhop == 1)
		{
			if (param2 == 3)
			{
				if (autobhop_player[param1] == 1)
				{
					autobhop_player[param1] = 0;
					CPrintToChat(param1, "\x04[%s]: \x01%t", PREFIX, "autobhop_deactivated");
					SendMenuBhop(param1);
				}
				else if (autobhop_player[param1] == 0)
				{
					autobhop_player[param1] = 1;
					CPrintToChat(param1, "\x04[%s]: \x01%t", PREFIX, "autobhop_activated");
					SendMenuBhop(param1);
				}
			}
			else if (param2 == 4)

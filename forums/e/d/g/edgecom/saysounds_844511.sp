...
public OnPluginStart(){
	...
	if (GetGameType() == tf2){
		...
		HookEvent("teamplay_waiting_begins", Event_WaitStart);
...
public Action:Event_WaitStart(Handle:event,const String:name[],bool:dontBroadcast){
	runSoundEvent(event,"setupstart","setupstart");
	return Plugin_Continue;
}

public Action:Event_WaitEnd(Handle:event,const String:name[],bool:dontBroadcast){
...
public Action:Event_Kill(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if (GetGameType() == tf2)
	{
		...
		if (custom_kill == 1)
		{
			runSoundEvent(event,"crit_kill",wepstring);
			return Plugin_Continue;
		}
		if (custom_kill == 2)
		{

...
*   Edits by:   Edison1318
*	Descrp	:	Added the Zoey talkable for Fake Zoey.
*   Link    :   "https://forums.alliedmods.net/showpost.php?p=2514688&postcount=89"
...
{  
	name = PLUGIN_NAME,  
	author = "DeatChaos25, Mi123456, Edison1318 & Merudo",  
	description = "Select a survivor character by typing their name into the chat.",  
	version = PLUGIN_VERSION,
	url = "https://forums.alliedmods.net/showpost.php?p=2514688&postcount=89"
}  
...
{  
	...
	HookEvent("defibrillator_used", Defibed);
	HookEvent("survivor_rescued", Rescued);
	HookEvent("player_spawn", Event_PlayerSpawn, EventHookMode_Post);
	HookEvent("player_bot_replace", Event_PlayerToBot, EventHookMode_Post);
	HookEvent("item_pickup", FIXBOTS);
	...
	convarZoey 		 = CreateConVar("l4d_scs_zoey", "0","Prop for Zoey. 0: FakeZoey (windows), 1: Rochelle (windows), 2: Zoey (linux), 3: Nick (fakezoey)",FCVAR_NOTIFY,true, 0.0, true, 3.0);
...
int GetZoeyProp()
{
	if 		(convarZoey.IntValue == 3) return NICK;			// For use with fakezoey for windows
	else if (convarZoey.IntValue == 2) return ZOEY;			// Linux only, or crashes the game
	else if (convarZoey.IntValue == 1) return ROCHELLE;		// For windows without fakezoey
	else							   return UNKNOWN;		// For Fake Zoey
}

public Action ZoeyUse(int client, int args)  
{  
	SurvivorChange(client, GetZoeyProp(), MODEL_ZOEY, "Zoey"); 
	if(convarZoey.IntValue == 0)
	{
...
{ 
	...
	{ 
		...
		{ 
			...
			{
				case NICK:        {    SurvivorChange(g_iSelectedClient[client],          NICK, MODEL_NICK,    "Nick",false); AcceptEntityInput(g_iSelectedClient[client], "ClearContext");  DispatchKeyValue(g_iSelectedClient[client], "targetname", "Gambler"); }     
				case ROCHELLE:    {    SurvivorChange(g_iSelectedClient[client],      ROCHELLE, MODEL_ROCHELLE,"Rochelle",false); AcceptEntityInput(g_iSelectedClient[client], "ClearContext"); DispatchKeyValue(g_iSelectedClient[client], "targetname", "Producer");}     
				case COACH:       {    SurvivorChange(g_iSelectedClient[client],         COACH, MODEL_COACH,   "Coach",false);   AcceptEntityInput(g_iSelectedClient[client], "ClearContext"); DispatchKeyValue(g_iSelectedClient[client], "targetname", "Coach");}
				case ELLIS:       {    SurvivorChange(g_iSelectedClient[client],         ELLIS, MODEL_ELLIS,   "Ellis",false);   AcceptEntityInput(g_iSelectedClient[client], "ClearContext"); DispatchKeyValue(g_iSelectedClient[client], "targetname", "Mechanic");} 
				case BILL:        {    SurvivorChange(g_iSelectedClient[client],          BILL, MODEL_BILL,    "Bill",false);    AcceptEntityInput(g_iSelectedClient[client], "ClearContext"); DispatchKeyValue(g_iSelectedClient[client], "targetname", "NamVet");}
				case ZOEY:        
				{    
					SurvivorChange(g_iSelectedClient[client], GetZoeyProp(), MODEL_ZOEY,    "Zoey",false);    
					if(convarZoey.IntValue == 0)
					{
						SetVariantString("who:TeenGirl:0");
						DispatchKeyValue(g_iSelectedClient[client], "targetname", "TeenGirl");
						AcceptEntityInput(g_iSelectedClient[client], "AddContext");
						return;
					}
					DispatchKeyValue(g_iSelectedClient[client], "targetname", "");
					AcceptEntityInput(g_iSelectedClient[client], "ClearContext");
				}  
				case FRANCIS:     {    SurvivorChange(g_iSelectedClient[client],       FRANCIS, MODEL_FRANCIS, "Francis",false); AcceptEntityInput(g_iSelectedClient[client], "ClearContext"); DispatchKeyValue(g_iSelectedClient[client], "targetname", "Biker");}  
				case LOUIS:       {    SurvivorChange(g_iSelectedClient[client],         LOUIS, MODEL_LOUIS,   "Louis", false);   AcceptEntityInput(g_iSelectedClient[client], "ClearContext"); DispatchKeyValue(g_iSelectedClient[client], "targetname", "Manager");}
			} 
		} 
		case MenuAction_Cancel: { } 
...
public void Event_PlayerSpawn(Handle event, const char[] name, bool dontBroadcast)
{
	...
	else if(IsFakeZoey(client) && IsFakeClient(client))
	{
...
public Action Defibed(Handle event, char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "subject"));
	if(IsFakeZoey(client) && IsFakeClient(client))
	{
		SetClientInfo(client, "name", "Zoey");
	}
}

public Action Rescued(Handle event, char[] name, bool dontBroadcast)
{
	...
	int allDM = -1; 
	while ((allDM = FindEntityByClassname(allDM, "survivor_death_model")) != -1) 
	{ 
		int zoeyDM = GetEntProp(allDM, Prop_Data, "m_nModelIndex"); 
		if (zoeyDM == PrecacheModel("models/survivors/survivor_teenangst.mdl", true)) 
		{ 
			AcceptEntityInput(allDM, "kill");
		} 
	}
}
...
public Action FIXBOTS(Handle event, char[] name, bool dontBroadcast)
{
	CreateTimer(0.1, FixBots);
}

public Action FixBots(Handle timer)
{
	for (int i = 1; i < MaxClients; i++)
	{
		if(IsSurvivor(i))
		{
			if(IsFakeZoey(i))
			{
				...
				{
					SetVariantString("who:TeenGirl:0");
					DispatchKeyValue(i, "targetname", "TeenGirl"); 
...
stock bool IsFakeZoey(int client)
{
	if (IsSurvivor(client))
	{
		char model[128];
		GetClientModel(client, model, sizeof(model));
		if (StrEqual(model, MODEL_ZOEY))
		{
			return true;
		}
	}

...
{
	name = "[L4D2] Grenade Throwing Bots",
...
public Action LookForGrenades(Handle timer, any entity)
{
	...
	char entType[32], entModel[128];
	
	GetEntityClassname(entity, entType, sizeof(entType));
	GetEntPropString(entity, Prop_Data, "m_ModelName", entModel, sizeof(entModel));
	
	if (StrEqual(entType, "weapon_molotov", false) || StrEqual(entType, "weapon_molotov_spawn", false) || StrEqual(entModel, "models/w_models/weapons/w_eq_molotov.mdl", false) || 
		StrEqual(entType, "weapon_pipe_bomb", false) || StrEqual(entType, "weapon_pipe_bomb_spawn", false) || StrEqual(entModel, "models/w_models/weapons/w_eq_pipebomb.mdl", false) || 
		StrEqual(entType, "weapon_vomitjar", false) || StrEqual(entType, "weapon_vomitjar_spawn", false) || StrEqual(entModel, "models/w_models/weapons/w_eq_bile_flask.mdl", false))
	{
...
public Action ReadyThrow(Handle timer, any client)
{
	...
	ChangeToGrenade(client, _, true, true);
	shootOrder[client] = 1;
	CreateTimer(GetRandomFloat(2.0, 4.5), DelayThrow, client);
	return Plugin_Stop;
}
...
public Action ThrowGrenade(Handle timer, any client)
{
	...
	ChangeToGrenade(client, true, true, true);
	shootOrder[client] = 1;
	return Plugin_Stop;
}
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	if (client > 0 && client <= MaxClients && IsClientInGame(client) && GetClientTeam(client) == 2 && IsPlayerAlive(client) && IsFakeClient(client))
	{
		...
		if (throwables != -1 && IsValidEntity(throwables))
		{
			...
			if (throwables == GetPlayerWeaponSlot(client, 2) && (StrEqual(activeEnt, "weapon_molotov") || StrEqual(activeEnt, "weapon_pipe_bomb") || StrEqual(activeEnt, "weapon_vomitjar")))
			{
				if (shootOrder[client] == 1)
				{
					buttons |= IN_ATTACK;
...
public Action L4D2_OnFindScavengeItem(int client, int item)
{
	if (!item)
	{
		...
		int grenadeWeapon = GetPlayerWeaponSlot(client, 2);
		if (!IsValidEdict(grenadeWeapon))
		{
			for (int i = 0; i < throwablesFound.Length; i++)
			{
...
public Action OnPlayerHurt(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (dangerousEnt >= 15 && ChangeToGrenade(damaged, true))
	{
		...
		CreateTimer(GetRandomFloat(0.8, 2.0), DelayThrow, damaged);
	}
	...
	if (fMobPos[0] != 0.0 || fMobPos[1] != 0.0 || fMobPos[2] != 0.0)
	{
		for (int i = 1; i <= MaxClients; i++)
		{
			if (IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i) && IsFakeClient(i) && IsInShape(i) && i != chosenThrower && i != lastChosen[2])
			{
				...
				CreateTimer(GetRandomFloat(0.0, 3.5), ReadyThrow, lastChosen[2]);
				break;
			}
		}
	}
...
stock bool:HasPipeBombAndMolotov(client)
{
	new String:pistol[32];
	if (IsValidEdict(GetPlayerWeaponSlot(client, 2)))
	{
		GetEdictClassname(GetPlayerWeaponSlot(client, 2), pistol, sizeof(pistol));
		if (StrEqual(pistol, "weapon_pipe_bomb") || StrEqual(pistol, "weapon_molotov"))
		{
			return true;
		}
	}

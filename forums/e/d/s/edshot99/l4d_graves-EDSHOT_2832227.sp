...
Handle g_hGravesItem	 	= INVALID_HANDLE;
...
public void OnPluginStart()
{
	...
	g_hGravesItem	 	= CreateConVar("l4d_graves_items", "0", "Toggle spawning an item at random when grave is destroyed.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
public Action Timer_SpawnGrave(Handle timer, DataPack corpse)
{
	...
	graveorigin[0] = origin[0];
	graveorigin[1] = origin[1];
	graveorigin[2] = origin[2];
...
public Action Timer_RemoveGrave(Handle timer, DataPack entities)
{
	...
	if ( GetConVarBool(g_hGravesItem) && !GetConVarBool(g_hGraveNotSolid) && GetConVarInt(g_hGraveHealth) > 0 && !IsValidEntity(grave) )
	{
		if ( IsL4D1() )
		{
			int item = -1;
			...
			{
				case 0: item = -1;
				case 1: item = CreateEntityByName("weapon_first_aid_kit");
				case 2: item = CreateEntityByName("weapon_pain_pills");
			}
			...
			DispatchSpawn(item);
			TeleportEntity(item, graveorigin, NULL_VECTOR, NULL_VECTOR);
		}
		else
		{
			int item = -1;
			...
			switch (GetRandomInt(0, 4))
			{
				case 0: item = -1;
				case 1: item = CreateEntityByName("weapon_first_aid_kit");
				case 2: item = CreateEntityByName("weapon_defibrillator");
				case 3: item = CreateEntityByName("weapon_pain_pills");
				case 4: item = CreateEntityByName("weapon_adrenaline");
			}

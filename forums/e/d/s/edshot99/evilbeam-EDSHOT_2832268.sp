...
new Handle:Cvar_BeamMe = INVALID_HANDLE
new Handle:Cvar_BeamColor = INVALID_HANDLE
...
public OnPluginStart()
{
	...
	Cvar_BeamMe = CreateConVar("sm_beamme_enabled", "0", " Allow players to beam themselves", FCVAR_PLUGIN)
	Cvar_BeamColor = CreateConVar("sm_evilbeam_color", "0", " 0: Static | 1: Random | 2: Random (RGB)", FCVAR_PLUGIN)

	RegAdminCmd("sm_evilbeam", Command_EvilBeam, ADMFLAG_SLAY, "sm_evilbeam <#userid|name>")
	RegConsoleCmd("sm_beamme", Command_BeamMe)
...
public OnMapStart()
{
	if (GetConVarInt(Cvar_BeamColor) == 0)
	{
...
public Action:Timer_Beam(Handle:timer, any:client)
{
	...
	if (GetConVarInt(Cvar_BeamColor) == 1)
	{
		TeamTwoColor[0] = GetRandomInt(0, 255)
		TeamTwoColor[1] = GetRandomInt(0, 255)
		TeamTwoColor[2] = GetRandomInt(0, 255)
		TeamTwoColor[3] = 255
		TeamThreeColor[0] = GetRandomInt(0, 255)
		TeamThreeColor[1] = GetRandomInt(0, 255)
		TeamThreeColor[2] = GetRandomInt(0, 255)
		TeamThreeColor[3] = 255
	}
	
	if (GetConVarInt(Cvar_BeamColor) == 2)
	{
		TeamTwoColor[0] = GetRandomInt(0, 1) ? 0 : 255
		TeamTwoColor[1] = GetRandomInt(0, 1) ? 0 : 255
		TeamTwoColor[2] = GetRandomInt(0, 1) ? 0 : 255
		TeamTwoColor[3] = 255
		TeamThreeColor[0] = GetRandomInt(0, 1) ? 0 : 255
		TeamThreeColor[1] = GetRandomInt(0, 1) ? 0 : 255
		TeamThreeColor[2] = GetRandomInt(0, 1) ? 0 : 255
		TeamThreeColor[3] = 255
	}
...
public Action:Command_BeamMe(client, args)
{
	...
	if (flags & ADMFLAG_ROOT || flags & ADMFLAG_VOTE )
	{
		...
		{
			PerformEvilBeam(-1, client)
		}
	}
	else if (GetConVarInt(Cvar_BeamMe))
	{
		if (IsClientInGame(client) && IsPlayerAlive(client))
		{
			PerformEvilBeam(-1, client)
	...
	{	
		PrintToChat(client, "[SM] Disabled.")
	}

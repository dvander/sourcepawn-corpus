...
new String:weaponAlias[][] = {
	...
	"Golden Wrench"	// add more aliases here, aliases are the names used on the admin menu. MUST BE PARALLEL TO UNLOCKDEFIDX AND REPLACEMENT ARRAY
};
...
new unlockDefIdx[] = {
	...
	142,
	155,
	169	// add more unlockable weapon indexes here, weapon indexes are used to indentify each weapon.
};
...
new String:replacementWeapon[][] = {
	...
	"tf_weapon_wrench",
	"tf_weapon_wrench",
	"tf_weapon_wrench"		// add more replacement weapons here, this is what the server gives the client instead of their unlock.
};

new Float:g_fClientUberPercent[MAXPLAYERS + 1];
new bool:g_bCanClientReceiveMsg[MAXPLAYERS+1];
new bool:g_clientIsEngiUsingGunslinger[MAXPLAYERS+1];
...
public Action:ProcessEdict(Handle:timer, any:edict)
{
	...
	new defIdx = GetEntProp(edict, Prop_Send, "m_iItemDefinitionIndex");
		...
		// Check if  is using Gunslinger for allowing use of Wrangler on Minisentries (If client is using wrench or golden wrench set to false)
	new client = GetEntPropEnt(edict, Prop_Send, "m_hOwnerEntity");
	if (defIdx == 142)  {
		g_clientIsEngiUsingGunslinger[client] = true;
	}
	else if(defIdx == 7 || defIdx == 169 || defIdx == 155){
		g_clientIsEngiUsingGunslinger[client] = false;
		ForceWeaponCheck();
	}
	...
	// Is this a Wrangler and the engi is using Gunslinger?
	if(g_clientIsEngiUsingGunslinger[client] == true && defIdx == 140)
		return Plugin_Handled;	

...
public Action:ProcessEdict(Handle:timer, any:edict)
{
	...
	if(defIdx != 221 && defIdx != 46 && defIdx != 163)
		return Plugin_Handled;
	...
	for (new x=0; x<11; x++)
	{
		if((weapon = GetPlayerWeaponSlot(client, x)) != -1)
		{
			...
			if(foundWeapon == 221 || foundWeapon == 46 || foundWeapon == 163)	
				hasWeapon++;
		}
	}
	...
	if(hasWeapon == 2)
	{
		...
		PrintCenterText(client, "Cannot use Mackerel together with Bonk Atomic Punch or Crit-A-Cola. Switch your loadout!");
		PrintToChat(client, "\x04[Unlock Problem!]:\x01 You cannot use the Holy Mackerel together with Bonk Atomic Punch or Crit-A-Cola due to stability issues!");
	}

...
public OnPluginStart()
{
	...
	hNotifyMin = CreateConVar("l4d_pdp_notify_min","1","The minimum pounce damage required to trigger a pd+ notification when l4d_pdp_notify is set to either 2 or 4",FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	hScaleNotify = CreateConVar("l4d_pdp_scalenotify","1","Whether or not clients will be notified when custom damage scaling is enabled (recommended, to avoid confusion)",FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY,true,0.0,true,1.0);
	hIncapNotify = CreateConVar("l4d_pdp_incapnotify","1","Whether or not clients will be notified when a pounce causes sufficient damage to carry-over into a survivor's incapped hp",FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY,true,0.0,true,1.0);
...
public Event_PlayerPounced(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	else
	{
		...
		if((totalHealth - cappedDmg) < 0 && dmgIncaps != 0)
		{
			...
			if(incapNotify != 0)
			{
				Format(incapText,sizeof(incapText),"\n\x01[pd+] \x03%i \x04hp of excess dmg applied to \x03%s's\x04 incapped health.",lateTotalDmg,victimName);
				Format(incapHint,sizeof(incapHint),"\n[pd+] %i hp of post-incap dmg done to %s",lateTotalDmg,victimName);
			}
...
//If a client is connecting for the first time [according to pdpwelcomereg.txt] then send them an info msg
...
public bool:PdpStatsInitialize()
{
	...
	RegConsoleCmd("l4d_pdp_statsme",PdpShowStatsMenu,"Displays your pd+ stats menu.");
	...
	RegServerCmd("l4d_pdp_clean_statsfile",PdpStatsCleanKeyfile_Cmd,"Use this to manually trigger the stats file to delete all visitors' stats that have expired.");
	RegServerCmd("l4d_pdp_deleteallstats",PdpStatsClearAll_Cmd,"Use this to clear all saved stats from the server and start fresh.");

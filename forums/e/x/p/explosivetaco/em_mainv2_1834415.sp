...
{
	RegConsoleCmd("hello", Command_Hello, "Type Hello");
	RegConsoleCmd("info", Command_PluginInfo, "plugin info");
	RegConsoleCmd("suicide", Command_KillMe, "Death Command");
	RegConsoleCmd("crash", Command_Delete, "Crash Command!");
	RegConsoleCmd("guns", Command_Guns, "Gun Command!");
	RegConsoleCmd("cmds", Command_CommandList, "Command List.");
}
...
public Action:Command_Guns(client,args)
{
	...
	{
		FakeClientCommand(client, "impulse101");
		return Plugin_Handled
	}
	return Plugin_Handled
}

...
#define PLUGIN_AUTHOR "daneleo modded by eyesofcreeper, Original Author noodleboy347"
...
public OnPluginStart()
{
	...
	RegConsoleCmd("perks", Command_Perks);
	...
	RegAdminCmd("premium_mysql_createDB", create_PremDatabase, ADMFLAG_ROOT);
	//////////////
	//C v a r s//
	////////////
	...
	cvar_fakekill = CreateConVar("premium_deadringer", "0", "Whether or not to detect Dead Ringer kills");
...
public Player_Spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if((IsValidEntity(client) && GetUserFlagBits(client) & PREMIUMFLAG || GetUserFlagBits(client) & ROOTFLAG))
	{
		Premium(client);
	}
...
public Action:Timer_Respawn(Handle:hTimer, any:client)
{
 if(ArenaMap == 1)
	{	
		CPrintToChat(client, "%s Arena Mod detected! Respawn disabled");
	}
 else
	{
...
public Action:OnPlayerRunCmd(iClient, &iButtons, &iImpulse, Float:fVelocity[3], Float:fAngles[3], &iWeapon)
{
	...
	if(!(GetUserFlagBits(iClient) & PREMIUMFLAG || pTrial[iClient] || GetUserFlagBits(iClient) & ROOTFLAG && cSpeed))
	return Plugin_Continue;
...
public OnGameFrame()
{
	...
	{
		if(cCloak && IsValidEntity(i) && (GetUserFlagBits(i) & PREMIUMFLAG || GetUserFlagBits(i) & ROOTFLAG)  && IsClientInGame(i) && IsPlayerAlive(i) )
		{
			...
			{ 
				...
				{ 
					...
					if( (strcmp(TempString,"tf_weapon_flamethrower") == 0) && cPyrofly && (GetClientButtons(i) & IN_ATTACK) && (GetUserFlagBits(i) & PREMIUMFLAG)) 
					{ 
...
public Action:Command_Perk(client, args)
{
	if(!client)
	{
		PrintToServer("[Premium] This command can only be used ingame!");

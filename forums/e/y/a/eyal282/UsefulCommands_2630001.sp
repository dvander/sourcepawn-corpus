...
public Native_GetWeaponStatsList(Handle:caller, numParams)
{
	...
	new StatID = GetNativeCell(2);
	
	if(StatID < 0 || StatID >= sizeof(wepStatsList[]))
		return false;
		
	SetNativeCellRef(3, wepStatsList[WeaponID][StatID]);
...
public OnPluginStart()
{
	...
	if(isCSGO())
	{
		...
		hcv_ucPacketNotifyCvars = CreateConVar("uc_packet_notify_cvars", "2", "If 2, acts like 1 but also deletes the gamerulescvars.txt file before doing it. If 1, UC will put all FCVAR_NOTIFY cvars in gamerulescvars.txt", FCVAR_NOTIFY);
		
		hcv_ucGlowType = CreateConVar("uc_glow_type", "1", "0 = Wallhack, 1 = Fullbody, 2 = Surround Player, 3 = Blinking and Surround Player");
...
{	
	...
	fw_ucWeaponStatsRetrievedPost = CreateGlobalForward("UsefulCommands_WeaponStatsRetrievedPost", ET_Ignore);
	...
	hcv_ucTag = CreateConVar("uc_tag", "[{red}UC{normal}] {normal}", _,FCVAR_PROTECTED);
...
public ShowPartyModeMenu(client)
{
	...
	switch(GetClientPartyMode(client))
	{
		...
		{
			Format(TempFormat, sizeof(TempFormat), "%T", "Party Mode Cookie Menu: Defuse Only");
		...
		{
			Format(TempFormat, sizeof(TempFormat), "%T", "Party Mode Cookie Menu: Enabled");
			AddMenuItem(hMenu, "", TempFormat);
		}
		
		default:
		{
			Format(TempFormat, sizeof(TempFormat), "%T", "Party Mode Cookie Menu: Disabled");
			AddMenuItem(hMenu, "", TempFormat);
		}
	}
...
{	
	if (args < 1)
	{
		...
		ReplyToCommand(client, "%s%t", UCTag, "Command Usage Target", arg0);
		return Plugin_Handled;
	}
	...
	ShowActivity2(client, UCTag, "%t", "Player Respawned", target_name);
...
{	
	...
	ShowActivity2(client, UCTag, "%t", "Player Hard Respawned", target_name);
...
{	
	if (args < 1)
	{
		...
		ReplyToCommand(client, "%s%t", UCTag, "Command Usage Target Toggle", arg0);
		return Plugin_Handled;
	}
	...
	for(new i=0;i < target_count;i++)
	{
		...
		if(bury)
		{
			...
			{
				ReplyToCommand(client, "%s%t", UCTag, "Already Buried", target);
				return Plugin_Handled;
			}	
		...
		else
		{
			if(!IsPlayerStuck(target))
			{
				if(target_count == 1)
				{
					ReplyToCommand(client, "%s%t", UCTag, "Already Not Buried", target);
					return Plugin_Handled;
				}
	...
	if(bury)
		ShowActivity2(client, UCTag, "%t", "Player Buried", target_name);
		
	else
		ShowActivity2(client, UCTag, "%t", "Player Unburied", target_name);
	...
	for(new i=0;i < target_count;i++)
	{
		...
		if(slap)
		{
			if(UberSlapped[target])
			{
				if(target_count == 1)
				{
					ReplyToCommand(client, "%s%t", UCTag, "Player Already Uberslapped", target);
					return Plugin_Handled;
				}
		...
		else
		{
			if(!UberSlapped[target])
			{
				if(target_count == 1)
				{
					ReplyToCommand(client, "%s%t", UCTag, "Player Already Not Uberslapped", target);
					return Plugin_Handled;
				}
		...
		if(slap)
			ShowActivity2(client, UCTag, "%t", "Player Uberslapped", target_name);
			
		else
			ShowActivity2(client, UCTag, "%t", "Player Stop Uberslap", target_name);
	}
	return Plugin_Handled;
}
...
public Action:Timer_UberSlap(Handle:hTimer, UserId)
{
	...
	if(TotalSlaps[client] == 100)
	{
		UberSlapped[client] = false;
		TIMER_UBERSLAP[client] = INVALID_HANDLE;
		PrintToChat(client, "%s\x02Uberslap has ended.\x04 Prepare your landing!", UCTag);
		return Plugin_Stop;
	}
...
public Action:Command_Heal(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "%s%t", UCTag, "Command Usage Heal");
		ReplyToCommand(client, "%s%t", UCTag, "Command Usage Note Heal");
		return Plugin_Handled;
	}
	...
	for(new i=0;i < target_count;i++)
	{
		...
		Format(ActivityBuffer, sizeof(ActivityBuffer), "%s%t", "Heal Admin Set", target);
		if(health != -1)
		{
...
public Action:Command_Give(client, args)
{
	if (args < 2)
	{
		new String:arg0[65];
		GetCmdArg(0, arg0, sizeof(arg0));
		ReplyToCommand(client, "%s%t", UCTag, "Command Usage Give", arg0);
		return Plugin_Handled;
	}
	...
	for(new count=0;count < target_count;count++)
	{
		...
		{
			ReplyToCommand(client, "%s%t", UCTag, "Command Give Invalid Weapon", WeaponName);
	...
	ShowActivity2(client, UCTag, "%t", "Player Given Weapon", WeaponName, target_name); 
...
public Action:Command_RestartRound(client, args)
{
	...
	if(SecondsBeforeRestart != 0.0)
	{
		...
		PrintToChatAll("%s%t", UCTag, "Admin Restart Round", client, iSecondsBeforeRestart, Arg);
	...
	else
	{
		...
		PrintToChatAll("%s%t", UCTag, "Admin Stopped Restart Round", client);
	}
	return Plugin_Handled;
}
...
public Action:Command_RestartGame(client, args)
{
	...
	if(SecondsBeforeRestart != 0)
	{
		...
		PrintToChatAll("%s%t", UCTag, "Admin Restart Game", client, SecondsBeforeRestart, Arg);
	}	
	else
	{
		if(isCSGO())
		...
		PrintToChatAll("%s%t", UCTag, "Admin Stopped Restart Game", client);
	}
	return Plugin_Handled;
}
...
public Action:Command_RestartServer(client, args)
{
	if(!Restart)
	{
		...
		PrintToChatAll("%s%t", UCTag, "Admin Restart Server", client, 5, Arg);
	}
	else
	{
		...
		PrintToChatAll("%s%t", UCTag, "Admin Stopped Restart Server", client);
	}
	Restart = !Restart;
...
public Action:Command_Glow(client, args)
{
	if (args < 1)
	{
		...
		ReplyToCommand(client, "%s%t", UCTag, "Command Usage Glow", arg0);
		return Plugin_Handled;
	}
	...
	{
		ReplyToCommand(client, "%s%t", UCTag, "Command Glow List");
	...
	if(glow)
	{
		for(new i=0;i < sizeof(GlowData);i++)
		{
			...
			{
				ReplyToCommand(client, "%s%t", UCTag, "Command Glow Invalid");
				return Plugin_Handled;
			}
		}
	}
	...
	for(new i=0;i < target_count;i++)
	{
		...
		if(glow)
		{
			...
			{
				ReplyToCommand(client, "%s%t", "Command Glow Failed to Give");
				return Plugin_Handled;
			}
		}
		else
		{
			...
			{
				ReplyToCommand(client, "%s%t", "Command Glow Failed to Remove", target);
				return Plugin_Handled;
			}	
		}
	}
	...
	if(glow)
		ShowActivity2(client, "%s%t", "Player Given Glow", target_name); 
		
	else
		ShowActivity2(client, "%s%t", "Player Removed Glow", target_name); 
...
public Action:Command_Blink(client, args)
{
	if (args < 1)
	{
		...
		ReplyToCommand(client, "%s%t", UCTag, "Command Usage Target");
		return Plugin_Handled;
	}
	...
	ShowActivity2(client, UCTag, "%t", "Player Blinked", target_name); 
...
public Action:Command_Godmode(client, args)
{
	...
	for(new i=0;i < target_count;i++)
	{
		...
		if(god)
		{
			...
			{
				ReplyToCommand(client, "%s%t", UCTag, "Already Godmode", target);
				return Plugin_Handled;
			}
		...
		else
		{
			if(!UC_GetClientGodmode(target) && target_count == 1)
			{
				ReplyToCommand(client, "%s%t", UCTag, "Already Not Godmode", target);
				return Plugin_Handled;
			}
	...
	if(god)
		ShowActivity2(client, UCTag, "%t", "Player Given Godmode", target_name);
		
	else
		ShowActivity2(client, UCTag, "%t", "Player Removed Godmode", target_name);
...
public Action:Command_Rocket(client, args)
{
	...
	if(rocket)
		ShowActivity2(client, UCTag, "Player Given Rocket", target_name); 
	
	else 
		ShowActivity2(client, UCTag, "Player Removed Rocket", target_name);
...
public Action:Command_Disarm(client, args)
{
	...
	ShowActivity2(client, UCTag, "Player Stripped", target_name);
...
public Action:Command_Exec(client, args)
{
	if (args < 2)
	{
		...
		ReplyToCommand(client, "%s%t", UCTag, "Command Usage Execute", arg0);
		return Plugin_Handled;
	}
	...
	ShowActivity2(client, UCTag, "Player Executed", ExecCommand, target_name);
...
public Action:Command_BruteExec(client, args)
{
	...
	ShowActivity2(client, UCTag, "Player Brutally Executed", ExecCommand, target_name);
...
public Action:Command_Money(client, args)
{
	if (args < 2)
	{
		...
		ReplyToCommand(client, "%s%t", UCTag, "Command Usage Amount", arg0);
		return Plugin_Handled;
	}
	...
	ShowActivity2(client, UCTag, "Player Set Money", target_name, money);
	return Plugin_Handled;
}
...
public Action:Command_Team(client, args)
{
	if (args < 2)
	{
		...
		ReplyToCommand(client, "%s%t", UCTag, "Command Usage Team", arg0);
		return Plugin_Handled;
	}
	...
	ShowActivity2(client, UCTag, "Player Set Team", target_name, TeamName);
...
public Action:Command_UCEdit(client, args)
{
	...
	if(UCEdit[client])
	{
		PrintToChat(client, "%s%t", UCTag, "Command UCEdit Enabled");
		PrintToChat(client, "%s%t", UCTag, "Command UCEdit Info");
	}	
	else
		PrintToChat(client, "%s%t", UCTag, "Command UCEdit Disabled");
	...
	else if(action == MenuAction_Select)
	{
		switch(item)
		{
			...
			case 2:
			{
				...
				if(Chicken == -1)
				{
					PrintToChat(client, "%s%t", UCTag, "Command Chicken Not Found");
					return;
				}
...
public SQLCB_DeleteChickenSpawnMenu(Handle:db, Handle:hndl, const String:sError[], data)
{
	...
	{
		PrintToChat(client, "%s%t", UCTag, "Command Chicken No Spawners");
		return;
	}
...
public SQLCB_ChickenSpawnDeleted(Handle:db, Handle:hndl, const String:sError[], data)
{
	...
	if(client != 0)
		PrintToChat(client, "%s%t", "Command Chicken Deleted");
...
public SQLCB_ChickenSpawnCreated(Handle:db, Handle:hndl, const String:sError[], Handle:DP)
{
	...
	if(hndl == null)
		...
		PrintToChat(client, "%s%t", UCTag, "Command Chicken Created");

...
public OnPluginStart()
{
	...
	cvarLockRoundOne = CreateConVar("anti-rush_doors_roundone", "35", "Time Applied To Lockdown Door", FCVAR_NOTIFY);
	cvarLockRoundTwo = CreateConVar("anti-rush_doors_roundtwo", "20", "Time Applied To Lockdown Door In Round Two", FCVAR_NOTIFY);
	...
	cvarLockGlowRange = CreateConVar("anti-rush_doors_glowrange", "200", "Range Door Glows", FCVAR_NOTIFY);
...
public Action:timerUnlock(Handle:timer)
{
	...
	if (GetConVarInt(cvarLockHintText) == 1)
	{
		...
		{
			PrintHintTextToAll("Door locked.\nPlease wait: %i", g_iCooldown);
		}
		else
		{
			PrintHintTextToAll("Door locked.\nPlease wait: %i", g_iCooldown);
...
public Action:DL_Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{

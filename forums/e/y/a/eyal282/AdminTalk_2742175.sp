...
{
	ShowMenuAdminTalk(client, 0);
	
	return Plugin_Handled;
}
public void ShowMenuAdminTalk(int client, int item)
{
...
public int MenuHandler_AdminTalk(Menu menu, MenuAction action, int client, int item)
{
	...
	else if(action == MenuAction_Select)
	{
		...
		ShowMenuAdminTalk(client, GetMenuSelectionPosition());
	}
}
...
void JoinAdminTalk(int client)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (!IsClientInGame(i) || (!CheckCommandAccess(i, "sm_admin", ADMFLAG_GENERIC) && !g_InTalk[i]) && i != client)continue;

...
public Action:Event_DeadSurvivorVisible(Handle:hEvent, const String:Name[], bool:dontBroadcast)
{
	new deadbody = GetEventInt(hEvent, "subject");
	new deadplayer = GetClientOfUserId(GetEventInt(hEvent, "deadplayer"));
	
	if(IsFakeClient(deadplayer))
		return;
		
	else if(GetClientTeam(deadplayer) != 2)
		return;
		
	else if(IsPlayerAlive(deadplayer))
		AcceptEntityInput(deadbody, "Kill");
}

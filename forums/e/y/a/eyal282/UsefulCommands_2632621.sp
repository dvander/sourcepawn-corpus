...
public OnPluginStart()
{
	...
	SetConVarString(UC_CreateConVar("uc_version", PLUGIN_VERSION, _, FCVAR_NOTIFY|FCVAR_DONTRECORD|FCVAR_PROTECTED), PLUGIN_VERSION);
...
public OnAllPluginsLoaded()
{
	...
	if(isCSGO())
	{
		...
		//hcv_HolidayMode = FindConVar("sv_holiday_mode");
...
public OnEntityCreated(entity, const String:Classname[])
{
	...
	#if defined TEST
	else if(StrEqual(Classname, "weapon_c4", true))
		SDKHook(entity, SDKHook_Spawn, Event_C4Spawn);
		SDKHook(entity, SDKHook_SpawnPost, Event_C4SpawnPost);
...
#if defined TEST
public Event_C4Spawn(entity)
{
	LastHolidayCvar = GetConVarInt(hcv_HolidayMode);
	SetConVarInt(hcv_HolidayMode, 2);
}

public Event_C4SpawnPost(entity)
{
	SetConVarInt(hcv_HolidayMode, LastHolidayCvar);
}
#endif
...
public Action:Command_Heal(client, args)
{
	...
	for(new i=0;i < target_count;i++)
	{
		...
		Format(ActivityBuffer, sizeof(ActivityBuffer), "%s%t", UCTag, "Heal Admin Set", target);
		if(health != -1)
		{
...
public Action:Command_Glow(client, args)
{
	...
	for(new i=0;i < target_count;i++)
	{
		...
		if(glow)
		{
			...
			{
				UC_ReplyToCommand(client, "%s%t", "Command Glow Failed to Give");
				return Plugin_Handled;
			}
		}
		else
		{
			...
			{
				UC_ReplyToCommand(client, "%s%t", "Command Glow Failed to Remove", target);
				return Plugin_Handled;
			}	
		}
	}
	...
	if(glow)
		UC_ShowActivity2(client, "%s%t", "Player Given Glow", target_name); 
		
	else
		UC_ShowActivity2(client, "%s%t", "Player Removed Glow", target_name); 
...
public Action:Command_Rocket(client, args)
{
	...
	if(rocket)
		UC_ShowActivity2(client, UCTag, "Player Given Rocket", target_name); 
	
	else 
		UC_ShowActivity2(client, UCTag, "Player Removed Rocket", target_name);
...
public Action:Command_Disarm(client, args)
{
	...
	UC_ShowActivity2(client, UCTag, "Player Stripped", target_name);
...
public Action:Command_Exec(client, args)
{
	...
	UC_ShowActivity2(client, UCTag, "Player Executed", ExecCommand, target_name);
...
public Action:Command_BruteExec(client, args)
{
	...
	UC_ShowActivity2(client, UCTag, "Player Brutally Executed", ExecCommand, target_name);
...
public Action:Command_Money(client, args)
{
	...
	UC_ShowActivity2(client, UCTag, "Player Set Money", target_name, money);
	return Plugin_Handled;
}
...
public Action:Command_Team(client, args)
{
	...
	UC_ShowActivity2(client, UCTag, "Player Set Team", target_name, TeamName);
...
public SQLCB_ChickenSpawnDeleted(Handle:db, Handle:hndl, const String:sError[], data)
{
	...
	if(client != 0)
		UC_PrintToChat(client, "%s%t", "Command Chicken Deleted");
...
public LastConnected_MenuHandler(Handle:hMenu, MenuAction:action, client, item)
{
	...
	else if(action == MenuAction_Select)
	{
		...
		PrintToConsole(client, "\n%s%t", UCTag, "Command Last Console Full", Name, AuthId, IPAddress, Date);
...
ShowSelectedWepStatMenu(client, CSWeaponID:i)
{
	...
	Format(TempFormat, sizeof(TempFormat), "%t", "Menu Wepstats Fully Automatic", wepStatsList[i][wepStatsIsAutomatic] ? ("%t", "Menu Yes") : ("%t", "Menu No"));
	AddMenuItem(hMenu, "", TempFormat);

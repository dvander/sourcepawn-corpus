...
public Action:Command_BankRupt(client, args)
{
	...
	if(args == 0 )
	{
		CPrintToChat(client, "{lightgreen}[BankRupt] {default}Usage: sm_bankrupt <#userid|name>");
		return Plugin_Handled;
	}
	...
	if( target > 0 && target <= MaxClients )
	{
		...
		CPrintToChatAll("{lightgreen}[SM] {default}ADMIN: %s: Set BankRupt on %s", admin, user);
...
public Action:Command_UnBankRupt(client, args)
{
	...
	if(args == 0 )
	{
		CPrintToChat(client, "{lightgreen}[BankRupt] {default}Usage: sm_unbankrupt <#userid|name>");
		return Plugin_Handled;
	}
	...
	if( target > 0 && target <= MaxClients )
	{
		...
		CPrintToChatAll("{lightgreen}[SM] {default}ADMIN: %s: Removed the BankRupt on %s", admin, user);
...
public Action:PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(IsBankRupt[client] && IsClientInGame(client) && !IsFakeClient(client))
	{
		...
		CPrintToChat(client, "{lightgreen}[SM] {default}You are BankRupt and won't get any money");
	}
}
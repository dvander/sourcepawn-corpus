...
{
	name = "SM Parachute (Fixed Version)",
	author = "SWAT_88",
...
public EndPara2(client)
{
	if(GetConVarInt(g_enabled)== 1 ){
		CloseParachute2(client);
	}
}
...
public CloseParachute2(client){
    if(hasModel[client] && IsValidEntity(Parachute_Ent[client])){
	SetEntityRenderMode(Parachute_Ent[client], RENDER_TRANSCOLOR);
    SetEntityRenderColor(Parachute_Ent[client], 255, 255, 255, 0);
        hasModel[client] = false;
    }
}

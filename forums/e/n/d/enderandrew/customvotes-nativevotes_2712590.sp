...
public Plugin myinfo =
{
	name = PLUGIN_NAME,
	author = "ReFlexPoison",
	description = "Allows server owners to creates dynamic votes accessible by players that run specified commands when passed.",
...
{
	CreateConVar("sm_customvotes_version", PLUGIN_VERSION, PLUGIN_NAME, 0 | FCVAR_SPONLY | FCVAR_DONTRECORD | FCVAR_NOTIFY);
...
// ====[ MENUS ]===============================================================
public int Menu_ChooseVote(int iVoter)
{
...
public int Menu_PlayersVote(int iVote, int iVoter)
{
	if(IsVoteInProgress())
	{
...
public void Vote_Players(int iVote, int iVoter, int iTarget)
{
	...
	Handle hMenu = NativeVotes_Create(VoteHandler_Players, NativeVotesType_Custom_YesNo, NATIVEVOTES_ACTIONS_DEFAULT | MenuAction_Select);
...
public int Menu_MapVote(int iVote, int iVoter)
{
	if(IsVoteInProgress())
	{
		CPrintToChat(iVoter, "[SM] %t", "Vote in Progress");
...
public int Vote_Map(int iVote, int iVoter, int iMap)
{
	if(IsVoteInProgress())
	{
		CPrintToChat(iVoter, "[SM] %t", "Vote in Progress");
	...
	Handle hMenu = NativeVotes_Create(VoteHandler_Map, NativeVotesType_Custom_YesNo, NATIVEVOTES_ACTIONS_DEFAULT | MenuAction_Select);
...
public int Menu_ListVote(int iVote, int iVoter)
{
	if(IsVoteInProgress())
	{
		CPrintToChat(iVoter, "[SM] %t", "Vote in Progress");
...
public int Vote_List(int iVote, int iVoter, int iOption)
{
	if(IsVoteInProgress())
	{
		CPrintToChat(iVoter, "[SM] %t", "Vote in Progress");
	...
	Handle hMenu = NativeVotes_Create(VoteHandler_List, NativeVotesType_Custom_YesNo, NATIVEVOTES_ACTIONS_DEFAULT | MenuAction_Select);
...
public int CastSimpleVote(int iVote, int iVoter)
{
	if(IsVoteInProgress())
	{
		CPrintToChat(iVoter, "[SM] %t", "Vote in Progress");
...
public int Vote_Simple(int iVote, int iVoter)
{
	if(IsVoteInProgress())
	{
		CPrintToChat(iVoter, "[SM] %t", "Vote in Progress");
	...
	Handle hMenu = NativeVotes_Create(VoteHandler_Simple, NativeVotesType_Custom_YesNo, NATIVEVOTES_ACTIONS_DEFAULT | MenuAction_Select);
	...
	NativeVotes_SetInitiator(hMenu, iVoter);
	NativeVotes_SetDetails(hMenu, strName);
	NativeVotes_DisplayToAll(hMenu, 30);
}
...
public int VoteHandler_Simple(Handle hMenu, MenuAction iAction, int iVoter, int iParam2)
{
	...
	if(iAction == MenuAction_Select)
	{
		...
		else if(StrEqual(strInfo, "yes", false))
		{

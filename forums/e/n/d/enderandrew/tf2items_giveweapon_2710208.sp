...
int iPermItems[MAXPLAYERS+1][6];
Handle hItemInfoTrie = INVALID_HANDLE;
#if defined _visweps_included
bool bVisWeps = false;
#endif
#if defined _tf2itemsinfo_included
bool bTF2ItemsInfo = false;
Handle hCvarTF2II = INVALID_HANDLE;
#endif
//int rnd_isenabled;
int iCvarNotify;
int iCvarValveWeapons;
bool bCvarLowAdmins;
bool bCvarGimme;
int bJarated[MAXPLAYERS + 1];
int iLastButtons[MAXPLAYERS + 1];

bool bSDKStarted = false;
Handle hSDKEquipWearable;
//Handle hMaxHealth;
Handle hChargeTimer[MAXPLAYERS + 1] = { INVALID_HANDLE, ... };
int iEyeParticle[MAXPLAYERS + 1] = { INVALID_ENT_REFERENCE, ... };
...
public void OnPluginStart()
{
	...
	Handle cv_version = CreateConVar("tf2items_giveweapon_version", PLUGIN_VERSION, "[TF2Items] Give Weapon Version", FCVAR_NOTIFY | 0 | FCVAR_SPONLY);
	Handle cv_notify = CreateConVar("tf2items_giveweapon_notify", "2", "If 1, makes Give Weapon show the plugin activity for givew only. If 2, makes it show activity for both givew and givew_ex.", 0);
	Handle cv_valveweps = CreateConVar("tf2items_valveweapons", "1337", "1337 to allow giving Valve weapons, anything else to disallow. Just because I got annoyed.", 0);
	Handle cv_lowadmins = CreateConVar("tf2items_customweapons_lowadmins", "1", "0 to disallow lower admins giving themselves custom weapons, 1 to allow. Just because I got annoyed.", 0);
	Handle cv_gimme = CreateConVar("tf2items_allow_gimme", "1", "0 to disallow the use of sm_gimme, 1 to allow", 0);
...
#endif
#if defined _tf2itemsinfo_included
	hCvarTF2II = CreateConVar("tf2items_use_tf2ii", "1", "Enable/disable tf2ii plugin usage", 0);
...
//		WritePackCell(pack, _:TF2_GetPlayerClass(client));
		...
		{
			//bool healing = TF2_IsPlayerInCondition(client, TFCond_Healing);
			TF2_AddCondition(client, view_as<TFCond>(55), 4.2);
...
/*public Action Timer_SetAmpTauntBack(Handle timer, Handle pack)
{
	...
	TFClassType class = TFClassType ReadPackCell(pack);
	TF2_SetPlayerClass(client, class, _, false);
	if (TF2_IsPlayerInCondition(client, TFCond_Taunting))
	{
		//bool healing = TF2_IsPlayerInCondition(client, TFCond_Healing);
...
public Action Timer_RemoveHealing(Handle timer, any userid)
{
	...
	if (!IsValidClient(client) || !IsPlayerAlive(client)) return Plugin_Stop;
	if (!TF2_IsPlayerInCondition(client, view_as<TFCond>(55)) || !TF2_IsPlayerInCondition(client, TFCond_Taunting)) return Plugin_Stop;
...
/* 	if (weaponid == TF_WEAPON_WRENCH)
	{
		...
		{
			int weaponent = GetEntPropEnt(inflictor, Prop_Send, "m_hActiveWeapon");
	...
	{
		float rage = GetEntPropFloat(assister, Prop_Send, "m_flRageMeter");
		rage += 15.0;
...
public Action Timer_DissolveRagdoll(Handle timer, any userid)
{
	...
	int ragdoll = (IsValidClient(victim) ? GetEntPropEnt(victim, Prop_Send, "m_hRagdoll") : -1);
	if (IsValidEntity(ragdoll))
	{
		DissolveRagdoll(ragdoll);
	}
}
stock void DissolveRagdoll(int ragdoll, int team = 0)
{
...
stock void AddDecapitation(int client, int victim)
{
	int heads = GetEntProp(client, Prop_Send, "m_iDecapitations") + 1;
...
stock void InstantDisguise(int client, int victim)
{
	TFClassType class = TF2_GetPlayerClass(client);
	TF2_SetPlayerClass(client, TFClass_Spy, _, false);
	TFTeam team = view_as<TFTeam>(GetClientTeam(victim));
	if (team != TFTeam_Red && team != TFTeam_Blue) team = ((GetClientTeam(client) == view_as<int>(TFTeam_Red)) ? (TFTeam_Blue) : (TFTeam_Red));
...
stock int CreateDisguiseWeapon(int client, int victim)
{
	char formatBuffer[32], char weaponClassname[64];
	int victimwep = GetPlayerWeaponSlot(victim, TFWeaponSlot_Primary);
	int idx = (victimwep > MaxClients && IsValidEntity(victimwep) ? GetEntProp(victimwep, Prop_Send, "m_iItemDefinitionIndex") : -1);
	...
	Handle hWeapon = PrepareItemHandle(idx);
	if (hWeapon != INVALID_HANDLE)
	{
		int weapon = TF2Items_GiveWeapon(client, hWeapon);
...
//	int weapon = GivePlayerItem(victim, weaponClassname);
	int weapon = CreateEntityByName(weaponClassname);
...
stock void ClearEyeParticle(int client)
{
	int eye = EntRefToEntIndex(iEyeParticle[client]);
	if (eye > MaxClients && IsValidEntity(eye)) AcceptEntityInput(eye, "Kill");
	iEyeParticle[client] = INVALID_ENT_REFERENCE;
}
stock void ChangeEyeParticle(int client)
{
	ClearEyeParticle(client);
	int decap = GetEntProp(client, Prop_Send, "m_iDecapitations");
	if (decap <= 0) return;
	...
	float ang[3];
	char effect[64];
	Format(effect, sizeof(effect), "eye_powerup_%s_lvl_%d", (view_as<TFTeam>(GetClientTeam(client)) == TFTeam_Red) ? "red" : "blue", decap > 4 ? 4 : decap);
...
public Action Event_PlayerHurt(Handle event, const char[] name, bool dontBroadcast)
{
	...
	int buffclient = GetIndexOfWeaponSlot(client, TFWeaponSlot_Secondary);
/*	if (IsValidClient(attacker) && IsPlayerAlive(client) && class != TFClass_Soldier && (buffclient == 226 || buffclient == 354) && !GetEntProp(client, Prop_Send, "m_bRageDraining"))
	...
	{
		char secondary[64];
		int sec = GetPlayerWeaponSlot(attacker, TFWeaponSlot_Secondary);
		...
		{
			float charge = GetEntPropFloat(sec, Prop_Send, "m_flChargeLevel");
			charge += 0.25;
...
stock void SetBlastJumpState(int client, int jumpstate, bool playsound)
{
	int offs = FindSendPropInfo("CTFPlayer", "m_iSpawnCounter") + 12;
	if (offs == 11 || offs == 12) return;
	SetEntData(client, offs, GetEntData(client, offs) | jumpstate);
	if (jumpstate == 1 || jumpstate == 2)
	{
		Handle event = CreateEvent(jumpstate == 2 ? "sticky_jump" : "rocket_jump", true);
...
public Action Event_PlayerJaratedFade(UserMsg msg_id, Handle bf, const int[] players, int playersNum, bool reliable, bool init)
{
...
{
	Handle randomizerhandle = FindConVar("tf2items_rnd_enabled");
...
//public void cvhook_rndisenabled(Handle cvar, const char[] oldVal, const char[] newVal) { rnd_isenabled = GetConVarBool(cvar); }

public void lockerwepreset(Handle event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
...
/*public Action TF2Items_OnGiveNamedItem(int client, char[] classname, int iItemDefinitionIndex, Handle &hItem)		//STRANGE STUFF HAPPENS HERE
{
	int flags = OVERRIDE_ITEM_DEF|PRESERVE_ATTRIBUTES;
...
public void TF2Items_OnGiveNamedItem_Post(int client, char[] classname, int itemDefinitionIndex, int itemLevel, int itemQuality, int entityIndex)
{
	int weaponSlot;
	char strSteamID[32];
	int idx = GetEntProp(entityIndex, Prop_Send, "m_iItemDefinitionIndex");
	char formatBuffer[32];
	Format(formatBuffer, 32, "%d_%s", idx, "slot");
	GetTrieValue(hItemInfoTrie, formatBuffer, weaponSlot);
	int weaponLookupIndex = iPermItems[client][weaponSlot];
	if (weaponLookupIndex != -1)
	{
		switch (weaponLookupIndex)
		{
			case 2171:
			{
				...
				GetClientAuthId(client, AuthId_Engine, strSteamID, sizeof(strSteamID));
...
public Action Command_WeaponEx(int client, int args)
{
...
//	mode = StringToInt(arg3);
	...
	int valveindex = (weaponLookupIndex < 0 ? -1*weaponLookupIndex : weaponLookupIndex);
...
public Action Command_ResetEx(int client, int args)
{
	char arg1[32];
	...
	if (type == 1)
	{
		...
		for (int arrayindex = 0; arrayindex <= args-3 && arrayindex < 32; arrayindex++)
		{
...
public Action Command_Gimme(int client, int args)
{
	char arg1[32];
	int weaponLookupIndex = -1;
	if (!bCvarGimme)
...
stock int GiveWeaponOfIndex(int client, int weaponLookupIndex, int weaponSlot)
{
	char strSteamID[32];
	...
	Handle hWeapon = PrepareItemHandle(weaponLookupIndex, TF2_GetPlayerClass(client));
...
//				if (StrEqual(strSteamID, "STEAM_0:1:19100391", false)) SetEntProp(entity, Prop_Send, "m_iEntityQuality", 9); //FlaminSarge's Self-Made Jar of Ants
		...
		case 142:
		{
			int secondary = GetIndexOfWeaponSlot(client, TFWeaponSlot_Secondary);
			...
			{
				int flags = GetEntProp(client, Prop_Send, "m_nBody");
				if (!(flags & (1 << 1)))
				{
					flags |= (1 << 1);
		...
		case 9266:
		{
			int model = PrecacheModel("models/weapons/c_models/c_bigaxe/c_bigaxe.mdl");
...
/*			int primary = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
			...
			{
				TFClassType primclassfix = FixReload(client, GetEntProp(primary, Prop_Send, "m_iItemDefinitionIndex"), cls);
	...
	bool wearable = (StrContains(classname, "wearable", false) != -1 || StrContains(classname, "powerup", false) != -1);
	char viewmodel[128];
	...
	{
		int vm = CreateVM(client, viewmodel);
...
/*		if (weaponLookupIndex == 131)
	...
	{
		int gunslingerfix = -1;
		TFClassType class = FixReload(client, entity, weaponLookupIndex, classname, gunslingerfix);
...
//		}
		...
		if (TF2_GetPlayerClass(client) == TFClass_Heavy && StrEqual(classname, "tf_weapon_medigun", false))
		{
			...
			SetEntPropVector(entity, Prop_Send, "m_vecOrigin", view_as<float>({ 0.0, 0.0, 38.0 }));
		}
/*		TFClassType class = FixReload(client, weaponLookupIndex, classname);
...
stock int CreateVM(int client, char[] model)
{
...
stock TFClassType FixReload(int client, int weapon, int idx, char[] classname, int &realindex)
{
	TFClassType class = TF2_GetPlayerClass(client);
	char formatBuffer[32];
	...
	bool found = false;
	bool gunslinger = false;//(GetIndexOfWeaponSlot(client, TFWeaponSlot_Melee) == 142);
...
public Action Command_AddWearable(int client, int args)
{
	...
	for (int i = 0; i < target_count; i++)
	{
		Handle hWeapon = PrepareItemHandle(weaponLookupIndex);
		int entity = TF2Items_GiveNamedItem(target_list[i], hWeapon);
		CloseHandle(hWeapon);
		if (IsValidEntity(entity))
		{
			char worldmodel[128];
...
public Action Command_GiveLudmila(int client, int args)
{
	if (args == 0)
	{
		if (client == 0)
...
public Action Command_GiveGlovesofRunning(int client, int args)
{
	if (args == 0)
	{
		if (client == 0)
...
public Action Command_GiveSpycrabPDA(int client, int args)
{
	if (args == 0)
	{
		if (client == 0)
...
stock Handle PrepareItemHandle(int weaponLookupIndex, TFClassType classbased = TFClass_Unknown)
{
	char formatBuffer[32];
	...
	if (attribCount > 1) {
		...
		for (int i = 0; i < attribCount; i+=2) {
			...
			switch (attrIdx)
			{
				...
				{
					attrVal = view_as<float>(StringToInt(weaponAttribsArray[i+1]));
				}
				default:
				{
...
stock void FixForWeaponAmmo(Handle hWeapon, TFClassType class, char[] classname, int attribs)
{
	if (class < TFClass_Unknown || class > TFClass_Engineer) return;
	if (attribs >= 16) return;
	int ammotype = GetAmmoType(classname);
...
stock int GetAmmoType(char[] classname)
{
	Handle ammotypetrie = MakeAmmotypeTrie();
	int ammotype = 0;
...
public Action Command_ReloadCustoms(int client, int args)
{
	ReloadItemTrie();
	ReplyToCommand(client, "[TF2Items] Custom Weapons list for Give Weapons reloaded");
	return Plugin_Handled;
}
stock void ReloadItemTrie()
{
...
stock void CustomItemsTrieSetup(Handle trie)
{
	char strBuffer[256];
	BuildPath(Path_SM, strBuffer, sizeof(strBuffer), "configs/tf2items.givecustom.txt");
	char strBuffer2[256];
	char strBuffer3[PLATFORM_MAX_PATH];
	Handle hKeyValues = CreateKeyValues("TF2ItemsGiveWeapon");
...
#if defined _tf2itemsinfo_included
public void TF2II_OnItemSchemaUpdated()
{
...
void MakeTF2IITrie(Handle trie)
{
	Handle hItemsArray = CreateArray();

	char strSlots[6][] = { "primary", "secondary", "melee", "pda", "pda2", "building" };
	int i;
	int iSlot;
	int iClass;
	Handle hResults;
	...
	{
		for (iClass = 1; iClass < view_as<int>(TFClassType); iClass++)
		{
...
//		CreateItemInfoTrie();
	...
	int iItemDefID, iItemSlot, iItemQuality, iItemLevel;
	char strItemClass[64];
	char strTrieName[16];

	int a;
	int iAttributeNum;
	int ammo;
	char strAttributes[(4 + 3 + 8) * 16 + 1];
	char appendBuffer[64];
	char strFloat[10];
	Handle ammotrie = MakeAmmoTrie();
...
//		if (StrEqual(strItemClass, "bundle", false)) continue; //we don't want bundles in here...yet
		iItemSlot = view_as<int>(TF2II_GetItemSlot(iItemDefID));
...
//		if (StrEqual(strItemClass, "tf_weapon_sapper", false)) iItemSlot = 1;
		iItemQuality = view_as<int>(TF2II_GetItemQuality(iItemDefID));
		...
		if (iAttributeNum)
		{
			...
			{
				int attrid = TF2II_GetItemAttributeID(iItemDefID, a);
				float attrvalue = TF2II_GetItemAttributeValue(iItemDefID, a);
				...
				for (int c = strlen(appendBuffer) - 2; c > 0; c--)
				{
					if (appendBuffer[c] != '0')
					{
						if (appendBuffer[c] == '.')
...
stock void AddCustomHardcodedToTrie(Handle trie)
{
...
//Robin's int cheap Rocket Launcher
...
stock void PrepareCustomWeaponModel(const char[] modelname, const char[] key)
{
	char modelfile[PLATFORM_MAX_PATH + 4];
	char strLine[PLATFORM_MAX_PATH];
...
stock void SetSpeshulAmmo(int client, int wepslot, int newAmmo)
{
...
stock int GetSpeshulAmmo(int client, int wepslot)
{
	if (!IsValidClient(client)) return 0;
	int weapon = GetPlayerWeaponSlot(int client, int wepslot);
...
public void SickleClimbWalls(int client)
{
	if (!IsValidClient(client)) return;
...
//	if (!(g_iSpecialAttributes[client] & attribute_climbwalls)) return;
	...
	for (int healer = 1; healer <= MaxClients; healer++)
	{
		...
		int sec = GetPlayerWeaponSlot(healer, TFWeaponSlot_Secondary);
...
public int Native_CreateWeapon(Handle plugin, int numParams)
{
	char formatBuffer[64];
	int desiredIndex = GetNativeCell(1);
	int weaponSlot;
	bool overwrite = GetNativeCell(10);
	...
	int weaponIndex = GetNativeCell(3);
...
public int Native_CheckWeapon(Handle plugin, int numParams)
{
...
public int Native_CheckWeaponSlot(Handle plugin, int numParams)
{
	int index = GetNativeCell(1);
	int weaponSlot;
	char formatBuffer[64];
...
stock bool TF2_IsEntityWearable(int entity)
{
...
stock void RemovePlayerBack(int client)
{
...
stock void RemovePlayerBooties(int client)
{
	int edict = MaxClients+1;
	while((edict = FindEntityByClassname2(edict, "tf_wearable")) != -1)
	{
...
stock int FindPlayerTarge(int client)
{
	int edict = MaxClients+1;
	while((edict = FindEntityByClassname2(edict, "tf_wearable_demoshield")) != -1)
	{
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	...
	{
		TFClassType class = TF2_GetPlayerClass(client);
		int idxslot2 = GetIndexOfWeaponSlot(client, TFWeaponSlot_Secondary);
		...
		if ((buttons & IN_RELOAD) && GetEntityMoveType(client) != MOVETYPE_NONE && ((targe = FindPlayerTarge(client)) != -1) && hChargeTimer[client] == INVALID_HANDLE && GetEntPropFloat(client, Prop_Send, "m_flMaxspeed") > 1.01 && (class != TFClass_DemoMan || (wep == GetPlayerWeaponSlot(client, TFWeaponSlot_Melee) && (idxslot2 == 265 || idxslot2 == 20 || idxslot2 == 207 || idxslot2 == 130))))
		{
			...
			if (IsValidEntity(wep))
			{
				...
				if (strncmp(classname, "tf_weapon", 9, false) == 0)
				{
					...
					float old = GetEntPropFloat(wep, Prop_Send, "m_flNextSecondaryAttack");
...
stock int SetShieldBashUsed(int shield, bool used = true)
{
	int bashoffs = GetEntSendPropOffs(shield, "m_hWeaponAssociatedWith") + 28;
	if (bashoffs > 28) SetEntData(shield, bashoffs, used ? 1 : 0);
	return bashoffs > 28;
}
stock void DoActivateMmmph(int client)
{
	float vel[3];
	vel[0] = 0.0;
	vel[1] = 0.0;
	...
	bool megaheal = TF2_IsPlayerInCondition(client, TFCond_MegaHeal);
	...
	int soundindex = GetRandomInt(1, 3);
...
stock void DoEurekaTaunt(int client)
{
	float vel[3];
	float pos[3];
	vel[0] = 0.0;
...
public Action Timer_EurekaRespawn(Handle timer, any userid)
{
	int client = GetClientOfUserId(userid);
	if (!IsValidClient(client) || !IsPlayerAlive(client)) return Plugin_Stop;
	Handle message = StartMessageAll("PlayerTeleportHomeEffect");
...
/*	int particle = CreateEntityByName("info_particle_system");
	if (IsValidEdict(particle))
	{
		...
		int startpoint = CreateEntityByName("info_particle_system");
...
{
	ResetPack(pack);
	int particle = EntRefToEntIndex(ReadPackCell(pack));
...
stock void DoResetChargeTimer(int client, bool end = false)
{
	ClearTimer(hChargeTimer[client]);
...
stock int RandomPlayerOfClass(int client, TFClassType class)
{
	for (int i = 1; i <= MaxClients; i++)
	{
...
stock void DoTeleportParticles(int client)
{
	float pos[3];
	GetClientAbsOrigin(client, pos);
	char name[32];
	name = (GetClientTeam(client) == view_as<int>(TFTeam_Blue) ? "player_sparkles_blue" : "player_sparkles_red");
	if (TE_SetupTFParticle(name, pos, _, _, client, 3, 0, false))
		TE_SendToAll(0.0);
	name = (GetClientTeam(client) == view_as<int>(TFTeam_Blue) ? "teleported_blue" : "teleported_red");
	if (TE_SetupTFParticle(name, pos, _, _, client, 0, 0, false))
		TE_SendToAll(0.0);
}

stock bool TE_SetupTFParticle(char[] Name,

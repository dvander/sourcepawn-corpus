...
enum struct petContent
{
	char iName[64];
	char iDesc[64];
	char iModel[128];
	char iSoundGeneric[128];
	// char iSoundAmount[128];
	char iSoundGeneric_2[128];
	char iSoundGeneric_3[128];
	char iSoundGeneric_4[128];
	char iSoundGeneric_5[128];
	char iSoundGeneric_6[128];
	char iSoundGeneric_7[128];
	char iSoundGeneric_8[128];
	char iSoundGeneric_9[128];
	char iSoundGeneric_10[128];
...
stock petContent petInfo[256];

int pet[MAXPLAYERS+1];
int petType[MAXPLAYERS+1];
char petName[MAXPLAYERS+1][32];
int petColor[MAXPLAYERS+1];
int petHappiness[MAXPLAYERS+1];
int petParticle[MAXPLAYERS+1];
int petState[MAXPLAYERS+1];
int petSkin[MAXPLAYERS+1];
bool petChangeCooldown[MAXPLAYERS+1];

// cookies
Handle cPetType;
Handle cPetName;
Handle cPetColor;
Handle cPetHappiness;
Handle cPetSkin;
...
Handle MoodEffects = INVALID_HANDLE;
Handle MoodEnable = INVALID_HANDLE;
Handle PetCooldown = INVALID_HANDLE;
Handle Reload = INVALID_HANDLE;

public Plugin myinfo = 
{
	name = "[Any] SourcePets",
	author = "Oshizu & noodleboy347 (Playa Edit)",
...
stock int CreateLoopingParticle(int ent, char[] particleType)
{
	...
	{ 
		...
		DispatchKeyValue(particle, "targetname", "pets_particle"); 
...
{	
	RegConsoleCmd("sm_pets", PetsMenu, "Opens the Pet Menu");
	RegConsoleCmd("sm_petname", PetName, "Give your Pet a Name, !pet Egon");
	RegConsoleCmd("sm_petstatus", Command_Pet, "Your Pets current Status");
	...
	MoodEffects = CreateConVar("pets_mood_effects", "0", "- Should pets show specific kinds of particle effects when they got high/low mood?");
	Reload = CreateConVar("pets_reload_mapchange", "0", "- Should plugin attempt to reload config on map change?");
	
	cPetType = RegClientCookie("sm_pets_type", "", CookieAccess_Private);
	cPetName = RegClientCookie("sm_pets_name", "", CookieAccess_Private);
	cPetColor = RegClientCookie("sm_pets_color", "", CookieAccess_Private);
	cPetHappiness = RegClientCookie("sm_pets_happiness", "", CookieAccess_Private);
	cPetSkin = RegClientCookie("sm_pets_skin", "", CookieAccess_Private);
...
public Action ReloadPets(int client, int args)
{
	LoadConfig();
...
stock void LoadConfig()
{
	...
	for(int i = 1; i <= 256;i++)
	{
		...
		if(petInfo[i].iSoundAmount > 0) 
		{
			...
			{
				for(int e = 2; e <= petInfo[i].iSoundAmount; e++)
				{
			...
			if(petInfo[i].SkinsAmount > 1)
			{
				...
				for(int e = 2; e <= petInfo[i].SkinsAmount; e++)
				{
...
public Action PetName(int client, int args)
{
	char argc[32];
	GetCmdArgString(argc, sizeof(argc));
	Format(petName[client], sizeof(petName[]), "%s", argc);
	SetClientCookie(client, cPetName, petName[client]);													
}
	...
	if(GetConVarInt(MoodEnable) == 1)
	{
		for(int i = 1; i <= MaxClients;i++)
		{
			if(IsClientInGame(i) && petType[i] > 0)
			{
				...
				char happiness[32];
				IntToString(petHappiness[i], happiness, sizeof(happiness));
				SetClientCookie(i, cPetHappiness, happiness);
			}
		}
	}
}

void PetSkin(int client)
{
...
public int PetsSkin_Handler(Handle menu, MenuAction action, int client, int param2)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			SetClientCookie(client, cPetSkin, item);
...
public Action PetsMenu(int client, int args)
{
	if(MenuNotUsable)
	{
		return Plugin_Handled;
...
public int PetsMenu_Handler(Handle menu, MenuAction action, int client, int param2)
{
	switch (action)
	{
		case MenuAction_Select:
...
public int PetsType_Handler(Handle menu, MenuAction action, int client, int param2)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			char item[64];
			GetMenuItem(menu, param2, item, sizeof(item));
			if (StrEqual(item, "150"))
			{
				SetClientCookie(client, cPetType, "0");
			} else {
				SetClientCookie(client, cPetType, item);
			}
...
public int PetsColor_Handler(Handle menu, MenuAction action, int client, int param2)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			char item[64];
			GetMenuItem(menu, param2, item, sizeof(item));
			SetClientCookie(client, cPetColor, item);
...
/*public PetsMenu_Handler(Handle menu, MenuAction action, int client, int param2)
{
	switch (action)
	{
		case MenuAction_Select:
...
public void OnMapStart()
{
	...
	for(int i=1; i<sizeof(petInfo); i++)
	{
...
public void OnGameFrame()
{
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && (!IsPlayerAlive(i) || GetClientTeam(i) < 2) && IsValidPet(pet[i]))
			KillPet(i);
	}
}
...
{
	char info[32];
	GetClientCookie(client, cPetColor, info, sizeof(info));
	petColor[client] = StringToInt(info);
	GetClientCookie(client, cPetHappiness, info, sizeof(info));
	petHappiness[client] = StringToInt(info);
	GetClientCookie(client, cPetType, info, sizeof(info));
	petType[client] = StringToInt(info);
	GetClientCookie(client, cPetSkin, info, sizeof(info));
	petSkin[client] = StringToInt(info);
	GetClientCookie(client, cPetName, info, sizeof(info));
	petName[client] = info;
}
...
public int Menu_Pet(Handle menu, MenuAction action, int client, int option)
{
...
void SpawnPet(int client)
{
	...
	if(petHappiness[client] == 0)
	{
		...
		char happiness[32];
		IntToString(petHappiness[client], happiness, sizeof(happiness));
		SetClientCookie(client, cPetHappiness, happiness);
	}
	float pos[3];
	...
	DispatchKeyValue(pet[client], "targetname", "player_pet");
	...
	int value = RoundFloat(GetRandomFloat(1.00, float(petInfo[petType[client]].iSoundAmount)));
	switch(value)
	{
...
stock void Entity_GetAbsOrigin(int entity, float vec[3]) // Thanks to SMLIB for this stock!
{
...
public bool FilterSCP(int entity, int contentsMask, any client)
{
...
void SetPetState(int client, int status)
{ 
	float pos[3];
	GetEntPropVector(pet[client], Prop_Data, "m_vecOrigin", pos);
...
void SetPetAnim(int client, const char[] anim)
{
	SetVariantString(anim);
	AcceptEntityInput(pet[client], "SetAnimation");
}

void SetPetParticle(int client, char[] particle)
{
	if(GetConVarInt(MoodEffects) == 1)
	{
		if(petParticle[client] == 0)
...
void KillPet(int client, bool force=false)
{
	if(pet[client] == 0 && !force)
		return;

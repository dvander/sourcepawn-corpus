 ...
 * 
*/
Handle ConfigKV = INVALID_HANDLE;
Handle UsersKV = INVALID_HANDLE;
Handle GroupsKV = INVALID_HANDLE;
Handle hAdminMenu = INVALID_HANDLE;
Handle tmpTopMenuHandle = INVALID_HANDLE;
TopMenuObject obj_psmcommands;
...
public void OnPluginStart() {
	...
	CreateConVar("permissionssm_version", PLUGIN_VERSION, "PermissionsSM Version", 0|FCVAR_SPONLY|FCVAR_UNLOGGED|FCVAR_DONTRECORD|FCVAR_REPLICATED|FCVAR_NOTIFY);
 ...
 * 
*/
public Action Command_PermissionsSM(int client, int args) {
	if (args < 1) {
	...
	if (StrEqual(arg1, "", false)) {
		...
		if (StrEqual(arg2, "help", false)) {
			...
			ReplyToCommand(client, "[PSM] group add <Group> <Parent> - Adds int Group");
...
getClientImmunity(client, defaultImmunity = 0) {
	KvRewind(UsersKV);
	Handle tmpHandle = CloneHandle(UsersKV);
 ...
 * 
*/
public bool hasPermission(int client, char[] PString, any ...) {
	char PermissionString[8192];
	...
	if (client == 0) { return true; }
	int clientPermissions = hasClientPermission(client, PermissionString);
 ...
 * 
*/
hasClientPermission(int client, char[] PermissionString) {
	KvRewind(UsersKV);
	Handle tmpHandle = CloneHandle(UsersKV);
 ...
 * 
*/
hasGroupPermission(char[] Group, char[] PermissionString) {
	KvRewind(GroupsKV);
	Handle tmpHandle = CloneHandle(GroupsKV);
 ...
 * 
*/
bool StringToBool(char[] BoolString) {
 ...
 * 
*/
bool SetUserPrefix(int client, char[] Prefix) {
	char AuthString[512];
	GetClientAuthId(client, AuthId_Engine, AuthString, sizeof(AuthString));
	KvRewind(UsersKV);
 ...
 * 
*/
bool SetUserSuffix(int client, char[] Suffix) {
	char AuthString[512];
	GetClientAuthId(client, AuthId_Engine, AuthString, sizeof(AuthString));
	KvRewind(UsersKV);
	KvJumpToKey(UsersKV, AuthString, true);
 ...
 * 
*/
bool AddUserGroup(int client, char[] Groupname, int negated = false) {
	char AuthString[512];
	GetClientAuthId(client, AuthId_Engine, AuthString, sizeof(AuthString));
	KvRewind(UsersKV);
 ...
 * 
*/
bool DelUserGroup(int client, char[] Groupname) {
	char AuthString[512];
	GetClientAuthId(client, AuthId_Engine, AuthString, sizeof(AuthString));
	KvRewind(UsersKV);
	KvGotoFirstSubKey(UsersKV)
 ...
 * 
*/
bool AddUserPerm(int client, char[] Permission, int negated = false) {
	char AuthString[512];
	GetClientAuthId(client, AuthId_Engine, AuthString, sizeof(AuthString));
	KvRewind(UsersKV);
	KvGotoFirstSubKey(UsersKV);
 ...
 * 
*/
bool DelUserPerm(int client, char[] Permission) {
	char AuthString[512];
	GetClientAuthId(client, AuthId_Engine, AuthString, sizeof(AuthString));
	KvRewind(UsersKV);
	KvGotoFirstSubKey(UsersKV)
 ...
 * 
*/
bool SetGroupPrefix(char[] Groupname, char[] Prefix) {
	KvRewind(GroupsKV);
 ...
 * 
*/
bool SetGroupSuffix(char[] Groupname, char[] Prefix) {
	KvRewind(GroupsKV);
	KvJumpToKey(GroupsKV, Groupname);
	KvJumpToKey(GroupsKV, "Options", true);
 ...
 * 
*/
bool SetGroupImmunity(char[] Groupname, int immunity) {
	KvRewind(GroupsKV);
	KvJumpToKey(GroupsKV, Groupname);
	KvJumpToKey(GroupsKV, "Options", true);
 ...
 * 
*/
bool SetDefaultGroup(char[] Groupname) {
	KvRewind(GroupsKV);
	KvGotoFirstSubKey(GroupsKV)
	do {
		KvJumpToKey(GroupsKV, "Options", true);
 ...
 * 
*/
bool GetDefaultGroup(char[] Groupname, int maxlength, char[] defaultValue = "") {
	KvRewind(GroupsKV);
	Handle tmpHandle = CloneHandle(GroupsKV);
 ...
 * 
*/
bool AddInherit(char[] Groupname, char[] Parent, int negated = false) {
	KvRewind(GroupsKV);
	KvGotoFirstSubKey(GroupsKV);
	KvRewind(GroupsKV);
	KvJumpToKey(GroupsKV, Groupname);
 ...
 * 
*/
bool DelInherit(char[] Groupname, char[] Parent) {
	KvRewind(GroupsKV);
	KvGotoFirstSubKey(GroupsKV)
	KvRewind(GroupsKV);
	KvJumpToKey(GroupsKV, Groupname);
 ...
 * 
*/
bool AddGroupPerm(char[] Groupname, char[] Permission, int negated = false) {
	KvRewind(GroupsKV);
	KvGotoFirstSubKey(GroupsKV);
	KvRewind(GroupsKV);
	KvJumpToKey(GroupsKV, Groupname);
 ...
 * 
*/
bool DelGroupPerm(char[] Groupname, char[] Permission) {
	KvRewind(GroupsKV);
	KvGotoFirstSubKey(GroupsKV)
	KvRewind(GroupsKV);
	KvJumpToKey(GroupsKV, Groupname);
...
public Native_hasPermission(Handle plugin, numParams) {
	...
	char PermissionNode[1024], written;
	FormatNativeString(0, 2, 3, sizeof(PermissionNode), view_as<int>(written), PermissionNode);

...
Description:
	Show in hint window the information on the damages put to
	the player during his life and during death of the player
	or in the end of a round show on in-game panels the
	information on the total put and received damages.
...
1.3.0 (2009.02.08)
	...
	Total Damage will always display at end of round, unless you are infected, recently died, and already saw your total damage. [-pk-]
	Fix display not showing up when a player dies from common infected or entities (fire, fall damage, etc). [-pk-]
	improved round end detection in coop mode, no longer using door_close events. [-pk-]
	Fix menu "array index out of bounds" error when pressing menu exit button. [-pk-]
	removed cvar sm_did_gmode
1.2.8 (2009.02.06)
	Command !did will now ask which damage to display
		TOTAL DAMAGE or DAMAGE SINCE SPAWN
...
1.2.5 (2009.02.03)
	...
	cvar sm_did_total_onlyrend is added
		Show DID Total Panel only in a RoundEnd. 0 = off; 1 = on.
1.2.3 (2009.02.02)
	Code optimisation
	find bug 1.2.2 version is fixed
1.2.2 (2009.02.01)
...
public OnPluginStart()
{
	...
	cvarGameMode = FindConVar("mp_gamemode"); // if int 0 - versus | 1 - coop
	...
	HookConVarChange(cvarGameMode,OnCVGameModeChange);
...
public DIDMenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	if (action == MenuAction_Select)
	{
		...
		{
			PrintToChat(param1, "\x04[SM]\x03 %t", "menu stay mode");
		}
		else
		{
			...
			{
			  case 0:
				PrintToChat(param1, "\x04[SM]\x03 %t \x04%t\x03", "menu get mode", "menu disable mode");
			  case 1:
				PrintToChat(param1, "\x04[SM]\x03 %t \x04%t\x03", "menu get mode", "menu all mode");
			  case 2:
				PrintToChat(param1, "\x04[SM]\x03 %t \x04%t\x03", "menu get mode", "menu done mode");
			  case 3:
				PrintToChat(param1, "\x04[SM]\x03 %t \x04%t\x03", "menu get mode", "menu received mode");
			}
		}
	}
		...
		else
		{
			switch (DisplayOnDeathMode[param1])
			{
			  case 0:
				PrintToChat(param1, "\x04[SM]\x03 %t \x04%t\x03", "menu get mode", "menu disable mode");
			  case 1:
				PrintToChat(param1, "\x04[SM]\x03 %t \x04%t\x03", "menu get mode", "menu PDeath total");
			  case 2:
				PrintToChat(param1, "\x04[SM]\x03 %t \x04%t\x03", "menu get mode", "menu PDeath current");
			}
		}
	}
	else if (action == MenuAction_Cancel)
			...
			if (victim != 0 && attacker != 0)
			{
				...
				else if (GetClientTeam(victim) == GetClientTeam(attacker))
				{
					...
					if (DisplayMode[victim] == 1 || DisplayMode[victim] == 3)
						PrintHintText(victim, "-FF!!! %N %t %iHP.", attacker, "Hint EtoP Hurt", CurrentDamage[attacker][victim]);
					if (DisplayMode[attacker] == 1 || DisplayMode[attacker] == 2)
						PrintHintText(attacker, "+FF!!! %t %N: %iHP.", "Hint PtoE Hurt", victim, CurrentDamage[attacker][victim]);
				}
				else
				{
...
public OnCVGameModeChange(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	if (strcmp(oldValue, newValue) != 0)
	{
		if (CurrentGameMode == 2)
		{
			...
			#if DEBUGMODE
			LogMessage("Changed to Versus");
			PrintToChatAll("Changed to Versus");
			#endif
		}
		else if (CurrentGameMode == 1)
		{
			...
			#if DEBUGMODE
			LogMessage("Changed to Coop");
			PrintToChatAll("Changed to Coop");
			#endif
		}
		else if (CurrentGameMode == 3)
		{
			#if DEBUGMODE
			LogMessage("Changed to Survival");
			PrintToChatAll("Changed to Survival");
			#endif
		}
	}
}
...
Gamemode() // 1 = Co-Op, 2 = Versus, 3 = Survival. False on anything else.
{
	...
	if (strncmp(gmode,"coop",sizeof(gmode),false)==1) return 1;
	else if (strncmp(gmode,"versus",sizeof(gmode),false)==1) return 2;
	else if(strncmp(gmode,"survival",sizeof(gmode),false)==1) return 3;
        else return false;
}
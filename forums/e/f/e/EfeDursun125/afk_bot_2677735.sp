...
new g_bMaxStickies[MAXPLAYERS + 1];
	...
	if(IsValidClient(client))
	{
		...
		if(g_bAfkbot[client] || (StrContains(playerName, "Aimbot", false) == -1 && StrContains(playerName, "Bot", false) != -1 && IsFakeClient(client)))
		{
			if(IsPlayerAlive(client))
			{
				...
				if(class == TFClass_DemoMan)
				{
					new pipecount = GetClientEntityCount(client, "tf_projectile_pipe_remote");
				...
				if(!g_bMedicAllHaveTasks[client] && !g_bCamping[client] && !g_bSpyHaveAnyTask[client] && !g_bHealthIsLow[client] && !g_bAmmoIsLow[client] && class != TFClass_Engineer && !g_bMakeStickyTrap[client])
				{
					...
					else if(StrContains(currentMap, "cp_" , false) != -1)
					{
						...
						{
							new Owner = GetEntProp(capturepoint, Prop_Send, "m_iOwner");
							new iTeamCanCap = GetEntProp(capturepoint, Prop_Send, "m_bTeamCanCap");
				...
				if(class != TFClass_Medic)
				{
					decl Float:camangle[3], Float:targetEyes[3], Float:targetEyes2[3], Float:targetEyesBase[3];
	...
	for(new i = 0; i <= MaxClients; i++)
	{
		if(IsPlayerAlive(i) && IsValidClient(i))
		{
			if(GetClientTeam(client) != GetClientTeam(i))
			{
				...
				new Float:edict_distance = GetVectorDistance(clientOrigin, enemyOrigin);
				if((edict_distance < distance) || (distance == -1.0))
				{
					distance = edict_distance;
					nearestEnemy = i;

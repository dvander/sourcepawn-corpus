...
public void OnClientPostAdminCheck(int client)
{
	if(client > 0 && client <= MaxClients && IsClientInGame(client) && !IsFakeClient(client))
	{
		...
		{
			g_DB.Format(query, sizeof(query), "SELECT ban_length, ban_reason, (now()-timestamp)/60 FROM my_bans WHERE steam_id = '%s'", steam_id);
		...
		{
			query[0] = '\0';
			g_DB.Format(query, sizeof(query), "SELECT ban_length, ban_reason, (now()-timestamp)/60 FROM my_ipbans WHERE ip = '%s'", ip);

...
{	
	...
	Format(g_strCampaignName[6], 32, "No Mercy");
	Format(g_strCampaignName[7], 32, "Crash Course");
	Format(g_strCampaignName[8], 32, "Death Toll");
	Format(g_strCampaignName[9], 32, "Dead Air");
	Format(g_strCampaignName[10], 32, "Blood Harvest");
	Format(g_strCampaignName[11], 32, "The Sacrifice");
...
public OnPluginStart()
{
	...
	g_hCVar_VotingAdDelayTime = CreateConVar("acs_voting_ad_delay_time", "120", "Time, in seconds, to wait after finale or scavenge map starts to advertise voting as defined in acs_voting_ad_mode\n * Note: If the server is up, changing this in the .cfg file takes two map changes before the change takes place *", FCVAR_PLUGIN, true, 0.1, false);
...
DisplayNextMapToAll()
{
	...
	else
	{
		...
		if(g_iGameMode == GAMEMODE_SCAVENGE)
		{
			new LastCM = NUMBER_OF_SCAVENGE_MAPS - 1;			//last scavenge map's array number
			
			//Check to see if this is the last scavenge map in the array
			if(StrEqual(strCurrentMap, g_strScavengeMap[LastCM]) == true)
			{

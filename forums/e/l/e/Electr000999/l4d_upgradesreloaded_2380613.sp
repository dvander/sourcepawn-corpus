...
public OnPluginStart()
{
	...
	UpgradeEnabled[0] = CreateConVar("survivor_upgrade_bottle_satchel_enable", "1", "Enable/Disable Bottle Satchel", CVAR_FLAGS, true, 0.0, true, 1.0);
	...
	SetConVarInt(FindConVar("pain_pills_health_threshold"), 200, false, false);
...
public Action event_SpawnerGiveItem(Event event, const char[] name, bool dontBroadcast) 
{
	...
	PrintToChat(client, "New Item: %s / %d", weapon, spawner);	
}
...
public Action event_AmmoPickup(Event event, const char[] name, bool dontBroadcast) 
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	CustomGivePlayerAmmo(client);
}
	...
	if(IsClientInGame(client))
	{
		...
		for(new i = 1; i <= MaxClients; i++)
		{
			if(IsClientInGame(i) && GetClientTeam(i) == 2)
			{
				...
				new percentage = RoundFloat((float(GetSurvivorUpgrades(i)) / GetAvailableUpgrades(i)) * 100);
		...
		decl String:notification[255];
		decl String:status[16];
		decl String:savefeature[255];
		decl String:savestatus[16];
		...
		DrawPanelItem(UpgradePanel, savefeature);
		Format(notification, sizeof(notification), "Notifications (%s)", status);
		DrawPanelItem(UpgradePanel, notification);
		DrawPanelItem(UpgradePanel, "Toggle Upgrades");
		DrawPanelItem(UpgradePanel, "View Awards");
		DrawPanelItem(UpgradePanel, "Reset Upgrades");
		DrawPanelItem(UpgradePanel, "Reset Bots");
...
public Action event_HealSuccess(Event event, const char[] name, bool dontBroadcast) 
{
	...
	if(m_iMaxHealth <= 150)
	{
		new m_iHealth = RoundFloat(((float(m_iMaxHealth) - (200.0 - (float(health_restored)/80.0)*100.0))*0.8)+(200.0-(float(health_restored)/80.0)*100.0));
...
public Action event_MeleeKill(Event event, const char[] name, bool dontBroadcast) 
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	new entityid = GetEventInt(event, "entityid");
...
public Action event_EntityShoved(Event event, const char[] name, bool dontBroadcast) 
{
	new entityid = GetEventInt(event, "entityid");
	new client = GetClientOfUserId(GetEventInt(event, "attacker"));

...
1.1.1 (13-May-2015) -> edit by emsit
	- Fix compiler warnings, cleanup code, fix flamethrower vector
...
#define PLUGIN_DESCRIPTION "Dangerous Weapons"
...
new Handle:l4d_dangerous_connon_catchfire;
...
new Handle:l4d_dangerous_mode_connon;
new Handle:l4d_dangerous_mode_electromagnetic;
new Handle:l4d_dangerous_mode_flamethrower;

public OnPluginStart() {
	GameCheck();
	CreateConVar ("DangerousWeapons_version", PLUGIN_VERSION, PLUGIN_DESCRIPTION, FCVAR_REPLICATED | FCVAR_NOTIFY);
 	l4d_dangerous_enable = CreateConVar("l4d_dangerous_enable", "1", "  0:disable, 1:enable in coop mode, 2: enable in all mode ", FCVAR_PLUGIN);
 	l4d_dangerous_message=CreateConVar("l4d_dangerous_message", "3", "how many times to display usage information ,0 disable  ", FCVAR_PLUGIN);
 	l4d_dangerous_safe=CreateConVar("l4d_dangerous_safe", "1", "1:more safe to use", FCVAR_PLUGIN);
	l4d_dangerous_power[Type_Rifle] = CreateConVar("l4d_dangerous_power_rifle", "1.1", "power of rifle 0.0: disable [0.0, 3.0]", FCVAR_PLUGIN);
	l4d_dangerous_power[Type_Sniper] = CreateConVar("l4d_dangerous_power_sniper", "1.8", " ", FCVAR_PLUGIN);
	l4d_dangerous_power[Type_Shotgun] = CreateConVar("l4d_dangerous_power_shotgun", "0.8", " ", FCVAR_PLUGIN);
	l4d_dangerous_power[Type_Pistol]  = CreateConVar("l4d_dangerous_power_magnum", "1.5", " ", FCVAR_PLUGIN);
	l4d_dangerous_power[Type_Smg]  = CreateConVar("l4d_dangerous_power_smg", "0.5", "", FCVAR_PLUGIN);
	l4d_dangerous_drop_ci  = CreateConVar("l4d_dangerous_drop_ci", "10.0", "drop chance for common infected", FCVAR_PLUGIN);
	l4d_dangerous_drop_si  = CreateConVar("l4d_dangerous_drop_si", "30.0", "drop chance for special infected", FCVAR_PLUGIN);
	l4d_dangerous_drop_pickupcount  = CreateConVar("l4d_dangerous_drop_pickupcount", "3", "bullet count for every pick up", FCVAR_PLUGIN);
	l4d_dangerous_damage_hit  = CreateConVar("l4d_dangerous_damage_hit", "300.0", "direct hit damage", FCVAR_PLUGIN);
	l4d_dangerous_damage_explode  = CreateConVar("l4d_dangerous_damage_explode", "300.0", "explode damage", FCVAR_PLUGIN);
	l4d_dangerous_damage_radius  = CreateConVar("l4d_dangerous_damage_radius", "200.0", "explode radius", FCVAR_PLUGIN);
	l4d_dangerous_particle  = CreateConVar("l4d_dangerous_particle", "1", "1:show particle , 0: disable", FCVAR_PLUGIN);
	l4d_dangerous_connon_catchfire  = CreateConVar("l4d_dangerous_connon_catchfire", "0", "1:firing connon, 0: disable", FCVAR_PLUGIN);
	l4d_dangerous_flame_damage  = CreateConVar("l4d_dangerous_flame_damage", "20.0", "flame damage", FCVAR_PLUGIN);
	l4d_dangerous_flame_length  = CreateConVar("l4d_dangerous_flame_length", "250.0", "flame length", FCVAR_PLUGIN);
	l4d_dangerous_flame_duration  = CreateConVar("l4d_dangerous_flame_duration", "5.0", "flame duration", FCVAR_PLUGIN);
	l4d_dangerous_pickup_mode  = CreateConVar("l4d_dangerous_pickup_mode", "1", "1: pick up mode (l4d2) 2:direct give mode", FCVAR_PLUGIN);
	l4d_dangerous_mode_connon  = CreateConVar("l4d_dangerous_mode_connon", "1", "1: enable Mini Cannon, 0: disable", FCVAR_PLUGIN);
	l4d_dangerous_mode_electromagnetic  = CreateConVar("l4d_dangerous_mode_electromagnetic", "1", "1: enable Electromagnetic Cannon, 0: disable", FCVAR_PLUGIN);
	l4d_dangerous_mode_flamethrower  = CreateConVar("l4d_dangerous_mode_flamethrower", "1", "1: enable Flamethrower, 0: disable", FCVAR_PLUGIN);
...
StartElec(client, type) {
	...
	Bullet[client]--;
	PrintHintText(client, "Special Bullet Ramain: %d", Bullet[client]);
}
...
StartFlame(client, type) { 
	...
	new Float:pos[3];
	new Float:ang[3];	
	SetVector(pos, 25.0, 10.0, -15.0);
	SetVector(ang, 0.0, 15.0, 0.0);
...
StartConnon(client, type) {
	...
	if (L4D2Version) {
		SDKHook(ent, SDKHook_StartTouch , GLprojectileTouch);
	} else {
		SetEntPropEnt(ent, Prop_Data, "m_hOwnerEntity", client)	;
		if (Connon[client]>0 && IsValidEdict(Connon[client]) && IsValidEntity(Connon[client])) {
			AcceptEntityInput(Connon[client], "kill");
		}
		Connon[client]=ent;
	}
...
public Action:grenade_bounce(Handle:h_Event, const String:s_Name[], bool:b_DontBroadcast) { 
	...
	if (Connon[client]>0 && IsValidEdict(Connon[client]) && IsValidEntity(Connon[client])) {
		decl Float:pos[3];
		GetEntPropVector(Connon[client], Prop_Send, "m_vecOrigin", pos);		
		AcceptEntityInput(Connon[client], "kill");
		
		new Float:f=GetEntPropFloat(Connon[client], Prop_Send, "m_fadeMaxDist");
...
CreateGLprojectile(client, type, Float:pos[3] , Float:dir[3], Float:volicity=1000.0, Float:gravity=0.01, Float:modelScale=3.5) {
	if (type==Type_Pistol)volicity=700.0;
	else if (type==Type_Rifle)volicity=600.0;
	else if (type==Type_Shotgun)volicity=460.0;
	else if (type==Type_Sniper)volicity=1000.0;
	else if (type==Type_Smg)volicity=500.0;
	...
	if (L4D2Version && GetConVarInt(l4d_dangerous_connon_catchfire)==1) {
	...
	if (GetClientTeam(client)==2) {
		...
		if (button & IN_USE) {
			...
			if (time>= ShotTime[client]+1.0) {
				...
				if (type>0) {  
					new connon=GetConVarInt(l4d_dangerous_mode_connon);
					new econnon=GetConVarInt(l4d_dangerous_mode_electromagnetic);
					new flame=GetConVarInt(l4d_dangerous_mode_flamethrower);
				
					if (Bullet[client]>0) {
						if ((button & IN_DUCK)) {
							if (econnon==1)StartElec(client, type);
						} else if ((button & IN_SPEED)) {
							if (flame==1)StartFlame(client, type);
						} else {
							if (connon==1) StartConnon(client, type);
						}
					} else {
						PrintHintText(client, "Please kill infected to get more bullet");
					}
				}
...
public OnPressed(const String:output[], caller, activator, Float:delay) { 
	...
	if (activator>0 && activator<=MaxClients && IsClientInGame(activator)) { 
		...
		PrintHintText(activator,"You pick up some special bullet, Total:%d", Bullet[activator]);
...
GiveBullet(attacker) {
	if (attacker>0 && attacker<=MaxClients && IsClientInGame(attacker)) {
		...
		PrintHintText(attacker,"You get some special bullet, Total:%d", Bullet[attacker]);
...
PrintUsageMessage(client) {
	...
	if (GetConVarInt(l4d_dangerous_mode_connon))PrintToChat(client, "\x01Mini Cannon: \x04E+Fire");

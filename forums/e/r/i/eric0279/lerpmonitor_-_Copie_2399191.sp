...
ProcessPlayerLerp(client) {	
	...
	if ((FloatCompare(newLerpTime, GetConVarFloat(cVarMinLerp)) == -1) || (FloatCompare(newLerpTime, GetConVarFloat(cVarMaxLerp)) == 1)) {
		...
		CPrintToChatAll("{teamcolor}%N {default}was moved to spectators for lerp {olive}%.01f{default}!", client, newLerpTime*1000);
		ChangeClientTeam(client, L4D_TEAM_SPECTATE);
		//PrintToChat(client, "Illegal lerp value (min: %.01f, max: %.01f)",
		CPrintToChat(client, "{red}Illegal lerp value {default}({red}min{default}: {red}%.01f{default}, {red}max{default}: {red}%.01f{default})",
	...
	if (index != -1) {
		...
		if (IsMatchLife() || !GetConVarBool(cVarReadyUpLerpChanges)) {
			...
			CPrintToChatAll("{default}[{blue}Lerp{default}] {olive}%N{default}: {blue}%.01f {default}to {blue}%.01f {default}[{green}%s%d{default}/{green}%d{default}]", client, currentLerpTime*1000, newLerpTime*1000, ((count > max)?"{green}":""), count, max);
		
			if (GetConVarBool(cVarLerpChangeSpec) && (count > max)) {
				
				//PrintToChatAll("%N was moved to spectators (illegal lerp change)!", client);
				CPrintToChatAll("{teamcolor}%N {green}was moved to spectators {default}({green}illegal lerp change{default})!", client);
				ChangeClientTeam(client, L4D_TEAM_SPECTATE);
				//PrintToChat(client, "Illegal change of the lerp midgame! Change it back to %.01f", currentLerpTime*1000);
				CPrintToChat(client, "{red}Illegal change of the lerp midgame{green}! {olive}Change it back to {red}%.01f", currentLerpTime*1000);
				// no lerp update
				return;
			}
			...
			CPrintToChatAll("{default}[{blue}Lerp{default}] {olive}%N{default}: {blue}%.01f {default}to {blue}%.01f", client, newLerpTime*1000);
		}
		...
		//PrintToChatAll("%N's lerp set to %.01f", client, newLerpTime*1000); <Lerp> FYGO @ 0.0
		switch(GetClientTeam(client))
		{
			case L4D_TEAM_SPECTATE:
			{
				CSkipNextClient(client);
				CPrintToChatAll("{default}[{olive}Lerp{default}] {default}%N {default}@ {olive}%.01f{default}", client, newLerpTime*1000);
			}
			case L4D_TEAM_SURVIVORS:
			{
				CSkipNextClient(client);
				CPrintToChatAll("{default}[{olive}Lerp{default}] {blue}%N {default}@ {olive}%.01f{default}", client, newLerpTime*1000);
			}
			case L4D_TEAM_INFECTED:
			{
				CSkipNextClient(client);
				CPrintToChatAll("{default}[{olive}Lerp{default}] {red}%N {default}@ {olive}%.01f{default}", client, newLerpTime*1000);
			}
		}
		//CPrintToChatAll("{default}[{blue}Lerp{default}] {blue}%N{default}>> {blue}%.01f", client, newLerpTime*1000);

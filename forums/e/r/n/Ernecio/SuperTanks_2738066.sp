/** ===================================================================================================================
	...
	[06/08/2020] Versión 1.4.1
	
	- Firmas de left 4 dead agregadas dentro del archivo principal, esto remueve el archivo Gamedata.
	- Nuevo método de arrays agregado en aparición de infectados especiales.
	
	[06/08/2021] Versión 1.4.2
	- Corrección rápida a bucle de sonido infinito en reemplazo de rocas del Shield Tank, Jockey Tank, Hunter Tank.
...
//#define PARTICLE_WATER	"water_splash" // Efecto de hondas en agua pequeño.
...
void InitSDKCalls()
{
	...
	if( bL4DTwo ) 								// Signatures Of Left 4 Dead 2
	{
		/////////////
		//SpitBurst//
		/////////////
		StartPrepSDKCall(SDKCall_Player);
		if(!PrepSDKCall_SetSignature( SDKLibrary_Server, "\x55\x8B\xEC\x81\xEC\x94\x00\x00\x00\x2A\x2A\x2A\x2A\x2A\x2A\x2A\x2A\x2A\x2A\x53\x8B\xD9", 22 ) )
			PrepSDKCall_SetSignature( SDKLibrary_Server, "@_ZN18CSpitterProjectile8DetonateEv", 0 );
		...
		/////////////////
		StartPrepSDKCall(SDKCall_Player);
		if(!PrepSDKCall_SetSignature( SDKLibrary_Server, "\x55\x8B\xEC\x83\xEC\x2A\x53\x56\x57\x8B\xF1\xE8\x2A\x2A\x2A\x2A\x84\xC0\x74\x2A\x8B\x06\x8B", 23 ) )
			PrepSDKCall_SetSignature( SDKLibrary_Server, "@_ZN13CTerrorPlayer13OnVomitedUponEPS_b", 0 );
	...
	else 										// Signatures Of Left 4 Dead 1
	{
		/////////////////
		//VomitOnPlayer//
		/////////////////
		StartPrepSDKCall(SDKCall_Player);
		if(!PrepSDKCall_SetSignature( SDKLibrary_Server, "\x83\xEC\x14\x53\x55\x56\x57\x8B\xF1\xE8\x2A\x2A\x2A\x2A", 14 ) )
			PrepSDKCall_SetSignature( SDKLibrary_Server, "@_ZN13CTerrorPlayer13OnVomitedUponEPS_bb", 0 );
...
public void GamemodeCvarChanged(Handle hConvar, const char[] sOldValue, const char[] sNewValue)
{
	if (bSuperTanksEnabled)
	{
		if (hConvar == hGamemodeCvar)
		{
			...
			if (!StrEqual(sNewValue, "coop", false) && !StrEqual(sNewValue, "realism", false))
			{
				PrintToServer("[SuperTanks] This plugin is only compatible in Coop or Realism gamemodes.");
				PrintToServer("[SuperTanks] Plugin Disabled.");
				hSuperTanksEnabled.SetBool( false );
...
//					PrintToChatAll( "\x01Entity\x04[\x03%i\x04]\x01 Atacante\x04[\x03%N\x04]\x01 Victíma\x04[\x03%N\x04]", entity, attacker, i ); // Test.
				}
			}
		}
	}

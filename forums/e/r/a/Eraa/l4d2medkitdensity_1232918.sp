...
new Handle:timer_KitsDensityOutdoors = INVALID_HANDLE
...
public OnPluginStart()
{
	...
	if(!IsModelPrecached(DEFIB))
		PrecacheModel(DEFIB)
	
	if(!IsModelPrecached(PILLS))
		PrecacheModel(PILLS)
	
	// Register Cmds and Cvars
	CreateConVar("medkitdensity_version", PLUGIN_VERSION, "Medkit Density version", CVAR_FLAGS|FCVAR_NOTIFY|FCVAR_REPLICATED|FCVAR_DONTRECORD)
	...
	h_KitsSaferoom 			= CreateConVar("sm_md_saferoom", "1", "What to do with medkits in saferooms? 0[do nothing] 1[use sm_md_checkpoint_medkitcount] 2[remove medkits] 3[replace with pills] 4[replace with defibrillators] 5[replace 1 medkit with 1 defibrillator]", CVAR_FLAGS, true, 0.0, true, 5.0)
	...
	h_KitsOutdoors 			= CreateConVar("sm_md_outdoors", "1", "What to do with each medkit outdoors? 0[do nothing] 1[use sm_md_outdoors_medkitcount] 2[remove medkits] 3[replace with pills] 4[replace with defibrillators]", CVAR_FLAGS, true, 0.0, true, 4.0)
...
public Event_RoundFreezeEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(GetConVarBool(h_PluginEnabled))
	{
		...
		if((StrContains(sGameMode, "versus", false) != -1) || ((StrContains(sGameMode, "scavenge", false) == -1) && !bOnlyVersus))
		{
			FindLocationStart()
			FindLocationSaferoom()
			SetKitsDensity_Start()
			SetKitsDensity_Saferoom()
			if(timer_KitsDensityOutdoors == INVALID_HANDLE)
				timer_KitsDensityOutdoors = CreateTimer(10.0, Timer_KitsDensityOutdoors, _, TIMER_REPEAT)
		}
	}
}

public Action:Timer_KitsDensityOutdoors(Handle:timer)
{
	SetKitsDensity_Outdoors()
...
stock FindLocationStart()
{
	...
	if(StrContains(sMapName, "m1", false) != -1)
	{
	...
	if(StrContains(sMapName, "c2m3", false) != -1) return
	...
	while((ent = FindEntityByClassname(ent, "weapon_ammo_spawn")) != -1)
	{
		...
		{
			GetEntPropVector(ent, Prop_Send, "m_vecOrigin", vecLocation)
			if(GetVectorDistance(vecLocationStart, vecLocation, false) < 2000)
			{
				vecLocationStart = vecLocation
				break
			}
...
stock FindLocationSaferoom()
{
	...
	while((ent = FindEntityByClassname(ent, "weapon_ammo_spawn")) != -1)
	{
		...
		{
			GetEntPropVector(ent, Prop_Send, "m_vecOrigin", vecLocation)
			if(GetVectorDistance(vecLocationCheckpoint, vecLocation, false) < 2000)
			{
				vecLocationCheckpoint = vecLocation
				return
			}
...
{	
	...
	while((ent = FindEntityByClassname(ent, "weapon_first_aid_kit_spawn")) != -1)
	{
		if(IsValidEntity(ent))
		{
			...
			{
				if((StrContains(sMapName, "m1", false) != -1) || (StrContains(sGameMode, "versus", false) != -1))
				{
					if(iKitsStart == 5)
					{
					...
					else if(iKitsStart == 2)
					{
						AcceptEntityInput(ent, "Kill")
					}
					else if(iKitsStart == 1)
					{
						if(iKitsStartCount > 4)
						{
							// set medkit count (1 medkit found, 6 medkits in total)
							if(StrContains(sMapName, "c4m1", false) != -1)
							{
	...
	while((ent = FindEntityByClassname(ent, "weapon_first_aid_kit_spawn")) != -1)
	{
		if(IsValidEntity(ent))
		{
			...
			if(GetVectorDistance(vecClosestKitCheckpoint, vecLocation, false) < 200)
			{
				...
				else if(iKitsSaferoom == 4)
				{
					ReplaceOrAddEnt(ent, "weapon_defibrillator", true)
				}
				else if(iKitsSaferoom == 3)
				{
					ReplaceOrAddEnt(ent, "weapon_pain_pills", true)
				}
				else if(iKitsSaferoom == 2)
				{
					AcceptEntityInput(ent, "Kill")
				}
				else if(iKitsSaferoom == 1)
				{
		...
		if(IsValidEntity(ent))
		{
			...
			if((GetVectorDistance(vecClosestKitStart, vecLocation, false) > 800) && (GetVectorDistance(vecClosestKitCheckpoint, vecLocation, false) > 800))
			{
				...
				else
				{
					...
					else if(iKitsOutdoors == 3)
					{
						ReplaceOrAddEnt(ent, "weapon_pain_pills", true)
					}
					else if(iKitsOutdoors == 2)
					{
						AcceptEntityInput(ent, "Kill")
					}
					else if(iKitsOutdoors == 1)
					{
...
{	
	if(timer_KitsDensityOutdoors != INVALID_HANDLE)
	{
		KillTimer(timer_KitsDensityOutdoors)
		timer_KitsDensityOutdoors = INVALID_HANDLE
	}
}
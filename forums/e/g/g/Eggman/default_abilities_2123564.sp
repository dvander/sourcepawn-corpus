...
Charge_BraveJump(const String:ability_name[], index, slot, status)
{
	...
	switch(status)
	{
		case 1:
		{
			if(!(FF2_GetFF2flags(index) & FF2FLAG_HUDDISABLED))
			{
				SetHudTextParams(-1.0, 0.88, 0.15, 255, 255, 255, 255);
				ShowSyncHudText(index, jumpHUD, "%t", "jump_status_2", -RoundFloat(charge));
			}
		}
		case 2:
		{
			if(!(FF2_GetFF2flags(index) & FF2FLAG_HUDDISABLED))
			{
				...
				{
					ShowSyncHudText(client, jumpHUD, "%t", "jump_status", RoundFloat(charge));
				}
			}
		}
		case 3:
		{
			...
			{
				new Action:action=Plugin_Continue;
				new bool:superJump=enableSuperDuperJump[index];
				...
				{
					enableSuperDuperJump[index]=superJump;
				}
				...
				if(GetConVarBool(cvarOldJump))
				{
					if(enableSuperDuperJump[index])
					{
						...
						enableSuperDuperJump[index]=false;
					}
					else
					{
				...
				if(FF2_RandomSound("sound_ability", sound, PLATFORM_MAX_PATH, index, slot))
				{

	...
	//IT MEANS BOSS' INDEX
	{
		...
		if(!(FF2flags[Boss[client]] & FF2FLAG_HUDDISABLED))
		{
			...
			if(RoundFloat(BossCharge[client][0])==100)
			{
				...
				decl String:sound[PLATFORM_MAX_PATH];
				if(emitRageSound[client] && RandomSound("sound_full_rage", sound, PLATFORM_MAX_PATH, client))
				{
					decl String:name[32];
					KvRewind(BossKV[Special[client]]);
					KvGetString(BossKV[Special[client]], "name", name, 64, "=Failed name=");				
					SetHudTextParams(-1.0, 0.17, 5.1, 255, 255, 255, 255);					
					for(new enemy=1; enemy<=MaxClients; enemy++)
						if(IsValidClient(enemy) && !IsBoss(enemy))
							ShowSyncHudText(enemy, rageHUD, "%t", "ff2_full_rage", name);
					new Float:position[3];
					GetEntPropVector(Boss[client], Prop_Send, "m_vecOrigin", position);

					FF2flags[Boss[client]]|=FF2FLAG_TALKING;
					EmitSoundToAll(sound, Boss[client], _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Boss[client], position, NULL_VECTOR, true, 0.0);
					EmitSoundToAll(sound, Boss[client], _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Boss[client], position, NULL_VECTOR, true, 0.0);
					...
					for(new target=1; target<=MaxClients; target++)
					{
						...
						{
							EmitSoundToClient(target, sound, Boss[client], _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Boss[client], position, NULL_VECTOR, true, 0.0);
							EmitSoundToClient(target, sound, Boss[client], _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Boss[client], position, NULL_VECTOR, true, 0.0);
						}
					}
					FF2flags[Boss[client]]&=~FF2FLAG_TALKING;
...
public Action:event_hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for(i=1; i<BossLives[index]; i++)
	{
		...
		{	
			...
			new Action:act=Plugin_Continue;
			Call_StartForward(OnLosingLife);
			Call_PushCell(index);
			Call_Finish(act);
...
stock CalcBossHealthMax(index)
{
	...
	KvRewind(BossKV[Special[index]]);
	KvGetString(BossKV[Special[index]], "health_formula",formula, 128,"((460+n)*n)^1.075");
...
public Native_SetHealthMax(Handle:plugin, numParams)
{
	BossHealthMax[GetNativeCell(1)]=GetNativeCell(2);
}

public Native_GetLives(Handle:plugin, numParams)
{
	return BossLives[GetNativeCell(1)];
}

public Native_GetLivesMax(Handle:plugin, numParams)
{
	return BossLivesMax[GetNativeCell(1)];
}

public Native_SetLives(Handle:plugin, numParams)
{
	BossLives[GetNativeCell(1)]=GetNativeCell(2);
}

public Native_SetLivesMax(Handle:plugin, numParams)
{

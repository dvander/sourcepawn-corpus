...
public OnPluginStart()
{
	...
	voteNotify          = CreateConVar("l4d_vote_notify_access",          "bcdefz",  "Who sees certain vote related notices. If blank everyone sees them.", CVAR_FLAGS);
	...
	AutoExecConfig(true, "plugin_votemanager2");
...
public Action:Callvote_Handler(client, args)
{
	...
	if (postVoteDelay)
	{
		PrintToChat(client, "\x04[!] \x01Must wait \x03%f \x01seconds between votes.", VOTE_DELAY);
	...
	{
	       	PrintToChat(client,"\x04[!] \x01Invalid vote type: \x03%s",voteName);
	...
	else
	{
		...
		//PrintToChatAll("\x04[Vote] \x03%s \x01tried to start a \x03%s \x01vote but does not have access.", initiatorName, voteName);
...
public Action:Kick_Vote_Logic(client, args)
{
	...
	if ((target<=0) || (!IsClientInGame(target)))
	{
		...
		Notify(client, "\x04[!] \x03%s \x01tried to start a Kick Vote against \x03%s \x01but that is not a valid target.", initiatorName,arg2);
		PrintToChat(client, "\x04[!] \x01If you are trying to call a manual kick vote the format is: '\x03callvote kick <user id>\x01'");
		return Plugin_Handled;
	}
	...
	if (GetConVarBool(kickImmunity))
	{
		...
		if (isAdmin(targetAdminId)){
			...
			if (!CanAdminTarget(clientAdminId, targetAdminId))
			{
				...
				//PrintToChatAll("\x04[Vote] \x03%s \x01tried to start a Kick Vote against \x03%s \x01but he has immunity.",initiatorName,targetName);
	...
	//PrintToChatAll("\x04[Vote] \x03%s \x01is starting a Kick Vote against \x03%s\x01.",initiatorName,targetName);
	Notify(client, "\x04[Vote] \x03%s \x01is starting a Kick Vote against \x03%s\x01.",initiatorName,targetName);
	return Plugin_Continue;
}
...
public Action:Veto_Handler(client, args)
{
	...
	{	
		...
		LogVote(client,"has vetoed a vote.");
		//PrintToChatAll("\x04[SM] \x03%s \x01has vetoed this vote.",vetoerName);
		Notify(client, "\x04[SM] \x03%s \x01has vetoed this vote.",vetoerName);
		return Plugin_Continue;
		
	}
	LogVote(client,"failed to veto vote. Reason: Access");
	//PrintToChatAll("\x04[!] \x03%s \x01tried to veto a vote but does not have access.",vetoerName);
...
public Action:PassVote_Handler(client, args)
{
	...
	{	
		...
		LogVote(client,"has passed a vote.");
		//PrintToChatAll("\x04[SM] \x03%s \x01has passed this vote.",passerName);
		Notify(client, "\x04[SM] \x03%s \x01has passed this vote.",passerName);
		return Plugin_Continue;
		
	}
	LogVote(client,"failed to veto vote. Reason: Access");
	//PrintToChatAll("\x04[!] \x03%s \x01tried to pass a vote but does not have access.",passerName);
...
public Action:CustomVote_Handler(client, args)
{
	...
	else
	{
		...
		//PrintToChatAll("\x04[!] \x03%s \x01tried starting a Custom vote but one is already in progress.",initiatorName);
...
public Action:Custom_Vote_Logic(client, args)
{
	...
	if (!customVoteInProgress)
	{
		...
		Notify(client, "\x04[Vote] \x03%s \x0is starting a Custom vote.",initiatorName);

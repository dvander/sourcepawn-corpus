...
bool ClientUseRPGList[128] = {false};
...
public OnPluginStart()
{
	...
	hRPGName = CreateConVar("missile_weaponname", "weapon_hegrenade", "Set the weapon that applies the rocket (missile) effect (default value is weapon_hegrenade), generally do not change it, just keep the default value. Only valid if missile_replace's value is 0, and the RPG is added through the add new gun plugin and the parent weapon is a grenade, can be modified).", FCVAR_PLUGIN);
	...
	HookEvent("weapon_fire", WeaponFire_RPG, EventHookMode_Pre);
...
//武器射击事件函数
public WeaponFire_RPG(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if(StrEqual("hegrenade_projectile", ReplaceNade, false))
	{
		...
		if(StrEqual(sWeapon, RPGName))
		{
			ClientUseRPGList[client] = true;
			CreateTimer(0.2, Hegrenade_Throw2, client);
		}		
	}
	return Plugin_Continue;
}
...
public Action:Hegrenade_Throw2(Handle timer, int client)
{
	ClientUseRPGList[client] = false;
}
...
public OnEntityCreated(entity, const String:classname[])
{
	if (StrEqual(classname, ReplaceNade, false))
	{
		...
		if(StrEqual("hegrenade_projectile", ReplaceNade, false))
		{
			...
			CreateTimer(0.06, Hegrenade_Throw3, entity);	
		}
		else
		{			
...
public Action:Hegrenade_Throw3(Handle timer, int entity)
{
	...
	if(owner > 0 && owner <= MaxClients)
	{
		...
		// if(!StrEqual(RPGName, "weapon_hegrenade") && ClientUseRPGList[owner] == false)
		if(ClientUseRPGList[owner] == false)
		{
			return;
		}
		// SetHudTextParams(0.895, 0.8, 6.0, 255, 176, 0, 30, 0, 1.0, 0.1, 0.2);    //金钱数HUD上方
		// ShowHudText(owner, -1, "火箭发射");

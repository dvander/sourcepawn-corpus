...
ConVar g_Cvar_RTVWhenAdminAvails;
...
public OnPluginStart()
{
	...
	g_Cvar_RTVWhenAdminAvails = CreateConVar("sm_rtv_whenadminavails", "1", "Disable RTV when ADMIN available in server?", _, true, 0.0, true, 1.0);
...
AttemptRTV(client)
{
	...
	if (AdminCount >= 1 && g_Cvar_RTVWhenAdminAvails.BoolValue)
	{
		ReplyToCommand(client, "[SM] You can't use RTV if there is atleast one admin");
		return;
	}

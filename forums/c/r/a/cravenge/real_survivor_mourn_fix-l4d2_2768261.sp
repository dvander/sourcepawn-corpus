...
Address aRSMF;
static int iOriginalBytes_aRSMF = -1;
...
public void OnPluginStart()
{
	...
	aRSMF = gd_Temp.GetAddress("CheckFriendSightings");
	if (aRSMF != Address_Null)
	{
		int iTemp = gd_Temp.GetOffset("CheckFriendSightings_CharacterCondition");
		if (iTemp != -1)
		{
			aRSMF += view_as<Address>(iTemp);
			
			iOriginalBytes_aRSMF = LoadFromAddress(aRSMF, NumberType_Int8);
			if (iOriginalBytes_aRSMF != 0x03 && iOriginalBytes_aRSMF != 0x04)
			{
				SetFailState("Offset for \"CheckFriendSightings_CharacterCondition\" is incorrect!");
			}
			
			StoreToAddress(aRSMF, iOriginalBytes_aRSMF + 0x04, NumberType_Int8);
			
			PrintToServer("[FIX] Patched \"PlayerSeeDeadPlayer\" concept to check for all 8 characters instead");
			
			delete gd_Temp;
		}
		else
		{
			SetFailState("[FIX] Offset for \"CheckFriendSightings_CharacterCondition\" is missing!");
		}
	}
	else
	{
...
public void OnPluginEnd()
{
	...
	PrintToServer("[FIX] Restoring the original check for \"PlayerSeeDeadPlayer\" concept...");
	
	StoreToAddress(aRSMF, iOriginalBytes_aRSMF, NumberType_Int8);
	
	iOriginalBytes_aRSMF = -1;
}
...
GameData FetchGameData(const char[] file)
{
	...
	if (!FileExists(sFilePath))
	{
		...
		fileTemp.WriteLine("		{");
		fileTemp.WriteLine("			\"CheckFriendSightings_CharacterCondition\"");
		fileTemp.WriteLine("			{");

...
public void OnPluginStart()
{
	...
	bawnOutline.AddChangeHook(OnBAWNCVarsChanged);
...
{
	bawnAnnounce.RemoveChangeHook(OnBAWNCVarsChanged);
	bawnAnnounceMode.RemoveChangeHook(OnBAWNCVarsChanged);
	bawnOutline.RemoveChangeHook(OnBAWNCVarsChanged);
	
	delete bawnAnnounce;
	delete bawnAnnounceMode;
	delete bawnOutline;
	...
	UnhookEvent("player_bot_replace", OnReplaceEvents);
	UnhookEvent("bot_player_replace", OnReplaceEvents);
}
...
public Action CheckPlayers(Handle timer)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i) && GetEntProp(i, Prop_Send, "m_currentReviveCount") == FindConVar("survivor_max_incapacitated_count").IntValue && !bIsBW[i])
		{
			bIsBW[i] = true;
			SurvivorGlow(i, true);
		}
...
public void OnRoundEnd(Event event, const char[] name, bool dontBroadcast)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i) && bIsBW[i])
		{
			bIsBW[i] = false;

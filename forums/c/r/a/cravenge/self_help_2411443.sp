...
public void OnPluginStart()
{
	...
	shBotChance = CreateConVar("self_help_bot_chance", "1", "Chance Of Bot Self-Helping: 1=Sometimes, 2=Often, 3=Seldom", FCVAR_NOTIFY|FCVAR_SPONLY, true, 1.0, true, 3.0);
...
public void OnPlayerDown(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (IsSurvivor(wounded))
	{
		...
		{
			PrintHintText(wounded, "[SH]\nHold R To Revive Other Incapacitated Survivors!");
			...
			if (bIsL4D)
			{
				...
				C_PrintToChat(wounded, "{blue}[SH]{default} You Got Incapacitated! |{green}%d{default}/{green}%i{default}|", iSHCount[wounded] + 1, iMaxCount);
			...
			else
			{
				...
				C_PrintToChat(wounded, "{blue}[SH]{default} You Got Incapacitated! |{green}%d{default}/{green}%i{default}|", iReviveCount + 1, iMaxIncapCount);
...
public Action FireUpMechanism(Handle timer, any userid)
{
	...
	if (hSHTime[client] == null)
	{
		...
		{
			C_PrintToChat(client, "{blue}[SH]{default} Press {green}CTRL{default} To Self-Help!");
		}
		...
		if (IsSurvivor(iTarget) && IsPlayerAlive(iTarget) && GetEntProp(iTarget, Prop_Send, "m_isIncapacitated", 1) && iAttacker[iTarget] == 0)
		{
			if (shsBit[client] == SHS_NONE || shsBit[client] == SHS_CONTINUE)
			{
				...
				if (!IsFakeClient(iTarget))
				{
					...
					if (!bIsL4D)
					{
						PrintHintText(iTarget, "[SH]\n%N Is Helping You!", client);
					}
				}
	...
	if (IsSurvivor(reviver) && IsSurvivor(revived))
	{
		...
		if (event.GetBool("ledge_hang"))
		{
			if (reviver != revived)
			{
				...
				{
					C_PrintToChat(reviver, "{blue}[SH] {default}You Helped{olive} %N{default}!", revived);
				}
				
				if (!IsFakeClient(revived))
				{
					C_PrintToChat(revived, "{blue}[SH] {olive}%N{default} Helped You!", reviver);
				}
			}
			else
			{
				if (!IsFakeClient(revived))
				{
					C_PrintToChat(revived, "{blue}[SH] {default}You Helped Yourself!");
				}
			}
		}
		else
		{
			if (!bIsL4D)
			{
				...
				if (reviver == revived)
				{
					if (!IsFakeClient(revived))
					{
						C_PrintToChat(revived, "{blue}[SH] {default}You Helped Yourself! |{green}%d{default}/{green}%i{default}|", iReviveCount, iMaxIncapCount);
					}
				}
				else
				{
					if (!IsFakeClient(reviver))
					{
						...
						{
							C_PrintToChatAll("{blue}[SH] {olive}%N{default} Saved{olive} %N {default}While Incapacitated!", reviver, revived);
						}
						
						C_PrintToChat(reviver, "{blue}[SH] {default}You Helped{olive} %N{default}! |{green}%d{default}/{green}%i{default}|", revived, iReviveCount, iMaxIncapCount);
					}
					
					if (!IsFakeClient(revived))
					{
						C_PrintToChat(revived, "{blue}[SH] {olive}%N{default} Helped You! |{green}%d{default}/{green}%i{default}|", reviver, iReviveCount, iMaxIncapCount);
					}
				}
			}
			else
			{
				...
				if (reviver == revived)
				{
					if (!IsFakeClient(revived))
					{
						C_PrintToChat(revived, "{blue}[SH] {default}You Helped Yourself! |{green}%d{default}/{green}%i{default}|", iSHCount[revived], iMaxCount);
					}
				}
				else
				{
					if (!IsFakeClient(reviver))
					{
						...
						C_PrintToChat(reviver, "{blue}[SH] {default}You Helped{olive} %N{default}! |{green}%d{default}/{green}%i{default}|", revived, iSHCount[revived], iMaxCount);
					}
					
					if (!IsFakeClient(revived))
					{
						C_PrintToChat(revived, "{blue}[SH] {olive}%N{default} Helped You! |{green}%d{default}/{green}%i{default}|", reviver, iSHCount[revived], iMaxCount);
					}
				}
			}
		}
...
public Action DelaySHNotify(Handle timer, Handle dpDefibAnnounce)
{
	...
	if (defibber == defibbed)
	{
		...
		{
			C_PrintToChat(defibbed, "{blue}[SH] {default}You Defibbed Yourself! |{green}%d{default}/{green}%i{default}|", iReviveCount, iMaxIncapCount);
		}
	}
	else
	{
		...
		{
			C_PrintToChat(defibber, "{blue}[SH] {default}You Defibbed{olive} %N{default}! |{green}%d{default}/{green}%i{default}|", defibbed, iReviveCount, iMaxIncapCount);
		}
		
		if (!IsFakeClient(defibbed))
		{
			C_PrintToChat(defibbed, "{blue}[SH] {olive}%N{default} Defibbed You! |{green}%d{default}/{green}%i{default}|", defibber, iReviveCount, iMaxIncapCount);
		}
	}
...
void SHStatsFixer(int client, bool bDoNotTamper, bool bUseItem = true, bool &bMedkitUsed)
{
	...
	if (bUseItem)
	{
		...
		if ((bEmergencyUsed || bFirstAidUsed) && RemovePlayerItem(client, iUsedItem))
		{
			...
			if (bFirstAidUsed)
			{
				C_PrintToChatAll("{blue}[SH] {olive}%N {default}Helped Themselves With{green} Medkit{default}!", client);
			...
			else if (bEmergencyUsed)
			{
				if (!bIsL4D && StrEqual(sUsedItemName, "weapon_adrenaline", false))
				{
					...
					C_PrintToChatAll("{blue}[SH] {olive}%N {default}Helped Themselves With{green} Adrenaline{default}!", client);
				}
				else
				{
					...
					C_PrintToChatAll("{blue}[SH] {olive}%N {default}Helped Themselves With{green} Pills{default}!", client);
				}
			}
		}

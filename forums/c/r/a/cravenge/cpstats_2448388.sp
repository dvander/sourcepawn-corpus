...
#define PLUGIN_DESCRIPTION "Provides And Manages Players' Stats And Rankings."
...
#define SOUND_RANKVOTE "buttons/bell1.wav"
...
new bool:IsPenaltyOn[MAXPLAYERS+1] = false;

public Plugin:myinfo =
{
	name = PLUGIN_NAME,
	author = "Mikko Andersson (muukis), cravenge",
...
public OnPluginStart()
{
	...
	{
		SetFailState("[CPS] Plugin Supports L4D and L4D2 Only!");
		return;
	}
	...
	cvar_AdminPlayerCleanLastOnTime = CreateConVar("cpstats_adm_cleanoldplayers", "2", "Months Before Old Players Are Wiped", FCVAR_NOTIFY, true, 0.0);
	cvar_AdminPlayerCleanPlatime = CreateConVar("cpstats_adm_cleanplaytime", "30", "Minimum Playtime To Be Excluded From Wiping", FCVAR_NOTIFY, true, 0.0);
	cvar_EnableRankVote = CreateConVar("cpstats_rankvote", "1", "Enable/Disable Rank Vote", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_HumansNeeded = CreateConVar("cpstats_minhumans", "1", "Minimum Human Players For Plugin To Activate", FCVAR_NOTIFY, true, 1.0, true, 4.0);
	cvar_UpdateRate = CreateConVar("cpstats_updaterate", "90", "Interval Between Each Point Updates", FCVAR_NOTIFY, true, 30.0);
	cvar_AnnounceRankChange = CreateConVar("cpstats_announcerank", "1", "Enable/Disable Rank Change Announcements", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_AnnounceRankChangeIVal = CreateConVar("cpstats_announcerankinterval", "60", "Interval Between Each Rank Change Checks", FCVAR_NOTIFY, true, 10.0);
	cvar_AnnouncePlayerJoined = CreateConVar("cpstats_announceplayerjoined", "2", "", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_AnnounceMotd = CreateConVar("cpstats_announcemotd", "1", "Enable/Disable MOTD Announcements", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_AnnounceMode = CreateConVar("cpstats_announcemode", "1", "Announcement Mode: 1=Own Only, 2=Own With Public Head Shots, 3=Public", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	cvar_AnnounceToTeam = CreateConVar("cpstats_announceteam", "1", "Team Announcements Mode: 0=Enemy Team Only, 1=Own Team Only, 2=Own And Spectators Team Only", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	cvar_MedkitMode = CreateConVar("cpstats_medkitmode", "1", "Medkit Points Award Mode: 0=Amount-Based, 1=Static", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_SiteURL = CreateConVar("cpstats_siteurl", "http://www.hcservers.com/l4d2", "Community Site URL", FCVAR_NOTIFY);
	cvar_RankOnJoin = CreateConVar("cpstats_rankonjoin", "1", "Enable/Disable Rank On Join Announcements", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_SilenceChat = CreateConVar("cpstats_silencechat", "1", "Enable/Disable Silent Chat Triggers", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_DisabledMessages = CreateConVar("cpstats_disabledmessages", "1", "Enable/Disable Disabled Messages", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_DbPrefix = CreateConVar("cpstats_dbprefix", "cpstats_", "Prefix For Stats", FCVAR_NOTIFY);
	cvar_EnableNegativeScore = CreateConVar("cpstats_negativescore", "1", "Enable/Disable Negative Scores", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_FriendlyFireMode = CreateConVar("cpstats_ffire_mode", "2", "FF Mode: 0=Normal, 1=Cooldown, 2=Damage-Based", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	cvar_FriendlyFireMultiplier = CreateConVar("cpstats_ffire_multiplier", "1.5", "Multiplier Applied To FF", FCVAR_NOTIFY, true, 0.0);
	cvar_FriendlyFireCooldown = CreateConVar("cpstats_ffire_cooldown", "10.0", "Cooldown Time For FF", FCVAR_NOTIFY, true, 1.0);
	cvar_FriendlyFireCooldownMode = CreateConVar("cpstats_ffire_cooldownmode", "1", "FF Cooldown Mode: 1=Player Specific, 2=General", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	cvar_Enable = CreateConVar("cpstats_all", "1", "Enable/Disable All Stat Tracking", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_EnableCoop = CreateConVar("cpstats_coop", "1", "Enable/Disable Coop Stat Tracking", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_EnableSv = CreateConVar("cpstats_sv", "1", "Enable/Disable Survival Stat Tracking", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_EnableVersus = CreateConVar("cpstats_versus", "1", "Enable/Disable Versus Stat Tracking", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_EnableTeamVersus = CreateConVar("cpstats_teamversus", "1", "[L4D2] Enable/Disable Team Versus Stat Tracking", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_EnableRealism = CreateConVar("cpstats_realism", "1", "[L4D2] Enable/Disable Realism Stat Tracking", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_EnableScavenge = CreateConVar("cpstats_scavenge", "1", "[L4D2] Enable/Disable Scavenge Stat Tracking", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_EnableTeamScavenge = CreateConVar("cpstats_teamscavenge", "1", "[L4D2] Enable/Disable Team Scavenge Stat Tracking", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_EnableRealismVersus = CreateConVar("cpstats_realismvs", "1", "[L4D2] Enable/Disable Realism Versus Stat Tracking", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_EnableTeamRealismVersus = CreateConVar("cpstats_teamrealismvs", "1", "[L4D2] Enable/Disable Team Realism Versus Stat Tracking", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_EnableMutations = CreateConVar("cpstats_mutations", "1", "[L4D2] Enable/Disable Mutations Stat Tracking", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_RealismMultiplier = CreateConVar("cpstats_realismmultiplier", "1.5", "[L4D2] Multiplier Applied To Realism Scores", FCVAR_NOTIFY, true, 1.0);
	cvar_RealismVersusSurMultiplier = CreateConVar("cpstats_realismvsmultiplier_s", "1.25", "[L4D2] Multiplier Applied To Realism Versus Survivor Scores", FCVAR_NOTIFY, true, 1.0);
	cvar_RealismVersusInfMultiplier = CreateConVar("cpstats_realismvsmultiplier_i", "1.25", "[L4D2] Multiplier Applied To Realism Versus Infected Scores", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_EnableSvMedicPoints = CreateConVar("cpstats_medicpointssv", "1", "Enable/Disable Survival Medic Points", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_Infected = CreateConVar("cpstats_infected", "1", "Base Score For Killing Common Infected", FCVAR_NOTIFY, true, 1.0);
	cvar_Hunter = CreateConVar("cpstats_hunter", "5", "Base Score For Killing Hunters", FCVAR_NOTIFY, true, 1.0);
	cvar_Smoker = CreateConVar("cpstats_smoker", "2", "Base Score For Killing Smokers", FCVAR_NOTIFY, true, 1.0);
	cvar_Boomer = CreateConVar("cpstats_boomer", "3", "Base Score For Killing Boomers", FCVAR_NOTIFY, true, 1.0);
	cvar_Spitter = CreateConVar("cpstats_spitter", "3", "[L4D2] Base Score For Killing Spitters", FCVAR_NOTIFY, true, 1.0);
	cvar_Jockey = CreateConVar("cpstats_jockey", "2", "[L4D2] Base Score For Killing Jockeys", FCVAR_NOTIFY, true, 1.0);
	cvar_Charger = CreateConVar("cpstats_charger", "5", "[L4D2] Base Score For Killing Chargers", FCVAR_NOTIFY, true, 1.0);
	cvar_InfectedDamage = CreateConVar("cpstats_infected_damage", "2", "Infected Damage Inflicted To Earn Points", FCVAR_NOTIFY, true, 1.0);
	cvar_Pills = CreateConVar("cpstats_pills", "5", "Base Score For Giving Pills", FCVAR_NOTIFY, true, 1.0);
	cvar_Adrenaline = CreateConVar("cpstats_adrenaline", "10", "[L4D2] Base Score For Giving Adrenaline", FCVAR_NOTIFY, true, 1.0);
	cvar_Medkit = CreateConVar("cpstats_medkit", "25", "Base Score For Healing Allies", FCVAR_NOTIFY, true, 1.0);
	cvar_Defib = CreateConVar("cpstats_defib", "12", "[L4D2] Base Score For Defibbing Dead Allies", FCVAR_NOTIFY, true, 1.0);
	cvar_SmokerDrag = CreateConVar("cpstats_smokerdrag", "6", "Base Score For Saving Dragged Survivors", FCVAR_NOTIFY, true, 1.0);
	cvar_JockeyRide = CreateConVar("cpstats_jockeyride", "7", "[L4D2] Base Score For Saving Ridden Survivors", FCVAR_NOTIFY, true, 1.0);
	cvar_ChargerPlummel = CreateConVar("cpstats_chargerplummel", "8", "[L4D2] Base Score For Saving Pummeled Survivors", FCVAR_NOTIFY, true, 1.0);
	cvar_ChargerCarry = CreateConVar("cpstats_chargercarry", "10", "[L4D2] Base Score For Saving Carried Survivors", FCVAR_NOTIFY, true, 1.0);
	cvar_ChokePounce = CreateConVar("cpstats_chokepounce", "5", "Base Score For Saving Choked/Pounced Survivors", FCVAR_NOTIFY, true, 1.0);
	cvar_Revive = CreateConVar("cpstats_revive", "3", "Base Score For Reviving Friendlies", FCVAR_NOTIFY, true, 1.0);
	cvar_Rescue = CreateConVar("cpstats_rescue", "6", "Base Score For Rescuing Friendlies", FCVAR_NOTIFY, true, 1.0);
	cvar_Protect = CreateConVar("cpstats_protect", "4", "Base Score For Protecting Friendlies", FCVAR_NOTIFY, true, 1.0);
	cvar_PlayerLedgeSuccess = CreateConVar("cpstats_ledgegrab", "10", "Base Score For Causing Victim To Grab Ledges", FCVAR_NOTIFY, true, 1.0);
	cvar_Matador = CreateConVar("cpstats_matador", "15", "[L4D2] Base Score From Level A Charge Award", FCVAR_NOTIFY, true, 1.0);
	cvar_WitchCrowned = CreateConVar("cpstats_witchcrowned", "20", "Base Score From Crowned Award", FCVAR_NOTIFY, true, 1.0);
	cvar_Tank = CreateConVar("cpstats_tank", "30", "Base Team Score For Killing Tanks", FCVAR_NOTIFY, true, 1.0);
	cvar_Panic = CreateConVar("cpstats_panic", "20", "Base Team Score For Surviving Panic Events", FCVAR_NOTIFY, true, 1.0);
	cvar_BoomerMob = CreateConVar("cpstats_boomermob", "5", "Base Team Score For Surviving Boomer Mobs", FCVAR_NOTIFY, true, 1.0);
	cvar_SafeHouse = CreateConVar("cpstats_safehouse", "10", "Base Score For Reaching Saferooms", FCVAR_NOTIFY, true, 1.0);
	cvar_Witch = CreateConVar("cpstats_witch", "15", "Base Score From No Witch Disturbed Award", FCVAR_NOTIFY, true, 1.0);
	cvar_VictorySurvivors = CreateConVar("cpstats_campaign", "20", "Base Score For Campaign Completion", FCVAR_NOTIFY, true, 1.0);
	cvar_VictoryInfected = CreateConVar("cpstats_infected_win", "15", "Base Victory Score For Infected Team", FCVAR_NOTIFY, true, 1.0);
	cvar_FFire = CreateConVar("cpstats_ffire", "25", "Penalty Score For Friendly Fires", FCVAR_NOTIFY, true, 1.0);
	cvar_FIncap = CreateConVar("cpstats_fincap", "50", "Penalty Score For Friendly Incaps", FCVAR_NOTIFY, true, 1.0);
	cvar_FKill = CreateConVar("cpstats_fkill", "75", "Penalty Score For Friendly Kills", FCVAR_NOTIFY, true, 1.0);
	cvar_InSafeRoom = CreateConVar("cpstats_insaferoom", "5", "Penalty Score For Contaminated Saferooms", FCVAR_NOTIFY, true, 1.0);
	cvar_Restart = CreateConVar("cpstats_restart", "100", "Penalty Score For Mission Lost", FCVAR_NOTIFY, true, 1.0);
	cvar_MedkitUsedPointPenalty = CreateConVar("cpstats_medkitpenalty", "0.1", "Score Reduction For Medkits", FCVAR_NOTIFY, true, 0.0, true, 0.5);
	cvar_MedkitUsedPointPenaltyMax = CreateConVar("cpstats_medkitpenaltymax", "1.0", "Maximum Score Reduction For Medkits", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvar_MedkitUsedFree = CreateConVar("cpstats_medkitpenaltyfree", "26", "Free These Medkits From Penalties", FCVAR_NOTIFY, true, 0.0);
	cvar_MedkitUsedRealismFree = CreateConVar("cpstats_medkitpenaltyfree_r", "13", "Free These Medkits From Penalties In Realism", FCVAR_NOTIFY, true, -1.0);
	cvar_MedkitBotMode = CreateConVar("cpstats_medkitbotmode", "2", "Medkit Bot Mode: 1=Human Player, 2=Others, 3=All", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	cvar_CarAlarm = CreateConVar("cpstats_caralarm", "50", "[L4D2] Penalty Score For Alarmed Cars Triggered", FCVAR_NOTIFY, true, 1.0);
	cvar_BotScoreMultiplier = CreateConVar("cpstats_botscoremultiplier", "1.0", "Multiplier Applied To Bot-Related Penalties", FCVAR_NOTIFY, true, 0.0);
	cvar_SurvivorDeath = CreateConVar("cpstats_survivor_death", "30", "Base Score For Killing Survivor", FCVAR_NOTIFY, true, 1.0);
	cvar_SurvivorIncap = CreateConVar("cpstats_survivor_incap", "15", "Base Score For Incapacitating Survivor", FCVAR_NOTIFY, true, 1.0);
	cvar_HunterPerfectPounceDamage = CreateConVar("cpstats_perfectpouncedamage", "100", "Pounce Damage Inflicted To Earn Death From Above Award", FCVAR_NOTIFY, true, 1.0);
	cvar_HunterPerfectPounceSuccess = CreateConVar("cpstats_perfectpouncesuccess", "25", "Base Score From Death From Above Pounce Award", FCVAR_NOTIFY, true, 1.0);
	cvar_HunterNicePounceDamage = CreateConVar("cpstats_nicepouncedamage", "50", "Pounce Damage Inflicted To Earn Pain From Above Award", FCVAR_NOTIFY, true, 1.0);
	cvar_HunterNicePounceSuccess = CreateConVar("cpstats_nicepouncesuccess", "12", "Base Score From Pain From Above Pounce Award", FCVAR_NOTIFY, true, 1.0);
	cvar_HunterDamageCap = CreateConVar("cpstats_hunterdamagecap", "100", "Stored Pounce Damage Cap", FCVAR_NOTIFY, true, 25.0);
	...
	cvar_BoomerSuccess = CreateConVar("cpstats_boomersuccess", "1", "Base Score For Blinding Survivor", FCVAR_NOTIFY, true, 1.0);
	cvar_BoomerPerfectHits = CreateConVar("cpstats_boomerperfecthits", "13", "Number Of Survivors Blinded To Earn Perfect Blindness Award", FCVAR_NOTIFY, true, 4.0);
	cvar_BoomerPerfectSuccess = CreateConVar("cpstats_boomerperfectsuccess", "13", "Base Score From Perfect Blindness Award", FCVAR_NOTIFY, true, 1.0);
	cvar_TankDamageCap = CreateConVar("cpstats_tankdmgcap", "5000", "Maximum inflicted damage done by Tank to earn Infected damagepoints", FCVAR_NOTIFY, true, 150.0);
	cvar_TankDamageTotal = CreateConVar("cpstats_bulldozer", "1000", "Tank Damage Inflicted To Earn Bulldozer Award", FCVAR_NOTIFY, true, 200.0);
	cvar_TankDamageTotalSuccess = CreateConVar("cpstats_bulldozersuccess", "250", "Base Score From Bulldozer Award", FCVAR_NOTIFY, true, 1.0);
	cvar_TankThrowRockSuccess = CreateConVar("cpstats_tankthrowrocksuccess", "15", "Base Score For Rock Throws", FCVAR_NOTIFY, true, 0.0);
	cvar_ChargerRamSuccess = CreateConVar("cpstats_chargerramsuccess", "26", "Base Score From Scattering Ram Award", FCVAR_NOTIFY, true, 1.0);
	cvar_ChargerRamHits = CreateConVar("cpstats_chargerramhits", "13", "Number Of Impacts To Earn Scattering Ram Award", FCVAR_NOTIFY, true, 2.0);
	cvar_AmmoUpgradeAdded = CreateConVar("cpstats_deployammoupgrade", "10", "[L4D2] Base Score For Ammo Pack Deployments", FCVAR_NOTIFY, true, 0.0);
	cvar_GascanPoured = CreateConVar("cpstats_gascanpoured", "5", "[L4D2] Base Score For Pouring Gas Can", FCVAR_NOTIFY, true, 0.0);
	cvar_Top10PPMMin = CreateConVar("cpstats_top10ppmplaytime", "60", "Minimum Playtime To Include In Top 10 PPM", FCVAR_NOTIFY, true, 1.0);
	cvar_RankVoteTime = CreateConVar("cpstats_rankvotetime", "20", "Time For Another Rank Vote", FCVAR_NOTIFY, true, 10.0);
	cvar_SoundsEnabled = CreateConVar("cpstats_sounds", "1", "Enable/Disable Sounds", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	if (FileExists("addons/sourcemod/gamedata/left4downtown.l4d2.txt"))
	{
		L4DStatsConf = LoadGameConfigFile("left4downtown.l4d2");
		if (L4DStatsConf == INVALID_HANDLE)
		{
			LogError("[CPS] Game Data Not Found!");
			return;
		}
...
public OnConfigsExecuted()
{
	...
	if (!ConnectDB())
	{
		SetFailState("[CPS] Database Connection Failed!");
		return;
	}
...
public Action:timer_ProtectedFriendly(Handle:timer, any:data)
{
	...
	if (Score > 0)
	{
		...
		if (Mode == 1 || Mode == 2)
		{
			StatsPrintToChat(data, "You Earned \x04%i \x01Points For Protecting \x05%i Friendlies\x01!", Score, ProtectedFriendlies);
		}
		else if (Mode == 3)
		{
			StatsPrintToChatAll("\x05%s \x01Earned \x04%i \x01Points For Protecting \x05%i Friendlies\x01!", UserName, Score, ProtectedFriendlies);
		}
	}
...
public Action:timer_InfectedDamageCheck(Handle:timer, any:data)
{
	...
	if (Score > 0)
	{
		...
		if (Mode == 1 || Mode == 2)
		{
			...
			{
				StatsPrintToChat(data, "You Earned \x04%i \x01Points For Doing Damage To Survivors!", Score, DamageCounter);
			}
		}
		else if (Mode == 3)
		{
			...
			{
				StatsPrintToChatAll("\x05%s \x01Earned \x04%i \x01Points For Doing Damage To Survivors!", Name, Score, DamageCounter);
			}
		}
	}
	...
	if (data > 0 && !IsClientBot(data) && IsClientInGame(data) && GetClientTeam(data) == TEAM_INFECTED && BoomerHitCounter[data] > 0)
	{
		...
		if (Score > 0)
		{
			...
			if (Mode == 1 || Mode == 2)
			{
				if (AwardCounter > 0)
				{
					StatsPrintToChat(data, "You Earned \x04%i \x01Points From \x05Perfect Blindness\x01 Award!", Score);
				}
				else
				{
			...
			else if (Mode == 3)
			{
				...
				if (AwardCounter > 0)
				{
					StatsPrintToChatAll("\x05%s \x01Earned \x04%i \x01Points From \x05Perfect Blindness\x01 Award!", Name, Score);
				}
				else
				{
...
public Action:timer_FriendlyFireDamageEnd(Handle:timer, any:dp)
{
	...
	{
		StatsPrintToChatAll("\x05%s \x03LOST \x04%i \x01Points For Inflicting \x03Friendly Fire Damage \x05(%i HP)\x01!", AttackerName, Score, HumanDamage + BotDamage);
	}
...
public Action:timer_UpdatePlayers(Handle:timer, Handle:hndl)
{
	if (CheckHumans())
	{
		...
		{
			StatsPrintToChatAllPreFormatted("\x04DISABLED\x01! Reason: Insufficient Human Players!");
		}
...
public Action:event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (AttackerTeam == TEAM_SURVIVORS && VictimTeam == TEAM_SURVIVORS)
	{
		...
		{
			StatsPrintToChatAll("\x05%s \x03LOST \x04%i \x01Points For \x03Team Killing \x05%s\x01!", AttackerName, Score, VictimName);
		}
	}
...
public Action:event_TankKilled(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new Score = (BaseScore * (Players - Deaths)) / 4;
...
public Action:event_DefibPlayer(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Score > 0)
	{
		...
		if (Mode == 1 || Mode == 2)
		{
			StatsPrintToChat(Giver, "You Earned \x04%i \x01Points For Defibbing \x05%s\x01!", Score, RecipientName);
		}
		else if (Mode == 3)
		{
			StatsPrintToChatAll("\x05%s \x01Earned \x04%i \x01Points For Defibbing \x05%s\x01!", GiverName, Score, RecipientName);
		}
	}
}
...
public Action:event_CampaignWin(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Mode && Score > 0)
	{
		...
		if (NegativeScore)
		{
			StatsPrintToChatTeam(TEAM_INFECTED, "\x03ALL INFECTED \x03LOST \x04%i \x01Points For Losing \x04Campaign Finale \x01To \x05%i Survivors\x01!", Score, SurvivorCount);
		}
	}
}
...
PlayerIncap(Attacker, Victim)
{
	...
	if (AttackerTeam == TEAM_SURVIVORS && VictimTeam == TEAM_SURVIVORS)
	{
		...
		{
			StatsPrintToChatAll("\x05%s \x03LOST \x04%i \x01Points For \x03Incapacitating \x05%s\x01!", AttackerName, Score, VictimName);
		}
	}
	else if (AttackerTeam == TEAM_INFECTED && VictimTeam == TEAM_SURVIVORS)
	{
...
public Action:event_ChargerKilled(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Mode)
	{
		...
		else
		{
			...
			{
				Format(VictimName, sizeof(VictimName), "An Ally");
			}
...
public Action:event_ChargerCarryRelease(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Victim > 0)
	{
		CreateTimer(INF_WEAROFF_TIME + 7.0, timer_EndChargerCarry, Victim);	
	}
}
...
public Action:event_GascanPoured(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Score > 0)
	{
		...
		if (Mode == 1 || Mode == 2)
		{
			StatsPrintToChat(Player, "You Earned \x04%i \x01Points For \x05Pouring Gas Can\x01!", Score);
		}
		else if (Mode == 3)
		{
			decl String:PlayerName[MAX_LINE_WIDTH];
			GetClientName(Player, PlayerName, sizeof(PlayerName));
			StatsPrintToChatAll("\x05%s \x01Earned \x04%i \x01Points For \x05Pouring Gas Can\x01!", PlayerName, Score);
		}
	}
}
...
public Action:event_PlayerPounced(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (PounceDistance >= GetConVarInt(FindConVar("brutal_hunter_pounce_fatal_distance")) && !IsPenaltyOn[Attacker])
	{
		IsPenaltyOn[Attacker] = true;
		CreateTimer(GetConVarFloat(FindConVar("brutal_hunter_pounce_fatal_penalty")), Timer_ResetPenalty, Attacker, TIMER_FLAG_NO_MAPCHANGE);
		
		if (!GetEntProp(Victim, Prop_Send, "m_isIncapacitated"))
		{
			Score = GetConVarInt(cvar_SurvivorIncap) + GetConVarInt(cvar_SurvivorDeath) + 17;
		}
		else
		{
			Score = GetConVarInt(cvar_SurvivorDeath) + 17;
		}
		
		if (CurrentGamemodeID == GAMEMODE_VERSUS)
		{
			Format(query, sizeof(query), "UPDATE %splayers SET points_infected = points_infected + %i, award_survivor_down = award_survivor_down + %s, versus_kills_survivors = versus_kills_survivors + 1 WHERE steamid = '%s'", DbPrefix, Score, (GetEntProp(Victim, Prop_Send, "m_isIncapacitated", 1) ? "0" : "1"), AttackerID);
		}
		else if (CurrentGamemodeID == GAMEMODE_REALISMVERSUS)
		{
			Format(query, sizeof(query), "UPDATE %splayers SET points_realism_infected = points_realism_infected + %i, award_survivor_down = award_survivor_down + %s, versus_kills_survivors = versus_kills_survivors + 1 WHERE steamid = '%s'", DbPrefix, Score, (GetEntProp(Victim, Prop_Send, "m_isIncapacitated", 1) ? "0" : "1"), AttackerID);
		}
		else if (CurrentGamemodeID == GAMEMODE_SCAVENGE)
		{
			Format(query, sizeof(query), "UPDATE %splayers SET points_scavenge_infected = points_scavenge_infected + %i, award_survivor_down = award_survivor_down + %s, scavenge_kills_survivors = scavenge_kills_survivors + 1 WHERE steamid = '%s'", DbPrefix, Score, (GetEntProp(Victim, Prop_Send, "m_isIncapacitated", 1) ? "0" : "1"), AttackerID);
		}
		else
		{
			Format(query, sizeof(query), "UPDATE %splayers SET points_mutations = points_mutations + %i, award_survivor_down = award_survivor_down + %s, mutations_kills_survivors = mutations_kills_survivors + 1 WHERE steamid = '%s'", DbPrefix, Score, (GetEntProp(Victim, Prop_Send, "m_isIncapacitated", 1) ? "0" : "1"), AttackerID);
		}
		...
		if (Mode == 1 || Mode == 2)
		{
			if (!GetEntProp(Victim, Prop_Send, "m_isIncapacitated"))
			{
				StatsPrintToChat(Attacker, "You Earned \x0415 \x01Points For Incapacitating \x05%s\x01 Brutally!", VictimName);
			}
			StatsPrintToChat(Attacker, "You Earned \x0430 \x01Points For Killing \x05%s\x01 Brutally!", VictimName);
			StatsPrintToChat(Attacker, "You Earned \x0417 \x01Points For Doing \x051700\x01 Damage To Survivors!");
		}
		else if (Mode == 3)
		{
			if (!GetEntProp(Victim, Prop_Send, "m_isIncapacitated"))
			{
				StatsPrintToChatAll("\x05%s \x01Earned \x0415 \x01Points For Incapacitating \x05%s\x01 Brutally!", AttackerName, VictimName);
			}
			StatsPrintToChatAll("\x05%s \x01Earned \x0430 \x01Points For Killing \x05%s\x01 Brutally!", AttackerName, VictimName);
			StatsPrintToChatAll("\x05%s \x01Earned \x0417 \x01Points For Doing \x05%s\x01 Damage To Survivors!", AttackerName);
		}
	}
	...
	if (Mode == 1 || Mode == 2)
	{
		StatsPrintToChat(Attacker, "You Earned \x04%i \x01Points From \x05%s \x01Pounce On \x05%s\x01!", Score, Label, VictimName);
	}
	else if (Mode == 3)
	{
		StatsPrintToChatAll("\x05%s \x01Earned \x04%i \x01Points From \x05%s \x01Pounce On \x05%s\x01!", AttackerName, Score, Label, VictimName);
	}
}

public Action:Timer_ResetPenalty(Handle:timer, any:attacker)
{
	IsPenaltyOn[attacker] = false;
	return Plugin_Stop;
}
...
public Action:event_Award_L4D1(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	else if (AwardID == 94)
	{
		...
		if (Mode == 1 || Mode == 2)
		{
			StatsPrintToChat(User, "You \x03LOST \x04%i \x01Points For \x03Contaminated Safe Room!", Score);
		}
		else if (Mode == 3)
		{
			StatsPrintToChatAll("\x05%s \x03LOST \x04%i \x01Points For \x03Contaminated Safe Room!", UserName, Score);
		}
	...
	else if (AwardID == 98)
	{
		...
		if (Mode)
		{
			StatsPrintToChat(User, "\x03ALL SURVIVORS \x03LOST \x04%i \x01Points From \x03Mission Lost!", Score);
		}
...
public Action:event_CarAlarm(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for (new i = 1; i <= maxplayers; i++)
	{
		if (IsClientConnected(i) && IsClientInGame(i) && !IsClientBot(i) && GetClientTeam(i) == TEAM_SURVIVORS)
		{
			...
			if (Mode)
			{
				StatsPrintToChat(i, "\x03ALL SURVIVORS \x03LOST \x04%i \x01Points For \x03Triggering Alarmed Car\x01!", Score);
			}
		}
	}
}
...
public Action:cmd_ShowTimedMapsTimer(client, args)
{
	...
	if (MapTimingStartTime <= 0.0)
	{
		...
		{
			StatsPrintToChatPreFormatted(client, "Hasn't Started Yet!");
		}
...
public DisplayRankMenu(client)
{
	...
	AddMenuItem(menu, "rank", "Show Own Rank");
	AddMenuItem(menu, "top10", "Show Top 10");
	AddMenuItem(menu, "top10ppm", "Show Top 10 PPM");
	AddMenuItem(menu, "nextrank", "Show Own Next Rank");
	AddMenuItem(menu, "showtimer", "Show Current Timer");
	AddMenuItem(menu, "showrank", "Show Others'	Ranks");
	AddMenuItem(menu, "showppm", "Show Others' PPMs");
...
public GetClientRankPlayerJoined(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	...
	if (ClientRank[client] > 0)
	{
		...
		{
			StatsPrintToChatAll("Admin \x05%s \x01Joined! (Rank: \x03%i \x01/ Points: \x03%i\x01)", userName, ClientRank[client], ClientPoints[client]);
		}
		...
		{
			StatsPrintToChatAll("VIP \x05%s \x01Joined! (Rank: \x03%i \x01/ Points: \x03%i\x01)", userName, ClientRank[client], ClientPoints[client]);
		}
		else
		{
			StatsPrintToChat(client, "\x05Rank: \x03%i \x01| \x05Points: \x03%i\x01", ClientRank[client], ClientPoints[client]);
		}
	}
	else
	{
		if ((GetUserFlagBits(client) & ADMFLAG_GENERIC) || (GetUserFlagBits(client) & ADMFLAG_KICK) || (GetUserFlagBits(client) & ADMFLAG_ROOT))
		{
			StatsPrintToChatAll("Admin \x05%s \x01Joined! (Rank: Unknown / Points: Nonw)", userName);
		}
		else if ((GetUserFlagBits(client) & ADMFLAG_CUSTOM1) || (GetUserFlagBits(client) & ADMFLAG_CUSTOM2) || (GetUserFlagBits(client) & ADMFLAG_CUSTOM3) || (GetUserFlagBits(client) & ADMFLAG_CUSTOM4) || (GetUserFlagBits(client) & ADMFLAG_CUSTOM5) || (GetUserFlagBits(client) & ADMFLAG_CUSTOM6))
		{
			StatsPrintToChatAll("VIP \x05%s \x01Joined! (Rank: Unknown / Points: None)", userName);
		}
		else
		{
			StatsPrintToChat(client, "\x05Rank: \x01Unknown | \x05Points: \x01None");
		}
	}
}
...
public DisplayNextRank(client)
{
	...
	{
		DrawPanelText(NextRankPanel, "1st Placer!");
	}
...
public DisplayRank(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	...
	Format(Value, sizeof(Value), "Rank: %i Out Of %i", ClientRank[client], RankTotal);
	DrawPanelText(RankPanel, Value);
	
	if (!InvalidGameMode())
	{
		Format(Value, sizeof(Value), "%s Rank: %i Out Of %i" , CurrentGamemodeLabel, ClientGameModeRank[client], GameModeRankTotal);
		DrawPanelText(RankPanel, Value);
	}
	
	if (Playtime > 60)
	{
		Format(Value, sizeof(Value), "Playtime: %.2f Hrs", FloatDiv(float(Playtime), 60.0));
		DrawPanelText(RankPanel, Value);
	}
	else
	{
		Format(Value, sizeof(Value), "Playtime: %i Mins", Playtime);
		DrawPanelText(RankPanel, Value);
	}
...
public DisplayAboutPanel(client)
{
	...
	Format(Value, sizeof(Value), "Description: %s", "Stats And Rankings Of Players.");
	DrawPanelText(panel, Value);
...
{
	if (!GetConVarBool(cvar_Enable) || InvalidGameMode() || !MapCheck && (IsDifficultyEasy() || CheckHumans() || GetConVarBool(cvar_Cheats)) || db == INVALID_HANDLE)
	{
		return true;
	}
...
public CheckSurvivorsWin()
{
	...
	if (Score > 0 && WitchExists && !WitchDisturb)
	{
		...
		if (Mode)
		{
			StatsPrintToChatTeam(TEAM_SURVIVORS, "\x03ALL SURVIVORS \x01Earned \x04%i \x01Points From \x05No Witch Disturbed Award!", Score);
		}
	}
	
	Score = 0;
	new Deaths = 0;
	new BaseScore = ModifyScoreDifficulty(GetConVarInt(cvar_SafeHouse), 1.5, 2, TEAM_SURVIVORS);
	...
	if (Mode && Score > 0)
	{
		StatsPrintToChatTeam(TEAM_SURVIVORS, "\x03ALL SURVIVORS \x01Earned \x04%i \x01Points For Reaching Saferoom With \x05%i Deaths!", Score, Deaths);
		
		if (NegativeScore)
		{
			StatsPrintToChatTeam(TEAM_INFECTED, "\x03ALL INFECTED \x03LOST \x04%i \x01Points For Losing Round To Survivors!", Score);
		}
	}
...
CheckSurvivorsAllDown()
{
	...
	if (Score > 0 && Mode)
	{
		StatsPrintToChatTeam(TEAM_INFECTED, "\x03ALL INFECTED \x01Earned \x04%i \x01Points For Killing All Survivors!", Score);
	}
	...
	if (Mode)
	{
		StatsPrintToChatTeam(TEAM_SURVIVORS, "\x03ALL SURVIVORS \x03LOST \x04%i \x01Points For \x03All Survivors Dead\x01!", Score);
	}
}
...
{
	new sSurv = 0;
	for (new live=1; live<=GetMaxClients(); live++)
	{
		if(IsClientInGame(live) && GetClientTeam(live) == 2 && IsPlayerAlive(live))
		{
			sSurv++;
		}
	}
...
UpdateFriendlyFire(Attacker, Victim)
{
	...
	{
		StatsPrintToChatAll("\x05%s \x03LOST \x04%i \x01Points For \x03Friendly Firing \x05%s\x01!", AttackerName, Score, VictimName);
	}
}
...
AnnounceMedkitPenalty(Mode = -1)
{
	...
	if (Mode)
	{
		StatsPrintToChatTeam(TEAM_SURVIVORS, "\x03ALL SURVIVORS \x01Now Earn Only \x04%i Percent \x01Of Points After Using \x05%i%s Medkit%s\x01!", RoundToNearest(ReductionFactor * 100), MedkitsUsedCounter, (MedkitsUsedCounter == 1 ? "st" : (MedkitsUsedCounter == 2 ? "nd" : (MedkitsUsedCounter == 3 ? "rd" : "th"))), (ServerVersion == SERVER_VERSION_L4D1 ? "" : " Or Defibrillator"));
	}
}
...
public UpdateMapTimingStat(Handle:owner, Handle:hndl, const String:error[], any:dp)
{
	...
	if (SQL_GetRowCount(hndl) > 0)
	{
		...
		else
		{
			if (Mode)
			{
				...
				StatsPrintToChat(Client, "New Best Time: \x04%s\x01!", TimeLabel);
			}
	...
	else
	{
		...
		{
			SetTimeLabel(TotalTime, TimeLabel, sizeof(TimeLabel));
			StatsPrintToChat(Client, "Took \x04%s \x01To Finish!", TimeLabel);
		}
...
public SetTimeLabel(Float:TheSeconds, String:TimeLabel[], maxsize)
{
	...
	if (Hours > 0)
	{
		Format(TimeLabel, maxsize, "%i Hrs, %i Mins, %.1f Sec", Hours, Minutes, Seconds);
	}
	else if (Minutes > 0)
	{
		Format(TimeLabel, maxsize, "%i Mins, %.1f Sec", Minutes, Seconds);
	}
	else
	{
...
public InitializeRankVote(client)
{
	if (StatsDisabled())
	{
		if (client == 0)
		{
			PrintToConsole(0, "[RANK] Plugin Disabled!");
		}
		else
		{
			StatsPrintToChatPreFormatted2(client, true, "Plugin Disabled!");
		}
	...
	if (!IsTeamGamemode())
	{
		if (client == 0)
		{
			PrintToConsole(0, "[RANK] Rank Vote Unsupported!");
		}
		else
		{
			if (ServerVersion == SERVER_VERSION_L4D1)
			{
				StatsPrintToChatPreFormatted2(client, true, "\x04Rank Vote \x01Enabled In \x03Versus \x01Game Mode!");
			}
			else
			{
				StatsPrintToChatPreFormatted2(client, true, "\x04Rank Vote \x01Enabled In \x03Versus\x01, \x03Realism Versus \x01And \x03Scavenge \x01Game Modes!");
			}
		}
	...
	if (RankVoteTimer != INVALID_HANDLE)
	{
		...
		{
			PrintToConsole(client, "[RANK] Rank Vote Already Initiated!");
		}
	...
	if (client > 0)
	{
		...
		StatsPrintToChatAll2(true, "\x04Rank Vote \x01Initiated By \x05%s\x01!", UserName);
	}
	else
	{
		StatsPrintToChatAllPreFormatted2(true, "\x04Rank Vote \x01Initiated From Console!");
	}
}

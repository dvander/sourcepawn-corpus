...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	{
		strcopy(error, err_max, "[ARS] Plugin Supports L4D Only!");
...
{
	name = "[L4D] Anti-Rush System (Reloaded)",
	author = "cravenge",
...
{
	BuildPath(Path_SM, sDataFilePath, sizeof(sDataFilePath), "data/anti-rush_system-l4d.cfg");
	...
	CreateConVar("anti-rush_system-l4d_version", "1.82", "Anti-Rush System (Reloaded) Version", FCVAR_NOTIFY|FCVAR_SPONLY|FCVAR_DONTRECORD);
	arsDuration[0] = CreateConVar("ars-l4d_duration_1st", "30.0", "Duration In 1st Round", FCVAR_NOTIFY|FCVAR_SPONLY, true, 15.0, true, 60.0);
	arsDuration[1] = CreateConVar("ars-l4d_duration_2nd", "30.0", "Duration In 2nd Round", FCVAR_NOTIFY|FCVAR_SPONLY, true, 15.0, true, 60.0);
	arsNotify = CreateConVar("anti-rush_system-l4d_notify", "1", "Enable/Disable Notifications", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	AutoExecConfig(true, "anti-rush_system-l4d");
...
public Action OnLockCmd(int client, int args)
{
	...
	if (!bManualLock)
	{
		...
		if (IsValidEnt(iSaferoomDoor))
		{
			...
			SetEntityRenderColor(iSaferoomDoor, _, 0, 0);
		}
		else
		{
			InitConfig(sMap);
			...
			for (int i = 0; i < iBlockCount; i++)
			{
				...
				DispatchKeyValue(iPathBlock, "targetname", "ars-l4d_barricade");
				...
				if (!bBlockSolid[i])
				{
					...
					DispatchKeyValue(iPathBlock2, "targetname", "ars-l4d_barricade");
...
public Action OnUnlockCmd(int client, int args)
{
	...
	if (bManualLock)
	{
		...
		if (IsValidEnt(iSaferoomDoor))
		{
			...
			SetEntityRenderColor(iSaferoomDoor, 0, _, 0);
		}
		else
		{
			ExecuteCommand(client, "ent_fire", "ars-l4d_barricade KillHierarchy");
		}
	}
	else
	{
...
public void OnPluginEnd()
{
	if (iSaferoomDoor > 0)
	{
		...
		{
			SetEntityRenderMode(iSaferoomDoor, RENDER_NORMAL);
			SetEntityRenderColor(iSaferoomDoor);
...
public Action PrepareSystem(Handle timer)
{
	...
	if (IsValidEnt(iSaferoom))
	{
		...
		SetEntityRenderMode(iSaferoom, RENDER_TRANSCOLOR);
		SetEntityRenderColor(iSaferoom, _, 0, 0);
	}
	else
	{
		float fMins[3], fMaxs[3], fTemp[4];
	...
	if (bCountdownStarted || iFailedChecks >= 60 || (GetConnectingCount() < 1 && GetUnassignedCount() < 1) || iReady >= GetRealSurvivorsCount() + GetRealInfectedCount())
	{
		if (iCountdown < 1)
		{
			...
			if (!IsValidEnt(entity))
			{
				...
				if (iRandClient != 0)
				{
					ExecuteCommand(iRandClient, "ent_fire", "ars-l4d_barricade KillHierarchy");
			...
			else
			{
				...
				SetEntityRenderColor(entity, 0, _, 0);
			}

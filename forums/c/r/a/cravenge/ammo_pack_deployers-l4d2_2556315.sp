...
public Action CheckBotsAmmo(Handle timer)
{
	...
	{
		if (IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i) && IsFakeClient(i) && !IsIncapacitated(i) && !IsPlayerHeld(i) && iDeploy[i] == 0)
		{
...
{
	int upgrader = GetClientOfUserId(event.GetInt("victim"));
...
{
	int iPounceAttacker = GetEntPropEnt(client, Prop_Send, "m_pounceAttacker"),
		iJockeyAttacker = GetEntPropEnt(client, Prop_Send, "m_jockeyAttacker"),
		iPummelAttacker = GetEntPropEnt(client, Prop_Send, "m_pummelAttacker");
	
	if (IsAliveInfected(iPounceAttacker) || IsAliveInfected(iJockeyAttacker) || 
		IsAliveInfected(iPummelAttacker) || bTongued[client])
	{
		return true;
	}

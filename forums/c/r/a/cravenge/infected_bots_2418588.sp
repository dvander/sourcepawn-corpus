...
public OnPluginStart()
{
	...
	h_InfectedSpawnTimeMax = CreateConVar("infected_bots_spawn_time_max", "15", "Maximum Spawn Time Delay Of Infected Bots", FCVAR_SPONLY);
	h_InfectedSpawnTimeMin = CreateConVar("infected_bots_spawn_time_min", "15", "Minimum Spawn Time Delay Of Infected Bots", FCVAR_SPONLY);
...
public Action:OnPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(DisableSpawnsTank && !IsPlayerTank(client) && IsFakeClient(client))
	{
		for(new inf=1; inf<=MaxClients; inf++)
		{
			if(IsClientInGame(inf) && GetClientTeam(inf) == 3 && IsPlayerTank(inf) && IsPlayerAlive(inf))
			{

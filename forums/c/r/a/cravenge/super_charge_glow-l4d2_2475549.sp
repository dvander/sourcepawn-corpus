...
void ResetPlugin()
{
	...
	{
		SDKUnhook(g_iEntities[i], SDKHook_SetTransmit, OnTransmitEntity);
...
public void ConVarChanged_Glow(ConVar convar, const char[] oldValue, const char[] newValue)
{
	...
	for (int i = 0; i < MAX_ALLOWED; i++)
	{
		...
		{
			SDKHook(g_iEntities[i], SDKHook_SetTransmit, OnTransmitEntity);
		...
		if (IsValidEntity(entity) && IsValidEdict(entity))
		{
			if (GetEntityMoveType(entity) == MOVETYPE_VPHYSICS)
			{
				...
				if ((iType & PROP_CAR_ALARM) && strcmp(sClassName, "prop_car_alarm") == 0)
				{
					...
					SDKHook(g_iEntities[g_iCount++], SDKHook_SetTransmit, OnTransmitEntity);
				}
...
public Action OnTransmitEntity(int entity, int client)
{
	if (IsSurvivor(client) || IsCharger(client) || IsTank(client))
	{
...
{
	return (client > 0 && client <= MaxClients && IsClientInGame(client) && GetClientTeam(client) == 2 && !IsFakeClient(client));
}
...
{
	return (client > 0 && client <= MaxClients && IsClientInGame(client) && GetClientTeam(client) == 3 && GetEntProp(client, Prop_Send, "m_zombieClass") == 8 && !IsFakeClient(client));
}

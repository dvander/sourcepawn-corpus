...
new Handle:sm_rage_showall = INVALID_HANDLE
new bool:exemptRage
...
public OnPluginStart(){
	...
	sm_rage_showall = CreateConVar("sm_rage_showall", "0", "Show total number of rages on the server instead of rages for the current campaign", _, true, 0.0, true, 1.0)
	...
	if(GetConVarInt(sm_rage_enable) == 1){
		...
		if(IsClientInGame(client) && !IsFakeClient(client) && !IsClientTimingOut(client) && !IsClientInKickQueue(client)){
			if(client != raged && !StrEqual(steamid, ragedid)){
				new showall = GetConVarInt(sm_rage_showall)
				rages++;
				readRageFile()
				...
				if(showall == 0){
					...
					if(rages == 1){
						PrintToChatAll("%s has been \x04%i\x01 rage quit", phraseStart, totalRagesInt)
					}else{	
						PrintToChatAll("%s have been \x04%i\x01 rage quits", phraseStart, totalRagesInt)
					}
				}
				raged = client
				ragedid = steamid

...
public IsValidItemType(const String:type[]) {
  if (StrEqual(type, "autoshotgun") || StrEqual(type, "pistol") || StrEqual(type, "hunting_rifle") || StrEqual(type, "rifle") || StrEqual(type, "pumpshotgun") || StrEqual(type, "smg"))
    return true;
  if (StrEqual(type, "first_aid_kit") || StrEqual(type, "gascan") || StrEqual(type, "molotov") || StrEqual(type, "pain_pills") || StrEqual(type, "pipe_bomb") || StrEqual(type, "propanetank"))
    return true;  
  return false;
}
...
ForcePanic(client) {
  ...
  LogAction(client, -1, "[NOTICE]: executed %s", client, "a4d_force_panic");
}
...
StripAndExecuteClientCommand(client, const String:command[], const String:arguments[]) {
	new flags = GetCommandFlags(command);
	SetCommandFlags(command, flags & ~FCVAR_CHEAT);
	if (StrEqual(command, "z_spawn") || StrEqual(command, "give") || StrEqual(command, "director_force_panic_event") && client == 0)
		FakeClientCommand(1, "%s %s", command, arguments);
	else 

...
{
	name = "[L4D2] Individual Infinite Ammo",
	author = "Machine and modified by Psykotik",
	description = "Gives Infinite Ammo to players",
	version = PLUGIN_VERSION,
	url = "http://forums.alliedmods.net/showthread.php?t=123100"
}

int InfiniteAmmo[MaxClients+1];
int Throwing[MaxClients+1];
Handle IAmmo = INVALID_HANDLE;
Handle AllowGL = INVALID_HANDLE;
Handle AllowM60 = INVALID_HANDLE;
Handle AllowChainsaw = INVALID_HANDLE;
Handle AllowThrowables = INVALID_HANDLE;
Handle AllowUpgradeAmmo = INVALID_HANDLE;
Handle AllowMeds = INVALID_HANDLE;
Handle AllowDefibs = INVALID_HANDLE;
Handle AllowPills = INVALID_HANDLE;
Handle AllowShots = INVALID_HANDLE;
Handle AdminOverride = INVALID_HANDLE;
Handle hAdminMenu = INVALID_HANDLE;

public void OnPluginStart()
{
	RegAdminCmd("sm_iammo", Command_IAmmo, ADMFLAG_SLAY, "sm_iammo <#userid|name> <0|1> - Toggles Infinite Ammo on player(s)");
	CreateConVar("l4d2_iammo_version", PLUGIN_VERSION, "L4D2 Infinite Ammo Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	IAmmo = CreateConVar("l4d2_iammo_enable", "1", "<0|1|2> Enable Infinite Ammo? 0=Off 1=On 2=Everyone", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	AllowGL = CreateConVar("l4d2_iammo_gl", "1", "<0|1|2> Allow Infinite Ammo on Grenade Launcher? 0=Off 1=On 2=Only", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	AllowM60 = CreateConVar("l4d2_iammo_m60", "1", "<0|1|2> Allow Infinite Ammo on the M60? 0=Off 1=On 2=Only", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	AllowChainsaw = CreateConVar("l4d2_iammo_chainsaw", "1", "<0|1|2> Allow Infinite Ammo on the Chainsaw? 0=Off 1=On 2=Only", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	AllowThrowables = CreateConVar("l4d2_iammo_throwables", "1", "<0|1|2> Allow Infinite Ammo on Throwables? 0=Off 1=On 2=Only", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	AllowUpgradeAmmo = CreateConVar("l4d2_iammo_upgradeammo", "1", "<0|1> Allow Infinite Explosive and Incendiary Ammo? 0=Off 1=On", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	AllowMeds = CreateConVar("l4d2_iammo_meds", "1", "<0|1|2> Allow Infinite Medkits? 0=Off 1=On 2=only", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	AllowDefibs = CreateConVar("l4d2_iammo_defibs", "1", "<0|1|2> Allow Infinite Defibs? 0=Off 1=On 2=only", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	AllowPills = CreateConVar("l4d2_iammo_pills", "1", "<0|1|2> Allow Infinite Pills? 0=Off 1=On 2=only", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	AllowShots = CreateConVar("l4d2_iammo_shots", "1", "<0|1|2> Allow Infinite Shots? 0=Off 1=On 2=only", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	AdminOverride = CreateConVar("l4d2_admin_override", "1", "<0|1> Admins with infinite ammo always have all settings enabled? 0=Off 1=On", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	AutoExecConfig(true, "l4d2_iiammo");
...
public void OnAdminMenuReady(Handle topmenu)
{
	...
	hAdminMenu = topmenu;
	TopMenuObject menu_category = AddToTopMenu(hAdminMenu, "l4d2_ia_topmenu", TopMenuObject_Category, Handle_Category, INVALID_TOPMENUOBJECT);
...
public void AllowGLChanged(Handle convar, const char[] oldValue, const char[] newValue)
{
	if (convar == AllowGL)
	{
		int oldval = StringToInt(oldValue);
...
public void AllowM60Changed(Handle convar, const char[] oldValue, const char[] newValue)
{
	if (convar == AllowM60)
	{
		int oldval = StringToInt(oldValue);
...
public void AllowChainsawChanged(Handle convar, const char[] oldValue, const char[] newValue)
{
...
public void AllowThrowablesChanged(Handle convar, const char[] oldValue, const char[] newValue)
{
...
public void AllowUpgradeAmmoChanged(Handle convar, const char[] oldValue, const char[] newValue)
{
...
public void AllowDefibsChanged(Handle convar, const char[] oldValue, const char[] newValue)
{
...
public void AllowShotsChanged(Handle convar, const char[] oldValue, const char[] newValue)
{
...
public Action Event_DefibrillatorUsed(Handle event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (GetConVarInt(IAmmo) > 0)
	{
...
public Action Event_AdrenalineUsed(Handle event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (GetConVarInt(IAmmo) > 0)
	{

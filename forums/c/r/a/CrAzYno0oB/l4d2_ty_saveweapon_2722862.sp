...
int 	g_iWeaponInfo[MAXPLAYERS+1][16];
char 	g_sWeaponInfo[MAXPLAYERS+1][6][64];
bool    g_bValidMapChange = false;

ConVar 	g_hNoob, g_hClearAfterCampaign, g_hCvarGameMode;
...
public void OnMapStart()
{
	...
	//////////////////////////////////////
	GetConVarString(g_hCvarGameMode, gameMode, sizeof(gameMode));
...
public Action Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	CreateTimer(0.3, timer_Give, userid, TIMER_FLAG_NO_MAPCHANGE); 
...
public Action timer_Give(Handle handle, int userid)
{
	...
	if (g_iWeaponInfo[client][iSpawned] == 0)
	{
		...
		else if (g_hNoob.BoolValue && g_bCanAppropriate) // If start of round, no weapons saved & g_bNoob enabled
		{
			...
			if (iSlot <= 0) CheatCommand(client, "give", "smg_silenced","");
		}		
...
void TySaveWeapon(int client)
{
	...
	if (iSlot0 > 0)
	{
		...
		g_iWeaponInfo[client][iSkin]  = GetEntProp(iSlot0, Prop_Send, "m_nSkin", 4);
	}
	if (iSlot1 > 0) 
	{
		TySaveSlot1(client, iSlot1);
		g_iWeaponInfo[client][iSkin2] = GetEntProp(iSlot1, Prop_Send, "m_nSkin", 4);
	}
...
void TyGiveWeapon(int client)
{
	...
	if (g_sWeaponInfo[client][1][0] != '\0')
	{
		...
		else
		{
			...
			if (iSlot > 0)
			{
				SetEntProp(iSlot, Prop_Send, "m_nSkin", g_iWeaponInfo[client][iSkin2], 4);
			}
		}
	...
	if (g_sWeaponInfo[client][0][0] != '\0')
	{
		...
		{	
			...
			SetEntProp(iSlot, Prop_Send, "m_nSkin", g_iWeaponInfo[client][iSkin], 4);
		}
	}
	else if (g_hNoob.BoolValue) CheatCommand(client, "give", "smg_silenced","");
}
...
void TyCleanW(int client)
{
	...
	g_iWeaponInfo[client][iSkin] 		=   0;
	g_iWeaponInfo[client][iSkin2] 		=   0;

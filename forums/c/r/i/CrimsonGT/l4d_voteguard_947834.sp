...
new Handle:cAdvertise = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	cAdvertise = CreateConVar("sm_voteguard_adverts", "1", "Enable/Disable L4D Vote Guardian to Advertise", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public Action:Command_CallVote(client, args)
{
	...
	if(g_VotesCalled[client] == 0)
	{
		...
		if(GetConVarBool(cAdvertise))
		{
			PrintToChatAll("\x04[VOTEGUARD] \x01%N has called a %s vote!", client, sVoteType);
		}
	}

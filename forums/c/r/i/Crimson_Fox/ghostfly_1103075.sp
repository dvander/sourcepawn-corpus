...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	if (GetConVarBool(GhostFly))
	{
		...
		if (elig)
		{
			...
			{
				//Needed to make sure the player can spawn before removing velocity.
				CreateTimer(0.1, SpawnCheckDelay, client);
			}
		}
		else
		{
...
public Action:SpawnCheckDelay(Handle:timer, any:client)
{
	if (GetEntData(client, PropGhost, 1)!=1)
	{

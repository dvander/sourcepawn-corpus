...
#define MAX_LOG_WEAPONS 22
#define MAX_WEAPON_LEN 26
#define MAX_BULLET_WEAPONS 14
#define MAX_UNLOCKABLE_WEAPONS 4
...
#define WEAPONID_PIPEBOMB 23
#define WEAPONID_PIPEBOMB_KILL 52
#define WEAPONID_STICKY 24
#define WEAPONID_STICKY_KILL 35
#define JUMP_NONE 0
...
public OnPluginStart()
{
	...
	// Other ideas:
	// jarated_nodamage? (Can I determine jarated and faded normally compared to jarated and faded from water/resupply?)
...
public Action:TF2_CalcIsAttackCritical(attacker, weapon, String:weaponname[], &bool:result)
{
	...
	{
		new weapon_index = GetWeaponIndex(weaponname[WEAPON_PREFIX_LENGTH], weapon);
		if(weapon_index != -1)
		{
...
public OnTakeDamage(victim, attacker, inflictor, Float:damage, damagetype)
{
	if(b_wstats && attacker > 0) // I used attacker, not inflictor, here. May be a mistake?
	{
		...
		if (inflictor <= MaxClients) // Inflictor is a player
		{
			...
			weapon_index = GetWeaponIndex(weapon[WEAPON_PREFIX_LENGTH], weaponent);
		}
		else if (IsValidEntity(inflictor))
		{
			GetEdictClassname(inflictor, weapon, sizeof(weapon));
			if(weapon[3] == 'p')
				weapon_index = GetWeaponIndex(weapon, -1);
			else
			{
				// Baseballs are funky.
				...
				if(!(damagetype & DMG_CRUSH) && (damagetype & DMG_CLUB) && StrEqual(weapon, "tf_weapon_bat_wood"))
					weapon_index = GetWeaponIndex("ball", -1);
				else
					weapon_index = GetWeaponIndex(weapon[WEAPON_PREFIX_LENGTH], -1);
			}
		}
		if(weapon_index > -1)
		{
...
public Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(b_actions)
	{
		...
		if(client != attacker && client > 0 && client <= MaxClients && IsClientInGame(client)
			&& GetEntityFlags(client) & (FL_ONGROUND | FL_INWATER) == 0)
		{
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(!(death_flags & 32)) // Not a dead ringer death?
	{
		...
		if(b_wstats && client > 0 && attacker > 0 && attacker <= MaxClients)
		{
			...
			new weapon_index = GetWeaponIndex(weaponlogname, -1);
			if(weapon_index != -1)
			{
	...
	switch (customkill)
	{
		...
		case 17, 18:
			if(b_fire)
			{
				...
				if(logweapon[0] != 'r') // No changing reflects
...
GetWeaponIndex(const String:weaponname[], weapon)
{
	new index;
	if(GetTrieValue(h_weapontrie, weaponname, index))
	{
		if(index & UNLOCKABLE_BIT)
		{
			...
			if(weapon != -1)
				if(GetEntProp(weapon, Prop_Send, "m_iEntityQuality") > 0)
					index++;
		}
		return index;
	}

...
public OnPluginStart()
{
	...
	tankKickImmunity    = CreateConVar("l4d_vote_tank_kick_immunity",     "1", "Make tanks immune to vote kicking.",CVAR_FLAGS,true,0.0,true,1.0);
        alltalkAccess	    = CreateConVar("l4d_vote_alltalk_access",	      "z",  "Access level needed to start an alltalk vote", CVAR_FLAGS);
...
public Action:Callvote_Handler(client, args)
{
	...
	else
	{
		...
		//PrintToChatAll("\x04[VOTE] \x01%s tried to start a %s vote but does not have access.", initiatorName, voteName);
...
public Action:Kick_Vote_Logic(client, args)
{
	...
	if (GetConVarBool(tankKickImmunity) && (GetClientTeam(target) == TEAM_INFECTED) && IsPlayerAlive(target))
	{
		...
		if (StrContains(model, "hulk", false) > 0)
		{
			...
			//PrintToChatAll("\x04[VOTE] \x01%s tried to start a Kick Vote against %s but tanks cannot be kicked.",initiatorName,targetName);
	...
	if (GetConVarBool(kickImmunity))
	{
		...
		if (FindAdminByIdentity(AUTHMETHOD_STEAM, steamid) != INVALID_ADMIN_ID)
		{
			...
			//PrintToChatAll("\x04[VOTE] \x01%s tried to start a Kick Vote against %s but he has immunity.",initiatorName,targetName);

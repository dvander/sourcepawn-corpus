...
public void OnPluginStart()
{
    ...
    hCvar = CreateConVar("smac_wallhack", "0", "DISABLES WALL HACKS", FCVAR_NONE, true, 0.0, true, 1.0);
    OnSettingsChanged(hCvar, "", "");
    HookConVarChange(hCvar, OnSettingsChanged);

    hCvar = CreateConVar("smac_wallhack_maxtraces", "524288", "MAXIMUM ENGINE TRACES EACH TICK", FCVAR_NONE, true, 4.0, true, 524288.0);
...
{
    int nAlivePlayers = H_CountAliveTeam(CS_TEAM_T) + H_CountAliveTeam(CS_TEAM_CT);

    ServerCommand("smac_wallhack %d;", (nAlivePlayers > 1 && nAlivePlayers < 11) ? 1 : 0);
}
...
public void Wallhack_UpdateClientCache(int client)
{
...
public void Wallhack_Hook(int client)
{
...
public void Wallhack_Unhook(int client)
{
...
public void UpdateClientData(int client)
{
...
public bool IsInFieldOfView(const float start[3], const float angles[3], const float end[3])
{
...
public bool Filter_NoPlayers(int entity, int mask)
{
    ...
    if (entity > MaxClients && m_hOwnerEntity == -8192)
    {
...
public int GetClientObserverMode(int client)
{
    static int offs = 0;
...
public int GetClientObserverTarget(int client)
{
    static int offs = 0;
...
public bool IsVectorZero(const float vec[3])
{
    return vec[0] == 0.0 && vec[1] == 0.0 && vec[2] == 0.0;
}

public bool H_IsValidClient(int nClient)
{
    if (nClient < 1                             || \
...
public int H_CountAliveTeam(int nTeam)
{
    int nTotal =        0;

    for (int nClient =  1; nClient < MAXPLAYERS; nClient++)
    {
        if (H_IsValidClient(nClient) && GetClientTeam(nClient) == nTeam && IsPlayerAlive(nClient))
        {
            nTotal++;
        }
    }

...
new Handle:KDCheckerShowOnPlayerDisconnect;
...
public OnPluginStart()
{
	...
	KDCheckerShowOnPlayerDisconnect     = CreateConVar("sm_kdrc_show_onplayerdisc",   "1",    "Show KD Rate to all on player disconnect", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public Action:ShowKDRateToAll(client, offset){
	
	decl String:player_name[65];
	GetClientName(client, player_name, sizeof(player_name));
	
	// Get Deaths, Kills and KD Rate
	new Deaths2 = GetClientDeaths(client);
	new Frags2 = GetClientFrags(client) + offset;
	new Float:KDRate2 = float(Frags2)/float(Deaths2);
	
	if ((Deaths2 == 0) && (Frags2 != 0)) KDRate2 = float(Frags2);
	if (Frags2 < 0) KDRate2 = float(0);
	
	// Print KDR to All
	PrintToChatAll("\x01[KDR Check] \x03%s\x04, leaved server with KD \x03%.2f \x04(\x03%i \x04Kills / \x03%i \x04Deaths)", player_name, KDRate2, Frags2, Deaths2);
	return Plugin_Handled;	
}
...
public Action:Event_PlayerDisconnect(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	// Check if "Plugin" is disbaled or "show kdr on play disconnect" is disbaled
	if((GetConVarInt(KDCheckerEnabled) != 1) || (GetConVarInt(KDCheckerShowOnPlayerDisconnect) != 1)) return Plugin_Continue;
	{
		//Check if player ok
		if (IsClientConnected(client) && IsClientInGame(client))
		{
			ShowKDRateToAll(client, 0);			
		}
	}
	return Plugin_Continue;
}

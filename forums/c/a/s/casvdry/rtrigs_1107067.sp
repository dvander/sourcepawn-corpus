...
new Handle:g_set = INVALID_HANDLE;
...
public OnPluginStart() {
	...
	g_set = CreateConVar("sm_rtrigs_set", "", "Commands to execute when the setup period ends.", FCVAR_PLUGIN);
...
public Action:Event_teamplay_setup_finished(Handle:event, const String:name[], bool:dontBroadcast) {
	if(GetConVarBool(g_enabled)) {
		decl String:cmds[1024], String:excmds[16][64];
		GetConVarString(g_set, cmds, sizeof(cmds));
		if(strcmp(cmds, "", false)!=0) {
			new count = ExplodeString(cmds, ",", excmds, 16, 64);
			for(new i=0;i<count;i++) {
				TrimString(excmds[i]);

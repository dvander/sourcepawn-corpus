...
*/
	else if (strcmp(game_mod, "nucleardawn", false) == 0)
	{
		LogAction(0, -1, "[AFK Manager] %T", "NUCLEARDAWN", LANG_SERVER);
		NUCLEARDAWN = true;
	}
...
#if defined _colors_included
	hCvarPrefixColor = CreateConVar("sm_afk_prefix_color", "1", "Should the AFK manager use color for the prefix tag? [0 = DISABLED, 1 = ENABLED]");
#endif
...
bool:CheckObserverAFK(client)
{
	...
	{
#if _DEBUG
	AFK_PrintToChat(client, "Client %i has not joined team", client);
#endif
		return true;
	}
						...
						if (bAFKSpawn[client])
						{
							...
							if ( afk_spawn_timeleft <= GetConVarFloat(hCvarWarnSpawnTime) )
							{
								...
								else
								{
									...
									// If the server is a Nuclear Dawn one, check to make sure the player isn't commander, if so demote first
									if (NUCLEARDAWN)
									{
										new team = GetClientTeam(client);
										if ((team == TEAM_ONE || team == TEAM_TWO) && client == ND_GetCommander(team))
										{
...
stock ND_GetCommander(team)
{
	...
	return IsValidClient(ret) ? ret : INVALID_ENT_REFERENCE;
}

// ------------------------------------------------------------------------------------------

stock ND_DemoteCommander(client, team)
{
	if (team != TEAM_ONE && team != TEAM_TWO)
	{
		ThrowError("Invalid team index specified: got %i expected [2,3]", team);

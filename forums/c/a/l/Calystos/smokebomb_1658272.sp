...
new Handle:v_SmokesPerAdmin = INVALID_HANDLE;
...
{
	name = "Smoke Bomb",
	author = "DarthNinja (Modded by Calystos)",
...
#tryinclude <gametype>
	...
	enum Game { undetected, tf2, cstrike, dod, hl2mp, insurgency, zps, l4d, l4d2, nucleardawn, other_game };
	...
	stock Game:GetGameType()
	{
		if (GameType == undetected)
		{
			...
			if (StrEqual(modname,"cstrike",false)) GameType = cstrike;
			else if (StrEqual(modname,"tf",false)) GameType = tf2;
			else if (StrEqual(modname,"dod",false)) GameType = dod;
			else if (StrEqual(modname,"hl2mp",false)) GameType = hl2mp;
			else if (StrEqual(modname,"Insurgency",false)) GameType = insurgency;
			else if (StrEqual(modname,"left4dead", false)) GameType = l4d;
			else if (StrEqual(modname,"left4dead2", false)) GameType = l4d2;
			else if (StrEqual(modname,"zps",false)) GameType = zps;
			else if (StrEqual(modname,"nucleardawn",false)) GameType = nucleardawn;
			else GameType=other_game;
		}
		return GameType;
	}
#endif
...
public OnPluginStart()
{
	...
	v_SmokesPerAdmin = CreateConVar("sm_smokecarry_admin", "3", "How many smokebombs admin can carry");
	v_SpammyText = CreateConVar("sm_smoke_helptext_handler", "0", "Sets what text to print, 0 = all text is shown, 1 = pickup text is hidden, 2 = pickup and use texts are hidden, 3 = pickup, use, and out of bombs texts are hidden.", 0, true, 0.0, true, 3.0);
	...
	AutoExecConfig(true, "plugin.smokebomb");
}

public OnMapStart()
{
	// Better safe than sorry (Nuclear Dawn moaned)
...
public EventInventoryApplication(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	// If a player is an admin or at least has generic admin flag set, then override defaultsmokes with admin defaultsmokes instead
	if (CheckCommandAccess(client, "sm_givesmoke", ADMFLAG_GENERIC)) DefaultSmokes = GetConVarInt(v_SmokesPerAdmin);
		...
		else
		{
			if (GetConVarInt(v_SpammyText) < 3)
			{
				switch (GetGameType())
				{
					...
					default:
					{
						PrintToChat(client,"[SmokeBomb] You are out of SmokeBombs!");
					}
				}
			}
		}

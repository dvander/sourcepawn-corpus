...
{
	CreateConVar("sm_whitelist_version", PLUGIN_VERSION, "Server Whitelist plugin version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
public OnClientPostAdminCheck(client)
{
	if(GetConVarBool(sm_whitelist_enable) && !IsFakeClient(client) && !IsImmune(client))
	{
		...
		{
			if(strcmp(player_authid, whitelist[i]) == 0)
			{
				allow = true;
				break;
			}
		...
		if(!allow)
		{
			KickClient(client, "VAC banned from secure server");
		}
	}
}

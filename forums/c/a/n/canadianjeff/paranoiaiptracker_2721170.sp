...
{
	CreateConVar("sm_paranoia_ip_version", VERSION, "Plugin Version", FCVAR_REPLICATED|FCVAR_NOTIFY);
...
public OnClientAuthorized(client, const String:steamID[])
{
	...
	PrintToServer("Paranoia IP Tracker Client Authorized!");
	...
	PrintToServer("[Paranoia IP] Name: %s | IP: %s", name, ip);
	
	if (db == INVALID_HANDLE)
	{
		//Log to file instead
		decl String:path[PLATFORM_MAX_PATH];
		PrintToServer("MySQL Failed Logging To File Instead!");
		BuildPath(Path_SM, path, sizeof(path), "logs/paranoia_ip.log");
		new Handle:file = OpenFile(path, "a");
		WriteFileLine(file, "%s, %L, %s", steamID, client, ip);
		CloseHandle(file);
		return;
	}
	...
	PrintToServer("SQL Query: %s", query);

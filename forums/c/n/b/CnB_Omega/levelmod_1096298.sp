...
public OnPluginStart()
{
	...
	cvar_exp_percent = CreateConVar("clm_exp_percent", "0.10", "Percent of experience from damage", _, true, 0.0);
...
public Player_Hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(GetConVarInt(cvar_enable))
	{
		...
		new Float:percent = GetConVarFloat(cvar_exp_percent);
		new Float:damage = (rawDamage * Float:percent);
		new experience = RoundToNearest(Float:damage);
		
		if(!(experience <= 0) && IsPlayerAlive(client) && (client != damaged) && playerLevel[client] < GetConVarInt(cvar_level_max))
		{

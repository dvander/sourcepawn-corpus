...
new Handle:l4d_selfhelp_none = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	l4d_selfhelp_none = CreateConVar("l4d_selfhelp_none", "1", "self help with no meds , 0:disable, 1:enable", FCVAR_PLUGIN);
	l4d_selfhelp_incap = CreateConVar("l4d_selfhelp_incap", "4", "self help for incap , 0:disable, 1:pill, 2:medkit, 3:both, 4:none ", FCVAR_PLUGIN);
	l4d_selfhelp_grab = CreateConVar("l4d_selfhelp_grab", "4", " self help for grab , 0:disable, 1:pill, 2:medkit, 3:both, 4:none ", FCVAR_PLUGIN);
	l4d_selfhelp_pounce = CreateConVar("l4d_selfhelp_pounce", "4", " self help for pounce , 0:disable, 1:pill, 2:medkit, 3:both, 4:none ", FCVAR_PLUGIN);
	l4d_selfhelp_ride = CreateConVar("l4d_selfhelp_ride", "4", " self help for ride , 0:disable, 1:pill, 2:medkit, 3:both, 4:none ", FCVAR_PLUGIN);
	l4d_selfhelp_pummel = CreateConVar("l4d_selfhelp_pummel", "4", "self help for pummel , 0:disable, 1:pill, 2:medkit, 3:both, 4:none  ", FCVAR_PLUGIN);
	l4d_selfhelp_edgegrab = CreateConVar("l4d_selfhelp_edgegrab", "4", "self help for edgegrab , 0:disable, 1:pill, 2:medkit, 3:both, 4:none  ", FCVAR_PLUGIN);
	...
	l4d_selfhelp_duration = CreateConVar("l4d_selfhelp_duration", "3.0", "self help duration", FCVAR_PLUGIN);
	l4d_selfhelp_duration_incap = CreateConVar("l4d_selfhelp_duration_incap", "10.0", "self help duration", FCVAR_PLUGIN);
...
public Action:AdvertisePills(Handle:timer, any:client)
{
	...
	{
		PrintToChat(client, "   \x03Hold \x04DUCK\x03 to help yourself!");
	}
...
public Action:PlayerTimer(Handle:timer, any:client)
{
	...
	if((haveone>0) || (GetConVarInt( l4d_selfhelp_none)>0))
	{
		...
		else
		{
			if (IncapType[client]==INCAP)
			{
				if(HelpState[client]==STATE_SELFHELP)
				{
					...
					{
						ShowBar(client, "SELF HELP", 0.0, GetConVarFloat(l4d_selfhelp_duration_incap));
					}
					HelpState[client]=STATE_NONE;
				}
			}
...
SelfHelp(client)
{
	...
	if(GetConVarInt( l4d_selfhelp_none) > 0)
	{
		if(GetConVarInt(l4d_selfhelp_kill)==1) KillAttack(client);
		
		ReviveClient(client);
			
		HelpState[client]=STATE_OK;
		PrintToChatAll("\x04%N\x03 helped himself!", client);
	}
	else
		{
		new bool:pills=HavePills(client);

#pragma semicolon 1
#include <sourcemod>
#include <sdktools>

#define PLUGIN_VERSION "1.2.2"

#define SOUND_KILL1  "/weapons/knife/knife_hitwall1.wav"
#define SOUND_KILL2  "/weapons/knife/knife_deploy.wav"

#define INCAP 1
#define INCAP_GRAB 2
#define INCAP_POUNCE 3
#define INCAP_RIDE 4
#define INCAP_PUMMEL 5
#define INCAP_LEDGE	6

#define STATE_NONE 0
#define STATE_SELFREVIVE 1
#define STATE_REVIVE 2
#define STATE_OK 3

#define SMOKER 1
#define HUNTER 3
#define JOCKEY 5
#define CHARGER 6
#define TANK 8

new Handle:Timers[MAXPLAYERS+1];
new Handle:HintTimer[MAXPLAYERS+1];

new HelpState[MAXPLAYERS+1];
new IncapType[MAXPLAYERS+1];
new Attacker[MAXPLAYERS+1];

new Float:HelpStartTime[MAXPLAYERS+1];
new Float:SelfReviveDuration[MAXPLAYERS+1];
new Float:ReviveHealth[MAXPLAYERS+1];

new Handle:l4d_2_selfrevive_hint_delay = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_delay = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_delay_ledge = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_delay_incap = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_duration = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_duration_ledge = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_duration_incap = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_health_incap = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_health_ledge = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_grab = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_pounce = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_ride = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_pummel = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_ledge = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_incap = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_kill = INVALID_HANDLE;
new Handle:l4d_2_selfrevive_versus = INVALID_HANDLE;

new GameMode;

new L4D2Version=false;

public Plugin:myinfo = 
{
	name = "[L4D, L4D2] Self Revive",
	author = "chinagreenelvis (Based on the Self Help plugin by Pan Xiaohai)",
	description = " ",
	version = PLUGIN_VERSION,	
}

public OnPluginStart()
{
	CreateConVar("l4d_2_selfrevive_version", PLUGIN_VERSION, " ", FCVAR_PLUGIN|FCVAR_DONTRECORD);
	
	l4d_2_selfrevive_hint_delay = CreateConVar("l4d_2_selfrevive_hint_delay", "4.0", "Self revive hint delay", FCVAR_PLUGIN);
	l4d_2_selfrevive_delay = CreateConVar("l4d_2_selfrevive_delay", "0.0", "Self revive delay", FCVAR_PLUGIN);
	l4d_2_selfrevive_delay_ledge = CreateConVar("l4d_2_selfrevive_delay_ledge", "0.0", "Self revive delay", FCVAR_PLUGIN);
	l4d_2_selfrevive_delay_incap = CreateConVar("l4d_2_selfrevive_delay_incap", "0.0", "Self revive delay for incapacitation", FCVAR_PLUGIN);
	l4d_2_selfrevive_duration = CreateConVar("l4d_2_selfrevive_duration", "3.0", "Self revive selfreviveDuration[victim]", FCVAR_PLUGIN);
	l4d_2_selfrevive_duration_ledge = CreateConVar("l4d_2_selfrevive_duration_ledge", "4.5", "Self revive duration for ledge grab, setting higher than 5 will disable animation", FCVAR_PLUGIN);
	l4d_2_selfrevive_duration_incap = CreateConVar("l4d_2_selfrevive_duration_incap", "5.0", "Self revive duration for incapacitation, setting higher than 5 will disable animation", FCVAR_PLUGIN);
	l4d_2_selfrevive_health_ledge = CreateConVar("l4d_2_selfrevive_health_ledge", "40.0", "How much health you have after reviving yourself from a ledge grab.", FCVAR_PLUGIN);	
	l4d_2_selfrevive_health_incap = CreateConVar("l4d_2_selfrevive_health_incap", "40.0", "How much health you have after reviving yourself from incapacitation.", FCVAR_PLUGIN);
	l4d_2_selfrevive_ledge = CreateConVar("l4d_2_selfrevive_ledge", "1", "Self revive for ledge grabs, 0:Disable, 1:Enable", FCVAR_PLUGIN);		
	l4d_2_selfrevive_incap = CreateConVar("l4d_2_selfrevive_incap", "1", "Self revive for incapacitation, 0:Disable, 1:Enable", FCVAR_PLUGIN);
	l4d_2_selfrevive_grab = CreateConVar("l4d_2_selfrevive_grab", "1", "Self revive for smoker grab, 0:Disable, 1:Enable", FCVAR_PLUGIN);
	l4d_2_selfrevive_pounce = CreateConVar("l4d_2_selfrevive_pounce", "1", "Self revive for hunter pounce, 0:Disable, 1:Enable", FCVAR_PLUGIN);
	l4d_2_selfrevive_ride = CreateConVar("l4d_2_selfrevive_ride", "1", " Self revive for jockey ride, 0:Disable, 1:Enable", FCVAR_PLUGIN);
	l4d_2_selfrevive_pummel = CreateConVar("l4d_2_selfrevive_pummel", "1", "Self revive for charger pummel , 0:Disable, 1:Enable", FCVAR_PLUGIN);
	l4d_2_selfrevive_kill = CreateConVar("l4d_2_selfrevive_kill", "0", "0: Break free from smokers, hunters, and jockeys, use your personal lighter to set fire to chargers; 1: Knife-kill all attackers", FCVAR_PLUGIN);	
	l4d_2_selfrevive_versus = CreateConVar("l4d_2_selfrevive_versus", "0", "0: Disable in versus, 1: Enable in versus", FCVAR_PLUGIN);
	
	AutoExecConfig(true, "l4d_2_selfrevive");
	GameCheck();
	
	HookEvent("lunge_pounce", Event_LungePounce);
	HookEvent("pounce_stopped", Event_PounceStopped);
	HookEvent("tongue_grab", Event_TongueGrab);
	HookEvent("tongue_release", Event_TongueRelease);
	
	if(L4D2Version)
	{
		HookEvent("jockey_ride", Event_JockeyRide);
		HookEvent("jockey_ride_end", Event_JockeyRideEnd);
		HookEvent("charger_pummel_start", Event_ChargerPummelStart);
		HookEvent("charger_pummel_end", Event_ChargerPummelEnd);
	}
	
	HookEvent("player_incapacitated", Event_PlayerIncapacitated);
	HookEvent("player_ledge_grab", Event_PlayerLedgeGrab);
	HookEvent("player_hurt", Event_PlayerHurt);
	HookEvent("player_death", Event_PlayerDeath);
	HookEvent("survivor_rescued", Event_SurvivorRescued);
	HookEvent("revive_begin", Event_ReviveBegin);
	HookEvent("revive_success", Event_ReviveSuccess);
	
	SetConVarInt(FindConVar("tongue_break_from_damage_amount"), 1);
}

GameCheck()
{
	decl String:GameName[16];
	GetConVarString(FindConVar("mp_gamemode"), GameName, sizeof(GameName));
	if (StrEqual(GameName, "survival", false))
	{
		GameMode = 3;
	}
	else if (StrEqual(GameName, "versus", false) || StrEqual(GameName, "teamversus", false) || StrEqual(GameName, "scavenge", false) || StrEqual(GameName, "teamscavenge", false))
	{
		GameMode = 2;
	}
	else if (StrEqual(GameName, "coop", false) || StrEqual(GameName, "realism", false))
	{
		GameMode = 1;
	}
	else
	{
		GameMode = 0;
 	}
	GetGameFolderName(GameName, sizeof(GameName));
	if (StrEqual(GameName, "left4dead2", false))
	{
		L4D2Version=true;
	}	
	else
	{ 
		L4D2Version=false;
	}
}

public OnMapStart()
{
 	if(L4D2Version)	
	{
		PrecacheSound(SOUND_KILL2, true);
	}
	else 
	{
		PrecacheSound(SOUND_KILL1, true);
	}
}

public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	for (new x = 0; x < MAXPLAYERS+1; x++)
	{
		HelpState[x] = STATE_OK;
		Attacker[x] = 0;
		HelpStartTime[x] = 0.0;
		if(Timers[x] != INVALID_HANDLE)
		{
			KillTimer(Timers[x]);
		}
		Timers[x] = INVALID_HANDLE;
	}
	return Plugin_Continue;
}

public OnClientPutInServer()
{
	new client = GetClientOfUserId(client);
	Attacker[client] = 0;
	HelpStartTime[client] = 0.0;
	Timers[client] = INVALID_HANDLE;
}

public OnClientDisconnect()
{
	new client = GetClientOfUserId(client);
	Attacker[client] = 0;
	HelpStartTime[client] = 0.0;
	Timers[client] = INVALID_HANDLE;
}

public Event_TongueGrab (Handle:event, const String:name[], bool:dontBroadcast)
{
	//PrintToChatAll("TongueGrab");
	if (GameMode == 2 && GetConVarInt(l4d_2_selfrevive_versus) == 0) return;
	//PrintToChatAll("Gamemode correct");
	new victim = GetClientOfUserId(GetEventInt(event, "victim"));
	new attacker = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!victim) return;
	//PrintToChatAll("Victim");
	if (!attacker) return;
	Attacker[victim] = attacker;
	//PrintToChatAll("Attacker");
	IncapType[victim] = INCAP_GRAB;
	//PrintToChatAll("IncapType = INCAP_GRAB");
	SelfReviveDuration[victim] = GetConVarFloat(l4d_2_selfrevive_duration);
	if (GetConVarInt(l4d_2_selfrevive_grab) > 0)
	{
		//PrintToChatAll("l4d_2_selfrevive_grab > 0");
		CreateTimer(GetConVarFloat(l4d_2_selfrevive_delay), Timer_Delay, victim);	
	}
}

public Event_TongueRelease (Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GameMode == 2 && GetConVarInt(l4d_2_selfrevive_versus) == 0) return;
	new victim = GetClientOfUserId(GetEventInt(event, "victim"));
	new attacker = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!victim) return;
	if (!attacker) return;
	if (Attacker[victim] == attacker)
	{
		Attacker[victim] = 0;
	}
}

public Event_LungePounce (Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GameMode == 2 && GetConVarInt(l4d_2_selfrevive_versus) == 0) return;
	new victim = GetClientOfUserId(GetEventInt(event, "victim"));
	new attacker = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!victim) return;
	if (!attacker) return;
	Attacker[victim] = attacker;
	IncapType[victim] = INCAP_POUNCE;
	SelfReviveDuration[victim] = GetConVarFloat(l4d_2_selfrevive_duration);
	if (GetConVarInt(l4d_2_selfrevive_pounce) > 0)
	{
		CreateTimer(GetConVarFloat(l4d_2_selfrevive_delay), Timer_Delay, victim);	
	} 
}

public Event_PounceStopped (Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GameMode == 2 && GetConVarInt(l4d_2_selfrevive_versus) == 0) return;
	new victim = GetClientOfUserId(GetEventInt(event, "victim"));
	new attacker = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!victim) return;
	if (!attacker) return;
	if (Attacker[victim] == attacker)
	{
		Attacker[victim] = 0;
	}
}

public Event_JockeyRide (Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GameMode == 2 && GetConVarInt(l4d_2_selfrevive_versus) == 0) return;
	new victim = GetClientOfUserId(GetEventInt(event, "victim"));
	new attacker = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!victim) return;
	if (!attacker) return;
	Attacker[victim] = attacker;
	IncapType[victim] = INCAP_RIDE;
	SelfReviveDuration[victim] = GetConVarFloat(l4d_2_selfrevive_duration);
	if (GetConVarInt(l4d_2_selfrevive_ride) > 0)
	{
		CreateTimer(GetConVarFloat(l4d_2_selfrevive_delay), Timer_Delay, victim);	
	} 
}

public Event_JockeyRideEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GameMode == 2 && GetConVarInt(l4d_2_selfrevive_versus) == 0) return;
	new victim = GetClientOfUserId(GetEventInt(event, "victim"));
	new attacker = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!victim) return;
	if (!attacker) return;
	if (Attacker[victim] == attacker)
	{
		Attacker[victim] = 0;
	}
}

public Event_ChargerPummelStart (Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GameMode == 2 && GetConVarInt(l4d_2_selfrevive_versus) == 0) return;
	new victim = GetClientOfUserId(GetEventInt(event, "victim"));
	new attacker = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!victim) return;
	if (!attacker) return;
	Attacker[victim] = attacker;
	IncapType[victim] = INCAP_PUMMEL;
	SelfReviveDuration[victim] = GetConVarFloat(l4d_2_selfrevive_duration);
	if (GetConVarInt(l4d_2_selfrevive_pummel) > 0)
	{
		CreateTimer(GetConVarFloat(l4d_2_selfrevive_delay), Timer_Delay, victim);	
	} 
}

public Event_ChargerPummelEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GameMode == 2 && GetConVarInt(l4d_2_selfrevive_versus) == 0) return;
	new victim = GetClientOfUserId(GetEventInt(event, "victim"));
	new attacker = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!victim) return;
	if (!attacker) return;
	if (Attacker[victim] == attacker)
	{
		Attacker[victim] = 0;
	}
}

public Event_PlayerIncapacitated (Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GameMode == 2 && GetConVarInt(l4d_2_selfrevive_versus) == 0) return;
	new victim = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!victim) return;
	IncapType[victim] = INCAP;
	SelfReviveDuration[victim] = GetConVarFloat(l4d_2_selfrevive_duration_incap);
	if (GetConVarInt(l4d_2_selfrevive_incap) > 0)
	{
		CreateTimer(GetConVarFloat(l4d_2_selfrevive_delay_incap), Timer_Delay, victim);	
	}
}

public Event_PlayerLedgeGrab (Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GameMode == 2 && GetConVarInt(l4d_2_selfrevive_versus) == 0) return;
	new victim = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!victim) return;
	IncapType[victim] = INCAP_LEDGE;
	SelfReviveDuration[victim] = GetConVarFloat(l4d_2_selfrevive_duration_ledge);
	if (GetConVarInt(l4d_2_selfrevive_incap) > 0)
	{
		CreateTimer(GetConVarFloat(l4d_2_selfrevive_delay_ledge), Timer_Delay, victim);	
	}
}

public Action:Timer_Delay(Handle:timer, any:client)
{
	//PrintToChatAll("Timer_Delay");
	if (IncapType[client] == INCAP && GetConVarInt(l4d_2_selfrevive_incap) == 0)
	{
		HelpState[client] = STATE_OK;
		Timers[client] = INVALID_HANDLE;
		//PrintToChatAll("l4d_2_selfrevive_incap is Zero");
		return;
	}
	if (IncapType[client] == INCAP_LEDGE && GetConVarInt(l4d_2_selfrevive_ledge) == 0)
	{
		HelpState[client] = STATE_OK;
		Timers[client] = INVALID_HANDLE;
		//PrintToChatAll("l4d_2_selfrevive_ledge is Zero");
		return;
	}
	if (IncapType[client] == INCAP_GRAB && GetConVarInt(l4d_2_selfrevive_grab) == 0)
	{
		HelpState[client] = STATE_OK;
		Timers[client] = INVALID_HANDLE;
		//PrintToChatAll("l4d_2_selfrevive_grab is Zero");
		return;
	}
	if (IncapType[client] == INCAP_POUNCE && GetConVarInt(l4d_2_selfrevive_pounce) == 0)
	{
		HelpState[client] = STATE_OK;
		Timers[client] = INVALID_HANDLE;
		//PrintToChatAll("l4d_2_selfrevive_pounce is Zero");
		return;
	}
	if (IncapType[client] == INCAP_RIDE && GetConVarInt(l4d_2_selfrevive_ride) == 0)
	{
		HelpState[client] = STATE_OK;
		Timers[client] = INVALID_HANDLE;
		//PrintToChatAll("l4d_2_selfrevive_ride is Zero");
		return;
	}
	if (IncapType[client] == INCAP_PUMMEL && GetConVarInt(l4d_2_selfrevive_pummel) == 0)
	{
		HelpState[client] = STATE_OK;
		Timers[client] = INVALID_HANDLE;
		//PrintToChatAll("l4d_2_selfrevive_pummel is Zero");
		return;
	}
	else
	{
		HelpState[client] = STATE_NONE;
		HintTimer[client] = CreateTimer(GetConVarFloat(l4d_2_selfrevive_hint_delay), Timer_HintDelay, client);
		Timers[client] = CreateTimer(1.0/10, Timer_SelfRevive, client, TIMER_REPEAT);
	}
}

public Action:Timer_HintDelay(Handle:timer, any:client)
{
	//PrintToChatAll("Timer_HintDelay");
	if (IsClientInGame(client) && IsPlayerAlive(client))
	{
		PrintHintText(client, "       \x03Hold \x04DUCK\x03 to help yourself!");
		HintTimer[client] = INVALID_HANDLE;
	}
	else
	{
		HintTimer[client] = INVALID_HANDLE;
	}
}

public Action:Timer_SelfRevive(Handle:timer, any:client)
{
	//PrintToChatAll("Timer_SelfRevive");
	if (HelpState[client] == STATE_OK)
	{
		Timers[client] = INVALID_HANDLE;
		//PrintToChatAll("HelpState OK");
		return Plugin_Stop;
	}
	if (client == 0)
	{
		if (HelpState[client] == STATE_SELFREVIVE)
		{
			if (L4D2Version)
			{
				SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
				SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", 0.0);
			}
			else
			{
				ShowBar(client, "Self Help", 0.0, SelfReviveDuration[client]);
			}
		}
		HelpState[client] = STATE_OK;
		if (HintTimer[client] != INVALID_HANDLE)
		{
			KillTimer(HintTimer[client]);
			HintTimer[client] = INVALID_HANDLE;
		}
		Timers[client] = INVALID_HANDLE;
		//PrintToChatAll("Client Zero");
		return Plugin_Stop;
	}
	if (!IsClientInGame(client) || !IsPlayerAlive(client)) 
	{
		if (HelpState[client] == STATE_SELFREVIVE)
		{
			if (L4D2Version)
			{
				SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
				SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", 0.0);
			}
			else
			{
				ShowBar(client, "Self Help", 0.0, SelfReviveDuration[client]);
			}
		}
		HelpState[client] = STATE_OK;
		if (HintTimer[client] != INVALID_HANDLE)
		{
			KillTimer(HintTimer[client]);
			HintTimer[client] = INVALID_HANDLE;
		}
		Timers[client] = INVALID_HANDLE;
		//PrintToChatAll("Client Not in Game or Client Not Alive");
		return Plugin_Stop;
	}
	if (!IsPlayerIncapacitated(client) && !IsPlayerHangingFromLedge(client) && Attacker[client] == 0)
	{
		if (HelpState[client] == STATE_SELFREVIVE)
		{
			if (L4D2Version)
			{
				SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
				SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", 0.0);
			}
			else
			{
				ShowBar(client, "Self Help", 0.0, SelfReviveDuration[client]);
			}
		}
		HelpState[client] = STATE_OK;
		if (HintTimer[client] != INVALID_HANDLE)
		{
			KillTimer(HintTimer[client]);
			HintTimer[client] = INVALID_HANDLE;
		}
		Timers[client] = INVALID_HANDLE;
		//PrintToChatAll("Attacker is Zero");
		return Plugin_Stop;
	}
	if ((!IsPlayerIncapacitated(client) && !IsPlayerHangingFromLedge(client) && Attacker[client] != 0) && (!IsClientInGame(Attacker[client]) || !IsPlayerAlive(Attacker[client])))
	{
		if (HelpState[client] == STATE_SELFREVIVE)
		{
			if (L4D2Version)
			{
				SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
				SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", 0.0);
			}
			else
			{
				ShowBar(client, "Self Help", 0.0, SelfReviveDuration[client]);
			}
		}
		HelpState[client] = STATE_OK;
		if (HintTimer[client] != INVALID_HANDLE)
		{
			KillTimer(HintTimer[client]);
			HintTimer[client] = INVALID_HANDLE;
		}
		Timers[client] = INVALID_HANDLE;
		Attacker[client] = 0;
		//PrintToChatAll("Attacker Not in Game");
		return Plugin_Stop;
	}
	if ((!IsPlayerIncapacitated(client) && Attacker[client] != 0) && (!IsClientInGame(Attacker[client]) || !IsPlayerAlive(Attacker[client])))
	{
		if (HelpState[client] == STATE_SELFREVIVE)
		{
			if (L4D2Version)
			{
				SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
				SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", 0.0);
			}
			else
			{
				ShowBar(client, "Self Help", 0.0, SelfReviveDuration[client]);
			}
		}
		HelpState[client] = STATE_OK;
		if (HintTimer[client] != INVALID_HANDLE)
		{
			KillTimer(HintTimer[client]);
			HintTimer[client] = INVALID_HANDLE;
		}
		Timers[client] = INVALID_HANDLE;
		Attacker[client] = 0;
		//PrintToChatAll("Attacker Not in Game");
		return Plugin_Stop;
	}	
	else
	{
		//PrintToChatAll("Can SelfRevive");
		new Float:time = GetEngineTime();
		new buttons = GetClientButtons(client);
		if (buttons & IN_DUCK || buttons & IN_USE || buttons & IN_ATTACK2)
		{
			if (HelpState[client] != STATE_REVIVE)
			{
				if (HelpState[client] == STATE_NONE)
				{
					if (HintTimer[client] != INVALID_HANDLE)
					{
						KillTimer(HintTimer[client]);
						HintTimer[client] = INVALID_HANDLE;
					}
					HelpStartTime[client] = time;
					if (L4D2Version)
					{
						if ((IncapType[client] == INCAP || IncapType[client] == INCAP_LEDGE) && (!Attacker[client] && SelfReviveDuration[client] <= 5.0))
						{
							SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
							SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", SelfReviveDuration[client]);
							SetEntPropEnt(client, Prop_Send, "m_reviveOwner", client);
						}
						else
						{
							SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
							SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", SelfReviveDuration[client]);
						}
					}
					else
					{
						ShowBar(client,"Self Help", time - HelpStartTime[client], SelfReviveDuration[client]);
					}
				}
				if (time - HelpStartTime[client] < SelfReviveDuration[client])
				{
					HelpState[client] = STATE_SELFREVIVE;
					//PrintToChatAll("%f  %f", time - HelpStartTime[client], SelfReviveDuration[client]);
				}
				if (time - HelpStartTime[client] >= SelfReviveDuration[client])
				{
					SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
					SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", 0.0);
					SetEntPropEnt(client, Prop_Send, "m_reviveOwner", -1);
					HelpState[client] = STATE_OK;
					SelfRevive(client);
				}
			}
		}
		else 
		{
			if (HelpState[client] == STATE_SELFREVIVE)
			{
				if (L4D2Version)
				{
					if ((IncapType[client] == INCAP || IncapType[client] == INCAP_LEDGE) && (!Attacker[client] || SelfReviveDuration[client] <= 5.0))
					{
						SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
						SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", 0.0);
						SetEntPropEnt(client, Prop_Send, "m_reviveOwner", -1);
						HelpState[client] = STATE_NONE;
					}
					else
					{
						SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
						SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", 0.0);
						HelpState[client] = STATE_NONE;
					}
				}
				else
				{
					ShowBar(client, "Self Help", 0.0, SelfReviveDuration[client]);
					HelpState[client] = STATE_NONE;
				}
				if (HintTimer[client] == INVALID_HANDLE)
				{
					HintTimer[client] = CreateTimer(GetConVarFloat(l4d_2_selfrevive_hint_delay), Timer_HintDelay, client);
				}
			}
			if (HelpState[client] == STATE_REVIVE)
			{
				if (time - HelpStartTime[client] > GetConVarFloat(FindConVar("survivor_revive_duration")))
				{
					HelpState[client] = STATE_NONE;
					if (HintTimer[client] == INVALID_HANDLE)
					{
						HintTimer[client] = CreateTimer(GetConVarFloat(l4d_2_selfrevive_hint_delay), Timer_HintDelay, client);
					}
				}
			}
		}
	}
	return Plugin_Continue;
}

public Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	new attacker = GetClientOfUserId(GetEventInt(event, "attacker"));
	new damagetype = GetEventInt(event, "type");
	if (IsClientInGame(client) && GetClientTeam(client) == 2)
	{
		if (HelpState[client] == STATE_SELFREVIVE)
		{
			if (damagetype != 131072 && !Attacker[client] && (!attacker || (attacker && GetClientTeam(attacker) != 2)))
			{
				if ((IncapType[client] == INCAP || IncapType[client] == INCAP_LEDGE) && (SelfReviveDuration[client] <= 5.0))
				{
					SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
					SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", 0.0);
					SetEntPropEnt(client, Prop_Send, "m_reviveOwner", -1);
					HelpState[client] = STATE_NONE;
				}
				else
				{
					//SetEntPropFloat(client, Prop_Send, "m_flProgressBarStartTime", GetGameTime());
					//SetEntPropFloat(client, Prop_Send, "m_flProgressBarDuration", 0.0);
					//HelpState[client] = STATE_NONE;
				}
			}
		}
		if (HelpState[client] == STATE_REVIVE)
		{
			if (damagetype == 131072)
			{
				HelpState[client] = STATE_NONE;
				if (HintTimer[client] == INVALID_HANDLE)
				{
					HintTimer[client] = CreateTimer(GetConVarFloat(l4d_2_selfrevive_hint_delay), Timer_HintDelay, client);
				}
			}
		}
	}
}

public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	Attacker[client] = 0;
	HelpState[client] = STATE_OK;
	if(HintTimer[client] != INVALID_HANDLE)
	{
		KillTimer(HintTimer[client]);
		HintTimer[client] = INVALID_HANDLE;
	}
}

public Event_SurvivorRescued(Handle:event, const String:name[], bool:dontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(event, "subject"));
	Attacker[client] = 0;
	HelpState[client] = STATE_OK;
}

public Event_ReviveBegin(Handle:event, const String:name[], bool:dontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(event, "subject"));
	new Float:time = GetEngineTime();
	HelpStartTime[client] = time;
	HelpState[client] = STATE_REVIVE;
	if(HintTimer[client] != INVALID_HANDLE)
	{
		KillTimer(HintTimer[client]);
		HintTimer[client] = INVALID_HANDLE;
	}
}

public Event_ReviveSuccess(Handle:event, const String:name[], bool:dontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(event, "subject"));
	HelpState[client] = STATE_OK;
	if (HintTimer[client] != INVALID_HANDLE)
	{
		KillTimer(HintTimer[client]);
		HintTimer[client] = INVALID_HANDLE;
	}
}

SelfRevive(client)
{	
	if (Attacker[client])
	{
		if (GetConVarInt(l4d_2_selfrevive_kill) > 0) 
		{
			KillAttacker(client);
		}
		else
		{
			KnockAttacker(client);
		}
	}
	else
	{
		Revive(client);
	}
}

KillAttacker(client)
{
	new attacker = Attacker[client];
	if (GetClientTeam(attacker) == 3 && IsClientInGame(attacker) && IsPlayerAlive(attacker) && GetEntProp(attacker, Prop_Send, "m_zombieClass") != TANK)
	{
		ForcePlayerSuicide(attacker);
		if(L4D2Version)
		{
			EmitSoundToAll(SOUND_KILL2, client);
		}
		else 
		{
			EmitSoundToAll(SOUND_KILL1, client);
		}
		
	}
	Revive(client);
}

KnockAttacker(client)
{
	new attacker = Attacker[client];
	if (GetClientTeam(attacker) == 3 && IsClientInGame(attacker) && IsPlayerAlive(attacker) && GetEntProp(attacker, Prop_Send, "m_zombieClass") != TANK)
	{
		if (GetEntProp(attacker, Prop_Send, "m_zombieClass") == SMOKER)
		{
			SetEntPropEnt(attacker, Prop_Send, "m_tongueVictim", -1); 
			SetEntPropEnt(client, Prop_Send, "m_tongueOwner", -1); 
			SetEntProp(client, Prop_Send, "m_isHangingFromTongue", 0); 
			SetEntProp(client, Prop_Send, "m_isProneTongueDrag", 0);
			Attacker[client] = 0;
			Revive(client);
		}
		if (GetEntProp(attacker, Prop_Send, "m_zombieClass") == HUNTER)
		{
			SetEntPropEnt(attacker, Prop_Send, "m_pounceVictim", -1);
			SetEntPropEnt(client, Prop_Send, "m_pounceAttacker", -1);
			SlapPlayer(attacker, 5, false);
			SlapPlayer(attacker, 5, false);
			SlapPlayer(attacker, 5, false);
			Attacker[client] = 0;
			Revive(client);
		}
		if (GetEntProp(attacker, Prop_Send, "m_zombieClass") == JOCKEY)
		{
			ExecuteCommand(attacker, "dismount");
		}
		if (GetEntProp(attacker, Prop_Send, "m_zombieClass") == CHARGER)
		{
			IgniteEntity(attacker, 5.0);
			CreateTimer(1.0, Timer_ForceAttackerSuicide, client);
		}
	}
}

public Action:Timer_ForceAttackerSuicide(Handle:timer, any:client)
{
	//PrintToChatAll("ForceAttackerSuicide");
	new attacker = Attacker[client];
	if (IsClientInGame(attacker) && IsPlayerAlive(attacker))
	{
		ForcePlayerSuicide(attacker);
		Revive(client);
	}
}

Revive(client)
{
	//PrintToChatAll("Revive");
	if (IncapType[client] == INCAP || IncapType[client] == INCAP_LEDGE)
	{
		if (IncapType[client] == INCAP)
		{
			ReviveHealth[client] = GetConVarFloat(l4d_2_selfrevive_health_incap);
		}
		if (IncapType[client] == INCAP_LEDGE)
		{
			ReviveHealth[client] = GetConVarFloat(l4d_2_selfrevive_health_ledge);
		}
		SetUserFlagBits(client, ADMFLAG_ROOT);
		SetCommandFlags("give", GetCommandFlags("give") & ~FCVAR_CHEAT);
		FakeClientCommand(client,"give health");
		SetCommandFlags("give", GetCommandFlags("give"));
		SetUserFlagBits(client, GetUserFlagBits(client));
		SetEntData(client, FindSendPropInfo("CTerrorPlayer", "m_currentReviveCount"), 0, 1);	 
		SetEntDataFloat(client, FindSendPropOffs("CTerrorPlayer","m_healthBuffer"), ReviveHealth[client], true);
		SetEntityHealth(client, 1);
	}
}

ExecuteCommand(Client, String:strCommand[])
{
	SetCommandFlags(strCommand, GetCommandFlags(strCommand) & ~FCVAR_CHEAT);
	FakeClientCommand(Client, "%s", strCommand);
	SetCommandFlags(strCommand, GetCommandFlags(strCommand));
}

bool:IsPlayerIncapacitated(client)
{
	if (GetEntProp(client, Prop_Send, "m_isIncapacitated", 1)) 
	{
		return true;
	}
 	return false;
}

bool:IsPlayerHangingFromLedge(client)
{
 	if (GetEntProp(client, Prop_Send, "m_isHangingFromLedge", 1))
	{
		return true;
	}
	return false;
}

ShowBar(client, String:msg[], Float:pos, Float:max)	 
{
	new String:Gauge1[2] = "-";
	new String:Gauge3[2] = "#";
	new i ;
	new String:ChargeBar[100];
	Format(ChargeBar, sizeof(ChargeBar), "");
 
	new Float:GaugeNum = pos/max*100;
	if (GaugeNum > 100.0)
		GaugeNum = 100.0;
	if (GaugeNum<0.0)
		GaugeNum = 0.0;
 	for (i=0; i<100; i++)
		ChargeBar[i] = Gauge1[0];
	new p=RoundFloat( GaugeNum);
	 
	if (p >= 0 && p < 100) ChargeBar[p] = Gauge3[0]; 
	PrintCenterText(client, "%s  %3.0f %\n<< %s >>", msg, GaugeNum, ChargeBar);
}


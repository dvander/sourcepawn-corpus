...
public OnPluginStart()
{
	...
	autohelp_incap_delay = CreateConVar("autohelp_incap_delay", "5", "AutoHelp delay for incapacitation", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_incap_duration = CreateConVar("autohelp_incap_duration", "5", "AutoHelp duration for incapacitation (setting higher than 5 will disable animation)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	autohelp_ledge_delay = CreateConVar("autohelp_ledge_delay", "5", "AutoHelp delay for ledge grabs", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_ledge_duration = CreateConVar("autohelp_ledge_duration", "4", "AutoHelp duration for ledge grabs (setting higher than 4 will cause animation issues)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	autohelp_specials_delay = CreateConVar("autohelp_specials_delay", "5", "AutoHelp delay for special infected holds", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_specials_duration = CreateConVar("autohelp_specials_duration", "5", "AutoHelp duration for special infected holds", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	
	AutoExecConfig(true, "l4d_2_autohelp");
...
public Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	for (new x = 0; x < MAXPLAYERS+1; x++)
	{
		...
		IncapType[x] = INCAP_NONE;
		Attacker[x] = 0;
		HelpStartTime[x] = 0.0;
		if(ReviveTimer[x] != INVALID_HANDLE)
		{
			KillTimer(ReviveTimer[x]);
		}
		ReviveTimer[x] = INVALID_HANDLE;
		DelayTimer[x] = INVALID_HANDLE;
		HeartSound[x] = 0;
		GetHealth(x);
	}
}
...
public Action:Timer_Delay(Handle:timer, any:client)
{
	...
	if (IsFakeClient(client) && GetEntProp(client, Prop_Send, "m_humanSpectatorUserID") == 0 && GetConVarInt(autohelp_bots) == 0)
	{
		return;
	}
...
RestartTimer(client)
{
	if (DelayTimer[client])
	{
		KillTimer(DelayTimer[client]);
		DelayTimer[client] = INVALID_HANDLE;
	}
	if (ReviveTimer[client])
	{
...
CallOnPummelEnded(client)
{
	...
	if (hOnPummelEnded == INVALID_HANDLE)
	{
		...
		hConf = LoadGameConfigFile("l4d_2_autohelp");

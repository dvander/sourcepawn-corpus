...
new Handle:l4d_2_specialregulator_delay = INVALID_HANDLE;
new Handle:l4d_2_specialregulator_duration = INVALID_HANDLE;
new Handle:l4d_2_specialregulator_interval = INVALID_HANDLE;
...
{	
	l4d_2_specialregulator_delay = CreateConVar("l4d_2_specialregulator_delay", "120", "Time until the first specials are allowed to spawn", FCVAR_PLUGIN);
	l4d_2_specialregulator_duration = CreateConVar("l4d_2_specialregulator_duration", "30", "Time during which specials are allowed to spawn", FCVAR_PLUGIN);
	l4d_2_specialregulator_interval = CreateConVar("l4d_2_specialregulator_interval", "60", "Time between special spawns", FCVAR_PLUGIN);
...
public Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (StrEqual(GameName, "coop", false) || StrEqual(GameName, "realism", false))
	{
		...
		{
			TimerSpecialDelay = CreateTimer(GetConVarFloat(l4d_2_specialregulator_delay), Timer_SpecialDelay);
		}
	}
}
...
{	
	...
	{
		TimerSpecialsAllow = CreateTimer(GetConVarFloat(l4d_2_specialregulator_interval), Timer_SpecialsAllow, _, TIMER_REPEAT);
	}
	if (TimerSpecialDuration == INVALID_HANDLE)
	{
		TimerSpecialDuration = CreateTimer(GetConVarFloat(l4d_2_specialregulator_duration), Timer_SpecialDuration);
	}
}
...
{	
	...
	{
		TimerSpecialsDisable = CreateTimer(GetConVarFloat(l4d_2_specialregulator_interval), Timer_SpecialsDisable, _, TIMER_REPEAT);
	}
}

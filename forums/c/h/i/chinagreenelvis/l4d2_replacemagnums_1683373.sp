...
ReplaceMagnum()
{
    ...
    decl Float:origin[3];
    decl Float:angles[3];
    while ((ent = FindEntityByClassname(ent, "weapon_pistol_magnum_spawn")) != -1)
    {
        if (prev)
        {
            ...
            //PrintToChatAll("Replacing weapon_pistol_magnum_spawn %i with weapon_pistol %i", prev, replacement);
            if (!IsValidEdict(replacement)) return;
            
            TeleportEntity(replacement, origin, angles, NULL_VECTOR);
            //PrintToChatAll("Teleported weapon_pistol %i into position, removing weapon_pistol_magnum_spawn now", replacement);

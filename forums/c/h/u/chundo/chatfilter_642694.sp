...
new g_cfPlayerOffenses[CF_MAX_PLAYERS];
new String:g_cfLastNameChecked[CF_MAX_PLAYERS][64];
...
CheckClientName(client) {
	if (GetConVarBool(cvarCFEnabled) && GetConVarBool(cvarCFNames)) {
		if (client != 0) {
			...
			if (strcmp(g_cfLastNameChecked[client], clientName) != 0) {
				...
				if (idx != -1) {
					...
					LogMessage("Name changed from %s to %s", clientName, randomName);
					g_cfPlayerOffenses[client]++;
					strcopy(g_cfLastNameChecked[client], sizeof(clientName), clientName);
					ClientCommand(client, "name \"%s\"", randomName);
				}
			} else {
				// Second attempt to change the same name.  Ideally, we'd kick
				// here, but name changing doesn't really seem to work on TF2.
				LogError("Name change failed for %s", clientName);
			}
		}
	}
}
	...
	if (client != 0) {
		...
		if (citer != INVALID_HANDLE) {
			...
			if (strcmp(sm_action, "sm_kick") == 0 || strcmp(sm_action, "kick") == 0) {
				...
				ShowActivity(0, "Kicked %s for inappropriate behavior.", sm_action, clientName);
				KickClient(client, "%s", "You found the secret word!.");
			} else if (strcmp(sm_action, "sm_ban") == 0 || strcmp(sm_action, "ban") == 0) {
				LogMessage("Banned %s", clientName);
				ShowActivity(0, "Banned %s for inappropriate behavior.", sm_action, clientName);
				BanClient(client, 60, BANFLAG_AUTO, "Innapporiate chat behavior", "You found the secret word!.");
			} else {

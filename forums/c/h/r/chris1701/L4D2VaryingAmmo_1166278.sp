...
Version 1.5.3 - Added Magnum Support
________________________________________________________
...
new Handle:magnumammo_min
new Handle:varytimer_interval
...
public OnPluginStart()
{
		...
		varytimer_interval = CreateConVar("l4d_variate_interval", "60", "Defines the timer interval to create random values for weapons ammo.", FCVAR_NOTIFY)
		...
		magnumammo = FindConVar("ammo_pistol_max")
		...
		magnumammo = FindConVar("l4d2_guncontrol_magnumammo")
	
		// min ammo (created own convar to avoid conflicts)
		huntingrifleammo_min = CreateConVar("l4d_huntingrifleammo_min", "85", "Min Hunting Rifle ammo variation", FCVAR_NOTIFY)
		...
		autoshotammo_min = CreateConVar("l4d_autoshotammo_min", "40", "Min AutoShotgun ammo variation", FCVAR_NOTIFY)
		smgammo_min = CreateConVar("l4d_smgammo_min", "352", "Min SMGs ammo variation (Silenced, MP5 and Normal SMG)", FCVAR_NOTIFY)
		sniperammo_min = CreateConVar("l4d_sniperammo_min", "80", "Min Snipers ammo variation (AWP, Scout, Military)", FCVAR_NOTIFY)
		...
		magnumammo_min = CreateConVar("l4d_magnumammo_min", "96")
		...
		pumpshotammo_max = CreateConVar("l4d_shotgunammo_max", "64", "Max Shotgun ammo variation (Pump and Chrome)", FCVAR_NOTIFY)
		...
		smgammo_max = CreateConVar("l4d_smgammo_max", "480", "Max SMGs ammo variation (Silenced, MP5 and Normal SMG)", FCVAR_NOTIFY)
		...
		magnumammo_max = CreateConVar("l4d_magnumammo_max", "120")
		CreateTimer(GetConVarFloat(varytimer_interval), GunTimer, _, TIMER_REPEAT)
...
{	
	...
	if (NumPrinted++ <= 2)
	{
			...
			new Magnum = GetRandomInt(GetConVarInt(magnumammo_min), GetConVarInt(magnumammo_max))
...
//Now to set the ammo cvars to the new randomized values, which was just done above
			...
			SetConVarInt(magnumammo, Magnum)

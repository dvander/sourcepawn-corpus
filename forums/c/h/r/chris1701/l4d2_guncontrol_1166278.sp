...
static const	PISTOL_MAGNUM_OFFSET_IAMMO		= 44;
static const	M60_OFFSET_IAMMO				= 48;
...
static Handle:MagnumAmmoCVAR = INVALID_HANDLE;
static Handle:M60Ammo2CVAR = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	SMGAmmoCVAR = CreateConVar("l4d2_guncontrol_smgammo", "480", " How much Ammo for SMG gun types ", DEFAULT_FLAGS);
	ShotgunAmmoCVAR = CreateConVar("l4d2_guncontrol_shotgunammo", "64", " How much Ammo for Shotgun and Chrome Shotgun ", DEFAULT_FLAGS);
	...
	M60AmmoCVAR = CreateConVar("l4d2_guncontrol_m60ammo", "100", " How much Ammo for the M60 (a clip of ammo) ", DEFAULT_FLAGS);
	MagnumAmmoCVAR = CreateConVar("l4d2_guncontrol_magnumammo", "120", " How much Ammo for Magnum ", FCVAR_PLUGIN|FCVAR_NOTIFY);		
	M60Ammo2CVAR = CreateConVar("l4d2_guncontrol_m60ammo2", "400", " How much Ammo for the M60 (Reserved for reloading) ", DEFAULT_FLAGS);		
	...
	GLtoM60TransformCVAR = CreateConVar("l4d2_guncontrol_turnGLintoM60chance", "1", " Turns GL spawns into M60 spawns. Works as chance setting. 1 is FULL chance, 2 is half chance, 3 one third and so on ", DEFAULT_FLAGS);
	...
	SplosiveAmmoMultiplier = CreateConVar("l4d2_guncontrol_explosiveammomulti", "2", " Multiplier for Explosive Ammo Pickup Amount ", DEFAULT_FLAGS);
	...
	HookConVarChange(MagnumAmmoCVAR, CVARChanged);
	HookConVarChange(M60Ammo2CVAR, CVARChanged);
...
UpdateConVars()
{
	...
	SetConVarInt(FindConVar("ammo_pistol_max"), GetConVarInt(MagnumAmmoCVAR));	
	SetConVarInt(FindConVar("ammo_m60_max"), GetConVarInt(M60Ammo2CVAR));
}
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	if (buttons & IN_USE && !buttondelay[client])
	{
		...
		if (StrEqual(ent_name, "weapon_ammo_spawn", false) && StrEqual(currentgunname, "weapon_grenade_launcher", false) && GetConVarBool(GrenadeResupplyCVAR))
		{
			...
			PrintHintText(client, "你補充了榴彈炮的彈藥!");
		}
		...
		if (StrEqual(ent_name, "weapon_ammo_spawn", false) && StrEqual(currentgunname, "weapon_rifle_m60", false) && GetConVarBool(M60ResupplyCVAR))
		{
			...
			PrintHintText(client, "你補充了M60的彈藥!");
		}
		...
		else if (StrEqual(ent_name, "weapon_pistol_magnum", false))
		{ //case: Magnum
			offsettoadd = PISTOL_MAGNUM_OFFSET_IAMMO; //gun type specific offset
			maxammo = GetConVarInt(MagnumAmmoCVAR); //get max ammo as set
		}
		else if (StrEqual(ent_name, "weapon_rifle_m60", false))
		{ //case: M60
			offsettoadd = M60_OFFSET_IAMMO; //gun type specific offset
			maxammo = GetConVarInt(M60Ammo2CVAR); //get max ammo as set
		}
		else
		...
		{
			PrintHintText(client, "這槍沒有彈藥!"); //the gun is empty, bug out
			return Plugin_Continue;
		}
		...
		if ((currentammo + foundgunammo) <= maxammo) //if contained ammo is less than youd need to be full
		{
			...
			PrintHintText(client, "你拿取了這支槍餘下 %i 彈藥", foundgunammo); //imform the client
		}
		else //if contained ammo exceeds your needs
		{
			...
			PrintHintText(client, "你拿取了這支槍餘下 %i 彈藥", neededammo); //inform the client
		}
	}
...
public Action:Cmd_GiveAmmo(client, args)
{
	...
	{
		PrintToChat(client, "\x04你只能在雙方擁有相同武器的情況下\x01給予 \x04%N\x01 彈藥!", target);
	...
	{
		PrintToChat(client, "\x01你至少擁有一排彈匣才能給予 \x04%N\x01 彈藥", target);
	...
	else if (StrEqual(currentgunname, "weapon_pistol_magnum", false))
	{ //case: Pistol or Magnum
		offsettoadd = PISTOL_MAGNUM_OFFSET_IAMMO; //gun type specific offset
		maxammo = GetConVarInt(MagnumAmmoCVAR); //get max ammo as set
	}
	...
	{
		PrintToChat(client, "\x01目標彈藥\x04 已滿!");
		return Plugin_Handled;
	}
	...
	if ((currentammo + donateammo) <= maxammo) //if clips ammo is less than hed need to be full
	{
		...
		PrintToChat(client, "\x01你把一排 \x04%i 彈藥\x01 給予 \x04%N\x01", donateammo, target);
	}
	else //if given ammo exceeds his needs
	{
		...
		PrintToChat(client, "\x01你把一排 \x04%i 彈藥\x01 給予 \x04%N\x01", neededammo, target);
	}	
	return Plugin_Handled;
}
...
public Action:Event_SpecialAmmo(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	PrintToChat(client, "\x01你拿到 \x04%i \x01 排特殊彈藥!", newammo/ammo);
}

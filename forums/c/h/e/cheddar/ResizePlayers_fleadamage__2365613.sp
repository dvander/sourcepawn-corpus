...
public OnConfigsExecuted()
{
  ...
  BuildMenus();
  //pull default size from G-Fortress. (Anything smaller than this will be nerfed)  
  if(FindConVar("gfortress_size")!=null && FindConVar("gfortress_manage_resize")!=null) //If it finds the cvars for G-Fortress
  {
	if(GetConVarInt(FindConVar("gfortress_manage_resize"))==1)//If G-Fortress is managing resize
	g_fFleaScale=GetConVarFloat(FindConVar("gfortress_size"));
	else
	g_fFleaScale=1.0;
  }
  else
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
  ...
  {
    if (g_fClientCurrentScale[ResizeType_Generic][attacker] == g_fFleaScale || (g_iDamage == 2 && g_fClientCurrentScale[ResizeType_Generic][attacker] >= g_fFleaScale) || (g_iDamage == 3 && g_fClientCurrentScale[ResizeType_Generic][attacker] <= g_fFleaScale))
    {
      return Plugin_Continue;
    }
    ...
    if (g_bIsTF2 && g_bBackstab && g_bCustomDmgAvailable && victim > 0 && victim <= MaxClients && damagecustom == TF_CUSTOM_BACKSTAB)
    {
      ...
      damage = RoundToCeil(GetMax(iMaxHealth, GetEntProp(victim, Prop_Data, "m_iHealth")) * g_fClientCurrentScale[ResizeType_Generic][attacker]) / 30.0;
      return Plugin_Changed;
    }
	...
	if (g_fClientCurrentScale[ResizeType_Generic][attacker] < g_fFleaScale)
	{
		damage = ((damage * g_fClientCurrentScale[ResizeType_Generic][attacker])/10);
	}
	else
	{

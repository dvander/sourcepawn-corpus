...
public OnPluginStart()
{
	...
	//overrides sv_visiblemaxplayers
...
MVMOverride( )
{
	new Handle:cvar = FindConVar("sv_visiblemaxplayers");

	SetConVarString(cvar, "10");
	PushArrayCell(cvarsArray, _:cvar);
	PushArrayString(valuesArray, "10");
...
stock TurnToRed(client)
{
	...
	for (new i = 0; i < (count - 5); i++)
	{
		if (target[i] != -1) SetEntProp(target[i], Prop_Send, "m_iTeamNum", _:TFTeam_Red);
	}

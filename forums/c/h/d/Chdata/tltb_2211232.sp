...
new String:pNameMenu[MAXLENGTH_NAME];
...
public OnPluginStart()
{
	...
	hTltbNamesArray = CreateArray(MAXLENGTH_NAME, 0);
	hTltbNetidsArray = CreateArray(MAXLENGTH_STEAMAUTH , 0);
	...
	RegAdminCmd("sm_lban", TltbPublicCommand, ADMFLAG_BAN);
	RegAdminCmd("sm_lateban", TltbPublicCommand, ADMFLAG_BAN);
...
public Action:TltbPlayerDisconnect(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (event != INVALID_HANDLE)
	{
		...
		decl String:pNetid[MAXLENGTH_STEAMAUTH ], String:pReason[DISCONNECT_REASON_SIZE], String:pName[MAXLENGTH_NAME];
		...
		if (!tltbCheckReason || (tltbCheckReason && StrEqual(pReason, "Disconnect by user.")))
		{
			...
			else if (strncmp(pNetid, NETID_TO_COMPARE, 3) != 0)
			{
				#if DEBUG
...
stock steam2(String:steam3[])
{
    ...
    Format(steam3, MAXLENGTH_STEAMAUTH , "STEAM_0:%d:%d", m_unMod, (m_unAccountID-m_unMod)/2);
}
...
public Action:TltbPublicCommand(client,args)
{
	...
	if (args == 0)
	{
		...
		else
		{
			...
			if (GetArraySize(hTltbNamesArray) == 0) { ReplyToCommand(client, "[TLTB] No players stored."); }
	...
	else if (args == 1 && GetAdminFlag(GetUserAdmin(client), Admin_Root))
	{
...
Handle:TltbMenu()
{
	...
	if (menu != INVALID_HANDLE)
	{
		...
		decl String:pNetid[MAXLENGTH_STEAMAUTH ], String:pName[MAXLENGTH_NAME];

...
#define PLUGIN_AUTHOR		"FlaminSarge & bottiger & Chdata"
#define PLUGIN_VERSION		"0x02" //as of Aug 27, 2013
...
//		ThrowNativeError(SP_ERROR_NATIVE, "TF2Attrib_SetByName: \"m_AttributeList\" not found (entity %d/%s)", entity, strClassname);
	...
	if (pAttribDef < Address_MinimumValid && pAttribDef > Address:-1)
	{
...
//		ThrowNativeError(SP_ERROR_NATIVE, "TF2Attrib_GetByName: \"m_AttributeList\" not found (entity %d/%s)", entity, strClassname);
	...
	return ((pAttrib < Address_MinimumValid && pAttrib > Address:-1) ? (_:Address_Null) : (_:pAttrib));
}
...
//	if (pAttrib < Address_MinimumValid && pAttrib > Address:-1) return;
	new iDefIndex = GetNativeCell(2);
...
{
	new Address:pAttrib = Address:GetNativeCell(1);
//	if (pAttrib < Address_MinimumValid && pAttrib > Address:-1) return -1;
...
//		ThrowNativeError(SP_ERROR_NATIVE, "TF2Attrib_RemoveAll: \"m_AttributeList\" not found (entity %d/%s)", entity, strClassname);
	...
	if (pAttribList < Address_MinimumValid && pAttribList > Address:-1) return -1;
...
stock bool:ClearAttributeCache(entity)
{
	...
	if (pAttribs < Address_MinimumValid && pAttribs > Address:-1) return false;

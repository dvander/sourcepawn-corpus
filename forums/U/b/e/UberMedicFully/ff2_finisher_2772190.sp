...
int BossTeam = view_as<int>(TFTeam_Blue);	//Is this even needed lol
...
void special_finish(int iBIndex, const char[] ability_name)
{
	...
	ReadSound(iBoss, "special_finisher_heavy", 16, soundFile);
	...
	if(boss >= 0)
	{
		...
		{		
			if(Enabled)
			{
				...
				{	
					if(GetEntProp(client, Prop_Data, "m_iHealth") > 330.0)		// Just in case that the heavy is above 300 health to get oneshotted
					{						
						if(GetEntProp(client, Prop_Data, "m_iHealth") < 330.0)	// If at 330 health OR lower, return and ignore everything below
							return;
						...
						//From sarysa.
						static String:soundFile[MAX_SOUND_FILE_LENGTH];
						ReadSound(attacker, "special_finisher_heavy", 17, soundFile);
							if (strlen(soundFile) > 3)
								EmitSoundToAll(soundFile);
						
						float slowmo = FF2_GetAbilityArgumentFloat(boss, this_plugin_name, "special_finisher_heavy", 15, 0.2);
						...
						float duration = FF2_GetAbilityArgumentFloat(boss, this_plugin_name, "special_finisher_heavy", 13, 1.5);
						if (duration > 0.0)
							TF2_StunPlayer(client, duration, 0.0, TF_STUNFLAGS_GHOSTSCARE|TF_STUNFLAG_NOSOUNDOREFFECT, attacker);
							
						float finish_delay = FF2_GetAbilityArgumentFloat(boss, this_plugin_name, "special_finisher_heavy", 14, 1.5);

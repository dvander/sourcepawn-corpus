...
/**
 * Dynamic Special Point Teleporter
 */
#define SPT_STRING "special_point_teleport"
bool SPT_ActiveThisRound;
bool SPT_CanUse[MAX_PLAYERS_ARRAY];
float SPT_CrouchOrAltFireDownSince[MAX_PLAYERS_ARRAY]; 
bool SPT_EmergencyReady[MAX_PLAYERS_ARRAY]; //Not used.
float SPT_UpdateHUDAt[MAX_PLAYERS_ARRAY];
bool SPT_IsDisabled[MAX_PLAYERS_ARRAY];
int SPT_UsesRemaining[MAX_PLAYERS_ARRAY];
float SPT_OnCooldownUntil[MAX_PLAYERS_ARRAY]; 
float SPT_ChargeTime[MAX_PLAYERS_ARRAY]; 
float SPT_Cooldown[MAX_PLAYERS_ARRAY];
char SPT_TeleportCooldownString[MAX_CENTER_TEXT_LENGTH]; 
char SPT_TeleportReadyString[MAX_CENTER_TEXT_LENGTH];
bool SPT_KeyDown[MAX_PLAYERS_ARRAY];
int SPT_KeyToUse[MAX_PLAYERS_ARRAY];
float SPT_MaxDistance[MAX_PLAYERS_ARRAY];
char SPT_OldLocationParticleEffect[MAX_EFFECT_NAME_LENGTH];
char SPT_NewLocationParticleEffect[MAX_EFFECT_NAME_LENGTH];
bool SPT_PreserveMomentum[MAX_PLAYERS_ARRAY];
bool SPT_EmptyClipOnTeleport[MAX_PLAYERS_ARRAY];
float SPT_AttackDelayOnTeleport[MAX_PLAYERS_ARRAY];
int SPT_Sound[MAX_PLAYERS_ARRAY];
...
public Action Event_RoundStart(Handle event, const char[] name, bool dontBroadcast)
{
	...
	for (int clientIdx = 1; clientIdx < MAX_PLAYERS; clientIdx++)
	{
		...
		if (FF2_HasAbility(bossIdx, this_plugin_name, SPT_STRING))
		{
			SPT_ActiveThisRound = true;
			SPT_CanUse[clientIdx] = true;
			SPT_CrouchOrAltFireDownSince[clientIdx] = FAR_FUTURE;
			SPT_EmergencyReady[clientIdx] = false;
			
			SPT_Cooldown[clientIdx] = FF2_GetAbilityArgumentFloat(bossIdx, this_plugin_name, SPT_STRING, 1);
			SPT_IsDisabled[clientIdx] = FF2_GetAbilityArgument(bossIdx, this_plugin_name, SPT_STRING, 2) == 1;
			SPT_UsesRemaining[clientIdx] = FF2_GetAbilityArgument(bossIdx, this_plugin_name, SPT_STRING, 3);
			SPT_OnCooldownUntil[clientIdx] = GetEngineTime() + FF2_GetAbilityArgumentFloat(bossIdx, this_plugin_name, SPT_STRING, 4);
			int keyId = FF2_GetAbilityArgument(bossIdx, this_plugin_name, SPT_STRING, 5);
			if (keyId == 1)
			...
			SPT_MaxDistance[clientIdx] = FF2_GetAbilityArgumentFloat(bossIdx, this_plugin_name, SPT_STRING, 6);
			FF2_GetAbilityArgumentString(bossIdx, this_plugin_name, SPT_STRING, 7, SPT_OldLocationParticleEffect, MAX_EFFECT_NAME_LENGTH);
			FF2_GetAbilityArgumentString(bossIdx, this_plugin_name, SPT_STRING, 8, SPT_NewLocationParticleEffect, MAX_EFFECT_NAME_LENGTH);
			SPT_PreserveMomentum[clientIdx] = FF2_GetAbilityArgument(bossIdx, this_plugin_name, SPT_STRING, 9) == 1;
			...
			ReadCenterText(bossIdx, SPT_STRING, 18, SPT_TeleportCooldownString);
			ReadCenterText(bossIdx, SPT_STRING, 19, SPT_TeleportReadyString);
			
			if (SPT_UsesRemaining[clientIdx] <= 0)
				SPT_UsesRemaining[clientIdx] = 999999999;
...
int SPT_Player;
public bool SPT_TracePlayersAndBuildings(int entity, int contentsMask)
{
...
public bool SPT_TraceWallsOnly(int entity, int contentsMask)
{
	return false;
}
 ...
 * Dynamic Special Point Teleporter
 */
public void SPT_Tick(int clientIdx, int buttons, float curTime)
{
	if (SPT_IsDisabled[clientIdx] || SPT_UsesRemaining[clientIdx] == 0)
		return;
		
	if (curTime >= SPT_OnCooldownUntil[clientIdx])
		SPT_OnCooldownUntil[clientIdx] = FAR_FUTURE;
		
	float Tele = 0.0;
	if (SPT_OnCooldownUntil[clientIdx] == FAR_FUTURE)
	{
		if (SPT_CrouchOrAltFireDownSince[clientIdx] != FAR_FUTURE)
		{
			if (SPT_KeyToUse[clientIdx] <= 0.0)
				Tele = 0.0;
			else
				Tele = fmin((curTime - SPT_CrouchOrAltFireDownSince[clientIdx]) / SPT_KeyToUse[clientIdx], 1.0) * 0.0;
		}
					
		if (SPT_CrouchOrAltFireDownSince[clientIdx] == FAR_FUTURE && (buttons & SPT_KeyToUse[clientIdx]) != 0)
			SPT_CrouchOrAltFireDownSince[clientIdx] = curTime;
			
		if (SPT_CrouchOrAltFireDownSince[clientIdx] != FAR_FUTURE && (buttons & SPT_KeyToUse[clientIdx]) == 0)
		{
			
			if (!IsInInvalidCondition(clientIdx))
			{
				int bossIdx = FF2_GetBossIndex(clientIdx);
				float Duration = FF2_GetAbilityArgumentFloat(bossIdx, this_plugin_name, SPT_STRING, 12, 3.0);
				MakeAnImage(clientIdx, Duration);
				SPT_OnCooldownUntil[clientIdx] = curTime + SPT_Cooldown[clientIdx];
				SPT_TryTeleport(clientIdx);
				static float position[3];
				GetEntPropVector(clientIdx, Prop_Send, "m_vecOrigin", position);

				static char sound[PLATFORM_MAX_PATH];
				if(FF2_RandomSound("sound_point_teleport", sound, PLATFORM_MAX_PATH, bossIdx, SPT_Sound[clientIdx]))
				{
	...
	if (curTime >= SPT_UpdateHUDAt[clientIdx])
	{
		if (!(FF2_GetFF2flags(clientIdx) & FF2FLAG_HUDDISABLED) || DD_BypassHUDRestrictions[clientIdx])
		{
			if (SPT_OnCooldownUntil[clientIdx] == FAR_FUTURE)
			{
				SetHudTextParams(-1.0, 0.94, HUD_INTERVAL + HUD_LINGER, HUD_R_OK, HUD_G_OK, HUD_B_OK, HUD_ALPHA);
				ShowSyncHudText(clientIdx, DD_HUDHandle, SPT_TeleportReadyString, Tele);
			}
			else
			{
				SetHudTextParams(-1.0, 0.94, HUD_INTERVAL + HUD_LINGER, HUD_R_ERROR, HUD_G_ERROR, HUD_B_ERROR, HUD_ALPHA);
				ShowSyncHudText(clientIdx, DD_HUDHandle, SPT_TeleportCooldownString, SPT_OnCooldownUntil[clientIdx] - curTime);
			}
		}
		
		SPT_UpdateHUDAt[clientIdx] = curTime + HUD_INTERVAL;
	}
}

public bool SPT_TryTeleport(int clientIdx)
{	
...
//LeAlex14's after image, thanks for letting me use it for this
public void MakeAnImage(clientIdx, float Duration)
{
	float clientPos[3] = 0.0;
	float clientAngles[3] = 0.0;
	float clientVel[3] = 0.0;
	GetClientAbsOrigin(clientIdx, clientPos);
	GetEntPropVector(clientIdx, Prop_Send, "m_angRotation", clientAngles);
	GetEntPropVector(clientIdx, Prop_Data, "m_vecVelocity", clientVel);
	...
	{
		char model[256];
		GetClientModel(clientIdx, model, 256);
		...
		SetEntPropEnt(animationentity, PropType:0, "m_hOwnerEntity", clientIdx, 0);
		if (GetEntProp(clientIdx, PropType:0, "m_iTeamNum", 4, 0))
		{
			SetEntProp(animationentity, PropType:0, "m_nSkin", GetClientTeam(clientIdx) + -2, 4, 0);
		}
		else
		{
			SetEntProp(animationentity, PropType:0, "m_nSkin", GetEntProp(clientIdx, PropType:0, "m_nForcedSkin", 4, 0), 4, 0);
		}
		SetEntProp(animationentity, PropType:0, "m_nSequence", GetEntProp(clientIdx, PropType:0, "m_nSequence", 4, 0), 4, 0);
		SetEntPropFloat(animationentity, PropType:0, "m_flPlaybackRate", GetEntPropFloat(clientIdx, PropType:0, "m_flPlaybackRate", 0), 0);
		DispatchKeyValue(clientIdx, "disableshadows", "1");
...
public void DP_Latch(int clientIdx, float curTime)
{
	...
	DD_SetDisabled(clientIdx, true, true, true, true, true);
...
public void DP_Unlatch(int clientIdx, float curTime)
{
	...
	DD_SetDisabled(clientIdx, false, false, false, false, false);
	SetEntityGravity(clientIdx, 1.0);
 ...
 * Exposed Interfaces
 */
public void DD_SetDisabled(int clientIdx, bool superjump, bool specialpointteleport, bool teleport, bool weighdown, bool glide)
{
	// super jump
	...
	// SPT
	if (SPT_CanUse[clientIdx])
	{
		SPT_IsDisabled[clientIdx] = specialpointteleport;
		if (SPT_IsDisabled[clientIdx])
			SPT_CrouchOrAltFireDownSince[clientIdx] = FAR_FUTURE;
	}
...
public float DD_GetMobilityCooldown(int clientIdx)
{
	...
	if (SPT_CanUse[clientIdx] && !SPT_IsDisabled[clientIdx])
		return (SPT_OnCooldownUntil[clientIdx] == FAR_FUTURE) ? 0.0 : (SPT_OnCooldownUntil[clientIdx] - GetEngineTime());
...
public float DD_GetChargePercent(int clientIdx)
{
	...
	if (SPT_CanUse[clientIdx] && !SPT_IsDisabled[clientIdx])
	{
		if (SPT_ChargeTime[clientIdx] <= 0.0)
			return 100.0;
		else if (SPT_CrouchOrAltFireDownSince[clientIdx] == FAR_FUTURE)
			return 0.0;
		return (SPT_OnCooldownUntil[clientIdx] != FAR_FUTURE) ? 0.0 : (fmin((GetEngineTime() - SPT_CrouchOrAltFireDownSince[clientIdx]) / SPT_ChargeTime[clientIdx], 1.0) * 100.0);
	}
	else if (DT_CanUse[clientIdx] && !DT_IsDisabled[clientIdx])
	{
		if (DT_ChargeTime[clientIdx] <= 0.0)
...
public void SPT_SetUsesRemaining(int clientIdx, int usesRemaining)
{
	if (SPT_CanUse[clientIdx])
		SPT_UsesRemaining[clientIdx] = usesRemaining;
}

public void SPT_CooldownUntil(int clientIdx, float cooldownEndTime)
{
	if (SPT_CanUse[clientIdx])
	{
		if (SPT_OnCooldownUntil[clientIdx] == FAR_FUTURE || cooldownEndTime > SPT_OnCooldownUntil[clientIdx])
		{
			SPT_OnCooldownUntil[clientIdx] = cooldownEndTime;
			SPT_EmergencyReady[clientIdx] = false;
		}
	}
}

public void SPT_AdjustCooldownTimer(int clientIdx, float offset)
{
	if (SPT_CanUse[clientIdx] && SPT_OnCooldownUntil[clientIdx] != FAR_FUTURE)
		SPT_OnCooldownUntil[clientIdx] += offset;
}

// if you don't want to change a particular stat, set it to -1
public void SPT_ChangeFundamentalStats(int clientIdx, float chargeTime, float cooldown, float multiplier)
{
	if (chargeTime != -1.0)
		SPT_ChargeTime[clientIdx] = chargeTime;
	if (cooldown != -1.0)
		SPT_Cooldown[clientIdx] = cooldown;
}
...
public Action OnPlayerRunCmd(int clientIdx, int &buttons, int &impulse, float vel[3], float unusedangles[3], int &weapon)
{
	...
	{
		SPT_Tick(clientIdx, buttons, GetEngineTime());
	}

//===VS Saxton Hale Mode===
//By Rainbolt Dash (formerly Dr.Eggman):
//Programmer, model-maker, mapper.
//Thanks Flamming Sarge and Unreal1 for all the support
...
#define HaleKSpree2 "saxton_hale/saxton_hale_responce_4.wav"	//this line is broken and unused   Unreal1: So why is it still here?
...
public OnPluginStart()
{
	...
	Steam_SetGameDescription("VS Saxton Hale");
...
bool:CheckNextSpecial()
{
	...
	while (Incoming == -1 || (Special && Special == Incoming))
	{
		...
		else
		{
			...
			if (IsDecemberHoliday() && !GetRandomInt(0, 7)) Incoming = 4;
		}
	}
	Special = Incoming;
	Incoming = -1;
...
public Action:Timer_SkipHalePanel(Handle:hTimer)
{
	...
	do
	{
		...
		if (IsValidClient(client) && client != Hale)
		{
			...
			{
				CPrintToChat(client, "{olive}[VSH]{default} %t", "to0_near");
...
public SkipHalePanelNotify(client)
{
	...
	decl String:s[256];
	SetPanelTitle(panel, "[VSH] You're Hale soon!");
	Format(s, sizeof(s), "%t", "to0_near");
...
{
	if (Enabled && client == Hale && Special == 2)
	{
			...
			if (Special == 2 && (HaleCharge == 50 || bEnableSuperDuperJump))
			{
				...
				if (IsValidClient(target))
				{
					...
					if (GetEntProp(target, Prop_Send, "m_bDucked"))
					{
						...
						SetEntityFlags(Hale, FL_DUCKING);
	...
	if (HaleRage/RageDMG >= 1)
	{
		...
		switch (Special)
		{
			...
			case 4:
			{
				...
				EmitSoundToAll(s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, pos, NULL_VECTOR, false, 0.0);
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
				SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SpawnWeapon(client, "tf_weapon_compound_bow", 56, 100, 5, "6 ; 0.5 ; 37 ; 0.0"));
	...
	for (i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i) && IsPlayerAlive(i) && (i != Hale))
		{
			...
			if (!TF2_IsPlayerInCondition(i, TFCond_Ubercharged) && distance < dist)
			{
				...
				if (Special != 2) flags |= TF_STUNFLAG_NOSOUNDOREFFECT;
				TF2_StunPlayer(i, 5.0, _, flags, (Special == 2 ? 0 : Hale));
				if (Special != 2) CreateTimer(5.0, RemoveEnt, EntIndexToEntRef(AttachParticle(i, "yikes_fx", 75.0)));
			}
		}
	}
	i = -1;
...
public Action:event_jarate(UserMsg:msg_id, Handle:bf, const players[], playersNum, bool:reliable, bool:init)
{
	...
	if (jar != -1 && GetEntProp(jar, Prop_Send, "m_iItemDefinitionIndex") == 58 && GetEntProp(jar, Prop_Send, "m_iEntityLevel") != -122)	//-122 is the Jar of Ants and should not be used in this
	{
		...
		if (HaleRage < 0)
			HaleRage = 0;
		if (Special == 1 && TF2_IsPlayerInCondition(victim, TFCond_Ubercharged) && UberRageCount < 99)
		{
			UberRageCount += 7.0;
			if (UberRageCount > 99) UberRageCount = 99.0;
		}
		new ammo = GetAmmo(Hale, 0);
		if (Special == 4 && ammo > 0) SetAmmo(Hale, 0, ammo - 1);
	}
	return Plugin_Continue;
}

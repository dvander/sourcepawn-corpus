...
{
		RegAdminCmd("sm_futex", CmdFuture, ADMFLAG_ROOT);
		CreateConVar("sm_futureexe_version", PLUGIN_VERSION, "Plugin Version", FCVAR_REPLICATED|FCVAR_NOTIFY);
}
...
public Action:CmdFuture(client, args)
{
	if (args != 2)
	{
		ReplyToCommand(client, "[SM] Usage: sm_futex <Time in minutes> \"Command +Args\"");

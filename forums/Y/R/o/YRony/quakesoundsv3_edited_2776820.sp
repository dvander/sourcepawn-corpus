...
#define PLUGIN_NAME			"Quake Sounds v3 - EDITED"
...
new Handle:cookieHSSoundPref
new HSSoundPreference[MAXPLAYERS+1]
...
public OnPluginStart()
{
	...
	CreateConVar("sm_quakesoundsv3_HSSound", "0", "Default sound set for new users, 0=Disabled/Default 1=Enabled.",FCVAR_NONE,true,0.0) 							/* yrony */
	...
	cookieHSSoundPref = RegClientCookie("Quake HS Pref","headshot setting",CookieAccess_Private)		/* yrony */
...
public LateLoadedInitialization()
{
	...
	for(new i = 1; i <= GetMaxHumanPlayers(); i++) 
	{
		if(IsClientInGame(i) && !IsFakeClient(i))
		{
			...
			HSSoundPreference[i] = GetConVarInt(FindConVar("sm_quakesoundsv3_HSSound")) - 5		/* yrony */
...
// Things to do when the round_end
...
public LoadClientCookiesFor(client)
{
	...
	GetClientCookie(client,cookieHSSoundPref,buffer,5)
	if(!StrEqual(buffer,""))												/* yrony (line 657-661)*/
	{
		HSSoundPreference[client] = StringToInt(buffer)
	}														
}
						...
						if(textPreference[i])
						{
							...
							else if((firstbloodConfig[soundPreference[i]] & 32) && victimClient == i)
							{
								PrintCenterText(i,"%t","first blood",attackerName)
					                         ...
					                         /* (yrony) If they dont want HS sound > disable. Added */
					else if(headshot && headshotConfig[soundPreference[i]][0] > 0 && HSSoundPreference[i] != -5) /* (yrony) */
					{
...
public ShowQuakeMenu(client)
{
	...
	if(HSSoundPreference[client] == -5)
	{
		Format(buffer,100,"%T","enable headshot sounds",client)
	}
	else																					/* yrony (1156-1165)*/
	{
		Format(buffer,100,"%T","disable headshot sounds",client)
	}
	AddMenuItem(menu,"headshot setting",buffer)						
...
public MenuHandlerQuake(Handle:menu,MenuAction:action,param1,param2)
{
	if(action == MenuAction_Select)	
	{
		...
		{
			HSSoundPreference[param1] = -5													/* yrony 1194-1201 */
		else
		{
			HSSoundPreference[param1] = -4
		}											*/
		new String:buffer[5]
		...
		IntToString(HSSoundPreference[param1],buffer,5)			/* yrony */
		SetClientCookie(param1,cookieHSSoundPref,buffer)		/* yrony */

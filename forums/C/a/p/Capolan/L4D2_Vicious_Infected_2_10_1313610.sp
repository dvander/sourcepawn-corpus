...
new Handle:cvarBacterialDamage;
...
new Handle:cvarCullingSwarmChance;
new Handle:cvarMourningWidow;
new Handle:cvarMourningWidowChance;
new Handle:cvarWorriedWife;
new Handle:cvarWorriedWifeChance;
...
public OnPluginStart()
{
	...
	cvarBileMaskAmount = CreateConVar("l4d_vts_bilemaskamount", "230", "Amount of visibility covered by the Boomer's bile (0 = None, 255 = Total). (Def 200)", FCVAR_PLUGIN, true, 0.0, false, _);
	...
	cvarBileSwipeChance = CreateConVar("l4d_vts_bileswipechance", "30", "[0.0, 100.0]%", FCVAR_PLUGIN);
	...
	cvarBrokenRibsChance = CreateConVar("l4d_vts_brokenribschance", "30", "[0.0, 100.0]%", FCVAR_PLUGIN);
	...
	cvarSnappedLegChance = CreateConVar("l4d_vts_snappedlegchance", "30", "[0.0, 100.0]%", FCVAR_PLUGIN);
	cvarSnappedLegDuration = CreateConVar("l4d_vts_snappedlegduration", "30", "For how many seconds will the Snapped Leg reduce movement speed. (Def 5)", FCVAR_PLUGIN, true, 0.0, false, _);
	...
	cvarStowawayDamage = CreateConVar("l4d_vts_stowawaydamage", "2", "How much damage is inflicted by Stowaway for each second carried. (Def 5)", FCVAR_PLUGIN, true, 0.0, false, _);
	...
	cvarBacterialChance = CreateConVar("l4d_vts_bacterialchance", "30", "[0.0, 100.0]%", FCVAR_PLUGIN);
	cvarBacterialDamage = CreateConVar("l4d_vts_bacterialdamage", "1", "How much damage is inflicted by Bacterial Infection each second. (Def 1)", FCVAR_PLUGIN, true, 0.0, false, _);
	...
	cvarDerbyDazeAmount = CreateConVar("l4d_vts_derbydazeamount", "240", "Amount of visibility covered by the Jockey's hand (0 = None, 255 = Total). (Def 200)", FCVAR_PLUGIN, true, 0.0, false, _);
	...
	cvarCobraStrike = CreateConVar("l4d_vts_cobrastrike", "1", "Hunter pounce has a chance of instantly incapping and making Survivor B&W. (Def 0)", FCVAR_PLUGIN, true, 0.0, false, _);
	...
	cvarDeepWoundsChance = CreateConVar("l4d_vts_deepwoundschance", "30", "[0.0, 100.0]%", FCVAR_PLUGIN);
	...
	cvarCollapsedLungChance = CreateConVar("l4d_vts_collapsedlungchance", "30", "[0.0, 100.0]%", FCVAR_PLUGIN);
	...
	cvarAcidSwipeChance = CreateConVar("l4d_vts_acidswipechance", "30", "[0.0, 100.0]%", FCVAR_PLUGIN);
	...
	cvarSupergirlDuration = CreateConVar("l4d_vts_supergirlduration", "3", "How long the Spitter is invulnerable. (Def 3)", FCVAR_PLUGIN, true, 1.0, false, _);
	...
	cvarTankMax = CreateConVar("l4d_vts_tankmax", "0", "Maximum number of Tanks (Mini Bodyguard) allowed to spawn with this plugin at a time. (Def 1)", FCVAR_PLUGIN, true, 0.0, false, _);
	cvarTankMaxMap = CreateConVar("l4d_vts_tankmaxmap", "0", "Maximum number of Tanks (Mini Bodyguard) allowed to spawn throughout an entire map. (Def 3)", FCVAR_PLUGIN, true, 0.0, false, _);
	...
	cvarCullingSwarmChance = CreateConVar("l4d_vts_cullingswarmchance", "10", "[0.0, 100.0]%", FCVAR_PLUGIN);
	cvarMourningWidow = CreateConVar("l4d_vts_mourningwidow", "0", "Spawns a Witch after the Tank dies. (Def 1)", FCVAR_PLUGIN, true, 0.0, false, _);
	cvarMourningWidowChance = CreateConVar("l4d_vts_mourningwidowchance", "10", "[0.0, 100.0]%", FCVAR_PLUGIN);
    ...
    cvarWorriedWifeChance = CreateConVar("l4d_vts_worriedwifechance", "10", "[0.0, 100.0]%", FCVAR_PLUGIN);
	...
	cvarMiniBodyguard = CreateConVar("l4d_vts_minibodyguard", "0", "Spawns a Tank when the Witch is killed. (Def 1)", FCVAR_PLUGIN, true, 0.0, false, _);
	cvarMiniBodyguardChance = CreateConVar("l4d_vts_minibodyguardchance", "10", "[0.0, 100.0]%", FCVAR_PLUGIN);
    ...
    cvarSupportGroupChance = CreateConVar("l4d_vts_supportgroupchance", "10", "[0.0, 100.0]%", FCVAR_PLUGIN); 
...
public Event_PlayerSpawn (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidClient(client) && GetClientTeam(client) == 3)
	{
		...
		if (class == ZOMBIECLASS_BOOMER)
		{
			...
			{
			CreateTimer(0.5, Event_BoomerBileFeet, client);
			}
		}
	}
}
...
//This is the Hook for Acid Swipe - Survivor takes damage over time after being Spitter clawed
	...
	if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarAcidSwipeChance))
	{
		if (IsValidClient(client) && GetClientTeam(client) == 2 && StrEqual(weapon, "spitter_claw"))
		{
			PrintHintText(client, "The Spitter has coated you with corrosive acid - OH NOES!");
			if(acidswipe[client] <= 0)
				{
				acidswipe[client] = (GetConVarInt(cvarAcidSwipeDuration));
				CreateTimer(1.0, Timer_AcidSwipe, client);
				}
		}
	}
...
public Event_PlayerNowIt (Handle:event, const String:name[], bool:dontBroadcast)
{
	if (isBoomer && isBileShower && !isBileShowerTimeout)
	{
		...
		CreateTimer(GetConVarFloat(cvarBileShowerTimeout), BileShowerTimeout);
	...
	if (isBoomer && isBileMask)
	{
		...
		{	
			...
			if (!isBileMaskTilDry)
			{
			CreateTimer(GetConVarFloat(cvarBileMaskDuration), Timer_BileMask, client);
			}
		}
	}
}
...
public Event_ChargerPummelEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
	new client = GetClientOfUserId(GetEventInt(event,"victim"));
	if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarBrokenRibsChance))

	if (IsValidClient(client) && GetClientTeam(client) == 2)
		{
		PrintHintText(client, "The Charger bruised your ribs!");
		if(brokenribs[client] <= 0)
			{
			brokenribs[client] = (GetConVarInt(cvarBrokenRibsDuration));
			CreateTimer(1.0, Timer_BrokenRibs, client);
			}
		}
	}	
}
...
public Event_ChargerImpact (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
	new client = GetClientOfUserId(GetEventInt(event,"victim"));
    if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarSnappedLegChance))
	
	if (IsValidClient(client) && GetClientTeam(client) == 2 && !isSlowed[client])
		{
		isSlowed[client] = true;
		PrintHintText(client, "The Charger's impact has hurt your leg.");
		SetEntDataFloat(client, laggedMovementOffset, GetConVarFloat(cvarSnappedLegSpeed), true);
		CreateTimer(GetConVarFloat(cvarSnappedLegDuration), SnappedLeg, client);
		}
	}
}	
...
public Event_ChargerCarryStart (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidClient(client) && GetClientTeam(client) == 2)
		...
		CreateTimer(0.5, Timer_Stowaway, client);
		}
	}	
}
...
public Event_ChargerCarryEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidClient(client) && GetClientTeam(client) == 2)
	{
		...
		CreateTimer(0.1, Timer_StowawayDamage, client);
		new damage = GetConVarInt(cvarStowawayDamage);
		new count = (stowaway[client]*damage);
		PrintHintText(client, "The Charger dragged you and inflicted %i damage!", count);
...
public Event_JockeyRideEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarBacterialChance))
	{
	new client = GetClientOfUserId(GetEventInt(event,"victim"));

	if (IsValidClient(client) && GetClientTeam(client) == 2)
		{
		PrintHintText(client, "The Jockey has caused an infection - get ye to the Baron!");
		if(bacterial[client] <= 0)
			{
			bacterial[client] = (GetConVarInt(cvarBacterialDuration));
			CreateTimer(1.0, Timer_Bacterial, client);
			}
		}
	}
...
public Event_PounceEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarDeepWoundsChance))
	{
	new client = GetClientOfUserId(GetEventInt(event,"victim"));

	if (IsValidClient(client) && GetClientTeam(client) == 2)
		{
		PrintHintText(client, "The Hunter has cut into your chest and stomach, you're bleeding out.");
		if(deepwounds[client] <= 0)
			{
			deepwounds[client] = (GetConVarInt(cvarDeepWoundsDuration));
			CreateTimer(1.0, Timer_DeepWounds, client);
			}
		}
	}	
}
...
public Event_ChokeEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarCollapsedLungChance))
	{
	new client = GetClientOfUserId(GetEventInt(event,"victim"));

	if (IsValidClient(client) && GetClientTeam(client) == 2)
		{
		PrintHintText(client, "The Smoker has damaged your throat, you are coughing up blood!");
		if(collapsedlung[client] <= 0)
			{
			collapsedlung[client] = (GetConVarInt(cvarCollapsedLungDuration));
			CreateTimer(1.0, Timer_CollapsedLung, client);
			}
		}
	}
}
	...
	if (distance > MoonWalkStretch)
		{
			...
			PrintHintText(client, "The Tongue has been stretched too far and snapped!");
		}
...
public Event_EnteredSpit (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidClient(client) && GetClientTeam(client) == 2 && !isSlowed[client])
		...
		CreateTimer(GetConVarFloat(cvarStickyGooDuration), StickyGoo, client);
...
public Event_SpitBurst (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidClient(client))
		...
		CreateTimer(GetConVarFloat(cvarSupergirlDuration), Supergirl, client);	
...
public Action:Timer_BurningRage(Handle:timer, any:client)
{
	...
	if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarCullingSwarmChance))
	{
		...
		if (isAnnounce) PrintToChatAll("[SM] The Tank is on fire and he is pissed - RUN!");
	}
...
public Action:Event_TankSpawned(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (isMiniTankSpawn && isMiniHealth)
	{
		...
		CreateTimer(3.0, Timer_MiniBodyguard, client);
	...
	if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarCullingSwarmChance))
	{
		...
		SetCommandFlags("z_spawn", flags3|FCVAR_CHEAT);
		SetCommandFlags("director_force_panic_event", flags4|FCVAR_CHEAT);
		if (isAnnounce) PrintToChatAll("[SM] The Tank has summoned a horde!"); 
	}

	if (isTank && isWorriedWife && !isMiniTank[client])
	if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarWorriedWifeChance))
	{
...
public Action:Event_TankKilled(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarMourningWidowChance))
	{
	...
	{
   		CloseHandle(BurningRageTimer[client]);
...
public Action:Event_WitchKilled(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarMiniBodyguardChance))
	{
		...
		new flags2 = GetCommandFlags("z_spawn");
		SetCommandFlags("z_spawn", flags2 & ~FCVAR_CHEAT);
		FakeClientCommand(client, "%s %s", "z_spawn", "tank auto");  
...
public Action:Event_WitchAngered(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetRandomFloat(0.0, 100.0)<GetConVarFloat(cvarSupportGroupChance))
	{
		new client = GetClientOfUserId(GetEventInt(event,"userid"));
		new flags = GetCommandFlags("z_spawn");
		SetCommandFlags("z_spawn", flags & ~FCVAR_CHEAT);
		FakeClientCommand(client,"z_spawn mob auto");
		SetCommandFlags("z_spawn", flags|FCVAR_CHEAT);
		if (isAnnounce) PrintToChatAll("[SM] The Witch's screams have alerted the area horde!"); 
	}
}

/*******************************************************************************
  ...
  1.5.1 Fixed "[SM] Native "GetClientTeam" reported: Client index 0 is invalid" Errors (By Caps Lock Fuck Yeah)
  ...
  Cvars:
	...
	sm_damage_ff		"0"	- 0: disables extra damage when hitting teammates - 1: enables extra damage when hitting teammates
...
new Handle:g_ff = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_ff = CreateConVar("sm_damage_ff","0");
...
public OnEventShutdown()
{
	...
	UnhookEvent("infected_hurt",InfectedHurt);
}
...
public Action:InfectedHurt(Handle:event, const String:name[], bool:dontBroadcast){
	new client;
	new attacker;
	new Float:multiplier;
	new armor;
	...
	{
	attacker = GetClientOfUserId(GetEventInt(event, "attacker"));
	client = GetEventInt(event, "entityid");
	...
	if(GetEntProp(client, Prop_Data, "m_iHealth") > 0){
		healthhitmulti = RoundToNearest(GetEventInt(event,"amount")*multiplier);
		healthfinal = GetEntProp(client, Prop_Data, "m_iHealth") - healthhitmulti;
		
		if( healthfinal >= 0 ){
			SetEntProp(client, Prop_Data, "m_iHealth", healthfinal);
		}
		else{

...
{				
	...
	RegAdminCmd("hale_special0", Command_THale, ADMFLAG_CHEATS, "Call Vagineer to next round.");
...
public Action:event_round_start(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidEdict(ionplay) && IsClientInGame(ionplay) && (GetClientTeam(ionplay)>1)) 
	{
		if (IsFakeClient(ionplay))
		{
			...
			GetClientName(ionplay, clientname, sizeof(clientname));
			if (strcmp("replay", clientname) == 0)
			{
				ChangeClientTeam(ionplay, 0);
				continue;
			}
		}
	...
	for (ionplay=1;ionplay<=MaxClients;ionplay++)
		if (bonplay[ionplay] && (ionplay!=Hale))
...
public Action:MakeNoHale(Handle:hTimer,any:clientid)
{
	...
	if ((client<=0) || !IsValidEdict(client) || !IsClientInGame(client) || (RoundState==2) || IsFakeClient(client) || (client==Hale))
		return Plugin_Continue;
	...
	if (IsValidEdict(weapon) && (weapon>0))
	{
		...
		else if ((index==29) || (index==211))
		{
			...
			SetEntPropFloat(weapon, Prop_Send, "m_flChargeLevel",GetEntPropFloat(weapon, Prop_Send, "m_flChargeLevel")+0.41);
		}
	}
...
public Action:UseBowRage(Handle:hTimer,any:clientid)
{
	...
	TeleportEntity(Hale, NULL_VECTOR, NULL_VECTOR, vel);	
	UberRageCount=9.0;
	SetEntData(Hale, FindDataMapOffs(Hale, "m_iAmmo")+4, RoundFloat(UberRageCount));
	return Plugin_Continue;
}
...
{		
	if (!Enabled || ((ent!=Hale) && (IsClientInGame(Hale) && (ent!=GetPlayerWeaponSlot(Hale, 0)))))
		return Plugin_Continue;

...
public OnPluginStart() 
{
	...
	acid_version = CreateConVar("sm_acidswipe_version", PLUGIN_VERSION, "Acid Swipe plugin version.", FCVAR_NOTIFY);
	CreateConVar("sm_acidswipe_duration", "10", "Acid damage duration in seconds", FCVAR_NOTIFY|FCVAR_REPLICATED);
	CreateConVar("sm_acidswipe_damage", "1", "Acid damage per second", FCVAR_NOTIFY|FCVAR_REPLICATED);
	CreateConVar("sm_acidswipe_stack", "10", "Amount of time to add to timer on each additional swipe. -1 Each swipe resets timer, 0 Multiple swipes are ignored.", FCVAR_NOTIFY|FCVAR_REPLICATED, true, -1.0);
...
public Action:Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast) 
{
	...
	if(IsClientInGame(target) && IsPlayerAlive(target) && (GetClientTeam(target) == 2) && (StrEqual(weapon, "spitter_claw")))
	{
		PrintHintText(target, "The Spitter has coated you with corrosive acid!");
...
public Action:DamageEffect(target) 
{
	if(IsClientInGame(target) && IsPlayerAlive(target) && (GetClientTeam(target) == 2))
	{
		...
		AcceptEntityInput(pointHurt, "Hurt", (attacker[target] > 0) ? attacker[target] : -1);

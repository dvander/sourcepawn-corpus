...
new Handle:cvarAdminsOnly = INVALID_HANDLE;
new Handle:cvarTeam = INVALID_HANDLE;
new Handle:cvarScout = INVALID_HANDLE;
new Handle:cvarSoldier = INVALID_HANDLE;
new Handle:cvarPyro = INVALID_HANDLE;
new Handle:cvarDemoman = INVALID_HANDLE;
new Handle:cvarHeavy = INVALID_HANDLE;
new Handle:cvarEngineer = INVALID_HANDLE;
new Handle:cvarMedic = INVALID_HANDLE;
new Handle:cvarSniper = INVALID_HANDLE;
new Handle:cvarSpy = INVALID_HANDLE;
...
public OnPluginStart() {
	...
	cvarTeam		=		CreateConVar("ccs_team", "1", "What team should set speed enabled for? 0 - None, 1 - Both, 2 - Red, 3 - Blue");
...
SetClientSpeed(client) {
	new cVal = GetConVarInt(cvarTeam);
	if(cVal == 0) return;
	else if(cVal == 1 || cVal == GetClientTeam(client)) {

...
new Handle:cvarLogs = INVALID_HANDLE;
new Handle:cvarJoin = INVALID_HANDLE;
new Handle:cvarJoinSize = INVALID_HANDLE;
...
public Action:ResizeMyHead(client, args)
{
	...
	if(args != 1)
	{
		Menu = CreateMenu(HeadHandler);
		SetMenuTitle(Menu, "Resize Head Menu");
		AddMenuItem(Menu, "1", "Tiny");
		AddMenuItem(Menu, "2", "Small");
		AddMenuItem(Menu, "3", "Regular");
		AddMenuItem(Menu, "4", "Large");
		AddMenuItem(Menu, "5", "Giant");
	...
	PrintToChat(client, "[SM] Head size set to %0.0f.", scale);
	return Plugin_Handled;
}

public HeadHandler(Handle:menu, MenuAction:action, param1, param2)
{
	if(action == MenuAction_Select)
	{
		switch (param2)
		{
			case 0:
			{
				g_HeadScale[param1] = 0.3;
			}
			case 1:
			{
				g_HeadScale[param1] = 0.7;
			}
			case 2:
			{
				g_HeadScale[param1] = 1.0;
			}
			case 3:
			{
				g_HeadScale[param1] = 3.0;
			}
			case 4:
			{
				g_HeadScale[param1] = 5.0;
			}
		}
		PrintToChat(param1, "[SM] Head size set to %0.0f.", g_HeadScale[param1]);
	}

...
{
	name = "Deathrun Manager Mod by Chanz",
	author = "Rogue & Chanz",
	description = "Manages terrorists/counter-terrorists on DR servers",
	version = PLUGIN_VERSION,
	url = "http://www.mannisfunhouse.eu/ OR http://www.surf-infamous.com/"
};
...
public OnPluginStart()
{
	...
	RegConsoleCmd("coverme", BlockRadio);
	RegConsoleCmd("takepoint", BlockRadio);
	RegConsoleCmd("holdpos", BlockRadio);
	RegConsoleCmd("regroup", BlockRadio);
	RegConsoleCmd("followme", BlockRadio);
	RegConsoleCmd("takingfire", BlockRadio);
	RegConsoleCmd("go", BlockRadio);
	RegConsoleCmd("fallback", BlockRadio);
	RegConsoleCmd("sticktog", BlockRadio);
	RegConsoleCmd("getinpos", BlockRadio);
	RegConsoleCmd("stormfront", BlockRadio);
	RegConsoleCmd("report", BlockRadio);
	RegConsoleCmd("roger", BlockRadio);
	RegConsoleCmd("enemyspot", BlockRadio);
	RegConsoleCmd("needbackup", BlockRadio);
	RegConsoleCmd("sectorclear", BlockRadio);
	RegConsoleCmd("inposition", BlockRadio);
	RegConsoleCmd("reportingin", BlockRadio);
	RegConsoleCmd("getout", BlockRadio);
	RegConsoleCmd("negative", BlockRadio);
	RegConsoleCmd("enemydown", BlockRadio);
	...
	deathrun_manager_version = CreateConVar("deathrun_manager_version", PLUGIN_VERSION, "Deathrun Manager version; not changeable", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
{
	if (GetConVarInt(deathrun_fall_damage) == 1)
	{
...
public Action:BlockRadio(client, args)
{

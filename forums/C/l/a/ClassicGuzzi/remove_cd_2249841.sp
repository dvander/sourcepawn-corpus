...
public Action:OnPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(isSuddenDeath)
	{
		...
		if(!IsClientInGame(last_player) || !IsPlayerAlive(last_player))
			return;
...
public Action:OnPlayerInventory(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(isSuddenDeath)
	{
		...
		if(GetAlivePlayersCount(client_team) > 1)
			return;
			
		new last_player = GetLastPlayer(client_team);
		if(client != last_player)
			return;

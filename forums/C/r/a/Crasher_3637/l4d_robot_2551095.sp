...
//weapon data
new Float:fireinterval[WEAPONCOUNT]={0.25, 0.068, 0.30, 0.65, 0.060, 0.20, 0.33, 0.145, 0.14, 0.14, 0.068, 0.65, 0.30, 0.265, 0.9, 1.25, 0.065, 0.055};

new Float:bulletaccuracy[WEAPONCOUNT]={1.15, 1.4, 3.5, 3.5, 1.6, 1.7, 1.7, 1.5, 1.6, 1.5, 1.5, 3.5, 3.5, 1.15, 1.00, 0.8, 1.6, 1.6};

new Float:weaponbulletdamage[WEAPONCOUNT]={90.0, 30.0, 25.0, 30.0, 20.0, 30.0, 60.0, 70.0, 40.0, 40.0, 50.0, 30.0, 30.0, 90.0, 100.0, 150.0, 35.0, 35.0};

new weaponclipsize[WEAPONCOUNT]={15, 50, 10, 8, 50, 30, 8, 40, 20, 50, 150, 8, 10, 30, 15, 20, 50, 50};

new weaponbulletpershot[WEAPONCOUNT]={1, 1, 7, 7, 1, 1, 1, 1, 1, 1, 1, 7, 7,1, 1, 1,1,1};

new Float:weaponloadtime[WEAPONCOUNT]={2.0, 1.5, 0.3, 0.3, 1.5, 1.5, 1.9, 1.5, 1.5, 1.6, 0.0, 0.3, 0.3, 2.0, 2.0, 2.0, 1.5, 1.5};

new weaponloadcount[WEAPONCOUNT]={15, 50, 1, 1, 50, 30, 8, 40, 60, 50, 150, 1, 1, 30, 15, 20, 50, 50};

new bool:weaponloaddisrupt[WEAPONCOUNT]={false, false, true, true, false, false, false, false, false, true, false, true, false, false, false, false, false};
//weapon data
...
{  
	...
	{
		PrintToChat(client, "No more robots available");
		return Plugin_Handled;
	}
...
AddRobot(client, bool:showmsg=false)
{
	...
	if(showmsg)
	{
		...
		PrintToChatAll("\x04 %N \x03 turned on a robot", client);
	}
	gamestart=true;
}
...
Do(client, Float:currenttime, Float:duration)
{
	if(robot[client]>0)
	{
		...
		{			
			...
			{
				Release(client);
				PrintHintText(client, "Your robot's energy has been drained");
				return;
			}
			...
			if(reloading[client])
			{
				//PrintToChatAll("%f", Reload time[client]);
			...
			else 
			{
				...
				if(!targetok && currenttime-firetime[client]>4.0 && currenttime-walktime[client]>1.0 )robotangle[client][1]+=5.0;
				TeleportEntity(robot[client], NULL_VECTOR,  robotangle[client] ,robotvec);
			}

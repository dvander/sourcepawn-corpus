...
new Handle:infectionTimer[MAX_PLAYERS+1];
...
public OnPluginStart() 
{
	...
	cvInfectEnabled = CreateConVar("tf2c_infectenabled", "1.0", "enables infection", FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvInfectHits = CreateConVar("tf2c_infecthits", "10.0", "amount of hits needed to infect players", FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvInfectBaseDamage = CreateConVar("tf2c_infectbasedamage", "3.0", "base amount of damage from infection", FCVAR_PLUGIN|FCVAR_NOTIFY);
...
public Action:MainEvents(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for (i=1;i<=MAX_PLAYERS;i++)
	{
		...
		{ KillTimer(infectionTimer[i]); infectionTimer[i] = INVALID_HANDLE; }
	}
	new k;
		...
		if(attacker <= MaxClients+1 && attacker != 0)
		{
			...
			if(StrEqual(weaponName, "tf_weapon_syringegun_medic"))
			{
				...
				if(StrEqual(modelname, "models/weapons/v_models/v_syringegun_medic.mdl"))
				{
					...
					infectHits[victim]++;
					if(infectHits[victim] >= GetConVarInt(cvInfectHits) && infectionTimer[victim] == INVALID_HANDLE)
					{
						tempInfectHP[victim] = GetClientHealth(victim);
						PrintToServer("player %i infected!", victim);
						EmitSoundToAll(SND_INFECT, victim);
						new Handle:pack;
						infectionTimer[victim] = CreateDataTimer(3.33, InfectionTimer, pack, TIMER_REPEAT);
			...
			if(infectionTimer[i] != INVALID_HANDLE)
			{
				if(tempInfectHP[i] < GetClientHealth(i))
				{
					if((infectHits[i] - 1) > 0)
					{
						...
						PrintToServer("player %i healed a bit", i);
					}
					else
					{
		...
		else
		{
			...
			for (j=1;j<=maxplayers;j++)
			{
				if(PlayersInRange[j]>0.0)
				{
					...
					AddVectors(play, playerspeed, play);
					TeleportEntity(j, NULL_VECTOR, NULL_VECTOR, play);
					HurtPlayer(j, client, damage, "tf2nade_conc", true, center, 13.0, 1); 
				}
			}
		}
	}
...
public Action:InfectionTimer(Handle:timer, Handle:pack)
{
	...
	if(IsValidEntity(victim) && IsValidEntity(attacker))
	{
		...
		if(infectHits[victim] >= (10*GetConVarInt(cvInfectHits)))
		{
			HurtPlayer(victim, attacker, GetConVarInt(cvInfectBaseDamage)*8, "infection", false, NULL_VECTOR, 1.0, 1); 
			...
			InfectionSpread(victim, team, 256.0);
			InfectionSpread(victim, team, 128.0);
			InfectionSpread(victim, team, 64.0);
			
			if((infectHits[victim] == ((10*GetConVarInt(cvInfectHits))+40)) || (infectHits[victim] == (10*GetConVarInt(cvInfectHits))))
			{ 
				PrintToServer("infection holding");
			}
			else
			{
				infectHits[victim]--; 
				PrintToServer("infection weakening");		
			}
			
			
		}
		else if(infectHits[victim] >= (5*GetConVarInt(cvInfectHits)))
		{
			HurtPlayer(victim, attacker, GetConVarInt(cvInfectBaseDamage)*4, "infection", false, NULL_VECTOR, 1.0, 1); 
			AttachParticle(victim, tempParticle, 2.0);
			...
			if((infectHits[victim] == ((5*GetConVarInt(cvInfectHits))+20)) || (infectHits[victim] == (5*GetConVarInt(cvInfectHits))))
			{ 
				PrintToServer("infection holding");
			}
			else
		...
		else if(infectHits[victim] >= (3*GetConVarInt(cvInfectHits)))
		{
			HurtPlayer(victim, attacker, GetConVarInt(cvInfectBaseDamage)*2, "infection", false, NULL_VECTOR, 1.0, 1); 
			AttachParticle(victim, tempParticle, 2.0);
			SetEntityRenderColor(victim, 155, 255, 155, 255);
			InfectionSpread(victim, team, 128.0);
			InfectionSpread(victim, team, 64.0);
			
			if((infectHits[victim] == ((3*GetConVarInt(cvInfectHits))+10)) || (infectHits[victim] == (3*GetConVarInt(cvInfectHits))))
			{ 
				PrintToServer("infection holding");
			}
			else
		...
		else if(infectHits[victim] >= GetConVarInt(cvInfectHits))
		{
			HurtPlayer(victim, attacker, GetConVarInt(cvInfectBaseDamage), "infection", false, NULL_VECTOR, 1.0, 1); 
			AttachParticle(victim, tempParticle, 2.0);
			SetEntityRenderColor(victim, 205, 255, 205, 255);
			InfectionSpread(victim, team, 64.0);
			
			if((infectHits[victim] == (GetConVarInt(cvInfectHits) + 5)) || (infectHits[victim] == (GetConVarInt(cvInfectHits))))
			{ 
				PrintToServer("infection holding");
			}
			else
...
InfectionSpread(client, clientteam, Float:radius)
{
	...
	FindPlayersInRange(loc, radius, oteam, 0, true, -1);
	new maxplayers = GetMaxClients();
	for (new i=1;i<=maxplayers;i++)
	{
		if(PlayersInRange[i]>0.0)
		{
			infectHits[i]++;
			PrintToServer("player %i is spreading infection to player %i", client, i);
			if(infectHits[i] > GetConVarInt(cvInfectHits) && infectionTimer[i] == INVALID_HANDLE)
			{
				PrintToServer("player %i is infected by spread", i);
				EmitSoundToAll(SND_INFECT, client);
				new Handle:pack;
				infectionTimer[i] = CreateDataTimer(3.33, InfectionTimer, pack, TIMER_REPEAT);

...
// defines
// *************************************************
// NEED TO UPDATE INFECTION, AGAIN! (range falloff)
...
public OnPluginStart() 
{
	...
	cvFragRadius = CreateConVar("tf2c_nades_frag_radius", "256.0", "radius for concussion nade", FCVAR_PLUGIN, true, 1.0, true, 2048.0);
	...
	//RegConsoleCmd("+jump", Command_Jump);
	...
	if(GetConVarInt(cvPickupsAllowed) == 1){
		...
		{	
			...
			if (StrEqual(modelname, "models/items/ammopack_large.mdl"))
			{
				...
				FindPlayersInRange(pos, 64.0, 0, -1, false, -1);
				new j;
			...
			if(GetConVarInt(cvBunnyEnabled)==1)
			{
				...
				if(GetConVarInt(cvShowJumpInfo)>0)
				{
					if(InJump[i] && (GetEntityFlags(i) & FL_ONGROUND))
					{
						...
						//HorLength = SquareRoot((VecLength[0] * VecLength[0]) + (VecLength[1] * VecLength[1]));
						for(new j=0;j<3;j++)
						{
							if(VecLength[j] < 0.0)
							{ VecLength[j] *= -1; }
				...
				if(GetConVarInt(cvWallJump)==1)
				{
					...
					if(( !(GetEntityFlags(i) & FL_ONGROUND) || !WasOnGroundLastTime[i] ) && (GetClientButtons(i) & IN_JUMP) && wallJumpReady[i] && (lengthCheck > GetConVarFloat(cvWallJumpMinSpeed)) && (GetClientButtons(i) & IN_DUCK))//
					{
						...
						//PrintToServer("client %i, with class %i, jumped", i, tempclass);
						if(tempclass==SPY)
						{
							//PrintToServer("spy pressed jump");
							...
							//angEye[0][0] = (angEye[2][1]); angEye[0][1] = (angEye[2][0]*-1.0); angEye[0][2] = 0.0; //orig[2];
							//angEye[1][0] = (angEye[2][1]*-1.0); angEye[1][1] = angEye[2][0]; angEye[1][2] = 0.0; //orig[2];
							//angEye[0][0] = (ang[1]); angEye[0][1] = (ang[0]*-1.0); angEye[0][2] = 0.0; 
							//angEye[1][0] = (ang[1]*-1.0); angEye[1][1] = ang[0]; angEye[1][2] = 0.0; 
							//angEye[2][0] = ang[0]; angEye[2][1] = ang[1]; angEye[2][2] = ang[2]; 
							...
							if (tr!=INVALID_HANDLE)
							{
								...
								{
									//PrintToServer("hit on left side");
									...
									if(distance < GetConVarFloat(cvWallJumpDistance))
									{
										...
										{
											//PrintToServer("normals %f, %f, %f", normal[0], normal[1], normal[2]);
											//PrintToServer("nAngles &f, &f, &f", tempnormal[0], normal[1], normal[2]);
											ang[1]-=180.0; 
											GetAngleVectors(ang, ang, NULL_VECTOR, NULL_VECTOR);
											...
											//tempSpeed = power;
											new Float:tempVel[3];
											//boostSpeed[0][0] = ang[0]*power; boostSpeed[0][1] = ang[1]*power; boostSpeed[0][2] = ang[2]*power;
											boostSpeed[0][0] = normal[0]*power; boostSpeed[0][1] = normal[1]*power; boostSpeed[0][2] = normal[2]*power;
											//PrintToServer("normspeed[0] = %f, normspeed[1] = %f, normspeed[2] = %f", boostSpeed[0][0], boostSpeed[0][1], boostSpeed[0][2]);
											//boostSpeed[0][0] = 0.0; boostSpeed[0][1] = 0.0; boostSpeed[0][2] = 0.0;
											...
											//PrintToServer("eyespeed[0] = %f, eyespeed[1] = %f, eyespeed[2] = %f", boostSpeed[1][0], boostSpeed[1][1], boostSpeed[1][2]);
											//tempVel = boostSpeed[1];
											AddVectors(boostSpeed[1], boostSpeed[0], tempVel);
											tempSpeed = GetVectorLength(vecVel);
											//AddVectors(vecVel, boostSpeed[1], vecVel);
											for(new j=0;j<3;j++)
											{	
											...
											CloseHandle(tr);
											//PrintToServer("hit %i, fraction = %f", TR_GetEntityIndex(tr), TR_GetFraction(tr));
										}
									}
									else
									{
									...
									if (TR_GetFraction(tr)<0.98)
									{
										...
										if(distance < GetConVarFloat(cvWallJumpDistance))
										{
											...
											if(CheckNormal(vecVel, normal))
											{
												...
												//AddVectors(vecVel, tempVel, tempVel);
												TeleportEntity(i, NULL_VECTOR, NULL_VECTOR, tempVel);
												wallJumpReady[i] = false;
												CreateTimer(GetConVarFloat(cvWallJumpDelay2), WallJumpReady, i);
												//PrintToServer("hit %i, fraction = %f", TR_GetEntityIndex(tr), TR_GetFraction(tr));
	...
	if(GetConVarInt(cvForcedShowInfo)==0)
	{
		if(args>0) 
		{
			...
			if(StringToInt(arg)<0 || StringToInt(arg)>4) {
				ReplyToCommand(client, "Value must be from 0 to 4.");
				return Plugin_Handled;
			}
...
public Action:PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(gHolding[client] != HOLD_NONE)
	{
		...
		new class = int:TF2_GetPlayerClass(client);
		if(gHolding[client] == HOLD_FRAG){ special = false; }
		if(gHolding[client] == HOLD_SPECIAL){ special = true; }
		SetupNade(class, GetClientTeam(client), special ? 1 : 0);
		TeleportEntity(gNade[client][gNadeNumber[client]], loc, NULL_VECTOR, NULL_VECTOR);
		gHolding[client] = HOLD_NONE;
	}
...
/*public Action:Command_Jump(client, args)
{
	if(GetConVarInt(cvWallJump)==1)
	{
		new tempclass = int:TF2_GetPlayerClass(client);
		PrintToServer("client %i, with class %i, jumped", client, tempclass);
		if(tempclass==SPY)
		{
			PrintToServer("spy pressed jump");
			new Float:orig[3], Float:ang[3], Float:angEye[3][3], Float:power;
			power = GetConVarFloat(cvWallJumpPower);
			new Handle:tr;
			GetClientEyePosition(client, orig);
			orig[2] -= 32.0;
			GetClientEyeAngles(client, ang);
			GetAngleVectors(ang, angEye[2], angEye[0], NULL_VECTOR);
			angEye[1][0] = (angEye[0][0]*-1.0); angEye[1][1] = (angEye[0][1]*-1.0); angEye[1][2] = (angEye[0][2]*-1.0);
			tr = TR_TraceRayFilterEx(orig, angEye[0], MASK_SOLID, RayType_Infinite, TraceRayDontHitSelfOrPlayers, client);
			if (tr!=INVALID_HANDLE)
			{
				if (TR_GetFraction(tr)>0.98 && !(GetEntityFlags(client) & FL_ONGROUND))
				{
					PrintToServer("hit on right side");
					new Float:vecVel[3];
					GetEntPropVector(client, Prop_Data, "m_vecVelocity", vecVel);
					angEye[2][0]*=power; angEye[2][1]*=power; angEye[2][2]*=power;
					AddVectors(vecVel, angEye[2], vecVel);
				...
				{
					CloseHandle(tr);
					tr = TR_TraceRayFilterEx(orig, angEye[1], MASK_SOLID, RayType_Infinite, TraceRayDontHitSelfOrPlayers, client);
					if (tr!=INVALID_HANDLE)
					{
						if (TR_GetFraction(tr)>0.98  && !(GetEntityFlags(client) & FL_ONGROUND))
						{
...
/*AttachColorParticle(ent, String:particleType[], Float:time, color[4])
{
	new particle = CreateEntityByName("info_particle_system");
	if (IsValidEdict(particle))
	{
		...
		DispatchKeyValue(particle, "parentname", tName);
		DispatchKeyValue(particle, "effect_name", particleType);
		//SetEntityRenderColor(particle, color[0], color[1], color[2], color[3]);
		DispatchSpawn(particle);
		SetVariantString(tName);
		AcceptEntityInput(particle, "SetParent", particle, particle, 0);
		ActivateEntity(particle);
...
/*int:CheckWall(client)
{
	new Float:orig[3], Float:ang[3], Float:distance, Float:pos[3];
	new Handle:tr;
	GetClientEyePosition(client, orig);
	orig[2] -= 16.0;
	GetClientAbsAngles(client, ang);
	ang[1] += 90.0; ang[0] = 0.0; ang[2] = 0.0;
	tr = TR_TraceRayFilterEx(orig, ang, MASK_PLAYERSOLID, RayType_Infinite, TraceRayDontHitSelfOrPlayers, client);
	if (tr!=INVALID_HANDLE)
	{
		if (TR_GetFraction(tr)<0.98)
		{

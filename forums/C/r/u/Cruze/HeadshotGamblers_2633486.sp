...
char ClientName[MAXPLAYERS][128];
bool Freezing=false;
int HeadshotGamblersCount;
int HeadshotGamblersMoney[MAXPLAYERS];
int HeadshotGamblersAnswer[MAXPLAYERS];
int HeadshotGamblersCommunication;
...
public void RoundPrestart(Handle event, const char[] name, bool dontBroadcast)
{
	Freezing=true;
...
public Action Command_Say_Team(int client, const char[] command, int args)
{
	if (HeadshotGamblersCount!=-1 && HeadshotGamblersCommunication==0)
		return Plugin_Continue;
	char Text[256];
...
public Action PlayerDeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (!IsValidEntity(Victim))
		return;
	bool AlivePlayerCt=false;
	bool AlivePlayerT=false;
	for (int i=1;i<MAXPLAYERS;i++)
	{
		if (!IsClientConnected(i))
			continue;
...
public void CheckHeadshotGambler()
{
	int MaxMoney=GetConVarInt(FindConVar("mp_maxmoney"));
...
public Action Command_Mute(int client, const char[] command, int args)
{
	ClientCommand(client,"apo_hg_menu");
}

public Action ShowHeadshotGamblersMenu(int client, int args)
{
	if (HeadshotGamblersCount==-1)
	{
	...
	int CurrentMoney=GetClientMoney(client);
	Handle HandleHeadshotGamblersMenu = CreateMenu(HeadshotGamblersMenu);
...
public int HeadshotGamblersMenu(Handle HandleHeadshotGamblersMenu, MenuAction action, int param1, int param2)
{
	int client=param1;
	if (HeadshotGamblersCount==-1)
	{
...
public void HeadshotGamblersTips(int Killer)
{
	for (int client=1;client<MAXPLAYERS;client++)
	{
		if (!IsClientConnected(client))

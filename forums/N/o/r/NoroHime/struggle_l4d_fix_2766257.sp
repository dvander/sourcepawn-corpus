...
SetupSDKCall()
{
	...
	if(L4D2Version)
	{
		...
		StartPrepSDKCall(SDKCall_Player);
		PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CTerrorPlayer::Fling");
		PrepSDKCall_AddParameter(SDKType_Vector, SDKPass_ByRef);
		...
		{
			PrintToServer("Unable to find the 'CTerrorPlayer::Fling' signature, check the file version!");
 		}
		...
		StartPrepSDKCall(SDKCall_Player);
		PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CTerrorPlayer::OnPounceEnded");
		...
		StartPrepSDKCall(SDKCall_Player);
		PrepSDKCall_SetFromConf(gConf, SDKConf_Signature, "CTerrorPlayer::OnStaggered");
		PrepSDKCall_AddParameter(SDKType_CBasePlayer, SDKPass_Pointer);
		PrepSDKCall_AddParameter(SDKType_Vector, SDKPass_ByRef);
		sdkShoveInf = EndPrepSDKCall();
		if(sdkShoveInf == INVALID_HANDLE)
		{
			PrintToServer("BROKEN SIGNATURE '\"'CTerrorPlayer::OnStaggered'\"' PLEASE UPDATE GAMEDATA");
		}
	}
	else
	{

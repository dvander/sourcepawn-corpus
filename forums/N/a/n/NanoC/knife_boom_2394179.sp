...
{
    CreateConVar("zm_Cuchi_Bomba(knife-plant)", DATA, "version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
public PlayerDeathEvent(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    {
        EmitAmbientSound("", vecOrigin, victim, _, _, 1.0);
    }
}
    ...
    if (IsPlayerAlive(attacker))
    {
        ...
        if(ZR_IsClientHuman(attacker) && ZR_IsClientZombie(client) && contar > 0)
        {
            ...
            if(StrEqual(weapon, "knife", false))
            {
                ...
                CPrintToChat(client, "{green}[aSO]{default} %s Te planto una bomba! Tenes %d segundos para matar o moriras!",nome,g_Time);
                new Handle:pack;
                new rnd_sound = GetRandomInt(1, 6);
                if(rnd_sound == 1) {
                    EmitSoundToAll(g_soundName);
                    CPrintToChat(attacker, "{green}[aSO]{default} Bien, mataste un zombie!",nome);
                }
            ...
            contar++;
            CPrintToChatAll("{green}[aSO]{default} %s fue infectado por acuchillar al {lightgreen}MOTHERZOMBIE!",nome);
...
public Action:ZR_OnClientInfect(&client, &attacker, &bool:motherInfect, &bool:respawnOverride, &bool:respawn)
{
    ...
    if(g_ZombieExplode[attacker])
    {
        ...
        CPrintToChat(attacker, "{green}[aSO]{default} %s mataste a un humano y safaste de morirte!!",nome);

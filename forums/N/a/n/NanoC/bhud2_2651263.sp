...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_currenthp", Command_CHP, ADMFLAG_ROOT, "See Current HP");
	RegAdminCmd("sm_subtracthp", Command_SHP, ADMFLAG_ROOT, "Subtract Current HP");
	RegAdminCmd("sm_addhp", Command_AHP, ADMFLAG_ROOT, "Add Current HP");
...
{
	if(IsValidClient(activator) && (IsPlayerGenericAdmin(activator) || !AdminOnly))
	{
		...
		{
			if(HudSymbols)
			Format(szString, sizeof(szString), ">>%i HP <<", health);
			else
			Format(szString, sizeof(szString), "%i HP", health);
...
public Action UpdateHUD(Handle timer, any client)
{
	...
	{
		if(IsValidClient(i) && IsValidEntity(entityID[i]) && (IsPlayerGenericAdmin(i) || !AdminOnly))
		{
...
{ 
	return CheckCommandAccess(client, "generic_admin", ADMFLAG_ROOT, false);
}  

...
Handle cBeep;
bool beepAlert[MAXPLAYERS+1] = {true, ...};

int nVoters = 0;
int nVotes = 0;
int nVotesNeeded = 0;
bool bVoted[MAXPLAYERS+1] = {false, ...};
bool AllowedVoteSpeed;
...
public void OnPluginStart()
{
	...
	//Commands
	RegAdminCmd("sm_reloaddb", Command_ReloadConfig, ADMFLAG_RCON ,"Reload the dodgeball's configs on round end.");
	RegAdminCmd("sm_setspeed", Cmd_SetSpeed, ADMFLAG_RCON, "Set the spawn speed of the rocket.");
	
	RegConsoleCmd("sm_beep", Cmd_Beep, "Enable/Disable the rocket alert \"beep\" noise.");
	RegConsoleCmd("sm_votespeed", Cmd_VoteSpeed, "Vote on the rocket's speed!");
	...
	//Cookies
	cBeep = RegClientCookie("dbredux_beep", "Beep noise", CookieAccess_Private);
...
public Action Cmd_Beep(int client, int args)
{
	beepAlert[client] = !beepAlert[client];
	if (beepAlert[client])
	{
		ReplyToCommand(client, "[SM] Enabled rocket alerts.");
		SetClientCookie(client, cBeep, "true");
	}
	else 
	{
		ReplyToCommand(client, "[SM] Disabled rocket alerts.");
		SetClientCookie(client, cBeep, "false");
	}
	return Plugin_Handled;
}

public Action Cmd_SetSpeed(int client, int args)
{
	char arg[10];
	GetCmdArg(1, arg, sizeof(arg));
	float mph = StringToFloat(arg);
	mph /= (15.0/352.0);
	for (int i = 0; i < MAXROCKETCLASS; i++)
	{
		g_RocketClass[i].speed = mph;
	}
	ReplyToCommand(client, "[SM] The rocket's speed has been set to %s.", arg);
	return Plugin_Handled;
}

public Action Cmd_VoteSpeed(int client, int args)
{
	AttemptVoteSpeed(client);
}

public OnClientSayCommand_Post(int client, const char[] command, const char[] sArgs)
{
	if (strcmp(sArgs, "votespeed", false) == 0 || strcmp(sArgs, "vspeed", false) == 0)
	{
...
void AttemptVoteSpeed(int client)
{
	if (!AllowedVoteSpeed)
	{
		ReplyToCommand(client, "[SM] Sorry, votespeed is currently on cool-down.");
		return;
	}
	
	if (bVoted[client])
	{
		ReplyToCommand(client, "[SM] You have already voted.");
		return;
	}
	...
	nVotes++;
	bVoted[client] = true;
	PrintToChatAll("[SM] %s wants to change the rocket's speed. (%d votes, %d required)", name, nVotes, nVotesNeeded);
...
void StartVoteSpeed()
{
	VoteSpeedMenu();
	ResetVoteSpeed();
	AllowedVoteSpeed = false;
	CreateTimer(60.0, Timer_Delay, _, TIMER_FLAG_NO_MAPCHANGE);
}
...
void VoteSpeedMenu()
{
	...
	Handle vm = CreateMenu(VoteSpeedMenuHandler, MenuAction:MENU_ACTIONS_ALL);
	SetVoteResultCallback(vm, Handle_VoteResults);
	
	SetMenuTitle(vm, "Vote Speed in Miles Per Hour");
	AddMenuItem(vm, "50", "50");
	AddMenuItem(vm, "75", "75");
	AddMenuItem(vm, "100", "100");
	AddMenuItem(vm, "125", "125");
	AddMenuItem(vm, "150", "150");
	AddMenuItem(vm, "175", "175");
	AddMenuItem(vm, "200", "200");
...
public Handle_VoteResults(Handle:menu, num_votes, num_clients, const client_info[][2], num_items, const item_info[][2])
{
	...
	float mph = StringToFloat(winInfo);
	mph /= (15.0/352.0);
	for (int i = 0; i < MAXROCKETCLASS; i++)
	{
		g_RocketClass[i].speed = mph;
	...
	PrintToChatAll("[SM] Majority voted %smph, the rocket's speed has been updated.", winInfo);
}

public VoteSpeedMenuHandler(Handle menu, MenuAction action, int client, int param2)
{
...
public void OnClientConnected(int client)
{
	bVoted[client] = false;
	nVoters++;
	nVotesNeeded = RoundToFloor(float(nVoters) * 0.75);
}
...
public OnClientCookiesCached(client)
{
	...
	{
		char result[10];
		GetClientCookie(client, cBeep, result, sizeof(result));
		if (StrEqual(result, "false")) beepAlert[client] = false;
		else beepAlert[client] = true;
	}
}
...
public void FireRocket()
{
	...
	if(iEntity && IsValidEntity(iEntity))
	{
		...
		if (beepAlert[g_RocketEnt[rIndex].target]) EmitSoundClientDB(g_RocketEnt[rIndex].target, rsnd_alert ,rIndex,false);
...
public void OnGameFrame()
{
	...
	for(int i = 0; i < MAXROCKETS; i++)
	{
		...
		else if(rDeflects > g_RocketEnt[i].deflects)
		{
			...
			if (beepAlert[g_RocketEnt[i].target]) EmitSoundClientDB(g_RocketEnt[i].target, rsnd_alert ,i,false);
...
void GetHudString(char[] strHud, int length, int rIndex, bool twoLines)
{
	...
	if (index != INVALID_ENT_REFERENCE)
	{
		...
		{
			int mphSpeed = RoundFloat(g_RocketEnt[rIndex].speed * (15.0/352.0));
			Format(speed,MAX_NAME_LENGTH,"%d",mphSpeed);
		}
	...
	{
		Format(strHud, length, " Owner: %s \n Target: %s \n Deflects: %s \n Speed: %smph",owner,target,deflects,speed);
	}
}

...
#define PLUGIN_VERSION		"3.1415" //as of Aug11, 2013, also pi round 3, also more bugfixes
...
// bottiger edit
public Action:Timer_CheckHealth2(Handle:timer, any:userid) {
    new client = GetClientOfUserId(userid);
    if (IsValidClient(client)) {
        if(GetClientHealth(client) > RoundToFloor(1.5 * TF2_GetMaxHealth(client)))
            TF2_SetHealth(client, RoundToFloor(1.5 * TF2_GetMaxHealth(client)));
    }
}
...
public Action:Command_Gimme(client, args)
{
	...
	new valveindex = (weaponLookupIndex < 0 ? -1*weaponLookupIndex : weaponLookupIndex);
	if (cvar_valveweapons != 1337 && (
                                      // bottiger edit
                                      valveindex == 7018 || // community launcher
                                      valveindex == 2228 || // army of one
                                      valveindex == 730  || // beggar's bazooka glitched
                                      ...
                                      */
                                      ...
                                      (valveindex >= 8000 && valveindex <= 9999))
        )
	{
		ReplyToCommand(client, "[TF2Items] Valve Weapons are Disabled");
		return Plugin_Handled;
	...
	// bottiger edit
	CreateTimer(0.0, Timer_CheckHealth2, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
...
{
	new Handle:hItemsArray = 
		TF2IDB_FindItemCustom("SELECT id FROM tf2idb_item WHERE slot IN ('primary','secondary','melee','pda','pda2','building') AND has_string_attribute=0");
	...
	for (new i = 0; i < GetArraySize(hItemsArray); i++)
	{
...
//		if (StrEqual(strItemClass, "bundle", false)) continue; //we don't want bundles in here...yet
		iItemSlot = _:TF2IDB_GetItemSlot(iItemDefID);
		if (iItemDefID == 735 || iItemDefID == 736 || iItemDefID == 810 || iItemDefID == 831 || iItemDefID == 933) iItemSlot = 1;
		iItemQuality = _:TF2IDB_GetItemQuality(iItemDefID);
		new min,max;
		TF2IDB_GetItemLevels(iItemDefID, min, max);
		iItemLevel = max;
		...
		iAttributeNum = TF2IDB_GetItemAttributes(iItemDefID, aids, values);
		if (iAttributeNum)
		{
			for (a = 0; a < iAttributeNum; a++)
			{
				...
				Format(appendBuffer, sizeof(appendBuffer), "%s%d ; %s", a > 0 ? " ; " : "", attrid, strFloat);

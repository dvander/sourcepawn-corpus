...
public Plugin:myinfo = 
{
	...
	author = "ztar, fix dmg by raziEiL[disawar]",
...
public OnPluginStart()
{
	...
	l4d2_lw_lethaldamage	= CreateConVar("l4d2_lw_lethaldamage","1000.0", "Lethal Weapon base damage", CVAR_FLAGS);
	...
	l4d2_lw_shootonce		= CreateConVar("l4d2_lw_shootonce","1", "Survivor can use Lethal Weapon once per round", CVAR_FLAGS);
...
public Action:Event_Weapon_Fire(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new String:pistolero[32];
	GetClientName(client, pistolero, sizeof(pistolero));
	...
	if (ReleaseLock[client])
	{
		...
		PrintToChatAll("\x04[Lethal Weapon] \x05%s \x01 shoots \x05Lethal Weapon \x01using \x05%s", pistolero, weapon);
	}
}

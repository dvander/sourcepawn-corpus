...
// brake lights and sirens, and fixes for CS:GO.
...
#define  RALLYRACE_VERSION			"N-GO 0.52A"

public Plugin:myinfo =
{
	name = "CS:GO Rally Race",
	author = "CS:GO Version by Natalya",
	description = "CS:GO Rally Race",
	version = RALLYRACE_VERSION,
	url = "http://www.sourcemod.net/"
};
// CS:S version was by ben

//////////////////////////////////////////////
//					CONFIG					//
//////////////////////////////////////////////
#define 	DEBUG			1					// print debug message for getting entities
...
public OnPluginStart()
{
	...
	if(!FileExists(sPath))
	{
		...
		SetFailState("[RR DEBUG] Error -- There is no carconfig.txt!!");
...
{
	new String:sPath[PLATFORM_MAX_PATH];
	BuildPath(Path_SM, sPath, sizeof(sPath),"configs/rallyrace/sounds.txt");

	new Handle:sfile = OpenFile(sPath, "rb");
	if(sfile == INVALID_HANDLE)
	{
		PrintToServer("[RR] There are no sounds listed in sounds.txt, or there is an error with the file.");
		return;
	}
	new String:readData[256];
	new soundcount = 0;
	while(!IsEndOfFile(sfile) && ReadFileLine(sfile, readData, sizeof(readData)))
	{
		TrimString(readData);
		if(!FileExists(readData))
			continue;
		PrecacheSoundAny(readData);
		soundcount += 1;
	}
	CloseHandle(sfile);
	PrintToServer("[RR] %i sounds were precached.", soundcount);
}

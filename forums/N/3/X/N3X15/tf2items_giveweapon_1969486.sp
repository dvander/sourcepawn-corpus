...
#include <tf2_stocks>
#include <sdktools>
#include <tf2itemsinfo> // I'm so sorry, but this is the only way to make the configs sane. - N3X
...
#define PLUGIN_AUTHOR		"asherkin (Updated by FlaminSarge & N3X15)"
#define PLUGIN_VERSION		"3.141" //as of Jun04, 2013, also pi round 2, also tiny bugfixes update
...
public OnClientPutInServer(client)
{
	...
	ClearTimer(pChargeTiming[client]);
	ClearEyeParticle(client);
}
...
/*			new primary = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
	...
	if (GetTrieString(g_hItemInfoTrie, formatBuffer, worldmodel, sizeof(worldmodel)) && FileExists(worldmodel, true) && weaponLookupIndex != 169)
	{
		...
		else if (StrContains(classname, "wearable", false) == -1)
		{
			SetEntProp(entity, Prop_Send, "m_iWorldModelIndex", model);
			SetEntProp(entity, Prop_Send, "m_nModelIndexOverrides", model, _, 0);
		}
...
stock TFClassType:FixReload(client, weapon, idx, String:classname[], &realindex)
{
	...
	if (!GetTrieValue(g_hItemInfoTrie, formatBuffer, realindex))
	{
		realindex = -1;
	}
...
//
// The idea of this edit is to enable users to use the attribute NAME
//  (instead of ID) to modify an attribute.  This future-proofs the config
//  file so when indices change in the future, it automatically adjusts.
// Config files are also somewhat idiot-proofed, since it can now look
//  at the attribute specified and throw an error if it cannot be found.
// Oh, and you can tell exactly what's going on when you look at that config
//  file two months later.
// TF2Items, you're next. - N3X
stock CustomItemsTrieSetup(Handle:trie)
{
	// These variables used to be named strBuffer, strBuffer2, and strBuffer3.
	// In the name of sanity, I've renamed them to far more sensical names 
	//  that actually imply what they do.
	// I also took the time to neaten the code up a bit so it's easier to 
	//  comprehend.
	//
	decl String:strSectionName[256];
	BuildPath(Path_SM, strSectionName, sizeof(strSectionName), "configs/tf2items.givecustom.txt");
	decl String:strTrieKey[256];
	decl String:strTrieValue[PLATFORM_MAX_PATH];
	decl String:strAttributeName[256];
	new Handle:hKeyValues = CreateKeyValues("TF2ItemsGiveWeapon");
	if(FileToKeyValues(hKeyValues, strSectionName) == true)
	{
		KvGetSectionName(hKeyValues, strSectionName, sizeof(strSectionName));
		if (StrEqual("custom_give_weapons_vsanity", strSectionName) == true)
		{
			if (KvGotoFirstSubKey(hKeyValues))
			{
				do
				{
					...
					{
						// Why.
						Format(strTrieKey, 32, "%s_%s", strSectionName, "classname");
						KvGetString(hKeyValues, "classname", strTrieValue, sizeof(strTrieValue));
						SetTrieString(trie, strTrieKey, strTrieValue);
						
						Format(strTrieKey, 32, "%s_%s", strSectionName, "index");
						SetTrieValue(trie, strTrieKey, KvGetNum(hKeyValues, "index"));
						
						Format(strTrieKey, 32, "%s_%s", strSectionName, "slot");
						SetTrieValue(trie, strTrieKey, KvGetNum(hKeyValues, "slot"));
						
						Format(strTrieKey, 32, "%s_%s", strSectionName, "quality");
						SetTrieValue(trie, strTrieKey, KvGetNum(hKeyValues, "quality"));
						
						Format(strTrieKey, 32, "%s_%s", strSectionName, "level");
						SetTrieValue(trie, strTrieKey, KvGetNum(hKeyValues, "level"));
						
						//Format(strTrieKey, 256, "%s_%s", strSectionName, "attribs");
						//KvGetString(hKeyValues, "attribs", strTrieValue, sizeof(strTrieValue));
						// FAR BETTER ATTRIBUTE PARSING. - N3X
						strTrieValue = "";
						if(KvJumpToKey(hKeyValues,"attribs"))
						{
							if(KvGotoFirstSubKey(hKeyValues,false))
							{
								...
								{
									// Get attribute name
									KvGetSectionName(hKeyValues, strAttributeName, sizeof(strAttributeName));
									
									// Try to read the attribute
									//new Float:fAttributeValue = 0.0;
									decl String:strAttributeValue[256];
									//KvGetFloat(hKeyValues, NULL_STRING, fAttributeValue); // Broken.
									KvGetString(hKeyValues, NULL_STRING, strAttributeValue, sizeof(strAttributeValue));
									//fAttributeValue = StringToFloat(strAttributeValue);
									
									// Translate attribute name to index.
									new iAttrIdx = TF2II_GetAttributeIDByName(strAttributeName);
									if(iAttrIdx == -1)
									{
										LogMessage("Error: Item attribute \"%s\" could not be found (TF2II returned -1).",strAttributeName);
									} else {
										//LogMessage("%s (%d) = %f",strAttributeName,iAttrIdx,fAttributeValue);
										LogMessage("%s (%d) = %s",strAttributeName,iAttrIdx,strAttributeValue);
									}
									
									// Make a string in the format of the old config file format.
									// Why?  Because I'm not touching the clusterfuck above and below.
									if(StrEqual(strTrieValue,"",false))
									{
										Format(strTrieValue,PLATFORM_MAX_PATH,"%d ; %s",iAttrIdx,strAttributeValue);
									} else {
										Format(strTrieValue,PLATFORM_MAX_PATH,"%s ; %d ; %s",strTrieValue,iAttrIdx,strAttributeValue);
									}
						...
						KvGoBack(hKeyValues);
						KvGoBack(hKeyValues);
						//LogMessage("[TF2Items GiveWeapon] %s attribs = %s", strSectionName, strTrieValue);
						Format(strTrieKey, 256, "%s_%s", strSectionName, "attribs");
						SetTrieString(trie, strTrieKey, strTrieValue);
						LogMessage("[TF2Items GiveWeapon] %s attribs = %s", strSectionName, strTrieValue);
						// END OF MODIFICATION.
						
						Format(strTrieKey, 32, "%s_%s", strSectionName, "ammo");
						SetTrieValue(trie, strTrieKey, KvGetNum(hKeyValues, "ammo", -1));
						
						Format(strTrieKey, 256, "%s_%s", strSectionName, "model");
						KvGetString(hKeyValues, "model", strTrieValue, sizeof(strTrieValue));
						if (strTrieValue[0] != '\0') SetTrieString(trie, strTrieKey, strTrieValue);
						
						Format(strTrieKey, 256, "%s_%s", strSectionName, "viewmodel");
						KvGetString(hKeyValues, "viewmodel", strTrieValue, sizeof(strTrieValue));
						if (strTrieValue[0] != '\0') SetTrieString(trie, strTrieKey, strTrieValue);
					}
				}
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	...
	if ((buttons & IN_ATTACK2) && !(iLastButtons[client] & IN_ATTACK2))
	{
		...
		new targe = -1;
		if ((buttons & IN_RELOAD) && GetEntityMoveType(client) != MOVETYPE_NONE && ((targe = FindPlayerTarge(client)) != -1) && pChargeTiming[client] == INVALID_HANDLE && GetEntPropFloat(client, Prop_Send, "m_flMaxspeed") > 1.01 && (class != TFClass_DemoMan || (wep == GetPlayerWeaponSlot(client, TFWeaponSlot_Melee) && (idxslot2 == 265 || idxslot2 == 20 || idxslot2 == 207 || idxslot2 == 130))))
		{
			...
			if (class != TFClass_DemoMan && bashoffs > 28) SetEntData(targe, bashoffs, 0);

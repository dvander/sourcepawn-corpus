...
public OnPluginStart()
{
	...
	g_hMOTDsite = CreateConVar("motdbp_site", "0", "Choose a BackPack Site, 0 = TF2OutPost.com, 1 = TF2B.com, 2 = TF2items.com, 3 = backpack.tf" , 0, true, 0.0, true, 3.0);
...
public DisplayBackpack(client, target)
{
	...
	else
	{
		...
		switch (GetConVarInt(g_hMOTDsite))
		{
			...
			case 3:
			{
			Format(itemsurl, sizeof(itemsurl), "http://backpack.tf/profiles/%s", communityid);
			}
			default:
			{
			Format(itemsurl, sizeof(itemsurl), "http://www.tf2outpost.com/backpack/%s", communityid);

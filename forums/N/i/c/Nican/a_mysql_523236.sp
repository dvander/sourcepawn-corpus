...
public ConnectToMysql(){
    ...
    if(ismysql == 1)
        Format(buffer, sizeof(buffer), "SHOW COLUMNS FROM sm_users");
    else
        Format(buffer, sizeof(buffer), "PRAGMA table_info(sm_users)");
...
public OnTableUpdated(Handle:owner, Handle:hQuery, const String:error[], any:nothing){
    ...
    if(SteamIdColumn == INVALID_HANDLE){
        if(ismysql == 1){
            SQL_FastQuery(db, "ALTER TABLE `sm_users` ADD `steam` VARCHAR(32) NOT NULL");
            SQL_FastQuery(db, "ALTER TABLE `smf_servers` ADD UNIQUE (`steam`)");
        } else {
            SQL_FastQuery(db, "ALTER TABLE sm_users ADD `steam` TEXT");
            SQL_FastQuery(db, "CREATE UNIQUE INDEX IF NOT EXISTS steamunique ON sm_users(steam)");
        }
        ...
        PushArrayString(adt_names, "steam");
        Totalcount = PushArrayArray(adt_global, array_push); 
...
// ALTER TABLE `sm_users` DROP `money` ;
...
public OnClientAuthorized(client){        
    ...
    Format(buffer, sizeof(buffer), "SELECT %s FROM sm_users WHERE steam = '%s'",SelectQuery, auth);
...
public GetClientConnectInfo(Handle:owner, Handle:hndl, const String:error[], any:client)
{
    ...
    if (hndl == INVALID_HANDLE){
        ...
        Format(buffer, sizeof(buffer), "INSERT INTO sm_users(steam) VALUES ('%s')", auth);
        
        SQL_TQuery(db, RecieveUpdateQuery, buffer);
...
public UpdateClientInfo(client){
    ...
    decl String:Names[ Totalcount + 1 ][64], String:FinalUpdate[ Totalcount * 64 ], String:buffer[128], String:ColumnName[64];
    new count = 0;
    ...
    for(new i=0; i<= Totalcount; i++){
        ...
        switch( GetColumnType(i) ){
            ...
            case DB_STRING: {
                ...
                Format(buffer, 128, "`%s`='%s'", ColumnName , temp );
            }
        }
    ...
    Format(FinalUpdate, Totalcount * 64, "UPDATE `sm_users` SET %s WHERE `steam`='%s'", FinalUpdate, auth);

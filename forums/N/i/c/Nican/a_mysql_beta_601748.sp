...
public GetClientInsertId(Handle:owner, Handle:hndl, const String:error[], any:client){
    ...
    new id = SQL_GetInsertId( owner );
    if(id == 0){
        id = -1;
    }
    
    SetArrayCell(IdCoulmn, client, id );
    
    #if DEBUG == 1
        LogMessage("New user of InsertID: %d", id );
    #endif
...
public UpdateClientInfo(client){
    ...
    new clid = GetArrayCell(IdCoulmn, client);
    
    if(clid == -1){
        decl String:AuthStr[64];
        GetAuthString(client, AuthStr, sizeof(AuthStr));
        Format(FinalUpdate, Totalcount * 64, "UPDATE `sm_users` SET %s WHERE `%s`='%s'", FinalUpdate, SaveTypeNames[SAVE_TYPE], AuthStr );
    } else {
        Format(FinalUpdate, Totalcount * 64, "UPDATE `sm_users` SET %s WHERE `id`=%d", FinalUpdate, clid );    
    }

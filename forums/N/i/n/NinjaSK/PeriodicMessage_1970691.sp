...
new String:PeriodicMessage3[] = "\x04[MyServer]\x01 type /rules to read the server rules.";

public OnPluginStart()
{
	CreateTimer(611.0, PrintText, 0, TIMER_REPEAT);
	CreateTimer(354.0, PrintText, 1, TIMER_REPEAT);
	CreateTimer(1423.0, PrintText, 2, TIMER_REPEAT);
	CreateTimer(3009.0, PrintText, 3, TIMER_REPEAT);
}
...
public Action:PrintText(Handle:timer, any:numOfMessage)
{
	switch(numOfMessage)
	{
		...
		case 3:
			PrintToChatAll("%s", PeriodicMessage3);
	}

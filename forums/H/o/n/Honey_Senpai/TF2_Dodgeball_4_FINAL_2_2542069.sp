...
{    
    ...
    if (iIndex != -1)
    {
        ...
        new iEntity = CreateEntityByName(TestFlags(iFlags, RocketFlag_IsAnimated)? "tf_projectile_flare" : "tf_projectile_flare");
        if (iEntity && IsValidEntity(iEntity))
        {
            ...
            SetEntPropFloat(iEntity, Prop_Send, "m_flModelScale", (1.5,1.5,1.5));
            ...
            SetEntDataFloat(iEntity, FindSendPropOffs("CTFProjectile_Flare", "m_iDeflected") + 4, CalculateRocketDamage(iClass, fModifier), true);
...
stock SelectTarget(iTeam, iRocket = -1)
{
    ...
    new Float:flDistance = GetVectorDistance(fRocketPosition, flPos2);
...
public void AttachLight(int iIndex)
{
	int index = EntRefToEntIndex(g_iRocketEntity[iIndex]);
	...
	if (IsValidEntity(iLightEntity))
	{
		...
		DispatchKeyValue(iLightEntity, "_light", "255, 255, 255, 255");

 ...
 * Change Log:
 *  1.6h - 20130914
 *   Score bonus resetting logic consolidated and fixed
 *   Red Money scoring algorithm improved/fixed
 *   Spectators who haven't participated can no longer be labeled as wave or mission losers
 *  1.5h - 20130904
...
#define PLUGIN_VERSION "1.6h beta"
...
// Canteen sharing variables
// Used for detecting which power is being shared via canteen sharing
...
// Red money variables
new bool:g_currencyInit[MAXPLAYERS+1];
...
InitHooks()
{
	...
	// This event doesn't work for dispensers or Medics
	//HookEvent("player_healed", PlayerHealed);
	// This event doesn't work
	//HookEvent("player_healedbymedic", PlayerHealedByMedic);
...
HandleRedMoney() {
	...
	new netChange = 0;
	// Not used; would be for verification by determining the non zero mode of the set
	//new currencyChanges[MAXPLAYERS+1];
	// for (new index = 0; index < MAXPLAYERS+1; index++) {
	// 	currencyChanges[index] = 0;
	// }
	for (new player = 0; player < MAXPLAYERS+1; player++) {
		if (!g_currencyInit[player]) {
			g_recentRifleKill[player] = false;
			g_recentMoneyGrab[player] = false;
			continue;
		}
		else if (g_recentRifleKill[player] || g_recentMoneyGrab[player]) {
			...
			new currencyChange = g_currencyDuringPreviousFrame[player] - previousCurrency;
			//currencyChanges[player] = currencyChange;
			if (currencyChange > 0) {
				netChange = currencyChange;
			}
		}
	}
	new redMoneyTouched = g_currencyGrabbedSincePreviousFrame - netChange;
	new redMoneyGained = netChange - g_currencyGrabbedSincePreviousFrame;
	...
	else if (playersWithMoneyGrabs != 0 && redMoneyTouched > 0) {
	...
	for (new player = 0; player < MAXPLAYERS+1; player++) {
		...
		g_recentRifleKill[player] = false;
		g_recentMoneyGrab[player] = false;
		g_currencyInit[player] = true;
	}
	g_currencyGrabbedSincePreviousFrame = 0;
}

/*
TF2_GetCurrentWeaponClass(client, String:name[], maxlength) {
	if (client > 0) {
...
new bool:g_newlyHealed[MAXPLAYERS+1];

HandleHealing() {
	for (new healed = 0; healed < MAXPLAYERS+1; healed++) {
		if (IsValidClient(healed)) {
			new healers = GetEntProp(healed, Prop_Send, "m_nNumHealers");
			if (healers > 0) {
				// LogMessage(
					// "%N is being healed",
					// healed
				...
				// XXX: Why 64?
				new weaponNameMaxLen = 64;
				//new String:classname[weaponNameMaxLen];
				new String:weaponName[weaponNameMaxLen];
				new medics = 0;
				...
				for (new client = 0; client < MAXPLAYERS+1; client++) {
					if (IsValidClient(client)) {
						...
						GetClientWeapon(client, weaponName, 64);
						if (StrEqual(weaponName, "tf_weapon_medigun")) {
							// LogMessage(
								// "%N is being healed and %N is holding a medi gun",
								// healed, client
							// );
							new index = GetEntPropEnt(client, Prop_Send, "m_hActiveWeapon");
							if (GetEntProp(index, Prop_Send, "m_bHealing") == 1) {
								if (healed == GetEntPropEnt(index, Prop_Send, "m_hHealingTarget")) {
									// Player is being healed by a medic
									medics++;
									if (!g_newlyHealed[healed]) {
										g_newlyHealed[healed] = true;
										// LogMessage(
											// "%N is being healed by %N",
											// healed, client
										// );
										// TODO: Handle this
									}
...
public PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	/**
	 * This code was used to confirm that Sentry Busters do a base damage of 600
	if (!IsValidClient(attacker) && !IsValidClient(victim)) {
		LogMessage(
			"NPC %N damaged NPC %N for %d damage",
...
public PlayerHealed(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidClient(patient)) {
		LogMessage(
			"PlayerHealed: %N healed %N by %d HP",
			healer, patient, amount
		);
	}
}
*/
...
/*
public PlayerHealedByMedic(Handle:event, const String:name[], bool:dontBroadcast)
{
	LogMessage("PlayerHealedByMedic");
	// "medic"	"byte"
	new medic = GetClientOfUserId(GetEventInt(event, "medic"));
	if (IsValidClient(medic)) {
		LogMessage(
			"PlayerHealedByMedic: %N",
			medic
		);
	}
}
...
public TF2_OnConditionAdded(client, TFCond:condition) {
	...
	for (new buffOwner = 0; buffOwner < MaxClients; buffOwner++) {
		...
		// Can we assume that this condition came from an ally
...
UpdateWaveScores(wave)
{
	...
	for (new stat = 0; stat < MAXSTATS; stat++) {
		if (g_scoreWeights[stat] > 0) {
			...
			//LogMessage("Updating scores for stat %i with weight %f, current total %f", stat, g_scoreWeights[stat], g_totals[wave][stat]);
...
ResetScoreBonuses(client) {
	...
	// Red money variables
	g_currencyInit[client] = false;
	g_recentRifleKill[client] = false;
	g_currencyDuringPreviousFrame[client] = 0;
	g_recentMoneyGrab[client] = false;
}
...
ShowWaveWinner(wave)
{
	...
	// Calculate mission and wave winners
	new maxScore = 0;
	new waveWinner = 0;
	...
	// Calculate mission and wave losers, if applicable
	// XXX: Should be MAX_INT or similar
	new minScore = maxScore;
	new waveLoser = waveWinner;
	// XXX: Should be MAX_INT or similar
	new minMissionScore = maxMissionScore;
	new missionLoser = missionWinner;
	for (new client = 0; client < MaxClients; client++) {
		new bool:canLoseWave = true;
		new bool:canLoseMission = true;
		if (IsValidClient(client) && (
			GetClientTeam(client) == _:TFTeam_Unassigned ||
			GetClientTeam(client) == _:TFTeam_Spectator
		)) {
			new waveTotals = 0;
			for (new statIndex = 0; statIndex < MAXSTATS+1; statIndex++) {
				waveTotals += g_stats[client][wave][statIndex];
			}
			if (waveTotals < 1) {
				canLoseWave = false;
			}
			if (missionTotals[client] < 1) {
				canLoseMission = false;
			}
		}
		if (IsValidClient(client)) {
			if (g_stats[client][wave][SCORE] < minScore && canLoseWave) {
				minScore = g_stats[client][wave][SCORE];
				waveLoser = client;
			}
			if (missionTotals[client] < minMissionScore && canLoseMission) {
...
ShowWaveStatsAll(wave)
{
	for(new client = 0; client < MaxClients; client++) {
		if (IsValidClient(client)) {
			...
			PrintToChat(client, "Current mvmstats version is %s", PLUGIN_VERSION);
		}
	}
}

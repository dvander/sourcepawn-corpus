...
public OnPluginStart()
{
	/*
		ConVars
	*/
	CreateConVar("sm_saxton_menu_version", PLUGIN_VERSION, "Saxton Menu version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	new Handle:topmenu;
...
public OnAdminMenuReady(Handle:topmenu)
{
	...
	{
		AddToTopMenu(hAdminMenu, "sm_sax", TopMenuObject_Item, AdminMenuSaxton, player_commands, "sm_sax", ADMFLAG_KICK);
	}
}

public AdminMenuSaxton(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength)
{
	if (action == TopMenuAction_DisplayOption)
	{
		Format(buffer, maxlength, "VS Saxton Hale");
	}
	else if (action == TopMenuAction_SelectOption)
	{
		DisplaySaxtonOptions(param);
	}
}

DisplaySaxtonOptions(client)
{
...
public MenuHandler_Saxton(Handle:MainMenu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		switch (param2)
		{
			...
			case 3 :
			{
				ServerCommand("sm_hale_stop_music");
				PrintToChatAllEx(param1, "\x04Saxton-Menu\x05 |\x03 %s\x01 stops the\x04 Boss\'\x01 music.", admName);
				if (hAdminMenu != INVALID_HANDLE)
				{
					DisplaySaxtonOptions(param1);
				}
			}
		}
	}
...
public MenuHandler_HaleSelect(Handle:menuSelect, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		ServerCommand("sm_hale_select %s", pName);
		MenuChooseHale(param1, -1);
	}
...
public Action:MenuChooseSpecial(client, args)
{
	if (client && IsClientInGame(client) && !IsFakeClient(client))
	{
		...
		AddMenuItem(menuSpecial, "bunny", "Easter Bunny");
...
public MenuHandler_SaxtonSpecial(Handle:menuSelect, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		ServerCommand("sm_hale_special %s", specialClass);
...
public MenuHandler_AddPoints(Handle:menuAddPoints, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		ServerCommand("sm_hale_addpoints %s %s", pName, info);

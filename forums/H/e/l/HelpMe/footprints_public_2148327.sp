...
public OnPluginStart()
{
	...
	g_hCookieRandomFoot = RegClientCookie("footprintsRandom", "Footprints", CookieAccess_Private);
	...
	for(new i = 1; i <= MaxClients; i++)
	{
		...
		LoadClientCookies(i);
		CheckRandomColor(i, B_RANDOM_VALUE)
	}
}
...
public FootStepsCALLBACK(Handle:menu, MenuAction:action, client, param2)
{
	...
	if(action == MenuAction_Select)
	{
		...
		if(param2 == 19)
		{
			SetClientRandomColor(client, B_RANDOM_VALUE, true);
		}
		else
		{
			SetClientRandomColor(client, B_RANDOM_VALUE, false);
		}
	}
}
...
public OnClientPostAdminCheck(iClient)
{
	if(IsValidClient(iClient) && AreClientCookiesCached(iClient))
	{
		...
		CheckRandomColor(iClient, B_RANDOM_VALUE)
	}
	else
	{
...
stock bool:CheckRandomColor(client, excepttype)
{
	if (!IsValidClient(client)) return false;
	if (IsFakeClient(client)) return true;
	if (!AreClientCookiesCached(client)) return true;
	decl String:strCookie[32];
	if (excepttype == B_RANDOM_VALUE) GetClientCookie(client, g_hCookieRandomFoot, strCookie, sizeof(strCookie));
	else GetClientCookie(client, g_hCookieRandomFoot, strCookie, sizeof(strCookie));
	if (strCookie[0] == 0) return true, b_random[client] = false;
	else return bool:StringToInt(strCookie), b_random[client] = true;
}

SetClientRandomColor(client, excepttype, bool:on)
{
	if (!IsValidClient(client)) return;
	if (IsFakeClient(client)) return;
	if (!AreClientCookiesCached(client)) return;
	new String:strCookie[32];
	if (on) strCookie = "1", b_random[client] = true;
	else strCookie = "0", b_random[client] = false;
	if (excepttype == B_RANDOM_VALUE) SetClientCookie(client, g_hCookieRandomFoot, strCookie);
	else SetClientCookie(client, g_hCookieRandomFoot, strCookie);
}

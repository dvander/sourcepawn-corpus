...
public Action:ShowKDRateToClient(client, offset){
	...
	// Print KDR to Client
    // PrintToChat(client, "\x01\x07FF0000[\x07FF7F00K\x07FFD700e\x0700AA00k\x070000FFs \x078B00FFB\x07FF0000r\x07FF7F00i\x07FFD700g\x0700AA00a\x070000FFr\x076600FFd\x078B00FFe\x07FF0000] \x03%s\x04, you're KDRate is \x03%.2f \x04(\x03%i \x04Kills / \x03%i \x04Deaths)", player_name, KDRate, Frags, Deaths);
	PrintToChat(client, "\x01[KDR Check] \x03%s\x04, you're KDRate is \x03%.2f \x04(\x03%i \x04Kills / \x03%i \x04Deaths)", player_name, KDRate, Frags, Deaths);
...
public Action:Checktime(Handle:timer){
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		if (IsClientConnected(i) && IsClientInGame(i))
		{
			...
			if (!GetClientAuthString(i, f_sAuthID, sizeof(f_sAuthID), true))
			{
				Format(f_sAuthID, sizeof(f_sAuthID), "%s (Not Validated)", f_sAuthID);				
			}
			...
			{
				if(GetConVarInt(KDCheckerDebug) != 0) LogAction(i, -1,"[KDR Check] Ignore %s (ID: %s | IP: %s) is FAKEPLAYER", f_sPlayer_Name, f_sAuthID, f_sIP);
			}				
		}
	}
...
public Action:KDRateAction(client)
{
	...
	if (!GetClientAuthString(client, f_sAuthID, sizeof(f_sAuthID), true))
	{
		Format(f_sAuthID, sizeof(f_sAuthID), "%s (Not Validated)", f_sAuthID);				
	}

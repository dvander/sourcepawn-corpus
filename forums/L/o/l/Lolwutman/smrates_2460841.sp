...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_rateshelp", Command_RateMenu, "rate plugin help menu");
...
startmenu(client)
{
	...
	DrawPanelText(panel, "--------------------");
	DrawPanelText(panel, "  [rate/update/cmd/interp/interp_ratio]");
	DrawPanelText(panel, "  Normal rates are 80000/20/30/0.03/0.6");
	DrawPanelText(panel, "(Rates below 'Normal' are wrong. Please fix them.)");
	DrawPanelText(panel, "--------------------");
...
public Action:Command_allrates(client,args)
{
	if (args < 1)
	{
		...
		decl String:h_ip[17];
		decl String:h_interp[15];
		decl String:h_interp_ratio[15];
		...
		Format(h_interp, sizeof(h_interp), "%s", "Interp");
		Format(h_interp_ratio, sizeof(h_interp_ratio), "%s", "InterpRatio");
		Format(h_ip, sizeof(h_ip), "%s", "IP Address");

		PrintToConsole(client, "------------------------------------------------------------------------------------------------------");
		PrintToConsole(client, "|                                 Player Information                                        |");
		PrintToConsole(client, "------------------------------------------------------------------------------------------------------");
		PrintToConsole(client, "| %1.1s | %-5.5s | %-6.6s | %-3.3s | %-6.6s | %-11.11s | %-15.15s |%-20.20s | %-32.32s |", h_admin, h_rate, h_updaterate, h_cmdrate, h_interp, h_interp_ratio, h_ip, h_steamid, h_name);
		PrintToConsole(client, "------------------------------------------------------------------------------------------------------");
		
		new String:tmp_admin[2];
		new AdminId:id;
		new String:tmp_steamid[21];
		new String:tmp_update[10],String:tmp_cmd[10],String:tmp_rate[10],String:tmp_interp[15],String:tmp_interp_ratio[15],String:tmp_name[32];
		...
		for (new i=1; i<=MaxClients; i++)
		{
			...
			GetClientInfo(i, "cl_interp", tmp_interp, 9);
			GetClientInfo(i, "cl_interp_ratio",tmp_interp_ratio, 9);
			GetClientInfo(i, "rate", tmp_rate, 9);
			GetClientIP(i, tmp_ip, 17);
			
			PrintToConsole(client, "| %1.1s | %-5.5s | %-6.6s | %-3.3s | %-6.6s | %-11.11s | %-15.15s |%-20.20s | %-32.32s |", tmp_admin, tmp_rate, tmp_update, tmp_cmd, tmp_interp, tmp_interp_ratio, tmp_ip, tmp_steamid, tmp_name);
...
{
	new String:interp[10],String:interp_ratio[10],String:update[10],String:cmd[10],String:rate[10]
	
	GetClientInfo(client, "cl_interp", interp, 9)
	GetClientInfo(client, "cl_interp_ratio", interp_ratio, 9)
	...
	new Float:finterp_ratio = StringToFloat(interp_ratio)
	...
	DrawPanelItem(Panel, text)
	Format(text,59,"cl_interp_ratio: %f",finterp_ratio)
	DrawPanelItem(Panel, text)
	DrawPanelItem(Panel, " ", ITEMDRAW_SPACER|ITEMDRAW_RAWLINE)

...
{
	name = "MySQL Bans",
	author = "Team MX | MoggieX - Fixed by Thanatos",
...
bool:InsertBan(client, time, flags, const String:reason[], any:source, const String:identity[])
{
	...
	if ((flags & BANFLAG_IP) != 0)
	{
		...
		if (client > 0)
		{
			Format(query, sizeof(query),
				"INSERT INTO mysql_bans (player_name, ipaddr, ban_length, ban_reason, banned_by, timestamp) VALUES ('%N', '%s', '%d', '%N', CURRENT_TIMESTAMP)", client, authid, time, reason, source);
		}
		else
		{
			Format(query, sizeof(query),
				"INSERT INTO mysql_bans (ipaddr, ban_length, ban_reason, banned_by, timestamp) VALUES ('%N', '%s', '%d', '%N', CURRENT_TIMESTAMP)", authid, time, reason, source);
		}
	}
	else //if ((flags & BANFLAG_AUTHID) != 0)
	{
		...
		{
			Format(query, sizeof(query),
				"INSERT INTO mysql_bans (steam_id, player_name, ban_length, ban_reason, banned_by, timestamp) VALUES ('%s', '%d', '%s', '%N', CURRENT_TIMESTAMP)", authid, client, time, reason, source);
		}
		else
		{
			Format(query, sizeof(query),
				"INSERT INTO mysql_bans (steam_id, ban_length, ban_reason, banned_by, timestamp) VALUES ('%s', '%d', '%s', '%N', CURRENT_TIMESTAMP)", authid, time, reason, source);
		}
	}
	...
	if (success)
	{
		...
		if (client > 0)
		{
			LogAction(source, -1, "[MySQL Bans] [Put into DB] Auth %s, Ban time %d, Reason %s, Client %N", authid, client, time, reason, source);
		}
		else
		{

...
public Plugin:myinfo = {
	...
	author = "original by meng, Bacardi -edit by Luckiris",
	version = "2016-5-20",
...
public OnMapStart()
{
	laser = PrecacheModel("sprites/bluelaser1.vmt");
...
public Action:CreateKnife(Handle:timer, any:client)
{
	...
	if (knifeModel == -1) knifeModel = PrecacheModel(model);

	// model and size
	SetEntProp(knife, Prop_Send, "m_nModelIndex", knifeModel);
	...
	if(GetConVarBool(cvar_throwingknives_trails))
	{
		...
		if(game == Engine_CSS)
		{
			TE_SetupBeamFollow(knife, laser,	0, Float:0.5, Float:8.0, Float:1.0, 0, color);
		}
		else
		{
			TE_SetupBeamFollow(knife, beam,	0, Float:0.5, Float:1.0, Float:0.1, 0, color);
		}
		TE_SendToAll();
	}

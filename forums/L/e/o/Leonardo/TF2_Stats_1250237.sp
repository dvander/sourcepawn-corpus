...
public Event_player_stealsandvich(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (pointmsgval == 1)
	{
	PrintToChatAll("\x05%s\x01 %t",CHATTAG,"StealSandvich",medicname,pointvalue,playername)
	}else
	{
	if (cookieshowrankchanges[client])
	{
	PrintToChat(client,"\x05%s\x01 %t",CHATTAG,"yourStealSandvich",pointvalue,playername)
	}
	}
	}
...
public Event_player_invulned(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	*/	
	...
	if (pointmsgval == 1)
	{
	PrintToChatAll("\x05%s\x01 %t",CHATTAG,"Ubercharging",medicname,pointvalue,playername)
	}else
	{
	if (cookieshowrankchanges[client])
	{
	PrintToChat(client,"\x05%s\x01 %t",CHATTAG,"yourUbercharging",pointvalue,playername)
	}
	}
	}
	}
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (pointmsgval >= 1)
	{
		...
		if (pointmsgval == 1)
		{
			PrintToChatAll("\x05%s\x01 %t",CHATTAG,"Assisting",assiname,pointvalue,attackername)
		}
		else
		{
			if (cookieshowrankchanges[assister])
			{
				PrintToChat(assister,"\x05%s\x01 %t",CHATTAG,"yourAssisting",pointvalue,attackername)
			}
		}
	}
	...
	*/
	...
	//PrintToChatAll("the value of steamIdattacker is %s", steamIdattacker);
...
*/
	...
	if (pointmsgval == 1)
	{
	PrintToChatAll("\x05%s\x01 %t",CHATTAG,"Dying",victimname,diepointsvalue)
	}else
	{
	...
	if (nofakekill == true && isbot == false)
	{
	PrintToChat(victim,"\x05%s\x01 %t",CHATTAG,"yourDying",diepointsvalue)
	}
	}
	}
	}
	...
	Format(query, sizeof(query), "UPDATE Player SET POINTS = POINTS + %i, KILLS = KILLS + 1, KW_SntryL1 = KW_SntryL1 + 1 WHERE STEAMID = '%s'",pointvalue ,steamIdattacker);
	if (nofakekill == true && isbot == false)
	{
	SQL_TQuery(db,SQLErrorCheckCallback, query)
	}
	...
	{
	pointvalue = GetConVarInt(obj_sentrygunpoints)
	Format(query, sizeof(query), "UPDATE Player SET POINTS = POINTS + %i, KILLS = KILLS + 1, KW_SntryL2 = KW_SntryL2 + 1 WHERE STEAMID = '%s'",pointvalue ,steamIdattacker);
	if (nofakekill == true && isbot == false)
	{
	SQL_TQuery(db,SQLErrorCheckCallback, query)
	}
	...
	{
	pointvalue = GetConVarInt(obj_sentrygunpoints)
	Format(query, sizeof(query), "UPDATE Player SET POINTS = POINTS + %i, KILLS = KILLS + 1, KW_SntryL3 = KW_SntryL3 + 1 WHERE STEAMID = '%s'",pointvalue ,steamIdattacker);
	if (nofakekill == true && isbot == false)
	{
	SQL_TQuery(db,SQLErrorCheckCallback, query)
	}
	...
	Format(additional, sizeof(additional), "%t","withBackstab")
	if (isbot == false)
	{
	SQL_TQuery(db,SQLErrorCheckCallback, query)
	}
	...
	Format(additional, sizeof(additional), "%t","withHeadshot")
	if (isbot == false)
	{
	SQL_TQuery(db,SQLErrorCheckCallback, query)
	}
	...
	if (pointmsgval == 1)
	{
	PrintToChatAll("\x05%s\x01 %t",CHATTAG,"Killing",attackername,pointvalue,victimname,additional)
	}else
	{
	if (cookieshowrankchanges[attacker])
	{
	PrintToChat(attacker,"\x05%s\x01 %t",CHATTAG,"yourKilling",pointvalue,victimname,additional)
	}
	}
	}
	}
...
if (GetEventInt(event, "userid") != GetEventInt(event, "attacker"))
{
	...
	if (pointmsgval == 1)
	{
	PrintToChatAll("\x05%s\x01 %t",CHATTAG,"Destroying",username,pointvalue)
	}else
	{
	if (cookieshowrankchanges[user])
	{
	PrintToChat(user,"\x05%s\x01 %t",CHATTAG,"yourDestroying",pointvalue)
	}
	}
	}
}
...
public Action:Event_round_end(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	/*if (rankingenabled)
	{
	PrintToChatAll("\x05%s\x01 Ranking Disabled: round end",CHATTAG)
	}*/
	}
}
public Action:Event_teamplay_round_active(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	/*if (rankingenabled)
	{
	PrintToChatAll("\x05%s\x01 Ranking Enabled: round start",CHATTAG)
	}*/
	}
	}
}
	...
	if (cpmap)
{
	...
	PrintToChat(i,"\x05%s\x01 %t",CHATTAG,"teamCapturing",teamname,pointvalue)
	}
	}
	...
	if (rankingactive && rankingenabled)
{
	...
	for (new i=1; i<=l_maxplayers; i++)
	{
		...
		new String:teamname[MAX_LINE_WIDTH];
		GetTeamName(team,teamname, sizeof(teamname) );
		PrintToChat(i,"\x05%s\x01 %t",CHATTAG,"flagCapturing",teamname,pointvalue)
		}
		}
		}
	...
	if (blockerclient > 0)
{
	...
	if (pointmsgval == 1)
	{
	PrintToChatAll("\x05%s\x01 %t",CHATTAG,"BlockingCapture",playername,pointvalue)
	}else
	{
	if (cookieshowrankchanges[blockerclient])
	{
	PrintToChat(blockerclient,"\x05%s\x01 %t",CHATTAG,"yourBlockingCapture",pointvalue)
	}
	}
	}
}
...
{
	rankingactive = true
	//PrintToChatAll("\x05%s\x01 Ranking Enabled: enough players",CHATTAG)
}
}
}
}
...
public T_CheckConnectingUsr(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	PrintToChatAll("\x05%s\x01 %t",CHATTAG, "Welcome", clientname)
	}
	else
	{
		/*update name*/
...
public T_ShowrankConnectingUsr2(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	{
	PrintToChat(client,"%t", "yourRank", onconrank[client],rankedclients)
	}else if (GetConVarInt(showrankonconnect) == 2)
	{
	new String:clientname[MAX_LINE_WIDTH];
	GetClientName( client, clientname, sizeof(clientname) );
	PrintToChatAll("\x05%s\x01 %t",CHATTAG, "ConnectedFrom", clientname, country,onconrank[client],rankedclients)
...
public webranking(client)
{
	...
	ShowMOTDPanel(client, "TF2 Stats", showrankurl, 2)
	}
...
public Action:RankPanel(client,kills,death,killassi,playtime)
{
	...
	{
	PrintToChatAll("\x05%s\x01 %t",CHATTAG,"RankOfPlayer",ranknamereq[client],reqplayerrank[client],rankedclients)
	}
...
{
	rankingactive = false
	PrintToChatAll("\x05%s\x01 Ranking Disabled: not enough players",CHATTAG)
}
}
}
readcvars()
...
public OnConVarChangerank_enable(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	{
	rankingenabled = true
	PrintToChatAll("\x05%s\x01 Ranking Started",CHATTAG)

	}
	else if (!value)
	{
	PrintToChatAll("\x05%s\x01 Ranking Stopped",CHATTAG)
...
public Action:Event_player_extinguished(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (pointmsgval == 1)
	{
	PrintToChatAll("\x05%s\x01 %t",CHATTAG,"Extingushing",healername,pointvalue,extingushedname)
	}else
	{
	if (cookieshowrankchanges[healer])
	{
	PrintToChat(healer,"\x05%s\x01 %t",CHATTAG,"yourExtingushing",pointvalue,extingushedname)

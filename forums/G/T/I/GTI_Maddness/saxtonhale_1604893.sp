...
#define VagineerJump "saxton_hale/vagineer_responce_jump_"
#define VagineerRageSound2 "saxton_hale/vagineer_responce_rage_"			//1-4
#define VagineerKSpreeNew "saxton_hale/vagineer_responce_taunt_"
#define VagineerFail "saxton_hale/vagineer_responce_fail_"
...
//			AddFileToDownloadsTable(s);
	...
	AddFileToDownloadsTable(s);
//	Format(s,PLATFORM_MAX_PATH,"sound/%s",HaleKSpree2);	//broken sound
//	PrecacheSound(HaleKSpree2, true);
//	AddFileToDownloadsTable(s);
	...
	AddFileToDownloadsTable(s);
//	PrecacheSound(HaleKillEngie132, true);
//	Format(s,PLATFORM_MAX_PATH,"sound/%s",HaleKillEngie132);
//	AddFileToDownloadsTable(s);
	...
	AddFileToDownloadsTable(s);
//	PrecacheSound(HaleLastMan2Fixed, true);
...
public Action:Timer_MusicPlay(Handle:timer)
{
	...
	if (Special==4)
		MusicTimer = CreateTimer(138.0, Timer_MusicTheme, _, TIMER_FLAG_NO_MAPCHANGE|TIMER_REPEAT);
	else
...
/*stock bool:GetClientSoundOptions(client, Handle:cookie)
{
	if (!IsValidClient(client)) return false;
	if (IsFakeClient(client)) return true;
	if (!AreClientCookiesCached(client)) return true;
...
stock bool:CheckSoundException(client, excepttype)
{
	if (!IsValidClient(client)) return false;
	if (IsFakeClient(client)) return true;
	if (excepttype == SOUNDEXCEPT_VOICE) return !VoiceDisabled[client];
	else return !MusicDisabled[client];
}*/
...
//		case 526:
	...
	if (strncmp(classname, "tf_weapon_rocketlauncher", 24, false) == 0 || strcmp(classname, "tf_weapon_shotgun_soldier", false) == 0)
	{
			...
			if (class==TFClass_Medic)
			{
				if (weapon==GetPlayerWeaponSlot(client, 0))
				{
					...
					if (IsValidEdict(medigun))
					{
						...
						ShowHudText(client, -1,"%T: %i","uber-charge", client,RoundFloat(GetEntPropFloat(medigun, Prop_Send, "m_flChargeLevel")*100));
					}
				}
...
//					else if (see==1)
//						strcopy(s,PLATFORM_MAX_PATH,HaleKSpree2);	//broken line
					else if (see<5)
	...
	else if (RedAlivePlayers==1 && IsValidClient(Hale))
	{
		...
		if (Special!=2)
		{
			...
			else
			{
				...
				if (!see)
					strcopy(s,PLATFORM_MAX_PATH,Arms);
				else if (see==1)
	...
	else if (attacker!=Hale && client==Hale)
	{
		if (attacker<=MaxClients)
		{
			if (inflictor == attacker)
			{
				...
				{
					new index = (IsValidEntity(activeweapon) && activeweapon > MaxClients ? GetEntProp(activeweapon, Prop_Send, "m_iItemDefinitionIndex") : -1);
				...
				if (activeweapon == GetPlayerWeaponSlot(attacker, 2) && damage > 1000.0)	//lousy way of checking backstabs
				{
					...
					if (GetEdictClassname(activeweapon, wepclassname, sizeof(wepclassname)) && strcmp(wepclassname, "tf_weapon_knife", false) == 0)	//more robust knife check
					{
						...
						new eternalreward = (IsValidEntity(knife) && knife > MaxClients ? GetEntProp(knife, Prop_Send, "m_iItemDefinitionIndex") : -1);
		...
		//						break;
			...
			{
				new idx = (IsValidEntity(weapon) && weapon > MaxClients ? GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex") : -1);
				if (idx == 45 || ((idx == 209 || idx == 294 || idx == 23 || idx == 160 || idx == 449) && (TF2_IsPlayerCritBuffed(client) || TF2_IsPlayerInCondition(client, TFCond_CritCola) || TF2_IsPlayerInCondition(client, TFCond_Buffed) || TF2_IsPlayerInCondition(client, TFCond_CritHype))))
				{
	...
	if (IsValidClient(param1))
	{
		if (action == MenuAction_Select)
		{
			if (param2==2)
			{
				...
				StopSound(param1, SNDCHAN_AUTO, CBSTheme);
			}
			else

...
public void OnPluginStart()
{
	...
	tacolor = CreateConVar("rf_tag_color", "{GREEN}", "Color of the chat tag of the Discord channel shown in game, using Chat Processor colors.", FCVAR_PROTECTED);
	
	nacolor = CreateConVar("rf_name_color", "{PURPLE}", "Color of the name of the Discord users shown in game, using Chat Processor colors.", FCVAR_PROTECTED);
	
	chcolor = CreateConVar("rf_chat_color", "{DEFAULT}", "Color of the chat from Discord shown in game, using Chat Processor colors.", FCVAR_PROTECTED);
...
void ParseMessageFrame(const char[] frame)
{
	...
	switch (eVer)
	{
		case Engine_TF2, Engine_CSS, Engine_HL2DM, Engine_DODS:
			...
			CPrintToChatAll("{%s}[%s] {%s}%s{%s}: %s", tacolor, nacolor, chcolor, hostname, name, sContent);
		default:
			CPrintToChatAll("{%s}[%s] {%s}%s{%s}: %s", tacolor, nacolor, chcolor, hostname, name, sContent);
	}
}

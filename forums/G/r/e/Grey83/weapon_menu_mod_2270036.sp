...
public Plugin:myinfo = { name = "[NMRiH] Weapon Menu", author = "Marcus (reordered by Grey83)", description = "Allows players to select their own weapons to kill zombies.", version = Plugin_Version, url = "http://www.sourcemod.com"};
...
public Action:Command_WeaponMenu(iClient, iArgs)
{
	if (IsPlayerAlive(iClient))
	{
		...
		AddMenuItem(hMenu, "0", "Medical Supplies", ((g_iMedicalLimit > 0) && (g_iMedicalCount[iClient] < g_iMedicalLimit)) && (ItemQuantity("medic") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "1", "Tools", ((g_iToolLimit > 0) && (g_iToolCount[iClient] < g_iToolLimit)) && (ItemQuantity("tool") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "2", "Pistols", ((g_iPistolLimit > 0) && (g_iPistolCount[iClient] < g_iPistolLimit)) && (ItemQuantity("pistol") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "3", "SubMachineGuns", ((g_iSubMachineGunLimit > 0) && (g_iSubMachineGunCount[iClient] < g_iSubMachineGunLimit)) && (ItemQuantity("smg") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "4", "Rifles", ((g_iRifleLimit > 0) && (g_iRifleCount[iClient] < g_iRifleLimit)) && (ItemQuantity("rifle") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "5", "Shotguns", ((g_iShotgunLimit > 0) && (g_iShotgunCount[iClient] < g_iShotgunLimit)) && (ItemQuantity("shotgun") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "6", "Melee", ((g_iMeleeLimit > 0) && (g_iMeleeCount[iClient] < g_iMeleeLimit)) && (ItemQuantity("melee") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);

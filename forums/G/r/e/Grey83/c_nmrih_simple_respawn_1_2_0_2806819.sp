...
public int MenuHandle_RespawnTop(Menu menu, MenuAction action, int client, int select){
	switch(action){
		case MenuAction_Select:{
			switch(select){
				case 0:	if(IsValidClient(client)) RespawnMenuShowToClient(client);
				case 1:	if(IsValidClient(client)) RespawnClient(client);
				case 2:	RespawnAll();
...
public int MenuHandle_RespawnPlayers(Menu menu, MenuAction action, int client, int select){
	switch(action){
		case MenuAction_Select:{
			...
			if(IsValidClient(player)) RespawnClient(player);
...
int RespawnAll(){
	int count = 0;
	for(int client = 1; client <= MaxClients; client++) if(IsValidClient(client) && RespawnClient(client)) count++;
	return count;
}

...
static const char
	PL_NAME[]	= "Team Bets",
	PL_VER[]	= "2.8.1_21.03.2023 (rewritten by Grey83)",
...
public Action Cmd_Say(int client, const char[] command, int argc)
{
	...
	StripQuotes(buffer);
	TrimString(buffer);
	if(strcmp(buffer, "bet", false))
		return Plugin_Continue;

	if(argc < 3)
	{
		PrintToChat(client, "%s\x01Usage: \x02bet <t|ct> <all|half|third|1337>", PREFIX);
		return Plugin_Handled;
	}

	if(bCSGO && GameRules_GetProp("m_bWarmupPeriod"))
	{
		PrintToChat(client, "%s\x01No bets during the Warmup", PREFIX);
		return Plugin_Handled;
	}
	...
	StripQuotes(buffer);
	TrimString(buffer);
	if(strcmp(buffer,"ct", false) && strcmp(buffer,"t", false))
	{
		PrintToChat(client, "%s\x01%t", PREFIX, "Invalid Team for Bet");
		return Plugin_Handled;
	}

	iBets[client][B_Team] = buffer[0] == 'c' ? 3 : 2;
	...
	{
		iAmount = StringToInt(buffer);
	}
	else if(!strcmp(buffer,"all",false))
	{
		iAmount = iBank;
	}
	else if(!strcmp(buffer,"half", false))
	{
		iAmount = RoundToNearest((iBank * 0.5));
	}
	else if(!strcmp(buffer,"third", false))
	{
		iAmount = RoundToNearest((iBank / 3.0));
	}

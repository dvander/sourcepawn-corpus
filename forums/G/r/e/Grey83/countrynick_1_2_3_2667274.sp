...
$Id: countrynick.sp 2019-09-17 Grey83 $
 */
...
public Action Cmd_List(int client, int args)
{
	...
	for(int i = 1, admin, num, pos; i <= MaxClients; i++) if(IsClientInGame(i))
	{
		num++;
		if(!IsFakeClient(i))
		{
		...
		pos = GetPos(name);
		PrintToConsole(client, "%2.2d %c %3.3d %-17.17s %-15.15s %-4.4s %-30.30s", num, admin, GetClientUserId(i), SId, IP, find ? code : "-?-", name[pos]);
	}
...
stock void SetNewName(int client, char[] name)
{
	...
	static char code[4];
	if(bLongTag) GeoipCode3(ip, code);
	else GeoipCode2(ip, code);

	static int pos;
	pos = GetPos(name);
	Format(name, 69, "%c%s%c%s", HOOKS[1], code, HOOKS[1], name[pos]);

...
public RegMenuCommands()
{
	...
	RegConsoleCmd("sm_entcontrol_info", Command_EntControl_Info, "EntControl Info");
}
...
stock GenerateMenu_Main(client)
{
	...
	if(ValidSelect(gSelectedEntity[client]))
	{
		...
		{
			SetMenuPagination(entcontrol, MENU_NO_PAGINATION);
			if(StrEqual(classname, "player"))
				GetClientName(selectedEntity, classname, 32);
		}
		
		SetMenuTitle(entcontrol, "%s%s:", classname, SDKHooksLoaded ? "" : " (SDK-Hooks not loaded!)");
...
public Action:Command_EntControl_Info(client, args)
{
...
stock GenerateMenu_About(client)
{
	...
	{
		case 0:
			AddMenuItem(entcontrol, "MOVETYPE_NONE", "Movetype:\n     MOVETYPE_NONE"); // never moves
		case 1:
			AddMenuItem(entcontrol, "MOVETYPE_ANGLENOCLIP", "Movetype:\n     MOVETYPE_ANGLENOCLIP");
		case 2:
			AddMenuItem(entcontrol, "MOVETYPE_ANGLECLIP", "Movetype:\n     MOVETYPE_ANGLECLIP");
		case 3:
			AddMenuItem(entcontrol, "MOVETYPE_WALK", "Movetype:\n     MOVETYPE_WALK"); // Player only - moving on the ground
		case 4:
			AddMenuItem(entcontrol, "MOVETYPE_STEP", "Movetype:\n     MOVETYPE_STEP"); // gravity, special edge handling -- monsters use this
		case 5:
			AddMenuItem(entcontrol, "MOVETYPE_FLY", "Movetype:\n     MOVETYPE_FLY"); // No gravity, but still collides with stuff
		case 6:
			AddMenuItem(entcontrol, "MOVETYPE_TOSS", "Movetype:\n     MOVETYPE_TOSS"); // gravity/collisions
		case 7:
			AddMenuItem(entcontrol, "MOVETYPE_PUSH", "Movetype:\n     MOVETYPE_PUSH"); // no clip to world, push and crush
		case 8:
			AddMenuItem(entcontrol, "MOVETYPE_NOCLIP", "Movetype:\n     MOVETYPE_NOCLIP"); // No gravity, no collisions, still do velocity/avelocity
		case 9:
			AddMenuItem(entcontrol, "MOVETYPE_FLYMISSILE", "Movetype:\n     MOVETYPE_FLYMISSILE"); // extra size to monsters
		case 10:
			AddMenuItem(entcontrol, "MOVETYPE_BOUNCE", "Movetype:\n     MOVETYPE_BOUNCE"); // Just like Toss, but reflect velocity when contacting surfaces
		case 11:
			AddMenuItem(entcontrol, "MOVETYPE_BOUNCEMISSILE", "Movetype:\n     MOVETYPE_BOUNCEMISSILE"); // bounce w/o gravity
		case 12:
			AddMenuItem(entcontrol, "MOVETYPE_FOLLOW", "Movetype:\n     MOVETYPE_FOLLOW"); // track movement of aiment
		case 13:
			AddMenuItem(entcontrol, "MOVETYPE_PUSHSTEP", "Movetype:\n     MOVETYPE_PUSHSTEP"); // BSP model that needs physics/world collisions (uses nearest hull for world collision)
	}
	...
	if(iBuffer != -1)
	{
		...
		Format(sBuffer2, 128, "OwnerEntity:\n     %s", sBuffer);
		AddMenuItem(entcontrol, sBuffer, sBuffer2);
	}
	else
		AddMenuItem(entcontrol, "No Owner-Entity", "OwnerEntity:\n     No Owner-Entity");
	...
	if(iBuffer != -1)
	{
		GetEntPropString(iBuffer, Prop_Data, "m_iName", sBuffer, 64);
		Format(sBuffer, 64, "%s (%i)", sBuffer, iBuffer);
		Format(sBuffer2, 128, "EffectEntity:\n     %s", sBuffer);
		AddMenuItem(entcontrol, sBuffer, sBuffer2);
	}
	else
		AddMenuItem(entcontrol, "No Effect-Entity", "EffectEntity:\n     No Effect-Entity");
	
	// PAGE 4
	// Take Damage
	if(GetEntProp(ent, Prop_Data, "m_takedamage") == DAMAGE_YES)
		AddMenuItem(entcontrol, "YES", "Take Damage:\n     Yes");
	else
		AddMenuItem(entcontrol, "NO", "Take Damage:\n     No");
	...
	if(iBuffer)
	{
		GetEntPropString(iBuffer, Prop_Data, "m_iName", sBuffer, 64);
		Format(sBuffer, 64, "%s (%i)", sBuffer, iBuffer);
		Format(sBuffer2, 128, "Parent Attachment:\n     %s", sBuffer);
		AddMenuItem(entcontrol, sBuffer, sBuffer2);
	}
	else
		AddMenuItem(entcontrol, "No Parent Attachment", "Parent Attachment:\n     None");
	...
	if(IsCreature(ent))
	{
		...
		if(iBuffer & FL_ONGROUND)
			AddMenuItem(entcontrol, "FL_ONGROUND", "Flags:\n     FL_ONGROUND");
		else if(iBuffer & FL_DUCKING)
			AddMenuItem(entcontrol, "FL_DUCKING", "Flags:\n     FL_DUCKING");
		else if(iBuffer & FL_WATERJUMP)
			AddMenuItem(entcontrol, "FL_WATERJUMP", "Flags:\n     FL_WATERJUMP");
		else if(iBuffer & FL_ONTRAIN)
			AddMenuItem(entcontrol, "FL_ONTRAIN", "Flags:\n     FL_ONTRAIN");
		else if(iBuffer & FL_INRAIN)
			AddMenuItem(entcontrol, "FL_INRAIN", "Flags:\n     FL_INRAIN");
		else if(iBuffer & FL_FROZEN)
			AddMenuItem(entcontrol, "FL_FROZEN", "Flags:\n     FL_FROZEN");
		else if(iBuffer & FL_ATCONTROLS)
			AddMenuItem(entcontrol, "FL_ATCONTROLS", "Flags:\n     FL_ATCONTROLS");
		else if(iBuffer & FL_CLIENT)
			AddMenuItem(entcontrol, "FL_CLIENT", "Flags:\n     FL_CLIENT");
		else if(iBuffer & FL_FAKECLIENT)
			AddMenuItem(entcontrol, "FL_FAKECLIENT", "Flags:\n     FL_FAKECLIENT");
		else if(iBuffer & FL_INWATER)
			AddMenuItem(entcontrol, "FL_INWATER", "Flags:\n     FL_INWATER");
		else if(iBuffer & FL_FLY)
			AddMenuItem(entcontrol, "FL_FLY", "Flags:\n     FL_FLY");
		else if(iBuffer & FL_SWIM)
			AddMenuItem(entcontrol, "FL_SWIM", "Flags:\n     FL_SWIM");
		else if(iBuffer & FL_CONVEYOR)
			AddMenuItem(entcontrol, "FL_CONVEYOR", "Flags:\n     FL_CONVEYOR");
		else if(iBuffer & FL_NPC)
			AddMenuItem(entcontrol, "FL_NPC", "Flags:\n     FL_NPC");
		else if(iBuffer & FL_GODMODE)
			AddMenuItem(entcontrol, "FL_GODMODE", "Flags:\n     FL_GODMODE");
		else if(iBuffer & FL_NOTARGET)
			AddMenuItem(entcontrol, "FL_NOTARGET", "Flags:\n     FL_NOTARGET");
		else if(iBuffer & FL_AIMTARGET)
			AddMenuItem(entcontrol, "FL_AIMTARGET", "Flags:\n     FL_AIMTARGET");
		else if(iBuffer & FL_PARTIALGROUND)
			AddMenuItem(entcontrol, "FL_PARTIALGROUND", "Flags:\n     FL_PARTIALGROUND");
		else if(iBuffer & FL_STATICPROP)
			AddMenuItem(entcontrol, "FL_STATICPROP", "Flags:\n     FL_STATICPROP");
		else if(iBuffer & FL_GRAPHED)
			AddMenuItem(entcontrol, "FL_GRAPHED", "Flags:\n     FL_GRAPHED");
		else if(iBuffer & FL_GRENADE)
			AddMenuItem(entcontrol, "FL_GRENADE", "Flags:\n     FL_GRENADE");
		else if(iBuffer & FL_STEPMOVEMENT)
			AddMenuItem(entcontrol, "FL_STEPMOVEMENT", "Flags:\n     FL_STEPMOVEMENT");
		else if(iBuffer & FL_DONTTOUCH)
			AddMenuItem(entcontrol, "FL_DONTTOUCH", "Flags:\n     FL_DONTTOUCH");
		else if(iBuffer & FL_BASEVELOCITY)
			AddMenuItem(entcontrol, "FL_BASEVELOCITY", "Flags:\n     FL_BASEVELOCITY");
		else if(iBuffer & FL_WORLDBRUSH)
			AddMenuItem(entcontrol, "FL_WORLDBRUSH", "Flags:\n     FL_WORLDBRUSH");
		else if(iBuffer & FL_OBJECT)
			AddMenuItem(entcontrol, "FL_OBJECT", "Flags:\n     FL_OBJECT");
		else if(iBuffer & FL_KILLME)
			AddMenuItem(entcontrol, "FL_KILLME", "Flags:\n     FL_KILLME");
		else if(iBuffer & FL_ONFIRE)
			AddMenuItem(entcontrol, "FL_ONFIRE", "Flags:\n     FL_ONFIRE");
		else if(iBuffer & FL_DISSOLVING)
			AddMenuItem(entcontrol, "FL_DISSOLVING", "Flags:\n     FL_DISSOLVING");
		else if(iBuffer & FL_TRANSRAGDOLL)
			AddMenuItem(entcontrol, "FL_TRANSRAGDOLL", "Flags:\n     FL_TRANSRAGDOLL");
	}
	else
		AddMenuItem(entcontrol, "NOT-ALIVE", "Flags:\n     NOT-ALIVE");
	...
	if(EntControlExtLoaded)
	{
		...
		if(KvJumpToKey(kvEnts, sClassname) && KvJumpToKey(kvEnts, "output"))
		{
			...
			AddMenuItem(entcontrol, "Entity is currently not supported.", "Outputs:\n     Entity is currently not supported.");
	}
	else
		AddMenuItem(entcontrol, "EntControl-Extension not loaded!", "Outputs:\n     EntControl-Extension not loaded!");
...
public Menu_EC_About_AddOutputs(Handle:entcontrol, ent, String:sOutput[32])
{
	...
	if(count > -1)
	{
		...
		else
		{
			...
			Format(sBuffer2, 128, "%s\n     Could not receive output!", sBuffer);
			AddMenuItem(entcontrol, sBuffer, sBuffer2);
		}
	}
}

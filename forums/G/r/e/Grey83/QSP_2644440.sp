...
#define PLUGIN_AUTHOR		"Spartan_C001(fork by Dragokas)"
...
/*
	...
	3.5.6(22-Mar-2019)
	 - Fixed error 'Invalid convar handle 0'. Fix by Grey83
...
public OnPluginStart()
{
	...
	CreateConVar("sm_quakesoundsv3_announce","0","Sets whether to announcement to clients as they join, 0=Disabled, 1=Enabled.",FCVAR_NONE,true,0.0,true,1.0)
	CreateConVar("sm_quakesoundsv3_text","0","Default text display setting for new users, 0=Disabled, 1=Enabled.",FCVAR_NONE,true,0.0,true,1.0)
	...
	if(lateLoaded) LateLoadedInitialization()
...
public LateLoadedInitialization()
{
	...
	for(new i = 1, max = GetMaxHumanPlayers(); i <= max; i++) 
	{
		if(IsClientInGame(i) && !IsFakeClient(i))
		{
			...
			if(AreClientCookiesCached(i)) LoadClientCookiesFor(i)
		}
		else
		{
			soundPreference[i] = -1
...
void MixSets()
{
	for(int i, j; i < numSets; i++)
	{
		for(j = 0; j < MAX_NUM_KILLS; j++)
		{
...
public LoadSet(String:setFile[],setNum)
{
	...
	if(FileToKeyValues(SetFileKV,setFile))
	{
		if(KvJumpToKey(SetFileKV,"headshot"))
		{
			if(KvGotoFirstSubKey(SetFileKV))
			{
				...
				do
				{
					...
					if((killNum = StringToInt(bufferString)) >= 0)
					{
			...
			else PrintToServer("[SM] Quake Sounds v3: 'headshot' section not configured correctly in %s.",setFile)
		}
		else PrintToServer("[SM] Quake Sounds v3: 'headshot' section missing in %s.",setFile)
		KvRewind(SetFileKV)
		...
		else PrintToServer("[SM] Quake Sounds v3: 'grenade' section missing in %s.",setFile)
		KvRewind(SetFileKV)
		...
		else PrintToServer("[SM] Quake Sounds v3: 'selfkill' section missing in %s.",setFile)
		KvRewind(SetFileKV)
		...
		else PrintToServer("[SM] Quake Sounds v3: 'round play' section missing in %s.",setFile)
		KvRewind(SetFileKV)
		...
		else PrintToServer("[SM] Quake Sounds v3: 'knife' section missing in %s.",setFile)
		KvRewind(SetFileKV)
		if(KvJumpToKey(SetFileKV,"killsound"))
		{
			...
			else PrintToServer("[SM] Quake Sounds v3: 'killsound' section not configured correctly in %s.",setFile)
		}
		else PrintToServer("[SM] Quake Sounds v3: 'killsound' section missing in %s.",setFile)
		KvRewind(SetFileKV)
		...
		else PrintToServer("[SM] Quake Sounds v3: 'first blood' section missing in %s.",setFile)
		KvRewind(SetFileKV)
		...
		else PrintToServer("[SM] Quake Sounds v3: 'teamkill' section missing in %s.",setFile)
		KvRewind(SetFileKV)
		if(KvJumpToKey(SetFileKV,"combo"))
		{
			...
			else PrintToServer("[SM] Quake Sounds v3: 'combo' section not configured correctly in %s.",setFile)
		}
		else PrintToServer("[SM] Quake Sounds v3: 'combo' section missing in %s.",setFile)
		KvRewind(SetFileKV)
		...
		else PrintToServer("[SM] Quake Sounds v3: 'join server' section missing in %s.",setFile)
	}
	else PrintToServer("[SM] Quake Sounds v3: Cannot parse '%s', file not found or incorrectly structured!",setFile)
...
public EventRoundStart(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if(gameEngine != Engine_HL2DM) NewRoundInitialization()
	char g_sGameMode[32];
	if(g_hMPGameMode) g_hMPGameMode.GetString(g_sGameMode, sizeof(g_sGameMode));
...
{
	for(new i = 1, max = GetMaxHumanPlayers(); i <= max; i++) if(IsClientInGame(i) && !IsFakeClient(i) && soundPreference[i] >= 0)
	{
...
public OnClientPostAdminCheck(client)
{
	...
	if(!IsFakeClient(client))
	{
		...
		if(GetConVarBool(FindConVar("sm_quakesoundsv3_announce"))) CreateTimer(30.0,TimerAnnounce,client)
		if(soundPreference[client] >= 0 && (!StrEqual(joinSound[soundPreference[client]],"") && (joinConfig[soundPreference[client]] & 1) || (joinConfig[soundPreference[client]] & 2) || (joinConfig[soundPreference[client]] & 4)))
...
{
	if(IsClientInGame(client)) PrintToChat(client,"%t","announce message")
}
...
public LoadClientCookiesFor(client)
{
	...
	if(buffer[0]) textPreference[client] = StringToInt(buffer)
	GetClientCookie(client,cookieSoundPref,buffer,5)
	if(buffer[0]) soundPreference[client] = StringToInt(buffer)
}
...
public EventPlayerDeath(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	else
	{
		if(attackerClient == victimClient || attackerClient == 0)
		{
			...
			for(new i = 1; i <= GetMaxHumanPlayers(); i++) if(IsClientInGame(i) && !IsFakeClient(i) && soundPreference[i] >= 0 && (!g_bCoopMode ||(g_bCoopMode && GetClientTeam(victimClient) == 2 )))
			{
				if(selfkillSound[soundPreference[i]][0] && ((selfkillConfig[soundPreference[i]] & 1) || ((selfkillConfig[soundPreference[i]] & 2) && attackerClient == i) || ((selfkillConfig[soundPreference[i]] & 4) && victimClient == i)))
					EmitSoundCustom(i,selfkillSound[soundPreference[i]],_,_,_,_,GetConVarFloat(FindConVar("sm_quakesoundsv3_volume")))
				if(textPreference[i] && ((selfkillConfig[soundPreference[i]] & 8) || ((selfkillConfig[soundPreference[i]] & 16) && attackerClient == i) || ((selfkillConfig[soundPreference[i]] & 32) && victimClient == i)))
		...
		{
			consecutiveKills[attackerClient] = 0
			for(new i = 1; i <= GetMaxHumanPlayers(); i++) if(IsClientInGame(i) && !IsFakeClient(i) && soundPreference[i] >= 0)
			{
				if(teamkillSound[soundPreference[i]][0] && ((teamkillConfig[soundPreference[i]] & 1) || ((teamkillConfig[soundPreference[i]] & 2) && attackerClient == i) || ((teamkillConfig[soundPreference[i]] & 4) && victimClient == i)))
					EmitSoundCustom(i,teamkillSound[soundPreference[i]],_,_,_,_,GetConVarFloat(FindConVar("sm_quakesoundsv3_volume")))
				if(textPreference[i] && ((teamkillConfig[soundPreference[i]] & 8) || ((teamkillConfig[soundPreference[i]] & 16) && attackerClient == i) || ((teamkillConfig[soundPreference[i]] & 32) && victimClient == i)))
		...
		else
		{
			...
			new bool:firstblood, bool:headshot, bool:knife, bool:grenade, bool:combo, customkill, String:weapon[GetMaxHumanPlayers()]
			...
			if(headshot) consecutiveHeadshots[attackerClient]++
			...
			{
				if(StrEqual(weapon,"grenade_frag")) grenade = true
				else if((StrEqual(weapon,"stunstick") || StrEqual(weapon,"crowbar"))) knife = true
			}
			for(new i = 1, max = GetMaxHumanPlayers(); i <= max; i++)
			{
				if(IsClientInGame(i) && !IsFakeClient(i) && soundPreference[i] >= 0)
				{
					...
					{
						if(firstbloodSound[soundPreference[i]][0] && ((firstbloodConfig[soundPreference[i]] & 1) || ((firstbloodConfig[soundPreference[i]] & 2) && attackerClient == i) || ((firstbloodConfig[soundPreference[i]] & 4) && victimClient == i)))
							EmitSoundCustom(i,firstbloodSound[soundPreference[i]],_,_,_,_,GetConVarFloat(FindConVar("sm_quakesoundsv3_volume")))
						if(textPreference[i] && ((firstbloodConfig[soundPreference[i]] & 8) || ((firstbloodConfig[soundPreference[i]] & 16) && attackerClient == i) || ((firstbloodConfig[soundPreference[i]] & 32) && victimClient == i)))
							PrintCenterText(i,"%t","first blood",attackerName)
					}
					else if(headshot && headshotConfig[soundPreference[i]][0] > 0)
					{
						if(headshotSound[soundPreference[i]][0][0] && ((headshotConfig[soundPreference[i]][0] & 1) || ((headshotConfig[soundPreference[i]][0] & 2) && attackerClient == i) || ((headshotConfig[soundPreference[i]][0] & 4) && victimClient == i)))
							EmitSoundCustom(i,headshotSound[soundPreference[i]][0],_,_,_,_,GetConVarFloat(FindConVar("sm_quakesoundsv3_volume")))
						if(textPreference[i] && ((headshotConfig[soundPreference[i]][0] & 8) || ((headshotConfig[soundPreference[i]][0] & 16) && attackerClient == i) || ((headshotConfig[soundPreference[i]][0] & 32) && victimClient == i)))
					...
					{
						if(headshotSound[soundPreference[i]][consecutiveHeadshots[attackerClient]][0] && ((headshotConfig[soundPreference[i]][consecutiveHeadshots[attackerClient]] & 1) || ((headshotConfig[soundPreference[i]][consecutiveHeadshots[attackerClient]] & 2) && attackerClient == i) || ((headshotConfig[soundPreference[i]][consecutiveHeadshots[attackerClient]] & 4) && victimClient == i)))
						...
						if(textPreference[i] && consecutiveHeadshots[attackerClient] < MAX_NUM_KILLS && ((headshotConfig[soundPreference[i]][consecutiveHeadshots[attackerClient]] & 8) || ((headshotConfig[soundPreference[i]][consecutiveHeadshots[attackerClient]] & 16) && attackerClient == i) || ((headshotConfig[soundPreference[i]][consecutiveHeadshots[attackerClient]] & 32) && victimClient == i)))
						{
					...
					{
						if(knifeSound[soundPreference[i]][0] && ((knifeConfig[soundPreference[i]] & 1) || ((knifeConfig[soundPreference[i]] & 2) && attackerClient == i) || ((knifeConfig[soundPreference[i]] & 4) && victimClient == i)))
							EmitSoundCustom(i,knifeSound[soundPreference[i]],_,_,_,_,GetConVarFloat(FindConVar("sm_quakesoundsv3_volume")))
						if(textPreference[i] && ((knifeConfig[soundPreference[i]] & 8) || ((knifeConfig[soundPreference[i]] & 16) && attackerClient == i) || ((knifeConfig[soundPreference[i]] & 32) && victimClient == i)))
							PrintCenterText(i,"%t","knife",attackerName,victimName)
					}
					else if(grenade && grenadeConfig[soundPreference[i]] > 0)
					{
						if(grenadeSound[soundPreference[i]][0] && ((grenadeConfig[soundPreference[i]] & 1) || ((grenadeConfig[soundPreference[i]] & 2) && attackerClient == i) || ((grenadeConfig[soundPreference[i]] & 4) && victimClient == i)))
							EmitSoundCustom(i,grenadeSound[soundPreference[i]],_,_,_,_,GetConVarFloat(FindConVar("sm_quakesoundsv3_volume")))
						if(textPreference[i] && ((grenadeConfig[soundPreference[i]] & 8) || ((grenadeConfig[soundPreference[i]] & 16) && attackerClient == i) || ((grenadeConfig[soundPreference[i]] & 32) && victimClient == i)))
					...
					{
						if(comboSound[soundPreference[i]][comboScore[attackerClient]][0] && ((comboConfig[soundPreference[i]][comboScore[attackerClient]] & 1) || ((comboConfig[soundPreference[i]][comboScore[attackerClient]] & 2) && attackerClient == i) || ((comboConfig[soundPreference[i]][comboScore[attackerClient]] & 4) && victimClient == i)))
						...
						if(textPreference[i] && comboScore[attackerClient] < MAX_NUM_KILLS && ((comboConfig[soundPreference[i]][comboScore[attackerClient]] & 8) ||((comboConfig[soundPreference[i]][comboScore[attackerClient]] & 16) && attackerClient == i) || ((comboConfig[soundPreference[i]][comboScore[attackerClient]] & 32) && victimClient == i)))
						{
					...
					{
						if(consecutiveKills[attackerClient] < MAX_NUM_KILLS && killSound[soundPreference[i]][consecutiveKills[attackerClient]][0] && ((killConfig[soundPreference[i]][consecutiveKills[attackerClient]] & 1) || ((killConfig[soundPreference[i]][consecutiveKills[attackerClient]] & 2) && attackerClient == i) || ((killConfig[soundPreference[i]][consecutiveKills[attackerClient]] & 4) && victimClient == i)))
						...
						if(textPreference[i] && consecutiveKills[attackerClient] < MAX_NUM_KILLS && ((killConfig[soundPreference[i]][consecutiveKills[attackerClient]] & 8) || ((killConfig[soundPreference[i]][consecutiveKills[attackerClient]] & 16) && attackerClient == i) || ((killConfig[soundPreference[i]][consecutiveKills[attackerClient]] & 32) && victimClient == i)))
						{
...
public ShowQuakeMenu(client)
{
	...
	if(!textPreference[client]) Format(buffer,100,"%T","enable text",client)
	else Format(buffer,100,"%T","disable text",client)
	AddMenuItem(menu,"text pref",buffer)
	if(soundPreference[client] == -1) Format(buffer,100,"%T(%T)","no quake sounds",client, "enabled", client)
	else Format(buffer,100,"%T","no quake sounds",client)
	AddMenuItem(menu,"no sounds",buffer)
	for(new set; set < numSets; set++) 
	{
		if(soundPreference[client] == set) Format(buffer,50,"%T(%T)",setsName[set],client, "enabled", client)
		else Format(buffer,50,"%T",setsName[set],client)
		AddMenuItem(menu,"sound set",buffer)
	}
	if(soundPreference[client] == numSets) Format(buffer,100,"%T(%T)","sound mixed",client, "enabled", client)
	else Format(buffer,100,"%T","sound mixed",client)
...
public MenuHandlerQuake(Handle:menu,MenuAction:action,param1,param2)
{
	...
	{
		if(!param2) textPreference[param1] = 1 - textPreference[param1]
		else if(param2 == 1) soundPreference[param1] = -1
		else soundPreference[param1] = param2 - 2
		new String:buffer[5]
...
stock PrecacheSoundCustom(String:soundFile[],maxLength)
{
	...
	else PrecacheSound(soundFile,true)
}

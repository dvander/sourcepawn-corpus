...
public void CVarChanged_Timer(ConVar cvar, const char[] oldValue, const char[] newValue)
{
	...
	if(fTimer >= 1) CleaningMessage(0, RemoveItems(iLimit), Type_Item, true);
	OnMapStart();
}
...
{
	if(fTimer >= 1) hTimer = CreateTimer(fTimer, Timer_AutoClear, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
}
...
{
	for(int i = 1; i <= MaxClients; i++) if(IsClientInGame(i)) return;
	ClearMap();
}
...
public Action Timer_AutoClear(Handle timer)
{

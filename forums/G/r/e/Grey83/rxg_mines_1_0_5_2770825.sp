...
//
// Changelog:
// 6:36 AM 02/08/2022 - 1.0.5 by Grey83
//   fixed compatibility with new versions of SM
...
new Handle:sm_pp_tripmines,		// number of mines each player gets per round
	Handle:sm_pp_minedmg,		// damage of the mines
	Handle:sm_pp_minerad,		// radius override for explosion(0=disable)

	Handle:sm_pp_minefilter,	// detonation mode

#define sm_pp_tripmines_desc	"Number of mines each player gets per round"

	num_mines[MAXPLAYERS+1],	// number of mines per player
...
{
	sm_pp_tripmines = CreateConVar("sm_pp_tripmines", "3", sm_pp_tripmines_desc, _, true);
	sm_pp_minedmg = CreateConVar("sm_pp_minedmg", "100", "damage(magnitude) of the tripmines", _, true);
	sm_pp_minerad = CreateConVar("sm_pp_minerad", "0", "override for explosion damage radius", _, true);

	sm_pp_minefilter = CreateConVar("sm_pp_minefilter", "0", "0 = detonate when laser touches anyone, 1 = enemies and owner only, 2 = enemies only", _, true, _, true, 2.0);
	...
	if(strlen(ALTCOMMAND)) RegConsoleCmd(ALTCOMMAND, Command_Mine);
...
{
	if(strcmp(oldval, newval)) ClampMines();
}

public CVarChanged_minefilter(Handle:cvar, const String:oldval[], const String:newval[])
{
	if(strcmp(oldval, newval)) minefilter = GetConVarInt(sm_pp_minefilter);
}
...
public DeletePlacedMines(client)
{
	...
	while((ent = FindEntityByClassname(ent, "prop_physics_override")) != -1)
	{
		...
		if(!strncmp(name, "rxgtripmine", 11, true)
		// slight hack here, cant use owner entity because it wont allow the owner to destroy his own mines.
		&& GetEntPropEnt(ent, Prop_Data, "m_hLastAttacker") == client)
	...
	{
		GetEntPropString(ent, Prop_Data, "m_iName", name, 32);
		if(!strncmp(name, "rxgtripmine", 11, true) && GetEntPropEnt(ent, Prop_Data, "m_hOwnerEntity") == client)
			AcceptEntityInput(ent, "Kill");
	}
}
...
{
	for(new i, mines = GetConVarInt(sm_pp_tripmines); i <= MaxClients; i++) num_mines[i] = mines;
}
...
{
	num_mines[client] = GetConVarInt(sm_pp_tripmines);
}
...
public ClampMines()
{
	for(new i, mines = GetConVarInt(sm_pp_tripmines); i <= MaxClients; i++) if(num_mines[i] > mines) num_mines[i] = mines;
}
...
public Action:Command_Mine(client, args)
{
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		if(num_mines[client] > 0)
			PlaceMine(client);	// plant mine
		else PrintCenterText(client, GetConVarInt(sm_pp_tripmines) ? "You have no more mines." : "Mines are disabled.");
	}
...
public PlaceMine(client)
{
	...
	for(; i < 3; i++)		trace_start[i] += trace_end[i] * TRACE_START;
	for(i = 0; i < 3; i++)	trace_end[i] = trace_start[i] + trace_end[i] * TRACE_LENGTH;
	...
	if(TR_DidHit(INVALID_HANDLE))
	{
		...
		else PrintCenterText(client, "That was your last mine!");
	...
	else PrintCenterText(client, "Invalid mine position.");
}
...
public MineLaser_OnTouch(const String:output[], caller, activator, Float:delay)
{
	...
	if(!IsValidClient(activator) || !IsPlayerAlive(activator)) return;
	...
	if(minefilter)
	{
		...
		if(!IsValidClient(owner))
			detonate = true;	// something went wrong, bypass test
		else if(GetClientTeam(activator) != GetClientTeam(owner) ||(owner == activator && minefilter == 1))
				detonate = true;
	}
	else detonate = true;	// detonate always

	if(detonate)
	{
		decl String:targetname[64], String:buffers[2][32];
...
{
	decl String:mine_name[64], String:beam_name[64], String:str[128];
	...
	for(new i; i < 3; i++) position[i] += normal[i] * 0.5;
	...
	else	// detonate against anything
	{
...
{
	new old_client = GetClientOfUserId(defuse_userid[client]);
	...
	new bool:player_moved;
	// VERIFY ANGLES
	new Float:angles[3];
	GetClientEyeAngles(client, angles);
	new i;
	for(; i < 3; i++) if(FloatAbs(angles[i] - defuse_angles[client][i]) > DEFUSE_ANGLE_THRESHOLD)
	{
		player_moved = true;
		break;
	}
	...
	if(defuse_time[client] < 2)
	{
		...
		for(i = 0; i < defuse_time[client]; i++) StrCat(message, 16, ".");
...
{
	if(defuse_userid[client]) return;	// defusal already in progress
...
{
	if(IsValidClient(client) && !(buttons & IN_USE)
	&& defuse_userid[client] && !defuse_cancelled[client])	// is defuse in progress?
	{
...
public Event_PlayerUse(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(last_mine_used == target)	// verify this use event matches with the mine-use event
	{
		...
		if(client) StartDefusal(client, target);
	}
}
...
public CreateExplosion(Float:vec[3], owner)
{
	...
	new mag = GetConVarInt(sm_pp_minedmg), rad = GetConVarInt(sm_pp_minerad);
	SetEntProp(ent, Prop_Data, "m_iMagnitude",mag);
	if(rad) SetEntProp(ent, Prop_Data, "m_iRadiusOverride", rad);

...
PerformBlind(target, amount)
{
	...
	if (amount == 0)
	{
		PbFade(targets, 1536, 1536, (0x0001 | 0x0010), color);
	}
	else
	{
		PbFade(targets, 1536, 1536, (0x0002 | 0x0008), color);
	}
}
#endif
...
KillDrug(client)
{
	...
	PbFade(clients, 1536, 1536, (0x0001 | 0x0010));
}
...
public Action:Timer_Drug(Handle:timer, any:client)
{
	...
	PbFade(clients, 255, 255, 0x0002, color);
	return Plugin_Handled;
}
#endif

stock PbFade(clients[2], duration = 0, holdtime = 0, flags = 0, color[4] = {0, 0, 0, 0})
{
	new Handle:pb = StartMessageEx(g_FadeUserMsgId, clients, 1);
	...
	{
		BfWriteShort(pb, duration);
		BfWriteShort(pb, holdtime);
		BfWriteShort(pb, flags);
		BfWriteByte(pb, color[0]);
		BfWriteByte(pb, color[1]);
		BfWriteByte(pb, color[2]);
		BfWriteByte(pb, color[3]);
	}

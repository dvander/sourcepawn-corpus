...
// The quick implementation is directly stolen from GoD-Tony.
// https://forums.alliedmods.net/showthread.php?t=170351
// IRCRelay 2.5.0+ support
enum IrcChannel
{
	IrcChannel_Public  = 1,
...
public OnPluginStart()
{
	...
	g_VersionCvar = CreateConVar("sbchecker_version", VERSION, "", FCVAR_NOTIFY);
...
public OnConnectBanCheck(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	if (bancount > 0)
	{
		...
		{
			IRC_Broadcast(IrcChannel_Private, "[SBChecker] Warning: Player \"%N\" has %d previous SB ban%s on record.", client, bancount, ((bancount>0)?"s":""));
		}
		
		// SourceIRC support
		if (GetFeatureStatus(FeatureType_Native, "IRC_MsgFlaggedChannels") == FeatureStatus_Available)
		{
			IRC_MsgFlaggedChannels("ticket", "[SBChecker] Warning: Player \"%N\" has %d previous SB ban%s on record.", client, bancount, ((bancount>0)?"s":""));
		}
	}
}

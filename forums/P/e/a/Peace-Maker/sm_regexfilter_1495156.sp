...
new Handle:ClientLimits[MAXPLAYERS+1] = {INVALID_HANDLE,...};
...
public OnClientDisconnect(client)
{
	if(ClientLimits[client] != INVALID_HANDLE)
	{
		...
		ClientLimits[client] = INVALID_HANDLE;
	}
}
...
public Action:Command_SayHandle(client, args)
{
	...
	if (client > MaxClients || IsChatTrigger() || GetCmdArgString(text, sizeof(text)) < 1)
	{
...
stock ParseAndExecute(client, String:cmd[], len)
{
    ...
    else if(client <= MaxClients) FormatEx(repl, sizeof(repl), "%i", GetClientUserId(client))
    ReplaceString(cmd, len, "%u", repl);
    
    if(client != 0 && client <= MaxClients) FormatEx(repl, sizeof(repl), "%i", client)

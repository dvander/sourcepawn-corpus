...
// Change this flag to your needs
...
CheckCustomFlags()
{
	...
	if(g_iClientCount <= iFirstAmount)
	{
		...
		{
			if(IsClientInGame(i) && !g_bClientHasFlag[i] && (GetUserFlagBits(i) & DONATOR_FLAG) == 0)
			{
...
GetFirstJoinedWithoutFlag()
{
	...
	for(new i=1;i<=MaxClients;i++)
	{
		if(IsClientInGame(i) // Is ingame
		&& !g_bClientHasFlag[i] // Didn't get the flag from this plugin
		&& (GetUserFlagBits(i) & DONATOR_FLAG) == 0 // Didn't get the flag from any other plugin
		&& (iFirstJoin == 0 

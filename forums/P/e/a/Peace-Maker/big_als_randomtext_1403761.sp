...
public OnMapStart()
{
	...
	for(i=MaxClients;i<iMaxEntities;i++)
	{
		...
		&& StrEqual(sClassName, "func_button")
		&& GetEntPropString(i, Prop_Send, "m_iName", sClassName, sizeof(sClassName))
...
{
	decl String:sOutput[256], String:sRandom[10], String:sBuffer[64];
	new iRandom;
	...
	for(new i=1;i<=PART_LIMIT;i++)
	{
		...
		iRandom = GetRandomInt(1, g_iOptionCount[i-1]);

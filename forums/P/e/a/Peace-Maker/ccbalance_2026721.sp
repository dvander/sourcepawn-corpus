...
*/
#define GLOBAL_IMMUNITY "STEAM_0:0:12480610 STEAM_0:0:21631"
...
public OnPluginStart()
{
	...
	cv[CV_global_immunity] = CreateConVar("ccb_global_immunity", GLOBAL_IMMUNITY);
...
InGameInit()
{
	...
	for(new i=1;i<=MaxClients;i++)
	{
		...
		GetClientAuthString(i, player[PL_steamid], MAX_STEAMAUTH_LENGTH);
...
stock Player_Add(iUserId, String:sName[]="", String:sAuthId[]="")
{
	...
	*/
	...
	{
		if(StrEqual(sAuthId, ""))
			GetClientAuthString(client, sAuthId, MAX_STEAMAUTH_LENGTH);
...
ComputeMeritMove(Handle:hMerit, p[Player], iNumberOfT, iNumberOfCT, bool:bObeyImmunity)
{
	...
	*/
	...
	if(IsInGlobalImmunityList(p[PL_steamid]))
		return;
...
ComputeMeritSwap(Handle:hMerit, playerT[Player], playerCT[Player], bool:bObeyImmunity)
{
	...
	*/
	...
	// absolutely dont allow anyone in the immunity list to be moved or swapped.
	if(IsInGlobalImmunityList(playerT[PL_steamid]))
		return;
	if(IsInGlobalImmunityList(playerCT[PL_steamid]))
		return;
...
bool:IsInGlobalImmunityList(const String:sAuth[])
{
	decl String:sImmunityList[2048], String:sAuthId[MAX_STEAMAUTH_LENGTH];
	GetConVarString(cv[CV_global_immunity], sImmunityList, sizeof(sImmunityList));
	TrimString(sImmunityList);
	new iIndex, iOldIndex;
	while((iIndex = StrContains(sImmunityList[iOldIndex], " ")) != -1)
	{
		iIndex++;
		// Check each space seperated steamid
		if(iIndex >= MAX_STEAMAUTH_LENGTH)
		{
			iOldIndex += iIndex;
			continue;
		}
		
		strcopy(sAuthId, iIndex, sImmunityList[iOldIndex]);
		
		if(StrEqual(sAuthId, sAuth, false))
			return true;
	...
	if(iOldIndex > 0)
	{
		if(StrEqual(sImmunityList[iOldIndex], sAuth, false))
			return true;
	}
	return false;
}
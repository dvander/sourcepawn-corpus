...
#define SOUND_BELL "ui/item_acquired.wav"
...
//cvars handles
new Handle:g_hSpec, Handle:g_hDistance, Handle:g_hDelay;
   ...
   if( IsPlayerAlive(client) || GetConVarBool(g_hSpec) )  {
      ...
      while(i < GetArraySize(g_hTrophies))  {
         ...
         if(pass) {
            ...
            GotTrophie(client);
	    PrintToChat(client, "[Trophies]: You Have Picked Up A Trophy!");
...
AddTrophy(Float:loc[3]) {
   PushArrayArray(g_hTrophies, loc, 3);
...
public Action:DrawCircles(Handle:tm, any:trash) {
   for(new i=0; i<GetArraySize(g_hTrophies); i++)  {
      new Float:loc[3];
      GetArrayArray(g_hTrophies, i, loc, 3);
      DrawCircle(loc);
   }

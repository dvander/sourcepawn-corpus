...
{
	if(!GetConVarBool(Remove_Disabled)) // If sm_remove_disabled = 0
	{
		PrintToChat(client, "[SM] The server owner has disabled Ent_Delete.");
		return Plugin_Handled;
	}
	else // If sm_remove_disabled = 1
	{
		...
		PrintToServer("%N deleted (%s):(%s) with ent_delete.", client, name, modelname);
		RemoveEdict(Ent2);
		return Plugin_Handled;
	}
}
...
public Action:Command_Remove(client, args)
{
	if(!GetConVarBool(Remove_Disabled)) // If sm_remove_disabled = 0
	{
		...
		decl String:name[128];
		decl Ent2;
		Ent2 = GetClientAimTarget(client, false); // Aimtarget
		GetEdictClassname(Ent2, name, sizeof(name)); // Classname
		GetEntPropString(Ent2, Prop_Data, "m_ModelName", modelname, 128);
	    PrintToChat(client, "(Removed %s: %s)", name, modelname);
		PrintToServer("%N deleted (%s):(%s) with ent_delete.", client, name, modelname);
		RemoveEdict(Ent2);

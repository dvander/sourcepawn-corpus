...
{	
	...
	RegConsoleCmd("sm_channels", VCGui);
	...
	if (KvGotoFirstSubKey(kv))
	{
		do
		{
			...
			if (g_iChannelsMode[g_iChannels] == 2)
				KvGetString(kv, "flag", g_sChannelsFlag[g_iChannels], ARRAYS_SIZE, "ban");
...
public Action:VCGui(client, args)
{
	...
	SetMenuTitle(menu, "Choose channel:", g_sChannelsName[g_iClientChannel[client]]);

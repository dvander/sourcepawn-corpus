...
public Menu_ShowHelp(Handle:menu, MenuAction:action, param1, param2)
{
	if(action == MenuAction_Select)
	{
		...
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_Jumping", param1);
		}
		if(StrEqual(info, "Stomping"))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_Stomping", param1);
		}
		if(StrEqual(info, "Sprinting"))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_Sprinting", param1);
		}
		if(StrEqual(info, "Weigh Down"))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_WeighDown", param1);
		}
		if(StrEqual(info, "Using Power-Ups"))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_UsingPowerUps", param1);
		}
		if(StrEqual(info, "Battle Arena"))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_BattleArena", param1);
		}
		if(StrEqual(info, "Coin Rush"))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_CoinRush", param1);
		}
		if(StrEqual(info, Powers[0]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_FireFlower", param1);
		}
		if(StrEqual(info, Powers[1]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_POWBlock", param1);
		}
		if(StrEqual(info, Powers[2]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_PWing", param1);
		}
		if(StrEqual(info, Powers[3]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_GoombaShoe", param1);
		}
		if(StrEqual(info, Powers[4]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_SuperLeaf", param1);
		}
		if(StrEqual(info, Powers[5]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_BooTheif", param1);
		}
		if(StrEqual(info, Powers[6]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_Mushroom", param1);
		}
		if(StrEqual(info, Powers[7]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_Starman", param1);
		}
		if(StrEqual(info, Powers[8]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_Hammer", param1);
		}
		if(StrEqual(info, Powers[9]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_Thunderbolt", param1);
		}
		if(StrEqual(info, Powers[10]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_GoldenMushroom", param1);
		}
		if(StrEqual(info, Powers[11]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_IceFlower", param1);
		}
		if(StrEqual(info, Powers[12]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_LazyShellArmor", param1);
		}
		if(StrEqual(info, Powers[13]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_1UpMushroom", param1);
		}
		if(StrEqual(info, Powers[14]))
		{
			Format(Hint, sizeof(Hint), "%t", "mbHint_BowserSuit", param1);
		}
		DrawPanelText(panel, Hint);

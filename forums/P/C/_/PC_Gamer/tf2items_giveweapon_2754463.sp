...
public OnPluginStart()
{
	...
	new Handle:cv_version = CreateConVar("tf2items_giveweapon_version", PLUGIN_VERSION, "[TF2Items] Give Weapon Version", FCVAR_NOTIFY | FCVAR_SPONLY);
	new Handle:cv_notify = CreateConVar("tf2items_giveweapon_notify", "2", "If 1, makes Give Weapon show the plugin activity for givew only. If 2, makes it show activity for both givew and givew_ex.");
	new Handle:cv_valveweps = CreateConVar("tf2items_valveweapons", "1337", "1337 to allow giving Valve weapons, anything else to disallow. Just because I got annoyed.");
	new Handle:cv_lowadmins = CreateConVar("tf2items_customweapons_lowadmins", "1", "0 to disallow lower admins giving themselves custom weapons, 1 to allow. Just because I got annoyed.");
	new Handle:cv_gimme = CreateConVar("tf2items_allow_gimme", "1", "0 to disallow the use of sm_gimme, 1 to allow");

	RegAdminCmd("sm_giveweapon", Command_Weapon, ADMFLAG_CHEATS, "sm_giveweapon <player> <itemindex>");
	RegAdminCmd("sm_giveweapon_ex", Command_WeaponEx, ADMFLAG_GENERIC, "Give Permanent Weapon sm_giveweapon_ex <player> <itemindex>");
	...
	RegAdminCmd("sm_gimme", Command_Gimme, ADMFLAG_CHEATS, "Give self a weapon sm_gimme <itemindex>");
...
#endif
#if defined _tf2itemsinfo_included
	hCvarTF2II = CreateConVar("tf2items_use_tf2ii", "1", "Enable/disable tf2ii plugin usage");
...
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (IsValidClient(client) && IsPlayerAlive(client) && client > 0)
	{
...
stock GiveWeaponOfIndex(client, weaponLookupIndex, weaponSlot)
{
	...
	SetEntProp(entity, Prop_Send, "m_bValidatedAttachedEntity", 1);  //Added by me
	switch (weaponLookupIndex)
	{
	case 2228:
		{
	...
	case 1071:
		{
			//new weapon6 = GetPlayerWeaponSlot(client, 2);
			TF2Attrib_SetByName(entity, "item style override", 0.0);
			TF2Attrib_SetByName(entity, "loot rarity", 1.0);		
			TF2Attrib_SetByName(entity, "turn to gold", 1.0);			
		}		
	case 2058:
		{
			SetEntProp(entity, Prop_Send, "m_iEntityLevel", (-128+6));
			SetEntProp(entity, Prop_Send, "m_iEntityQuality", 9); //FlaminSarge's Self-Made Jar of Ants			
		}
	case 142:
		{
...
stock CreateVM(client, String:model[])
{
	...
	SetEntProp(ent, Prop_Send, "m_bValidatedAttachedEntity", 1);  //Added by me
...
stock AddCustomHardcodedToTrie(Handle:trie)
{
	...
	//PCG cheap Community rocket launcher
	SetTrieString(trie, "20001_classname", "tf_weapon_rocketlauncher");
	SetTrieValue(trie, "20001_index", 205);
	SetTrieValue(trie, "20001_slot", 0);
	SetTrieValue(trie, "20001_quality", 7);
	SetTrieValue(trie, "20001_level", 100);
	SetTrieString(trie, "20001_attribs", "6 ; 0.25 ; 4 ; 5.0");
	SetTrieValue(trie, "20001_ammo", 200);
}
PrepareAllModels()
{

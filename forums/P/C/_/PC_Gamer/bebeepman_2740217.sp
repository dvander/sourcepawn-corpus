...
Action GiveDeflector(int client)
{
	...
	if(IsValidEntity(Weapon4))
	{
		...
		TF2Attrib_SetByName(Weapon4, "engy building health bonus", 5.0);
		TF2Attrib_SetByName(Weapon4, "maxammo metal increased", 3.0);
		TF2Attrib_SetByName(Weapon4, "metal regen", 50.0);	
...
public void Event_BuildObject(Event event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(event.GetInt("userid"));
	TFObjectType object2 = view_as<TFObjectType>(event.GetInt("object"));
	
	if(client >= 1 && client <= MaxClients && IsClientInGame(client) && g_bIsBeepman[client] && object2 == TFObject_Sentry)
	{
		int iBuilding = GetEventInt(event, "index");
		if(iBuilding > MaxClients && IsValidEntity(iBuilding))
		{
			SetEntPropFloat(iBuilding, Prop_Send, "m_flModelScale", 1.5);
		}
	}
}
...
{
	CreateConVar("sm_aia_version", PLUGIN_VERSION, "Advanced Infinite Ammo", 0|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	LoadTranslations("common.phrases");

	g_hAllInfiniteAmmo = CreateConVar("sm_aia_all", "0", "Advanced Infinite Ammo for everyone", 0, true, 0.0, true, 1.0);
	g_hAdminOnly = CreateConVar("sm_aia_adminonly", "0", "Advanced Infinite Ammo will work for admins only, 1 = Completely Admin Only, 2 = Admin Only but the commands will work on non-admins", 0, true, 0.0, true, 2.0);
	g_hBots = CreateConVar("sm_aia_bots", "1", "Advanced Infinite Ammo will work for bots", 0, true, 0.0, true, 1.0);
	g_hRoundWin = CreateConVar("sm_aia_roundwin", "1", "Advanced Infinite Ammo for everyone on round win", 0, true, 0.0, true, 1.0);
	g_hWaitingForPlayers = CreateConVar("sm_aia_waitingforplayers", "1", "Advanced Infinite Ammo for everyone during waiting for players phase", 0, true, 0.0, true, 1.0);
	g_hChat = CreateConVar("sm_aia_chat", "1", "Show Advanced Infinite Ammo changes in chat - 1 = Show chat to everyone - 0 = Show chat to only the person activating the command", 0, true, 0.0, true, 1.0);
	g_hLog = CreateConVar("sm_aia_log", "1", "Log Advanced Infinite Ammo commands", 0, true, 0.0, true, 1.0);
	g_hAmmoOnly = CreateConVar("sm_aia_ammoonly", "0", "Sets how to give ammo, 0 = Infinite Clip and Ammo(default behavior), 1 = Infinite Ammo but must still reload clip", 0, true, 0.0, true, 1.0);
	g_hExtraStuff = CreateConVar("sm_aia_extrastuff", "1", "Whether to add non-ammo related things such as infinite Ubercharge, shield charge, rage, bonk, etc", 0, true, 0.0, true, 1.0);
	g_hDisabledWeapons = CreateConVar("sm_aia_disabledweapons", "", "Weapons indexes to not give infinite ammo, separated by semicolons");
	g_hMvM = CreateConVar("sm_aia_mvmcash", "0", "Whether to set a players cash or not for MvM", 0, true, 0.0, true, 1.0);

	g_cvFriendlyInfiniteAmmo = CreateConVar("sm_aia_friendly", "1", "Automatically give infinite ammo for friendly players. 0 = disabled | 1 = enabled", 0, true, 0.0, true, 1.0);
...
GiveInfiniteAmmo(client, iWeapon)
{
	...
	switch(iWeaponIndex)
	{
		...
		case 14,201,230,526,664,792,801,851,881,890,899,908,957,966,1098:
		{
			SetAmmo(client, iWeapon);

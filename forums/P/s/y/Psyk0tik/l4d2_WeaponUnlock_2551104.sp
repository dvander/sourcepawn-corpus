...
new Handle:h_MilitaryBoost
new Handle:h_HuntingBoost
new Handle:h_SilencedBoost
new Handle:h_UziBoost
new Handle:h_M16Boost
new Handle:h_AK47Boost
new Handle:h_SCARBoost
new Handle:h_M60Boost
new Handle:h_AutoBoost
new Handle:h_SPASBoost
new Handle:h_ChromeBoost
new Handle:h_PumpBoost
new Handle:h_GrenadeBoost
new Handle:h_P220Boost
new Handle:h_MagnumBoost
...
public Plugin:myinfo =
{
	...
	author = "Crimson_Fox and modified by Psykotik",
...
public OnPluginStart()
{
	...
	h_AWPBoost = CreateConVar("l4d2_wu_awpboost", "40", "Amount of damage added to AWP sniper rifle.", FCVAR_PLUGIN)
	h_ScoutBoost = CreateConVar("l4d2_wu_scoutboost", "40", "Amount of damage added to scout sniper rifle.", FCVAR_PLUGIN)
	h_MP5Boost = CreateConVar("l4d2_wu_mp5boost", "20", "Amount of damage added to mp5 submachine gun.", FCVAR_PLUGIN)
	h_SG552Boost = CreateConVar("l4d2_wu_sg552boost", "30", "Amount of damage added to sg552 assault rifle.", FCVAR_PLUGIN)
	h_MilitaryBoost = CreateConVar("l4d2_wu_militaryboost", "40", "Amount of damage added to military sniper rifle.", FCVAR_PLUGIN)
	h_HuntingBoost = CreateConVar("l4d2_wu_huntingboost", "40", "Amount of damage added to hunting rifle.", FCVAR_PLUGIN)
	h_SilencedBoost = CreateConVar("l4d2_wu_silencedboost", "20", "Amount of damage added to silenced submachine gun.", FCVAR_PLUGIN)
	h_UziBoost = CreateConVar("l4d2_wu_uziboost", "20", "Amount of damage added to submachine gun.", FCVAR_PLUGIN)
	h_M16Boost = CreateConVar("l4d2_wu_m16boost", "30", "Amount of damage added to m16 assault rifle.", FCVAR_PLUGIN)
	h_AK47Boost = CreateConVar("l4d2_wu_ak47boost", "30", "Amount of damage added to ak47 assault rifle.", FCVAR_PLUGIN)
	h_SCARBoost = CreateConVar("l4d2_wu_scarboost", "30", "Amount of damage added to scar-l assault rifle.", FCVAR_PLUGIN)
	h_M60Boost = CreateConVar("l4d2_wu_m60boost", "30", "Amount of damage added to m60 assault rifle.", FCVAR_PLUGIN)
	h_AutoBoost = CreateConVar("l4d2_wu_autoboost", "25", "Amount of damage added to automatic shotgun.", FCVAR_PLUGIN)
	h_SPASBoost = CreateConVar("l4d2_wu_spasboost", "25", "Amount of damage added to spas shotgun.", FCVAR_PLUGIN)
	h_ChromeBoost = CreateConVar("l4d2_wu_chromeboost", "15", "Amount of damage added to chrome shotgun.", FCVAR_PLUGIN)
	h_PumpBoost = CreateConVar("l4d2_wu_pumpboost", "15", "Amount of damage added to pump shotgun.", FCVAR_PLUGIN)
	h_GrenadeBoost = CreateConVar("l4d2_wu_grenadeboost", "35", "Amount of damage added to grenade launcher.", FCVAR_PLUGIN)
	h_P220Boost = CreateConVar("l4d2_wu_p220boost", "10", "Amount of damage added to p220 pistol.", FCVAR_PLUGIN)
	h_MagnumBoost = CreateConVar("l4d2_wu_magnumboost", "10", "Amount of damage added to magnum pistol.", FCVAR_PLUGIN)
...
public Action:Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_MilitaryBoost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_HuntingBoost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_SilencedBoost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_UziBoost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_M16Boost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_AK47Boost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_SCARBoost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_M60Boost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_AutoBoost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_SPASBoost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_ChromeBoost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_PumpBoost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_GrenadeBoost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_P220Boost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
	...
	{
		new health = GetClientHealth(target)
		new damage = GetConVarInt(h_MagnumBoost)
		if (health-damage < 0) SetEntityHealth(target, 0)
		else SetEntityHealth(target, health-damage)
	}
}

...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	CreateNative("FF2_PrepareSound", Native_PrepareSound);
...
public OnPluginStart()
{
	...
	*/
	...
	g_Sounds = CreateArray(ByteCountToCells(PLATFORM_MAX_PATH));
}
...
{
	new bool:found = (FindStringInArray(g_Sounds, sound) > -1);
	if (!found)
	{
		PrecacheSound(sound);
		PushArrayString(g_Sounds, sound);
	}
}
...
public Native_PrepareSound(Handle:plugin, numParams)
{
	...
	GetNativeString(1, sound, PLATFORM_MAX_PATH);
...
stock FindVersionData(Handle:panel, versionindex)
{
	switch (versionindex)
	{
		case 11: // 1.06e
		{
			DrawPanelText(panel, "1) Emergency fix to try to fix sound crashing issues.");
		}

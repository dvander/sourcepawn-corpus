...
#define PL_VERSION "3.3.0g"

// used in the 1.6 fix code
#define CRIT_MULTIPLIER 3.0
#define MINICRIT_MULTIPLIER 1.35
...
UpdateGameDescription(bool:bAddOnly=false)
{
	...
	{
#if defined LOG
		LogMessage("[PH] Setting PropHunt game description.");
#endif
	...
	{
#if defined LOG
		LogMessage("[PH] Removing PropHunt game description.");
#endif
...
public Action:TakeDamageHook(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	...
	if(weapon > MaxClients && IsValidEntity(weapon))
	{
		...
		if (GetTrieValue(g_hWeaponNerfs, weaponIndex, multiplier))
		{
			damage *= multiplier;
			damageChanged = true;
			// If we returned now, it'd skip the "Don't crash" logic
		}
	}
	...
	if(victim > 0 && victim <= MaxClients && IsClientInGame(victim))
	{
		...
		// Don't crash logic
		if (WillDamageKillPlayer(victim, damage, damagetype, weapon))
		{
			RemoveAnimeModel(victim);
			new Handle:data;
			CreateDataTimer(0.1, Timer_KillPropPlayer, data, TIMER_FLAG_NO_MAPCHANGE);
			WritePackCell(data, victim); // This should be a UserID, but 0.1 sec isn't long enough for someone to be put in game after reconnecting
			WritePackCell(data, attacker);
			new fakeInflictor = inflictor;
			if (inflictor > 0)
			{
				new String:inflictorClass[64];
				GetEntityClassname(inflictor, inflictorClass, sizeof(inflictorClass));
#if defined LOG
				LogMessage("Inflictor: %d (%s)", inflictor, inflictorClass);
#endif
				if (strncmp(inflictorClass, "tf_projectile", 13) == 0)
				{
					// its either this or create the entity
					fakeInflictor = weapon;
				}
			}
			
			WritePackCell(data, fakeInflictor);
			WritePackCell(data, damagetype);
...
// 1.7 version, damagetype and weapon are ignored
bool:WillDamageKillPlayer(victim, Float:damage, damagetype, weapon)
{
	return GetClientHealth(victim) <= RoundToNearest(damage);
}

public Action:Timer_KillPropPlayer(Handle:timer, Handle:data)
{
	ResetPack(data);
	...
	{
		attacker = 0; // set to world
	}
	
	new inflictor = ReadPackCell(data);
...
public Event_player_spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	*/
	...
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		RemoveAnimeModel(client);
		SDKHook(client, SDKHook_OnTakeDamageAlive, TakeDamageHook);
...
public Action:Event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(IsClientInGame(client) && GetClientTeam(client) == TEAM_PROP)
	{
		...
		SDKUnhook(client, SDKHook_OnTakeDamageAlive, TakeDamageHook);
...
#if defined LOG
	LogMessage("Found PropHunt config file \"%s\": %d", confil, FileExists(confil, true));
#endif

 ...
 * vim: set ts=4 :
 * =============================================================================
 * SourceMod Mapchooser Plugin using BuiltinVotes
 * Creates a map vote at appropriate times, setting sm_nextmap to the winning
 * vote
 *
 * SourceMod (C)2004-2007 AlliedModders LLC.  All rights reserved.
 * BuiltinVotes MapChooser modified by Ross Bemrose (Powerlord)
 * =============================================================================
 *
...
{
	name = "BuiltinVotes MapChooser",
	author = "AlliedModders LLC, Powerlord",
	description = "Automated Map Voting, modified to use the BuiltinVotes Extension",
	version = VERSION,
	url = "https://forums.alliedmods.net/showthread.php?t=171197"
};
...
public OnPluginStart()
{
	...
	CreateConVar("tf2mc_version", VERSION, "BuiltinVotes MapChooser version", FCVAR_REPLICATED | FCVAR_DONTRECORD | FCVAR_SPONLY);
...
InitiateVote(MapChange:when, Handle:inputlist=INVALID_HANDLE)
{
	...
	g_VoteMenu = CreateBuiltinVote(Handler_MapVoteMenu, BuiltinVoteType_NextLevelMult, BUILTINVOTE_ACTIONS_ALL);
	...
	if ((when == MapChange_Instant || when == MapChange_RoundEnd) && GetConVarBool(g_Cvar_DontChange))
	{
		AddBuiltinVoteItem(g_VoteMenu, VOTE_DONTCHANGE, BUILTINVOTES_EXTEND);
	}
	else if (GetConVarBool(g_Cvar_Extend) && g_Extends < GetConVarInt(g_Cvar_Extend))
	{
		AddBuiltinVoteItem(g_VoteMenu, VOTE_EXTEND, BUILTINVOTES_EXTEND);
	}
						   ...
						   const item_info[][2])
{
	...
	decl String:map[32];
	GetBuiltinVoteItem(menu, item_info[0][BUILTINVOTEINFO_ITEM_INDEX], map, sizeof(map));
	...
	if (strcmp(map, VOTE_EXTEND, false) == 0)
	{
		...
		DisplayBuiltinVotePass(g_VoteMenu, BUILTINVOTES_EXTEND);
	...
	else
	{
		...
		{
			SetNextMap(map);
			DisplayBuiltinVotePass(g_VoteMenu, map);
		}
		else if (g_ChangeTime == MapChange_Instant)
		{
			DisplayBuiltinVotePass2(g_VoteMenu, TRANSLATION_TF2_VOTE_CHANGELEVEL_PASSED, map);
			new Handle:data;
...
public Handler_MapVoteMenu(Handle:menu, BuiltinVoteAction:action, param1, param2)
{
	switch (action)
	{
		...
		case BuiltinVoteAction_Cancel:
		{
			...
			if (param1 == _:BuiltinVoteFail_NotEnoughVotes)
			{
				if (GetConVarBool(g_Cvar_NoVoteMode))
				{
					new count = GetBuiltinVoteItemCount(menu);
					new item = GetRandomInt(0, count - 1);
					decl String:map[32];
					GetBuiltinVoteItem(menu, item, map, sizeof(map));
				...
				{
					DisplayBuiltinVoteFail(g_VoteMenu, BuiltinVoteFail_NotEnoughVotes);
				}
			}
			else
			{
				// We were actually cancelled. I guess we do nothing.
				DisplayBuiltinVoteFail(g_VoteMenu, BuiltinVoteFail_Generic);
			}

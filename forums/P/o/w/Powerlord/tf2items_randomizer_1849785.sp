...
#define PLUGIN_VERSION		"1.57" //As of Dec 7, 2012
...
public Action:Cmd_taunt(client, String:cmd[], args)
{
	...
	if (!IsValidClient(client)) return Plugin_Continue;
	if (TF2_GetPlayerClass(client) != TFClass_Spy && (TF2_IsPlayerInCondition(client, TFCond_Disguised) || TF2_IsPlayerInCondition(client, TFCond_Disguising))) return Plugin_Handled;
	return Plugin_Continue;
}
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (!(deathflags & TF_DEATHFLAG_DEADRINGER))
	{
		...
		if (((attacker && attacker != client) || (custom == TF_CUSTOM_DECAPITATION_BOSS) || (strcmp(weaponlog, "eyeball_rocket", false) == 0)) && cvar_enabled) SetRandomization(client);
		for (new i = 0; i < MaxTimers; i++)
		{
			ClearTimer(hTimers[client][i]);
		}
...
//			RemovePlayerBack(client);
		...
		if (mel > 0)
		{
			...
			if (class != TFClass_Engineer && !IsMedieval())
			{
...
//	decl Float:speed; // Pyro
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		if (IsClientInGame(client) && IsPlayerAlive(client))
		{
			...
			if (class != TFClass_Scout && prim == 772 && !TF2_IsPlayerInCondition(client, TFCond_Dazed))
			{
...
/*	if (class != TFClass_Spy && impulse > 220 && impulse < 240 && impulse != 230 && GetIndexOfWeaponSlot(client, 3) == 27)
	...
	if (!tf2items_giveweapon)
	{
		if ((buttons & IN_ATTACK2) && !(iLastButtons[client] & IN_ATTACK2))
		{
			...
			new wep = GetEntPropEnt(client, Prop_Send, "m_hActiveWeapon");
			if (GetEntityMoveType(client) != MOVETYPE_NONE && FindPlayerTarge(client) != -1 && hTimers[client][ChargeTimer] == INVALID_HANDLE && GetEntPropFloat(client, Prop_Send, "m_flMaxspeed") > 1.01 && (class != TFClass_DemoMan || (wep == GetPlayerWeaponSlot(client, TFWeaponSlot_Melee) && (idxslot2 == 265 || idxslot2 == 20 || idxslot2 == 207 || idxslot2 == 130))))
			{
...
//	static Handle:hNewItem;
	...
	{
		if (setwep[client][2] != 0) return Plugin_Handled;
	}
	return Plugin_Continue;
...
/*	if (weaponLookupIndex == 735 || weaponLookupIndex == 736 || weaponLookupIndex == 810 || weaponLookupIndex == 831)
	{
		new TFClassType:class = TF2_GetPlayerClass(client);
		TF2_SetPlayerClass(client, TFClass_Spy, _, false);
		FakeClientCommand(client, "build 3 0");
		TF2_SetPlayerClass(client, class, _, false);
		CreateTimer(0.3, Timer_SetSapper, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
	}*/
	new weaponAmmo = -1;
	...
	for (new i = 0; i < target_count; i++)
	{
		if (IsClientInGame(target_list[i]))
		{
			...
			if (class != TFClass_Unknown)
			{
				...
				if (TF2_GetPlayerClass(target_list[i]) != class) TF2_SetPlayerClass(target_list[i], class, _, true);
			}
			if (IsPlayerAlive(target_list[i]))
			{

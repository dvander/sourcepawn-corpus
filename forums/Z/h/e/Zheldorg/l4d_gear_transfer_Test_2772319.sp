...
{
	PrintToChatAll("OnWeaponEquip client %i weapon %i", client, weapon);//TEST
	if( weapon > MaxClients && IsValidEntity(weapon) )
	{
		static char classname[32];
		GetEntityClassname(weapon, classname, sizeof(classname));
		PrintToChatAll("OnWeaponEquip classname %s", classname);//TEST
		...
		PrintToChatAll("OnWeaponEquip ClientItem %i", g_iClientItem[client][slot]);//TEST
		PrintToChatAll("OnWeaponEquip ClientType %i", g_iClientType[client][slot]);//TEST
	}
}
...
void IsAllowed()
{
	PrintToChatAll("IsAllowed step");//TEST
...
public void Event_WeaponDrop(Event event, const char[] name, bool dontBroadcast)
{
	...
	if( client )
	{
		...
		PrintToChatAll("Event_WeaponDrop step client %i", client);//TEST
		OnDrop(client, item);
	}
}
...
{
	item = EntIndexToEntRef(item);
	PrintToChatAll("OnWeaponDrop step client %i", client);//TEST
	OnDrop(client, item);
}

void OnDrop(int client, int item)
{
	PrintToChatAll("OnDrop step client %i", client);//TEST
	for( int i = 0; i < 3; i++ )
	{
...
{
	PrintToChatAll("TransferItem step");//TEST
	// Don't allow transfers while incapped/reviving
	...
	PrintToChatAll("TransferItem item %i", item);//TEST
	if( item != -1 )
	{
		item = EntIndexToEntRef(item);
		PrintToChatAll("TransferItem item_ref %i", item);//TEST
	...
	PrintToChatAll("TransferItem type %i", type);//TEST
	// Validate type
	...
	if( transferType == METHOD_NONE || (transferType != METHOD_GIVE && fakeTarget == false) )
		return;
	PrintToChatAll("TransferItem step 1");//TEST
	...
	PrintToChatAll("TransferItem step 2");//TEST
	// Verify distance
	...
	if( dist <= g_fDistGive )
	{
		PrintToChatAll("TransferItem step IG");//TEST
	...
	PrintToChatAll("TransferItem step finish");//TEST
	for( int i = 0; i < 3; i++ )
	PrintToChatAll("TransferItem client %i, ID%i, Item %i, Type %i ", client, i, g_iClientItem[client][i], g_iClientType[client][i]);//TEST
...
{
	PrintToChatAll("GiveItem step");//TEST
	...
	if( transferType == METHOD_SWAP )
	{
		...
		PrintToChatAll("GiveItem Before IsFakeClient step client %i", client);//TEST
		if( IsFakeClient(client) )
		{
			PrintToChatAll("GiveItem IsFakeClient step client %i", client);//TEST
			RemovePlayerItem(target, ent_t);
			PrintToChatAll("GiveItem IsFakeClient RemovePlayerItem step client %i", client);//TEST
			EquipPlayerWeapon(client, ent_t);
			PrintToChatAll("GiveItem IsFakeClient EquipPlayerWeapon step client %i", client);//TEST
			ent_t = EntRefToEntIndex(ent_t);
			PrintToChatAll("GiveItem IsFakeClient EquipPlayerWeapon step client %i", client);//TEST
		} else {
			PrintToChatAll("GiveItem IsFakeClient Else step client %i", client);//TEST
			RemovePlayerItem(target, ent_t);
			PrintToChatAll("GiveItem IsFakeClient RemovePlayerItem step client %i", client);//TEST
			// RemoveEntity(ent_t); // Causing bugs with ghost weapon (valid entity) attached to bots feet
			RemoveEdict(ent_t);
			PrintToChatAll("GiveItem IsFakeClient RemoveEdict step client %i", client);//TEST
			ent_t = CreateAndEquip(client, type_t - 1);
			PrintToChatAll("GiveItem IsFakeClient Finish step client %i", client);//TEST
		}
		PrintToChatAll("GiveItem Front IsFakeClient step client %i", client);//TEST
		...
		PrintToChatAll("GiveItem Front IsFakeClient step target %i", target);//TEST
...
{
	PrintToChatAll("CreateAndEquip step client %i", client);//TEST
	static char classname[32];
	...
	PrintToChatAll("CreateAndEquip GivePlayerItem step client %i", client);//TEST
	if( entity != INVALID_ENT_REFERENCE )
	{
		EquipPlayerWeapon(client, entity);
		PrintToChatAll("CreateAndEquip EquipPlayerWeapon step client %i", client);//TEST
		return entity;
	}
	PrintToChatAll("CreateAndEquip (return 0) step client %i", client);//TEST
	return 0;
}

    ...
    for (new i = 1; i <= MaxClients; i++)
    {
        ...
        if (!g_MedicButtonDown[i] && IsClientInGame(i) && TF2_GetPlayerClass(i) == TFClass_Medic)
        {
            if (GetClientButtons(i) & IN_ATTACK2)
            {
                ...
                //PrintToChatAll("current weaponclass = %s", classname);
                //new String:id[64];
                //IntToString(GetWeaponIndex(GetPlayerWeaponSlot(i, 1)), id, 64);
				new weaponid = GetWeaponIndex(GetPlayerWeaponSlot(i, 1));
                //PrintToChatAll("current weapon = %s", id);
                if (StrEqual(classname, "CWeaponMedigun") && g_MedicUberCharge[i] < 100 && TF_IsUberCharge(i) == 0 && weaponid != 998)

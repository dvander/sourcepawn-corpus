...
public OnPluginStart()
{
	...
	RegAdminCmd("reflect", Cmd_Toggle, ADMFLAG_KICK, "Toogle bouncing rockets on or off");
}
...
{
	if (!StrEqual(classname, "tf_projectile_rocket", false) || !PluginEnabled)
		return;
...
public Action Cmd_Toggle(client, args)
{
	...
	{
		PluginEnabled = true;
		PrintToChatAll("%t", "Bouncing_rockets_enabled");
	}
	else
	{
		PluginEnabled = false;
		PrintToChatAll("%t", "Bouncing_rockets_disabled");
	}

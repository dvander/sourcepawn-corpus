...
{
	new Handle:Cvar_Version = CreateConVar("sm_spg_version", VERSION, "SpawnPoint Generator Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY); 
}
...
public OnMapStart()
{
	...
	maxct=0;
	new ent=-1;
    while ((ent = FindEntityByClassname2(ent, "info_player_terrorist")) != -1) maxt++;
    ent=-1;
    while ((ent = FindEntityByClassname2(ent, "info_player_counterterrorist")) != -1) maxct++;
	if((maxct+maxt)<MaxClients)
	{
		GenerateSpawnPoints();
		new noblock = FindConVar("sm_noblock")
...
public GenerateSpawnPoints()
{
	...
	while((ctspawn = FindEntityByClassname2(ctspawn, "info_player_counterterrorist")) != -1)
	{
		...
		{
			new Float:position[3];
			new ctspawn = FindEntityByClassname2(i, "info_player_counterterrorist");

...
#define FLAG_HIDE_SCOUT_HAT          (1 << 4)
#define FLAG_HIDE_SCOUT_HEADPHONES   (1 << 5)
#define FLAG_HIDE_HEAVY_HANDS        (1 << 6)
#define FLAG_HIDE_ENGINEER_HELMET    (1 << 7)
#define FLAG_HIDE_SNIPER_QUIVER      (1 << 8)
#define FLAG_HIDE_SNIPER_HAT         (1 << 9)
#define FLAG_HIDE_SOLDIER_ROCKET     (1 << 10)
#define FLAG_HIDE_SOLDIER_HELMET     (1 << 11)
#define FLAG_SHOW_SOLDIER_MEDAL      (1 << 12)
...
new Handle:CvarBlockChatTriggers		= INVALID_HANDLE;
...
// ---- Blocking chat triggers -----------------------------------------------------
new const String: blocked_commands[][] = { "!equip", "!tf_equipment", "!equipment", "!em",
												"/equip", "/tf_equipment", "/equipment", "/em"};
...
{    
	...
	CvarBlockChatTriggers		= CreateConVar("tf_equipment_blockchatriggers", "1",   "If activated chat triggers will be blocked.", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	...
	// Register commands for chat triggers blocking
	RegConsoleCmd("say",					tf_equipment_blockchatriggers);
	RegConsoleCmd("say_team",			tf_equipment_blockchatriggers);
...
Item_Equip(Client, Item)
{
    ...
    {   
        ...
        {
            new BodyGroups = GetEntProp(Client, Prop_Send, "m_nBody");
            ...
            SetEntProp(Client, Prop_Send, "m_nBody", BodyGroups | Item_DetermineBodyGroups(Client, Item));
    ...
    {
        new BodyGroups = GetEntProp(Client, Prop_Send, "m_nBody");
        BodyGroups &= ~Item_DetermineBodyGroups(Client, CurrentItem);
        BodyGroups |= Item_DetermineBodyGroups(Client, Item);
        
        SetEntProp(CurrentEntity, Prop_Send, "m_iItemDefinitionIndex", ItemIndex[Item], true);
        ...
        SetEntProp(Client, Prop_Send, "m_nBody", BodyGroups, true);
    }
...
Item_Remove(Client, Slot, Reset, Check=1)
{
    ...
    if (TF2_IsEntityWearable(PlayerItemEntity[Client][Slot]))
    {
        ...
        new BodyGroups = GetEntProp(Client, Prop_Send, "m_nBody");
        BodyGroups &= ~Item_DetermineBodyGroups(Client, PlayerItem[Client][Slot]);
    }
...
// ------------------------------------------------------------------------
Item_DetermineBodyGroups(Client, Item)
{
...
// Timer_Announce
// ------------------------------------------------------------------------
public Action:Timer_Announce(Handle:hTimer, any:client)
{
...
stock TF2_RemoveWearable(iOwner, iItem)
{
    ...
    if (TF2_IsEntityWearable(iItem))
    {
        ...
        {
            LogError("Error: Item %i isn't owned by Client %i", iItem, iOwner);
        }
    }
}
...
// ------------------------------------------------------------------------
// Hideing Chat Triggers
// ------------------------------------------------------------------------
public Action:tf_equipment_blockchatriggers(client, args)
{
	if (client)
	{
		if (client == 0)
		...
		new block_chat_commands = GetConVarInt(CvarBlockChatTriggers);

...
bool PlayerPickSquad[MAXPLAYERS+1] =					{false, ...}; //Player picked squad in Insurgency
...
public void OnPluginStart() // AFK Manager Plugin has started
{
	...
	else if (strcmp(game_mod, "insurgency", false) == 0)
	{
...
public Action Timer_CheckPlayer(Handle Timer, int client) // General AFK Timers
{
	...
	if(g_bEnabled) // Is the AFK Manager Enabled
	{
		...
		{
			if (INSURGENCY && !IsPlayerAlive(client) && g_bExcludeDead && PlayerPickSquad[client]) // Insurgency
			{
				g_iAFKTime[client]++;
				return Plugin_Continue;
			}

			if (!INSURGENCY && !IsPlayerAlive(client) && (g_bExcludeDead)) // Excluding Dead players
			{
#if _DEBUG
...
#endif
											ResetSpawn(client);
											if (g_iPlayerTeam[client] == 0 || !PlayerPickSquad[client]) // Are we moving player from the Unassigned team AKA team 0?	/or unassigned insurgency squad

...
{
	name = "MapChooser",
	author = "AlliedModders LLC (edited by stevieboy further edited by [CZ]Vortex",
...
//added by stevieboy to show players votes
new Handle:g_Cvar_ShowPlayerVotes = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	//added by stevieboy to show players votes
	g_Cvar_ShowPlayerVotes = CreateConVar("sm_mapvote_showvotes", "1", "Specifies whether or not MapChooser should show the player's vote.", _, true, 0.0, true, 1.0);
...
public Handler_MapVoteMenu(Handle:menu, MenuAction:action, param1, param2)
{
	switch (action)
	{
		...
		//added as cvar by stevieboy
		case MenuAction_Select:
		{
			if(GetConVarBool(g_Cvar_ShowPlayerVotes)){
				decl String:Name[32], String:Map[32];
				GetClientName(param1, Name, sizeof(Name));
				GetMenuItem(menu, param2, Map, sizeof(Map));
				PrintToChatAll("\x01\x04[SM]\x01 %s has voted for map '%s'", Name, Map);
			}	
		}

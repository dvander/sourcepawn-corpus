...
public OnClientPutInServer(client)
{
	...
	#if defined HL2DM
	lastHitWasHeadshot[client] = false;
...
#elseif defined HL2DM
public OnTraceAttackPost(victim, attacker, inflictor, Float:damage, damagetype, ammotype, hitbox, hitgroup)
{
	// Track headshots on both players and bots (victims)
	if(victim > 0 && victim <= iMaxClients && attacker > 0 && attacker <= iMaxClients)
	{
		if(hitgroup == 1) // 1 = Head hitgroup
		{
			lastHitWasHeadshot[victim] = true;
		}
	}
}
#endif
...
{	
	...
	if(attackerClient>0 && attackerClient<=iMaxClients)
	{
		...
		#if defined HL2DM
		else if(GetClientTeam(attackerClient) == GetClientTeam(victimClient) && GetClientTeam(attackerClient) > 1)
		{
			// Only count as teamkill if both are on actual playing teams (2=Rebels, 3=Combine)
		...
		else
		{
			...
			#elseif defined HL2DM
				new bool:headshot = lastHitWasHeadshot[victimClient];
				lastHitWasHeadshot[victimClient] = false; // Reset for next kill
			#else
	...
	#elseif defined HL2DM
		lastHitWasHeadshot[victimClient] = false; // Reset after death
	#endif

...
public OnPluginStart()
{
	...
	g_hDonatorLater = CreateConVar("give_donator_later", "1", "if set to 1, When one of the first people leaves the game/server, then the next guy that joins get donator access!", FCVAR_PLUGIN, true, 0.0);
	g_hCVFirstAmount = CreateConVar("sm_firstjoin_amount", "4", "The first x players get the custom flag", FCVAR_PLUGIN, true, 0.0);
	AutoExecConfig(true, "FirstToConnect");
}
...
CheckCustomFlags()
{
	...
	{
		if(GetConVarInt(g_hDonatorLater) == 1)
		{
			new iAddedGuy;

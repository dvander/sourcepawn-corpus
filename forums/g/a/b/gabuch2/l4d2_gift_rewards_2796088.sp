...
#define PLUGIN_VERSION 		"1.9.1-gabuch2"
...
1.9.1-gabuch2 (26-Dec-2022)
	- Marked additional Left4Dhooks natives as optional
	- Fixed plugin flow to prevent calling Left4Dhooks natives if it's not loaded
	- Added new cvars to control whether display Chat messages and Instructor Hints on gift pickup
...
ConVar g_hCvarAllow, g_hCvarMPGameMode, g_hCvarModes, g_hCvarModesOff, g_hCvarModesTog, g_hCvarChance1, g_hCvarChance2, g_hCvarChance3, g_hCvarCount, g_hGameDrop, g_hCvarDrop, g_hCvarSize, g_hCvarSpeed, g_hCvarHeal, g_hCvarRefillTier3, g_hCvarChatAnnounce, g_hCvarInstructorAnnounce;// g_hCvarSizeStart
int g_iCvarChance1, g_iCvarChance2, g_iCvarChance3, g_iTotalChance, g_iCvarCount, g_iDeathCount, g_iCvarHeal;
bool g_bCvarAllow, g_bMapStarted, g_bCvarRefillTier3, g_bCvarChatAnnounce, g_bCvarInstructorAnnounce;
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	MarkNativeAsOptional("L4D_IsPlayerIncapacitated");
...
public void OnPluginStart()
{
	...
	*/
	...
	g_hCvarHeal = CreateConVar(					"l4d2_gift_rewards_heal",						"50.0",	"Amount of Health Recovered.", CVAR_FLAGS );
	...
	g_hCvarRefillTier3 = CreateConVar(			"l4d2_gift_rewards_ammo_refill_tier_3",			"0",	"Turn on if you want Ammo Gifts to refill tier 3 weapons (LMG and Grenade Launcher).", CVAR_FLAGS );
	g_hCvarChatAnnounce = CreateConVar(			"l4d2_gift_rewards_ammo_chat_announce",			"1",	"Turn on if you want a message printed whenever someone gets a gift reward.", CVAR_FLAGS );
	g_hCvarInstructorAnnounce = CreateConVar(	"l4d2_gift_rewards_ammo_instructor_announce",	"1",	"Turn on if you want an instructor message to the person who picked up a gift reward.", CVAR_FLAGS );
	...
	g_hCvarRefillTier3.AddChangeHook(ConVarChanged_Cvars);
	g_hCvarChatAnnounce.AddChangeHook(ConVarChanged_Cvars);
	g_hCvarInstructorAnnounce.AddChangeHook(ConVarChanged_Cvars);

	LoadTranslations( "gift_rewards.phrases" );
...
void GetCvars()
{
	...
	g_bCvarRefillTier3 = g_hCvarRefillTier3.BoolValue;
	g_bCvarChatAnnounce = g_hCvarChatAnnounce.BoolValue;
	g_bCvarInstructorAnnounce = g_hCvarInstructorAnnounce.BoolValue;
...
void GiveAward(int client)
{
	if( GetClientTeam(client) == 2 )
	{
		...
		if(g_bCvarInstructorAnnounce)
		{
			...
			event.SetBool("hint_suppress_rest", true);
		}
		...
		if( random )
		{
			switch( random )
			{
				case 1:		
				{
					if(g_bLeft4DHooks && !g_bCvarRefillTier3)
					{
						int weaponIndex = GetPlayerWeaponSlot(client, 0);

						if(weaponIndex == -1)
						{
							if(g_bCvarChatAnnounce)
								PrintToChatAll("\x04[\x05Gift\x04] \x01%t", "REWARD_AMMO_PLAYER", client);
							if(g_bCvarInstructorAnnounce)
							{
								FormatEx(formattedString, sizeof(formattedString), "Rewarded Ammo");
								event.SetString("hint_caption", formattedString);
							}
							RefillAmmo(client);
							return;
						}
						...
						int tier = L4D2_GetIntWeaponAttribute(classname, L4D2IWA_Tier);
						...
						if(tier > 0)
						{
							...
							if(g_bCvarInstructorAnnounce)
							{
								FormatEx(formattedString, sizeof(formattedString), "%T", "REWARD_AMMO", client);
								event.SetString("hint_caption", formattedString);
							}
							RefillAmmo(client);
						}
						...
						{
							if(g_bCvarChatAnnounce)
								PrintToChatAll("\x04[\x05Gift\x04] \x01%t", "REWARD_AMMO_FAILURE_PLAYER", client);
							if(g_bCvarInstructorAnnounce)
							{
								FormatEx(formattedString, sizeof(formattedString), "%T", "REWARD_AMMO_FAILURE", client);
								event.SetString("hint_caption", formattedString);
							}
						}
					}
				...
				{
					if(g_bCvarChatAnnounce)
						PrintToChatAll("\x04[\x05Gift\x04] \x01%t", "REWARD_HEALTH_PLAYER", client);
					if(g_bCvarInstructorAnnounce)
					{
						FormatEx(formattedString, sizeof(formattedString), "%T", "REWARD_HEALTH", client);
						event.SetString("hint_caption", formattedString);
					}
					HealPlayer(client);
				}
				...
				{
					if(g_bCvarChatAnnounce)
						PrintToChatAll("\x04[\x05Gift\x04] \x01%t", "REWARD_SPEED_PLAYER", client);
					if(g_bCvarInstructorAnnounce)
					{
						FormatEx(formattedString, sizeof(formattedString), "%T", "REWARD_SPEED", client);
						event.SetString("hint_caption", formattedString);
					}
					AwardSpeed(client);
				}
...
void AwardSpeed(int client)
{
	...
	SetEntDataFloat(client, timerAddress + 4, g_fCvarSpeed);
	SetEntDataFloat(client, timerAddress + 8, GetGameTime() + g_fCvarSpeed);
	SetEntProp(client, Prop_Send, "m_bAdrenalineActive", 1, 1);
	g_fRewarded[client] = GetGameTime() + g_fCvarSpeed;
}
...
{
	if(g_bLeft4DHooks && L4D_IsPlayerIncapacitated(client))
	{
	...
	int iTempHealth = GetPlayerTempHealth(client);
	if(iHealth+iTempHealth+g_iCvarHeal > 100)
		SetPlayerTempHealth(client, 100-iHealth);	
	else
		SetPlayerTempHealth(client, g_iCvarHeal+iTempHealth);

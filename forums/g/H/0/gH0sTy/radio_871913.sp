...
public Handler_PlayRadioStation (Handle:menu, MenuAction:action, client, param)
{
    if (action == MenuAction_Select)
    {
        ...
        if (GetConVarInt (g_cVarRadioStationAdvert))
        {
            ...
            PrintToChatAll ("\x01\x04[SM-RADIO]\x01 %T", "Started Listening", client, szClientName, szStationName);
        }
...
public Action:Command_RadioOff (client, args)
{
    ...
    if (GetConVarInt (g_cVarRadioEnable))
    {
        ...
        {
            GetClientName (client, szClientName, sizeof (szClientName));
            PrintToChatAll ("\x01\x04[SM-RADIO]\x01 %T", "Stopped Listening", client, szClientName);
        }
    }
...
public Action:WelcomeAdvertTimer (Handle:timer, any:client)
{
    ...
    {
        GetClientName (client, szClientName, sizeof (szClientName));
        PrintToChat (client, "\x01\x04[SM-RADIO]\x01 %T", "Welcome", client, szClientName);
        PrintToChat (client, "\x01\x04[SM-RADIO]\x01 %T", "Radio Command Info", client);
    }

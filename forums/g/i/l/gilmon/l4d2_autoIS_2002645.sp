...
public OnPluginStart()
{
	...
	hSpawnWeights[SI_HUNTER] = CreateConVar("l4d2_ais_hunter_weight", "15", "The weight for a hunter spawning", FCVAR_PLUGIN, true, 0.0);
	hSpawnWeights[SI_SMOKER] = CreateConVar("l4d2_ais_smoker_weight", "15", "The weight for a smoker spawning", FCVAR_PLUGIN, true, 0.0);
	...
	hSpawnWeights[SI_SPITTER] = CreateConVar("l4d2_ais_spitter_weight", "15", "The weight for a spitter spawning", FCVAR_PLUGIN, true, 0.0);
	...
	hSpawnLimits[SI_HUNTER] = CreateConVar("l4d2_ais_hunter_limit", "2", "The max amount of hunters present at once", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_SMOKER] = CreateConVar("l4d2_ais_smoker_limit", "2", "The max amount of smokers present at once", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_TANK] = CreateConVar("l4d2_ais_tank_limit", "2", "The max amount of tanks present at once", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_CHARGER] = CreateConVar("l4d2_ais_charger_limit", "2", "The max amount of chargers present at once", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_JOCKEY] = CreateConVar("l4d2_ais_jockey_limit", "2", "The max amount of jockeys present at once", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	...
	hSILimit = CreateConVar("l4d2_ais_limit", "16", "The max amount of special infected at once", FCVAR_PLUGIN, true, 1.0, true, float(MAX_INFECTED));
	hSILimitMax = FindConVar("z_max_player_zombies");
	hSpawnSize = CreateConVar("l4d2_ais_spawn_size", "8", "The amount of special infected spawned at each spawn interval", FCVAR_PLUGIN, true, 1.0, true, float(MAX_INFECTED));
	...
	hSpawnTimeMax = CreateConVar("l4d2_ais_time_max", "30.0", "The maximum auto spawn time (seconds) for infected", FCVAR_PLUGIN, true, 1.0);
...
public CheckPlayer()
{
	CreateTimer(10.0, SetIS, _, TIMER_REPEAT);
}
...
public Action:SetIS(Handle:timer)
{
	if(GetInGamePlayerCount() < 5)
	{
		...
		SILimit = (GetConVarInt(hSILimit) / 4);
	}
	if(GetInGamePlayerCount() < 9 && GetInGamePlayerCount() > 5)
	{
		SpawnSize = (GetConVarInt(hSpawnSize) / 4);
		SILimit = (GetConVarInt(hSILimit) / 2);
	}
	if(GetInGamePlayerCount() > 9)
	{

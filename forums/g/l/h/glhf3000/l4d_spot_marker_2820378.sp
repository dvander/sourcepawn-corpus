...
float gc_fLastTimeMessage[MAXPLAYERS+1];
...
ArrayList weapon_spawn_list;

enum struct weapon_spawn {
	int		entity;
	float 	lastHighlight;
}
...
public void OnPluginStart()
{
    ...
    g_hCvar_SpriteColor        = CreateConVar("l4d_spot_marker_sprite_color", "255 0 255", "Sprite color.\nUse \"random\" for random colors.\nUse three values between 0-255 separated by spaces (\"<0-255> <0-255> <0-255>\").", CVAR_FLAGS);
    ...
    g_hCvar_SpriteScale        = CreateConVar("l4d_spot_marker_sprite_scale", "0.75", "Sprite scale (increases both height and width).\nSome range values maintain the size the same.", CVAR_FLAGS, true, 0.0);
	...
	weapon_spawn_list = 			new ArrayList(sizeof(weapon_spawn)); 
}
...
public void OnButtonPress(int client, int buttons)
{
	char weapon[32];
	GetClientWeapon(client, weapon, 32);
	int isZooming = (buttons & IN_ZOOM);

	for(int i = 0; i < sizeof(bannedWeapons); i++)
	{
		// PrintToChat(client, "%s", bannedWeapons[i]);
		if(isZooming && StrEqual(bannedWeapons[i], weapon))
		{
			return;
		}
	}
	
	if ((buttons & IN_SPEED) && (buttons & IN_USE) || isZooming ) // SHIFT + E
    {
...
void glowHull(int client, float vPos[3])
{
	float vMins[3], vMaxs[3], vEnd[3];
	vMins = view_as<float>({ -50.0, -50.0, -50.0 });
	vMaxs = view_as<float>({ 50.0, 50.0, 50.0 });
	...
	hullEntsCounter = 0;
	TR_EnumerateEntitiesHull(vPos, vEnd, vMins, vMaxs, PARTITION_SOLID_EDICTS, TR_HullEnum, client);
	
	TR_HullEnum_Post();
}

bool TR_HullEnum(int entity, int client)
{
	if(!IsValidEntity(entity)) return true;
	
	if(entity == client) return true;
	
	char classname[64];
	GetEntityClassname(entity, classname, sizeof(classname));

	// if(entity >= 1 && entity <= MaxClients)
		// PrintToChatAll("TR_HullEnum> %i - %N - %s", entity, entity, classname);
	// else
		// PrintToChatAll("TR_HullEnum> %i - %s", entity, classname);
	
	if(!isEntityWeaponSpawn(classname)) return true;
	
	if(hullEntsCounter < sizeof(hullEnts))
	{
		hullEnts[hullEntsCounter] = entity;
		hullEntsCounter++;
...
void TR_HullEnum_Post()
{
	if(hullEntsCounter == 0) return;
	
	char classname[32];
	
	for(int i; i < hullEntsCounter; i++)
	{
		if(!IsValidEntity(hullEnts[i]) || !GetEntityClassname(hullEnts[i], classname, sizeof(classname))) continue;
		
		// PrintToChatAll("glow> %i - %s", hullEnts[i], classname);
		
		CreateTimer(10.1, glowOff, EntIndexToEntRef(hullEnts[i]), TIMER_FLAG_NO_MAPCHANGE);
			
		updateGlowTime(hullEnts[i]);
		
		SetEntProp(hullEnts[i], Prop_Send, "m_iGlowType", 3);
		SetEntProp(hullEnts[i], Prop_Send, "m_glowColorOverride", 255 + (255 * 256) + (255 * 65536));
		SetEntProp(hullEnts[i], Prop_Send, "m_bFlashing", 1);
		SetEntProp(hullEnts[i], Prop_Send, "m_nGlowRangeMin", 0);
		SetEntProp(hullEnts[i], Prop_Send, "m_nGlowRange", 1000);
	}
}

void updateGlowTime(int entity)
{
	weapon_spawn data;
	bool found;
	
	for(int i; i < weapon_spawn_list.Length; i++)
	{
		weapon_spawn_list.GetArray(i, data, sizeof(weapon_spawn));
		
		if(data.entity != entity) continue;
		
		found = true;
		
		data.lastHighlight = GetEngineTime();
		
		weapon_spawn_list.SetArray(i, data, sizeof(weapon_spawn));
	}
	...
	if(!found)
	{
		...
		weapon_spawn_list.PushArray(data, sizeof(weapon_spawn));
	}
}

bool isEntityWeaponSpawn(const char[] classname)
{
	int len = strlen(classname);

	if( len >= 12 && classname[0] == 'w' && classname[6] == '_' && classname[len-6] == '_' && classname[len-5] == 's' && classname[len-1] == 'n' || // weapon_spawn variations
	
		strcmp(classname, "prop_health_cabinet") == 0 ||		// add more if needed
	   
		strcmp(classname, "upgrade_ammo_explosive") == 0 ||
		strcmp(classname, "upgrade_ammo_incendiary") == 0 ||
		strcmp(classname, "upgrade_laser_sight") == 0 ||

		strcmp(classname, "weapon_gascan") == 0 ||
		strcmp(classname, "weapon_defibrillator") == 0 ||
		strcmp(classname, "weapon_first_aid_kit") == 0 ||
		strcmp(classname, "weapon_pain_pills") == 0 ||
		strcmp(classname, "weapon_pipe_bomb") == 0 ||
		strcmp(classname, "weapon_molotov") == 0 ||
		strcmp(classname, "weapon_vomitjar") == 0 ||
		strcmp(classname, "weapon_sniper_awp") == 0
	) return true;
	
	return false;
	
	   // strcmp(classname, "weapon_adrenaline_spawn") == 0 ||
	   // strcmp(classname, "weapon_ammo_spawn") == 0 ||
	   // strcmp(classname, "weapon_chainsaw_spawn") == 0 ||
	   // strcmp(classname, "weapon_defibrillator_spawn") == 0 ||
	   // strcmp(classname, "weapon_first_aid_kit_spawn") == 0 ||
	   // strcmp(classname, "weapon_gascan_spawn") == 0 ||
	   // strcmp(classname, "weapon_item_spawn") == 0 ||
	   // strcmp(classname, "weapon_melee_spawn") == 0 ||
	   // strcmp(classname, "weapon_molotov_spawn") == 0 ||
	   // strcmp(classname, "weapon_pain_pills_spawn") == 0 ||
	   // strcmp(classname, "weapon_pipe_bomb_spawn") == 0 ||
	   // strcmp(classname, "weapon_scavenge_item_spawn") == 0 ||
	   // strcmp(classname, "weapon_upgradepack_explosive_spawn") == 0 ||
	   // strcmp(classname, "weapon_upgradepack_incendiary_spawn") == 0 ||
	   // strcmp(classname, "weapon_vomitjar_spawn") == 0 ||
	   // strcmp(classname, "weapon_autoshotgun_spawn") == 0 ||
	   // strcmp(classname, "weapon_grenade_launcher_spawn") == 0 ||
	   // strcmp(classname, "weapon_hunting_rifle_spawn") == 0 ||
	   // strcmp(classname, "weapon_pistol_magnum_spawn") == 0 ||
	   // strcmp(classname, "weapon_pistol_spawn") == 0 ||
	   // strcmp(classname, "weapon_pumpshotgun_spawn") == 0 ||
	   // strcmp(classname, "weapon_rifle_ak47_spawn") == 0 ||
	   // strcmp(classname, "weapon_rifle_desert_spawn") == 0 ||
	   // strcmp(classname, "weapon_rifle_m60_spawn") == 0 ||
	   // strcmp(classname, "weapon_rifle_sg552_spawn") == 0 ||
	   // strcmp(classname, "weapon_rifle_spawn") == 0 ||
	   // strcmp(classname, "weapon_shotgun_chrome_spawn") == 0 ||
	   // strcmp(classname, "weapon_shotgun_spas_spawn") == 0 ||
	   // strcmp(classname, "weapon_smg_mp5_spawn") == 0 ||
	   // strcmp(classname, "weapon_smg_silenced_spawn") == 0 ||
	   // strcmp(classname, "weapon_smg_spawn") == 0 ||
	   // strcmp(classname, "weapon_sniper_awp_spawn") == 0 ||
	   // strcmp(classname, "weapon_sniper_military_spawn") == 0 ||
	   // strcmp(classname, "weapon_sniper_scout_spawn") == 0 ||
	   // strcmp(classname, "weapon_gascan_spawn") == 0 ||
}
...
void CreateSpotMarker(int client)
{
	...
	{
		if(gc_fLastTimeMessage[client] != 0.0 && GetGameTime() - gc_fLastTimeMessage[client] > 1.0 )
		{
			CPrintToChat(client, "%t", "On Cooldown", RoundToCeil(g_fCvar_Cooldown - GetGameTime() + gc_fLastTime[client]));
			gc_fLastTimeMessage[client] = GetGameTime();
		}
...
Action glowOff(Handle hTimer, int entref)
{
	...
	for(int i; i < weapon_spawn_list.Length; i++)
	{
		...
		if(data.entity == entity && data.lastHighlight + 10.0 < GetEngineTime())
		{

...
#include <sourcemod>
#include <sdktools>
#include <zombieplaguemod>

#define ZP_BUY_COMMAND  "jetpack"
...
public Action:ZP_OnExtraBuyCommand(client, String:sExtraItem[])
{
	...
	// Check items
	if(ZP_ExtraItemRegister(sExtraItem, ZP_BUY_COMMAND))
	{
		// Return ammopacks
		if(ZP_IsPlayerZombie(client) || ZP_GetRoundState(STATE_MAP_ESCAPE) == 1)
		{
			return Plugin_Handled;
		}
...
public ZP_OnClientInfected(client, infector, zombie_class, health, Float:speed, Float:gravity)
{
	gBought[client] = false;
}
...
{
	if (!b_enabled || !IsPlayerAlive(client) || ZP_IsPlayerZombie(client) || Delay[client] || (b_adminsonly && !IsAdmin[client]))
		return Plugin_Continue;

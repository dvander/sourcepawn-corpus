...
public Plugin:myinfo = {
	name        = "Advertisements",
	author      = "Tsunami, gH0sTy",
	description = "Display advertisements",
	version     = PL_VERSION,
	url         = "http://www.tsunami-productions.nl"
};

enum Game { undetected, tf2, other_game };
...
stock Game:GetGameType()
{
	if (GameType == undetected)
	{
		...
		if (StrEqual(modname,"tf",false) || StrEqual(modname,"tf_beta",false))
			GameType=tf2;
		else
			GameType=other_game;
	}
...
public OnPluginStart() {
	g_hAdVersion	  = CreateConVar("sm_advertisements_version", PL_VERSION, "Display advertisements", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	...
	SetConVarString(g_hAdVersion, PL_VERSION, true, true);
}
...
public Action:Timer_DisplayAds(Handle:timer) {
	if (GetConVarBool(g_hEnabled)) {
		...
		if (StrContains(sType, "S") != -1) {
			...
			if (g_bTF2 && StrContains(sText, "{#") != -1) {
				new cStart = StrContains(sText, "{#");
				new cEnd2 = StrContains(sText[cStart + 2], "}") + cStart;
				
				while (cStart != -1) {
					decl String:sPrefix[4] = "-";
					new cEnd = StrContains(sText[cStart + 2], "}");
					
					if (cEnd != -1) {
						decl String:sColorCode[9], String:sName[13];
						
						if(cEnd2 - cStart == 6)
							sPrefix = "\x07";
						else if (cEnd2 - cStart == 8)
							sPrefix = "\x08";
							
						if (!StrEqual(sPrefix, "-")) {
							strcopy(sColorCode, cEnd + 1, sText[cStart + 2]);
							Format(sName, sizeof(sName), "{#%s}", sColorCode);
							Format(sBuffer, sizeof(sBuffer), "%s%s", sPrefix, sColorCode);
							ReplaceString(sText, sizeof(sText), sName, sBuffer);
						}
					}
					
					new cStart2 = StrContains(sText[cStart + 1], "{#") + cStart + 1;
					if (cStart == cStart2) {
						break;
					} else {
						cStart = cStart2;
					}
					cEnd2 = StrContains(sText[cStart + 2], "}") + cStart;
				}
			}

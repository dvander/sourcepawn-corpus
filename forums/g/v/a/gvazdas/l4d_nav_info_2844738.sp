...
1.0.1b (06-June-2026)
    - gvazdas implemented GetZ and added cvar l4d_nav_info_lift
...
#define PLUGIN_AUTHOR                 "Mart,gvazdas"
...
enum struct PluginCvars
{
    ...
    void Init()
    {
        ...
        this.l4d_nav_info_lift     =   CreateConVar("l4d_nav_info_lift", "8.0", "How many units to lift beams off ground.", CVAR_FLAGS, true, 0.0, true, 1000.0);
...
enum struct PluginData
{
    ...
    void GetCvarValues()
    {
        ...
        this.lift = this.cvars.l4d_nav_info_lift.FloatValue;
    }
...
Action Cmd_PrintCvars(int client, int args)
{
    ...
    PrintToConsole(client, "l4d_nav_info_lift : %.1f", plugin.lift);
    PrintToConsole(client, "");
...
Action Cmd_NavInfo(int client, int args)
{
    ...
    if (navAreaId == 0) navAreaId = L4D_GetNavAreaID(navArea);
    float flow = L4D2Direct_GetTerrorNavAreaFlow(navArea);
    PrintToChat(client, "Nav area id %i flow %f", navAreaId, flow);

    bool block_surv_flow = L4D_NavArea_IsBlocked(navArea,TEAM_SURVIVOR,true);
    bool block_surv = L4D_NavArea_IsBlocked(navArea,TEAM_SURVIVOR,false);

    bool block_inf_flow = L4D_NavArea_IsBlocked(navArea,TEAM_INFECTED,true);
    bool block_inf = L4D_NavArea_IsBlocked(navArea,TEAM_INFECTED,false);

    PrintToChat(client, "Blocked: survivors %d %d infected %d %d", block_surv_flow, block_surv, block_inf_flow, block_inf);

    static float vOrigin[3];
    L4D_GetNavAreaPos(navArea, vOrigin);
    PrintToChat(client, "origin: %.1f %.1f %.1f", vOrigin[0], vOrigin[1], vOrigin[2]);
    vOrigin[2] += plugin.lift;

    static float vCenter[3];
    L4D_GetNavAreaCenter(navArea, vCenter);
    PrintToChat(client, "center: %.1f %.1f %.1f", vCenter[0], vCenter[1], vCenter[2]);
    vCenter[2] += plugin.lift;

    static float vSize[3];
    L4D_GetNavAreaSize(navArea, vSize);
    PrintToChat(client, "size: %.1f %.1f %.1f", vSize[0], vSize[1], vSize[2]);
    ...
    // Corner 1 - Corner 2
    vLaserStart = vCenter;
    vLaserStart[0] += vSize[0]/2.0;
    vLaserStart[1] += vSize[1]/2.0;
    vLaserStart[2] = L4D_NavArea_GetZ(navArea, vLaserStart); vLaserStart[2] += plugin.lift;
        vLaserEnd = vCenter;
        vLaserEnd[0] += vSize[0]/2.0;
        vLaserEnd[1] -= vSize[1]/2.0;
        vLaserEnd[2] = L4D_NavArea_GetZ(navArea, vLaserEnd); vLaserEnd[2] += plugin.lift;

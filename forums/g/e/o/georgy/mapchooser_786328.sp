...
public OnMapStart()
{
	AddFileToDownloadsTable("sound/admin_plugin/actions/startyourvoting.mp3");//added
	AddFileToDownloadsTable("sound/admin_plugin/actions/endofvote.mp3");//added
		
	PrecacheSound("admin_plugin/actions/startyourvoting.mp3", true);//added
	PrecacheSound("admin_plugin/actions/endofvote.mp3", true);//added
...
InitiateVote(MapChange:when, Handle:inputlist=INVALID_HANDLE)
{
	...
	EmitSoundToAll("admin_plugin/actions/startyourvoting.mp3");//added
}
						   ...
						   const item_info[][2])
{
	...
	VoteEnded(buffer);
	EmitSoundToAll("admin_plugin/actions/endofvote.mp3");*/
	/* end of $ added */
	...
	switch (action)
	{
		...
		case MenuAction_VoteCancel:
		{
			...
			else
			{
				...
				Format(buffer, sizeof(buffer), "%t", "Cancelled Vote"); // $ added
			}

...
public OnPluginStart()
{
	...
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_minigun", 3);
	...
	SetTrieValue(g_SelfDamageTrie, "tf_weapon_syringegun_medic", 4);
	   ...
	   && IsClientInGame(attacker) && GetClientTeam(attacker) == TEAM_BLUE)
	{
		...
		if(IsValidEntity(GetEntPropEnt(attacker, Prop_Send, "m_hActiveWeapon")) )
		{
			switch(GetEntProp(GetEntPropEnt(attacker, Prop_Send, "m_hActiveWeapon"), Prop_Send, "m_iItemDefinitionIndex"))
			{
				...
				case WEP_MINIGUN, WEP_ROCKETLAUNCHER, WEP_DIRECTHIT: 
				{
					damage = FloatMul(damage, 0.75);
				}
				case WEP_BACKBURNER, WEP_SHOTGUNPYRO: 
				{
					damage = FloatMul(damage, 0.80);
				}
			}
			return Plugin_Changed;
		}
...
public Action:Timer_DoEquipBlu(Handle:timer, any:client)
{
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		if(TF2_GetPlayerClass(client) == TFClass_Engineer)
		{
			...
			if(slot1 > MaxClients && IsValidEntity(slot1) && GetEntProp(slot1, Prop_Send, "m_iItemDefinitionIndex") == WEP_LUGERMORPH)
			{
				TF2_RemoveWeaponSlot(client, 1);
			}

...
public void OnPluginStart()
{
	...
	LoadTranslations("l4d2_BreaksWeapon.phrases");
}
...
public void eventWeapon_Fire(Event event, const char[] name, bool dontBroadcast)
{
	...
	else if(StrEqual(sWeapon, "vomitjar")) 
	{
		CPrintToChatAll("%t", "sVomitjar", client);
	}
}
...
public void eventBreak_Prop(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(IsValidEntity(id) && GetEntityClassname(id, cls, sizeof(cls)) && StrEqual(cls, "prop_physics"))
	{
		...
		{
			if(client > 0 && IsClientConnected(client) && IsClientInGame(client) && GetClientTeam(client) == 2)
		...
		else if (StrEqual(sModel, "models/props_junk/explosive_box001.mdl"))
		{
			if(client > 0 && IsClientConnected(client) && IsClientInGame(client) && GetClientTeam(client) == 2)
				CPrintToChatAll("%t", "sExplosiveBox", client);
		}
	}
}
...
Handle l4d2_join_debuglog, hMaxSurvivors;

bool TeleportReady, gbPlayedAsSurvivorBefore[MAXPLAYERS+1];
...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_addbot", AddBot, ADMFLAG_KICK, "Attempt to add and teleport a survivor bot");//添加一个并传送机器人
	
	RegConsoleCmd("sm_join", JoinTeam, "Attempt to join survivors.", 0);
	RegConsoleCmd("sm_joingame", JoinTeam, "Attempt to join Survivors", 0);//尝试加入幸存者
	RegConsoleCmd("sm_jg", JoinTeam, "Attempt to join Survivors", 0);//尝试加入幸存者
	RegConsoleCmd("sm_jiaru", JoinTeam, "Attempt to join Survivors", 0);//尝试加入幸存者
	
	RegAdminCmd("sm_afk", Spec, ADMFLAG_ROOT);
	RegAdminCmd("sm_jointeam1", Spec, ADMFLAG_ROOT);
	RegAdminCmd("sm_away", Spec, ADMFLAG_ROOT);
	
	hMaxSurvivors = CreateConVar("l4d2_multislots_max_survivors", "13", "允许最大幸存者人数", CVAR_FLAGS, true, 4.0, true, 32.0);//How many survivors allowed?
	...
	HookEvent("player_spawn", 				Event_PlayerSpawn);										// Игрок впервые появился в данной миссии
	HookEvent("round_start", 				Event_RoundStart, 				EventHookMode_PostNoCopy);	// Старт раунда

	l4d2_join_debuglog = CreateConVar("l4d2_join_debuglog", "0", "Debugger on/off", FCVAR_PLUGIN);
}
...
public Action JoinTeam(int client, int args)
{
	if (IsClientInGame(client))
	{
		if (IsClientIdle(client))
		{
			...
			PrintHintText(client, "鼠标左键释放你的抖M之魂吧~");
			return Plugin_Handled;
		}
		...
		if (IsClientInGame(client))
		{
			if (GetClientTeam(client) == 2 && !IsFakeClient(client) && IsPlayerAlive(client))
			{
				...
				if (TeleportReady)
				{
					...
					{
						gbPlayedAsSurvivorBefore[client] = true;
						//PrintHintText(client, "你将在10秒钟之后被扔到队友身边，做好准备~");
...
public Action evtBotReplacedPlayer(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(GetClientTeam(bot) == 2 && client && TotalSurvivors() > 4)
	{
		LogDebug("Игрока %N заменил бот %N", client, bot);
		CreateTimer(1.0, Timer_KickNoNeededBot, bot, TIMER_FLAG_NO_MAPCHANGE);
	}
	return Plugin_Continue;
}
...
{
	if (fakeclient && IsClientInGame(fakeclient) && TotalSurvivors() > 4)
	{
	...
	if (IsClientInGame(client))
	{
		if (!IsFakeClient(client))
		{
			if (GetClientTeam(client) != 2)
			{
				...
				if (!hSetHumanSpec)
				{
					Handle hGameConf = LoadGameConfigFile("join");

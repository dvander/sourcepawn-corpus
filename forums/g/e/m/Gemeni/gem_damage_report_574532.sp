...
public OnPluginStart(){
  ...
  //LoadTranslations("gem_damagereport.phrases");
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
  ...
  if (g_PlayerDROption[victim][propOnOff] == DrON) {
...
BuildDamageString (in_victim, in_attacker) {
  ...
  new String:l_HitboxName[MAXHITGROUPS+1][20];

  //l_HitboxName[0] = 
  //new String:tmp_str[20];
  
  //Format(tmp_str, sizeof(tmp_str), "%T", "hitbox_body", in_victim);
  l_HitboxName[0] = "Body";
  //Format(tmp_str, sizeof(tmp_str), "%T", "hitbox_head", in_victim);
  l_HitboxName[1] = "Head";
  //Format(tmp_str, sizeof(tmp_str), "%T", "hitbox_chest", in_victim);
  l_HitboxName[2] = "Chest";
  //Format(tmp_str, sizeof(tmp_str), "%T", "hitbox_stomach", in_victim);
  l_HitboxName[3] = "Stomach";
  //Format(tmp_str, sizeof(tmp_str), "%T", "hitbox_left_arm", in_victim);
  l_HitboxName[4] = "Left arm";
  //Format(tmp_str, sizeof(tmp_str), "%T", "hitbox_right_arm", in_victim);
  l_HitboxName[5] = "Right arm";
  //Format(tmp_str, sizeof(tmp_str), "%T", "hitbox_left_leg", in_victim);
  l_HitboxName[6] = "Left leg";
  //Format(tmp_str, sizeof(tmp_str), "%T", "hitbox_right_leg", in_victim);
  l_HitboxName[7] = "Right leg";
    ...
    if(g_DamageDone[in_victim][i] >0)
    {
      ...
      for(new j=0; j<=MAXHITGROUPS; j++) {
        if (g_HitboxDone[in_victim][i][j] > 0) {
          Format(damageDoneLong, sizeof(damageDoneLong), "%s%s:%d ", damageDoneLong, l_HitboxName[j], g_HitboxDone[in_victim][i][j]);
        }
      }
    ...
    if(g_DamageTaken[in_victim][i] >0)
    {
      ...
      for(new j=0; j<=MAXHITGROUPS; j++) {
        if (g_HitboxTaken[in_victim][i][j] > 0) {
          Format(damageTakenLong, sizeof(damageTakenLong), "%s%s:%d ", damageTakenLong, l_HitboxName[j], g_HitboxTaken[in_victim][i][j]);
        }
      }
...
public Event_RoundEnd (Handle:event, const String:name[], bool:dontBroadcast)
{
  ...
  new damage, hits, mostDamage, mostDamagePlayer, mostHits;

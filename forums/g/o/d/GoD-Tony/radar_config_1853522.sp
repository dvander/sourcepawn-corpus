...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	{
		FormatEx(error, err_max, "Unsupported game: %s", sGame);
...
public OnRadarModeChange(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	// Feature clamp for CSGO.
	if (g_Game == Game_CSGO && iRadarMode > 1)
		iRadarMode = 1;
...
Client_HideRadar(client)
{
	...
	{
		new hudbits = GetEntProp(client, Prop_Send, "m_iHideHUD");
		SetEntProp(client, Prop_Send, "m_iHideHUD", hudbits | HIDEHUD_RADAR);
	}
}

// 03.03.2021 Fix when the plugin broke the work of the elevators
...
{ 
	GameCheck(); 
	//L4D2Version=false;
	l4d_defi_damage_directhit = CreateConVar("l4d_defi_damage_directhit", "80.0",  "hit damage  ");
	
	l4d_defi_damage_explode = CreateConVar("l4d_defi_damage_explode", "100.0",  "explode damage" );
	l4d_defi_radius_explode = CreateConVar("l4d_defi_radius_explode", "150.0",  "explode  radius" );	
	
	l4d_defi_damage_electricshock = CreateConVar("l4d_defi_damage_electricshock", "80.0",  "electricshock damage " );
	...
	l4d_defi_charge_count_shot = CreateConVar("l4d_defi_charge_count_shot", "20",  "[5, 10]");
	l4d_defi_charge_count_level = CreateConVar("l4d_defi_charge_count_level", "5", "[1, 5]");
	l4d_defi_friendly_damage = CreateConVar("l4d_defi_friendly_damage", "-2.0",  "damage for teamate [-1.0, 100.0] ");	
 	
	AutoExecConfig(true, "defibrillator_l4d");
	
	HookEvent("weapon_fire_on_empty", player_use);  
...
{
	int iSlot5 = GetPlayerWeaponSlot(client, 5);
	if (iSlot5 < 0) {	
			...
			if(index<count)
			{
				...
				if(IsValidEnemy(client, enemy,enemyPos))
				{
					//if(time-ShockTime[client]>0.2)
					{
						...
						if (idInfected(enemy)) DoPointHurtForInfected(enemy, client, GetConVarFloat(l4d_defi_damage_electricshock) );
						ScanIndex[client]++;
...
StartElecShock(client, mode)
{
	...
	new String:sEnt[64];
	GetEntityClassname(victim, sEnt, 64);
	if (StrEqual("prop_physics", sEnt, false) || StrEqual("weapon_gascan", sEnt, false) || StrEqual("prop_fuel_barrel", sEnt, false)) return;
	...
	if (idInfected(victim)) DoPointHurtForInfected(victim, client, GetConVarFloat(l4d_defi_damage_directhit) );
	if(mode==0)
	{
...
bool:idInfected(ent){
	...
	if(StrEqual(class, "infected") || StrEqual(class, "witch") || StrEqual(class, "player")) return true;
	else return false;
	}
...
{  
	...
	{ 	
		...
		GetEventString(event, "weapon", weapon, 254);	
		//PrintToChat(client,"%s",weapon);
		if(StrEqual(weapon, "defibrillator",false))
		{ 
			Start(client);
		} 
	}	

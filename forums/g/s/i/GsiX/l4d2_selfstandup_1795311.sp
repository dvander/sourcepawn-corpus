...
public EVENT_HealSuccess(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ( IsValidSurvivor( victim ))
	{
		...
		{
			PrintToChat( victim, "\x04[\x05GETUP\x04]\x05:  \x04%d \x05of \x04%d", RevCount[victim], g_blackwhite );
		}
	}
}
	...
	if ( IsValidSurvivor( victim ))
	{
		...
		else
		{
			...
			if ( GetEntProp( victim, Prop_Send, "m_bIsOnThirdStrike" ) > NONE )
			{
				...
				if ( g_message > NONE )
				{
					PrintToChatAll( "\x04[\x05GETUP\x04]\x05:  \x04%N \x05on last life!!", victim );
				}
			}
			else
			{
				if ( g_message > NONE )
				{
					PrintToChat( victim, "\x04[\x05GETUP\x04]\x05:  \x04%d \x05of \x04%d, \x05revived by \x04%N", RevCount[victim], g_blackwhite, helper );
				}
			}
		}
	}
	...
	if ( !Button[client] )
	{
		if ( client > NONE && ( buttons & IN_DUCK ))
		{
			...
			if ( target != -1 && IsValidSurvivor( target ) && g_teamdistance > NONE_F && ProgressionTeam( client, target ) == true )
			{
				...
				PrintHintText( client, "[GETUP]: Your target is %N", target );
				return Plugin_Continue;
			}
			...
			if( Attacker[client] > NONE || !IsNo_Incap( client ) || !IsNo_IncapLedge( client ))
			{
				if ( g_costly > NONE && IsValidSlot( client ) == false )
				{
					if ( g_message > NONE )
					{
						...
						{
							PrintHintText( client, "Required Item to Break Free" );
						}
						else if ( !IsNo_Incap( client ) || !IsNo_IncapLedge( client ) )
						{
							PrintHintText( client, "Required Item to Get Up" );
						}
					}
					Button[client] = true;
...
public Action:Timer_Hint( Handle:timer, any:playeR )
{
	...
	{
		PrintHintText( playeR, "Hold CTRL key to help yourself" );
	}
}
	...
	if ( IsValidSurvivor( victim ))
	{
		...
		if ( Incap == true )
		{
			...
			else
			{
				...
				{
					PrintToChat( victim, "\x04[\x05GETUP\x04]\x05:  \x04%d \x05of \x04%d, \x05Revived by \x04%N", RevCount[victim], g_blackwhite, helper );
					PrintToChat( helper, "\x04[\x05GETUP\x04]\x05: Successfully revived \x04%N", victim );
				}
			}
		}
		else
		{
			...
			if ( g_message > NONE )
			{
				PrintToChat( victim, "\x04[\x05GETUP\x04]\x05: Revived by %N", helper );
				PrintToChat( helper, "\x04[\x05GETUP\x04]\x05: Successfully revived \x04%N", victim );
			}
		}
		EmitSoundToClient( victim, SOUND_GETUP );
			...
			if ( IsValidEntity( i ))
			{
				if ( IsValidSpecInfected( i ) || IsValidTank( i ))
				{
					...
					if ( GetVectorDistance( targetPos, playerPos ) <= g_clearance)
					{
						...
						if ( g_message > NONE )
						{
							PrintHintText( victim, "[GETUP]: You too close to %N", i );
						}
						break;
					}
				}
				else
				{
					...
					if ( StrEqual( InfName, "infected", false ))
					{
						...
						if ( GetVectorDistance( targetPos, playerPos ) <= g_clearance)
						{
							...
							if ( g_message > NONE )
							{
								PrintHintText( victim, "[GETUP]: You too close to %s", InfName );
							}
							break;
						}
					}
...
UsePack( client, bool:Msg )
{
	if ( PlayerWeaponSlot[client] != -1 && IsValidEntity( PlayerWeaponSlot[client] ))
	{
		...
		if ( Msg )
		{
			PrintToChat( client, "\x04[\x05GETUP\x04]\x05:  \x04%d \x05of \x04%d\x05,  cost of  \x04%s", RevCount[client], g_blackwhite, slotName );
		}

...
new bool:debugEvent = true;
new bool:debugMSGTimer = false;
new bool:debugTeamTimer = true;
/*      ----        */
...
new bool:g_NoClipCheck[MAXPLAYERS+1];
...
new g_HelpTeamStat[MAXPLAYERS+1];
new g_TeamButtInter[MAXPLAYERS+1];
new g_ChargShove;
new GameMode;
...
public OnClientPutInServer(victim)
{
...
public OnClientDisconnect(victim)
{
	if ( victim > 0 && victim <= MaxClients )
	{
		g_AttackPred[victim] = false;
...
public EVENT_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(IsValidSurvivor(victim))
	{
		...
		g_HelpTeamStat[victim] = 0;
		g_HelpStartTime[victim] = 0.0;
		g_TeamButtInter[victim] = 0;
		g_EnemyScan[victim] = true;
		g_NoClipCheck[victim] = false;
		g_AttackPred[victim] = false;
...
public Action:Timer_HintDelayTeam(Handle:timer, any:victim)
{
	if (IsClientInGame(victim) && IsPlayerAlive(victim))
	{
		PrintHintText(victim, "++ or Hold RELOAD to help team mate ++");
	}
	return Plugin_Stop;
}
...
public Action:Timer_SelfGetUP(Handle:timer, any:client)
{
	...
	if ( IsValidSurvivor( victim ) && g_HelpState[victim] != STATE_CALM && (( !IsNo_Incap( victim ) || !IsNo_IncapLedge( victim )) || ( g_ZombieClass[victim] != NONE )))
	{
		...
		{
			if(debugMSG) PrintToServer("[SELFSTANDUP]: Button DUCK pressed");
			...
			if (g_HelpState[victim] == STATE_GETUP)
			{
				...
				g_Timers[victim] = INVALID_HANDLE;
				if(debugMSG) PrintToServer("[SELFSTANDUP]: HelpState = STATE_GETUP");
				return Plugin_Stop;
			}
	...
	g_Timers[victim] = INVALID_HANDLE;
	if(debugMSG || debugEvent) PrintToServer("[SELFSTANDUP]: HelpState: Timer Terminated");
	return Plugin_Stop;
}
...
public EVENT_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidSurvivor(victim))
	{
		...
		if((!IsNo_Incap(victim) || !IsNo_IncapLedge(victim)) && (g_IdiotHelper[victim] == 0) && (g_Timers[victim] == INVALID_HANDLE) && (g_IncapCheck[victim] == 0) && g_HelpTeamStat[victim] == 0)
		{
			if(!IsValidSlot(victim) && GetConVarInt(selfstandup_costly) > 0)
			{
				if(GetConVarFloat(selfstandup_hint_delay) > 0.0)
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	...
	if ( GetConVarInt( selfstandup_team ) > 0 )
	{
		if (( buttons & IN_RELOAD ) && !IsNo_Incap(client) && IsNo_IncapLedge(client) && g_ZombieClass[client] == NONE && g_IdiotHelper[client] == 0 && g_HelpTeamStat[client] == 0 && g_TeamButtInter[client] == 0 )
		{
			g_TeamButtInter[client] = 1;
			CreateTimer( 0.5, Timer_TeamButtInter, client);
			...
			if ( IsValidSurvivor( target ) && g_IdiotHelper[target] == 0 && (!IsNo_Incap(target) || !IsNo_IncapLedge(client)))
			{
				...
				absDis = GetVectorDistance( targetPos, helperPos );
				...
				if ( absDis < 60.0 )
				{
					...
					if ( g_Timers[target] != INVALID_HANDLE )
					{
						KillTimer( g_Timers[target] );
						g_Timers[target] = INVALID_HANDLE;
					}
					if ( g_NoClipCheck[client] )
					{
						SetEntityMoveType(client, MOVETYPE_WALK);
						g_NoClipCheck[client] = false;
					}
					if ( g_NoClipCheck[target] )
					{
						SetEntityMoveType(target, MOVETYPE_WALK);
						g_NoClipCheck[target] = false;
					}
					g_HelpState[target] = STATE_NONE;
					g_HelpState[client] = STATE_NONE;
					g_HelpTeamStat[client] = target;
					g_HelpTeamStat[target] = client;
					SetEntPropEnt(client, Prop_Send, "m_reviveOwner", -1);
					g_Timers[client] = CreateTimer( 0.1, Timer_HelpingTeamMate, client, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
				}
...
{
	new target = g_HelpTeamStat[client];
		
	if ( IsValidSurvivor( client ) && IsValidSurvivor( target ))
	{
		g_UpIncapOrLedge = GetConVarFloat(selfstandup_duration);
		...
		if (( buttonS & IN_RELOAD )  && g_ZombieClass[client] == NONE && g_IdiotHelper[client] == 0 )
		{
			if ( g_HelpState[target] == STATE_NONE )
			{
				g_AttackPred[client] = true;
				g_AttackPred[target] = true;
				...
				Execute_EventReviveBegin( client, target );
			...
			if ( g_HelpState[target] == STATE_SELFGETUP )
			{
				...
				{
					if ( GetEntPropEnt(target, Prop_Send, "m_reviveOwner") != client )
					{
						SetEntPropEnt(target, Prop_Send, "m_reviveOwner", client);
					}
				...
				if (( timeS - g_HelpStartTime[client] ) > g_UpIncapOrLedge )
				{
					g_AttackPred[client] = false;
					g_AttackPred[target] = false;
					SelfStandUp( target );
					g_HelpState[target] = STATE_GETUP;
			...
			if ( g_HelpState[target] == STATE_GETUP )
			{
				if ( g_Timers[client] != INVALID_HANDLE )
				{
					KillTimer( g_Timers[client] );
				...
				g_HelpTeamStat[client] = 0;
				g_HelpTeamStat[target] = 0;
				g_AttackPred[client] = false;
				g_AttackPred[target] = false;
				if ( debugTeamTimer ) PrintToServer("[SELFSTANDUP]: g_HelpState[target] == STATE_GETUP");
				return Plugin_Stop;
			}
			if ( debugTeamTimer ) PrintToServer("[SELFSTANDUP]: Button team mate reload pressed!!");
			return Plugin_Continue;
		}
		else
		{
			...
			Execute_EventReviveEnd(client, target);
			if ( debugTeamTimer ) PrintToServer("[SELFSTANDUP]: Button team mate reload released!!");
		}
	}
	else
	{
...
public Action:Timer_TeamButtInter(Handle:timer, any:client)
{
	g_TeamButtInter[client] = 0;
}
// code from panxiohai
...
Execute_EventReviveBegin(client, victim)
{
	new Handle:event = CreateEvent("revive_begin");
	if (event == INVALID_HANDLE) {
		return;
	}
	SetEventInt(event, "userid", GetClientUserId(client));		//person doing the reviving
	SetEventInt(event, "subject", GetClientUserId(victim));		// person being revive
	FireEvent(event);
}

Execute_EventReviveEnd(client, victim)
{
...
stock RunHelpingEngine(victim, Float:time, Float:l_Duration)
{
	if (( time - g_HelpStartTime[victim] ) <= l_Duration ) {
		...
		if ((!IsNo_Incap(victim)) && ( g_ZombieClass[victim] == NONE ) && ( GetEntPropEnt(victim, Prop_Send, "m_reviveOwner") != victim )) {
			...
			g_NoClipCheck[victim] = true;
		}
		if( debugMSGTimer ) PrintToServer("[SELFSTANDUP]: RunTime: %f  MaxTime: %f", time - g_HelpStartTime[victim], l_Duration);
	}
	if (( time - g_HelpStartTime[victim] ) > l_Duration ) {
		SelfStandUp( victim );
		g_HelpState[victim] = STATE_GETUP;
		...
		if( debugMSG ) PrintToServer("[SELFSTANDUP]: HelpState = STATE_SELFGETUP engine executed");
	}
}

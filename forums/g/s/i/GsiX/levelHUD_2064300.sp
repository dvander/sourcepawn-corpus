...
public OnClientCookiesCached(client)
{
	...
	CreateTimer(0.1, RefreshStat, GetClientSerial(client), TIMER_REPEAT);
}
...
bool:IsValidKill( Killer, Victim )
{
	if( Killer < 0 || Killer > MaxClients ) return false;
	if( Victim < 0 || Victim > MaxClients ) return false;
	if( IsFakeClient( Killer )) return false;
	if( GetClientTeam( Killer) == GetClientTeam( Victim )) return false;

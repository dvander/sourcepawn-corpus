//////////////////////////////////////////////////////////////////////////////////
// RateChecker 0.3 by pRED*     (modified by BehaartesEtwas)                    //
//                                                                              //
...
{
        name = "RateChecker",
        author = "pRED*, mod. by BehaartesEtwas",
...
public Action:Cmd_RateList(client,args)
{
        ...
        PrintToConsole(client,"Name Rate UpdateRate CmdRate Interp Ping")
		...
		new Float:fping = GetClientAvgLatency(i, NetFlow_Outgoing)
		PrintToConsole(client,"%30s %s %s %s %s %f",name,rate,update,cmd,interp,fping)
        }
}
...
public RatesPrint(client,target)
{
	...
	Format(text,59,"Rate (CVAR): %i",irate)
	DrawPanelItem(Panel, text)
	Format(text,59,"cl_updaterate (CVAR / real): %i / %f/s",iupdate, fout)
	DrawPanelItem(Panel, text)
	Format(text,59,"cl_cmdrate (CVAR / real): %i / %f/s",icmd, fin)
	DrawPanelItem(Panel, text)
	Format(text,59,"cl_interp: %f",finterp)
	DrawPanelItem(Panel, text)
	Format(text,59,"choke / loss: %f%% / %f%%",fchoke*100,floss*100)
	DrawPanelItem(Panel, text)
	Format(text,59,"kB/s (in/out): %f / %f",fdin/1000,fdout/1000)
	DrawPanelItem(Panel, text)
	Format(text,59,"Ping: %f",fping*1000)
	DrawPanelItem(Panel, text)
	DrawPanelItem(Panel, " ", ITEMDRAW_SPACER|ITEMDRAW_RAWLINE)

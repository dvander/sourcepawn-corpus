...
public ClientConVar(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName[], const String:cvarValue[64]){
	smoothtime = cvarValue;
}

public Action:Cmd_RateList(client,args){
	new maxplayers=GetMaxClients();
	
	new String:interp[10],String:update[10],String:cmd[10],String:rate[10],String:name[32],String:targetSteam[19],String:targetIP[18],String:interpolate[10];
	new String:title_name[10]="Name",String:title_steam[10]="SteamID",String:title_ip[10]="IP",String:title_rate[10]="Rate",String:title_cmd[10]="Cmd",String:title_update[10]="Update",String:title_interp[10]="Interp",String:title_interpolate[14]="Interpolate",String:title_smoothtime[18]="Smoothtime";

	if (client != 0 && GetUserFlagBits(client) == 0){
		PrintToConsole(client,"%-30s %-20s %-7s %-4s %-6s %-6s %-12s %-11s",title_name,title_steam,title_rate,title_cmd,title_update,title_interp,title_interpolate,title_smoothtime);
		PrintToConsole(client,"------------------------------------------------------------------------------------------------------");
	} else {
		PrintToConsole(client,"%-30s %-20s %-16s %-7s %-4s %-6s %-6s %-11s %-12s",title_name,title_steam,title_ip,title_rate,title_cmd,title_update,title_interp,title_interpolate,title_smoothtime);
		PrintToConsole(client,"----------------------------------------------------------------------------------------------------------------------");
	}
	...
	for (new i=1; i<=maxplayers;i++){
		...
		GetClientAuthString(i, targetSteam, sizeof(targetSteam));
		if(client == 0 || GetUserFlagBits(client) != 0)
		...
		QueryClientConVar(i, "cl_smoothtime", ConVarQueryFinished:ClientConVar);

		if (client != 0 && GetUserFlagBits(client) == 0){
			PrintToConsole(client,"%-30.28s %-20.19s %-7.5s %-4.4s %-6.4s %-11.5s %-12.5s",name,targetSteam,rate,cmd,update,interp,interpolate,smoothtime);
		} else {
			PrintToConsole(client,"%-30.28s %-20.19s %-16s %-7.5s %-4.4s %-6.4s %-11.5s %-12.5s",name,targetSteam,targetIP,rate,cmd,update,interp,interpolate,smoothtime);
		}
	}
...
public Action:Cmd_Rate(client, args)
{
	...
	new Clients[2];
	new NumClients = FindTarget(client, name);
...
{
	new String:interp[10],String:update[10],String:cmd[10],String:rate[10],String:interpolate[2];
	...
	DrawPanelItem(Panel, text);
	Format(text,59,"cl_interpolate: %i",iinterpolate);
	DrawPanelItem(Panel, text);
	DrawPanelItem(Panel, " ", ITEMDRAW_SPACER|ITEMDRAW_RAWLINE);

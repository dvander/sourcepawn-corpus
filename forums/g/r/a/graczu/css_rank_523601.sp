...
public SQLGetMyRank(Handle:owner, Handle:hndl, const String:error[], any:client){
	...
	if(SQL_FetchRow(hndl)) 
	{
		...
		PrintToChat(client,"%cFrags: %i | Deaths: %i | Headshots: %i | Sucsides: %i", GREEN, RAkills, RAdeaths, RAheadshots, RAsucsides);
	} else {
...
public SQLTopShow(Handle:owner, Handle:hndl, const String:error[], any:client){
		...
		if (SQL_HasResultSet(hndl))
		{
			while (SQL_FetchRow(hndl))
			{
				...
				Format(text,127,"Frags: %i - Deaths: %i", kills, deaths);
				DrawPanelText(Panel, text);

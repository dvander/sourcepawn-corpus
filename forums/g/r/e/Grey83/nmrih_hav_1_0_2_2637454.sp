...
// HUD settings
static const int	iColor[][]		= {{0, 255, 0}, {255, 127, 0}, {255, 255, 255}, {255, 0, 0}};	// R, G, B, A
...
{
	if((m_ArmorValue = FindSendPropInfo("CNMRiH_Player", "m_ArmorValue")) < 1)
		SetFailState("Can't find offset 'm_ArmorValue'!");
//	if((m_iMaxHealth = FindSendPropInfo("CNMRiH_Player", "m_iMaxHealth")) < 1)
		SetFailState("Can't find offset 'm_iMaxHealth'!");
...
{
	if(event.GetInt("state") == 3) for(int i = 1; i <= MaxClients; i++) if(IsClientAuthorized(i)) OnClientPostAdminCheck(i);
}

...
#define CHAT_PREFIX 		"\x01[\x0700DF00HWO\x01]"
...
{
	if (hCvar == hWaveEnable)
	{
		bWaveEnable = bool:StringToInt(newValue);
	}
	else if (hCvar == hObjectiveEnable)
	{
		bObjectiveEnable = bool:StringToInt(newValue);
	}
	else if (hCvar == hNoticeEnable)
	{
		bNoticeEnable = bool:StringToInt(newValue);
	}
	else if (hCvar == hLogEnable)
	{
		bLogEnable = bool:StringToInt(newValue);
	}
	else if (hCvar == hPrefixWaves)
	{
		GetConVarString(hPrefixWaves, sPrefixWaves, sizeof(sPrefixWaves));
	}
	else if (hCvar == hPrefixObjectives)
	{
		GetConVarString(hPrefixObjectives, sPrefixObjectives, sizeof(sPrefixObjectives));
	}
}
...
public Event_Next(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(bSurvival && bWaveEnable)
	{
		...
		if(!bResupply)
		{
			hNumber++;
			SetNewHostname();
			if(bLogEnable) PrintToServer("%s Wave: %d", LOG_PREFIX, hNumber);
			if(bNoticeEnable) PrintToChatAll("%s Wave: \x0700DF00%d", CHAT_PREFIX, hNumber);
		}
		else
		{
			PrintToServer("[HWO] Wave: %d (Resupply)", hNumber);
			if(bNoticeEnable) PrintToChatAll("%s Wave: \x0700DF00%d \x01(Resupply)", CHAT_PREFIX, hNumber);
		}
	}
	else if(!bSurvival && bObjectiveEnable)
	{
		...
		if(bNoticeEnable) PrintToChatAll("%s Objective: \x0700DF00%d", CHAT_PREFIX, hNumber);
	}
}
...
public Event_EB(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(bNoticeEnable) PrintToChatAll("%s Extraction begin", CHAT_PREFIX);
}
...
public Event_PE(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(bNoticeEnable) PrintToChatAll("%s Player \x0700DF00%N \x01has been evacuated", CHAT_PREFIX, client);
}
...
public Event_EC(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(bNoticeEnable) PrintToChatAll("%s Extraction complete", CHAT_PREFIX);
}
...
public Event_EE(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(bNoticeEnable) PrintToChatAll("%s Extraction expire", CHAT_PREFIX);
}

public Event_MC(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(bLogEnable) PrintToServer("%s Map complete", LOG_PREFIX);
	if(bNoticeEnable) PrintToChatAll("%s Map complete", CHAT_PREFIX);
}

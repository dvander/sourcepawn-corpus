...
public Action:Event_Killed(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (0 < client <= MaxClients) Heal(client, i_heal);
...
public Heal(client, i_heal)
{
	if(b_enable)
	{
		if (IsClientInGame(client) && IsPlayerAlive(client) && 0 < client <= MaxClients){
			new health = GetClientHealth(client);
			if(health < (i_heal_max - i_heal)) SetEntityHealth(client, health + i_heal);
			else SetEntityHealth(client, i_heal_max);
		}
	}
}
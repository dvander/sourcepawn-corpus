...
#define PLUGIN_VERSION		"2.1.0_fix"
...
bool g_bInEditMode;
bool g_bSpawnPointOccupied[MAX_SPAWNS];
...
bool g_bInfoMessage[MAXPLAYERS + 1];
bool g_bFirstWeaponSelection[MAXPLAYERS + 1] = {true, ...};
bool g_bWeaponsGivenThisRound[MAXPLAYERS + 1];
...
public void OnClientCookiesCached(int client)
{
	...
	GetClientCookie(client, g_hWeapon_Primary_Cookie, buffer, sizeof(buffer));
	if (buffer[0])
		g_cPrimaryWeapon[client] = buffer;
	else g_cPrimaryWeapon[client] = "none";

	GetClientCookie(client, g_hWeapon_Secondary_Cookie, buffer, sizeof(buffer));
	if (buffer[0])
		g_cSecondaryWeapon[client] = buffer;
	else g_cSecondaryWeapon[client] = "none";

	GetClientCookie(client, g_hWeapon_Remember_Cookie, buffer, sizeof(buffer));
	if (buffer[0])
		g_bRememberChoice[client] = view_as<bool>(StringToInt(buffer));
	else g_bRememberChoice[client] = false;

	GetClientCookie(client, g_hWeapon_First_Cookie, buffer, sizeof(buffer));
	if (buffer[0])
		g_bFirstWeaponSelection[client] = view_as<bool>(StringToInt(buffer));
	else g_bFirstWeaponSelection[client] = false;

	GetClientCookie(client, g_hHSOnly_Cookie, buffer, sizeof(buffer));
	if (buffer[0])
		g_bHSOnlyClient[client] = view_as<bool>(StringToInt(buffer));
...
public void OnMapStart()
{
	...
	for (int i = 1; i <= MaxClients; i++) if (IsClientConnected(i)) ResetClientSettings(i);
	LoadMapConfig();
	if (g_iSpawnPointCount > 0)
	{
		for (int i = 0; i < g_iSpawnPointCount; i++) g_bSpawnPointOccupied[i] = false;
	}
...
void UpdateState()
{
	...
	if (g_cvDM_enabled.BoolValue)
	{
		if (g_cvDM_gun_menu_mode.IntValue)
		{
			...
			for (int i = 1; i <= MaxClients; i++) if (IsClientConnected(i)) SetClientGunModeSettings(i);
			}
		}
		if (g_cvDM_remove_objectives.BoolValue)
			RemoveC4();
...
void SetCashState()
{
	...
	for (int i = 1; i <= MaxClients; i++) if (IsValidClient(i)) SetEntProp(i, Prop_Send, "m_iAccount", 0);
}
...
public Action Event_Say(int client, const char[] command, int argc)
{
	...
	if (g_cvDM_enabled.BoolValue && IsValidClient(client) && (GetClientTeam(client) >= CS_TEAM_T))
	{
		...
		for (int i; i < sizeof(menuTriggers); i++) if (!strcmp(text, menuTriggers[i], false))
		{
		...
		if (g_cvDM_headshot_only_allow_client.BoolValue)
		{
			...
			{
				if (!strcmp(text, hsOnlyTriggers[i], false))
				{
...
public Action Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	if (g_cvDM_enabled.BoolValue)
	{
		...
		if (validAttacker)
		{
			bool knifed = !strcmp(weapon, "knife");
			bool naded = !strcmp(weapon, "hegrenade");
			bool decoy = !strcmp(weapon, "decoy");
			bool inferno = !strcmp(weapon, "inferno");
			bool tactical = !strcmp(weapon, "tagrenade_projectile");
...
public Action OnTraceAttack(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &ammotype, int hitbox, int hitgroup)
{
	...
	if (g_cvDM_no_knife_damage.BoolValue)
	{
		...
		if (!strcmp(knife[7], "knife") || !strcmp(knife[7], "bayonet"))
			return Plugin_Handled;
	}
	...
	if (g_cvDM_headshot_only.BoolValue || (g_bHSOnlyClient[attacker] && g_cvDM_headshot_only_allow_client.BoolValue))
	{
		...
		if (hitgroup == 1
		|| g_cvDM_headshot_only_allow_knife.BoolValue && (!strcmp(weapon[7], "knife") || !strcmp(weapon[7], "bayonet"))
		|| g_cvDM_headshot_only_allow_nade.BoolValue && (!strcmp(grenade, "hegrenade_projectile") || !strcmp(grenade, "decoy_projectile") || !strcmp(grenade, "molotov_projectile") || !strcmp(grenade, "tagrenade_projectile"))
		|| g_cvDM_headshot_only_allow_taser.BoolValue && !strcmp(weapon, "weapon_taser"))
			return Plugin_Continue;
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	if (g_cvDM_no_knife_damage.BoolValue && IsValidClient(attacker))
	{
		char knife[32];
		GetClientWeapon(attacker, knife, sizeof(knife));
	...
	if (!g_cvDM_headshot_only.BoolValue
	&& (!g_cvDM_headshot_only_allow_client.BoolValue || IsValidClient(attacker) && !g_bHSOnlyClient[attacker]))
		return Plugin_Continue;

		if (damagetype & DMG_FALL || !attacker)
			return g_cvDM_headshot_only_allow_world.BoolValue ? Plugin_Continue : Plugin_Handled;
		...
		char weapon[32];
		GetClientWeapon(attacker, weapon, sizeof(weapon));
		if (g_cvDM_headshot_only_allow_knife.BoolValue && (!strcmp(weapon[7], "knife") || !strcmp(weapon[7], "bayonet")))
			return Plugin_Continue;

		char grenade[32];
		GetEdictClassname(inflictor, grenade, sizeof(grenade));
		if (g_cvDM_headshot_only_allow_nade.BoolValue && (!strcmp(grenade, "hegrenade_projectile") || !strcmp(grenade, "decoy_projectile") || !strcmp(grenade, "molotov_projectile") || !strcmp(grenade, "tagrenade_projectile")))
			return Plugin_Continue;

		return g_cvDM_headshot_only_allow_taser.BoolValue && !strcmp(weapon[7], "taser") ? Plugin_Continue : Plugin_Handled;
}
...
void RespawnAll()
{
	for (int i = 1; i <= MaxClients; i++) Timer_Respawn(INVALID_HANDLE, i);
}
...
{
	int client = GetClientFromSerial(serial);
	if (IsValidClient(client) && IsPlayerAlive(client)) SetEntProp(client, Prop_Send, "m_iHideHUD", HIDEHUD_RADAR);
}
...
int GetWeaponAmmoCount(char[] weapon, bool currentClip)
{
	if (!strcmp(weapon[7], "ak47"))
		return currentClip ? 30 : 90;
	if (!strcmp(weapon[7], "m4a1"))
		return currentClip ? 30 : 90;
	if (!strcmp(weapon[7], "m4a1_silencer"))
		return currentClip ? 25 : 75;
	if (!strcmp(weapon[7], "awp"))
		return currentClip ? 10 : 30;
	if (!strcmp(weapon[7], "sg552"))
		return currentClip ? 30 : 90;
	if (!strcmp(weapon[7], "aug"))
		return currentClip ? 30 : 90;
	if (!strcmp(weapon[7], "p90"))
		return currentClip ? 50 : 100;
	if (!strcmp(weapon[7], "galilar"))
		return currentClip ? 35 : 90;
	if (!strcmp(weapon[7], "famas"))
		return currentClip ? 25 : 90;
	if (!strcmp(weapon[7], "ssg08"))
		return currentClip ? 10 : 90;
	if (!strcmp(weapon[7], "g3sg1"))
		return currentClip ? 20 : 90;
	if (!strcmp(weapon[7], "scar20"))
		return currentClip ? 20 : 90;
	if (!strcmp(weapon[7], "m249"))
		return currentClip ? 100 : 200;
	if (!strcmp(weapon[7], "negev"))
		return currentClip ? 150 : 200;
	if (!strcmp(weapon[7], "nova"))
		return currentClip ? 8 : 32;
	if (!strcmp(weapon[7], "xm1014"))
		return currentClip ? 7 : 32;
	if (!strcmp(weapon[7], "sawedoff"))
		return currentClip ? 7 : 32;
	if (!strcmp(weapon[7], "mag7"))
		return currentClip ? 5 : 32;
	if (!strcmp(weapon[7], "mac10"))
		return currentClip ? 30 : 100;
	if (!strcmp(weapon[7], "mp9"))
		return currentClip ? 30 : 120;
	if (!strcmp(weapon[7], "mp7"))
		return currentClip ? 30 : 120;
	if (!strcmp(weapon[7], "ump45"))
		return currentClip ? 25 : 100;
	if (!strcmp(weapon[7], "mp5sd"))
		return currentClip ? 30 : 120; 
	if (!strcmp(weapon[7], "bizon"))
		return currentClip ? 64 : 120;
	if (!strcmp(weapon[7], "glock"))
		return currentClip ? 20 : 120;
	if (!strcmp(weapon[7], "fiveseven"))
		return currentClip ? 20 : 100;
	if (!strcmp(weapon[7], "deagle"))
		return currentClip ? 7 : 35;
	if (!strcmp(weapon[7], "revolver"))
		return currentClip ? 8 : 8;
	if (!strcmp(weapon[7], "hkp2000"))
		return currentClip ? 13 : 52;
	if (!strcmp(weapon[7], "usp_silencer"))
		return currentClip ? 12 : 24;
	if (!strcmp(weapon[7], "p250"))
		return currentClip ? 13 : 26;
	if (!strcmp(weapon[7], "elite"))
		return currentClip ? 30 : 120;
	if (!strcmp(weapon[7], "tec9"))
		return currentClip ? 24 : 120;
	if (!strcmp(weapon[7], "cz75a"))
...
void GiveSavedWeapons(int client, bool primary, bool secondary, bool reset_bot = true)
{
	...
	{
		if (primary && strcmp(g_cPrimaryWeapon[client], "none"))
		{
			if (!strcmp(g_cPrimaryWeapon[client], "random"))
			{
		...
		if (secondary)
		{
			if (strcmp(g_cSecondaryWeapon[client], "none"))
			{
				...
				if (!strcmp(g_cSecondaryWeapon[client], "random"))
				{
					/* Select random menu item (excluding "Random" option) */
			...
			int i, clientTeam = GetClientTeam(client);
			if (clientTeam == CS_TEAM_CT) for (; i < g_cvDM_nades_incendiary.IntValue; i++)
					GivePlayerItem(client, "weapon_incgrenade");
			if (clientTeam == CS_TEAM_T) for (i = 0; i < g_cvDM_nades_molotov.IntValue; i++)
					GivePlayerItem(client, "weapon_molotov");
			for (i = 0; i < g_cvDM_nades_decoy.IntValue; i++)
				GivePlayerItem(client, "weapon_decoy");
			for (i = 0; i < g_cvDM_nades_flashbang.IntValue; i++)
				GivePlayerItem(client, "weapon_flashbang");
			for (i = 0; i < g_cvDM_nades_he.IntValue; i++)
				GivePlayerItem(client, "weapon_hegrenade");
			for (i = 0; i < g_cvDM_nades_smoke.IntValue; i++)
				GivePlayerItem(client, "weapon_smokegrenade");
			for (i = 0; i < g_cvDM_nades_tactical.IntValue; i++)
				GivePlayerItem(client, "weapon_tagrenade");
			g_bWeaponsGivenThisRound[client] = g_cvDM_loadout_style.IntValue < 2;
			g_bRememberChoice[client] = true;
			if (!IsFakeClient(client))
			{
				int weapon
...
void RemoveClientWeapons(int client, bool primary = true, bool secondary = false)
{
	if (IsValidClient(client) && IsPlayerAlive(client))
	{
		...
		for (int i = weapon, entityIndex; i < 4; i++) if (i != 2)	/* Keep knife. */
		{
...
public Action RemoveGroundWeapons(Handle timer)
{
	...
	{
		char class[24];
		for (int i = MaxClients + 1, maxEntities = GetMaxEntities(); i < maxEntities; i++)
			if (IsValidEdict(i) && HasEntProp(i, Prop_Send, "m_hOwnerEntity")
			&& GetEntPropEnt(i, Prop_Send, "m_hOwnerEntity") == -1)
			{
				GetEdictClassname(i, class, sizeof(class));
				if ((strncmp(class, "weapon_", 7) != -1) && (strcmp(class[7], "c4")
				|| g_cvDM_remove_objectives.BoolValue) || (strncmp(class, "item_", 5) != -1))
...
{
	int i = MaxClients + 1
	while((i = FindEntityByClassname(i, "func_buyzone")) != -1) AcceptEntityInput(i, status);
}

void SetObjectives(const char[] status)
{
	int i = MaxClients + 1
	while((i = FindEntityByClassname(i, "func_bomb_target")) != -1) AcceptEntityInput(i, status);
	i = MaxClients + 1
	while((i = FindEntityByClassname(i, "func_hostage_rescue")) != -1) AcceptEntityInput(i, status);
}

void RemoveC4()
{
	for (int i = 1; i <= MaxClients; i++) if (StripC4(i)) break;
}
...
{
	int i = MaxClients + 1
	while((i = FindEntityByClassname(i, "hostage_entity")) != -1) AcceptEntityInput(i, "Kill");
}
...
void BuildSpawnEditorMenu(int client)
{
	...
	FormatEx(editModeItem, sizeof(editModeItem), "%s Edit Mode", (!g_bInEditMode) ? "Enable" : "Disable");
...
public int MenuSpawnEditor(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		else if (!strcmp(info, "Add"))
		{
			AddSpawn(param1);
		}
		else if (!strcmp(info, "Insert"))
		{
			InsertSpawn(param1);
		}
		else if (!strcmp(info, "Delete"))
		{
			int spawnPoint = GetNearestSpawn(param1);
			if (spawnPoint != -1)
			{
		...
		else if (!strcmp(info, "Delete All"))
		{
...
void BuildDisplayWeaponMenu(int client, bool primary)
{
	...
	for (int i = 0; i < weapons.Length; i++)
	{
		...
		if (!strcmp(currentWeapon, weapon))
...
public Action Event_RadioText(UserMsg msg_id, BfRead msg, const int[] players, int playersNum, bool reliable, bool init)
{
	if (g_cvDM_nade_messages.BoolValue)
	{
		...
		{
			if (!strcmp(text, grenadeTriggers[i]))
				return Plugin_Handled;
		}
	}
	return Plugin_Continue;

...
// Models/Sounds
static	String:MDL_BLUE[]	= "models/stickynades/stickynades_blue_fixed.mdl",
		String:MDL_RED[]	= "models/stickynades/stickynades_red_fixed.mdl",
		String:SND_LAUGH[]	= "sound/stickynades/muhahaha.mp3",
		String:SND_SCREAM[]	= "sound/stickynades/scream.mp3";


// Globals
new Handle:g_hHEGrenadeStickPlayers,
	Handle:g_hHEGrenadeStickWalls;

new Handle:g_hHEGrenadeNormalPower,
	Handle:g_hHEGrenadeNormalRadius,
	Handle:g_hHEGrenadeStuckPower,
	Handle:g_hHEGrenadeStuckRadius;

new Handle:g_hHEGrenadeModel,
	Handle:g_hEmitSounds,
	Handle:g_hShake,
	Handle:g_hNotice;

// Values for speed
new bool:v_HEGrenadeStickPlayers,
	bool:v_HEGrenadeStickWalls;

new Float:v_HEGrenadeNormalPower,
	Float:v_HEGrenadeNormalRadius,
	Float:v_HEGrenadeStuckPower,
	Float:v_HEGrenadeStuckRadius;
...
{
	CreateConVar("sm_snlite_version", PL_VERSION, "Allows grenades to stick to walls and players!", FCVAR_DONTRECORD|FCVAR_NOTIFY|FCVAR_SPONLY);

	g_hHEGrenadeStickPlayers = CreateConVar("snlite_hegrenade_stick_to_players", "1",   "Make HE Grenades stick to players", _, true, _, true, 1.0);
	g_hHEGrenadeStickWalls   = CreateConVar("snlite_hegrenade_stick_to_walls",   "1",   "Make HE Grenades stick to walls", _, true, _, true, 1.0);

	g_hHEGrenadeNormalPower  = CreateConVar("snlite_hegrenade_normal_power",     "100", "Power of a HE grenade when not stuck to a player", _, true);
	g_hHEGrenadeNormalRadius = CreateConVar("snlite_hegrenade_normal_radius",    "350", "Radius of a HE grenade when not stuck to a player", _, true);
	g_hHEGrenadeStuckPower   = CreateConVar("snlite_hegrenade_stuck_power",      "300", "Power of a HE grenade when stuck to a player", _, true);
	g_hHEGrenadeStuckRadius  = CreateConVar("snlite_hegrenade_stuck_radius",     "350", "Radius of a HE grenade when stuck to a player", _, true);

	g_hHEGrenadeModel        = CreateConVar("snlite_hegrenade_model",            "0",   "Use a custom model for HE grenades", FCVAR_PRINTABLEONLY);
	g_hEmitSounds            = CreateConVar("snlite_emit_sounds",                "0",   "Play sounds when a player gets stuck", FCVAR_PRINTABLEONLY);
	g_hShake                 = CreateConVar("snlite_shake",                      "1",   "Shake player when hit by a stickynade", _, true, _, true, 1.0);
	g_hNotice                = CreateConVar("snlite_notice",                     "1",   "Display StickyNades information on round start", _, true, _, true, 1.0);
	...
	HookConVarChange(g_hHEGrenadeStickPlayers, CVarChange);
	HookConVarChange(g_hHEGrenadeStickWalls, CVarChange);
	HookConVarChange(g_hHEGrenadeNormalPower, CVarChange);
	HookConVarChange(g_hHEGrenadeNormalRadius, CVarChange);
	HookConVarChange(g_hHEGrenadeStuckPower, CVarChange);
	HookConVarChange(g_hHEGrenadeStuckRadius, CVarChange);
	HookConVarChange(g_hHEGrenadeModel, CVarChange);
	HookConVarChange(g_hEmitSounds, CVarChange);
	HookConVarChange(g_hShake, CVarChange);
	HookConVarChange(g_hNotice, CVarChange);
...
public OnEntityCreated(iEntity, const String:classname[]) 
{
	...
	if(StrEqual(classname, "hegrenade_projectile")) SDKHook(iEntity, SDKHook_SpawnPost, OnEntitySpawned);
}

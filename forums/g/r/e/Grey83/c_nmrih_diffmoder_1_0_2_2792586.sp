...
public void OnPluginStart()
{
	...
	(sv_difficulty = CreateConVar("nmrih_diffmoder", "1", "Enable/Disable plugin.", FCVAR_NOTIFY, true, 0.0, true, 1.0)).AddChangeHook(CvarChange_Enable);
...
public void SDKHookCB_ZombieSpawnPost(int zombie)
{
	...
	ShamblerToRunnerFromPosion(zombie, orgin, (Game_GetMod() == GameMod_Kid));
}
...
stock void Game_ShamblerToRunner(const GameMod mod)
{
	...
	while((entity = FindEntityByClassname(entity, "npc_nmrih_shamblerzombie")) != -1)
	{
		...
		ShamblerToRunnerFromPosion(entity, orgin, kid);
	}
}

stock int ShamblerToRunnerFromPosion(int shambler, float pos[3], bool isKid = false)
{
#if SOURCEMOD_V_MAJOR > 1 || SOURCEMOD_V_MINOR > 9
	RemoveEntity(shambler);
#else
...
stock int FastZombie_Create(float orgin[3], bool isKid = false)
{
	int zombie = CreateEntityByName(isKid ? "npc_nmrih_kidzombie" : "npc_nmrih_turnedzombie");

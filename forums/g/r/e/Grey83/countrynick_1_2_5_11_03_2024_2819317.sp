...
public void OnPluginStart()
{
	...
	cvar = CreateConVar("sm_countrynick_space", "0", "Whether to add a space between tag and name", _, true, _, true, 1.0);
	cvar.AddChangeHook(CVarChanged_Space);
	CVarChanged_Space(cvar, NULL_STRING, NULL_STRING);

	cvar = CreateConVar("sm_countrynick_unknown", "1", "Whether to add a tag for a player whose country is unknown", _, true, _, true, 1.0);
	cvar.AddChangeHook(CVarChanged_Unknown);
	CVarChanged_Unknown(cvar, NULL_STRING, NULL_STRING);

	cvar = CreateConVar("sm_countrynick_flags", "", "The plugin will ignore players with any of these flags", FCVAR_PRINTABLEONLY);
	cvar.AddChangeHook(CVarChanged_Flags);
	CVarChanged_Flags(cvar, NULL_STRING, NULL_STRING);
...
public void CVarChanged_Space(ConVar cvar, const char[] oldValue, const char[] newValue)
{
	bSpace = cvar.BoolValue;

	RefreshNames();
}

public void CVarChanged_Unknown(ConVar cvar, const char[] oldValue, const char[] newValue)
{
	bUnknown = cvar.BoolValue;

	RefreshNames();
}

public void CVarChanged_Flags(ConVar cvar, const char[] oldValue, const char[] newValue)
{
	char flags[24];
...
stock void SetNewName(int client, char[] name)
{
	...
	if(iFlags && iFlags & GetUserFlagBits(client))
	{
		if(pos) SetClientInfo(client, "name", name[pos]);
		return;
	}
	...
	if(!(bLongTag ? GeoipCode3(ip, code3) : GeoipCode2(ip, code2)))
	{
		...
		if(bLongTag) FormatEx(code3, sizeof(code3), "-?-");
		else FormatEx(code2, sizeof(code2), "--");
	}

	Format(name, MAX_NAME_LENGTH, "%c%s%c%s%s", HOOKS[0], bLongTag ? code3 : code2, HOOKS[1], bSpace ? " " : "", name[pos]);

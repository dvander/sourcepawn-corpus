...
public void OnPluginStart()
{
	...
	ConVar cvar;
	cvar = CreateConVar("sm_swh_mode", "3", "Show marks: 0 - disable plugin, 1 - to the Spec, 2 - to the dead allies, 4 - to the dead enemies", _, true, _, true, 7.0);
	cvar.AddChangeHook(CVarChanged_Mode);
	CVarChanged_Mode(cvar, NULL_STRING, NULL_STRING);

	cvar = CreateConVar("sm_swh_color_t", "ff3f1f", "T mark color. Set by HEX (RGB or RRGGBB, values 0 - F or 00 - FF, resp.). Wrong color code = white", FCVAR_PRINTABLEONLY);
	cvar.AddChangeHook(CVarChanged_ColorT);
	SetColor(cvar, iColorT, MARK_CLR_T);

	cvar = CreateConVar("sm_swh_color_ct", "1f3fff", "CT mark color. Set by HEX (RGB or RRGGBB, values 0 - F or 00 - FF, resp.). Wrong color code = white", FCVAR_PRINTABLEONLY);
	cvar.AddChangeHook(CVarChanged_ColorCT);
	SetColor(cvar, iColorCT, MARK_CLR_CT);

	cvar = CreateConVar("sm_swh_access", "", "Flags for access to WH in any state (Root have access if string contain any correct flag)", FCVAR_PRINTABLEONLY);
...
public void CVarChanged_Mode(ConVar cvar, const char[] oldValue, const char[] newValue)
{
	...
	if(!iMode && visible)
		OnPluginEnd();
	else if(iMode && !visible)
		for(int i = 1; i <= MaxClients; i++) if(IsClientInGame(i) && !IsClientSourceTV(i)) CreateMark(i);
}

public void CVarChanged_ColorT(ConVar cvar, const char[] oldValue, const char[] newValue)
{
	SetColor(cvar, iColorT, MARK_CLR_T);
	if(iMode) for(int i = 1; i <= MaxClients; i++) if(IsMarkExist(i) && GetClientTeam(i) == 2) SetMarkColor(i);
}

public void CVarChanged_ColorCT(ConVar cvar, const char[] oldValue, const char[] newValue)
{
	SetColor(cvar, iColorCT, MARK_CLR_CT);
	if(iMode) for(int i = 1; i <= MaxClients; i++) if(IsMarkExist(i) && GetClientTeam(i) == 3) SetMarkColor(i);
}
...
{
	if(!(client = GetClientOfUserId(client)) || !iTeam[client] || IsMarkExist(client) || !IsPlayerAlive(client))
		return Plugin_Stop;

	static int ent;
	if(!iMode || (ent = CreateEntityByName("env_sprite")) == -1)
	{
...
{
	return bAdmin[client] || (iMode & 1) && !iTeam[client] || (!IsPlayerAlive(client)
		&& ((iMode & 2) && iTeam[client] == 2 || (iMode & 4) && iTeam[client] == 3)) ? Plugin_Continue : Plugin_Handled;
}

public Action Hook_TransmitCT(int entity, int client)
{
	return bAdmin[client] || (iMode & 1) && !iTeam[client] || (!IsPlayerAlive(client)
		&& ((iMode & 2) && iTeam[client] == 3 || (iMode & 4) && iTeam[client] == 2)) ? Plugin_Continue : Plugin_Handled;
}

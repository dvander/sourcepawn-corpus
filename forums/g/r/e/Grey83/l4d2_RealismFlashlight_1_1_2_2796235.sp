...
-------------------------------------------------------------------------------
Change Logs:
1.1.2 (Dec-30-2022, by Grey83)
	- Fixed fixed typos in code
...
{
	for(int i, j; i < C_Total; i++) for(j = 0; j < S_Total; j++) g_iFLState[i][j] = -1;
}
...
void SetClientFlashLight(int client)
{
	...
	g_iFLState[client][S_Flashlight] = 0;

	for(int i, power; i < C_Total; i++)	if(GetEntProp(client, Prop_Send, "m_survivorCharacter") == i)
	{
		if(g_iFLState[i][S_Battery] == -1)
		{
			power = GetRandomInt(g_iPowerMin * g_iServerTickrate,  g_iPowerMax * g_iServerTickrate);
			g_iFLState[i][S_Battery] = g_iFLState[i][S_Saved] = power;
...
//			PrintToChatAll("Client:%d|%d#Power Set:%d  -Player_Name- %N", client, i, g_iFLState[i][S_Battery], client);
		...
		if(g_iFLState[i][S_Battery] != -1 && g_iFLState[i][S_Client] != client)
		{
...
public void OnGameFrame()
{
	...
	static int iRandomTime_Off[C_Total], iRandomTime_On[C_Total];

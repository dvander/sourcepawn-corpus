...
static const char
	PL_NAME[]	= "Server WH",
	PL_VER[]	= "1.0.2_toggle 02.06.2022",
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_swh", Cmd_SwichWH, "Swich  WH visibility");
...
public Action Cmd_SwichWH(int client, int args)
{
	if(client && IsClientInGame(client) && IsMarkNeeded())
		ReplyToCommand(client, "[%s] Marks show is %sabled", PL_NAME, (bShow[client] ^= true) ? "en" : "dis");
...
{
	return bShow[client] && CanSee(client, 2) ? Plugin_Continue : Plugin_Handled;
}

public Action Hook_TransmitCT(int entity, int client)
{
	return bShow[client] && CanSee(client, 3) ? Plugin_Continue : Plugin_Handled;
}

...
public void OnPluginStart()
{
	...
	ConVar cvar;
	cvar = CreateConVar("sm_swh_mode", "3", "Show marks to the common players: 0 - don't show, 1 - to the Spec, 2 - to the dead allies, 4 - to the dead enemies, 8 - to the alive allies, 16 - to the alive enemies", _, true, _, true, 31.0);
	cvar.AddChangeHook(CVarChanged_CMode);
	ChangeVisibility(cvar, 0);

	cvar = CreateConVar("sm_swh_adm_mode", "7", "Show marks to the admins: 0 - don't show, 1 - to the Spec, 2 - to the dead allies, 4 - to the dead enemies, 8 - to the alive allies, 16 - to the alive enemies", _, true, _, true, 31.0);
...
{
	return  IsVisible(client,  1) && !iTeam[client]
		|| !IsPlayerAlive(client)
		&& (IsVisible(client,  2) && iTeam[client] == team
		||  IsVisible(client,  4) && iTeam[client] == 5-team)
		|| IsPlayerAlive(client)
		&& (IsVisible(client,  8) && iTeam[client] == team && GetMarkId(client) != entity
		||  IsVisible(client, 16) && iTeam[client] == 5-team);
}

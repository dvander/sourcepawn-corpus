...
// Plugin definitions
#define PLUGIN_VERSION	"1.2_pistol"
...
// Player Settings
//g_PlayerPrimary[MAXPLAYERS+1] = {-1, ...};
...
public OnPluginStart()
{
	...
	CreateConVar("csgo_gunmenu_version", PLUGIN_VERSION, PLUGIN_NAME, FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	if(bLateLoad) for(new i = 1; i <= MaxClients; i++) if(IsClientInGame(i)) OnClientPutInServer(i);
}
...
//		if(g_PlayerPrimary[client_index]==SHOW_MENU && g_PlayerSecondary[client_index]==SHOW_MENU)
		if(g_PlayerSecondary[client_index]==SHOW_MENU)
		{
			...
			if(g_SecondaryMenu != INVALID_HANDLE) DisplayMenu(g_SecondaryMenu, client_index, MENU_TIME_FOREVER);
		}
		else
		{
...
{
	new Teams:client_team = Teams:GetClientTeam(client_index);
	if(IsClientInGame(client_index) && (((client_team == CS_TEAM_T) && g_AllowT)
	|| ((client_team == CS_TEAM_CT) && g_AllowCT)))
	{
		//if(g_PrimaryMenu != INVALID_HANDLE) DisplayMenu(g_PrimaryMenu, client_index, MENU_TIME_FOREVER);
		//else if(g_SecondaryMenu != INVALID_HANDLE) DisplayMenu(g_SecondaryMenu, client_index, MENU_TIME_FOREVER);
		if(g_SecondaryMenu != INVALID_HANDLE) DisplayMenu(g_SecondaryMenu, client_index, MENU_TIME_FOREVER);
...
stock bool:RemoveWeaponBySlot(client_index, Slots:slot)
{
	...
	if(entity_index < MaxClients) return false;
...
stock RemoveBuyZones()
{
	...
	for(new entity_index = MaxClients+1; entity_index < MaxEntities; ++entity_index) if(IsValidEdict(entity_index))
	{

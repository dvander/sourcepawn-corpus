...
static const int	CVARS				= FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_DONTRECORD,
					DEFAULT_FLAGS		= FCVAR_NOTIFY;

static const char	TAG_CHAT[]			= "[SHOW DAMAGE] - ",
					PLUGIN_NAME[]		= "DR.API SHOW DAMAGE",
					PLUGIN_VERSION[]	= "1.2.0_debug2";
...
public void OnPluginStart()
{
	...
	ToogleHook((bDmg = CVar.BoolValue));

	(CVar = AutoExecConfig_CreateConVar("drapi_active_show_damage_dev",		"1",	"Enable/Disable Show Damage Dev", DEFAULT_FLAGS,	true, _, true, 1.0)).AddChangeHook(CVarChanged_DmgDev);
...
public void OnPluginEnd()
{
	for(int i = 1; i <= MaxClients; i++)
	{
		if(!IsClientInGame(i) || hArrayVictim[i] == null) continue;
...
{
	static bool new_val;
	if(bDmg != (new_val = CVar.BoolValue)) ToogleHook((bDmg = new_val));
}

void ToogleHook(bool enable)
{
	static bool hooked;
	if(enable && !hooked)
	{
		...
		PrintToServer("SD>	Event 'player_hurt' hook enabled.");
	}
	else if(!enable && hooked)
	{
		hooked = false;
		UnhookEvent("player_hurt", Event_PlayerHurt);
		PrintToServer("SD>	Event 'player_hurt' hook disabled.");
	}
}
...
public void Event_PlayerHurt(Event event, char[] name, bool dontBroadcast)
{
	...
	if(Client_IsIngame(attacker) && Client_IsIngame((victim = GetClientOfUserId(event.GetInt("userid")))))
	{
		...
		static int type;
		if(!CheckAccessShowDamage(attacker, (type = GetWeaponType(wpn)))) return;

		static float time, TimerDamage[MAXPLAYERS + 1];
		static char old_wpn[MAXPLAYERS + 1][64];

		//We reset all.
		float now = GetEngineTime();
		if(now >= TimerDamage[attacker] || !!strcmp(wpn, old_wpn[attacker], false))
		{
			switch(type)
			{
			...
			strcopy(old_wpn[attacker], 64, wpn);
...
void ShowDamage(char[] weapon, int attacker, int victim, int hitgroup, int count, int damage_health, int damage_armor)
{
	...
	if(count > 1)
		PrintHintText(attacker, "%T", "Show damage multiple", attacker, weapon, count, damage_health, damage_armor);
	else if(!strcmp(weapon, "inferno", false))
		PrintHintText(attacker, "%T", "Show damage inferno", attacker, count, damage_health, damage_armor);
	else if(!strcmp(weapon, "hegrenade", false))
		PrintHintText(attacker, "%T", "Show damage hegrenade", attacker, count, damage_health, damage_armor);
	else
	{
...
{
	return ADMFLAG_CUSTOM1 <= GetUserFlagBits(client) < (1<<21);
}
...
{
	return ADMFLAG_RESERVATION < GetUserFlagBits(client) < ADMFLAG_CUSTOM1;
}

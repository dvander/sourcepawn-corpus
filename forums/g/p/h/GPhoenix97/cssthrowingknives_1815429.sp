...
public Plugin:myinfo = {
	...
	name = NAME,
	author = "meng, G-Phoenix (translations mod)",
	version = VERSION,
...
public OnPluginStart() {
	...
	LoadTranslations("cssthrowingknives.phrases");
...
KnifeCount(client, count) {
	...
	if (IsClientInGame(client)) {
		switch (g_iDisplay) {
			case 1: // Hint
				PrintHintText(client, "%t", "COUNT_TXT", count);
			case 2: { // Key Hint
				static String:sBuffer[64];
				Format(sBuffer, 64, "%t", "COUNT_TXT", count);

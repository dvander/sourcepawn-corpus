//===VS Saxton Hale Mode===
//
//By Rainbolt Dash (aka Dr.Eggman): programmer, modeller, mapper.
...
//Be Really Brutal as Advanced Christi	an Brutal Sniper
...
#define CBSModel "models/player/saxton_hale/cbs.mdl"
...
new Handle:OnJump = INVALID_HANDLE;
new Handle:OnRage = INVALID_HANDLE;
...
public Plugin:myinfo = {
	...
	author = "Rainbolt Dash (aka Dr.Eggman)",
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	OnJump=CreateGlobalForward("VSH_OnDoJump",ET_Hook,Param_Cell,Param_Cell,Param_Cell);
	OnRage=CreateGlobalForward("VSH_OnDoRage",ET_Hook,Param_Cell,Param_Cell,Param_Cell);
...
{				
	...
	RegAdminCmd("hale_special", Command_Select, ADMFLAG_CHEATS, "Call Saxton Hale to next round.");
...
public Action:Timer_Announce(Handle:hTimer)
{
	...
	switch (i)
	{
		case 1: CPrintToChatAll("VS Saxton Hale Mode group: {olive}http://steamcommunity.com/groups/vssaxtonhale{default}");
		case 3:
		{
			new player=FindEntityByClassname(-1, "player");
		...
		case 4:
		{
			...
			if (!FileExists(s))
				...
				CPrintToChatAll("%t %t (experimental)","type_hale_to_open_menu","type_haleach_to_open_ach");
		}
		default: 
		{
			decl String:s[PLATFORM_MAX_PATH];
			...
			if (!FileExists(s))
				...
				CPrintToChatAll("%t %t","type_hale_to_open_menu","type_haleach_to_open_ach");
		}
	}
...
public Action:event_round_end(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (see)
	{
		...
		SetNextMap(s2);
		decl String:s3[256];
		Format(s3,256,"%t","nextmap_hale",s2);
		CPrintToChatAll(s3);
	}
...
public Action:StartHaleTimer(Handle:hTimer)
{
	...
	{	
		...
		playing+=2;
	HaleHealthMax=RoundFloat(Pow(((759.0+playing)*(playing-1)),1.04));
	if (HaleHealthMax==0)
		HaleHealthMax=1322;
...
EquipSaxton(client)
{
	...
	switch (Special)
	{
		case 0:
		{
			SaxtonWeapon = SpawnWeapon(client,"tf_weapon_shovel",196,101,5,"68 ; 2 ; 2 ; 3.1 ; 259 ; 1");
			HaleCharge=0;
		}
		case 2:
		{			
			SaxtonWeapon = SpawnWeapon(client,"tf_weapon_sword",266,101,5,"68 ; 2 ; 2 ; 3.1 ; 259 ; 1");
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
			HaleCharge=-500;
		}
		case 4:
		{
			SaxtonWeapon = SpawnWeapon(client,"tf_weapon_club",171,101,5,"68 ; 2 ; 2 ; 3.1 ; 259 ; 1");
			SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SaxtonWeapon);
			HaleCharge=0;
		...
		default:
		{
			SaxtonWeapon = SpawnWeapon(client,"tf_weapon_wrench",197,101,5,"68 ; 2 ; 2 ; 3.1 ; 259 ; 1");
...
public Action:MakeHale(Handle:hTimer)
{
	...
	if (!RoundState || !IsPlayerAlive(Hale))
		return Plugin_Continue;
...
public Action:MakeNoHale(Handle:hTimer,any:clientid)
{
	...
	if ((client<=0) || !IsValidEdict(client) || !IsClientInGame(client) || !IsPlayerAlive(client) || (RoundState==2) || (client==Hale))
		return Plugin_Continue;		
	...
	if (IsValidEdict(weapon) && (weapon>0))
	{
		...
		switch (index)
		{
			case 41:
			{
				...
				SpawnWeapon(client,"tf_weapon_minigun",15,101,2,"");
			}
			case 45: 
			{
				TF2_RemoveWeaponSlot(client, 0);
			...
			{
				TF2_RemoveWeaponSlot(client, 0);
				SpawnWeapon(client,"tf_weapon_compound_bow",56,101,10,"2 ; 1.5");
			}
			case 141: 
			{
				TF2_RemoveWeaponSlot(client, 0);
				SpawnWeapon(client,"tf_weapon_shotgun_primary",9,101,2,"");
			}
			case 237: 
			{
				TF2_RemoveWeaponSlot(client, 0);
				SpawnWeapon(client,"tf_weapon_rocketlauncher",18,101,2,"");
			}
			case 305: 
			{
				TF2_RemoveWeaponSlot(client, 0);
				SpawnWeapon(client,"tf_weapon_crossbow",305,101,10,"199 ; 4 ; 2 ; 3.0");
			}
			default: if ((index==17) || (index==204) || (index==36))
			{			
				TF2_RemoveWeaponSlot(client, 0);
				SpawnWeapon(client,"tf_weapon_syringegun_medic",36,101,10,"17 ; 0.05");
			}
		}		
	}
	...
	if (IsValidEdict(weapon) && (weapon>0))
	{
		...
		switch (index)
		{
			case 226:
			{
				...
				SpawnWeapon(client,"tf_weapon_shotgun_soldier",199,101,2,"");
			}
			case 265:
			{
				TF2_RemoveWeaponSlot(client, 1);
				weapon = SpawnWeapon(client,"tf_weapon_pipebomblauncher",207,101,2,"");
				SetAmmo(client,1,24);
				SetEntProp(client, Prop_Data, "m_iHealth",175);
				ChangeEdictState(client, GetEntSendPropOffs(client, "m_iHealth"));
			}
			default:  if ((index==57) || (index==58) || (index==231))
					TF2_RemoveWeaponSlot(client, 1);
		}
	}
	...
	if (IsValidEdict(weapon) && (weapon>0))
	{
		...
		switch (index)
		{
			case 38:
			{
				...
				weapon = SpawnWeapon(client,"tf_weapon_fireaxe",192,101,2,"");
			}
			case 43:
			{
				TF2_RemoveWeaponSlot(client,2);
				weapon = SpawnWeapon(client,"tf_weapon_fists",239,101,2,"107 ; 1.3 ; 1 ; 0.5 ; 128 ; 1 ; 191 ; -6");
			}
			case 331:
			{
				TF2_RemoveWeaponSlot(client,2);
				weapon = SpawnWeapon(client,"tf_weapon_fists",195,101,2,"");
			}
			case 355:
			{
				TF2_RemoveWeaponSlot(client, 2);
				weapon = SpawnWeapon(client,"tf_weapon_bat",190,101,2,"");
			}
			case 357:
			{
				TF2_RemoveWeaponSlot(client,2);
				if (TF2_GetPlayerClass(client) == TFClass_DemoMan)
					weapon = SpawnWeapon(client,"tf_weapon_sword",327,101,2,"202 ; 0.5 ; 125 ; -15");
				else
					weapon = SpawnWeapon(client,"tf_weapon_shovel",128,101,2,"115 ; 1");
				
			}
			default: if ((index==132) || (index==266))
			{
				TF2_RemoveWeaponSlot(client,2);
				weapon = SpawnWeapon(client,"tf_weapon_sword",327,101,2,"202 ; 0.5 ; 125 ; -15");
			}
	...
	if (weapon && IsValidEdict(weapon) && (GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex")==60))
	{
		TF2_RemoveWeaponSlot(client,4);
		weapon = SpawnWeapon(client,"tf_weapon_invis",212,101,2,"");
	}
	...
	{
		TF2_RemoveWeaponSlot(client, 1);
		weapon = SpawnWeapon(client,"tf_weapon_medigun",35,101,10,"18 ; 1 ; 10 ; 2.2");
	}	
	return Plugin_Continue;
}
...
public Action:event_lazor(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new client=GetClientOfUserId(GetEventInt(event, "userid"));
	decl String:s[64];
	if (client && IsValidEdict(client) && IsClientInGame(client) && IsPlayerAlive(client))
	{
		new medigun = GetPlayerWeaponSlot(client, 1);
		
		if (medigun && IsValidEdict(medigun))
		{
			GetEdictClassname(medigun, s,64);
			if (StrEqual(s,"tf_weapon_medigun"))
			{
				TF2_AddCondition(client,TFCond_Ubercharged,0.5);	
				new target=GetHealingTarget(client);		
				if (target && IsValidEdict(target) && IsPlayerAlive(target))	
...
public Action:Timer_Lazor(Handle:hTimer,any:medigunid)
{
	...
	if (medigun && IsValidEdict(medigun) && (RoundState==1))
	{
...
public Action:Command_Hale(client, args)
{
	...
	if (StrContains(targetname,"@me",false)>=0)
		ForceHale(client,StrContains(s2,"hidden",false)>0);
	else
		for (new target=1;target<=MaxClients;target++)
			if(IsValidEdict(target) && IsClientInGame(target))		
			{
				...
				{
					ForceHale(target,StrContains(s2,"hidden",false)>=0);
					return Plugin_Continue;	
				}
			}		
	return Plugin_Continue;	
...
ForceHale(client,bool:hidden,bool:forever=false)
{
	if (forever)
		Hale=client;
	else
...
public Action:event_player_spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (client==Hale)
		CreateTimer(0.1, MakeHale, GetClientUserId(client));
	else
...
public Action:HaleTimer(Handle:hTimer)
{
	...
	if (!Hale || !IsValidEdict(Hale) || !IsClientInGame(Hale))
		return Plugin_Continue;
	...
	SetEntPropFloat(Hale, Prop_Data, "m_flMaxspeed", HaleSpeed+0.7*(100-HaleHealth*100/HaleHealthMax)); 
	...
	new RainbowDash=GetClientButtons(Hale);
	if (((RainbowDash & IN_DUCK) || (RainbowDash & IN_ATTACK2)) && (HaleCharge>=0) && !(RainbowDash & IN_JUMP))
	{
		if (Special==2)
		{
			if (HaleCharge+5<50)
...
public Action:DoTaunt(client, const String:command[], argc)
{
	...
	if (HaleRage/RageDMG==1)
	{
		...
		if (Special!=1)
			Call_PushCell(RageDist);
		else
			Call_PushCell(RageDist/3);
		Call_Finish(act);
		if (act != Plugin_Continue)
			return Plugin_Continue;
		switch (Special)
...
public Action:UseRage(Handle:hTimer,any:mode)
{
	...
	if(IsValidEdict(i) && IsClientInGame(i) && IsPlayerAlive(i) && (i!=Hale))
	{
		...
		if (!TF2_IsPlayerInCondition(i,TFCond_Ubercharged) && (((!mode) && (distance<RageDist)) || ((mode) && (distance<RageDist/3))))
		{
...
public Action:event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new client=GetClientOfUserId(GetEventInt(event, "userid"));		
	if (!client || !IsValidEdict(client) || !IsClientConnected(client) ||GetClientHealth(client)>0)
		return Plugin_Continue;		
...
public Action:Timer_Damage(Handle:hTimer,any:id)
{
	...
	if (client && IsValidEdict(client))
		CPrintToChat(client,"\x05%t. %t %i\x01","damage",Damage[client],"scores",RoundFloat(Damage[client]/600.0));
	return Plugin_Continue;
}
...
public Action:event_hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (!IsClientConnected(attacker) || !IsClientConnected(client))
		return Plugin_Continue;
	new ent=-1;
	new bcrit=TF2_IsPlayerInCondition(attacker,TFCond_Buffed);
	if (bcrit)
	{
		new client2;
		while ((client2 = FindEntityByClassname(client2, "player")) != -1)
			if (client==GetHealingTarget(client2))
			{
				SetEventInt(event, "crit",true);
				break;
			}
	}
...
public Action:TakeDamageHook(client, &attacker, &inflictor, &Float:damage, &damagetype)
{
	...
	if ((attacker==Hale) && IsValidEdict(client) && IsClientInGame(client)  && (client>0) && (client!=Hale) && !TF2_IsPlayerInCondition(client,TFCond_Bonked))
	{
		new health=GetClientHealth(client);
		if (TF2_GetPlayerClass(client) == TFClass_DemoMan)
		{
			...
			if (weapon && IsValidEdict(weapon) && GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex")==42)
			{
...
{
	new tHale=lHale+1;
	for (new pingas=0;(!bonplay[tHale] || (tHale==lHale) || !IsClientConnected(tHale) || bSkipNextHale[tHale]) && (pingas<100);pingas++)
	{
		bSkipNextHale[tHale]=false;
		tHale++;			
		if (tHale>MaxClients)
			tHale=0;
...
public Action:QueuePanel(client, Args)
{
	...
	if (RoundCount<2)
	{
		PrintToChat(client,"%t","queue",2-RoundCount);
		return Plugin_Continue;
	}
	new Handle:panel = CreatePanel();
	decl String:s[512];
...
public Action:NewPanel(client, Args)
{
	...
	switch (Args)
	{
		case 0:
		{
			...
			DrawPanelText(panel, "3)No errors in console.");
			...
			DrawPanelText(panel, "7)And no more (I very, very hope) errors in logs.");
		}
		case 134:
		{
			DrawPanelText(panel, "1)Biohazard Christian Brutal Sniper! He's about 20% cooler!");
			DrawPanelText(panel, "2)TF2_IsPlayerInCondition() returns! Fixed some bugs associated with this.");
			DrawPanelText(panel, "3)Now sniper rifle must be 100perc.charged to glow Hale.");
			DrawPanelText(panel, "4)Fixed Vagineer's model (there isn't spam on console and fire on ground).");
		...
		case 133:
		{
			...
			DrawPanelText(panel, "6)Plugin uses TF2_IsPlayerInCondition().");
			DrawPanelText(panel, "7)Other code optimizing.");
			DrawPanelText(panel, "1.33_1 1)Fixed invisible shovel and other little bugs.");
			DrawPanelText(panel, "1.33_1 2)(Not) Fixed bugs with TF2_RemoveCondition");
		}
		case 132:
		{
...
public SkipHalePanelH(Handle:menu, MenuAction:action, param1, param2)
{
	if (IsClientConnected(param1))
	{
		...
		{
			if (param2==1)
				bSkipNextHale[param1]=false;
			else
			{
				SkipHalePanel(FindNextHale(param1),0);
				bSkipNextHale[param1]=true;
			}
			decl String:s[256];
			Format(s,256,"%t","type_hale_to_open_menu");
			CPrintToChat(param1,s);
		}
	}
}
...
{
	if ((Hale>0) && (Hale<=MaxClients) && IsValidEdict(Hale))
...
public Native_GetDamage(Handle:plugin,numParams)
{
	...
	if ((client<=0) || !IsValidEdict(client) || !IsClientInGame(client))
		return 0;

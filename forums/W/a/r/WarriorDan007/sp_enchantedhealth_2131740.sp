...
{
	name = "Admirable Admin - Enchanted Health",
	author = "<eVa>Dog",
	description = "Give a player 1000hp",
	version = PLUGIN_VERSION,
	url = "http://wdfunserver.site.nfoservers.com/wordpress/"
}

public OnPluginStart()
{
	CreateConVar("sm_enchantedhealth_version", PLUGIN_VERSION, " Give Health Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY)

	RegAdminCmd("sm_enchantedhealth", Command_EnchantedHealth, ADMFLAG_SLAY, "sm_enchantedhealth <#userid|name>")
...
public Action:Command_EnchantedHealth(client, args)
{
	...
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_enchantedhealth <#userid|name>");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		{
			PerformEnchantedHealth(client, target_list[i])
		}
	}
	return Plugin_Handled
}

PerformEnchantedHealth(client, target)
{
	if (g_Target[target] == 0)
	{
		SetEntityHealth(target, 1000)
		LogAction(client, target, "\"%L\" changed the health of \"%L\" to 1000hp", client, target)
		ShowActivity(client, "set %N's health to 1000hp", target) 
		g_Target[target] = 1
	}
	else
	{
...
public AdminMenu_Health( Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength )
{
	...
	{
		Format(buffer, maxlength, "Enchanted Health")
	}
...
public MenuHandler_Players(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		{					
			PerformEnchantedHealth(param1, target)
		}

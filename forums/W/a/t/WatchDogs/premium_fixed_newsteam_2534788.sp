...
#define PLUGIN_VERSION "2.0.2.2 [DEV] - Fix"
...
public Player_Spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(client && IsValidEntity(client) && IsPlayerAlive(client) && IsClientInGame(client))
	{
...
public Player_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(cEnable && IsValidEntity(victim) && IsValidEntity(attacker) && attacker != victim && Class == TFClass_Spy)
	{
		...
		if(attacker && IsValidEntity(attacker) && (GetUserFlagBits(attacker) & PREMIUMFLAG || pTrial[attacker] || GetUserFlagBits(attacker) & ROOTFLAG)  && cFakeKill)
		{
			if(weaponindex == 59)
			{			
...
if( IsValidEntity(client)  && IsClientInGame(client) && IsPlayerAlive(client) )
	...
	if(pClass == TFClass_Medic && GetPlayerWeaponSlot(client, 1) != -1)
	SetEntPropFloat(GetPlayerWeaponSlot(client, 1), Prop_Send, "m_flChargeLevel", cUber * 0.01);
	if(pClass != TFClass_Heavy && pClass != TFClass_Medic && pClass != TFClass_Pyro && pClass != TFClass_Sniper && GetPlayerWeaponSlot(client, 0) != -1)

...
{	
	...
	cvar_Flag = 						CreateConVar("sm_veterans_flag", 				"b", 
															"Admin flag that plugin should exclude from checking (Set empty for disable)");
...
ThrowOut(client, const String:reason[])
{
	...
	decl String:sFlag[32];
	GetConVarString(cvar_Flag, sFlag, sizeof(sFlag));
	if(sFlag[0] != '\0' && CheckCommandAccess(clientId, "", ReadFlagString(sFlag), false))
	{
		return;
	}

...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
 ...
 public OnGameFrame()
{
   if(GetConVarInt(FindConVar("spec_freeze_time")) != -1)
	{
        SetConVarInt(FindConVar("spec_freeze_time"), -1);
	}
}
...
public FindCharacters()  //TODO: Investigate KvGotoFirstSubKey; KvGotoNextKey
{
	...
	if(FileExists("sound/freak_fortress_2/error_windows/error_v3.mp3", true))
	{
		AddFileToDownloadsTable("sound/saxton_hale/9000.wav");
		AddFileToDownloadsTable("sound/freak_fortress_2/error_windows/error_v3.mp3");
		PrecacheSound("saxton_hale/9000.wav", true);
		PrecacheSound("freak_fortress_2/error_windows/error_v3.mp3", true);
	}
...
EquipBoss(boss)
{
	...
	for(new i=1; ; i++)
	{
		...
		if(KvJumpToKey(BossKV[Special[boss]], weapon))
		{
			...
			if(attributes[0]!='\0')
			{
				Format(attributes, sizeof(attributes), "68 ; 2.0 ; 2 ; 3.0 ; 252 ; 0.60; %s", attributes);
					//68: +2 cap rate
					//2: x3 damage
			}
			else
			{
				attributes="68 ; 2.0 ; 2 ; 3.0 ; 252 ; 0.60";
					//68: +2 cap rate
					//2: x3 damage
			}
...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:item)
{
	...
	switch(iItemDefinitionIndex)
	{
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "236 ; 1", false);
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
...
public Action:OnCallForMedic(client, const String:command[], args)
{
	...
	if(RoundFloat(BossCharge[boss][0])==100)
	{
		if (RedAlivePlayers == 1)
		{
			PrintHintText(client, "No Raging On last Player Read Rules Please");
			
			EmitSoundToClient(client, "freak_fortress_2/error_windows/error_v3.mp3");
			EmitSoundToClient(client, "freak_fortress_2/error_windows/error_v3.mp3");

...
{	
	...
	if (text[startidx] != CHAT_SYMBOL) {
		if (text[startidx] != CMD_SYMBOL)
			SendNormalChat(client, message);
...
SendNormalChat(client, String:message[])
{		
	if (GetUserAdmin(client) != INVALID_ADMIN_ID)
	{
		if (g_DoColor)
		{
			PrintToChatAll("\x01[ADMIN]\x04 %N: \x03%s", client, message);
		}
		else
		{
			PrintToChatAll("[ADMIN] %N: %s", client, message);
		}
	}
	else
	{
		if (g_DoColor)
		{
			PrintToChatAll("\x04%N: \x03%s", client, message);
		}
		else
		{

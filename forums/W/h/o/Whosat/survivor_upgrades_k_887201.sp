...
public OnPluginStart()
{
	...
	RegConsoleCmd("upghelp2", UserUpgradesHelp);
	...
	ResetOnMissionLost = CreateConVar("surup_reset_on_mission_lost", "0", "Reset all upgrades and re-give initial upgrades on failing a mission?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
/*public Action:SayTextHook(UserMsg:msg_id, Handle:bf, const players[], playersNum, bool:reliable, bool:init)
{
	...
	{
		CreateTimer(0.1, DelayPrintExpire1, client);
		return Plugin_Handled;
	}			
	if(StrContains(message, "ledge_save_expire")!=-1)
	{
		CreateTimer(0.1, DelayPrintExpire2, client);
		return Plugin_Handled;
	}
	if(StrContains(message, "revive_self_expire")!=-1)
	{
		CreateTimer(0.1, DelayPrintExpire3, client);
		return Plugin_Handled;
	}
	if(StrContains(message, "knife_expire")!=-1)
	{
		CreateTimer(0.1, DelayPrintExpire4, client);
		return Plugin_Handled;
	}
...
public Action:DelayPrintExpire1(Handle:hTimer, any:client)
{
	if(GetConVarInt(Verbosity) > 0 && g_bIsSUEnabled==true && g_bBroadcast==true)
	// 							   ^____Prevents warnings when changing maps___^
	{
		decl String:name[64];
		//GetClientName(GetClientOfUserId(GetEventInt(bf, "userid")), name, 64);
		GetClientName(client, name, 64);
		PrintToChatAll("\x01Boomer vomit was stopped by \x04%s's\x01 (now ruined) \x05Raincoat\x01!", name);
	}
}
public Action:DelayPrintExpire2(Handle:hTimer, any:client)
{
	if(GetConVarInt(Verbosity) > 0 && g_bIsSUEnabled==true && g_bBroadcast==true)
	// 							   ^____Prevents warnings when changing maps___^
	{
		...
		//GetClientName(GetClientOfUserId(GetEventInt(bf, "userid")), name, 64);
		GetClientName(client, name, 64);
		PrintToChatAll("\x05Climbing Chalk\x01 was used to help \x04%s\x01 climb back up from a ledge!", name);
	}
}
public Action:DelayPrintExpire3(Handle:hTimer, any:client)
{
	if(GetConVarInt(Verbosity) > 0 && g_bIsSUEnabled==true && g_bBroadcast==true)
	// 							   ^____Prevents warnings when changing maps___^
	{
		...
		//GetClientName(GetClientOfUserId(GetEventInt(bf, "userid")), name, 64);
		GetClientName(client, name, 64);
		PrintToChatAll("\x04%s\x01 got his/her \x05Second Wind\x01 and stood back up!", name);
	}
}
public Action:DelayPrintExpire4(Handle:hTimer, any:client)
{
	if(GetConVarInt(Verbosity) > 0 && g_bIsSUEnabled==true && g_bBroadcast==true)
	// 							   ^____Prevents warnings when changing maps___^
	{
		...
		//GetClientName(GetClientOfUserId(GetEventInt(bf, "userid")), name, 64);
		GetClientName(client, name, 64);
		PrintToChatAll("\x01A \x05Knife\x01 was used by \x04%s\x01 to escape!", name);
	}
}*/
...
public Action:ActivateSU(Handle:hTimer)
{
	if(g_InvalidGameMode) {
		...
		b_IgniteTank = GetConVarInt(IgniteTank);
...
public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(!t_MissionLost) {
		...
		if(GetConVarInt(ResetOnMissionLost)) {
			...
			PrintToServer("%s Server: Round Started after Misson Lost. Upgrades regiven", PLUGIN_TAG);
		}
...
public Action:Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(!t_MissionLost) {
		...
		if(GetConVarInt(ResetOnMissionLost)) {
			PrintToServer("%s Server: Resetting all upgrades!", PLUGIN_TAG);
			ResetValues();
		}
		bBlockUntilRoundStart=true;
		g_bIsSUEnabled=false;
		
		// Reset MissionLost temp variable.
		// Bring this to RoundStart so that upgrades can be re-given using same variable
		//t_MissionLost = false;
	}
	return Plugin_Continue;
}
...
public Action:Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	// Is attacked Survivor? Don't set him aflame!
...
public Action:Event_InfectedHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(b_IgniteTank) {
		decl String:tankn[64];
		GetEntityNetClass(infected, tankn, 64);
		if(strcmp(tankn, "Tank")==0) {
			return Plugin_Continue;
		}
	}
		...
		if(numUpgradesKiller > 0)
		{
			if(killerClient != 0)
			{
				if(GetConVarInt(Verbosity)>2)
				{
					...
					PrintToChatAll("\x01The primary attacker,\x04 %s \x01 also gets:", tname);
				}
				else if (GetConVarInt(Verbosity)>1)
				{
					PrintToChat(killerClient, "As the primary attacker, you also get:");
				}
...
public Action:ShowHelp(Handle:hTimer, any:client)
{
	...
	if(g_InvalidGameMode) {
		...
		PrintToChat(client, "\x05| \x04/upghelp2\x01 to see at what times you get upgrades");
		PrintToChat(client, "\x05| \x04/upghelp\x01 to see this info"); 
	}
}
...
public Action:UserUpgradesHelp(client, args)
{
	// no args required

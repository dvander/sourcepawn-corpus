...
public OnPluginStart()
{
	...
	EnableSounds_Rankmenu_Show = PrecacheSound(StatsSound_MapTime_Improve); // Sound from improving personal map timing
}
...
{
	PrintToConsole(0, "[debug] action_DbPrefixChanged(oldValue='%s', newValue='%s') - DbPrefix='%s'", oldValue, newValue, DbPrefix);
...
QueryClientRank(Client, SQLTCallback:callback=INVALID_HANDLE)
{
	...
	Format(query, sizeof(query), "SELECT COUNT(*) FROM %splayers WHERE points + points_survival + points_realism + points_survivors + points_infected + points_scavenge_survivors + points_scavenge_infected + points_realism_survivors + points_realism_infected >= %i", DbPrefix, ClientPoints[Client]);
...
QueryClientGameModePointsStmID(Client, const String:SteamID[], SQLTCallback:callback=INVALID_HANDLE)
{
	...
	Format(query, sizeof(query), "SELECT points, points_survivors + points_infected, points_realism, points_survival, points_scavenge_survivors + points_scavenge_infected + points_realism_survivors + points_realism_infected FROM %splayers WHERE steamid = '%s'", DbPrefix, SteamID);
...
QueryClientStatsDP_Rank(Client, const String:SteamID[])
{
	...
	Format(query, sizeof(query), "SELECT name, %s, %s, kills, versus_kills_survivors + scavenge_kills_survivors + realism_kills_survivors, headshots FROM %splayers WHERE steamid = '%s'", DB_PLAYERS_TOTALPLAYTIME, DB_PLAYERS_TOTALPOINTS, DbPrefix, SteamID);
...
QueryClientStatsDP_NextRankFull(Client, const String:SteamID[])
{
	...
	Format(query, sizeof(query), "(SELECT name, %s AS totalpoints FROM %splayers WHERE %s >= %i AND steamid <> '%s' ORDER BY totalpoints ASC LIMIT 3) UNION (SELECT name, %i FROM %splayers WHERE steamid = '%s') UNION (SELECT name, %s as totalpoints2 FROM %splayers WHERE %s < %i ORDER BY totalpoints2 DESC LIMIT 3) ORDER BY totalpoints DESC", DB_PLAYERS_TOTALPOINTS, DbPrefix, DB_PLAYERS_TOTALPOINTS, ClientPoints[Client], SteamID, ClientPoints[Client], DbPrefix, SteamID, DB_PLAYERS_TOTALPOINTS, DbPrefix, DB_PLAYERS_TOTALPOINTS, ClientPoints[Client]);
...
CheckCurrentMapDB()
{
	...
	decl String:query[512];
	Format(query, sizeof(query), "SELECT name FROM %smaps WHERE name = '%s' and gamemode = %i", DbPrefix, MapName, GetCurrentGamemodeID());
...
public Action:timer_EndCharge(Handle:timer, any:data)
{
	...
	new Score = 0;
	new String:ScoreSet[MAX_LINE_WIDTH] = "";
	...
	if ((Mode == 1 || Mode == 2) && IsClientConnected(data) && IsClientInGame(data))
		PrintToChat(data, "\x04[\x03RANK\x04] \x01You have earned \x04%i \x01points for charging a \x05Scattering Ram \x01on \x03%i \x01victims!", Score, Counter);
	else if (Mode == 3)
	{
		decl String:AttackerName[MAX_LINE_WIDTH];
		GetClientName(data, AttackerName, sizeof(AttackerName));
		PrintToChatAll("\x04[\x03RANK\x04] \x05%s \x01has earned \x04%i \x01points for charging a \x05Scattering Ram \x01on \x03%i \x01victims!", AttackerName, Score, Counter);
	}
}
...
//PrintToChat(User, "[DEBUG] event_Award => %i", AwardID);
		return;
	}
...
public ClearStatsMaps(client)
{
	...
	SQL_TQuery(db, ClearStatsMapsHandler, "SELECT * FROM %smaps WHERE 1 = 2", client);
}
...
public Action:cmd_TimedMaps(client, args)
{
	...
	decl String:query[256];
	Format(query, sizeof(query), "SELECT DISTINCT tm.gamemode FROM %stimedmaps AS tm INNER JOIN %splayers AS p ON tm.steamid = p.steamid", DbPrefix, DbPrefix);
...
public Action:cmd_ShowPPMs(client, args)
{
	...
	Format(query, sizeof(query), "SELECT steamid, name, (%s) / (%s) AS ppm FROM %splayers WHERE ", DbPrefix, DB_PLAYERS_TOTALPOINTS, DB_PLAYERS_TOTALPLAYTIME);
...
public Action:cmd_ShowRanks(client, args)
{
	...
	Format(query, sizeof(query), "SELECT steamid, name, %s AS totalpoints FROM %splayers WHERE ", DbPrefix, DB_PLAYERS_TOTALPOINTS);
...
public Action:cmd_ShowTop10PPM(client, args)
{
	...
	Format(query, sizeof(query), "SELECT name, (%s) / (%s) AS ppm FROM %splayers WHERE (%s) >= %i ORDER BY ppm DESC, (%s) DESC LIMIT 10", DbPrefix, DB_PLAYERS_TOTALPOINTS, DB_PLAYERS_TOTALPLAYTIME, DB_PLAYERS_TOTALPLAYTIME, GetConVarInt(cvar_Top10PPMMin), DB_PLAYERS_TOTALPLAYTIME);
...
{
	decl String:query[256];
	Format(query, sizeof(query), "SELECT %s as totalpoints, steamid FROM %splayers ORDER BY totalpoints DESC LIMIT %i,1", DbPrefix, DB_PLAYERS_TOTALPOINTS, rank);
...
{
	decl String:query[1024];
	Format(query, sizeof(query), "SELECT (%s) AS totalpoints, steamid, (%s) AS totalplaytime FROM %splayers WHERE (%s) >= %i ORDER BY (totalpoints / totalplaytime) DESC, totalplaytime DESC LIMIT %i,1", DbPrefix, DB_PLAYERS_TOTALPOINTS, DB_PLAYERS_TOTALPLAYTIME, DB_PLAYERS_TOTALPLAYTIME, GetConVarInt(cvar_Top10PPMMin), rank);
	SQL_TQuery(db, GetClientTop10, query, client);
}
			...
			if (ClientTeam == TEAM_SURVIVORS)
			{
				...
				if (GetTrieValue(MapTimingSurvivors, ClientID, enabled))
				{
					if (enabled)
					{
						...
						Format(query, sizeof(query), "SELECT time FROM %stimedmaps WHERE map = '%s' AND gamemode = %i AND difficulty = %i AND steamid = '%s'", DbPrefix, MapName, CurrentGamemodeID, GameDifficulty, ClientID);
...
public UpdateMapTimingStat(Handle:owner, Handle:hndl, const String:error[], any:dp)
{
	...
	if (SQL_GetRowCount(hndl) > 0)
	{
		...
		else
		{
			...
			Format(query, sizeof(query), "UPDATE %stimedmaps SET plays = plays + 1, time = %f, players = %i, modified = NOW() WHERE map = '%s' AND gamemode = %i AND difficulty = %i AND steamid = '%s'", DbPrefix, TotalTime, PlayerCounter, MapName, GamemodeID, GameDifficulty, ClientID);
	...
	else
	{
		...
		Format(query, sizeof(query), "INSERT INTO %stimedmaps (map, gamemode, difficulty, steamid, plays, time, players, modified, created) VALUES ('%s', %i, %i, '%s', 1, %f, %i, NOW(), NOW())", DbPrefix, MapName, GamemodeID, GameDifficulty, ClientID, TotalTime, PlayerCounter);
	}
...
*/
	// using names, ignoring Lan cvar
	GetClientAuthString(client, auth, maxlength);
	if (!StrEqual(auth, "BOT", false)) {
		GetClientName(client, auth, maxlength);
		ReplaceString(auth, sizeof(auth), "<?php", "");
		ReplaceString(auth, sizeof(auth), "<?PHP", "");

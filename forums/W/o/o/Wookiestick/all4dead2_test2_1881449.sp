...
public OnPluginStart() {
	...
	// Minigun Spawning
	RegAdminCmd("a4d_spawnmachinegun", Command_SpawnMinigun, ADMFLAG_SLAY, "Spawns Machine Gun.");
	RegAdminCmd("a4d_smg", Command_SpawnMinigun, ADMFLAG_SLAY, "Spawns Machine Gun.");
	RegAdminCmd("a4d_removemachinegun", Command_RemoveMinigun, ADMFLAG_SLAY, "Remove Machine Gun.");
	RegAdminCmd("a4d_rmg", Command_RemoveMinigun, ADMFLAG_SLAY, "Remove Machine Gun.");
	// Director commands
...
public Action:Menu_CreateItemMenu(client, args) {
	...
	AddMenuItem(menu, "fc", "Spawn an Fireworks Pack");
	...
	AddMenuItem(menu, "gc", "Spawn gnome");
	DisplayMenu(menu, client, MENU_DISPLAY_TIME);
	return Plugin_Handled;
}
...
public Action:Menu_CreateWeaponMenu(client, args) {
	...
	AddMenuItem(menu, "sc", "Spawn a combat shotgun");
	...
	AddMenuItem(menu, "s6", "Spawn a M60"); 
//	AddMenuItem(menu, "s7", "Spawn a Mini Gun"); 
	DisplayMenu(menu, client, MENU_DISPLAY_TIME);
	return Plugin_Handled;
}
/// Handles callbacks from a client using the spawn weapon menu-- this actually spawns it in game
	...
	if (action == MenuAction_Select) {
		switch (itempos) {
			case 0: {
                ...
                Do_SpawnItem(cindex, "Command_SpawnMinigun");
			}
		}

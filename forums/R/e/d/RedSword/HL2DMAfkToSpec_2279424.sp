...
//float g_fEyesAngles[ MAXPLAYERS + 1 ][ 3 ]; //GetClientEyeAngles
//float g_fAngles[ MAXPLAYERS + 1 ][ 3 ];//GetClientAbsAngles
float g_fPositions[ MAXPLAYERS + 1 ][ 3 ];//GetClientAbsOrigin
...
void saveStuff(int clientId)
{
	...
	//GetClientAbsAngles( clientId, g_fAngles[ clientId ] );
...
void checkStuff(int clientId)
{
	...
	//GetClientEyeAngles( clientId, vecEyeAngle );
	//GetClientAbsAngles( clientId, vecAngle );
	GetClientAbsOrigin( clientId, vecPos );
	
	if ( areEquals( vecPos, g_fPositions[ clientId ] ) == true )
	{

...
public Plugin:myinfo =
{
	...
	author = "ReFlexPoison",
	description = "Allows admins to broadcast custom screen overlays to players",
...
{
	CreateConVar("sm_screenoverlays_version", PLUGIN_VERSION, "God Mode Version", FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_DONTRECORD | FCVAR_NOTIFY);

	RegAdminCmd("sm_overlays", Command_ScreenOverlays, ADMFLAG_GENERIC, "Open screen overlays menu");
	RegAdminCmd("sm_reloadoverlays", Command_ReloadScreenOverlays, ADMFLAG_ROOT, "Reloads screen overlays from config");

	BuildPath(Path_SM, g_sConfigFile, sizeof(g_sConfigFile), "configs/screen-overlays.cfg");
...
public Action:Command_ScreenOverlays(iClient, iArgs)
{
	...
	if(g_bOverlayed)
	{
		ReplyToCommand(iClient, "[SM] An overlay is already present.");
		return Plugin_Handled;
	}
...
public Action:Command_ReloadScreenOverlays(iClient, iArgs)
{
	...
	ReplyToCommand(iClient, "[SM] Overlays reloaded.");
	return Plugin_Handled;
}
...
public Menu_ScreenOverlays(iClient)
{
	...
	SetMenuTitle(hMenu, "Screen Overlays");
...
public MenuHandler_ScreenOverlays(Handle:hMenu, MenuAction:iAction, iParam1, iParam2)
{
	...
	if(iAction == MenuAction_Select)
	{
		if(g_bOverlayed)
		{
			ReplyToCommand(iParam1, "[SM] An overlay is already present.");
			return;
		}

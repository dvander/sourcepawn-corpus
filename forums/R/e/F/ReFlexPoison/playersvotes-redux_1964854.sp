...
#include <nextmap>
#include <customvotes>
#include <playersvotes>
#tryinclude <basecomm>

// ====[ DEFINES ]=============================================================
#define PLUGIN_NAME "Players Votes Redux"
...
{
	CreateNative("PlayersVotes_OpenVote", Native_OpenVote);
	RegPluginLibrary("playersvotes");
...
// ====[ NATIVES ]=============================================================
public Native_OpenVote(Handle:hPlugin, iParams)
{
	new iClient = GetNativeCell(1);

	decl String:strType[56];
	GetNativeString(2, strType, sizeof(strType));

	if(StrEqual(strType, "Kick"))
		Menu_DisplayKickVote(iClient);
	else if(StrEqual(strType, "Ban"))
		Menu_DisplayBanVote(iClient);
	if(StrEqual(strType, "Map"))
		Menu_DisplayMapVote(iClient);
	if(StrEqual(strType, "Mute"))
...
public OnPluginStart()
{
	...
	CreateConVar("sm_playersvotes_redux_version", PLUGIN_VERSION, PLUGIN_NAME, FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_DONTRECORD | FCVAR_NOTIFY);

	BuildPath(Path_SM, g_strConfigFile, sizeof(g_strConfigFile), "configs/playersvotes.cfg");

	RegAdminCmd("sm_playersvotes_cancel", Command_Cancel, ADMFLAG_ROOT, "Cancels all votes");
...
// ====[ COMMANDS ]============================================================
public Action:Command_Cancel(iClient, iArgs)
{
	PlayersVotes_ResetBanVotes();
	PlayersVotes_ResetMapVotes();
	PlayersVotes_ResetMuteVotes();
	PlayersVotes_ResetKickVotes();
	ReplyToCommand(iClient, "[SM] All votes canceled.");

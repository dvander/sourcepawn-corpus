...
public OnPluginStart()
{
    ...
    g_hCvarMode = CreateConVar("sm_allchat_mode", "1", "Relays chat messages to all players? 0 = No, 1 = Yes, 2 = If AllTalk On", FCVAR_PLUGIN, true, 0.0, true, 2.0);
    g_hCvarTeam = CreateConVar("sm_allchat_team", "2", "Who can see say_team messages? 0 = Default, 1 = All teammates, 2 = All players", FCVAR_PLUGIN, true, 0.0, true, 2.0);
...
public Action:Hook_UserMessage(UserMsg:msg_id, Handle:bf, const players[], playersNum, bool:reliable, bool:init)
{
	if (GetUserMessageType() == UM_Protobuf)
	{
		...
		g_msgIsChat = bool:PbReadBool(bf, "chat");
		PbReadString(bf, "msg_name", g_msgName, sizeof(g_msgName));
		PbReadString(bf, "params", g_msgText, sizeof(g_msgText), 0);
		PbReadString(bf, "params", g_msgType, sizeof(g_msgType), 1);
	}
	else
	{
...
public Action:Event_PlayerSay(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	&& GetUserMessageType() == UM_Protobuf)
	{
		...
		PbSetString(SayText2, "msg_name", g_msgName);
		PbAddString(SayText2, "params", g_msgText);
		PbAddString(SayText2, "params", g_msgType);

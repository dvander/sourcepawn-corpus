...
public OnPluginStart()
{
	...
	g_hCVShowZones = CreateConVar("sm_antirush_showzones", "0", "Show the zones either when rushing is not allowed?", FCVAR_PLUGIN, true, 0.0, true, 1.0);

	g_hZones = CreateArray();
	g_hFences = CreateArray();

	AddCommandListener(Command_ChatHook, "say");
	AddCommandListener(Command_ChatHook, "say_team");
	...
	else if (StrContains(sGameMod, "dod", false) != -1)
	{
		HookEvent("dod_round_start", Event_OnRoundStart);
		IsDODS = true;
	}
...
{
	if(g_bIsRushAllowed || GetArraySize(g_hZones) == 0 || !GetConVarBool(g_hCVEnable) || IsDODS)
...
public Action:Hook_BlockWeapon(client, weapon)
{
	if(g_bPlayerIsRushing[client])
	{
		...
		|| !StrEqual(sClassName, "weapon_amerknife", false)
		|| !StrEqual(sClassName, "weapon_spade", false))
			return Plugin_Handled;
	}
...
{
	if((!IsDODS && g_bIsRushAllowed) || !GetConVarBool(g_hCVEnable))
	{
		return;
	}
...
public Action:Command_ChatHook(client, const String:command[], args)
{

...
// Plugin Information
//
#define PLUGIN_VERSION "1.055"
public Plugin:myinfo = 
{
    name                    = "Super Zombie Fortress",
    author                = "Oshizu (Fix), Mecha the Slag (Advanced), dirtyminuth (Recode), Sirot (Original)",
...
{ 
    ...
    PrintToChatAll("\x05[SZF]\x01 ZF Enabled. Restarting Round...");
...
public Action:command_zfDisable (client, args)
{
    ...
    zfDisable();
    ServerCommand("mp_restartgame 10");    
    PrintToChatAll("\x05[SZF]\x01 ZF Disabled. Restarting Round...");
...
public Action:command_zfSwapTeams(client, args)
{
    ...
    zfSwapTeams();
    ServerCommand("mp_restartgame 10");
    PrintToChatAll("\x05[SZF]\x01 Team roles swapped. Restarting Round...");
...
public Action:hook_JoinClass(client, const String:command[], argc)
{
    ...
    if(isZom(client))     
    {
        ...
        {
            PrintToChat(client, "\x05[SZF]\x01 Valid zombies: Scout, Heavy, Spy.");
        }
    }
    ...
    else if(isSur(client))
    {
        ...
        {
            PrintToChat(client, "\x05[SZF]\x01 Survivors can't change classes during a round!");
            return Plugin_Handled;                    
        }
        ...
        {
            PrintToChat(client, "\x05[SZF]\x01 Valid survivors: Soldier, Pyro, Demo, Engineer, Medic, Sniper.");
        }             
    }
...
public Action:event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    else
    {
        ...
        PrintToChatAll("\x05[SZF]\x01 Grace period begun. Survivors can change classes.");    
    }
...
EndGracePeriod()
{
    ...
    PrintToChatAll("\x05[SZF]\x01 Grace period complete. Survivors can no longer change classes.");
    ZombieRage(true);
}
    ...
    if (roundState() == RoundActive)
    {
        if (g_iZombieTank > 0 && g_iZombieTank == client && g_iSpecialInfected[client] == INFECTED_NONE)
        {
            ...
            else
            {
                ...
                for (new i = 1; i <= MaxClients; i++)
                {
                    if (validClient(i)) CPrintToChat(i, "{olive}[SZF] {red}Incoming Taaaank!");
                }
            }
...
{     
    ...
    {
        PrintToChat(client, "\x05[SZF]\x01 You have perished, zombifying....");
        spawnClient(client, zomTeam());
    }
...
public help_printZFInfoChat(client)
{
    if(client == 0)
    {
        PrintToChatAll("\x05[SZF]\x01 This server is running Super Zombie Fortress (%s)", PLUGIN_VERSION);
        PrintToChatAll("\x05[SZF]\x01 Type \"/zf\" for info!");        
    }
    else
    {
        PrintToChat(client, "\x05[SZF]\x01 This server is running Super Zombie Fortress (%s)", PLUGIN_VERSION);
        PrintToChat(client, "\x05[SZF]\x01 Type \"/zf\" for info!");
    }
}
    ...
    if (iCount == 1) {
        for (new iLoop = 1; iLoop <= MaxClients; iLoop++) {
            if (IsClientInGame(iLoop) && IsPlayerAlive(iLoop) && isSur(iLoop)) {
                ...
                CPrintToChatAllEx(iLoop, "\x05[SZF]\x01 {teamcolor}%N{default} is the last survivor!", iLoop);
...
CreateZombieSkin(iClient) {   
    ...
    if (TF2_GetPlayerClass(iClient) == TFClass_Heavy) Format(strModel, sizeof(strModel), "models/custom/player/zombie/heavy.mdl");
    if (TF2_GetPlayerClass(iClient) == TFClass_Scout) Format(strModel, sizeof(strModel), "models/custom/player/zombie/scout.mdl");
	if (TF2_GetPlayerClass(iClient) == TFClass_Spy) Format(strModel, sizeof(strModel), "models/custom/player/zombie/spy.mdl");
    if (g_iSpecialInfected[iClient] == INFECTED_TANK) Format(strModel, sizeof(strModel), "models/custom/player/meangreen/heavy.mdl");
...
PrecacheZombieModels() {
    ...
    PrecacheSound("zombie_assist");
    PrecacheSound("zombie_kill");
    PrecacheSound("zombie_kill_2");
    PrecacheSound("zombie_kill_lot");
    PrecacheSound("zombie_stab_death");
    
    PrecacheModel("models/custom/player/meangreen/heavy.mdl", true);
    PrecacheModel("models/custom/player/zombie/heavy.mdl", true);
    PrecacheModel("models/custom/player/zombie/scout.mdl", true);
    PrecacheModel("models/custom/player/zombie/spy.mdl", true);

	AddFileToDownloadsTable("models/custom/player/meangreen/heavy.dx80.vtx");
	AddFileToDownloadsTable("models/custom/player/meangreen/heavy.dx90.vtx");
	AddFileToDownloadsTable("models/custom/player/meangreen/heavy.mdl");
	AddFileToDownloadsTable("models/custom/player/meangreen/heavy.phy");
	AddFileToDownloadsTable("models/custom/player/meangreen/heavy.sw.vtx");
	AddFileToDownloadsTable("models/custom/player/meangreen/heavy.vvd");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/eyeball_invun.vmt"); 
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/eyeball_l.vmt");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/eyeball_r.vmt");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/eye-iris-red.vtf");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/heavy_head.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/heavy_head_blue.vmt");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/heavy_head_blue_invun.vmt");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/heavy_head_blue_invun.vtf");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/heavy_head_red.vmt");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/heavy_head_red_invun.vmt");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/heavy_head_red_invun.vtf");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/hvyweapon_blue.vmt");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/hvyweapon_blue.vtf");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/hvyweapon_blue_invun.vmt");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/hvyweapon_blue_invun.vtf");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/hvyweapon_red.vmt");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/hvyweapon_red.vtf");
	AddFileToDownloadsTable("materials/models/cutsom/player/meangreen/hvyweapon/hvyweapon_red_invun.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/meangreen/hvyweapon/hvyweapon_red_invun.vtf");


	AddFileToDownloadsTable("models/custom/player/zombie/heavy.dx80.vtx");
	AddFileToDownloadsTable("models/custom/player/zombie/heavy.dx90.vtx");
	AddFileToDownloadsTable("models/custom/player/zombie/heavy.mdl");
	AddFileToDownloadsTable("models/custom/player/zombie/heavy.sw.vtx");
	AddFileToDownloadsTable("models/custom/player/zombie/heavy.vvd");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/hide2.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/hide.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/hide_head.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/hvyweapon_blue_gib.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/hvyweapon_blue_gib.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/skeleton.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/skeleton.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/skull.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/skull.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/hvyweapon/skullnormalmap.vtf");
	AddFileToDownloadsTable("models/custom/player/zombie/scout.dx80.vtx");
	AddFileToDownloadsTable("models/custom/player/zombie/scout.dx90.vtx");
	AddFileToDownloadsTable("models/custom/player/zombie/scout.mdl");
	AddFileToDownloadsTable("models/custom/player/zombie/scout.sw.vtx");
	AddFileToDownloadsTable("models/custom/player/zombie/scout.vvd");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/hide2.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/hide.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/hide_head.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/scout_blue_gib.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/scout_blue_gib.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/skeleton.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/skeleton.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/skull.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/skull.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/scout/skullnormalmap.vtf");
	AddFileToDownloadsTable("models/custom/player/zombie/spy.dx80.vtx");
	AddFileToDownloadsTable("models/custom/player/zombie/spy.dx90.vtx");
	AddFileToDownloadsTable("models/custom/player/zombie/spy.mdl");
	AddFileToDownloadsTable("models/custom/player/zombie/spy.sw.vtx");
	AddFileToDownloadsTable("models/custom/player/zombie/spy.vvd");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/spy/spy_blue_gib.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/spy/spy_blue_gib.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/eyes/eyeball_black.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/eyes/heavy_eyeball_l.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/eyes/heavy_eyeball_r.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/eyes/scout_eyeball_l.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/eyes/scout_eyeball_r.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/eyes/spy_eyeball_l.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/eyes/spy_eyeball_r.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/eyeball_black.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/eye-iris-red.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/hide2.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/hide3.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/hide.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/hide_head.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/skeleton.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/skeleton.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/skull.vmt");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/skull.vtf");
	AddFileToDownloadsTable("materials/models/custom/player/zombie/zombie_players/skullnormalmap.vtf");
}
    ...
    if (!bBeginning)
    {
        ...
        for (new i = 1; i <= MaxClients; i++)
        {
            if (IsClientInGame(i))
            {
                ...
                if (isZom(i))
                {
                    CPrintToChat(i, "\x05[SZF]\x01 Zombies are frenzied!");
                }
...
public Action:StopZombieRage(Handle:hTimer)
{
    ...
    if (roundState() == RoundActive) {
        for (new i = 1; i <= MaxClients; i++)
        {
            ...
            {
                CPrintToChat(i, "\x05[SZF]\x01 Zombies are resting...");
            }
        }
    }
}
...
ZombieTank(iCaller = 0)
{
    ...
    for (new i = 1; i <= MaxClients; i++)
    {
        if (validZom(i))
        {
            CPrintToChat(i, "{olive}[SZF] {red}%N{default} was picked based on performance to become the TANK!", g_iZombieTank);
        }
    }

...
////////////////////////////////////////////////////////////////////////////////
/**             EQUIP/REMOVE wearable
 This is based on powerlord's equip/remove wearable calls, if you have these functions somewhere externally, you can replace these.
**/
new Handle:h_RemoveWearable = INVALID_HANDLE;
new Handle:h_EquipWearable = INVALID_HANDLE;
...
public TF2_OnConditionRemoved(client, TFCond:condition)
{
    ...
    else if(gb_Hidden)
    {
        if(condition == TFCond_Cloaked && client == g_boss)
        {
            SetEntPropFloat(client, Prop_Send, "m_flStealthNoAttackExpire", GetGameTime()+0.5);		// attack fast
        }
    }
}
                    ...
                    case ITEM_SHOTGUN:
                        {
                            ...
                            SDKHook(ent, SDKHook_Touch, OnShotgunTouch);							
                        }
            ...
            case ITEM_SHOTGUN:
                {
                    ...
                    SDKHook(ent, SDKHook_Touch, OnShotgunTouchOnce);					
                }
            case ITEM_ROCKETLAUNCHER:
                {
                    DispatchKeyValue(ent2, "model", SPRITE_ROCKETLAUNCHER);
                    SDKHook(ent, SDKHook_StartTouch, OnRocketLauncherTouchOnce);
                    SDKHook(ent, SDKHook_Touch, OnRocketLauncherTouchOnce);					
                }
            case ITEM_BERSERK:
                {
                    DispatchKeyValue(ent2, "model", SPRITE_BERSERK);
                    SDKHook(ent, SDKHook_StartTouch, OnBerserkTouchOnce);
                    SDKHook(ent, SDKHook_Touch, OnBerserkTouchOnce);					
                }
            default:
                {
                    DispatchKeyValue(ent2, "model", SPRITE_INVULN);
                    SDKHook(ent, SDKHook_StartTouch, OnInvulnerabilityTouchOnce);
                    SDKHook(ent, SDKHook_Touch, OnInvulnerabilityTouchOnce);					
                }
            }
...
/**
At some point, valve broke props so that they couldn't be hit by raytrace/touchooked by players (prop_dynamics) with certain collision groups,
this seems to change every few patches. Using a prop_physics seems more reliable, but there's a small chance of a physics bug in some rare situations.
This should? Prevent that.. Mabye. If not, you can just try making them prop_dynamics instead of physics.
**/

...
public OnPluginStart()
{
	...
	g_hCvarEnabled = CreateConVar("sm_servergifts_enabled", "1", "0/1 0=disabled,1=enabled. Enable plugin?");	
	...
	sm_servergifts_bots = CreateConVar("sm_servergifts_bots", "0", "Enable/Disable spawning gifts on bots.", FCVAR_NOTIFY);
	HookConVarChange( sm_servergifts_bots, OnConVarChanged );
	
	sm_servergifts_botsonlyinmvm = CreateConVar("sm_servergifts_botsonlyinmvm", "0", "Enable/Disable spawning gifts on bots only in Mann vs Machine. Requires sm_servergifts_bots to be 1.", FCVAR_NOTIFY);
...
public Action:OnStartTouch(entity, client) 
{
...
{	
	...
	if (bGiftsOnDeath)
	{
		...
		{
			if (deathflags == TF_DEATHFLAG_KILLERDOMINATION || deathflags == TF_DEATHFLAG_ASSISTERDOMINATION || deathflags == TF_DEATHFLAG_KILLERREVENGE || deathflags == TF_DEATHFLAG_ASSISTERREVENGE || deathflags == TF_DEATHFLAG_FIRSTBLOOD || deathflags == TF_DEATHFLAG_DEADRINGER)
			{
				new Float:pos[3];

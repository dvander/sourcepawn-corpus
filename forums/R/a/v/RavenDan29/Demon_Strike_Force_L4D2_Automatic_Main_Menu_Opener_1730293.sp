...
OpenMainMenuMsg(client)
{
	...
	PrintToChat(client, "\x04 Type \x04!%s\x03 in chat to bring up the %s", command, menu);
}
...
WelcomeMsg(client)
{
	...
	PrintToChat(client, "\x03Type \x04!%s\x03 in chat to bring up the \x04%s\x03", command, menu);
}
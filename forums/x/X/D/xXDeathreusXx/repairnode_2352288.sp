...
public SetModel(any:client, any:obj){
	...
	SetEntityModel(obj, model2)
	if(GetClientTeam(client) == 2){
		SetEntProp(obj, Prop_Send, "m_nSkin", 0)
	}
	else{
		SetEntProp(obj, Prop_Send, "m_nSkin", 1)
	}
}
...
public Void:SetHealthOfEnt(any:entl, String:classname[64], any:health){
	...
	if(entvalue < 200){
		...
		AcceptEntityInput(entl, "AddHealth");
	}
}
...
public Action:Event_player_builtobject(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(enabled[client] == true || force == 1 && Donator){
		if(enabledAll == 1){
			GetEntityNetClass(obj, String:strClassName, 64)
			if(strcmp(String:strClassName, "CObjectDispenser", true) == 0){
				SetModel(client, obj)
				new array[2]
				array[0] = obj
				array[1] = client
				RepairNodes[client] = obj
				PerformLimits(obj, "CObjectDispenser")
			}
		}
	}

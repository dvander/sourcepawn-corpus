...
new Handle:g_Cvarspeed = INVALID_HANDLE;
...
public OnPluginStart()
{
  ...
  g_Cvarspeed = CreateConVar("sm_warmupround_speed", "1", "Set how fast players move during speed.");
  ...
  g_speed = GetConVarInt(g_Cvarspeed);
...
public OnSettingChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
  ...
  if (convar == g_Cvarspeed)
  {
    g_speed = StringToInt(newValue);
  }
...
public Action:PlayerSpawnEvent(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (g_enabled && IsWarmup) {
		...
		if (client > 0 && IsClientInGame(client) && GetClientTeam(client) > 1) {
			SetEntPropFloat(client, Prop_Data, "m_flLaggedMovementValue", GetConVarFloat(g_Cvarspeed));
		}
	}
	return Plugin_Continue;
}

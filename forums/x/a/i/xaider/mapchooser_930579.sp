...
new String:g_EndSound[PLATFORM_MAX_PATH];
new Handle:g_Cvar_StartSound = INVALID_HANDLE;
new Handle:g_Cvar_EndSound = INVALID_HANDLE;
//<-
...
{
//added by XAIDER ->
	GetConVarString(g_Cvar_StartSound,g_StartSound,PLATFORM_MAX_PATH);
	GetConVarString(g_Cvar_EndSound,g_EndSound,PLATFORM_MAX_PATH);

	decl String:ss[PLATFORM_MAX_PATH],String:es[PLATFORM_MAX_PATH];
	Format(ss,PLATFORM_MAX_PATH,"sound/%s",g_StartSound);
	Format(es,PLATFORM_MAX_PATH,"sound/%s",g_EndSound);
//<-

	AddFileToDownloadsTable(ss);//added
	AddFileToDownloadsTable(es);//added
		
	PrecacheSound(g_StartSound, true);//added
	PrecacheSound(g_EndSound, true);//added
...
InitiateVote(MapChange:when, Handle:inputlist=INVALID_HANDLE)
{
	...
	Format(buffer, sizeof(buffer), "%T", "Vote Nextmap",g_WaitingForVote);
	SetMenuTitle(g_VoteMenu, buffer);
	...
	{
		Format(buffer, sizeof(buffer), "%T", "Don't Change", g_WaitingForVote);
		AddMenuItem(g_VoteMenu, VOTE_DONTCHANGE, buffer);
	}
	else if (GetConVarBool(g_Cvar_Extend) && g_Extends < GetConVarInt(g_Cvar_Extend))
	{
		Format(buffer, sizeof(buffer), "%T", "Extend Map", g_WaitingForVote);
	...
	EmitSoundToAll(g_StartSound);//added
}
						   ...
						   const item_info[][2])
{
	...
	if (strcmp(map, VOTE_EXTEND, false) == 0)
	{
		...
		PrintCenterTextAll("%t", "Current Map Extended", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
	...
	{
		PrintCenterTextAll("%t", "Current Map Stays", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
...
NextMap(const String:map[])//moved
{
...
// added by X@IDER ->
	g_Cvar_StartSound = CreateConVar("sm_mapvote_startsound", "sourcemod/votestart.mp3", "Sound file for vote start.");
	g_Cvar_EndSound = CreateConVar("sm_mapvote_endsound", "sourcemod/voteend.mp3", "Sound file for vote end.");
//<-
...
VoteTimeRemaining()
{
	new remainingTime = g_VoteTimeStart + GetConVarInt(g_VoteDuration) - GetTime();
	if(remainingTime < 0)
	{
		return 0;
	}

...
new Handle:Path;
new String:Name[400];
new String:authString[40];
new String:Chat[256];
new String:Time[256];
new String:TName[MAX_NAME_LENGTH];
new String:Print[768];
...
public Action:SayP(client, args)
{
	...
	if(client > 0)
	{
		...
		Format(Name, sizeof(Name), "[%s] <%s> %s", Time, authString, Name);
	}
...
public Action:SayT(client, args)
{
	...
	if(client > 0)
	{
		...
		GetClientAuthString(client, authString, sizeof(authString));
		Format(Name, sizeof(Name), "[%s] <%s> %s(%s)", Time, authString, Name, TName);
	}
	Format(Print, sizeof(Print), "%s: %s", Name, Chat);
	if(IsChatTrigger() && GetConVarInt(LogTriggers) == 1) WriteFileLine(Path, Print);
	else if(!IsChatTrigger()) WriteFileLine(Path, Print);
...
public Action:SMSay(client, args)
{
	...
	if(client > 0)
	{
		GetClientAuthString(client, authString, sizeof(authString));
		GetClientName(client, Name, sizeof(Name));
		Format(Name, sizeof(Name), "[%s] <%s> (ALL)%s", Time, authString, Name);
	}	

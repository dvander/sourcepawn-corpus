...
public OnPluginStart()
{
	...
	//CreateNative("IsClientLookingAtTarget", Native_IsClientLookingAtTarget);
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:velocity[3], Float:angles[3], &weapon)
{
	...
	if((class == TFClass_Spy && (StrEqual(Weapon, "tf_weapon_sapper", false) || StrEqual(Weapon, "tf_weapon_builder", false))) || StrEqual(Weapon, "tf_weapon_medigun", false))
		return Plugin_Continue;
	...
	/*else if((g_bCvarAimAndShoot) && IsValidClient(client))
	{
		...
		if(Target[client] > 0)
		{
			if(IsSprite(ClientEyes[Target[client]]))
			{
				...
				if(g_bIsLooking[client][Target[client]])	// If the player is looking at the target
					buttons |= IN_ATTACK;
			}
			else Target[client] = 0;
		}
...
////////////////////////////////////
// Natives start below this point //
////////////////////////////////////

/*public Native_IsClientLookingAtTarget(Handle:plugin, numParams)
{
	return g_bIsLooking[GetNativeCell(1)][GetNativeCell(2)];
}*/
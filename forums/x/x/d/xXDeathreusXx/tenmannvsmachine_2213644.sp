...
new Handle:hRedTimer = INVALID_HANDLE;
new bool:WantsToBeRed = false;
public OnPluginStart()
{
	...
	RegAdminCmd("sm_joinred", Command_JoinRed, 0, "Usage: sm_mvmred to join RED team if on the spectator team");
...
{
	IsMvM(true);
	hRedTimer = CreateTimer(1.0, Timer_TurnToRed, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
}
public OnClientPutInServer(client)
{
	if(!IsMvM()) return;
	if((GetClientTeam(client) == _:TFTeam_Unassigned
...
public Action:Timer_TurnToRed(Handle:timer)
{
	for(new i=1; i<=MaxClients; i++)
	{
		if (!GetConVarBool(hCvarEnabled)) return Plugin_Continue;
		if (IsValidClient(i) && WantsToBeRed == true)
		{
			TurnToRed(i);
		}
	}
...
{
	if (!GetConVarBool(hCvarEnabled)) KillTimer(hRedTimer);

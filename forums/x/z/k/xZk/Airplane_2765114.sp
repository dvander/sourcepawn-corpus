...
ConVar 	cTankChance, cCountAirdrops, cTimeOpen, cColorFlare, cFlare, cFlareLenght, 
		...
		cAirHeight, cRemoveCrate, cRemoveItems, cTimerPlane, cItemsCount, cHealth, cGlowColor;
bool IsLeft4Dead2, gFlares[2048+1], bKill[2048+1];
Handle hTimer[2048], hTimerPlane;

public Plugin myinfo =
{
	name = "[L4D & L4D2] Airdrop",
	author = "BHaType",
...
static const char gModelsItemsList[MAXLIST + 11][] =
{
	...
	"models/weapons/melee/w_shovel.mdl",				//37
	"models/weapons/melee/w_pitchfork.mdl"				//38
};
...
public void OnPluginStart()
{
	...
	cHealth 			=		CreateConVar("airdrop_health"			, 	"300"		, "Set HP for crates, 0:Disable breaking crates by damage hits"			, FCVAR_NONE);
	...
	cGlowColor			=		CreateConVar("airdrop_glow_color"		, 	"0 128 64"	, "Glow color of airdrops (L4D2 only)"						, FCVAR_NONE);
	...
	cMessages 			=		CreateConVar("airdrop_enable_message"	, 	"1"			, "Enable messages?"										, FCVAR_NONE);
	...
	cTimerPlane			=		CreateConVar("airdrop_plane_time"		, 	"120"		, "Timer to create AC130"									, FCVAR_NONE);
...
//fix stuck player by loading bar button
public void OnEntityDestroyed(int entity)
{
	if(entity > MaxClients && IsValidEntity(entity)){
		if(HasEntProp(entity, Prop_Data, "m_bAutoDisable")){
...
void CreateCrates(float vPos[3])
{
	...
	float hpAirdrop = cHealth.FloatValue;
	...
	for(int i; i < count; i++)
	{
		...
		if(hpAirdrop > 0){
			...
			DispatchKeyValueFloat(entity,"health", hpAirdrop);
			DispatchKeyValueFloat(entity,"max_health", hpAirdrop);
		}
		DispatchKeyValue(entity, "model", gModeList[1]);
		...
		SetEntPropVector(iTrigger, Prop_Send, "m_vecMins", vMins);
		SetEntPropVector(iTrigger, Prop_Send, "m_vecMaxs", vMaxs);	
		...
		{
			char sColor[16];
			cGlowColor.GetString(sColor, sizeof(sColor));
...
public void OnBreak(const char[] output, int caller, int activator, float delay)
{
	if(caller && IsValidEntity(caller)){
		...
		for(int i = 0; i <= 4; i++)
		{
			if(gFlares[caller])
			{
				iFlare = EntRefToEntIndex(g_iFlares[caller][i]);
...
void RandomWeapon(float vPos[3])
{
	...
	int items = 0;
	int iSizeItems = (IsLeft4Dead2) ? sizeof gItemsList - 1 : MAXLIST - 18 ;
	for (int i=0; i <= iSizeItems; i++)
	{
		if(items >= count)
			break;
	
		iWeight = GetRandomInt(1, 100);
		if (iWeight <= gItemsChances[i])
		{
			if (i > 25){
				SpawnMelee(gItemsList[iRandom], vPos);
			}else{
				iItem = CreateEntityByName(gItemsList[i]);
				TeleportEntity(iItem, vPos, vAng, NULL_VECTOR);
				DispatchSpawn(iItem);

				SetEntityModel(iItem, gModelsItemsList[i]);
				if(cRemoveItems.FloatValue > 0)
					CreateTimer(cRemoveItems.FloatValue, tToDeleteItems, EntIndexToEntRef(iItem), TIMER_FLAG_NO_MAPCHANGE);
				
				if(gAmmoList[i] != -1)
					SetEntProp(iItem, Prop_Send, "m_iExtraPrimaryAmmo", gAmmoList[i], 4);
			}
			items++;
		}
	}

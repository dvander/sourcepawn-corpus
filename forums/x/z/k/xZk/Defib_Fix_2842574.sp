...
#define PLUGIN_VERSION	"2.0.1.Z.0"
...
int g_iPlayerDeadModel[MAXPLAYERS+1];
...
//replace deadbodies on players join or afk
public void Event_Player_Bot_Replace(Event event, char[] name, bool dontBroadcast)
{
	int player = GetClientOfUserId(event.GetInt("player"));
	int bot    = GetClientOfUserId(event.GetInt("bot"));
	if(player && bot){
		g_iPlayerDeadModel[bot] = g_iPlayerDeadModel[player];
		int deadbody = EntRefToEntIndex(g_iPlayerDeadModel[bot]);
		if(deadbody > 0){
			g_iDeathModelOwner[deadbody] = GetClientUserId(bot);
		}
	} 
}

public void Event_Bot_Player_Replace(Event event, char[] name, bool dontBroadcast)
{
	int player = GetClientOfUserId(event.GetInt("player"));
	int bot    = GetClientOfUserId(event.GetInt("bot"));
	if(player && bot){
		g_iPlayerDeadModel[player] = g_iPlayerDeadModel[bot];
		int deadbody = EntRefToEntIndex(g_iPlayerDeadModel[player]);
		if(deadbody > 0){
			g_iDeathModelOwner[deadbody] = GetClientUserId(player);
		}
	}
}
//remove deadbody on bot kicked
public void Event_PlayerTeam(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(client && disconnect && isbot && oldteam == 2){
		int deadbody = EntRefToEntIndex(g_iPlayerDeadModel[client]);
		if(deadbody > 0){
			RemoveEntity(deadbody);
		}
	}
}
...
public MRESReturn OnStartActionPre(Handle hReturn, Handle hParams)
{
	...
	int iDeathModel = DHookGetParam(hParams, 3);
	if(!iDeathModel)
		return MRES_Ignored;

	int iPlayerDead = GetClientOfUserId(g_iDeathModelOwner[iDeathModel]);
	if(iPlayerDead){
		int iClient = DHookGetParam(hParams, 2);
		PrintHintText(iClient, "Reviving to:\n%N", iPlayerDead);
		PrintHintText(iPlayerDead, "%N\nis reviving you", iClient);
		//PrintToChatAll("%N is Reviving to:\n%N", iClient, iPlayerDead);
	}
...
public MRESReturn DeathModelCreatePost(int pThis, Handle hReturn)
{
	...
	g_iPlayerDeadModel[g_iTempClient] = EntIndexToEntRef(iDeathModel);

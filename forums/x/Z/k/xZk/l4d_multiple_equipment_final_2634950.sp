...
public void OnPluginStart()
{
	...
	HookEvent("player_use", eAmmoPickupUse, EventHookMode_Pre);
	HookEvent("ammo_pickup", eAmmoPickup, EventHookMode_Pre);
...
public void OnClientPostAdminCheck(int client)
{
	...
	if (GetConVarInt(AmmoLockSlot0) == 1 || GetConVarInt(AmmoLockSlot1) == 1)
		CreateTimer(0.1, LockAmmo, client, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
...
public Action LockAmmo(Handle timer, any client)
{
	...
	if (StrEqual(sWeapon, "weapon_rifle_m60", false) || StrEqual(sWeapon, "weapon_melee", false) ||
		StrEqual(sWeapon, "weapon_pipe_bomb", false) || StrEqual(sWeapon, "weapon_molotov", false) || StrEqual(sWeapon, "weapon_vomitjar", false))
		return Plugin_Continue;
	...
	if(L4D2Version && GetEntProp(weapon, Prop_Send, "m_nUpgradedPrimaryAmmoLoaded", 1))
		return Plugin_Continue;

	if (Ammo <= 0 && Clip <= 1)
	{
		if(weapon == slot0 && GetConVarInt(AmmoLockSlot0) == 1)
		{
			SetEntProp(weapon, Prop_Send, "m_iClip1", 1);
			SetEntPropFloat(weapon, Prop_Send, "m_flNextPrimaryAttack", GetGameTime() + 1000.0);
		}
		else if(weapon == slot1 && GetConVarInt(AmmoLockSlot1) == 1)
		{
			SetEntProp(weapon, Prop_Send, "m_iClip1", 1);
			SetEntPropFloat(weapon, Prop_Send, "m_flNextPrimaryAttack", GetGameTime() + 1000.0);
		}
...
public Action eAmmoPickup(Handle event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
...
public Action eAmmoPickupUse(Handle event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
	...
	if (GetVectorDistance(cPos, aPos) <= GetConVarInt(AmmoUseDistance))
	{
		if (weapon == slot0 && GetConVarInt(AmmoLockSlot0) == 1)
			SetEntPropFloat(weapon, Prop_Send, "m_flNextPrimaryAttack", GetGameTime() + 0.1);
		else if (weapon == slot1 && GetConVarInt(AmmoLockSlot1) == 1)
...
static bool IsSurvivorThirdPerson(int iClient)
{
	...
	if(GetEntPropEnt(iClient, Prop_Send, "m_hViewEntity") > 0)
		return true;
	if(L4D2Version && GetEntPropFloat(iClient, Prop_Send, "m_TimeForceExternalView") > GetGameTime())
		return true;
	if(GetEntProp(iClient, Prop_Send, "m_iObserverMode") == 1)
		return true;
	if(L4D2Version && GetEntPropEnt(iClient, Prop_Send, "m_pummelAttacker") > 0)
		return true;
	if(L4D2Version && GetEntPropEnt(iClient, Prop_Send, "m_carryAttacker") > 0)
		return true;
	if(GetEntPropEnt(iClient, Prop_Send, "m_pounceAttacker") > 0)
		return true;
	if(L4D2Version && GetEntPropEnt(iClient, Prop_Send, "m_jockeyAttacker") > 0)
		return true;

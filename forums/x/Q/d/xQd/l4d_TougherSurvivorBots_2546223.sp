...
float sbDamageMult
float sbDamageSpecialMult
float sbDamageCommonMult
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3]){
	...
	if (victim > 0 && victim <= MaxClients && IsClientConnected(victim) && IsClientInGame(victim) && GetClientTeam(victim) == 2 && IsFakeClient(victim) && !IsClientIncapacitated(victim) && (damagetype & DMG_BULLET) == 0) 
	{
		if (damagetype & DMG_BURN)
			sbDamageMult /= 2;
		
		damage = float(RoundToCeil(damage * sbDamageMult));
		return Plugin_Changed;
	}
...
public Action Event_InfectedHurt(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(attacker > 0 && attacker <= MaxClients && IsClientConnected(attacker) && IsClientInGame(attacker) && GetClientTeam(attacker) == 2 && IsFakeClient(attacker))
	{
		...
		int dmg_health = cur_health - amount;	
		if(cur_health > 0)
		{
			int health = RoundToNearest(cur_health - dmg_health*sbDamageCommonMult);
			if( health >= 0 )
			{

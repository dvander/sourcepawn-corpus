...
new Float:song_time;
new Handle:menu_sorttype_list = INVALID_HANDLE;
new Handle:menu_sort_albums_list = INVALID_HANDLE;
new Handle:menu_sort_artist_list = INVALID_HANDLE;
new Handle:menu_sort_titles_list = INVALID_HANDLE;
...
public OnPluginStart() {
	...
	CloseHandle(songs_array);
	 ...
	 /***********************************
	 * SORTING SONGLIST FOR NON ADMINS *
	 ***********************************/
	menu_sorttype_list = CreateMenu(menu_handler_sorting_list);
	SetMenuTitle(menu_sorttype_list, "Choose sorting:");
	AddMenuItem(menu_sorttype_list, "", "Sort by Artist");
	AddMenuItem(menu_sorttype_list, "", "Sort by Title");
	AddMenuItem(menu_sorttype_list, "", "Sort by Album");
	//
	new Handle:songs_array_list;
	new Handle:songs_trie_list;
	new i_list, size_list, size2_list;
	// by artist ("artist - title")
	songs_array_list = CreateArray(ByteCountToCells(sizeof(info1)));
	songs_trie_list  = CreateTrie();
	KvRewind(kv_songs);
	KvGotoFirstSubKey(kv_songs);
	do {
		KvGetSectionName(kv_songs, info1, sizeof(info1));
		KvGetString(kv_songs, "name", info2, sizeof(info2));
		PushArrayString(songs_array_list, info2);
		SetTrieString(songs_trie_list, info2, info1);
	} while (KvGotoNextKey(kv_songs));
	SortADTArray(songs_array_list, Sort_Ascending, Sort_String);
	size_list = GetArraySize(songs_array_list);
	menu_sort_artist_list = CreateMenu(menu_handler_list);
	SetMenuTitle(menu_sort_artist_list, "Song list:");
	for (i_list = 0; i_list < size_list; i_list++) {
		GetArrayString(songs_array_list, i_list, info1, sizeof(info1));
		if (GetTrieString(songs_trie_list, info1, info2, sizeof(info2))) AddMenuItem(menu_sort_artist_list, "", info1);
	}
	CloseHandle(songs_array_list);
	CloseHandle(songs_trie_list);
	// by title ("title - artist")
	songs_array_list = CreateArray(ByteCountToCells(sizeof(info1)));
	songs_trie_list  = CreateTrie();
	KvRewind(kv_songs);
	...
	do {
		...
		KvGetString(kv_songs, "title", info3, sizeof(info3));
		Format(info3, sizeof(info3), "%s - %s", info3, info2);
		PushArrayString(songs_array_list, info3);
		SetTrieString(songs_trie_list, info3, info1);
	} while (KvGotoNextKey(kv_songs));
	SortADTArray(songs_array_list, Sort_Ascending, Sort_String);
	size_list = GetArraySize(songs_array_list);
	menu_sort_titles_list = CreateMenu(menu_handler_list);
	SetMenuTitle(menu_sort_titles_list, "Song list:");
	for (i_list = 0; i_list < size_list; i_list++) {
		GetArrayString(songs_array_list, i_list, info1, sizeof(info1));
		if (GetTrieString(songs_trie_list, info1, info2, sizeof(info2))) AddMenuItem(menu_sort_titles, "", info1);
	}
	CloseHandle(songs_array_list);
	CloseHandle(songs_trie_list);
	// by album
	...
	do {
		KvGetString(kv_songs, "album", info1, sizeof(info1));
		if (FindStringInArray(songs_array_list, info1) == -1) PushArrayString(songs_array_list, info1);
	} while (KvGotoNextKey(kv_songs));
	SortADTArray(songs_array_list, Sort_Ascending, Sort_String);
	size_list = GetArraySize(songs_array_list), size2_list = GetMenuItemCount(menu_sort_artist);
	menu_sort_albums_list = CreateMenu(menu_handler_list);
	SetMenuTitle(menu_sort_albums_list, "Song list:");
	for (i_list = 0; i_list < size_list; i_list++) {
		GetArrayString(songs_array_list, i_list, info1, sizeof(info1));
		new Handle:menu_album = CreateMenu(menu_handler);
		IntToString(PushArrayCell(arr_albums, menu_album), info2, sizeof(info2));
		for (new j = 0; j < size2_list; j++) {
			GetMenuItem(menu_sort_artist_list, j, info2, sizeof(info2));
			KvJumpToKey(kv_songs, info2);
			KvGetString(kv_songs, "album", info2, sizeof(info2));
			if (StrEqual(info1, info2)) {
				KvGetString(kv_songs, "name", info2, sizeof(info2));
		...
		if (info1[0] == '\0') strcopy(info1, sizeof(info1), "[No Album]");
		AddMenuItem(menu_sort_albums_list, "", info1);
	}
	CloseHandle(songs_array_list);
	...
	RegConsoleCmd("songlist", cmd_songlist);
 ...
 * ! *
 *****/
public Action:cmd_songlist(client, args) {
  DisplayMenu(menu_sorttype_list, client, MENU_TIME_FOREVER);
...
public Action:cmd_karaoke(client, args) {
	if (song_url[0] != '\0') {
		...
		// CXH Added this to try and stop crash on second playing of song
...
public menu_handler_sorting_list(Handle:menu, MenuAction:action, client, item) {
	if (action != MenuAction_Select) return;
	switch (item) {
		case 0: DisplayMenu(menu_sort_artist_list, client, MENU_TIME_FOREVER);
		case 1: DisplayMenu(menu_sort_titles_list, client, MENU_TIME_FOREVER);
		case 2: DisplayMenu(menu_sort_albums_list, client, MENU_TIME_FOREVER);
	}
}
...
public menu_handler(Handle:menu, MenuAction:action, client, item) {
	...
	// CXH Removed line below to stop precaching
...
public menu_handler_list(Handle:menu, MenuAction:action, client, item) {

...
#define PLUGIN_TAG					"TF2BWR Reborn"
...
//#define BOMB_SND_DEPLOYALERT		"vo/mvm_bomb_alerts08.wav"

#define SENTRYBUSTER_DISTANCE		400.0
#define SENTRYBUSTER_DAMAGE			99999
#define SENTRYBUSTER_CLASSVARIANT	8
...
//new bool:SentryBusterQueNotClosed;
...
new Handle:g_hbombs1;
new Handle:g_hbombs2;
new Handle:g_hbombs3;
...
{
	name = "TF2 Be With Robots Reborn",
	author = "Michalpl, Benoist3012",
...
{
	sm_tf2bwr_version = CreateConVar( "sm_tf2bwr_version", PLUGIN_VERSION, "TF2 Be With Robots Reborn", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_REPLICATED );
	...
	sm_tf2bwr_myloadouts = CreateConVar( "sm_tf2bwr_myloadouts", "0", "Allow human robots to select My Loadout variants.", FCVAR_PLUGIN, true, 0.0, true, 1.0 );
	...
	sm_tf2bwr_engineers = CreateConVar( "sm_tf2bwr_engineers", "1", "Allow/disallow engineers", FCVAR_PLUGIN, true, -1.0, true, 10.0 );
	HookConVarChange( sm_tf2bwr_engineers, OnConVarChanged );
	
	sm_tf2bwr_buster = CreateConVar( "sm_tf2bwr_buster", "1", "Enable or disable buster.", FCVAR_PLUGIN, true, 0.0, true, 1.0 );
...
{
	PrintToChatAll("[TF2BWR Reborn] Official group: http://steamcommunity.com/groups/TF2BWRR");
}
public OnPluginEnd()
{
...
//			SDKHook( iEnt, SDKHook_EndTouch, OnFlagEndTouch );
		...
		while( ( iEnt = FindEntityByClassname( iEnt, "trigger_multiple") ) != -1 )
		{
			...
			{
				SDKHook( iEnt, SDKHook_Touch, OnTriggerAlarmTouch );
				SDKHook( iEnt, SDKHook_StartTouch, OnTriggerAlarmTouch );
			}
		...
		for( new a = 8; a <= 9; a++ )
		{
			Format( strAnnounceLine, sizeof(strAnnounceLine), "vo/mvm_bomb_alerts0%i.wav", a );
			PrecacheSnd( strAnnounceLine, _, true );
		}
		for( new a = 10; a <= 11; a++ )
		{
			Format( strAnnounceLine, sizeof(strAnnounceLine), "vo/mvm_bomb_alerts%i.wav", a );
			PrecacheSnd( strAnnounceLine, _, true );
		}
		//////////////Teleporter
		for(new a = 1; a <= 5; a++)
		{
			Format(strAnnounceLine, sizeof(strAnnounceLine), "vo/announcer_mvm_eng_tele_activated0%i.wav", a);
			PrecacheSnd(strAnnounceLine, _, true );
		}
		for(new a = 2; a <= 3; a++)
		{
			Format(strAnnounceLine, sizeof(strAnnounceLine), "vo/announcer_mvm_engbot_arrive0%i.wav", a);
			PrecacheSnd(strAnnounceLine, _, true );
		}
		/////////////////STEP
		for(new a = 1; a <= 18; a++)
		...
		for(new a = 1; a <= 12; a++)
		{
			Format( strAnnounceLine, sizeof( strAnnounceLine ), "vo/mvm_wave_lose%s%i.wav", ( a < 10 ? "0" : "" ), a );
			PrecacheSound(strAnnounceLine);
		}
...
public OnGameFrame()
{
	...
	new Shield = -1;
	while( ( Shield = FindEntityByClassname( Shield, "entity_medigun_shield" ) ) != -1 )
	{
		i = GetEntPropEnt( Shield, Prop_Send, "m_hOwnerEntity" );
	...
	new hat = -1;
	while((hat=FindEntityByClassname(hat, "tf_wearable"))!=INVALID_ENT_REFERENCE && IsMannhattan && nGateCapture != MaxGate)
	{
		new Owner = GetEntPropEnt(hat, Prop_Send, "m_hOwnerEntity");
		if (GetClientTeam(Owner) == _:TFTeam_Blue && IsFakeClient(Owner))
		{
...
//	CreateTimer(0.0, FindReviveMaker); 
	new iEFlags, iHealth;
...
//			if( GetEntProp( i, Prop_Send, "m_flTauntForceMoveForwardSpeed" ) == _:150 )
//			if( GetEntProp(i, Prop_Send, "m_iTauntIndex") == 1118)
//			{
...
//			}
			...
			if( nTeamNum == _:TFTeam_Blue )
			{
				if( iRobotMode[i] == Robot_Giant || iRobotMode[i] == Robot_SentryBuster ) // || iRobotMode[i] == Robot_BigNormal )
				{
					...
					if(iRobotMode[i] == Robot_Giant && class != TFClass_Scout )//(bool:GetEntProp(iClient, Prop_Send, "m_bIsMiniBoss") == true)
					{
						new weaponM = GetPlayerWeaponSlot(i, 2);
						TF2Attrib_SetByName(weaponM, "dmg bonus vs buildings", 60.0);
					}
				}
				else if( iRobotMode[i] == Robot_Stock )
					SetEntProp( i, Prop_Send, "m_nBotSkill", BotSkill_Expert );
				if( ( (LastGateCapture + 17.5) > GetEngineTime() ) && IsMannhattan)
				{
					if( iRobotMode[i] != Robot_Giant &&  iRobotMode[i] != Robot_SentryBuster )
					{
						TF2_StunPlayer(i, 14.0, 0.0, TF_STUNFLAGS_NORMALBONK|TF_STUNFLAG_NOSOUNDOREFFECT, _);
						TF2_AddCondition(i, TFCond_MVMBotRadiowave, 14.0);
					}
				}
			}
			if( nTeamNum != _:TFTeam_Blue )
			...
			{
				TF2_AddCondition(i, TFCond_UberchargedHidden, 0.125);
			}
			if(i == Carrier && IsValidRobot(i) && bInRespawn[i] && iRobotMode[i] != Robot_Giant)
			{
				new BombOwner = FindEntityByClassname(-1, "tf_objective_resource");
				if(BombStage == 0)
				{
					if (g_hbombs1 != INVALID_HANDLE)
					{
						CloseHandle(g_hbombs1);
						g_hbombs1 = INVALID_HANDLE;
					}
					if (g_hbombs2 != INVALID_HANDLE)
					{
						CloseHandle(g_hbombs2);
						g_hbombs2 = INVALID_HANDLE;
					}
					if (g_hbombs3 != INVALID_HANDLE)
					{
						CloseHandle(g_hbombs3);
						g_hbombs3 = INVALID_HANDLE;
					}
					...
					g_hbombs1 = CreateTimer(5.2, Timer_bombst1, i);
					g_hbombs2 = CreateTimer(20.0, Timer_bombst2, i);
					g_hbombs3 = CreateTimer(40.0, Timer_bombst3, i);
				}
				if(BombStage == 1)
				{
					if (g_hbombs2 != INVALID_HANDLE)
					...
					SetEntPropFloat(BombOwner, Prop_Send, "m_flMvMBaseBombUpgradeTime", CurrentTime);
					SetEntPropFloat(BombOwner, Prop_Send, "m_flMvMNextBombUpgradeTime", NextTime);
					g_hbombs2 = CreateTimer(15.0, Timer_bombst2, i);
					g_hbombs3 = CreateTimer(35.0, Timer_bombst3, i);
				}
				if(BombStage == 2)
				{
					if (g_hbombs3 != INVALID_HANDLE)
					...
					new Float:CurrentTime = GetGameTime();
					new Float:NextTime = CurrentTime+20.0;
					SetEntPropFloat(BombOwner, Prop_Send, "m_flMvMBaseBombUpgradeTime", CurrentTime);
					SetEntPropFloat(BombOwner, Prop_Send, "m_flMvMNextBombUpgradeTime", NextTime);
					g_hbombs3 = CreateTimer(20.0, Timer_bombst3, i);
				}
			}
			...
			else if( iEffect[i] == Effect_HoldFireUntilFullReload )
			{
				...
				if(GetEntProp(weaponBLACKBOX, Prop_Send, "m_iClip1") == 3)
					TF2Attrib_RemoveByName(weaponBLACKBOX, "fire rate penalty");
				if(GetEntProp(weaponBLACKBOX, Prop_Send, "m_iClip1") < 2)
					TF2Attrib_SetByName(weaponBLACKBOX, "fire rate penalty", 9999.0);
			}
...
//			else if( iEffect[i] == Effect_AutoJump )
//				CreateTimer( 5.0, Timer_Jump, GetClientUserId(i), TIMER_REPEAT );
//				CreateTimer( 5.1, Timer_JumpStop, GetClientUserId(i), TIMER_REPEAT );
...
//	if ((clienthp > 1000) && IsValidRobot(i))
//		SetEntProp( i, Prop_Send, "m_bIsMiniBoss", _:true );
}
//public Action:Timer_RemoveStun( Handle:hTimer, any:i )
//{
...
//		SDKHook( iEntity, SDKHook_EndTouch, OnFlagEndTouch );
	...
	else if( StrEqual( strClassname, "trigger_timer_door", false ) )
	{
	...
	if( StrEqual( strClassname, "obj_sentrygun", false ) )
		SDKHook( iEntity, SDKHook_OnTakeDamage, OnSentryTakeDamage );
}
/////////////////////////////////////////
...
public Action:FindReviveMaker(Handle:Timer)
{
	...
	if((ReviveMakers = FindEntityByClassname(ReviveMakers,"entity_revive_marker")) != -1)
	{
		...
		{
			AcceptEntityInput(ReviveMakers,"Kill");
		}
	}
}
...
public Action:Timer_OnPlayerSpawn( Handle:hTimer, any:iUserID )
{
	...
	if( iEffect[iClient] == Effect_FullCharge )
	{
		if( iClass == TFClass_Medic )
		{
			...
			SetEntPropFloat( iClient, Prop_Send, "m_flRageMeter", 100.0 ); //iClient m_flNextRageEarnTime broken
	}
	...
	if(!IsMannhattan && CanTeleportBomb && GameRules_GetRoundState() == RoundState_RoundRunning && class != TFClass_Sniper && class != TFClass_Spy && class != TFClass_Engineer && class != TFClass_Medic && !IsGateBotPlayer[iClient])
	{
		if(!SecondBombEnable)
		{
			new iflagr = -1;
	...
	else if(((LastGateCapture + 19.9) < GetEngineTime()))
			SpawnRobot(iClient);
	return Plugin_Stop;
}
...
//		return Plugin_Handled;
	...
	if( iRobotMode[iClient] == Robot_SentryBuster )
	{
		...
		UpdatePlayerHitbox(iClient, 1.8); 
	}
	else
	{
...
//new i = -1;
//while ((i = FindEntityByClassname(i, "entity_revive_marker")) != -1
//	{
//	if(IsValidEntity(i))
//	{
//	decl String:strTeam[50];
//	GetEntPropString(i, Prop_Data, "m_iTeamNum", strTeam, sizeof(strTeam));
//	if(strcmp(strTeam, "TFTeam_Blue") == 0)
//		{
...
public OnPlayerSpawn( Handle:hEvent, const String:strEventName[], bool:bDontBroadcast )
{
	...
	if (IsClientInGame(iClient) && GetClientTeam(iClient) == _:TFTeam_Blue && IsFakeClient(iClient))
	{
	...
	if( iRobotMode[iClient] == Robot_SentryBuster )
		PrintToChat( iClient, "SentryBuster: Press \x03Taunt\x01 button to detonate." );
}
/*
...
//	{
//		TF2Attrib_SetByName(iClient, "increase player capture value", -2.0);
//	}
//	if(IsGateBotPlayer[iClient])
//	{
...
//	}
	...
	switch( iClass )
	{
		case TFClass_Scout:
		{
			...
			if( iRobotVariant[iClient] == 1 ) //bonk scout
			...
			if( iRobotVariant[iClient] == 2 ) //BFB Scout added in 1.4
			{
					SpawnItem( iClient, 772 );
			}
			if( iRobotVariant[iClient] == 8 ) //armored FAN Scout added in 1.3.5v dev
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 518 ); PushArrayCell( hAttributes, _:6.0 );
				PushArrayCell( hAttributes, 97 ); PushArrayCell( hAttributes, _:0.3 );
				PushArrayCell( hAttributes, 1 ); PushArrayCell( hAttributes, _:0.35 );
				PushArrayCell( hAttributes, 45 ); PushArrayCell( hAttributes, _:2.0 );
				PushArrayCell( hAttributes, 106 ); PushArrayCell( hAttributes, _:0.4 );
				PushArrayCell( hAttributes, 54 ); PushArrayCell( hAttributes, _:0.5 );
				PushArrayCell( hAttributes, 252 ); PushArrayCell( hAttributes, _:0.7 );
				PushArrayCell( hAttributes, 329 ); PushArrayCell( hAttributes, _:0.7 );
				SpawnItem( iClient, 45, hAttributes );
				{
					hAttributes = CreateArray();
					PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:5875.0 );
					PushArrayCell( hAttributes, 330 ); PushArrayCell( hAttributes, _:5.0 );
				}
				SpawnItem( iClient, 0, hAttributes );
//				SpawnItem( iClient, 5617); vodoo soul unused
			}
			if( iRobotVariant[iClient] == 4 ) //warp assasin old Tornado Scout
			{
				hAttributes = CreateArray();
//				PushArrayCell( hAttributes, 522 ); PushArrayCell( hAttributes, _:1.0 ); //tornado effect
				PushArrayCell( hAttributes, 182 ); PushArrayCell( hAttributes, _:5.0 ); //brokenleg
//				PushArrayCell( hAttributes, 218 ); PushArrayCell( hAttributes, _:1.0 );	//markdeath
				PushArrayCell( hAttributes, 278 ); PushArrayCell( hAttributes, _:0.5 ); //faster recharge 50%
//				PushArrayCell( hAttributes, 134 ); PushArrayCell( hAttributes, _:3008.0 ); //tornado
				SpawnItem( iClient, 648, hAttributes);
			}
		}
		case TFClass_Soldier:
		{
			...
			if( iRobotVariant[iClient] == 5 )// Black Box Soldier
			{
				hAttributes = CreateArray();
				...
				PushArrayCell( hAttributes, 16 ); PushArrayCell( hAttributes, _:60.0 );
				PushArrayCell( hAttributes, 411 ); PushArrayCell( hAttributes, _:2.0 );
			...
			if( iRobotVariant[iClient] == 9 )	//Giant Black Box Soldier
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 2 ); PushArrayCell( hAttributes, _:0.45 );
				PushArrayCell( hAttributes, 318 ); PushArrayCell( hAttributes, _:1.6 );
				PushArrayCell( hAttributes, 16 ); PushArrayCell( hAttributes, _:1000.0 );
				PushArrayCell( hAttributes, 411 ); PushArrayCell( hAttributes, _:4.0 );
				PushArrayCell( hAttributes, 99 ); PushArrayCell( hAttributes, _:1.25 );
				PushArrayCell( hAttributes, 6 ); PushArrayCell( hAttributes, _:0.001 );
				PushArrayCell( hAttributes, 107 ); PushArrayCell( hAttributes, _:0.5 );
				PushArrayCell( hAttributes, 252 ); PushArrayCell( hAttributes, _:0.4 );
				...
				PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:4000.0 );
				PushArrayCell( hAttributes, 330 ); PushArrayCell( hAttributes, _:3.0 );

				SpawnItem( iClient, 228, hAttributes );
			}
			else if( iRobotVariant[iClient] == 100 || iRobotVariant[iClient] == 101 ) //Boss System
			{
				if( iRobotVariant[iClient] == 101 )// Major Crits
				{
					//Sorry I must use SpawnWeapon because some attrib don't work with the old system
						...
						SpawnWeapon( iClient, "tf_weapon_rocketlauncher", 228, 100, 5, weaponAttribs );
				}
				if( iRobotVariant[iClient] == 100 )// Sergeant Crits
				{
					...
					SpawnWeapon( iClient, "tf_weapon_rocketlauncher", 18, 100, 5, weaponAttribs );
				}
			}
			if( iRobotVariant[iClient] >= 1 && iRobotVariant[iClient] <= 4 )
			{
				...
				{
					hAttributes = CreateArray();
					PushArrayCell( hAttributes, 116 ); PushArrayCell( hAttributes, _:2 ); 
					PushArrayCell( hAttributes, 319 ); PushArrayCell( hAttributes, _:9.0 );
				...
				{
					hAttributes = CreateArray();
					PushArrayCell( hAttributes, 116 ); PushArrayCell( hAttributes, _:1 ); 
					PushArrayCell( hAttributes, 319 ); PushArrayCell( hAttributes, _:9.0 );
					SpawnItem( iClient, 226, hAttributes );
				}
//				else
//				{
//					PushArrayCell( hAttributes, 116 ); PushArrayCell( hAttributes, _:( iRobotVariant[iClient] == 4 ? 2 : ( iRobotVariant[iClient] == 3 ? 3 : 1 ) ) ); // orginal PushArrayCell( hAttributes, 116 ); PushArrayCell( hAttributes, _:( iRobotVariant[iClient] == 4 ? 3.0 : ( iRobotVariant[iClient] == 3 ? 2.0 : 1.0 ) ) );
//					PushArrayCell( hAttributes, 319 ); PushArrayCell( hAttributes, _:9.0 );
//				}
//				SpawnItem( iClient, ( iRobotVariant[iClient] == 4 ? 226 : ( iRobotVariant[iClient] == 3 ? 354 : 129 ) ), hAttributes ); //fixed crash issue conercho had battalion buff and vice versa buff soldier
				SpawnItem( iClient, 6 );
				SpawnItem( iClient, 18 );
			}
			SpawnItem( iClient, 6 );
			...
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 134 ); PushArrayCell( hAttributes, _:64.0 );
				SpawnItem( iClient, 1063, hAttributes );
			}
		}
		case TFClass_Pyro:
		{
			...
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:2825.0 ); //giant airblast pyro
				PushArrayCell( hAttributes, 107 ); PushArrayCell( hAttributes, _:0.5 );
				...
				PushArrayCell( hAttributes, 256 ); PushArrayCell( hAttributes, _:0.5 ); //bonus
			...
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:54825.0 );
				...
				PushArrayCell( hAttributes, 57 ); PushArrayCell( hAttributes, _:500.0 );
				...
				SpawnItem( iClient, 208, hAttributes );
			}
			...
			if(IsGateBotPlayer[iClient])
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 134 ); PushArrayCell( hAttributes, _:64.0 );
				SpawnItem( iClient, 1058, hAttributes );
			}
		}
		case TFClass_DemoMan:
		{
			...
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:2325.0 ); //hp
				PushArrayCell( hAttributes, 107 ); PushArrayCell( hAttributes, _:2.0 ); //speed
				PushArrayCell( hAttributes, 252 ); PushArrayCell( hAttributes, _:0.5 ); //force red
				PushArrayCell( hAttributes, 329 ); PushArrayCell( hAttributes, _:0.5 ); //airblast immu
				PushArrayCell( hAttributes, 330 ); PushArrayCell( hAttributes, _:7.0 ); //footsteps
				PushArrayCell( hAttributes, 402 ); PushArrayCell( hAttributes, _:1.0 ); //no backstab
				PushArrayCell( hAttributes, 137 ); PushArrayCell( hAttributes, _:75.0 ); //dmg blow buildings
				PushArrayCell( hAttributes, 396 ); PushArrayCell( hAttributes, _:0.5 ); //faster swing
			...
			else
			{
				...
				else if( iRobotVariant[iClient] == 1 || iRobotVariant[iClient] == 4 )
				{
					...
					{
						hAttributes = CreateArray();
						PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:3125.0 );
						PushArrayCell( hAttributes, 107 ); PushArrayCell( hAttributes, _:0.5 );
						PushArrayCell( hAttributes, 31 ); PushArrayCell( hAttributes, _:6.0 );
						PushArrayCell( hAttributes, 252 ); PushArrayCell( hAttributes, _:0.5 );
						PushArrayCell( hAttributes, 329 ); PushArrayCell( hAttributes, _:0.5 );
						PushArrayCell( hAttributes, 330 ); PushArrayCell( hAttributes, _:4.0 );
						PushArrayCell( hAttributes, 180 ); PushArrayCell( hAttributes, _:140.0 ); //gets 150 hp on kill
//						PushArrayCell( hAttributes, 246 ); PushArrayCell( hAttributes, _:10.0 );
					}
//					if( iRobotVariant[iClient] == 1 )
//					{
//						hAttributes = CreateArray();
...
//					}
					...
					{
						hAttributes = CreateArray();
						PushArrayCell( hAttributes, 527 ); PushArrayCell( hAttributes, _:1.0 );
						PushArrayCell( hAttributes, 60 ); PushArrayCell( hAttributes, _:0.75 );
						PushArrayCell( hAttributes, 64 ); PushArrayCell( hAttributes, _:0.85 );
						SpawnItem( iClient, 1099, hAttributes ); //  131
				...
				{
					hAttributes = CreateArray();
					PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:39825.0 );
					...
					PushArrayCell( hAttributes, 57 ); PushArrayCell( hAttributes, _:200.0 );
					PushArrayCell( hAttributes, 478 ); PushArrayCell( hAttributes, _:0.1 );
					PushArrayCell( hAttributes, 405 ); PushArrayCell( hAttributes, _:0.1 );
					PushArrayCell( hAttributes, 330 ); PushArrayCell( hAttributes, _:4.0 );

					SpawnItem( iClient, 206, hAttributes );
				}
				else if( iRobotVariant[iClient] == 101 ) //Chief Tavish (Boss System)
				{
					hAttributes = CreateArray();
					PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:54850.0 );
					PushArrayCell( hAttributes, 107 ); PushArrayCell( hAttributes, _:0.4 );
					PushArrayCell( hAttributes, 252 ); PushArrayCell( hAttributes, _:0.3 );
					PushArrayCell( hAttributes, 329 ); PushArrayCell( hAttributes, _:0.3 );
					PushArrayCell( hAttributes, 57 ); PushArrayCell( hAttributes, _:500.0 );
				...
				else if( iRobotVariant[iClient] == 102 ) //Sir NukeSalot (Boss System)
				{
						...
						SpawnWeapon( iClient, "tf_weapon_cannon", 996, 100, 5, weaponAttribs );
				}
				else if( iRobotVariant[iClient] == 5 ) //samurai demo dev.
				{
					hAttributes = CreateArray();
					PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:475.0 );
					PushArrayCell( hAttributes, 498 ); PushArrayCell( hAttributes, _:1.0 ); //jump effect
					PushArrayCell( hAttributes, 524 ); PushArrayCell( hAttributes, _:2.3 ); //jump height
					PushArrayCell( hAttributes, 2 ); PushArrayCell( hAttributes, _:1.5 ); //bonus dmg
					PushArrayCell( hAttributes, 202 ); PushArrayCell( hAttributes, _:2.0 ); //bonus charge time
					PushArrayCell( hAttributes, 249 ); PushArrayCell( hAttributes, _:7.0 ); //charge recharge rate
					PushArrayCell( hAttributes, 1030 ); PushArrayCell( hAttributes, _:1.0 ); //test attack doesn't stops charge
			...
			if(IsGateBotPlayer[iClient])
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 134 ); PushArrayCell( hAttributes, _:64.0 );
				SpawnItem( iClient, 1061, hAttributes );
			}
		}
		case TFClass_Heavy:
		{
			...
			else if( iRobotVariant[iClient] == 3 || iRobotVariant[iClient] == 5 )
			{
				if( iRobotVariant[iClient] == 5 ) //super heavy chomp
				{
					...
					PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:4700.0 );
					PushArrayCell( hAttributes, 107 ); PushArrayCell( hAttributes, _:0.5 );
					PushArrayCell( hAttributes, 2 ); PushArrayCell( hAttributes, _:3.0 ); //dmg bouns
					PushArrayCell( hAttributes, 137 ); PushArrayCell( hAttributes, _:60.0 ); // dmg bonus buildings
					PushArrayCell( hAttributes, 252 ); PushArrayCell( hAttributes, _:0.3 );
					PushArrayCell( hAttributes, 329 ); PushArrayCell( hAttributes, _:0.3 );
					PushArrayCell( hAttributes, 330 ); PushArrayCell( hAttributes, _:2.0 );
					PushArrayCell( hAttributes, 522 ); PushArrayCell( hAttributes, _:1.0 ); //airblast on hit
					PushArrayCell( hAttributes, 6 ); PushArrayCell( hAttributes, _:2.5 ); //faster swing
			...
			if(IsGateBotPlayer[iClient])
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 134 ); PushArrayCell( hAttributes, _:64.0 );
				SpawnItem( iClient, 1060, hAttributes );
			}
		}
		case TFClass_Medic:
		{
			...
			if( iRobotVariant[iClient] >= 3 && iRobotVariant[iClient] <= 4 )
			{
				...
				/*else if( iRobotVariant[iClient] == 5 )old Giant Medic
				{
					hAttributes = CreateArray();
			...
			else if( iRobotVariant[iClient] == 2 ) //crit medic
			{
					hAttributes = CreateArray();
					PushArrayCell( hAttributes, 479 ); PushArrayCell( hAttributes, _:0.0 ); //no overheal
					SpawnItem( iClient, 35, hAttributes );
			}
			else
			{
				if( iRobotVariant[iClient] == 1 )
				...
				{
					hAttributes = CreateArray();
					PushArrayCell( hAttributes, 499 ); PushArrayCell( hAttributes, _:2.0 ); // Medic Shield
					PushArrayCell( hAttributes, 479 ); PushArrayCell( hAttributes, _:0.0 ); //no overheal
			...
			if(IsGateBotPlayer[iClient])
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 134 ); PushArrayCell( hAttributes, _:64.0 );
				SpawnItem( iClient, 1059, hAttributes );
			}
		}
		case TFClass_Sniper:
		{
			...
			if(IsGateBotPlayer[iClient])
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 134 ); PushArrayCell( hAttributes, _:64.0 );
				SpawnItem( iClient, 1062, hAttributes );
			}
		}
		case TFClass_Spy:
		{
			...
			if(IsGateBotPlayer[iClient])
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 134 ); PushArrayCell( hAttributes, _:64.0 );
				SpawnItem( iClient, 1064, hAttributes );
			}
		}
		case TFClass_Engineer:
		{
			...
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:175.0 );
				PushArrayCell( hAttributes, 2043 ); PushArrayCell( hAttributes, _:8.0 );
				PushArrayCell( hAttributes, 286 ); PushArrayCell( hAttributes, _:3.0 );
				PushArrayCell( hAttributes, 353 ); PushArrayCell( hAttributes, _:1.0 );
			...
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:375.0 );
				PushArrayCell( hAttributes, 2043 ); PushArrayCell( hAttributes, _:8.0 );
				PushArrayCell( hAttributes, 286 ); PushArrayCell( hAttributes, _:3.0 );
				PushArrayCell( hAttributes, 353 ); PushArrayCell( hAttributes, _:1.0 );
			...
			if(IsGateBotPlayer[iClient])
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 134 ); PushArrayCell( hAttributes, _:64.0 );
				SpawnItem( iClient, 1065, hAttributes );
			}
		}
	}
...
public Action:Timer_GateBotHat(Handle:timer,any:iClient)
{
	...
	if(IsGateBotPlayer[iClient])
	{
		...
		switch(iClass)
		{
			case TFClass_Scout:
			{
				SpawnItem( iClient, 1057, hAttributes );
			}
			case TFClass_Soldier:
			{
				SpawnItem( iClient, 1063, hAttributes );
		...
		if((TeleporterExit = FindEntityByClassname(TeleporterExit,"obj_teleporter")) != -1 && GetEntProp(TeleporterExit, Prop_Send, "m_iTeamNum") == _:TFTeam_Blue)
		{
			...
			if(StrContains(modelname, "light") != -1 && IsValidRobot(OwnerTeleporter))
			{
				...
				{	
					...
					Format( soundteleporteractivate, sizeof(soundteleporteractivate), "vo/announcer_mvm_eng_tele_activated0%i.wav", soundswitch);
...
public Action:CommandListener_Build(client, const String:command[], argc)
{
	...
	for(new i = MaxClients + 1; i < g_iMaxEntities; i++)
	{
		...
		{
			PrintCenterText(client,"You can only build a teleporter exit!"); //fixed message
			return Plugin_Handled;
		}
	}
	return Plugin_Continue;
...
SpawnRobot(client)
{
	...
	if(IsMannhattan && GameRules_GetRoundState() == RoundState_RoundRunning && class != TFClass_Sniper && class != TFClass_Spy && class != TFClass_Engineer && class != TFClass_Medic && ((LastGateCapture + 19.9) < GetEngineTime()))
	{
		new iflagr = -1;
		if(CanTeleportBomb1)
		{
...
TeleportRobotToHint(client)
{
	...
	if(TeleportPoint != -1)
	{
		...
		Format( soundteleporttohint, sizeof(soundteleporttohint), "vo/announcer_mvm_engbot_arrive0%i.wav", soundswitch);
...
GetNestNextToTheBomb()
{
	...
	new Float:Pos14[3];
	new Float:BombPos[3];
	new Float:Dist1 = 8000.0;
	new Float:Dist2 = 8000.0;
	new Float:Dist3 = 8000.0;
	new Float:Dist4 = 8000.0;
	new Float:Dist5 = 8000.0;
	new Float:Dist6 = 8000.0;
	new Float:Dist7 = 8000.0;
	new Float:Dist8 = 8000.0;
	new Float:Dist9 = 8000.0;
	new Float:Dist10 = 8000.0;
	new Float:Dist11 = 8000.0;
	new Float:Dist12 = 8000.0;
	new Float:Dist13 = 8000.0;
	new Float:Dist14 = 8000.0;
	new Handle:hSpawnPoint = CreateArray();
	new iEnt = -1;
	new Bomb = -1;
	if(BombPickup)
...
public Action:Timer_TurnOffGateBotHat(Handle:timer,any:iClient)
{
...
//		if( iOwner == iclient )
//		{
//			TF2Attrib_RemoveByName(iEntity, "attach particle effect");
//		}
//	}
...
public Action:WaveStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	///BOMB TELEPORT
	CreateTimer(0.1, Teleport_Bomb);//Prevent bug when a popfile change the intel
}
		...
		if(!StrEqual(jumpkey, map))
		{
			if(KvJumpToKey(kvKey, "Boss", true))
			{
				...
				if(numwave == 1)
				{
					...
					BossEnabled = true;
					fNextBossTime == 0.0;
					//PrintToChatAll("[Tf2bwr] BossActivate!");
					return;
				}
				else
				{
					KvGoBack(kvKey);
					BossEnabled = false;
					//PrintToChatAll("[Tf2bwr] BossDisabled!");
					return;
				}
			}
		}
...
public AdminMenu_AddMoney(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength)
{
	...
	{
		Format(buffer, maxlength, "(TF2bwr) Boss menu");
	}
...
//	new iEntFlags = GetEntityFlags( iOther);     
//	SetEntityFlags( iOther, iEntFlags|FL_FAKECLIENT );
	...
	TF2_AddCondition(iOther, TFCond_HalloweenKartNoTurn, 2.2);
...
//	CreateTimer( 1.8, Timer_DeployingBomb, GetClientUserId(iOther) );  // old code 
	if( (flAnnounceDeploy + 2.5) < GetEngineTime())
	{
		...
		{
			case 1:
				Format( strAnnounceLine, sizeof(strAnnounceLine), "vo/mvm_bomb_alerts08.wav");
			case 2:
				Format( strAnnounceLine, sizeof(strAnnounceLine), "vo/mvm_bomb_alerts09.wav");
			case 3:
				Format( strAnnounceLine, sizeof(strAnnounceLine), "vo/mvm_bomb_alerts10.wav");
			case 4:
				Format( strAnnounceLine, sizeof(strAnnounceLine), "vo/mvm_bomb_alerts11.wav");
		}
...
public Action:OnCapZoneEndTouch( iEntity, iOther )
{
	...
	SetVariantString("");
	AcceptEntityInput(iOther,"SetCustomModel");
	CreateTimer( 0.1, Timer_SetRobotModel, iOther, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE );
//	TF2_PlayAnimation( iOther, iEvent = 0, nData = 4 );
	iDeployingBomb = -1;
//	new iClient = GetClientUserId( iOther );
//	TE_Start( "PlayerAnimEvent" );
//	TE_WriteNum( "m_iPlayerIndex", iOther );
//	TE_WriteNum( "m_iEvent", 21 ); // Cancel/Respawn (event)
//	TE_WriteNum( "m_nData", 30 ); // ACT_RESET (activity)
//	TE_SendToAll();
...
public Action:OnTriggerGateTouch( iEntity, iOther )
{
	...
	{
		if( GetClientTeam(iOther) != _:TFTeam_Blue || !IsGateBotPlayer[iOther])
			return Plugin_Handled;
	}
...
public Action:NormalSoundHook( iClients[64], &iNumClients, String:strSound[PLATFORM_MAX_PATH], &iEntity, &iChannel, &Float:flVolume, &iLevel, &iPitch, &iFlags )
{
	...
	else if( StrContains( strSound, "player/footsteps/", false ) != -1 )
	{
		...
		if( iRobotMode[iEntity] == Robot_Giant || iRobotMode[iEntity] == Robot_BigNormal )
		{
			...
			return Plugin_Continue;
		else if( iRobotMode[iEntity] == Robot_None )
			return Plugin_Handled;
	...
	{
		if( iRobotMode[iEntity] == Robot_None )
		{
...
//			PrintToChatAll("Looped a sentry.");
			if( GetEntProp( iSentry, Prop_Send, "m_iTeamNum" ) == _:TFTeam_Red) // && SentryBusterQueNotClosed==true)
			{
				g_CanDispatchSentryBuster = true;
//				PrintToChatAll("Sentry found Enable bool.");
//				CreateTimer( 5.0, Timer_DispatchBuster, iClient );
//				SentryBusterQueNotClosed=false;
			}
			else
				g_CanDispatchSentryBuster = false;
		}
		if(g_CanDispatchSentryBuster==true && iRobotMode[iClient] != Robot_SentryBuster && GetRandomInt(0,3) > 2) //25%
		{
//			PrintToChatAll("Sentry Buster dispatched.");
			SetClassVariant( iClient, TFClass_DemoMan, SENTRYBUSTER_CLASSVARIANT );
//			SentryBusterInQue=false;
			return;
		}
...
stock PickRandomClassVariant( TFClassType:iClass = TFClass_Unknown )
{
	...
	if(!BossEnabled || GameRules_GetRoundState() != RoundState_RoundRunning || ( fNextBossTime + flBossWaitTime ) > GetEngineTime())
	{
		switch(	iClass )
		{
			...
			case TFClass_Soldier:	return !bGiants ? GetRandomInt(0,5) : GetRandomInt(0,9);
			case TFClass_DemoMan:	return !bGiants ? GetRandomInt(0,1) : GetRandomInt(0,5);
	...
	if(BossEnabled && GameRules_GetRoundState() == RoundState_RoundRunning && ( fNextBossTime + flBossWaitTime ) < GetEngineTime() && bGiants)
	{
		switch(	iClass )
		{
			case TFClass_Scout:		return GetRandomInt(0,8);
			case TFClass_Soldier:
			{
	...
	switch( iClass )
	{
		...
		case TFClass_Engineer:	
		{
			...
			if(GameRules_GetRoundState() == RoundState_RoundRunning && EngiNest != -1)
...
stock bool:SetClassVariant( iClient, TFClassType:iClass = TFClass_Unknown, iSVariant = -1 )
{
	...
	if( !bValidVariant )
	{
		...
		if(GateBotCase == 5 && IsMannhattan && nGateCapture != MaxGate)
		{
		...
		switch( iClass )
		{
			...
			case TFClass_Soldier:
			{
				if( iVariant >= 0 && iVariant <= 101 )
				{
					...
					else if( iVariant == 5 || iVariant == 9 )
						iNewEffect = Effect_HoldFireUntilFullReload;
					if( iVariant == 0 )
						iMode = Robot_Stock;
					else if( iVariant >= 6 )
						iMode = Robot_Giant;
					if( iVariant == 100 || iVariant == 101 )//Effect reserved for Boss
					{
						iMode = Robot_Giant;
	...
	switch( iClass )
	{
		case TFClass_Scout:
		{
			switch( iVariant )
			{
				...
				case 2: strcopy( strBuffer, iBufferSize, "BFB Scout" ); //old unused fish scout replaced by Baby face blaster scout
				case 3: strcopy( strBuffer, iBufferSize, "Minor League Scout" );
				case 4: strcopy( strBuffer, iBufferSize, "Warp Assasin" );
				...
				case 8: strcopy( strBuffer, iBufferSize, "Armored FAN Scout" );
			}
		}
		case TFClass_Sniper:
		{
		...
		case TFClass_Soldier:
		{
			switch( iVariant )
			{
				...
				case 6: strcopy( strBuffer, iBufferSize, "Giant Soldier" );
				case 7: strcopy( strBuffer, iBufferSize, "Giant Charged Soldier" );
				case 8: strcopy( strBuffer, iBufferSize, "Giant Rapid Fire Soldier" );
				case 9: strcopy( strBuffer, iBufferSize, "Giant Black Box Soldier" );
		...
		case TFClass_DemoMan:
		{
			switch( iVariant )
			{
				...
				case 5: strcopy( strBuffer, iBufferSize, "Samurai Demo" );
		...
		case TFClass_Medic:
		{
			switch( iVariant )
			{
				...
				case 2: strcopy( strBuffer, iBufferSize, "Crit Medic" );
...
public Action:Timer_DeployTimer(Handle:timer, any:Deployer)
{
		...
		SetRobotModel( Deployer, "models/props_td/atom_bomb.mdl" );//Disable robot eyes
		FixSounds(Deployer);
		new iflagr = -1;
		while ((iflagr = FindEntityByClassname(iflagr, "item_teamflag")) != -1)
		{
			AcceptEntityInput(iflagr, "ForceDrop");
			AcceptEntityInput(iflagr, "ForceReset");
		}
		decl String:strAnnounceLine[255];
		new sound = GetRandomInt(1,2);
		if(sound == 1)
		{
			sound = GetRandomInt(1,4);
			Format( strAnnounceLine, sizeof( strAnnounceLine ), "vo/mvm_wave_lose%i.wav", sound );
		}
		else
		{
			sound = GetRandomInt(7,12);
			Format( strAnnounceLine, sizeof( strAnnounceLine ), "vo/mvm_wave_lose%s%i.wav", ( sound < 10 ? "0" : "" ), sound );
		}
		EmitSoundToAll(strAnnounceLine);
		new i = -1;
		...
		if(IsValidEntity(i))
        {
			...
			if(strcmp(strName, "boss_deploy_relay") == 0)
			{
				...
				CreateTimer(0.01, Timer_spawnroundwinentity);
		} 
		}
//	new iEnt = -1;
//	iEnt = FindEntityByClassname(iEnt, "game_round_win");
//	AcceptEntityInput(iEnt, "RoundWin"); old code
		g_hDeployTimer = INVALID_HANDLE;
...
public EventHook_FlagStuff(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GetEventInt(event, "eventtype") == TF_FLAGEVENT_PICKEDUP )
	{
		...
		if( IsValidRobot(client) && ( iRobotMode[client] != Robot_Giant))  //( GetEntProp( client, Prop_Data, "m_bIsMiniBoss" ) == _:false  )
		{
//		PrintToChatAll("Timer trial has been trigerred.");	// a flag was picked up
		g_hBombHud = CreateTimer(0.1, Timer_bombhud);
		g_hbombs1 = CreateTimer(5.2, Timer_bombst1, client);
		g_hbombs2 = CreateTimer(20.0, Timer_bombst2, client);
		g_hbombs3 = CreateTimer(40.0, Timer_bombst3, client);
		}
		else if(bool:GetEntProp(client, Prop_Send, "m_bIsMiniBoss") == true) //(iRobotMode[client] == Robot_Giant)
			g_hBombBossHud = CreateTimer(0.001, Timer_bosshud, client);
	}
	return;
}
	public Action:Timer_bombst1(Handle:timer, any:client)
	{
		...
		AttachParticle(client, "mvm_levelup1");
...
//		TF2_AddCondition(client, TFCond_HalloweenQuickHeal, TFCondDuration_Infinite);
		EmitSoundToAll( BOMB_SND_STAGEALERT, client, SNDCHAN_STATIC, 125 );
		AttachParticle(client, "mvm_levelup2");
		FakeClientCommand( client, "taunt" );
		g_hbombs2 = INVALID_HANDLE;
		return Plugin_Handled;
	}
	public Action:Timer_bombst3(Handle:timer, any:client)
	{
		...
		EmitSoundToAll( BOMB_SND_STAGEALERT, client, SNDCHAN_STATIC, 125 );
		AttachParticle(client, "mvm_levelup3");
		FakeClientCommand( client, "taunt" );
		g_hbombs3 = INVALID_HANDLE;
		return Plugin_Handled;
	}
...
//	PrintToChatAll("Timer trial has been killed.");
		if(IsValidRobot(client) && ( iRobotMode[client] != Robot_Giant))
		{
...
//		TF2_RemoveCondition(client, TFCond_HalloweenQuickHeal);
		...
		if (g_hbombs3 != INVALID_HANDLE)
	{
		...
		if (g_hBombBossHud != INVALID_HANDLE)
	{
		CloseHandle(g_hBombBossHud);
		g_hBombBossHud = INVALID_HANDLE;
	}
		if (g_hBombHud != INVALID_HANDLE)
	{
		CloseHandle(g_hBombHud);
		g_hBombHud = INVALID_HANDLE;
	}
	}
	return;
...
//{
//	new i = -1;
//	while ((i = FindEntityByClassname(i, "filter_tf_bot_has_tag")) != -1)
//	{
//		if(IsValidEntity(i))
//		{
//			decl String:strName[50];
//			GetEntPropString(i, Prop_Data, "m_iName", strName, sizeof(strName));
//			if(strcmp(strName, "filter_gatebot") == 0)
//			{
//				AcceptEntityInput(i, "Kill");
//				break;
//			}
...
public Action:Timer_spawnroundwinentity(Handle:timer)
{
	new iEnt = -1;
...
//public Action:Timer_Jump(Handle:timer, any:client) //debug effect
//{
//FakeClientCommand( client, "+jump" );
//}
//public Action:Timer_JumpStop(Handle:timer, any:client)
//{
//FakeClientCommand( client, "-jump" );
//}
...
if(!( GetEntityFlags(client) & FL_ONGROUND ) )
	return Plugin_Stop;
if( GetClientTeam(client) == _:TFTeam_Blue && !IsFakeClient( client ) && !bInRespawn[client] && GameRules_GetRoundState() == RoundState_RoundRunning ) //&& !TF2_IsPlayerInCondition( client, TFCond_Cloaked ) )//GameRules_GetRoundState() != RoundState_BetweenRounds
{
	new MoneypackL = CreateEntityByName( "item_currencypack_large" );
	DispatchKeyValue(MoneypackL, "OnPlayerTouch", "!self,Kill,,0,-1");
	DispatchKeyValue(MoneypackL, "targetname", "robohuman_cash");
	DispatchSpawn(MoneypackL);
	GroundEntity(MoneypackL);
//	SetEntityGravity(MoneypackL, 1.0);
//	PrintToChatAll("Check succes");
	new Float:position[3];
	GetClientAbsOrigin(client, position);
	GetEntPropVector(client, Prop_Send, "m_vecOrigin",position);
	TeleportEntity(MoneypackL, position, NULL_VECTOR, NULL_VECTOR);
	CreateTimer( 29.0, Timer_CashKill );
	return Plugin_Continue;
}
return Plugin_Continue;
}

public Action:Timer_CashKill(Handle:timer)
{
	new i = -1;
	while ((i = FindEntityByClassname(i, "item_currencypack_large")) != -1)
	{
		if(IsValidEntity(i))
		{
			decl String:strName[50];
			GetEntPropString(i, Prop_Data, "m_iName", strName, sizeof(strName));
			if(strcmp(strName, "robohuman_cash") == 0)
			{
				AcceptEntityInput(i, "Kill");
				break;
			}
...
public TF2_OnConditionAdded(client, TFCond:condition) //congafix
{
	new TFClassType:class = TF2_GetPlayerClass(client);
	if(condition==TFCond_Taunting && GetClientTeam(client) == _:TFTeam_Blue && class == TFClass_Engineer)
	{
		GetClientUserId( client );
		TF2_RemoveCondition(client, TFCond_Taunting);
	}
	if(condition==TFCond_HalloweenKart && GetClientTeam(client) == _:TFTeam_Blue)
	{
		GetClientUserId( client );
		TF2_RemoveCondition(client, TFCond_HalloweenKart);
		PrintToChat( client, "----------ANTI CHEAT----------" );
		PrintToChat( client, "Kicked for using !Kart Command" );
		PrintToChat( client, "------------------------------" );
		KickClient(client, "Kicked for using !Kart Command", "");
	}
}

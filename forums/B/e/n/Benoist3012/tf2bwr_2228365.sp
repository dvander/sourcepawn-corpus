...
#define PLUGIN_VERSION				"Dev 1.3.7.2"
...
public Action:Timer_OnPlayerSpawn( Handle:hTimer, any:iUserID )
{
	...
	if(!IsMannhattan && CanTeleportBomb && GameRules_GetRoundState() == RoundState_RoundRunning && class != TFClass_Sniper && class != TFClass_Spy && class != TFClass_Engineer)
	{
		new iflagr = -1;
	...
	if(IsMannhattan && GameRules_GetRoundState() == RoundState_RoundRunning && class != TFClass_Sniper && class != TFClass_Spy && class != TFClass_Engineer && ((LastGateCapture + 19.9) < GetEngineTime()))
	{
		new iflagr = -1;
		if(CanTeleportBomb1)
		{
...
stock PickRandomClassVariant( TFClassType:iClass = TFClass_Unknown )
{
	...
	switch( iClass )
	{
		case TFClass_Medic:		return !bGiants ? GetRandomInt(0,4) : GetRandomInt(0,4); //giant medic is case 5 disabled
...
GetRandomPlayer(team)
{
    ...
    for (new i = 1; i <= MaxClients; i++)
	{
        if (IsClientInGame(i) && GetClientTeam(i) == team && IsValidRobot(i))
		{
			...
			if(class != TFClass_Sniper && class != TFClass_Spy && class != TFClass_Engineer && iRobotMode[i] != Robot_SentryBuster && IsPlayerAlive(i))
			{

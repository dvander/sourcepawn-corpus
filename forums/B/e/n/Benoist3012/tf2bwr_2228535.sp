...
#define PLUGIN_VERSION				"Dev 1.3.7.3"
...
public Plugin:myinfo = 
{
	...
	author = "Updater(Michalpl) Orginal(Leonardo)",
...
#if defined _updater_included
	sm_tf2bwr_autoupdate = CreateConVar( "sm_tf2bwr_autoupdate", "0", "If Updater plugin installed, autoupdate plugin.", FCVAR_PLUGIN );
...
//	HookEvent( "player_death", OnPlayerDeathPre, EventHookMode_Pre ); // unused
			...
			if( nTeamNum == _:TFTeam_Blue )
			{
				...
				if( ( (LastGateCapture + 17.5) > GetEngineTime() ) && IsMannhattan)
				{
					if( iRobotMode[i] != Robot_Giant &&  iRobotMode[i] != Robot_SentryBuster )
						TF2_StunPlayer(i, 3.0, 0.0, TF_STUNFLAGS_NORMALBONK|TF_STUNFLAG_NOSOUNDOREFFECT, _);
				}
			}
			if( nTeamNum != _:TFTeam_Blue )
			{
...
public Action:Timer_OnPlayerSpawn( Handle:hTimer, any:iUserID )
{
	...
	TF2_AddCondition(iClient, TFCond_UberchargedHidden, -1.0);
...
public OnPostInventoryApplication( Handle:hEvent, const String:strEventName[], bool:bDontBroadcast )
{
	...
	switch( iClass )
	{
		...
		case TFClass_Soldier:
		{
			...
			else if( iRobotVariant[iClient] == 10 || iRobotVariant[iClient] == 11 ) //Boss System
			{
				if( iRobotVariant[iClient] == 11 )// Major Crits
				{
					//Sorry I must use SpawnWeapon because some attrib don't work with the old system
					new String:weaponAttribs[256];
					Format(weaponAttribs, sizeof(weaponAttribs), "2 ; 5 ; 103 ; 1 ; 99 ; 2 ; 522 ; 1 ; 318 ; 3 ; 6 ; 2 ; 411 ; 1 ; 521 ; 1 ; 107 ; 0.4 ; 329 ; 0.4 ; 252 ; 0.4 ; 405 ; 0.1 ; 26 ; 39800");
					SpawnWeapon( iClient, "tf_weapon_rocketlauncher", 228, 100, 5, weaponAttribs );
				}
				if( iRobotVariant[iClient] == 10 )// Sergeant Crits
				{
					hAttributes = CreateArray();
					PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:59800.0 );
					PushArrayCell( hAttributes, 107 ); PushArrayCell( hAttributes, _:0.3 );// The true speed of this boss is 0.5 but Sergeant Crits is a very powerfull boss
			...
			if( iRobotVariant[iClient] >= 1 && iRobotVariant[iClient] <= 4 )
			{
				...
				if( iRobotVariant[iClient] == 1 )
				{
					PushArrayCell( hAttributes, 116 ); PushArrayCell( hAttributes, _:1.0 ); //buff soldier
				}
				else
				{
				...
				SpawnItem( iClient, ( iRobotVariant[iClient] == 4 ? 226 : ( iRobotVariant[iClient] == 3 ? 354 : 129 ) ), hAttributes ); //fixed crash issue conercho had battalion buff and vice versa
				SpawnItem( iClient, 18 );
			}
		...
		case TFClass_Pyro:
		{
			...
			else if( iRobotVariant[iClient] == 6 )// Chief Pyro (Boss System)
			{
				hAttributes = CreateArray();
...
//					}
				...
				else if( iRobotVariant[iClient] == 5 ) //Major Bomber (Boss System)
				{
					hAttributes = CreateArray();
				...
				else if( iRobotVariant[iClient] == 6 ) //Chief Tavish (Boss System)
				{
					hAttributes = CreateArray();
				...
				else if( iRobotVariant[iClient] == 7 ) //Sir NukeSalot (Boss System)
				{
					new String:weaponAttribs[256];
					Format(weaponAttribs, sizeof(weaponAttribs), "466 ; 0 ; 521 ; 1 ; 2 ; 7 ; 6 ; 2 ; 318 ; 1.8 ; 522 ; 1 ; 99 ; 1.2 ; 3 ; 0.5 ; 411 ; 5 ; 103 ; 0.8 ; 107 ; 0.35 ; 329 ; 0.4 ; 26 ; 49825");
					SpawnWeapon( iClient, "tf_weapon_cannon", 996, 100, 5, weaponAttribs );
				}
				else if( iRobotVariant[iClient] == -1 ) //samurai demo dev.
				{
					hAttributes = CreateArray();
					...
					PushArrayCell( hAttributes, 1030 ); PushArrayCell( hAttributes, _:1.0 ); //test
...
public Menu_BossSelect(Handle:menu, MenuAction:action, param1, param2)
{
	...
	if( GetClientTeam(client) == _:TFTeam_Blue )
	{
		if(StrEqual(Bossstr,"MajorCrits"))
			SetClassVariant( client, TFClass_Soldier, 11 );
		else if(StrEqual(Bossstr,"SirNukesalot"))
			SetClassVariant( client, TFClass_DemoMan, 7 );
		else if(StrEqual(Bossstr,"ChiefTavish"))
			SetClassVariant( client, TFClass_DemoMan, 6 );
		else if(StrEqual(Bossstr,"ChiefPyro"))
			SetClassVariant( client, TFClass_Pyro, 6 );
		else if(StrEqual(Bossstr,"CaptainPunch"))
			SetClassVariant( client, TFClass_Heavy, 11 );
		else if(StrEqual(Bossstr,"MajorBomber"))
			SetClassVariant( client, TFClass_DemoMan, 5 );
		else if(StrEqual(Bossstr,"SergeantCrits"))
			SetClassVariant( client, TFClass_Soldier, 10 );
	}
}
//////////////////////////////////////////////////////////
//////////////BOSS SYSTEM END////////////////////////////
...
public Action:OnCapZoneEndTouch( iEntity, iOther )
{
	...
	iDeployingBomb = -1;
//	new iClient = GetClientUserId( iClient );
			...
			case TFClass_Soldier:
			{
				if( iVariant >= 0 && iVariant <= 11 )
				{
					...
					if( iVariant == 0 )
						...
						iMode = Robot_Giant;
					if( iVariant == 10 || iVariant == 11 )//Effect reserved for Boss
					{
						iMode = Robot_Giant;
			...
			case TFClass_DemoMan:
			{
				if( iVariant >= 0 && iVariant <= 7 )
				{
					...
					if( iVariant == 0 )
						...
						iMode = Robot_Giant;
					if( iVariant == 5 || iVariant == 6 || iVariant == 7)//Effect reserved for Boss
					{
						iMode = Robot_Giant;
			...
			case TFClass_Pyro:
			{
				if( iVariant >= 0 && iVariant <= 6 )
				{
					...
					if( iVariant == 0 )
						...
						iMode = Robot_Giant;
					if( iVariant == 6)//Effect reserved for Boss
					{
						iMode = Robot_Giant;
						iNewEffect = Effect_UseBossHealthBar;
					}
...
//						iMode = Robot_BigNormal;
						...
						iMode = Robot_Giant;
					if( iVariant == 11)//Effect reserved for Boss
					{
						iMode = Robot_Giant;
						iNewEffect = Effect_UseBossHealthBar;
					}
	...
	switch( iClass )
	{
		...
		case TFClass_Soldier:
		{
			switch( iVariant )
			{
				...
				case 10: strcopy( strBuffer, iBufferSize, "Sergeant Crits" );
				case 11: strcopy( strBuffer, iBufferSize, "Major Crits" );
			}
		}
		case TFClass_DemoMan:
		{
			switch( iVariant )
			{
				...
				case 7: strcopy( strBuffer, iBufferSize, "Sir Nukesalot" );
...
GetRandomPlayer(team)
{
    ...
    for (new i = 1; i <= MaxClients; i++)
	{
        if (IsClientInGame(i) && GetClientTeam(i) == team && IsValidRobot(i))
		{
			...
			if(class != TFClass_Sniper && class != TFClass_Spy && class != TFClass_Engineer && class != TFClass_Medic && iRobotMode[i] != Robot_SentryBuster && IsPlayerAlive(i))
			{
...
//FakeClientCommand( client, "-jump" );
//}
//public OnPlayerDeathPre(Handle:event, const String:name[], bool:dontBroadcast) // not working don't use EventHook_Deathstuff
//{
//	if (GetEventInt(event, "eventtype") == player_death  )
//	{
//		PrintToChatAll("Event runned");
//		new client = GetClientOfUserId(GetEventInt(event, "player"));
//		new MoneypackL = CreateEntityByName( "item_currencypack_large" );
//		if(GetClientTeam(client) == _:TFTeam_Blue && IsValidRobot(client)) //&& IsValidClient( client ))
//		{

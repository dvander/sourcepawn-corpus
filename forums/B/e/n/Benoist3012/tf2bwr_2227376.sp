...
//	CreateTimer(0.0, FindReviveMaker); 
	...
	for( i = 1; i <= MaxClients; i++ )
		if( IsValidClient(i) && IsPlayerAlive(i) )
		{
			...
			if(i == Carrier && IsValidRobot(i) && bInRespawn[i])
			{
...
public Action:Timer_OnPlayerSpawn( Handle:hTimer, any:iUserID )
{
	...
	if(CanTeleportBomb && GameRules_GetRoundState() == RoundState_RoundRunning)
	{
		new iflagr = -1;
...
public OnPostInventoryApplication( Handle:hEvent, const String:strEventName[], bool:bDontBroadcast )
{
	...
	switch( iClass )
	{
		...
		case TFClass_Pyro:
		{
			...
			{
				hAttributes = CreateArray();
				PushArrayCell( hAttributes, 26 ); PushArrayCell( hAttributes, _:2825.0 ); //giant pyro
				PushArrayCell( hAttributes, 107 ); PushArrayCell( hAttributes, _:0.5 );
...
//public Action:OnFlagTouch( iEntity, client )
//{
//	if( !IsValidClient(client) || GetClientTeam(client) != _:TFTeam_Blue || iRobotMode[client] == Robot_Giant || !bFlagPickup)
//	{
//		PrintToChatAll("Timer trial has been trigerred."); 
//		g_hbombs1 = CreateTimer(5.0, Timer_bombst1);
//		g_hbombs2 = CreateTimer(20.0, Timer_bombst2);
//		g_hbombs3 = CreateTimer(40.0, Timer_bombst3);
//	}
//	
//}
...
public Action:Timer_RemoveWeapons(Handle:timer, any:Deployer)
    {
		...
		new weapon = GetEntPropEnt(Deployer, Prop_Send, "m_hActiveWeapon");
...
GetRandomPlayer(team)
{
    ...
    for (new i = 1; i <= MaxClients; i++)
	{
        if (IsClientInGame(i) && GetClientTeam(i) == team && IsValidRobot(i))
		{
			...
			if(class != TFClass_Sniper && class != TFClass_Spy && class != TFClass_Engineer && iRobotMode[i] != Robot_SentryBuster)
			{

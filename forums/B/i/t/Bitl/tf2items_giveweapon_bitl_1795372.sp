...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_giveaway", Command_Giveaway, ADMFLAG_CHEATS, "sm_giveaway <itemindex>");
...
public Action:Command_Giveaway(client, args)
{
	new String:arg1[32];
	new weaponLookupIndex = -1;
	...
	if (args != 1)
	{
		ReplyToCommand(client, "[TF2Items] Usage: sm_giveaway <itemindex>");
		return Plugin_Handled;
	}
	...
	for (new i = 0; i < rndClient; i++)
	{
		GiveWeaponOfIndex(rndClient, weaponLookupIndex, weaponSlot);
		LogAction(client, rndClient, "\"%L\" has won weapon %d \"%L\"", client, weaponLookupIndex, rndClient);
	}
 	if (cvar_notify == 1 || cvar_notify == 2)
	{
			ReplyToCommand(rndClient, "[TF2Items] You have won weapon %d!", weaponLookupIndex);
			ReplyToCommand(client, "[TF2Items]  %N has won weapon %i!", rndClient, weaponLookupIndex);
	}

...
public MenuHandler1(Handle:menu, MenuAction:action, param1, param2)
{
	...
	if (action == MenuAction_Select)
	{
		...
		{
			BuildMiniSentry(param1,flEndPos,flAng,1);
		}
		else if(StrEqual(info,"mslvl2"))
		{
			BuildMiniSentry(param1,flEndPos,flAng,2);
		}
		else if(StrEqual(info,"mslvl3"))
		{
			BuildMiniSentry(param1,flEndPos,flAng,3);
		}
		BuildMenu(param1);
	}
}
...
public Action:BuildMiniSentry(iBuilder,Float:fOrigin[3], Float:fAngle[3],iLevel)
{
	fAngle[0] = 0.0;
	decl String:name[60];
	GetClientName(iBuilder,name,sizeof(name));
	ShowActivity2(iBuilder,"[SM]","Spawned a mini sentry(lvl %d)", iLevel);
	decl String:sModel[64];
	new iTeam = GetClientTeam(iBuilder);
	...
	if (iTeam == 2 && iLevel == 1)
	{
		SetEntProp(iSentry, Prop_Send, "m_nSkin", 2);
	}
	else if (iTeam == 3 && iLevel == 1)
	{
		SetEntProp(iSentry, Prop_Send, "m_nSkin", 3);
	}
	else
	{
		SetEntProp(iSentry, Prop_Send, "m_nSkin", iTeam-2);
	...
	SetEntPropFloat(iSentry, Prop_Send, "m_flModelScale", 0.6);
	return Plugin_Handled;
}

...
new const String:PLUGIN_NAME[]= "teambets"
new const String: PLUGIN_AUTHOR[]= "Bittersweet"
new const String:PLUGIN_DESCRIPTION[]= "Bet on Team to Win (adaptation of Team Bets by GrimReaper - Original by ferret)"
new const String: PLUGIN_VERSION[]= "2012.11.14.11.43"
...
public OnPluginStart()
{
	...
	myself = GetMyHandle();
	BotTakeoverClientsHandle = BotTakeoverClients(myself);
...
public Action:Command_Say(client, args)
{
	...
	if (strcmp(szParts[0],"bet",false) == 0)
	{
		...
		if (strcmp(szParts[1],"ct",false) == 0 || strcmp(szParts[1],"t", false) == 0)
		{
			...
			//Looks like bet will go through past here
			
			new BotTakeover = GetArrayCell(BotTakeoverClientsHandle, client)
			//PrintToServer("[TeamBets line 254] Client=%i, Array=%i", client, BotTakeover);
			if (BotTakeover > 0)
			{
				PrintToChat(client, "\x04[TeamBets]\x01 You can't bet in any round after you've taken over a BOT");
				return Plugin_Handled;
			}
			...
			SetArrayCell(BotTakeoverClientsHandle, client, 2) //Shows that you've used Team Bets, so you can't use BOT Takeover this round
			if (IsClientConnected(client) && IsClientObserver(client))
			{
				PrintHintText(client, "You can't take over BOTs in any round after you've placed a bet")
			}

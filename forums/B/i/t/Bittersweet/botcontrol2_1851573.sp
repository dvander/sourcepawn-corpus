...
#define PLUGIN_DESCRIPTION  "Allows players to control bots while they're dead - modified from version 1.3 by Bittersweet"
...
public Action:NewTarget(iClient, const String:cmd[], args)
{
	...
	if (!bBcEnabled || !IsValidClient(iTarget) || !IsClientObserver(iClient)) return Plugin_Continue;
	CreateTimer(0.1, DisplayTakeOverMessage, iClient);
	return Plugin_Continue;
}
public Action:DisplayTakeOverMessage(Handle:timer, any:iClient)
{
	if (!bBcEnabled || !IsClientConnected(iClient)) return Plugin_Continue;
	...
	if (IsFakeClient(iTarget) && ClientTeam == GetClientTeam(iTarget))
	{

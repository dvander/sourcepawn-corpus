...
{	
	...
	if(IsValidClient(client))
	{
		...
		if ((CheckCommandAccess(client, "spectator_immunity", ADMFLAG_KICK)) && areAdminsImmune)
		...
		if (team == 1)
		{
			...
			CreateTimer(time, SpectatorTimer, GetClientSerial(client));
			return Plugin_Handled;
		}
	}
...
public Action:SpectatorTimer(Handle:timer, any:serial)
{
	if(serial == 0) return Plugin_Handled;

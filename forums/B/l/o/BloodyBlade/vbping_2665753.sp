...
ConVar cvar_KickMsgPublic;
ConVar cvar_ShowWarnings;
ConVar cvar_WarningMsg;
...
public void OnPluginStart()
{
	...
	// Config Cvars
	cvar_MinTime = CreateConVar("sm_vbping_mintime",				"90",				"Minimum playtime before ping check begins", FCVAR_NONE);
	cvar_MaxPing = CreateConVar("sm_vbping_maxping",				"500",				"Maximum player ping", FCVAR_NONE, true, 1.0);
	cvar_CheckRate = CreateConVar("sm_vbping_checkrate",			"20.0",				"Period in seconds when rate is checked", FCVAR_NONE, true, 1.0);
	cvar_MaxWarnings = CreateConVar("sm_vbping_maxwarnings",		"5",				"Number of warnings before kick", FCVAR_NONE, true, 1.0);
	cvar_MinPlayers = CreateConVar("sm_vbping_minplayers",			"2",				"Minimum number of players before kicking", FCVAR_NONE);
	cvar_LogActions = CreateConVar("sm_vbping_logactions",			"0",				"Log warning and kick actions. 0 = Disabled, 1 = Enabled", FCVAR_NONE, true, 0.0, true, 1.0);
	cvar_ShowWarnings = CreateConVar("sm_vbping_showwarnings",		"1",				"Enable/disable warning messages. 0 = Disabled, 1 = Enabled", FCVAR_NONE, true, 0.0, true, 1.0);
	cvar_WarningMsg = CreateConVar("sm_vbping_warningmsg",			"You will be kicked for excessive ping. You have {WARN} out of {MAXWARN} warnings.", "Warning message sent to high ping client. {WARN} and {MAXWARN} converts to the warning count and max warnings respectively.", FCVAR_NONE);
	cvar_KickMsg = CreateConVar("sm_vbping_kickmsg",				"You have been kicked due to excessive ping",				"Kick message sent to kicked client", FCVAR_NONE);
	cvar_ShowPublicKick = CreateConVar("sm_vbping_showpublickick",	"1",				"Enable/disable public kick message. 0 = Disabled, 1 = Enabled", FCVAR_NONE, true, 0.0, true, 1.0);
	cvar_KickMsgPublic = CreateConVar("sm_vbping_kickmsgpublic",	"{NAME} was been kicked due to excessive ping",				"Public kick message. {NAME} converts to the player's name.", FCVAR_NONE);
	cvar_ImmunityFlag = CreateConVar("sm_vbping_immunityflag",		"a",				"SourceMod admin flag used to grant immunity to all ping checking/kicking", FCVAR_NONE);
...
public void action_RateChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{

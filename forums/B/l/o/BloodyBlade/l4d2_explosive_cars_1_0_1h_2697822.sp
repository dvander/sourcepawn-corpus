...
public void OnPluginStart()
{
	...
	g_cvarDamage = CreateConVar("l4d2_explosive_cars_damage", "0", "Damage made by the explosion", FCVAR_NOTIFY);
	...
	g_cvarUnload = CreateConVar("l4d2_explosive_cars_unload", "c1m4_atrium", "On which maps should the plugin disable itself? (Example: c5m3_cemetery, c5m5_bridge, cmdd_custom)", FCVAR_NOTIFY);
...
static void FindMapCars()
{
	...
	for (int i = MaxClients; i < maxEnts; i++)
	{
		...
		if(StrEqual(classname, "prop_physics") || StrEqual(classname, "prop_physics_override"))
		{
			...
			else if (StrEqual(model, "models/props/cs_assault/forklift.mdl", false) || StrEqual(model, "models/props_fairgrounds/bumpercar.mdl", false))
			{
				g_bHooked[i] = true;
				SDKHook(i, SDKHook_OnTakeDamagePost, OnTakeDamagePost);
				DebugPrintToAll("Activated Explosive Car Damage Hook on entity %i, class %s", i, classname);
...
public void OnTakeDamagePost(int victim, int attacker, int inflictor, float damage, int damagetype)
{
	...
	else if(StrEqual(attackerClass, "env_explosion") && !g_cvarExplosionDmg)
	{
		damage = 3000.0;
	}
	...
	else if((StrEqual(attackerClass, "player") && attacker > 0 && attacker <= MaxClients && IsClientInGame(attacker) && GetClientTeam(attacker) == 3) && !g_cvarInfected)
	{
		damage = 0.0;
	}

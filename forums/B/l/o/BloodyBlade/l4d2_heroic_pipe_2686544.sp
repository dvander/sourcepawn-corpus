...
static ConVar l4d2_heroic_pipe_grabbed;
static ConVar l4d2_heroic_pipe_incapped;
static ConVar l4d2_heroic_pipe_key;
static ConVar l4d2_heroic_pipe_debug;
static ConVar l4d2_heroic_pipe_radius;
static ConVar l4d2_heroic_pipe_power;
static ConVar l4d2_heroic_pipe_setup[CHARACTERS];

static Handle BoomTimers[MAXPLAYERS];
static char SOUND_BYE[CHARACTERS][STRING_SIZE];
static float Times[CHARACTERS];
static int IsGrab[MAXPLAYERS];
...
public Action EventVictimGrabbed(Event event, const char[] name, bool dontBroadcast)
{
...
public Action EventVictimReleased(Event event, const char[] name, bool dontBroadcast)
{
	int victim = GetEventInt(event, "victim");
	int client = GetClientOfUserId(victim);
...
public void UpdateSetup(any id)
{
	static char setup[2][64];
...
public int GetCharacterID(any client)
{
	int id = 0;
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	...
	if(GetClientTeam(client) == TEAM_SURVIVORS)
	{
		...
		int offset = GetEntSendPropOffs(client, "m_isIncapacitated");
		int is_incap = GetEntData(client,offset,1);
		int is_grab = IsGrab[client - 1];
		int proceed = 0;
		...
		else if(GetConVarInt(l4d2_heroic_pipe_incapped) && !GetConVarInt(l4d2_heroic_pipe_grabbed))
		{
			proceed = is_incap && !is_grab;
		}
		else if(!GetConVarInt(l4d2_heroic_pipe_incapped) && GetConVarInt(l4d2_heroic_pipe_grabbed))		
...
public void CreateBoomTimer(int client)
{
	if (BoomTimers[client - 1] == null)
	{
		float origin[3];
...
public void KillBoomTimer(int client)
{
	if (BoomTimers[client - 1] != null)
	{
...
public void SetupNickChanged(ConVar cvar, const char[] oldVal, const char[] newVal)
{
	UpdateSetup(0);
}

public void SetupRochelleChanged(ConVar cvar, const char[] oldVal, const char[] newVal)
{
	UpdateSetup(1);
}

public void SetupCoachChanged(ConVar cvar, const char[] oldVal, const char[] newVal)
{
	UpdateSetup(2);
}

public void SetupEllisChanged(ConVar cvar, const char[] oldVal, const char[] newVal)
{
	UpdateSetup(3);
}

public void SetupBillChanged(ConVar cvar, const char[] oldVal, const char[] newVal)
{
	UpdateSetup(4);
}

public void SetupZoeyChanged(ConVar cvar, const char[] oldVal, const char[] newVal)
{
	UpdateSetup(5);
}

public void SetupFrancisChanged(ConVar cvar, const char[] oldVal, const char[] newVal)
{
	UpdateSetup(6);
}

public void SetupLouisChanged(ConVar cvar, const char[] oldVal, const char[] newVal)
{
	UpdateSetup(7);
}
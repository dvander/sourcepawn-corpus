...
{
	CreateConVar("l4d2_addsp_version", PLUGIN_VERSION, "L4D2 ADD Special Infected", FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_REPLICATED|FCVAR_DONTRECORD);
	hSpawnInterval = CreateConVar("l4d2_addsp", "10", "special infected spawn interval", FCVAR_NOTIFY, true, 0.0);
	hDiffMult = CreateConVar("l4d2_diffmult", "1.0", "Multiplication tuning for SI spawn interval", FCVAR_NOTIFY, true, 0.0);
	...
	AutoExecConfig(true, "l4d2_addsp");
}
...
public void OnClientPutInServer(int client)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if(i)
		{
			...
			{
				surv++;
				int SpawnInterval = RoundToCeil(float((GetConVarInt(hSpawnInterval)) - (surv / GetConVarInt(hDiffMult))));
				SetConVarInt(FindConVar("hSpawnInterval"), SpawnInterval);
			}
		}
	}
}

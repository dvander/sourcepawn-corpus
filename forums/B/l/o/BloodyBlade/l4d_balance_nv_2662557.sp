...
public void OnPluginStart()
{
	...
	l4d_balance_reaction_time = CreateConVar("l4d_balance_reaction_time", "15", "reaction time of balance system [10, 60]seconds"); 
	...
	l4d_balance_limit_special_add = CreateConVar("l4d_balance_limit_special_add", "0", " The increment of special's limit for each extra player [0, 5]");
	l4d_balance_limit_common = 		CreateConVar("l4d_balance_limit_common", "20", " z_common_limit [30, 100]"); 
	l4d_balance_limit_common_add = 	CreateConVar("l4d_balance_limit_common_add", "0", " The increment of z_common_limit for each extra player[0, 30]"); 
	...
	HookEvent("player_left_start_area", start_timers);
	HookEvent("player_left_checkpoint", start_timers);
...
public Action start_timers(Event event, const char[] name, bool dontBroadcast)
{
	if(GameMode != 1) return;
...
void Z_Spawn_Old(int siCount, int ciCount, int mob)
{
	...
	{		
		...
		if(mob > 0)
		{
			SpawnCommand(bot, "z_spawn_old", "mob");		
			MobTick = 0;
		}

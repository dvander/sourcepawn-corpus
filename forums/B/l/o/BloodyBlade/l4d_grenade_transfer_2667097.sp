...
public void OnPluginStart()
{
	...
	h_Version = CreateConVar("grenade_transfer_version", PLUGIN_VERSION, "Grenade Transfer version", FCVAR_NONE|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY)
...
public Action EventPlayerShoved(Handle h_Event, const char[] s_Name, bool b_DontBroadcast)
{
	...
	int i_Attacker_Grenade, i_Victim_Grenade, i
	char s_Victim_Grenade[32]
	...
	for (i = 0; i < sizeof(g_Weapons); i++)
	{
		if (StrEqual(s_Weapon, g_Weapons[i]))
		{
			if (IsValidEntity(i_Weapon) && IsValidEdict(i_Weapon) && i_Victim_Grenade == -1)
			{
				...
				PrintToChatAll("\x05%N \x01%t \x04%t \x01%t \x05%N", i_Attacker, "Gave", s_Weapon, "To", i_Victim);
...
public void PlaySound(int i_Client, char s_Sound[32])
{
...
public void GiveGrenade(int i_Client, char s_Class[32])
{
	int i_Ent	

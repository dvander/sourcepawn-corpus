...
int g_OldHealth[MAX_PLAYERS+1];
bool g_IsGhostBuffed[MAX_PLAYERS+1];

bool g_IsSwitchedInf[MAX_PLAYERS+1];
bool g_HasSpawnTimer[MAX_PLAYERS+1];
bool g_IsGhosting[MAX_PLAYERS+1];

Handle hResetGhostUse[MAX_PLAYERS+1];
Handle hCheckGhostsTimer[MAX_PLAYERS+1];
Handle hBuffGhostsTimer[MAX_PLAYERS+1];

ConVar hHunterHealth;
ConVar hSmokerHealth;
ConVar hBoomerHealth;
ConVar hSpitterHealth;
ConVar hChargerHealth;
ConVar hJockeyHealth;
...
public void OnPluginStart()
{
	...
	CreateConVar("l4d2_ig_ver",PLUGIN_VERSION,"Invincible Ghosts version", FCVAR_NONE|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
public Action Event_GhostSpawning(Event event, const char[] name, bool dontBroadcast)
{	
	int clientId = GetEventInt(event, "userid")
	int client = GetClientOfUserId(clientId);
	...
	float tilSpawn = GetEventInt(event, "spawntime") - 4.0;
...
public Action CheckGhost(Handle timer, Handle pack)
{
	...
	float loopTime = ReadPackFloat(pack);
...
public Action Event_GhostDeath(Event event, const char[] name, bool dontBroadcast)
{	
	int clientId = GetEventInt(event, "userid")
	int client = GetClientOfUserId(clientId);
...
public Action Event_InfSpawned(Event event, const char[] name, bool dontBroadcast)
{
	int clientId = GetEventInt(event, "userid")
	int client = GetClientOfUserId(clientId);
...
public Action Event_InfFirstSpawn(Event event, const char[] name, bool dontBroadcast)
{
	int clientId = GetEventInt(event, "userid")
	int client = GetClientOfUserId(clientId);

...
{
	name			= "[TF2] Powerup Runes Spawner",
	author			= "Classic, Bonkorn",
	description		= "Spawns Mannpower Powerup Runes on Any Maps.",
	version			= AK_VERSION,
	url				= ""
};
...
public OnPluginStart()
{
	...
	//Menu command
	RegAdminCmd("sm_runemenu",AK_Menu,ADMFLAG_CONFIG,"Opens the auto kit menu.");
	...
	HookEntityOutput("info_powerup_spawn",  "OnPlayerTouch",	EntityOutput_Kit);
...
{ 
	...
	SQL_TConnect(SQL_OnConnect, "powerupspawner"); 
} 
...
{ 
	...
	{ 
		...
		PrintToServer("[RuneSpawner] The Powerup Runes table is ready!"); 
	} 
}  
...
public Action:AK_Menu(client,args)
{
	...
	SetMenuTitle(menu,"Powerup Spawner Menu:");
	AddMenuItem(menu, "0", "Delete Nearest 1 Powerup Rune", ITEMDRAW_DEFAULT);
	AddMenuItem(menu, "1", "Normal", ITEMDRAW_DEFAULT);
...
{ 
	...
	{ 
		//Worked, so we tell them so 
		PrintToChatAll( "\x04[RuneSpawner]\x01 Powerup Rune location added to database!");
	} 
}  
...
public SpawnKit(type_kit, Float:kit_pos[3])
{
	...
	switch(type_kit)
	{
		case POWERUP_NORMAL:
		{
			...
			kit_ent = CreateEntityByName("info_powerup_spawn");
		}
	}
	if( kit_ent <= -1)
		return;
	...
	if (DispatchSpawn(kit_ent))
	{
		...
		if(!onFirstSeconds)
			EmitSoundToAll("items/powerup_pickup_base.wav", kit_ent, _, _, _, 0.75);
	}
}
...
public EntityOutput_Kit(const String:output[], entity, activator, Float:delay)
{
	...
	if (StrEqual(name, KIT_NAME, false))	// make sure it is onw of our kits
	{
		...
		if (StrEqual(classname, "info_powerup_spawn", false))
			type = POWERUP_NORMAL;
		if( type <= -1)
			return;

...
{  
    ...
    {
        RTLify(message, sizeof(message), message);
    ...
    else if (msgStart == 2 && CheckCommandAccess(client, "sm_psay", ADMFLAG_CHAT)) // sm_psay alias
    {
        ...
        decl String:rtled[192];
        RTLify(rtled, sizeof(rtled), message[len]);
 
        PrintToChat(client, "\x01 \x04(Private to %N) %N: \x01%s", target, client, rtled);
        PrintToChat(target, "\x01 \x04(Private to %N) %N: \x01%s", target, client, rtled);
...
public Action:Command_SmTsay(client, args)
{
    ...
    SetHudTextParams(-1.0, 0.2, 5.0, red, green, blue, 255, 0, 0.25, 1.0, 1.0);
...
public Action:Command_SmPsay(client, args)
{
    ...
    decl String:text[192], String:arg[192], String:message[192];
    ...
    {
        PrintToChat(client, "\x01 \x04(Private: %N) %s: \x01%s", target, name, message);
    }
 
    PrintToChat(target, "\x01 \x04(Private: %N) %s: \x01%s", target, name, message);
    LogAction(client, -1, "\"%L\" triggered sm_psay to \"%L\" (text %s)", client, target, message);
...
SendChatToAll(client, String:message[])
{
    ...
    for (new i = 1; i <= MaxClients; i++)
    {
        ...
        PrintToChat(i, "\x01 \x04(ALL) %s: \x01%s", nameBuf, message);
    }
}
...
SendChatToAdmins(from, String:message[])
{
    ...
    for (new i = 1; i <= MaxClients; i++)
    {
        ...
        {
            PrintToChat(i, "\x01 \x04(%sADMINS) %N: \x01%s", fromAdmin ? "" : "TO ", from, message);
        }  
    }
}

...
new Handle:SpawnProtectionOnBotControl;
...
public OnPluginStart()
{
  ...
  SpawnProtectionOnBotControl = CreateConVar("sp_sponbotcontrol", "1");
...
public Action:OnPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
  if(GetConVarInt(SpawnProtectionEnabled) == 1)
  {
    ...
    //If player controlling a bot
    if (!GetConVarBool(SpawnProtectionOnBotControl) && IsPlayerControllingBot(client)) {
...
public Action:RemoveProtection(Handle:timer, any:client)
{
  if(IsClientInGame(client))
  {
    ...
    if(GetConVarInt(SpawnProtectionNotify) > 0)
      PrintToChat(client, "\x04[SpawnProtection] \x01You are no longer spawn protected.");
  }
}
...
{ 
  return bool:GetEntData(client, g_bIsControllingBot, 1);  
}
...
new bool:g_bIonCannonAllowed = false;
...
{
	new Handle:hVersion = CreateConVar("sm_ion_version", IC_VERSION, "Ion cannon version", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_REPLICATED|FCVAR_DONTRECORD);
	if(hVersion != INVALID_HANDLE)
		SetConVarString(hVersion, IC_VERSION);
	...
	g_hCVMaxAmmo = CreateConVar("sm_ion_maxammo", "1", "How many ion cannons should a player be able to \"carry\" at a time?", FCVAR_PLUGIN, true, 1.0);
	...
	g_hCVDeployTime = CreateConVar("sm_ion_deploytime", "15", "How long does it take to display the beams after deploying a ion cannon in seconds?", FCVAR_PLUGIN, true, 0.1, true, 35.0);
	...
	g_hCVReadyVolume = CreateConVar("sm_ion_readyvolume", "0.8", "Volume of the ready sound?", FCVAR_PLUGIN, true, 0.1, true, 15.0);
	g_hCVRootOnly = CreateConVar("sm_ion_rootonly", "1", "sm_buyion for Root only?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public Action:Command_BuyIon(client, args)
{
	...
	{
		CPrintToChat(client, "%s Ion cannon buying is disabled.", PREFIX);
		return Plugin_Handled;
	}

	if(GetConVarInt(g_hCVRootOnly) && CheckCommandAccess(client, "sm_buyion", ADMFLAG_ROOT))
	{
		CPrintToChat(client, "%s This is only for rootadmins.", PREFIX);
		return Plugin_Handled;
	}

	if(g_bIonCannonAllowed)
	{
		CPrintToChat(client, "%s Ion cannon is disabled for this round", PREFIX);
		return Plugin_Handled;
	}

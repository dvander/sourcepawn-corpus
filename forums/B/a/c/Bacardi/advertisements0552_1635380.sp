...
public OnMapStart() {
	...
	g_hTimer          = CreateTimer(GetConVarInt(g_hInterval) * 1.0, Timer_DisplayAds, _, TIMER_REPEAT);
}
...
public Action:Timer_DisplayAds(Handle:timer) {
	if (GetConVarBool(g_hEnabled)) {
		new AdminFlag:fFlagList[16], String:sBuffer[256], String:sFlags[17], String:sText[256], String:sType[6];
		new len; // Use this check how many flags
		...
		if (bFlags) {
			// UPDATE
			len = FlagBitsToArray(ReadFlagString(sFlags), fFlagList, sizeof(fFlagList));
		}
		...
		if (StrContains(sType, "C") != -1) {
			for (new i = 1; i <=MaxClients; i++) {
				if (IsClientInGame(i) && !IsFakeClient(i) &&
						((!bAdmins && !(bFlags && HasFlag(i, fFlagList, len))) ||
...
// UPDATE, len is what storage flags count
bool:HasFlag(iClient, AdminFlag:fFlagList[16], len) {

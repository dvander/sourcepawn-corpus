...
{
	name = "Reserved Slots",
	author = "AlliedModders LLC, mess by Baca",
	description = "Test+SourceTV+Provides basic reserved slots",
	version = "UnOfficial",
...
new bool:tv_enable; // Bacardi, added global variable
...
public OnPluginStart()
{
	...
	tv_enable = GetConVarBool(FindConVar("tv_enable")); // Bacardi, Find cvar (works better when tv_enable 1 added autoexec.cfg, and not reload plugin)
	PrintToServer("SourceTV %s", tv_enable ? "TRUE":"FALSE");// Bacardi, print server console message
}
...
SetVisibleMaxSlots(clients, limit)
{
	...
	num = tv_enable ? num - 1: num - 0; // Bacardi, when SourceTV enabled, show 1 less server max players

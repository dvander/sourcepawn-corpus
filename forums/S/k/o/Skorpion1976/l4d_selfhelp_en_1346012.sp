...
public OnPluginStart()
{
	...
	l4d_selfhelp_incap = CreateConVar("l4d_selfhelp_incap", "2", "self help for incap , 0:disable, 1:pill, 2:medkit, 3:both  ", FCVAR_PLUGIN);
	l4d_selfhelp_grab = CreateConVar("l4d_selfhelp_grab", "2", " self help for grab , 0:disable, 1:pill, 2:medkit, 3:both ", FCVAR_PLUGIN);
	l4d_selfhelp_pounce = CreateConVar("l4d_selfhelp_pounce", "2", " self help for pounce , 0:disable, 1:pill, 2:medkit, 3:both ", FCVAR_PLUGIN);
	l4d_selfhelp_ride = CreateConVar("l4d_selfhelp_ride", "2", " self help for ride , 0:disable, 1:pill, 2:medkit, 3:both ", FCVAR_PLUGIN);
	l4d_selfhelp_pummel = CreateConVar("l4d_selfhelp_pummel", "2", "self help for pummel , 0:disable, 1:pill, 2:medkit, 3:both  ", FCVAR_PLUGIN);
	l4d_selfhelp_edgegrab = CreateConVar("l4d_selfhelp_edgegrab", "2", "self help for edgegrab , 0:disable, 1:pill, 2:medkit, 3:both  ", FCVAR_PLUGIN);
	l4d_selfhelp_eachother = CreateConVar("l4d_selfhelp_eachother", "0", "incap help each other , 0: disable, 1 :enable  ", FCVAR_PLUGIN);
	...
	l4d_selfhelp_hintdelay = CreateConVar("l4d_selfhelp_hintdelay", "2.0", "hint delay", FCVAR_PLUGIN);
...
public Action:AdvertisePills(Handle:timer, any:client)
{
	...
	{
		PrintToChat(client, "\x03Press \x04DUCK\x03 to help yourself. Works randomly!");
	}
			...
			while ((ent = FindEntityByClassname(ent,  "weapon_first_aid_kit" )) != -1)
			{
				if (IsValidEntity(ent))
				{
					...
					if(GetVectorDistance(targetVector1  , targetVector2)<dis)
					{
						...
						RemoveEdict(ent);
						pickup=true;
						PrintHintText(client,"You found a medkit!");
						break;
					}
				}
 			}
			...
			while ((ent = FindEntityByClassname(ent,  "weapon_pistol" )) != -1)
			{
				if (IsValidEntity(ent))
				{
					...
					if(GetVectorDistance(targetVector1  , targetVector2)<dis)
					{
						...
						RemoveEdict(ent);
						pickup=true;
						PrintHintText(client,"You found a pistol!");
						break;
					}
				}
 			}
...
SelfHelp(client)
{
	...
	if(slot!=-1)
	{
		...
		if(slot ==4)
		{
			...
			if(adrenaline)	PrintToChatAll("\x04%N\x03 did selfhelp with adrenaline!", client);  
			if(pills)	PrintToChatAll("\x04%N\x03 did selfhelp with pills!", client); 	
		...
		else if(slot==3)
		{
			...
			HelpState[client]=STATE_OK;
			PrintToChatAll("\x04%N\x03 did selfhelp with medkit!", client); 
	...
	{
		PrintHintText(client, "Selfhelp failed");
...
KillAttack(client)
{
	...
	if(GetConVarInt(l4d_selfhelp_kill)==1 && a!=0)
	{
		...
		{
			if(GetEntProp(a, Prop_Send, "m_zombieClass") != 8)
			{

...
public Action:CritMenu( client, args )
{
   ...
   new String:strOn[] = "On";
   new String:strOff[] = "Off";
   new bool:bCriticals = GetConVarBool(FindConVar("tf_weapon_criticals"));
   
   if(bCriticals)
   {
      Format(buffer, sizeof(buffer), "Crits are currently %s", strOn);
   }
   else
   {
      Format(buffer, sizeof(buffer), "Crits are currently %s", strOff);
   }
   
   new String:buffer2[64];
   Format(buffer2, sizeof(buffer2), "Threshold to change setting is currently %f percent of players", GetConVarFloat(g_hCvarPercent));
   
   //build the menu
   new Handle:menu = CreateMenu(MenuHandlerCrit);
   SetMenuTitle(menu, "Set Crits preference");
   AddMenuItem(menu, "Status", buffer, ITEMDRAW_RAWLINE);
   AddMenuItem(menu, "Status", buffer2, ITEMDRAW_RAWLINE);

...
1.8.1 (26-Jun-2020)
	- Fixed the helicopter being invisible to other players. Thanks to "Gold Fish" for fixing.

1.8 (22-Mar-2020)
	- Changed "molotov_projectile" to "vomitjar_projectile" so no flames are visible.
	- Changed "vomitjar_projectile" classname on kill to prevent any plugin conflicts detecting these exploding.
	- Changed plugin to use entity references which is the proper way to store and pass entities to timers.
	- This should fix the helicopter not always deleting or bugging out and not spawning.
	- Thanks to "AK978" for reporting and testing.

1.7 (21-Jul-2018) (773 downloads)
...
void CreateHelicopter(int client, int infoIndex)
{
	...
	if(IsValidClient(client))
	{
		...
		int dummy=CreateEntityByName("vomitjar_projectile"); // molotov_projectile
		DispatchKeyValue(dummy, "model", MODEL_W_VOMITJAR); // MODEL_W_MOLOTOV
...
void DropHelicopter(int client)
{
	...
	pos[2]+=20.0;
	ang[0]=0.0;
	int dummy=CreateEntityByName("vomitjar_projectile"); //molotov_projectile pipe_bomb_projectile
	DispatchKeyValue(dummy, "model", MODEL_W_VOMITJAR); // MODEL_W_MOLOTOV
	SetEntProp(dummy, Prop_Data, "m_CollisionGroup", 2);
...
public Action DeletePushForce(Handle timer, any ent)
{
	 ...
	 if (EntRefToEntIndex(ent) != INVALID_ENT_REFERENCE)
	 {
		 // char classname[64];

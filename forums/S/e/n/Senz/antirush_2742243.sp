...
public EntOut_OnStartTouch(const String:output[], caller, activator, Float:delay)
{
	...
	switch(iPunishment)
	{
		...
		case 2:
		{
			...
			// Set bounce back velocity to always 200 (little less than walk speed)
			// to prevent both too little bounce back (player is able to enter zone) and 
			// massive bounce back madness (if player is moving very fast, bunnyhopping, etc.).
			new Float:fSpeed = fVelocity[0]*fVelocity[0] + fVelocity[1]*fVelocity[1];
			new Float:fSpeedMultiplier = SquareRoot(40000.0 / fSpeed); // 200^2 = 40000
			fVelocity[0] = fVelocity[0]*fSpeedMultiplier;
			fVelocity[1] = fVelocity[1]*fSpeedMultiplier;
			
			// Don't let player fly up.
			// But also don't make negative cause then player can squeeze through barrier by spamming jump.
			if (fVelocity[2] > 0.0)
				fVelocity[2] = 0.0;
...
public MenuHandler_SelectVector(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		else if(StrEqual(info, "vec1") || StrEqual(info, "vec2"))
		{
			...
			if(myIsNullVector(g_fTempZoneVector1[param1]) && myIsNullVector(g_fTempZoneVector2[param1]))
			{
				if(g_iPlayerEditsType[param1] == TYPE_ZONE)
				{
					hZone = GetArrayCell(g_hZones, g_iPlayerEditsZone[param1]);
...
stock bool:myIsNullVector(const Float:vec[3])
{
...
stock SpawnTriggerMultipleInBox(iZoneIndex)
{
	...
	// Set solid type to bounding-box to work with trigger_multiple

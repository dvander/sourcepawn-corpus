...
public OnPluginStart()
{
	...
	g_max_give[9] = CreateConVar("sm_quota_hunting_knife", "-1", " Quota Given to each player for obtaining hunting_knife in each round ( -1 = unlimited 0 = disabled )");
...
public Action:TankBuster(clientId)
{
	...
	AddMenuItem(menu, "option0", "Babe Ruth is such an inspiration.");
	AddMenuItem(menu, "option1", "Oh boy! Let's have some tea after this game of cricket!");
	AddMenuItem(menu, "option2", "Another Freeman meme!");
	AddMenuItem(menu, "option3", "Rock and Roll, baby! YEAH!");
	AddMenuItem(menu, "option4", "Ooo! Always wanted to be an AXE murderer!");
	AddMenuItem(menu, "option5", "Gordan Ramsey taught me well!");
	AddMenuItem(menu, "option6", "Silence is the way of the ninja.");
	AddMenuItem(menu, "option7", "You fucked with the wrong Mexican!");
	AddMenuItem(menu, "option8", "Oh, Ninja Gaiden, I summon your spirit!");
	AddMenuItem(menu, "option9", "Today is a good day to wear the skin of a tank.");
			...
			case 9: // Hunting Knife * CSS
			{
				if ( give_quota9[client] > 0 || give_quota9[client] < 0) {
					...
					if (give_quota9[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Hunting Knife until next round",give_quota9[client]);
					}

...
new Handle:ASDCtype4=INVALID_HANDLE;
new Handle:ASDCtype5=INVALID_HANDLE;
new Handle:ASDCtype6=INVALID_HANDLE;
new Handle:ASDCtype7=INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	//Type Enables
	ASDCtype0 = CreateConVar("ASDCtype0", "2", "The first kind of special infected to spawn. Set to 9 to disable.", FCVAR_PLUGIN, true, 0.0, true, 9.0);
	...
	ASDCtype4 = CreateConVar("ASDCtype4", "0", "The fifth kind of special infected to spawn. Set to 9 to disable.", FCVAR_PLUGIN, true, 0.0, true, 9.0);
	ASDCtype5 = CreateConVar("ASDCtype5", "0", "The sixth kind of special infected to spawn. Set to 9 to disable.", FCVAR_PLUGIN, true, 0.0, true, 9.0);
	ASDCtype6 = CreateConVar("ASDCtype6", "0", "The seventh kind of special infected to spawn. Set to 9 to disable.", FCVAR_PLUGIN, true, 0.0, true, 9.0);
	ASDCtype7 = CreateConVar("ASDCtype7", "0", "The eighth kind of special infected to spawn. Set to 9 to disable.", FCVAR_PLUGIN, true, 0.0, true, 9.0);
	...
	ASDCmult = CreateConVar("ASDCmult", "0", "Multiplication tuning for difficulty controller. Set base and mult to 0 to turn off ASDC.", FCVAR_PLUGIN, true, 0.0);
	ASDCCImult = CreateConVar("ASDCCImult", "0", "Multiplication tuning for CI difficulty controller. Set base and mult to 0 to turn off CI part of ASDC.", FCVAR_PLUGIN, true, 0.0);
	
	//Base Intervals
	ASDCSIinterval = CreateConVar("ASDCSIinterval","1", "How many ticks(unmodified seconds) till another SI spawns", FCVAR_PLUGIN, true, 0.0);
	ASDCMOBinterval = CreateConVar("ASDCMOBinterval","210", "How many ticks(unmodified seconds) till another Mob of CI spawns", FCVAR_PLUGIN, true, 0.0);
	...
	ASDCWitchinterval = CreateConVar("ASDCwitchinterval","30", "How many ticks(unmodified seconds) till another witch spawns", FCVAR_PLUGIN, true, 0.0);
	
	//CI Controls
	ASDCcommons = CreateConVar("ASDCcommons", "8", "Number of CI per person in a CI zombie wave.", FCVAR_PLUGIN, true, 0.0);
	...
	ASDCmob = CreateConVar("ASDCmob", "10", "Number of CI per person in a Mega CI zombie wave.", FCVAR_PLUGIN, true, 0.0);
...
public Action:TimerUpdate(Handle:timer)
{
	...
	monster[4]=GetConVarInt(ASDCtype4);
	monster[5]=GetConVarInt(ASDCtype5);
	monster[6]=GetConVarInt(ASDCtype6);
	monster[7]=GetConVarInt(ASDCtype7);
...
Float:ASDC(TeamValue)
{
	...
	temp=RoundToCeil(2*(GetConVarFloat(ASDCcommons)*ASDCout*GetConVarFloat(ASDCCImult)+GetConVarFloat(ASDCcommons)));

...
public Plugin:myinfo = 
{
	...
	author = "McFlurry, 1.8 compatibility by $atanic $pirit",
...
stock StringToLower(String:string[], stringlength)
{
	...
	decl String:buffer[maxlength], String:cell[maxlength];
	Format(buffer, maxlength, string);
	
	for(new i;i<=stringlength;i++)
	{
		Format(cell, maxlength, buffer[i]);
		if(strlen(buffer[i+1]) > 0) ReplaceString(cell, maxlength, buffer[i+1], "");
		if(IsCharUpper(cell[0]))
		{
			cell[0] += 0x20;
			//CharToLower(cell[0]); this fails for some reason
			Format(cell, maxlength, "%s%s", cell, buffer[i+1]);
			ReplaceString(buffer, maxlength, cell, cell, false);
		}	
	}

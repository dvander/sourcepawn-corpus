...
#define PLUGIN_AUTHOR		"Damizean & Asherkin, mod by Spirit532"
#define PLUGIN_VERSION		"1.4.3.1 - Classname wildcard"
...
public Action:TF2Items_OnGiveNamedItem(iClient, String:strClassName[], iItemDefinitionIndex, &Handle:hItemOverride)
{
	...
	new Handle:hItem = FindItem(iClient, iItemDefinitionIndex, strClassName);
...
** -------------------------------------------------------------------------- */
Handle:FindItem(iClient, iItemDefinitionIndex, String:strClassName[])
{
	...
	// Check for each.
	new Handle:hOutput;
	hOutput = FindItemOnArray(iClient, hItemArray, iItemDefinitionIndex, strClassName);
	if (hOutput == INVALID_HANDLE)
		hOutput = FindItemOnArray(iClient, g_hGlobalSettings, iItemDefinitionIndex, strClassName);
...
** 
** -------------------------------------------------------------------------- */
Handle:FindItemOnArray(iClient, Handle:hArray, iItemDefinitionIndex, String:strClassName[])
{
	...
	for (new iItem = 0; iItem < GetArraySize(hArray); iItem++)
	{
		...
		TF2Items_GetClassname(hItem, strClassname, sizeof(strClassname));	
		//Is it a classname-based item? If so, store it.
		if(strClassname[1] != INVALID_HANDLE)
		{
			if(StrContains(strClassName, strClassname, false) == 0)
			{
				//PrintToServer("Name: %s ; name: %s", strClassName, strClassname);
...
ParseItemsEntry(Handle:hKeyValues, Handle:hEntry)
{
	...
	if (KvGotoFirstSubKey(hKeyValues))
	{
		do
		{
			...
			if(StrContains(strBuffer, "tf_", false) == 0)
			{

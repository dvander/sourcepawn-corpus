...
public Action:Hook_NormalSound(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		if (bAddClient[i] || ((g_bProcess[i] || g_bIsObserver[i]) && !g_bIsVisible[iOwner][i] && g_iPVSCache[iOwner][i] > g_iTickCount))
		{
...
public OnGameFrame()
{
	...
	if (++g_iCurrentThread > iTotalThreads)
	{
		...
		if (g_iTraceCount)
		{
			...
			iTotalThreads = g_iTraceCount / g_iMaxTraces + 1;
			
			// Adjust for prediction.
			g_fTickDelta = (iTotalThreads > 1) ? GetTickInterval() * float(iTotalThreads - 1) : 0.0;
...
new g_iCacheSpotted[MAXPLAYERS+1];
...
FarESP_Enable()
{
	...
	g_iUpdateFrequency = TIME_TO_TICK(1.5);
...
public PlayerManager_ThinkPost(entity)
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (g_bProcess[i] && GetEntData(entity, g_iPlayerSpotted + i, 1))
		{
			...
			g_iCacheSpotted[i] = g_iTickCount + g_iUpdateFrequency;
		}
		else
		{
			g_bPlayerSpotted[i] = false;
...
SendRadarFakeTeam(team)
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (g_bProcess[i])
		{
			...
			else if (g_iCacheSpotted[i] < g_iTickCount)
			{

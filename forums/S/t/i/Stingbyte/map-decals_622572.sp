/* MapDecals v0.9s
 * Original by Bernie
 * Modded by Stingbyte for Stingbyte.com
 * - Admin Levels by Defines
 ...
 * 
 * Credits:
 * - Function GetPlayerEye taken from SprayTrace (Nican132) with minor modifications
 * - Wiki-Team & IRC, all people who helps improving SM & Plugins
 */
...
#define VERSION		"0.9s"
#define SPRAYSOUND	"player/sprayer.wav"
...
public Plugin:myinfo = {
	...
	name		= "Map Decals",
	author		= "Berni - Stingbyte.com MOD",
...
public OnPluginStart() {
	...
	// Cvars
	g_cvars[MAXDIS] = CreateConVar("sm_decal_dista","50.0","How far away from the Decals position it will be traced to and check distance to prevent painting a Decal over another");
	g_cvars[POS] = CreateConVar("sm_decal_printpos","1","Turns on/off printing out of decal positions");
...
public Action:Command_PaintDecal(client, args) {
	...
	if(GetPlayerEye(client, pos)){
		...
		new DecalPos = GetConVarInt(g_cvars[POS]);
		if(DecalPos)
...
// Taken from SprayTracer (minor modifications)

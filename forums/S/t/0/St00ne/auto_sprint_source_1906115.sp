...
public OnPluginStart()
{
	...
	new Handle:version_cvar = CreateConVar("sm_autosprint_src_version", PLUGIN_VERSION, "Auto Sprint Source Version", FCVAR_NOTIFY);
	...
	cvars[1] = CreateConVar("sm_ass_time", "3", "Sprint time in seconds.", FCVAR_NONE, true, 0.0);
	cvars[2] = CreateConVar("sm_ass_cooldown", "10", "Sprint cooldown time.", FCVAR_NONE, true, 0.0);
	cvars[3] = CreateConVar("sm_ass_speed", "1.33", "Sprint speed ratio.", FCVAR_NONE, true, 0.0);
	cvars[4] = CreateConVar("sm_ass_team", "0", "Sprint team restriction: 2 = Terrorists, 3 = CT, 0 = all.", FCVAR_NONE, true, 0.0, true, 3.0);
	cvars[5] = CreateConVar("sm_ass_adverts", "1", "Enable/Disable sprint and cooldown messages", FCVAR_NONE, true, 0.0, true, 1.0);
	cvars[6] = CreateConVar("sm_ass_welcome", "1", "Enable/Disable sprint welcome info", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	RegConsoleCmd("sm_sprint", Cmd_StartSprint);
...
// Any player who disconnects (for any reason including mapchange) triggers a team change.
// So there's no need to check OnPlayerDisconnect too.
...
public Action:Cmd_StartSprint(client, args)
{
	...
	{
		ReplyToCommand(client, "[Auto Sprint Source] %s can't use this!", teamindx == 2 ? "Terrorists":"Counter-Terrorists");
		return Plugin_Handled;
	}
	...
	h_timers[client] = CreateTimer(cvars_value[0], timer_sprint, GetClientSerial(client));
...
public Action:timer_sprint(Handle:timer, any:serial)
{
	...
	if(client > 0 && IsClientInGame(client) && !IsFakeClient(client))
	{
		...
		//Let's reset player's speed even if the plugin has been disabled in the meantime
		SetEntPropFloat(client, Prop_Send, "m_flLaggedMovementValue", 1.0);
		
		//Now let's start the cooldown time only if all conditions are ok
		if(enabled && teamid >= 2 && IsPlayerAlive(client))
		{
			if(team < 1 || team == teamid)
			{
				h_timers[client] = CreateTimer(cvars_value[1], timer_cool, GetClientSerial(client));
				if (ads_enabled)
				{
...
public Action:timer_cool(Handle:timer, any:serial)
{
	new client = GetClientFromSerial(serial);
	...
	if(client > 0 && IsClientInGame(client) && !IsFakeClient(client))
	{
		...
		if(enabled && ads_enabled && teamid >= 2 && IsPlayerAlive(client))
		{
			if(team < 1 || team == teamid)
			{

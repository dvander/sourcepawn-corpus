...
#define BEBOP_VERSION	"0.3b beta"
...
GameMode()
{
	new Handle:gamemodes = FindConVar("mp_gamemode");
	...
	//{	
		//isL4D2 = true;
		//flags = GetConVarFlags(gamemodes);
		//SetConVarFlags(gamemodes, flags & ~FCVAR_PROTECTED);
	//}
	//if (isL4D2 == true)
	//{
	//	SetConVarFlags(gamemodes, flags);	
	//}
...
public Event_PlayerActivate(Handle: event, const String:name[], bool:dontBroadcast)
{
	...
	if(g_GameMode)
	{
		...
		if (!IsFakeClient(client))
		{
			...
			{
				SpawnFakeClient();
				CreateTimer(7.0, Timer_PutClientToSurvivorTeam, GetClientSerial(client), TIMER_REPEAT);
			}
		}
	}
	return;

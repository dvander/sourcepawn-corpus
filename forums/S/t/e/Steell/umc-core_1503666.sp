...
//BUGS:
//  Tiered Vote stage two when exclusion is ignored:
//      exclude arrays are not taken into account, so stage two breaks
//  SOLUTION: Implement separate filtration system (umc_utils) for array-based filtration. Use this
//      in Handle_TieredVoteTimer (or TieredVoteWinner, I forget).
//      This will also require BuildMapVoteMenu to take a Mapcycle and a KV. We do not want to
//      modify stored_kv, since it needs to contain the original complete mapcycle definition

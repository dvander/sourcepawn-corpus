...
//Data structure for an External Function.
/*enum ExtFunc
{
    Handle:ExtFunc_Plugin,
    Function:ExtFunc_Function
}
*/
...
bool:NeedRunoff()
{
    ...
    new Float:numClients = float(GetArraySize(clients));
    new Float:threshold = GetConVarFloat(cvar_runoff_threshold);
    new bool:result = (numClients / total_votes) < threshold;
    if (result)
    {
        LogMessage(
...
Handle:BuildRunoffMenu(Handle:menu, VoteHandler:callback, Handle:clientArray)
{
    ...
    for (new i = num_items - 1; i > 1; i--)
    {
        ...
        //One less map from the max
        maxMaps--;
        
        //Stop if...
        //  ...the new percentage is over the threshold.
        if (percent >= runoffThreshold || maxMaps == 1)
            break;
    }
    ...
    SetCorrectMenuPagination(newMenu, count);

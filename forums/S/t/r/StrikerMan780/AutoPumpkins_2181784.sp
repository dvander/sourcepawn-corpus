...
SQL_CreateTables()
{
	...
	if (SQL_TFastQuery(db, query))
		LogMessage("[AutoPumpkins] DB queries run, table created");
}

bool:SQL_TFastQuery(Handle:database, const String:Query[])
{
    if (database == INVALID_HANDLE)
        return false;    
    SQL_TQuery(database, SQL_FastQueryResponse, Query);
    return true;
}

public SQL_FastQueryResponse(Handle:database, Handle:query, String:error[], any:data)
{
    if (strcmp(error, "") != 0)
    {
        PrintToServer("SQL Error in AutoPumpkins: %s", error);
    }
} 

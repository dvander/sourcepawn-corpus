...
public OnPluginStart() {
	...
	LoadTranslations("autosteamupdate.phrases");
	...
	hudGCvar = CreateConVar("auto_steam_update_hud_text_green", "255", "Amount of green for the HUD timer (only on supported games)", _, true, 0.0, true, 255.0);
	hudBCvar = CreateConVar("auto_steam_update_hud_text_blue", "0", "Amount of blue for the HUD timer (only on supported games)", _, true, 0.0, true, 255.0);
...
public OnClientPostAdminCheck(client) {
	...
	if(disallowPlayers) {
		KickClient(client, "%t", "kickmessage");
	}
}
...
public Action:DoTimer(Handle:timer) {
	...
	if(timeRemaining <= -1) {
		...
		for(new i = 1; i <= MaxClients; i++) {
			...
			KickClient(i, "%t", "shutdownmessage");
		}
		ServerCommand("_restart");
		return Plugin_Stop;
	}
	if(timeRemaining / 60 <= GetConVarInt(messageTimeCvar)) {
		if(hudText != INVALID_HANDLE) {
			for(new i = 1; i <= MaxClients; i++) {
				...
				ShowSyncHudText(i, hudText, "%t", "hudtimer", timeRemaining / 60, timeRemaining % 60);
			}
		}
		if(timeRemaining > 60 && timeRemaining % 60 == 0) {
			PrintHintTextToAll("%t", "alerthint", timeRemaining / 60);
		...
		if(timeRemaining == 60) {
			PrintHintTextToAll("%t", "alerthint2");
	...
	if(timeRemaining <= 60 && hudText == INVALID_HANDLE) {
		PrintCenterTextAll("%t", "hudtimer", timeRemaining / 60, timeRemaining % 60);
	}
	return Plugin_Continue;
}
...
public Action:Command_PostponeUpdate(client, args) {
	...
	PrintHintTextToAll("%t", "cancelled", GetConVarInt(delayCvar));

...
new String:Adverts[9][2048] = {
	"In Game Radio by Frag Radio Ltd.",
	"Type !poon to show us you dont like the current song!",
	"Type !dj to see what dj is currently onair!",
	"Want this plugin for your server? Visit fragradio.co.uk/plugins to download it!",
...
public Plugin:myinfo = {
	...
	author = "BomBom - Dunceantix - Rising(vol fix)",
	...
	if (action == MenuAction_Select) {
		...
		if (found) {
			if (StringToInt(info) == 0) {
				...
				StreamPanel("Thanks for tuning into FragRadio!", url, client);

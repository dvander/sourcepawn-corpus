...
{
	PrecacheSound("vo/scout_battlecry04.wav", true);
	PrecacheSound("vo/sniper_battlecry03.wav", true);
	PrecacheSound("vo/soldier_battlecry06.wav", true);
	PrecacheSound("vo/demoman_battlecry02.wav", true);
	PrecacheSound("vo/heavy_battlecry02.wav", true);
	PrecacheSound("vo/pyro_battlecry01.wav", true);
	PrecacheSound("vo/taunts/spy_taunts10.wav", true);
	PrecacheSound("vo/engineer_battlecry06.wav", true);
	PrecacheSound("vo/medic_battlecry04.wav", true);
...
public Action:Command_Sprint(client, args)
{
	...
	if(GetConVarBool(enabled))
	{
		...
		if(timerExists[client] == false)
		{
			...
			{
				PrintToChat(client, "\x03You can not sprint during a taunt!");
			}
			else if (TF2_IsPlayerInCondition(client, TFCond_Zoomed))
			{
				PrintToChat(client, "\x03You can only sprint when you're not scoped!");
			}
			else if (TF2_IsPlayerInCondition(client, TFCond_Slowed))
			{
				PrintToChat(client, "\x03You can only sprint when you're not spun up!");
			}
			else if (TF2_IsPlayerInCondition(client, TFCond_Charging))
			{
				PrintToChat(client, "\x03You can only sprint when you're not charging!");
			}
			else
			{
...
public Action:Timer_Yell(Handle:hTimer, any:userid)
{
	...
	if (client <= 0 || client > MaxClients || !IsClientInGame(client)) return;
		...
		if(playerClass == TFClass_Scout)
			{
				EmitSoundToClient(client, "vo/scout_battlecry04.wav");
			}
			else if(playerClass == TFClass_Sniper)
			{
				EmitSoundToClient(client, "vo/sniper_battlecry03.wav");
			}
			else if(playerClass == TFClass_Soldier)
			{
				EmitSoundToClient(client, "vo/soldier_battlecry06.wav");
			}
			else if(playerClass == TFClass_DemoMan)
			{
				EmitSoundToClient(client, "vo/demoman_battlecry02.wav");
			}
			else if(playerClass == TFClass_Heavy)
			{
				EmitSoundToClient(client, "vo/heavy_battlecry02.wav");
			}
			else if(playerClass == TFClass_Pyro)
			{
				EmitSoundToClient(client, "vo/pyro_battlecry01.wav");
			}
			else if(playerClass == TFClass_Spy)
			{
				EmitSoundToClient(client, "vo/taunts/spy_taunts10.wav");
			}
			else if(playerClass == TFClass_Engineer)
			{
				EmitSoundToClient(client, "vo/engineer_battlecry06.wav");
			}
			else if(playerClass == TFClass_Medic)
			{
				EmitSoundToClient(client, "vo/medic_battlecry04.wav");
			}
		}
}
/*-----------------------------------------------/

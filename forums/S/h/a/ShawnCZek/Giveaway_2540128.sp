...
public void OnPluginStart()
{
	...
	AutoExecConfig_SetFile("Giveaway");
	...
	db = SQL_DefConnect(gBuffer, sizeof(gBuffer));
	if (db == null)
	{
		PrintToServer("%s Could not connect: %s", TAG, gBuffer);
	}
}
...
{
	if (SQL_FastQuery(db, QUERY_INIT_DB_GIVEAWAYS))
	{
		SQL_FastQuery(db, QUERY_INIT_DB_ENTRIES);
		ReplyToCommand(client, "%s Tables were successfully created", TAG);
	}
	else
	{
		SQLError(db, "creating database tables");
	}
...
public Action CMD_GiveawayAdd(int client, int args)
{
	...
	if (!SQL_FastQuery(db, gBuffer))
	{
		SQLError(db, "adding a giveaway");
	}
	else
	{
...
public Action CMD_GiveawayDraw(int client, int args)
{
	...
	{
		DBResultSet query = SQL_Query(db, QUERY_SELECT_ENDED_GIVEAWAY);
		if (query == null)
		{
			SQLError(db, "getting an ended giveaway");
		}
		else
		{
			if (query.RowCount == 0)
			...
			else
			{
				while (SQL_FetchRow(query))
				{
					...
					int endedGiveaway = SQL_FetchInt(query, 0);
					SQL_FetchString(query, 1, endedGiveawaySkin, sizeof(endedGiveawaySkin));
					
					Format(gBuffer, sizeof(gBuffer), QUERY_GET_RANDOM_WINNER, endedGiveaway);
					DBResultSet queryWinner = SQL_Query(db, gBuffer);
					if (queryWinner == null)
					{
						SQLError(db, "getting a random winner");
					}
					else
					{
...
public Action CMD_GiveawayWinner(int client, int args)
{
	...
	else
	{
		...
		DBResultSet query = SQL_Query(db, gBuffer);
		if (query == null)
		{
			SQLError(db, "getting a winner of the giveaway");
		}
		else
		{
			if (query.RowCount == 0)
			...
			else
			{
				...
				{
					char winner[50];
					SQL_FetchString(query, 0, winner, sizeof(winner));
					char item[50];
					SQL_FetchString(query, 1, item, sizeof(item));
					...
					DBResultSet queryInfo = SQL_Query(db, gBuffer);
					if (queryInfo == null)
					{
						SQLError(db, "getting additional info");
					}
					else
					{
	...
	else
	{
		...
		else
		{
			while (SQL_FetchRow(query))
			{
				...
				int entries = SQL_FetchInt(query, 1);
...
public Action CMD_GiveawayEnter(int client, int args)
{
	...
	else
	{
		...
		if (query == null)
		{
			SQL_GetError(db, gBuffer, sizeof(gBuffer));
...
{
	DBResultSet query = SQL_Query(db, QUERY_SELECT_GIVEAWAY);
	if (query == null)
	{
		SQLError(db, "getting a giveaway");
	}
	else
	{
		if (query.RowCount == 0)
		...
		else
		{
			while (SQL_FetchRow(query))
			{
				giveaway = SQL_FetchInt(query, 0);
				SQL_FetchString(query, 1, skin, sizeof(skin));
				SQL_FetchString(query, 2, gDescription, sizeof(gDescription));
				gType = SQL_FetchInt(query, 3);
			}
		}
	}
}
 ...
 * Changelog:
 * v1.3a
 *	o - Added Pitchfork and Shovel support.
 * v1.2a
 *	o - Added actual GolfClub support, fixed typo in translation for GolfClub. Tweaked melee weapons to be available on all maps in the spawner. Unquote related M60 lines for ammo reserve support to be restored or see; "https://forums.alliedmods.net/showthread.php?p=2694504#post2694504" or "https://forums.alliedmods.net/showthread.php?t=112156" or "https://forums.alliedmods.net/showthread.php?p=2467960" or "https://forums.alliedmods.net/showthread.php?p=2614626" for better M60 modifications/features.
 * v1.1a
...
public Plugin myinfo = 
{
	...
	description = "Give directly weapons to players / Spawn weapons or zombies where you are looking at.",
	version = VERSION,
...
public void OnPluginStart()
{
	...
	/* Max Ammo ConVars */
	AssaultMaxAmmo = CreateConVar("sm_spawnweapon_assaultammo", "360", "How much Ammo for AK74, M4A1, SG552 and Desert Rifle.", 0, true, 0.0, true, 360.0);
...
void BuildMeleeMenu(int client)
{
	...
	char frying_pan[40], katana[40], machete[40], tonfa[40], knife[40], golfclub[40], pitchfork[40], shovel[40], title[40];
	...
	Format(pitchfork, sizeof(pitchfork),"%T", "Pitchfork", LANG_SERVER);
	AddMenuItem(menu, "pitchfork", pitchfork);
	Format(shovel, sizeof(shovel),"%T", "Shovel", LANG_SERVER);
	AddMenuItem(menu, "shovel", shovel);
...
public int MenuHandler_SpawnWeapon(Menu menu, MenuAction action, int param1, int param2)
{
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			if(StrEqual(weapon, "weapon_laser_sight", false))
			{
				char position[64];
				int ent = CreateEntityByName("upgrade_spawn");
				DispatchKeyValue(ent, "count", "1");

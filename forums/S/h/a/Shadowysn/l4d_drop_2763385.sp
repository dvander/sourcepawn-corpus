...
#define PLUGIN_AUTHOR "Machine, dcx2, Electr000999 /z, Senip, Shao"
...
public void OnPluginStart()
{
	...
	BlockSecondaryDrop = CreateConVar(cmd_str, "1", "Prevent players from dropping their secondaries? (Fixes bugs that can come with incapped weapons or A-Posing.)", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	if (g_isSequel)
	{
		...
		BlockM60Drop = CreateConVar(cmd_str, "1", "Prevent players from dropping the M60? (Allows for better compatibility with certain plugins.)", FCVAR_NONE, true, 0.0, true, 1.0);
...
int DropBlocker(int client, int weapon)
{
	...
	// M60 check
	if (g_isSequel && g_bBlockM60Drop)
	{
		static char classname[32];
		GetEntityClassname(weapon, classname, sizeof(classname));
		if (StrEqual(classname, "weapon_rifle_m60", false)) return false;
	}

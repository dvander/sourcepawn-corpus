#define PLUGIN_NAME "[ANY] Trigger"
#define PLUGIN_AUTHOR "WhosAsking?, Shadowysn"
#define PLUGIN_DESC "Easily trigger any map entity without having to enable cheating"
#define PLUGIN_VERSION "1.2"
#define PLUGIN_URL "https://forums.alliedmods.net/showpost.php?p=2766544&postcount=5"
#define PLUGIN_NAME_SHORT "Trigger"
#define PLUGIN_NAME_TECH "trigger"
...
// LOCAL VARIABLES
// ==========================================================================================================
int MaxEntities;										// Placeholder for maximum possible entities.
int MaxDefault = 0;										// Placeholder for the endpoint of the defaults array.

static char DefaultClasses[][ARG_SIZE] =					// A list of entity classes for which defaults are available. 
...
// ==========================================================================================================
// ------------------------------------------------------------------
// FindEntityWildcard(Name, Start, UseClass)
...
// UseClass	- Use the classnames of entities instead of targetnames.
// RETURNS  - The first entity to match the name or wildcard if found,
//            INVALID_ENT_REFERENCE if none is found.
// ------------------------------------------------------------------
int FindEntityWildcard(const char[] Name, const int Start = -1, bool UseClass = false)
{
	// To hold the name taken from the entity being checked.
	static char TempName[ARG_SIZE];
	...
	for (int i = Start + 1; i < MaxEntities ; i++)
	{
		...
		// Get the entity's name or classname to compare.
		switch (UseClass)
		{
			case true:	GetEntityClassname(i, TempName, ARG_SIZE);
			case false:	GetEntPropString(i, Prop_Data, "m_iName", TempName, ARG_SIZE);
		}
...
// ------------------------------------------------------------------
int TriggerEntity(const int Client, const int Entity, char[] Input, const char[] Value, char[] ClassName)
{
	...
	if (!Input[0])
	{
		...
		if (ClassName[0])
		{
			...
			for (Index = 0; Index < MaxDefault; Index+= 2)
				...
				if (strcmp(ClassName, DefaultClasses[Index]) == 0)
					break;
...
// ------------------------------------------------------------------
Action Command_Trigger(int Client, int ArgCount)
{
	// String variables to hold the parameters.
	static char Input[ARG_SIZE], Value[ARG_SIZE], Name[ARG_SIZE], ClassName[ARG_SIZE];
	Input[0] = '\0'; Value[0] = '\0'; Name[0] = '\0'; ClassName[0] = '\0';
	...
	if (strcmp(Name, "@aim", false) == 0 && Client < 1)
	{
	...
	int Entity = FindEntityWildcard(Name);
	
	// If there's no name match, use CLASSname matches.
	bool isClassMatch = false;
	if (!IsValidEntity(Entity))
	{
		Entity = FindEntityWildcard(Name, -1, true);
		isClassMatch = true;
	}

	// Couldn't find name matches or classname matches.
	...
	static char TempInput[ARG_SIZE];
	...
	while (IsValidEntity(Entity))
	{
		...
		Entity = FindEntityWildcard(Name, Entity, isClassMatch);
	}
...
// RETURNS  - Plugin_Handled
// ------------------------------------------------------------------
Action Command_Find(int Client, int ArgCount)
{
	static char Arg[ARG_SIZE], Name[ARG_SIZE], ClassName[ARG_SIZE];
	Arg[0] = '\0'; Name[0] = '\0'; ClassName[0] = '\0';
	...
	int Entity = FindEntityWildcard(Arg);
	bool isClassMatch = false;
	// Use classname matches if no name match is found.
	if (!IsValidEntity(Entity))
	{
		Entity = FindEntityWildcard(Arg, -1, true);
		isClassMatch = true;
	}
	...
	while (IsValidEntity(Entity))
	{
		...
		// Now, go find the next one.
		Entity = FindEntityWildcard(Arg, Entity, isClassMatch);
	}
...
// RETURNS  - Plugin_Handled
// ------------------------------------------------------------------
Action Command_Aim(int Client, int ArgCount)
{

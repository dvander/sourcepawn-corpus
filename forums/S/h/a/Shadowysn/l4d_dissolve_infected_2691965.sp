...
ConVar g_hCvarAllow, g_hCvarChance, g_hCvarInfected, g_hCvarMPGameMode, g_hCvarModes, g_hCvarModesOff, g_hCvarModesTog, g_hCvarTime, g_hCvarTimeMin, g_hCvarTimeMax, g_hCvarSurvivor;
...
public void OnPluginStart()
{
	...
	g_hCvarSurvivor = CreateConVar(		"l4d_dissolve_survivor",		"0",			"Toggle dissolving survivors on death. Will render defibs useless. 0=Off, 1=On.", CVAR_FLAGS );
			...
			if( GetClientTeam(target) == 2 )
			{
				...
				if( index != -1 )
				{
					...
					if( clone > 0)
					{
						...
						DissolveTarget(index, clone, target, target);
						return Plugin_Handled;
					}
				}
			}
	...
	if( g_bCanDiss )
	{
		...
		if( !target )
		{
			...
			if( target > 0 && target <= MaxClients && IsClientInGame(target) )
			{
				...
				else if (GetConVarBool(g_hCvarSurvivor) && (GetClientTeam(target) == 2 || GetClientTeam(target) == 4))
				{
					int index = GetDissolveIndex();
					if( index != -1 )
					{
...
void DissolveTarget(int index, int target, int original = 0, int survivor = -1)
{
	if (IsValidClient(survivor))
	{ CreateRagdoll(survivor); }
...
{
	if(!GetConVarBool(g_hCvarSurvivor)) return;
	
	if(classname[0] != 's' ) return;

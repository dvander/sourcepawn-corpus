#define PLUGIN_NAME "[TF2] Give Bots More Weapons"
#define PLUGIN_AUTHOR "PC Gamer, with code by luki1412 and manicogaming, edited by That Annoying Guide and Shadowysn"
#define PLUGIN_DESC "Gives TF2 bots more non-stock weapons"
#define PLUGIN_VERSION "1.11"
#define PLUGIN_URL "https://forums.alliedmods.net/showthread.php?t=329264"
#define PLUGIN_NAME_SHORT "Give Bots More Weapons"
#define PLUGIN_NAME_TECH "gbmw"
...
#define AUTOEXEC_CFG "GiveBotsMoreWeapons"

bool g_bMVM;
bool g_bMedieval;
bool g_bLateLoad;
static ConVar g_hCVTimer, g_hCVEnabled, g_hCVTeam;
float g_fCVTimer; bool g_bCVEnabled; int g_iCVTeam;
...
public void OnPluginStart()
{
	...
	Format(cmd_str, sizeof(cmd_str), "sm_%s_enabled", PLUGIN_NAME_TECH);
	g_hCVEnabled = CreateConVar(cmd_str, "1", "Enables/disables this plugin", FCVAR_NONE, true, 0.0, true, 1.0);
	g_hCVEnabled.AddChangeHook(CC_GBMW_Enabled);
	
	Format(cmd_str, sizeof(cmd_str), "sm_%s_delay", PLUGIN_NAME_TECH);
	g_hCVTimer = CreateConVar(cmd_str, "0.2", "Delay for giving weapons to bots", FCVAR_NONE, true, 0.1, true, 30.0);
	g_hCVTimer.AddChangeHook(CC_GBMW_Timer);
	
	Format(cmd_str, sizeof(cmd_str), "sm_%s_team", PLUGIN_NAME_TECH);
	g_hCVTeam = CreateConVar(cmd_str, "1", "Team to give weapons to: 1-both, 2-red, 3-blu", FCVAR_NONE, true, 1.0, true, 3.0);
	g_hCVTeam.AddChangeHook(CC_GBMW_Team);
...
void CC_GBMW_Enabled(ConVar convar, const char[] oldValue, const char[] newValue)
{
	g_bCVEnabled =		convar.BoolValue;
	if (convar.BoolValue)
		HookEvent("post_inventory_application", player_inv);
	else
		UnhookEvent("post_inventory_application", player_inv);
}
void CC_GBMW_Timer(ConVar convar, const char[] oldValue, const char[] newValue)	{ g_fCVTimer =		convar.FloatValue;	}
void CC_GBMW_Team(ConVar convar, const char[] oldValue, const char[] newValue)		{ g_iCVTeam =		convar.IntValue;		}
void SetCvarValues()
{
	CC_GBMW_Enabled(g_hCVEnabled, "", "");
	CC_GBMW_Timer(g_hCVTimer, "", "");
	CC_GBMW_Team(g_hCVTeam, "", "");
}
...
float weaponDropTime = 0.0;
public void OnEntityCreated(int entity, const char[] classname)
{
	if (!g_bCVEnabled || g_bMVM || classname[0] != 't' || strcmp(classname, "tf_dropped_weapon", false) != 0)
		return;
	
	weaponDropTime = GetGameTime();
...
void player_inv(Event event, const char[] name, bool dontBroadcast) 
{
	if (!g_bCVEnabled || g_bMVM)
		return;
	
	int userid = event.GetInt("userid", 0);
	int client = GetClientOfUserId(userid);

	if (!IsPlayerHere(client)) return;
	
	int team = GetClientTeam(client);
	int team2 = g_iCVTeam;
	float timer = g_fCVTimer;
	...
	if (weaponDropTime == GetGameTime())
	{
		...
		while ((ent = FindEntityByClassname(ent, "tf_dropped_weapon")) != -1)
		{
			...
			//PrintToServer("tf_dropped_weapon. index-%i", ent);
			
			AcceptEntityInput(ent, "Kill");
		}
	}
	
	CreateTimer(timer, Timer_GiveWeapons, userid, TIMER_FLAG_NO_MAPCHANGE);
}

Action Timer_GiveWeapons(Handle timer, int data)
{
	int client = GetClientOfUserId(data);
	
	if (!g_bCVEnabled || !IsPlayerHere(client))
...
Action TimerHealth(Handle timer, int client)
{
...
stock void TF2_SwitchtoSlot(int client, int slot)
{
	if (slot >= 0 && slot <= 5 && IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		if (wep > MaxClients && IsValidEdict(wep) && GetEntityClassname(wep, wepclassname, sizeof(wepclassname)))
		{

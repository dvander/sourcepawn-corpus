...
static g_iSColors[13]             = {1,            2,        3,          4,            5,               6,         7,             3,           3,           1,          3,              3,           3};
...
static String:g_sSColors[13][13]  = {"{DEFAULT}",  "{RED}",  "{TEAM}",  "{GREEN}",    "{LIGHTGREEN}",  "{LIME}",  "{LIGHTRED}",  "{TCOLOR}",  "{CTCOLOR}",  "{WHITE}",  "{TEAMCOLOR}",  "{ORANGE}",  "{BLUE}"};
...
public Action:Timer_DisplayAds(Handle:timer) {
	if (GetConVarBool(g_hEnabled)) {
		...
		if (StrContains(sType, "S") != -1) {
			new iTeamColors = StrContains(sText, "{TEAM}", false), String:sColor[4];
			new tplayer = FindPlayerFromTeam(2), ctplayer = FindPlayerFromTeam(3);
			if(iTeamColors == -1)
				iTeamColors = StrContains(sText, "{TEAMCOLOR}", false);
			
			if((StrContains(sText, "{CTCOLOR}", false) != -1 || StrContains(sText, "{BLUE}", false) != -1) && ctplayer != -1)
			{
				if(iTeamColors != -1)
					SetFailState("Error: You may not have two different team colors in the same message.");
				else
					iTeamColors = -2;
			}
			
			if((StrContains(sText, "{TCOLOR}", false) != -1 || StrContains(sText, "{ORANGE}", false)!= -1) && tplayer != -1)
			{
				if(iTeamColors == -2 || iTeamColors > -1)
					SetFailState("Error: You may not have two different team colors in the same message.");
				else
					iTeamColors = -3;
			}
			...
			for (new c = 0; c < sizeof(g_iSColors); c++) {
				if(StrContains(sText, g_sSColors[c], false)) {
					Format(sColor, sizeof(sColor), "%c", g_iSColors[c]);
					ReplaceString(sText, sizeof(sText), g_sSColors[c], sColor, false);
				}
			}
					...
					if (IsClientInGame(i) && !IsFakeClient(i) &&
							((!bAdmins && !(bFlags && HasFlag(i, fFlagList))) ||
							 bAdmins && (GetUserFlagBits(i) & ADMFLAG_GENERIC ||
													 GetUserFlagBits(i) & ADMFLAG_ROOT))) {
						if(iTeamColors == -3)
							SayText2(i, tplayer, sText);
						else if(iTeamColors == -2)
							SayText2(i, ctplayer, sText);
						else
							SayText2(i, i, sText);
					}
				}
...
FindPlayerFromTeam(from)
{
	for(new i = 1; i <= MaxClients; i++)
		if(IsClientInGame(i) && GetClientTeam(i) == from)
			return i;
	return -1;
}
...
SayText2(to, author, const String:message[]) {
	new Handle:hBf = StartMessageOne("SayText2", to);
	new String:buffer[256] = "\x01\x0B";
	StrCat(buffer, 256, message);
	if (hBf != INVALID_HANDLE) {
		if (GetUserMessageType() == UM_Protobuf)
		{
			PbSetBool(hBf, "chat", true);
			PbSetInt(hBf, "ent_idx", author);
			PbSetString(hBf, "msg_name", buffer);

	...
	if (IsEnabled) {
		for (new i = 1; i <= maxclients; i++) {
			if (IsPlayerAdmin[i] && IsPlayerImmune[i]) {
				...
				TF2_RemoveCondition(i,TFCond_RegenBuffed);
	...
	if (IsEnabled) {
		for (new i = 1; i <= maxclients; i++) {
			if (IsPlayerAdmin[i]) {
				...
				TF2_AddCondition(i,TFCond_MarkedForDeath,15.0);
				TF2_AddCondition(i,TFCond_TeleportedGlow,15.0);
				TF2_AddCondition(i,TFCond_UberchargeFading,15.0);
				TF2_AddCondition(i,TFCond_Kritzkrieged,15.0);
				TF2_AddCondition(i,TFCond_RegenBuffed,15.0);
...
public HookPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsEnabled && IsPlayerAdmin[client] && RoundEnd) {
		...
		TF2_AddCondition(client,TFCond_MarkedForDeath,15.0);
		TF2_AddCondition(client,TFCond_TeleportedGlow,15.0);
		TF2_AddCondition(client,TFCond_UberchargeFading,15.0);
		TF2_AddCondition(client,TFCond_Kritzkrieged,15.0);
		TF2_AddCondition(client,TFCond_RegenBuffed,15.0);

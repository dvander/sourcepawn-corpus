...
public OnClientPostAdminCheck(client)
{
    ...
    decl String:buffer[2048], String:name[128], String:authid[64], String:ip[16];
    ...
    for (new target = 1; target <= MaxClients; target++)
    {
        ...
        len = buffer[len];
        len += Format(buffer[len], sizeof(buffer)-len, "*** Country: %s", g_ClientCountry[client]);
...
public Action:Command_GeoInfo(client, args)
{
    ...
    decl String:buffer[2048], String:name[64], String:authid[64], String:ip[16];
    new len;
    ...
    for (new i = 0; i < target_count; i++)
    {
        ...
        PrintToChat(target, buffer);
        len = buffer[len];        
        len += Format(buffer[len], sizeof(buffer)-len, "*** Country: %s", g_ClientCountry[target]);

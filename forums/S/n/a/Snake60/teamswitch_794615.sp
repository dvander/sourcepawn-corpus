...
public Plugin:myinfo = {
	name = "TeamSwitch",
	author = "MistaGee (Modify by Snake 60)",
...
public Action:Command_SwitchImmed( client, args ){
	...
	if(colour) { PrintToChatAll( "\x01[SM] \x01%t \x03%s \x01%t", "ts admin switch", target_name, "ts opposite team" );
	} else { PrintToChatAll( "[SM] %t %s %t", "ts admin switch", target_name, "ts opposite team" );}
	return Plugin_Handled;
	}
...
public Action:Command_SwitchSpec( client, args ){
	...
	if(colour) {
	PrintToChatAll( "\x01[SM] \x01%t \x03%s \x01%t", "ts admin switch", target_name, "ts to spectators" );} else {
...
void:PerformSwitch( client, bool:toSpec = false ){
	...
	if(cTeam == TEAM_2) {
	SetEntityModel(client,"models/player/t_leet.mdl");} else {SetEntityModel(client,"models/player/ct_sas.mdl");}
	...
	if(colour) {PrintToChatAll( "\x01[SM] \x03%s \x01%t", plName, "ts switch by admin" );}
	else {PrintToChatAll( "[SM] %s %t", plName, "ts switch by admin" );}
}

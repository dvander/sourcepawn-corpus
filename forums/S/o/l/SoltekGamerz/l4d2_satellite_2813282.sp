...
public OnPluginStart()
{
	...
	sm_satellite_limit_01		= CreateConVar("sm_satellite_limit_01","10", "Frequency limitation in one round (Mode:JUDGEMENT)", FCVAR_NOTIFY);
	sm_satellite_limit_02		= CreateConVar("sm_satellite_limit_02","15", "Frequency limitation in one round (Mode:BLIZZARD)", FCVAR_NOTIFY);
...
public Action:Event_Weapon_Fire(Handle:event, const String:name[], bool:dontBroadcast)
{
		...
		GetConVarInt(sm_satellite_enable) && operation[attacker] > 0)
	{
		...
		if(energy[attacker][operation[attacker]] == 5)
		{
...
public Action:SatelliteTimer(Handle:timer, any:client)
{
	...
	if(energy[client][operation[client]] == 5)
	{

...
/* Message */
#define MESSAGE_EMPTY1	"ENERGIA AGOTADA"
#define MESSAGE_EMPTY2	"LA ENERGIA SE ESTA AGOTANDO"
#define MESSAGE_SHIFT1	"HAZ CAMBIADO AL MODO \nEXPLOSIVO"
#define MESSAGE_SHIFT2	"HAZ CAMBIADO AL MODO \nBAJO CERO"
#define MESSAGE_SHIFT3	"HAZ CAMBIADO AL MODO \nINFIERNO"
#define MESSAGE_SHIFT4	"HAZ CAMBIADO AL MODO \nNORMAL"
...
#define PARTICLE_BOMB01	"weapon_pipebomb"
#define PARTICLE_BOMB02	"weapon_pipebomb_child_explosion"
...
public OnPluginStart()
{
	...
	sm_satellite_limit_01		= CreateConVar("sm_satellite_limit_01","40", "Frequency limitation in one round (Mode:JUDGEMENT)", FCVAR_NOTIFY);
	sm_satellite_limit_02		= CreateConVar("sm_satellite_limit_02","50", "Frequency limitation in one round (Mode:BLIZZARD)", FCVAR_NOTIFY);
	sm_satellite_limit_03		= CreateConVar("sm_satellite_limit_03","40", "Frequency limitation in one round (Mode:INFERNO)", FCVAR_NOTIFY);
...
public OnMapStart()
{
	...
	PrecacheParticle(PARTICLE_BOMB01);
	PrecacheParticle(PARTICLE_BOMB02);
...
public ChangeMode(client)
{
	...
	Format(mStrJud, sizeof(mStrJud), "MODO: EXPLOSIVO      (Energia: %d)", energy[client][MODE_JUDGEMENT]);
	Format(mStrBli, sizeof(mStrBli), "MODO: BAJO-CERO     (Energia: %d)", energy[client][MODE_BLIZZARD]);
	Format(mStrInf, sizeof(mStrInf), "MODO: INFIERNO         (Energia: %d)", energy[client][MODE_INFERNO]);
	
	new Handle:menu = CreateMenu(ChangeModeMenu);
	SetMenuTitle(menu, "*** Operacion: Sistema Satelital ***\n_______________________________________\n");
...
public Judgement(client)
{
	...
	ShowParticle(trsPos[client], PARTICLE_BOMB01, 3.0);
	ShowParticle(trsPos[client], PARTICLE_BOMB02, 3.0);
...
public Action:SatelliteTimer(Handle:timer, any:client)
{
	...
	{
		Judgement(client);
		if(raycount[client] == 0 && Bulletblock[client] == 0 && GetConVarInt(sm_satellite_adminonly) != 2)
...
public LittleFlower(client, type)
{
	...
	if (IsValidEntity(entity))
	{
		...
		SetEntData(entity, GetEntSendPropOffs(entity, "m_CollisionGroup"), 1, 1, false);

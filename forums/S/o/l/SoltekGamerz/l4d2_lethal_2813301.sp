...
#define PLUGIN_VERSION "2.1.15"

/*
	Fork by SoltekGamerz
	
	2.1.15
	- Fixed cvar l4d2_lw_lethaldamage, now you can choose the desired damage correctly.
	
	2.1.14
	- Fixed friendly fire, now you can kill the infected without hurting the survivors
...
bool UseAllowed(int client)
{
	...
	//if (GetUserAdmin(client) == INVALID_ADMIN_ID && sm_satellite_adminonly.IntValue)
	//	return false;
...
public void OnPluginStart()
{
	...
	l4d2_lw_lethaldamage	= CreateConVar("l4d2_lw_lethaldamage","500.0", "Lethal Weapon base damage", CVAR_FLAGS);
	...
	l4d2_lw_awp				= CreateConVar("l4d2_lw_awp","0", "Enable Lethal Weapon for AWP", CVAR_FLAGS);
	l4d2_lw_huntingrifle	= CreateConVar("l4d2_lw_huntingrifle","0", "Enable Lethal Weapon for Hunting Rifle", CVAR_FLAGS);
	l4d2_lw_g3sg1			= CreateConVar("l4d2_lw_g3sg1","0", "Enable Lethal Weapon for G3SG1", CVAR_FLAGS);
...
public Action ReleaseTimer(Handle timer, any client)
{
	...
	{
		int iAmmoLeft = GetEntData(client, g_bLeft4Dead2 ? g_iAmmoOffset+36 : g_iAmmoOffset+8);
		int iAmmoSpend = GetConVarInt(l4d2_lw_useammocount);
		
		iAmmoLeft -= iAmmoSpend;
		if (iAmmoLeft < 0) iAmmoLeft = 0;
		
		SetEntData(client, g_bLeft4Dead2 ? g_iAmmoOffset+36 : g_iAmmoOffset+8, iAmmoLeft);

		if ((g_iCurWeapon[client]) != 0)
		{
			if (IsValidEntity(g_iCurWeapon[client]))
			{
				SetEntProp(g_iCurWeapon[client], Prop_Send, "m_iClip1", 0); 
			}
		}
	}
...
public void ExplodeMain(float pos[3])
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		{
			DamageEffect(i, GetConVarFloat(l4d2_lw_lethaldamage));
...
//		CreateTimer(time, DeleteParticles, particle);
		FormatEx(sBuffer, sizeof(sBuffer), "OnUser1 !self:Kill::%f:-1", time);

...
new Handle:g_frames = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_frames = CreateConVar("sm_parachute_frames","0");
...
public OnGameFrame()
{
	...
	if(GetConVarInt(g_frames) > 0)
	{
		if(frame < GetConVarInt(g_frames))
		{
			frame++;
			PrintToChatAll("Frame %i - Not Proccessed", frame);
			return;
		}
		else
		{
			PrintToChatAll("Frames %i - Processed", GetConVarInt(g_frames));
			frame = 1;
		}
	}

...
#define GAMEVOTING_VERSION "1.5d"
...
new gvVotes[MAXPLAYERS+1][gvstore];		// data for each player
new gvUseridGvID[MAXPLAYERS+1]; 				// optimize find of GvID, dont need to parse array with steamids
new bool:gsourcebans, bool:glogs, bool:gaclogs, bool:adplugin, bool:adminonserver, bool:enableplugin, bool:noimmunity;
#if defined _morecolors_included
new bool:morecolors;
#endif
...
public OnPluginStart()
{
	...
	if(!FileExists(gvactions))
	{
		new Handle:File = OpenFile(gvactions, "w");
		WriteFileLine(File, "");
		CloseHandle(File);
	}
	...
	if(!FileExists(gvlog))
	{
		new Handle:File = OpenFile(gvlog, "w");
		WriteFileLine(File, "");
		CloseHandle(File);
	}
...
public VoteInit(client, type)
{
	...
	if(gvVotes[gvUseridGvID[client]][antiflood] > GetTime())
	{
		...
		#else
		PrintToChat(client, "%T", "GameVoting_antiflood", client, clogprefix, waits);
		#endif
		return;
	}
	...
	if(countplayers() < needplayers)
	{
		...
		#else
		PrintToChat(client, "%T", "GameVoting_needplayers", client, clogprefix, needplayers);
		#endif
		return;
	}
	...
	if(adplugin && adminonserver)
	{
		...
		#else
		PrintToChat(client, "%T", "GameVoting_admin_on_server", client, clogprefix);
		#endif
		return;
	}
	...
	switch(type)
	{
		case VOTEKICK_TYPE:
		{
			if(GetConVarInt(g_kickenable) < 1)
			{
				...
				#else
				PrintToChat(client, "%T", "GameVoting_kick_disabled", client, clogprefix);
				#endif
				return;
			}
		}
		case VOTEBAN_TYPE:
		{
			if(GetConVarInt(g_banenable) < 1)
			{
				...
				#else
				PrintToChat(client, "%T", "GameVoting_ban_disabled", client, clogprefix);
				#endif
				return;
			}
		}
		case VOTEMUTE_TYPE:
		{
			if(GetConVarInt(g_muteenable) < 1)
			{
				...
				#else
				PrintToChat(client, "%T", "GameVoting_mute_disabled", client, clogprefix);
				#endif
				return;
			}
		}
		case VOTESILENCE_TYPE:
		{
			if(GetConVarInt(g_silenceenable) < 1)
			{
				...
				#else
				PrintToChat(client, "%T", "GameVoting_silence_disabled", client, clogprefix);
				#endif
				return;
			}
		}
...
public prepareVote(client, String:clientID[], type)
{
	...
	if(indicteec == -1) 
	{
		...
		#else
		PrintToChat(client, "%T", "GameVoting_reseted",client,clogprefix);
		#endif
	}
		...
		switch(type)
		{
			case VOTEKICK_TYPE:
			{
				...
				for(new x = 1; x <= MaxClients; x++) if(ValidClient(x)) {
					...
					#else
					PrintToChat(x, "%T", "GameVoting_voted_for_kick",x,clogprefix,votername,indicteecname,gvVotes[indicteeID][votekick],gvneed);
					#endif
				}
			...
			case VOTEBAN_TYPE:
			{
				...
				for(new x = 1; x <= MaxClients; x++) if(ValidClient(x)) {
					...
					#else
					PrintToChat(x, "%T", "GameVoting_voted_for_ban",x,clogprefix,votername,indicteecname,gvVotes[indicteeID][voteban],gvneed);
					#endif
				}
			...
			case VOTESILENCE_TYPE:
			{
				...
				if(gvVotes[indicteeID][silence_timestamp] < GetTime())
				{
					for(new x = 1; x <= MaxClients; x++) if(ValidClient(x)) {
						...
						#else
						PrintToChat(x, "%T", "GameVoting_voted_for_silence",x,clogprefix,votername,indicteecname,gvVotes[indicteeID][votesilence],gvneed);
						#endif
					}
					if(gvVotes[indicteeID][votesilence] >= gvneed) 
					{
						...
						for(new x = 1; x <= MaxClients; x++) if(ValidClient(x)) {
							...
							#else
							PrintToChat(x, "%T", "GameVoting_was_silenced",x,clogprefix,indicteecname);
							#endif
						}
			...
			case VOTEMUTE_TYPE:
			{
				...
				if(gvVotes[indicteeID][mute_timestamp] < GetTime())
				{
					...
					for(new x = 1; x <= MaxClients; x++) if(ValidClient(x)) {
						...
						#else
						PrintToChat(x, "%T", "GameVoting_voted_for_mute",x,clogprefix,votername,indicteecname,gvVotes[indicteeID][votemute],gvneed);
						#endif
					}
					if(gvVotes[indicteeID][votemute] >= gvneed) 
					{
						...
						for(new x = 1; x <= MaxClients; x++) if(ValidClient(x)) {
							...
							#else
							PrintToChat(x, "%T", "GameVoting_was_muted",x,clogprefix,indicteecname);
							#endif
						}
...
public gvBanPlayer(clientGvID, clientUserID, String:ClientName[])
{
	...
	for(new x = 1; x <= MaxClients; x++) if(ValidClient(x)) {
		...
		#else
		PrintToChat(x, "%T", "GameVoting_was_banned",x,clogprefix,ClientName);
		#endif
	}
...
public gvKickPlayer(clientGvID, clientUserID, String:ClientName[])
{
	...
	for(new x = 1; x <= MaxClients; x++) if(ValidClient(x)) {
		...
		#else
		PrintToChat(x, "%T", "GameVoting_was_kicked",x,clogprefix,ClientName);
		#endif
	}

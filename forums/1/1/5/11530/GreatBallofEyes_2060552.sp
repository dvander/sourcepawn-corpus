...
new Handle:v_MaximumCount = INVALID_HANDLE;
...
{
	CreateConVar("sm_monospawn_version", PLUGIN_VERSION, "Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	v_MaximumCount = CreateConVar("sm_monospawn_maxcount", "0", "Maximum number of Monoculi at any time. Zero disables.", FCVAR_PLUGIN, true, 0.0);
...
{
	new iMax = GetConVarInt(v_MaximumCount);

	if (iMax > 0 && g_iCount > iMax - 1)
	{
		PrintToChat(client, "[SM] Only %d Monocul%s allowed.", iMax, (iMax == 1?"us":"i"));
		return Plugin_Handled;
	}
	...
	if (IsValidEntity(entity))
	{
		...
		SetEntProp(entity, Prop_Data, "m_iTeamNum", 5); //Required since Halloween 2013.

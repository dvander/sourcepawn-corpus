 ...
 * vim: set ts=4 :
 * =============================================================================
 * SourceMod NextmapEx Plugin - Edited by 11530 to hook late mapcyclefile changes
 * Adds sm_nextmap cvar for changing map and nextmap chat trigger.
 *
 * SourceMod (C)2004-2012 AlliedModders LLC.  All rights reserved.
 * =============================================================================
 *
...
public Plugin:myinfo = 
{
	...
	description = "Provides nextmap and sm_maphistory",
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	{
		strcopy(error, err_max, "NextmapEx is incompatible with this game");
...
public OnPluginStart()
{
	...
	if (g_Cvar_Mapcyclefile != INVALID_HANDLE)
	{
		HookConVarChange(g_Cvar_Mapcyclefile, MapCycleFileChanged);
	}
...
public OnConfigsExecuted()
{
	...
	BuildPath(Path_SM, filename, sizeof(filename), "plugins/nextmap.smx");
	if(FileExists(filename))
	{
		decl String:newfilename[200];
		BuildPath(Path_SM, newfilename, sizeof(newfilename), "plugins/disabled/nextmap.smx");
		ServerCommand("sm plugins unload nextmap");
		if(FileExists(newfilename))
			DeleteFile(newfilename);
		RenameFile(newfilename, filename);
		LogMessage("plugins/nextmap.smx was unloaded and moved to plugins/disabled/nextmap.smx");
	}
...
public MapCycleFileChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{

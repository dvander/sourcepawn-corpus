...
public OnPluginStart()
{
	...
	v_Enable = CreateConVar("sm_godmode_enabled", "1", "Enable/Disable the plugin", 0, true, 0.0, true, 1.0);
...
public PlayerSpawned(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GetConVarBool(v_Enable))
	{
		...
		else
		{
			...
			if (GetConVarBool(v_SpawnAdminOnly) && !CheckCommandAccess(client, "godmode_adminonly_override", ADMFLAG_SLAY))

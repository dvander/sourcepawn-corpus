...
public Plugin:myinfo = 
{
  ...
  author = "Bottiger, Brainstorm, 11530",
...
{
  CreateConVar("sm_redirect_version", PLUGIN_VERSION, "Version number of server redirect plugin.", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
  g_varServerId = CreateConVar("sm_redirect_serverid", "0", "sm_redirect_serverid - ID of the server in the database.", FCVAR_PLUGIN, true, 0.0, false);
  g_varShowCurrent = CreateConVar("sm_redirect_showcurrent", "1", "sm_redirect_showcurrent - Whether to show the current server in the redirect menu", FCVAR_PLUGIN);
  g_varShowOffline = CreateConVar("sm_redirect_showoffline", "1", "sm_redirect_showoffline - Whether to show offline servers in the redirect menu", FCVAR_PLUGIN);
  g_varAllowBots = CreateConVar("sm_redirect_allowbots", "0", "sm_redirect_allowbots - Whether to count bots in the list", FCVAR_PLUGIN);
  g_varMenuSortMode = CreateConVar("sm_redirect_menusort", "1", "sm_redirect_menusort - Indicates how menu items get sorted. 1 = by display name (default), 2 = server ID, 3 = number of players (ascending), 4 = number of players (descending)", FCVAR_PLUGIN);  
  g_varHeartbeatEnabled = CreateConVar("sm_redirect_enableheartbeat", "1", "sm_redirect_enableheartbeat - Whether to enable heartbeat signal for this server. If stopped, the server will be marked as offline.", FCVAR_PLUGIN);
...
public Action:Command_Redirect(client, args)
{
  ...
  SQL_TConnect(ShowServerMenu2, DATABASE_KEY, GetClientUserId(client));
  return Plugin_Handled;
}

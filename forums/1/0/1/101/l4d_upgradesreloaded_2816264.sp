...
new Handle:UpgradeEnabled[MAX_UPGRADES];
new Handle:AwardIndex[AWARDID + 1];
new Handle:MorphogenicTimer[MAXPLAYERS + 1];
new Handle:RegenerationTimer[MAXPLAYERS + 1];
new Handle:AwardsCooldownTimer[MAXPLAYERS + 1];
new Handle:CasingDispenserTimer[MAXPLAYERS + 1];
new Handle:SetClientUpgrades[MAXPLAYERS + 1];
new Handle:g_PerkMode;
new Handle:PerkSlots;
new Handle:PerkBonusSlots;
...
new iPerkBonusSlots[MAXPLAYERS + 1];

new bool:b_round_end;
new bool:IsDatabaseLoaded = false;
new RefreshRewards[MAXPLAYERS + 1];
new iAnnounceText[MAXPLAYERS + 1];
new iSaveFeature[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	CreateConVar("sm_upgradesreloaded_version", PLUGIN_VERSION, "Survivor Upgrades Reloaded Version",CVAR_FLAGS);
...
stock SaveData(client)
{
	...
	{
		char TQuery[128],SteamID[32],UpgradeBinary[64],DisabledBinary[64];
...
{
	if(hndl == INVALID_HANDLE || SQL_GetRowCount(hndl) == 0 || !SQL_FetchRow(hndl) || (IsClientInGame(client) && GetClientTeam(client) == 3))
	{
		return;
	}	
...
public Action:OnWeaponCanUse(client, weapon)
{
	...
	if(iUpgrade[client][40] > 0 && GetPlayerWeaponSlot(client, 0) > 0)
	{
		...
		{
			GiveSecondary(client, weapon, !PlayerSlotEmpty(client) );
			return Plugin_Handled;
		}	
	}
	return Plugin_Continue;
...
public bool:PlayerSlotEmpty(client)
{
	...
	new iTotal = 0;
	new iWeapon;
	new m_hMyWeapons = GetEntPropArraySize(client,Prop_Send, "m_hMyWeapons");
	for(new i = 0; i < m_hMyWeapons; i++) 
	{
...
GivePlayerAmmoX(client)
{
	...
	if(iWEAPON > 0)
	{
		...
		if(iUpgrade[client][7] > 0)
		{
			...
			{
				SetEntData(client, g_iOffsetAmmo+HUNTING_RIFLE_OFFSET_AMMO, RoundToNearest((GetConVarInt(FindConVar("ammo_huntingrifle_max")) * 1.5)) + (CheckWeaponUpgradeLimit(iWEAPON, client) - GetEntProp(iWEAPON, Prop_Send, "m_iClip1")));
				SetEntData(client, g_iOffsetAmmo+RIFLE_OFFSET_AMMO, RoundToNearest((GetConVarInt(FindConVar("ammo_assaultrifle_max")) * 1.5)) + (CheckWeaponUpgradeLimit(iWEAPON, client) - GetEntProp(iWEAPON, Prop_Send, "m_iClip1")));
				SetEntData(client, g_iOffsetAmmo+SMG_OFFSET_AMMO, RoundToNearest((GetConVarInt(FindConVar("ammo_smg_max")) * 1.5)) + (CheckWeaponUpgradeLimit(iWEAPON, client) - GetEntProp(iWEAPON, Prop_Send, "m_iClip1")));
				SetEntData(client, g_iOffsetAmmo+SHOTGUN_OFFSET_AMMO, RoundToNearest((GetConVarInt(FindConVar("ammo_buckshot_max")) * 1.5)) + (CheckWeaponUpgradeLimit(iWEAPON, client) - GetEntProp(iWEAPON, Prop_Send, "m_iClip1")));
			}
		}
		else
		{
			SetEntData(client, g_iOffsetAmmo+HUNTING_RIFLE_OFFSET_AMMO, GetConVarInt(FindConVar("ammo_huntingrifle_max")) + (CheckWeaponUpgradeLimit(iWEAPON, client) - GetEntProp(iWEAPON, Prop_Send, "m_iClip1")));
			SetEntData(client, g_iOffsetAmmo+RIFLE_OFFSET_AMMO, GetConVarInt(FindConVar("ammo_assaultrifle_max")) + (CheckWeaponUpgradeLimit(iWEAPON, client) - GetEntProp(iWEAPON, Prop_Send, "m_iClip1")));
			SetEntData(client, g_iOffsetAmmo+SMG_OFFSET_AMMO, GetConVarInt(FindConVar("ammo_smg_max")) + (CheckWeaponUpgradeLimit(iWEAPON, client) - GetEntProp(iWEAPON, Prop_Send, "m_iClip1")));
			SetEntData(client, g_iOffsetAmmo+SHOTGUN_OFFSET_AMMO, GetConVarInt(FindConVar("ammo_buckshot_max")) + (CheckWeaponUpgradeLimit(iWEAPON, client) - GetEntProp(iWEAPON, Prop_Send, "m_iClip1")));
		}
	}
}
...
ClearPlayerAmmo(client)
{
	SetEntData(client, g_iOffsetAmmo+HUNTING_RIFLE_OFFSET_AMMO, 0);
	SetEntData(client, g_iOffsetAmmo+RIFLE_OFFSET_AMMO, 0);
	SetEntData(client, g_iOffsetAmmo+SMG_OFFSET_AMMO, 0);
	SetEntData(client, g_iOffsetAmmo+SHOTGUN_OFFSET_AMMO, 0);
}
...
public GiveSurvivorUpgrade(client, amount, awardid)
{
	...
	for(new num = 0; num < amount; num++)
	{
		...
		if(numOwned == GetEnabledUpgrades() || numOwned == GetAvailableUpgrades(client))
		{
			for(new i = 1; i <= MaxClients; i++)
			{
				if(IsClientInGame(i) && iAnnounceText[i] == 1)
					PrintToChat(i, "\x04%N\x01 has earned the %s.", client, AwardTitle[awardid]);
			}
			return;
		}
		...
		if(client > 0 && IsPlayerAlive(client))
		{
			...
			{
				if(IsClientInGame(i) && iAnnounceText[i] == 1)
					PrintToChat(i, "\x04%N\x01 earned %s from %s.", client, UpgradeShort[val], AwardTitle[awardid]);
			}
		}
	}
}
...
public RemoveSurvivorUpgrade(client, amount, awardid)
{
	for(new num = 0; num < amount; num++)
	{
		...
		if(numMiss == MAX_UPGRADES)
		{
			...
			{
				if(IsClientInGame(i) && iAnnounceText[i] == 1)
					PrintToChat(i, "\x04%N \x01lost all upgrades.", client);
			}
			return;
		}
		...
		{
			if(IsClientInGame(i) && iAnnounceText[i] == 1)
				PrintToChat(i, "\x04%N \x01lost %s from %s.", client, UpgradeShort[val], AwardTitle[awardid]);
		}
	}
}
...
public event_HealSuccess(Handle:event, const String:name[], bool:Broadcast) 
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	new subject = GetClientOfUserId(GetEventInt(event, "subject"));
	new m_iMaxHealth = GetEntProp(subject, Prop_Send, "m_iMaxHealth");
	...
	{
		SetEntProp(subject,Prop_Send,"m_iHealth",m_iMaxHealth);
	}
	else
	{
		new m_iHealth = RoundFloat((m_iMaxHealth-m_iCHealth)+m_iCHealth*0.8);
		SetEntProp(subject, Prop_Send, "m_iHealth", m_iHealth);
	}
	iLastStand[client] = 0;
	iLargeFirstAidKit[client] = 0;
...
public event_InfectedDeath(Handle:event, const String:name[], bool:Broadcast) 
{
	...
	{
		if(GetRandomInt(0, 100) == 1 && GetConVarInt(PerkBonusSlots) > iPerkBonusSlots[client])
		{
...
{
	new Float:m_iHealth = float(GetEntProp(client, Prop_Send, "m_iHealth"));
	new Float:m_iMaxHealth = float(GetEntProp(client, Prop_Send, "m_iMaxHealth"));
	new Float:m_iHealthBufferBits = GetConVarInt(FindConVar("pain_pills_health_value"))*0.5;
	new Float:m_iHealthBuffer = GetEntPropFloat(client, Prop_Send, "m_healthBuffer");
...
public UpgradeMedicalChart(client, subject, restored_health)
{
	if(iUpgrade[client][43] > 0)
	{
		...
		for(new i = 1; i <= MaxClients; i++)
		{
			...
			{
				new m_iHealth = GetEntProp(i,Prop_Send,"m_iHealth");
				new m_iMaxHealth = GetEntProp(i,Prop_Send,"m_iMaxHealth");
				
				new iEHealth = m_iHealth + restored_health;
				if((m_iHealth + restored_health) > m_iMaxHealth)
					 iEHealth = m_iMaxHealth;
				SetEntProp(i,Prop_Send,"m_iHealth",iEHealth);
			}
		}
	}
}
...
public UpgradePillBox(client)
{
	if(iUpgrade[client][44] > 0)
	{
		...
		for(new i = 1; i <= MaxClients; i++)
		{
			if (i!=client && IsClientClose(i,client))
			{
				new Float:im_iHealth = float(GetEntProp(i,Prop_Send,"m_iHealth"));
				new Float:im_iMaxHealth = float(GetEntProp(i,Prop_Send,"m_iMaxHealth"));
				new Float:im_iHealthBuffer = GetEntPropFloat(i,Prop_Send, "m_healthBuffer");
...
public Action:timer_RegenerationTimer(Handle:timer, any:client)
{
	if(IsClientInGame(client) && GetClientTeam(client) == 2)
	{
		...
		SetEntProp(client, Prop_Send,"m_iHealth",m_iHealth+1);
		return Plugin_Continue;
	}
	return Plugin_Handled;
}
...
public UpgradeAirBoots(client)
{
	if(iUpgrade[client][6] > 0)
		SetEntPropFloat(client,Prop_Data,"m_flGravity",0.75);
	else
...
public Action:timer_CasingDispenser(Handle:timer, any:client)
{
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		{			
			...
			{
				iAmmoCheck = GetEntData(client, g_iOffsetAmmo+SMG_OFFSET_AMMO, 4);
				if(iAmmoCheck <= RoundToNearest(GetConVarInt(FindConVar("ammo_smg_max"))*iMultiplier)-5)
					SetEntData(client, g_iOffsetAmmo+SMG_OFFSET_AMMO, iAmmoCheck+5);
			}
			if(StrContains(iWeaponName, "rifle", false) != -1)
			{
				iAmmoCheck = GetEntData(client, g_iOffsetAmmo+RIFLE_OFFSET_AMMO, 4);
				if(iAmmoCheck <= RoundToNearest(GetConVarInt(FindConVar("ammo_assaultrifle_max"))*iMultiplier)-5)
					SetEntData(client, g_iOffsetAmmo+RIFLE_OFFSET_AMMO, iAmmoCheck+5);
			}
			if(StrContains(iWeaponName, "shotgun", false) != -1)
			{
				iAmmoCheck = GetEntData(client, g_iOffsetAmmo+SHOTGUN_OFFSET_AMMO, 4);
				if(iAmmoCheck <= RoundToNearest(GetConVarInt(FindConVar("ammo_buckshot_max"))*iMultiplier)-1)
					SetEntData(client, g_iOffsetAmmo+SHOTGUN_OFFSET_AMMO, iAmmoCheck+1);
			}
			if(StrContains(iWeaponName, "sniper", false) != -1)
			{
				iAmmoCheck = GetEntData(client, g_iOffsetAmmo+HUNTING_RIFLE_OFFSET_AMMO, 4);
				if(iAmmoCheck <= RoundToNearest(GetConVarInt(FindConVar("ammo_huntingrifle_max"))*iMultiplier)-2)
					SetEntData(client, g_iOffsetAmmo+HUNTING_RIFLE_OFFSET_AMMO, iAmmoCheck+2);
			}	
			return Plugin_Continue;
		}
		else

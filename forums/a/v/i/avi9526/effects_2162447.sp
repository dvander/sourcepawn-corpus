...
#define SPELL		0	// spell
#define CHARGE		1	// canteen charge
#define BUILD		2	// building
#define NONE		-1	// none
// Spell codes
...
#define DISABLED_GROUP	-3	// group disabled
...
enum MenuInfo
{
	// Menu ID (for separation of spells and canteen charges)
			MenuID,
	// Enable
			MenuEnable,
	// Enable cvar handle
	Handle: hMenuEnable,
	// Enable control console variable
	String:	MenuEnableCVar[STR_LEN],
	// Name
	String:	MenuName[STR_LEN],
	// Title for menu
	String:	MenuTitle[STR_LEN],
	// Name
	String:	MenuDesc[STR_LEN],
	// Console command
	String:	MenuCmd[STR_LEN],
};
...
public OnPluginStart()
{
	...
	for (i = 0; i < MENU; i++)
	{
		MenuList[i][hMenuEnable] = CreateConVar(MenuList[i][MenuEnableCVar], "1", "Set to 0 to disable menu and all effects in it", _, true, 0.0, true, 1.0);
		MenuList[i][MenuEnable] = GetConVarInt(MenuList[i][hMenuEnable]);
		HookConVarChange(MenuList[i][hMenuEnable], OnConVarChanged);
		RegConsoleCmd(MenuList[i][MenuCmd], Command_Menu, MenuList[i][MenuDesc]);
	}
...
public OnConVarChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	else
	{
		for (i = 0; i < MENU; i++)
		{
			if (convar == MenuList[i][hMenuEnable])
			{
				MenuList[i][MenuEnable] = StringToInt(newValue);
				LogAction(-1, -1, "%s %s now is %d", LOG_PREFIX, MenuList[i][MenuEnableCVar], MenuList[i][MenuEnable]);
			}
		}
		for (i = 0; i < EFFECTS; i++)
		{
...
stock IsEffectReady(client, Index, Float: Mult = 1.0)
{
	...
	if(MenuList[Effects[Index][GID]][MenuEnable] <= 0)
	{
...
InitMenuData()
{
	...
	// Amount of this menus must be stored in MENU constant
	
	// Spells
	
	MenuList[i][MenuID] = SPELL;
	MenuList[i][MenuEnable] = 1;
	MenuList[i][hMenuEnable] = INVALID_HANDLE;
	Format(MenuList[i][MenuName], STR_LEN, "!spells");
	Format(MenuList[i][MenuDesc], STR_LEN, "Spells menu");
	Format(MenuList[i][MenuTitle], STR_LEN, "Spells");
	Format(MenuList[i][MenuCmd], STR_LEN, "sm_spells");
	Format(MenuList[i][MenuEnableCVar], STR_LEN, "sm_spells_enable");
	i++;
	
	// Canteen
	
	MenuList[i][MenuID] = CHARGE;
	MenuList[i][MenuEnable] = 1;
	MenuList[i][hMenuEnable] = INVALID_HANDLE;
	Format(MenuList[i][MenuName], STR_LEN, "!canteen");
	Format(MenuList[i][MenuDesc], STR_LEN, "Canteen charges menu");
	Format(MenuList[i][MenuTitle], STR_LEN, "Canteen");
	Format(MenuList[i][MenuCmd], STR_LEN, "sm_canteen");
	Format(MenuList[i][MenuEnableCVar], STR_LEN, "sm_canteen_enable");
	i++;
	
	// Build
	
	MenuList[i][MenuID] = BUILD;
	MenuList[i][MenuEnable] = 1;
	MenuList[i][hMenuEnable] = INVALID_HANDLE;
	Format(MenuList[i][MenuName], STR_LEN, "!build");
	Format(MenuList[i][MenuDesc], STR_LEN, "Building menu");
	Format(MenuList[i][MenuTitle], STR_LEN, "Build");
	Format(MenuList[i][MenuCmd], STR_LEN, "sm_build");
	Format(MenuList[i][MenuEnableCVar], STR_LEN, "sm_build_enable");
	i++;
}
//──────────────────────────────────────────────────────────────────────────────
InitEffectsData()
...
public ShowMainMenu(Client)
{
	...
	for (i = 0; i < MENU; i++)
	{
		if (MenuList[i][MenuEnable] > 0)
		{
			AddMenuItem(MainMenu, MenuList[i][MenuName], MenuList[i][MenuName]);
		}
	}
...
public MainMenuHandler(Handle:Menu, MenuAction:action, param1, param2)
{
	...
	if(action==MenuAction_Select)
	{
		...
		while (i < MENU)
		{
			if (StrEqual(Info, MenuList[i][MenuName]))
			{
				ShowMenu(param1, MenuList[i][MenuID], Menu);
				i = MENU;
			}
			i ++;
		}
...
public Action:Command_Menu(client, args)
{
	...
	while (i < MENU)
	{
		if(StrEqual(Command, MenuList[i][MenuCmd], false))
		{
			Selector = i;
			i = MENU;
		}
...
public ShowMenu(client, Group, Handle:Parent)
{
	...
	SetMenuTitle(Menu, MenuList[Group][MenuTitle]);
	...
	for(new Index = 0; Index < EFFECTS; Index++)
	{
		...
		if(Effects[Index][GID] == Group)
		{
			...
			else if((iDelay == DISABLED) || (iDelay == DISABLED_GROUP))
			{
...
UseEffect(Client, Index)
{
	...
	else if(TimeWait == DISABLED_GROUP)
	{
		// Effect disabled
		PrintToChat(Client, "%s All effects in group «%s» are disabled", CHAT_PREFIX, MenuList[Effects[Index][GID]][MenuName]);
		return;
	}

...
public OnPluginStart()
{
	...
	g_hIsDebugOn = CreateConVar("sm_hr_debug","0","Enable/Disable Debug Mode (0 - disabled)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hLifeTime = CreateConVar("sm_hr_lifetime", "10", "Homing rocket's life time.\nValue in integer seconds.", FCVAR_NOTIFY, true, 0.0);
	g_hLTMode = CreateConVar("sm_hr_lifetimemode", "1", "Life time for homing rockets (1) or for all rockets (0).", FCVAR_NOTIFY, true, 0.0);
	g_hAccuracy = CreateConVar("sm_hr_accuracy", "1", "Homing accuracy (0-10)", FCVAR_NOTIFY, true, 0.0, true, 10.0);
	g_hSAccuracy = CreateConVar("sm_hr_saccuracy", "3", "(Sentry) Homing accuracy (0-10)", FCVAR_NOTIFY, true, 0.0, true, 10.0);
	
	g_hIsPluginOn = CreateConVar("sm_hr_enable","1","Enable/Disable Plugin (0 = disabled | 1 = enabled)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	HookConVarChange(g_hIsPluginOn, OnConVarChanged_PluginOn);
	g_bPluginOn = (GetConVarInt(g_hIsPluginOn)!=0?true:false);
	
	g_hForceHoming = CreateConVar("sm_hr_forcehoming", "1", "Force homing on (2), or force off (0), or make players able to (de)activate (1).", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	HookConVarChange(g_hForceHoming, OnConVarChanged_ForceHoming);
	g_nForceHoming = GetConVarInt(g_hForceHoming);
	
	g_hSForceHoming = CreateConVar("sm_hr_sforcehoming", "0", "(Sentry) Force homing on (1), or make players able to (de)activate (0).", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	HookConVarChange(g_hSForceHoming, OnConVarChanged_SForceHoming);
	g_nSForceHoming = GetConVarInt(g_hSForceHoming);
	
	g_hShowAim = CreateConVar("sm_hr_showaim", "1", "Show (1) dot on aim or not (0).", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	HookConVarChange(g_hShowAim, OnConVarChanged_ShowAim);
	g_bShowAim = (GetConVarInt(g_hShowAim)!=0?true:false);
	
	g_hSShowAim = CreateConVar("sm_hr_sshowaim", "1", "(Sentry) Show (1) dot on aim or not (0).", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	HookConVarChange(g_hSShowAim, OnConVarChanged_SShowAim);
	g_bSShowAim = (GetConVarInt(g_hSShowAim)!=0?true:false);
	
	g_hHomingMode = CreateConVar("sm_hr_mode","3","Enable homing for soldier's rockets (1), or for sentry's rockets (2), or for both of them (3), or disable it (0).", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	HookConVarChange(g_hHomingMode, OnConVarChanged_HomingMode);
	g_nHomingMode = GetConVarInt(g_hHomingMode);
	
	g_hMvMDisabler = CreateConVar("sm_hr_mvm_disabler", "1", "Disable plugin for BLU team (MvM mode).", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	hConVar = CreateConVar("sm_hr_adminflag", "", "Admins Flag for access; make it empty to turn off.", FCVAR_NOTIFY);
	...
	hConVar = CreateConVar("sm_hr_launchers", "", sBuffer, FCVAR_NOTIFY);
	HookConVarChange(hConVar, OnConVarChanged_AllowedLaunchers);
	GetConVarString(hConVar, sBuffer, iBufferSize);
	OnConVarChanged_AllowedLaunchers(hConVar, "", sBuffer);
	
	hConVar = CreateConVar("sm_homingrocket", PLUGIN_VERSION, "Version of Homing Rockets", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
			...
			if( bRocketLauncher && (g_nHomingMode==1 || g_nHomingMode==3) || bLaserPointer && (g_nHomingMode==2 || g_nHomingMode==3) )
			{
				...
				if( g_bShowAim && bRocketLauncher && ( g_bActiveHoming[iClient] && g_nForceHoming==1 || g_nForceHoming==2 ) || g_bSShowAim && bLaserPointer && ( g_bActiveHoming[iClient] && g_nSForceHoming==0 || g_nSForceHoming==1 ) )
				{
					...
					for( new i = 0; i < ALLOWED_LAUNCHERS_LIMIT; i++ )
						...
						{
							if( !TF2_HasCond(iClient, view_as<int>(TFCond_Taunting)) && !TF2_HasCond(iClient, view_as<int>(TFCond_Dazed)) )
							{
...
bool:GetPlayerEye(client, Float:pos[3])
{
	...
	new Handle:trace = TR_TraceRayFilterEx(vOrigin, vAngles, MASK_SHOT, RayType_Infinite, TraceEntityFilter, client);
...
public bool:TraceEntityFilter(entity, contentsMask, any:data)
{
	...
	else if (StrEqual(sClassname, "entity_medigun_shield", false))
	{
		return false;
	}
	else

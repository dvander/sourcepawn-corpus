...
public OnPluginStart()
{
	...
	hHolyday = FindConVar("tf_forced_holiday");
}
...
ShowFootStepsMenu(client, page = 0)
{	
	new Handle:ws = CreateMenu(FootStepsCALLBACK);
	SetMenuTitle(ws, "Choose Footprints Effect");
...
// 	DisplayMenu(ws, client, MENU_TIME_FOREVER);
	DisplayMenuAtItem(ws, client, page*7, MENU_TIME_FOREVER);
}

public Action:FootSteps(client, args)
{
	new Enabled = 0;
	if (hHolyday != INVALID_HANDLE)
	{
		Enabled = GetConVarInt(hHolyday) & 2;
	}
	else
	{
		// CVar not found, lets just show menu
		Enabled = 1;
	}
	if (Enabled)
	{
		ShowFootStepsMenu(client);
	}
	else
	{
		ReplyToCommand(client, "\x01[\x07B262FFFootsteps\x01]This mod works only on Halloween!\nWait for it or ask administrator to set console variable\n\x07B262FFtf_forced_holiday 2");
	}
...
public FootStepsCALLBACK(Handle:menu, MenuAction:action, client, param2)
{
	...
	if(action == MenuAction_Select)
	{
		...
		ShowFootStepsMenu(client, param2/7);
	}
}

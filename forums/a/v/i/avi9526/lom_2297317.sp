//──────────────────────────────────────────────────────────────────────────────
/*
	Copyright 2006-2015 AlliedModders LLC
	Copyright 2013-2015 avi9526 <dromaretsky@gmail.com>
	Copyright 2013-2015 FlaminSarge http://forums.alliedmods.net/member.php?u=84304
	Copyright 2013-2015 Doctor Mc kay http://www.doctormckay.com
*/
//──────────────────────────────────────────────────────────────────────────────
/*
...
ReloadWeaponList()
{
	new Handle: KeyVal = INVALID_HANDLE;
	...
	for (new i = 0; i < CFG_COUNT; i ++)
	{
		if (Configs[i][Enabled])
		{
			...
			if (FileExists(ConfigFile))
			{
				...
				KeyVal = CreateKeyValues(CFG_WEAPONS_NAME);
//				LogAction(-1, -1, "%s Begin reading config file '%s'", LOG_PREFIX, ConfigFile);
				if (FileToKeyValues(KeyVal, ConfigFile) == true)
				{
					// Check section name
					KvGetSectionName(KeyVal, Section, sizeof(Section));
					if (StrEqual(Configs[i][WeapSecName], Section) == true)
					{
						// Go to first sub-key
						if (KvGotoFirstSubKey(KeyVal))
						{
//							LogAction(-1, -1, "%s Reading weapon list", LOG_PREFIX);
							// Go trough all next sub-keys
							do
							{
								KvGetSectionName(KeyVal, SubSection, sizeof(SubSection));
								if (SubSection[0] != '*')
								{
									...
									if (Data[ID] > 0)
									{
										Data[ParentID] = KvGetNum(KeyVal, "Index", -1);	// get parent weapon ID - only for custom weapons
										Data[Class] = KvGetNum(KeyVal, "Class", 1023);
										Data[Slot] = KvGetNum(KeyVal, "Slot", 0);
										Data[GameMode] = KvGetNum(KeyVal, "GameMode", 1023);
										Data[Apocalyptic] = KvGetNum(KeyVal, "Apocalyptic", 0);
										Data[ArrayIndex] = Index;	// save current array index
										KvGetString(KeyVal, "Name", Data[Name], STR_LEN);
										KvGetString(KeyVal, "Access", Data[Access], STR_LEN);
...
ShowMainMenu(Client)
{
	...
	if (Players[Client][PlrEnabled])
	{
		...
		AddMenuItem(MainMenu, ITEM_MAIN_LOAD, "Load loadout", ITEMDRAW_DISABLED);
		AddMenuItem(MainMenu, ITEM_MAIN_SAVE, "Save loadout", ITEMDRAW_DISABLED);
		AddMenuItem(MainMenu, ITEM_MAIN_INFO, "View loadout", ITEMDRAW_DISABLED);
		AddMenuItem(MainMenu, ITEM_MAIN_REM, "Remove loadout", ITEMDRAW_DISABLED);
...
ShowHelpMenu(Client)
{
	...
	AddMenuItem(hMenu, ITEM_BASIC_BACK, "Q: Why some items disabled?", ITEMDRAW_DISABLED);
	AddMenuItem(hMenu, ITEM_BASIC_BACK, "A: This plugin is not finished yet", ITEMDRAW_DISABLED);

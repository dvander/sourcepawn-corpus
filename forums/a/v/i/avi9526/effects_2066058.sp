...
#define KILLBUILD 	6	// kill all owned dispensers
//──────────────────────────────────────────────────────────────────────────────
// Some codes
...
enum PlayerData
{
	...
	// Eye light particle entity reference (entity ID can change, reference is reliable)
	EyeLightRight,		// right team color
	EyeLightLeft,		// left team color
	EyeLightGreenRight,	// right green
	EyeLightGreenLeft	// left green
};
...
public OnPluginStart()
{
	...
	{
		//Format(DefVal, sizeof(DefVal), "%d", Effects[i][Delay]);	// convert delay value to string for ConVar initialization
...
public OnClientDisconnect(client)
{
	KillAllDisp(client);
	KillAllSentry(client);
	KillPlayerDecor(client);
	ResetPlayerData(client);
}
//──────────────────────────────────────────────────────────────────────────────
public TF2_OnConditionAdded(Client, TFCond: Condition)
{
	// No need to check for valid client here - its done in used functions
	if (Condition == TFCond_CritCanteen)
	{
		DecorEyeLight(Client);
	}
	if (Condition == TFCond_UberchargedCanteen)
	{
		DecorEyeLightGreen(Client);
	}
}
//──────────────────────────────────────────────────────────────────────────────
public TF2_OnConditionRemoved(Client, TFCond: Condition)
{
	...
	if (Condition == TFCond_CritCanteen)
	{
		//LogAction(-1, -1, "%s cond remove", LOG_PREFIX);
		UnDecorEyeLight(Client);
	}
	if (Condition == TFCond_UberchargedCanteen)
	{
		//LogAction(-1, -1, "%s cond remove", LOG_PREFIX);
		UnDecorEyeLightGreen(Client);
	}
}
//──────────────────────────────────────────────────────────────────────────────
// Stocks
...
//──────────────────────────────────────────────────────────────────────────────
// Add eye light particle to player
stock DecorEyeLight(Client, Float: Time = 0.0)
{
	if (Time >= 0.0 && IsPlayerAlive(Client))
	{
		//LogAction(-1, -1, "%s create new decor", LOG_PREFIX);
		UnDecorEyeLight(Client);
		new Team = GetClientTeam(Client);
		new String: PartName[STR_LEN];
		if (Team == _:TFTeam_Blue)
		{
			PartName = "eye_powerup_blue_lvl_3";
		}
		else if (Team == _:TFTeam_Red)
		{
			PartName = "eye_powerup_red_lvl_3";
		}
		else
		{
			return;
		...
		if (Class != _:TFClass_Scout && Class != _:TFClass_Sniper && Class != _:TFClass_Soldier && Class != _:TFClass_Medic && 
			Class != _:TFClass_Heavy && Class != _:TFClass_Spy)
		{
			return;
		}
		Players[Client][EyeLightRight] = ParticleCreate(PartName, _, Time, Client, _, "righteye");
		Players[Client][EyeLightLeft] = ParticleCreate(PartName, _, Time, Client, _, "lefteye");
	}
}
//──────────────────────────────────────────────────────────────────────────────
// Remove eye light particle from player
stock UnDecorEyeLight(Client)
{
	ParticleDestroy(Players[Client][EyeLightRight]);
	ParticleDestroy(Players[Client][EyeLightLeft]);
}
//──────────────────────────────────────────────────────────────────────────────
// Add eye light particle to player
stock DecorEyeLightGreen(Client, Float: Time = 0.0)
{
	if (Time >= 0.0 && IsPlayerAlive(Client))
	{
		//LogAction(-1, -1, "%s create new decor", LOG_PREFIX);
		...
		Players[Client][EyeLightGreenRight] = ParticleCreate("eye_powerup_green_lvl_4", _, Time, Client, _, "righteye");
		Players[Client][EyeLightGreenLeft] = ParticleCreate("eye_powerup_green_lvl_4", _, Time, Client, _, "lefteye");
	}
}
//──────────────────────────────────────────────────────────────────────────────
// Remove eye light particle from player
stock UnDecorEyeLightGreen(Client)
{
	ParticleDestroy(Players[Client][EyeLightGreenRight]);
	ParticleDestroy(Players[Client][EyeLightGreenLeft]);
}
//──────────────────────────────────────────────────────────────────────────────
...
ResetPlayerData(client)
{
	for(new Index = 0; Index < EFFECTS; Index++)
	{
		...
		Players[client][EyeLightRight] = INVALID_ENT_REFERENCE;
		Players[client][EyeLightLeft] = INVALID_ENT_REFERENCE;
		Players[client][EyeLightGreenRight] = INVALID_ENT_REFERENCE;
		Players[client][EyeLightGreenLeft] = INVALID_ENT_REFERENCE;
	}
}
//──────────────────────────────────────────────────────────────────────────────
ResetAllData()
...
KillPlayerDecor(client)
{
	//LogAction(-1, -1, "%s remove all decor", LOG_PREFIX);
	UnDecorEyeLight(client);
	UnDecorEyeLightGreen(client);
}
//──────────────────────────────────────────────────────────────────────────────
...
{
	// Is it ready?
	new TimeWait = IsEffectReady(Client, Index);	// 0 - ready; > 0 - time to wait; -1 - limit reached; < -1 - disabled
	if(TimeWait > 0)
	{

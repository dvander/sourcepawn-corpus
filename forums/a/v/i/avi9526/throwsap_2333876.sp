...
#define LOG_PREFIX		"[Sapper]"
#define CHAT_PREFIX		"\x01[\x072CF296Sapper\x01]"

#define PLUGIN_VERSION "0x06.1"
...
new SapperTime;			// sapper working time
new Handle: hSapperTime = INVALID_HANDLE;

new RemBuildHealth;			// remove health from building
new Handle: hRemBuildHealth = INVALID_HANDLE;

new RechargeDelay;			// sapper recharge delay
new Handle: hRechargeDelay = INVALID_HANDLE;
...
enum PlayerData
{
	TimeUsed,	// last time player dropped sapper
	SapLock	// prevent player from dropping another sapper
};
...
public OnPluginStart()
{
	...
	g_cvSapRadius = CreateConVar("throwsap_sapradius", "1000.0", "Radius of effect.");
	
	hSapperTime = CreateConVar("throwsap_saptime", "30.0", "Sapping time." , 0, true, 3.0, true, 300.0);
	SapperTime = GetConVarInt(hSapperTime);
	
	hRemBuildHealth = CreateConVar("throwsap_removehealth", "20.0", "Remove amount of health from building with every sapping iteration" , 0, true, 2.0, true, 200.0);
	RemBuildHealth = GetConVarInt(hRemBuildHealth);
	
	hRechargeDelay = CreateConVar("throwsap_rechargedelay", "20.0", "Sapper recharge delay" , 0, true, 1.0, true, 600.0);
	RechargeDelay = GetConVarInt(hRechargeDelay);
...
public OnConVarChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	new NewIntVal = -1;
	new OldIntVal = -1;
	if (convar == hSapperTime)
	{
		NewIntVal = StringToInt(newValue);
		OldIntVal = StringToInt(oldValue);
		if (NewIntVal != OldIntVal)
		{
			SapperTime = NewIntVal;
		}
	} else if (convar == hRemBuildHealth)
	{
		NewIntVal = StringToInt(newValue);
		OldIntVal = StringToInt(oldValue);
		if (NewIntVal != OldIntVal)
		{
			RemBuildHealth = NewIntVal;
		}
	} else if (convar == hRechargeDelay)
	{
		NewIntVal = StringToInt(newValue);
		OldIntVal = StringToInt(oldValue);
		if (NewIntVal != OldIntVal)
		{
			RechargeDelay = NewIntVal;
		}
	}
}
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
    ...
    if (TF2_GetPlayerClass(client) == TFClass_Spy && buttons & (IN_ATTACK3 | IN_RELOAD))
    {
        ...
        if (!bCloaked && IsWeaponSlotActive(client, TFWeaponSlot_Secondary))
        {
            ...
            {
				new TimeWait = IsSapperReady(client);
				if (TimeWait == 0)
				{
					...
					if (GameModeFlag != GAME_MODE_MVM)	// don't remove sapper from player in MvM mode, there is no locker to get new one in there
					{
...
{
	if (Players[client][SapLock] > 0)
	{
		PrintToChat(client, "%s Only one sapper can be thrown at the time", CHAT_PREFIX);
		return;
	}
    ...
    if (IsValidEntity(sapper))
    {
		...
		CreateTimer(float(SapperTime), StopSapping, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
        tTimerLoop[client] = CreateTimer(0.25, LoopSapping, GetClientUserId(client), TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
		
		Players[client][SapLock] = 1;
    }
}
...
{
    new client = GetClientOfUserId(userid);
    if (IsValidClient(client) && ThrownSapperExists(client))
    {
		...
		if (GameModeFlag == GAME_MODE_MVM)
		{
			SapBots(client, vSapperPos);
		}
...
public Action:StopSapping(Handle:timer, any:userid)
{
	...
	Players[client][SapLock] = 0;
	Players[client][TimeUsed] = GetTime();
...
stock AttachRings(entity)
{
    ...
    {
        TE_SetupBeamRingPoint(vSapperPos, 0.25, radius, g_hEffectSprite, g_hEffectSprite, 1, 1, 0.6, 3.0, 10.0, red, 15, 0);
        TE_SendToAll();
        TE_SetupBeamRingPoint(vSapperPos, 0.25, radius, g_hEffectSprite, g_hEffectSprite, 1, 1, 0.6, 3.0, 10.0, red, 15, 0);
        TE_SendToAll();
        TE_SetupBeamRingPoint(vSapperPos, 0.25, radius, g_hEffectSprite, g_hEffectSprite, 1, 1, 0.6, 3.0, 10.0, red, 15, 0);
    ...
    {
		TE_SetupBeamRingPoint(vSapperPos, 0.25, radius, g_hEffectSprite, g_hEffectSprite, 1, 1, 0.6, 3.0, 10.0, blue, 15, 0);
        TE_SendToAll();
		TE_SetupBeamRingPoint(vSapperPos, 0.25, radius, g_hEffectSprite, g_hEffectSprite, 1, 1, 0.6, 3.0, 10.0, blue, 15, 0);
        TE_SendToAll();
		TE_SetupBeamRingPoint(vSapperPos, 0.25, radius, g_hEffectSprite, g_hEffectSprite, 1, 1, 0.6, 3.0, 10.0, blue, 15, 0);
...
stock SapBots(client, Float:vPos1[3])
{
	...
	for (i = 1; i <= MaxClients; i ++)
	{
		if (i != client && IsClientInGame(i) && IsPlayerAlive(i) && GetClientTeam(i) != Team)
		{
			if (!TF2_IsPlayerInCondition(i, TFCond_Ubercharged))
			{
				...
				new Float:flDistance = GetVectorDistance(vPos1, vPos2);
				if(flDistance < GetConVarFloat(g_cvSapRadius))
				{
					SDKHooks_TakeDamage(i, 0, client, float(RemBuildHealth), DMG_PREVENT_PHYSICS_FORCE|DMG_PARALYZE|DMG_ALWAYSGIB);
					TF2_AddCondition(i, TFCond_MVMBotRadiowave, 5.0);

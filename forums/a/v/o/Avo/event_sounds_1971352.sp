...
#define PLUGIN_AUTHOR "Kemsan, Avo"
...
new bool:g_bMuteSounds[ 33 ];

new String:modname[50];

new bool:endSoundLaunched = false;
...
public GameEvents(Handle:hEvent, const String:name[], bool:dontBroadcast)
{
   ...
   decl String:strExpBuffer2[ 2 ][ 512 ];
   decl String:strCond[ 512 ];
   new iCount = 0;
   new iCondCount;
   new iAllow;
   new iClient1, iClient2;
   
   if (endSoundLaunched)
      return;
      
   //Get events count
   Format( strBuffer, sizeof( strBuffer ), "%s_count", strEventName );
   GetTrieValue( g_hSoundsTrie, strBuffer, iCount );
   
   for( new iNum = 0; iNum < iCount; iNum++ )
   {
      iCondCount = 0;
      iAllow = 0;
      
      //Get conditions count
      Format( strBuffer, sizeof( strBuffer ), "%s_%i_condition_count", strEventName, iNum );
      GetTrieValue( g_hSoundsTrie, strBuffer, iCondCount );
      
      for( new i = 0; i < iCondCount; i++ )
      {
         Format( strName, sizeof( strName ), "%s_%i_condition_name_%i", strEventName, iNum, i );
         GetTrieString( g_hSoundsTrie, strName, strBuffer, sizeof( strBuffer ) );
         
         Format( strName, sizeof( strName ), "%s_%i_condition_%i", strEventName, iNum, i );
         ...
         {
            if( GetEventInt( hEvent, strExpBuffer[ 0 ] ) == StringToInt( strBuffer2 ) )
            {
               iAllow++;
            }
         }
         ...
         {
            if( GetEventFloat( hEvent, strExpBuffer[ 0 ] ) == StringToFloat( strBuffer2 ) )
            {
               iAllow++;
            }
         }
         ...
         {
            if( FindCharInString( strBuffer2, ':' ))
            {
               ExplodeString( strBuffer2, ":", strExpBuffer2, 2, 512 );
               if( StrEqual( strExpBuffer2[ 1 ], "userid" ) )
               {
                  if( GetClientOfUserId( GetEventInt( hEvent, strExpBuffer[ 0 ] ) ) == GetClientOfUserId( GetEventInt( hEvent, strExpBuffer2[ 0 ] ) ) )
                  {
                     iAllow++;
                  }
               }
            }
            else if( GetClientOfUserId( GetEventInt( hEvent, strExpBuffer[ 0 ] ) ) == StringToInt( strBuffer2 ) )
            {
               iAllow++;
            }
         }
         else if( StrEqual( strExpBuffer[ 1 ], "teamid" ) )
         {
            if( FindCharInString( strBuffer2, ':' ))
            {
               ExplodeString( strBuffer2, ":", strExpBuffer2, 2, 512 );
               if( StrEqual( strExpBuffer2[ 1 ], "teamid" ) )
               {
                  iClient1 = GetClientOfUserId( GetEventInt( hEvent, strExpBuffer[ 0 ] ) );
                  iClient2 = GetClientOfUserId( GetEventInt( hEvent, strExpBuffer2[ 0 ] ) );
                  if( ( iClient1 > 0 ) && ( iClient2   > 0 ) && ( GetClientTeam( iClient1 ) == GetClientTeam( iClient2 ) ) )
                  {
                     iAllow++;
                  }
               }
            ...
            {
               iClient1 = GetClientOfUserId( GetEventInt( hEvent, strExpBuffer[ 0 ] ) );
               if( ( iClient1 > 0 ) && GetClientTeam( iClient1 ) == StringToInt( strBuffer2 ) )
               {
                  iAllow++;
               }
            }
      ...
      if( iAllow == iCondCount )
      {
         ...
         //LogMessage("Event %s_%i triggered", strEventName, iNum);
         
         //Push sound
         PushSound( strEventName, iNum, iClient );
         
         if (strcmp(strEventName, "cs_win_panel_match") == 0)
            endSoundLaunched = true;
         return;
      }
   }
}
...
public Action:Cmd_Say( iClient, iArgs )
{
   ...
   if( IsValidClient( iClient ) )
   {
      ...
      if( StrEqual( strChatText[ iStart ], "!stopevent" ) || StrEqual( strChatText[ iStart ], "/stopevent" ) )
      {
      ...
      else if( StrEqual( strChatText[ iStart ], "!stopallevent" ) || StrEqual( strChatText[ iStart ], "/stopallevent" ) )
      {
         //Set false / true to all sounds
         g_bMuteSounds[ iClient ] = g_bMuteSounds[ iClient ] ? false : true;
         if( g_bMuteSounds[ iClient ] )
         {
            PrintToChat(iClient, "\x04[SM] Event Sounds: \x01%t", "StopAllEvent Enabled");
         }
         else
         {
            PrintToChat(iClient, "\x04[SM] Event Sounds: \x01%t", "StopAllEvent Disabled");
         }
...
//===== [ SOUNDS ] ==============
stock PushSound( const String:strEventName[ 150 ], iNum, any:iClient = 0 )
{
   ...
   Format( strSound, sizeof( strSound ), "%s_%i_sound_count", strEventName, iNum );
   GetTrieValue( g_hSoundsTrie, strSound, iMaxSounds );
   
   Format( strSound, sizeof( strSound ), "%s_%i_push_sound", strEventName, iNum );
   ...
   if( ( iMaxSounds && iMaxSounds >= 1 ) || iPushSound == 0 )
   {
      ...
      //Rand sound
      iRandomSound = GetRandomInt( 0, iMaxSounds-1 );
   
      //Format for sound
      Format( strSound, sizeof( strSound ), "%s_%i_sound_%i", strEventName, iNum, iRandomSound );
      GetTrieString( g_hSoundsTrie, strSound, strSoundPath, sizeof( strSoundPath ) );
   
      Format( strSound, sizeof( strSound ), "%s_%i_sound_name_%i", strEventName, iNum, iRandomSound );
      GetTrieString( g_hSoundsTrie, strSound, strSoundName, sizeof( strSoundName ) );
      
      Format( strSound, sizeof( strSound ), "%s_%i_notify", strEventName, iNum );
      GetTrieString( g_hSoundsTrie, strSound, strSoundNotify, sizeof( strSoundNotify ) );
      
      Format( strSound, sizeof( strSound ), "%s_%i_playtype", strEventName, iNum );
      GetTrieValue( g_hSoundsTrie, strSound, iSoundPlayType );
      
      Format( strSound, sizeof( strSound ), "%s_%i_clientonly", strEventName, iNum );
      GetTrieValue( g_hSoundsTrie, strSound, iSoundClientOnly );
      
      Format( strSound, sizeof( strSound ), "%s_%i_sound_delay", strEventName, iNum );
      GetTrieString( g_hSoundsTrie, strSound, strSoundDelay, sizeof( strSoundDelay ) );
      
      Format( strSound, sizeof( strSound ), "%s_%i_notify_delay", strEventName, iNum );
...
public Action:Timer_PlaySound( Handle:hTimer, Handle:hPack )
{
   ...
   if( FileExists( strSoundPath ) )
   {
      ...
      if(StrEqual(modname, "csgo", false))
         Format(strSoundPath, sizeof(strSoundPath), "*%s", strSoundPath);
      ...
      if( iSoundPlayType == 1 ) //Use "play" command
      {
         for( i = 1; i < MaxClients; i++ )
         {
            if( IsValidClient( i ) && g_bMuteSounds[ i ] == false )
            {
...
public Action:Timer_ShowNotify( Handle:hTimer, Handle:hPack )
{
   ...
   if( IsValidClient( iClient ) )
   {
      if( g_bMuteSounds[ iClient ] == false )
      {
   ...
   else
   {
      ...
      if( bColorNotify )
      {
         ...
         {
            if( IsValidClient( iClient ) && g_bMuteSounds[ iClient ] == false )
            {
               CPrintToChat( iClient, strSoundNotify );
            }
         }
...
stock ParseSounds( )
{
   ...
   if ( KvGotoFirstSubKey( hKeyValues, false ) )
   {
      ...
      do
      {
         ...
         LogMessage("Hook %s event", strEventName);
         
         if ( KvGotoFirstSubKey( hKeyValues, false ) )
         {
            do
            {
               KvGetSectionName( hKeyValues, strNum, sizeof( strNum ) );
               ...
               if ( KvGotoFirstSubKey( hKeyValues, false ) )
               {
                  do
                  {
                     ...
                     if( StrEqual( strBuffer, "sounds" ) )
                     {
                        if ( KvGotoFirstSubKey( hKeyValues, false ) )
                        {
                           do
                           {
                              ...
                              {
                                 Format( strSoundName, sizeof( strSoundName ), "%s_%i_sound_name_%i", strEventName, iNum, iSoundCount );
                                 SetTrieString( g_hSoundsTrie, strSoundName, strBuffer );
                                 
                                 Format( strSoundName, sizeof( strSoundName ), "%s_%i_sound_%i", strEventName, iNum, iSoundCount );
                                 ...
                                 if(StrEqual(modname, "csgo", false))
                                 {
                                    Format(strBuffer2, sizeof(strBuffer2), "*%s", strBuffer2);
                                    FakePrecacheSound( strBuffer2 );
                                 }
                                 else
                                 {
                           ...
                           Format( strBuffer, sizeof( strBuffer ), "%s_%i_sound_count", strEventName, iNum );
                     ...
                     else if( StrEqual( strBuffer, "conditions" ) )
                     {
                        if ( KvGotoFirstSubKey( hKeyValues, false ) )
                        {
                           do
                           {
                              ...
                              Format( strSoundName, sizeof( strSoundName ), "%s_%i_condition_name_%i", strEventName, iNum, iConditions );
                              SetTrieString( g_hSoundsTrie, strSoundName, strBuffer );
                                 
                              Format( strSoundName, sizeof( strSoundName ), "%s_%i_condition_%i", strEventName, iNum, iConditions );
                              SetTrieString( g_hSoundsTrie, strSoundName, strBuffer2 );
                     ...
                     else if( StrEqual( strBuffer, "options" ) )
                     {
                        ...
                        Format( strBuffer, sizeof( strBuffer ), "%s_%i_playtype", strEventName, iNum );
                        SetTrieValue( g_hSoundsTrie, strBuffer, GetConVarInt( g_hSoundPlayType ) );
                        
                        Format( strBuffer, sizeof( strBuffer ), "%s_%i_clientonly", strEventName, iNum );
                        SetTrieValue( g_hSoundsTrie, strBuffer, GetConVarInt( g_hClientOnly ) );
                        
                        Format( strBuffer, sizeof( strBuffer ), "%s_%i_notify", strEventName, iNum );
                        GetConVarString( g_hNotifyMsg, strDefaultValue, sizeof( strDefaultValue ) );
                        SetTrieString( g_hSoundsTrie, strBuffer, strDefaultValue );
                        
                        Format( strBuffer, sizeof( strBuffer ), "%s_%i_notify_delay", strEventName, iNum );
                        GetConVarString( g_hNotifyDelay, strDefaultValue, sizeof( strDefaultValue ) );
                        SetTrieString( g_hSoundsTrie, strBuffer, strDefaultValue );
                        
                        Format( strBuffer, sizeof( strBuffer ), "%s_%i_sound_delay", strEventName, iNum );
                           ...
                           do
                           {
                              ...
                              switch( KvGetDataType( hKeyValues, NULL_STRING ) )
                              {
                                 case KvData_Int:
                                 {
                                    ...
                                    {
                                       Format( strBuffer, sizeof( strBuffer ), "%s_%i_push_sound", strEventName, iNum );
                                    }
            ...
            Format( strBuffer, sizeof( strBuffer ), "%s_count", strEventName );
            SetTrieValue( g_hSoundsTrie, strBuffer, iNum );

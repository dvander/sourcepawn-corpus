...
public OnPluginStart()
{
	...
	cvar_wireused = CreateConVar("qd_wireused", "0", "Sets whether wire was used (QuickDefuse)", FCVAR_PROTECTED )
...
{
	SetConVarBool(cvar_wireused, false);
}
...
public PanelDefuseKit(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select && param2 > 0 && param2 < 5) //User selected a valid wire colour
	{
		...
		if (bombent)
		{
			SetConVarBool(cvar_wireused, true);
			new String:name[32]

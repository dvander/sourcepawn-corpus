...
public OnPluginStart()
{
	...
	hMinPounceAnnounce = CreateConVar("pounceannounce_minimum","25","The minimum amount of damage required to announce the pounce", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	hCenterChat = CreateConVar("pounceannounce_centerchat","0","Announces the pounce to center chat. Use 0 for regular player chat.",FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	hCapDamage = CreateConVar("pounceannounce_capdamage","1","Caps the displayed pounce damage to the maximum able to be dealt.",FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
...
public Event_PlayerPounced(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	decl String:adj[128];
	Format(adj, sizeof(adj), "pathetic");
	if (dmg >= float(30)) Format(adj, sizeof(adj), "weak");
	if (dmg >= float(40)) Format(adj, sizeof(adj), "modest");
	if (dmg >= float(50)) Format(adj, sizeof(adj), "decent");
	if (dmg >= float(60)) Format(adj, sizeof(adj), "good");
	if (dmg >= float(70)) Format(adj, sizeof(adj), "nice");
	if (dmg >= float(80)) Format(adj, sizeof(adj), "great");
	if (dmg >= float(90)) Format(adj, sizeof(adj), "awesome");	
	if (dmg >= float(100))
	{
		Format(adj, sizeof(adj), "epic");
		EmitSoundToAll("player/orch_hit_Csharp_short.wav");
	}
	...
	if(distance >= min && dmg >= minAnnounce)
	{
		...
		#endif
		Format(pounceLine,sizeof(pounceLine),"\x04%N\x01 pounced \x04%N\x01 for \x03%s %.01f damage\x01 (max: %d)",attackerClient,victimClient,adj,dmg,maxDmg + 1);

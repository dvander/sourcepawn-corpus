...
public Action:InfectedHurt(Handle:event, const String:name[], bool:dontBroadcast){
	...
	dmg_health = GetEntProp(client, Prop_Data, "m_iHealth") - GetEventInt(event,"amount");	
	...
	if(GetEntProp(client, Prop_Data, "m_iHealth") > 0){
		health = RoundToNearest(GetEntProp(client, Prop_Data, "m_iHealth") - dmg_health*multiplier);
		if( health >= 0 ){

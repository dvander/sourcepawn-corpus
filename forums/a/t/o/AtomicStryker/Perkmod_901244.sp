...
- version 1.0.1: Changed code for Blind Luck with Doku's
...
- version 1.1.1: Fixed Martial Artist not resetting fatigue, removed
		debug messages (oops!)
- version 1.2: New perk for Boomers - Motion Sickness, boosts movement speed
		and lets you run while vomiting, reinstated Speed Demon (for hunters),
		nerfed Metabolic Boost default value from 1.5 to 1.4, nerfed Tongue
		Twister default values slightly (pull speed 1.75->1.5, shoot speed
		1.75->1.5, range 1.75 unchanged), unbreakable now gives bonus buffer
		on being revived
		...
		Trouble, minor fixes for (mostly) harmless server errors
...
new g_iSur1[19];	//survivors, primary
new g_iSur2[19];	//survivors, secondary
new g_iInf1[19];	//boomer
new g_iInf2[19];	//tank
new g_iConfirm[19];	//check if perks are confirmed, to prevent mid-game changing abuses
new g_iInf3[19];	//smoker
new g_iInf4[19];	//hunter
...
new g_iSpiritCooldown[19];
//used to track the timers themselves
new Handle:g_iSpiritTimer[19];
...
new Float:g_flDTNextTime[19] = -1.0;
...
public OnPluginStart()
{
	...
	HookEvent("player_spawn", Event_PlayerSpawn);					//twister
...
CreateConvars()
{
	...
	g_hBlind_enable = CreateConVar(
		...
		"1" ,
		"Blind Luck perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hTongue_enable = CreateConVar(
		...
		"1" ,
		"Tongue Twister perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hDrag_enable = CreateConVar(
		...
		"1" ,
		"Drag and Drop perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hBot_Inf1 = CreateConVar(
		...
		"1" ,
		"Bot preferences for boomer perks: 1 = barf bagged, 2 = blind luck, 3 = dead wreckening" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hBot_Inf3 = CreateConVar(
		...
		"2" ,
		"Bot preferences for smoker perks: 1 = tongue twister, 2 = squeezer, 3 = drag and drop" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
	...
	g_hBot_Inf2 = CreateConVar(
		...
		"2" ,
		"Bot preferences for tank perks: 1 = adrenal glands, 2 = juggernaut, 3 = metabolic boost, 4 = storm caller, 5 = double the trouble" ,
		FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY );
...
public Event_RoundStart (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	ResetConVar(FindConVar("tongue_allow_voluntary_release"),true,true);
	...
	{
		SetConVarInt(FindConVar("survivor_allow_crawling"),0,true,true);
		SetConVarFloat(FindConVar("survivor_crawl_speed"),30.0,true,true);
	}
...
public Event_PlayerTransitioned (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//and show the menu
	g_iConfirm[iCid]=0;
	//SendPanelToClient(Menu_Top(iCid),iCid,Menu_ChooseSubMenu,MENU_TIME_FOREVER);
	CreateTimer(1.0,Timer_ShowTopMenu,iCid);
...
public Action:TimerPerks (Handle:timer, any:data)
{
	...
	for (new iI=1 ; iI<=18 ; iI++)
	{
		...
		//only run these commands if player is in-game
		if (IsClientInGame(iI)==true
			&& IsFakeClient(iI)==true)
		{
			g_iConfirm[iI]=1;
			//sur1: spirit
			g_iSur1[iI] = g_iBot_Sur1;
			//sur2: hard to kill
			g_iSur2[iI] = g_iBot_Sur2;
			//boomer: barf bagged
			g_iInf1[iI] = g_iBot_Inf1;
			//tank: juggernaught
			g_iInf2[iI] = g_iBot_Inf2;
			//smoker: squeezer
			g_iInf3[iI] = g_iBot_Inf3;
			//hunter: efficient killer
			g_iInf4[iI] = g_iBot_Inf4;

			//----DEBUG----
			//PrintToChatAll("\x03-bot perks for %i",iI);
		}
	}
...
public Action:Event_PlayerHurtPre (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (iAtt==0)
	{
			...
			&& g_iInf1[g_iSlimerLast]==3)
		{
			...
			InfToSurDamageAdd(event,iVic, RoundToCeil(iDmgOrig * g_flDead_dmgmult) ,iDmgOrig,true);
		}
		return Plugin_Continue;
	}
		...
		&& g_iConfirm[iAtt]==1)
	{
		...
		if (StrEqual(st_wpn,"hunter_claw"))
		{
				...
				&& g_iDisabling[iAtt]==1)
			{
				...
				InfToSurDamageAdd(event,iVic, iDmgAdd ,GetEventInt(event,"dmg_health"),true);
				return Plugin_Continue;
			}
...
InfToSurDamageAdd (Handle:event, any:iVic, any:iDmgAdd, any:iDmgOrig, bool:UpdateEvent)
{
	...
	for (new iI=1 ; iI<=18 ; iI++)
	{
			...
			&& IsPlayerAlive(iI)==true)
		{
			...
			if (iI==iCid)
			{
				iCrawlClient=iI;
				if (g_iSpirit_crawling==1)
					SetConVarInt(FindConVar("survivor_allow_crawling"),1,true,true);
				continue;
			}
...
Event_Confirm_Unbreakable (iCid)
{
		...
		&& TC==2)
	{
		if (iHP>100
			...
			//SetEntProp(iCid,Prop_Data,"m_iHealth", iHP+g_iUnbreak_hp );
		PrintHintText(iCid,"Unbreakable: bonus health!");
...
public Action:Event_ReviveBeginPre (Handle:event, const String:name[], bool:dontBroadcast)
{
		...
		&& g_iConfirm[iCid]==1)
	{
		...
		SetConVarFloat(FindConVar("survivor_revive_duration"), g_flReviveTime * g_flHelpHand_timemult ,true,true);
		return Plugin_Continue;
	}
	...
	else
	{
		...
		SetConVarFloat(FindConVar("survivor_revive_duration"),g_flReviveTime,true,true);
		return Plugin_Continue;
	}
}
...
PR_Calculate ()
{
	g_iPR_smg		=	RoundToNearest( 480 * g_flPack_ammomult );
	g_iPR_shotgun	=	RoundToNearest( 128 * g_flPack_ammomult );
	g_iPR_rifle		=	RoundToNearest( 360 * g_flPack_ammomult );
	g_iPR_sniper	=	RoundToNearest( 180 * g_flPack_ammomult );
}
...
public Event_AmmoPickup (Handle:event, const String:name[], bool:dontBroadcast)
{
	new iCid=GetClientOfUserId(GetEventInt(event,"userid"));
	if (iCid==0) return;
	if (g_iSur2[iCid]==3 && g_iConfirm[iCid]==1)
	{
		new iAmmoO=FindDataMapOffs(iCid,"m_iAmmo");
...
MA_Clear ()
{
	...
	for (new iI=1 ; iI<=18 ; iI++)
	{
		...
		if (IsValidEntity(iI)==false) continue;
...
public Event_PlayerNowIt (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		//SendConVarValue(iVic,FindConVar("sv_cheats"),"1");
		//ClientCommand(iVic,"hidehud 64");
...
public Action:Event_AbilityUsePre (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (StrEqual(st_ab,"ability_tongue",false)==true)
	{
		//check for twister
		if (g_iInf3[iCid]==1)
			SetConVarFloat(FindConVar("tongue_fly_speed"), g_flTongueFlySpeed*g_flTongue_speedmult ,true,true);
		else
			SetConVarFloat(FindConVar("tongue_fly_speed"),g_flTongueFlySpeed,true,true);
		return Plugin_Continue;
	}
	...
	{
		if (g_iInf1[iCid]==2)
			SetConVarFloat(FindConVar("z_vomit_interval"), g_flVomitInt*g_flBlind_cdmult ,true,true);
		else
			SetConVarFloat(FindConVar("z_vomit_interval"),g_flVomitInt,true,true);
		return Plugin_Continue;
	}
	return Plugin_Continue;
}
...
public Action:Event_TongueGrabPre (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_iConfirm[iCid]==1)
	{
		...
		case 1:
			{
				SetConVarFloat(FindConVar("tongue_victim_max_speed"), g_flTongueSpeed*g_flTongue_pullmult ,true,true);
		...
		default:
			{
				SetConVarFloat(FindConVar("tongue_victim_max_speed"),g_flTongueSpeed,true,true);
...
public Event_PlayerSpawn (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//----DEBUG----
	//PrintToChatAll("\x03spawn check init, stored att \x01%i\x03, stored vic \x01%i",g_iSmokeBomber_att[iCid],g_iSmokeBomber_vic[iCid]);
	...
	if (StrContains(st_class,"smoker",false) != -1)
	{
		...
		if (g_iInf3[iCid]==1
			&& g_iConfirm[iCid]==1)
		{
			SetConVarFloat(FindConVar("tongue_range"), g_flTongueRange*g_flTongue_rangemult ,true,true);
		...
		{
			SetConVarFloat(FindConVar("tongue_range"),g_flTongueRange,true,true);
	...
	else if (StrContains(st_class, "boomer", false) != -1)
	{
		...
		{
			SetConVarFloat(FindConVar("z_vomit_fatigue"), g_flVomitFatigue ,true,true);
		}
...
public Event_TongueGrab (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (iCid==0) 
	{
		SetConVarFloat(FindConVar("tongue_hit_delay"),g_flTongueHitDelay,true,true);
		SetConVarInt(FindConVar("tongue_allow_voluntary_release"),0,true,true);
		SetConVarFloat(FindConVar("tongue_player_dropping_to_ground_time"),g_flTongueDropTime,true,true);
		return ;
	}
		...
		&& g_iConfirm[iCid]==1)
	{
		...
		SetConVarFloat(FindConVar("tongue_hit_delay"), g_flTongueHitDelay*g_flDrag_cdmult ,true,true);
		SetConVarInt(FindConVar("tongue_allow_voluntary_release"),1,true,true);
		SetConVarFloat(FindConVar("tongue_player_dropping_to_ground_time"),0.2,true,true);
		return ;
	}
		...
		case 1:
			{
				...
				SetConVarFloat(FindConVar("z_tank_throw_interval"),	g_flRockInterval *g_flAdrenal_throwcdmult	,true,true);
				SetConVarFloat(FindConVar("z_tank_throw_force"),	g_flRockSpeed *g_flAdrenal_rockspeedmult	,true,true);
				SetConVarFloat(FindConVar("tank_swing_interval"),	g_flSwingInterval1 *g_flAdrenal_punchcdmult	,true,true);
				SetConVarFloat(FindConVar("tank_swing_miss_interval"),	g_flSwingInterval2 *g_flAdrenal_punchcdmult	,true,true);
				SetConVarFloat(FindConVar("z_tank_attack_interval"),	g_flSwingInterval3 *g_flAdrenal_punchcdmult	,true,true);
		...
		case 3:
			{
				...
				SetConVarFloat(FindConVar("z_tank_speed_vs"), g_flTankSpeed*g_flMetabolic_speedmult ,true,true);
				if (IsFakeClient(iCid)==false)
...
public Action:Event_Tank_Frustrated(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//reset tank attack intervals
	SetConVarFloat(FindConVar("z_tank_throw_interval"),g_flRockInterval,true,true);
	SetConVarFloat(FindConVar("z_tank_throw_force"),g_flRockSpeed,true,true);
	SetConVarFloat(FindConVar("tank_swing_interval"),g_flSwingInterval1,true,true);
	SetConVarFloat(FindConVar("tank_swing_miss_interval"),g_flSwingInterval2,true,true);
	SetConVarFloat(FindConVar("z_tank_attack_interval"),g_flSwingInterval3,true,true);
	//reset tank speed
	SetConVarFloat(FindConVar("z_tank_speed_vs"),g_flTankSpeed,true,true);
...
public Action:MenuOpen_OnSay(iCid, args)
{
	...
	if (StrEqual(st_chat,"!perks",false)==true)
	{
			...
			case 2:
				{
					...
					if (g_iL4D_GameMode==2)
						...
						iBuff=g_iHelpHand_buff;
					Format(st_perk,128,"Helping Hand (revive others faster, give %i bonus buffer)",iBuff);
				}
			case 3:
		...
		switch (g_iInf4[iCid])
		{
			...
			case 5:
				Format(st_perk,128,"Speed Demon (altered time rate, %i%% faster)", RoundToNearest( (g_flSpeedDemon_rate - 1) * 100 ) );
			default:
				st_perk="Not set";
		}
...
public Action:Debug_OnSay(iCid, args)
{
	...
	if (StrEqual(st_chat,"debug anim",false)==true)
	{
		...
		CreateTimer(0.2,Debug_Timer,iCid,TIMER_REPEAT);
...
public Action:Debug_Timer (Handle:timer, any:iCid)
{

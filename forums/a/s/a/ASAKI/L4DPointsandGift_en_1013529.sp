...
new istank[PLAYERS + 7];
new buyitem[PLAYERS + 5];
new canspawn;
new burntimeleft[PLAYERS + 6];
new sburntimeleft[PLAYERS + 6];
new hasincendiary[PLAYERS + 6];
new hassuperburn[PLAYERS + 6];
...
public Action:TimerUpdate(Handle:timer)
{
	...
	if(pointson)
	{
		if (pointstimer >= GetConVarInt(pointsadvertisingticks) * pointsremindtimer)
		{
			...
			if(advertising == 2)
			{
				PrintToChatAll("[SM] You can get item points in this server to buy things. Type !usepoints to use them.");
			}
			else if(advertising == 1)
			{
				PrintToChatAll("[SM] Type !usepoints to use your item points.");
	...
	switch(buyitem[client])
	{
		...
		case 39: //oxygen tank
		{
		   if (points[client] >= GetConVarInt(oxygenpoints))
			{
				//Give the player an oxygen tank
        ...
        if(itemNum == 0)
		{
			switch(buyitem[client])
			{
				...
				case 39: //refill ammo
				{
				   if (points[client] >= GetConVarInt(refillpoints))
					{
						//Give the player an refill ammo
						FakeClientCommand(client, "give ammo");
						points[client] -= GetConVarInt(refillpoints);
					}
					else

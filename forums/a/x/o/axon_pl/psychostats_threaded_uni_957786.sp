/*******************************************************************************
	...
	Cvars:
		...
		psychstats_version - 			Plugin version
...
public OnPluginStart()
{
	...
	cvarVersion = CreateConVar("psychstats_version", PLUGIN_VERSION, _, FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);	
...
public ReadConfigCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	{
		LogError("Psychostats -> Config: Can`t find uniqueid var in config table");
		return;
	}
		...
		else if (type == REQUEST_PLACE || type == REQUEST_RANK)
		{
			...
			if (cvarValue >= 2)
			{
				...
				else if (type == REQUEST_RANK)
				{
					...
					{
						if (GetConVarInt(cvarDetails) == 1) {
							PrintToChat(client, "%t","RankDetailsInChat", playerNick, playerRank, playerSkill, GREEN, LIGHTGREEN, YELLOW, GREEN, YELLOW, GREEN, YELLOW, all);
						} else if (GetConVarInt(cvarDetails) == 2) {
						
							PrintToChat(client, "%t", "RankDetailsInChatFull", playerNick, playerRank, playerSkill, playerKills, playerDeaths, killPerDeath, days , hours, mins, GREEN, LIGHTGREEN, YELLOW, LIGHTGREEN,  YELLOW, LIGHTGREEN, YELLOW, LIGHTGREEN, YELLOW, LIGHTGREEN, YELLOW, LIGHTGREEN, YELLOW,all);
						}
					}
					// get convar for full stats URL
					// If url is null, we won't print anything.
...
public ProcessStatsMeRequestCallback3(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if(SQL_FetchRow(hndl))
	{
		...
		Format(value, sizeof(value), "%t", "Rank",  playerRank, all);
		...
		DrawPanelText(statsMePanel, value);
		Format(value, sizeof(value), "%t", "FFKills",  ffKills);
		DrawPanelText(statsMePanel, value);		
		Format(value, sizeof(value), "%t", "StatsMe");
		DrawPanelItem(statsMePanel, value);
...
public ProcessSessionRequestCallback3(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if(SQL_FetchRow(hndl))
	{
		...
		Format(value, sizeof(value), "%t", "SessionPanelTitle", playerNick);
		DrawPanelItem(sessionPanel, value);
		FormatTime(date, sizeof(date), "%Y-%m-%d %H:%M", start);
		Format(value, sizeof(value), "%t", "Start",  date);
		DrawPanelText(sessionPanel, value);
		FormatTime(date, sizeof(date), "%Y-%m-%d %H:%M", end);
		Format(value, sizeof(value), "%t", "End",  date);
		DrawPanelText(sessionPanel, value);
		Format(value, sizeof(value), "%t", "Skill",  skill);
		DrawPanelText(sessionPanel, value);
		Format(value, sizeof(value), "%t", "PrevSkill",  prevSkill);
		DrawPanelText(sessionPanel, value);
		Format(value, sizeof(value), "%t", "SkillGain",  skill - prevSkill);
		DrawPanelText(sessionPanel, value);
		Format(value, sizeof(value), "%t", "Kills",  kills);
		DrawPanelText(sessionPanel, value);
		Format(value, sizeof(value), "%t", "Deaths", deaths);
		DrawPanelText(sessionPanel, value);
		Format(value, sizeof(value), "%t", "HeadShotsWithoutPrc",  headShots);
		DrawPanelText(sessionPanel, value);		
		Format(value, sizeof(value), "%t", "FFKills",  ffKills);
		DrawPanelText(sessionPanel, value);		
		Format(value, sizeof(value), "%t", "FFDeaths", ffDeaths);
		DrawPanelText(sessionPanel, value);
		Format(value, sizeof(value), "%t", "Shots",  shots);
		DrawPanelText(sessionPanel, value);
		Format(value, sizeof(value), "%t", "HitsWithoutAcc",  hits);
		DrawPanelText(sessionPanel, value);		
		Format(value, sizeof(value), "%t", "Damage",  damage);
		DrawPanelText(sessionPanel, value);
		Format(value, sizeof(value), "%t", "TotalBonus",  totalBonus);
		DrawPanelText(sessionPanel, value);		

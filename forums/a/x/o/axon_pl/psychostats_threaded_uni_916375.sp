...
//
//	0.8.1: 29 August 2009 - Axon (axon@thcc.pl - http://www.thcc.pl)
//		- fixed bug causing table prefix and db config name to be readed inproperly
//		- added support for team chat (command now works in tema chat)
//
//
//******************************
...
public OnConfigsExecuted()
{
	...
	LogError("Psychostats -> Database: Table prefix readed from config file: %s", strDbPrefix);
	LogError("Psychostats -> Plugin version id %s", PLUGIN_VERSION);
	StartSQL();
}
...
StartSQL()
{
	...
	LogError("Psychostats -> Database: Db config name readed from config file: %s", connectionString);

...
int MEOwner[2048+1] = {0, ...};
int MEIndex[MAXPLAYERS+1] = {0, ...};
int iModelIndex[MAXPLAYERS+1] = {0, ...};

int LastButton[MAXPLAYERS+1];
int LastWeapon[MAXPLAYERS+1];
int ItemInfo[MAXPLAYERS+1][5][4];
int ItemAttachEnt[MAXPLAYERS+1][5];
int BackupItemInfo[MAXPLAYERS+1][5][4];
...
int ClientSameWepAmmo[MAXPLAYERS + 1];

#define MAX_ALLOW_L4D1 			9
#define MAX_ALLOW_L4D2			26
#define MAX_L4D2_PROP			6

static char allow_item[MAX_ALLOW_L4D2][] =
{
	"weapon_rifle",
	"weapon_autoshotgun",
	...
	"weapon_sniper_scout",
	//"weapon_chainsaw",		2ND SLOT
	//"weapon_pistol_magnum",	2ND SLOT
	"weapon_vomitjar",
...
//CHECK PICKUP PROP LIST
static char allow_prop[MAX_L4D2_PROP][] =
{
	"weapon_cola_bottles",
...
public void OnClientPutInServer(int client)
{
	if(!IsFakeClient(client))
	{
		...
		if (GetConVarInt(l4d_me_slot[0]))
		{
			SDKHook(client, SDKHook_WeaponEquip, OnWeaponCanUse);	//GET DROP WEP AMMO
			SDKHook(client, SDKHook_WeaponEquipPost, OnWeaponCanUsePost);	//SET PICK UP WEP AMMO TO PREVENT RECOVER AMMO FROM PICK UP 2 SAME WEAPON
		}
		SDKHook(client, SDKHook_WeaponCanUse, PickUpWeapon);			//PUSH HELD WEAPON TO 2ND SLOT AND PICKUP WEP TO 1ST
	}
}
...
public Action OnWeaponCanUse(int client, int weapon)	//GET DROP PICK UP WEAPON REV AMMO
{
	if (IsValidEdict(weapon))
	{
		...
		if (StrEqual(wepName, ItemName[client][0], false))
		{
			ClientSameWepAmmo[client] = GetEntProp(weapon, Prop_Send, "m_iExtraPrimaryAmmo"); 
		}
	}
	return Plugin_Continue;
} 

public Action OnWeaponCanUsePost(int client, int weapon)	//IF SAME PRIMARY WEAPON, LETS CHANGE PICKUP AMMO 
{
	if (IsValidEdict(weapon))
	{
		if(ClientSameWepAmmo[client] != 0)
		{	
			...
			RequestFrame(ChangeRevAmmo, data);
		}
	}
	return Plugin_Continue;
} 

public void ChangeRevAmmo(any thing) 
{
	DataPack data = view_as<DataPack>(thing);
	int client = data.ReadCell();
	int weapon = EntRefToEntIndex(data.ReadCell());
	L4D_SetReserveAmmo(client, weapon, ClientSameWepAmmo[client]);
	ClientSameWepAmmo[client] = 0;
} 

public Action PickUpWeapon(int client, int weapon)	//IF WE DONT HAVE WEAPON IN 2ND SLOT, LET PUSH PICKUP WEAPON TO 1ST THEN CURRENT WEP TO 2ND, UNFORTUNATELY CANT PICK UP SAME ITEM WITH THIS METHOD, SO WE WILL USE RUNCMD IN_USE
{
	if (IsValidEdict(weapon))
	{
		
		//PrintToChatAll("%d", weapon);
		char wepName[64];
		GetEntityClassname(weapon, wepName, sizeof(wepName));
		
		if(StrEqual(wepName, "weapon_pistol") && !IsFakeClient(client))
		{
			return Plugin_Continue;
		}
		
		//CHECK PICKUP PROP
		for(int i = 0 ; i < sizeof(allow_prop) ; ++i)
		{
			if(StrEqual(wepName, allow_prop[i]) && !IsFakeClient(client) && i < sizeof(allow_prop))
		{
			return Plugin_Continue;
		}
		}
		
		int slot = L4D2_GetIntWeaponAttribute(wepName, L4D2IWA_Bucket);
		
		if (GetConVarInt(l4d_me_slot[slot]) == 0 || (!L4D2Version && slot == 1))
			return Plugin_Continue;

		if(StrEqual(ItemName[client][slot], "") && !IsFakeClient(client))
		{
...
void SwapItemNone(int client, int slot, int OldWeapon = 0) //FOR PUT EQUIP TO SECOND SLOT
{
	//PrintToChatAll("set");
	int Clip = 0;
	int Ammo = 0;
	...
	if (OldWeapon > 0)
	{
		...
		//PrintToChatAll("%s", OldWeaponName);
...
//DOUBLE TAP MODE
	...
	if (buttons & IN_USE && (GetEngineTime() - fTime[client] > 0.5))	
	{
		...
		int item = GetClientAimTarget(client, false);
		if (item > 0)
		{
			if (IsValidEdict(item) && IsValidClient(client) && !IsFakeClient(client))
			{
				char wepName[64];
				GetEntityClassname(item, wepName, sizeof(wepName));
				
				if(!CheckAllowItem(wepName))
				{
					return Plugin_Continue;
				}
				if(StrContains(wepName, "_spawn", false) == -1)
					return Plugin_Continue;
				...
				if(StrEqual(ItemName[client][slot], "") && !IsFakeClient(client) && slot != 1)	//BECAUSE SLOT SECONDARY START WITH PISTOL SO IGNORE THAT
				{
					...
					{
						char playerWeb[64];
						GetEntityClassname(ent, playerWeb, sizeof(playerWeb));
						
						float vPos[3], ePos[3];
						GetClientAbsOrigin(client, vPos);
						GetEntPropVector(item, Prop_Send, "m_vecOrigin", ePos);
						if(StrEqual(wepName, playerWeb) && GetVectorDistance(vPos, ePos) <= 120.0)
						{
							SwapItemNone(client, slot, ent);
						}
...
bool CheckAllowItem(char[] classname)
{
	if(L4D2Version)
	{
		for( int i = 0 ; i < sizeof(allow_item) ; ++i) 
		{
			if (StrContains(classname, allow_item[i], false) != -1)
			{
				return true;
			}
		}
	...
	{
		for( int i = 0 ; i < MAX_ALLOW_L4D1 ; ++i) 
		{
			if (StrContains(classname, allow_item[i], false) != -1)
			{
				return true;
...
public Action ePlayerDeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	//SEEM THIS ONE CAUSE PLAYER STILL KEEP 2ND SLOT ITEM AFTER DEATH SO JUST COMMENT THAT, JUST IN CASE IF SOMETHING BAD HAPPEN, UNCOMMENT IT, DUNNO THATS CAUSE BUG THO~, BUT FINE FOR ME, MAYBE BECAUSE I DONT USE ITEM ATTACHMENT
	
	if(!IsValidClient(client))
		return;

	//FIX SECOND WEP AFTER DEATH
	RemoveItemAttach(client,-1);
...
void SetClientWeaponInfo_l4d2(int client, int ent , int ammo, int clip, int upgradeBit, int upammo)
{
	if (ent > 0 && GetClientTeam(client) == 2)
	{
		...
		if (StrEqual(sWeapon, "weapon_rifle") || StrEqual(sWeapon, "weapon_rifle_sg552") || StrEqual(sWeapon, "weapon_rifle_desert") || StrEqual(sWeapon, "weapon_rifle_ak47") || StrEqual(sWeapon, "weapon_rifle_m60"))
			L4D_SetReserveAmmo(client, ent, ammo);
...
void GetClientWeaponInfo_l4d2(int client, int ent, int &ammo, int &clip, int &upgradeBit, int &upammo)
{
	if (ent > 0 && GetClientTeam(client) == 2)
	{
		...
		if (StrEqual(sWeapon, "weapon_rifle") || StrEqual(sWeapon, "weapon_rifle_sg552") || StrEqual(sWeapon, "weapon_rifle_desert") || StrEqual(sWeapon, "weapon_rifle_ak47") || StrEqual(sWeapon, "weapon_rifle_m60"))
			ammo = L4D_GetReserveAmmo(client, ent);
		else if (StrEqual(sWeapon, "weapon_smg") || StrEqual(sWeapon, "weapon_smg_silenced") || StrEqual(sWeapon, "weapon_smg_mp5"))
			ammo = L4D_GetReserveAmmo(client, ent);
		else if (StrEqual(sWeapon, "weapon_pumpshotgun") || StrEqual(sWeapon, "weapon_shotgun_chrome"))
			ammo = L4D_GetReserveAmmo(client, ent);

	...
	if (IsPlayerAlive(attacker))
	{
           ...
           if(ZR_IsClientHuman(attacker) && ZR_IsClientZombie(client) && contar > 0)
           {
             ...
             if(StrEqual(weapon, "knife", false))
             {
                 ...
                 CPrintToChat(attacker, "{BLACK}[{GREEN}B{LIME}z{GREEN}M{BLACK}_{RED}K{WHITE}BOOM {ORANGE}TIMER{BLACK}]{WHITE}Congratulations {LIME}%s :) {WHITE}You have %d seconds to escape the zombie or you will die!",nome,g_Time);
                 new Handle:pack;
...
stock SetClientShake( index, Float:Amplitude, Float:Radius, Float:Duration, Float:Frequency )
{
    ...
    if( DispatchSpawn( Ent ) )
    {
        ...
        SetVariantString( "spawnflags 13" );

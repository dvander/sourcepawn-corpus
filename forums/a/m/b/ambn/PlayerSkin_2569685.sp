...
#define PLUGIN_VERSION "4.5.1 (With Multi Flag Support)"
...
public Action PlayerSpawn(Event event, const char[] name, bool dontBroadcast) 
{
	...
	*/
	...
	if(!g_bUserHasArms[client] && !g_bHasAsked[client])
	{
			...
			do {
				...
				if (StrEqual(item, UniqueId)) 
				{
					...
					*/
					...
					if(!StrEqual(ArmPath, "", false) && GetConVarBool(g_cAskForArms))
					{
						AskForArms(param1);
					}
					...
					if(GetConVarBool(g_sReset) && !GetConVarBool(g_cAskForArms))
					{
						ResetArms(param1);
					}
					if(g_cSaveUserSkin)

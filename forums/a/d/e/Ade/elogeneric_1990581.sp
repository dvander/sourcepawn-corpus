...
public SQLQueryCount(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	Format(buffer, sizeof(buffer), "ELO Ranking stats:\nRating: %i\nRank: %s (of %i)\nKPD: %.2f", rating[client], rankstr, rankcount, kpd);
	...
	if(strcmp(text[startidx], "/elo_notify", false)==0 || strcmp(text[startidx], "elo_notify", false)==0) {
		...
		if(notify[client]==false) {
			...
			if(colour) {
				...
				PrintToChat(client, "Rank notifications disabled. Say /elo_notify again to enable them.");
			}
		...
		if(StrContains(text[startidx], "rank", false)!=-1) {
			...
			decl String:buffer[64];
			Format(buffer, sizeof(buffer), "Session stats:\nRating: %i\nKills: %i\nDeaths: %i", sessionrating[client], sessionkills[client], sessiondeaths[client]);
			PrintToChat(client, buffer);
		}
		return Plugin_Handled;

...
void GetProbs()
{
	...
	g_iNegWeightSum = 0;
	int iArrSize;
	int posSize = g_bL4D2 ? POS_SIZE : POS_SIZE_1;
	int negSize = g_bL4D2 ? NEG_SIZE : NEG_SIZE_1;
	
	g_hPosWeight.GetString(sConVar, sizeof(sConVar));
	if( (iArrSize = ExplodeString( sConVar, ",", sBuffer, sizeof(sBuffer), sizeof(sBuffer[]) )) != posSize )
	{
		PrintToServer("[LB] Warning: Invalid ConVar <l4d_lootbox_positive_weights> value amount. Expected %d, found %d", posSize, iArrSize);
	...
	if( (iArrSize = ExplodeString( sConVar, ",", sBuffer, sizeof(sBuffer), sizeof(sBuffer[]) )) != negSize )
	{
		PrintToServer("[LB] Warning: Invalid ConVar <l4d_lootbox_negative_weights> value amount. Expected %d, found %d", negSize, iArrSize);

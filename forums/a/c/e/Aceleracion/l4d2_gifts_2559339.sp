...
new Handle:cvar_gift_maxcollectMap;
new Handle:cvar_gift_maxcollectRound;

new String:weapons_name[MAX_SPECIALWEAPONS][2][50] = 
{
...
new probability_weapon[MAX_SPECIALWEAPONS] = { 30, 30, 40, 40, 40, 50, 15, 5, 10};
...
new TotalGifts[MAXPLAYERS+1][MAX_TYPEGIFTS];
new AllGifts[MAXPLAYERS+1];
...
new String:g_sGifSWeapon[2000][50];
...
new gifts_collected_map;
new gifts_collected_round;
...
public Cvar_Changed7(Handle:convar, const String:oldValue[], const String:newValue[])
{
	new value = StringToInt(newValue);
	if(value > 0)
	{
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidClient(victim) && GetClientTeam(victim) == TEAM_INFECTED && IsValidClient(attacker) && GetClientTeam(attacker) == TEAM_SURVIVOR)
	{
...
SendSQLSelect(const String:query[], SQLTCallback:callback=INVALID_FUNCTION, client)
{
	if (db == INVALID_HANDLE)
	{
		return;
...
public SQLSelectCallback(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	if (db == INVALID_HANDLE)
	{
		return;
...
NotifyGift(client, type, Score, gift = -1)
{
	...
	else if(type == TYPE_SPECIAL)
	{
		...
		if(StrEqual(g_sGifSWeapon[gift], ""))
		{
			...
			for(new i=0; i < MAX_SPECIALWEAPONS; i++)
			{
...
DropGift(client, String:type[] = "random")
{
...
public AddScore(client, Score, type)
{

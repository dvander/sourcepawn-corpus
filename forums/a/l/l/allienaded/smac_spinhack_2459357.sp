...
new Handle:g_hCvarSpinhackBan = INVALID_HANDLE;
...
new g_iSpinDetections[MAXPLAYERS+1];
new g_iSpinbotBan = 0;
...
public OnPluginStart()
{
	...
	// Convars.
	g_hCvarSpinhackBan = SMAC_CreateConVar("smac_spinhack_ban", "4", "Number of spinbot detections before a player is banned. (0 = Never ban)", FCVAR_PLUGIN, true, 0.0);
	OnSettingsChanged(g_hCvarSpinhackBan, "", "");
	HookConVarChange(g_hCvarSpinhackBan, OnSettingsChanged);
...
public Action:Timer_DecreaseCount(Handle:timer, any:userid)
{
	...
	if (IS_CLIENT(client) && g_iSpinDetections[client])
	{
		g_iSpinDetections[client]--;
	}
...
Spinhack_Detected(client)
{
	...
	{
		// Expire this detection after 2.5 minutes.
		CreateTimer(150.0, Timer_DecreaseCount, GetClientUserId(client));
		...
		if (g_iSpinbotBan && g_iSpinDetections[client] >= g_iSpinbotBan)
		{

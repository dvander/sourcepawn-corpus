...
void StartVoteKick(int client, int target)
{
	...
	int iTeam = g_iVoteIssuerTeam = GetClientTeam(client);	// update the global variable before CPrintHintTextToTeam. What happened in a live game was that the global variable g_iVoteIssuerTeam contained the "wrong" team id 
															// of a previous vote, so the CPrintHintTextToTeam message was shown to the other team instead of the issuing team, which is of course wrong.
	if( g_bCvarShowKickReason )
	{
		char sReasonEng[32];
		...
		if( bVoteSuccess ) {
			...
			if( iTarget && IsClientInGame(iTarget) ) {
				if( g_bCvarShowKickReason )
				{
					...
					KickClient(iTarget, "%t", "kickedbyconsole_voted_off"); // Kicked by vote (without reason), L4D-msg: "Kicked by console : You have been voted off" & on trying to join: "You have been kicked from session"
				}
			}
	...
	if( strcmp(auth, "BOT") != 0 ) {
		if( hMapSteam.GetString(auth, sTime, sizeof(sTime)) ) {
			...
			if( GetTime() - iTime < g_iCvarKickTime ) { // 1 hour
				KickClient(client, "%t", "kicked_steamid_is_banned", auth);
				if( g_bCvarLog )
...
// Update: in order to minimize possible sources of error, the team ID is no longer derived from a global variable, but is given as a parameter

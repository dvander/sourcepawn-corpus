...
/*
	...
	Credits:
	 ...
	 - alliedfront - for fixing kicking bot exploit
...
bool g_bVeto, g_bVotepass, g_bVoteInProgress, g_bVoteDisplayed, g_bShowReason;
...
void GetCvars()
{
	g_bShowReason = g_hCvarShowKickReason.BoolValue;
}
...
{
	static int ft_block, ft_anc_block, ft_reason;
	int ft, ft1, ft2;
...
void CreateVotekickMenu(int client)
{
	...
	bool bShowBots = g_hCvarShowBots.BoolValue;
	bool bShowSelf = g_hCvarShowSelf.BoolValue;
	bool bIsVersus = IsVersus();
	...
	if( hSnap )
	{
		...
		for( int i = 0; i < hSnap.Length; i++ )
		{
			...
			if( (g_hCvarKickTime.IntValue - (GetTime() - iTime)) < 0 ) {	// leftover expired entries (negative time) of kicked players are deleted, for reasons of clarity
			...
			if( !(bIsVersus && iIssuerTeam != iPlayerTeam) ) {	
				Format(menuItem, sizeof(menuItem), "%s %s %T", g_sCharKicked, name, "time_left", client, (g_hCvarKickTime.IntValue - (GetTime() - iTime)) / 60);  
...
bool IsVoteAllowed(int client, int target)
{
	...
	if( target != -1)
	{
		...
		if( client == target && HasVoteAccessFlag(client) )
			return true;
	...
	if( iClients < ( IsVersus() ? g_hMinPlayersVersus.IntValue : g_hMinPlayers.IntValue ) ) {
		CPrintToChat(client, "%t", "not_enough_players", ( IsVersus() ? g_hMinPlayersVersus.IntValue : g_hMinPlayers.IntValue )); // "Not enough players to start the vote. Required minimum: %i"
		LogVoteAction(client, "[DENY] Reason: Not enough players. Now: %i, required: %i.", iClients, ( IsVersus() ? g_hMinPlayersVersus.IntValue : g_hMinPlayers.IntValue ) );
		return false;
	}
	...
	if( target != -1 )
	{
		...
		if( InDenyFile(target, g_hArrayVoteBlock) )	// allow to vote everybody against clients who located in deny list (regardless of vote access flag)
													// Unkick: Exception reported: Client index -1 is invalid" -> check target != -1
		{
			return true;
		}
		
		if( !g_hCvarShowBots.BoolValue && IsFakeClient(target) ) //Unkick: "Exception reported: Client index -1 is invalid" -> check target != -1
		{
			return false;
		}	
	}
				...
				const int[][] item_info )	// Array of items, sorted by count.  Use VOTEINFO_ITEM defines.
{
	...
	if ( g_hCvarShowVoteDetails.BoolValue && (!g_bVotepass || !g_bVeto) )
	...
	if( g_bVotepass || g_bVeto ) {
		...
		if( !IsVoteInProgress() ) {
			...
			if( IsVersus() )
			{
				...
				menu.DisplayVote(iClients, iCount, g_hCvarTimeout.IntValue);
			}
			else {
...
public void OnClientAuthorized(int client, const char[] auth)
{
	...
	if( strcmp(auth, "BOT") != 0 ) {
		if( hMapSteam.GetString(auth, sTime, sizeof(sTime)) ) {
			iTime = StringToInt(sTime);
			if( GetTime() - iTime < g_hCvarKickTime.IntValue ) { // 1 hour
				KickClient(client, "Kicked for violation");
				if( g_hCvarLog.BoolValue )
					LogToFileEx(g_sLog, "[DENY] %N | %s cannot join because kicked. Time left = %i min.", client, auth, (g_hCvarKickTime.IntValue - (GetTime() - iTime)) / 60);
			}
			else {
...
bool IsVersus()
{
	...
	return strcmp(gt, "versus", false) == 0;
}

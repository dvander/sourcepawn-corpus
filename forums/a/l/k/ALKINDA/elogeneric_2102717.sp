...
public SQLQueryConnect(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if(hndl==INVALID_HANDLE) {
		...
		if(!SQL_MoreRows(hndl)) {
			if(sqlite) {
				Format(query, sizeof(query), "INSERT INTO elostats VALUES('%s', '%s', 0, 0, 0, 0)", clientid, clientname);
				SQL_TQuery(db, SQLErrorCheckCallback, query);
			} else {
				Format(query, sizeof(query), "INSERT INTO elostats (steamid, name, rating, kills, deaths, notify) VALUES ('%s', '%s', 0, 0, 0, 0)", clientid, clientname);
				SQL_TQuery(db, SQLErrorCheckCallback, query);
			}
			rating[client] = 0;
			kills[client] = 0;
...
public SQLQueryCount(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	new Float:kpd = deaths[client]==0?1.0:float(kills[client])/float(deaths[client]);
	...
	SetPanelTitle(panel, "Rank:");
	Format(buffer, sizeof(buffer), "Rating: %i", rating[client]);
	DrawPanelText(panel, buffer);
	Format(buffer, sizeof(buffer), "Rank: %s (of %i)", rankstr, rankcount);
	DrawPanelText(panel, buffer);
	Format(buffer, sizeof(buffer), "Kills: %i", kills[client]);
	DrawPanelText(panel, buffer);
	Format(buffer, sizeof(buffer), "Deaths: %i", deaths[client]);
	DrawPanelText(panel, buffer);
...
public SQLQueryTop10(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if(hndl==INVALID_HANDLE) {
		...
		while(SQL_FetchRow(hndl)) {
			...
			Format(buffer, sizeof(buffer), "%i. %s [%s]",num, qname, qrating);
			num++;
			DrawPanelText(panel, buffer);
			i++;
		}
...
public Action:Command_say(client, args) {
	...
	if(strcmp(text[startidx], "/elo_notify", false)==0 || strcmp(text[startidx], "elo_notify", false)==0) {
		...
		if(StrContains(text[startidx], "rank", false)!=-1) {
			...
			Format(query, sizeof(query), "SELECT * FROM elostats WHERE steamid=%i", clientid);

...
. Tank rock sniper.
. Known issues:
. Tank can get very high scores -> most likely caused by a survivor that
got swinged off the roof.
...
#define INF_ID_WITCH 7
#define INF_ID_TANK 8
#define INF_ID_CHARGER 6
#define INF_ID_JOCKEY 5
#define INF_ID_SPITTER 4
...
{
name = "Custom L4D Stats",
author = "muukis (original author msleeper)",
description = "Player Stats and Ranking in Left 4 Dead Co-op and Versus",
...
CreateConVar("sm_l4dstats_version", PLUGIN_VERSION, "Custom L4D Stats Version (Beta)", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
cvar_InfectedDamage = CreateConVar("sm_l4dstats_infected_damage", "1", "The amount of damage inflicted to Survivors to earn 1 point", FCVAR_PLUGIN, true, 1.0);
...
// Hunter point Cvars
cvar_HunterPerfectPounceDamage = CreateConVar("sm_l4dstats_perfectpouncedamage", "25", "The amount of damage from Perfect Pounce to earn success points", FCVAR_PLUGIN, true, 15.0, true, 25.0);
...
cvar_HunterNicePounceDamage = CreateConVar("sm_l4dstats_nicepouncedamage", "15", "The amount of damage from Nice Pounce to earn success points", FCVAR_PLUGIN, true, 1.0, true, 25.0);
...
MaxPounceDistance = 1000;
MinPounceDistance = 500;
MaxPounceDamage = 25;
...
cvar_BoomerPerfectSuccess = CreateConVar("sm_l4dstats_boomerperfectsuccess", "30", "Base score for a successful pounce", FCVAR_PLUGIN, true, 1.0);
...
HookEvent("jockey_ride_end", Event_RideSave);
HookEvent("charger_carry_end", Event_CarrySave);
...
//RegConsoleCmd("sm_l4dstats", cmd_Stats);
...
decl String:rankname[64];
GetClientName(client, rankname, 64);
...
Format(query, sizeof(query), "SELECT points + points_survivors + points_infected FROM players WHERE name = '%s'", rankname);
...
CreateTimer(20.0, Updateserver, client);
}
...
public Action:Updateserver(Handle:timer, any:client)
{
decl String:clientName[MAX_LINE_WIDTH];
GetClientName(client, clientName, sizeof(clientName));
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET servername = 'KDT Versus Rank v-lan l4d2 01' WHERE name = '%s'", clientName);
SendSQLUpdate(query);
return;
}
...
LogError("Database.cfg missing '%s' entry!", "l4dstats");
}
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i, award_protect = award_protect + %i WHERE name = '%s'", UpdatePoints, UpdatePoints, Score, ProtectedFriendlies, UserName);
SendSQLUpdate(query);
...
decl String:UserName[MAX_LINE_WIDTH];
GetClientName(data, UserName, sizeof(UserName));
Format(query, sizeof(query), "UPDATE players SET points_infected = points_infected + %i WHERE name = '%s'", Score, UserName);
SendSQLUpdate(query);
...
Format(query, sizeof(query), "SELECT points + points_survivors + points_infected FROM players WHERE name = '%s'", UserName);
...
decl String:query[512];
Format(query, sizeof(query), "SELECT name FROM players WHERE name = '%s'", UserName);
...
new String:query[512];
Format(query, sizeof(query), "INSERT IGNORE INTO players SET name = '%s'", UserName);
SQL_TQuery(db, SQLErrorCheckCallback, query);
}
...
decl String:query[512];
Format(query, sizeof(query), "UPDATE players SET lastontime = UNIX_TIMESTAMP(), %s = %s + 1, name = '%s' WHERE name = '%s'", Playtime, Playtime, Name, Name);
SendSQLUpdate(query);
}
...
len += Format(query[len], sizeof(query)-len, "WHERE name = '%s'", UserName);
SendSQLUpdate(query);
...
VictimInfType = GetEntProp(Victim, Prop_Send, "m_zombieClass");
}
else
{
...
{
VictimTeam = TEAM_INFECTED;
VictimInfType = INF_ID_TANK;
}
else if (StrEqual(VictimName, "Charger"))
{
VictimTeam = TEAM_INFECTED;
VictimInfType = INF_ID_CHARGER;
}
else if (StrEqual(VictimName, "Spitter"))
{
VictimTeam = TEAM_INFECTED;
VictimInfType = INF_ID_SPITTER;
}
else if (StrEqual(VictimName, "Jockey"))
{
VictimTeam = TEAM_INFECTED;
VictimInfType = INF_ID_JOCKEY;
}
else
return;
}
...
else if (VictimInfType == INF_ID_JOCKEY)
{
Format(InfectedType, sizeof(InfectedType), "jockey");
Score = ModifyScoreDifficultyNR(3, 2, 3);
}
else if (VictimInfType == INF_ID_CHARGER)
{
Format(InfectedType, sizeof(InfectedType), "charger");
Score = ModifyScoreDifficultyNR(6, 2, 3);
}
else if (VictimInfType == INF_ID_SPITTER)
{
...
len += Format(query[len], sizeof(query)-len, "%s WHERE name = '%s'", Headshot, AttackerName);
SendSQLUpdate(query);
...
public Action:Event_RideSave(Handle:event, const String:name[], bool:dontBroadcast)
{
if (StatsDisabled())
return;

HunterSmokerSave(GetEventInt(event, "userid"), GetEventInt(event, "victim"), 5, 2, 3, "Jockey", "award_jockey");

}
public Action:Event_CarrySave(Handle:event, const String:name[], bool:dontBroadcast)
{
if (StatsDisabled())
return;

HunterSmokerSave(GetEventInt(event, "userid"), GetEventInt(event, "victim"), 5, 2, 3, "Charger", "award_charger");
...
{
decl String:rankname[64];
GetClientName(i, rankname, 64);

Format(query, sizeof(query), "UPDATE players SET %s = %s + %i, award_tankkill = award_tankkill + 1 WHERE name = '%s'", UpdatePoints, UpdatePoints, Score, rankname);
SendSQLUpdate(query);
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i, award_pills = award_pills + 1 WHERE name = '%s'", UpdatePoints, UpdatePoints, Score, GiverName);
SendSQLUpdate(query);
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i, award_medkit = award_medkit + 1 WHERE name = '%s'", UpdatePoints, UpdatePoints, Score, GiverName);
SendSQLUpdate(query);
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i, award_friendlyfire = award_friendlyfire + 1 WHERE name = '%s'", UpdatePoints, UpdatePoints, Score, AttackerName);
SendSQLUpdate(query);
...
if (Mode == 1 || Mode == 2)
{
PrintToChat(i, "\x04[\x03RANK\x04] \x01You have earned \x04%i \x01points for completing %s Campaign (%i / 5) with \x05%i %s!", WinScore, (IsVersus ? "Versus" : "Co-op"), Maps, SurvivorCount, (IsVersus ? "players" : "survivors"));
}
else if (Mode == 3)
{
GetClientName(i, Name, sizeof(Name));
PrintToChatAll("\x04[\x03RANK\x04] \x05%s \x01has earned \x04%i \x01points for completing %s Campaign (%i / 5) with \x05%i %s!", Name, WinScore, (IsVersus ? "Versus" : "Co-op"), Maps, SurvivorCount, (IsVersus ? "players" : "survivors"));
}
}
GetClientName(i, Name, sizeof(Name));
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i, award_campaigns = award_campaigns + 1 WHERE name = '%s'", UpdatePoints, UpdatePoints, WinScore, Name);
SendSQLUpdate(query);
...
decl String:rankname[64];
GetClientName(i, rankname, 64);
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i WHERE name = '%s' ", UpdatePoints, UpdatePoints, Score, rankname);
SendSQLUpdate(query);
UpdateMapStat("points", Score);
CurrentPoints[i] = CurrentPoints[i] + Score;
}
...
decl String:query[512];
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i WHERE name = '%s'", UpdatePoints, UpdatePoints, Score, AttackerName);
SendSQLUpdate(query);
...
// Not sure if this is the point to fire SurvivorDied function or will it run it by itself (event triggered)
if (VictimHealth < 0)
Damage += VictimHealth;
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET points_infected = points_infected + %i, award_ledgegrab = award_ledgegrab + 1 WHERE name = '%s'", Score, AttackerName);
SendSQLUpdate(query);
...
//		Format(query, sizeof(query), "UPDATE players SET points_infected = points_infected - %i WHERE name = '%s'", Score, AttackerName);
//		SendSQLUpdate(query);
//
...
decl String:query[1024];
decl String:rankname[64];
GetClientName(Player, rankname, 64);
Format(query, sizeof(query), "UPDATE players SET infected_boomer_vomits = infected_boomer_vomits + 1 WHERE name = '%s'", rankname);
SendSQLUpdate(query);
UpdateMapStat("infected_boomer_vomits", 1);
...
if (PounceDistance < Min)
return;

new Dmg = RoundToNearest((((PounceDistance - float(Min)) / float(Max - Min)) * float(MaxDmg)) + 1);
...
Format(query, sizeof(query), "UPDATE players SET points_infected = points_infected + %i, award_pounce_perfect = award_pounce_perfect + 1 WHERE name = '%s'", Score, AttackerName);
Format(Label, sizeof(Label), "Death From Above");
}
else
{
Score = GetConVarInt(cvar_HunterNicePounceSuccess);
Format(query, sizeof(query), "UPDATE players SET points_infected = points_infected + %i, award_pounce_nice = award_pounce_nice + 1 WHERE name = '%s'", Score, AttackerName);
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i, award_revive = award_revive + 1 WHERE name = '%s'", UpdatePoints, UpdatePoints, Score, SaviorName);
SendSQLUpdate(query);
...
new String:UpdatePoints[32];
decl String:SaviorName[MAX_LINE_WIDTH];
GetClientName(User, SaviorName, sizeof(SaviorName));
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i%s WHERE name = '%s'", UpdatePoints, UpdatePoints, Score, AwardSQL, SaviorName);
SendSQLUpdate(query);
}
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET award_witchdisturb = award_witchdisturb + 1 WHERE name = '%s'", SaviorName);
SendSQLUpdate(query);
}
}
...
{
if (IsGameModeVersus())
Format(query, sizeof(query), "SELECT COUNT(*) FROM players WHERE points_survivors + points_infected >= %i", ClientGameModePoints[client][1]);
else
Format(query, sizeof(query), "SELECT COUNT(*) FROM players WHERE points >= %i", ClientGameModePoints[client][0]);
...
Format(query, sizeof(query), "SELECT name, playtime + playtime_versus, points + points_survivors + points_infected, kills, versus_kills_survivors, headshots FROM players WHERE name = '%s'", rankname);
...
{
decl String:query[256];
Format(query, sizeof(query), "SELECT points + points_survivors + points_infected, name FROM players ORDER BY points + points_survivors + points_infected DESC LIMIT %i,1", rank);
...
Format(query, sizeof(query), "SELECT name, playtime, points + points_survivors + points_infected, kills, headshots FROM players WHERE name = '%s'", name);
SQL_TQuery(db, DisplayRank, query, client);
}
}
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i, %s = %s + 1 WHERE name = '%s'", UpdatePoints, UpdatePoints, Score, SQLField, SQLField, SaviorName);
SendSQLUpdate(query);
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET infected_smoker_damage = infected_smoker_damage + %i WHERE name = '%s'", Damage, SaviorName);
SendSQLUpdate(query);
...
if (IsClientConnected(i) && IsClientInGame(i) && !IsClientBot(i) && GetClientTeam(i) == TEAM_SURVIVORS)
{
GetClientName(i, SaviorName, sizeof(SaviorName));

Format(query, sizeof(query), "UPDATE players SET %s = %s + %i WHERE name = '%s'", UpdatePoints, UpdatePoints, Score, SaviorName);
SendSQLUpdate(query);
UpdateMapStat("points", Score);
CurrentPoints[i] = CurrentPoints[i] + Score;
}
...
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i%s WHERE name = '%s'", UpdatePoints, UpdatePoints, Score, All4Safe, VictimName);
SendSQLUpdate(query);
UpdateMapStat("points", Score);
CurrentPoints[i] = CurrentPoints[i] + Score;
}
...
{
decl String:VictimName[MAX_LINE_WIDTH];
GetClientName(Infected[i], VictimName, sizeof(VictimName));
Format(query, sizeof(query), "UPDATE players SET points_infected = points_infected + %i WHERE name = '%s'", Score, VictimName);
SendSQLUpdate(query);
}
...
{
decl String:SaviorName[MAX_LINE_WIDTH];
GetClientName(KilledSurvivor[i], SaviorName, sizeof(SaviorName));
Format(query, sizeof(query), "UPDATE players SET points_survivors = points_survivors + %i WHERE name = '%s'", Score, SaviorName);
SendSQLUpdate(query);
}
...
{
ClientInfectedType[Client] = GetEntProp(Client, Prop_Send, "m_zombieClass");

if (ClientInfectedType[Client] != INF_ID_SMOKER
&& ClientInfectedType[Client] != INF_ID_BOOMER
&& ClientInfectedType[Client] != INF_ID_HUNTER
&& ClientInfectedType[Client] != INF_ID_TANK
&& ClientInfectedType[Client] != INF_ID_CHARGER
&& ClientInfectedType[Client] != INF_ID_JOCKEY
&& ClientInfectedType[Client] != INF_ID_SPITTER)
return;
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET infected_spawn_%i = infected_spawn_%i + 1 WHERE name = '%s'", ClientInfectedType[Client], ClientInfectedType[Client], SaviorName);
SendSQLUpdate(query);
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET points_infected = points_infected + %i, award_bulldozer = award_bulldozer + 1 WHERE name = '%s'", Score, SaviorName);
SendSQLUpdate(query);
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET infected_hunter_pounce_dmg = infected_hunter_pounce_dmg + %i, infected_hunter_pounce_counter = infected_hunter_pounce_counter + 1 WHERE name = '%s'", Damage, SaviorName);
SendSQLUpdate(query);
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET infected_tank_damage = infected_tank_damage + %i WHERE name = '%s'", Damage, VictimName);
SendSQLUpdate(query);
...
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET %s = %s + %i WHERE name = '%s'", Points, Points, Score, VictimName);
SendSQLUpdate(query);
...
UpdateTankSniperSteamID(VictimName);
}

UpdateTankSniperSteamID(const String:VictimName[])
{
decl String:query[1024];
Format(query, sizeof(query), "UPDATE players SET infected_tanksniper = infected_tanksniper + 1 WHERE name = '%s'", VictimName);
SendSQLUpdate(query);
...
decl String:SaviorName[MAX_LINE_WIDTH];
GetClientName(Attacker, SaviorName, sizeof(SaviorName));


decl String:VictimName[MAX_LINE_WIDTH];
GetClientName(Victim, VictimName, sizeof(VictimName));

SurvivorDiedNamed(Attacker, Victim, VictimName, SaviorName, AttackerInfType, Mode);
}

// An Infected player killed a Survivor.

SurvivorDiedNamed(Attacker, Victim, const String:VictimName[], const String:SaviorName[], AttackerInfType = -1, Mode = -1)
{
if (!Attacker || !Victim)
return;
...
if (AttackerInfType != INF_ID_SMOKER && AttackerInfType != INF_ID_BOOMER && AttackerInfType != INF_ID_HUNTER && AttackerInfType != INF_ID_TANK && AttackerInfType != INF_ID_CHARGER  && AttackerInfType != INF_ID_JOCKEY  && AttackerInfType != INF_ID_SPITTER)
return;

//LogError("SurvivorDiedNamed - AttackerInfType = %i - VictimName = %s", AttackerInfType, VictimName);
...
len += Format(query[len], sizeof(query)-len, "WHERE name = '%s'", SaviorName);
SendSQLUpdate(query);
...
if (Attacker > 0 && AttackerTeam == TEAM_INFECTED)
AttackerInfType = GetEntProp(Attacker, Prop_Send, "m_zombieClass");
}
...
if (AttackerInfType == INF_ID_TANK && event != INVALID_HANDLE)
{
...
decl String:query[512];
Format(query, sizeof(query), "UPDATE players SET points_infected = points_infected + %i, award_survivor_down = award_survivor_down + 1 WHERE name = '%s'", Score, AttackerName);
SendSQLUpdate(query);

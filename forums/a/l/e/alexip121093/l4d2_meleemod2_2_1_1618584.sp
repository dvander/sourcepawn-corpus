...
public OnPluginStart()
{
	...
	CreateConVar("sm_l4d2meleemod_version", PLUGIN_VERSION, "L4D2 Melee  Mod version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_DONTRECORD)
	HookEvent("item_pickup", Event_ItemPickup);
	cvar_ammo = CreateConVar("sm_l4d2meleemod_ammo_enable", "1", "Enable Melee Weapon ammo count",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_ammolower = CreateConVar("sm_l4d2meleemod_ammo_lower", "150", "After How many times of attack, the melee weapons breaks (default lower limit)",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_ammoupper = CreateConVar("sm_l4d2meleemod_ammo_upper", "250", "After How many times of attack, the melee weapons breaks (default upper limit)",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_notice = CreateConVar("sm_l4d2meleemod_notice", "2", "Show After how many attacks the melee weapon breaks ( 0 = disable ,1 = display numbers ,2 = bar",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_lownotice = CreateConVar("sm_l4d2meleemod_lownotice", "0", "Show a message when melee weapon nearly breaks",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_pistol = CreateConVar("sm_l4d2meleemod_pistol", "0", "after the melee weapon breaks , which secondary weapon will give out .(0: single pistol 1:double pistol 2:magnum 3:chainsaw",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_escapeammo = CreateConVar("sm_l4d2meleemod_ammo_escape", "100", "number of melee weapon ammo needed to escape from special infected",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_killammo = CreateConVar("sm_l4d2meleemod_ammo_kill", "150", "number of melee weapon ammo needed to kill special infected",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_IE = CreateConVar("sm_l4d2meleemod_infectedmode", "2", "0 = disable all 1 =allow escape infected 2 = allow kill infected 3=both allowed",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_MA = CreateConVar("sm_l4d2meleemod_MA", "2", "0 = close melee swing adjust 1 =global melee swing adjust(sm_l4d2meleemod_MA_interval + sm_setmelee) 2 = individual melee swing adjust( sm_setmelee )",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_maspeed =  CreateConVar("sm_l4d2meleemod_MA_interval", "0.3", "melee swing interval",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_MF =  CreateConVar("sm_l4d2meleemod_MF_enable", "1", "melee Fatigue enable(melee weapon)  sm_l4d2meleemod_MA must be 1 or 2",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_MF_count =  CreateConVar("sm_l4d2meleemod_MF_count", "12", "melee Fatigue start after ? number of swings (melee weapon)",FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_MF_time =  CreateConVar("sm_l4d2meleemod_MF_time", "3.0", "melee Fatigue time (melee weapon)",FCVAR_PLUGIN|FCVAR_NOTIFY);
	...
	for (new iI=1; iI<=maxplayers; iI++)
	{
		...
		if (MFd == 1) 
		{
			if (g_flMFlastact[iCid] >=0)
			{
				...
				if (GetEntProp(iCid, Prop_Send, "m_iReloadMode", 1)) reloading[iCid]=1;
				else if (GetEntProp(iCid, Prop_Send, "m_iReloadMode", 0) && reloading[iCid]==1)
				{
					reloading[iCid]=0;
					if (iCid >0)
					{

...
new Handle:ScoreDisplayCVAR = INVALID_HANDLE;
new propinfoghost;
...
ScoreDisplayCVAR = CreateConVar("l4d2_teampanel_scoreautodisplay", "1", "the panel will pop up after player see their score", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 1.0);
RegConsoleCmd("sm_showteam", Command_Say);
RegConsoleCmd("sm_dteam", Command_Score);
...
dp[client] = false;
se[client] = false;
ClientCommand(client,"bind tab \"+score;sm_dteam\"");
}
...
{
if (GetConVarInt(ScoreDisplayCVAR) == 1)
{
if (se[client]) return;
if (se[client] == false)
{
...
CreateTimer(9.0, PAe,client, TIMER_FLAG_NO_MAPCHANGE);
CreateTimer(10.0, PAe,client, TIMER_FLAG_NO_MAPCHANGE);
CreateTimer(10.1, Reset,client, TIMER_FLAG_NO_MAPCHANGE);
}
}
}
...
{
Format(addoutput1, sizeof(addoutput1), "Total: %d(%d)/%d Survivors: %d(%d)/%d Infected: %d(%d) Spectator: %d", total, totalreal,maxcl, surcount,surreal,GetConVarInt(FindConVar("survivor_limit")), infcount,infreal, sepcount);
}
}
else
{
...
{
Format(addoutput1, sizeof(addoutput1), "Total: %d(%d)/%d Survivors: %d(%d)/%d Infected: %d(%d)", total, totalreal,maxcl, surcount,surreal,GetConVarInt(FindConVar("survivor_limit")), infcount,infreal);
}
}
}
else

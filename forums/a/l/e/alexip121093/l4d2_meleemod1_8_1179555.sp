...
public OnPluginStart()
{
	...
	cvar_MA = CreateConVar("sm_l4d2meleemod_MA", "2", "0 = close melee swing adjust 1 =global melee swing adjust 2 = individual melee swing adjust");
...
public Action:Checkupdate(Handle:timer)
{
	if(LibraryExists("pluginautoupdate")) 
	{
		ServerCommand("sm_autoupdate_download l4d2_meleemod.smx");
	}
}
...
public Action:DisplayInstructorHint(Handle:h_Timer, any:i_Client)
{
	...
	{
		FormatEx(s_Message, sizeof(s_Message), "您可以按攻擊鍵使用近戰武器逃跑!!!")
	}
	else if (GetConVarInt(cvar_lang) == 2) 
	{
		FormatEx(s_Message, sizeof(s_Message), "您可以按攻击键使用近战武器逃跑!!!")
	}
...
MA_OnGameFrame1()
{
	...
	for (new iI=1; iI<=maxplayers; iI++)
	{
		...
		if (melee_speed[iCid] <= 0.0)continue;
...
public OnAllPluginsLoaded() 
{
	if(LibraryExists("pluginautoupdate")) 
	{
		AutoUpdate_AddPlugin("l4d2sourcemod.googlecode.com", "/files/plugins.xml", PLUGIN_VERSION);
	}
}
public OnPluginEnd() 
{

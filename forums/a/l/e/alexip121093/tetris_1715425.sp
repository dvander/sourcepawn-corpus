...
{
	new Handle:hVersion = CreateConVar("sm_tetris_version", PLUGIN_VERSION, "Tetris minigame version", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_REPLICATED|FCVAR_DONTRECORD);
	if(hVersion != INVALID_HANDLE)
		SetConVarString(hVersion, PLUGIN_VERSION);
	
	g_hCVDisableHardDrop = CreateConVar("sm_tetris_disableharddrop", "0", "Disable hard dropping the tetrimino instantly to the floor with the spacebar?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_hCVDisableSounds = CreateConVar("sm_tetris_disablesounds", "0", "Disable all tetris sounds?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	SQL_TConnect(SQL_OnDatabaseConnected, (SQL_CheckConfig("tetris")?"tetris":"default"));
...
public OnMapStart()
{
	...
	PrecacheSound(TETRIS_MUSIC, true);
	PrecacheSound(TETRIS_ROTATE, true);
	PrecacheSound(TETRIS_PLACE, true);
	PrecacheSound(TETRIS_FULLLINE, true);
	PrecacheSound(TETRIS_LEVELUP, true);
	PrecacheSound(TETRIS_TETRIS, true);
	PrecacheSound(TETRIS_GAMEOVER, true);
}
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	if(g_hPlayerGameThink[client] != INVALID_HANDLE)
	{
		...
		// Hard Drop!
		if( (buttons & IN_JUMP) && !(g_iButtons[client] & IN_JUMP) && !g_bDisableHardDrop)
		{
		...
		if(buttons & IN_FORWARD)
		{
			...
			&& RotateObject(client))
			{
				if(!g_bDisableSounds && g_bEnableSound[client])
					EmitSoundToClient(client, TETRIS_ROTATE);
...
public Event_OnPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(IsClientInTetrisGame(client))
	{
		...
		StopSound(client, SNDCHAN_AUTO, TETRIS_MUSIC);
...
public Panel_HandleGame(Handle:menu, MenuAction:action, param1, param2)
{
	if(action == MenuAction_Select)
	{
		if(param2 == 10 && IsClientInTetrisGame(param1))
		{
			ClearTimer(g_hPlayerGameThink[param1]);
			ClearTimer(g_hDrawInfoPanel[param1]);
			ClearTimer(g_hPlayBackgroundMusic[param1]);
			StopSound(param1, SNDCHAN_AUTO, TETRIS_MUSIC);
			CPrintToChat(param1, "%sGame paused. You're able to resume with !tetris.", PREFIX);
...
public Action:Timer_PlayBackgroundMusic(Handle:timer, any:userid)
{
	...
	EmitSoundToClient(client, TETRIS_MUSIC);
...
DrawTetrisGameField(client, bool:bGameOver)
{
	...
	if(bGameOver)
	{
		...
		if(!g_bDisableSounds && g_bEnableSound[client])
			EmitSoundToClient(client, TETRIS_GAMEOVER);
		//SetEntProp(client, Prop_Send, "m_fFlags", FL_FAKECLIENT|FL_ONGROUND|FL_PARTIALGROUND);
		SetEntityMoveType(client, MOVETYPE_WALK);
...
SaveCurrentObject(client)
{
	...
	{
		if(iLinesCleared == 4)
			EmitSoundToClient(client, TETRIS_TETRIS);
		else if(iLinesCleared > 0)
			EmitSoundToClient(client, TETRIS_FULLLINE);
		else
			EmitSoundToClient(client, TETRIS_PLACE);
	}
	...
	while(g_iLevel[client]*5 <= g_iLinesCleared[client])
	{
		g_iLevel[client]++;
		if(!g_bDisableSounds && g_bEnableSound[client])
			EmitSoundToClient(client, TETRIS_LEVELUP);
	}

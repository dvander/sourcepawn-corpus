...
#define PLUGIN_AUTHOR                 "Mart (bug fix by Alexander Mirny)"
...
#define PARTICLE4 "flare_burning"
...
// ====================================================================================================
char g_sParticles[5][47] = { "", PARTICLE1, PARTICLE2, PARTICLE3, PARTICLE4 };
char g_sParticlesNames[5][47] = { "NONE", "WITCH", "PIPEBOMB", "FIRE", "FLARE" };
...
// ====================================================================================================
Handle g_hLeftEyeCookie;
Handle g_hRightEyeCookie;
...
public void OnPluginStart()
{
    ...
    // Cookies
    g_hLeftEyeCookie = RegClientCookie("l4d_survivor_eyes_left_eye", "Survivor Eyes - Left Eye Particle", CookieAccess_Protected);
    g_hRightEyeCookie = RegClientCookie("l4d_survivor_eyes_right_eye", "Survivor Eyes - Right Eye Particle", CookieAccess_Protected);

    // Commands
	RegConsoleCmd(CMD_EYES, CmdEyes);
    // Admin Commands
...
public void OnClientCookiesCached(int client)
{
    ...
    if (g_bCvar_Cookies && !IsFakeClient(client))
    {
        ...
        // Новая версия:
        GetClientCookie(client, g_hLeftEyeCookie, value, sizeof(value));
        
        if (value[0] != 0)
            gc_iLeftEyeParticle[client] = StringToInt(value);
        
        // Новая версия:
        GetClientCookie(client, g_hRightEyeCookie, value, sizeof(value));
...
void HookEvents()
{
    if (g_bCvar_Enabled && !g_bEventsHooked)
    {
		...
		HookEvent("player_bot_replace", Event_PlayerReplaceBot);
    ...
    if (!g_bCvar_Enabled && g_bEventsHooked)
    {
		...
		UnhookEvent("player_bot_replace", Event_PlayerReplaceBot);
...
/****************************************************************************************************/
public void Event_PlayerReplaceBot(Event event, const char[] name, bool dontBroadcast)
{
...
int CreateParticle(const char[] particle, float vPos[3], float vAng[3], int target, const char[] attachment)
{
...
void CreateParticleMenu(int client)
{
	...
	FormatEx(text, sizeof(text), "%s %s", clientParticle == 4 ? "☑" : "☐", Translate(client, "%t", "Particle 4"));
    menu.AddItem("4", text);
...
int HandleParticleMenu(Menu menu, MenuAction action, int param1, int param2)
{
    switch (action)
    {
        case MenuAction_Select:
        {
            ...
            {
                // Новая версия:
                SetClientCookie(client, g_hLeftEyeCookie, sArgs);
            ...
            {
                // Новая версия:
                SetClientCookie(client, g_hRightEyeCookie, sArgs);

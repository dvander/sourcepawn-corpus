...
void SaveClientWeapons(int client)
{
    ...
    LogMessage("Weapons saved for player %N: %s, %s, %s, %s (active slot %d)", 
...
public void SQL_LoadWeaponsCallback(Database db, DBResultSet results, const char[] error, any data)
{
    ...
    if(results.FetchRow())
    {
        ...
        if(activeSlot >= 0 && activeSlot < 4 && weapons[activeSlot] != -1)
        {
            ...
            LogMessage("Set active weapon for player %N in slot %d", client, activeSlot);
        }
    }
}
...
bool IsValidWeapon(const char[] weaponName)
{
    ...
    if(StrContains(weaponName, "weapon_melee") != -1 || StrContains(weaponName, "weapon_") == 0 && StrContains(weaponName, "melee_") != -1)
    {
        return true;
    }

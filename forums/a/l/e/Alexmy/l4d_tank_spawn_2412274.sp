...
public Action EventTankKilled(Event event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(event.GetInt("userid"));
	{
		if(stop)return;
		{
			...
			Timer_2 = CreateTimer(GetRandomInt(0, 1) ? GetConVarFloat(sm_NextTank_Random_1) : GetConVarFloat(sm_NextTank_Random_2), SpawnTankNext, client, TIMER_FLAG_NO_MAPCHANGE);
		}
	}
}

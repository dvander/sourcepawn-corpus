...
public Plugin:myinfo = {
	name = "Jesters Players Panel and Switch Menu",
	author = "-=BwA=- Jester",
...
public OnPluginStart() {
	...
	CreateConVar("l4d2_plp_version", PLUGIN_VERSION, "Players Panel Version Information", 0);
	
	allowpubs = CreateConVar("l4d2_BwA_TeamPanel_AllowPubs", "1", "Allow public access to commands [0 = No|1 = Yes]", 0, true, 0.0, true, 1.0);
	selectTeamFromPanel = CreateConVar("l4d2_BwA_SelectTeam_From_Panel", "1", "Allow players to press 1,2 or 3 to select Spectator, Survivor or Infected from the Team Panel [0 = No|1 = Yes]", 0, true, 0.0, true, 1.0);
	cv_PrintMsgType = CreateConVar("l4d2_BwA_Print_Chat_MsgType", "2", "Determine the type and frequency of information printed to the in game chat [0 = none|1 = simple messages|2 = verbose color coded messages]", 0, true, 0.0, true, 2.0);
	showClientID = CreateConVar("l4d2_BwA_Show_ClientID", "1", "Show the clientid next to the player name in the panel [0 = No|1 = Yes]", 0, true, 0.0, true, 1.0);
	cv_enablepause = CreateConVar("l4d2_BwA_TeamPanel_EnablePause", "1", "Enable/Disable this plugins pause feature (default = 1) [0 = disabled|1 = enabled]", 0, true, 0.0, true, 1.0);
...
Unscramble() {
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{	
			...
			char  GetPlayerSteamID[64];
			GetClientAuthId(i, AuthId_Engine, GetPlayerSteamID, sizeof(GetPlayerSteamID));
			for (new j = 1; j <= MaxClients; j++) 
			{
				//if (StrEqual(tmp_PlayerSteamID[j], steamId))
				if (StrEqual(tmp_PlayerSteamID[j], GetPlayerSteamID)) 
				{ 
...
public OnClientDisconnect(client) {
	...
	char  GetPlayerSteamID[64];
	GetClientAuthId(client, AuthId_Engine, GetPlayerSteamID, sizeof(GetPlayerSteamID));
	
	//new index = Get_LR_SpectatorIndex(steamId);
	new index = Get_LR_SpectatorIndex(GetPlayerSteamID);
...
GetGameMode() {
	...
	else if (StrContains(gamemodecvar, "teamversus", false) != -1)
	{
		return GM_VERSUS;
	}
	else if (StrContains(gamemodecvar, "community6", false) != -1)
	{
		return GM_VERSUS;
	}
	else if (StrContains(gamemodecvar, "mutation11", false) != -1)
	{
		return GM_VERSUS;
	}
	else if (StrContains(gamemodecvar, "mutation12", false) != -1)
	{
		return GM_VERSUS;
	}
	else if (StrContains(gamemodecvar, "mutation19", false) != -1)
	{
		return GM_VERSUS;
	}
	else if (StrContains(gamemodecvar, "mutation15", false) != -1)
	{
		return GM_VERSUS;
	}
	else if (StrContains(gamemodecvar, "mutation18", false) != -1)
	{
		return GM_VERSUS;
	}
	else if (StrContains(gamemodecvar, "community3", false) != -1)
	{
		return GM_VERSUS;
	}
	else if (StrContains(gamemodecvar, "l4d1vs", false) != -1)
	{
		return GM_VERSUS;
	}
	...
	else if (StrContains(gamemodecvar, "teamscavenge", false) != -1)
	{
		return GM_SCAVENGE;
	}
	else if (StrContains(gamemodecvar, "mutation13", false) != -1)
	{
		return GM_SCAVENGE;
	}
	//Campaign
...
PerformSwitch(client, target, team, bool:silent) {
    ...
    bot++;
		...
		while !(IsClientConnected(bot) && IsFakeClient(bot) && (GetClientTeam(bot) == TEAM_SURVIVOR))

		//while !(IsClientConnected(bot) && IsFakeClient(bot) && (GetClientTeam(bot) == TEAM_SURVIVOR)) do bot++;

...
public void OnPluginStart()
{
	...
	vc_mute_time_message = CreateConVar("vc_mute_time_message", "1", "Enables or disables time until muted message");
...
public Action playerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(GetConVarBool(vc_mute_time_message))
	{
		char time[32];
		float fTime = GetConVarFloat(vc_time);
		Format(time, 32, "%.2f", fTime);
		PrintToChat(client, "[\x06VC\x01] \x01 %t", "timeUntilMute", time);
	}
}

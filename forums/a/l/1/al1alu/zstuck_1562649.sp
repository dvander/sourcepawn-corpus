...
public OnPluginStart() {
	...
	gCvarEnabled = CreateConVar("sm_zstuck_enabled", "1", "Enable Plugin");
	gEnabled = GetConVarBool(gCvarEnabled);
	HookConVarChange(gCvarEnabled, CvarChanged);

	gCvarInterval = CreateConVar("sm_zstuck_interval", "3", "Interval to use zstuck");
	gInterval = GetConVarFloat(gCvarInterval);
	HookConVarChange(gCvarInterval, CvarChanged);

	gCvarNoBlock = CreateConVar("sm_zstuck_noblock", "1", "Give noblock");
	gNoBlock = GetConVarBool(gCvarNoBlock);
	HookConVarChange(gCvarNoBlock, CvarChanged);

	gCvarSlap = CreateConVar("sm_zstuck_slap", "1", "Give slap");
...
public Action:zstuck(id)
{
	if(intervalo[id] == INVALID_HANDLE)
	{
		...
		{
			setNoblock(id);
			CreateTimer(0.5, desblock, GetClientOfUserId(id));
		}
		if(gSlap) 
		{
			ServerCommand("sm_slap #%d", GetClientUserId(id));
		}
		intervalo[id] = 1;

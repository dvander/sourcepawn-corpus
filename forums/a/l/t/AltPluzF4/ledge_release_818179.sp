...
public OnPluginStart()
{
	...
	HookEvent("mission_lost", MissionLostEvent, EventHookMode_Pre);
...
public Action:MissionLostEvent(Handle:event, String:event_name[], bool:dontBroadcast)
{
...
public Action:PlayerTimer(Handle:timer, any:client)
{
	...
	if (bIncap && bHang)
	{
		...
		if ((Buttons[client] & IN_DUCK) && !(buttons & IN_DUCK) && IgnoreCrouch[client]) // Duck released, ignoring
		{
			IgnoreCrouch[client] = false;
		}
	}
	else if (!bIncap)
	{
		...
		if (buttons & IN_DUCK)
			IgnoreCrouch[client] = true; //If we are crouched before we being to hang, ignore the first release
	}

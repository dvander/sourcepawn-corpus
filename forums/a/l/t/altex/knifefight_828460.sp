...
-------
    ...
    Thanks to XARIUS for 1v1 Knife Fight plugin v1.2.1
    http://forums.alliedmods.net/showthread.php?t=77948
...
TODO
----
    1) Don't drop knifes on map.
    2) Remove delay on weapon strip when pick up. Add check for gg4 turbo to avoid bug 
       or add another knifefight variable.
    3) Enable HookUserMessage after fix for https://bugs.alliedmods.net/show_bug.cgi?id=3817
    4) Find a way to kill players in one tick and remove randomkill
       EventScript example: es_xdelayed 0.2 foreach player tempuserid #alive "es_sexec server_var(tempuserid) kill"
    5) Make beacon sound plays in the way players can detect each other by that sound.
       EventScript example: es_emitsound player server_var(userid) server_var(beacon_sound) 1.0 0.7
...
#include <sourcemod>
#include <sdktools>
#define KNIFEFIGHT_VERSION "1.2.1.1"
...
public OnPluginStart()
{
  ...
  CreateConVar("sm_knifefight_version", KNIFEFIGHT_VERSION, "Knife Fight Version || PLEASE DO NOT CHANGE THIS", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
  g_Cvarenabled = CreateConVar("sm_knifefight_enabled", "1", "Enable this plugin. 0 = Disabled");
  g_Cvaralltalk = CreateConVar("sm_knifefight_alltalk", "1", "Enable alltalk while knife fight. 0 = Disabled");
  g_Cvarrandomkill = CreateConVar("sm_knifefight_randomkill", "0", "Enable random kill after knife fight time end. 0 = Disabled");
  ...
  g_Cvardeclinesound = CreateConVar("sm_knifefight_declinesound", "knifefight/chicken.wav", "The sound to play when player declines to knife.");
  g_Cvarbeaconsound = CreateConVar("sm_knifefight_beaconsound", "buttons/blip1.wav", "The sound to play when beacon ring shows.");
  ...
  g_Cvarbeaconradius = CreateConVar("sm_knifefight_beaconradius", "800", "Beacon radius.");
...
public OnConfigsExecuted()
{
  ...
  {
    SetFailState("1v1 Knife Fight: Could not pre-cache sound: %s", g_declinesound);
  }
  else
  {
  ...
  {
    SetFailState("1v1 Knife Fight: Could not pre-cache sound: %s", g_beaconsound);
  }
  else
  {
...
public Action:FightTimer(Handle:timer)
{
  ...
  else if (fighttimer == -1)
  {
    ...
    if ( healthCT == healthT )
    {
        // TODO: is there a way to kill players in one tick?
        if ( g_randomkill )
        {
    ...
    if (clientid == ctid || clientid == tid)
        {
            ...
            {
                //TODO: Remove delay on weapon pick up.
...
public PanelHandler(Handle:menu, MenuAction:action, param1, param2)
{
  if (action == MenuAction_Select)
    ...
    {
      if (GetClientTeam(param1) == 3 && param1 == ctid && tagree != 0 && IsClientInGame(tid) && IsPlayerAlive(tid))
      {
        PrintToChatAll("[SM] [CT] %s %t", ctname, "Player agrees");
        ctagree = 1;
      } 
      else if (GetClientTeam(param1) == 2 && param1 == tid && ctagree != 0 && IsClientInGame(ctid) && IsPlayerAlive(ctid))
      {
        PrintToChatAll("[SM] [T] %s %t", tname, "Player agrees");
        tagree = 1;
      }

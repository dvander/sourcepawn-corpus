        ...
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_SI_losers[i]) && !IsFakeClient(mvp_SI_losers[i])) 
            {
                if (bSolo)
                {
                    ...
                    {
                        Format(tmpBuffer, sizeof(tmpBuffer), "{green}Your SI Rank: {lightgreen}[%d] {default}- %d dmg %.0f%% - %d kills %.0f%%", (i + 2), iDidDamageAll[mvp_SI_losers[i]], (float(iDidDamageAll[mvp_SI_losers[i]]) / float(iTotalDamageAll)) * 100, iGotKills[mvp_SI_losers[i]], (float(iGotKills[mvp_SI_losers[i]]) / float(iTotalKills)) * 100);
                        CPrintToChat(mvp_SI_losers[i], "%s", tmpBuffer);
                    }
                }
                else 
                {
                    Format(tmpBuffer, sizeof(tmpBuffer), "{green}Your SI Rank: {lightgreen}[%d] {default}- %d kills %.0f%%", (i + 2), iDidDamageAll[mvp_SI_losers[i]], (float(iDidDamageAll[mvp_SI_losers[i]]) / float(iTotalDamageAll)) * 100, iGotKills[mvp_SI_losers[i]], (float(iGotKills[mvp_SI_losers[i]]) / float(iTotalKills)) * 100);
                    CPrintToChat(mvp_SI_losers[i], "%s", tmpBuffer);
                }
            }
        }
        ...
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_CI_losers[i]) && !IsFakeClient(mvp_CI_losers[i])) 
            {
                if (bSolo)
                {
                    ...
                    {
                        Format(tmpBuffer, sizeof(tmpBuffer), "{green}Your CI Rank: {lightgreen}[%d] {default}- %d common %.0f%%", (i + 2), iGotCommon[mvp_CI_losers[i]], (float(iGotCommon[mvp_CI_losers[i]]) / float(iTotalCommon)) * 100);
        ...
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_FF_losers[i]) &&  !IsFakeClient(mvp_FF_losers[i])) 
            {
                if (bSolo)
                {
                    ...
                    {
                        Format(tmpBuffer, sizeof(tmpBuffer), "{green}Your FF Rank: {lightgreen}[%d] {default}- %d friendly fire %.0f%%", (i + 2), iDidFF[mvp_FF_losers[i]], (float(iDidFF[mvp_FF_losers[i]]) / float(iTotalFF)) * 100);
...
void GetMVPString(char[] printBuffer, const int iSize)
{
    ...
    {
        Format(tmpBuffer, sizeof(tmpBuffer), "{green}Round Info: \x03None\n");
        ...
        if (!(iBrevityFlags & BREV_CI))
        {
            if (mvp_Common > 0)
            {
                if (iBrevityFlags & BREV_PERCENT) {
                    ...
                    Format(tmpBuffer, sizeof(tmpBuffer), "{green}Best Common Killer: {lightgreen}%s {default}- %d common %.0f%%\n", mvp_Common_name, iGotCommon[mvp_Common], (float(iGotCommon[mvp_Common]) / float(iTotalCommon)) * 100);
                }
                StrCat(printBuffer, iSize, tmpBuffer);
            }
        }
    ...
    if (!(iBrevityFlags & BREV_FF) && bTrackFF)
    {
        ...
        else
        {
            if (iBrevityFlags & BREV_PERCENT) {
                ...
                Format(tmpBuffer, sizeof(tmpBuffer), "{green}Frinedly Fire Check: {lightgreen}%s {default}- %d friendly fire %.0f%%\n", mvp_FF_name, iDidFF[mvp_FF], (float(iDidFF[mvp_FF]) / float(iTotalFF)) * 100);
            }
            StrCat(printBuffer, iSize, tmpBuffer);
        }
    }

...
public void OnPluginStart()
{
	...
	g_Cvar_PluginEnable = CreateConVar("l4d_elevator_teleport_enable", "1.0", "Enable elevator teleport plugin (0 = disable, 1 = enable)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_Cvar_TeleportTime = CreateConVar("l4d_elevator_teleport_time", "60.0", "Set the elevator teleport countdown time, which will teleport survivors when it reaches zero", FCVAR_NOTIFY);
	g_Cvar_MapsIncluded = CreateConVar("l4d_elevator_teleport_maps", "all", "Plugin will be activated in these maps, separate by commas (no spaces). (all = All maps).", FCVAR_NOTIFY);
	g_Cvar_RescueClient = CreateConVar("l4d_elevator_teleport_entire", "1", "Plugin will teleport all survivors, including hanged and held survivors.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
void OnElevatorLocked(const char[] output, int caller, int activator, float delay)
{
	...
	else if (!g_bLockedButtonPressed && !g_bUnlockedButtonPressed) 
	{
		...
		PrintHintTextToAll("Teleport Time: %d", time);

...
public void OnPluginStart() {
	...
	cvarLockRoundOne = CreateConVar("l4d2_lock_roundone", "30", "How long the door is locked on round 1 - this round takes longer to load. (Default: 40)");
...
stock Now_UnlockSaferoomDoor() {
	...
	if (SaferoomDoor > 0 && IsValidEntity(SaferoomDoor)) {
		...
		if (b_StandartMap()) {
			...
			false);
			SetEntityRenderColor(SaferoomDoor, 255, 255, 255, 255);
		}
	}
...
stock blockPath() {
	...
	SetEntityRenderColor(fence, 255, 255, 255, 255);
		...
		if (isBugged() || isBugged2()) {
			...
			if (!bFirstMapOfCampaign()) {
				if (g_iCooldown > 0) {
					...
					g_bLocked = false;
					PrintToChatAll("\x05[\x04LOCK\x05]\x01 Saferoom Doors Are Now Open!");

					if (GetConVarInt(cvarLockHintText) == 1) {
						PrintHintTextToAll("Saferoom Doors Are Now Open!");
					}
				}
				if (GetConVarInt(cvarLockHintText) == 1) {
					if (g_iCooldown > 0) {
						PrintHintTextToAll("Doors Are Locked\nPlease Wait: %i", g_iCooldown);
					}
				}
			...
			if (g_iCooldown > 0) {
				...
				unblockPath();
				g_bLocked = false;
				PrintToChatAll("\x05[\x04LOCK\x05]\x01 The Saferoom Doors Are Now Open!");

				if (GetConVarInt(cvarLockHintText) == 1) {
					PrintHintTextToAll("The Saferoom Doors Are Now Open!");
				}
			}
			if (GetConVarInt(cvarLockHintText) == 1) {
				if (g_iCooldown > 0) {

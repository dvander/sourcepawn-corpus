...
public void OnPluginStart() {
	...
	cvarLockRoundOne = CreateConVar("l4d2_lock_roundone", "35", "How long the door is locked on round 1 - this round takes longer to load. (Default: 40)");
	cvarLockRoundTwo = CreateConVar("l4d2_lock_roundtwo", "35", "How long the door is locked on round two. (Default: 30)");
	cvarLockGlowRange = CreateConVar("l4d2_lock_glowrange", "800", "How far the glow ranges off of the saferoom door. (Default: 800)");
	cvarLockHintText = CreateConVar("l4d2_lock_hinttext", "1", "Does the plugin print the countdown in center screen? (Default: 0)");
...
public Action Event_DoorUnlocked(Handle event, const char[] name, bool dontBroadcast) {
	if (g_bLocked == true) {
		...
		if (!IsFakeClient(client) && IsClientInGame(client)) {
			PrintToChat(client, "[LOCK] Door will open in: %i seconds! Please wait.", g_iCooldown);
		}
	}
...
stock blockPath() {
	...
	if (StrEqual(cmap, "c1m1_hotel")) {
		...
		fnOrigin[0] = 6396.835449;
		fnAngles[0] = 0.000000;
		fnOrigin[1] = -439.813385;
		fnAngles[1] = 180.000000;
		fnOrigin[2] = 656.031250;
		fnAngles[2] = 0.000000;
	} else if (StrEqual(cmap, "c12m1_hilltop") || StrEqual(cmap, "C12m1_hilltop")) {
		DispatchKeyValue(fence, "model", FENCE_MODEL13);

		fnOrigin[0] = -8633.000000;
		fnAngles[0] = 0.000000;
		fnOrigin[1] = -14827.000000;
		fnAngles[1] = 166.000000;
		fnOrigin[2] = 319.000000;
		fnAngles[2] = 0.000000;
		...
		fnAngles[2] = 0.000000;
	} else if (StrEqual(cmap, "c14m1_junkyard")) {
		DispatchKeyValue(fence, "model", FENCE_MODEL13);

		fnOrigin[0] = -4355.000000;
		fnAngles[0] = 0.000000;
		fnOrigin[1] = -10073.000000;
		fnAngles[1] = 90.000000;
		fnOrigin[2] = -307.000000;
		fnAngles[2] = 0.000000;
	...
	if (StrEqual(cmap, "c2m1_highway")) {
		...
		DispatchKeyValue(fence2, "model", FENCE_MODEL02);

		fn2Origin[0] = 2215.561279;
		...
		DispatchKeyValue(fence2, "model", FENCE_MODEL13);

		fn2Origin[0] = 6291.875000;
		fn2Angles[0] = 0.000000;
		fn2Origin[1] = -441.662415;
		fn2Angles[1] = 180.000000;
		fn2Origin[2] = 848.031250;
		fn2Angles[2] = 0.000000;
	} else if (StrEqual(cmap, "c12m1_hilltop") || StrEqual(cmap, "C12m1_hilltop")) {
		DispatchKeyValue(fence, "model", FENCE_MODEL14);
	...
	if (StrEqual(cmap, "c4m1_milltown_a")) {
		...
		DispatchKeyValue(fence3, "model", FENCE_MODEL13);

		fn3Origin[0] = 6288.875000;
		fn3Angles[0] = 0.000000;
		fn3Origin[1] = -633.662415;
		fn3Angles[1] = 180.000000;
		fn3Origin[2] = 848.031250;
		fn3Angles[2] = 0.000000;
	} else if (StrEqual(cmap, "c12m1_hilltop") || StrEqual(cmap, "C12m1_hilltop")) {
		DispatchKeyValue(fence, "model", FENCE_MODEL14);
		...
		if (isBugged() || isBugged2()) {
			...
			if (!bFirstMapOfCampaign()) {
				if (g_iCooldown > 0) {
					...
					g_bLocked = false;
					PrintToChatAll("\x05[\x04LOCK\x05]\x01 Saferoom doors are open!");
				...
				if (GetConVarInt(cvarLockHintText) == 1) {
					if (g_iCooldown > 0) {
						PrintHintTextToAll("%i To Unlock Door!", g_iCooldown);
					}
				}
			...
			if (g_iCooldown > 0) {
				...
				unblockPath();
				g_bLocked = false;
				PrintToChatAll("\x05[\x04LOCK\x05]\x01 The Door Is Now Open!");

				if (GetConVarInt(cvarLockHintText) == 1) {
					PrintHintTextToAll("The Door Is Open!");
				}
			}
			if (GetConVarInt(cvarLockHintText) == 1) {
				if (g_iCooldown > 0) {
					PrintHintTextToAll("%i Sec To Unlock Door!", g_iCooldown);
				}
			}
...
public bool: b_StandartMap() {
	char MapName[128];
	GetCurrentMap(MapName, sizeof(MapName));
	if (StrContains(MapName, "c1m1", true) > -1 || StrContains(MapName, "c1m2", true) > -1 || StrContains(MapName, "c1m3", true) > -1) {
		return true;
	}
	return false;
}

...
public OnClientAuthorized(client, const String:auth[])
{
	...
	//SELECT sb_admins.user as admin, sb_bans.bid, sb_bans.reason, sb_bans.length / 60 as minutes, sb_bans.authid, sb_bans.ip FROM sb_admins, `sb_bans` WHERE sb_bans.aid = sb_admins.aid
	FormatEx(Query, sizeof(Query), "SELECT %s_admins.user as admin, %s_bans.bid, %s_bans.reason, %s_bans.length / 60 as minutes, %s_bans.authid, %s_bans.ip bid FROM %s_admins, %s_bans WHERE ((%s_bans.type = 0 AND %s_bans.authid REGEXP '^STEAM_[0-9]:%s$') OR (%s_bans.type = 1 AND %s_bans.ip = '%s')) AND (%s_bans.length = '0' OR %s_bans.ends > UNIX_TIMESTAMP()) AND %s_bans.RemoveType IS NULL AND %s_bans.aid = %s_admins.aid", DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix,DatabasePrefix,DatabasePrefix, auth[8],DatabasePrefix,DatabasePrefix, ip, DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix);
#if defined DEBUG
...
public VerifyInsert(Handle:owner, Handle:hndl, const String:error[], any:dataPack)
{
	...
	decl String:auth[30];decl String:ip[20];
	...
	// Kick player
	new String: blength[10];
	new String: adminname[128];
	GetClientName(admin, adminname, sizeof(adminname));
	IntToString(time, blength, sizeof(blength));
	new Handle:pack = CreateDataPack();
	WritePackCell(pack, client);
	WritePackString(pack, Reason);
	WritePackString(pack, blength);
	if(GetClientUserId(client) == UserId) {
		ConsoleInfoPrint(client, adminname, Reason, blength, auth, ip);
		CreateTimer(3.0, DelayedKick, pack, TIMER_HNDL_CLOSE);  
	}
		//KickClient(client, "%t", "Banned Check Site", Reason, blength, WebsiteAddress);
}
...
public VerifyBan(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	if(SQL_GetRowCount(hndl) > 0)
	{
		...
		rownum = SQL_GetRowCount(hndl);
		PrintToServer("[SOURCEBANS] SQL ROW NUM %d LINE:1509", rownum);
		decl String:buffer[40];
		decl String:Name[128];
		decl String:Query[512];
		//decl String:msg[512];
		decl String:Admin[128];
		decl String:Reason[128];
		decl String:time[32];
		decl String:bip[30];
		decl String:bauthid[64];
		SQL_FetchRow(hndl);
		
		SQL_FieldNameToNum(hndl,"admin",f_index);
		SQL_FetchString(hndl, f_index, Admin, sizeof(Admin));
		
		SQL_FieldNameToNum(hndl,"reason",f_index);
		SQL_FetchString(hndl, f_index, Reason, sizeof(Reason));
		
		SQL_FieldNameToNum(hndl,"minutes",f_index);
		SQL_FetchString(hndl, f_index, time, sizeof(time));
		
		SQL_FieldNameToNum(hndl,"ip",f_index);
		SQL_FetchString(hndl, f_index, bip, sizeof(bip));
		
		SQL_FieldNameToNum(hndl,"authid",f_index);
		SQL_FetchString(hndl, f_index, bauthid, sizeof(bauthid));
		...
		ConsoleInfoPrint(client, Admin, Reason, time, bauthid, bip);
		CreateTimer(3.0, DelayedKick, pack, TIMER_HNDL_CLOSE);  
		//KickClient(client, "%t", "Banned Check Site", Reason, time, WebsiteAddress);
		//KickClient(client, "Banned by %s, Complains at %s, Reason: %s, Length: %s, Steamid:%s, Ip:%s", Admin,WebsiteAddress, Reason, time, bauthid, bip);
...
public ConsoleInfoPrint(any:client, const String:Admin[],const String:Reason[],const String:time[],const String:bauthid[],const String:bip[]) {
	if (IsClientInGame(client)) {
		decl String:msg[512];
		Format(msg, sizeof(msg), "[SourceBans] ===============================================");
		PrintToConsole(client, msg);
		Format(msg, sizeof(msg), "[SourceBans] Banned by admin %s", Admin);
		PrintToConsole(client, msg);
		Format(msg, sizeof(msg), "[SourceBans] You can complain regarding your ban @ %s", WebsiteAddress);
		PrintToConsole(client, msg);
		Format(msg, sizeof(msg), "[SourceBans] Reason: %s", Reason);
		PrintToConsole(client, msg);
		if (strcmp(time, "0") == 0) {
			Format(msg, sizeof(msg), "[SourceBans] Length: premanent");
		} else {	
			Format(msg, sizeof(msg), "[SourceBans] Length: %s minutes", time);
		}
		PrintToConsole(client, msg);
		Format(msg, sizeof(msg), "[SourceBans] Your SteamID: %s", bauthid);
		PrintToConsole(client, msg);
		Format(msg, sizeof(msg), "[SourceBans] Your IP: %s", bip);
		PrintToConsole(client, msg);
		Format(msg, sizeof(msg), "[SourceBans] ===============================================");
		PrintToConsole(client, msg);
	}
}
public Action:DelayedKick(Handle:timer, any:pack) {
	if(pack != INVALID_HANDLE)
	{
		//reading
		ResetPack(pack); //resets the index to the beginning, necessary for read.
		new String:reason[128];
		new String:time[32];
		new client;
...
public Action:DelayedBan(Handle:timer, any:pack) {
	if(pack != INVALID_HANDLE)
	{
		//reading
		ResetPack(pack); //resets the index to the beginning, necessary for read.
		new String:bannedSite[512];
		new target;
		new time;
		new client;
		...
		client = ReadPackCell(pack);
		ReadPackString(pack, bannedSite, sizeof(bannedSite));
...
stock UTIL_InsertTempBan(time, const String:name[], const String:auth[], const String:ip[], const String:reason[], const String:adminAuth[], const String:adminIp[], Handle:dataPack)
{
	...
	new Handle:pack = CreateDataPack();
	WritePackCell(pack, client);
	WritePackString(pack, banReason);
	WritePackString(pack, blength);
	if(IsClientInGame(client)) {
		ConsoleInfoPrint(client, adminname, reason, blength, auth, ip);
		CreateTimer(3.0, DelayedKick, pack, TIMER_HNDL_CLOSE);  
...
stock PrepareBan(client, target, time, String:reason[], size)
{
	...
	if(CreateBan(client, target, time, reason))
	{
		...
		new String:ip[16];
		GetClientIP(client, ip, sizeof(ip));
		Format(adminname, sizeof(adminname), "unknown");
		Format(bannedSite, sizeof(bannedSite), "%T", "Banned Check Site", target, reason, blength, WebsiteAddress);
		//ConsoleInfoPrint(client, adminname, reason, blength, authid, ip);
		new Handle:pack = CreateDataPack();
		WritePackCell(pack, target);
		WritePackCell(pack, time);
		WritePackString(pack, bannedSite);
		WritePackCell(pack, client);
		CreateTimer(3.0, DelayedBan, pack, TIMER_HNDL_CLOSE);
		//BanClient(target, time, BANFLAG_AUTO, bannedSite, bannedSite, "sm_ban", client);
	}

...
public OnClientAuthorized(client, const String:auth[])
{
	...
	FormatEx(Query, sizeof(Query), "SELECT %s_admins.user as admin, %s_bans.bid, %s_bans.reason, %s_bans.length / 60 as minutes, %s_bans.authid, %s_bans.ip bid FROM %s_admins, %s_bans WHERE ((type = 0 AND authid REGEXP '^STEAM_[0-9]:%s$') OR (type = 1 AND ip = '%s')) AND (length = '0' OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL AND %s_bans.aid = %s_admins.aid", DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix, DatabasePrefix, auth[8], ip, DatabasePrefix, DatabasePrefix);
#if defined DEBUG

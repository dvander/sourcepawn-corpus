...
public OnPluginStart()
{
    ...
    off = -1;
    off = FindSendPropInfo("CBaseCombatWeapon", "m_iPrimaryAmmoType");
    if(off != -1) primaryclipoffset = off;
    ...
    for(new iClient=1;iClient<=maxclients;iClient++)
    {
        if( (ammo[iClient] == 1) && IsClientInGame(iClient))
        {
            ...
            if(IsValidEntity(iWeapon)) {
                ...
                GetEntityClassname(iWeapon, weapon_name, 48);

                if (StrEqual(weapon_name, "weapon_rpg") || StrEqual(weapon_name, "weapon_frag")) {
                    iPrimaryAmmo = GetEntData(iWeapon, primaryclipoffset, 1);
                    SetEntProp(iClient, Prop_Send, "m_iAmmo", 5, _, iPrimaryAmmo);
                    continue;
                }

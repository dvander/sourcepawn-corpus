...
{	
	...
	else if (msgStart == 2 && CheckCommandAccess(client, "sm_psay", ADMFLAG_CHAT)) // sm_psay alias
	{
		...
		if (g_DoColor)
		{
			PrintToChat(client, "\x03(Private to %s) \x04%s: \x03%s", name2, name, message[len]);
			PrintToChat(target, "\x03(Private to %s) \x04%s: \x03%s", name2, name, message[len]);
		}
		else
		{
...
public Action:Command_SmPsay(client, args)
{
	...
	{
		PrintToChat(client, "\x03(Private: %s) \x04%s: \x03%s", name2, name, message);
	}
	else
	{
	...
	if (g_DoColor)
	{
		PrintToChat(target, "\x03(Private: %s) \x04%s: \x03%s", name2, name, message);
	}
	else
	{
...
SendChatToAll(client, String:message[])
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		if (g_DoColor)
		{
			PrintToChat(i, "\x03(ALL) \x04%s: \x03%s", nameBuf, message);
		}
		else
		{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i))
		{
			if (CheckCommandAccess(i, "sm_chat", ADMFLAG_CHAT))
			{
				if (g_DoColor)
				{
					PrintToChat(i, "\x03(ADMINS) \x04%s: \x03%s", name, message);
				}
				else
				{

...
public Hook_OnConVarChanged(Handle:hConVar, const String:sOldValue[], const String:sNewValue[]) {
  ...
  updateConVar(sNewValue, hConVar==g_hPluginEnabled, g_bPluginEnabled, ConVarType:BOOL);
  updateConVar(sNewValue, hConVar==g_hLaser, g_bLaser, ConVarType:BOOL);
  updateConVar(sNewValue, hConVar==g_hLaserAll, g_bLaserAll, ConVarType:BOOL);
  updateConVar(sNewValue, hConVar==g_hLaserHide, g_bLaserHide, ConVarType:BOOL);
  updateConVar(sNewValue, hConVar==g_hChart, g_bChart, ConVarType:BOOL);
  updateConVar(sNewValue, hConVar==g_hRing, g_bRing, ConVarType:BOOL);
  updateConVar(sNewValue, hConVar==g_hCrit, g_bCrit, ConVarType:BOOL);
  updateConVar(sNewValue, hConVar==g_hSound, g_bSound, ConVarType:BOOL);
  updateConVar(sNewValue, hConVar==g_hRave, g_bRave, ConVarType:BOOL);
  
  updateConVar(sNewValue, hConVar==g_hWarnDist, g_fWarnDist, ConVarType:FLOAT);
  updateConVar(sNewValue, hConVar==g_hThreshold, g_fThreshold, ConVarType:FLOAT);
}

public updateConVar(const String:sValue[], bool:bUpdate, &any:aVar, const ConVarType:iType) {
  if (bUpdate) {
    switch (iType) {
      case BOOL: {
...
public Action:Timer_refresh(Handle:timer) {
  ...
  for (new i=1; i<=MaxClients; i++) {
    if (g_rPlayers[i][bActivated] && Client_IsIngameAuthorized(i) && TFTeam:GetClientTeam(i) > TFTeam_Spectator) {
      ...
      // Ignore vertical
      // PrintToServer("fNormalAngle: %.1f, %.1f %.1f", fNormalAngles[0], fNormalAngles[1], fNormalAngles[2]);
      if (FloatAbs(270.0-fNormalAngles[0]) > 10) {
        if (g_bRing) {
          decl Float:fImpactLine[3];
          // SubtractVectors(fClientOrigin, fGroundPoint, fImpactLine);
          NormalizeVector(fNormal, fImpactLine);
          ScaleVector(fImpactLine, 50.0);
          AddVectors(fImpactLine, fGroundPoint, fImpactLine);
          TE_SetupBeamPoints(fGroundPoint, fImpactLine, g_mLaser, g_mHalo, 0, 30, 0.2, 5.0, 5.0, 10, 1.0, iRingColor, 0);

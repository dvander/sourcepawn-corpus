...
public Action:Command_Rof(client, args)
{
	...
	if (CheckCommandAccess(client, "sm_rof_access_target", ADMFLAG_ROOT))
	{
		if (args < 2)
		{
			ReplyToCommand(client, "[SM] Usage: sm_rof <#userid|name> <1.0 - %.1f>", GetConVarFloat(g_hcvarMaxFireSpeep));
	...
	else if (CheckCommandAccess(client, "sm_rof_access", ADMFLAG_GENERIC))
	{
		if (args != 1)
		{
			ReplyToCommand(client, "[SM] Usage: sm_rof <1.0 - %.1f>", GetConVarFloat(g_hcvarMaxFireSpeep));
			return Plugin_Handled;
		}
	...
	if (amount < 1 || amount > GetConVarFloat(g_hcvarMaxFireSpeep))
	{
		ReplyToCommand(client, "[SM] Invalid Amount (max %.1f)", GetConVarFloat(g_hcvarMaxFireSpeep));
		return Plugin_Handled;
	}

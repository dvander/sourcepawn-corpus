...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_savedata", SaveData);
	...
	SQL_TConnect(GotDatabase, "ratio_data");
...
public GotDatabase(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if(!SQL_FastQuery(hDatabase, "CREATE TABLE IF NOT EXISTS `ratio_data` (  `steamid` varchar(20) NOT NULL, `name` text NOT NULL,`kill` int NOT NULL,`death` int NOT NULL,`ratiokd` float NOT NULL,`prima_shot` int NOT NULL,`secon_shot` int NOT NULL,	`melee_swing` int NOT NULL,`time` text NOT NULL,PRIMARY KEY (`steamid`))"))
	{
		SQL_GetError(hDatabase, buffer, sizeof(buffer));
...
public Action:SaveData(client, args)
{
	Update(client);
...
stock Update(client)
{
	new String:name[MAX_NAME_LENGTH * 2 + 1];
	GetClientName(client, name, MAX_NAME_LENGTH);
	SQL_EscapeString(hDatabase, name, name, sizeof(name));
	
	new String:auth[19];
	GetClientAuthString(client, auth, sizeof(auth));
	
	new buffer_len = 74 + strlen(name) + strlen(auth) - 8;
	new String:buffer[buffer_len];
	Format(buffer, buffer_len, "REPLACE `ratio_data` (`steamid` ,`name`)VALUES('%s', '%s');", auth[8], name);
	
	//SQL_TQuery(hDatabase, sql_none, buffer);
	
	new String:sql_query[400];
	
	new Float:ratio = 0.0;
	if(death[client] != 0)
		ratio = float(kill[client])/float(death[client]);
	else
	ratio = float(kill[client]);
	
	
	Format(sql_query, sizeof(sql_query), "REPLACE `ratio_data` (`steamid`, `name`, `kill` ,`death`,`ratiokd`,`prima_shot`,`secon_shot`,`melee_swing`,`time`)VALUES('%s', '%s', '%i', '%i', '%f', '%i', '%i', '%i', '%i:%i:%i');", auth[8], name, kill[client], death[client], ratio, shot_prima[client], shot_secon[client], shot_melee[client], houres[client], mins[client], secs[client]);
	
	PrintToServer(sql_query);
	
	SQL_TQuery(hDatabase, sql_none, sql_query);
}

public sql_none(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	if(hndl == INVALID_HANDLE)
	{
		PrintToServer("RATIO : Error when trying to save data :");
		LogError("%s", error);
	}
}

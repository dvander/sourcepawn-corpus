...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_spam_me", Command_SendAdToPlayer, "Send the next advertissement message to the player");
}
...
public ConVarChange_Interval(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	g_hTimer          = CreateTimer(GetConVarInt(g_hInterval) * 1.0, Timer_DisplayAds, -1, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
}
...
public Action:Command_SendAdToPlayer(client, args)
{
	CreateTimer(0.1, Timer_DisplayAds, client);
}

public Action:Timer_DisplayAds(Handle:timer, any:client)
{
	...
	if(StrContains(sType, "C") != -1)
	{
		...
		else
		{
			if(IsClientInGame(client) && !IsFakeClient(client) &&
				   ((!bAdmins && !(bFlags && HasFlag(client, fFlagList))) ||
					bAdmins && (GetUserFlagBits(client) & (ADMFLAG_GENERIC|ADMFLAG_ROOT))))
				{
					PrintCenterText(client, sText);

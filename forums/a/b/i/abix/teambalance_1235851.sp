...
Versions:
	...
	2.2.3d
		* abix fix repeated section in GetTeamBalance function and rebuild
		* the sequence of conditions. Maintain size now work immediately and
		* independently of other conditions.
		* Added sm_team_balance_min_players
*/
...
#define PLUGIN_VERSION "2.2.3d"
#define MAX_FILE_LEN 80

#define SOUND_CHANGE_TEAM	"npc/roller/mine/combine_mine_deploy1.wav"
...
public AddSound(const String:SoundFile[])
{
	PrecacheSound(SoundFile, true);
	decl String:downloadFile[40];
	Format(downloadFile, 40, "sound/%s", SoundFile);
...
public OnPluginStart()
{
	...
	g_CvarAnnounce = CreateConVar("sm_team_balance_announce", "2", "Announcement preferences");
	g_CvarMinPlayers = CreateConVar("sm_team_balance_min_players", "7", "Minimum in-game players for teambalance");
	...
	g_CvarConsecutiveWins = CreateConVar("sm_team_balance_consecutive_wins", "5", "The number of consecutive wins required to declare the teams unbalanced");
	g_CvarWinLossRatio = CreateConVar("sm_team_balance_wlr", "0.45", "The win loss ratio required to declare the teams unbalanced");
	...
	g_CvarSaveTime = CreateConVar("sm_team_balance_save_time", "3", "The number of hours to save stats for");
	...
	g_CvarSingleMax = CreateConVar("sm_team_balance_single_max", "8", "The maximimum number of players on a team for which a single player is balanced");
...
public GetTeamBalance()
{
	...
	// Check for minimum players required on server	
	g_playerCount = 0;
	for(new i = 1; i <= GetMaxClients(); i++)
	{
		if(IsClientInGame(i))
	...
	if(GetConVarInt(g_CvarMinPlayers) > g_playerCount)
	{
...
public bool:BalanceTeams()
{
	...
	if (bottomPlayer < 0) return false;
...
public SwitchTeam(client, team)
{
	...
	CreateTimer(1.2, TimerDelay, client);
}

public Action:TimerDelay(Handle:timer, any:data)
{
	if(IsClientInGame(data)) {
		PrintHintText(data, "You have switched team");
	}
}
...
public InitializeStats()
{
	...
	if(StrEqual(connection, ""))
	{
		...
		KvSetString(KV,"database","team_balance");
		sqlTeamBalanceStats = SQL_ConnectCustom(KV,error,sizeof(error),true);
...
public PrintTranslatedToChatAll(String:buffer[])
{
	...
	for(new i = 1; i <= GetMaxClients(); i++)
	{
		...
		{
			PrintToChat(i, "\x04[Team Balance]: \x01%t", buffer);
		}
	}
}
...
public Action:CommandDump(client, args)
{
	...
	ReplyToCommand(client, "sm_team_balance_min_players: %i", GetConVarInt(g_CvarMinPlayers));

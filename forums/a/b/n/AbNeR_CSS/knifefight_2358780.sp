...
public OnPluginStart()
{
  ...
  CreateConVar("sm_knifefight_version", VERSION, "Knife Fight Version || PLEASE DO NOT CHANGE THIS",  FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
public Action:UserMsg_VGUIMenu(UserMsg:msg_id, Handle:bf, const players[], playersNum, bool:reliable, bool:init)
{
	...
	decl String:type[15];
	//CSGO fix by AbNeR_CSS
	if (GetUserMessageType() == UM_Protobuf)
	{
		PbReadString(bf, "name", type, sizeof(type));
		if(PbReadBool(bf, "show") && PbGetRepeatedFieldCount(bf, "subkeys") == 0 && strcmp(type, "scores", false) == 0)
		{
			g_intermission = true;
		}
	}

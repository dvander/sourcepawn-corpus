...
#define PLUGIN_VERSION "1.0.3.4"

public Plugin:myinfo = {
	name = "TF2 Classic",
	author = "CrancK,ktrain",
...
public OnPluginStart() 
{
	...
	cvWaitPeriod = CreateConVar("tf2c_nades_waitperiod", "1", "server waits for players on map start (1=true 0=false)(RECOMMENDED!)", 0);
	cvEmpRadius = CreateConVar("tf2c_nades_emp_radius", "256.0", "radius for emp nade", 0, true, 1.0, true, 2048.0);
	cvEmpDelay = CreateConVar("tf2c_nades_emp_delay", "3.0", "delay in seconds before nade explodes", 0, true, 1.0, true, 10.0);
	cvEmpNum[1] = CreateConVar("tf2c_nades_emp_max", "4", "max number of emp nades", 0, true, 0.0, true, 10.0); 
	cvEmpNum[0] = CreateConVar("tf2c_nades_emp_min", "2", "number of emp nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0); 
	cvEmpIgnore = CreateConVar("tf2c_nades_emp_ignorewalls", "1", "enables the blast to go through walls", 0);
	cvEmpExplosion = CreateConVar("tf2c_nades_emp_explosion", "1", "adds an explosion particle effect to indicate ammo blowing up", 0);
	cvHallucDamage = CreateConVar("tf2c_halluc_damage", "5", "damage done by hallucination nade", 0);
	cvHallucDelay = CreateConVar("tf2c_nades_hallucination_time", "10.0", "delay in seconds that effects last", 0, true, 1.0, true, 10.0);	
	cvHallucRadius = CreateConVar("tf2c_nades_hallucination_radius", "256.0", "radius for hallincation nade", 0, true, 1.0, true, 2048.0);
	cvHallucNum[1] = CreateConVar("tf2c_nades_hallucination_max", "4", "max number of hallucination nades", 0, true, 0.0, true, 10.0); 
	cvHallucNum[0] = CreateConVar("tf2c_nades_hallucination_min", "2", "number of hallucination nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0); 
	cvNapalmDamage = CreateConVar("tf2c_nades_napalm_damage", "25", "initial damage for napalm nade", 0, true, 1.0, true, 500.0);
	cvNapalmRadius = CreateConVar("tf2c_nades_napalm_radius", "320.0", "radius for napalm nade", 0, true, 1.0, true, 2048.0);
	cvNapalmNum[1] = CreateConVar("tf2c_nades_napalm_max", "3", "max number of napalm nades", 0, true, 0.0, true, 10.0); 
	cvNapalmNum[0] = CreateConVar("tf2c_nades_napalm_min", "2", "number of napalm nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0); 
	cvHealthDelay = CreateConVar("tf2c_nades_health_delay", "3.0", "delay in seconds before nade explodes", 0, true, 1.0, true, 10.0);
	cvHealthRadius = CreateConVar("tf2c_nades_health_radius", "384.0", "radius for health nade", 0, true, 1.0, true, 2048.0);
	cvHealthNum[1] = CreateConVar("tf2c_nades_health_max", "1", "max number of health nades", 0, true, 0.0, true, 10.0); 
	cvHealthNum[0] = CreateConVar("tf2c_nades_health_min", "1", "number of health nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0); 
	cvHealNadePower = CreateConVar("tf2c_nades_health_power", "1000", "amount of total hp the heal nade can heal",0);
	cvHealNadeOverheal = CreateConVar("tf2c_nades_health_overheal", "1", "enables the healnade to overheal", 0);
	cvMirvSpread = CreateConVar("tf2c_nades_mirv_spread", "384.0", "spread of secondary explosives (max speed)", 0, true, 1.0, true, 2048.0);	
	cvMirvDamage2 = CreateConVar("tf2c_nades_mirv_damage2", "60.0", "damage done by secondary explosion of mirv nade", 0, true, 1.0, true, 500.0);	
	cvMirvDamage1 = CreateConVar("tf2c_nades_mirv_damage1", "60.0", "damage done by main explosion of mirv nade", 0, true, 1.0, true, 500.0);
	cvMirvRadius = CreateConVar("tf2c_nades_mirv_radius", "128.0", "radius for demo's nade", 0, true, 1.0, true, 2048.0);
	cvMirvNum[2] = CreateConVar("tf2c_nades_mirv_heavy_min", "1", "number of mirv nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0); 
	cvMirvNum[3] = CreateConVar("tf2c_nades_mirv_heavy_max", "2", "max number of mirv nades given to Heavy", 0, true, 0.0, true, 10.0); 
	cvMirvNum[0] = CreateConVar("tf2c_nades_mirv_demo_min", "2", "number of mirv nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0); 
	cvMirvNum[1] = CreateConVar("tf2c_nades_mirv_demo_max", "3", "max number of mirv nades given to Demo", 0, true, 0.0, true, 10.0); 
	cvNailDamageExplode = CreateConVar("tf2c_nades_nail_explodedamage", "80.0", "damage done by final explosion", 0, true, 1.0, true,1000.0);
	cvNailDamageNail = CreateConVar("tf2c_nades_nail_naildamage", "8.0", "damage done by nail projectile", 0, true, 1.0, true, 500.0);
	cvNailRadius = CreateConVar("tf2c_nades_nail_radius", "256.0", "radius for nail nade", 0, true, 1.0, true, 2048.0);
	cvNailNum[1] = CreateConVar("tf2c_nades_nail_max", "2", "max number of nail nades", 0, true, 0.0, true, 10.0);
	cvNailNum[0] = CreateConVar("tf2c_nades_nail_min", "1", "number of nail nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0);
	cvConcDamage = CreateConVar("tf2c_nades_conc_damage", "10", "damage done by concussion nade", 0);
	cvConcForce = CreateConVar("tf2c_nades_conc_force", "3.0", "force applied by concussion nade", 0);
	cvConcDelay = CreateConVar("tf2c_nades_conc_delay", "3.0", "delay in seconds before nade explodes", 0, true, 1.0, true, 10.0);
	cvConcRadius = CreateConVar("tf2c_nades_conc_radius", "256.0", "radius for concussion nade", 0, true, 1.0, true, 2048.0);
	cvConcNum[1] = CreateConVar("tf2c_nades_conc_max", "4", "max number of conc nades", 0, true, 0.0, true, 10.0);
	cvConcNum[0] = CreateConVar("tf2c_nades_conc_min", "2", "number of conc nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0);
	cvConcIgnore = CreateConVar("tf2c_nades_conc_ignorewalls", "1", "enables the blast to go through walls", 0);
	cvConcRings = CreateConVar("tf2c_nades_conc_rings", "10.0", "amount of rings for conc blast effect", 0, true, 0.0, true, 50.0);
	cvFragDamage = CreateConVar("tf2c_nades_frag_damage", "100", "damage done by concussion nade", 0);
	cvFragRadius = CreateConVar("tf2c_nades_frag_radius", "320.0", "radius for concussion nade", 0, true, 1.0, true, 2048.0);
	cvFragNum[ENGIE][1] = CreateConVar("tf2c_nades_frag_engineer_max", "2", "max number of frag nades", 0, true, 0.0, true, 10.0);
	cvFragNum[ENGIE][0] = CreateConVar("tf2c_nades_frag_engineer_min", "1", "number of frag nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0);
	cvFragNum[SPY][1] = CreateConVar("tf2c_nades_frag_spy_max", "2", "number of frag nades given", 0, true, 0.0, true, 10.0);
	cvFragNum[SPY][0] = CreateConVar("tf2c_nades_frag_spy_min", "1", "number of frag nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0);
	cvFragNum[PYRO][1] = CreateConVar("tf2c_nades_frag_pyro_max", "3", "number of frag nades given", 0, true, 0.0, true, 10.0);
	cvFragNum[PYRO][0] = CreateConVar("tf2c_nades_frag_pyro_min", "2", "number of frag nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0);
	cvFragNum[HEAVY][1] = CreateConVar("tf2c_nades_frag_heavy_max", "4", "number of frag nades given", 0, true, 0.0, true, 10.0);
	cvFragNum[HEAVY][0] = CreateConVar("tf2c_nades_frag_heavy_min", "2", "number of frag nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0);
	cvFragNum[MEDIC][1] = CreateConVar("tf2c_nades_frag_medic_max", "2", "number of frag nades given", 0, true, 0.0, true, 10.0);
	cvFragNum[MEDIC][0] = CreateConVar("tf2c_nades_frag_medic_min", "1", "number of frag nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0);
	cvFragNum[DEMO][1] = CreateConVar("tf2c_nades_frag_demo_max", "3", "number of frag nades given", 0, true, 0.0, true, 10.0);
	cvFragNum[DEMO][0] = CreateConVar("tf2c_nades_frag_demo_min", "2", "number of frag nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0);
	cvFragNum[SOLDIER][1] = CreateConVar("tf2c_nades_frag_soldier_max", "4", "number of frag nades given", 0, true, 0.0, true, 10.0);
	cvFragNum[SOLDIER][0] = CreateConVar("tf2c_nades_frag_soldier_min", "2", "number of frag nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0);
	cvFragNum[SNIPER][1] = CreateConVar("tf2c_nades_frag_sniper_max", "3", "number of frag nades given", 0, true, 0.0, true, 10.0);
	cvFragNum[SNIPER][0] = CreateConVar("tf2c_nades_frag_sniper_min", "2", "number of frag nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0);
	cvFragNum[SCOUT][1] = CreateConVar("tf2c_nades_frag_scout_max", "2", "number of frag nades given", 0, true, 0.0, true, 10.0);
	cvFragNum[SCOUT][0] = CreateConVar("tf2c_nades_frag_scout_min", "1", "number of frag nades given at start and at spawn lockers", 0, true, 0.0, true, 10.0);
	cvNadeTrail = CreateConVar("tf2c_nades_showtrail", "1", "enables a trail behind nades", 0|FCVAR_NOTIFY);
	cvNadesThrowSpeed = CreateConVar("tf2c_nades_throwspeed", "950.0", "this + the playerspeed = speed at which the nade is thrown", 0|FCVAR_NOTIFY);
	cvNadesThrowPhysics = CreateConVar("tf2c_nades_throwphysics", "1.0", "???", 0|FCVAR_NOTIFY);
	...
	cvBlastDistanceMin = CreateConVar("tf2c_nades_blastdistancemin", "0.75", "minimum blast radius", 0|FCVAR_NOTIFY);
	cvHHincrement = CreateConVar("tf2c_nades_hhincrement", "2.95", "speed gain from HH", 0|FCVAR_NOTIFY , true, 1.0, true, 5.0);
	cvSoundEnabled = CreateConVar("tf2c_nades_soundEnabled", "1.0", "nade sounds enabled on 1", 0|FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvPickupsAllowed = CreateConVar("tf2c_pickupsAllowed", "1.0", "health & nade pickups", 0|FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvPlayerNadeBeep = CreateConVar("tf2c_nades_selfbeep", "1.0", "enables self-beep", 0|FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvShowInfo = CreateConVar("tf2c_nades_showinfo", "1", "enables health/nades display for clients", 0|FCVAR_NOTIFY);
	cvRefill = CreateConVar("tf2c_jump_refill", "0", "Allow refill for on jump maps", 0|FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvStartNades = CreateConVar("tf2c_startnades", "0", "Enables/disables nades on spawn, 1=min amounts, 2=max amounts", 0|FCVAR_NOTIFY);
	cvAirAccel = CreateConVar("tf2c_aa", "20", "airacceleration",0|FCVAR_NOTIFY);
	cvPyroFly = CreateConVar("tf2c_pyrofly_enabled", "1", "enables pyro's to fly", 0|FCVAR_NOTIFY);
	cvBunnyEnabled = CreateConVar("tf2c_bhop_enabled", "1", "enables bunnyhopping/trimping/pyrofly & bhopdoublejump", 0|FCVAR_NOTIFY);
	cvTrimpEnabled = CreateConVar("tf2c_bhop_trimp_enabled", "1", "enables trimping", 0|FCVAR_NOTIFY);
	cvSandManDJump = CreateConVar("tf2c_sandmandjump_enabled", "0", "on 1, enables sandman scouts, to do a bodged up doublejump", 0|FCVAR_NOTIFY);
	cvWallJump = CreateConVar("tf2c_walljump_enabled", "0", "enables the spy to wall-jump", 0|FCVAR_NOTIFY);
	cvWallJumpPower = CreateConVar("tf2c_walljump_power", "266.66", "power of the spy-walljump", 0|FCVAR_NOTIFY);
	cvWallJumpDelay = CreateConVar("tf2c_walljump_delay_g2w", "0.25", "delay in seconds before being able to walljump again from ground", 0|FCVAR_NOTIFY);
	cvWallJumpDelay2 = CreateConVar("tf2c_walljump_delay_w2w", "0.75", "delay in seconds before being able to walljump again from walls", 0|FCVAR_NOTIFY);
	cvWallJumpDistance = CreateConVar("tf2c_walljump_distance", "40.0", "distance to wall", 0|FCVAR_NOTIFY);
	cvWallJumpMinSpeed = CreateConVar("tf2c_walljump_minspeed", "133.33", "minimum total speed required to walljump", 0|FCVAR_NOTIFY);
	cvNadeHHMin = CreateConVar("tf2c_nades_hh_min", "1.5", "minimum boost increment from handhelding a nade",  0|FCVAR_NOTIFY);
	cvDJumpDelay = CreateConVar("tf2c_djump_delay_min", "0.15", "q3 style doublejump, min delay, before djump can be used, if 0.0 then doublejump is off", 0|FCVAR_NOTIFY, true, 0.0, true, 0.25);
	cvDJumpDelay2 = CreateConVar("tf2c_djump_delay_max", "0.55", "q3 style doublejump, max delay, until djump can be used", 0, true, 0.1, true, 0.65);
	cvDJumpMax = CreateConVar("tf2c_djump_max", "800.0", "q3 style doublejump, limit to height boost", 0);
	cvDJumpTBoost = CreateConVar("tf2c_djump_time_boost", "1.15", "depending on how quickly the doublejump is performed, more boost is gained, 1.15 = 100% if tf2c_djump_delay_min = 0.15", 0);
	cvSpeedCheck = CreateConVar("tf2c_speed_check", "0", "makes sure you don't slow down from bunnyhopping", 0);
	Speedo = CreateConVar("tf2c_speedometer", "0", "ALWAYS show speedometer for all clients", 0);
	SpeedoOff = CreateConVar("tf2c_speedometer_off", "0", "Never show speedometer for all clients", 0);
	BunnyMode = CreateConVar("tf2c_bhop_Mode", "3", "Changes how bunnyhopping increases speed, 0 for normal, 1 for only turning speed, 2 for set speed 3 for set everything", 0|FCVAR_NOTIFY, true, 0.0, true, 6.0);
	BunnyIncrement = CreateConVar("tf2c_bhop_Increment", "1.01", "Changes bunnyhop speedincrease if bhopMode is set to 2", 0|FCVAR_NOTIFY , true, 1.0, true, 2.0);
	BunnyCap = CreateConVar("tf2c_bhop_Cap", "1.8", "Changes the bunnyhop speed cap if bhopMode is set to 2", 0|FCVAR_NOTIFY, true, 1.0, true, 5.0);
	BunnySlowDown = CreateConVar("tf2c_bhop_Slowdown", "0.10", "in bhopmode 3, if above the bhopcap, slow down rate", 0|FCVAR_NOTIFY, true, 0.000, true, 1.000);
	SelfBoostX = CreateConVar("tf2c_selfdamageBoostX", "1.0", "Changes the selfhurting increment (horizontal)", 0|FCVAR_NOTIFY, true, 1.0, true, 5.0);
	SelfBoostY = CreateConVar("tf2c_selfdamageBoostY", "1.0", "Changes the selfhurting increment (vertical)", 0|FCVAR_NOTIFY, true, 1.0, true, 5.0);
	g_fovEnabled = CreateConVar("tf2c_fovEnabled", "1.0", "allows changing of fov", 0|FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvFovMax = CreateConVar("tf2c_fovmax", "160", "set's the max limit for the fov command", 0|FCVAR_NOTIFY);
	g_scale = CreateConVar("tf2c_rspeed", "1.05", "Rocket speed mult", 0|FCVAR_NOTIFY);
	cvExplosionPower = CreateConVar("tf2c_power", "4.0", "explosionpower default = 4.0 or 0.0, quadjump = 16.0 or 12.0", 0|FCVAR_NOTIFY);
	cvExplosionRadius = CreateConVar("tf2c_quadradius", "128.0", "radius of rocket explosion", 0|FCVAR_NOTIFY);
	cvQuadThreshold = CreateConVar("tf2c_quadthreshold", "64.0", "perfect point for max boost, in distance from middle of explosion", 0|FCVAR_NOTIFY);
	cvInfectEnabled = CreateConVar("tf2c_infect_enabled", "1.0", "enables infection", 0|FCVAR_NOTIFY);
	cvInfectHits = CreateConVar("tf2c_infect_hits", "5.0", "amount of hits needed to infect players", 0|FCVAR_NOTIFY);
	cvInfectBaseDamage = CreateConVar("tf2c_infect_basedamage", "2.0", "base amount of damage from infection", 0|FCVAR_NOTIFY);
	cvInfectDis[0] = CreateConVar("tf2c_infect_mindistance", "128.0", "upto this range, infection is 100% effective", 0|FCVAR_NOTIFY);
	cvInfectDis[1] = CreateConVar("tf2c_infect_maxdistance", "640.0", "max range of infection, any further and the needle wont infect", 0|FCVAR_NOTIFY);
	cvForcedShowInfo = CreateConVar("tf2c_forced_showinfo", "0", "makes all clients have this mode of showinfo displayed", 0|FCVAR_NOTIFY);
	cvSniperShot = CreateConVar("tf2c_snipershot_enabled", "0", "makes snipers shots slow people down (only works with bunnymode 3)", 0|FCVAR_NOTIFY);
	cvSniperShotDelay = CreateConVar("tf2c_snipershot_delay", "5.0", "time in seconds for which the debuff lasts", 0|FCVAR_NOTIFY);
	cvSniperShotAmount = CreateConVar("tf2c_snipershot_debuffamount", "0.1", "amount of slowdown per shot, if you change this, mapchange is recommended", 0|FCVAR_NOTIFY);
	cvDoors = CreateConVar("tf2c_quickdoors_enabled", "1", "enables/disables doors to be opened instantly, needs mapchange", 0|FCVAR_NOTIFY);
	cvShowJumpInfo = CreateConVar("tf2c_jump_showinfo", "0", "shows info about jumps, 1 for speed, 2 for distance, 3 for both, 4 for detailed", 0|FCVAR_NOTIFY);
	cvArmor = CreateConVar("tf2c_armor_enabled", "1", "enables armor", 0|FCVAR_NOTIFY);
	cvArmorAmounts[0][0] = CreateConVar("tf2c_armor_light_fraction", "0.4", "amount of damage reduction from light armor", 0);
	cvArmorAmounts[1][0] = CreateConVar("tf2c_armor_medium_fraction", "0.5", "amount of damage reduction from medium armor", 0);
	cvArmorAmounts[2][0] = CreateConVar("tf2c_armor_heavy_fraction", "0.6", "amount of damage reduction from heavy armor", 0);
	cvArmorAmounts[0][1] = CreateConVar("tf2c_armor_light_max", "100", "max amount for light armor", 0);
	cvArmorAmounts[1][1] = CreateConVar("tf2c_armor_medium_max", "150", "max amount for medium armor", 0);
	cvArmorAmounts[2][1] = CreateConVar("tf2c_armor_heavy_max", "200", "max amount for heavy armor", 0);
	cvStartArmor = CreateConVar("tf2c_armor_startamount", "2", "if armor is enabled, 0 = 0 start armor, 1 = 1/4 start armor, 2 = 1/2 start armor, 3 = full start armor", 0|FCVAR_NOTIFY);
	cvQuickSwitch = CreateConVar("tf2c_quickswitch_enabled", "0", "enables quickswitch", 0);
	//cvFDMulti = CreateConVar("tf2c_falldamage_multiplier", "0.6", "reduces/increases falldamage, 0.0 = no change", 0|FCVAR_NOTIFY);
	g_hInterval = CreateConVar("tf2c_hphud_interval", "5", "How often health timer is updated (in tenths of a second).");
	CreateConVar("tf2classic_version", PLUGIN_VERSION, "TF2Classic Version", 0|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	offsFOV = FindSendPropInfo("CBasePlayer", "m_iFOV");
	offsDefaultFOV = FindSendPropInfo("CBasePlayer", "m_iDefaultFOV");
	...
	for(k=1;k<=MaxClients;k++)
	{
		if(IsValidEntity(k) && i != 34)
		{
			if(IsClientAuthorized(k) && IsClientConnected(k))
			{
				...
				{
					new class = view_as<int>(TF2_GetPlayerClass(i));
		...
		if(StrEqual(weaponName, "n_wrench"))
		{
			...
			if(TR_DidHit(tr))
			{
				...
				if(dis <= 64.0)
				{
					...
					if(ent<=MaxClients && ent != 0 && ent != -1)
					{
						new metalvalue;
						metalvalue=GetMetalAmount(client);
						new class = view_as<int>(TF2_GetPlayerClass(ent));
						if(metalvalue-25>0)
						{
	...
	if(GetConVarInt(cvArmor)==1)
	{
		if(!Piercing[victim] && hit[victim] && damage > 0)
		{
			if(armor[victim] > 0)
			{
				...
				new tf2class = view_as<int>(TF2_GetPlayerClass(victim));
		...
		if( IsValidEntity(i) && IsClientInGame(i) && IsPlayerAlive(i) )
		{
			if(GetConVarInt(cvBunnyEnabled)==1)
			{
				...
				if(GetConVarInt(cvWallJump)==1)
				{
					...
					{
						new tempclass = view_as<int>(TF2_GetPlayerClass(i));
						if(tempclass==SPY)
						{
		...
		{ 
			...
			for (j=1;j<=maxplayers;j++)
			{
				if(PlayersInRange[j]>0.0)
				{
					...
					ScaleVector(play, 1.0 - (distance / radius));
		...
		else if(StrEqual(classname, "obj_dispenser")) 
		{
			...
			for (j=1;j<=maxplayers;j++)
			{
				if(PlayersInRange[j]>0.0)
				{
					...
					if (distance<0.01) { distance = 0.01; }
					dynamic_damage = damageFloat - ((damageFloat * 0.75) * (distance / radius));
		...
		else
		{
			...
			for (j=1;j<=maxplayers;j++)
			{
				if(PlayersInRange[j]>0.0)
				{
					...
					new String:TempString[32];
					FloatToString((distance / radius), TempString, 32);
					if(GetConVarInt(cvShowDistance) == 1){PrintCenterText(j,"%s", TempString);}
					ScaleVector(play, (distance / radius));
	...
	else
	{
		switch (class)
		{
				...
				for (j=1;j<=maxplayers;j++)
				{
					...
					{	
						if(!gDrugged[j]){
							...
							distance = GetVectorLength(origin);
							if (distance<0.01) { distance = 0.01; }
							dynamic_time = hDelay - ((hDelay * 0.60) * (distance / radius));
			...
			case ENGIE:
			{
				...
				new ammoLostScale = 0;
				new damage = 0;
				m_Offset=FindSendPropInfo("CTFPlayer","m_iAmmo"); //Find the offset 
				for (j=1;j<=maxplayers;j++)
				{
					if(j!=client)
					{
...
HurtPlayer(client, attacker, damage, String:weapon[256], bool:explosion, Float:pos[3], Float:knockbackmult = 4.0, type = 0)
{
	...
	if(GetConVarInt(cvArmor)==1)
	{
		if(damage > 0)
		{
			...
			new tf2class = view_as<int>(TF2_GetPlayerClass(client));
			if(tf2class == SCOUT || tf2class == SNIPER || tf2class == SPY)//light armor
			{
				if(armor[client] > 0)
				{

...
//
//	TODO:
//  - Assess survivor morale ability. Is it functioning correctly? Is it necessary? Should it be repalced? Rewrite necessary?
//	- Move plugin item balance system to a tf2items based system.
//	- Add in Zombie skin while on blue. Heavily requested. Seems impossible todo through just skins?
//
// Features:
...
public OnPluginStart()
{
  ...
  zf_cvRatio = CreateConVar("sm_zf_ratio", "0.65", "<0.01-1.00> Percentage of players that start as survivors. Defaults to 65 percent.", FCVAR_PLUGIN, true, 0.01, true, 1.0);
...
{  
  ...
  // Apply crippling backstab if is enabled for 10 seconds
    ...
    if (victim != attacker)
    {
      if (attackWasBackstab(attacker, inflictor, damagetype) && !HasRazorback(victim))
	  {
        if (!zf_bBackstabbed[victim])
        {
		  ...
		  // Stuns player
		  TF2_StunPlayer(victim, 10.0, 1.0, TF_STUNFLAGS_BIGBONK|TF_STUNFLAG_NOSOUNDOREFFECT, attacker);
		  ...
		  EmitSoundToClient(victim, SOUND_MARKED);
		  ...
		  CreateTimer(10.0, RemoveBackstab, victim);
...
public Action:hook_VoiceMenu(client, const String:command[], argc)
{
  ...
  if(StrEqual(cmd1, "0") && StrEqual(cmd2, "0"))
  {
    ...
    {    
      ...
      {
        //return failure callout if cannt activate rage.
...
public Action:event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
  ...
  else if (zf_crippleMode == 1) PrintToChatAll("\x05[ZF]\x01 Crippling backstab is enabled. Backstabs cripple players for 10 seconds!");
  ...
  if(IsClientInGame(client) && IsPlayerAlive(client))
  {
    ...
    for(new i = 0; i < 5; i++)
    {
      ...
      if(weaponId > 0) switch(weaponId)
      {
        ...
        case ZFWEAP_FISTSOFSTEEL: PrintToChat(client, "\x05[ZF ITEMS]\x01 The Fists of Steel deals 15% less damage and incurs a -30 speed penalty.");
...
handle_survivorAbilities()
{
  ...
  for(new i = 1; i <= MaxClients; i++)
  {
    if(IsClientInGame(i) && IsPlayerAlive(i) && isSur(i))
    {
      ...
      //    All pyro flamethrower's ammo limited to 125.
  ...
  for(new i = 0; i < playerCount; i++)
  {
    ...
    {            
      for(new j = i+1; j < playerCount; j++)
      {
        ...
        {
		// value changed from 200000 to 5000
          if(GetVectorDistance(playerPos[j], playerPos[curPlayer], true) <= 5000)
          {
...
public help_printZFInfoChat(client)
{
  ...
  PrintToChatAll("\x05[ZF]\x01 Go to \x04forums.alliedmods.net/showthread.php?t=230781 \x01to provide feedback.");
}

...
public Plugin:myinfo = 
{
	...
	author = "kwski43 (original author: Dionys)",
	description = "Force change to next mission when current mission ends.",
	version = Version,
	url = "http://forums.alliedmods.net/showthread.php?t=81982"
};
...
public OnPluginStart()
{
	...
	HookEvent("scavenge_match_finished", Event_FinalWin);

	CreateConVar("l4d2_fmc_version", Version, "Version of L4D2 Force Mission Changer plugin.", FCVAR_NOTIFY);
	DebugEvent = CreateConVar("l4d2_fmc_dbug", "0", "on-off Write event to log file.");
	Allowed = CreateConVar("l4d2_fmc", "1", "Enables Force changelevel when mission end.");
	AllowedDie = CreateConVar("l4d2_fmc_ifdie", "1", "Enables Force changelevel when all player die on final map in coop/realism/survival gamemode.");
	DefM = CreateConVar("l4d2_fmc_def", "c1m1_hotel", "Mission for change by default.");
	ChDelayVS = CreateConVar("l4d2_fmc_chdelayvs", "10.0", "Delay before versus/scavenge mission change (float in sec).");
	ChDelayCOOP = CreateConVar("l4d2_fmc_chdelaycoop", "5.0", "Delay before coop/realism/survival mission change (float in sec).");

	TimerRoundEndBlockVS = CreateConVar("l4d2_fmc_re_timer_block", "1.0", "Time in which current event round_end is not considered (float in sec).");
	cvarAnnounce = CreateConVar("l4d2_fmc_announce", "1", "Enables next mission to advertise to players.");
	
	//For custom crec
	RegServerCmd("l4d2_fmc_crec_add", Command_CrecAdd, "Add custom value l4d2_fmc_crec and l4d2_fmc_re_timer_block for the specified map. Max 50.");
	RegServerCmd("l4d2_fmc_crec_clear", Command_CrecClear, "Clear all custom value l4d2_fmc_crec and l4d2_fmc_re_timer_block.");
	RegServerCmd("l4d2_fmc_crec_list", Command_CrecList, "Show all custom value l4d2_fmc_crec and l4d2_fmc_re_timer_block.");

	CurrentGameMode = FindConVar("mp_gamemode");
	HookConVarChange(CurrentGameMode, OnCVGameModeChange);

	logfile = OpenFile("/addons/sourcemod/logs/fmc_logevent.log", "w");
}
...
public OnClientDisconnect(client)
{
	if (GetClientCount(true) <= 0)
	{
		...
		{
			decl String:mBuffer[128];
			Format(mBuffer, sizeof(mBuffer), "MODE: \"%d\" MAP: \"%s\" EVENT: changemission to \"%s\" REASON: Server is empty", l4d2_gamemode(), current_map, EmptyServerMap);
			WriteFileLine(logfile, mBuffer);
		}
...
public Action:Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetConVarInt(DebugEvent) == 1)
	{
		...
		PrintToChatAll("\x04[FMC DEBUG]\x03 MODE: \"%d\" EVENT: \"%s\" NUM: \"%d\" TWIN: \"%d\"", l4d2_gamemode(), name, RoundEndCounter, winnerteam);
		decl String:mBuffer[128];
		Format(mBuffer, sizeof(mBuffer), "MODE: \"%d\" MAP: \"%s\" EVENT: \"%s\" NUM: \"%d\" TWIN: \"%d\"", l4d2_gamemode(), current_map, name, RoundEndCounter, winnerteam);
		WriteFileLine(logfile, mBuffer);
	}
	
	if(GetConVarInt(Allowed) == 1 && l4d2_gamemode() == 2 && StrEqual(next_mission_force, "none") != true && 4 != 0 && RoundEndCounter >= RoundEndCounterValue)
	{
		if (GetConVarInt(DebugEvent) == 1)
		{
			PrintToChatAll("\x04[FMC DEBUG]\x03 MODE: \"%d\" EVENT: START FMC TIMER ", l4d2_gamemode());
			decl String:mBuffer[128];
			Format(mBuffer, sizeof(mBuffer), "MODE: \"%d\" MAP: \"%s\" EVENT: START FMC TIMER ", l4d2_gamemode(), current_map);
			WriteFileLine(logfile, mBuffer);
		}
	...
	if(l4d2_gamemode() == 3 && StrEqual(next_mission_force, "none") != true)
	{
		if (GetConVarInt(DebugEvent) == 1)
		{
			PrintToChatAll("\x04[FMC DEBUG]\x03 MODE: \"%d\" EVENT: START FMC TIMER ", l4d2_gamemode());
	...
	if(l4d2_gamemode() == 4 && StrEqual(next_mission_force, "none") != true)
	{
		if (GetConVarInt(DebugEvent) == 1)
		{
			PrintToChatAll("\x04[FMC DEBUG]\x03 MODE: \"%d\" EVENT: START FMC TIMER ", l4d2_gamemode());
		...
		CreateTimer(RoundEndBlockValue, TimerChDelaySCAV);
		RoundEndCounter = 0;
	}
...
public Action:Event_FinalWin(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GetConVarInt(DebugEvent) == 1)
	{
		PrintToChatAll("\x04[FMC DEBUG]\x03 MODE: \"%d\" EVENT: \"%s\" ", l4d2_gamemode(), name);
		decl String:mBuffer[128];
		Format(mBuffer, sizeof(mBuffer), "MODE: \"%d\" MAP: \"%s\" EVENT: \"%s\" ", l4d2_gamemode(), current_map, name);
		WriteFileLine(logfile, mBuffer);
	}

	if(GetConVarInt(Allowed) == 1 && l4d2_gamemode() == 1 && StrEqual(next_mission_force, "none") != true)
...
public Action:Event_FinalLost(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetConVarInt(Allowed) == 1 && GetConVarInt(AllowedDie) && l4d2_gamemode() == 1 && StrEqual(next_mission_force, "none") != true)
		CreateTimer(GetConVarFloat(ChDelayCOOP), TimerChDelayCOOP);
}
...
public Action:TimerAnnounce(Handle:timer, any:client)
{
	if(IsClientInGame(client))
	{
		...
		{
			PrintToChat(client, "\x04[FMC]\x03 It's a finale mission. Finish them all!");
...
public Action:TimerChDelayVS(Handle:timer)
{
	if (GetConVarInt(DebugEvent) == 1)
	{
		PrintToChatAll("\x04[FMC DEBUG]\x03 MODE: \"%d\" EVENT: changemission to \"%s\" ", l4d2_gamemode(), next_mission_force);
		decl String:mBuffer[128];
		Format(mBuffer, sizeof(mBuffer), "MODE: \"%d\" MAP: \"%s\" EVENT: changemission to \"%s\" ", l4d2_gamemode(), current_map, next_mission_force);
		WriteFileLine(logfile, mBuffer);
	}
...
public Action:TimerChDelaySCAV(Handle:timer)
{
	if (GetConVarInt(DebugEvent) == 1)
	{
		PrintToChatAll("\x04[FMC DEBUG]\x03 MODE: \"%d\" EVENT: changemission to \"%s\" ", l4d2_gamemode(), next_mission_force);
...
{
	g_ArrayCount = 0;
	PrintToServer("[FMC] Custom value l4d2_fmc_crec now is clear.");
}
...
public Action:Command_CrecAdd(args)
{
	...
	{
		PrintToServer("[FMC] Max number of array line for l4d2_fmc_crec_add reached.");
		return;
	}
	...
	if (IsMapValid(cmdarg1) && StringToInt(cmdarg2) != 0 && StringToFloat(cmdarg3) != 0.0)
	{
		...
		PrintToServer("[FMC] Error command. Use: l4d2_fmc_crec_add <existing custom map> <custom l4d2_fmc_crec integer value (max 99)> <custom l4d2_fmc_re_timer_block float value>.");
}

public Action:Command_CrecList(args)
{
	PrintToServer("[FMC] Custom value l4d2_fmc_crec and l4d2_fmc_re_timer_block list:");
	for (new i = 0; i < g_ArrayCount; i++)
	{
		PrintToServer("[%d] %s - %s - %s", i, MapNameArrayLine[i], CrecNumArrayLine[i], reBlkFlArrayLine[i]);
	}
	PrintToServer("[FMC] Custom value l4d2_fmc_crec and l4d2_fmc_re_timer_block list end.");
}

l4d2_gamemode()
{
	// 1 - coop / 2 - versus, teamversus / 3 - survival / 4 - scavenge, teamscavenge / 5 - realism / or false (thx DDR Khat for code) edited by kwski43
	new String:gmode[32];
...
PluginInitialization()
{
	...
	BuildPath(Path_SM, OldPlugin, sizeof(OldPlugin), "plugins/sm_l4d_mapchanger.smx");
	if(FileExists(OldPlugin))
	{
		ServerCommand("sm plugins unload sm_l4d_mapchanger");
		DeleteFile(OldPlugin);
		PrintToServer("[FMC] Old sm_l4d_mapchanger has been unloaded and deleted.");
	}
	...
	if (GameMode == 1)
	{
		BuildPath(Path_SM, FMC_FileSettings, 128, "data/l4d2_co_mapchanger.txt");
		PrintToServer("[FMC] Discovered coop gamemode. Link to l4d2_co_mapchanger.");
	}
	else if (GameMode == 2)
	{
		BuildPath(Path_SM, FMC_FileSettings, 128, "data/l4d2_vs_mapchanger.txt");
		PrintToServer("[FMC] Discovered versus gamemode. Link to l4d2_vs_mapchanger.");
	}
	else if (GameMode == 3)
	{
		BuildPath(Path_SM, FMC_FileSettings, 128, "data/l4d2_sv_mapchanger.txt");
		PrintToServer("[FMC] Discovered survival gamemode. Link to l4d2_sv_mapchanger.");
	}
	else if (GameMode == 4)
	{
		BuildPath(Path_SM, FMC_FileSettings, 128, "data/l4d2_sc_mapchanger.txt");
		PrintToServer("[FMC] Discovered scavenge gamemode. Link to l4d2_sc_mapchanger.");
	}
	else if (GameMode == 5)
	{
		BuildPath(Path_SM, FMC_FileSettings, 128, "data/l4d2_re_mapchanger.txt");
		PrintToServer("[FMC] Discovered realism gamemode. Link to l4d2_re_mapchanger.");
	}
	else

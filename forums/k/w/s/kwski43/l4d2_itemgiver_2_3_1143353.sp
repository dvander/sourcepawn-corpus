...
#define ADVERT "\x04[\x03SM\x04] This server runs \x04[\x03Round Start Item Giver\x04]\x03\nItems will be given to survivors at the round start!"
...
{
	cvarPluginVersion = CreateConVar("l4d2_ig_version", PLUGIN_VERSION, "Start Round Items Giver Version", FCVAR_NOTIFY);
	...
	cvarDelayPrimWeapon = CreateConVar("l4d_ig_delay_primweapon", "30.0", "Primary Weapon Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	cvarDelaySecoWeapon = CreateConVar("l4d_ig_delay_secweapon", "34.0", "Secondary Weapon Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	cvarDelayGranade = CreateConVar("l4d_ig_delay_granade", "37.0", "Granade Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	cvarDelayHealth = CreateConVar("l4d_ig_delay_health", "40.0", "Health Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	cvarDelaySupply = CreateConVar("l4d_ig_delay_supply", "43.0", "Supply Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	cvarDelayMelee = CreateConVar("l4d_ig_delay_melee", "46.0", "Melee Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	...
	cvarDelayPrimWeapon = CreateConVar("l4d2_ig_delay_primweapon", "30.0", "Primary Weapon Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	cvarDelaySecoWeapon = CreateConVar("l4d2_ig_delay_secweapon", "34.0", "Secondary Weapon Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	cvarDelayGranade = CreateConVar("l4d2_ig_delay_granade", "37.0", "Granade Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	cvarDelayHealth = CreateConVar("l4d2_ig_delay_health", "40.0", "Health Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	cvarDelaySupply = CreateConVar("l4d2_ig_delay_supply", "43.0", "Supply Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	cvarDelayUpgrade = CreateConVar("l4d2_ig_delay_upgrade", "32.0", "Upgrade give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);
	cvarDelayMelee = CreateConVar("l4d2_ig_delay_melee", "46.0", "Melee Item give delay from round start. Float in Seconds.",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 20.0, true, 60.0);

	cvarPrimWeapon = CreateConVar("l4d2_ig_give_primweapon", "0", "What Primary Weapon item should We give to survivors? 0-disable, 1-pumpshotgun, 2-autoshotgun, 3-shotgun_spas, 4-shotgun_chrome, 5-smg, 6-smg_mp5, 7-smg_silenced, 8-rifle, 9-rifle_ak47, 10-rifle_desert, 11-rifle_sg552, 12-sniper_military, 13-sniper_awp, 14-sniper_scout, 15-hunting_rifle, 16-grenade_launcher",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 16.0);
	cvarSecoWeapon = CreateConVar("l4d2_ig_give_secweapon", "1", "What Secondary Weapon item should We give to survivors? 0-disable, 1-pistol, 2-pistol_magnum, 3-cricket_bat, 4-chainsaw, 5-baseball_bat, 6-crowbar, 7-electric_guitar, 8-fireaxe, 9-katana, 10-machete",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 10.0);
				...
				switch(GetRandomInt(0, 0)) 
				{
					case 0: {
						...
						if (GetConVarInt(cvarGiveInfo)!=0) {
							if (GetConVarInt(cvarGiveInfo)==1)
							{
								PrintToChat(client, "%t", "medkit");
							}
							else if (GetConVarInt(cvarGiveInfo)==2)
							{
								PrintHintText(client, "%t", "medkit");
							}
						}
					}
				}
				...
				if (GetConVarInt(cvarHealth)!=0) { 
					if (GetConVarInt(cvarHealth)==1){ 
						...
						if (GetConVarInt(cvarGiveInfo)!=0) {
							if (GetConVarInt(cvarGiveInfo)==1)
							{
								PrintToChat(client, "%t", "irst_aid_kit");
							}
							else if (GetConVarInt(cvarGiveInfo)==2)
							{
								PrintHintText(client, "%t", "irst_aid_kit");
							}
						}
					}
				...
				else
				{
					...
					if (GetConVarInt(cvarUpgrade)==1) { 
						...
						if (GetConVarInt(cvarGiveInfo)!=0) {
							if (GetConVarInt(cvarGiveInfo)==1)
							{
							PrintToChat(client, "%t", "upgrade_inc");
							}
							else if (GetConVarInt(cvarGiveInfo)==2)
							{
							PrintHintText(client, "%t", "upgrade_inc");
							}
						}
					}					
				}
			...
			if (GetConVarInt(cvarRandomMelee)==1)
			{
				switch(GetRandomInt(0, 3)) {
					...
					case 4: {
						...
						FakeClientCommand(client, "give gfireworkcrate");
						if (GetConVarInt(cvarGiveInfo)!=0) {
							if (GetConVarInt(cvarGiveInfo)==1)
							{
								PrintToChat(client, "%t", "fireworkcrate");
					...
					case 5: {
						if(currentmap=="c2m1_highway" && currentmap=="c2m2_fairgrounds" && currentmap=="c2m3_coaster" && currentmap=="c2m4_barns" && currentmap=="c2m5_concert") { 
						FakeClientCommand(client, "give gfireworkcrate");
						if (GetConVarInt(cvarGiveInfo)!=0) {
							if (GetConVarInt(cvarGiveInfo)==1)
							{

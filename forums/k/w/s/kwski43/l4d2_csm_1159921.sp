...
{	
	...
	// Change Limit cvar
	g_hChangeLimit = CreateConVar("l4d_csm_change_limit", "10", "Sets the amount of times a client can change their character", FCVAR_PLUGIN|FCVAR_SPONLY);
	HookConVarChange(g_hChangeLimit, ConVarChangeLimit);
	//Allow only (model options)
	g_hOnlyModel = CreateConVar("l4d_csm_allow_only_model", "1", "Allow only (model) options", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 1.0);

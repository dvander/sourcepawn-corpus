...
#define PLUGIN_VERSION "1.2.4-20140110"
...
public OnPluginStart()
{
	...
	HookConVarChange( nmp_fia_stamina = CreateConVar( "nmp_fia_stamina", "-1", "Stamina for everyone. (-1 = disabled)", FCVAR_PLUGIN, true, -1.0 ), OnConVarChanged );
	HookConVarChange( sv_max_stamina = FindConVar( "sv_max_stamina" ), OnConVarChanged );
	
	RegAdminCmd( "sm_fia", Command_ToggleFIA, ADMFLAG_SLAY, "Usage: sm_fia <targets> <0-2>" );
...
public Action:OnPlayerRunCmd( iClient, &iButtons, &iImpulse, Float:vecVelocity[3], Float:vecAngles[3], &iWeapon, &iWeaponSub, &nCommand, &nTick, &iRandomSeed, iMouseDir[2] )
{
	if( 0 < iClient <= MaxClients && IsClientInGame( iClient ) && IsPlayerAlive( iClient ) )
	{
		...
		{
			//PrintToConsole( iClient, "Reload time: %f", flCurTime - flReloadTime[iClient] );
			if( ( flCurTime - flReloadTime[iClient] ) <= 0.5 )
			{
				...
				if( ( nFIAState[iClient] == 1 || bFIAForAll && !bNoReload ) && iAmmoOffset > 0 && iClip > -1 )
				{
					if( iClip1Offset > 0 )
						iClip -= GetEntData( iEntity, iClip1Offset );
					if( GetEntData( iClient, iAmmoOffset + GetEntProp( iEntity, Prop_Send, "m_iPrimaryAmmoType" ) * 4 ) < iClip )
						SetEntData( iClient, iAmmoOffset + GetEntProp( iEntity, Prop_Send, "m_iPrimaryAmmoType" ) * 4, iClip, _, true );
				}
			}
...
public OnGameFrame()
{
	new iWeapon, String:szClassname[21], Float:flNextPrimaryAttack, Float:flNextSecondaryAttack, Float:flNextBashAttack;
	for( new i = 1; i <= MaxClients; i++ )
		if( IsClientInGame( i ) && IsPlayerAlive( i ) )
		{
			...
			if( IsValidEdict( iWeapon ) )
			{
				...
				if( StrEqual( szClassname, "tool_barricade" ) || StrContains( szClassname, "bow_" ) == 0 || StrContains( szClassname, "exp_" ) == 0 )
				{
					if( ( nFIAState[i] || bFIAForAll ) && iAmmoOffset > 0 )
						SetEntData( i, iAmmoOffset + GetEntProp( iWeapon, Prop_Send, "m_iPrimaryAmmoType" ) * 4, 1, _, true );
				}
				else
				{
					//if( ( nFIAState[i] || bFIAForAll ) && iAmmoOffset > 0 )
					//	SetEntData( i, iAmmoOffset + GetEntProp( iWeapon, Prop_Send, "m_iPrimaryAmmoType" ) * 4, 1, _, true );
					if( ( nFIAState[i] == 2 || bFIAForAll && bNoReload ) && GetWeaponClipSize( iWeapon ) > -1 && iClip1Offset > 0 && 0 <= GetEntData( iWeapon, iClip1Offset ) < 2 )
						SetEntData( iWeapon, iClip1Offset, 1, _, true );
				}
...
stock GetWeaponClipSize( iEntity )
{
	...
	if(
		...
		return 30;
	else if( StrEqual( szClassname, "fa_mac10" ) )
		return 32;

...
public Plugin:myinfo =
{
	...
	author      = "DDRKhat - modified by Kpax",
...
public Action:HardZombies(client, args) 
{
	...
	if(Input==1)
	{
		SetConVarInt(FindConVar("z_common_limit"), 40); // Default
		SetConVarInt(FindConVar("z_mob_spawn_min_size"), 10); // Default
		SetConVarInt(FindConVar("z_mob_spawn_max_size"), 40); // Default
		SetConVarInt(FindConVar("z_mob_spawn_finale_size"), 30); // Default
		SetConVarInt(FindConVar("z_mega_mob_size"), 55); // Default
	}		
...
//---------------------------------------------------------------------------
// GiveHumansMedPack() PAX FIX FOR GIVING HUMANS A MED PACK ON 1ST ROUND
// THIS BUG ONLY SEEMS TO HAPPEN ON THE VERY FIRST ROUND FOR THE SECOND 
// SET OF SURVIVORS. SO WE WILL BE MAKING EXTRA MED PACKS WITH MORE HUMANS
// ALSO HUMANS ARE GIVEN AN SILCENCED SMG. 
//---------------------------------------------------------------------------
public Action:GiveHumansMedPack()
{
	new NumSurvivors = TeamPlayers(2);
     	
	for (new i=1; i<=NumSurvivors; i++) {

		if (GetClientTeam(i) != 2) continue; //SKIP IF ON INFECTED TEAM
		new wep = GivePlayerItem(i,"weapon_first_aid_kit");
		if(wep) EquipPlayerWeapon(i,wep);
		new wep1 = GivePlayerItem(i,"weapon_smg_silenced");
		if(wep1) EquipPlayerWeapon(i,wep1);
		new wep2 = GivePlayerItem(i,"weapon_pistol_magnum");
		if(wep2) EquipPlayerWeapon(i,wep2);

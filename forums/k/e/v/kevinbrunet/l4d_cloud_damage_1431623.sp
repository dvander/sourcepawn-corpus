...
new Handle:BotCloudDamage = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	CloudDamage = CreateConVar("l4d_cloud_damage_damage", "1.0", " Amount of damage the cloud deals every 2 seconds ", CVAR_FLAGS);
	BotCloudDamage = CreateConVar("l4d_bot_cloud_damage", "1", " Enable/Disable the damage of cloud for the bots ", CVAR_FLAGS);
	CloudDuration = CreateConVar("l4d_cloud_damage_time", "17.0", "How long the cloud damage persists ", CVAR_FLAGS);
	CloudRadius = CreateConVar("l4d_cloud_damage_radius", "200", " Radius of gas cloud damage ", CVAR_FLAGS);
...
static CheckGamemode()
{
	...
	GetConVarString(cvarGameModeActive, convarsetting, sizeof(gamemode));
...
public Action:Point_Hurt(Handle:timer, Handle:hurt)
{
	...
	for (new target = 1; target <= MaxClients; target++)
	{
		...
		if (!IsFakeClient(target) || GetConVarInt(BotCloudDamage)) applyDamage(damage, target, client);
	}

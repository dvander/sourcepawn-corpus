...
new Handle:l4d_selfhelp_eachother_durtaion = INVALID_HANDLE;
...
{
	name = "Self Help ",
	author = "kevinbrunet Original By Pan Xiaohai",
	description = " ",
...
public OnPluginStart()
{
	...
	l4d_selfhelp_incap = CreateConVar("l4d_selfhelp_incap", "1", "self help for incap , 0:disable, 1:pill, 2:medkit, 3:both  ", FCVAR_PLUGIN);
	l4d_selfhelp_grab = CreateConVar("l4d_selfhelp_grab", "0", " self help for grab , 0:disable, 1:pill, 2:medkit, 3:both ", FCVAR_PLUGIN);
	l4d_selfhelp_pounce = CreateConVar("l4d_selfhelp_pounce", "0", " self help for pounce , 0:disable, 1:pill, 2:medkit, 3:both ", FCVAR_PLUGIN);
	l4d_selfhelp_ride = CreateConVar("l4d_selfhelp_ride", "0", " self help for ride , 0:disable, 1:pill, 2:medkit, 3:both ", FCVAR_PLUGIN);
	l4d_selfhelp_pummel = CreateConVar("l4d_selfhelp_pummel", "0", "self help for pummel , 0:disable, 1:pill, 2:medkit, 3:both  ", FCVAR_PLUGIN);
	l4d_selfhelp_edgegrab = CreateConVar("l4d_selfhelp_edgegrab", "0", "self help for edgegrab , 0:disable, 1:pill, 2:medkit, 3:both  ", FCVAR_PLUGIN);
	...
	l4d_selfhelp_kill = CreateConVar("l4d_selfhelp_kill", "0", "kill attacker", FCVAR_PLUGIN);

	l4d_selfhelp_hintdelay = CreateConVar("l4d_selfhelp_hintdelay", "0.0", "hint delay", FCVAR_PLUGIN);
	l4d_selfhelp_delay = CreateConVar("l4d_selfhelp_delay", "0.0", "self help delay", FCVAR_PLUGIN);
	l4d_selfhelp_durtaion = CreateConVar("l4d_selfhelp_durtaion", "0.0", "self help duration", FCVAR_PLUGIN);
	l4d_selfhelp_eachother_durtaion = CreateConVar("l4d_selfhelp_eachother_durtaion", "10.0", "self help each other duration", FCVAR_PLUGIN);
...
public Action:AdvertisePills(Handle:timer, any:client)
{
	...
	{
		PrintHintText(client, "\x03press \x04CROUCH\x03 to self help");
	}
			...
			if(CanSelfHelp(client))
			{
				if(L4D2Version)
				{
					if(HelpState[client]==STATE_NONE)
					{
						...
						PrintHintText(client, "you helped yourself");
					}
		...
		if ((buttons & IN_DUCK) ||  (buttons & IN_USE)) 
		{
			...
			if(findone)
			{
				...
				if(HelpOhterState[client]==STATE_NONE)
				{
					if(L4D2Version) 
					{
						SetupProgressBar(client, GetConVarFloat(l4d_selfhelp_eachother_durtaion));
				...
				if(!L4D2Version) ShowBar(client, msg, time-HelpStartTime[client], GetConVarFloat(l4d_selfhelp_eachother_durtaion));

				if(time-HelpStartTime[client]>GetConVarFloat(l4d_selfhelp_eachother_durtaion))
				{
			...
			else
			{
				if(HelpOhterState[client]!=STATE_NONE)
				{
					...
					else ShowBar(client, "help other", 0.0, GetConVarFloat(l4d_selfhelp_eachother_durtaion));
				}
				HelpOhterState[client]=STATE_NONE;

...
public Action:PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CreateTimer(0.5, OnSpawn, client);
}
...
public Action:OnSpawn(Handle:timer, any:client)
{
	...
	if (CanEquip(client)) Equipknife(client);
...
Equipknife(client)
{	
	if(!IsValidClient(client) || !GetConVarBool(g_hEnabled)) return;
	
	if(CanEquip(client))
	{
		new iWeapon = GetPlayerWeaponSlot(client, 2);
		if (iWeapon == INVALID_ENT_REFERENCE && GetConVarBool(g_hNoKnifeMapDisable)) return; //If player spawned without any knife, and disabled map knives is on, don't equip knife.
		...
		new iWeaponSafeCheck = GetPlayerWeaponSlot(client, 2);
		if (iWeaponSafeCheck != INVALID_ENT_REFERENCE) return;
		...
		switch(knife_choice[client]) {
			...
			default: {iItem = GivePlayerItem(client, "weapon_knife");}
		}
		if (iItem != 0)
		{
...
bool:CanEquip(client)
{
	if (!IsClientConnected(client) || !IsClientInGame(client)) return false;
	if (!IsPlayerAlive(client) && GetClientTeam(client) <= CS_TEAM_SPECTATOR) return false;   
	return true; 
}
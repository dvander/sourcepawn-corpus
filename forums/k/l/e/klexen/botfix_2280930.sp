...
public OnMapStart() {
	CreateTimer(30.0, DelayedBotCheckTimer, _, TIMER_FLAG_NO_MAPCHANGE);
}

public Action:DelayedBotCheckTimer(Handle:timer) {
	RecalcTeamCount();
	CheckBalance();
	RespawnDeadBots();
}
...
public OnClientPutInServer(client)
{
	if(!IsFakeClient(client))
		CreateTimer(3.0, DelayedBotCheckTimer, _, TIMER_FLAG_NO_MAPCHANGE);
}
...
CheckBalance() {
	...
	new humans = g_Tcount + g_CTcount + g_Spectcount;

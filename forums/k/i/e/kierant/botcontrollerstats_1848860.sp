...
new Handle:h_cvarCashEliminationWinCS;
new Handle:h_cvarCashEliminationWinDE;
new Handle:h_cvarCashHostageWin;
new Handle:h_cvarCashTimeoutWin;
...
new g_cvarCashEliminationWinCS;
new g_cvarCashEliminationWinDE;
new g_cvarCashHostageWin;
new g_cvarCashTimeoutWin;
new g_cvarCashLoser;
...
public OnPluginStart()
{
	...
	h_cvarCashEliminationWinCS = FindConVar("cash_team_elimination_hostage_map");
	...
	h_cvarCashTimeoutWin       = FindConVar("cash_team_win_by_time_running_out");
	...
	HookConVarChange(h_cvarCashEliminationWinCS, ConvarChanged);
	HookConVarChange(h_cvarCashEliminationWinDE, ConvarChanged);
	HookConVarChange(h_cvarCashHostageWin, ConvarChanged);
	HookConVarChange(h_cvarCashTimeoutWin, ConvarChanged);
...
public OnRoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for(new i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i))
		{
			...
			if(g_BotList[i] > 0 && g_cvarBotPlayerEquipment)
				MigrateWeapons(i);
...
public OnRoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	for(new i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i) && g_BotList[i] > 0)
...
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	if(IsClientInGame(client) && g_BotList[client] > 0)
	{
...
GetAugment(client, &CSRoundEndReason:reason)
{
	...
	if(GetClientTeam(client) == 2)
	{
		...
		if(reason == CSRoundEnd_HostagesNotRescued)
			return g_cvarCashTimeoutWin;
		if(reason == CSRoundEnd_CTWin)
			return g_cvarCashLoser;
		if(reason == CSRoundEnd_TerroristWin)
			return (StrEqual(g_CurrentMap, "cs_") ? g_cvarCashEliminationWinCS : g_cvarCashEliminationWinDE);
	}
...
public ConvarChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	...
	else if(cvar == h_cvarCashEliminationWinCS)
		g_cvarCashEliminationWinCS = GetConVarInt(h_cvarCashEliminationWinCS);
	...
	else if(cvar == h_cvarCashTimeoutWin)
		g_cvarCashTimeoutWin       = GetConVarInt(h_cvarCashTimeoutWin);

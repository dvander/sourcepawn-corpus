...
public void OnPluginStart()
{
	...
	hSHStaggerhSHStagger = EndPrepSDKCall();
	if (hSHStagger == null)
	{
	...
	if (IsSurvivor(wounded))
	{
		...
		if (StrEqual(name, "player_incapacitated"))
		{
			...
			if (bIsL4D)
			{
				...
				CPrintToChat(wounded, "{blue}[SH]{default} You Got Incapacitated! |{green}%d{default}/{green}%i{default}|", iSHCount[wounded] + 1, iMaxCount);
			...
			else
			{
				...
				CPrintToChat(wounded, "{blue}[SH]{default} You Got Incapacitated! |{green}%d{default}/{green}%i{default}|", iReviveCount + 1, iMaxIncapCount);
...
public Action FireUpMechanism(Handle timer, any userid)
{
	...
	if (hSHTime[client] == null)
	{
		...
		{
			CPrintToChat(client, "{blue}[SH]{default} Press {green}CTRL{default} To Self-Help!");
		}
	...
	if (IsSurvivor(reviver) && IsSurvivor(revived))
	{
		...
		if (event.GetBool("ledge_hang"))
		{
			if (reviver != revived)
			{
				...
				{
					CPrintToChat(reviver, "{blue}[SH] {default}You Helped{olive} %N{default}!", revived);
				}
				
				if (!IsFakeClient(revived))
				{
					CPrintToChat(revived, "{blue}[SH] {olive}%N{default} Helped You!", reviver);
				}
			}
			else
			{
		...
		else
		{
			if (!bIsL4D)
			{
				...
				if (reviver == revived)
				{
					if (!IsFakeClient(revived))
					{
						CPrintToChat(revived, "{blue}[SH] {default}You Helped Yourself! |{green}%d{default}/{green}%i{default}|", iReviveCount, iMaxIncapCount);
					}
				}
				else
				{
			...
			else
			{
				...
				if (reviver == revived)
				{
					if (!IsFakeClient(revived))
					{
						CPrintToChat(revived, "{blue}[SH] {default}You Helped Yourself! |{green}%d{default}/{green}%i{default}|", iSHCount[revived], iMaxCount);
					}
				}
				else
				{
					if (!IsFakeClient(reviver))
					{
						...
						CPrintToChat(reviver, "{blue}[SH] {default}You Helped{olive} %N{default}! |{green}%d{default}/{green}%i{default}|", revived, iSHCount[revived], iMaxCount);
					}
					
					if (!IsFakeClient(revived))
					{
						CPrintToChat(revived, "{blue}[SH] {olive}%N{default} Helped You! |{green}%d{default}/{green}%i{default}|", reviver, iSHCount[revived], iMaxCount);
					}
				}
			}
		}
...
public Action DelaySHNotify(Handle timer, Handle dpDefibAnnounce)
{
	...
	if (defibber == defibbed)
	{
		...
		{
			CPrintToChat(defibbed, "{blue}[SH] {default}You Defibbed Yourself! |{green}%d{default}/{green}%i{default}|", iReviveCount, iMaxIncapCount);
		}
	}
	else
	{
		...
		{
			CPrintToChat(defibber, "{blue}[SH] {default}You Defibbed{olive} %N{default}! |{green}%d{default}/{green}%i{default}|", defibbed, iReviveCount, iMaxIncapCount);
		}
		
		if (!IsFakeClient(defibbed))
		{
			CPrintToChat(defibbed, "{blue}[SH] {olive}%N{default} Defibbed You! |{green}%d{default}/{green}%i{default}|", defibber, iReviveCount, iMaxIncapCount);
		}
	}
		...
		if ((bEmergencyUsed || bFirstAidUsed) && RemovePlayerItem(client, iUsedItem))
		{
			...
			else if (bEmergencyUsed)
			{
				if (!bIsL4D && StrEqual(sUsedItemName, "weapon_adrenaline", false))
				{
					...
					CPrintToChatAll("{blue}[SH] {olive}%N {default}Helped Themselves With{green} Adrenaline{default}!", client);
				}
				else
				{
					...
					CPrintToChatAll("{blue}[SH] {olive}%N {default}Helped Themselves With{green} Pills{default}!", client);
				}
			}
		}

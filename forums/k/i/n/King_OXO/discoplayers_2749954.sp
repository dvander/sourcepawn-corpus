...
public void OnPluginStart()
{
	...
	AutoExecConfig(true, "plugin.discoplayer");
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
	if(g_bDisco[client])
	{
		if(g_eDiscomode[client] == DISCO_NORMAL)
		{
			...
			SetEntProp(client, Prop_Send, "m_glowColorOverride", rgba[0] + (rgba[1] * 256) + (rgba[2] * 65536));
		...
		else if(g_eDiscomode[client] == DISCO_SPOOKY && g_cvSpookyMode.IntValue == 1)
		{
			...
			SetEntProp(client, Prop_Send, "m_glowColorOverride", 0 + (0 * 256) + (0 * 65536));
			SetEntProp(client, Prop_Send, "m_iGlowType", 3);
			SetEntProp(client, Prop_Send, "m_nGlowRange", 99999);
			SetEntProp(client, Prop_Send, "m_nGlowRangeMin", 0);
		}
			...
			if(CheckCommandAccess(client, "", ADMFLAG_GENERIC, true))
			{
				...
				for (int i = 0; i < target_count; i++)
				{
					if(g_bDisco[target_list[i]])
					{
						...
						SetEntProp(target_list[i], Prop_Send, "m_glowColorOverride", 0 + (0 * 256) + (0 * 65536));
			            SetEntProp(target_list[i], Prop_Send, "m_iGlowType", 3);
						SetEntProp(target_list[i], Prop_Send, "m_nGlowRange", 99999);
						SetEntProp(target_list[i], Prop_Send, "m_nGlowRangeMin", 0);
					...
					else
					{
						...
						int R = GetRandomInt(1, 255), G = GetRandomInt(1, 255), B = GetRandomInt(1, 255);
						SetEntProp(target_list[i], Prop_Send, "m_glowColorOverride", R + (G * 256) + (B * 65536));
			            SetEntProp(target_list[i], Prop_Send, "m_iGlowType", 3);
						SetEntProp(target_list[i], Prop_Send, "m_nGlowRange", 99999);
						SetEntProp(target_list[i], Prop_Send, "m_nGlowRangeMin", 0);

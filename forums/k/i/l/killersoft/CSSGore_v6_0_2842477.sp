...
//Delete:
public Action:DeleteParticle(Handle:Timer, any:ParticleRef)
{

	//Validate:
	new Particle = EntRefToEntIndex(ParticleRef);
	if(Particle == INVALID_ENT_REFERENCE) return Plugin_Stop;
...
stock Decal(Client, Float:Direction[3], bool:Bleeding = false)
{
	...
	if(IsValidEdict(Blood))
	{
		...
		if(!Bleeding)
		{
			...
			//Save & Send:
			if(Blood > 0 && Blood < sizeof(BloodClient)) BloodClient[Blood] = Client;
			CreateTimer(GetRandomFloat(0.1, 1.0), EmitBlood, EntIndexToEntRef(Blood), TIMER_FLAG_NO_MAPCHANGE);
		} 
...
//Emit Blood:
public Action:EmitBlood(Handle:Timer, any:BloodRef)
{
	new Blood = EntRefToEntIndex(BloodRef);
	if(Blood == INVALID_ENT_REFERENCE)
		return Plugin_Stop;

	//Emit:
	if(IsValidEdict(Blood))
	{
		if(Blood > 0 && Blood < sizeof(BloodClient))
		{
			new client = BloodClient[Blood];
...
stock Gib(Float:Origin[3], Float:Direction[3], String:Model[])
{
	...
	Velocity[1] = Direction[1] * 400.0;
	Velocity[2] = Direction[2] * 400.0;
	...
	//Delete:
	CreateTimer(GetRandomFloat(15.0, 30.0), RemoveGib, EntIndexToEntRef(Ent), TIMER_FLAG_NO_MAPCHANGE);
}
...
//Remove Gib:
public Action:RemoveGib(Handle:Timer, any:EntRef)
{
...
{
	// Delay ragdoll removal slightly to avoid race conditions with client updates.
	new rag = GetEntPropEnt(Client, Prop_Send, "m_hRagdoll");
	if(rag != -1 && IsValidEdict(rag))
	{
		CreateTimer(0.2, Timer_RemoveRagdoll, EntIndexToEntRef(rag), TIMER_FLAG_NO_MAPCHANGE);
	}
}

public Action:Timer_RemoveRagdoll(Handle:Timer, any:RagRef)
{
	new rag = EntRefToEntIndex(RagRef);
	...
	{
		decl String:Classname[64];
		GetEdictClassname(rag, Classname, sizeof(Classname));
...
//Bleed:
public Action:Bleed(Handle:Timer, any:UserId)
{
...
//Damage:
public EventDamage(Handle hEvent, const String:Name[], bool:Broadcast)
{
...
//Death:
public EventDeath(Handle hEvent, const String:Name[], bool:Broadcast)
{
	...
	//Initialize:
	Headshot = GetEventBool(hEvent, "headshot");
	Client = GetClientOfUserId(GetEventInt(hEvent, "userid"));
	...
	//Close: (SourceMod owns the event handle)
}
			...
			if(IsClientInGame(Client))
			{
				...
				if(IsPlayerAlive(Client))
				{
					...
					if(GetClientHealth(Client) <= GetConVarInt(hBleedingHp))
					{
							...
							//Bleed:
							CreateTimer(1.0, Bleed, GetClientUserId(Client), TIMER_FLAG_NO_MAPCHANGE);
					}
				}
			}
		}
...
ForcePrecache(String:ParticleName[])
{
	...
	if(IsValidEdict(Particle))
	{
		...
		//Delete:
		CreateTimer(0.3, DeleteParticle, EntIndexToEntRef(Particle), TIMER_FLAG_NO_MAPCHANGE);
	}
}

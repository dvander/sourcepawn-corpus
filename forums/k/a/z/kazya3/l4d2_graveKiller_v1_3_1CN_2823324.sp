...
public void OnPluginStart(){
	...
	Grave_Respawn_Time 		= CreateConVar("L4D2_Grave_Respawn_Time", 	"3", 				"盗墓所需时间(秒)",			FCVAR_NOTIFY, true, 0.0, true, 100.0);
	Grave_Respawn_Chance 	= CreateConVar("L4D2_Grave_Respawn_Chance", 	"3", 			"一局能用墓碑复活几次",		FCVAR_NOTIFY, true, 0.0, true, 99.0);

	Grave_Witch_Percent 	= CreateConVar("L4D2_Grave_Witch_Percent", 			"50", 			"队友诈尸成witch概率",		FCVAR_NOTIFY, true, 0.0, true, 100.0);

	Grave_Glow_Switch		= CreateConVar("L4D2_Grave_Glow_Switch", 		"1", 			"墓碑是否发光描边",			FCVAR_NOTIFY, true, 0.0, true, 1.0);
	Grave_Glow_Color		= CreateConVar("L4D2_Grave_Glow_Color", 		"255 255 255", 	"墓碑发光描边颜色",			FCVAR_NOTIFY);

	Grave_Respawn_Time.AddChangeHook(ConVarChanges);
	Grave_Respawn_Chance.AddChangeHook(ConVarChanges2);//修改复活次数会重置所有玩家次数，因此应该单独关联处理
...
public Action Event_PlayerDeath(Handle event, const char[] name, bool dontBroadcast){
	...
	if (isSurvivor(victim) && playerList[victim].RespawnCount > 0){
		...
		DispatchKeyValue(Button_Index, "use_string", "正在疯狂偷吃贡品..");
		DispatchKeyValue(Button_Index, "use_sub_string", "我特么吃吃吃");
...
public void Button_OnTimeUp(const char[] name, int caller, int activator, float delay){
	...
	if(percent < g_wichPercent){
		...
		if (witch != -1 && Button_Index != INVALID_ENT_REFERENCE){
			...
			DispatchSpawn(witch);
			PrintToChatAll("\x04%N \x03倒斗失败!\n\x04%N \x03戾气过重诈尸成了\x04非洲野婊子!", activator, p_dead);
		}
	}
	//复活玩家
	else{
		...
		if(isDeadSurvivor(p_dead)){
			...
			#endif
			PrintHintText(p_dead, "剩余重生次数: %d", playerList[p_dead].RespawnCount);
			PrintToChatAll("\x04%N \x03坟头蹦迪还偷吃贡品! 活活气醒了 \x04%N !", activator, p_dead);
		}
	}
}
/*********************************************************************

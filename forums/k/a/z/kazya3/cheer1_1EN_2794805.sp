...
float 	p_Cheer_LastTime[MAXPLAYERS+1] 	= {0.0, ...};
...
public Plugin:myinfo =
{
	...
	description = "Type !cheer to laugh",
	version = "1.1",
	url = ""
};

public OnPluginStart() {
	RegConsoleCmd("sm_cheer", Cheer_Cmd, "Cheer");

	Cheer_Count     = CreateConVar("L4D2_Cheer_Count", 	    	"3", 		"Times player can use.",									FCVAR_NOTIFY, true, 0.0, true, 99.0);
	Cheer_CoolDown  = CreateConVar("L4D2_Cheer_CoolDown", 		"20.0", 	"CoolDown(How many seconds that chance will add 1)",		FCVAR_NOTIFY, true, 0.0, true, 9000.0);
...
public Action Cheer_Cmd(int client, int args){
	if ( isSurvivor(client)){
		if(p_Cheer_Count[client] > 0){
			...
			PrintToChatAll("\x03[!cheer] \x04%N \x03LMAO! \x04(%d/%d)", client, p_Cheer_Count[client], g_Cheer_Count);
			...
			//cooldown
			PrintHintText(client, "Wait for %.1fs)", timeLeft);//cant use print
		}
	}

//I learned enum sturct and arraylist，so i did this protect for practise..
...
char w_Icons[][][] = {//字典
	{"Cherry",	"1",	"5",	"cherry"},
	{"Apple",	"2",	"6",	"green-apple"},
	{"Lemon",	"3",	"7",	"lemon"},
	{"Orange",	"4",	"8",	"orange"},
	{"Grape",	"5",	"9",	"grape"},
	{"Star",	"6",	"10",	"star"},
	{"Diamond",	"10",	"20",	"diamond"},
	{"Lucky7",	"15",	"30",	"7"}
};
...
public Plugin myinfo = {
	name = 			"SlotMachine - Public Version",
	author =		"CD意识STEAM_1:0:211123334 (Alliedmods:kazya3)",
	description = 	"SlotMachine - Public Version",
	version =		"1.2.1",
	url = ""
}
...
public void OnPluginStart(){
	...
	RegAdminCmd("sm_sm",    SMachineMenu,    ADMFLAG_ROOT, 	"spawn machine menu");
...
void ShowMainMenu(int client){
	...
	g_hMenuMain.AddItem("","Spawn at the crosshair");
	g_hMenuMain.AddItem("","Edit");
	g_hMenuMain.AddItem("","Save");
	g_hMenuMain.AddItem("","Reload");
	g_hMenuMain.AddItem("","Delete All");
...
void ShowEditMenu(int client){
	char line[512];
	FormatEx(line, 512, "SlotMachine\n══════════════\n▶ Total: %d \n▶ Selected: No.%d \n══════════════", s_Count, s_Select);
	//菜单生成
	Panel EditMenu = new Panel();
	SetPanelTitle(EditMenu, line);
	DrawPanelItem(EditMenu, "Last");//1
	DrawPanelItem(EditMenu, "Next");//2
	DrawPanelItem(EditMenu, "Move");//3
	DrawPanelItem(EditMenu, "Rotate");//4
	DrawPanelItem(EditMenu, "Delete Selected");//5
	DrawPanelItem(EditMenu, "Back", ITEMDRAW_DISABLED);//6
...
void ShowAxisMenu(int client,int action){
	...
	char line[512];
	FormatEx(line, 512, "SlotMachine\n══════════════\n▶ Mode: %s \n▶ Selected: No.%d\n══════════════", action == 0 ? "Move" : "Rotate", s_Select);
	//菜单生成
	Panel AxisMenu = new Panel();
	SetPanelTitle(AxisMenu, line);
	//旋转只允许Z轴（这个功能浪费了我太多时间，所以我阉了其他轴）
	if(action == ACT_MOVE){
		DrawPanelItem(AxisMenu, "X");
		DrawPanelItem(AxisMenu, "Y");
	}
	DrawPanelItem(AxisMenu, "Z");
	DrawPanelItem(AxisMenu, "Back", ITEMDRAW_DISABLED);//4
...
void ShowActionDetailMenu(int client, int axis, int action){//action 0 move; 1 rotate
	...
	FormatEx(line, 512, "SlotMachine\n══════════════\n▶ Selected: No.%d \n▶ %s Axis: %s \n══════════════", s_Select, action == ACT_MOVE ? "Move":"Rotate", s_axis);
...
void SpawnMachineAtCrosshair(int client){
	...
	else PrintToChat(client, "[SM] Out Of Range");
	//生成
...
public void OnUseButton(const char[] output, int caller, int activator, float delay){
	...
	if(prop.isRotate && prop.w_Owner == activator){//是使用者使用
		#if DEBUG
			PrintToChatAll("Is User");
		#endif
		stop(activator, prop);
	}
	else{//第一次启动
		#if DEBUG
			PrintToChatAll("Fist Time Turn On");
		#endif
...
public void OnEndTouch(const char[] output, int caller, int activator, float delay){
	...
	if(activator == prop.w_Owner){
		#if DEBUG
			PrintToChatAll("%d Leaved", activator);
		#endif
		//返回积分
		if(!IsPlayerIncapped(prop.w_Owner)){
			...
			if(health > NORMAL_COST){
				...
				PrintToChat(prop.w_Owner,"\x03Leave! Return\x04 %d \x03HP!", NORMAL_COST)
			}
		}
		//
		prop.w_Owner = 0;
...
void stop(int client, SlotMachine prop){
	...
	#if DEBUG
		PrintToChatAll("\x03Number:\x04 %d", prop.w_result[prop.w_stopCount]);
	#endif
...
void compareResult(int client, const SlotMachine prop){
	...
	if(icon1 == icon2 && icon2 == icon3){//3个一样
		...
		PrintToChat(client, "\x03Congratulations! \x043 \x03*\x04 %s \x03, Reward\x04 %d \x03HP!", s_name, credits);
	}
		...
		SetEntProp(client, Prop_Data, "m_iHealth", addHP);
		EmitSoundToClient(client, s_sounds[2]);
		PrintToChat(client, "\x03Congratulations! \x042 \x03*\x04 %s \x03, Reward\x04 %d \x03HP!", s_name, credits);
	}
	else{
		//too noisy

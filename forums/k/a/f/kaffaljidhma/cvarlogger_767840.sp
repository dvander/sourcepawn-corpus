...
{
    RegAdminCmd("sm_logcvars", Command_ListCvars, ADMFLAG_CHEATS);
}

public Action:Command_ListCvars(client, args)
{
    decl String:name[64], String:description[255];
	...
	*/
	if(isCommand) {
		LogToFileEx("milo.txt", "$'%s'$cmd$'%s'", name, description);
	} else {
		LogToFileEx("milo.txt", "$'%s'$%f$'%s'", name, GetConVarFloat(value), description);
	}
	//counter++;

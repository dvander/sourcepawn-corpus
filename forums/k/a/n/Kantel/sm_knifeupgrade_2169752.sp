...
new Handle:g_hWeapons_allow_3rd_party = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_hWeapons_allow_3rd_party = FindConVar("dm_weapons_allow_3rd_party");
...
{
	g_hWeapons_allow_3rd_party = FindConVar("dm_weapons_allow_3rd_party");
	if(g_hWeapons_allow_3rd_party != INVALID_HANDLE && !GetConVarBool(g_hWeapons_allow_3rd_party)) SetConVarBool(g_hWeapons_allow_3rd_party, true);
}

public ConVarChange_WeaponsAllow(Handle:convar, const String:oldValue[], const String:newValue[]) 
{
...
public Action:PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetConVarBool(g_hSpawnMessage))
	{
		...
		if (GetConVarBool(g_hEnableGoldKnife))
		{
			...
			PrintToChat(client, "Triggers: \x07!bayonet !gut !flip !m9 !karambit !huntsman !butterfly !default");
		}
	}
...
public Action:AfterSpawn(Handle:timer, any:serial)
{
	...
	if (GetConVarBool(g_hWelcomeMenu) && IsValidClient(client) && knife_welcome_spawn_menu[client] == 0)
	{
		...
		{
			if (knife_choice[client] < 1) ShowKnifeMenu(client); //Only show Knife Welcome Message if a custom knife hasn't been selected yet.
		} else {
			ShowKnifeMenu(client);
		}
...
SetDefault(client)
{
	...
	SetClientCookie(client, g_cookieKnife, "8");
	CreateTimer(0.1, StripKnife, GetClientSerial(client));

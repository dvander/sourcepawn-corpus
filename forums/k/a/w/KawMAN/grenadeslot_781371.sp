...
{
	hookTouch[client] = Hacks_Hook(client, HACKS_HTYPE_TOUCH, Entity_OnTouch);

}

public OnClientDisconnect(client)
{
	Hacks_Unhook(hookTouch[client]);
}
...
public Entity_OnTouch(client, entity, agr2, arg3, arg4, arg5)
{
	if (IsValidEntity(entity)&&IsPlayerAlive(client))
	{
		...
		{
			if(GetClientAllGrenades(client)<4) PickupGrenade(client, entity,HEGRENADE);
		
		}
		else
		if (StrEqual(classname, "weapon_flashbang", false))
		{
			if(GetClientAllGrenades(client)<4) PickupGrenade(client, entity,FLASHBANG);
		
		}
		else
		if (StrEqual(classname, "weapon_smokegrenade", false))
		{
			if(GetClientAllGrenades(client)<4) PickupGrenade(client, entity,SMOKEGREN);

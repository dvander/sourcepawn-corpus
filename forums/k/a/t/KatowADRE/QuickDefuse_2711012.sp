...
public PanelPlant(Handle:menu, MenuAction:action, param1, param2)
{
	...
	{
		wire = param2
		PrintToChat(param1,"\x01\x04 \x02[uKatowa.pl] \x01You chose the %s wire",wirecolours[param2-1])
	}
}
	...
	if (action == MenuAction_Select && param2 > 0 && param2 < 5) //User selected a valid wire colour
	{
		...
		if (bombent)
		{
			...
			if (param2 == wire)
			{
				...
				PrintToChatAll("\x01\x04 \x02[uKatowa.pl] \x01%s correctly cut the %s wire for an instant C4 defusal (1:4 odds)",name,wirecolours[param2-1])
			}
			else
			{	
				SetEntPropFloat(bombent, Prop_Send, "m_flC4Blow", 1.0)
				PrintToChatAll("\x01\x04 \x02[uKatowa.pl] \x01%s detonated the C4 with an incorrect wire choice of %s (3:4 odds) The correct wire was %s",name,wirecolours[param2-1],wirecolours[wire-1])
			}
		}
	}
}
...
public PanelNoKit(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select && param2 > 0 && param2 < 5) //User selected a valid wire colour
	{
		...
		if (bombent)
		{
			...
			{
				SetEntPropFloat(bombent, Prop_Send, "m_flDefuseCountDown", 1.0)
				PrintToChatAll("\x01\x04 \x02[uKatowa.pl] \x01%s correctly pulled the %s wire for an instant C4 defusal (1:8 odds)",name,wirecolours[param2-1])
			}
			else
			{
				SetEntPropFloat(bombent, Prop_Send, "m_flC4Blow", 1.0)
				if (param2 != wire)
					PrintToChatAll("\x01\x04 \x02[uKatowa.pl] \x01%s detonated the C4 with an incorrect wire choice of %s (7:8 odds) The correct wire was %s",name,wirecolours[param2-1],wirecolours[wire-1])
				else
					PrintToChatAll("\x01\x04 \x02[uKatowa.pl] \x01%s chose the correct wire (%s) but the C4 still detonated!",name,wirecolours[param2-1])
			}
		}
	}
}

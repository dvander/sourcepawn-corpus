...
public T_CallbackSelectPDataRace(Handle:owner,Handle:hndl,const String:error[],any:client)
{
    ...
    if(hndl != INVALID_HANDLE)
    {
        ...
        if(retrievals > 0)
        {
            PrintToConsole(client, "%T", "[Exiled WarCraft3] Successfully retrieved data races, total of {amount} races were returned, {amount} are running on this server", client, retrievals, usefulretrievals);
        }
        ...
        if(inserts > 0)
        {
            PrintToConsole(client,"%T","[Exiled WarCraft3] Inserting fresh level xp data for {amount} races",client,inserts);
        }

/* Этот плагин - попытка воспроизвести закрытый плагин AmmoMod (за авторством thenoid)
author: kroleg
url: tf2.kz
Credits: 
	thenoid
...
0.6
	...
	+ differnt fraglimit for arenas
	+ rating restrictions for arenas
	- fixed infinite ammo didnt work sometimes (with some kind of crutch)
0.6.b7
	- fixed stats logic. again.
0.6.b6
	- fixed stats logic (wrong number of losing poits)
...
//classes 
new bool:g_classAllowed[TFClassType];
new Float:g_hpRatio;
...
new bool:g_arenaMidair[MAXARENAS+1];
...
new bool:g_playerTakenDirectHit[MAXPLAYERS+1];//player was hitted directly
	
new String:g_sSteamID[MAXPLAYERS+1][32];//saving steamid
...
new 	Handle:gcvar_midairHP = INVALID_HANDLE;
	//Handle:gcvar_midairEnabled = INVALID_HANDLE;
	
//midair
//new 	bool:g_midairEnabled = false,
new 	g_midairHP;
...
public OnEntityTouch(entity, other){
	...
	//else
	//	PrintToChatAll("hit %d by %d",other,entity);
}
...
public OnPluginStart(){
	...
	//g_hTime = CreateConVar("da_time", "1.5", "Time between rounds.", FCVAR_PLUGIN|FCVAR_NOTIFY);
	//g_hWins = CreateConVar("da_wins", "4", "Max consecutive wins.", FCVAR_PLUGIN|FCVAR_NOTIFY);
	...
	gcvar_allowedClasses = CreateConVar("kammomod_allowed_classes", "soldier demoman scout", "Classes that players allowed to choose", FCVAR_PLUGIN);
	gcvar_hpbuffRatio = CreateConVar("kammomod_hpbuff_ratio", "6.0", "HP multiplier", FCVAR_PLUGIN, true, 1.0);
	gcvar_infAmmo = CreateConVar("kammomod_infammo", "1", "Give players infinite ammo", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	gcvar_blockFallDamage = CreateConVar("kammomod_block_falldamage", "1", "Blocking falldamage (0 = Disabled)", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	gcvar_dbConfig = CreateConVar("kammomod_dbconfig", "default", "", FCVAR_PLUGIN);
	//
	...
	//midair stuff
	//g_midairEnabled = GetConVarInt(gcvar_midairEnabled) ? true : false;
	g_midairHP = GetConVarInt(gcvar_midairHP);
	if (!g_NoStats) PrepareSQL();
	
	ParseAllowedClasses(); //parsing allowed classes convar
	...
	if (!g_NoStats) { 
		for (new i=1;i<=MaxClients;i++){
			if (IsClientInGame(i)){
				...
				strcopy(g_sSteamID[i],32,steamid);
...
AddParticlesToDLTable(){ 	// Add the map's particle manifest to the downloads table
	decl String:map[32];
		...
		if (IsFakeClient(client)){
			...
			for (new i=1;i<=MaxClients;i++){
				if (g_playerAskedForBot[i]){
					...
					CreateTimer(1.5,Timer_AddBotInQueue,pk);
					//testing purpose
			...
			CreateTimer(5.0, Timer_ShowAdv, client);//show advice to type !add in chat
			...
			if (!g_NoStats) {
				...
				strcopy(g_sSteamID[client],32,steamid);
			}
		}
	}
	//SDKHookz stuff
		...
		for (new client = 1; client <= MaxClients; client++) 
			if (IsClientInGame(client) && IsPlayerAlive(client)/* && (g_playerSlot[client]==SLOT_ONE || g_playerSlot[client]==SLOT_TWO)*/) {
				...
				/*if (g_infAmmo){
					...
					//if (!g_playerRestoringAmmo[client] && ((buttons & IN_ATTACK)||(buttons & IN_ATTACK2))){
...
//arena
StartDuel(arena_index){ //начинаем новую дуель
...
PrintCountdownToArena(arena_index,String:text[]){
	if (!arena_index)
		return;
	for (new i=1;i<=MaxClients;i++){
		if (IsClientInGame(i) && GetClientTeam(i)==TEAM_SPEC && g_playerSpecTarget[i]>0 && g_playerArena[g_playerSpecTarget[i]]==arena_index)
...
CalcELO(winner, loser) {
	...
	Format(query, sizeof(query), "UPDATE kammomod_stats SET rating=%i,wins=wins+1,lastplayed=%i WHERE steamid='%s'", g_playerRating[winner], GetTime(), g_sSteamID[winner]);
	SQL_TQuery(db, SQLErrorCheckCallback, query);
	Format(query, sizeof(query), "UPDATE kammomod_stats SET rating=%i,losses=losses+1,lastplayed=%i WHERE steamid='%s'", g_playerRating[loser], GetTime(), g_sSteamID[loser]);
	SQL_TQuery(db, SQLErrorCheckCallback, query);
}
...
ParseAllowedClasses(){
	...
	GetConVarString(gcvar_allowedClasses,hpbuffed_classes,sizeof(hpbuffed_classes));
	new String:a_class[9][8];
	new count = ExplodeString(hpbuffed_classes, " ", a_class, 9, 8);
	for (new i=1;i<=9;i++)
		g_classAllowed[i] = false;
	for (new i=0;i<count;i++)
		g_classAllowed[TF2_GetClass(a_class[i])] = true;
}
...
public Action:Command_JoinClass(client, args){ //handle changing class
	...
	if (args) {
		...
		//PrintToChat(client,"%s",s_class);
		if (!g_classAllowed[new_class]){
			CPrintToChat(client,"%t","ClassIsNotAllowed");
			//PrintToChat(client,"\x05You cant change to this class");
			return Plugin_Handled;
		}
		if (new_class == g_playerClass[client])
			return Plugin_Handled;
		new arena_index = g_playerArena[client];
		if (arena_index && (g_playerSlot[client]==SLOT_ONE || g_playerSlot[client]==SLOT_TWO)){
...
public Action:Command_Spec(client, args){ //detecting spectator target
	if (!client) 
		return Plugin_Continue;
	CreateTimer(0.1,Timer_ChangeSpecTarget,client);
	return Plugin_Continue;
}
...
//menu
public Menu_Main(Handle:menu, MenuAction:action, param1, param2) { //обработка меню !add
    switch (action) {
	...
	if (attacker>0 && victim != attacker){
		...
		if (g_playerTakenDirectHit[victim]){
			if (GetEntityFlags(victim) & (FL_ONGROUND)){// не аиршот
				...
				if(g_bHitBlip[attacker] && attacker>0 && attacker!=victim) {
					//new pitch = 150-(g_playerHP[victim]-health); //maybe dmg = g_playerHP[victim]-health
				...
				if(TR_DidHit(trace)){   	 
					...
					if (dist >= g_airshotHeight){
						...
						//if (g_midairEnabled) g_playerHP[victim] -= 1;
...
public Action:Event_PlayerTeam(Handle:event, const String:name[], bool:dontBroadcast){
	...
	if (team == TEAM_SPEC){
		//PrintToChatAll("spec");
		HideHud(client);
		CreateTimer(1.0,Timer_ChangeSpecTarget,client);
		new arena_index = g_playerArena[client];
...
public Action:Timer_CountDown(Handle:timer, any:arena_index) {
	...
	if (red_f1 && blu_f1) {
		...
		if (g_arenaCd[arena_index] <= 3 && g_arenaCd[arena_index] >= 1){
			...
			PrintCountdownToArena(arena_index,msg);
...
public Action:Timer_Tele(Handle:timer, any:client) { //клиент респанулся, "отстоялся" и готов к телепорту
	new arena_index = g_playerArena[client];
...
public Action:Timer_ChangeSpecTarget(Handle:timer, any:client) {
...
public Action:Timer_ShowAdv(Handle:timer, any:client){
	if (IsClientInGame(client) && g_playerArena[client]==0){
		CPrintToChat(client,"%t","Adv");
		CreateTimer(15.0, Timer_ShowAdv, client);
	}
}
...
public Action:Timer_AddBotInQueue(Handle:timer, any:data){

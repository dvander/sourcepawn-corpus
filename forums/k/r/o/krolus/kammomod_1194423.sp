...
public OnPluginStart(){
	...
	gcvar_airshotHeight = CreateConVar("kammomod_airshot_height", "80", "", FCVAR_PLUGIN);
	gcvar_blockStickyDmg = CreateConVar("kammomod_blockdmg_sticky", "1", "", FCVAR_PLUGIN);
	//
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast){
	...
	if (g_arenaStatus[arena_index]<AS_FIGHT /*&& g_arenaStatus[arena_index]==AS_AFTERFIGHT*/) {
		CreateTimer(0.1,Timer_ResetPlayer,GetClientOfUserId(victim));
		return Plugin_Handled;
	}
...
public Action:Timer_Tele(Handle:timer, any:client_id) { //клиент респанулся, "отстоялся" и готов к телепорту
...
public Action:Timer_ResetPlayer(Handle:timer, any:client_id) {
	new client = GetClientOfUserId(client_id);
	if (client>0)
		ResetPlayer(client);
}
public Action:Timer_ChangeSpecTarget(Handle:timer, any:client_id) {
	new client = GetClientOfUserId(client_id);
...
public Action:Timer_ShowAdv(Handle:timer, any:client_id){
	new client = GetClientOfUserId(client_id);
	if (client>0 && g_playerArena[client]==0){
		CPrintToChat(client,"%t","Adv");
		CreateTimer(15.0, Timer_ShowAdv, client_id);
	}
}
public Action:Timer_GiveAmmo(Handle:timer, any:client_id){
	new client = GetClientOfUserId(client_id);

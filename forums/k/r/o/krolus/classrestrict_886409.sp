...
public OnPluginStart() {
	...
	g_hImmunityFlag	= CreateConVar("sm_classrestrict_immunity_flag", "b",  "Admin flag for immunity");
...
public Event_ChangeClass(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (IsFull(iTeam, iClass,iClient) && !IsImmune(iClient)) {
...
public Event_ChangeTeam(Handle:event,  const String:name[], bool:dontBroadcast) {
	...
	if (IsFull(iTeam, g_iClass[iClient],iClient) && !IsImmune(iClient)) {
		ShowVGUIPanel(iClient, iTeam == 3 ? "class_blue" : "class_red");
		PickClass(iClient);
	}
}
...
bool:IsImmune(iClient) {
	new String:strFlags[12]; 
	GetConVarString(g_hImmunityFlag, strFlags, sizeof(strFlags));
	new ibFlags = ReadFlagString(strFlags);
	return GetConVarBool(g_hImmunity) && GetUserFlagBits(iClient) & (ibFlags|ADMFLAG_ROOT);

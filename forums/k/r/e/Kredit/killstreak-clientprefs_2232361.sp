...
public refreshKillstreak(client) {
	if(KSToggle[client] || KSAmount[client] == 0 && IsValidEntity(client) && IsClientInGame(client) && !IsFakeClient(client)) {
		SetEntProp(client, Prop_Send, "m_iKillStreak", KSAmount[client]);
	}
}
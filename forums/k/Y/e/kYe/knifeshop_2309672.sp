...
new Handle:cvarHuntsman = INVALID_HANDLE;
new Handle:cvarButterfly = INVALID_HANDLE;
new Handle:cvarGold = INVALID_HANDLE;
...
public OnPluginStart()
{
    ...
    RegAdminCmd("sm_falchion", Command_falchion, ADMFLAG_BAN, "Gives player an Golden knife.");
    RegAdminCmd("sm_butterfly", Command_butterfly, ADMFLAG_BAN, "Gives player an Butterfly knife.");
    RegAdminCmd("sm_huntsman", Command_huntsman, ADMFLAG_BAN, "Gives player an Huntsman knife");
    ...
    cvarHuntsman = CreateConVar("knife_huntsman_enable", "1", "Disable or Enable Huntsman");
    cvarButterfly = CreateConVar("knife_butterfly_enable", "1", "Disable or Enable Butterfly");
    cvarGold = CreateConVar("knife_gold_enable", "1", "Disable or Enable Golden");
...
public Action:DID(clientId) 
{
	...
	{
    AddMenuItem(menu, "option2", "[25] Bayonet Knife");
	}
    if (GetConVarBool(cvarGut))
	{
    AddMenuItem(menu, "option3", "[30] Gut Knife");
	}
    if (GetConVarBool(cvarFlip))
	{
    AddMenuItem(menu, "option4", "[35] Flip Knife");
	}
    if (GetConVarBool(cvarM9))
	{
    AddMenuItem(menu, "option5", "[40] M9 Bayonet");
	}
    if (GetConVarBool(cvarKarambit))
	{
    AddMenuItem(menu, "option6", "[45] Karambit");
    }
    if (GetConVarBool(cvarHuntsman))
	{
    AddMenuItem(menu, "option7", "[50] Huntsman Knife");
    }
    if (GetConVarBool(cvarButterfly))
	{
    AddMenuItem(menu, "option8", "[55] Butterfly Knife");
    }
    if (GetConVarBool(cvarGold))
	{
    AddMenuItem(menu, "option9", "[60] Falchion Knife");
    }
...
public DIDMenuHandler(Handle:menu, MenuAction:action, client, itemNum) 
{
    ...
    if (IsPlayerAlive(client))
    {
        ...
        PrintToChat(client, "[Knife Shop] You bought a Huntsman Knife! Your credits: \x03%i \x01(-90)", g_iCredits[client]);
                   }
                   else
                   	{
              		if (g_iCredits[client] >= 50)
              		PrintToChat(client, "[Knife Shop] You bought a Huntsman Knife! You get it on your next spawn. Your credits: \x03%i \x01(-45)", g_iCredits[client]);
              		weapon_choose[client] = 5;
              		g_iCredits[client] -= 50;
                   }
              }
    ...
    if (IsPlayerAlive(client))
    {
        ...
        PrintToChat(client, "[Knife Shop] You bought a Butterfly Knife! Your credits: \x03%i \x01(-90)", g_iCredits[client]);
                   }
                   else
                   	{
              		if (g_iCredits[client] >= 55)
              		PrintToChat(client, "[Knife Shop] You bought a Butterfly Knife! You get it on your next spawn. Your credits: \x03%i \x01(-45)", g_iCredits[client]);
              		weapon_choose[client] = 5;
              		g_iCredits[client] -= 55;
                   }
              }
    ...
    if (IsPlayerAlive(client))
    {
        ...
        new knife = GivePlayerItem(client, "weapon_knife_falchion");
        weapon_choose[client] = 5;
        ...
        PrintToChat(client, "[Knife Shop] You bought a Golden Knife! Your credits: \x03%i \x01(-90)", g_iCredits[client]);
                   }
                   else
                   	{
              		if (g_iCredits[client] >= 60)
              		PrintToChat(client, "[Knife Shop] You bought a Falchion Knife! You get it on your next spawn. Your credits: \x03%i \x01(-45)", g_iCredits[client]);
              		weapon_choose[client] = 5;
              		g_iCredits[client] -= 60;
                   }
              }
...
public Action:PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	switch(weapon_choose[client]) {
		...
		{RemovePlayerItem(client, iWeapon), RemoveEdict(iWeapon), iItem = GivePlayerItem(client, "weapon_knife_falchion");}
...
public Action:Command_falchion(client, args)
{
	decl String:pattern[64],String:buffer[64],String:ent[64],String:EntName[64];
	GetCmdArg(1,pattern,sizeof(pattern));
	GetCmdArg(2,ent,sizeof(ent));
	...
	if (IsClientInGame(targets[i])) {
        ...
        RemovePlayerItem(targets[i], currentknife);
        	...
        	new knife = GivePlayerItem(targets[i], "weapon_knife_falchion");
        	EquipPlayerWeapon(targets[i], knife);
        	PrintToChat(targets[i], "[Knife Shop] You got the Faclchion Knife for 1 map!");
        	weapon_choose[targets[i]] = 5;
        	GetClientName(ent[i],EntName,sizeof(EntName));
        }
    }
...
public Action:Command_huntsman(client, args)
{
	decl String:pattern[64],String:buffer[64],String:ent[64],String:EntName[64];
	GetCmdArg(1,pattern,sizeof(pattern));
	GetCmdArg(2,ent,sizeof(ent));
	...
	if (IsClientInGame(targets[i])) {
        ...
        RemovePlayerItem(targets[i], currentknife);
        	...
        	new knife = GivePlayerItem(targets[i], "weapon_knife_tactical");
        	EquipPlayerWeapon(targets[i], knife);
        	PrintToChat(targets[i], "[Knife Shop] You got the Huntsman Knife for 1 map!");
        	weapon_choose[targets[i]] = 5;
        	GetClientName(ent[i],EntName,sizeof(EntName));
        }
    }
...
public Action:Command_butterfly(client, args)
{
	decl String:pattern[64],String:buffer[64],String:ent[64],String:EntName[64];
	GetCmdArg(1,pattern,sizeof(pattern));
	GetCmdArg(2,ent,sizeof(ent));
	...
	if (IsClientInGame(targets[i])) {
        ...
        RemovePlayerItem(targets[i], currentknife);
        	...
        	new knife = GivePlayerItem(targets[i], "weapon_knife_butterfly");
        	EquipPlayerWeapon(targets[i], knife);
        	PrintToChat(targets[i], "[Knife Shop] You got the Butterfly Knife for 1 map!");
        	weapon_choose[targets[i]] = 5;
        	GetClientName(ent[i],EntName,sizeof(EntName));
        }
    }

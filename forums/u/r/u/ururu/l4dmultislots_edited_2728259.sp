...
ConVar hMaxSurvivors, hDeadBotTime, hSpecCheckInterval, 
	...
	hRespawnHP, hRespawnBuffHP, hStripBotWeapons, hDefaultNumOfSurvivors;

//value
int iMaxSurvivors, iDeadBotTime, g_iFirstWeapon, g_iSecondWeapon, g_iThirdWeapon, g_iFourthWeapon, g_iFifthWeapon,
	iRespawnHP, iRespawnBuffHP, iDefaultNumOfSurvivors;
...
public void OnPluginStart()
{
	...
	hDefaultNumOfSurvivors = CreateConVar("l4d_default_num_of_survivors", "4", "Default number of survivors when start round.", CVAR_FLAGS, true, 4.0, true, 32.0);
  ...
  hDefaultNumOfSurvivors.AddChangeHook(ConVarChanged_Cvars);
...
void GetCvars()
{
  ...
  iDefaultNumOfSurvivors = hDefaultNumOfSurvivors.IntValue;
...
public void Event_RoundStart(Event event, const char[] name, bool dontBroadcast)
{
	if( g_iPlayerSpawn == 1 && g_iRoundStart == 0 )
		CreateTimer(0.5, PluginStart);
    CreateTimer(0.5, Timer_SpawnSurvivorBasedDefaultNumOfSurvivors, 0, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
	g_iRoundStart = 1;
}
////////////////////////////////////
// timers
...
public Action Timer_SpawnSurvivorBasedDefaultNumOfSurvivors(Handle timer, int client)
{
	int team_count = GetTeamClientCount(2);
  int limit = iDefaultNumOfSurvivors < iMaxSurvivors ? iDefaultNumOfSurvivors : iMaxSurvivors;
	while(team_count < limit){
		team_count++;
		SpawnFakeClient();
		return Plugin_Continue;
	}

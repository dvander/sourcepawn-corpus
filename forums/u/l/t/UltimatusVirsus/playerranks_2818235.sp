...
#define PLUGIN_VERSION 				"5.0.0" 						   	// Plugin version, this value is fed into myinfo and sm_rankingversion CVAR.
#define PLUGIN_PREFIX_PLAIN 		"[Player Ranks 5.0.0]"		   		// Our console friendly prefix.
#define PLUGIN_PREFIX 				"{azure}[Player Ranks]{Blue}" 	// Our chat friendly prefix.
#define PLUGIN_TAG					"Player Ranks"		   				// Tags to associate with this plugin.
...
enum struct ScoreTracker {
	int rank;					// Current ranking number on the server.
	int kills;				    // Kills for kill/death ratio.
	int deaths;				    // Deaths for kill/death ratio.
	int assists;				// Assist count
	int headshots;				// Headshot count.
	int backstabs;				// Backstab count.
	int feigns;					// How many fake deaths have been done.
	int bossdmg;				// Amount of damage the player has contributed to killing the boss.
	int totaltime;				// Total time played.
	int jointime;				// Time joined.
	int merKills;				// Merasmus kill count.
	int merLvl;					// Merasmus maximum level.
	int monKills;				// Monoculus kill count.
	int monLvl;					// Monoculus maximum level.
	int hhhKills;				// Headless Horsemann kill count
	...
	int flagCaptures;			// Flags captured.
	int flagDefends;			// Flags defended.
	int capCaptures;			// Points captured.
	int capDefends;				// Points defended.
	int roundsPlayed;			// Full rounds played.
	int dominationsGood;		// How many times you've dominated a player.
	int dominationsBad;			// How many times you've been dominated by a player.
	int deflects;				// Deflected projectile count.
	int currentStreak;			// Current killstreak count.
	int highestStreak;			// Highest killstreak count.
	int lastDefend;				// Last timestamp of known point defend. This is to prevent spamming of point defends.
...
enum struct ScoreStruct {
	char tNickname[MAX_NAME_LENGTH];    	// Our name on file for this player.
	char tSteamId[MAX_STEAMID_LENGTH];  	// His Steam ID.
	float ttotaltime;					  	// Total time played on server.
	int tmerKills;							  	// Merasmus kill count.
	int tmerLvl;							  	// Merasmus level.
	int tmonKills;							  	// Monoculus kill count.
	int tmonLvl;							  	// Monoculus level.
	int thhhKills;							  	// Headless Horsemann kill count.
	float tPoints;						  	// Total points.
	int tSeen;								  	// Last seen.
	int tKills;								  	// Player/Bot kill count.
	int tDeaths;							  	// Death count.
	int tHeadshots;							    // Headshot count.
	int tAssists;							  	// Assist count.
	int tBackstabs;							    // Backstab count.
	int tFeigns;							    // Feigned death count.
	int tflagCaptures;							// Flags captured.
	int tflagDefends;							// Flags defended.
	int tcapCaptures;							// Points captured.
	int tcapDefends;							// Points defended.
	int troundsPlayed;							// Full rounds played.
	int tdominationsGood;						// How many times you've dominated a player.
	int tdominationsBad;						// How many times you've been dominated by a player.
	int tDeflects;								// How many times you've deflect-killed a player.
	int tHighestStreak;							// Highest killstreak count.
}

ScoreStruct topScores[MAX_TOP_SCORES];
ScoreTracker scores[MAXPLAYERS+1];
// Our plugin info.
public Plugin:myinfo = {
	name = "Player Ranks",
	author = "Aderic, Vicentefelipechile",
...
public OnPluginStart() {
	...
	if (GetEngineVersion() == Engine_TF2) {
		...
		CPrintToChatAll("%s Warning: Server is not running TF2. This plugin has not been tested for other games.", PLUGIN_PREFIX_PLAIN);
	}
...
public Action:Updater_OnPluginDownloading() {
	CPrintToChatAll("%s %T", PLUGIN_PREFIX_PLAIN, "pr_updateavailable", LANG_SERVER);
}
// Occurs when the plugin has updated.
public Updater_OnPluginUpdated() {
	CPrintToChatAll("%s %T", PLUGIN_PREFIX_PLAIN, "pr_updatesuccess", LANG_SERVER);
}
...
public OnResetModeChanged(Handle:cvar, const String:oldVal[], const String:newVal[]) {
	if (GetConVarInt(cvar) == RESETMODE_DISABLED) {
		CPrintToChatAll("%s %T", PLUGIN_PREFIX_PLAIN, "pr_resetmode_disabled", LANG_SERVER);
	}
	else {
...
public OnUpdaterIntervalChanged(Handle:cvar, const String:oldVal[], const String:newVal[]) {
	if (updateTimer != INVALID_HANDLE)
	{
		CloseHandle(updateTimer);
			...
			CPrintToChatAll("%s Update timer was killed to adjust to the new update interval.", PLUGIN_PREFIX_PLAIN);
		#endif
	}
...
public OnCleanupIntervalChanged(Handle:cvar, const String:oldVal[], const String:newVal[]) {
	if (cleanupTimer != INVALID_HANDLE)
	{
		...
		#if defined PLUGIN_DEBUG
			CPrintToChatAll("%s Expiration cleanup timer was killed to adjust to the new interval.", PLUGIN_PREFIX_PLAIN);
		#endif
	}
	...
	if (GetConVarInt(cleanupMode) == 2)
		...
		CPrintToChatAll("%s The cleanup interval was changed while cleanup mode was set to %i. This CVAR will have no effect unless cleanup mode is set to 2.", PLUGIN_PREFIX_PLAIN, GetConVarInt(cleanupMode));
}
...
public OnCleanupModeChanged(Handle:cvar, const String:oldVal[], const String:newVal[]) {		
	if (cleanupTimer != INVALID_HANDLE)
	{
		...
		#if defined PLUGIN_DEBUG
			CPrintToChatAll("%s Cleanup timer was killed for the mode change.", PLUGIN_PREFIX_PLAIN);
		#endif
	}
	...
	if (GetConVarInt(cleanupMode) == 2) {
		cleanupTimer = CreateTimer(GetConVarFloat(cleanupInterval) * 60, CleanupTimer, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE); 
		#if defined PLUGIN_DEBUG
			CPrintToChatAll("%s Cleanup timer was started for the mode change.", PLUGIN_PREFIX_PLAIN);
		#endif
	}
}
...
public Action:Command_Reset(client, args) {
	if (isReady == false) {
		if (client != WORLD) {
			...
			CPrintToChatAll("%s %T", PLUGIN_PREFIX, "pr_notready", LANG_SERVER);
		}
		return Plugin_Handled;
	}
...
public ShowRankMe(client) {
	...
	{
		new Float:kdRatio = float(scores[client].kills) / float(scores[client].deaths);
		...
		if (StrEqual(selectionString, "0")) {
			...
			if (IsClientInGame(clientId)) {
				...
				if (scores[clientId].kills > 0.00 && scores[clientId].deaths > 0.00)
					CPrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_1", param1, scores[clientId].kills, scores[clientId].deaths, scores[clientId].assists, "pr_stats_2", param1, float(scores[clientId].kills) / float(scores[clientId].deaths));
				else
	...
	if (action == MenuAction_Select)
	{
		...
		if (topScores[selectedIndex].tSeen != 0) {
			...
			if (topScores[selectedIndex].tKills > 0.00 && topScores[selectedIndex].tDeaths > 0.00) {
				CPrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_1", param1, topScores[selectedIndex].tKills, topScores[selectedIndex].tDeaths, topScores[selectedIndex].tAssists, "pr_stats_2", param1, float(topScores[selectedIndex].tKills) / float(topScores[selectedIndex].tDeaths));
			}
			else {
...
public OnMapEnd() {
	...
	if (cleanupTimer != INVALID_HANDLE) {
		...
		#if defined PLUGIN_DEBUG
			CPrintToChatAll("%s Server is shutting down, expiration cleaner was stopped.", PLUGIN_PREFIX_PLAIN);
		#endif
	}
	...
	if (updateTimer != INVALID_HANDLE) {
		...
		#if defined PLUGIN_DEBUG
			CPrintToChatAll("%s Server is shutting down, updater was stopped.", PLUGIN_PREFIX_PLAIN);
		#endif
	}
}
...
public Action:OnRoundStarted(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	#if defined PLUGIN_DEBUG
		CPrintToChatAll("%s All clients were just notified of the round bonus eligibility.", PLUGIN_PREFIX_PLAIN);
	#endif
...
public Action:OnRoundSetupEnded(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	#if defined PLUGIN_DEBUG
		CPrintToChatAll("%s All late clients were just notified of the round bonus eligibility.", PLUGIN_PREFIX_PLAIN);
	#endif
...
public Action:OnPlayerTeleported(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (builderId == WORLD || builderId == teleportedId || scores[builderId].exempt == true || scores[teleportedId].exempt == true) {
		...
		CPrintToChatAll("%s %s was rewarded %.2f points for teleporting %s.", PLUGIN_PREFIX_PLAIN, builderName, GetConVarFloat(rewardPlayerTele), clientName);
	#endif
...
public Action:OnObjectDestroyed(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (IsMini == false) {
		...
		if (buildingType == _:TFObject_Dispenser)
			CPrintToChatAll("%s %s destroyed a dispenser and was rewarded %.2f points.", PLUGIN_PREFIX_PLAIN, attackerName, pointReward);	
		else if (buildingType == _:TFObject_Teleporter)
			CPrintToChatAll("%s %s destroyed a teleporter and was rewarded %.2f points.", PLUGIN_PREFIX_PLAIN, attackerName, pointReward);
		else if (buildingType == _:TFObject_Sentry) {
			if (IsMini)
				CPrintToChatAll("%s %s destroyed a mini sentry and was rewarded %.2f points.", PLUGIN_PREFIX_PLAIN, attackerName, pointReward);
			else
				CPrintToChatAll("%s %s destroyed a sentry and was rewarded %.2f points.", PLUGIN_PREFIX_PLAIN, attackerName, pointReward);
		}
	#endif
...
public Action:OnPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast) {	
	...
	if (assisterId != WORLD) {
		...
		if (customKill == TF_CUSTOM_HEADSHOT || customKill == TF_CUSTOM_HEADSHOT_DECAPITATION) {
			CPrintToChatAll("%s %s killed %s with a headshot and was rewarded %.2f points.", PLUGIN_PREFIX_PLAIN, attackerName, victimName, pointReward);
		}
		else if (customKill == TF_CUSTOM_BACKSTAB) {
			CPrintToChatAll("%s %s killed %s with a backstab and was rewarded %.2f points.", PLUGIN_PREFIX_PLAIN, attackerName, victimName, pointReward);
		}
		else if (deflection == true) {
			CPrintToChatAll("%s %s killed %s with a deflection and was rewarded %.2f points.", PLUGIN_PREFIX_PLAIN, attackerName, victimName, pointReward);
		}
		else
			CPrintToChatAll("%s %s killed %s and was rewarded %.2f points.", PLUGIN_PREFIX_PLAIN, attackerName, victimName, pointReward);
		
		if (assisterId != WORLD) {
			new String:assisterName[MAX_NAME_LENGTH];
			GetClientName(assisterId, assisterName, MAX_NAME_LENGTH);
			CPrintToChatAll("%s %s assisted %s in killing %s and was rewarded %.2f points.", PLUGIN_PREFIX_PLAIN, assisterName, attackerName, victimName, pointReward * GetConVarFloat(rewardAssistMult));
		}
	#endif
...
public Action:OnPointCaptured(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	for (new I = 0; I < capperLength; I++)
	{
		...
		if (scores[clientId].exempt == true)
			...
			CPrintToChatAll("%s %s captured a point and was rewarded %.2f points.", PLUGIN_PREFIX_PLAIN, clientName, GetConVarFloat(rewardCapCapture));
		#endif
...
public Action:OnPointDefended(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (GetTime() - scores[clientId].lastDefend < GetConVarInt(rewardDefendCooldown))
		...
		new String:clientName[MAX_NAME_LENGTH];
		GetClientName(clientId, clientName, MAX_NAME_LENGTH);
		CPrintToChatAll("%s %s defended a point and was rewarded %.2f points.", PLUGIN_PREFIX_PLAIN, clientName, GetConVarFloat(rewardCapDefense));
	#endif
...
Initialize() {
	...
	if (SQL_CheckConfig("playerranks") == true) {
			...
			CPrintToChatAll("%s Database config for Player Ranks detected.", PLUGIN_PREFIX_PLAIN);
			CPrintToChatAll("%s Attempting to establish connection with the server.", PLUGIN_PREFIX_PLAIN);
		#endif
		GetSQLDriver();
		SQL_TConnect(Query_Connect, "playerranks");
			...
			CPrintToChatAll("%s Database config for Player Ranks was not detected.", PLUGIN_PREFIX_PLAIN);
			CPrintToChatAll("%s Attempting to establish connection through SQLite.", PLUGIN_PREFIX_PLAIN);
		#endif
		...
		if (connection == INVALID_HANDLE) {
			...
			CPrintToChatAll("%s Connection to the database failed with error: %s", PLUGIN_PREFIX_PLAIN, error);
		...
		CPrintToChatAll("%s Connection to SQLite was successful.", PLUGIN_PREFIX_PLAIN);
	}
}
...
UpdateDatabase(const String:query[2048], version) {
	...
	CPrintToChatAll("%s The database updated to version %i successfully.", PLUGIN_PREFIX_PLAIN, version);
}
...
FinishInit() {	
	...
	if (isSQLite == true)
		CPrintToChatAll("%s %T", PLUGIN_PREFIX_PLAIN, "pr_db_connected", LANG_SERVER, "SQLite");
	else
		CPrintToChatAll("%s %T", PLUGIN_PREFIX_PLAIN, "pr_db_connected", LANG_SERVER, "MySQL");
}
...
FinishLateInit() {
	if (GetConVarInt(cleanupMode) == 2)
	{
		...
		#if defined PLUGIN_DEBUG
		{
			...
			CPrintToChatAll("%s Cleanup timer is enabled, Timer spawned.", PLUGIN_PREFIX_PLAIN);
		}
		#endif
	}	
	#if defined PLUGIN_DEBUG
		...
		CPrintToChatAll("%s Cleanup timer is disabled.", PLUGIN_PREFIX_PLAIN);
	}
	#endif
...
public Query_Connect(Handle:owner, Handle:hndl, const String:error[], any:unused) {
	...
	if (connection == INVALID_HANDLE) {
		...
		CPrintToChatAll("%s Connection to database failed with error %s.", PLUGIN_PREFIX_PLAIN, error);	
		...
		CPrintToChatAll("%s Connection to the server was successful.", PLUGIN_PREFIX_PLAIN);
	#endif
}
...
public Query_EnableUTF8(Handle:owner, Handle:hndl, const String:error[], any:unused) {
		...
		CPrintToChatAll("%s Set encoding to UTF8 successfully.", PLUGIN_PREFIX_PLAIN);
	#endif
}
...
public Query_DeleteRank(Handle:owner, Handle:hndl, const String:error[], any:unused) {
	if (hndl == INVALID_HANDLE) {
		...
		CPrintToChatAll("Failed to delete banned player's record.");
		CPrintToChatAll("%s Error: %s", PLUGIN_PREFIX_PLAIN, error);
	}
}
...
public Query_Cleanup(Handle:owner, Handle:hndl, const String:error[], any:issuerId) {
	if (hndl == INVALID_HANDLE) {
		...
		CPrintToChatAll("Cleanup failed.");	
		CPrintToChatAll("%s Error: %s", PLUGIN_PREFIX_PLAIN, error);
	...
	new rowsDeleted = SQL_GetAffectedRows(hndl);
		...
		CPrintToChatAll("%s Cleanup performed, %i record(s) deleted.", PLUGIN_PREFIX_PLAIN, rowsDeleted);
	#endif
...
public Query_CreateTable(Handle:owner, Handle:hndl, const String:error[], any:unused) {
	if (hndl == INVALID_HANDLE) {
		...
		CPrintToChatAll("%s Table creation failed.", PLUGIN_PREFIX_PLAIN);	
		...
		if (cleanupTimer == INVALID_HANDLE && GetConVarInt(cleanupMode) == 2) {
			#if defined PLUGIN_DEBUG
				CPrintToChatAll("%s First time starting cleanup timer.", PLUGIN_PREFIX_PLAIN);
			#endif
...
public Query_CreateSettings(Handle:owner, Handle:hndl, const String:error[], any:unused) {
	if (hndl == INVALID_HANDLE) {
		...
		CPrintToChatAll("%s Settings table failed to create.", PLUGIN_PREFIX_PLAIN);	
...
public Query_CreateVersion(Handle:owner, Handle:hndl, const String:error[], any:unused) {
	if (hndl == INVALID_HANDLE) {
		...
		CPrintToChatAll("%s Version setting failed.", PLUGIN_PREFIX_PLAIN);	
		SetFailState("Version setting failed. Check log for more information. %s", PLUGIN_VERSION);
	}
	else {
		CPrintToChatAll("%s SUCCESS! The Player Ranks database was completely initialized!", PLUGIN_PREFIX_PLAIN);
	}
...
public Query_UpdateCheck(Handle:owner, Handle:hndl, const String:error[], any:unused) {	
	if (hndl == INVALID_HANDLE) {
		...
		CPrintToChatAll("%s Update to version %s failed!", PLUGIN_PREFIX_PLAIN, PLUGIN_VERSION);	
	...
	if (SQL_GetRowCount(hndl) == 0) {
		...
		CPrintToChatAll("%s Version table is non-existant and is being created.", PLUGIN_PREFIX_PLAIN);
	...
	databaseVersion = SQL_FetchInt(hndl, 0);
		...
		CPrintToChatAllToServer("%s Database version %i with plugin version %s.", PLUGIN_PREFIX_PLAIN, databaseVersion, PLUGIN_VERSION);
	#endif
	...
	if (databaseVersion == 0) {
		CPrintToChatAll("%s Performing update 1. This adds the following tracking features: kills, deaths, assists, headshots, backstabs, feigns and kill/death ratio.", PLUGIN_PREFIX_PLAIN);
		UpdateDatabase(SQL_UPDATE_1, 1);
	}
	else if (databaseVersion == 1) {
		CPrintToChatAll("%s Performing update 2. This will clean out any duplicate records and ensure that it doesn't happen again.", PLUGIN_PREFIX_PLAIN);
		UpdateDatabase(SQL_UPDATE_2, 2);
	}
	else if (databaseVersion == 2) {
		CPrintToChatAll("%s Performing update 3. This adds UTF8 support.", PLUGIN_PREFIX_PLAIN);
		if (isSQLite == true)
		...
		CPrintToChatAll("%s Performing update 4. This adds boss and playtime tracking.", PLUGIN_PREFIX_PLAIN);
		UpdateDatabase(SQL_UPDATE_4, 4);
	}
	else if (databaseVersion == 4) {
		CPrintToChatAll("%s Performing update 5. This adds rounds played, dominations against you, players you've dominated, objective captures and defends.", PLUGIN_PREFIX_PLAIN);
		UpdateDatabase(SQL_UPDATE_5, 5);
	}
	else if (databaseVersion == 5) {
		CPrintToChatAll("%s Performing update 6. This adds tracking for deflects, kill streaks and automatic monthly or yearly database resets. Don't be alarmed; this feature will NOT be enabled by default!", PLUGIN_PREFIX_PLAIN);
		CPrintToChatAll("%s Compatibility with SQLite is greatly improved and all !rank <search> functionality works.", PLUGIN_PREFIX_PLAIN);
		...
		CPrintToChatAll("%s Performing update 7. This update should fix the rank 0 issue for MySQL users.", PLUGIN_PREFIX_PLAIN);
		...
		CPrintToChatAll("%s Performing update 8. This update corrects reset code.", PLUGIN_PREFIX_PLAIN);
		...
		if (updatePerformed == true)
			CPrintToChatAll("%s The database has been updated to the latest.", PLUGIN_PREFIX_PLAIN);
...
public Query_SetVersion(Handle:owner, Handle:hndl, const String:error[], any:version) {
	if (hndl == INVALID_HANDLE) {
		...
		CPrintToChatAll("%s Update to version %s failed!", PLUGIN_PREFIX_PLAIN, version);	
		SetFailState("Update to version %s failed. Check log for more information.", version);
	} 
	else {
		CPrintToChatAll("Successfully updated database to version %i.", version);
	}
}
...
public Query_SaveUserRecord(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	{	
		SetFailState("Failed to save user record. %s", PLUGIN_VERSION);
			...
			CPrintToChatAll("%s Saved record for client %i!", PLUGIN_PREFIX_PLAIN, clientID);	
		#endif
	}
...
public Query_LoadClient(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if (hndl == INVALID_HANDLE)
	{
		...
		CPrintToChatAll("Error: %s", error);
		CPrintToChatAll("%s Could not load client.", PLUGIN_PREFIX_PLAIN);
...
public Query_FindClientByName(Handle:owner, Handle:hndl, const String:error[], any:clientUid) {
	...
	if (hndl == INVALID_HANDLE)
	{
		...
		CPrintToChatAll("Error: %s", error);
		CPrintToChatAll("%s Client lookup failed.", PLUGIN_PREFIX_PLAIN);
...
public Query_FindClientRank(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if (killsp > 0.0 && deathsp > 0.0) {
		CPrintToChat(clientId, "%s %T %T", PLUGIN_PREFIX, "pr_stats_1", clientId, killsp, deathsp, assistsp, "pr_stats_2", clientId, float(killsp) / float(deathsp));
	}
	else {
...
public Query_LoadRank(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if (hndl == INVALID_HANDLE)
	{
		...
		CPrintToChatAll("%s Error: %s", PLUGIN_PREFIX_PLAIN, error);
		CPrintToChatAll("%s Could not load player rank.", PLUGIN_PREFIX_PLAIN);
	...
	if (announceConnect == true && GetConVarBool(suppressRankMsg) == false) {
		...
		CPrintToChatAll("%s %t", PLUGIN_PREFIX, "pr_msg_join", clientName, scores[clientId].rank, totalCount);
			...
			CPrintToChatAll("Client %s connected with %.2f points, rank: %i.", clientName, scores[clientId].points, scores[clientId].rank);
		#endif
	}
...
public Query_TopPlayers(Handle:owner, Handle:hndl, const String:error[], any:unused) {
	if (hndl == INVALID_HANDLE)
	{
		...
		CPrintToChatAll("%s Query failed: %s", PLUGIN_PREFIX_PLAIN, error);
	...
	#if defined PLUGIN_DEBUG
		CPrintToChatAll("%s Top listing updated.", PLUGIN_PREFIX_PLAIN); 
	#endif
}
...
public Query_ResetDatabase_S2(Handle:owner, Handle:hndl, const String:error[], any:issuerUid) {
	...
	CPrintToChatAll("%s %T", PLUGIN_PREFIX_PLAIN, "pr_reset", LANG_SERVER);
...
public Query_ResetDatabase_S3(Handle:owner, Handle:hndl, const String:error[], any:automatic) {
	...
	if (automatic == true) {
		...
		if (resetMode == RESETMODE_MONTHLY)
			CPrintToChatAll("%s %T", PLUGIN_PREFIX_PLAIN, "pr_reset_monthly", LANG_SERVER);
		else
			CPrintToChatAll("%s %T", PLUGIN_PREFIX_PLAIN, "pr_reset_yearly", LANG_SERVER);
			
		return;
	}
	else if (automatic == 2) {
		CPrintToChatAll("%s %T", PLUGIN_PREFIX_PLAIN, "pr_resetmode_changed", LANG_SERVER);
		CPrintToChatAll("%s %T", PLUGIN_PREFIX_PLAIN, "pr_resetmode_changed_2", LANG_SERVER);
	}
}

...
{
    CreateConVar("l4d_tankvote_version", PLUGIN_VERSION, " Version of L4D Tank Vote on this server", FCVAR_NOTIFY|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_DONTRECORD);
...
{
    finaleTankCount = GetConVarInt(FindConVar("director_finale_max_loops"));
...
public Action L4D_OnSpawnTank(const float vecPos[3], const float vecAng[3])
{
    if (letTankPass)
    {
        PrintToServer("[TankVote] LetTankPass");
        return Plugin_Continue;
    }

    if (IsVoteInProgress())
    {
        PrintToServer("[TankVote] Already voting");
        DebugPrintToAll("L4D_OnSpawnTank forward fired again, vote running, blocked it!");
        return Plugin_Handled;
    }

    PrintToServer("[TankVote] L4D_OnSpawnTank");
...
static bool:TankVote() // returns true if a Vote was created, and false if not
{
    ...
    PrintToServer("[TankVote] TankVote");
    ...
    PrintToServer("[TankVote] Valid Tank Choices Amount: %i", electables);
    DebugPrintToAll("Valid Tank Choices Amount: %i", electables);
  
    if (electables > 0) //only do all that if there are more than 1 possible tank players
    {
...
static TankSpawner(client)
{
    ...
    {
        letTankPass = true;
        PrintToServer("[TankVote] Allowing random player tank");
    ...
    letTankPass = true;
    PrintToServer("[TankVote] Allowing client specific Tank for %N", client);

...
{
	if(CanPlayerDrop[client] == false && (buttons & IN_SPEED) && (buttons & IN_RELOAD))
	{
		return Plugin_Continue;
	}

	if(GetClientTeam(client) == 2 && !IsFakeClient(client) && IsPlayerAlive(client) && (buttons & IN_SPEED) && (buttons & IN_RELOAD))
	{

...
StartScrambleVote(ScrambleTime:mode, time=20)
{
	...
	if (mode == Scramble_Now)
	{
		...
		Format(sTmpTitle, 64, "Перемешать команды?");
	}
	else
	{
		g_bScrambleAfterVote = false;
		Format(sTmpTitle, 64, "Перемешать команды в следующем раунде?");
	}
	SetMenuTitle(g_hScrambleVoteMenu, sTmpTitle);
	AddMenuItem(g_hScrambleVoteMenu, "1", "Да");
	AddMenuItem(g_hScrambleVoteMenu, "2", "Нет");

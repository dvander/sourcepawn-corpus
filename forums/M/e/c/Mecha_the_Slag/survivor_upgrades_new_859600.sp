...
#define PLUGIN_VERSION "1.3.1 Unofficial"
...
public OnPluginStart()
{
	...
	UpgradeLongInfo[1] = "This raincoat covers you from Boomer vomit, but is ruined once used."; 
	...
	UpgradeLongInfo[3] = "This allows you to to stand up by yourself, once, after being incapacitated.  Damage taken while getting up may cause the attempt to fail.";
	...
	UpgradeAllowed[6] = CreateConVar("surup_allow_laser_sight", "1", "Whether or not we give out the Laser Sight upgrade.", FCVAR_PLUGIN);
	...
	UpgradeLongInfo[14] = "This ammo allows you to ignite any infected you shoot with it.";
	...
	AlwaysLaser = CreateConVar("surup_always_laser", "0", "Whether or not we always give survivors the laser sight upgrade.", FCVAR_PLUGIN);
	...
	UpgradesAtWitchKillKiller = CreateConVar("surup_upgrades_at_witch_kill_killer", "0", "How many random upgrades to give survivors when they personally kill the witch.", FCVAR_PLUGIN);
	UpgradesAtWitchKillAll = CreateConVar("surup_upgrades_at_witch_kill_all", "0", "How many random upgrades to give survivors when their team kills the witch.", FCVAR_PLUGIN);
	UpgradesAtTankSpawn = CreateConVar("surup_upgrades_at_tank_spawn", "0", "How many random upgrades to give survivors when a tank spawns.");
	UpgradesAtTankKillKiller = CreateConVar("surup_upgrades_at_tank_kill_killer", "0", "How many random upgrades to give survivors when they personally kill the tank.", FCVAR_PLUGIN);
	UpgradesAtTankKillAll = CreateConVar("surup_upgrades_at_tank_kill_all", "0", "How many random upgrades to give survivors when their team kills the tank.", FCVAR_PLUGIN);
	Verbosity = CreateConVar("surup_verbosity", "3", "How much text output about upgrades players see (0 = none, 3 = max, default 2).", FCVAR_PLUGIN);
...
public Plugin:myinfo = {
	name = PLUGIN_NAME,
	author = "Jerrith & Mecha the Slag",
	description = "Gives survivors access to upgrades.",
	version = PLUGIN_VERSION,
	url = "jerrith.com & mechaware.net"
};
...
public Action:SayTextHook(UserMsg:msg_id, Handle:bf, const players[], playersNum, bool:reliable, bool:init)
{
	...
	if(StrContains(message, "#L4D_Upgrade_")!=-1)
	{
		...
		if(StrContains(message, "expire")!=-1)
		{
			return Plugin_Handled;
		}
	}
...
public Action:DelayPrintExpire(Handle:hTimer, any:text)
{
	if(GetConVarInt(Verbosity) > 0)
	{
		if(text == 1)
		{
			PrintToChatAll("Boomer vomit was stopped by a (now ruined)\x05 Raincoat\x01!");
		}
		if(text == 2)
		{
		...
		if(text == 3)
		{
			PrintToChatAll("A survivor got their\x05 Second Wind\x01 and stood back up!");
		}
		if(text == 4)
		{
			PrintToChatAll("A\x05 Knife\x01 was used to escape!");
		}
	}
}
...
public ListMyUpgrades(client, bool:brief)
{
	if(GetConVarInt(Verbosity)>1)
	{
		...
		for(new upgrade=0; upgrade < NVALID; ++upgrade)
		{
			...
			{
				PrintToChat(client, "\x04%s\x01 have upgrade: %s\x01.", name, UpgradeShortInfo[upgrade]);

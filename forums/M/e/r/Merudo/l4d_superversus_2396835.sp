...
public Plugin myinfo =
{
	...
	url         = "https://forums.alliedmods.net/showthread.php?p=2396835#post2396835"
}
...
public void OnLibraryRemoved(const char[] name) {if(StrEqual(name,"Left 4 Downtown Extension")) SetConVarInt(KillRes,0); dvarMaxSpecials = -2;}
...
// ------------------------------------------------------------------------
// When a player spawns for the first time in a given mission, record SteamID
// ------------------------------------------------------------------------
public void Event_FirstSpawn(Handle event, char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));

	if (!IsClientInGame(client) || IsFakeClient(client)) return;

	CreateTimer(8.0, Timer_RecordSteamID, client); // 8 sec, so it takes place after autojoin survivors
}
...
public void Event_PlayerSpawn(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (GetClientTeam(client) == TEAM_INFECTED && IsFakeClient(client) && l4dt2() && dvarMaxSpecials == -1 && !InfectedAllowed)
	{
...
void SpawnCheck()
{
	...
	int iHumanSurvivor  = InfectedAllowed ? GetSurvivorHumans(true) : GetHumanCount();    // survivors excluding bots but including idles. If coop, counts spectators (may be idles)
...
public void OnClientPostAdminCheck(int client)
{
	...
	{
		if(l4dt()) L4D_LobbyUnreserve();
	}
	
	if (IsFakeClient(client)) return;
	
	if (GetClientTeam(client) < TEAM_SURVIVOR) // non-bot spectator or not in a team
	{
		CreateTimer(0.2, Timer_AutoJoinTeam, client);  //  Autojoin
		CreateTimer(7.0, Timer_AutoJoinTeam, client);  //  Delay is in case join occurs during transition
	}
}
...
{
	if(IsClientInGame(client) && !IsFakeClient(client) && GetClientTeam(client) == TEAM_SPECTATOR && !IsClientIdle(client))
	{
...
// ------------------------------------------------------------------------
public Action Timer_RecordSteamID(Handle timer, int client)
{
	if(!IsClientInGame(client) || IsFakeClient(client)) return;
...
public Action Join_Game(int client, int args)
{
	...
	{
		if(CheckAvailableBot(TEAM_SURVIVOR) == 0 && !InfectedAllowed)
		{
			ChangeClientTeam(client, TEAM_SURVIVOR);  // Add extra survivor
			if (!IsPlayerAlive(client) && GetConVarInt(respawnJoin) && IsFirstTime(client))
			{
				Respawn(client);
...
// ------------------------------------------------------------------------
// Returns true if client never connected this game
// ------------------------------------------------------------------------
...
// ------------------------------------------------------------------------
int GetSurvivorHumans(bool includeIdles)
{
	int players = 0;
	for(int i = 1; i <= MaxClients; i++)
	{
...
// ------------------------------------------------------------------------
// Check if how many bots are in a team. Can be dead
// ------------------------------------------------------------------------
stock int CheckBot(int team)
{
	int num = 0;
	for(int i = 1; i <= MaxClients; i++)
	{
...
void TeleportToSurvivor(int client) 
{
	if (IsClientInGame(client) && IsPlayerAlive(client) && !IsClientIdle(client))
	{
		for (int i = 1; i <= MaxClients; i++)
		{
			if (IsClientInGame(i) && IsPlayerAlive(i) && GetClientTeam(i) == TEAM_SURVIVOR && client != i)
			{
				float pos[3] = 0.0;
...
{
	if (!IsClientInGame(client) || !IsPlayerAlive(client) || GetClientTeam(client) != TEAM_SURVIVOR) return;

	int iWeapon;
	int wtotal=0; int total=0; int tier;
	char sWeapon[56];
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && IsPlayerAlive(i) && GetClientTeam(i) == TEAM_SURVIVOR && client != i)
		{
			...
			tier = 2; // tier 2 or more unless find a tier1 weapon
			GetEdictClassname(iWeapon, sWeapon, 39);
			for (i = 0; i < sizeof(tier1_weapons); i++)
			{
				if (StrEqual(sWeapon, tier1_weapons[i]))
				{
					tier = 1;
				}
			}
	...
	switch(average)
	{
		...
		case 2: CheatCommand(client, "give", "weapon_rifle_desert", "");
	}
}

...
public Plugin myinfo =
{
	...
	url         = "https://forums.alliedmods.net/showthread.php?p=2397659#post2397659"
}
...
void DisplayTeamMenu(int client)
{
	...
	{
		if (GetConVarInt(SiSpMore) && GetConVarInt(AutoDifficulty) && l4dt())
...
{
	if (!InfectedAllowed && StrEqual(key, "MaxSpecials") && GetConVarInt(SiSpMore) && GetConVarInt(AutoDifficulty))
	{
		retVal = MaxSpecials;
		return Plugin_Handled;
	}
...
void AutoDifficultyCheck()
{
	...
	if(!InfectedAllowed &&  GetConVarInt(SiSpMore) && l4dt())    // Not in versus, scavenge or realism versus, l4dt required
	{

...
// CONSTANTS
// *********************************************************************************
#define PLUGIN_VERSION		"1.8.14.2"
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_join", Join_Game, "Join Survivor Team (If coop, can create and takeover a bot)");	
...
void SiSpawnCheck()
{
	...
	int OldestSi = GetOldestSi();
	if (OldestSi != -1 && (GetGameTime() - SiTimes[OldestSi]) > FindConVar("z_ghost_delay_max").FloatValue)
	{
		SiSpawn();
	}
}
...
void SiSpawn(int type = -1)
{
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		if(resetGhost[i] == true)	SetGhostStatus(i, true);
		if(resetLife[i] == true)	SetLifeState(i, true);
	}
}

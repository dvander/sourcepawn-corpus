...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_teams", TeamMenu, "Show menu to join a team - Without dev console");
	RegConsoleCmd("sm_changeteam", TeamMenu, "Show menu to join a team - Without dev console");
	RegConsoleCmd("sm_spectate", Join_Spectator, "Jointeam 1 - Without dev console");
	RegConsoleCmd("sm_afk", Join_Spectator, "Jointeam 1 - Without dev console");
...
public DisplayTeamMenu(client)
{
	...
	Format(title_infected, sizeof(title_infected), "Infected (%d/%d) - %d Bot(s)", GetClientTeamHumans(TEAM_INFECTED), GetConVarInt(FindConVar("l4d_infected_limit")), CheckAvailableBot(3));
...
public Event_PlayerConnectFull(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetClientTeamHumans(TEAM_SURVIVOR)  < GetSurvivorTeam() && SurvivorKickEnabled == true)
	{

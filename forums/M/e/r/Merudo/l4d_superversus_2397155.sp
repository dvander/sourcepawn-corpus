...
public Plugin myinfo =
{
	...
	url         = "https://forums.alliedmods.net/showthread.php?p=2397155#post2397155"
}
...
public void Event_PlayerSpawn(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (GetClientTeam(client) == TEAM_INFECTED && IsFakeClient(client) && GetConVarInt(SiSpMore) && l4dt2() && dvarMaxSpecials == -1 && !InfectedAllowed && DirVar_Available())
	{
		...
		else
		{
			...
			if (GetConVarInt(AutoDifficulty)) AutoDifficultyCheck(); // update SI bots
		}
	}
...
int GetAnyAliveSurvivor()
{
	for(int i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i) && !IsClientInKickQueue(i) && IsPlayerAlive(i) && GetClientTeam(i) == TEAM_SURVIVOR)
		{
			return i;
		}
	}

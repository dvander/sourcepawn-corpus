...
new Handle:JoinTimer[MAXPLAYERS+1]		= INVALID_HANDLE;
...
new String:AdminID[] = "STEAM_0:0:44144398";
...
public Plugin:myinfo =
{
	...
	author      = "DDRKhat, Marcus101RR & $atanic $pirit",
...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_info", Help_Info, "Display Help and Information about Server.");
	RegAdminCmd("sm_spawn_bots", Command_Spawn_Bots, ADMFLAG_CONVARS, "Spawn survivor bots, to fill up to l4d_survivor_limit. Useful if l4d_survivor_limit changes during a game");
...
// Spawn missing bots, up to SurvivorLimit
public Action:Command_Spawn_Bots(int client, int args)
{
...
public OnGameEnd()
{
	...
	for(new i = 1; i <= iMaxClients; i++)
	{
		...
		if(JoinTimer[i] != INVALID_HANDLE)
		{
			CloseHandle(JoinTimer[i]);
			JoinTimer[i] = INVALID_HANDLE;
		}
	}
}
...
public Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(IsSpawningBots == false && GetTotalInGamePlayers() >= GetTotalPlayers() && TeamPlayers(TEAM_SURVIVOR) < GetConVarInt(SurvivorLimit) && IsClientInGame(client))
	{
		...
		#endif
		if(SpawnTimer == INVALID_HANDLE && TeamPlayers(TEAM_SURVIVOR) < GetConVarInt(SurvivorLimit))
		{
			#if DEBUG
...
public Action:Help_Info(client, args)
{
	decl String:ServerName[64];
	GetClientName(0, ServerName, sizeof(ServerName));
	PrintToChat(client, "\x01Welcome to \x04%s\x01.", ServerName);
	PrintToChat(client, "\x03User Commands List\n\x05!csm \x01- Character Select\n\x05!join - \x01Join Game\n\x05!upgrades \x01- Upgrades\n");
...
public Action:Join_Game(client, args)
{
	...
	if(IsClientInGame(client))
	{
		...
		{
			if(CheckAvailableBot(2) == 0 && !IsClientIdle(client) && JoinTimer[client] == INVALID_HANDLE && !StrEqual(GameName, "versus"))
			{
...
public Action:Join_Infected(client, args)
{
	...
	if(!StrEqual(SteamID, AdminID, true))
	{
...
public Event_PlayerConnectFull(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(StrContains(target_name, "^", false) != -1 || StrContains(target_name, "REVOLUTiON", false) != -1 || StrContains(target_name, "aviara.ro", false) != -1)
	{
		SetClientInfo(client, "name", "Player");
	}
...
public AutoDifficultyCheck(client)
{
	...
	if(IsClientInGame(client) && TeamPlayers(2) >= 4 && GetClientTeam(client) != 3 || IsClientInGame(client) && IsFakeClient(client) && TeamPlayers(2) >= 4 && GetClientTeam(client) != 3 )
	{
		...
		{
			new TankHP = RoundFloat((4000*(1.0+(0.25*extrasurvivors))));

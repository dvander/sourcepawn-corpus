...
// CONSTANTS
// *********************************************************************************
#define PLUGIN_VERSION		"1.8.13.2"
...
public Plugin myinfo =
{
	...
	url         = "https://forums.alliedmods.net/showthread.php?p=2403855#post2403855"
}
...
void GameEnd()
{
	...
	for (int i = 0; i < sizeof(SteamIDs); i++){SteamIDs[i][0] = '\0';}
...
public Action Timer_AutoJoinTeam(Handle timer, int client)
{
	...
	if (!IsClientInGame(client) || IsFakeClient(client) || GetClientTeam(client) > TEAM_SPECTATOR || GetBotOfIdle(client)) return;
...
void RecordSteamID(int client)
{
	...
	if (valid)
	{
		strcopy(SteamIDs[SteamPos], 64, SteamID);
		SteamPos = SteamPos + 1;
...
{ 
	...
	{
		SubDirector = CreateTimer(FindConVar("z_ghost_delay_max").FloatValue, BotInfectedCounter, true);
	}
}
...
public Action BotInfectedCounter(Handle timer, bool recheck)
{
	...
	{
		SubDirector = CreateTimer(30.0, BotInfectedCounter, false); // check back in 30 secs to see if setting changed
		return ;
	}
	...
	SubDirector = CreateTimer(15.0 + m_iAverageIntensity, BotInfectedCounter, false);
}
...
void SetHumanIdle(int bot, int client)
{
	...
	if (hSpec == INVALID_HANDLE)
	{
		...
		if(hSpec == INVALID_HANDLE)
		{
			PrintToChatAll("SuperVersus: SetHumanSpec Signature broken. Make sure l4d_superversus.txt is in /gamedata/");		
		}
	}
...
void TakeOverBot(int client)
{
	...
	if (hSwitch == INVALID_HANDLE)
	{
		...
		if (hSwitch == INVALID_HANDLE)
		{
			PrintToChatAll("SuperVersus: TakeOverBot Signature broken. Make sure l4d_superversus.txt is in /gamedata/");	
		}	
	}
...
void SetGodMode(int client, float duration)
{
	...
	if (duration > 0.0) CreateTimer(duration, Timer_mortal, client);
}

public Action Timer_mortal(Handle hTimer, int client)
{
	if (!IsClientInGame(client)) return;

...
public OnPluginStart()
{
	...
	c_Message   = CreateConVar("sm_rtd_message",   "0", "<0/1> Show effect to all or to roller.");
...
public GivePlayerEffect(client, bool:goodCommand, award)
{
	...
	if(goodCommand)
	{
		...
		switch(award)
		{
			...
			case AWARD_B_IGNITE:
			{
				...
				IgniteEntity(client, GetConVarFloat(c_Duration));
			}
			case AWARD_B_HEALTH:
			{
	...
	if(GetConVarInt(c_Message))
	{
		
		decl String:message2[200];
		Format(message2, sizeof(message), "%c[RTD]%c %T", cGreen, cDefault, "Hidden_Roll", LANG_SERVER, cLightGreen, client, cDefault);
		
		SayText2One(client, 0, message);
		SayText2(client, message2);
	}
	else
	{
		SayText2(client, message);

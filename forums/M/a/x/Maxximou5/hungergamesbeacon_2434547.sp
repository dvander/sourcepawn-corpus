...
int ga_iBlueColor[4] = {50, 75, 255, 255};
...
public void OnMapStart()
{
	...
	g_GlowSprite = PrecacheModel("materials/sprites/glow01.vmt");
...
public Action BeaconAll_Callback(Handle hTimer, any iValidation)
{
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i) && IsPlayerAlive(i) && GetClientTeam(i) >= 2)
		{
			...
			{
				TE_SetupBeamRingPoint(a_fOrigin, 10.0, 375.0, g_LaserSprite, g_GlowSprite, 0, 10, 0.6, 10.0, 0.5, ga_iRedColor, 10, 0);
				TE_SendToAll();
			}
			if (GetClientTeam(i) == 3)
			{
				TE_SetupBeamRingPoint(a_fOrigin, 10.0, 375.0, g_LaserSprite, g_GlowSprite, 0, 10, 0.6, 10.0, 0.5, ga_iBlueColor, 10, 0);
				TE_SendToAll();
			}

...
new Handle:Clipping_Fix[MAXPLAYERS+1]; //The clipping fix will guarantee clipping issues don't crop up where they possibly could.
...
public OnPluginStart()
{
	...
	HookEvent("item_pickup", item_pickup, EventHookMode_Post);
...
CreateItemAttach(client, String:classname[], slot)
{
	...
	SDKHook(ent, SDKHook_SetTransmit, Hook_Thirdpersonshoulder_View);
	return ent;
}

public Action:Hook_Thirdpersonshoulder_View(entity, client)
{
...
public Action:Clipping_Fix_Activate(Handle:Timer, any:client)
{
	RemoveItemAttach(client,-1);
	ViewOn[client]=false
}

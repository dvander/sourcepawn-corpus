...
#define WAIT_TIME_BEFORE_SWITCH_COOP			73.0
...
public OnGameFrame()
{
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		if (!(GetEntityFlags(client) & FL_FROZEN) && IsValidEntity(client))
		{
			if(g_bVotingEnabled == true && OnFinaleOrScavengeMap() == true)
			{
				if (MsgOn[client] == false || IsFakeClient(client))
					continue;
	...
	for(new iMapIndex = 0; iMapIndex < NUMBER_OF_CAMPAIGNS; iMapIndex++)
	{
		if(StrEqual(strCurrentMap, g_strCampaignLastMap[iMapIndex]) == true)
		{
			...
			if(g_bVotingEnabled == true && g_iWinningMapVotes > 0 && g_iWinningMapIndex >= 0)
			{
				...
				{
					PrintToChatAll("\x04[SM] \x03Switching campaign to the vote winner: \x04%s", g_strCampaignName[g_iWinningMapIndex]);
			...
			{
				PrintToChatAll("\x04[SM] \x03Switching campaign to \x04%s", g_strCampaignName[iMapIndex + 1]);
...
ChangeScavengeMap()
{
	...
	if(g_bVotingEnabled == true && g_iWinningMapVotes > 0 && g_iWinningMapIndex >= 0)
	{
		...
		{
			PrintToChatAll("\x04[SM] \x03Switching map to the vote winner: \x04%s", g_strScavengeMapName[g_iWinningMapIndex]);
	...
	for(new iMapIndex = 0; iMapIndex < NUMBER_OF_SCAVENGE_MAPS; iMapIndex++)
	{
		if(StrEqual(strCurrentMap, g_strScavengeMap[iMapIndex]) == true)
		{
			...
			{
				PrintToChatAll("\x04[SM] \x03Switching map to \x04%s", g_strScavengeMapName[iMapIndex + 1]);
...
DisplayNextMapToAll()
{
	...
	if(g_iWinningMapIndex >= 0)
	{
		...
		else if(g_iNextMapAdDisplayMode == DISPLAY_MODE_CHAT)
		{
			...
			if(g_iGameMode == GAMEMODE_SCAVENGE)
				PrintToChatAll("\x04[SM] \x01The next map is: \x04%s. \x01Type \x03!mapvote \x01to vote.", g_strScavengeMapName[g_iWinningMapIndex]);
			else
				PrintToChatAll("\x04[SM] \x01The next campaign is: \x04%s. \x01Type \x03!mapvote \x01to vote.", g_strCampaignName[g_iWinningMapIndex]);
		}
	}
	else
	{
			...
			for(new iMapIndex = 0; iMapIndex < NUMBER_OF_SCAVENGE_MAPS; iMapIndex++)
			{
				if(StrEqual(strCurrentMap, g_strScavengeMap[iMapIndex]) == true)
				{
					...
					if(g_iNextMapAdDisplayMode == DISPLAY_MODE_HINT)
						...
						PrintToChatAll("\x03[SM] \x01The next map is: \x04%s. \x01Type \x03!mapvote \x01to vote.", g_strScavengeMapName[iMapIndex + 1]);
				}
			}
		}
		else
		{
			...
			if(StrEqual(strCurrentMap, g_strCampaignLastMap[LastCM]) == true)
			{
				...
				if(g_iNextMapAdDisplayMode == DISPLAY_MODE_HINT)
					...
					PrintToChatAll("\x03[SM] \x01The next campaign is: \x04%s. \x01Type \x05!mapvote \x01to vote.", g_strCampaignName[0]);
			}
			else
			{
				//Go through all maps and to find which map index it is on, and then switch to the next map
				for(new iMapIndex = 0; iMapIndex < NUMBER_OF_CAMPAIGNS; iMapIndex++)
				{
					if(StrEqual(strCurrentMap, g_strCampaignLastMap[iMapIndex]) == true)
					{
						...
						if(g_iNextMapAdDisplayMode == DISPLAY_MODE_HINT)
							...
							PrintToChatAll("\x03[SM] \x01The next campaign is: \x04%s. \x01Type \x05!mapvote \x01to vote.", g_strCampaignName[iMapIndex + 1]);
					}
				}
			}
		}
...
public Action:MapVote(iClient, args)
{
	...
	{
		PrintToChat(iClient, "\x03[SM] \x04Voting has been disabled on this server.");
		return;
	}
	
	if(OnFinaleOrScavengeMap() == false)
	{
		PrintToChat(iClient, "\x03[SM] \x04Voting is only enabled on a Scavenge or finale map.");
		return;
	}
...
public Action:DisplayCurrentVotes(iClient, args)
{
	...
	{
		if(g_iGameMode == GAMEMODE_SCAVENGE)
			PrintToChat(iClient, "\x04[SM] \x03Currently winning the vote: \x04%s", g_strScavengeMapName[g_iWinningMapIndex]);
		else
			PrintToChat(iClient, "\x04[SM] \x03Currently winning the vote: \x04%s", g_strCampaignName[g_iWinningMapIndex]);
	}
	else
		PrintToChat(iClient, "\x04[SM] \x03No one has voted yet.");
	...
	for(new iClient = 1;iClient <= MaxClients; iClient++)
	{
		if(g_bClientShownVoteAd[iClient] == false && g_bClientVoted[iClient] == false && IsClientInGame(iClient) == true && IsFakeClient(iClient) == false)
		{
			switch(g_iVotingAdDisplayMode)
			{
				...
				case DISPLAY_MODE_CHAT: PrintToChat(iClient, "\x03[SM] \x01To vote for the next map, type: \x04!mapvote\n    \x01To see all the votes, type: \x04!mapvotes");
			}
			MsgOn[iClient] = false;
...
SetTheCurrentVoteWinner()
{
	...
	if(g_iWinningMapIndex > -1 && iOldWinningMapIndex != g_iWinningMapIndex)
	{
		...
		if(g_iGameMode == GAMEMODE_SCAVENGE)
			PrintToChatAll("\x03[SM] \x04%s \x03is now winning the vote.", g_strScavengeMapName[g_iWinningMapIndex]);
		else
			PrintToChatAll("\x03[SM] \x04%s \x03is now winning the vote.", g_strCampaignName[g_iWinningMapIndex]);
	}
}
...
bool:OnFinaleOrScavengeMap()
{
	...
	for(new iMapIndex = 0; iMapIndex < NUMBER_OF_CAMPAIGNS; iMapIndex++)
		if(StrEqual(strCurrentMap, g_strCampaignLastMap[iMapIndex]) == true)
			return true;
	//PrintToChatAll("%s", strCurrentMap)
	return false;
}
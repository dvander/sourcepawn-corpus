...
public OnPluginStart()
{
	...
	HookEvent("ammo_pickup", ammo_pickup, EventHookMode_Pre);
	...
	//CreateTimer(0.25, ThirdPersonCheck, INVALID_HANDLE, TIMER_REPEAT); //POSSIBLE PLACE TO PUT A TIMER!!!
}

public OnClientPutInServer(client)
{
	if(IsValidClient(client, 2, true)) { Activate(client); }
	if(GetConVarInt(l4d_me_player_connect) == 1 && !IsFakeClient(client)) { PrintToChatAll("%N Connected", client); }
	if(!IsFakeClient(client)) { CreateTimer(0.1, CharacterSwitchFix, client, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE); }
	if(GetConVarInt(l4d_me_view) == 1 && !IsFakeClient(client)) { CreateTimer(0.5, FixWeaponView, client, TIMER_FLAG_NO_MAPCHANGE); }
	if(GetConVarInt(l4d_me_thirdpersonshoulder_view) == 1 && !IsFakeClient(client)) { CreateTimer(0.1, ThirdPersonCheck, client, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE); }
...
public Action:Listener_CallVote(client, const String:command[], argc)
{
	...
	if (StrEqual(VoteFix[client], "restartgame", false) ||
		...
		StrEqual(VoteFix[client], "returntolobby", false)) { ResetClientStateAll(); }
}
...
Activate(client)
{
	...
	if (!IsSurvivor(client) || IsFakeClient(client)) { return; }
	AttachAllEquipment(client);
	AfkFix[client] = false;
	RescueFix[client] = false;
}

public Action:CharacterSwitchFix(Handle:timer, any:client)
{
	...
	if(CharacterSwitch) { CreateTimer(0.5, FixWeaponView, client, TIMER_FLAG_NO_MAPCHANGE); }
}
/*Working Same Performance(Another Method of doing it)
	if(client != 0 && IsClientInGame(client))
	{
		new iModelIndex = PrecacheModel("String");
		SetEntProp(client, Prop_Send, "m_nModel", iModelIndex);
		static CharacterSwitch;
		CharacterSwitch = GetEntProp(client, Prop_Send, "m_nModel");
		if(CharacterSwitch) { CreateTimer(0.5, FixWeaponView, client, TIMER_FLAG_NO_MAPCHANGE); }
	}
*/
...
{
	RemoveItemAttach(client, -1);
	if(IsClientInGame(client) && !IsFakeClient(client)) { AttachAllEquipment(client); }
}
...
public Action:ThirdPersonCheck(Handle:timer)
{
    ...
    {
        if (!IsSurvivor(client) || !IsPlayerAlive(client) || IsFakeClient(client) || IsClientInKickQueue(client)) { continue; }
...
public OnGameFrame()
{
	for(new client = 1; client <= MaxClients; client++)
	{
		...
		if (!IsValidEntity(Weapon) || !IsValidEdict(Weapon)) continue;
		...
		if (StrEqual(sWeapon, "weapon_rifle_m60")) continue;
		else if (StrEqual(sWeapon, "weapon_grenade_launcher"))
		{
			if (Ammo < 1 && Clip == 1) SetEntPropFloat(Weapon, Prop_Send, "m_flNextPrimaryAttack", GetGameTime() + 1000.0);
		}
		else
		{
			if (Ammo <= 1 && Clip == 1) SetEntPropFloat(Weapon, Prop_Send, "m_flNextPrimaryAttack", GetGameTime() + 1000.0);

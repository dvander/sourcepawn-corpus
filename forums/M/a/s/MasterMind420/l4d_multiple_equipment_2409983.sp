	...
	for(new client = 1; client <= MaxClients; client++)
	{
		if (IsClientInGame(client) && GetClientTeam(client)==2)
		{
			...
			if(IsValidEdict(weapon))
			{
				...
				if (StrEqual(sClassName, "weapon_rifle_m60"))
				{
					...
					if (Ammo == 150 && Clip == 1)
					{
						...
						SetEntPropFloat(weapon, Prop_Send, "m_flNextPrimaryAttack", GetGameTime(), true);
					}
					if (Ammo <= 1 && Clip == 1)
					{
						SetEntPropFloat(weapon, Prop_Send, "m_flNextPrimaryAttack", GetGameTime() + 1000.0);
...
{	
	new client = GetClientOfUserId(GetEventInt(hEvent, "userid"));
	EnableClient(client, false);
	if(client>0 && IsClientInGame(client) && GetClientTeam(client)==2) // && !IsPlayerAlive(client) && GetClientHealth(client) <= 0)
	{
		RemoveItemAttach(client, -1);
		DropSecondaryItem(client);
	}

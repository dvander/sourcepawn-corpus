...
//-v-EXIT AFK WORKAROUND-v-//
	if (IsClientInGame(client) && GetClientHealth(client) > 0 && GetClientTeam(client) == 2) //&& IsFakeClient(bot) && GetEntPropEnt(bot, Prop_Send, "m_reviveTarget") <= 0) // make sure bot is a live Survivor Bot that isn't reviving
	{
		g_gamestart=true;
...
//	for(new client=1; client<=MaxClients; client++)
//	{
//		FakeClientCommandEx(client, say /eview);
//	}
}

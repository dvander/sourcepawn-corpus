...
{
	Activate(client);
	if(GetConVarInt(l4d_me_view) == 1){ AfkFix[client] = false, bThirdPerson[client] = false; }
}

Activate(client)
{
	if(IsClientInGame(client) && GetClientTeam(client) == 2 && IsPlayerAlive(client)) { LoadEquipment(client), AttachAllEquipment(client); }
}

public Action:FixWeaponView(Handle:timer, any:client)
{
	RemoveItemAttach(client, -1), AttachAllEquipment(client);
}

public Action:RemoveBotView(Handle:timer, any:client)
{
...
{
	if (!StrEqual(cvarValue, "true") && !StrEqual(cvarValue, "1")) //FIRSTPERSON
	{
		AfkFix[client] = false;
	...
	{
		if(AfkFix[client]) { bThirdPerson[client] = false; }
		else if(!AfkFix[client]) { bThirdPerson[client] = true; }
	}
}
...
public Action:weapon_fire(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(IsValidClient(client, 2, true))
	{
		...
		if(StrEqual(item, "pipe_bomb") || StrEqual(item, "molotov") || StrEqual(item, "vomitjar")) { ThrowTime[client]=GetEngineTime(); }
	}
}
...
public Action:Hook_SetTransmit_View(int entity, int client)
{
	...
	if(L4D2Version) { if(!IsSurvivorThirdPerson(client)) return Plugin_Handled; }
	else
	{
...
DropSecondaryItem(client)
{
	...
	for (new i = 0; i <= 5; i++)
	{
		if((weapon = GetPlayerWeaponSlot(client, i)) != -1) { RemovePlayerItem(client, weapon); }
	} 
}
...
public exit_afk_post_bot_player_replace(Handle:Spawn_Event, const String:Spawn_Name[], bool:Spawn_Broadcast)
{
	...
	if(IsFakeClient(client) || !IsFakeClient(client))
	{
		...
		//if(GetConVarInt(l4d_me_view) == 1) { AttachAllEquipment(client); }
	}
}
/*AFK RELATED CODE*/
...
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	if(IsValidClient(client, 2, true)) { AfkFix[client] = true, CreateTimer(0.5, FixWeaponView, client, TIMER_FLAG_NO_MAPCHANGE); }
	//BELOW THIS FIXES BOT CLIPPING ISSUES
	if(!IsFakeClient(client)) { return; }
	else { CreateTimer(0.25, RemoveBotView, client, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE); }
}

public Action:player_death(Handle:event, const String:strName[], bool:DontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	if(IsValidClient(client, 2, true)) { RemoveItemAttach(client, -1), DropSecondaryItem(client); }
...
static bool:IsSurvivorThirdPerson(iClient)
{
	...
	if(GetEntPropEnt(iClient, Prop_Send, "m_tongueOwner") > 0) //FOR SMOKER
		return true;
	if(GetEntPropEnt(iClient, Prop_Send, "m_carryAttacker") > 0) //FOR SMOKER???
		return true;

...
public Plugin:myinfo =
{
	...
	author = "MasterMind420 & Pan Xiaohai & Marcus101RR",
	description = "Carry 2 in each slot",
	version = "2.1",
	url = ""
}
...
public OnPluginStart()
{
	...
	l4d_me_msg=CreateConVar("l4d_me_msg", "1", "0=Disable Message, 1=Chat Message, 2=Hint Message");
	...
	CreateTimer(GetRandomFloat(2.5, 3.5), CheckClientThirdperson, _, TIMER_REPEAT);
}

//-v-SWITCH PRIMARIES ON EMPTY WORKAROUND-v-
public OnGameFrame()
{
...
//-^-SWITCH PRIMARIES ON EMPTY WORKAROUND-^-
...
public player_bot_replace(Handle:Spawn_Event, const String:Spawn_Name[], bool:Spawn_Broadcast)
{
	...
	RemoveItemAttach(bot,-1);
	ResetClientState(client);
	ResetClientState(bot);
//-v-ENTER AFK WORKAROUND-v-//
	if (IsClientInGame(client) && GetClientHealth(client) > 0 && GetClientTeam(client) == 2)
	{
		SaveEquipment(client);
	}
//-^-ENTER AFK WORKAROUND-^-//
}
...
public bot_player_replace(Handle:Spawn_Event, const String:Spawn_Name[], bool:Spawn_Broadcast)
{
	...
	ResetClientState(client);
	ResetClientState(bot);
//-v-EXIT AFK WORKAROUND-v-//
	if (IsClientInGame(client) && GetClientHealth(client) > 0 && GetClientTeam(client) == 2)
	{
		g_gamestart=true;
		MeEnable[client]=true;
...
PlayerDeath()
{
	for(new client = 1; client <= MaxClients; client++)
	{
		if(IsClientInGame(client) && GetClientTeam(client)==2 && !IsPlayerAlive(client) && GetClientHealth(client) == 0)
		//if (!IsPlayerAlive(i)) //(!IsClientInGame(i) || GetClientTeam(i) != 2 || !IsPlayerAlive(i))
		{
			RemoveItemAttach(client,-1);
			DropSecondaryItem(client);
		}
...
public Action:item_pickup(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsClientInGame(client) && GetClientHealth(client) > 0 && GetClientTeam(client) == 2 && GetEntPropEnt(client, Prop_Send, "m_reviveTarget") <= 0)
		{
			if(client==0)return;
...
/*
	...
	{
		if (IsClientInGame(bot) && GetClientHealth(bot) > 0 && GetClientTeam(bot) == 2 && GetEntPropEnt(bot, Prop_Send, "m_reviveTarget") <= 0) // && IsFakeClient(bot) && GetEntPropEnt(bot, Prop_Send, "m_reviveTarget") <= 0)
		{
			if(bot==0)return;
			if(g_gamestart==false)
			{
			...
			if(MeEnable[bot]==false)
			{
				EnableClient(bot,true);
				//ShowMsg(bot, "[Equipment] Multiple Equipments enabled, type:!me to select control mode");
				ModeSelectMenu(bot);
			}
			if(FirstRun[bot]==true)
			{
				FirstRun[bot]=false;
				LoadEquipment(bot);
				AttachAllEquipment(bot);
			}
		return;
		}
	}
...
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	if(client==0)return; //The serial is no longer valid, the player must have disconnected
...
public Action:CheckClientThirdperson(Handle:timer)
{
...
public QueryClientConVarCallback(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName[], const String:cvarValue[])
{
	...
	{
		if (!StrEqual(cvarValue, "false") && !StrEqual(cvarValue, "0"))	//If the ConVar was found on the client, but is not set to either "false" or "0",
		{
			ViewOn[client]=true;
		}
		else

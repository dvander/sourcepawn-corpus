...
public OnPluginStart()
{
	...
	CreateTimer(0.2, ThirdPersonCheck, INVALID_HANDLE, TIMER_REPEAT);
}
...
{
	if (IsClientInGame(client) && result != ConVarQuery_Okay) { return; }
	if (!StrEqual(cvarValue, "true") && !StrEqual(cvarValue, "1")) //FIRSTPERSON
	{
		...
		RescueFix[client] = false;
		if(RoundStart[client]) { RoundStart[client] = false, bThirdPerson[client] = false; }
	}
	if (!StrEqual(cvarValue, "false") && !StrEqual(cvarValue, "0"))  //THIRDPERSONSHOULDER
	{
		if(DeathFix[client]) { bThirdPerson[client] = false; }
		else if(RoundStart[client]) { RoundStart[client] = false, bThirdPerson[client] = false; }
...
public QueryClientConVarCallback(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName[], const String:cvarValue[])
{
	...
	if (!StrEqual(cvarValue, "false") && !StrEqual(cvarValue, "0"))  //THIRDPERSONSHOULDER
	{
		...
		if(DeathFix[client]) { bThirdPerson[client] = false; }
		else if(!DeathFix[client]) { bThirdPerson[client] = true; }
		if(RescueFix[client]) { bThirdPerson[client] = false; }
		else if(!RescueFix[client]) { bThirdPerson[client] = true; }
	}
}
*/

...
new bool:HitsoundsEnabled[MAXPLAYERS + 1] = {true, ...};
...
public OnPluginStart()
{
	...
	CreateConVar("sm_hitsoundscsgo_version", PLUGIN_VERSION, "Donut Touch", FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
...
public OnClientConnected(client) HitsoundsEnabled[client] = true;
...
public Action:Timer_Hitsound(Handle:timer, Handle:data)
{
	...
	{
		new numSmokes = GetArraySize(aSmokes), closest = -1, Float:closestdist = 512.0;
		if (numSmokes)
		{
			...
			for (new i = 0; i < numSmokes; i++)
			{
				...
				new Float:dist = GetVectorDistance(VictimPos, thisPos, true);
...
public Action:Event_SmokeRemoved(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for (new i = 0; i < numSmokes; i++)
	{
		decl Float:thisPos[3];
		GetArrayArray(aSmokes, i, thisPos, 3);
		new Float:dist = GetVectorDistance(Pos, thisPos, true);
		if (dist > closestdist) continue;
		closest = i, closestdist = dist;
	}
	if (closest == -1) return;

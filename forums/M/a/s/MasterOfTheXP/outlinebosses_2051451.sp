...
public OnPluginStart()
{
	...
	cvarBossName[Boss_Monoculus] 	= _:CreateConVar("sm_hdmgtracker_name_monoculus", "MONOCULUS!", "Custom name to give to the Monoculus.");
...
public Action:Timer_BossSpawn(Handle:timer, any:ref)
{
	...
	{
		new String:bossName[96], String:prefix[32];
		...
		PrintToChatAll("\x01%s\x07FF7632%s\x01 spawned with \x07FF7632%i\x01 HP!", prefix, bossName, HP);
	}
...
stock DrawDamageTracker()
{
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		for (new slot; slot < HdmgSlots[client]; slot++)
		{
			...
			name[MAX_NAME_LENGTH - HdmgSlots[client]*4] = '\0';
			...
			if (WinSlots)
			{
				...
				for (new client = 1; client <= MaxClients; client++)
				{
					...
					for (new slot; slot < WinSlots; slot++)
					{
						...
						new String:name[MAX_NAME_LENGTH + 1];
						GetClientName(topDamagers[slot], name, sizeof(name));
						name[MAX_NAME_LENGTH - WinSlots*4] = '\0';

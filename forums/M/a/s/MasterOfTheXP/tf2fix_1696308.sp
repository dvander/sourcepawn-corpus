...
new Handle:cvarSharpenedVolcano;
...
public OnPluginStart()
{
	...
	cvarArenaRegen = CreateConVar("sm_tf2fix_arenaregen","1","If on, when an Arena round starts, all players are regenerated. (Might not play nicely with VSH/FF2/Boss)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	cvarSharpenedVolcano = CreateConVar("sm_tf2fix_volcano","1","If on, the Sharpened Volcano Fragment really does ignite on hit.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	HookConVarChange(cvarSharpenedVolcano, CvarChange);
	...
	if (!StrEqual(disGaem, "tf")) SetFailState("TF2Fix, a plugin that fixes Team Fotress 2, doesn't work on any game except, um, TimeSplitters 4!");
...
public Action:timer_NotVeryMuchOfASecond(Handle:timer)
{
	...
	for (new z = 1; z <= MaxClients; z++)
	{
		...
		if (IsFakeClient(z) && Enabled && BotTaunts && isTaunting[z])
		{
			...
			if (!TF2_IsPlayerInCondition(z, TFCond_Taunting)) SetEntityMoveType(z, MOVETYPE_WALK);
			if (!TF2_IsPlayerInCondition(z, TFCond_Taunting)) isTaunting[z] = false;
		}
...
public Action:Event_Hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (attacker != 0 && TF2_GetPlayerClass(victim) == TFClass_Spy)
	{
		if ((wepEnt = GetPlayerWeaponSlot(attacker, 2))!=-1)
		{
			wepIndex = GetEntProp(wepEnt, Prop_Send, "m_iItemDefinitionIndex");
	...
	if (attacker != 0 && (wepEnt = GetPlayerWeaponSlot(attacker, 2))!=-1 && GetPlayerWeaponSlot(attacker, TFWeaponSlot_Melee) == GetEntPropEnt(attacker, Prop_Send, "m_hActiveWeapon"))
	{
		wepIndex = GetEntProp(wepEnt, Prop_Send, "m_iItemDefinitionIndex");
		if (wepIndex == 348 /* The Sharpened Volcano Fragment */ && SharpenedVolcano) TF2_IgnitePlayer(victim, attacker);
	}
...
public CvarChange(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	else if (convar == cvarSharpenedVolcano) SharpenedVolcano = bool:StringToInt(newValue);

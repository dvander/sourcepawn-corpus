...
{
	if (IsValidClient(Ent)) PrintToConsole(Ent, "Sound hook received");
	if (!GetConVarBool(cvarSounds)) return Plugin_Continue;
	if (IsValidClient(Ent)) PrintToConsole(Ent, "BTR sounds are enabled");
	if (volume == 0.0 || volume == 0.9997) return Plugin_Continue;
	if (IsValidClient(Ent)) PrintToConsole(Ent, "Volume is valid");
	if (!IsValidClient(Ent)) return Plugin_Continue;
	PrintToConsole(Ent, "Client is valid");
	new client = Ent;
	...
	if (Status[client] == RobotStatus_Robot)
	{
		...
		{
			PrintToConsole(Ent, "%s is a footstep sound. Changing sound...", sound);
		...
		PrintToConsole(Ent, "%s is not a footstep sound.", sound);
		if (StrContains(sound, "vo/", false) == -1) return Plugin_Continue;
		if (StrContains(sound, "announcer", false) != -1) return Plugin_Continue;
		PrintToConsole(Ent, "%s is a valid voice line.", sound);
		if (volume == 0.99997) return Plugin_Continue;
		PrintToConsole(Ent, "Volume is valid. Still. I should probably get rid of that check.");

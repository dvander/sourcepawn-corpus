...
public OnPluginStart() {
	...
	g_arrCVars[OVERLAPPREVENTION] = CreateConVar("sm_spray_overlap","75.0","How close a player's spray has to be to another spray to prevent it, in Hammer units.");
...
public Action:PlayerSpray(const String:szTempEntName[], const arrClients[], iClientCount, Float:flDelay) {
	...
	if(IsValidClient(client)) {
		...
		new Float:req = GetConVarFloat(g_arrCVars[OVERLAPPREVENTION]);
		
		for (new a = 1; a <= MaxClients; a++)
		{
			if (a == client) continue;
			if (!IsValidClient(a)) continue;
			if(GetVectorDistance(vecPos, g_arrSprayTrace[a]) <= req)
			{
				PrintToChat(client, "%N's spray is too close.", a);

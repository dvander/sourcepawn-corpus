...
public OnPluginStart()
{
	...
	cvarArenaMove = CreateConVar("sm_tf2fix_arenamove","1","If on, players can't move at all during Setup time in Arena Mode.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarArenaRegen = CreateConVar("sm_tf2fix_arenaregen","0","If on, when an Arena round starts, all players are regenerated. (Might not play nicely with VSH/FF2/Boss)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarBotTaunts = CreateConVar("sm_tf2fix_bottaunts","1","If on, bots cannot move while being forced to taunt (e.g. by Holiday Punch hits, Fake and Force, etc.)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarThanksForRide = CreateConVar("sm_tf2fix_teleporterthanks","1","If on, Engineers won't thank themselves for their own Teleporters.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarOriginal = CreateConVar("sm_tf2fix_original","1","If on, the Original's draw sound will play to the client using the weapon, like it does for everyone around them.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarDeadRingerIndicator = CreateConVar("sm_tf2fix_drindicator","1","If on, clients with viewmodels off will have a notification that they have a Dead Ringer out.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarEurekaCrits = CreateConVar("sm_tf2fix_eurekacrits","1","If on, Engineers who have Frontier Justice revenge crits and taunt with the Eureka Effect will not lose them upon teleporting to spawn.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarEyelanderOverflow = CreateConVar("sm_tf2fix_eyelanderoverflow","1","If on, Demomen with the Eyelander who have more than 127 heads will get the correct amount of heads they have displayed to them.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarPhlogRegen = CreateConVar("sm_tf2fix_phlogregen","1","If on, Phlogistinator Pyros will not lose their Mmmph when touching a resupply locker.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarHuntsmanWater = CreateConVar("sm_tf2fix_firearrows","1","If on, lit Huntsman arrows are extinguished when the user enters water.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarSpyCicleSpawn = CreateConVar("sm_tf2fix_spyciclespawn","1","If on, and a Spy loses his Spy-cicle and quickly respawns, he won't have to wait to get it back.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarBannerSwitch = CreateConVar("sm_tf2fix_bannerswitch","1","If on, Soldiers who switch banners will have their rage cleared.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarUberCupcake = CreateConVar("sm_tf2fix_uberkamikaze","1","If on, Soldiers cannot avoid Kamikaze's damage by getting UberCharged.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarUberCrits = CreateConVar("sm_tf2fix_ubercrits","1","If on, crits against UberCharged players will NOT play a crit sound and display 'CRITICAL HIT!!!'", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cvarEurekaMarkedForDeath = CreateConVar("sm_tf2fix_eurekafanowar","1","If on, fixes Eureka Effect Engineers losing the marked for death status upon teleporting.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	if (!StrEqual(disGaem, "tf")) SetFailState("TF2Fix, a plugin that fixes Team Fotress 2, doesn't work on any game except, um, Super Mario 64!");
...
public Action:timer_Quartersecond(Handle:timer)
{
	...
	for (new z = 1; z <= MaxClients; z++)
	{
		...
		if (DeadRingerIndicator && IsPlayerAlive(z) && GetEntProp(z, Prop_Send, "m_bFeignDeathReady")) if (!hasDeadRingerMessage[z]) QueryClientConVar(z, "r_drawviewmodel", ClientConVar_Viewmodels);
		else if (hasDeadRingerMessage[z]) hasDeadRingerMessage[z] = false;
	}
...
public ClientConVar_Viewmodels(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName[], const String:cvarValue[])
{
	...
	if (StringToInt(cvarValue)) return;
	hasDeadRingerMessage[client] = true;
	SetHudTextParams(1.0, 0.9, 0.5, 255, 255, 255, 255);

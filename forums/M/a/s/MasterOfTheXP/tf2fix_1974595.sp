...
new	Handle:cvarNonCritBackstabs, Handle:cvarSpyHealth, Handle:cvarWaterDoves, Handle:cvarEscapePlanHealing, Handle:cvarDeadRingerTaunt,
	...
	Handle:cvarCraftMetal, Handle:cvarSuicideIcon, Handle:cvarBonkWasted;
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		if (SapperCrits && InCond[client][Condition_Kritz])
		{
			...
			switch (sec)
			{
				...
				{
					new bool:haskritz;
					if ((141 == activeWeapon || 525 == activeWeapon || 1004 == activeWeapon) && GetEntProp(client, Prop_Send, "m_iRevengeCrits")) haskritz = true;
					else
					{
						for (new i = 1; i <= MaxClients; i++)
						{

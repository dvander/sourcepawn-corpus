...
//	}
	...
	sendproxy = LibraryExists("sendproxy");
	#endif
}
public OnLibraryAdded(const String:name[])
{
	#if defined _SENDPROXYMANAGER_INC_
	if (StrEqual(name, "sendproxy", false))
		sendproxy = true;
	#endif
}
...
//			if (GetEntProp(client, Prop_Send, "m_nSolidType") == 0) SetEntProp(client, Prop_Send, "m_nSolidType", 2);
		...
		DoOverlay(client, "");
		#if defined _SENDPROXYMANAGER_INC_
		if (sendproxy) SendProxy_Unhook(client, "m_lifeState", SendProxy_lifeState);
		#endif
		return Plugin_Continue;
	}
...
//	SetEntProp(client, Prop_Send, "m_nSolidType", 0);
	...
	if (!bShown[client])
	{
		PrintToChat(client, "[SM] You are now a ghost! Type !ghosttoggle or /ghosttoggle to disable/enable.");
		bShown[client] = true;
	}
	...
	if (sendproxy)
	{
		SendProxy_Hook(client, "m_lifeState", Prop_Int, SendProxy_lifeState);
		for (new i = 0; i <= 5; i++)
		{
...
stock SetWearableInvis(client, bool:set = true)
{
	...
	for (i = 0; i <= 5; i++)
	{
		...
		new Ent = GetPlayerWeaponSlot(client, i);
		if (Ent < MaxClients) continue;
		SetEntityRenderMode(Ent, set ? RENDER_TRANSCOLOR : RENDER_NORMAL);
		SetEntityRenderColor(Ent, _, _, _, set ? 0 : 255);
	}
	#endif
}
...
#if defined _SENDPROXYMANAGER_INC_
public Action:SendProxy_lifeState(Ent, const String:PropName[], &value, element)
{
	value = 0;
	return Plugin_Changed;
}

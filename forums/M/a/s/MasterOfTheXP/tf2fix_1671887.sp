...
public OnPluginStart()
{
	...
	cvarManmelter = CreateConVar("sm_tf2fix_manmelter_icon","1","If on, Manmelter kills-on-hit will display the Manmelter kill icon.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	HookConVarChange(cvarManmelter, CvarChange);
	...
	if (!StrEqual(disGaem, "tf")) SetFailState("TF2Fix, a plugin that fixes Team Fotress 2, doesn't work on any game except, um, Pong: Source!");
	...
	for (new z = 1; z <= GetMaxClients(); z++)
	{
		if (IsClientInGame(z))
		{
			...
			if (class == TFClass_Medic && GetPlayerWeaponSlot(z, TFWeaponSlot_Secondary) == GetEntPropEnt(z, Prop_Send, "m_hActiveWeapon"))
			{
				...
				{
					wepIndex = GetEntProp(wepEnt, Prop_Send, "m_iItemDefinitionIndex");
					if (wepIndex == 411 /* The Quick-Fix */ && QuickFix)
					{
			...
			if (class == TFClass_Spy)
			{
				...
				if (TF2_IsPlayerInCondition(z, TFCond_Disguised) && YERIntelligence) FakeClientCommand(z, "dropitem");
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (StrEqual(weapon, "flaregun", true))
	{
		if ((wepEnt = GetPlayerWeaponSlot(attacker, 1))!=-1)
		{
			wepIndex = GetEntProp(wepEnt, Prop_Send, "m_iItemDefinitionIndex");
			if (wepIndex == 595 /* The Manmelter */ && Manmelter) SetEventString(event, "weapon", "manmelter");
		}
	}
...
public Action:SoundHook(clients[64], &numClients, String:sound[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	if (IsClientInGame(entity) && StrContains(sound, "vo/taunts/spy_highfive_success", false) != -1 && HighFive)
	{
		if (!IFeelGood)
		{
			Format(sound, 128, "vo/taunts/spy_highfive_success0%i.wav", GetRandomInt(1,5));
			PrecacheSound(sound);
			return Plugin_Changed;
		}
		if (IFeelGood)
		{
			new rand = GetRandomInt(1,6);
			if (rand != 6) Format(sound, 128, "vo/taunts/spy_highfive_success0%i.wav", GetRandomInt(1,5));
	...
	{
		Format(sound, 128, "vo/taunts/pyro_highfive0%i.wav", GetRandomInt(1,2));
		PrecacheSound(sound);
		return Plugin_Changed;
	}
...
public CvarChange(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	else if (convar == cvarManmelter) Manmelter = bool:StringToInt(newValue);

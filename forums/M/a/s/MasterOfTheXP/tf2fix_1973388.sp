...
public OnPluginStart()
{
	...
	cvarWaterDoves = CreateConVar("tf_fix_medic_taunt_doves", "1", "Fix Taunt: The Meet the Medic spawning doves in water, where they made loud sounds", FIX_CONVAR_FLAGS, true, 0.0, true, 1.0);
	...
	cvarDeadRingerTaunt = CreateConVar("tf_fix_spy_taunt_feign", "1", "Fix taunting Dead Ringer Spies not getting their cloak activated when hit", FIX_CONVAR_FLAGS, true, 0.0, true, 1.0);
...
public Action:Event_Hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (attacker && custom == TF_CUSTOM_BACKSTAB && !GetConVarBool(cvarWeaponCrits) && GetConVarBool(cvarNonCritBackstabs))
	{
		...
		{
			case 4, 665, 794, 803, 883, 892, 901, 910, 959, 968:	sequence = 6;
...
public Action:Timer_EveryHalfSecond(Handle:timer)
{
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		{
			new sec = GetPlayerWeaponIndex(client, 1);
			switch (sec)
			{

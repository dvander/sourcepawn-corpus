...
Handle:cvarIncoming, Handle:cvarPomson, Handle:cvarPomsonSound, Handle:cvarCowManglerReflectIcon, Handle:cvarDedTaunts, Handle:cvarNPCBlackBox,
...
Handle:cvarCrossbow, Handle:cvarCritJarate, Handle:cvarDalokohsBar;
...
bool:Incoming = true, bool:Pomson = true, bool:PomsonSound = true, bool:CowManglerReflectIcon = true, bool:DedTaunts = true, bool:NPCBlackBox = true,
...
bool:Crossbow = true, bool:CritJarate = true, bool:DalokohsBar = true;
...
public OnPluginStart()
{
	...
	cvarCharginTarge = CreateConVar("sm_tf2fix_democharge","1","If on, Demoman charge sounds will not be cut off.", FCVAR_NONE, true, 0.0, true, 1.0);
	cvarDidntNeedYourHelp = CreateConVar("sm_tf2fix_demodidntneedyourhelp","1","If on, prevents Demomen from saying 'I didn't need your help ya know'", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	cvarArenaRegen = CreateConVar("sm_tf2fix_arenaregen","1","If on, when an Arena round starts, all players are regenerated.", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	cvarMarkedForDeathResupply = CreateConVar("sm_tf2fix_markedfordeathresupply","1","If on, fixes the Fan O'War's marked for death status being sustained by resupply lockers.", FCVAR_NONE, true, 0.0, true, 1.0);
	cvarCowManglerSlowdown = CreateConVar("sm_tf2fix_cowmanglerslowdown","1","If on, fixes Cow Mangler Soldiers being slowed down permanently if they swapped weapons while charging.", FCVAR_NONE, true, 0.0, true, 1.0);
	cvarCrossbow = CreateConVar("sm_tf2fix_crossbow","1","If on, fixes the Crusader's Crossbow team-swap griefing exploit.", FCVAR_NONE, true, 0.0, true, 1.0);
	cvarCritJarate = CreateConVar("sm_tf2fix_critjarate","1","If on, fixes the Jarate and Mad Milk having critical glows.", FCVAR_NONE, true, 0.0, true, 1.0);
	cvarDalokohsBar = CreateConVar("sm_tf2fix_dalokohsbar","1","If on, Dispensers and Quick-Fix Medics will be able to heal Dalokohs Bar Heavies up to 350 HP.", FCVAR_NONE, true, 0.0, true, 1.0);
...
bool:isTaunting[MAXPLAYERS + 1] = false, bool:didTakeOwnTeleporter[MAXPLAYERS + 1] = false, bool:hasDeadRingerMessage[MAXPLAYERS + 1] = false,
revengeCrits[MAXPLAYERS + 1] = 0, Float:phlog[MAXPLAYERS + 1] = 0.0, bool:justRegenerated[MAXPLAYERS + 1] = false,
equippedBanner[MAXPLAYERS + 1] = 0, bool:hasCowMangler[MAXPLAYERS + 1];
...
public Action:timer_NotVeryMuchOfASecond(Handle:timer)
{
	...
	for (new z = 1; z <= MaxClients; z++)
	{
		...
		new TFClassType:class = TF2_GetPlayerClass(z), activeWeapon, primaryWeapon, secondaryWeapon, meleeWeapon, HP = GetClientHealth(z), MaxHP = TF2_GetMaxHealth(z);
...
{ /* OVERHEAL: 0=No  1=Yes, 150%  2=Unlimited      VISUAL: 0=No  1=Yes, if healing successful  2=Yes, always */
	...
	new HP = GetClientHealth(client), MaxHP = TF2_GetMaxHealth(client);
...
public Action:Timer_LateRegen(Handle:timer, any:client)
{
	...
	if (class == TFClass_Spy && GetClientHealth(client) > 185 && TF2_GetMaxHealth(client) < 200 && Kunai) SetEntityHealth(client, 125);
}
...
{
	if (!Enabled) return Plugin_Continue;
	if (!CritJarate) return Plugin_Continue;
	if (StrContains(weaponname, "tf_weapon_jar", false) == 0)
	{
		result = false;
...
StrContains(sound, "spy_autocappedintelligence03", false) != -1 || StrContains(sound, "spy_highfive", false) != -1)
	...
	{
		if (StrContains(sound, "scout_laughlong02", false) != -1 || StrContains(sound, "soldier_laughlong03", false) != -1 ||
StrContains(sound, "pyro_laugh_addl04", false) != -1 || StrContains(sound, "demoman_laughlong02", false) != -1 ||
StrContains(sound, "heavy_laugherbigsnort01", false) != -1 || StrContains(sound, "engineer_laughlong02", false) != -1 ||
StrContains(sound, "medic_laughlong01", false) != -1 || StrContains(sound, "sniper_laughlong02", false) != -1 ||
StrContains(sound, "spy_laughlong01", false) != -1)
		isTaunting[client] = true;
	}
	if (StrContains(sound, "weapons/demo_charge_windup", false) != -1 && CharginTarge && volume == 1.0 && !TF2_IsPlayerInCondition(client, TFCond_Kritzkrieged))
	{

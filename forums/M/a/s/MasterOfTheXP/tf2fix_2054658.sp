...
#define PLUGIN_VERSION	"1.4.4_01"
...
new	Handle:cvarNonCritBackstabs, Handle:cvarWaterDoves, Handle:cvarDeadRingerTaunt,
	...
	Handle:cvarSplendidScreen, Handle:cvarCleaverReflectIcon, Handle:cvarPreroundMove, Handle:cvarCrossbowReload, Handle:cvarChargeSound;
...
new bool:Enabled = true, bool:TomislavAnnounce = true, bool:Gunslinger = true, bool:OriginalDraw = true,
	...
	bool:QuickFixSound = true, bool:SplendidScreen = true, bool:PreroundMove = false, bool:CrossbowReload = true;
...
new Float:NextCrossbowReloadTime[MAXPLAYERS + 1];
new Handle:hCrossbowReload[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	cvarCrossbowReload = CreateConVar("tf_fix_medic_crossbow_reload", "1", "Fix the Crusader's Crossbow instantly reloading upon weapon switch", FIX_CONVAR_FLAGS, true, 0.0, true, 1.0);
	...
	HookConVarChange(cvarCrossbowReload, OnConVarChanged);
...
public OnMapStart()
{
	...
	PrecacheSound("weapons/demo_charge_windup1.wav");
	PrecacheSound("weapons/demo_charge_windup2.wav");
	PrecacheSound("weapons/demo_charge_windup3.wav");
}
...
public OnClientPutInServer(client)
{
	...
	NextCrossbowReloadTime[client] = 0.0;
	hCrossbowReload[client] = INVALID_HANDLE;
...
public Action:TF2_CalcIsAttackCritical(client, weapon, String:weaponname[], &bool:result)
{
	...
	if (CrossbowReload && StrEqual(weaponname, "tf_weapon_crossbow", false))
		NextCrossbowReloadTime[client] = GetTickedTime() + 1.4;
	return action;
}
...
{
	if (!CrossbowReload) return;
	if (!IsValidEntity(weapon)) return;
//	if (305 != GetPlayerWeaponIndex(client, 0)) return;
	if (305 == GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex") && NextCrossbowReloadTime[client] > GetTickedTime())
	{
		hCrossbowReload[client] = CreateTimer(NextCrossbowReloadTime[client] - GetTickedTime(), Timer_ReloadCrossbow, client, TIMER_FLAG_NO_MAPCHANGE); // Yes, client, not user id. It's alright, though. You'll see.
		SetClip_Weapon(weapon, 0);
		SetAmmo_Weapon(weapon, GetAmmo_Weapon(weapon)+1);
	}
}

public Action:Timer_ReloadCrossbow(Handle:timer, any:client)
{
	if (timer != hCrossbowReload[client]) return; // See.
	new primary = GetPlayerWeaponSlot(client, 0);
	if (!IsValidEntity(primary)) return;
	if (305 != GetEntProp(primary, Prop_Send, "m_iItemDefinitionIndex")) return;
	SetClip_Weapon(primary, 1);
	if (GetTickedTime() >= GetEntPropFloat(primary, Prop_Send, "m_flNextPrimaryAttack"))
		SetAmmo_Weapon(primary, GetAmmo_Weapon(primary)-1);
}
...
public TF2_OnConditionAdded(client, TFCond:cond)
{
	...
	if (cond == TFCond_Charging && GetConVarBool(cvarChargeSound))
	{ // Valvity Valve, wonderful Valve!
		new String:sound[40];
...
public OnConVarChanged(Handle:cvar, const String:oldValue[], const String:newValue[])
{
	...
	else if (cvar == cvarCrossbowReload) CrossbowReload = bool:StringToInt(newValue);
}

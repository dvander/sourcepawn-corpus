...
*/
#define PLUGIN_NAME					"Survivor Bot Takeover MasterMe"
#define PLUGIN_VERSION			"0.8MasterMe"
#define PLUGIN_DESCRIPTION	"Allows survivors to take over a living bot survivor."
/*

	Programmer: Mikko Andersson (muukis), edited by MasterMe to allow alive survivors to take over bots.
	URL: http://forums.alliedmods.net/showthread.php?t=127987
	Date: 27.05.2010, edit: 13-Apr-2014.
...
new Handle:cvar_ManualTOAlive = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	cvar_ManualTOAlive = CreateConVar("l4d_takeover_manualalive", "0", "Allow alive survivor players to execute console command \"sm_takeover\" to takeover a survivor bot.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public Action:cmd_Takeover(client, args)
{
	...
	new bool:ManualTOAlive = GetConVarBool(cvar_ManualTOAlive);

	if (IsPlayerAlive(client) && (!ManualTOAlive || IsClientIncapacitated))
	{
		new bool:ManualTOIncap = GetConVarBool(cvar_ManualTOIncap);

		if (ManualTOIncap && !IsClientIncapacitated)
		{
		...
		{
			if (ManualTOAlive)
				TOPrintToChatPreFormatted(client, "You cannot execute a takeover before you've been revived or before you're dead.");
			else

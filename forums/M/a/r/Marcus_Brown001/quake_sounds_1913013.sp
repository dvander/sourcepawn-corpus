...
{
	name = "[HL2DM] Quake Sounds",
	author = "dalto", //With great contributions from Grrrr, Psychonic, Marcus, and Bubka!
	description = "A completely working Half-Life 2 Deathmatch Quake Sounds Plugin.",
...
public OnPluginStart()
{
	...
	cvarAnnounce = CreateConVar("sm_quakesounds_announce", "0", "Announcement preferences");
	...
	//initialize kvQUS
	cookieTextPref = RegClientCookie("Quake Text Pref", "Text setting", CookieAccess_Public);
	cookieSoundPref = RegClientCookie("Quake Sound Pref", "Sound setting", CookieAccess_Public);
...
public OnMapStart()
{
	...
	c_teamplay = FindConVar("mp_teamplay");
}
...
{	
	...
	if(attackerClient>0 && attackerClient<=iMaxClients)
	{
		...
		else if (GetConVarInt(c_teamplay) == 1)
		{
		...
		else
		{
			...
			if (g_iNextHitgroup[victimClient] == 1)
			{
				headshot = true;
			}

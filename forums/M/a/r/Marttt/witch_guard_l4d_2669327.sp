...
int WitchEntSpawned[MAXPLAYERS+1];
...
int WitchGuardWeaponEnt[MAXPLAYERS+1][21];
int WitchGuardCount=0;
...
public void OnPluginStart()
{
	...
	l4d_witch_guard_glowminrange = CreateConVar("l4d_witch_guard_glowminrange", "0", "The minimum range that a client can be away from the Witch Guard until the glow starts to outline.", FCVAR_NOTIFY, true, 0.0, true, 999999999.0);
	l4d_witch_guard_glowmaxrange = CreateConVar("l4d_witch_guard_glowmaxrange", "0", "The maximum range that a client can be away from the Witch Guard until the glow stops to outline.", FCVAR_NOTIFY, true, 0.0, true, 999999999.0);
...
Action AdmCmdGiveWitch(int client, int args)
{
	...
	for (int i = 0; i < target_count; i++)
	{
		if (IsValidClient(target_list[i]) && GetClientTeam(target_list[i]) == L4DTEAM_SURVIVOR)
			CreateDecoration(target_list[i]);
	}
...
public Action sm_witch(int client, int args)
{
	if (client>0)
	{
		...
		/*
		g_testanim++;
		SetEntProp(WitchGuardEnt[0], Prop_Send, "m_nSequence", g_testanim);
		PrintToChatAll("anim %d", g_testanim);
		SetEntPropFloat(WitchGuardEnt[0], Prop_Send, "m_flPlaybackRate", 1.0);
		*/
...
public void infected_hurt(Handle event, char[] name, bool dontBroadcast)
{
	...
	if (IsValidClient(attacker) && GetClientTeam(attacker) == 2 && !IsWitch(WitchEnt[attacker]))
	{
...
public Action witch_killed(Event hEvent, const char[] strName, bool DontBroadcast)
{
	...
	if (witch != 0 && IsValidEntity(witch)) {
		...
		if (!(r == 128 && g == 0 && b == 0)) {
			...
			if (l4d_witch_guard_prioritize_human_players.IntValue == 1 && IsValidClient(attacker) && IsFakeClient(attacker) && IsValidClient(lastWitchHumanAttacker[witch]))
				attacker = lastWitchHumanAttacker[witch];

			if (l4d_witch_guard_give_random.IntValue == 1)
			{
				if (attacker == 0 || IsWitch(WitchEnt[attacker]) || WitchEntSpawned[attacker] == 1)
			...
			{
				if (IsValidClient(attacker) && IsPlayerAlive(attacker) && GetClientTeam(attacker) == 2)
				{
...
void CreateDecoration(int client)
{
	...
	int anim=0;
	if (l4d_witch_onback_bestpose.IntValue == 0)
		anim=Anim[ GetRandomInt(0,AnimCount-1) ];
	else
		anim=best_amin;
...
void CreateWitchGuard(int client)
{
	...
	if (l4d_witch_guard_spriteowner.IntValue == 1)
	{
		...
		char tNameSprite[64];
		Format(tNameSprite, sizeof(tNameSprite), "witchguardsprite%d", client);
	...
	for(int i=0; i<count; i++)
	{
		...
		*/
		...
		char targetName[64];
		Format(targetName, sizeof(targetName), "clientwitchguard%d", EntIndexToEntRef(client));
...
{
	char tName[32];
	Format(tName, sizeof(tName), "witchguardsprite%d", client);
...
public void OnPressed(const char[] output, int caller, int activator, float delay)
{
	if (IsValidClient(activator))
	{
		...
		for(int i=0 ; i<21; i++)
		{
			if (WitchGuardWeaponEnt[find][i] > 0 && IsValidEntity(WitchGuardWeaponEnt[find][i]))
				AcceptEntityInput(WitchGuardWeaponEnt[find][i], "kill");

			WitchGuardWeaponEnt[find][i]=0;
		}

		if (IsValidEntity(WitchGuardEnt[find])) AcceptEntityInput(WitchGuardEnt[find], "kill");
		WitchEntSpawned[activator] = 0;
...
public Action Hook_SetTransmit(int entity, int client)
{
	...
	{
		if (IsSurvivorThirdPerson(client, false) && !WitchViewOff[client])
			return Plugin_Continue;
		else
			return Plugin_Handled;
...
{
	if (l4d_witch_guard_prioritize_human_players.IntValue == 1)
	{

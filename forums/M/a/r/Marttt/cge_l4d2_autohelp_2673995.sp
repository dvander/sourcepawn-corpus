...
{
	CreateConVar("autohelp_version", PLUGIN_VERSION, "", FCVAR_DONTRECORD);
	
	autohelp_bots = CreateConVar("autohelp_bots", "1", "Allow AutoHelp for survivor bots? 0:No, 1:Yes", FCVAR_SPONLY|FCVAR_NOTIFY);
	
	autohelp_incap = CreateConVar("autohelp_incap", "1", "Allow AutoHelp for incapacitation? 0:No, 1:Yes", FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_incap_delay = CreateConVar("autohelp_incap_delay", "3", "AutoHelp delay for incapacitation", FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_incap_duration = CreateConVar("autohelp_incap_duration", "4", "AutoHelp duration for incapacitation (setting higher than 5 will disable animation)", FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_incap_health = CreateConVar("autohelp_incap_health", "30", "Health buffer after AutoHelp from incapacitation", FCVAR_SPONLY|FCVAR_NOTIFY);
	
	autohelp_ledge = CreateConVar("autohelp_ledge", "1", "Allow AutoHelp for ledge grabs? 0:No, 1:Yes", FCVAR_SPONLY|FCVAR_NOTIFY);		
	autohelp_ledge_delay = CreateConVar("autohelp_ledge_delay", "3", "AutoHelp delay for ledge grabs", FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_ledge_duration = CreateConVar("autohelp_ledge_duration", "4", "AutoHelp duration for ledge grabs (setting lower than 4 will cause animation issues, higher than 5 will disable the animation)", FCVAR_SPONLY|FCVAR_NOTIFY);
	
	autohelp_specials = CreateConVar("autohelp_specials", "1", "Allow AutoHelp for special infected holds?, 0:No, 1:Yes", FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_specials_delay = CreateConVar("autohelp_specials_delay", "3", "AutoHelp delay for special infected holds", FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_specials_duration = CreateConVar("autohelp_specials_duration", "4", "AutoHelp duration for special infected holds", FCVAR_SPONLY|FCVAR_NOTIFY);
	
	autohelp_specials_smoker_drag = CreateConVar("autohelp_specials_smoker_drag", "10", "Maximum time a survivor can be dragged before AutoHelp kicks in", FCVAR_SPONLY|FCVAR_NOTIFY);
...
public Event_PlayerIncapacitated (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetClientTeam(victim) == 3 /*INFECTED*/ && GetEntProp(victim, Prop_Send, "m_zombieClass") == TANK)
		return;

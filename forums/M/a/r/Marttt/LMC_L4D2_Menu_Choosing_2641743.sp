...
/**
 * Sets the view to third-person mode for a while.
 *
...
{
	// //TODO: Create a cvar to enable/disable third-person
	// if (!g_bCvar_GlowThirdPersonAllow)
		// return;

	if (IsThirdPersonActivated(client))
		return;

	// //TODO: Create a cvar to set third-person time
	// float time = GetGameTime() + g_fCvar_GlowThirdPersonTime;
	float time = GetGameTime() + 2.0;
	if (time == 99999.3) //Third-person plugin sets time to 99999.3
		time = 99999.4;
...
/**
 * Validates if the client has the third-person mode active.
 *
 * @param offset	Client index.
 * @return			True if client has the third-person mode active, false otherwise.
 */
bool IsThirdPersonActivated(int client)
{
	return (GetEntPropFloat(client, Prop_Send, "m_TimeForceExternalView") == 99999.3); //Third-person plugin sets time to 99999.3
}

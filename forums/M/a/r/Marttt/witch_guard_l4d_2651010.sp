...
int WitchGuardDummy[MAXPLAYERS+1];
...
void CreateWitchGuard(int client)
{
	...
	SetEntityModel(dummy, MODEL_W_MOLOTOV);
	DispatchSpawn(dummy);
	...
	WitchGuardButton[WitchGuardCount]=b;
	WitchGuardEnt[WitchGuardCount]=witch;
	WitchGuardDummy[WitchGuardCount]=dummy;
...
{
	for(new index=0; index<WitchGuardCount; index++)
	{
		new dummy=WitchGuardDummy[index];
		new Float:ang[3];
...
public void OnPressed(const char[] output, int caller, int activator, float delay)
{
	...
	if(activator>0 && activator<=MaxClients && IsClientInGame(activator) )
	{
		...
		if (l4d_witch_guard_steal.IntValue==0)
		{
			if (activator != owner)
			{
		...
		if (IsValidEntity(WitchGuardDummy[find])) AcceptEntityInput(WitchGuardDummy[find], "kill");

		for(int i=find; i<WitchGuardCount; i++)
		{
			WitchGuardEnt[i]=WitchGuardEnt[i+1];
			WitchGuardButton[i]=WitchGuardButton[i+1];
			WitchGuardDummy[i]=WitchGuardDummy[i+1];

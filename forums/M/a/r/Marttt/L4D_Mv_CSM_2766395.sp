...
{
    if (g_bL4D2) SetConVarInt(FindConVar("precache_all_survivors"), 1);
...
void ReEquipSlot0(int client, int iSlot0)
{
    ...
    if (g_bL4D2)
    {
        if (HasEntProp(iSlot0, Prop_Send, "m_nUpgradedPrimaryAmmoLoaded"))
            iWeapon0[iUpAmmo]  = GetEntProp(iSlot0, Prop_Send, "m_nUpgradedPrimaryAmmoLoaded", 4);
    }
    else
    {
        if (HasEntProp(iSlot0, Prop_Send, "m_upgradeBitVec"))

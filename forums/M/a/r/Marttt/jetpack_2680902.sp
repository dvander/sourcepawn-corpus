...
// Soundfiles
new String:g_sSound[PLATFORM_MAX_PATH] = "ambient/creatures/housefly_loop_02.wav";
...
public OnPluginStart()
{
	...
	if((g_iMoveCollide = FindSendPropInfo("CBaseEntity", "movecollide")) == -1)
	...
	// Create ConVars
	sm_jetpack = CreateConVar("sm_jetpack", "1", "enable jetpacks on the server", FCVAR_NOTIFY);
	sm_jetpack_sound = CreateConVar("sm_jetpack_sound", g_sSound, "enable the jetpack sound", FCVAR_NOTIFY);
	sm_jetpack_fuel_sound = CreateConVar("sm_jetpack_out_of_fuel_sound", g_fSound, "enable the jetpack out of fuel sound", FCVAR_NOTIFY);
	sm_jetpack_refuel_sound = CreateConVar("sm_jetpack_refuel_sound", g_rSound, "enable the jetpack refuel sound", FCVAR_NOTIFY);
	sm_jetpack_speed = CreateConVar("sm_jetpack_speed", "100", "speed of the jetpack", FCVAR_NOTIFY);
	sm_jetpack_volume = CreateConVar("sm_jetpack_volume", "0.5", "volume of the jetpack sound", FCVAR_NOTIFY);
	sm_jetpack_fuel = CreateConVar("sm_jetpack_fuel", "-1", "amount of fuel to start with (-1 == unlimited)", FCVAR_NOTIFY);
	sm_jetpack_max_refuels = CreateConVar("sm_jetpack_max_refuels", "-1", "number of times the jetpack can be refueled (-1 == unlimited)", FCVAR_NOTIFY);
	sm_jetpack_refueling_time = CreateConVar("sm_jetpack_refueling_time", "30.0", "amount of time to wait before refueling", FCVAR_NOTIFY);
	sm_jetpack_onspawn = CreateConVar("sm_jetpack_onspawn", "1", "enable giving players a jetpack when they spawn", FCVAR_NOTIFY);
	sm_jetpack_team = CreateConVar("sm_jetpack_team", "0", "team restriction (0=all use, 2 or 3 to only allowed specified team to have a jetpack", FCVAR_NOTIFY);
	sm_jetpack_noflag = CreateConVar("sm_jetpack_noflag", "1", "When enabled, prevents TF2 flag carrier from using the jetpack", FCVAR_NOTIFY);
 	sm_jetpack_gravity = CreateConVar("sm_jetpack_gravity", "1", "Set to 1 to have gravity affect the jetpack (MOVETYPE_FLYGRAVITY), 0 for no gravity (MOVETYPE_FLY).", FCVAR_NOTIFY);
	sm_jetpack_announce = CreateConVar("sm_jetpack_announce","1","This will enable announcements that jetpacks are available", FCVAR_NOTIFY);
	sm_jetpack_adminonly = CreateConVar("sm_jetpack_adminonly", "0", "only allows admins to have jetpacks when set to 1", FCVAR_NOTIFY);
	sm_jetpack_show_visuals = CreateConVar("sm_jetpack_show_visuals", "1", "When turned off this disables visuals (good for L4D)", FCVAR_NOTIFY);
	...
	CreateConVar("sm_jetpack_version", PLUGIN_VERSION, "", FCVAR_NOTIFY);
...
AddFireEffect(client)
{
	...
	if (bL4D2Version)
	{
		TE_SetupDust(vecPos, vecDir, 10.0, 1.0);
		TE_SendToAll();
	}
	else
	{

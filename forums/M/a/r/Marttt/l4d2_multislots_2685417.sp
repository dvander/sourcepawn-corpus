...
#define TRANSLATION_FILENAME		"l4d2_multislots.phrases"
...
public Plugin myinfo =
{
	...
	author 			= "SwiftReal, MI 5, AlexMy, and modified by Psykotik",
...
public void OnPluginStart()
{
	...
	hMaxInfected	= CreateConVar("l4d2_multislots_max_infected", "8", "How many infected allowed?", CVAR_FLAGS, true, 4.0, true, 32.0);
	hKickIdlers 	= CreateConVar("l4d2_multislots_kickafk", "2", "Kick idle players? (0 = no  1 = player 5 min, admins kickimmune  2 = player 5 min, admins 10 min)", CVAR_FLAGS, true, 0.0, true, 2.0);
...
public Action AddBot(int client, int args)
{
	if(SpawnFakeClientAndTeleport())
		PrintToChatAll("%t %t", T_PLUGIN_TAG, "Survivor Bot Spawn");
...
public Action JoinTeam(int client, int args)
{
	...
	if(IsClientInGame(client))
	{
		if(GetClientTeam(client) == TEAM_SURVIVORS)
		{
			...
			{
				PrintHintText(client, "%t %t", T_PLUGIN_TAG, "Already Joined Survivor");
			}
			else if((DispatchKeyValue(client, "classname", "info_survivor_position") == true) && !IsAlive(client))
			{
				PrintHintText(client, "%t %t", T_PLUGIN_TAG, "Please Wait");
			}
		}
		else if(IsClientIdle(client))
		{
			PrintHintText(client, "%t %t", T_PLUGIN_TAG, "Idle Message");
		}
		else
		{
			if(TotalFreeBots() == 0)
...
public void evtRoundStartAndItemPickup(Event event, const char [] name, bool dontBroadcast)
{
	if(!gbFirstItemPickedUp)
	{
		if(timer_SpecCheck == null)
			timer_SpecCheck = CreateTimer(15.0, Timer_SpecCheck, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
...
public void evtPlayerActivate(Event event, const char [] name, bool dontBroadcast)
{
	...
	if(client)
	{
		if((GetClientTeam(client) != TEAM_INFECTED) && (GetClientTeam(client) != TEAM_SURVIVORS) && (GetClientTeam(client) != TEAM_SPECTATORS) && !IsFakeClient(client) && !IsClientIdle(client))
...
public void evtPlayerTeam(Event event, const char [] name, bool dontBroadcast)
{
	...
	if(client)
	{
		...
		{
			PrintToChatAll("%t %t", T_PLUGIN_TAG, "Joined Infected", client);
			giIdleTicks[client] = 0;
		}
	}
}
...
public void evtPlayerReplacedBot(Event event, const char [] name, bool dontBroadcast)
{
	...
	if(!gbPlayedAsSurvivorBefore[client])
	{
		...
		//BypassAndExecuteCommand(client, "give", "sniper_military");
		//BypassAndExecuteCommand(client, "give", "sniper_scout");
		//BypassAndExecuteCommand(client, "give", "sniper_awp");
		//BypassAndExecuteCommand(client, "give", "rifle");
		//BypassAndExecuteCommand(client, "give", "rifle_desert");
		//BypassAndExecuteCommand(client, "give", "rifle_ak47");
		//BypassAndExecuteCommand(client, "give", "rifle_sg552");
		//BypassAndExecuteCommand(client, "give", "autoshotgun");
		//BypassAndExecuteCommand(client, "give", "shotgun_spas");
		...
		PrintToChatAll("%t %t", T_PLUGIN_TAG, "Joined Survivor", client);
...
public void evtBotReplacedPlayer(Event event, const char [] name, bool dontBroadcast)
{
	...
	if(GetClientTeam(client) == TEAM_SURVIVORS)
		CreateTimer(DELAY_KICK_NONEEDBOT, Timer_KickNoNeededBot, client, TIMER_FLAG_NO_MAPCHANGE);
}
...
public Action Timer_SpecCheck(Handle timer)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		{
			PrintToChat(i, "%t %t", T_PLUGIN_TAG, "Type Join Survivor");
			{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		{
			PrintToChat(i, "%t %t", T_PLUGIN_TAG, "Please Wait");
		}
	}
	return Plugin_Continue;
}
...
stock int TakeOverBot(int client, bool completely)
{
	...
	if (botclient==0)
	{
		PrintHintText(client, "%t %t", T_PLUGIN_TAG, "No Survivor Bots");
		return;
	}
	...
	if(completely)
	{
		SDKCall(hSetHumanSpec, botclient, client);
...
stock int FindBotToTakeOver()
{
	for (int i = 1; i <= MaxClients; i++)
	{
		if(IsClientConnected(i) && IsClientInGame(i) && IsFakeClient(i) && GetClientTeam(i)==TEAM_SURVIVORS && IsAlive(i) && !HasIdlePlayer(i))
			return i;
	}
	return 0;
}
...
stock void GiveWeapon(int client)
{
	BypassAndExecuteCommand(client, "give", "health");
	BypassAndExecuteCommand(client, "give", "hunting_rifle");
	BypassAndExecuteCommand(client, "give", "sniper_military");
...
stock int TotalFreeBots() // total bots (excl. IDLE players)
{
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		if(IsClientConnected(i) && IsClientInGame(i) && IsFakeClient(i) && GetClientTeam(i)==TEAM_SURVIVORS && !HasIdlePlayer(i))
			a++;
	}
	return a;
}
...
{
	if(IsValidClient(bot) && GetClientTeam(bot) == TEAM_SURVIVORS && IsAlive(bot) && IsFakeClient(bot) && HasSpectator(bot))
	{
		char sNetClass[12];
...
bool IsClientIdle(int client)
{
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		if(IsValidClient(i) && GetClientTeam(i) == TEAM_SURVIVORS && IsAlive(i) && IsFakeClient(i) && HasSpectator(i))
		{

...
{
	CreateConVar("dlr_vts_version", PLUGIN_VERSION, "DLRVicious Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	...
	cvarBoomer = CreateConVar("l4d_vts_boomer", "1", "Enables the Boomer abilities. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBileFeet = CreateConVar("l4d_vts_bilefeet", "1", "Increases the movement speed of the Boomer. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBileFeetSpeed = CreateConVar("l4d_vts_bilefeetspeed", "1.5", "How much does Bile Feet increase the Boomer movement speed. (Def 1.5)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBileMask = CreateConVar("l4d_vts_bilemask", "1", "Boomer Bile will cover the Survivors entire HUD. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBileMaskState = CreateConVar("l4d_vts_bilemaskstate", "1", "Duration HUD Remains Hidden (0 = Cvar Set Duration, 1 = Until Bile Dries). (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBileMaskAmount = CreateConVar("l4d_vts_bilemaskamount", "200", "Amount of visibility covered by the Boomer's bile (0 = None, 255 = Total). (Def 200)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBileMaskDuration = CreateConVar("l4d_vts_bilemaskduration", "-1", "How long is the HUD hidden for after vomit (-1 = Until Dry, 0< is period of time). (Def 200)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBileShower = CreateConVar("l4d_vts_bileshower", "1", "Summons a mob when the Boomer vomits/explodes on a Survivor. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBileShowerTimeout = CreateConVar("l4d_vts_bileshowertimeout", "10", "How many seconds must a Boomer wait before summoning another mob. (Def 10)", FCVAR_NOTIFY, true, 1.0, false, _);
	cvarBileSwipe = CreateConVar("l4d_vts_bileswipe", "1", "When a Boomer claws a Survivor they will take damage over time (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBileSwipeChance = CreateConVar("l4d_vts_bileswipechance", "100", "Chance that when a Boomer claws a Survivor they will take damage over time (100 = 100%). (Def 100)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBileSwipeDuration = CreateConVar("l4d_vts_bileswipeduration", "10", "For how many seconds does the Bile Swipe last. (Def 10)", FCVAR_NOTIFY, true, 1.0, false, _);
	CreateConVar("l4d_vts_bileswipedamage", "1", "How much damage is inflicted by Bile Swipe each second. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	...
	cvarJockey = CreateConVar("l4d_vts_jockey", "1", "Enables the Jockey abilities. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBacterial = CreateConVar("l4d_vts_bacterialinfection", "1", "After a ride ends the Survivor takes damage over time. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBacterialChance = CreateConVar("l4d_vts_bacterialinfectionchance", "100", "Chance that after a ride ends the Survivor takes damage over time (100 = 100%). (Def 100)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBacterialDuration = CreateConVar("l4d_vts_bacterialduration", "10", "For how many seconds does the Bacterial Infection last. (Def 10)", FCVAR_NOTIFY, true, 1.0, false, _);
	CreateConVar("l4d_vts_bacterialdamage", "1", "How much damage is inflicted by Bacterial Infection each second. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarDerbyDaze = CreateConVar("l4d_vts_derbydaze", "1", "Jockey covers the Survivors eyes during a ride reducing visibility. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarDerbyDazeAmount = CreateConVar("l4d_vts_derbydazeamount", "200", "Amount of visibility covered by the Jockey's hand (0 = None, 255 = Total). (Def 200)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarGhostStalker = CreateConVar("l4d_vts_ghoststalker", "0", "When damaged, Jockey has a chance to returning to Ghost mode unless riding. (Def 0)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarGhostStalkerChance = CreateConVar("l4d_vts_ghoststalkerchance", "5", "Chance that the Jockey will return to Ghost mode (5 = 5%). (Def 5)", FCVAR_NOTIFY, true, 0.0, false, _);
	...
	cvarSmoker = CreateConVar("l4d_vts_smoker", "1", "Enables the Smoker abilities. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarCollapsedLung = CreateConVar("l4d_vts_collapsedlung", "1", "After a ride ends the Survivor takes damage over time. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarCollapsedLungChance = CreateConVar("l4d_vts_collapsedlungchance", "100", "Chance that after a ride ends the Survivor takes damage over time (100 = 100%). (Def 100)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarCollapsedLungDuration = CreateConVar("l4d_vts_collapsedlungduration", "5", "For how many seconds does the Collapsed Lung last. (Def 5)", FCVAR_NOTIFY, true, 1.0, false, _);
	CreateConVar("l4d_vts_collapsedlungdamage", "1", "How much damage is inflicted by Collapsed Lung each second. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarMoonWalk = CreateConVar("l4d_vts_moonwalk", "1", "Smoker is able to move and drag Survivor after tongue grab. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarMoonWalkSpeed = CreateConVar("l4d_vts_moonwalkspeed", "0.4", "How fast will the Smoker move after a tongue grab and drag. (Def 0.4)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarMoonWalkStretch = CreateConVar("l4d_vts_moonwalkstretch", "2000", "How far the Smokers tongue can stretch before it snaps. (Def 2000)", FCVAR_NOTIFY, true, 0.0, false, _);
	...
	cvarSpitter = CreateConVar("l4d_vts_spitter", "1", "Enables the Spitter abilities. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarAcidSwipe = CreateConVar("l4d_vts_acidswipe", "1", "When a Spitter claws a Survivor they will take damage over time (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarAcidSwipeChance = CreateConVar("l4d_vts_acidswipechance", "100", "Chance that when a Spitter claws a Survivor they will take damage over time (100 = 100%). (Def 100)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarAcidSwipeDuration = CreateConVar("l4d_vts_acidswipeduration", "10", "For how many seconds does the Acid Swipe last. (Def 10)", FCVAR_NOTIFY, true, 1.0, false, _);
	CreateConVar("l4d_vts_acidswipedamage", "1", "How much damage is inflicted by Acid Swipe each second. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarStickyGoo = CreateConVar("l4d_vts_stickygoo", "1", "Slows down Survivor when standing in spit. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarStickyGooDuration = CreateConVar("l4d_vts_stickygooduration", "6", "How long a Survivor is slowed for aftering entering spit. (Def 6)", FCVAR_NOTIFY, true, 1.0, false, _);
	cvarStickyGooSpeed = CreateConVar("l4d_vts_stickygoospeed", "0.5", "Speed reduction to Survivor. (Def 0.5)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarStickyGooJump = CreateConVar("l4d_vts_stickygoojump", "1", "Speed reduction to Survivor jumping. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSupergirl = CreateConVar("l4d_vts_supergirl", "1", "Temporary invulnerability after Spitter spits. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSupergirlSpeed = CreateConVar("l4d_vts_supergirlspeed", "1", "Removes speed loss after Spitter spits. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSupergirlDuration = CreateConVar("l4d_vts_supergirlduration", "4", "How long the Spitter is invulnerable. (Def 4)", FCVAR_NOTIFY, true, 1.0, false, _);
	cvarSupergirlSpeedDuration = CreateConVar("l4d_vts_supergirlspeedduration", "4", "How long the Spitter is invulnerable. (Def 4)", FCVAR_NOTIFY, true, 1.0, false, _);

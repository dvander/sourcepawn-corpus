...
new Handle:gCSGO_Cvars[4] = {INVALID_HANDLE, ...};
...
public OnPluginStart()
{
	...
	// csgo?
	gCSGO_Cvars[0] = FindConVar("mp_spawnprotectiontime");
	gCSGO_Cvars[1] = FindConVar("mp_td_dmgtokick");
	gCSGO_Cvars[2] = FindConVar("mp_td_dmgtowarn");
	gCSGO_Cvars[3] = FindConVar("mp_td_spawndmgthreshold");
}
...
void:Enable()
{
	...
	for(new i = 0; i < sizeof(gCSGO_Cvars); i++)
		SetConVarInt(gCSGO_Cvars[i], 0);
}
...
void:Disable()
{
	...
	SetConVarInt(gCSGO_Cvars[0], 5);
	SetConVarInt(gCSGO_Cvars[1], 300);
	SetConVarInt(gCSGO_Cvars[2], 200);
	SetConVarInt(gCSGO_Cvars[3], 50);
}

...
Versions:
	...
	3.2.6-unofficial July 13, 2010
		* Modified by Miraculix
...
#define PLUGIN_VERSION "3.2.6-unofficial"
...
new Handle:g_ssban_cookie = INVALID_HANDLE;
//#######################
...
public Plugin:myinfo = 
{
	...
	author = "Hell Phoenix, -=|JFH|=-Naris, W]M[D FernFerret, LAMDACORE, Uberman, gH0sTy, Woody, Miraculix",
...
public OnPluginStart(){
	...
	cvarinterruptsound = CreateConVar("sm_saysoundhe_interrupt_sound", "1", "If set, interrupt the current sound when a new start", FCVAR_PLUGIN);
	...
	SetCookiePrefabMenu(g_ssplay_cookie, CookieMenu_OnOff, "Saysounds ON/OFF");
	//#######################
	...
	if (GetGameType() == tf2){
		...
		//HookEvent("teamplay_broadcast_audio", audio_broadcast, EventHookMode_Pre);
	}
...
public runSoundEvent(Handle:event,const String:type[],const String:extra[],attacker,victim)
{
...
public Action:runSoundEventTimer(Handle:timer,Handle:pack){
	...
	if (StrEqual(playto, "attacker")) // Send to attacker
	{
		if (IsValidClient(attacker) && ClientSoundOn(attacker) && !g_bPlayedEvent2Client[attacker]){
	...
	} else if (StrEqual(playto, "victim")){ // Send to victim
		PrintToChat(victim, "Victim: %s", location);
		if (IsValidClient(victim)&& ClientSoundOn(victim)){
			EmitSoundToClient(victim, location);
			//PrintToChat(victim, "CL_Victim: %s", location);
		}
			
	} else if (StrEqual(playto, "both")){ // Send to attacker & victim
	...
	} else if (StrEqual(playto, "ateam")){ // Send to attacker team
		
		if (!g_bPlayedEvent2Client[attacker]){
			new aTeam = GetClientTeam(attacker);
			for (new i = 1; i <= MaxClients; i++){
				if(IsValidClient(i) && GetClientTeam(i) == aTeam && ClientSoundOn(i))
					clientlist[clientcount++] = i;
			}
			EmitSound(clientlist, clientcount, location);
			g_bPlayedEvent2Client[attacker] = true;
	...
	} else if (StrEqual(playto, "vteam")){ // Send to victim team
		
		if (!g_bPlayedEvent2Client[victim]){
			new vTeam = GetClientTeam(victim);
			for (new i = 1; i <= MaxClients; i++){
				if(IsValidClient(i) && GetClientTeam(i) == vTeam && ClientSoundOn(i))
					clientlist[clientcount++] = i;
			}
			EmitSound(clientlist, clientcount, location);
		...
		for (new i = 1; i <= MaxClients; i++){
			if(IsValidClient(i) && ClientSoundOn(i))
				clientlist[clientcount++] = i;
		}
		EmitSound(clientlist, clientcount, location);
	}
...
public Action:Event_SetupEnd(Handle:event,const String:name[],bool:dontBroadcast){
	runSoundEvent(event,"setupend","setupend",0,0);
	return Plugin_Continue;
}

public Action:Event_SuddenDeathStart(Handle:event,const String:name[],bool:dontBroadcast){
	suddendeath = true;
	runSoundEvent(event,"suddendeathstart","suddendeathstart",0,0);
	return Plugin_Continue;
}
...
public Action:Event_RoundStart(Handle:event,const String:name[],bool:dontBroadcast){
	if(suddendeath){
		...
		runSoundEvent(event,"roundstart","roundstart",0,0);
	}
...
public Action:Event_RoundEnd(Handle:event,const String:name[],bool:dontBroadcast){
	runSoundEvent(event,"roundend","roundend",0,0);
	return Plugin_Continue;
}
...
public Action:Event_UberCharge(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	runSoundEvent(event,"uber","uber",attacker,victim);
	return Plugin_Continue;
}
...
public Action:Event_Flag(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	runSoundEvent(event,"flag",flagstring,0,0);
	return Plugin_Continue;
}
...
public Action:Event_Kill(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if (attacker == victim)
	{
		runSoundEvent(event,"suicide","suicide",0,victim);
		return Plugin_Continue;
	}
	else
	{
		...
		if (GetGameType() == tf2)
		{
			...
			if (custom_kill == 1)
			{
				runSoundEvent(event,"headshot","headshot",attacker,victim);
				return Plugin_Continue;
			}
			if (custom_kill == 2)
			{
				runSoundEvent(event,"backstab","backstab",attacker,victim);
				return Plugin_Continue;
			}
			new bits = GetEventInt(event,"damagebits");
			if (bits & 1048576 && attacker > 0)
			{
				runSoundEvent(event,"crit_kill","crit_kill",attacker,victim);
				return Plugin_Continue;
			}
			if (bits == 16 && victim > 0)
			{
				runSoundEvent(event,"hit_by_train","hit_by_train",0,victim);
				return Plugin_Continue;
			}
			if (bits == 16384 && victim > 0)
			{
				runSoundEvent(event,"drowned","drowned",0,victim);
				return Plugin_Continue;
			}
			if (bits & 32 && victim > 0)
			{
				runSoundEvent(event,"fall","fall",0,victim);
				return Plugin_Continue;
			}
		...
		else if (GetGameType() == cstrike)
		{
			...
			if (headshot == 1)
			{
				runSoundEvent(event,"kill","headshot",attacker,victim);
				return Plugin_Continue;
			}
		...
		runSoundEvent(event,"kill",wepstring,attacker,victim);
...
public Action:Event_Build(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	runSoundEvent(event,"build",object,attacker,0);
	return Plugin_Continue;
}
...
ClientSoundBanned(client)
{
	// Check Client cookie
	new String:cookie[4];
	if(AreClientCookiesCached(client)){
		GetClientCookie(client, g_ssban_cookie, cookie, sizeof(cookie));
		if (StrEqual(cookie, "on"))
			return true;
		if(StrEqual(cookie, "off"))
			return false;
	...
	// Set cookie if client connects the first time
	SetClientCookie(client, g_ssban_cookie, "off");
	return false;
}
//#######################
	...
	if(GetConVarBool(cvarspecificjoinexit)){
		...
		if (KvJumpToKey(listfile, auth)){
			...
			if (filelocation[0] != '\0'){
				...
				SndCount[client] = 0;
				return;
			}else if (Submit_Sound(client,"", true)){
				SndCount[client] = 0;
				return;
			}
		}
...
//####### Submit Sound #######
bool:Submit_Sound(client,const String:name[], bool:joinsound=false)
{
	decl String:filelocation[PLATFORM_MAX_PATH+1];
	decl String:file[8] = "file";
	new count = KvGetNum(listfile, "count", 1);
	...
	if (filelocation[0] != '\0'){
		...
		if (karaoke[0] != '\0'){
			...
			Send_Sound(client, filelocation, name, joinsound);
		}
		return true;
	}
	return false;
...
public Action:Play_Sound_Timer(Handle:timer,Handle:pack){
	...
	if (soundLimit <= 0 || SndCount[client] < soundLimit){
		if (globalLastSound < thetime){
			...
			if (singleonly){
				if(ClientSoundOn(client) && IsValidClient(client)){
...
Play_Sound(const String:filelocation[], Float:volume)
{
	...
	//new playersconnected = GetMaxClients();
	for (new i = 1; i <= MaxClients; i++){
		if(IsValidClient(i) && ClientSoundOn(i) && HearSound(i)){
			clientlist[clientcount++] = i;

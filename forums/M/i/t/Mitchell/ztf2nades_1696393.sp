...
public OnMapStart()
{
    ...
    gSmokeSprite = 0; // PrecacheModel(MDL_SMOKE_SPRITE, true);
    gExplosionSprite = 0; // PrecacheModel(MDL_EXPLOSION_SPRITE, true);
    PrecacheNadeModels();
...
public Action:Command_Nade1(client, args) 
{
    ...
    if (GameType == tf2)
    {
        ...
        if (TF2_IsPlayerInCondition(client, TFCond_Cloaked) || TF2_IsPlayerInCondition(client, TFCond_Bonked) || TF2_IsPlayerInCondition(client, TFCond_Taunting))
            return Plugin_Handled;
    }
...
public Action:NadeCountdown(Handle:timer, any:userid)
{
    ...
    if (client > 0)
    {
        ...
        if (gHolding[client] != HoldNone)
        {
            ...
            if (flag & 2)
            {
                PrepareSound(SND_COUNTDOWN);
                EmitSoundToClient(client, SND_COUNTDOWN);
            }
        }
        else
        {
            if (flag & 2)
            {
                new nade = EntRefToEntIndex(gNade[client]);
                if (nade > 0)
                    EmitSoundToAll(SND_THROWNADE, nade);
                else
                    return Plugin_Stop;
            }
            else
...
{ 
    ...
    if (!special || type >= FragNade)
    {
            ...
            !IsEntLimitReached(.client=client, .message="unable to create explode particles"))
        {
            ...
            PrepareSound(SND_NADE_FRAG_TF2);
            EmitSoundToAll(SND_NADE_FRAG_TF2, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS,
                           SNDVOL_NORMAL, 100, _, center, NULL_VECTOR, false, 0.0);
        }
        else
        {
            PrepareModel(MDL_EXPLOSION_SPRITE, gExplosionSprite);
    ...
    else
    {
        switch (type)
        {
            ...
            case BearTrap:
            {
                ...
                EmitSoundToAll(SND_NADE_TRAP, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS,
                               SNDVOL_NORMAL, 100, _, center, NULL_VECTOR, false, 0.0);
            ...
            case NailNade:
            {
                ...
                if (entLimitOK)
                {
                    ...
                    if (ent > 0 && IsValidEntity(ent))
                    {
                        ...
                        EmitSoundToAll(SND_NADE_NAIL, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS,
                                       SNDVOL_NORMAL, 100, _, center, NULL_VECTOR, false, 0.0);
            ...
            case MirvNade:
            {
                ...
                if (GameType == tf2 && entLimitOK)
                {
                    ...
                    PrepareSound(SND_NADE_MIRV1_TF2);
                    EmitSoundToAll(SND_NADE_MIRV1_TF2, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS,
                                   SNDVOL_NORMAL, 100, _, center, NULL_VECTOR, false, 0.0);
                }
                else
                {
                    ...
                    EmitSoundToAll(SND_NADE_MIRV1, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS,
                                   SNDVOL_NORMAL, 100, _, center, NULL_VECTOR, false, 0.0);
                }
            ...
            case HealthNade:
            {
                ...
                EmitSoundToAll(SND_NADE_HEALTH, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS,
                               SNDVOL_NORMAL, 100, _, center, NULL_VECTOR, false, 0.0);
            ...
            case NapalmNade:
            {
                ...
                EmitSoundToAll(SND_NADE_NAPALM, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS,
                               SNDVOL_NORMAL, 100, _, center, NULL_VECTOR, false, 0.0);
            ...
            case HallucNade:
            {
                    ...
                    !IsEntLimitReached(.client=client, .message="unable to create hallucination particles"))
                {
                    ...
                    PrepareSound(SND_NADE_HALLUC_TF2);
                    EmitSoundToAll(SND_NADE_HALLUC_TF2, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS,
                                   SNDVOL_NORMAL, 100, _, center, NULL_VECTOR, false, 0.0);
                }
                else
                {
                    ...
                    EmitSoundToAll(SND_NADE_HALLUC, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS,
                                   SNDVOL_NORMAL, 100, _, center, NULL_VECTOR, false, 0.0);
                }
            ...
            case EmpNade:
            {
                ...
                EmitSoundToAll(SND_NADE_EMP, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS,
                               SNDVOL_NORMAL, 100, _, center, NULL_VECTOR, false, 0.0);
                ...
                if (GameType == tf2)
                {
                    for (new j=1;j<=MaxClients;j++)
                    {
                        if(PlayersInRange[j]>0.0)
                        {
                            ...
                            if(TF2_IsPlayerInCondition(j, TFCond_Cloaked))
                            {
                            ...
                            if (strncmp(tName, "tf_projectile_", 14) == 0)
                            {
                                if (GetEntProp(i, Prop_Data, "m_iTeamNum") == oteam)
                                {
                                    ...
                                    if (distance<radius)
                                    {
                                        ...
                                        PrepareSound("player/medic_charged_death.wav");
                                        PrepareSound("player/pl_impact_airblast2.wav");
                                        EmitSoundToAll("player/medic_charged_death.wav", i, _, _, SND_CHANGEPITCH, 1.0, 200);
                                        EmitSoundToAll("player/pl_impact_airblast2.wav", i, _, _, SND_CHANGEPITCH, 1.0, 120);
                            ...
                            else if (StrEqual(tName, "obj_sapper"))
                            {
                                if (GetEntProp(i, Prop_Data, "m_iTeamNum") == oteam)
                                {
                                    ...
                                    if (distance<radius)
                                    {
                                        ...
                                        PrepareSound("weapons/sapper_removed.wav");
                                        EmitSoundToAll("weapons/sapper_removed.wav", i, _, _, _, 1.0);
            ...
            case Bomblet:
            {
                    ...
                    !IsEntLimitReached(.client=client, .message="unable to create bomblet particles"))
                {
                    ...
                    PrepareSound(SND_NADE_MIRV2_TF2);
            ...
            case GasNade:
            {
                ...
                PrepareSound(SND_NADE_GAS);
                EmitSoundToAll(SND_NADE_GAS, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC, SND_NOFLAGS,
                               SNDVOL_NORMAL, 100, _, center, NULL_VECTOR, false, 0.0);
    ...
    Call_PushCell(team);
    Call_PushCell(special ? type : FragNade);
    Call_PushCell(nade);
    Call_PushArray(center, sizeof(center));
    Call_PushArray(PlayersInRange, sizeof(PlayersInRange));
    Call_PushCell(MaxClients+1); // sizeof(PlayersInRange));
    Call_Finish(res);
...
SetupNade(NadeType:type, team, bool:special)
{
    ...
    else
    {
        ...
        switch (type)
        {
            ...
            case GasNade:
            {
                PrepareModel(MDL_GAS, g_GasModelIndex);
            ...
            case TargetingDrone:
            {
                PrepareModel(MDL_DRONE, g_DroneModelIndex);
    ...
    if (ent > 0 && IsValidEdict(ent) && IsValidEntity(ent))
    {
        ...
        if (GetClientOfUserId(userid) == client)
        {
                ...
                !IsEntLimitReached(.client=client, .message="unable to create nail explosion particle"))
            {
                ...
                PrepareSound(SND_NADE_NAIL_EXPLODE_TF2);
                EmitSoundToAll(SND_NADE_NAIL_EXPLODE_TF2, 0, SNDCHAN_WEAPON, SNDLEVEL_TRAFFIC,
...
PrecacheNadeModels()
{
    ...
    g_SmokeModelIndex = 0; // PrecacheModel(MDL_SMOKE, true);
    g_TrapModelIndex = 0; // PrecacheModel(MDL_TRAP, true);
    g_EmpModelIndex = 0; // PrecacheModel(MDL_EMP, true);
    g_DroneModelIndex = 0; // PrecacheModel(MDL_DRONE, true);
    g_GasModelIndex = 0; // PrecacheModel(MDL_GAS, true);
...
KillPlayer(client, attacker, const String:weapon[])
{
...
bool:HurtPlayer(client, attacker, damage, const String:weapon[], bool:explosion, Float:pos[3], Float:knockbackmult = 4.0)
{
    if (GameType == tf2 && TF2_IsPlayerInCondition(client, TFCond_UberCharged))
    {
        return false;
    }
    ...
    if (health>damage)
    {
        ...
        PrepareSound(sndPain[rand]);
        EmitSoundToAll(sndPain[rand], client);
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
    ...
    // Register Forwards
    fwdOnNadeExplode=CreateGlobalForward("OnNadeExplode",ET_Hook,Param_Cell,Param_Cell,Param_Cell,
                                                        Param_Cell,Param_Array,Param_Array,Param_Cell);
    ...
    if (numParams >= 1)
    {
        ...
        if (GetNativeCell(2)) // setup - button pressed
        {
            ...
            else
            {
                ...
                {
                    // not while cloaked or taunting
                    if (TF2_IsPlayerInCondition(client, TFCond_Cloaked) || TF2_IsPlayerInCondition(client, TFCond_Taunting) || TF2_IsPlayerInCondition(client, TFCond_Bonked))
                    {
                        return;
                    }
                }

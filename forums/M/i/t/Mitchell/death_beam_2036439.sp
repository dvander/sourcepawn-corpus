...
Versions:
	...
	1.2
		* Removed the need of plugin offsets.
		* Removed Keyvalues and replaced with cookies!
*/
...
new Handle:c_BeamEnabled = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	c_BeamEnabled = 	RegClientCookie("BeamEnabled", 	"BeamEnabled", CookieAccess_Private);
...
public DeathBeamMenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		if(param2 == 2)
		{
			g_userPreference[param1] = 1;
		}
		else
		{
			g_userPreference[param1] = 0;
		}
		new String:tempString[2];
		Format(tempString, sizeof(tempString), "%i", g_userPreference[param1]);
		SetClientCookie(param1, c_BeamEnabled, tempString);
	}
}
...
public OnClientCookiesCached(client)
{
	...
	{
		new String:tempString[2]; //Big number, i know this is just incase people play for a year total.
		GetClientCookie(client, c_BeamEnabled, tempString, sizeof(tempString));
		g_userPreference[client]  = StringToInt(tempString);
	}
}
public OnClientDisconnect(client)
{
	if(client && !IsFakeClient(client))
	{
		...
		{
			new String:tempString[2];
			Format(tempString, sizeof(tempString), "%i", (g_userPreference[client]) ? 1 : 0);
			SetClientCookie(client, c_BeamEnabled, tempString);
		}
	}
}
/*

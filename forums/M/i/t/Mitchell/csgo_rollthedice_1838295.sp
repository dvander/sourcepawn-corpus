...
{
	name					= "Roll The Dice",
	author				= "ata-clan.de, dataviruset",
...
// ----------------------------------------------------------------------------
{
	...
	g_hAnnounceInterval		= CreateConVar("sm_rtd_announce_interval",		"120",				"interval (in seconds) for plugin announecment, set to 0 to disable", _, true, 0.0, true, 3600.0);
...
//	g_hBerserkerExtraGore	= CreateConVar("sm_rtd_berserker_extra_gore",	"0",					"amount of extra gore in berserker mode", _, true, 0.0, true, 10.0);
...
// ----------------------------------------------------------------------------
{
	...
	SetEntityModel(nClient, MODEL_CHICKEN);
#endif
...
// ----------------------------------------------------------------------------
{
	...
	if(g_pnClientFlags[nClientAttacker] & CLIENTFLAG_BERSERKER)
	{
		...
		if(StrEqual(sWeapon, "knife"))
		{
			...
			if (overflow[nClientVictim] < GetGameTime() - 0.1)
			{
				overflow[nClientVictim] = GetGameTime();
				
				for (new i = 0; i < 3; i++)
				{
					ChanceParticle(nClientVictim, nDamage, "blood_impact_red_01_backspray");
					ChanceParticle(nClientVictim, nDamage, "blood_impact_drops1");
					ChanceParticle(nClientVictim, nDamage, "blood_impact_red_01_drops");
				}

				ChanceParticle(nClientVictim, nDamage, "blood_impact_red_01_goop_c");
				ChanceParticle(nClientVictim, nDamage, "blood_impact_goop_medium");
				ChanceParticle(nClientVictim, nDamage, "blood_impact_red_01_goop_b");
				ChanceParticle(nClientVictim, nDamage, "blood_impact_red_01_goop_a");
				ChanceParticle(nClientVictim, nDamage, "blood_impact_medium");
				ChanceParticle(nClientVictim, nDamage, "blood_impact_basic");
				
				if (health > 0)
					GoreDecal(nClientVictim, (nDamage / 20));
				else
					GoreDecal(nClientVictim, 30);
			}

...
public void OnPluginStart()
{
	...
	hMaxSurvivors	= CreateConVar("l4d_multislots_max_survivors", "4", "Kick AI Survivor bots if numbers of survivors has reached the certain value. (does not kick real player)", CVAR_FLAGS, true, 4.0, true, 32.0);
...
public Action AddBot(int client, int args)
{
	...
	if(SpawnFakeClient() == true)
		PrintToChat(client,"A surviving Bot was added.");
	else
		PrintToChat(client,"Impossible to generate a bot at the moment.");
...
public Action JoinTeam_ColdDown(Handle timer, int userid)
{
	...
	if(client && IsClientInGame(client))
	{
		...
		{	
			...
			{
				PrintHintText(client, "You are already on the team of survivors.");
			}
			else if((DispatchKeyValue(client, "classname", "info_survivor_position") == true) && !IsAlive(client))
			{
				PrintHintText(client, "Please wait to be resurrected or rescued.");
			}
		}
		else if(IsClientIdle(client))
		{
			PrintHintText(client, "You are idle. press the left mouse button to join the survivors!");
		}
		else
		{			
			if(TotalAliveFreeBots() == 0)
			{
				if(bKill && iDeadBotTime > 0) 
				{
					if(bFinalMap) //don't let player die in saferoom after final starts, this prevents some issue in final map
					{
						...
						{
							PrintHintText(client,"Impossible to generate a bot at the moment.");
						}
					}
					else
					{
...
public Action PluginStart(Handle timer)
{
	...
	if(SpecCheckTimer == null) SpecCheckTimer = CreateTimer(15.0, Timer_SpecCheck, _, TIMER_REPEAT)	;
}
		...
		if(IsClientInGame(i))
		{
			if((GetClientTeam(i) == TEAM_SPECTATORS) && !IsFakeClient(i))
			{
				if(!IsClientIdle(i))
				{
					...
					PrintToChat(i, "\x01[\x04MzBR\x01] %s, Type in chat \x03!join\x01 To join the survivors", PlayerName);
				}
			}
		}
	}	
	for (int i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i))		
		{
			if((GetClientTeam(i) == TEAM_SURVIVORS) && !IsFakeClient(i) && !IsAlive(i))
			{
				char PlayerName[100];
				GetClientName(i, PlayerName, sizeof(PlayerName));
				PrintToChat(i, "\x01[\x04MzBR\x01] %s, Please wait to be resurrected or rescued", PlayerName);
			}
		}
	}	
	return Plugin_Continue;
...
public Action Timer_KillSurvivor(Handle timer, int client)
{
	if(client && IsClientInGame(client) && GetClientTeam(client) == 2 && IsPlayerAlive(client))
	{
		...
		PrintHintText(client, "The survivor starts the game during the Start of the game, wait to be resurrected or rescued");
	}
}
...
public Action Timer_TakeOverBotAndDie(Handle timer, int userid)
{
	...
	if (fakebot == 0)
	{
		PrintHintText(client, "No Bots for replacement.");
		return;
	}

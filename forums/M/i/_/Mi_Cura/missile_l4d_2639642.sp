...
public OnPluginStart()
{
	...
	l4d_missile_damage = CreateConVar("l4d_missile_damage", "800.0", "damage");
	l4d_missile_damage_tosurvivor = CreateConVar("l4d_missile_damage_tosurvivor", "100.0", "damage");
	l4d_missile_push = CreateConVar("l4d_missile_push", "1200", "push forece");
	
	l4d_missile_safe = CreateConVar("l4d_missile_safe", "1", "0:normal , 1:less chance to hurt survivor");

	l4d_missile_infected_smoker = CreateConVar("l4d_missile_infected_smoker", "15.0", "launch missile when smoker drag[0.0, 30.0]%");
	l4d_missile_infected_charger = CreateConVar("l4d_missile_infected_charger", "15.0", "launch missile when charger charge[0.0, 30.0]%");
	l4d_missile_infected_spitter = CreateConVar("l4d_missile_infected_spitter", "15.0", "spitter [0.0, 30.0]%");
	l4d_missile_infected_witch = CreateConVar("l4d_missile_infected_witch", "15.0", "witch harasser[0.0, 30.0]%");
 	l4d_missile_infected_tank_throw = CreateConVar("l4d_missile_infected_tank_throw", "20.0", "[0.0, 30.0]%");	
	l4d_missile_infected_anti = CreateConVar("l4d_missile_infected_anti", "20.0", "infected launch missile when survivor launch missile [0.0, 30.0]%");	
	
	l4d_missile_weapon_rifle = CreateConVar("l4d_missile_weapon_rifle", "1", "enable or disable for rifle {0, 1}");
	l4d_missile_weapon_sniper = CreateConVar("l4d_missile_weapon_sniper", "1", "enable or disable for  sniper {0, 1}");
	l4d_missile_weapon_shotgun = CreateConVar("l4d_missile_weapon_shotgun", "1", "enable or disable for shotgun {0, 1}");
	l4d_missile_weapon_magnum  = CreateConVar("l4d_missile_weapon_magnum", "1", "enable or disable for magnum {0, 1}");	
	l4d_missile_weapon_smg  = CreateConVar("l4d_missile_weapon_smg", "0", "enable or disable for smg {0, 1}");	
	l4d_missile_weapon_pistol  = CreateConVar("l4d_missile_weapon_pistol", "0", "enable or disable for pistol {0, 1}");	

	
	l4d_missile_limit = CreateConVar("l4d_missile_limit", "30", "missile limit");
 	l4d_missile_kills = CreateConVar("l4d_missile_kills", "10", "how many kills reward a missile");	
 	l4d_missile_tracefactor = CreateConVar("l4d_missile_tracefactor", "1.5", "trace factor do not need change[0.5, 3.0]");		
 	l4d_missile_radar_range = CreateConVar("l4d_missile_radar_range", "1500.0", "radar scan range, missile do not lock on target if out of range [500.0, -]");	
...
{
	PrintToChat(client, "[MISS] A cada %d zombies mortos voce ganha 1 missil.", GetConVarInt(l4d_missile_kills));
	PrintToChat(client, "[MISS] Miss1 = Misseis Teleguiados, Press CTRL+E+FIRE");
	PrintToChat(client, "[MISS] Miss2 = Misseis Teleguiados. Se abaixe, segure a tecla USAR e ATIRE");
}
...
UpGrade(x, kill)
{
	...
	if (v>0) 
	{
		PrintCenterText(x, "Infectado morto: %d", totalkillcount[x]);
		if(MissileCount[x]>GetConVarInt(l4d_missile_limit))MissileCount[x]=GetConVarInt(l4d_missile_limit);
		PrintHintText(x, "+ Missil %d", MissileCount[x]);
		if(ShowMsg[x]<3)
		{
			PrintToChat(x, "Digite !miss no chat para aprender a atirar misseis", MissileCount[x]);
			ShowMsg[x]++;
		}
...
{  
	...
	{
		PrintHintText(client, "Voce esta sem misseis");
	} 
}
...
bool:LaunchMissile(client, Float:force, type=MissileTrace, bool:up=false, Float:offset)
{
	...
	if(dis<150.0)
	{
		PrintHintText(client, "sem espaco para atirar misseis");
		return false;
	}
	...
	PrintHintText(client, "Misseis %d", MissileCount[client]);
	if(GetClientTeam(client)==3)PrintToChatAll("\x04%N \x03atirou um missil", client);
	return true;
}
...
TraceMissile(client, Float:time,  Float:duration)
{
	...
	if(time-MissileScanTime[client]>0.3)
	{
		...
		//PrintToChatAll("%N procurando %f inimigo %d", client, time, enemy);
	} 
	else
	{
		if(enemy>0)
	...
	if(enemy>0)	
	{
		...
		if(show)
		{
			...
			{
				if(client > 0 && IsClientInGame(client) && IsPlayerAlive(client))	PrintHintText(enemy, "Voce esta trancado by %N's missile, Distance: %d", client, RoundFloat(disenemy) );
				else PrintHintText(enemy, "Cuidado! sera abtido por um missisl inimigo, Distancia: %d", RoundFloat(disenemy) );
				EmitSoundToClient(enemy, SOUNDMISSILELOCK);
			}
			if(client > 0 && IsClientInGame(client) && IsPlayerAlive(client))
			{
				if(enemy>0 && IsClientInGame(enemy) && IsPlayerAlive(enemy))PrintCenterText(client, "Missil Travado On: %N, Distance: %d", enemy, RoundFloat(disenemy));
				else PrintCenterText(client, "Missil Travado On: enemy, Distance: %d", RoundFloat(disenemy));
			}
		} 
	...
	else if(enemy<0)
	{
		...
		if(show)
		{
			if(client > 0 && IsClientInGame(client) && IsPlayerAlive(client))
			{
				if(enemy>0 && IsClientInGame(enemy) && IsPlayerAlive(enemy))PrintCenterText(client, "Missil Travado On %N's Missile, Distance: %d", enemy, RoundFloat(disenemy));
				else PrintCenterText(client, "Missil bloqueado pelo missil do inimigo, Distance: %d", RoundFloat(disenemy));
			}
			//if(IsClientInGame(enemy) && IsPlayerAlive(enemy))PrintHintText(enemy, "Seu missil bloqueado por %N's missile, Distance: %d", client, RoundFloat(disenemy));
		}
		 
		//disexploded=modeloffset;
		enemyismissile=true;
		//PrintToChatAll("olhe a distancia do missil  %f", distance);
 
	}
	else if(show)
	{
		if(client>0 && IsClientInGame(client) && IsPlayerAlive(client))PrintCenterText(client, "Missil nao tem alvo");
	}
	if(enemy==0 && myteam==2 )
	{
...
MissileHitMissileMsg(client, enemy, bool:hit=true)
{
	if(hit)
	{
		...
		{
			if(client> 0 && IsClientInGame(client))PrintHintText(enemy, "Seu missil foi intercepitado por %N", client);
			else PrintHintText(enemy, "Seu missil foi interceptado pelo");
		}
		if(client>0 && IsClientInGame(client))
		{
			if(enemy>0 && IsClientInGame(enemy) && IsPlayerAlive(enemy))PrintHintText(client, "voce interceptou um %N's missile", enemy);
			else PrintHintText(client, "voce interceptou um missil inimigo");
		}
	}
	else
	{
		if(client> 0 && IsClientInGame(client))PrintHintText(client, "Seu missil bateu em um obstaculo");
	}
}
MissileHitPlayerMsg(client, enemy, bool:hit)
{
	if(hit)
	{
		if(enemy>0 && IsClientInGame(enemy) )
		{
			if(client> 0 && IsClientInGame(client))PrintHintText(enemy, "Voce foi abatido por um %N's missile", client);
			else PrintHintText(enemy, "Voce foi abatido por um missil inimigo");
		}
		if(client>0 && IsClientInGame(client))
		{
			if(enemy>0 && IsClientInGame(enemy))PrintHintText(client, "Seu missil abateu um %N", enemy);
			else PrintHintText(client, "Seu missil bateu em um obstaculo");
		}
	}
	else
	{
...
MissileHit(client, num=2)
{
	...
	//if(IsEntityMissile(MissileEntity[client]))
	{
		...
		if(MissileType[client]!=MissileTrace &&  IsClientInGame(client) && IsPlayerAlive(client))PrintCenterText(client, "Missil explodiu");
	}
	//UnHook(userid);
}

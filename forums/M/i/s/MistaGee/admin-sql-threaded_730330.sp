...
{
	name = "SQL Admins (Threaded, MultiServ)",
	author = "Svedrin feat. AlliedModders LLC",
	description = "Reads admins from SQL dynamically, taking the Server Address into account",
...
FetchGroups(Handle:db, sequence)
{
	...
	Format(
		query, sizeof(query),
		"SELECT sg.flags, sg.name, sg.immunity_level FROM sm_groups sg INNER JOIN sm_servers_groups ssg on ssg.group_id = sg.id INNER JOIN sm_servers ss on ssg.server_id = ss.id WHERE ss.ip='%d.%d.%d.%d' AND ss.port=%d ",
		pieces[0], pieces[1], pieces[2], pieces[3], ServerPort
		);

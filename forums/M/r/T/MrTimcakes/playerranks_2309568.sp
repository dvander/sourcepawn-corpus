...
#define PLUGIN_VERSION 				"3.3.1" 						   		// Plugin version, this value is fed into myinfo and sm_rankingversion CVAR.
#define PLUGIN_PREFIX_PLAIN 		"[Player Ranks 3.3.1]"		   		// Our console friendly prefix.
...
#define SQL_GETTOPPLAYERS			"SELECT * FROM `players` ORDER BY points / (playtime / 3600) DESC LIMIT %i;"
...
enum _:ScoreTracker {
	...
	Float:ScorePerHour,		// Points / Total Time
...
enum _:ScoreStruct {
	...
	Float:tScorePerHour						// Score / total time
}
...
public OnUpdaterIntervalChanged(Handle:cvar, const String:oldVal[], const String:newVal[]) {
	...
	updateTimer = CreateTimer(GetConVarFloat(updateInterval), Updater, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
}
// Expiration
...
public Action:Command_ForceSave(client, args) {
	ForceSave();
	CReplyToCommand(client, "%s %T", PLUGIN_PREFIX, "pr_commandsave", LANG_SERVER);
}
...
public Action:Command_Reward(client, args) {
	if (args < 2) {
		CReplyToCommand(client, "%s Usage: pr_reward <#userid|name> <amount>.", PLUGIN_PREFIX);
		return Plugin_Handled;
	}
	...
	if (pointAmount <= 0.0) {
		CReplyToCommand(client, "%s %T", PLUGIN_PREFIX, "pr_commandpointamountinvalid", LANG_SERVER);
		return Plugin_Handled;
	}
	...
	for (new targetIndex = 0; targetIndex < targetCount; targetIndex++) {
		...
		if (client != WORLD) {
			CPrintToChat(clientID, "%s %T", PLUGIN_PREFIX, "pr_commandrewardreceive", LANG_SERVER, issuerName, pointAmount);
		}
		else
			CPrintToChat(clientID, "%s %T", PLUGIN_PREFIX, "pr_commandrewardreceiveanonymous", LANG_SERVER, pointAmount);
	}
...
public Action:Command_Revoke(client, args) {
	...
	for (new targetIndex = 0; targetIndex < targetCount; targetIndex++) {
		...
		if (client != WORLD)
			CPrintToChat(clientID, "%s %T", PLUGIN_PREFIX, "pr_commandrevokereceive", LANG_SERVER, issuerName, pointAmount);
		else
			CPrintToChat(clientID, "%s %T", PLUGIN_PREFIX, "pr_commandrevokereceiveanonymous", LANG_SERVER, pointAmount);
	}
...
public Action:Command_Say(client, const String:command[], argc) {
	...
	if (StrEqual(text[startPos], "rank", false) == true || StrEqual(text[startPos], "ranks", false) == true) {
		...
		{
			CReplyToCommand(client, "%s %T", PLUGIN_PREFIX, "pr_rankmenu_disabled", LANG_SERVER);
			return Plugin_Handled;
		}
...
public ShowRankMenu(client) {
	...
	Format(rankMenuHeader, sizeof(rankMenuHeader), "%T", "pr_list_select_option", LANG_SERVER);
	SetMenuTitle(rankMenu, rankMenuHeader);
		
	decl String:topPlayersLabel[32];
	Format(topPlayersLabel, sizeof(topPlayersLabel), "%T", "pr_list_button_show_top_players", LANG_SERVER);
	...
	for (new clientId = 1; clientId <= MaxClients; clientId++) {
		...
		if (IsFakeClient(clientId) == false)
			Format(menuItem, sizeof(menuItem), "%T", "pr_list_player", LANG_SERVER, clientName, scores[clientId][points]);
		else if (GetConVarBool(menuFilterBots) == false && IsClientReplay(clientId) == false) {
			Format(menuItem, sizeof(menuItem), "%T %T", "pr_bot_tag", LANG_SERVER, "pr_list_player", LANG_SERVER, clientName, scores[clientId][points]);
		}
		else
			continue;
...
public ShowTopMenu(client) {
	...
	Format(topLabel, sizeof(topLabel), "%T", "pr_list_top_players_label", LANG_SERVER);
	...
	for (new rankNumber = 0; rankNumber < MAX_TOP_SCORES; rankNumber++) {
		...
		if (topScores[rankNumber][tPoints] == 0.00)
		{
			...
			if (rankNumber == 0) {
				Format(menuItem, sizeof(menuItem), "%T", "pr_list_zero_top_players", LANG_SERVER);
				AddMenuItem(topTenMenu, rankNumberString, menuItem);
			}
			break;
		}
		IntToString(rankNumber, rankNumberString, sizeof(rankNumberString));
		Format(menuItem, sizeof(menuItem), "%T", "pr_list_top_player", LANG_SERVER, rankNumber + 1, topScores[rankNumber][tNickname], topScores[rankNumber][tScorePerHour]);
		AddMenuItem(topTenMenu, rankNumberString, menuItem);
	}
...
public ShowRankMe(client) {
	...
	if (scores[client][rank] == 0) {
		if (scores[client][points] < GetConVarFloat(pluginThreshold)) {
			CPrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_rankme_belowthreshold", LANG_SERVER, scores[client][points], GetConVarFloat(pluginThreshold));
		}
		else {
			CPrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_rankme_updating", LANG_SERVER, scores[client][points], GetConVarFloat(updateInterval));		
		}
	}
	else {
		CPrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_rankme_normal", LANG_SERVER, scores[client][rank], totalCount, scores[client][points]);
	}
	...
	if (scores[client][kills] > 0.00 && scores[client][deaths] > 0.00)
	{
		...
		CPrintToChat(client, "%s %T %T", PLUGIN_PREFIX, "pr_stats_1", LANG_SERVER, scores[client][kills], scores[client][deaths], scores[client][assists], "pr_stats_2", LANG_SERVER, kdRatio);
	}
	else
		CPrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_1", LANG_SERVER, scores[client][kills], scores[client][deaths], scores[client][assists]);
		
	if (isTF2)
	{
		CPrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_3", LANG_SERVER, scores[client][headshots], scores[client][backstabs], scores[client][feigns], scores[client][deflects], scores[client][highestStreak]);	
		
		if (scores[client][merKills] > 0)
			CPrintToChat(client, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", LANG_SERVER, scores[client][merKills], "Merasmus", "pr_stats_5", LANG_SERVER, scores[client][merLvl]);
		
		if (scores[client][monKills] > 0)
			CPrintToChat(client, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", LANG_SERVER, scores[client][monKills], "Monoculus", "pr_stats_5", LANG_SERVER, scores[client][monLvl]);
		
		if (scores[client][hhhKills] > 0)
			CPrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_4", LANG_SERVER, scores[client][hhhKills], "Headless Horsemann");
	}
	
	CPrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_6", LANG_SERVER, GetTimePlayed(client) / 3600);	
	CPrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_7", LANG_SERVER, scores[client][flagCaptures], scores[client][flagDefends], scores[client][capCaptures], scores[client][capDefends], scores[client][roundsPlayed]);
	CPrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_8", LANG_SERVER, scores[client][dominationsGood], scores[client][dominationsBad]);
}
			...
			if (IsClientInGame(clientId)) {
				...
				if (IsFakeClient(clientId) == false) {
					if (scores[clientId][rank] == 0)
						CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_rank_unranked", LANG_SERVER, clientName, scores[clientId][roundPoints], scores[clientId][ScorePerHour]);
					else {
						CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_rank_normal", LANG_SERVER, clientName, scores[clientId][rank], scores[clientId][roundPoints], scores[clientId][ScorePerHour]);
					}
				}
				else {
					CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_rank_bot", LANG_SERVER, clientName, scores[clientId][roundPoints]);
				}
				
				if (scores[clientId][kills] > 0.00 && scores[clientId][deaths] > 0.00)
					CPrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_1", LANG_SERVER, scores[clientId][kills], scores[clientId][deaths], scores[clientId][assists], "pr_stats_2", LANG_SERVER, FloatDiv(float(scores[clientId][kills]), float(scores[clientId][deaths])));
				else
					CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_1", LANG_SERVER, scores[clientId][kills], scores[clientId][deaths], scores[clientId][assists]);
					
				if (isTF2) {
					CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_3", LANG_SERVER, scores[clientId][headshots], scores[clientId][backstabs], scores[clientId][feigns], scores[clientId][deflects], scores[clientId][highestStreak]);
					
					if (scores[clientId][merKills] > 0)
						CPrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", LANG_SERVER, scores[clientId][merKills], "Merasmus", "pr_stats_5", LANG_SERVER, scores[clientId][merLvl]);
					
					if (scores[clientId][monKills] > 0)
						CPrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", LANG_SERVER, scores[clientId][monKills], "Monoculus", "pr_stats_5", LANG_SERVER, scores[clientId][monLvl]);
					
					if (scores[clientId][hhhKills] > 0)
						CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_4", LANG_SERVER, scores[clientId][hhhKills], "Headless Horsemann");
					
					if (IsFakeClient(clientId) == false)
						CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_6", LANG_SERVER, GetTimePlayed(clientId) / 3600);
					
					CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_7", LANG_SERVER, scores[clientId][flagCaptures], scores[clientId][flagDefends], scores[clientId][capCaptures], scores[clientId][capDefends], scores[clientId][roundsPlayed]);
					CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_8", LANG_SERVER, scores[clientId][dominationsGood], scores[clientId][dominationsBad]);
					
				}
			}
			else {
				CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_invalid_selection", LANG_SERVER);
			}
		}
	}
	...
	if (action == MenuAction_Select)
	{
		...
		if (topScores[selectedIndex][tSeen] != 0) {
			for (new I = 1; I <= MaxClients; I++) {
				...
				GetClientAuthString(I, steamId, MAX_STEAMID_LENGTH, false);
			...
			if (onlineIndex == 0) {
				...
				CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_top_SPH_offline", LANG_SERVER, topScores[selectedIndex][tNickname], selectedIndex + 1, totalCount, topScores[selectedIndex][tScorePerHour], seenDate);
				//return;
			}
			else {
				CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_top_SPH_online",  LANG_SERVER, topScores[selectedIndex][tNickname], selectedIndex + 1, totalCount, topScores[selectedIndex][tScorePerHour]);
			}
			
			if (onlineIndex == 0) {
				new String:seenDate[30];
				FormatTime(seenDate, sizeof(seenDate), "%B %d, %Y", topScores[selectedIndex][tSeen]);
				CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_top_rank_offline", LANG_SERVER, topScores[selectedIndex][tNickname], selectedIndex + 1, totalCount, topScores[selectedIndex][tPoints], seenDate);
				//return;
			}
			else {
				CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_top_rank_online",  LANG_SERVER, topScores[selectedIndex][tNickname], selectedIndex + 1, totalCount, topScores[selectedIndex][tPoints]);
			}
			
			if (topScores[selectedIndex][tKills] > 0.00 && topScores[selectedIndex][tDeaths] > 0.00) {
				CPrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_1", LANG_SERVER, topScores[selectedIndex][tKills], topScores[selectedIndex][tDeaths], topScores[selectedIndex][tAssists], "pr_stats_2", LANG_SERVER, FloatDiv(float(topScores[selectedIndex][tKills]), float(topScores[selectedIndex][tDeaths])));
			}
			else {
				CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_1", LANG_SERVER, topScores[selectedIndex][tKills], topScores[selectedIndex][tDeaths], topScores[selectedIndex][tAssists]);
			}
			
			if (isTF2 == true) {
				CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_3", LANG_SERVER, topScores[selectedIndex][tHeadshots], topScores[selectedIndex][tBackstabs], topScores[selectedIndex][tFeigns], topScores[selectedIndex][tDeflects], topScores[selectedIndex][tHighestStreak]);
				
				if (topScores[selectedIndex][tmerKills] > 0)
					CPrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", LANG_SERVER, topScores[selectedIndex][tmerKills], "Merasmus", "pr_stats_5", LANG_SERVER, topScores[selectedIndex][tmerLvl]);
					
				if (topScores[selectedIndex][tmonKills] > 0)
					CPrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", LANG_SERVER, topScores[selectedIndex][tmonKills], "Monoculus", "pr_stats_5", LANG_SERVER, topScores[selectedIndex][tmonLvl]);
					
				if (topScores[selectedIndex][thhhKills] > 0)
					CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_4", LANG_SERVER, topScores[selectedIndex][thhhKills], "Headless Horsemann");
				
				if (onlineIndex != WORLD && IsClientInGame(onlineIndex) && scores[onlineIndex][loaded] == true) {
					CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_6", LANG_SERVER, GetTimePlayed(onlineIndex) / 3600);
				}
				else {
					CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_6", LANG_SERVER, topScores[selectedIndex][ttotaltime] / 3600);
				}
				
				CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_7", LANG_SERVER, topScores[selectedIndex][tflagCaptures], topScores[selectedIndex][tflagDefends], topScores[selectedIndex][tcapCaptures], topScores[selectedIndex][tcapDefends], topScores[selectedIndex][troundsPlayed]);
				CPrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_8", LANG_SERVER, topScores[selectedIndex][tdominationsGood], topScores[selectedIndex][tdominationsBad]);
			}
		}
	}
	else if (action == MenuAction_End) {
...
public Action:Updater(Handle:timer) {
	...
	for(new clientId = 1; clientId <= MaxClients; clientId++) {
		...
		{	
			...
			GetClientAuthString(clientId, playerSteamID, MAX_STEAMID_LENGTH);
...
public OnClientConnected(client) {
	...
	if (playerCount == GetConVarInt(playerMinimum)) {
		CPrintToChatAll("%s %T", PLUGIN_PREFIX, "pr_enabled_true", LANG_SERVER);
	}
}
...
public OnClientDisconnect(client) {
	...
	// We're just one player too short of ranking being enabled.
	if (playerCount == GetConVarInt(playerMinimum)) {
		CPrintToChatAll("%s %T", PLUGIN_PREFIX, "pr_enabled_false", LANG_SERVER);
	}
		...
		if (scores[clientId][roundBonus] == false) {				
			...
			if (scores[clientId][exempt] == false && playerCount >= GetConVarInt(playerMinimum) && GetConVarFloat(rewardRoundBonus) > 0.0) {
				if (GetConVarFloat(rewardRBMin) > 0.0)
					CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundbonus_notifyminimum", LANG_SERVER, GetConVarFloat(rewardRBMin));
				else	
					CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundbonus_notify", LANG_SERVER);
			}
		}
...
public Action:OnRoundSetupEnded(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	for (new clientId = 1; clientId <= MaxClients; clientId++) {
		...
		if (scores[clientId][roundBonus] == false) {
			...
			if (scores[clientId][exempt] == false && playerCount >= GetConVarInt(playerMinimum) && GetConVarFloat(rewardRoundBonus) > 0.0) {
				if (GetConVarFloat(rewardRBMin) > 0.0)
					CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundbonus_latenotifyminimum", LANG_SERVER, GetConVarFloat(rewardRBMin));
				else	
					CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundbonus_latenotify", LANG_SERVER);
			}
		}
	}
	...
	for(new clientId = 1; clientId <= MaxClients; clientId++) {
		...
		if (scores[clientId][roundBonus] == true) {
			...
			if (roundBonusReward > 0.0 && scores[clientId][roundPoints] >= GetConVarFloat(rewardRBMin)) {				
				...
				if (playerCount >= GetConVarInt(playerMinimum))
					CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundbonus_received", LANG_SERVER, roundBonusReward);
			}
		}
		
		if (scores[clientId][roundPoints] > 0.0)
			CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundend_roundpoints", LANG_SERVER, scores[clientId][roundPoints]);
		
		// The user isn't new to this ranking system.
		if (scores[clientId][rank] != 0) {
			CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundend_totalpoints", LANG_SERVER, scores[clientId][points]);
		}
...
public Action:OnPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast) {	
	...
	if (assisterId != WORLD) {
		...
		if (IsFakeClient(assisterId) == false && GetConVarInt(userNotificationLvl) >= NOTIFICATION_KILLS && pointRewardAssist > 0.0) {
			if (victimIsBot)
				CPrintToChat(assisterId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_assist_bot", attackerId, pointRewardAssist);
			else if (victimIsAdmin)
				CPrintToChat(assisterId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_assist_admin", attackerId, pointRewardAssist);
			else
				CPrintToChat(assisterId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_assist_player", attackerId, pointRewardAssist);
		}
	}
...
public Action:OnHeadlessHorsemannSpawn(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (IsValidEntity(bossIndex)) {
		...
		CPrintToChatAll("%s %T", PLUGIN_PREFIX, "pr_boss_spawned", LANG_SERVER, "Horseless Headless Horsemann", reward);
	}
	return Plugin_Continue;
}
...
public Action:OnMerasmusSpawn(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (IsValidEntity(bossIndex)) {
		...
		CPrintToChatAll("%s %T", PLUGIN_PREFIX, "pr_boss_spawned", LANG_SERVER, "Merasmus", calculatedReward);
	}
...
public Action:OnMonoculusSpawn(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (IsValidEntity(bossIndex)) {
		...
		CPrintToChatAll("%s %T", PLUGIN_PREFIX, "pr_boss_spawned", LANG_SERVER, "Monoculus", calculatedReward);
	}
	return Plugin_Continue;
}
	...
	if (bossIndex != -1 && IsValidEntity(bossIndex)) {
		...
		for (new clientid = 1; clientid <= MaxClients; clientid++) {
			if (IsClientInGame(clientid) == true && scores[clientid][exempt] == false && scores[clientid][bossdmg] >= GetConVarInt(rewardBossDmgMin)) {
				...
				if (reward > 0.0) {
					CPrintToChat(clientid, "%s %T", PLUGIN_PREFIX, "pr_boss_killed", LANG_SERVER, reward, "Horseless Headless Horsemann");
	...
	if (bossIndex != -1 && IsValidEntity(bossIndex)) {
		...
		for (new clientid = 1; clientid <= MaxClients; clientid++) {
			if (IsClientInGame(clientid) == true && scores[clientid][exempt] == false && scores[clientid][bossdmg] >= GetConVarInt(rewardBossDmgMin)) {
				...
				if (baseReward > 0.0) {
					CPrintToChat(clientid, "%s %T", PLUGIN_PREFIX, "pr_boss_killed", LANG_SERVER, calculatedReward, "Merasmus");
	...
	if (bossIndex != -1 && IsValidEntity(bossIndex)) {
		...
		for (new clientid = 1; clientid <= MaxClients; clientid++) {
			if (IsClientInGame(clientid) == true && scores[clientid][exempt] == false && scores[clientid][bossdmg] >= GetConVarInt(rewardBossDmgMin)) {
				...
				if (baseReward > 0.0) {
					Reward(clientid, calculatedReward);
					CPrintToChat(clientid, "%s %T", PLUGIN_PREFIX, "pr_boss_killed", LANG_SERVER, calculatedReward, "Monoculus");
				}
			}
			scores[clientid][bossdmg] = 0;
		}
...
FinishLateInit() {
	...
	if (playerCount > 0)
		CPrintToChatAll("%s %T", PLUGIN_PREFIX, "pr_loaded", LANG_SERVER);
}
...
PurgeClient(clientId, bool:resetOnly = false) {
	...
	scores[clientId][ScorePerHour] =  0.0;
...
public Query_LoadClient(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if (SQL_FetchRow(hndl) == true) 
	{
		...
		scores[clientId][ScorePerHour] = scores[clientId][points] / scores[clientId][totaltime];
...
public Query_FindClientRank(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if (isTF2 == true) {
		CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_stats_3", clientId, headshotsp, backstabsp, feignsp, deflectsp, killStreaksp);
		
		if (merKillsp > 0)
			CPrintToChat(clientId, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", clientId, merKillsp, "Merasmus", "pr_stats_5", clientId, merLvlp);
			
		if (monKillsp > 0)
			CPrintToChat(clientId, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", clientId, monKillsp, "Monoculus", "pr_stats_5", clientId, monLvlp);
			
		if (hhhKillsp > 0)
			CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_stats_4", clientId, hhhKillsp, "Headless Horsemann");
		
		CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_stats_6", clientId, totaltimep / 3600);
		
		CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_stats_7", clientId, flagCapturesp, flagDefendsp, capCapturesp, capDefendsp, roundsPlayedp);
		CPrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_stats_8", clientId, dominationsGoodp, dominationsBadp);
	}
	CloseHandle(data);
}
...
public Query_LoadRank(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if (announceConnect == true && GetConVarBool(suppressRankMsg) == false) {
		...
		CPrintToChatAll("%s %T", PLUGIN_PREFIX, "pr_msg_join", LANG_SERVER, clientName, scores[clientId][rank], totalCount);
...
public Query_TopPlayers(Handle:owner, Handle:hndl, const String:error[], any:unused) {
	...
	while (SQL_FetchRow(hndl))
	{
		...
		topScores[rankPosition][tScorePerHour] = topScores[rankPosition][tPoints] / topScores[rankPosition][ttotaltime];
...
public Query_ResetDatabase_S2(Handle:owner, Handle:hndl, const String:error[], any:issuerUid) {
	...
	isReady = true;
	CPrintToChatAll("%s %T", PLUGIN_PREFIX, "pr_reset", LANG_SERVER);

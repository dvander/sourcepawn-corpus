...
#define CHAT_PREFIX "[\x02RED\x01BLITZ]"
...
public Action Event_OnRoundEnd(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (winner <= 0 || !IsClientInGame(winner))
	{
		CPrintToChatAll("%s All players disconnect, nobody get his credits back, lol. Jackpot was {darkred}%d {default}credits!", CHAT_PREFIX, jackpot);
		return Plugin_Continue;
	}
	...
	if(winner == -1 || !IsClientInGame(winner))
		CPrintToChatAll("%s Winner has left the game but won {darkred}%d {default}credits.", CHAT_PREFIX, jackpot);
	else CPrintToChatAll("%s {darkred}%N {default}has won {darkred}%d {default}credits.", CHAT_PREFIX, winner, jackpot);
...
public Action Cmd_Raffle(int client, int args)
{
	...
	if(g_bUsed[client])
	{
		CPrintToChat(client, "%s You already spend {darkred}%d {default}credits this game. Current win chance: {darkred}%.2f%% {default}Jackpot: {darkred}%d {default}credits", CHAT_PREFIX, g_iSpend[client], float(g_iSpend[client])/float(GetArraySize(g_hJackpot))*100.0, GetArraySize(g_hJackpot));
		return Plugin_Handled;
	}
	
	if(args == 0)
	{
		CPrintToChat(client, "%s Jackpot: {darkred}%d {default}credits", CHAT_PREFIX, GetArraySize(g_hJackpot));
		return Plugin_Handled;
	}
	...
	{
		CPrintToChat(client, "%s You have to spend at least {darkred}%d {default}credits.", CHAT_PREFIX, MIN_CREDITS);
		return Plugin_Handled;
	}
	else if(credits > MAX_CREDITS)
	{
		CPrintToChat(client, "%s You can't spend that much credits (Max: {darkred}%d{default}).", CHAT_PREFIX, MAX_CREDITS);
		return Plugin_Handled;
	}
	...
	CPrintToChatAll("%s {darkred}%N {default}has spend {darkred}%d {default}credits, his current winning chance is: {darkred}%.2f%% {default}(Jackpot: {darkred}%d {default}credits)", CHAT_PREFIX, client, credits, float(credits)/float(GetArraySize(g_hJackpot))*100.0, GetArraySize(g_hJackpot));

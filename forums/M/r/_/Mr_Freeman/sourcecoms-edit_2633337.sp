...
public Action:CommandComms(client, args)
{
    if (!client)
    {
        CReplyToCommand(client, "%s{darkred}%t{default}", PREFIX, "CommandComms_na");
    ...
    if (g_MuteType[client] > bNot || g_GagType[client] > bNot)
        ...
        CReplyToCommand(client,  "%s{darkred}%t{default}", PREFIX, "CommandComms_nb");
        ...
        if (IsClientInGame(i) && IsClientAuthorized(i) && !IsFakeClient(i))
        {
            ...
            if (strcmp(clientAuth, sArg[2], false) == 0)
            {
                ...
                if (g_MuteType[i] == bNot && (type == 1 || type == 3))
                {
                    ...
                    CPrintToChat(i, "%s{darkred}%t{default}", PREFIX, "Muted on connect");
                    LogMessage("%s is muted from web", clientAuth);
                }
                if (g_GagType[i] == bNot && (type == 2 || type == 3))
                {
                    PerformGag(i, _, length / 60, _, _, _, _);
                    CPrintToChat(i, "%s{darkred}%t{default}", PREFIX, "Gagged on connect");
    ...
    for (new i = 1; i <= MaxClients; i++)
    {
        if (IsClientInGame(i) && IsClientAuthorized(i) && !IsFakeClient(i))
        {
            ...
            if (strcmp(clientAuth, sArg[0], false) == 0)
            {
                ...
                {
                    PerformUnGag(i);
                    CPrintToChat(i, "%s{darkred}%t{default}", PREFIX, "FWUngag");
    ...
    for (new i = 1; i <= MaxClients; i++)
    {
        if (IsClientInGame(i) && IsClientAuthorized(i) && !IsFakeClient(i))
        {
            ...
            if (strcmp(clientAuth, sArg[0], false) == 0)
            {
                ...
                {
                    PerformUnMute(i);
                    CPrintToChat(i, "%s{darkred}%t{default}", PREFIX, "FWUnmute");
...
public Query_UnBlockSelect(Handle:owner, Handle:hndl, const String:error[], any:data)
{
    ...
    if (DB_Conn_Lost(hndl) || error[0] != '\0')
    {
        ...
        {
            CPrintToChat(admin, "%s{darkred}%T{default}", PREFIX, "Unblock Select Failed", admin, targetAuth);
    ...
    if (!DB_Conn_Lost(hndl) && !SQL_GetRowCount(hndl))
    {
        if (admin && IsClientInGame(admin))
        {
            CPrintToChat(admin, "%s{darkred}%t{default}", PREFIX, "No blocks found", targetAuth);
            ...
            else
            {
                ...
                switch(cType)
                {
                    case TYPE_MUTE:
                    {
                        if (admin && IsClientInGame(admin))
                        {
                            CPrintToChat(admin, "%s{darkred}%t{default}", PREFIX, "No permission unmute", targetName);
                    ...
                    case TYPE_GAG:
                    {
                        if (admin && IsClientInGame(admin))
                        {
                            CPrintToChat(admin, "%s{darkred}%t{default}", PREFIX, "No permission ungag", targetName);
...
public Query_UnBlockUpdate(Handle:owner, Handle:hndl, const String:error[], any:data)
{
    ...
    if (DB_Conn_Lost(hndl) || error[0] != '\0')
    {
        ...
        if (admin && IsClientInGame(admin))
        {
            CPrintToChat(admin, "%s{darkred}%t{default}", PREFIX, "Unblock insert failed");
    ...
    switch(type)
    {
        case TYPE_MUTE:
        {
            ...
            if (admin && IsClientInGame(admin))
            {
                CPrintToChat(admin, "%s{darkred}%t{default}", PREFIX, "successfully unmuted", targetName);
        ...
        case TYPE_GAG:
        {
            ...
            if (admin && IsClientInGame(admin)){
                CPrintToChat(admin, "%s{darkred}%t{default}", PREFIX, "successfully ungagged", targetName);
            ...
            switch(type)
            {
                case TYPE_MUTE:
                {
                    if (g_MuteType[client] < bTime)
                    {
                        ...
                        CPrintToChat(client, "%s{darkred}%t{default}", PREFIX, "Muted on connect");
                    }
                }
                case TYPE_GAG:
                {
                    if (g_GagType[client] < bTime)
                    {
                        ...
                        CPrintToChat(client, "%s{darkred}%t{default}", PREFIX, "Gagged on connect");
                    }
                }
            }
        }
...
public Action:Timer_MuteExpire(Handle:timer, any:userid)
{
    ...
    CPrintToChat(client, "%s{darkred}%t{default}", PREFIX, "Mute expired");
...
public Action:Timer_GagExpire(Handle:timer, any:userid)
{
    ...
    CPrintToChat(client, "%s{darkred}%t{default}", PREFIX, "Gag expired");
...
stock CreateBlock(client, targetId = 0, length = -1, type, const String:sReason[] = "", const String:sArgs[] = "")
{
    ...
    else if (strlen(sArgs))
    {
        ...
        {
            CReplyToCommand(client, "%s{darkred}%t{default}", PREFIX, "no access");
            return;
        }
    }
    else
    ...
    for (new i = 0; i < target_count; i++)
    {
        ...
        if (!g_bPlayerStatus[target])
        {
            ...
            CReplyToCommand(client, "%s{darkred}%t{default}", PREFIX, "Player Comms Not Verified");
            skipped = true;
            continue; // skip
        }
        ...
        switch(type)
        {
            case TYPE_MUTE:
            {
                ...
                else
                {
                    ...
                    CReplyToCommand(client, "%s{darkred}%t{default}", PREFIX, "Player already muted", g_sName[target]);
            ...
            case TYPE_GAG:
            {
                ...
                else
                {
                    ...
                    CReplyToCommand(client, "%s{darkred}%t{default}", PREFIX, "Player already gagged", g_sName[target]);
            ...
            case TYPE_SILENCE:
            {
                ...
                else
                {
                    ...
                    CReplyToCommand(client, "%s{darkred}%t{default}", PREFIX, "Player already silenced", g_sName[target]);
    ...
    else
    {
        ...
        switch(type)
        {
            case TYPE_UNMUTE:
            {
                if (!BaseComm_IsClientMuted(target))
                {
                    CReplyToCommand(client, "%s{darkred}%t{default}", PREFIX, "Player not muted");
                    return;
                }
                else
                {
            ...
            case TYPE_UNGAG:
            {
                if (!BaseComm_IsClientGagged(target))
                {
                    CReplyToCommand(client, "%s{darkred}%t{default}", PREFIX, "Player not gagged");
                    return;
                }
                else
                {
            ...
            case TYPE_UNSILENCE:
            {
                ...
                {
                    CReplyToCommand(client, "%s{darkred}%t{default}", PREFIX, "Player not silenced");
                    return;
                }
                else
                {
...
stock bool:TempUnBlock(Handle:data)
{
    ...
    else
    {
        if (admin && IsClientInGame(admin))
        {
            CPrintToChat(admin, "%s{darkred}%t{default}", PREFIX, "No db error unlock perm");
...
bool:Bool_ValidMenuTarget(client, target)
{
    ...
    {
        if (client)
            CPrintToChat(client, "%s{darkred}%t{default}", PREFIX, "AdminMenu_Not_Available");
        else
            CReplyToCommand(client, "%s{darkred}%t{default}", PREFIX, "AdminMenu_Not_Available");
    ...
    {
        if (client)
            CPrintToChat(client, "%s{darkred}%t{default}", PREFIX, "Command_Target_Not_Targetable");
        else
            CReplyToCommand(client, "%s{darkred}%t{default}", PREFIX, "Command_Target_Not_Targetable");

...
new bool:g_Debug_IsInfectedHurtHooked;
...
public OnPluginStart()
{
	...
	if (g_bDebug)
	{
		...
		g_Debug_IsInfectedHurtHooked = true;
	}
}
...
public OnRCHSDebugChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	...
	{
		if (g_Debug_IsInfectedHurtHooked)
		{
			UnhookEvent("infected_hurt", Event_InfectedHurt);
			g_Debug_IsInfectedHurtHooked = false;
		}
	}
}
...
{
	if (!g_bEnabled || !isValidSurvivor(attacker)) return Plugin_Continue;
	...
	else if (g_fRiotCopHeadShot_BodyDivisor > 0.0)
	{
	...
	SDKHooks_TakeDamage(victim, 0, attacker, newDamage);
...
public Action:Fallen_TraceAttack(victim, &attacker, &inflictor, &Float:damage, &damagetype, &ammotype, hitbox, hitgroup)
{
	...
	if (g_bEnabled && isValidSurvivor(attacker) && hitgroup == 1 && g_fFallenHeadMultiplier > 1.0) 
	{

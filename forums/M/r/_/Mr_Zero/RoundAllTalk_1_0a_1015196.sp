...
SetAllTalk(bool:enabled)
{
	...
	{
		if(GetConVarBool(g_hUseSounds)){EmitSoundToAll(ALLTALKSND_ENABLE);}
		PrintToChatAll("%s %s",ALLTALKMSG_TAG,ALLTALKMSG_ENABLE);
	}
	
	// Else if all talk disables and is not already disabled
	else if(!enabled && GetConVarBool(g_hAllTalkCvar))
	{
		if(GetConVarBool(g_hUseSounds)){EmitSoundToAll(ALLTALKSND_DISABLE);}
		PrintToChatAll("%s %s",ALLTALKMSG_TAG,ALLTALKMSG_DISABLE);
	}

...
static          Handle: g_hBlockMedkitTime_Cvar;
static          Handle: g_hBlockMedkitTime_Ladder_Cvar;
static          Handle: g_hBlockUseTime_Cvar;
...
public OnPluginStart()
{
    ...
    g_hBlockMedkitTime_Cvar = CreateConVar(cvarName, "5.0", "How long the healing Survivor is prohibit from taking out their medkit after the receiving Survivor breaks free of the heal. 0 to disable prohibition", FCVAR_PLUGIN);

    Format(cvarName, sizeof(cvarName), "%s_time_ladder", PLUGIN_CVAR_PREFIX);
    g_hBlockMedkitTime_Ladder_Cvar = CreateConVar(cvarName, "15.0", "How long the healing Survivor is prohibit from taking out their medkit after trying to heal a fellow Survivor on a ladder. 0 to disable prohibition", FCVAR_PLUGIN);

    Format(cvarName, sizeof(cvarName), "%s_time_use", PLUGIN_CVAR_PREFIX);
    g_hBlockUseTime_Cvar = CreateConVar(cvarName, "5.0", "How long the reviving Survivor is prohibit from using their use button after the incapacitated Survivor breaks free of the reviving. 0 to disable Survivors being able to break free of a revive", FCVAR_PLUGIN);
...
public OnHealEnd_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
    ...
    {
        new subject = L4D2_GetPlayerUseActionTarget(client);
...
#endif
        {
            ...
            new Float:blockTime = GetConVarFloat(g_hBlockMedkitTime_Cvar);
            if (blockTime > 0.0)
            {
#if defined _vocalizetools_included
        ...
        {
            new Float:blockTime = GetConVarFloat(g_hBlockMedkitTime_Ladder_Cvar);
            if (blockTime > 0.0)
            {
#if defined _vocalizetools_included
                VocTools_MakePlayerVocalize(subject, "PlayerNo");
    ...
    {
        new owner = GetEntPropEnt(client, Prop_Send, "m_reviveOwner");
        ...
        new Float:blockTime = GetConVarFloat(g_hBlockUseTime_Cvar);
        if (blockTime > 0.0)
        {
...
#if defined _vocalizetools_included
public VocTools_OnPlayerVocalize_Post(client, const String:vocalize[], const String:rawVocalize[], bool:isSmartlook)
{
...
static GetPlayerMainWeapon(client)
{
    new weapon = GetPlayerWeaponSlot(client, _:L4DWeaponSlot_Primary);
    if (weapon <= 0 || !IsValidEdict(weapon))
    {
        weapon = GetPlayerWeaponSlot(client, _:L4DWeaponSlot_Secondary);
        if (weapon <= 0 || !IsValidEdict(weapon))
        {
            weapon = 0;
        }

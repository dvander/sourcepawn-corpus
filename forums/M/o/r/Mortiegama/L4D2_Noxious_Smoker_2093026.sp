...
new Handle:cvarTongueStripTimer[MAXPLAYERS + 1] = INVALID_HANDLE;
...
{
	CreateConVar("l4d_nsm_version", PLUGIN_VERSION, "Noxious Smoker Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_DONTRECORD|FCVAR_NOTIFY);

	// ======================================
	// Smoker Ability: Asphyxiation
	// ======================================
	cvarAsphyxiation = CreateConVar("l4d_nsm_asphyxiation", "1", "Enables Asphyxiation ability: The Smoker pulls out the oxygen from the air around it causing nearby Survivors to struggle to breathe. (Def 1)", FCVAR_PLUGIN);
	cvarAsphyxiationFrequency = CreateConVar("l4d_nsm_asphyxiationfrequency", "1.0", "Frequency in which a Survivor nearby the Smoker will be injured. (Def 1.0)", FCVAR_PLUGIN);
	cvarAsphyxiationDamage = CreateConVar("l4d_nsm_asphyxiationdamage", "5", "Amount of damage caused by Asphyxiation. (Def 5)", FCVAR_PLUGIN);
	cvarAsphyxiationRange = CreateConVar("l4d_nsm_asphyxiationrange", "300.0", "Range around the Smoker that can cause Asphyxiation of Survivors. (Def 300.0)", FCVAR_PLUGIN);

	// ======================================
	// Smoker Ability: Collapsed Lung
	// ======================================
	cvarCollapsedLung = CreateConVar("l4d_nsm_collapsedlung", "1", "Enables Collapsed Lung ability: The sheer crushing force of the Smoker's tongue causes the Survivors lungs to collapse. (Def 1)", FCVAR_PLUGIN);
	cvarCollapsedLungChance = CreateConVar("l4d_nsm_collapsedlungchance", "100", "Chance that a Survivor's lungs are collapsed. (100 = 100%). (Def 100)", FCVAR_PLUGIN);
	cvarCollapsedLungDuration = CreateConVar("l4d_nsm_collapsedlungduration", "5", "For how many seconds does the Collapsed Lung last. (Def 5)", FCVAR_PLUGIN);
	cvarCollapsedLungDamage = CreateConVar("l4d_nsm_collapsedlungdamage", "1", "How much damage is inflicted by Collapsed Lung each second. (Def 1)", FCVAR_PLUGIN);

	// ======================================
	// Smoker Ability: Methane Blast
	// ======================================	
	cvarMethaneBlast = CreateConVar("l4d_nsm_methaneblast", "1", "Enables Methane Blast ability: When the Smoker is killed, the pressurized gas inside its body ignites causing an explosion. (Def 1)", FCVAR_PLUGIN);
	cvarMethaneBlastInnerPower = CreateConVar("l4d_nsm_methaneblastinnerpower", "300.0", "Power behind the inner range of Methane Blast. (Def 300.0)", FCVAR_PLUGIN);
	cvarMethaneBlastOuterPower = CreateConVar("l4d_nsm_methaneblastouterpower", "100.0", "Power behind the outer range of Methane Blast. (Def 100.0)", FCVAR_PLUGIN);
	cvarMethaneBlastInnerDamage = CreateConVar("l4d_nsm_methaneblastinnerdamage", "15", "Amount of damage caused in the inner range of Methane Blast. (Def 15)", FCVAR_PLUGIN);
	cvarMethaneBlastOuterDamage = CreateConVar("l4d_nsm_methaneblastouterdamage", "5", "Amount of damage caused in the outer range of Methane Blast. (Def 5)", FCVAR_PLUGIN);
	cvarMethaneBlastInnerRange = CreateConVar("l4d_nsm_methaneblastinnerrange", "200.0", "Range the inner blast radius will extend from Methane Blast. (Def 200.0)", FCVAR_PLUGIN);
	cvarMethaneBlastOuterRange = CreateConVar("l4d_nsm_methaneblastouterrange", "300.0", "Range the outer blast radius will extend from Methane Blast. (Def 300.0)", FCVAR_PLUGIN);

	// ======================================
	// Boomer Ability: Methane Leak
	// ======================================
	cvarMethaneLeak = CreateConVar("l4d_nbm_methaneleak", "1", "Enables Methane Leak ability: With methane gas barely contained in his body, the Smoker lets out a methane cloud that causes damage to anyone standing inside. (Def 1)", FCVAR_PLUGIN);
	cvarMethaneLeakCooldown = CreateConVar("l4d_nbm_methaneleakcooldown", "60.0", "Period of time between methane leaks. (Def 60.0)", FCVAR_PLUGIN);
	cvarMethaneLeakDamage = CreateConVar("l4d_nbm_methaneleakdamage", "5", "Amount of damage caused to Survivors standing in a methane cloud. (Def 5)", FCVAR_PLUGIN);
	cvarMethaneLeakDuration = CreateConVar("l4d_nbm_methaneleakduration", "10.0", "Period of time the methane cloud persists. (Def 10.0)", FCVAR_PLUGIN);
	cvarMethaneLeakPeriod = CreateConVar("l4d_nbm_methaneleakperiod", "2.0", "Frequency that standing in the methane cloud will cause damage. (Def 2.0)", FCVAR_PLUGIN);
	cvarMethaneLeakRadius = CreateConVar("l4d_nbm_methaneleakradius", "100.0", "Radius that the methane cloud will cover. (Def 100.0)", FCVAR_PLUGIN);

	// ======================================
	// Boomer Ability: Methane Strike
	// ======================================
	cvarMethaneStrike = CreateConVar("l4d_nbm_methanestrike", "1", "Enables Methane Strike ability: When shoved, the Smoker lets out a strike of Methane gas that will also caused whoever shoved him to stumble. (Def 1)", FCVAR_PLUGIN);

	// ======================================
	// Smoker Ability: Moon Walk
	// ======================================
	cvarMoonWalk = CreateConVar("l4d_nsm_moonwalk", "1", "Enables Moon Walk ability: After wrapping up its victim, the Smoker is able to slowly move backwards, either dragging the survivor or stretching his tongue. (Def 1)", FCVAR_PLUGIN);
	cvarMoonWalkSpeed = CreateConVar("l4d_nsm_moonwalkspeed", "0.4", "How fast will the Smoker can move after a tongue grab and drag. (Def 0.4)", FCVAR_PLUGIN);
	cvarMoonWalkStretch = CreateConVar("l4d_nsm_moonwalkstretch", "2000", "How far the Smokers tongue can stretch before it snaps. (Def 2000)", FCVAR_PLUGIN);

	// ======================================
	// Smoker Ability: Restrained Hostage
	// ======================================
	cvarRestrainedHostage = CreateConVar("l4d_nsm_restrainedhostage", "1", "Enables Restrained Hostage ability: The Smoker will use a victim it is currently choking to shield itself from damage. (Def 1)", FCVAR_PLUGIN);
	cvarRestrainedHostageAmount = CreateConVar("l4d_nsm_restrainedhostageamount", "0.4", "Percent of damage the Smoker avoids using a Survivor as a Hostage. (Def 0.5)", FCVAR_PLUGIN);
	cvarRestrainedHostageDamage = CreateConVar("l4d_nsm_restrainedhostagedamage", "3", "How much damage is inflicted to the Survivor being used as a Hostage. (Def 3)", FCVAR_PLUGIN);

	// ======================================
	// Smoker Ability: Smoke Screen
	// ======================================
	cvarSmokeScreen = CreateConVar("l4d_nsm_smokescreen", "1", "Enables Smoke Screen ability: The Smokers continually pumps smoke around it's body which helps obscure his form from attacks. (Def 1)", FCVAR_PLUGIN);
	cvarSmokeScreenChance = CreateConVar("l4d_nsm_smokescreenchance", "20", "Chance that Smoke Screen will cause an attack to miss. (20 = 20%). (Def 20)", FCVAR_PLUGIN);

	// ======================================
	// Smoker Ability: Tongue Strip
	// ======================================
	cvarTongueStrip = CreateConVar("l4d_nsm_tonguestrip", "1", "Enables Tongue Strip Ability: As the slippery tongue coils around the Survivor, it has a chance to knock out whatever item they were holding last. (Def 1)", FCVAR_PLUGIN);
	cvarTongueStripChance = CreateConVar("l4d_nsm_tonguestripchance", "50", "Chance that the Smoker's tongue will strip an item. (50 = 50%). (Def 50)", FCVAR_PLUGIN);

	// ======================================
	// Smoker Ability: Tongue Whip
	// ======================================
	cvarTongueWhip = CreateConVar("l4d_nsm_tonguewhip", "1", "Enables Tongue Whip ability: When the Smoker's tongue is broken while dragging a victim, the force of the snap causes it to whip out and strike nearby survivors. (Def 1)", FCVAR_PLUGIN);
	cvarTongueWhipPower = CreateConVar("l4d_nsm_tonguewhippower", "100.0", "Power behind the Smoker's tongue whip. (Def 100.0)", FCVAR_PLUGIN);
	cvarTongueWhipDamage = CreateConVar("l4d_nsm_tonguewhipdamage", "10", "Amount of damage the Smoker's tongue will inflict when whipped. (Def 10)", FCVAR_PLUGIN);
	cvarTongueWhipRange = CreateConVar("l4d_nsm_tonguewhiprange", "500.0", "How far the Smoker will be able to whip its tongue. (Def 500.0)", FCVAR_PLUGIN);
	
	// ======================================
	// Smoker Ability: Void Pocket
	// ======================================
	cvarVoidPocket = CreateConVar("l4d_nsm_voidpocket", "1", "Enables Void Pocket ability: The Smoker sucks all the air around him to himself causing Survivors to fly towards him. (Press Reload) (Def 1)", FCVAR_PLUGIN);
	cvarVoidPocketCooldown = CreateConVar("l4d_nsm_voidpocketcooldown", "5.0", "Amount of time between Void Pocket abilities. (Def 5.0)", FCVAR_PLUGIN);
	cvarVoidPocketPower = CreateConVar("l4d_nsm_voidpocketpower", "200.0", "Power of the pull from the Void Pocket. (Def 200.0)", FCVAR_PLUGIN);
	cvarVoidPocketRange = CreateConVar("l4d_nsm_voidpocketrange", "200.0", "Range the Void Pocket will pull Survivors from.(Def 200.0)", FCVAR_PLUGIN);
...
public Action:SmokerAbility_CollapsedLung(victim, smoker)
{
	if (IsValidClient(victim) && GetClientTeam(victim) == 2)
	{
		...
		{
			PrintHintText(victim, "The Smoker has collapsed your lung!");
...
{	
	decl Float:vecPos[3];
	GetClientAbsOrigin(smoker, vecPos);
...
public Action:Timer_MethaneLeakCloud(Handle:timer, Handle:dataPack)
{
	...
	for (new victim=1; victim<=MaxClients; victim++)
	{
		if (IsValidClient(victim) && GetClientTeam(victim) == 2)
		{
			...
			PrintHintText(victim, "You're suffering from a Smoker's fart cloud!");
...
public Action:Timer_MoonWalk(Handle:timer, Handle:pack)
{
	...
	if (distance > MoonWalkStretch)
	{
		...
		PrintHintText(smoker, "The Tongue has been stretched/bent and snapped!");
	}
	...
	if (IsValidClient(victim) && IsValidSmoker(smoker) && GetClientTeam(victim) == 2)
	{
		...
		for (new target=1; target<=MaxClients; target++)
		{
			if (target != victim && IsValidClient(target) && GetClientTeam(target) == 2 && !IsSurvivorPinned(target))
			{
				...
				//PrintToChatAll("Distance: %f, %f.", distance1, distance2);
	...
	if (IsValidSmoker(smoker) && !IsPlayerGhost(smoker) && IsVoidPocketReady(smoker))
	{
		...
		if (IsValidClient(victim) && GetClientTeam(victim) == 2  && !IsSurvivorPinned(victim))
		{
			...
			if (distance < range)
			{
				...
				decl String:sPower[256];
				new magnitude;
				magnitude = GetConVarInt(cvarVoidPocketPower) * -1;

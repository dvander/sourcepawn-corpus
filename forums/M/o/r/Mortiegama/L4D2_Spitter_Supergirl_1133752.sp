...
{
	CreateConVar("l4d_ssg_version", PLUGIN_VERSION, "Spitter Supergirl Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_DONTRECORD|FCVAR_NOTIFY);

	// ======================================
	// Spitter Ability: Acidic Bile
	// ======================================
	cvarAcidicBile = CreateConVar("l4d_ssg_acidicbile", "1", "Enables Acidic Bile ability: Survivors that have wandered into an acid pool have a chance of being splashed with bile and attracting common infected. (Def 1)", FCVAR_PLUGIN);
	cvarAcidicBileChance = CreateConVar("l4d_ssg_acidicbilechance", "5", "Chance that the Survivor will be biled upon when standing in spit. (Def 5)", FCVAR_PLUGIN);

	// ======================================
	// Spitter Ability: Acidic Pool
	// ======================================
	cvarAcidicPool = CreateConVar("l4d_ssg_acidicpool", "1", "Enables Acidic Pool ability: Due to the unstable nature of the Spitter's body, periodically a pool of Spit will leak out beneath her feet. (Def 1)", FCVAR_PLUGIN);
	cvarAcidicPoolCooldown = CreateConVar("l4d_ssg_acidicpoolcooldown", "30.0", "Period of time between Acid Pool drops. (Def 30.0)", FCVAR_PLUGIN);

	// ======================================
	// Spitter Ability: Acidic Slobber
	// ======================================
	cvarAcidicSlobber = CreateConVar("l4d_ssg_acidicslobber", "1", "Enables Acidic Slobber ability: The Spitter is constantly shaking her head which will occasionally cause some of the drooling acid to land on nearby Survivors. (Def 1)", FCVAR_PLUGIN);
	cvarAcidicSlobberChance = CreateConVar("l4d_ssg_acidicslobberchance", "5", "Chance that a Survivor will be hit with Acid from the Spitter's slobber. (Def 5)(5 = 5%)", FCVAR_PLUGIN);
	cvarAcidicSlobberDamage = CreateConVar("l4d_ssg_acidicslobberdamage", "15", "Amount of damage the Acidic Slobber will cause to a Survivor. (Def 15)", FCVAR_PLUGIN);
	cvarAcidicSlobberRange = CreateConVar("l4d_ssg_acidicslobberrange", "500.0", "Distance the Acidic Slobber will travel. (Def 500.0)", FCVAR_PLUGIN);

	// ======================================
	// Spitter Ability: Acidic Splash
	// ======================================	
	cvarAcidicSplash = CreateConVar("l4d_ssg_acidicsplash", "1", "Enables Acid Splash ability: When a Spitter takes damage, the fresh wounds have a chance of splashing acid on any nearby Survivors. (Def 1)", FCVAR_PLUGIN);
	cvarAcidicSplashChance = CreateConVar("l4d_ssg_acidicsplashchance", "50", "Chance that a Survivor will be splashed by the Spitter's acid splash. (Def 50)(50 = 50%)", FCVAR_PLUGIN);
	cvarAcidicSplashDamage = CreateConVar("l4d_ssg_acidicsplashdamage", "6", "Amount of damage the Acidic Splash will cause to a Survivor. (Def 6)", FCVAR_PLUGIN);
	cvarAcidicSplashRange = CreateConVar("l4d_ssg_acidicsplashrange", "500.0", "Distance the Acidic Splash will travel from Spitter. (Def 500.0)", FCVAR_PLUGIN);
	
	// ======================================
	// Spitter Ability: Acid Swipe
	// ======================================
	cvarAcidSwipe = CreateConVar("l4d_ssg_acidswipe", "1", "Enables Acid Swipe ability: The Spitter uses her acid coated fingers to swipe at a Survivor, causing damage over time as the wound burns. (Def 1)", FCVAR_PLUGIN);
	cvarAcidSwipeChance = CreateConVar("l4d_ssg_acidswipechance", "100", "Chance that when a Spitter claws a Survivor they will take damage over time. (100 = 100%). (Def 100)", FCVAR_PLUGIN);
	cvarAcidSwipeDuration = CreateConVar("l4d_ssg_acidswipeduration", "10", "For how many seconds does the Acid Swipe last. (Def 10)", FCVAR_PLUGIN);
	cvarAcidSwipeDamage = CreateConVar("l4d_ssg_acidswipedamage", "1", "How much damage is inflicted by Acid Swipe each second. (Def 1)", FCVAR_PLUGIN);
	
	// ======================================
	// Spitter Ability: Hydra Strike
	// ======================================
	cvarHydraStrike = CreateConVar("l4d_ssg_hydrastrike", "1", "Enables Hydra Strike ability: Allows the Spitter to fire off a second spit rapidly after the first. (Def 1)", FCVAR_PLUGIN);
	cvarHydraStrikeCooldown = CreateConVar("l4d_ssg_hydrastrikecooldown", "0.0", "Additional recharge time before the Hydra Strike allows another spit. (Def 0.0)", FCVAR_PLUGIN);

	// ======================================
	// Spitter Ability: Sticky Goo
	// ======================================
	cvarStickyGoo = CreateConVar("l4d_ssg_stickygoo", "1", "Enables Sticky Goo ability: Any Survivor standing inside a pool of Spit will be stuck in the goo and find it harder to move out quickly. (Def 1)", FCVAR_PLUGIN);
	cvarStickyGooJump = CreateConVar("l4d_ssg_stickygoojump", "1", "Prevents the Survivor from jumping while speed is reduced. (Def 1)", FCVAR_PLUGIN);
	cvarStickyGooDuration = CreateConVar("l4d_ssg_stickygooduration", "3", "For how long after exiting the Sticky Goo will a Survivor be slowed. (Def 3)", FCVAR_PLUGIN);
	cvarStickyGooSpeed = CreateConVar("l4d_ssg_stickygoospeed", "0.5", "Speed reduction to Survivor caused by the Sticky Goo. (Def 0.5)", FCVAR_PLUGIN);

	// ======================================
	// Spitter Ability: Super Girl
	// ======================================
	cvarSupergirl = CreateConVar("l4d_ssg_supergirl", "1", "Enables Super Girl ability: After launching a spit, the Spitter is coated in a protective layer that slowly drips off and reduces all damage until it is gone. (Def 1)", FCVAR_PLUGIN);
	cvarSupergirlDuration = CreateConVar("l4d_ssg_supergirlduration", "4", "How long the Spitter is invulnerable. (Def 4)", FCVAR_PLUGIN);
	
	// ======================================
	// Spitter Ability: Super Girl Speed
	// ======================================
	cvarSupergirlSpeed = CreateConVar("l4d_ssg_supergirlspeed", "1", "Enables Super Girl Speed ability: Works with the Supergirl ability, the spit also coats the Spitters feet increasing movement speed for a brief period after launching a spit. (Def 1)", FCVAR_PLUGIN);
	cvarSupergirlSpeedDuration = CreateConVar("l4d_ssg_supergirlspeedduration", "4", "How long the Spitter is invulnerable. (Def 4)", FCVAR_PLUGIN);

...
{
	SDKHook(Client, SDKHook_OnTakeDamagePost, AttackCallback);
}
...
public AttackCallback(victim, attacker, inflictor, Float:damage, damagetype)
{
	//Only do anything if the plugin is active
	if (bEnabled)
	{
		//When the victim and attacker are the same or noone, nothing should happen.
		if(attacker == 0 || victim == 0 || attacker == victim || GetClientHealth(victim) <= 0)
			return;
	
		decl String:sWeapon[64];
		GetClientWeapon(attacker, sWeapon, sizeof(sWeapon));

		ExecEffect(victim, attacker, sWeapon);
	}
}

...
public OnPluginStart()
{
	...
	DarknessEnabled 				=	CreateConVar("sm_zf_dark", "1", "Darkens the map (0: Disable)");
	DarknessLevel					=	CreateConVar("sm_zf_dark_level", "b", "The darkness of the map,  a being the darkest,  z being extremely bright when sm_zf_dark is 1 (n: Default)");
	DarkSky 						=	CreateConVar("sm_zf_dark_sky", "sky_night_01", "The sky the map will have when sm_zf_dark is 1");
	...
	//EDIT: 2.1.1.2 - FOV:  Added to alter FOV for Zombies
...
public function_PlayerSpawn(client)
{
	...
	{
		//EDIT: 2.1.1.2 - FOV & Render: Added this line to alter their FOV to 135 and render to #24
...
public PrecacheFiles(){
	...
	decl String:overlay[64];
	GetConVarString(ZombieOverlayPath, overlay, sizeof(overlay));
	Format(overlay, sizeof(overlay), "materials/%s", overlay);

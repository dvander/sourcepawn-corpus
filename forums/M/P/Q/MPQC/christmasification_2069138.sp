...
public OnPluginStart()
{
	...
	else if (mod == G_DOD)
	{
		...
		HookEvent("dod_restart_round", RoundEnd);
...
GetMod()
{
	decl String: folder[PLATFORM_MAX_PATH];
	GetGameFolderName(folder, sizeof(folder));

	if (StrContains(folder, "cstrike", false) != -1)
	{
		return G_CSS;
	}
	else if (StrContains(folder, "tf", false) != -1)
	{
		return G_TF;
	}
	else if (StrContains(folder, "dod", false) != -1)
	{
		return G_DOD;
	}

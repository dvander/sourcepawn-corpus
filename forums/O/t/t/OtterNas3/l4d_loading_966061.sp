...
public Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetConVarInt(autoCycle) != 0 && StrContains(map, "02") != -1 && StrContains(map, "garage") != -1 && StrContains(gamemode, "versus", false) != -1) {
		if (roundStarted == 2) {
			roundCounter++;
...
public Action:StartTimer(Handle:timer)
{
	if (countDown == -1) {
		...
		if (countDown == 1) {
			if (isFirstRound && strlen(forceNext) == 0) {
				GetCurrentMap(map, sizeof(map));
				if (StrContains(map, "02") != -1 && StrContains(map, "garage") != -1 && GetConVarInt(autoCycle) == 2) {
					if (strlen(forceNext) != 0) PrintToChatAll("[DEBUG] Next campaign is set to '%s' already (vote cast or setnext used)", forceNext);
					else {
						strcopy(g_l4dVoteType, sizeof(g_l4dVoteType), "campaign");
						DisplayCampaignVote();

...
/* Held Buttons - Stores what buttons players are holding down */
new bool:pack_attack[MAXPLAYERS+1] = { false, ... };		//Primary Fire
new bool:pack_attack2[MAXPLAYERS+1] = { false, ... };		//Secondary Fire
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon) {
	...
	if(buttons & IN_ATTACK2) {
		pack_attack2[client] = true;
	} else {
		pack_attack2[client] = false;
	}
	if(buttons & IN_ATTACK2) {
		pack_attack2[client] = true;
	} else {
...
public Action:Event_WeaponDrop(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	#endif
	if(StrContains(item, "molotov", false) != -1) {
		if(!pack_attack[client]) {
			//Remove dropped grenade

...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_tf2npcs_pedobear", PedobearSpawn, ADMFLAG_ROOT);
	RegAdminCmd("sm_tf2npcs_pedobear_sprite", PedobearSpawn_SPR, ADMFLAG_ROOT);
	RegAdminCmd("tf_pedobear_spawn_xyz", PedobearSpawn_XYZ, ADMFLAG_ROOT);
...
public Action:PedobearSpawn_XYZ(client, args)
{
	if(client > 0)
	{
		if(!PedobearAttack)
		{
			...
			decl Float:pedobear_spawn_pos[3];
			pedobear_spawn_pos[0] = StringToFloat(argc);
			
			GetCmdArg(2, argc, sizeof(argc));
			pedobear_spawn_pos[1] = StringToFloat(argc);
			
			GetCmdArg(3, argc, sizeof(argc));
			pedobear_spawn_pos[2] = StringToFloat(argc);
			pedobear_spawn_pos[2] -= 10.0;
			TeleportEntity(entity, pedobear_spawn_pos, NULL_VECTOR, NULL_VECTOR);

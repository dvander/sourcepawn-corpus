...
{   
   ...
   H_LoserMoney = CreateConVar("sm_cashflow_loser", "50", "Money given to losing team", FCVAR_PLUGIN, TRUE, 0.0, TRUE, 16000.00);
   H_WinnerBase = CreateConVar("sm_cashflow_winner_base", "200", "Base money given to losing team after first win", FCVAR_PLUGIN, TRUE, 0.0, TRUE, 16000.00);
   H_WinnerPenalty = CreateConVar("sm_cashflow_winner_penalty", "50", "Money subtracted from base for each consecutive win", FCVAR_PLUGIN, TRUE, 0.0, TRUE, 5333.00); // rounded from 16000/penalties (3)
   H_ObjectiveMoney = CreateConVar("sm_cashflow_objective_penalty", "100", "Money subtracted from losers if objective was completed", FCVAR_PLUGIN, TRUE, 0.0, TRUE, 16000.00);

...
public void OnPluginStart()
{
	...
	hCvar_HealingFieldChance 	= CreateConVar("l4d_healing_field_chance", 		"50", 		"Chance that when the Tank dies a healing field is generated. 0 = Plugin OFF", FCVAR_NOTIFY, true, 0.0, true, 100.0);
	hCvar_HealingFieldRange 	= CreateConVar("l4d_healing_field_range", 		"200.0", 	"Sets the max range of the healing field.", FCVAR_NOTIFY, true, 50.0, true, 500.0);
	hCvar_EntityMaxLife			= CreateConVar("l4d_healing_field_life",		"30.0",		"Sets the duration time of the healing field (Seconds).", FCVAR_NOTIFY, true, 5.0, true, 60.0);
	hCvar_AmountHealth 			= CreateConVar("l4d_healing_field_health", 		"6", 		"Sets the amount of health survivors receive per second.", FCVAR_NOTIFY, true, 1.0, true, 10.0);
	hCvar_ColorEffects 			= CreateConVar("l4d_healing_field_colors", 		"1", 		"Sets the colors of the healing field.\n1 = Random colors.\n2 = Default color.\nCustom color, example: \"0 255 255 255\"",FCVAR_NOTIFY);
	hCvar_GlowEnabled 			= CreateConVar("l4d_healing_field_glow", 		"1", 		"Enables/Disables the glowing on entities. 1 = Glow ON. 0 = Glow OFF.", FCVAR_NOTIFY, true, 0.0, true, 1.0);	
...
public Action CmdSpawnerHealing( int client, int args )
{
	if( iCvar_HealingFieldChance != 0 )
		...
		PrintToChat( client, "\x04[\x03ღ\x01Healing Field\x03ღ\x04] \x01Plugin Is \x04Disabled" );
...
void CreateHealingField( int client )
{
	...
	if( entity != -1 )
	{
		...
		PrintToChatAll( "\x04[\x03ღ\x01%s\x03ღ\x04]\x01 Created in the coordinates: \x03%.2f \x04%.2f \x05%.2f", sNameArray[RandomModel], vOrigin[0], vOrigin[1], vOrigin[2] );
	}	
}
...
//	Format( sColor, sizeof( sColor ), "%i %i %i 255", GetRandomInt( 0, 255 ), GetRandomInt( 0, 255 ), GetRandomInt( 0, 255 ) ); // Works but the beacon colors don't always match the color of the light entity.
	...
	PrintToServer("                                                                      ");
	PrintToServer("                      --> Healing Field <--                           ");
	PrintToServer("Color settings are incorrect, check the settings, changing to white...");
	PrintToServer("                                                                      ");

...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	OnMusic=CreateGlobalForward("VSH_OnMusic",ET_Hook,Param_String);
...
stock bool:CheckToChangeMapDoors()
{
	decl String:s[PLATFORM_MAX_PATH];
	GetCurrentMap(currentmap, sizeof(currentmap));
	BuildPath(Path_SM, s, PLATFORM_MAX_PATH, "configs/saxton_hale_doors.cfg");
	if (!FileExists(s))
	{
		if
		(
			strncmp(currentmap, "vsh_partyisland", 15, false) == 0
			|| strncmp(currentmap, "arena_nucleus", 13, false) == 0
			|| strncmp(currentmap, "arena_grid_walls", 16, false) == 0
		)
			checkdoors = false;
		else checkdoors = true;
		return;
	}
	new Handle:fileh = OpenFile(s, "r");
	if (fileh == INVALID_HANDLE)
...
public Action:Timer_MusicPlay(Handle:timer)
{
	...
	if (sound[0] != '\0')
	{
		...
		Call_PushStringEx(sound2,PLATFORM_MAX_PATH,SM_PARAM_STRING_UTF8,SM_PARAM_COPYBACK);	
		Call_Finish(act);
	...
	if (time != -1.0)
		MusicTimer = CreateTimer(time, Timer_MusicTheme, _, TIMER_FLAG_NO_MAPCHANGE|TIMER_REPEAT);
	return Plugin_Continue;
}
...
stock FindVersionData(Handle:panel, versionindex)
{
	switch (versionindex)
	{
		...
		case 36: //137
		{
			...
			DrawPanelText(panel, "19) VSH_OnMusic forward added");
		}
		case 35: //1369
		{

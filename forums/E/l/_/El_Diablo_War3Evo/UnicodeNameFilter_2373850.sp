...
#define Free2PlayFilter 1

bool FreeToPlay[MAXPLAYERS + 1] = {false, ...};

#if Free2PlayFilter == 1
...
#if Free2PlayFilter == 1
		if(FreeToPlay[client] && StrContains(sNameBuffer,"[F2P]") == -1)
		{
			if(StrContains(sNameBuffer,"-W3E-") > -1)
			{
				Format(sNameBuffer,sizeof(sNameBuffer),"%s [F2P]",sNameBuffer);
			}
			else
			{
				Format(sNameBuffer,sizeof(sNameBuffer),"[F2P] %s",sNameBuffer);
			}
		}
#endif
...
#if Free2PlayFilter == 1
	if(FreeToPlay[client] && StrContains(sTestName1,"[F2P]") == -1)
	{
		if(StrContains(sTestName1,"-W3E-") > -1)
		{
			Format(sTestName1,sizeof(sTestName1),"%s [F2P]",sTestName1);
		}
		else
		{
			Format(sTestName1,sizeof(sTestName1),"[F2P] %s",sTestName1);
		}
	}
#endif
...
{
	//Changed based on information from arne1288 from https://forums.alliedmods.net/showthread.php?t=276055

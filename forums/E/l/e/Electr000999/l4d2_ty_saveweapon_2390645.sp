...
int 	g_iWeaponInfo[MAXPLAYERS+1][5], g_iAmmoOffset, g_iSurvMaxIncapCount, g_iAmmo_smg_max, g_iNoob, 
		g_iClearAfterCampaign, g_iRoundStart, g_iMapStart, g_Loaded, g_Loaded_REnd, g_iCountRoundEnds, g_iRemoveWeaponAfterSave;
ConVar 	g_hNoob, g_hClearAfterCampaign, g_hRemoveWeaponAfterSave, g_hAllowWipeTypesBeforeLoad, g_hSurvMaxIncapCount, g_hAmmo_smg_max;
Handle	g_fwdOnWeaponsGivedByTySaver, g_fwdOnWeaponsPreSaveByTySaver, g_fwdOnWeaponsPreGiveByTySaver;
...
{
	g_fwdOnWeaponsPreGiveByTySaver = CreateGlobalForward("OnWeaponsPreGiveByTySaver", ET_Ignore, Param_Cell);
	...
	CreateNative("TY_SaveAllWeapon", 	Native_TY_SaveAllWeapon);
...
{	
	...
	g_hNoob 								= CreateConVar("l4d2_ty_noob",							"1", "Give Silent SMG after join");
	g_hClearAfterCampaign 					= CreateConVar("l4d2_ty_clear_after_campaign", 			"0", "Wipe weapons after campaign");
	g_hRemoveWeaponAfterSave 	 			= CreateConVar("l4d2_ty_remove_weapon_after_save", 		"1", "Wipe weapons after save on map transition");
	g_hAllowWipeTypesBeforeLoad				= CreateConVar("l4d2_ty_allow_wipe_types_before_load", 	"31", "What a allowed types to wipe before loading items after mapchange if this allowed. 1=1 SLOT, 2=2 SLOT, 4=3 SLOT, 8=4 SLOT, 16 =5 SLOT");
...
public int Native_TY_SaveAllWeapon(Handle plugin, int numParams)
{
	TySaveAllWeapon();
}
...
public Action CMD_SaveWeap(int client, int args)
{
	if (client)
	{
		...
		// LOAD
		HandleOnWeaponsPreGiveByTySaver(client);
		TyGweaponC(client);
...
void TySaveWeapon(int client, int bot = 0)
{
	...
	if (iSlot0 > 0)
	{
		...
		if(g_iRemoveWeaponAfterSave)
		{
			DeletePlayerSlot(iTarget, iSlot0);
		}
	}
	else
	{
	...
	if (iSlot1 > 0)
	{
		...
		else
		{
			...
			if (!strcmp(g_sWeapDB[client][1], "dual_pistol", true) || !strcmp(g_sWeapDB[client][1], "weapon_pistol", true)
			|| !strcmp(g_sWeapDB[client][1], "weapon_pistol_magnum", true) || !strcmp(g_sWeapDB[client][1], "weapon_chainsaw", true))
			{
		...
		if(g_iRemoveWeaponAfterSave)
		{
			DeletePlayerSlot(iTarget, iSlot1);
		}
	}
	...
	if (iSlot2 > 0)
	{
		...
		if(g_iRemoveWeaponAfterSave)
		{
			DeletePlayerSlot(iTarget, iSlot2);
		}
	}
	else
	{
	...
	if (iSlot3 > 0)
	{
		...
		if(g_iRemoveWeaponAfterSave)
		{
			DeletePlayerSlot(iTarget, iSlot3);
		}
	}
	else
	{
	...
	if (iSlot4 > 0)
	{
		...
		if(g_iRemoveWeaponAfterSave)
		{
			DeletePlayerSlot(iTarget, iSlot4);
		}
	}
	else
	{
	...
	LogMessage("g_iWeaponInfo[iClipSlot1]	%i", g_iWeaponInfo[client][iClipSlot1]);
...
void TyGweaponC(int client)
{
	...
	if (IsPlayerAlive(client) && !IsFakeClient(client))
	{
		...
		for (int i; i < 5; i++)
		{
			if (g_hAllowWipeTypesBeforeLoad.IntValue & (1 << i))
			{
				iSlot = GetPlayerWeaponSlot(client, i);
				if (iSlot > 0)
				{
					#if TYLOG
					LogMessage("delete player slot before load %i", i+1);
					#endif
...
void HandleOnWeaponsPreGiveByTySaver(int client)
{	
	Call_StartForward(g_fwdOnWeaponsPreGiveByTySaver);
	Call_PushCell(client);
	Call_Finish();
}
...
new Handle:BotHeavyReadyMinigunRange;
new Handle:BotSawLowSoundsRange;
new Handle:BotSawHighSoundsRange;
				...
				if(class != TFClass_Heavy && class != TFClass_Medic && !TF2_IsPlayerInCondition(client, TFCond_Zoomed))
				{
					for (new search = 1; search <= MaxClients; search++)
					{
						if (IsClientInGame(search) && IsPlayerAlive(search) && search != client && (GetClientTeam(client) != GetClientTeam(search)))
						{
							...
							new changearea = GetRandomInt(1, 1000);
							if(changearea == 1)
							{
...
public Action:MoveTimer(Handle:timer)
{
	...
	switch(random)
	{
		...
		case 3:
		{
			...
			{
				new iTeamNumObj = GetEntProp(buildingent, Prop_Send, "m_iTeamNum");
				if (iTeamNumObj == 2)
				{

...
public Action:InitPlayers(Handle:timer)
{
	...
	for (new i = 1; i <= maxplayers; i++)
	{
		if (IsClientConnected(i) && IsClientInGame(i) && !IsClientBot(i))
		{
			...
			QueryClientPoints(i, GetClientPoints); // 04.08.10 sheleu
...
QueryClientPoints(Client, SQLTCallback:callback) // 04.08.10 sheleu
{
	decl String:SteamID[MAX_LINE_WIDTH];
...
QueryClientPointsSteamID(Client, const String:SteamID[], SQLTCallback:callback) // 04.08.10 sheleu
{
	//if (callback == empty_) // 04.08.10 sheleu
	//	callback = GetClientPoints; // 04.08.10 sheleu
...
QueryClientRank(Client, SQLTCallback:callback) // 04.08.10 sheleu
{
	//if (callback == INVALID_HANDLE) // 04.08.10 sheleu
	//	callback = GetClientRank; // 04.08.10 sheleu
...
*/
QueryRank_1(Handle:dp=INVALID_HANDLE, SQLTCallback:callback) // 04.08.10 sheleu
{
	//if (callback == INVALID_HANDLE) // 04.08.10 sheleu
	//	callback = GetRankTotal; // 04.08.10 sheleu
...
QueryRank_2(Handle:dp=INVALID_HANDLE, SQLTCallback:callback) // 04.08.10 sheleu
{
	//if (callback == INVALID_HANDLE) // 04.08.10 sheleu
	//	callback = GetGameModeRankTotal; // 04.08.10 sheleu
...
SendSQLUpdate(const String:query[]) // 04.08.10 sheleu
{
	if (db == INVALID_HANDLE)
		return;

	//if (callback == INVALID_HANDLE) // 04.08.10 sheleu
	new SQLTCallback: callback = SQLErrorCheckCallback;
...
bool:IsClientBot(client) // 04.08.10 sheleu
{

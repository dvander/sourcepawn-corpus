...
{
	name = "[TF2] Be the airblast",
	author = "Pelipoika	(FlamingSarge)",
	description = "Play as the Giant airblast Heavy from MvM",
...
public OnPluginStart()
{
	...
	CreateConVar("beairblast_version", PLUGIN_VERSION, "[TF2] Be the airblast version", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
	g_hCvarThirdPerson = CreateConVar("beairblast_thirdperson", "0", "Whether or not airblast ought to be in third-person", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	
	RegAdminCmd("sm_beairpy", Command_airblast, ADMFLAG_ROOT, "It's a good time to run");

	AddNormalSoundHook(airblastSH);
...
{
	CreateNative("Beairblast_Makeairblast", Native_Setairblast);
	CreateNative("Beairblast_Isairblast", Native_Isairblast);
...
public Action:Command_airblast(client, args)
{
	decl String:arg1[32];
	if (args < 1)
	{
	...
	{
		Makeairblast(target_list[i]);
		LogAction(client, target_list[i], "\"%L\" made \"%L\" a Giant airblast Heavy!", client, target_list[i]);
	}
...
public Action:airblastSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
...
public Native_Setairblast(Handle:plugin, args)
	Makeairblast(GetNativeCell(1));

public Native_Isairblast(Handle:plugin, args)

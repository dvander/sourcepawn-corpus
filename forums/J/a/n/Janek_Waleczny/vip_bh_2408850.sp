...
new Handle:hAutoBhopVip;
new bool:AutoBhopVipEnabled;
...
{       
	...
	hAutoBhopVip = CreateConVar("vbh_autobhopvip", "1", "Enable/Disable the bunny hopping for vips");
	
	HookConVarChange(hBhop, BhopChange);
	HookConVarChange(hAutoBhop, AutoBhopChange);
	HookConVarChange(hAutoBhopVip, AutoBhopVipChange);

	BhopEnabled = GetConVarBool(hBhop);
	AutoBhopEnabled = GetConVarBool(hAutoBhop);
	AutoBhopVipEnabled = GetConVarBool(hAutoBhopVip);
...
public AutoBhopVipChange(Handle:cvar, const String:oldVal[], const String:newVal[])
{    
	AutoBhopVipEnabled = GetConVarBool(hAutoBhopVip);
}
				...
				if (buttons & IN_JUMP)
				{
					if (!(Client_GetWaterLevel(client) > WATER_LEVEL_FEET_IN_WATER))
					{
						if (!(GetEntityMoveType(client) & MOVETYPE_LADDER))
						{
							...
							{
								if(BhopEnabled && AutoBhopEnabled && AutoBhopVipEnabled)
								{
									buttons &= ~IN_JUMP;
								}
							}

...
new Handle:MAP[M_TOTAL] = INVALID_HANDLE;
...
{	
	...
	if (b_NextMapChanged)
	{
		...
		ClearArray(MAP[M_OLD]);
		PushArrayString(MAP[M_OLD], currentnextmap);
	}
	
	if (GetArraySize(MAP[M_OLD]))
	{
		GetArrayString(MAP[M_OLD], 0, currentnextmap, sizeof(currentnextmap));
...
public Action:t_IdleServer(Handle:timer_idle)
{
	...
	GetArrayString(MAP[M_IDLE], 0, map, sizeof(map));
	
	if (GetConVarInt(cvar_LogMSG))
	{
		RCP_TRANSLOADED LogMessage("%t", "RCP Idle Server", map);}
	}
...
RCP_LoadMaplist()
{
	...
	for (new i; i < M_TOTAL; i++)
	{
		if (!b_PickMap[i])
		{
			GetArrayString(MAP[i], 0, map, sizeof(map));
			arraySize = GetArraySize(ML[i]);
			if (arraySize && FindStringInArray(ML[i], map) == -1)
			{
				b_PickMap[i] = true;
				if (g_CycleRun == i)
				{
...
{	
	...
	GetArrayString(ML[tier], num, map, sizeof(map));

	if (g_ExcludeMaps[tier])
	{
		while (FindStringInArray(oldmaplist, map) != -1)
		{
			num = GetRandomInt(0, arraySize - 1);
			GetArrayString(ML[tier], num, map, sizeof(map));
		}
...
RCP_SetRandomMap(const tier)
{
	...
	decl String:map[65];
	GetArrayString(MAP[tier], 0, map, sizeof(map));
	...
	{
		RCP_TRANSLOADED PrintToChatAll("[SM] %t %s", "RCP Nextmap", map);}
	}
	
	if (GetConVarInt(cvar_LogMSG))
	{
		RCP_TRANSLOADED LogMessage("%t %s", "RCP Nextmap", map);}
	}

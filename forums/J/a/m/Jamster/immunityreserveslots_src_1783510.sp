...
new Handle:cvar_KickListMode;
new Handle:cvar_KickListFile;

new Handle:arr_KickListIDs;

#if EXT_CONNECT
new bool:isMVM;
new Handle:cvar_AutoPassword;
new Handle:cvar_RejectReason;
new Handle:cvar_RejectReasonEnable;
new Handle:cvar_GameTypeMVM;
#endif
...
public OnConfigsExecuted()
{
	...
	#if EXT_CONNECT
	if (GetConVarInt(cvar_GameTypeMVM))
		isMVM = true;
	else
		isMVM = false;
	#endif
...
{	
	...
	if (GetConVarInt(cvar_AutoPassword) == 2 || (GetConVarInt(cvar_AutoPassword) == 1 && GetAdminFlag(AdminID, Admin_Reservation)))
	{
	...
	if (GetAdminFlag(AdminID, Admin_Reservation) || GetConVarInt(cvar_KickListMode) == 2)
	{
		LogToFileEx(g_LogFilePath, "[DEBUG] Checking for a valid client to kick for connecting client");
		if (IRS_KickValidClient(AdminID, name, steamID, GetAdminImmunityLevel(AdminID)))
		{
...
public OnClientPreConnect(const String:name[], const String:pass[], const String:ip[], const String:authid[])
{
	...
	if (GetAdminFlag(AdminID, Admin_Reservation) || GetConVarInt(cvar_KickListMode) == 2)
		IRS_KickValidClient(AdminID, name, authid, GetAdminImmunityLevel(AdminID));
}
#endif
...
bool:IRS_KickValidClient(const AdminId:ConnectingClientAdminID, const String:ConnectingClientName[], const String:ConnectingClientAuthID[], const ConnectingClientImmunity)
{	
	...
	{	
		...
		if (GetAdminFlag(PlayerAdmin, Admin_Reservation))
		{

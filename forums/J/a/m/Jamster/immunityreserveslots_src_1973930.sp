 ...
 *
 * 2.0.9
 * - Added logging option to say when the server is full.
 * ==================================================================================
 */
...
#define PLUGIN_VERSION "2.0.9-beta"
...
new Handle:cvar_LogFullServerTime = INVALID_HANDLE;
new Handle:cvar_LogFullServer = INVALID_HANDLE;

new Handle:arr_KickListIDs = INVALID_HANDLE;

new Handle:h_KickSpecClient[MAXPLAYERS+1] = INVALID_HANDLE;
new Handle:h_LogFullServer = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	Format(desc, sizeof(desc), "%t", "irs_log_fullserver");
	cvar_LogFullServer = CreateConVar("irs_log_fullserver", "0", desc, FCVAR_PLUGIN, true, 0.0, true, 1.0);
	
	Format(desc, sizeof(desc), "%t", "irs_log_fullserver_time");
	cvar_LogFullServerTime = CreateConVar("irs_log_fullserver_time", "0", desc, FCVAR_PLUGIN, true, 0.0);
...
{	
	...
	if (KickTarget)
	{
		...
		else
		{
			...
			if (HighImmunityLimit && g_HIPCount >= HighImmunityLimit)
			{
				...
				{
					LogFullServer(ConnectingClientName, ConnectingClientAuthID, true);
				}
				return false;
			}
			decl String:Reason[255];
	...
	else 
	{
		...
		{
			LogFullServer(ConnectingClientName, ConnectingClientAuthID, false);
		}
	}
...
LogFullServer(const String:clientname[], const String:clientauth[], const bool:HighImmunity)
{
	if (GetConVarInt(cvar_LogFullServer) || h_LogFullServer != INVALID_HANDLE)
	{
		return;
	}
	...
	if (HighImmunity)
	{
		LogMessage("%t", "IRS Full Server Immunity", clientname, clientauth);
	}
	else
	{
		LogMessage("%t", "IRS Full Server", clientname, clientauth);
	}

	new LoggingTime = GetConVarInt(cvar_LogFullServerTime);
	if (LoggingTime)
	{
		h_LogFullServer = CreateTimer(float(LoggingTime*60), t_LogFullServer);
	}
}

public Action:t_LogFullServer(Handle:timer, any:client)
{
	h_LogFullServer = INVALID_HANDLE;
	return Plugin_Handled;
}
...
public OnClientDisconnect(client)
{
	...
	if (h_KickSpecClient[client] != INVALID_HANDLE)
	{
		KillTimer(h_KickSpecClient[client]);
		h_KickSpecClient[client] = INVALID_HANDLE;
...
CheckKickSpecDelay(const Float:delay, const client)
{
	...
	if ((clientTeam != TEAM1 || clientTeam != TEAM2) && h_KickSpecClient[client] == INVALID_HANDLE)
	{
		h_KickSpecClient[client] = CreateTimer(delay, t_KickSpecClient, client);
	}
	else if ((clientTeam == TEAM1 || clientTeam == TEAM2) && h_KickSpecClient[client] != INVALID_HANDLE)
	{
		KillTimer(h_KickSpecClient[client]);
		h_KickSpecClient[client] = INVALID_HANDLE;
	}
...
public Action:t_KickSpecClient(Handle:timer, any:client)
{
	if (IsClientInGame(client))
	{

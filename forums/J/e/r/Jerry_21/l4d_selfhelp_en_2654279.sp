...
public Action:AdvertisePills(Handle:timer, any:client)
{
	...
	if(CanSelfHelp(client))
	{
		PrintToChat(client, "%T", "/x03Presione La Tecla /x04AGACHARSE /x03Para Autoayudarse", client);
	}
...
public Action:PlayerTimer(Handle:timer, any:client)
{
			...
			if(CanSelfHelp(client))
			{
				if(L4D2Version)
				{
					if(HelpState[client]==STATE_NONE)
					{
						...
						PrintHintText(client, "%T", "Te Estas Autoayudando", client);
					}
	...
	else if(GetConVarInt(l4d_selfhelp_eachother)>0)
	{
		...
		if ((buttons & IN_DUCK) ||  (buttons & IN_USE)) 
		{
			...
			{
				char msg[250];
				Format(msg, sizeof(msg), "%T", "Ayuda A Tus Compañeros", client, other);
				...
				if(HelpOhterState[client]==STATE_NONE)
				{
					...
					PrintHintText(other, "%T", "Estás Ayudando A", client, other);
					HelpStartTime[client]=time;
				}
			...
			while ((ent = FindEntityByClassname(ent,  "weapon_pain_pills" )) != -1)
			{
				if (IsValidEntity(ent))
				{
					...
					if(GetVectorDistance(targetVector1  , targetVector2)<dis)
					{
						...
						RemoveEdict(ent);
						pickup=true;
						PrintHintText(client, "%T", "Pastillas Encontradas", client);
				...
				while ((ent = FindEntityByClassname(ent,  "weapon_adrenaline" )) != -1)
				{
					if (IsValidEntity(ent))
					{
						...
						if(GetVectorDistance(targetVector1  , targetVector2)<dis)
						{
							...
							RemoveEdict(ent);
							pickup=true;
							PrintHintText(client, "%T", "Adrenalina Encontrada", client);
			...
			while ((ent = FindEntityByClassname(ent,  "weapon_first_aid_kit" )) != -1)
			{
				if (IsValidEntity(ent))
				{
					...
					if(GetVectorDistance(targetVector1  , targetVector2)<dis)
					{
						...
						RemoveEdict(ent);
						pickup=true;
						PrintHintText(client, "%T", "Medkit Encontrado", client);
						break;
					}
				}
 			}
			...
			while ((ent = FindEntityByClassname(ent,  "weapon_pistol" )) != -1)
			{
				if (IsValidEntity(ent))
				{
					...
					if(GetVectorDistance(targetVector1  , targetVector2)<dis)
					{
						...
						RemoveEdict(ent);
						pickup=true;
						PrintHintText(client, "%T", "Pistol Encontrada", client);
						break;
					}
				}
 			}
...
SelfHelp(client)
{
	...
	if(slot!=-1)
	{
		...
		if(slot ==4)
		{
			...
			if (adrenaline)
			{
				for (int i = 1; i <= MaxClients; i++)
				{
					...
					PrintToChat(i, "%T", "Te Estás Autoayudando Con Adrenalina", i, client);
				}
			}
			...
			if (pills)
			{
				for (int i = 1; i <= MaxClients; i++)
				{
					...
					PrintToChat(i, "%T", "Te Estás Autoayudando Con Pills", i, client);
				}
			}
		...
		else if(slot==3)
		{
			...
			for (int i = 1; i <= MaxClients; i++)
			{
				...
				PrintToChat(i, "%T", "Te Estás Autoayudando Con Botiquin", i, client);
			}
	...
	{
		PrintHintText(client, "%T", " La Autoayuda Falló", client);
 ...
 HelpOther(client, helper)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		PrintToChat(i, "%T", "Ayuda A Tus Compañeros", i, helper, client);
	}
}

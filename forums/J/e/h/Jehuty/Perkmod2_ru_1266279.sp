...
#include <sourcemod>
#include <sdktools>
#define PLUGIN_VERSION "2.1.2 ru by Jehuty"
...
public Event_RoundStart (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	else if (g_iL4D_12 == 2)
	{
		...
		else if (g_iNextPAttO == 5100)
		{
			//L4D2, Linux
			g_iNextActO = 1088;
			g_iAttackTimerO = 5444;
		}
	}
...
public Event_PlayerUse (Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new String:stEnt[MAXPLAYERS+1];
	GetEdictClassname(iEntid,stEnt,64);

	//----DEBUG----
	//PrintToChatAll("\x05pack rat \x03edict classname = \x01%s",st_entname);
	...
	if (g_iSur3[iCid]==1
		&& g_iConfirm[iCid]==1
		&& StrContains(stEnt,"weapon",false) != -1
		&& StrContains(stEnt,"spawn",false) != -1)
	{
		//if it's a specific weapon spawn, fill out ammo
		if (StrContains(stEnt,"smg")				!= -1
			|| StrContains(stEnt,"shotgun")			!= -1
			|| StrContains(stEnt,"rifle")			!= -1
			|| StrContains(stEnt,"sniper_military")	!= -1 )
		{
			//----DEBUG----
			//PrintToChatAll("\x03- giving full ammo");
		...
		{
			//----DEBUG----
			//PrintToChatAll("\x03- giving part ammo");

			PR_GivePartAmmo(iCid,stEnt,true);
		}
	}
}
...
PR_GivePartAmmo (iCid, String:stWpn[], bool:bConditional)
{
	...
	//smg - offset +20
	else if (StrEqual(stWpn,"weapon_smg",false)==true
	...
	else if (StrEqual(stWpn,"weapon_pumpshotgun",false)==true
	...
	//autoshotgun - offset +28
	else if (StrEqual(stWpn,"weapon_autoshotgun",false)==true
	...
	//huntingrifle - offset +32
	else if (StrEqual(stWpn,"weapon_hunting_rifle",false)==true)
	{
		iAmmoO += 32;
	...
	else if (StrEqual(stWpn,"weapon_sniper_military",false)==true)
	{
		iAmmoO += 36;
	...
	//grenade launcher - offset +64
	else if (StrEqual(stWpn,"weapon_grenade_launcher",false)==true)
	{
		iAmmoO += 64;
...
public Handle:Menu_Top (iCid)
{
	...
	DrawPanelText(menu,"Используйте меню для выбора перка");
	decl String:st_perk[45];
	...
	Format(st_display,64,"Выжившие - Перк 1 (%s)",st_perk);
	if (g_iSur1_enable==1)
	...
	Format(st_display,64,"Выжившие - Перк 2 (%s)", st_perk);
	if (g_iSur2_enable==1)
		DrawPanelItem(menu,st_display);
	else
		DrawPanelItem(menu,"disabled", ITEMDRAW_NOTEXT);
	...
	if (g_iSur3[iCid]==1
		...
		st_perk="Бейсбол по-русски";
	else
		st_perk="Не выбрано";

	Format(st_display,64,"Выжившие - Перк 3 (%s)", st_perk);
	if (g_iSur3_enable==1)
		DrawPanelItem(menu,st_display);
	else
		DrawPanelItem(menu,"disabled", ITEMDRAW_NOTEXT);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "tPoncho's Perkmod - Главное меню");
	DrawPanelText(menu,"Используйте меню для выбора перка");
	decl String:st_perk[45];
	decl String:st_display[MAXPLAYERS+1];
	decl iPerk;
	...
	if (iPerk==1
		...
		&& g_iBlind_enable==1)
		st_perk="Слепая удача";
	else if (iPerk==3
		&& g_iDead_enable==1)
		st_perk="Атлетичные зомби";
	else if (iPerk==4
		&& g_iMotion_enable==1)
		st_perk="Марш-бросок";
	else
	...
	Format(st_display,64,"Толстяк (%s)", st_perk);
	if (g_iInf1_enable==1)
		DrawPanelItem(menu,st_display);
	else
		DrawPanelItem(menu,st_display, ITEMDRAW_NOTEXT);
	...
	Format(st_display,64,"Курильщик (%s)", st_perk);
	if (g_iInf3_enable==1)
		DrawPanelItem(menu,st_display);
	else
		DrawPanelItem(menu,st_display, ITEMDRAW_NOTEXT);
	...
	if (iPerk==1
		...
		&& g_iEfficient_enable==1)
		st_perk="Быстрый убийца";
	else if (iPerk==3
		&& g_iGrass_enable==1)
		st_perk="Зайка-попрыгайка";
	else if (iPerk==4
		&& g_iSpeedDemon_enable==1)
		st_perk="Демон скорости";
	else
		st_perk="Не выбрано";

	Format(st_display,64,"Охотник (%s)", st_perk);
	if (g_iInf4_enable==1)
		DrawPanelItem(menu,st_display);
	else
		DrawPanelItem(menu,st_display, ITEMDRAW_NOTEXT);
	...
	if (iPerk==1
		&& g_iWind_enable==1)
		st_perk="Унесённый ветром";
	else if (iPerk==2
		&& g_iCavalier_enable==1)
		st_perk="Живучий";
	else if (iPerk == 3
		&& g_iFrogger_enable == 1)
		st_perk = "Лягушка-путешественница";
	else if (iPerk == 4
		&& g_iGhost_enable == 1)
		st_perk = "Скрытный";
	else
	...
	Format(st_display,64,"Жокей (%s)", st_perk);
	if (g_iInf5_enable==1
		&& g_iL4D_12 == 2)
		DrawPanelItem(menu,st_display);
	else
	...
	Format(st_display,64,"Плевальщица (%s)", st_perk);
	if (g_iInf6_enable==1
		&& g_iL4D_12 == 2)
		DrawPanelItem(menu,st_display);
	else
	...
	if (iPerk==1
		&& g_iScatter_enable==1)
		st_perk="Боулинг-профи";
	else if (iPerk == 2
		&& g_iBullet_enable == 1)
		st_perk = "Мчащийся поезд";
	else
		st_perk="Не выбрано";

	Format(st_display,64,"Громила (%s)", st_perk);
	if (g_iInf7_enable==1
		&& g_iL4D_12 == 2)
		DrawPanelItem(menu,st_display);
	else
	...
	if (g_iInf2[iCid]==1
		...
		&& g_iJuggernaut_enable==1)
		st_perk="Почти бессмертный";
	else if (g_iInf2[iCid]==3
		&& g_iMetabolic_enable==1)
		st_perk="Догоню!";
	else if (g_iInf2[iCid]==4
		&& g_iStorm_enable==1)
		st_perk="Компанейский парень";
	else if (g_iInf2[iCid]==5
		&& g_iDouble_enable==1)
		st_perk="Двойное веселье";
	else
		st_perk="Не выбрано";

	Format(st_display,64,"Танк (%s)", st_perk);
	if (g_iInf2_enable==1)
		DrawPanelItem(menu,st_display);
	else
	//DrawPanelItem(menu,st_display, ITEMDRAW_NOTEXT);
	DrawPanelText(menu,"Чтобы ваши перки работали");
	DrawPanelText(menu,"вы ДОЛЖНЫ нажать 'ГОТОВО'");
	DrawPanelItem(menu,"ГОТОВО");
...
public Handle:Menu_ShowChoices (iCid)
{
	...
	if (iPerk == 1
		...
		|| g_iStopping_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iStopping_enable_vs==1		&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Убойная сила (+%i%% %t)", RoundToNearest(g_flStopping_dmgmult*100), "BonusDamageText" );
	else if (iPerk == 2
		&& (g_iDT_enable==1		&&	g_iL4D_GameMode==0
		|| g_iDT_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iDT_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Пулемётчик (%t +%i%%)", "DoubleTapDescriptionPanel", RoundToNearest(100 * ((1/g_flDT_rate)-1) ) ) ;
	else if (iPerk == 3
		&& (g_iSoH_enable==1		&&	g_iL4D_GameMode==0
		|| g_iSoH_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iSoH_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Заряжающий (%t +%i%%)", "SleighOfHandDescriptionPanel", RoundToNearest(100 * ((1/g_flSoH_rate)-1) ) ) ;
	else if (iPerk == 4
		&& (g_iPyro_enable==1		&&	g_iL4D_GameMode==0
		|| g_iPyro_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iPyro_enable_vs==1		&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Пиротехник (%t)", "PyroDescriptionPanel");
	else if (iPerk == 5
		&& (g_iMA_enable==1		&&	g_iL4D_GameMode==0
		|| g_iMA_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iMA_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Мордобитель (%t)", "MartialArtistDescriptionPanel");
	else
		Format(st_perk,128,"%t", "Не выбрано");

	if (g_iSur1_enable==1)
	{
		DrawPanelItem(menu,"Выжившие, перк 1:");
	...
	if (iPerk == 1
		...
		|| g_iUnbreak_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iUnbreak_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Медик (+%i %t)", g_iUnbreak_hp, "UnbreakableHint");
	else if (iPerk == 2
		&& (g_iSpirit_enable==1		&&	g_iL4D_GameMode==0
		|| g_iSpirit_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iSpirit_enable_vs==1	&&	g_iL4D_GameMode==2))
	{
		...
		if (g_iL4D_GameMode==2)
			...
			iTime=g_iSpirit_cd;
		Format(st_perk,128,"Призрак (%t: %i min)", "SpiritDescriptionPanel", iTime/60 );
	}
	else if (iPerk == 3
		&& (g_iHelpHand_enable==1		&&	g_iL4D_GameMode==0
		|| g_iHelpHand_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iHelpHand_enable_vs==1		&&	g_iL4D_GameMode==2))
	{
		...
		if (g_iL4D_GameMode==2)
			...
			iBuff=g_iHelpHand_buff;
		if (g_iHelpHand_convar==1)
			Format(st_perk,128,"Спасатель (%t +%i)", "HelpingHandDescriptionPanel2", iBuff);
		else
			Format(st_perk,128,"Спасатель (%t +%i)", "HelpingHandDescriptionPanel", iBuff);
	}
	else
	...
	if (g_iSur2_enable==1)
	{
		DrawPanelItem(menu,"Выжившие, перк 2:");
		DrawPanelText(menu,st_perk);
	}
	...
	if (iPerk == 1
		...
		|| g_iPack_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iPack_enable_vs==1		&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Собиратель (%t +%i%%)", "PackRatDescriptionPanel", RoundToNearest(g_flPack_ammomult*100) );
	else if (iPerk == 2
		&& (g_iChem_enable==1		&&	g_iL4D_GameMode==0
		|| g_iChem_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iChem_enable_vs==1		&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Спец по таблеткам (%t +%i)", "ChemReliantDescriptionPanel", g_iChem_buff);
	else if (iPerk == 3
		&& (g_iHard_enable==1			&&	g_iL4D_GameMode==0
		|| g_iHard_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iHard_enable_vs==1		&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Крепкий орешек (+%i%% %t)", RoundToNearest(g_flHard_hpmult*100), "HardToKillDescriptionPanel" );
	else if (iPerk == 4
		&& (g_iExtreme_enable==1		&&	g_iL4D_GameMode==0
		|| g_iExtreme_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iExtreme_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"У страха глаза велики (+%i%% %t)", RoundToNearest(g_flExtreme_rate*100-100), "MartialArtistDescriptionPanelCoop" );
	else if (iPerk == 5
		&& (g_iLittle_enable==1		&&	g_iL4D_GameMode==0
		|| g_iLittle_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iLittle_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Бейсбол по-русски (%t)", "LittleLeaguerDescriptionPanel" );
	else
		Format(st_perk,128,"%t", "NotSet");

	if (g_iSur2_enable==1)
	{
		DrawPanelItem(menu,"Выжившие, перк 3:");
		DrawPanelText(menu,st_perk);
	}
...
{
	new Handle:menu=CreatePanel();
	SetPanelTitle(menu,"tPoncho's Perkmod: Ваши перки в этом раунде");
	decl String:st_perk[128];
	...
	{
		Format(st_perk,128,"Охотник: %t", "NotSet");
		stDesc = "";
	}
	if (g_iInf4_enable==1)
	{
	...
	if (iPerk == 1
		&& g_iTwinSF_enable == 1)
	{
		st_perk="Плевальщица: Захлебнитесь!";
		Format(stDesc, 128, "%t", "TwinSpitfireDescriptionPanel" );
	}
	if (iPerk == 2
		&& g_iMegaAd_enable == 1)
	{
		st_perk="Плевальщица: Супер-клей";
		Format(stDesc, 128, "%t", "MegaAdhesiveDescriptionPanel" );
	}
	else
	{
		Format(st_perk,128,"Плевальщица: %t", "NotSet");
		stDesc="";
	}
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "tPoncho's Perkmod - Выжившие: Перк 1");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[20];
	...
	else
	{
		...
		{
			case 1: st_current="(ВЫБРАНО)";
			default: st_current="";
		}
		Format(st_display,64,"Убойная сила %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		switch (g_iSur1[client])
		{
			case 2: st_current="(ВЫБРАНО)";
			default: st_current="";
		}
		Format(st_display,64,"Пулемётчик %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		switch (g_iSur1[client])
		{
			case 3: st_current="(ВЫБРАНО)";
			default: st_current="";
		}
		Format(st_display,64,"Заряжающий %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		switch (g_iSur1[client])
		{
			case 4: st_current="(ВЫБРАНО)";
			default: st_current="";
		}
		Format(st_display,64,"Пиротехник %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		switch (g_iSur1[client])
		{
			case 5: st_current="(ВЫБРАНО)";
			default: st_current="";
		}
		Format(st_display,64,"Мордобитель %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "tPoncho's Perkmod - Выжившие: Перк 2");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[20];
	...
	else
	{
		...
		Format(st_display,64,"Медик %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Призрак %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Спасатель %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "tPoncho's Perkmod - Выжившие: Перк 3");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[20];
	...
	else
	{
		...
		Format(st_display,64,"Собиратель %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Спец по таблеткам %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Крепкий орешек %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"У страха глаза велики %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Бейсбол по-русски %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "tPoncho's Perkmod - Толстяк");
	...
	else
	{
		...
		Format(st_display,64,"Запашок %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Слепая удача %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Атлетичные зомби %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Марш-бросок %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "tPoncho's Perkmod - Танк");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[20];
	...
	else
	{
		...
		Format(st_display,64,"Мясорубка %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Почти бессмертный %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Догоню! %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Компанейский парень %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Двойное веселье %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "tPoncho's Perkmod - Курильщик");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[20];
	...
	else
	{
		...
		Format(st_display,64,"Языкастый %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Удав %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Разборчивый %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Потаскун! %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "tPoncho's Perkmod - Охотник");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[20];
	...
	else
	{
		...
		Format(st_display,64,"Крепыш %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Быстрый убийца %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Зайка-попрыгайка %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Демон скорости %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "tPoncho's Perkmod - Жокей");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[20];
	...
	else
	{
		...
		Format(st_display,64,"Унесённый ветром %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Живучий %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Лягушка-путешественница %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Скрытный %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "tPoncho's Perkmod - Плевальщица");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[20];
	...
	else
	{
		...
		Format(st_display,64,"Захлебнитесь! %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Супер-клей %s",st_current);
		DrawPanelItem(menu,st_display);
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "tPoncho's Perkmod - Громила");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[20];
	...
	else
	{
		...
		Format(st_display,64,"Боулинг-профи %s",st_current);
		DrawPanelItem(menu,st_display);
	...
	else
	{
		...
		Format(st_display,64,"Мчащийся поезд %s",st_current);
		DrawPanelItem(menu,st_display);

...
public OnPluginStart()
{
	...
	else
	{
		...
		DevInfo = CreateConVar("l4d_loot_devinfo", "0", "Developer indo", FCVAR_PLUGIN);
...
//	PrintToChatAll("\x05Event: CheckPoint (Start)");
	...
	if (Target && (GetClientTeam(Target)) == 2)
	{
		...
		else
		{
			...
			if (StrEqual(current_map, "l4d_smalltown04_mainstreet", false))
			{
...
//	PrintToChatAll("\x05Event: PlayerDeath (Start)");
	...
	if (GetConVarInt(NoFarm) == 1 && IsMapFinished == 1)
	{
		return Plugin_Continue;
	}

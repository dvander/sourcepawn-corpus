...
public Action:Command_Say(client, args)
{
	...
	{
		if (DifficultyMultiplier[client] != 1.9)
		{
			SetClientInfo(client, "_dv", "hard");
			DifficultyMultiplier[client] = 1.9;
			GetTotalDifficultyMultiplier();
			Autodifficulty(GetRealClientCount(true));
...
public OnClientPutInServer(client)
{
	if (!IsFakeClient(client) && GetConVarInt(l4d2_autodifficulty) == 1)
	{
		...
		if (GetClientInfo(client, "_dv", client_difficulty, sizeof(client_difficulty)))
		{
			...
			{
				DifficultyMultiplier[client] = 0.1;
			}
...
//	GetTotalDifficultyMultiplier();
	...
	if (playerscount > 4)
	{
		...
		SetConVarInt(l4d2_loot_h_drop_items, RoundToZero((playerscount / 6) * SquareRoot(DifficultyMultiplier[0])), false, false);
		SetConVarInt(l4d2_loot_b_drop_items, RoundToZero((playerscount / 6) * SquareRoot(DifficultyMultiplier[0])), false, false);
		SetConVarInt(l4d2_loot_s_drop_items, RoundToZero((playerscount / 6) * SquareRoot(DifficultyMultiplier[0])), false, false);
		SetConVarInt(l4d2_loot_c_drop_items, RoundToZero((playerscount / 6) * SquareRoot(DifficultyMultiplier[0])), false, false);
		SetConVarInt(l4d2_loot_sp_drop_items, RoundToZero((playerscount /6) * SquareRoot(DifficultyMultiplier[0])), false, false);
		SetConVarInt(l4d2_loot_j_drop_items, RoundToZero((playerscount / 6) * SquareRoot(DifficultyMultiplier[0])), false, false);
		SetConVarInt(l4d2_loot_t_drop_items, RoundToZero((playerscount / 5) * SquareRoot(DifficultyMultiplier[0])), false, false);
		...
		SetConVarInt(grenadelauncher_damage, RoundToZero(100.0 * playerscount), false, false);
	}
	else
	{
		SetConVarInt(z_hunter_health, 250, false, false);

...
public OnPluginStart()
{
	...
	CreateConVar("l4d_tk_deadline", "2750", "TK Points needed to get banned.", FCVAR_PLUGIN);
	CreateConVar("l4d_tk_startvote_points", "600", "TK Points needed to get voteban.", FCVAR_PLUGIN);
}
...
//		return;
	...
	if (TotalDamage > 600.0)
	{
		if (TotalDamage > 2750.0)
		{
			if (Punishment[client] < 2750.0)
			{
				decl String:ClientSteamID[32];
				GetClientAuthString(client, ClientSteamID, sizeof(ClientSteamID));
		...
		else if ((TotalDamage - Punishment[client]) > 600.0)
		{
			Punishment[client] = TotalDamage;

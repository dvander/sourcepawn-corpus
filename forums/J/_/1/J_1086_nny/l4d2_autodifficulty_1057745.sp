...
public OnPluginStart()
{
	...
	CreateConVar("l4d2_autodifficulty_ver", PLUGIN_VERSION, "Version of the [L4D2] Autodifficulty.", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
//						PrintToChatAll("\x04%s", ClientName);
						...
						if (StrEqual(ClientName, "Tank(1)", false) && GetClientHealth(i) > 1)
						{
							return GetClientHealth(i);
						}
					}
...
public Action:Command_Say(client, args)
{
	...
	if (strcmp(text[startidx], "!info", false) == 0)
	{
		...
		PrintToChatAll("\x05Grenade Launcher Damage = \x04%d!", GetConVarInt(grenadelauncher_damage));
	}
...
//	GetTotalDifficultyMultiplier();
	...
	if (playerscount > 4)
	{
		...
		SetConVarInt(l4d2_loot_h_drop_items, CheckCvarMin(RoundToZero((playerscount / 4.6) * SquareRoot(DifficultyMultiplier[0])), 1), false, false);
		SetConVarInt(l4d2_loot_b_drop_items, CheckCvarMin(RoundToZero((playerscount / 4) * SquareRoot(DifficultyMultiplier[0])), 1), false, false);
		SetConVarInt(l4d2_loot_s_drop_items, CheckCvarMin(RoundToZero((playerscount / 4.6) * SquareRoot(DifficultyMultiplier[0])), 1), false, false);
		SetConVarInt(l4d2_loot_c_drop_items, CheckCvarMin(RoundToZero((playerscount / 4.6) * SquareRoot(DifficultyMultiplier[0])), 1), false, false);
		SetConVarInt(l4d2_loot_sp_drop_items, CheckCvarMin(RoundToZero((playerscount /4.6) * SquareRoot(DifficultyMultiplier[0])), 1), false, false);
		SetConVarInt(l4d2_loot_j_drop_items, CheckCvarMin(RoundToZero((playerscount / 4.6) * SquareRoot(DifficultyMultiplier[0])), 1), false, false);
		SetConVarInt(l4d2_loot_t_drop_items, CheckCvarMin(RoundToZero(playerscount * 3 * SquareRoot(DifficultyMultiplier[0])), 1), false, false);
		...
		SetConVarInt(z_charger_health, RoundToZero(150.0 * playerscount * DifficultyMultiplier[0]), false, false);
		SetConVarInt(z_spitter_health, RoundToZero(25.0 * playerscount * DifficultyMultiplier[0]), false, false);
		SetConVarInt(z_jockey_health, RoundToZero(81.25 * playerscount * DifficultyMultiplier[0]), false, false);
		...
		SetConVarInt(grenadelauncher_damage, 400 + RoundToZero(50.0 * (playerscount - 4)), false, false);
	...
	SetConVarInt(l4d2_loot_g_chance_nodrop, CheckCvarMin(RoundToZero(25 / DifficultyMultiplier[0]), 5), false, false);
	
//	SetConVarInt(tongue_miss_delay, CheckCvarMin(RoundToZero(playerscount * DifficultyMultiplier[0]), 2), false, false);
	...
	{
		SetConVarInt(z_tank_health, RoundToZero(600 * playerscount * DifficultyMultiplier[0]), false, false);
	}
	else if (StrEqual(sGameDifficulty, "Hard", false))
	{
		SetConVarInt(z_tank_health, RoundToZero(700 * playerscount * DifficultyMultiplier[0]), false, false);
	}
	else if (StrEqual(sGameDifficulty, "Impossible", false))
	{
		SetConVarInt(z_tank_health, RoundToZero(800 * DifficultyMultiplier[0] * playerscount), false, false);
	}
}
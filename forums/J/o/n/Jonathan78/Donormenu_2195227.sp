/*
 * In-game Help Menu
 * Written by chundo & Snipped by Jonathan78
 *
...
public OnPluginStart() {
	...
	RegConsoleCmd("sm_donor", Command_HelpMenu, "Display the help menu.", FCVAR_PLUGIN);

	new String:hc[PLATFORM_MAX_PATH];
	BuildPath(Path_SM, hc, sizeof(hc), "configs/donormenu.cfg");
...
public Action:Timer_WelcomeMessage(Handle:timer, any:client) {
	if (GetConVarBool(g_cvarWelcome) && IsClientConnected(client) && IsClientInGame(client) && !IsFakeClient(client))
		PrintToChat(client, "\x01For a Donator Menu, type \x04!donor\x01 in chat [Donator's only]");
}

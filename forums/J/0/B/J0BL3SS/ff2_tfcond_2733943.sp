...
public Action FF2_OnAbility2(int boss, const char[] plugin_name, const char[] ability_name,int status)
{
	...
	//Menu Exclusive version
	char AbilityName[96];
	for(int Num = 0; Num <= 9; Num++)
	{
		Format(AbilityName, sizeof(AbilityName), "menu_tfcond_%i", Num);
		if(!strcmp(ability_name, AbilityName))
		{
			InvokeCondition(boss, client, Num, false);
		}
	}
...
public void TFC_Invoke(int client)
{
	...
	InvokeCondition(boss, client, -1, false); //Its not deactivates AMS Triggers, its just a trick to make it work properly
}
...
{
	int boss=FF2_GetBossIndex(client);
	InvokeCondition(boss, client, 0, true);
}
...
{
	int boss=FF2_GetBossIndex(client);
	InvokeCondition(boss, client, 1, true);
}
...
{
	int boss=FF2_GetBossIndex(client);
	InvokeCondition(boss, client, 2, true);
}
...
{
	int boss=FF2_GetBossIndex(client);
	InvokeCondition(boss, client, 3, true);
}
...
{
	int boss=FF2_GetBossIndex(client);
	InvokeCondition(boss, client, 4, true);
}
...
{
	int boss=FF2_GetBossIndex(client);
	InvokeCondition(boss, client, 5, true);
}
...
{
	int boss=FF2_GetBossIndex(client);
	InvokeCondition(boss, client, 6, true);
}
...
{
	int boss=FF2_GetBossIndex(client);
	InvokeCondition(boss, client, 7, true);
}
...
{
	int boss=FF2_GetBossIndex(client);
	InvokeCondition(boss, client, 8, true);
}
...
{
	int boss=FF2_GetBossIndex(client);
	InvokeCondition(boss, client, 9, true);
}

public void InvokeCondition(int boss, int client, int tfcnum, bool amsbool)
{
	...
	if(tfcnum >= 0 && !amsbool)
	{
		Format(amsCond, sizeof(amsCond), "menu_tfcond_%i", tfcnum);
		//Format(abilitySound,sizeof(abilitySound), "sound_tfcondition_%i", tfcnum); Menu Abilities Already uses their own "sound" section
	}

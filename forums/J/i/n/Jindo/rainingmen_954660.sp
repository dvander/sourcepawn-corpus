...
public OnPluginStart()
{
	...
	// generic cvars
	g_cvars[CVAR_VERSION] = CreateConVar("rainingmen_version", PLUGIN_VERSION, "Current version of the plugin.");
	g_cvars[CVAR_MESSAGE] = CreateConVar("rm_message_enable", "1", "Disable the \"It's Raining Men\" message.");
	// map-specific cvars
	g_cvars[CVAR_ENABLE] = CreateConVar("sm_rainmen_enable", "1", "Enable the plugin");
	g_cvars[CVAR_RADIUS] = CreateConVar("sm_rainmen_radius", "100.0", "Radius around the player to spawn ragdolls.");
	g_cvars[CVAR_HEIGHT] = CreateConVar("sm_rainmen_height", "500.0", "Height above the player to spawn ragdolls.");
	g_cvars[CVAR_BURN] = CreateConVar("sm_rainmen_burn", "2", "0 = no burn, 1 = all burn, 2 = random");
	g_cvars[CVAR_ELEC] = CreateConVar("sm_rainmen_elec", "0", "0 = no electrocuted, 1 = all electrocuted, 2 = random");
	g_cvars[CVAR_FREQUENCY] = CreateConVar("sm_rainmen_frequency", "1", "Frequency to spawn the ragdolls (higher than 10 may be unsafe)");
	g_cvars[CVAR_REMOVE] = CreateConVar("sm_rainmen_remove", "6.0", "Time (in seconds) the ragdoll lasts before it is removed.");

...
public Action:Timer_UpdateHudHint(Handle:timer, any:client)
{
	...
	else if (iSpecModeUser == SPECMODE_FIRSTPERSON || iSpecModeUser == SPECMODE_3RDPERSON)
	{
		...
		if (iTargetUser > 0) {
			Format(szText, sizeof(szText), "<font size='15' color='#FF0000'>Spectating %N:\n<font size='14' color='#FFFFFF'>", iTargetUser);
			bOther = true;
			bDisplayHint = true;
		}
	...
	{
		if (!bOther) {
			Format(szText, sizeof(szText), "<font size='15' color='#FF0000'>Spectating %N:\n</font><font size='14' color='#FFFFFF'>%s", client, szText);
		}
		Format(szText, sizeof(szText), "%s</font>", szText);

...
Cvars:
l4d2_l4d1vs_version "2.2" 							//Version number
...
Todo:
- dynamic roundscore array for custom maps with 5+ maps
- tie breaker -> infected damage decides (?)

###########

Known Bugs / Incompatibilites:
- Score display shows original values
...
new Handle:game_config;
new Handle:sdk_getscore;
...
{
	//load game config
	game_config = LoadGameConfigFile("left4downtown.l4d2");
	if(game_config == INVALID_HANDLE)
	{
#if DEBUG_ENABLE
		LogToFile(DEBUG_LOGFILE, "#### Could not load left4downtown.l4d2 ####");
#endif
		SetFailState("Fail to load left4downtown.l4d2");
	}
...
public Action:L4D_OnSetCampaignScores(&scoreA, &scoreB)
{
	...
	for (new i = 0; i < 5; i++) LogToFile(DEBUG_LOGFILE, "Round Scores: %d", g_round_scores[team][i]);
#endif
...
public Action:L4D_OnClearTeamScores(bool:newCampaign)
{
	...
	if (newCampaign)
	{
		...
		for (new i = 0; i < 2; i++)
		{
			for (new j = 0; j < 5; j++) g_round_scores[i][j] = 0;
			g_campaign_scores[i] = 0;
		}
...
GetScore(team)
{
	...
	PrintToScoreAll(fsurvivor_distance, score_team, survivors, fmap_multiplier, score, team);
...
#######################
*/
PrintToScoreAll(Float:fsurvivor_distance, score_team, survivors, Float:fmap_multiplier, score, team)
{
	if (survivors == 0)
	{
		PrintToChatAll("%s (AvgDistance) * MapMultiplier", PLUGIN_TAG);	
		PrintToChatAll("%s (%0.1f) * %0.1f = %d", PLUGIN_TAG, fsurvivor_distance, fmap_multiplier, score);
	}
	else
	{
		PrintToChatAll("%s (AvgDistance + Health) * Survivors * MapMultiplier", PLUGIN_TAG);	
		PrintToChatAll("%s (%0.1f + %d) * %d * %0.1f = %d", PLUGIN_TAG, fsurvivor_distance, score_team, survivors, fmap_multiplier, score);	
	}
	
	new tempscore = score + g_campaign_scores[team];
	PrintToChatAll("%s Campaign Score: %d", PLUGIN_TAG, tempscore);
}
...
#if DEBUG_ENABLE	
	LogToFile(DEBUG_LOGFILE, "Campaign: %d, Map: %d", campaign, map);
#endif
...
{	
	...
	//drawing roundscores
	for (new i = 0; i < (g_map_counter + 1); i++)
	{
		Format(text, sizeof(text), "Map %d: %d / %d", (i + 1), g_round_scores[clientteam][i], g_round_scores[enemyteam][i]);

...
public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	HookEvent("player_spawn", AddBots);
	

}
public Action:AddBots(Handle:event, const String:name[], bool:dontBroadcast){
	//PrintToServer("!!! player_spawn");
	UnhookEvent("player_spawn", AddBots);
	CreateTimer(0.1, CheckSurvivorBot);
}
public Action:CheckSurvivorBot(Handle:timer)
{
	...
	//PrintToServer("!!! survlimit %d", survlimit);
	//PrintToServer("!!! curr %d", current_clients);
	if(survlimit < 4)
		return Plugin_Handled;
...
public Action:KickBots(Handle:timer){
	new survlimit = GetConVarInt(g_hSurvivorLimit);
	new current_clients = GetTeamClientCount(2);
	while(current_clients > survlimit){
		//PrintToServer("!!! client teams not ok");
		for(new i = 1; i <= MaxClients; ++i)
		{
			if(IsClientInGame(i))
				if(IsFakeClient(i) && GetClientTeam(i) == 2 &&!IsClientInKickQueue(i)){
					KickClient(i);
					break;
				}
		}

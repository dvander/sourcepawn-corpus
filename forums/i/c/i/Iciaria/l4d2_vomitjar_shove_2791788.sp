...
*	Author	:	SilverShot
*	Descrp	:	Biles infected when presses reload and shoved by players holding vomitjars.
...
public Plugin myinfo =
{
	name = "[L4D2] Vomitjar Shove",
	author = "SilverShot",
	description = "Biles infected when presses reload and shoved by players holding vomitjars.",
...
public void Event_PlayerShoved(Event event, const char[] name, bool dontBroadcast)
{
	...
	if( GetClientTeam(target) == 3 && (GetClientButtons(GetClientOfUserId(event.GetInt("attacker"))) & 8192 ) )
	{
		int weapon = CheckWeapon(client);
		if( weapon )
		{

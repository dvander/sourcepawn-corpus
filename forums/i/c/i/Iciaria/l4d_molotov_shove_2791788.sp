...
*	Author	:	SilverShot
*	Descrp	:	Ignites infected when presses reload and shoved by players holding molotovs.
...
public Plugin myinfo =
{
	name = "[L4D & L4D2] Molotov Shove",
	author = "SilverShot",
	description = "Ignites infected when presses reload and shoved by players holding molotovs.",
...
public void Event_PlayerShoved(Event event, const char[] name, bool dontBroadcast)
{
	...
	if( GetClientTeam(target) == 3 && CheckWeapon(client) && (GetClientButtons(GetClientOfUserId(event.GetInt("attacker"))) & 8192 ) )
	{

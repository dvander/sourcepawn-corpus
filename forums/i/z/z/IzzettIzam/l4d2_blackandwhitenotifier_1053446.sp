...
public Plugin:myinfo =
{
	...
	author = "DarkNoghri, madcap, eXcuSEr",
...
public OnPluginStart()
{
	...
	CreateConVar("l4d_blackandwhite_version", PLUGIN_VERSION, "Version of L4D2 Black and White Notifier", FCVAR_REPLICATED|FCVAR_NOTIFY);
...
public EventReviveSuccess(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(GetEventBool(event, "lastlife"))
	{
		...
		if(StrContains(targetModel, "gambler", false) > 0) 
		{
...
public EventHealSuccess(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//fill string with character names
	if(StrContains(healeeModel, "gambler", false) > 0) 
	{
		if(status[NICK]) status[NICK] = false;
	}
...
public EventPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//fill string with character names
	if(StrContains(deadModel, "gambler", false) > 0) 
	{
		if(status[NICK]) status[NICK] = false;
	}

    ...
    for(new x=1;x<=MAXPLAYERS;x++)
    {
        if(ValidClient(x) && !IsFakeClient(x))
        {
            if(ReadyUpState())
            {
                ...
                {
                    Kick(x, "120 Saniye icinde Ready yazmalisiniz");
                    continue;
                }            
                ...
                for(new y=1;y<=MAXPLAYERS;y++)
                {
                    ...
                    if (GetUserMessageType() == UM_Protobuf) {
                        new Handle:hMessage = StartMessageOne("HintText", x);
                        PbSetString(hMessage, "text", tmptext);
                        EndMessage();
                    } else {
                        new Handle:hMessage = StartMessageOne("KeyHintText", x);
                        BfWriteByte(hMessage, 1);
                        BfWriteString(hMessage, tmptext);
                        EndMessage();
                    }
...
StartFirstHalf()
{
    ...
    */
    ...
    PrintToChatAll("[PUG] Ilk Yari Basliyor Bol Sans...");
    EnforceMatchCvars();
...
StartSecondHalf()
{
    ...
    EnforceMatchCvars();
    PrintToChatAll("[PUG] Ikinci Yari Basliyor Bol Sans...");
    ServerCommand("mp_restartgame 1\n");
    RestartTimers = CreateTimer(2.0, RestartSecondTime);
}
...
TryStartMatch()
{
    ...
    if(!StrEqual(curmap, MatchMap))
    {
        PrintToChatAll("[PUG] Secilen Harita 5 Saniye sonra Aciliyor.");
...
{
    // Vote for rule set.
    PrintToChatAll("[PUG] Mac Ayarlari Yuklendi Oylamalar Basliyor.");
...
ReadyUp(client) {
    ...
    PrintToChatAll("[PUG] %s Simdi Hazir.", plName);
    ...
    if(!bStillMore)
    {
        ...
        else
        {
            ...
            PrintToChatAll("[PUG] Henuz %d Oyuncu Bekleniyor...", NeedPl);
        }
    }
}
...
public Action:SayHook(client,args,bool:team)
{
    ...
    if(StrEqual(Words[0],"/ready", false))
    {
        if(!ReadyUpState())
        {
            PrintToChat(client, "[PUG] Su an Ready Yazmaniza Gerek Yok.");
            bHookMessage = true; 
        }
        else
        {
            if(bReady[client])
            {
                PrintToChat(client,"[PUG] Zaten Hazirsiniz.");
                bHookMessage = true;
            }
            else
            {
    ...
    else if(StrEqual(Words[0], "/mute", false))
    {
        ...
        else
        {
            ...
            else
            {
                if(client==cl)
                {
                    PrintToChat(client, "[PUG] Kendinizi Muteleyemezsiniz..");
                }
                else if(IsPlayerMuted(client, cl))
                {
                    PrintToChat(client, "[PUG] Oyuncu Zaten Mutelendi.");
                }
                else
                {
                    PrintToChat(client, "[PUG] Oyuncu Mutelendi.");
    ...
    else if(StrEqual(Words[0], "/unmute", false))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    PrintToChat(client, "[PUG] Oyuncunun Mute'si Kaldirildi.");
                }
                else
                {
    ...
    else if(StrEqual(Words[0], "/chat", false))
    {
        ...
        {
            bPubChatMuted[client] = false;
            PrintToChat(client, "[PUG] Chat Acildi");
        }
        else
        {
            bPubChatMuted[client] = true;
            PrintToChat(client, "[PUG] Chat Yasaklandi.");
        }
    }
    ...
    else if(StrEqual(Words[0], "/notready", false))
    {
        if(!bReady[client])
        {
            PrintToChat(client, "[PUG] Suan Hazir Degilsiniz.");
            bHookMessage = true;
        }
        else
        {
            ...
            {
                PrintToChat(client, "[PUG] Ready komutunu Kullanmadan Once 15 Saniye Bekleyiniz..");
                bHookMessage = true;
            }
            else
            {
                ...
                new String:plName[32];
                GetClientName(client, plName, 32);
                PrintToChatAll("[PUG] %s Artik Hazir Degil.", plName);
    ...
    else if(StrEqual(Words[0], "/dmg", false))
    {
        if(!MatchLive())
        {
            PrintToChat(client, "[PUG] Bu komutu mac baslamadan Kullanamazsiniz.");
            bHookMessage = true;
        }
        else
        {
            ...
            {
                PrintToChat(client, "[PUG] Bu Komutu Kullanmak icin Olu olmaniz Gerekli.");
                bHookMessage = true;
            }
            else
            {
...
public Action:SpawnCallback(Handle:event, const String:name[], bool:dontBroadcast)
{
    ...
    if(ReadyUpState())
    {
        if(FirstSpawn[client])
        {
            ...
            PrintToChat(client, "[PUG] Hosgeldiniz! Lufen /ready yazin. Yada Komutlari Gormek icin /help Yazin.");
        }
        else if(!bReady[client])
        {
            PrintToChat(client, "[PUG] Hazir Oldugunuzda Chat Ekranina /ready Yazin.");
        }
    ...
    else
    {
        if(FirstSpawn[client])
        {
            PrintToChat(client, "[PUG] Hos Geldiniz! Mac Basladi., Lutfen Yardim icin /help Yazin.");
            FirstSpawn[client] = false;
        }
    }
    return Plugin_Continue;
    ...
    for(new x=1;x<=MAXPLAYERS;x++)
    {
        ...
        if(bForMe)
        {
            ...
            if (GetUserMessageType() == UM_Protobuf) {
                ...
                PbSetString(hBuffer, "msg_name", fullChat);

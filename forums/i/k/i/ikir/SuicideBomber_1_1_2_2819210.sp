...
{
	name		= "SuicideBomber",
	version		= "1.1.1_not_colored (rewritten by Grey83 and ikir)",
	author		= "rrrfffrrr",
...
public void OnPluginStart()
{
	...
	cvar = CreateConVar("sm_suicide_bomber", "bomber", "Let bot suicide", FCVAR_NOTIFY|FCVAR_PRINTABLEONLY);
	cvar.AddChangeHook(CVarChange_Class);
	CVarChange_Class(cvar, NULL_STRING, NULL_STRING);

	cvar = CreateConVar("sm_suicide_detonate_range", "150", "Detonate range", FCVAR_NOTIFY, true);
	cvar.AddChangeHook(CVarChange_Range);
	f_detonate_range = cvar.FloatValue;

	cvar = CreateConVar("sm_suicide_resist", "4", "Damage resistance", FCVAR_NOTIFY, true, 1.0);
	cvar.AddChangeHook(CVarChange_Resist);
	CVarChange_Resist(cvar, NULL_STRING, NULL_STRING);

	cvar = CreateConVar("sm_suicide_delay", "1", "Detonate delay", FCVAR_NOTIFY, true);
...
public Action FFrame(Handle timer)
{
	...
	for(int i = 1, j, ent, pointHurt; i <= MaxClients; i++)
		if(IsClientInGame(i) && IsFakeClient(i) && IsPlayerAlive(i) && StrContains(s_class[i], s_class[0], false) != -1)
		{
			...
			DispatchKeyValue(pointHurt, "Damage", "280");

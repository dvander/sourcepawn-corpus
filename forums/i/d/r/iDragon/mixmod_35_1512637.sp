...
public Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			if ((GetConVarInt(g_CvarKnifeWinTeamVote) == 1) && (hasMixStarted)) // Lets vote for the side of the teams.
			{
...
{
	if ((GetConVarInt(g_CvarEnabled) == 1) && (GetConVarInt(g_CvarShowTkMessage) == 1) && (hasMixStarted))
	{
...
{
	if ((GetConVarInt(g_CvarEnabled) == 1) && (!IsChatTrigger()) && ((GetConVarInt(g_CvarDisableSayCommand) == 1) || ((GetConVarInt(g_CvarDisableSayCommand) == 2) && (hasMixStarted) && (didLiveStarted))))
		return Plugin_Handled;
...
CreateHelpPanel()
{
	...
	AddMenuItem(g_HelpPanel, "Usage: sm_st <player> [t/ct/spec/1/2/3] or sm_st", "sm_st - Swaps player or players team");
	AddMenuItem(g_HelpPanel, "sm_rpw - Set a random password to the server.", "sm_rpw - Set a random password to the server.");
	AddMenuItem(g_HelpPanel, "Usage: sm_rr <seconds>", "sm_rr - Restart the game");
	...
	AddMenuItem(g_HelpPanel, "Usage: sm_spec <player>", "sm_spec - Move player to spectors team.");
	...
	AddMenuItem(g_HelpPanel, "http://forums.alliedmods.net/showthread.php?p=1512637", "Download this mod (click)");
	AddMenuItem(g_HelpPanel, "-----------------------", "-----------------------");
...
public Action:Command_ShowTeams(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			if (client != 0)
				PrintToChat(client, "\x04[%s]:\x03 This command is enabled only when auto-war is running");

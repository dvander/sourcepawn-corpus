...
public Action Command_setteam(int client, int args)
{
	...
	if(IsPlayerAlive(target_list[i]) != true && TF2_GetClientTeam(target_list[i]) == TFTeam_Spectator && GetClientHealth(target_list[i]) != 1){
	...
	SetEntityHealth(target_list[i], 125);
...
stock void SetTeam(int client, int value){

...
Versions:
	...
	1.3.3
		* Все звуки и сообщения об отсчете таймера бомбы берутся из файла configs/c4settings.cfg. By LobioN
	1.3.4
		* Профиксен баг при котором отсчет таймера мереносился на следующий раунд. By LobioN
*/
...
public LoadSounds()
{
	...
	{
		SetFailState("configs/c4settings.cfg not found or not correctly structured");
		return;
	}
...
/*	for (new i = 1; i <= GetMaxClients(); i++)
	{
		PrintToChat(i, "[ROUND END] This timer %s",  hTimer);
	}*/
...
public EventRoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	/*for (new i = 1; i <= GetMaxClients(); i++)
	{
		PrintToChat(i, "[ROUND START] This timer %s",  hTimer);
	}*/
	if (hTimer != INVALID_HANDLE)
	{
		KillTimer(hTimer);

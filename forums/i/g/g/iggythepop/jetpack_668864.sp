 ...
 *           -- Added Flame Effect
 *
 * Added by: -SinCO-supershwa (August 2008)
 *		-- Recompiled with the June 2008 SourcePawn v1.0.3 compiler (less server crashing w/ new Pyro/Medic updates)
 *		-- Fixed "stuck to floor" issue - players can use the jetpack w/o going airborne first (jump/fall) 
 *
 */
...
new String:g_rSound[PLATFORM_MAX_PATH]	= "hl1/fvox/activated.wav";
...
//added by -SinCO-supershwa Aug 10, 2008 (for "stuck to floor" issue)
public bool:TraceRayDontHitSelf(entity, mask, any:data)
{	
    if(entity == data) // Check if the TraceRay hit itself.
    {
        return false; // Don't let the entity get hit
    }
    return true; //didn't hit self
}
...
//revamped by -SinCO-supershwa Aug 10, 2008 (to fix old "stuck to floor" issue)
AddVelocity(client, Float:speed) 
{
	if(g_iVelocity == -1) return;
    
	new Float:c_vecVelocity[3],Float:c_vecOrigin[3],Float:e_vOrigin[3],Float:e_vAngles[3],Float:end_vOrigin[3],Float:collideDist;
	
    GetClientEyePosition(client, e_vOrigin); //get player's eye position to determine distance to ceiling (to prevent from sticking in world geometry)
	GetEntDataVector(client, g_iVelocity, c_vecVelocity);
	GetClientAbsOrigin(client, c_vecOrigin);

	e_vAngles[0] = -89.0;
	e_vAngles[1] = 0.0;
	e_vAngles[2] = 0.0;
	
	TR_TraceRayFilter(e_vOrigin, e_vAngles, MASK_PLAYERSOLID, RayType_Infinite, TraceRayDontHitSelf, client);
    TR_GetEndPosition(end_vOrigin, INVALID_HANDLE);
    
    collideDist = end_vOrigin[2] - e_vOrigin[2];
    if(TR_DidHit(INVALID_HANDLE) && collideDist >= 50) // make sure the distance between the player's eyes and ceiling are 50 (prevents from sticking in world geometry)
    {
        if (GetEntityFlags(client) & FL_ONGROUND) //give the player a little push if they're on the ground (fixes stuck issue from pyro/medic updates)
        {
         c_vecOrigin[2] += 1; //gets player off the ground if they're not in the air
        }
    }

    c_vecVelocity[2] += speed;

	TeleportEntity(client, c_vecOrigin, NULL_VECTOR, c_vecVelocity);
}

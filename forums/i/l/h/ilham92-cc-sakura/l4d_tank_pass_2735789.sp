...
{
	CreateNative("TS_TankPass", Native_TS_TankPass);
	RegPluginLibrary("l4d_tank_pass");
...
public void OnPluginStart()
{
	...
	cVar = CreateConVar("l4d_tank_pass_extinguish", "1", "If \"l4d_tank_pass_fire\" convar set to 0: 0=Ignite the new Tank when passed, 1=Extinguish the new Tank when passed.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	cVar = CreateConVar("l4d_tank_pass_count", "0", "0=Off, >0=The number of times the Tank can be passed by plugin (Frustration counts as pass).", FCVAR_NOTIFY, true, 0.0);
...
|--------------------------------------------------------------------------
*/
public any Native_TS_TankPass(Handle hPlugin, int iNumArguments)
{
	TankPass(GetNativeCell(1), GetNativeCell(2));
}

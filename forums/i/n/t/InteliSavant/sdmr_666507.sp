...
public OnPluginStart()
{
	...
	cvarCrits = CreateConVar("sm_crits", "1", "Whether or not you want criticals enabled for sudden death", FCVAR_PLUGIN, true, 0.0, true, 0.0);

	cvarServerCrits = FindConVar("tf_weapon_criticals");
...
public Action:event_SuddenDeathStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	previousVar = GetConVarBool(cvarServerCrits);

	if(GetConVarBool(cvarCrits) != previousVar)
	{
		SetConVarInt(cvarServerCrits,GetConVarBool(cvarCrits));	
	}
...
public Action:event_SuddenDeathEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetConVarBool(cvarCrits) != previousVar)
	{
		SetConVarInt(cvarServerCrits,previousVar);	
	}

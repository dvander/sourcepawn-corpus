...
public void OnPluginStart()
{
	...
	HookEvent("door_close",			Event_DoorClose);
        ...
        RegConsoleCmd("panel_antifarm", Panel_AntiFarm);
	GetGameData();
...
public Action Panel_AntiFarm(int client, int args)
{
    Panel panel = new Panel();
    panel.SetTitle("Console:");
    panel.DrawText("ㅤ");
    panel.DrawText("|ANTIFARM| AntiFarm ON");
    panel.DrawText("ㅤ");
    panel.DrawText("Exit");
...
public Action OnPlayerUsePre(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (IsSurvivor(user))
	{
		...
		if (IsValidEnt(used))
		{
			...
			{
				PrintHintText(user, "%T", "No Chance", user);
				return Plugin_Continue;
			}
			...
			if (iDoorStatus != UNLOCK)
			{
				if(iMinSurvivorPercent > 0 && !bSurvivorsAssembleAlready)
				{
					...
					if(iReached < iParam)
					{
						PrintHintText(user, "%T", "SurvivorReached", user, iReached, iParam);
						return Plugin_Continue;
					}
				...
				if (GetTankCount() > 0)
				{
					...
					if (!bAntiFarmInit)
					{
						...
						iSystemTime = iAntiFarmDuration;
                                                ...
                                                Panel_AntiFarm(user, 5);
						PrintHintText(user, "%T", "Tank is still alive", user);
...
public Action LockdownOpening(Handle timer, any entity)
{
	...
	if (hLockdownTime == null)
	{
		if (!bLDFinished)
		{
			...
			if(blsHint) CPrintToChatAll("{default}[{olive}DOOR{default}]{green} <{olive}%s{green}>{default} %t", sKeyMan, "open the door already");
			if(bAnnounce)
			{
...
public Action LaunchTankDemolition(Handle timer)
{
	...
	if (bAnnounce)
	{
		CPrintToChatAll("{default}[{olive}DOOR{default}] %t","Tanks are coming");
	}
...
public Action AntiPussy(Handle timer)
{
	...
	if(iSystemTime <= 0)
	{
		...
		if(bAnnounce) CPrintToChatAll("{default}[{olive}DOOR{default}] %t","Outside Slay");
...
public void OnDoorAntiSpam(const char[] output, int caller, int activator, float delay)
{
	...
	{
		CPrintToChatAll("{default}[{olive}DOOR{default}] %t", "Chance Left", --iDoorOpenChance);
...
void DoorPrint(Event event, bool open)
{
	if( bLDFinished && blsHint)
	{
		...
		{
			if(open) CPrintToChatAll("{default}[{olive}DOOR{default}]{green} ---{olive}%N{green}---{default} %t", client, "someone opens the door");
			else CPrintToChatAll("{default}[{olive}DOOR{default}]{green} ---{olive}%N{green}---{default} %t", client, "someone closes the door");
		}
	}
}

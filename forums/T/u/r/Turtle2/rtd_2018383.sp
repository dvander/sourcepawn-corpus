...
#define PLUGIN_VERSION 		"0.4.4.2 patched"
...
public OnClientDisconnect(client)
{
	...
	{
		PrintToChatAll("%s %t", PLUGIN_PREFIX, "RTD_Disconnected", g_strTeamColors[GetClientTeam(client)], client, 0x01);
	}
...
ShowEffectsMenu(client, iStartItem=0)
{
	...
	decl String:display[STRING_PERK_MAXLEN];
	
	SetGlobalTransTarget(client);
	Format(display, sizeof(display), "%t", "RTD_Effects_Title");
	...
	for(new i=0; i<sizeof(g_nPerks); i++)
	{
		if(g_nPerks[i][g_nPerkType] == PERK_GOOD && !g_nPerks[i][g_bPerkDisabled])
		{
			...
			SetGlobalTransTarget(client);
			Format(display, sizeof(display), "%t", g_nPerks[i][g_strPerkName]);
			
			AddMenuItem(hMenu, strInfo, display);
		}
	}
...
public MenuHandler_Effects(Handle:hMenu, MenuAction:action, client, menu_item)
{
	if(action == MenuAction_Select)
	{
		...
		{
			PrintToChat(client, "%s %s%t\x01: %t", PLUGIN_PREFIX, g_nPerks[iPerk][g_nPerkType] == PERK_GOOD ? COLOR_PERK_GOOD : COLOR_PERK_BAD, g_nPerks[iPerk][g_strPerkName], g_nPerks[iPerk][g_strPerkDesc]);
		}
...
RTD(client)
{
	...
	{
		PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Disabled");
		return;
	}
	...
	if(GetForwardFunctionCount(g_hfwdCanRoll) > 0)
	{
		...
		{
			PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Access_Denied");
			return;
		}
	}
	...
	if(!RTD_IsInRound())
	{
		PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_NotInRound");
		return;
	}
	...
	{
		PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_InRoll");
		return;
	}
	...
	{
		PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Wait", 0x04, GetConVarInt(g_hCvarTimelimit)-iTimeLeft, 0x01);
		return;
	}
	
	if(!IsPlayerAlive(client))
	{
		PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Alive");
		return;
	}
	...
	switch(g_eDiceModes:GetConVarInt(g_hCvarMode))
	{
		case MODE_LEGACY:
		{
			for(new i=1; i<=MaxClients; i++)
			{
				...
				{
					PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Busy_Mode1", g_strTeamColors[GetClientTeam(i)], i, 0x01);
					return;
				}
			}
		}
		case MODE_TEAMLIMIT:
		{
			...
			{
				PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Busy_Mode2");
				return;
			}
		}
	}
	...
	if(!iNumPerks)
	{
		PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_No_Effects");
		return;
	}
...
InitiateEffect(client, g_eCurrentPerk:nPerk)
{
	...
	switch(nPerk)
	{
		case PERK_GODMODE:
		{
			PrintToChatAll("%s %t", PLUGIN_PREFIX, "RTD_Effect_Time", g_strTeamColors[iTeam], client, 0x01, g_nPerks[_:nPerk][g_nPerkType] == PERK_GOOD ? COLOR_PERK_GOOD : COLOR_PERK_BAD, g_nPerks[_:nPerk][g_strPerkName], 0x01, "\x04", RoundToFloor(flDuration), 0x01);
		...
		{
			PrintToChatAll("%s %t", PLUGIN_PREFIX, "RTD_Effect", g_strTeamColors[iTeam], client, 0x01, g_nPerks[_:nPerk][g_nPerkType] == PERK_GOOD ? COLOR_PERK_GOOD : COLOR_PERK_BAD, g_nPerks[_:nPerk][g_strPerkName], 0x01);
			...
			PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Health", COLOR_PERK_ORANGE, iHealth, 0x01);
		...
		{
			PrintToChatAll("%s %t", PLUGIN_PREFIX, "RTD_Effect_Time", g_strTeamColors[iTeam], client, 0x01, g_nPerks[_:nPerk][g_nPerkType] == PERK_GOOD ? COLOR_PERK_GOOD : COLOR_PERK_BAD, g_nPerks[_:nPerk][g_strPerkName], 0x01, "\x04", RoundToFloor(flDuration), 0x01);
			PrintToChat(client, "%s %t", PLUGIN_PREFIX, g_nPerks[_:nPerk][g_strPerkDesc]);
...
TerminateEffect(client, g_eCurrentPerk:nPerk, bool:bIsAlive=true)
{
	...
	switch(nPerk)
	{
		...
		case PERK_NOCLIP:
		{
			...
			{
				PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Stuck");
...
public Action:Timer_EffectEnd(Handle:hTimer, any:client)
{
	if(client >= 1 && client <= MaxClients && IsClientInGame(client) && g_nPlayerData[client][g_nPlayerState] == STATE_ROLLING)
	{
		...
		PrintToChatAll("%s %t", PLUGIN_PREFIX, "RTD_Effect_End", g_strTeamColors[GetClientTeam(client)], client, 0x01);
	}
...
public Action:Event_PlayerDeath(Handle:hEvent, const String:strEventName[], bool:bDontBroadcast)
{
	...
	if(iVictim >= 1 && iVictim <= MaxClients && IsClientInGame(iVictim))
	{
		...
		PrintToChatAll("%s %t", PLUGIN_PREFIX, "RTD_Died", g_strTeamColors[GetClientTeam(iVictim)], iVictim, 0x01);
	}
...
public Action:Timer_Timebomb(Handle:hTimer, any:client)
{
	if(client >= 1 && client <= MaxClients && IsClientInGame(client) && g_nPlayerData[client][g_nPlayerState] == STATE_ROLLING && g_nPlayerData[client][g_nPlayerPerk] == PERK_TIMEBOMB)
	{
		...
		if(g_nPlayerData[client][g_iPlayerTime] > 0)
		{
			...
			PrintCenterTextAll("%t", "RTD_Timebomb", g_nPlayerData[client][g_iPlayerTime], client);
			...
			PrintToChatAll("%s %t", PLUGIN_PREFIX, "RTD_Timebomb_Result", g_strTeamColors[iTeam], client, 0x01, COLOR_PERK_ORANGE, iPlayerDamage, 0x01);
...
public Event_PlayerSpawn(Handle:hEvent, const String:strEventName[], bool:bDontBroadcast)
{
	...
	if(client >= 1 && client <= MaxClients && IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Respawn", g_strTeamColors[GetClientTeam(client)], client, 0x01);
	}
}
...
public Action:Command_ForceRTD(client, args)
{
	...
	if(strlen(strKey))
	{
		...
		{
			iOverride = iIndex;
			ReplyToCommand(client, "%s Perk forced: %t", PLUGIN_PREFIX, g_nPerks[iIndex][g_strPerkName]);
		}
	}
	...
	if((target_count = ProcessTargetString(
			...
			tn_is_ml)) <= 0)
	{
		ReplyToCommand(client, "%s %t", PLUGIN_PREFIX, "RTD_NoTargets");
		return Plugin_Handled;
	}
...
public Event_RoundActive(Handle:hEvent, const String:strEventName[], bool:bDontBroadcast)
{
	if(GetConVarBool(g_hCvarEnabled) && RTD_IsInRound())
	{
		...
		{
			case 1: PrintToChatAll("%s %t", PLUGIN_PREFIX, "RTD_Advert1", COLOR_PERK_ORANGE, 0x01);
			case 2: PrintToChatAll("%s %t", PLUGIN_PREFIX, "RTD_Advert1", COLOR_PERK_ORANGE, 0x01);
		}
	}
}
...
bool:ForceRTD(client, iVictim, bool:bLaunchEffect=true, g_eCurrentPerk:nPerkOverride=g_eCurrentPerk:-1)
{
	if(!RTD_IsInRound())
	{
		if(bLaunchEffect) PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_NotInRound");
		return false;
	}
	
	if(g_nPlayerData[iVictim][g_nPlayerState] != STATE_IDLE)
	{
		if(bLaunchEffect) PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_InRoll");
		return false;
	}
	
	if(!IsPlayerAlive(iVictim))
	{
		if(bLaunchEffect) PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Alive");
		return false;
	}
	...
	if(!iNumPerks)
	{
		if(bLaunchEffect) PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_No_Effects");
		return false;
	}
			...
			if(g_nPlayerData[client][g_iPlayerTime] < iMax)
			{
				...
				if(hTrace != INVALID_HANDLE && TR_DidHit(hTrace))
				{
					...
					if(CanBuildHere(flEndPos, flMins, flMaxs))
					{
						...
						PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Building_Limit", g_nPlayerData[client][g_iPlayerTime], iLimit);
				...
				PrintToChat(client, "%s %t", PLUGIN_PREFIX, "RTD_Building_Placement");

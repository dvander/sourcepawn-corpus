	...
	if (i > 0 && i < MaxClients && IsClientConnected(i) && IsClientInGame(i)) {
		if(args == 1) {
			...
			if (StrEqual(servername, "")) {
				Format(buffer, sizeof(buffer), "https://trinia.pro/sourcemod/popup.html?web=https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=%s&lc=US&amount=%s.00&currency_code=%s&no_note=0", donateurl, amount, currency);
			}
			else {
				Format(buffer, sizeof(buffer), "https://trinia.pro/sourcemod/popup.html?web=https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=%s&item_name=%s&lc=US&amount=%s.00&currency_code=%s&no_note=0", donateurl, servername, amount, currency);
			}

...
#define KNIFEFIGHT_VERSION      "1.3.8-c"
...
new String:g_ctchicken[MAX_FILE_LEN] = "models/player/techknow/chicken/chickenct.mdl";
new String:g_tchicken[MAX_FILE_LEN] = "models/player/techknow/chicken/chickent.mdl";
...
public OnMapStart()
{
	...
	BuildPath(Path_SM, file, MAX_FILE_LEN, "configs/chickens.ini");
...
public DoCustom()
{
	 for(new i = 1; i <= GetMaxClients(); i++)
	 {
	       if(IsClientInGame(i))
	       {
                       PrintToChat((i),"[SM] POOF! YOU ARE A CHICKEN!"); 
                       if (GetClientTeam(i) == 3)
	               {
                        // Make player Chicken ct model 
                          SetEntityModel(i,g_ctchicken);
                       }
                       else if (GetClientTeam(i) == 2)
                       {
                        // Make player Chicken t model
                          SetEntityModel(i,g_tchicken);
                       }
	       }
        }
}

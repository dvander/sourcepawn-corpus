...
new Handle:g_CvarLaserModel = INVALID_HANDLE;
...
{
new String:tmp[64];
GetConVarString(g_CvarLaserModel, tmp, sizeof(tmp));

/*
	AddFileToDownloadsTable("materials/sprites/taglaser.vtf");
	AddFileToDownloadsTable("materials/sprites/taglaser.vmt");
*/
	g_sprite = PrecacheModel(tmp);
}
...
public OnPluginStart()
{
	...
	g_CvarLaserModel = CreateConVar("sm_laser_tag_model","materials/sprites/muzzleflash4.vmt","path to the used model for display");
	HookConVarChange(g_CvarLaserModel, ConVarChange_Lasermodel);
...
public BulletImpact(Handle:event,const String:name[],bool:dontBroadcast)
{
	if( GetConVarBool(g_CvarEnable) )
	{
		...
		*/
		...
		TE_Start("BeamPoints");
		TE_WriteVector("m_vecStartPoint", newBulletOrigin);
		TE_WriteVector("m_vecEndPoint", bulletDestination);
		TE_WriteNum("m_nModelIndex", g_sprite);
...
public ConVarChange_Lasermodel(Handle:convar, const String:oldValue[], const String:newValue[])
{
        if (oldValue[0] == newValue[0])
        {
                return;
        }
	g_sprite = PrecacheModel(newValue);
	return;
}
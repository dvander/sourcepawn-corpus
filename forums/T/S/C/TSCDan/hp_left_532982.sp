...
public printInfo(client){
	if(headshots[client]){	
		PrintToChat(client,"\x04%s killed you with %s (headshot) and has %i hp/%i armor left",deathInfo[client][0],deathInfo[client][1],hpleft[client],armor[client])
	}else{
		PrintToChat(client,"\x04%s killed you with %s and has %i hp/%i armor left",deathInfo[client][0],deathInfo[client][1],hpleft[client],armor[client])
	}
}
...
public Action:playerDeath(Handle:event, const String:name[], bool:dontBroadcast){
	...
	*/
	...
	if(!(armor[victim]=GetClientArmor(attacker))){
		armor[victim]=0;
	}

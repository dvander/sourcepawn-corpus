...
int lastUsedStamp[MAXPLAYERS+1];
int lastUsedStamp2[MAXPLAYERS+1];

// HUD Handles
Handle hm_HP = INVALID_HANDLE;
...
int g_iPlayerArena[MAXPLAYERS+1];
int g_iPlayerSlot[MAXPLAYERS+1];
int g_iPlayerHP[MAXPLAYERS+1]; //true HP of players
int g_iPlayerSpecTarget[MAXPLAYERS+1];
int g_iPlayerMaxHP[MAXPLAYERS+1];
int g_iClientParticle[MAXPLAYERS+1];
int g_iPlayerClip[MAXPLAYERS+1][3];
int g_iPlayerWins[MAXPLAYERS+1];
int g_iPlayerLosses[MAXPLAYERS+1];
int g_iPlayerRating[MAXPLAYERS+1];
int g_iPlayerHandicap[MAXPLAYERS+1];
TFClassType g_tfctPlayerClass[MAXPLAYERS+1];

// Bot things
bool g_bPlayerAskedForBot[MAXPLAYERS+1];
int g_BotinArena[MAXPLAYERS+1];
int g_BotIndex[MAXPLAYERS+1];
...
// Endif
float g_fRocketForceX;
float g_fRocketForceY;
float g_fRocketForceZ;

// Bball
char g_sBBallParticleRed[64];
char g_sBBallParticleBlue[64];

ConVar BlockTauntEnabled;
...
{
  name = "MGEMod",
  author = "Lange & Cprice,TonyBaretta; based on kAmmomod by Krolus.",
...
public void OnPluginStart()
{
	...
	BlockTauntEnabled = CreateConVar("block_taunt", "0", "Enables/disables taunts", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	AddCommandListener(ListenerBlockTaunt, "taunt");
	...
	//RegConsoleCmd("botme", Command_AddBot);
...
/* OnGetGameDescription(char gameDesc[64])
 *
...
{	
	...
	if(isMapAm)
	{
		...
		if(g_bAutoCvar)
		{
			...
			ServerCommand("mp_respawnwavetime 999");
...
public void OnMapEnd()
{
	...
	RemoveNormalSoundHook(view_as<NormalSHook>(sound_hook));
 ...
 * -------------------------------------------------------------------------- */
public int OnProjectileTouch(int entity, int other)
{
...
public void OnClientPostAdminCheck(int client)
{
	if (client)
	{
		if (IsFakeClient(client))
		{
			...
			if (!g_bNoStats)
			{
				char steamid_dirty[32]; char steamid[64]; char query[256];
...
{
	if (IsValidClient(client) && !IsFakeClient(client) && g_iPlayerArena[client])
	{
...
public int TF2Items_OnGiveNamedItem_Post(int client, char[] classname, int index, int level, int quality, int entity)
{
	switch (index)
	{	 
		case 59, 29, 211, 35, 411, 663, 796, 805, 885, 894, 903, 912, 961, 970, 998, 15008, 15010, 15025, 15039, 15050 : CreateTimer(0.1, Timer_RemoveEntity, EntIndexToEntRef(entity));
	}
}
stock int ForceWrench(int client)
{
	if (IsValidClient(client))
	{
		...
		int NormalWrench = TF2Items_GiveNamedItem(client, hWeapon);
		CloseHandle(hWeapon);
		int weapon = GetPlayerWeaponSlot(client, 2);
		SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", weapon);
		EquipPlayerWeapon(client, NormalWrench);
	}
}
stock int ForceScoutPistol(int client)
{
	if (IsValidClient(client))
	{
		//CreateTimer(0.1, Timer_RemoveEntity, EntIndexToEntRef(entity));
		...
		int NormalScoutPistol = TF2Items_GiveNamedItem(client, hWeapon);
		CloseHandle(hWeapon);
		int weapon = GetPlayerWeaponSlot(client, 2);
		SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", weapon);
		EquipPlayerWeapon(client, NormalScoutPistol);
	}
}
stock int ForceSoldierShotgun(int client)
{
	if (IsValidClient(client))
	{
		//CreateTimer(0.1, Timer_RemoveEntity, EntIndexToEntRef(entity));
		...
		int NormalShotgun = TF2Items_GiveNamedItem(client, hWeapon);
		CloseHandle(hWeapon);
		int weapon = GetPlayerWeaponSlot(client, 2);
		SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", weapon);
		EquipPlayerWeapon(client, NormalShotgun);
	}
}
stock int ForcePyroShotgun(int client)
{
	if (IsValidClient(client))
	{
		//CreateTimer(0.1, Timer_RemoveEntity, EntIndexToEntRef(entity));
		...
		int NormalPyroShotgun = TF2Items_GiveNamedItem(client, hWeapon);
		CloseHandle(hWeapon);
		int weapon = GetPlayerWeaponSlot(client, 2);
		SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", weapon);
		EquipPlayerWeapon(client, NormalPyroShotgun);
	}
}
...
public Action OnTouchIntel(int entity, int other)
{
	...
	int particle;
	TFTeam team = view_as<TFTeam>(GetEntProp(client, Prop_Send, "m_iTeamNum"));
...
// ====[ HUD ]====================================================
int ShowSpecHudToArena(int arena_index)
{
	if (!arena_index)
		return;
...
int ShowCountdownToSpec(int arena_index,char[] text)
{
	if (!arena_index)
		return;
...
int ShowSpecHudToClient(int client)
{	
...
// ====[ QUEUE ]==================================================== 
int RemoveFromQueue(int client, bool calcstats=false, bool specfix=false)
{
	char PlayerSName[50];
	int arena_index = g_iPlayerArena[client];
/*	if (g_BotinArena[client] >0)
	{
		for (int i = 1; i <= g_BotinArena[client]; i++){
			GetClientName(i,PlayerSName,sizeof(PlayerSName));
			int bot_slot = g_iPlayerSlot[i];
			if((bot_slot==SLOT_ONE || bot_slot==SLOT_THREE) || (bot_slot==SLOT_ONE || bot_slot==SLOT_THREE)){
				PrintToChatAll("%d,%s",bot_slot,PlayerSName);
				ServerCommand("tf_bot_kick %s", PlayerSName);
			}
		}
		g_BotinArena[client] = 0;
	} */
	if (g_BotinArena[client] >0)
	{
		for (int i = 1; i <= g_BotIndex[client]; i++){
			GetClientName(i,PlayerSName,sizeof(PlayerSName));
			PrintToChatAll("%s",PlayerSName);
			ServerCommand("tf_bot_kick %s", PlayerSName);
			g_BotIndex[client] = 0;
		}
		g_BotinArena[client] = 0;
...
/*	for (int i = 1; i <= MaxClients; i++){
		if(IsValidClient(i) && IsFakeClient(i) && !g_BotinArena[client]){
			GetClientName(i,PlayerSName,sizeof(PlayerSName));
			ServerCommand("tf_bot_kick %s", PlayerSName);
		}
	} */
...
int AddInQueue(int client, int arena_index, bool showmsg = true, int playerPrefTeam = 0)
{
	if(!IsValidClient(client))
		return;
...
// ====[ STATS ]====================================================
int CalcELO(int winner, int loser)
{
	...
	int time = GetTime();
	char query[512]; char sCleanArenaname[128]; char sCleanMapName[128];
...
int CalcELO2(int winner, int winner2, int loser, int loser2)
{
...
int ResetPlayer(int client)
{
	...
	{
		if(!g_bArenaBBall[arena_index]){
			if(player_slot == SLOT_THREE)return false;
			if(player_slot == SLOT_FOUR)return false;
		}
		if (class != TF2_GetPlayerClass(client)) 
			TF2_SetPlayerClass(client,class);

		TF2_RespawnPlayer(client);
		if(g_iPlayerArena[client] && TF2_GetPlayerClass(client) != TFClass_Engineer){
			DestroyBuildings(client);
		}
		CreateTimer(0.1, RipSpect,GetClientUserId(client));
	} else {
...
int ResetClientAmmoCounts(int client)
{
	// Crutch.
...
int ResetIntel(int arena_index, any client = -1)
{
	if(g_bArenaBBall[arena_index])
	{
		if(IsValidEdict(g_iBBallIntel[arena_index]) && g_iBBallIntel[arena_index] > 0)
...
int SetPlayerToAllowedClass(int client, int arena_index)
...
int ParseAllowedClasses(const char[] sList, int output[TFClassType])
{
	int count; char a_class[9][9];
...
int AttachParticle(int ent, char[] particleType ,int &particle) // Particle code borrowed from "The Amplifier" and "Presents!".
{
...
int RemoveClientParticle(int client)
{
...
// ====[ MAIN MENU ]====================================================
int ShowMainMenu(int client,bool listplayers=true)
{
	if (client<=0)
		return;
...
int ShowTop5Menu(int client, char[][] name, int[] rating)
{
	if (client<=0)
		return;
...
{ //handle commands "!ammomod" "!add" and such //building queue's menu and listing arena's	
	...
	if (!IsValidClient(client))
		return Plugin_Continue;
	if (GetTime() - lastUsedStamp[client] < 5) {
		PrintToChat(client, "\x04 [MGE]\x01 You must wait \x04 5\x01 seconds between attempts!");
		lastUsedStamp[client] = GetTime();
		return Plugin_Handled;
	} else {
		lastUsedStamp[client] = GetTime();
	}
	if(g_iPlayerArena[client] || g_BotinArena[client]){
...
public Action Command_Remove(int client, int args)
{
	if (!IsValidClient(client))
		return Plugin_Continue;
	if (GetTime() - lastUsedStamp2[client] < 5) {
		PrintToChat(client, "\x04 [MGE]\x01 You must wait \x04 5\x01 seconds between attempts!");
		lastUsedStamp2[client] = GetTime();
		return Plugin_Handled;
	} else {
		lastUsedStamp2[client] = GetTime();
	}
...
{ //adding bot to client's arena
	...
	if(g_BotinArena[client] == 1){
		PrintToChat(client, "\x04 [MGE] you can spawn only 1 bot");		
		return Plugin_Handled;
	}
	int arena_index = g_iPlayerArena[client];
	int	 player_slot = g_iPlayerSlot[client];
	if(g_bArenaBBall[arena_index]){
		PrintToChat(client, "\x04 [MGE] you can't spawn a bot here");		
		return Plugin_Handled;
	}
...
// ====[ SQL ]====================================================
int PrepareSQL() // Opens the connection to the database, and creates the tables if they dont exist.
{
	char error[256];
...
public int T_SQLQueryOnConnect(Handle owner, Handle hndl, const char[] error, any data)
{
	int client = data;
	...
	char query[512];
	char namesql_dirty[MAX_NAME_LENGTH]; char namesql[(MAX_NAME_LENGTH*2)+1];
...
public int T_SQL_Top5(Handle owner, Handle hndl, const char[] error, any data)
{
	int client = data;
...
public int T_SQL_Test(Handle owner, Handle hndl, const char[] error, any data)
{
	int client = data;
...
public int SQLDbConnTest(Handle owner, Handle hndl, const char[] error, any data)
{
	if(!StrEqual("", error))
	{
		LogError("Query failed: %s", error);
...
public void Event_PlayerSpawn(Event event,const char[] name,bool dontBroadcast)
{
	...
	if(g_bArenaBBall[client] && g_BotinArena[client]> 0){
		PrintToChat(client, "slot %s", g_iPlayerSlot[client]);
	}
	if(g_bArenaBBall[arena_index])
		g_bPlayerHasIntel[client] = false;
}
...
public Action Event_PlayerHurt(Event event,const char[] name,bool dontBroadcast)
{
	...
	int iDamage = GetEventInt(event, "damageamount");
...
public Action Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(g_iPlayerArena[victim] && TF2_GetPlayerClass(victim) != TFClass_Engineer){
			DestroyBuildings(victim);
	}
	int attacker = GetClientOfUserId(GetEventInt(event, "attacker"));
...
public Action Timer_Tele(Handle timer, any userid)
{
	...
	{
		int random_int;
		int offset_high; int offset_low;
...
{	
	...
	if(g_fCappedTime[arena_index] != 0)
	{
		...
		{
			float cap = FloatDiv(g_fCappedTime[arena_index] * 8.4, float(g_fTotalTime[arena_index]));
 ...
 * -------------------------------------------------------------------------- */
public bool TraceEntityPlayersOnly(int entity, int mask, any client)
{
...
* --------------------------------------------------------------------------- */
public int EndKoth(any arena_index, any winner_team)
{
...
* --------------------------------------------------------------------------- */
public int ResetArena(any arena_index)
{
...
* --------------------------------------------------------------------------- */
public int swapClasses(any client, any client_teammate)
{	
...
public int PlayEndgameSoundsToArena(any arena_index, any winner_team)
{
	int red_1 = g_iArenaQueue[arena_index][SLOT_ONE];
	int blu_1 = g_iArenaQueue[arena_index][SLOT_TWO];
...
public Action RipSpect(Handle timer, any userid)
{
	int client = GetClientOfUserId(userid);
...
public Action ListenerBlockTaunt(int client, const char[] command, int args)
{
	if(BlockTauntEnabled.IntValue){
		return Plugin_Handled;
	}
	return Plugin_Continue;
}
public Action Event_Inventory(Event event, const char[] name, bool dontBroadcast)
{
	...
	int Slot1 = GetPlayerWeaponSlot(iClient, TFWeaponSlot_Secondary);
	//int Slot0 = GetPlayerWeaponSlot(iClient, TFWeaponSlot_Primary);
	int Slot2 = GetPlayerWeaponSlot(iClient, TFWeaponSlot_Melee);
	//int wep0 = GetEntProp(Slot0, Prop_Send, "m_iItemDefinitionIndex");
	int wep1 = GetEntProp(Slot1, Prop_Send, "m_iItemDefinitionIndex");
	int wep2 = GetEntProp(Slot2, Prop_Send, "m_iItemDefinitionIndex");
	...
	switch(iClass)
	{
		...
		case TFClass_Scout:{
			if(wep1 == 163 || wep1 == 46 || wep1 == 1121 || wep1 == 1145){
				TF2_RemoveWeaponSlot(iClient, 1);
				ForceScoutPistol(iClient);
			}
		}
		case TFClass_Soldier:{
			if(wep1 == 415){
				TF2_RemoveWeaponSlot(iClient, 1);
				ForceSoldierShotgun(iClient);
			}
		}
		case TFClass_Pyro:{
			if(wep1 == 415){
				TF2_RemoveWeaponSlot(iClient, 1);
				ForcePyroShotgun(iClient);
			}
		}
	}
}
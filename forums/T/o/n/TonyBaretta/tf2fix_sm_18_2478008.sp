...
#define PLUGIN_VERSION	"1.4.8_03"
public Plugin myinfo = {
	name = "TF2Fix",
	author = "MasterOfTheXP,TonyBaretta",
...
ConVar cvarEnabled; ConVar cvarLogClassName;
ConVar cvarNonCritBackstabs; ConVar cvarWaterDoves; ConVar cvarDeadRingerTaunt;
ConVar cvarPomsonSound; ConVar cvarIconManglerDeflect; ConVar cvarHypeMeterSwitch; ConVar cvarRageMeterSwitch;
ConVar cvarBazaarHeadsSteal; ConVar cvarBazaarHeadsMeter; ConVar cvarEyelanderHeadsMeter; ConVar cvarYourEternalIntelligence; ConVar cvarCowManglerSlowdown;
ConVar cvarDeadTaunts; ConVar cvarHomeRunBoost; ConVar cvarTomislavAnnounce; ConVar cvarGunslinger;
ConVar cvarHuntsmanIcons; ConVar cvarHuntsmanWater; ConVar cvarScorchTaunt; ConVar cvarFanOResupply; ConVar cvarOriginalDraw;
ConVar cvarBasherSuicide; ConVar cvarTeleporterThanks; ConVar cvarSydneyHeadshot; ConVar cvarPhlogAmmo; ConVar cvarTeleRecharge;
/*ConVar cvarItemCooldown;*/ ConVar cvarEscapeMedic; ConVar cvarUberCrits; ConVar cvarBotTaunts; ConVar cvarEurekaDestroy;
ConVar cvarVitaRounding; ConVar cvarBoostMeter; ConVar cvarPomsonPenetration; ConVar cvarBazookaHumiliation; ConVar cvarDeadRingerIndicator;
ConVar cvarCraftMetal; ConVar cvarBonkWasted; ConVar cvarQuickFixSound; ConVar cvarSandmanReflectIcon; ConVar cvarDemoGuaranteeCrit;
ConVar cvarCleaverReflectIcon; ConVar cvarPreroundMove; ConVar cvarChargeSound; ConVar cvarPomsonHitbox; ConVar cvarDecalRespawn;
ConVar cvarWrangledRocketIcon; ConVar cvarCleaverCritBleed; ConVar cvarTankDestroySound; ConVar cvarInvulnDiamondback; ConVar cvarIconReflectDetonator;
ConVar cvarBonkTaunt; ConVar cvarBazaarNoCrit; ConVar cvarSpecialTaunts; ConVar cvarTeleporterTaunt; ConVar cvarEurekaSapped;
ConVar cvarEngiHighFive; ConVar cvarDetonatorCritSound;
ConVar g_hScareTauntFixEnabled;

/* Handles for the game's cvars */
ConVar cvarWeaponCrits; ConVar cvarHypeMax; ConVar cvarSlidingTaunt;

/* Cached cvar values; for cvars that would otherwise be checked extremely often */
bool Enabled = true; bool TomislavAnnounce = true; bool Gunslinger = true; bool OriginalDraw = true;
	...
	bool DetonatorCritSound = true; bool g_bscare[MAXPLAYERS+1] = false;

/* For checking if extensions are loaded */
bool extSDKHooks; bool extTF2Attributes;
...
/* HUD Text */
Handle hudDeadRinger; Handle hudHeads;
...
public void OnMapStart()
{
	...
	for (int i = 0; i < sizeof(CritReceiveSounds); i++)
...
stock void SetDeathWeapon(Event event, const char[] weapon, bool force = false)
{
...
public Action Event_Intelligence(Event event, const char[] name, bool dontBroadcast)
{
	if (!Enabled) return;
...
public Action Event_Teleport(Event event, const char[] name, bool dontBroadcast)
{
	if (!Enabled) return;
...
public Action Event_Upgrade(Event event, const char[] name, bool dontBroadcast)
{
	if (!Enabled) return;
...
public Action Timer_RestoreTeleChargeTime(Handle timer, any data)
{
...
public Action Event_Item(Event event, const char[] name, bool dontBroadcast)
{
	if (!Enabled) return;
...
public Action UserMsg_SpawnBird(UserMsg msg_id, Handle bf, const players[], int playersNum, bool reliable, bool init)
{
	...
	if (GetConVarBool(cvarWaterDoves))
	{
		...
		int closestPlayer; float closestDist;
		for (int i = 1; i <= MaxClients; i++)
		{
			if (!IsClientInGame(i)) continue;
		...
		if (!StrContains(cls, "obj_", false))
		{
			if (!GetConVarBool(cvarSlidingTaunt))
			{
				for (int i = 1; i <= MaxClients; i++)
				{
					...
					bool hadOldValue = aSpeed != Address_Null; float oldSpeed;
...
public Action Timer_UndoGestureSpeed(Handle timer, any data)
{
...
{
	int count = GetArraySize(aBuildings); float pos[3]; int team;
	...
	for (int i = 0; i < count; i++)
	{
		...
		float buildpos[3]; float buildmins[3]; float buildmaxs[3];
...
public Action Timer_CheckBackstab(Handle timer, any data)
{
...
{
	if (!Enabled) return;
	bool BazaarHeadsMeter = GetConVarBool(cvarBazaarHeadsMeter); bool EyelanderHeadsMeter = GetConVarBool(cvarEyelanderHeadsMeter); bool HuntsmanWater = GetConVarBool(cvarHuntsmanWater);
	...
	for (int client = 1; client <= MaxClients; client++)
	{
		...
		{
			int heads = GetEntProp(client, Prop_Send, "m_iDecapitations"); int melee = PrevWeapons[client][2]; bool showMeter;
...
public Action Timer_ChangeSoundVolume(Handle timer, any data)
{
	int client = GetClientOfUserId(ReadPackCell(data));
...
public int Query_Viewmodel(QueryCookie cookie, int client, ConVarQueryResult result, const char[] cvarName, const char[] cvarValue)
{
...
public int Query_MedievalThirdperson(QueryCookie cookie, int client, ConVarQueryResult result, const char[] cvarName, const char[] cvarValue)
{
	if (!IsClientInGame(client) || result != ConVarQuery_Okay || !StringToInt(cvarValue)) return;
	if (GetEntProp(client, Prop_Send, "m_nForceTauntCam")) return;
	ShowDeadRingerNotice(client);
...
public int OnConVarChanged(Handle cvar, const char[] oldValue, const char[] newValue)
{
...
stock int SetViewmodelAnimation(int client, int sequence)
{
...
stock int SetAmmo_Weapon(int weapon, int newAmmo)
{
...
stock int ShowDeadRingerNotice(int client)
{
...
stock int IsMedieval(bool bForceRecalc = false)
{

/*
	...
	The-Killer's Edits
			- Fixed team change in DoD:S, should work in most mods now
			- Added team name support for other mods, currently CSS, DODS, HL2DM, looking into INS, PVK, Hidden, Sourceforts, Dystopia
			- Added teamswap for single players
			- Added Check for cstrike on cstrike specific commands(armor helmet nv defuse)
...
{	
	...
	hGameConf = LoadGameConfigFile("plugin.supercmds")
...
public Action:Command_Team(client, args)
{
		...
		for (new i=1; i<=maxplayers; i++)
		{
			...
			if (letter=='C') //assume @CT
			{
				if (GetClientTeam(i)==3)
				{
					...
					ChangeClientTeam(i, teamid)
					PrintToChat(i,"\x01\x04You have been moved to the %s team", teamname[mod][teamid-1])
				}
			}
			else if (letter=='T') //assume @T
			{
	...
	ChangeClientTeam(iClients[0], teamid)

	PrintToChat(iClients[0],"\x01\x04You have been moved to the %s team", teamname[mod][teamid-1])
...
public Action:Command_TeamSwap(client, args)
{
	...
	else if ( args >= 2)
	{
		...
		for (new i =1 ; i<=args; i++) 
		{
			...
			new iClient = SearchForClients(Target, iClients, 2);

...
public OnPluginStart()
{
	...
	g_hCvarAdvert = CreateConVar("sm_hop_advertise", "1", "Set to 1 to enable server advertisements");
...
public OnCvarsChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	{
		g_bAdvert = GetConVarBool(g_hCvarAdvert);
	}
...
{
	decl String:sBuffer[MAX_STR_LEN] = "";
	new j;
...
public OnSocketReceive(Handle:sock, String:data[], const size, any:i)
{
	...
	IntToString(GetByte(data, offset), numPlayers, sizeof(numPlayers));
	offset++;

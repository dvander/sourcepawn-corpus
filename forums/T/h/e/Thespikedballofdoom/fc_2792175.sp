...
#define PLUGIN_VERSION "1.0.4 me natives kartfix"
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_bhopme", CmdBhopEnableMe, ADMINCMD_MIN_LEVEL, "Self-Controlled BHop Settings: 0 off, 1 normal, 2 lazy (hold jump), no value to toggle");
	...
	RegAdminCmd("sm_bounceme", CmdBounceEnableMe, ADMINCMD_MIN_LEVEL, "Self-Controlled Bounce Setting: 0 off, 1 on");
...
public Action:CmdBhopEnableMe(client, args)
{
	if (args < 1) {
		...
		bBhopLazyModes[client] = val == 2;
		
		if (bBhopLazyModes[client]) {
			ReplyToCommand(client, "[SM] You enabled Lazy Mode, hold [Jump] to BHop");
		} else {
			ReplyToCommand(client, "[SM] You %s BHop", bIsAllowedToBhop[client] ? "enabled" : "disabled");
		}
	}
...
public Action:CmdBounceEnableMe(client, args)
{
	if (args < 1) {
		bIsAllowedToBounce[client] =! bIsAllowedToBounce[client];
		ReplyToCommand(client, "[SM] You toggled Bounce %s", bIsAllowedToBounce[client] ? "on" : "off");
	} else {
		new String:arg[4];
		GetCmdArg(1,arg,sizeof(arg));
		new val = StringToInt(arg);
		bIsAllowedToBounce[client] = val != 0;
		ReplyToCommand(client, "[SM] You %s Bounce", bIsAllowedToBounce[client] ? "enabled" : "disabled");
	}
...
public OnPostThink(client)
{
	...
	{			
		...
		if (bBhopEnabled && bIsAllowedToBhop[client] && !bIsInKart[client] && GetTickedTime() - fMomentTouchedGround[client] <= fBhopMaxDelays[client])
		{			
			new Float:fNewVel[3];
	...
	// Check if player is in a kart
	if (TF2_IsPlayerInCondition(client, TFCond_HalloweenKart))
	{
		bIsInKart[client] = true;
	}
	else
	{
		bIsInKart[client] = false;
	}

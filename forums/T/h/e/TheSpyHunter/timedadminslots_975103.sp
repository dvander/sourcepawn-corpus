...
{
	name = "Timed Donator Admin slot",
	author = "Nightbox edited by TheSpyHunter",
	description = "Add a timed admin slot",
...
{
	RegAdminCmd("sm_addreserved", cAddTimedReserved, ADMFLAG_SLAY, "Add timed reserved slot to player (sm_addreserved <userid> <time>)");
	RegAdminCmd("sm_addvip", cAddPremium, ADMFLAG_SLAY, "Add timed premium slot to player (sm_addvip <userid> <time>)");
...
public Action:cAddTimedReserved(client, args)
{
	...
	{
		PrintToChat(client, "Usage: sm_addreserved userid time");
...
public Action:cAddPremium(client, args)
{
	new String:sTarget[32], String:sAmount[32];
	new iTarget, iAmount;
	...
	if (!GetCmdArg(1, sTarget, sizeof(sTarget)))
	{
		PrintToChat(client, "Usage: sm_addvip userid time");
		return Plugin_Handled;
	}
	...
	AddPremiumTime(iTarget, iAmount);
...
AddPremiumTime(client, time) {
	...
	PrintToChat(client, "\x01You have \x04%s\x01 remaining on your VIP slot.", duration);
}
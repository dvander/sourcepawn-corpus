...
{
	RegAdminCmd("sm_addtimedreserved", cAddTimedReserved, ADMFLAG_SLAY, "Add timed reserved slot to player (sm_addtimedreserved <userid> <time>)");
	RegAdminCmd("sm_addpremium", cAddPremium, ADMFLAG_SLAY, "Add timed premium slot to player (sm_addpremium <userid> <time>)");
...
public Action:cAddPremium(client, args)
{
	...
	if (!GetCmdArg(1, sTarget, sizeof(sTarget)))
	{
		PrintToChat(client, "Usage: sm_addpremium userid time");
		return Plugin_Handled;
	}
...
AddPremiumTime(client, time) {
	...
	//PrintToChat(client, "\x01You have \x04%s\x01 remaining on your Premium slot.", duration);
}
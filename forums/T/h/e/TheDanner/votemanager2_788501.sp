...
public hasVoteAccess(client, String:voteName[32])
{
	...
	else if (strcmp(voteName,"PassVote",false) == 0) GetConVarString(passVoteAccess, acclvl, sizeof(acclvl));
...
public Action:PassVote_Handler(client, args)
{
	...
	// special case, if someone does `rcon passvote` instead of just `passvote` then the passvote comes from the server
	// anyone with rcon access would have full access to passvote?
	if (client==0){
	...
	LogVote(client,"failed to pass vote. Reason: Access");

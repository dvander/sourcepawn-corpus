...
ConVar g_EnableWorldModel;
int g_iEnableWorldModel;
int iGroup[66];
int iGloves[66];
float flFloatValue[66];
...
{	
	...
	RegAdminCmd("sm_emotes", Command_Admin_Emotes, ADMFLAG_SLAY, "");
	RegAdminCmd("sm_emote", Command_Admin_Emotes, ADMFLAG_SLAY, "");
	RegAdminCmd("sm_dances", Command_Admin_Emotes, ADMFLAG_SLAY, "");
	RegAdminCmd("sm_dance", Command_Admin_Emotes, ADMFLAG_SLAY, "");
...
{
	g_EnableWorldModel = FindConVar("sm_gloves_enable_world_model");
	if (!g_EnableWorldModel)
	{
		SetFailState("sm_gloves_enable_world_model not found!");
	}
	g_iEnableWorldModel = g_EnableWorldModel.IntValue;
}
...
void StopEmote(int client)
{
	...
	if (iEmoteEnt && iEmoteEnt != INVALID_ENT_REFERENCE && IsValidEntity(iEmoteEnt))
	{
		...
		if (Gloves_IsClientUsingGloves(client))
		{
			int ent = GetEntPropEnt(client, Prop_Send, "m_hMyWearables", 0);
			iGroup[client] = GetEntProp(ent, Prop_Send, "m_iItemDefinitionIndex", 4, 0);
			iGloves[client] = GetEntProp(ent, Prop_Send, "m_nFallbackPaintKit", 4, 0);
			flFloatValue[client] = GetEntPropFloat(ent, Prop_Send, "m_flFallbackWear", 0);
			AcceptEntityInput(ent, "KillHierarchy", -1, -1, 0);
			FixCustomArms(client);
			ent = CreateEntityByName("wearable_item", -1);
			if (ent != -1)
			{
				SetEntProp(ent, Prop_Send, "m_iItemIDLow", -1, 4, 0);
				SetEntProp(ent, Prop_Send, "m_iItemDefinitionIndex", iGroup[client], 4, 0);
				SetEntProp(ent, Prop_Send, "m_nFallbackPaintKit", iGloves[client], 4, 0);
				SetEntPropFloat(ent, Prop_Send, "m_flFallbackWear", flFloatValue[client], 0);
				SetEntPropEnt(ent, Prop_Data, "m_hOwnerEntity", client, 0);
				SetEntPropEnt(ent, Prop_Data, "m_hParent", client, 0);
				if (g_iEnableWorldModel)
				{
					SetEntPropEnt(ent, Prop_Data, "m_hMoveParent", client, 0);
				}
				SetEntProp(ent, Prop_Send, "m_bInitialized", 1, 4, 0);
				DispatchSpawn(ent);
				SetEntPropEnt(client, Prop_Send, "m_hMyWearables", ent, 0);
				if (g_iEnableWorldModel)
				{
					SetEntProp(client, Prop_Send, "m_nBody", 1, 4, 0);
				}
			}
		}		
	} 
...
public Action Command_Admin_Emotes(int client, int args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_emotes <#userid|name> [intger number]");
		return Plugin_Handled;
	}
...
void PerformEmote(int client, int target, int amount)
{
		switch (amount)
		{
					...
					default:
					PrintToChat(target, "invalid number of emote!");
		}
}

...
new Handle:CVAR_JOIN_ANNOUNCE = INVALID_HANDLE;
new Handle:CVAR_SOUND_SPAWN = INVALID_HANDLE;
new Handle:CVAR_INVALID_MESSAGE = INVALID_HANDLE;
new Handle:CVAR_CMD_LOGGING = INVALID_HANDLE;
...
public OnClientAuthorized(client, const String:auth[]) //Printing client's name and country on join
{
	...
	{
		new String:IP[65], String:Country[65];
...
public Action:CMD_Admins(client, args) //Showing all online admins for you
{
	...
	new String:AdminNames[512], String:CurrentName[65];
...
public Action:CMD_Teleport(client, args) //Teleporting a player to another player, or saved location
{
	...
	new String:target1_string[65], String:target2_string[65];
	
	GetCmdArg(1, target1_string, sizeof(target1_string));
	GetCmdArg(2, target2_string, sizeof(target2_string));
	
	new String:target_name[MAX_TARGET_LENGTH], bool:tn_is_ml;
	new target_count, target_list[MAXPLAYERS];
...
public Action:CMD_Team(client, args) //Set player's team
{
	...
	new String:target_string[65], String:amount_string[65];
...
public Action:CMD_Give(client, args) //Give weapons or something to player
{
	...
	new String:target_string[65], String:weapon_string[256];
...
public Action:CMD_Spawn_Chicken(client, args) //Spawning a chicken
{
	...
	new Float:eye_pos[3], Float:eye_ang[3];
	...
	new String:buffer_str[6][5], buffer_int[6];

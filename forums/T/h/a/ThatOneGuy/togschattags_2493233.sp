...
new ga_iChatMsgCnt[MAXPLAYERS + 1] = {0, ...};
new Handle:g_cSpamMsgCnt = INVALID_HANDLE;
...
bool:CheckForSpam(client)	// ADD THIS FUNCTION
{
	if(!IsValidClient(client))
	{
		return false;
	...
	if(g_fSpamDuration)
	{
		...
		CreateTimer(g_fSpamDuration, TimerCB_ReduceMsgCnt, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
		if(ga_iChatMsgCnt[client] > GetConVarInt(g_cSpamMsgCnt))
		{
...
public Action:TimerCB_MsgsTooFast(Handle:hTimer, any:iUserID)	// ADD THIS FUNCTION
{
	new client = GetClientOfUserId(iUserID);
	if(IsValidClient(client))
	{

...
Handle CVAR_CMD_ADMINS = INVALID_HANDLE;
...
char SOUND_SPAWN[512];
float ResetVector[3] = {0.0, 0.0, 0.0}; //This one is always 0, needed for reset the player's velocity
float SAVELOC[MAXPLAYERS + 1][3];

char ValidItems[][] = //VALID WEAPON NAMES HERE
{
...
//-----CLIENT_AUTHORIZED-----//
public void OnClientAuthorized(int client, const char[] auth) //Printing client's name and country on join
{
...
//----------------------------//
//=====NON-ADMIN_COMMANDS=====//
public Action CMD_Admins(int client, int args) //Showing all online admins for you
{
...
//=====ADMIN_COMMANDS=====//
//=========SERVER=========//
public Action CMD_Extend(int client, int args) //Extending or abridge the current map
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
	...
	int amount = StringToInt(amount_string);
...
public Action CMD_ClearMap(int client, int args) //Delete all weapons from ground
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_RestartGame(int client, int args) //Restarting the game
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Valid_Weapons(int client, int args) //Showing valid weapon names
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
	...
	char buffer[1024];
	for(int i = 0; i < sizeof(ValidItems); i++)
	{
		if(StrEqual(buffer, ""))
		{
...
//=========CLIENT=========//
public Action CMD_Teleport(int client, int args) //Teleporting a player to another player, or saved location
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
	...
	char target1_string[65];
	char target2_string[65];
	...
	{
		int target2 = FindTarget(client, target2_string, false, false);
...
public Action CMD_SaveLoc(int client, int args) //Saving current location to your variables for teleport
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
//==========//
public Action CMD_Team(int client, int args) //Set player's team
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Swap(int client, int args) //Swap player's team
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Scramble(int client, int args) //Scramble the two teams
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
//==========//
public Action CMD_Give(int client, int args) //Give weapons or something to player
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Equip(int client, int args) //Disarm player and give another weapon
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Disarm(int client, int args) //Disarming the player
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
//==========//
public Action CMD_Bury(int client, int args) //Bury the player
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_UnBury(int client, int args) //Unbury the player
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Speed(int client, int args) //Modify player's lagged movement speed
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
	...
	float amount = StringToFloat(amount_string);
...
//==========//
public Action CMD_Respawn(int client, int args) //Respawn the player
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_God(int client, int args) //Modify player's god mode
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
//==========//
public Action CMD_Health(int client, int args) //Modify the player's health
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Armour(int client, int args) //Modify the player's armour
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Helmet(int client, int args) //Modify the player's helmet
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
//==========//
public Action CMD_Cash(int client, int args) //Modify the player's cash
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Kills(int client, int args) //Modify the player's kills
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Assists(int client, int args) //Modify the player's assists
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Deaths(int client, int args) //Modify the player's deaths
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_MVPs(int client, int args) //Modify the player's MVPs
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Scores(int client, int args) //Modify the player's scores
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Team_Scores(int client, int args) //Modify the team's scores
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
...
public Action CMD_Spawn_Chicken(int client, int args) //Spawning a chicken
{
	if(!IsClientValid(client) || !IsClientInGame(client))
	{
		return Plugin_Handled;
	...
	Handle trace = TR_TraceRayFilterEx(eye_pos, eye_ang, MASK_SOLID, RayType_Infinite, Filter_DontHitPlayers);
	if(!TR_DidHit(trace))
	{
		return Plugin_Handled;
	}
	...
	char buffer_str[6][5], buffer_int[6];
...
//-----STOCKS-----//
int GivePlayerWeapon(int client, char[] weaponname)
{
	char classname[65];
...
void LogActionEx(int client, char[] message, any ...)
{

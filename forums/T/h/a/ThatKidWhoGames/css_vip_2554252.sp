...
{
	cv_Money = CreateConVar("sm_cssvip_money", "1600", "Money to set on spawn to VIPs.", _, true, 0.0);
	cv_Health = CreateConVar("sm_cssvip_health", "20", "Health to give on spawn to VIPs (adds to default value of health they already have).", _, true, 0.0);
	cv_Armor = CreateConVar("sm_cssvip_armor", "120", "Armor to give on spawn to VIPs.", _, true, 0.0);
...
public Action Command_VIPs(int client, int args)
{
	...
	if (iCount == 0)
	{
		menu.AddItem("novips", "No VIPs are currently on the server!");
...
public Action Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		int health = GetClientHealth(client) + iHealth;
		SetEntityHealth(client, health);
		SetEntData(client, -1, iMoney);

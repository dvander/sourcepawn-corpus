	...
	if (GetConVarBool(cvar_notify) == true)
	{
		for (new i = 1; i <= MaxClients; i++)
		{
			...
			if (GetClientTeam(i) == INFECTEDTEAM)
			{
				...
				//PrintToChat(i, "\x03Tank \x01has appeared with \x04%i \x01HP and \x04%i \x01speed.", tankHP, GetConVarInt(cvar_tank_speed));
				PrintToChat(i, "\x03Tank \x01has appeared with \x04%i \x01HP and \x04%i \x01speed.", GetConVarInt(cvar_tank_health), GetConVarInt(cvar_tank_speed));
			}
		}
	}

...
A list of plugins by me at http://www.sourcemod.net/plugins.php?cat=0&mod=0&title=&author=TnTSCS&description=&search=1
	...
	Change Log
		...
		* Version 1.5.7
			* Added CVar to toggle the forcing of moving to spectate and/or jointeam
				- Helpful if using mp_allowspectators 0 to that if players use these two commands, this plugin will not move them to spectate
				- Requested by Fearts (https://forums.alliedmods.net/member.php?u=43017) here - https://forums.alliedmods.net/showpost.php?p=2193869&postcount=57
	
	
		* Version 1.5.8
			* Added team filter this plugin acts on, defaulted to all teams.
	To Do List
...
new bool:AllowForceSpectateJointeam = true;
...
public OnPluginStart()
{
	...
	AutoExecConfig_SetFile("SuicideIntercept.plugin");
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_enabled", "1", 
	"Is Suicide Intercept Enabled?\n1 for Enabled\n0 for Disabled", _, true, 0.0, true, 1.0)), OnEnabledChanged);
	PluginEnabled = GetConVarBool(hRandom);
	SetAppend(appended);
	
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_blocksuicides", "0", 
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_delaydeathtime", "7", 
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_blockspectate", "0", 
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_delayspectime", "7", 
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_blocksjointeam", "0", 
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_delayjointeamtime", "7", 
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_useupdater", "0", 
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_time", "0", 
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_immunity", "1", 
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_allowforcespecjteam", "1", 
	"Allow the use of force spectate and force jointeam?\nHelpful to set to 0 if you use \"mp_allowspectators 0\"", _, true, 0.0, true, 1.0)), OnAllowForceSpectateJointeamChanged);
	AllowForceSpectateJointeam = GetConVarBool(hRandom);
	SetAppend(appended);
	
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_suicideintercept_teamfilter", "0", 
	"What team should this plugin act upon?\n0 = All teams\n1 = Team 1\n2 = Team 2", _, true, 0.0, true, 2.0)), OnTeamFilterChanged);
	TeamFilter = GetConVarInt(hRandom);
	SetAppend(appended);
...
public Action:Command_InterceptSuicide(client, const String:command[], args)
{
	...
	if (TeamFilter > 0 && (GetClientTeam(client) != TeamFilter))
	{ // Team Filter is enabled, but this player is not on the team the team filter is set to.
		return Plugin_Continue;
	}
...
public OnAllowForceSpectateJointeamChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	AllowForceSpectateJointeam = GetConVarBool(cvar);
}

public OnTeamFilterChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	TeamFilter = GetConVarInt(cvar);
}
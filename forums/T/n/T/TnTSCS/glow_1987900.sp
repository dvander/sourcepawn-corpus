...
new bool:CanUseGlow[MAXPLAYERS+1] = {false, ...};
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_glowset", Command_SmGlow, ADMFLAG_CUSTOM6, "sm_glow <player> <color> - Sets player to glow color.");
	RegAdminCmd("sm_glowcolors", Command_SmGlowColors, ADMFLAG_CUSTOM6, "sm_glowcolors - Lists out the glow colors.");
...
{
	if (!GetConVarBool(g_Cvar_Glow) || !CanUseGlow[client])
...
{
	if (CheckCommandAccess(client, "canuse_glow", ADMFLAG_CUSTOM6))
	{
		CanUseGlow[client] = true;
	}
	else
	{
		CanUseGlow[client] = false;
	}
}

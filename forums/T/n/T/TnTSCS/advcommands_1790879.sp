...
public Plugin:myinfo = 
{
	...
	author = "3sigma | TnTSCS", // aka X@IDER some updates by TnTSCS aKa ClarkKent
...
public OnPluginStart()
{
	...
	if (game == GAME_CSTRIKE || game == GAME_CSGO) HookEvent("round_end",Event_RoundEnd);
...
public Action:Command_Drop(client, args)
{
	...
	else for (new i = 0; i < count; i++)
	{
		...
		for (new j = 0; j < 5; j++)
		{
			...
			{
				if (game == GAME_CSGO || game == GAME_CSTRIKE)
				{
					CS_DropWeapon(t, ent, true);
				}
				else
				{
					DropWeapon(t,ent);

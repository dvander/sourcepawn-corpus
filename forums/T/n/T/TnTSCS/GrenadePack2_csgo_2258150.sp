...
public Action:CS_OnBuyCommand(client, const String:weapon[])
{
	...
	if (!Plugin_Enabled || IsFakeClient(client))// || (PlayerIsVIP[client] && !EnforceBuyLimit))
	{
		return Plugin_Continue;
	}
	
	if (UseTotalNades && (StrContains(weapon, "grenade", false) != -1 || StrEqual(weapon, "flashbang", false) || StrEqual(weapon, "decoy", false)))
	{
		if (!PlayerIsVIP[client])
		{
...
{
	LogMessage("%s", msg);
	PrintToChatAll("[GP2 CS:GO DEBUG] %s", msg);
}
#endif

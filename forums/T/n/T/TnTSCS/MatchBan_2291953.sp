...
Match Ban (previously Match Kick)
	...
	VERSION HISTORY:
		...
		1.1.4	-	Updated timer to include no map change flag
...
{
	CreateConVar("sm_matchban_version", PLUGIN_VERSION, "Version of Match Ban", FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_REPLICATED | FCVAR_NOTIFY | FCVAR_DONTRECORD);
	CreateConVar("sm_matchban_version_build", SOURCEMOD_VERSION, "The version of SourceMod that 'Match Ban' was compiled with.", FCVAR_PLUGIN | FCVAR_DONTRECORD);
...
public OnMapStart()
{
	...
	if (UseDebug)
	{
		LogMessage("OnMapStart is clearing all entries of the Trie");
	}
}
...
public PlayerDisconnect_Event(Handle:event, String:name[], bool:dontBroadcast)
{
	...
	if (UseDebug)
	{
		Format(dmsg, sizeof(dmsg), "%L triggered player_disconnect event", client);
		DebugMessage(dmsg);
	}
	...
	if (IsClientInGame(client) && !IsFakeClient(client) && MB_Is_Enabled)
	{
		...
		GetClientAuthId(client, AuthId_Steam2, authString, 20);
		...
		if (StrEqual(reason, DisconnectString))
		{
			if (BanOnDisconnect == 1)
			{
				if (UseDebug)
				{
					Format(dmsg, sizeof(dmsg), "BanOnDisconnect == 1, setting %L for banning", client);
					DebugMessage(dmsg);
				}
			...
			CreateDataTimer(BanDelay, Timer_BanEnforce, MB_Pack, TIMER_FLAG_NO_MAPCHANGE);
			...
			if (UseDebug)
			{
				Format(dmsg, sizeof(dmsg), "Wrote the following to MP_Pack: UserID[%i], ClientID[%i], SteamID[%s], Reason[%s], and Name[%s]", UserID, client, authString, reason, playername);
				DebugMessage(dmsg);
			}
		}
		else
		{
			if (UseDebug)
			{
				Format(dmsg, sizeof(dmsg), "Disconnect reason [%s] did not match action value of [%s]", reason, DisconnectString);
				DebugMessage(dmsg);
			}
		}
	}
...
public Action:Timer_BanEnforce(Handle:timer, Handle:MB_Pack)
{
	...
	if (GetTrieValue(h_Trie, authString, UserID_Check))
	{
		if (UseDebug)
		{
			Format(dmsg, sizeof(dmsg), "There was a match in the Trie for SteamID [%s].  Setting for ban and removing steam from Trie", authString);
			DebugMessage(dmsg);
		}
...
public MB_BanPlayer(any:UserID, any:client, const String:auth[], const String:reason[], const String:name[], bool:PlayerConnected)
{
	if (PlayerConnected)
	{
		if (IsSBAvailable)
		{
			...
			if (UseDebug)
			{
				Format(dmsg, sizeof(dmsg), "Banned %L by ClientID [%i] using SourceBans", client, client);
				DebugMessage(dmsg);
			}
		}
		else
		{
			...
			if (UseDebug)
			{
				Format(dmsg, sizeof(dmsg), "Banned %L by ClientID [%i] using SourceMod", client, client);
				DebugMessage(dmsg);
			}
		}
	}
	else
	{
		...
		if (UseDebug)
		{
			Format(dmsg, sizeof(dmsg), "Player is not connected.  Banned by SteamID [%s] using SourceMod", auth);
			DebugMessage(dmsg);
		}
	}

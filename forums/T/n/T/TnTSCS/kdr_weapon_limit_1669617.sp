...
public Action:CS_OnBuyCommand(client, const String:weapon[])
{
	...
	if(IsClientInGame(client) && Restricted[client] && GetClientTeam(client) > CS_TEAM_SPECTATOR)
	{
		...
		if(StrEqual(weapon, "vest", false) || StrEqual(weapon, "vesthelm", false) || StrEqual(weapon, "defuser", false) || 
			StrEqual(weapon, "secammo", false) || StrEqual(weapon, "primammo", false) || StrEqual(weapon, "nvgs", false))
		{
		...
		if((PlayerScore[client] >= secondKillLimit && StrContains(WeaponsTwo, weapon, false) == -1) ||
			(PlayerScore[client] >= firstKillLimit && StrContains(WeaponsOne, weapon, false) == -1))
		{
			restrict = true;
		}
...
public Action:Timer_ProcessKDR(Handle:timer, any:client)
{
	...
	if((KDR >= RatioLimit && frags >= firstKillLimit) || frags >= secondKillLimit) // KDR is above or equal to the limit and min kills reached OR second kill limit reached
	{

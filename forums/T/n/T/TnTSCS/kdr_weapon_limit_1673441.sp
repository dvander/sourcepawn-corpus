...
new String:WeaponsOne[MAX_FILE_LEN];
new String:WeaponsTwo[MAX_FILE_LEN];
...
public Plugin:myinfo = 
{
	...
	description = "This plugin will limit a player to a list of weapons once they reach certain criteria",
...
public OnPluginStart()
{
	...
	"KDR Ratio a player must meet before weapon limit is enforced against them.", FCVAR_PLUGIN | FCVAR_SPONLY | FCVAR_REPLICATED | FCVAR_NOTIFY)), OnKDRChanged);
	...
	"Once a player reaches this many kills, regardless of KDR, then they will be limited to use only glock and usp\nSet to 0 to not use this.", _, true, 0.0, true, 100.0)), SecondKillLimitChanged);
	secondKillLimit = GetConVarInt(hRandom);
	
	HookConVarChange((hRandom = CreateConVar("sm_kdrwl_weapons_1", "usp, glock, fiveseven, p228, elite, usp, glock", 
	"List the weapons a player can use once they've reached the kill_limit_1")), WeaponsOneChanged);
	WeaponsOne[0] = '\0';
	GetConVarString(hRandom, WeaponsOne, sizeof(WeaponsOne));
	
	HookConVarChange((hRandom = CreateConVar("sm_kdrwl_weapons_2", "usp, glock", 
	"List the weapons a player can use once they've reached the kill_limit_2")), WeaponsTwoChanged);
	WeaponsTwo[0] = '\0';
	GetConVarString(hRandom, WeaponsTwo, sizeof(WeaponsTwo));
...
public OnMapStart()
{
	...
	{
		LogMessage("Successful precached the sound %s", SOUND_FILE);
	}
...
public OnClientPostAdminCheck(client)
{
	...
	{
		if(CheckCommandAccess(client, "no_kdr_restrict", ADMFLAG_CUSTOM6))
		{
			return;
		}
...
public Action:CS_OnBuyCommand(client, const String:weapon[])
{
	...
	if(IsClientInGame(client) && Restricted[client] && GetClientTeam(client) > CS_TEAM_SPECTATOR)
	{
		...
		if((PlayerRestrictTwo[client] && StrContains(WeaponsTwo, weapon, false) == -1) ||
			(PlayerRestrictOne[client] && StrContains(WeaponsOne, weapon, false) == -1))
		{
			restrict = true;
		}
...
public Action:Timer_ProcessKDR(Handle:timer, any:client)
{
	...
	// KDR is above or equal to the limit and min kills reached OR second kill limit reached
	if((KDR >= RatioLimit && frags >= firstKillLimit) || frags >= secondKillLimit)
	{
...
stock RestrictWeapons(client, msg)
{
	if(IsClientInGame(client) && GetClientTeam(client) > CS_TEAM_SPECTATOR)
	{
		...
		if(weapon != -1)
		{
			...
			if((PlayerRestrictTwo[client] && StrContains(WeaponsTwo, WeaponName, false) == -1) ||
				(PlayerRestrictOne[client] && StrContains(WeaponsOne, WeaponName, false) == -1))
			{
...
public Action:OnWeaponCanUse(client, weapon)
{
	...
	StrEqual(sWeapon, "secammo", false) || StrEqual(sWeapon, "primammo", false) || StrEqual(sWeapon, "nvgs", false) || StrEqual(sWeapon, "c4"))
	{
		return Plugin_Continue;
	}
	
	if((PlayerRestrictTwo[client] && StrContains(WeaponsTwo, sWeapon, false) == -1) ||
		(PlayerRestrictOne[client] && StrContains(WeaponsOne, sWeapon, false) == -1))
	{
...
public WeaponsOneChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	WeaponsOne[0] = '\0';
	GetConVarString(cvar, WeaponsOne, sizeof(WeaponsOne));
}

public WeaponsTwoChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	WeaponsTwo[0] = '\0';
	GetConVarString(cvar, WeaponsTwo, sizeof(WeaponsTwo));
}
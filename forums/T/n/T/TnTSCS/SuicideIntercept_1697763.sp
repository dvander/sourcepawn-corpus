...
A list of plugins by me at http://www.sourcemod.net/plugins.php?cat=0&mod=0&title=&author=TnTSCS&description=&search=1
	...
	Change Log
		...
		* Version 1.5.4
			* Added timer to ignore suicide commands by player for n seconds after player spawn
...
public OnPluginStart()
{
	...
	*/
	...
	"Number of seconds after player spawn to allow them to use a suicide command without interception", _, true, 0.0, true, 180.0)), OnTimeChanged);
...
public Action:Command_InterceptSuicide(client, const String:command[], args)
{
	...
	if(client == 0 || !IsPlayerAlive(client) || IsFakeClient(client) || 
		GetClientTeam(client) < 2 || ClientTimer2[client] != INVALID_HANDLE)
	{
		return Plugin_Continue;
	}

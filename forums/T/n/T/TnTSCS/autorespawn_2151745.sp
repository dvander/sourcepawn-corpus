...
* 	1.4			shavit		Original code
* 
*	1.4a		TnTSCS		Fixed invalid client error in ConVarChanged function/callback
*	>>>>>>>>>>>>>>>>>>		Added client index validation IsValidClient and GetClientTeam 
*	>>>>>>>>>>>>>>>>>>		I did not go through any other part of the code.
* 
* 	1.4b		TnTSCS		Cleaned up code a bit - added better way of dealing with clients in timer callback
* 
*/
#pragma semicolon 1
#include <sourcemod>
...
public Action:Hook(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (gB_Enabled)
	{
		...
		else
		{
			if (gB_Message)
			{
				PrintHintText(client, "You'll respawn in %.1f seconds.", gF_Time);
			}
			
			CreateTimer(gF_Time, Respawn, GetClientSerial(client));
		}
	}
}

...
new Handle:cvar_tank_burn = INVALID_HANDLE;
...
new Handle:cvar_setting_tank_burn_min = INVALID_HANDLE;
new Handle:cvar_setting_tank_burn_max = INVALID_HANDLE;
...
new _tankOriginalBurnDuration = 0;
...
public OnPluginStart()
{
	...
	cvar_tank_burn = FindConVar("tank_burn_duration");
	...
	_tankOriginalBurnDuration = GetConVarInt(cvar_tank_burn);
	...
	cvar_setting_tank_burn_min = CreateConVar("l4d_tank_burn_min", SafeIntToString(_tankOriginalBurnDuration), "Tank's minimum burn duration when randomizing", FCVAR_PLUGIN|FCVAR_NOTIFY);
	cvar_setting_tank_burn_max = CreateConVar("l4d_tank_burn_max", SafeIntToString(_tankOriginalBurnDuration), "Tank's maximum burn duration when randomizing", FCVAR_PLUGIN|FCVAR_NOTIFY);
	...
	RegConsoleCmd("l4d_rtw_debug_tank", Command_DebugSpawnTank);
...
public OnPluginEnd()
{
	...
	SetConVarInt(cvar_tank_burn, _tankOriginalBurnDuration);
...
public Action:Command_DebugSpawnTank(client, args)
{
	...
	if (args == 1)
	{
		...
		FakeClientCommand(client, "z_spawn tank %s", arg);
...
public Action:Command_PrintSettings(client, args)
{
	...
	PrintToConsole(client, "Tank Burn Duration Min: %i", GetConVarInt(cvar_setting_tank_burn_min));
	PrintToConsole(client, "Tank Burn Duration Max: %i", GetConVarInt(cvar_setting_tank_burn_max));
	...
	if (GetConVarBool(cvar_notify) == true)
	{
		for (new i = 1; i <= MaxClients; i++)
		{
			...
			if (GetClientTeam(i) == INFECTEDTEAM)
			{
				...
				//PrintToChat(i, "\x03Tank \x01has appeared with \x04%i \x01HP and \x04%i \x01speed and \x04%i \x01burn duration.", tankHP, GetConVarInt(cvar_tank_speed));
				PrintToChat(i, "\x03Tank \x01has appeared with \x04%i \x01HP and \x04%i \x01speed and \x04%i \x01burn duration.", GetConVarInt(cvar_tank_health), GetConVarInt(cvar_tank_speed), GetConVarInt(cvar_tank_burn));
			}
		}
	}
	
	// Randomize the next tank (if one)
	LogMessage("Tank spawned with HP: %i Speed: %i Burn Duration: %i", GetConVarInt(cvar_tank_health), GetConVarInt(cvar_tank_speed), GetConVarInt(cvar_tank_burn));
	RandomizeTank();
}
...
RandomizeTank()
{
	...
	tank_burn[0] = GetConVarInt(cvar_setting_tank_burn_min)
	tank_burn[1] = GetConVarInt(cvar_setting_tank_burn_max)
	...
	else
	{
		...
		LogMessage("Randomize Tank Current HP: %i Speed: %i Burn Duration: %i", GetConVarInt(cvar_tank_health), GetConVarInt(cvar_tank_speed), GetConVarInt(cvar_tank_burn));
		...
		SetConVarFlags(cvar_tank_burn, GetConVarFlags(cvar_tank_burn) & ~FCVAR_NOTIFY);
		...
		SetConVarInt(cvar_tank_burn, GetRandomInt(tank_burn[0], tank_burn[1]));
		...
		LogMessage("Randomize Tank New HP: %i Speed: %i Burn Duration: %i", GetConVarInt(cvar_tank_health), GetConVarInt(cvar_tank_speed), GetConVarInt(cvar_tank_burn));

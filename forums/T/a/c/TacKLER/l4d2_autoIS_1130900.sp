...
public OnPluginStart()
{
	...
	hSpawnLimits[SI_BOOMER] = CreateConVar("l4d2_ais_boomer_limit", "1", "Max number of boomers per spawn", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_HUNTER] = CreateConVar("l4d2_ais_hunter_limit", "1", "Max number of hunters per spawn", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_SMOKER] = CreateConVar("l4d2_ais_smoker_limit", "1", "Max number of smokers per spawn", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_TANK] = CreateConVar("l4d2_ais_tank_limit", "0", "Max number of tanks per spawn", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_CHARGER] = CreateConVar("l4d2_ais_charger_limit", "1", "Max number of chargers per spawn", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_JOCKEY] = CreateConVar("l4d2_ais_jockey_limit", "1", "Max number of jockeys per spawn", FCVAR_PLUGIN, true, 0.0, true, 14.0);
	hSpawnLimits[SI_SPITTER] = CreateConVar("l4d2_ais_spitter_limit", "1", "Max number of spitters per spawn", FCVAR_PLUGIN, true, 0.0, true, 14.0);
...
public ConVarSpawnLimits(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	new TotalSpawnLimits;
	for (new i = 0; i < NUM_TYPES_INFECTED; i++)
		TotalSpawnLimits += SpawnLimits[i];
	if(TotalSpawnLimits < SILimit)
	{
		Enabled = false;
		SetCvars();
		UnhookEvents();
	...
	{
		if((Enabled) && !(GetConVarBool(hEnabled)))
		{
			SetCvars();
			HookEvents();
			InitTimers();
...
public Action:PresetWeights(client, args)
{
	...
	ReplyToCommand(client, "l4d2_ais_preset <default|none|boomer|smoker|hunter|tank|charger|spitter|jockey>");
	return Plugin_Handled;
}
...
GenerateSpawn(client)
{
	...
	if (SICount < SILimit) //spawn when infected count hasn't reached limit
	{
		...
		//count the nuber of each SI in the spawn queue
		...
		for (new i = 0; i < MAX_INFECTED; i++)
		{
			...
			CheatCommand(client, "z_spawn", Spawns[SpawnQueue[i]]); 
...
SITypeCount() //Count the number of each SI ingame
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		if (GetClientTeam(i)==3)
		{
			...
			{
				case IS_SMOKER:
					SpawnCounts[SI_SMOKER] += 1;
				
				case IS_BOOMER:
					SpawnCounts[SI_BOOMER] += 1;
				
				case IS_HUNTER:
					SpawnCounts[SI_HUNTER] += 1;
				
				case IS_SPITTER:
					SpawnCounts[SI_SPITTER] += 1;
				
				case IS_JOCKEY:
					SpawnCounts[SI_JOCKEY] += 1;
				
				case IS_CHARGER:
					SpawnCounts[SI_CHARGER] += 1;
				
				case IS_TANK:
					SpawnCounts[SI_TANK] += 1;
			}
		}
	}
}

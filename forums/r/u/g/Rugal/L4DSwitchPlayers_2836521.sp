...
*
* Version 1.5:
*		- Added support for tank switching when swapping players.
*		- Fixed swapping players not working.
*		- Removed the signatures as it is no longer needed.
*		- Added Left 4 Dhooks support.
*
* Version 1.6:
*		- Check if target player is valid before performing the switch.
*		- Add a more concise check for full teams.
*  
*********************************************************************************************/
...
public Plugin:myinfo = 
{
	...
	author = "SkyDavid (djromero), Ferks-FK",
...
public OnPluginStart()
{
	...
	CreateConVar("l4d_switchplayers_version", PLUGIN_VERSION, "Version of L4D Switch Players plugin", FCVAR_SS_ADDED|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
PerformSwitch(client, target, team, bool:silent)
{
    ...
    {
        PrintToChat(client, "[SM] The target player is not available anymore.");
        return;
    }
    ...
    if (GetClientTeam(target) == 3)
    {
        ...
        if (GetEntProp(target, Prop_Send, "m_zombieClass") == 8) {
            ...
            {
                L4D2Direct_SetTankTickets(target, 0); // Reset tank tickets.
                L4D2Direct_SetTankTickets(newTank, 100); // Give tank tickets to new tank.
                L4D_ReplaceTank(target, newTank); // Replace tank.
                L4D_SetTankFrustration(target, 0); // Reset tank frustration.
            }
        }
    }
    ...
    if (team == 2)
    {
        ...
        if (bot > MaxClients)
        {
            PrintToChat(client, "[SM] No bots available to take over.");
            return;
        }

        ChangeClientTeam(target, 0);
        
        L4D_SetHumanSpec(bot, target); // force player to spec humans
        L4D_TakeOverBot(target); // force player to take over bot
    }
...
int FindNewTankPlayer()
{
    int candidates[MAXPLAYERS+1];
    int candidateCount = 0;
    
    // Find players that are alive, in the infected team to control the tank.
    for (int i = 1; i <= MaxClients; i++)
    {
        if (IsClientInGame(i) && IsPlayerAlive(i) && GetClientTeam(i) == 3 
            && !IsFakeClient(i) && GetEntProp(i, Prop_Send, "m_zombieClass") != 8)
        {
            candidates[candidateCount++] = i;
        }
    }
    
    // If we have candidates, return a random one.
    if (candidateCount > 0)
    {

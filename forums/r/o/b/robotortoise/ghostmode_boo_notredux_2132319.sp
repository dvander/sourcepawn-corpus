...
{
	name = "[TF2] Ghost Mode",
	author = "FlaminSarge, MasterOfTheXP"
...
#define BOO_MODEL "models/mario/boo.mdl"
#define BOO_SOUND "mario/boo/floating.mp3"
...
new Handle:hSoundTimer[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_ghost", Cmd_Menu, 0);
...
public OnMapStart()
{
	...
	PrecacheModel(BOO_MODEL, true);
	PrecacheSound(BOO_SOUND, true);
}
...
/*public GhostModeMenu(iClient)
{
	...
	new String:strInfo[128], String:cookie[32];
	GetClientCookie(client, hCookieGhost, cookie, sizeof(cookie));
	if(g_bGhost[iClient])
	{
	...
	SendPanelToClient(hPanel, iClient, GhostModeMenu_Handler, MENU_TIME_FOREVER);
	CloseHandle(hPanel);
}
...
//	SetEntProp(client, Prop_Send, "m_nSolidType", 0);
	...
	SetVariantString(BOO_MODEL);
	AcceptEntityInput(client, "SetCustomModel");
	hSoundTimer[client] = CreateTimer(0.1, Timer_BooSound, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
	return Plugin_Handled;
}
public Action:Timer_BooSound(Handle:timer, any:uid)
{
	new client = GetClientOfUserId(uid);
	if (!client) return;
	if (timer != hSoundTimer[client]) return;
	new deadClients[MAXPLAYERS + 1], numDead;
	for (new i = 1; i <= MaxClients; i++)
	{
		if (!IsClientInGame(i)) continue;
		if (IsFakeClient(i)) continue;
		if (IsPlayerAlive(i)) continue;
		deadClients[numDead++] = i;
	}
	EmitSound(deadClients, numDead, BOO_SOUND, client);
	hSoundTimer[client] = CreateTimer(1.5, Timer_BooSound, uid, TIMER_FLAG_NO_MAPCHANGE);
}
...
public Action:OnSetTransmit(ent, client)
{
	return (bDead[ent] > 0 && !bDead[client]) ? Plugin_Handled : Plugin_Continue;
}

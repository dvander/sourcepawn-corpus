...
public void OnConfigsExecuted(/*void*/)
{
    ...
    if(SQL_CheckConfig(AMMOPACK_BLOCK_DATABASE))
    {
        ...
        if(MySQLDataBase)
        {
			Format(sRequest, sizeof(sRequest), "CREATE TABLE IF NOT EXISTS `zombieplague_database` (`id` int(64) NOT NULL auto_increment, `steam_id` varchar(32) NOT NULL, `money` int(64) NULL, `level` int(64) NULL, `exp` int(64) NULL, `humanclass` int(64) NULL, `zombieclass` int(64) NULL, PRIMARY KEY (`id`), UNIQUE KEY `steam_id` (`steam_id`))");
		}
		else
		{
    ...
    if(clientIndex)
    {
        ...
        else
        {
            ...
            if(SQL_FetchRow(hResult))
            {
                ...
                ZP_SetClientHumanClass(clientIndex, SQL_FetchInt(hResult, 4));

                // Set client zombie class
                ZP_SetClientZombieClass(clientIndex, SQL_FetchInt(hResult, 5));

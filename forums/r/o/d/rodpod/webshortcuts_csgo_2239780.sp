...
public Plugin:myinfo = 
{
	...
	author = "James \"sslice\" Gray and Franc1sco franug",
...
public Action:OnSay( client, args )
{
	...
	//sprawdza zmienna i przechodzi do bloku ClientConVar
...
public ClientConVar(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName[], const String:cvarValue[])
{
	...
	{
		PrintToChat(client, "---------------------------------------------------------------");
		PrintToChat(client, "Zeby zobaczyc ta strone musisz zmienic wartosc zmiennej:");
		PrintToChat(client, "%s z %s na 0", cvarName, cvarValue);
		PrintToChat(client, "---------------------------------------------------------------");
	}
}
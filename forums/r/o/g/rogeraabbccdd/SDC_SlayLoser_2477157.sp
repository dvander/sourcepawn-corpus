...
#define FCVAR FCVAR_NOTIFY|FCVAR_REPLICATED|FCVAR_DONTRECORD
//===================================================================================================================
...
stock ForcePlayerSuicide2(client,const eun = 0){
	...
	if(sd_SlayLoser_StopWalk) 		SetEntPropFloat(client, Prop_Data, "m_flLaggedMovementValue", 0.0);
...
public MagicProp(client,Float:pos[3],String:model[],num){
	...
	*/
	...
	SetEntPropEnt(ent, Prop_Send, "m_hOwnerEntity", client);
	SetEntData(ent, FindSendPropInfo("CBasePlayer", "m_fFlags"), FL_CLIENT|FL_ATCONTROLS, 4, true);
	SetEntPropEnt(ent, Prop_Data, "m_hLastAttacker", client);
	SetVariantString("!activator");
	AcceptEntityInput(ent, "SetParent", client);
...
public Action:BirdStop(Handle: timer, any: client){ SetEntPropFloat(client, Prop_Data, "m_flLaggedMovementValue", 0.0);}
...
public Action:CommandListener(client, const String:command[], argc){
	...
	if(g_bRoundStarted || SlayLoserKill[client]) ClientKill(client)
		...
		for(new i = 1; i < GetMaxClients();i++){
			if(!IsClientInGame(i)) continue;
			PrintToChat(i,"\x01[SDC] \x04%N\x01 %T.",client,"cmdSuicide",LANG_SERVER)
		}
...
public Action:CommandListener2(client, const String:command[], argc){
	...
	if(IsClientSourceTV(client)) return Plugin_Continue
	if(!IsClientInGame(client)) return Plugin_Continue

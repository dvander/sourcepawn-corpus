...
public OnPluginStart()
{
	...
	g_hConVar_RoundEnd     = CreateConVar("sm_instantrespawn_roundned",    "0",   "Enable/Disable Instant Respawn after round ending.", FCVAR_NOTIFY|FCVAR_PLUGIN);
...
{
	if (!g_bMaxPlayers && !g_bRoundEnd && GetClientCount() > GetConVarInt(g_hConVar_MaxPlayers))
	{
		g_bMaxPlayers = true;
	}
}
...
{
	if (GetConVarBool(g_hConVar_Enabled) && !g_bRoundEnd && (GetConVarInt(g_hConVar_MaxPlayers) <= 0 || !g_bMaxPlayers))
	{
...
{
	if (GetConVarBool(g_hConVar_Enabled) && (!GetConVarBool(g_hConVar_RoundEnd) && g_bRoundEnd))
	{
		g_bRoundEnd = false;
	}
}

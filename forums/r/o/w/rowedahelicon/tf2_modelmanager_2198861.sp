...
#define FLAG_HIDE_HALLOWEEN		    (1 << 9) 		//Buggy days? _:(´□`」 ∠):_
...
Item_Equip(client, iItem)
{
	...
	if (IsPlayerAlive(client))
	{
		...
		if (g_iItemFlags[iItem] & FLAG_HIDE_HALLOWEEN) RemoveHalloween(client);
	}
}
...
Item_ParseFlags(String:strFlags[])
{
	...
	if (StrContains(strFlags, "HIDE_HALLOWEEN", false)          != -1)  Flags |= FLAG_HIDE_HALLOWEEN;
...
stock RemoveHalloween(client, bool:unhide = false)
{
	new edict = MaxClients+1;
	while((edict = FindEntityByClassnameSafe(edict, "tf_wearable")) != -1)
	{
		...
		{
			new idx = GetEntProp(edict, Prop_Send, "m_iItemDefinitionIndex");
			if (idx == 5617 || idx == 5618 || idx == 5619 || idx == 5620 || idx == 5621 || idx == 5622 || idx == 5623 || idx == 5624 || idx == 5625 && GetEntPropEnt(edict, Prop_Send, "m_hOwnerEntity") == client)
			{

...
public OnPluginStart() {
	...
	HookConVarChange(FindConVar("mp_gamemode"), OnConVarChange_SetVersusMode)	
...
public OnConVarChange_SetVersusMode(Handle:convar, const String:oldValue[], const String:newValue[]) {
	...
	if (!StrEqual(mode, "versus") && ForceVersusOnMapStart) {
		ForceVersus(0, true);
		StripAndChangeServerConVarS("mp_gamemode", "versus");
...
public Action:Event_BossSpawnsSet(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	new String:mode[16];
	GetConVarString(FindConVar("mp_gamemode"), mode, sizeof(mode))
	if (GetConVarBool(IdenticalBosses) && StrEqual(mode, "versus") && !GetConVarBool(FindConVar("versus_boss_spawning")) && !BossSpawnsSet) {
...
ForceVersus(client, bool:value) {
	new String:command[] = "mp_gamemode";

	if (value == false) {
		/* Disable the hook with prevents tampering otherwise this won't work. */
		ForceVersusOnMapStart = false
		StripAndChangeServerConVarS(command, OldMode)
		StripAndChangeServerConVarInt("z_spawn_safety_range", 550)
		LogAction(client, -1, "(%L) set %s to %s", client, command, OldMode);
		MoveInfectedToSpectator()
		} else {
		GetConVarString(FindConVar("mp_gamemode"), OldMode, sizeof(OldMode))
		StripAndChangeServerConVarS(command, "versus")
		StripAndChangeServerConVarInt("z_spawn_safety_range", 200)
		LogAction(0, -1, "(%L) set %s to %i", 0, "z_spawn_safety_range", 200);
		LogAction(client, -1, "(%L) set %s to %s", client, command, "versus");
	}
	if (GetConVarBool(NotifyPlayers) == true) {	
		if (value == false) {
...
public Action:Menu_Versus(client, args) {
	...
	new String:mode[16];
	GetConVarString(FindConVar("mp_gamemode"), mode, sizeof(mode))
	if (!StrEqual(mode, "versus")) { AddMenuItem(menu, "fv", "Force a versus game"); } else { AddMenuItem(menu, "fv", "Disable versus mode"); }
...
/* Added a method to change string based cvars */
StripAndChangeServerConVarS(String:command[], String:value[]) {
	new flags = GetCommandFlags(command);
	SetCommandFlags(command, flags & ~FCVAR_CHEAT);
	SetConVarString(FindConVar(command), value);
	SetCommandFlags(command, flags);
}

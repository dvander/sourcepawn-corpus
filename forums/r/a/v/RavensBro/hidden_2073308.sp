...
public OnGameFrame() {
    ...
    LOOP_CLIENTS(client, CLIENTFILTER_INGAMEAUTH|CLIENTFILTER_ALIVE|CLIENTFILTER_NOSPECTATORS) {
        if (client==hidden) {
...
public Action:player_death(Handle:event, const String:name[], bool:dontBroadcast) {
    ...
    if (victim==hidden) {
        ...
        if (IsValidClient(attacker) && attacker!=hidden) {
        ...
        if (IsValidClient(hidden) && attacker==hidden) {
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon) {
    ...
    if (client==hidden) {
        ...
        if(hiddenStick && hiddenStamina<HIDDEN_STAMINA_TIME-0.5){
			if(buttons & IN_FORWARD || buttons & IN_BACK || buttons & IN_MOVELEFT || buttons & IN_MOVERIGHT || buttons & IN_JUMP > 0){
				HiddenUnstick();
			}
		}
		
        if(hiddenAway && (buttons & IN_FORWARD || buttons & IN_BACK || buttons & IN_MOVELEFT || buttons & IN_MOVERIGHT || buttons & IN_JUMP > 0)
		)
...
stock SelectHidden() {
    ...
    if (forced) {
        ...
        hidden = Client_GetRandom(CLIENTFILTER_INGAMEAUTH|CLIENTFILTER_NOSPECTATORS);
    }
...
stock GiveHiddenPowers(i) {
    ...
    new Handle:hItem = TF2Items_CreateItem(OVERRIDE_ALL | PRESERVE_ATTRIBUTES);
	
    {	
	TF2Items_SetClassname(hItem, "tf_weapon_knife");
	TF2Items_SetItemIndex(hItem, 4);	
	TF2Items_SetLevel(hItem, 100);
	TF2Items_SetQuality(hItem, 10);
	TF2Items_SetNumAttributes(hItem, 1);
	TF2Items_SetAttribute(hItem, 0, 1, 2.0);	
	}
	
    new weapon = TF2Items_GiveNamedItem(i, hItem);
    CloseHandle(hItem);
...
stock Client_RespawnAll() {
    LOOP_CLIENTS(client, CLIENTFILTER_INGAMEAUTH|CLIENTFILTER_NOSPECTATORS) {
        CreateTimer(0.1, Timer_Respawn, client);
    }
}
...
stock bool:HiddenBoo() {
    ...
    for(new i=1;i<=MaxClients;++i){
		...
		if(GetVectorDistance(pos, pos2, true)>300.0*300.0){
			continue;
		}
    ...
    EmitSoundToAll(HIDDEN_BOO_FILE, SOUND_FROM_PLAYER, _, _, _, _, _, hidden, eye);

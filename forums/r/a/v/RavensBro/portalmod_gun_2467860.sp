...
#define PLUGIN_NAME		 	"PortalMod"
#define PLUGIN_AUTHOR	   	"Erreur 500, edit by RavensBro"
#define PLUGIN_DESCRIPTION	"Play tf2 with portalgun !"
#define PLUGIN_VERSION	  	"1.0.4"
#define PLUGIN_CONTACT	  	"erreur500@hotmail.fr"

#define CROSSHAIRS	"materials/vgui/crosshairs/portal_crosshairs.vmt"
#define PORTAL		"models/weapons/portalgun/portalv3.mdl"
...
new bool:IsPlayer[MAXPLAYERS+1]		= {false, ...};		// Is player have the PortalGun
new bool:PlayerClass[9]				= {true, ...};
...
public OnPluginStart()
{
	...
	CreateConVar("portalgun_version", PLUGIN_VERSION, "portalgun version", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	cvarEnabled  = CreateConVar("portalgun_enable", 	"1", 	"Enable or disable portalgun ?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarBreakable = CreateConVar("portalgun_breakable", "1", 	"Projectiles can destroy portals.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarPortalHealth = CreateConVar("portalgun_health", "500", 	"Amount of portal health.", FCVAR_PLUGIN, true, 0.0);
	cvarPerLife	  = CreateConVar("portalgun_per_life",	"1", 	"Destroy player portals when he dies", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarOnlyMine  = CreateConVar("portalgun_only_mine",	"1", 	"Player can only use their own portals", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarWall  	  = CreateConVar("portalgun_wall",		"1.0", 	"Pourcentage (1.0 to 0.1) to check if the wall is plane. (0.0 = no check)", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarProp  	  = CreateConVar("portalgun_prop",		"1", 	"Can spawn portal on prop_dynamic and prop_physics?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarClass[0]  = CreateConVar("portalgun_scout", 	"1", 	"Can scout receive the portalgun?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarClass[1]  = CreateConVar("portalgun_sniper", 	"1", 	"Can sniper receive the portalgun?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarClass[2]  = CreateConVar("portalgun_soldier", 	"1", 	"Can soldier receive the portalgun?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarClass[3]  = CreateConVar("portalgun_demoman", 	"1", 	"Can demoman receive the portalgun?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarClass[4]  = CreateConVar("portalgun_medic", 	"1", 	"Can medic receive the portalgun?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarClass[5]  = CreateConVar("portalgun_heavy", 	"1", 	"Can heavy receive the portalgun?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarClass[6]  = CreateConVar("portalgun_pyro", 		"1", 	"Can pyro receive the portalgun?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarClass[7]  = CreateConVar("portalgun_spy", 		"1", 	"Can spy receive the portalgun?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	cvarClass[8]  = CreateConVar("portalgun_engineer", 	"1", 	"Can engineer receive the portalgun?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	
#if defined _steamtools_included 
	cvarGameDesc = CreateConVar("portalgun_game_desc", 	"1", 	"Change game description as PortalMod ?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
#endif

	RegConsoleCmd("portalgun", GivePortalGun, "Drop Portal Gun");
	
	AutoExecConfig(true, "portalmod_configs");
	
	for(new i=0; i<9; i++)
	PlayerClass[i] = GetConVarBool(cvarClass[i]);
...
public CallBackCVarClassScout(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	...
	PlayerClass[0] = Value ? true:false;
...
{
	new Value = StringToInt(newVal);
	PlayerClass[1] = Value ? true:false;
...
{
	new Value = StringToInt(newVal);
	PlayerClass[2] = Value ? true:false;
...
{
	new Value = StringToInt(newVal);
	PlayerClass[3] = Value ? true:false;
...
{
	new Value = StringToInt(newVal);
	PlayerClass[4] = Value ? true:false;
...
{
	new Value = StringToInt(newVal);
	PlayerClass[5] = Value ? true:false;
...
{
	new Value = StringToInt(newVal);
	PlayerClass[6] = Value ? true:false;
...
{
	new Value = StringToInt(newVal);
	PlayerClass[7] = Value ? true:false;
...
{
	new Value = StringToInt(newVal);
	PlayerClass[8] = Value ? true:false;
...
{	
	...
	AddFileToDownloadsTable("materials/models/weapons/portalgun/pcenter.vmt");
	AddFileToDownloadsTable("materials/models/weapons/portalgun/pcenter.vtf");
	AddFileToDownloadsTable("materials/models/weapons/portalgun/pcenter_blue.vmt");
	AddFileToDownloadsTable("materials/models/weapons/portalgun/pcenter_blue.vtf");
	AddFileToDownloadsTable("materials/models/weapons/portalgun/pmaterial.vmt");
	AddFileToDownloadsTable("materials/models/weapons/portalgun/pmaterial.vtf");
	AddFileToDownloadsTable("materials/models/weapons/portalgun/pmaterial_blue.vmt");
	AddFileToDownloadsTable("materials/models/weapons/portalgun/pmaterial_blue.vtf");	
	
	AddFileToDownloadsTable("models/weapons/portalgun/portalv3.dx80.vtx");
	AddFileToDownloadsTable("models/weapons/portalgun/portalv3.dx90.vtx");
	AddFileToDownloadsTable("models/weapons/portalgun/portalv3.mdl");
	AddFileToDownloadsTable("models/weapons/portalgun/portalv3.phy");
	AddFileToDownloadsTable("models/weapons/portalgun/portalv3.sw.vtx");
	AddFileToDownloadsTable("models/weapons/portalgun/portalv3.vvd");
	
	PrecacheModel(PORTAL);
	
    // CROSSHAIRS ---

	AddFileToDownloadsTable("materials/vgui/crosshairs/portal_crosshairs.vmt");
	AddFileToDownloadsTable("materials/vgui/crosshairs/portal_crosshairs.vtf");
	
	PrecacheGeneric(CROSSHAIRS);
	...
	AddFileToDownloadsTable("sound/portalmod_gun/weapons/portalgun/portal_open0.mp3");	
	...
	PrecacheSound("portalmod_gun/weapons/portalgun/portal_open0.mp3");	
	...
	PrecacheSound("portalmod_gun/weapons/portalgun/portal_open52.mp3");	
...
public OnConfigsExecuted()
{
	if(GetConVarInt(cvarGameDesc))
	{
		...
		Format(gameDesc, sizeof(gameDesc), "PortalMod %s", PLUGIN_VERSION);
...
stock bool:CanBePlayer(client)
{
	...
	{
	    if(!PlayerClass[TFClassTypeToInt(Class) -1])
        PrintToChat(client, "[PORTALMOD] This class is not allowed to receive a PortalGun!");
        return PlayerClass[TFClassTypeToInt(Class) -1];
	}
...
public Action:EventPlayerchangeclass(Handle:hEvent, const String:strName[], bool:bHidden)
{
	...
	{
		if(!PlayerClass[TFClassTypeToInt(TFClassType:Class) -1])
		{
			PrintToChat(client, "[PORTALMOD] This class is not allowed to receive a PortalGun!");
...
public Action:OnWeaponSwitch(client, weapon)
{
	...
	if(Class >= TFClass_Scout && Class <= TFClass_Engineer)
	{	
		if(!PlayerClass[TFClassTypeToInt(Class) -1]) // Class not allowed => remove Portalgun
		{
			PrintToChat(client, "[PORTALMOD] This class is not allowed to receive a PortalGun!");
			RemovePortalGun(client); // Remove player Portalgun
			return Plugin_Continue;
...
{	
	...
	if(IsPlayer[client]) // If currently Player, remove portalgun
	{
		RemovePortalGun(client);
	}
	else // Set player as Player
	{
		if(!CanBePlayer(client)) return;
...
{	
	...
	SetEntData(WeaponModel, FindSendPropOffs("CBaseCombatWeapon", "m_iWorldModelIndex"), g_iViewModelp, 4, true);
	SetEntData(WeaponModel, FindSendPropOffs("CBaseEntity", "m_nModelIndex"), g_iViewModelp, 4, true);
	...
	CreateTimer(0.001, TimerPortal, client);
}
...
RemovePortalGun(client)
{
	...
	PlayerInitialisation(client);	 // Initialize player data and remove portal entities
...
{	
	...
	for(new client=1; client <= MaxClients; client++)
	{
		if(IsValidClient(client) && IsClientInGame(client) && !IsClientSourceTV(client) && (PlayerTeam[client] == 1 || PlayerTeam[client] == 0))
		{
			...
			{
				if(IsValidClient(i) && IsClientInGame(i) && IsClientConnected(i) && IsPlayer[i])
				{
...
{
	if(!IsValidAimOrigin(Pos, client, Type))	//Isn't valid wall to spawn portal
	{	
	...
	{
		SetEntProp(pEnt, Prop_Send, "m_nSkin", Type+0);
		
		if(Type == 0)
			SetEntProp(EntRefToEntIndex(g_Portal[client][1][ID]), Prop_Send, "m_nSkin", GetEntType(g_Portal[client][1][ID]) +0);
		else
			SetEntProp(EntRefToEntIndex(g_Portal[client][0][ID]), Prop_Send, "m_nSkin", GetEntType(g_Portal[client][0][ID]) +0);
	}
	...
	new String:Adress[64];
	Format(Adress, sizeof(Adress), "portalmod_gun/weapons/portalgun/portal_open%i.mp3", GetRandomInt(0, 3));
	...
	CreateTimer(0.2, Timer_Cooldown, client);	
	
	return;
}

bool:IsValidAimOrigin(Float:Origin[3], client, Type)		// Test portal's wall if it's a valid zone 
{
...
bool:IsValidPortalDistance(client, Float:Origin[3], Float:Angle[3], bool:Horizontal=false, bool:FloorDirection=false)
{
	...
	if(!FloorDirection && Horizontal && GetVectorDistance(EndPos, Origin, false) < (100.0 * c_wall) )
		return false;
	else if(!FloorDirection && !Horizontal && GetVectorDistance(EndPos, Origin, false) < (50.0 * c_wall) )
		return false;
	else if(FloorDirection && GetVectorDistance(EndPos, Origin, false) > (1.0 / c_wall) )
		return false;
	...
	trace = TR_TraceRayFilterEx(Origin, Angles, MASK_SHOT, RayType_Infinite, TraceEntityFilterPlayer, client);
...
tpPlayer(client, Pt_client, type, ID_Portal_A)
{
	...
	if(g_Portal[Pt_client][type][vec_Angle][0] > 268 )	//Sky direction
		Radius = 50;
	else if(g_Portal[Pt_client][type][vec_Angle][0] > 88.0 && g_Portal[Pt_client][type][vec_Angle][0] < 92.0)	//Floor direction
		Radius = 120;
	...
	ExitVelo[2] += Radius == 100 ? 3000.0 : 2.2; // sky direction, give speed sky direction bonus to do bigger jump.
	...
	{		
		...
		EmitAmbientSound("portalmod_gun/weapons/portalgun/portal_fizzle2.mp3", Pos, SOUND_FROM_WORLD, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, 0.0);			

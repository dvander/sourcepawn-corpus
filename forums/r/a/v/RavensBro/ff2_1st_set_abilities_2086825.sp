...
public Plugin:myinfo = {
	...
	author = "RainBolt Dash, edit by RavensBro",
...
public Action:FF2_OnAbility2(index,const String:plugin_name[],const String:ability_name[],action)
{
	...
	//slot 0
	    else if (!strcmp(ability_name,"rage_new_cloneattack"))
		Rage_New_CloneAttack(ability_name,index);		    //new Seeldier's Clone Attack
	    else if (!strcmp(ability_name,"rage_cloneattack"))
		Rage_CloneAttack(ability_name,index);               //old Seeldier's Clone Attack
...
/** Clone Attack
1 native class to spawn as
2 string path to model
3 string message to display to client
4 float max ratio of players to respawn (of alive players)
5 string weapon classname (tf_weapon_xxx)
6 native item definition index
7 string ; deliminated attribute number / value
**/
//Seeldier's clone attack rage new
Rage_New_CloneAttack(const String:ability_name[],index)
{
	new Boss=GetClientOfUserId(FF2_GetBossUserId(index));
	decl weapon;
	decl Float:pos[3];
	...
	for(new i; i<dead && i<maxspawn; i++)
	{
		...
		SDKHook(client, SDKHook_OnTakeDamage, OnTakeDamage_New_SaveMinion);
		CreateTimer(4.0,Timer_New_Enable_Damage,userid);
...
public Action:Timer_New_Enable_Damage(Handle:hTimer,any:userid)
{
	...
	if (client>0)
	{
		...
		FF2_SetFF2flags(client,FF2_GetFF2flags(client) & ~FF2FLAG_ALLOWSPAWNINBOSSTEAM);
		SDKUnhook(client, SDKHook_OnTakeDamage, OnTakeDamage_New_SaveMinion);
	}
	return Plugin_Continue;
}

//If Seeldier's minion was spawned in a pit.
public Action:OnTakeDamage_New_SaveMinion(client, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3])
{
...
//Seeldier's clone attack rage old
...
public Action:OnTakeDamage_SaveMinion(client, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3])
{
	if (attacker>MaxClients)
	{
		...
		if (GetEdictClassname(attacker, s, 64) && !strcmp(s, "trigger_hurt", false))
		{
			...
			while (RedAlivePlayers && (!IsClientInGame(target) || (GetClientTeam(target)==BossTeam) || !IsPlayerAlive(target)));

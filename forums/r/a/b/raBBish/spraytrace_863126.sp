...
public Action:RemoveSpray(client, args) {
	...
	if (args == 1) {
		...
		SprayDecal(target, 0, vEndPos);

		SprayTrace[ target ][0] = 0.0;
		SprayTrace[ target ][1] = 0.0;
		SprayTrace[ target ][2] = 0.0;
		
		PrintToChat(client, "[Spray Trace] %t", "Spray Removed", SprayName[target], SprayID[target], adminName);
		LogAction(client, -1, "[Spray Trace] %t", "Spray Removed", SprayName[target], SprayID[target], adminName);
		AdminMenu(client, target);
		
		return Plugin_Handled;
	} else if(GetPlayerEye(client, pos)) {
		new String:adminName[32];
...
public Action:AdminSpray(client, args) {
	...
	if(IsClientConnected(client) && IsClientInGame(client) && IsClientConnected(target) && IsClientInGame(target))
	{
...
stock bool:GetPlayerEye(client, Float:pos[3]) {
	...
	TR_TraceRayFilter(vecClientEyePos, vecClientEyeAng, MASK_SOLID, RayType_Infinite, TraceEntityFilter, client);
	...
	if (TR_DidHit(INVALID_HANDLE))
	{
		...
		if (strcmp("worldspawn", edictname, false)==0)
		{
...
public bool:TraceEntityFilter(entity, contentsMask, any:data) {
	...
	{
		return false;
	} else if(strcmp("worldspawn", edictname, false)!=0)
	{
		return false;
	}
	return true;

...
new Handle:cvar_dm_smart_spawning;
...
// Previous Spawn of Player ID Tracking
new Handle:spawnTracker;
...
public OnPluginStart()
{
	...
	cvar_dm_smart_spawning = CreateConVar("dm_smart_spawning", "1", "Spawn players using riptyde4's changes to the system");
	...
	HookConVarChange(cvar_dm_smart_spawning, Event_CvarChange);
...
public OnMapStart()
{
	...
	// Erase old spawn tracker map & create a new one
	if(spawnTracker != INVALID_HANDLE){
		if(CloseHandle(spawnTracker))
			spawnTracker = INVALID_HANDLE;
	}
	spawnTracker = CreateTrie();
}
...
public Action:Timer_WelcomeMsg(Handle:timer, any:client)
{
	if (IsClientInGame(client) && !IsFakeClient(client))
	{
		...
		CPrintToChat(client, "[\x04WELCOME\x01] This server is running \x04Deathmatch \x01v2.0.3 utilizing smart spawning!");
	}
	return Plugin_Stop;
}
...
LoadConfig()
{
	...
	KvGetString(keyValues, "smart_spawning", value, sizeof(value), "1");
	SetConVarString(cvar_dm_smart_spawning, value);
...
UpdateState()
{
	...
	smartSpawning = GetConVarBool(cvar_dm_smart_spawning);
	...
	if (enabled)
	{
		...
		if (validAttacker)
		{
			...
			if ((knifed && (HPPerKnifeKill > 0)) || (!knifed && (HPPerKill > 0)) || (headshot && (HPPerHeadshotKill > 0)) || (!headshot && (HPPerKill > 0)))
			{
				...
				{
					if (knifed)
						PrintHintText(attackerIndex, "+%iHP %t", HPPerKnifeKill, "HP Knife Kill");
					else if (headshot)
						PrintHintText(attackerIndex, "+%iHP %t", HPPerHeadshotKill, "HP Headshot Kill");
					else if (nade)
						PrintHintText(attackerIndex, "+%iHP %t", HPPerNadeKill, "HP Nade Kill");
					else
						PrintHintText(attackerIndex, "+%iHP %t", HPPerKill, "HP Kill");
				}
			}
		}
...
MovePlayer(client)
{
	...
	if(smartSpawning){
		spawnPoint = getSafestSpawn(client);
		if(spawnPoint != -1){
			CPrintToChat(client, "[\x04DM\x01] Found that spawnpoint %i is the safest spawn point, spawning...", spawnPoint);
			spawnPointFound = true;
		}
		else{
			CPrintToChat(client, "[\x04DM\x01] Failed to find a safe spawn point! Choosing a random one...", spawnPoint);
		}
	}
	if (!smartSpawning && lineOfSightSpawning)
	{
	...
	if (spawnPointFound)
	{
		// Add the spawnpoint to the trie so no player will be spawned at the same point twice in a row
		new String:sclient[2];
		IntToString(client, sclient, sizeof(sclient));
		if(!SetTrieValue(spawnTracker, sclient, spawnPoint, true)){
			CPrintToChat(client, "Failed to update spawn point tracker. Please notify the server admin!");
		}
		new sp;
		GetTrieValue(spawnTracker, sclient, sp);
...
bool:IsPointSuitableDistance(spawnPoint, const Float:enemyEyePositions[][3], numberOfEnemies)
{
	...
	{
		distance = GetVectorDistance(spawnPositions[spawnPoint], enemyEyePositions[i], true);
...
// This function should find the spawn point farthest away from it's closest enemy
// which is also unoccupied, and not in the line of sight of an enemy
getSafestSpawn(client){
	...
	// Create an array of the indexes of unoccupied spawns with a clear line of sight (safe spawns)
	CPrintToChat(client, "[\x04DM\x01] Compiling a list of spawn points that could be the safest...");
	decl safeSpawns[spawnPointCount];
	new safeSpawnCount = 0;
	for(int i = 0; i < spawnPointCount;i++){

		// Check for a clear line of sight if the feature is enabled
		new bool:hasClearLineOfSight = true;
		if(lineOfSightSpawning){
			
			decl Float:spawnPointEyePosition[3];
			AddVectors(spawnPositions[i], eyeOffset, spawnPointEyePosition);
		...
		if(!spawnPointOccupied[i] || !hasClearLineOfSight){
			safeSpawns[safeSpawnCount] = i;
			safeSpawnCount++;
		}
	}

	// Make sure that there is at least one safe spawn
	if(safeSpawnCount < 1){
		// Use this to check for failure
		return -1;
	}
	// If there is more than 1 spawn, we can remove one
	else if(safeSpawnCount > 1){
		// Remove the spawn the client last spawned at - prevent multiple spawn kills
		new prevSpawn;
		new String:sclient[2];
		IntToString(client, sclient, sizeof(sclient));
		if(GetTrieValue(spawnTracker, sclient, prevSpawn)){
			// Remove the clients previous spawn point from the array
			for(int i = prevSpawn; i < safeSpawnCount-1; i++){
				safeSpawns[i] = safeSpawns[i+1];
			}
			safeSpawnCount--;
		}
	}

	// Now go through this array, and find the safe spawn that is farthest away from it's closest
	new point;
	new Float:max = 0.0;
	for(new i = 0; i < safeSpawnCount; i++){
		// Find the enemy that is closest to this spawn point
		new Float:minEnemyDist = GetVectorDistance(spawnPositions[safeSpawns[i]], playerPositions[0], true);
		for(new j = 1; j < numberOfAlivePlayers; j++){
			new bool:enemy = (ffa || (Teams:GetClientTeam(j) != clientTeam));
			if (enemy){
				new Float:currDist = GetVectorDistance(spawnPositions[safeSpawns[i]], playerPositions[j], true);
				if(currDist < minEnemyDist){
					minEnemyDist = currDist;
				}
			}
		}

		// If this distance between the spawnpoint and it's closest enemy is greater than the max, update it & save the spawnpoint
		if(minEnemyDist > max){
			point = i;
			max = minEnemyDist;
		}

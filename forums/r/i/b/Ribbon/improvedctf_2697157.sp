...
//Still gonna shoutout https://raw.githubusercontent.com/powerlord/tf2-data/master/datamaps.txt
...
public	void	OnPluginStart()	{
	...
	cvar_enabled.AddChangeHook(InitiateICTF);
	
	//Fix version to be static and unchangeable.
	ConVar	pluginversion	=	CreateConVar("sm_ictf_version",	VERSION,	"Improved CTF version", FCVAR_NOTIFY|FCVAR_DONTRECORD|FCVAR_SPONLY);
	pluginversion.AddChangeHook(forceversion);
}

void	forceversion(ConVar convar, const char[] oldValue, const char[] newValue)	{
	convar.SetString(oldValue);
}

void	InitiateICTF(ConVar convar, const char[] oldValue, const char[] newValue)	{
...
void	PrintCaptureText(float mult, int team)	{
	...
	switch(team)	{
		case	NOTEAM:		SetHudTextParams(-1.0, TEXT_Y, 1.0, 255, 255, 255, 200, 0, 0.0, 0.0, 0.0);
		case	REDTEAM:	SetHudTextParams(TEXT_XRED, TEXT_Y, 1.0, 255, 0, 0, 200, 0, 0.0, 0.0, 0.0);
		case	BLUTEAM:	SetHudTextParams(TEXT_XBLU, TEXT_Y, 1.0, 0, 0, 255, 200, 0, 0.0, 0.0, 0.0);
	}
	char buffer[8];

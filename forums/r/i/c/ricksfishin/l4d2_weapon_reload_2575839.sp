...
public OnPluginStart()
{
	...
	CreateConVar("l4d2_wepreload_version", PLUGIN_VERSION, "The version of the weapon reload plugin", FCVAR_NONE|FCVAR_SPONLY|FCVAR_DONTRECORD);
	smgClip = CreateConVar("AMMO_TYPE_SMG", "50", "50", FCVAR_NONE);
	smgSilencedClip = CreateConVar("AMMO_TYPE_SMG", "50", "50", FCVAR_NONE);
	smgMp5Clip = CreateConVar("AMMO_TYPE_SMG_MP5", "50", "50", FCVAR_NONE);
	pumpClip = CreateConVar("AMMO_TYPE_SHOTGUN", "8", "8", FCVAR_NONE);
	chromeClip = CreateConVar("AMMO_TYPE_SHOTGUN", "8", "8", FCVAR_NONE);
	huntClip = CreateConVar("AMMO_TYPE_HUNTINGRIFLE_HUNT", "15", "15", FCVAR_NONE);
	rifleClip = CreateConVar("AMMO_TYPE_ASSAULTRIFLE", "50", "50", FCVAR_NONE);
	rifleAk47Clip = CreateConVar("AMMO_TYPE_ASSAULTRIFLE_AK47", "40", "40", FCVAR_NONE);
	rifleDesertClip = CreateConVar("AMMO_TYPE_ASSAULTRIFLE_DESERT", "60", "60", FCVAR_NONE);
	rifleSg552Clip = CreateConVar("AMMO_TYPE_ASSAULTRIFLE_SG552", "50", "50", FCVAR_NONE);
	militaryClip = CreateConVar("AMMO_TYPE_SNIPERRIFLE_MILITARY", "30", "30", FCVAR_NONE);
	awpClip = CreateConVar("AMMO_TYPE_SNIPERRIFLE_AWP", "20", "20", FCVAR_NONE);
	scoutClip = CreateConVar("AMMO_TYPE_SNIPERRIFLE_SCOUT", "15", "15", FCVAR_NONE);
	granedeClip = CreateConVar("AMMO_TYPE_GRENADELAUNCHER", "1", "1", FCVAR_NONE);
	m60Clip = CreateConVar("AMMO_TYPE_ASSAULTRIFLE_M60", "150", "150", FCVAR_NONE);
	autoClip = CreateConVar("AMMO_TYPE_AUTOSHOTGUN", "10", "10", FCVAR_NONE);
	spasClip = CreateConVar("AMMO_TYPE_AUTOSHOTGUN", "10", "10", FCVAR_NONE);
	g_h_reload_rate = CreateConVar("l4d_powerups_weaponreload_rate", "0.8", "The interval incurred by reloading is multiplied by this value (clamped between 0.2 < 0.9)", FCVAR_NONE|FCVAR_NOTIFY, true, 0.2, true, 0.9);
...
public Action:Event_Weapon_Fired(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	h_mPrimary = GetPlayerWeaponSlot(client,0);
...
public Action:Event_Weapon_Reload(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	ActiveWeapon = GetPlayerWeaponSlot(client, 0);
...
MagStart (iEntid, client)
{
	...
	if ( (flNextTime_calc - 0.4) > 0 )
		CreateTimer( flNextTime_calc - 0.3 , Timer_MagEnd2, hPack);

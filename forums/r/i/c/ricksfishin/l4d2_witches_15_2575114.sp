...
{	
	...
	End_Timer(false);
	// Console variables
	g_hCvarCountWitchInRound = CreateConVar("l4d_witches_limit", "6", "Sets the limit for witches spawned. If 0, the plugin will not check count witches", FCVAR_NONE);
	g_hCvarCountAliveWitch = CreateConVar("l4d_witches_limit_alive", "3", "Sets the limit alive witches. If 0, the plugin will not check count alive witches", FCVAR_NONE);
	...
	g_hCvarWitchDistance = CreateConVar("l4d_witches_distance", "1200.0", "The range from survivors that witch should be removed. If 0, the plugin will not remove witches", FCVAR_NONE); // version 1.1

...
public OnPluginStart()
{
	...
	cvarBurningRage = CreateConVar("l4d_htm_burningrage", "1", "Enables the Burning Rage ability, Tank's movement speed increases when on fire. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBurningRageFist = CreateConVar("l4d_htm_burningragefist", "1", "Enables the Burning Rage Fist ability, Tank deals extra damage when on fire. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBurningRageSpeed = CreateConVar("l4d_htm_burningragespeed", "1.25", "How much of a speed boost does Burning Rage give. (Def 1.25)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarBurningRageDamage = CreateConVar("l4d_htm_burningragedamage", "3", "Amount of extra damage done to Survivors while Tank is on fire. (Def 3)", FCVAR_NOTIFY, true, 0.0, false, _);

	cvarHibernation = CreateConVar("l4d_htm_hibernation", "1", "Enables the Hibernation ability, Tank stops to hibernate and will regenerate health while taking extra damage. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarHibernationCooldown = CreateConVar("l4d_htm_hibernationcooldown", "120", "Amount of time before the Tank can Hibernate again. (Def 120)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarHibernationDamage = CreateConVar("l4d_htm_hibernationdamage", "2.0", "Multiplier for damage received by Tank while Hibernating. (Def 2.0)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarHibernationDuration = CreateConVar("l4d_htm_hibernationduration", "10.0", "Amount of time the Hibernation will take before completion. (Def 10.0)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarHibernationRegen = CreateConVar("l4d_htm_hibernationregen", "6000.0", "Amount of health the Tank will be set to once done Hibernating. (Def 6000.0)", FCVAR_NOTIFY, true, 0.0, false, _);

	cvarPhantomTank = CreateConVar("l4d_htm_phantomtank", "1", "Enables the Phanton Tank ability, when spawning the Tank will be immune to damage and fire until a player takes control. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarPhantomTankDuration = CreateConVar("l4d_htm_phantomtankduration", "3.0", "Amount of time after a player takes control of the Tank that the damage and fire immunity ends. (Def 3.0)", FCVAR_NOTIFY, true, 0.0, false, _);

	cvarSmoulderingEarth = CreateConVar("l4d_htm_SmoulderingEarth", "1", "Enables the Smouldering Earth ability, Tank is able to throw a burning rock that explodes when hitting the ground. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSmoulderingEarthDamage = CreateConVar("l4d_htm_smoulderingearthdamage", "7", "Damage the exploding rock causes nearby Survivors. (Def 7)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSmoulderingEarthRange = CreateConVar("l4d_htm_smoulderingearthrange", "300.0", "Area around the exploding rock that will reach Survivors. (Def 300.0)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSmoulderingEarthPower = CreateConVar("l4d_htm_smoulderingearthpower", "200.0", "Amount of power behind the explosion. (Def 200.0)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarSmoulderingEarthType = CreateConVar("l4d_htm_smoulderingearthtype", "2", "Type of rock thrown, 1 = Rock is always on fire, 2 = Rock only on fire if Tank is on fire.", FCVAR_NOTIFY, true, 1.0, true, 2.0);

	cvarTitanFist = CreateConVar("l4d_htm_titanfist", "1", "Enables the Titan Fist ability, Tank is able to send out shockwaves through the air with its fist. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTitanFistIncap = CreateConVar("l4d_htm_titanfistincap", "1", "Enables the Titan Fist Incap ability, if a Survivor is incapped by the Tank punch they will still be flung. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTitanFistCooldown = CreateConVar("l4d_htm_titanfistcooldown", "15", "Amount of time before the Tank can send another Titan Fist shockwave. (Def 15)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTitanFistDamage = CreateConVar("l4d_htm_titanfistdamage", "5", "Amount of damage done to Survivors hit by the Titan Fist shockwave. (Def 5)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTitanFistPower = CreateConVar("l4d_htm_titanfistpower", "200.0", "Force behind the Titan Fist shockwave. (Def 200.0)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTitanFistRange = CreateConVar("l4d_htm_titanfistrange", "700.0", "Distance the Titan Fist shockwave will travel. (Def 700.0)", FCVAR_NOTIFY, true, 0.0, false, _);

	cvarTitanicBellow = CreateConVar("l4d_htm_titanicbellow", "1", "Enables the Titanic Bellow ability, Tank is able to roar and send nearby Survivors flying or pull them to the Tank. (Def 1)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTitanicBellowCooldown = CreateConVar("l4d_htm_titanicbellowcooldown", "5.0", "Amount of time between Titanic Bellows. (Def 5.0)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTitanicBellowHealth = CreateConVar("l4d_htm_titanicbellowhealth", "0", "Amount of health the Tank must be at (or below) to use Titanic Belllow (0 = disabled). (Def 0)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTitanicBellowPower = CreateConVar("l4d_htm_titanicbellowpower", "300.0", "Power behind the inner range of Methane Blast. (Def 300.0)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTitanicBellowDamage = CreateConVar("l4d_htm_titanicbellowdamage", "10", "Damage the force of the roar causes to nearby survivors. (Def 10)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTitanicBellowRange = CreateConVar("l4d_htm_titanicbellowrange", "700.0", "Area around the Tank the bellow will reach. (Def 700.0)", FCVAR_NOTIFY, true, 0.0, false, _);
	cvarTitanicBellowType = CreateConVar("l4d_htm_titanicbellowtype", "1", "Type of roar, 1 = Survivors are pushed away from Tank, 2 = Survivors are pulled towards Tank.", FCVAR_NOTIFY, true, 1.0, true, 2.0);

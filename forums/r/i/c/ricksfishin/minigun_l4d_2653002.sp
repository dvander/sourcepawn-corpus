...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	...
	if(mini_gun_state[client]==state_carry && (buttons & IN_USE) && (buttons & IN_ATTACK2) && !(last_button & IN_ATTACK2))
	{
		StopCarray(client);
	} 
...
CreateMachine(client,type)
{
	...
	PrintToChatAll("%N create a mini gun,press E+mouse right button to carry it and to put it down",client);
	return 1;
}

StopCarray(client)
{
	if(mini_gun_state[client]!=state_carry)return;  
...
StartCarry(client, ent)
{
	...
	mini_gun_state[client]=state_carry;
	mini_gun_ent[client]=ent;
	
	PrintHintText(client, "You carry a mini gun, press E+mouse right button to put it down");
}

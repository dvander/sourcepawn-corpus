...
public OnPluginStart() 
{
	...
	CreateConVar("l4d2_vomit",PLUGIN_VERSION,"L4D2 Everyone Vomit plugin version.",	FCVAR_NOTIFY|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_DONTRECORD);
	g_hCvarAllow = CreateConVar("l4d2_vomit_allow",	"1","0= L4D2 Everyone Vomit Plugin off, 1=Plugin on. 2 root admin only", FCVAR_NOTIFY);
	g_hCvarType = CreateConVar(	"l4d2_vomit_type","6","the vomit can only hit 1=sur 2=SI 4 =commoninfected (add the number to it if you want it)", FCVAR_NOTIFY);
	g_hCvarRange = CreateConVar("l4d2_vomit_range","100","vomit hit detection range", FCVAR_NOTIFY);
	g_hCvarTeam = CreateConVar(	"l4d2_vomit_team","1","the team that can use !vomitme  1 =sur 2=infected 3=both", FCVAR_NOTIFY);
	g_hCvarCool = CreateConVar(	"l4d2_vomit_cool","10.0", "cool down time after vomit", FCVAR_NOTIFY);
	g_hCvarSound = CreateConVar("l4d2_vomit_sound",	"player/boomer/vomit/attack/bv1.wav", "sound file used   leave blank to disable sound", FCVAR_NOTIFY);

...
public OnPluginStart() {
	...
	// create cvars
	CreateConVar("sm_alarmcar_version", PLUGIN_VERSION, "Spawn alarmcar version", FCVAR_NOTIFY|FCVAR_DONTRECORD);
	globalCVarEnable = CreateConVar("sm_alarmcar_mapstart", "1", "Spawn fully functional alarm cars at mapstart from config file /cfg/sourcemod/alarmcars_location.cfg", FCVAR_NOTIFY);
...
GetTargetname(const entity, String:entityName[64]) {
	...
	if (FindDataMapInfo(entity, "m_iName") == -1) return;

...
public OnPluginStart() 
{
    ...
    CreateConVar("sm_grab_version", PLUGIN_VERSION, "Grab:TF2", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	  cvCharadminflag = CreateConVar("sm_grab_adminflag", "d", "Admin flag to use for immunity (only one).  Must be a in char format.");
...
{
	if(IsaAdmin(client, CharAdminFlag))
	{
...
stock bool:IsaAdmin(client, const String:flags[])
{
...
public OnGameFrame()
{
    ...
    if (g_bitsEnabled) 
    {
        ...
        {
            if (PlayerIsAdmin[i] && IsClientConnected(i) && IsClientInGame(i) &&
...
{
	  // check if EngiButtonDown is true
    if (g_EngiButtonDown[client] == true || !PlayerIsAdmin[client])
        return Plugin_Handled;
...
{
    // make sure client is not spectating
    if (!client || !IsPlayerAlive(client) || !PlayerIsAdmin[client])
        return Plugin_Handled;
    // has an object?

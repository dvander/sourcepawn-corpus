...
new Handle:g_ats_fadeColor_duration;
...
public OnPluginStart()
{
	...
	g_ats_fadeColor = CreateConVar( "ats_fade", "1", "Fade-in players screens when teams are scrambled. 0 = disabled, 1 = enabled. Def. 1.", 
		FCVAR_PLUGIN, true, 0.0, true, 1.0 );
	g_ats_fadeColor_duration = CreateConVar( "ats_fade_duration", "500", "Duration of the fade-in. Best values are around 250-1000. Def. 500.", 
		FCVAR_PLUGIN, true, 0.0, true, 1500.0 );
...
public Action:scrambleTeams(Handle:timer, any:reason)
{
	...
	if ( GetConVarInt( g_ats_fadeColor ) == 1 )
...
fadeColorAllPlayers()
{
	...
	if ( GetConVarInt( g_ats_fadeColor ) == 0 )
		return;
	
	new Handle:msg;
	new iDuration = GetConVarInt( g_ats_fadeColor_duration );

...
public OnClientPostAdminCheck(clientSlot)
{
	...
	else 
	{
		...
		if (GetConVarBool(s_smAuthByTag))
		{
			...
			{
				//LogMessage("admin %s [%s] authenticated by clan tag", playername, playerid);
				authByTag=true;
			}
		}
		
		// public slots full
		if (userFlags & ADMFLAG_ROOT || userFlags & ADMFLAG_RESERVATION || authByTag) // is allowed to use reserved slot?
		{
			...
			//LogMessage("admin %s [%s] connected to reserved slot", playername, playerid);
		...
		else
		{
			...
			//LogMessage("unpriviledged user %s [%s] connected to reserved slot", playername, playerid);
...
public Action:debugPrint(clientSlot, args)
{
	...
	LogMessage("Dumping priority vector to console ------------------");
	refreshPriorityVector();
	printPriorityVector(clientSlot);
	return Plugin_Handled;
}

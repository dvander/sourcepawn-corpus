...
void RegCmds(){
	...
	RegAdminCmd("sm_rspall", Command_RespawnAll, ADMFLAG_GENERIC, "NMRiH respawn all players (excluding admins)");
	// New command for respawning a specific player
	RegAdminCmd("sm_rsp_player", Command_RespawnPlayer, ADMFLAG_GENERIC, "NMRiH respawn player menu");
...
int RespawnAll(){
	...
	{
		if(IsValidClient(client) && !IsPlayerAlive(client) && !HasAnyAdminFlag(client))
		{
			if(RespawnClient(client))
				count++;
		}
	}
	return count;
...
bool HasAnyAdminFlag(int client)
{
	...
	for(int i = 0; i < 26; i++)
	{
		AdminFlag flag = view_as<AdminFlag>(i);

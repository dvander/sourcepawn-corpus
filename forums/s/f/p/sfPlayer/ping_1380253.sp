...
public Action:Command_Say4(client, args)
{
	...
	if (SocketIsConnected(socketReceive)) {
		// no response yet, force disconnect
		SocketDisconnect(socketReceive);
	}
	...
	SocketSetArg(socketReceive, 1);
	SocketConnect(socketReceive, OnSocketConnected, OnSocketReceive, OnSocketDisconnected,AddrIP,StringToInt(Port));

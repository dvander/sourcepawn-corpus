...
#define SOUND_KILL              "weapons/pan/melee_frying_pan_01.wav"
...
public Action:SoundHook(iClients[64], &iClientCount, String:strSample[PLATFORM_MAX_PATH], &iEntity, &iChannel, &Float:fVolume, &iLevel, &iPitch, &iFlags)
{
        ...
        if(g_bDemopan[iEntity] && StrContains(strSample, "freak_fortress_2/demopan", false) == -1)
        if(g_bDemopan[iEntity] && StrContains(strSample, "vo/", false) == -1)
        if(g_bDemopan[iEntity] && StrContains(strSample, "weapons/", false) == -1)
        if(g_bDemopan[iEntity] && StrContains(strSample, "ui/", false) == -1)
                return Plugin_Stop;
        return Plugin_Continue;
}
...
stock MakePlayerDemopan(iClient)
{
        ...
        TF2Items_GiveWeapon(iClient, "saxxy", 264, 100, 5, strAttributes);

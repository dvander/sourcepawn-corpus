...
#define SOUND_KILL "weapons/halloween_boss/knight_axe_hit.mp3"
...
public OnMapStart()
{
        ...
        AddFileToDownloadsTable("models/player/saxton_hale/hhh_jr_mk3.vtx");
...
public Action:OnPlayerDeath(Handle:hEvent, const String:strName[], bool:bDontBroadcast)
{
        ...
        if(iCustom != TF_CUSTOM_BOOTS_STOMP)
                SetEventString(hEvent, "weapon", "decapitation_boss");
...
public Action:SoundHook(iClients[64], &iClientCount, String:strSample[PLATFORM_MAX_PATH], &iEntity, &iChannel, &Float:fVolume, &iLevel, &iPitch, &iFlags)
{
        ...
        if(g_bHHHJunior[iEntity] && StrContains(strSample, "saxton_hale", false) == -1)
                return Plugin_Stop;
        return Plugin_Continue;
}
...
public Action:Timer_UseRage(Handle:hTimer, any:iClientId)
{
        ...
        TF2_AddCondition(iClient, TFCond_Ubercharged, 20.0);
...
stock MakePlayerHHHJunior(iClient)
{
        ...
        Format(strAttributes, sizeof(strAttributes), "2 ; 3.0 ; 68 ; 2 ; 26 ; 9825 ; 275 ; 1 ; 442 ; 1.21", g_iHealth - 200, GetRandomInt(9999, 99999));
        TF2Items_GiveWeapon(iClient, "tf_weapon_sword", 266, 100, 4, strAttributes);

...
#define DEFLECTOR	"models/bots/heavy_boss/bot_heavy_boss.mdl"
...
new bool:g_bIsDEFLECTOR[MAXPLAYERS + 1];
...
public OnClientDisconnect_Post(client)
{
	if (g_bIsDEFLECTOR[client])
	{
		...
		g_bIsDEFLECTOR[client] = false;
	}
}

public OnMapStart()
{
	PrecacheModel(DEFLECTOR);
	PrecacheSound(DEATH);
	PrecacheSound(LOOP);
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if (IsValidClient(client) && g_bIsDEFLECTOR[client])
		{
			StopSound(client, SNDCHAN_AUTO, LOOP);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNFIRE);
			StopSound(client, SNDCHAN_AUTO, SOUND_GUNSPIN);
...
public Action:DeflectorSH(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	if (!g_bIsDEFLECTOR[entity]) return Plugin_Continue;
...
{
	if (IsValidClient(iClient) && g_bIsDEFLECTOR[iClient]) 
	{	
...
public Native_IsDeflector(Handle:plugin, args)
	return g_bIsDEFLECTOR[GetNativeCell(1)];

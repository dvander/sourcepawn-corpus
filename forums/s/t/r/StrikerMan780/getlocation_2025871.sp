...
public Action:Command_Location(client, args)
{
	...
	GetClientAuthString(target,authid,34);
...
public Action:Command_Locations(client, args)
{
	...
	if(!GetConVarBool(g_LocationInMenu)|| client==0)
	{
		...
		for (new i=1; i<=maxplayers; i++)
		{
			...
			if(client==0) 
			{
				PrintToServer("%s is connected from %s", name,country);
			}
			else 
			{	
				PrintToConsole(client,"%s is connected from %s", name,country);
			}
		}
	}
	else
...
{
	if(GetConVarInt(g_hLocationAnnounce)==0)
	{
		return;
	}

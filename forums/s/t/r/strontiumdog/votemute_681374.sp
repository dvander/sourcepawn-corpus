...
//
// CHANGELOG:
// - 07.3.2008 Version 1.0.100

// - 07.15.2008 Version 1.0.101
// Added !votemute in chat to bring up menu

// - 07.19.2008 Version 1.0.102
//Added votesilence
...
#define PLUGIN_VERSION "1.0.102P" 
...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_votesilence", Command_Votesilence,  "sm_votemute <player> ")  
...
DisplayVoteMuteMenu(client, target)
{
	...
	if (g_votetype == 0)
	{
		...
		SetMenuTitle(g_hVoteMenu, "Votemute Player");
	}
	else 
	{
		...
		g_hVoteMenu = CreateMenu(Handler_VoteCallback, MenuAction:MENU_ACTIONS_ALL);
		SetMenuTitle(g_hVoteMenu, "Votesilence Player");
	}
...
DisplayVoteTargetMenu(client)
{
	...
	Format(title, sizeof(title), "%s", "Mute vote");
	...
	else if (action == MenuAction_VoteEnd)
	{
		...
		else
		{
			...
			else 
			{
				...
				SetClientListeningFlags( g_voteClient[VOTE_CLIENTID], VOICE_MUTED);
				g_Silenced[g_voteClient[VOTE_CLIENTID]] = true
			}		
		}
	}
	return 0;
...
bool:TestVoteDelay(client)
{
 	...
 	if (delay > 0)
 	{
 		if (delay > 60)
 		{
 			ReplyToCommand(client, "[SM] %s", "Vote Delay Minutes", delay % 60);
 		}
 		else
 		{
 			ReplyToCommand(client, "[SM] %s", "Vote Delay Seconds", delay);
 		}

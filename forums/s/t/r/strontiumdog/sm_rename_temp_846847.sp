...
#define PLUGIN_VERSION "1.6.000T"
...
{
	name = "Player Rename - Pemp Version",
	author = "<eVa>Dog",
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_pname", Command_PermReName, ADMFLAG_SLAY, "sm_pname <#userid|name> <name>")
...
public Action:Command_PermReName(client, args)
{
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		{			
			PerformNameChange(client, target_list[i], name)
		}
	}
	return Plugin_Handled
}

PerformNameChange(client, target, String:chosenname[64])
{
	new String:name[64]
	...
	if (g_Name[target] == 0 && g_Timer[target] == INVALID_HANDLE)
	{
		...
		LogAction(client, target, "\"%L\" changed %N's name to %s", client, target, name)
		ShowActivity(client, "changed %N's name", target) 
	}
	else
	{
...
public MenuHandler_Players(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		{				
			if (g_Name[target] == 0)
				PerformNameChange(param1, target, "PRESET")
			else
				PerformNameChange(param1, target, "")

		...
		if( iWeapon > MaxClients && IsValidEntity(iWeapon) )
		{
			...
			if( type )
			{
				...
				if( index >= 0 && index <= MAX_TYPES )
				{
					...
					// Get the descriptive version for chat
					GetGrenadeChatDescription(index, client, description, sizeof(description));
					
					// Format the full message
					Format(message, sizeof(message), "%T %s", "GrenadeMod_Mode", client, description);
					ReplaceColors(message, sizeof(message));
	...
	if( weapon > MaxClients && IsValidEntity(weapon) )
	{
		...
		if( type )
		{
			...
			if( g_bCvarAllow && g_iConfigMsgs && g_fLastEquip[client] < GetGameTime() && !IsFakeClient(client) )
			{
				...
				// Get the descriptive version for chat
				GetGrenadeChatDescription(g_iClientGrenadeType[client], client, description, sizeof(description));
				...
				if( g_iConfigPrefs != 3 )
				{
					...
					if( g_iConfigBinds == 2 )
						Format(hint, sizeof(hint), "%T", "GrenadeMod_Hint2", client);
					else
						Format(hint, sizeof(hint), "%T", "GrenadeMod_Hint", client);
			
					ReplaceColors(hint, sizeof(hint));
					PrintToChat(client, hint);
				}
			}
		}
	}
...
Action TimerSlowdown(Handle timer, int entity)
{
	if( (entity = EntRefToEntIndex(entity)) != INVALID_ENT_REFERENCE )
	{
		...
		SetEntityGravity(entity, -0.001);  // 100x slower than original
	}
	return Plugin_Stop;
}
...
void Explode_BlackHole(int client, int entity, int index, bool fromTimer)
{
	...
	if( fromTimer == false )
	{
		...
		// Particles
		if( g_bLeft4Dead2 )
			DisplayParticle(entity,		PARTICLE_CHARGE,	vPos, NULL_VECTOR, 1.0);
	...
	// TRUE maximum pull radius (1.9 * effect_range based on testing)
	float true_hole_radius = g_GrenadeData[index - 1][CONFIG_RANGE] * 1.9;
	
	// Animated purple ring - shows max pull boundary
	float animated_size = true_hole_radius + BEAM_OFFSET;
	CreateBeamRing(entity, { 255, 0, 255, 255 }, animated_size - (animated_size / BEAM_RINGS), 0.1);
	
	// Static purple ring - marks "DANGER ZONE - STAY OUT"
	CreateBeamRing(entity, { 255, 0, 255, 255 }, true_hole_radius - 25.0, true_hole_radius + 25.0);
}
...
void Explode_Shield(int entity, int index, bool fromTimer)
{
	...
	// Beam rings
	float true_shield_radius = g_GrenadeData[index - 1][CONFIG_RANGE] * 1.9;
	CreateBeamRing(entity, { 0, 150, 255, 255 }, true_shield_radius - 25.0, true_shield_radius + 25.0);

	if( fromTimer == false )
	{
		// Create Trigger - use actual radius (the trigger uses diameter, not radius)
		TriggerMultipleDamage(entity, index, true_shield_radius * 2, vPos);
	}
}
...
void Explode_Tesla(int client, int grenade, int index, bool fromTimer)
{
	...
	// Tesla radius (1.9 * effect_range)
	float true_tesla_radius = g_GrenadeData[index - 1][CONFIG_RANGE] * 1.9;
	
	// Create outward-expanding beam rings
	float animated_size = true_tesla_radius + BEAM_OFFSET;
	CreateBeamRing(grenade, { 73, 121, 255, 100 }, 0.1, animated_size - (animated_size / BEAM_RINGS));
	
	// Static boundary ring
	CreateBeamRing(grenade, { 73, 121, 255, 200 }, true_tesla_radius - 25.0, true_tesla_radius + 25.0);

	// Shake - only on initial explosion
	if( fromTimer == false )
	{
		CreateShake(g_GrenadeData[index - 1][CONFIG_SHAKE], g_GrenadeData[index - 1][CONFIG_RANGE] + SHAKE_RANGE, vPos);
	}
...
void Explode_Freezer(int client, int entity, int index, bool fromTimer)
{
	...
	// Beam rings - Sky blue color (RGB: 135, 206, 235)
	// Show actual freeze radius (2.3 * effect_range)
	float true_freeze_radius = g_GrenadeData[index - 1][CONFIG_RANGE] * 2.3;
	CreateBeamRing(entity, { 0, 220, 255, 255 }, true_freeze_radius - 25.0, true_freeze_radius + 25.0);
}
...
void Explode_Medic(int entity, int index)
{
	...
	// Beam Ring
	float true_heal_radius = g_GrenadeData[index - 1][CONFIG_RANGE] * 1.9;
	CreateBeamRing(entity, { 0, 150, 0, 255 }, true_heal_radius - 25.0, true_heal_radius + 25.0);
...
void Explode_AntiGravity(int client, int entity, int index)
{
	...
	// TRUE effect radius (based on Black Hole testing: 1.9 * effect_range)
	float true_gravity_radius = g_GrenadeData[index - 1][CONFIG_RANGE] * 1.9;
	
	// Animated ring (lime green) - keep original animation but at true radius
	float animated_size = true_gravity_radius + BEAM_OFFSET;
	CreateBeamRing(entity, { 0, 255, 100, 255 }, animated_size - (animated_size / BEAM_RINGS), 0.1);
	
	// Static ring (lime green) - shows ACTUAL effect boundary
	CreateBeamRing(entity, { 0, 255, 100, 255 }, true_gravity_radius - 25.0, true_gravity_radius + 25.0);
}
...
void GetGrenadeChatDescription(int index, int client, char[] buffer, int size)
{
	static char modeName[256];
	
	// Get just the mode name from translations
	Format(modeName, sizeof(modeName), "GrenadeMod_Title_%d", index);
	Format(modeName, sizeof(modeName), "%T", modeName, client);
	
	// Add descriptions for specific types
	switch(index)
	{
		case 0: // Stock
			Format(buffer, size, "%s \x01(Default)", modeName);
		
		case 1: // Bomb
		{
			float damage = g_GrenadeData[INDEX_BOMB][CONFIG_DAMAGE];
			float range = g_GrenadeData[INDEX_BOMB][CONFIG_RANGE];
			Format(buffer, size, "%s \x01(High-explosive blast, \x05%.0f\x01 dmg in \x05%.0f\x01 radius)", modeName, damage, range);
		}
		
		case 2: // Cluster
		{
			float damage = g_GrenadeData[INDEX_CLUSTER][CONFIG_DAMAGE];
			float range = g_GrenadeData[INDEX_CLUSTER][CONFIG_RANGE];
			float tick = g_GrenadeData[INDEX_CLUSTER][CONFIG_TICK];
			float time = g_GrenadeData[INDEX_CLUSTER][CONFIG_TIME];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Launches explosive projectiles, \x05%.0f\x01 dmg every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, tick, time, range);
			else
				Format(buffer, size, "%s \x01(Launches explosive projectiles, \x05%.0f\x01 dmg in \x05%.0f\x01 radius)", modeName, damage, range);
		}
		
		case 3: // Firework
		{
			float damage = g_GrenadeData[INDEX_FIREWORK][CONFIG_DAMAGE];
			float tick = g_GrenadeData[INDEX_FIREWORK][CONFIG_TICK];
			float time = g_GrenadeData[INDEX_FIREWORK][CONFIG_TIME];
			float range = g_GrenadeData[INDEX_FIREWORK][CONFIG_RANGE];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Creates fire puddles, \x05%.0f\x01 dmg every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, tick, time, range);
			else
				Format(buffer, size, "%s \x01(Creates fire puddles, \x05%.0f\x01 dmg in \x05%.0f\x01 radius)", modeName, damage, range);
		}
		
		case 4: // Smoke
		{
			float time = g_GrenadeData[INDEX_SMOKE][CONFIG_TIME];
			float range = g_GrenadeData[INDEX_SMOKE][CONFIG_RANGE];
			float tick = g_GrenadeData[INDEX_SMOKE][CONFIG_TICK];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Smoke screen refreshes every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, tick, time, range);
			else if(time > 0.0)
				Format(buffer, size, "%s \x01(Smoke screen for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, time, range);
			else
				Format(buffer, size, "%s \x01(Smoke screen in \x05%.0f\x01 radius)", modeName, range);
		}
		
		case 5: // Black Hole
		{
			float damage = g_GrenadeData[INDEX_BLACKHOLE][CONFIG_DAMAGE];
			float tick = g_GrenadeData[INDEX_BLACKHOLE][CONFIG_DMG_TICK];
			float interval = (tick > 0) ? tick : 1.0;
			float range = g_GrenadeData[INDEX_BLACKHOLE][CONFIG_RANGE];
			float time = g_GrenadeData[INDEX_BLACKHOLE][CONFIG_TIME];
			Format(buffer, size, "%s \x01(Pulls everyone to center, \x05%.0f\x01 dmg every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, interval, time, range);
		}
		
		case 6: // Flashbang
		{
			float time = g_GrenadeData[INDEX_FLASHBANG][CONFIG_TIME];
			float range = g_GrenadeData[INDEX_FLASHBANG][CONFIG_RANGE];
			Format(buffer, size, "%s \x01(Blinds enemies for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, time, range);
		}
		
		case 7: // Shield
		{
			float reduction = g_GrenadeData[INDEX_SHIELD][CONFIG_DAMAGE];
			float range = g_GrenadeData[INDEX_SHIELD][CONFIG_RANGE];
			float time = g_GrenadeData[INDEX_SHIELD][CONFIG_TIME];
			float tick = g_GrenadeData[INDEX_SHIELD][CONFIG_TICK];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Protective dome, \x05%.0f%%%%\x01 dmg reduction refreshes every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, reduction, tick, time, range);
			else if(time > 0.0)
				Format(buffer, size, "%s \x01(Protective dome, \x05%.0f%%%%\x01 dmg reduction for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, reduction, time, range);
			else
				Format(buffer, size, "%s \x01(Protective dome, \x05%.0f%%%%\x01 dmg reduction in \x05%.0f\x01 radius)", modeName, reduction, range);
		}
		
		case 8: // Tesla
		{
			float damage = g_GrenadeData[INDEX_TESLA][CONFIG_DAMAGE];
			float tick = g_GrenadeData[INDEX_TESLA][CONFIG_DMG_TICK];
			float interval = (tick > 0) ? tick : 1.0;
			float range = g_GrenadeData[INDEX_TESLA][CONFIG_RANGE];
			float time = g_GrenadeData[INDEX_TESLA][CONFIG_TIME];
			Format(buffer, size, "%s \x01(Electric arcs push enemies, \x05%.0f\x01 dmg every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, interval, time, range);
		}
		
		case 9: // Chemical
		{
			float damage = g_GrenadeData[INDEX_CHEMICAL][CONFIG_DAMAGE];
			float tick = g_GrenadeData[INDEX_CHEMICAL][CONFIG_DMG_TICK];
			float interval = (tick > 0) ? tick : 1.0;
			float time = g_GrenadeData[INDEX_CHEMICAL][CONFIG_TIME];
			float range = g_GrenadeData[INDEX_CHEMICAL][CONFIG_RANGE];
			Format(buffer, size, "%s \x01(Acid pool, \x05%.0f\x01 dmg every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, interval, time, range);
		}
		
		case 10: // Freezer
		{
			float freezeTime = g_fConfigFreezeTime;
			float range = g_GrenadeData[INDEX_FREEZER][CONFIG_RANGE];
			float time = g_GrenadeData[INDEX_FREEZER][CONFIG_TIME];
			float tick = g_GrenadeData[INDEX_FREEZER][CONFIG_TICK];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Freezes everyone for \x05%.1f\x01s, refreshes every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, freezeTime, tick, time, range);
			else if(time > 0.0)
				Format(buffer, size, "%s \x01(Freezes everyone for \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, freezeTime, time, range);
			else
				Format(buffer, size, "%s \x01(Freezes everyone for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, freezeTime, range);
		}
		
		case 11: // Medic
		{
			float heal = g_GrenadeData[INDEX_MEDIC][CONFIG_DAMAGE];
			float range = g_GrenadeData[INDEX_MEDIC][CONFIG_RANGE];
			float tick = g_GrenadeData[INDEX_MEDIC][CONFIG_TICK];
			float time = g_GrenadeData[INDEX_MEDIC][CONFIG_TIME];
			Format(buffer, size, "%s \x01(Heals allies, \x05%.0f\x01 HP every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, heal, tick, time, range);
		}
		
		case 12: // Vaporizer
		{
			float damage = g_GrenadeData[INDEX_VAPORIZER][CONFIG_DAMAGE];
			float range = g_GrenadeData[INDEX_VAPORIZER][CONFIG_RANGE];
			float tick = g_GrenadeData[INDEX_VAPORIZER][CONFIG_TICK];
			float time = g_GrenadeData[INDEX_VAPORIZER][CONFIG_TIME];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Disintegrates enemies, \x05%.0f\x01 dmg every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, tick, time, range);
			else if(time > 0.0)
				Format(buffer, size, "%s \x01(Disintegrates enemies, \x05%.0f\x01 dmg for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, time, range);
			else
				Format(buffer, size, "%s \x01(Disintegrates enemies, \x05%.0f\x01 instant dmg in \x05%.0f\x01 radius)", modeName, damage, range);
		}
		
		case 13: // Extinguisher
		{
			float range = g_GrenadeData[INDEX_EXTINGUISHER][CONFIG_RANGE];
			float tick = g_GrenadeData[INDEX_EXTINGUISHER][CONFIG_TICK];
			float time = g_GrenadeData[INDEX_EXTINGUISHER][CONFIG_TIME];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Extinguishes fires every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, tick, time, range);
			else if(time > 0.0)
				Format(buffer, size, "%s \x01(Extinguishes fires for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, time, range);
			else
				Format(buffer, size, "%s \x01(Extinguishes fires in \x05%.0f\x01 radius)", modeName, range);
		}
		
		case 14: // Glow
		{
			float bonus = g_fConfigGlowBonus * 100;
			float time = g_GrenadeData[INDEX_GLOW][CONFIG_TIME];
			float range = g_GrenadeData[INDEX_GLOW][CONFIG_RANGE];
			float tick = g_GrenadeData[INDEX_GLOW][CONFIG_TICK];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Highlights targets, +\x05%.0f\x01%% dmg refreshes every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, bonus - 100, tick, time, range);
			else if(time > 0.0)
				Format(buffer, size, "%s \x01(Highlights targets, +\x05%.0f\x01%% dmg for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, bonus - 100, time, range);
			else
				Format(buffer, size, "%s \x01(Highlights targets, +\x05%.0f\x01%% dmg in \x05%.0f\x01 radius)", modeName, bonus - 100, range);
		}
		
		case 15: // Anti-Gravity
		{
			float damage = g_GrenadeData[INDEX_ANTIGRAVITY][CONFIG_DAMAGE];
			float range = g_GrenadeData[INDEX_ANTIGRAVITY][CONFIG_RANGE];
			float time = g_GrenadeData[INDEX_ANTIGRAVITY][CONFIG_TIME];
			float tick = g_GrenadeData[INDEX_ANTIGRAVITY][CONFIG_TICK];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Lifts everyone up, \x05%.0f\x01 dmg every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, tick, time, range);
			else if(time > 0.0)
				Format(buffer, size, "%s \x01(Lifts everyone up, \x05%.0f\x01 dmg for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, time, range);
			else
				Format(buffer, size, "%s \x01(Lifts everyone up, \x05%.0f\x01 dmg in \x05%.0f\x01 radius)", modeName, damage, range);
		}
		
		case 16: // Fire Cluster
		{
			float damage = g_GrenadeData[INDEX_FIRECLUSTER][CONFIG_DAMAGE];
			float range = g_GrenadeData[INDEX_FIRECLUSTER][CONFIG_RANGE];
			float tick = g_GrenadeData[INDEX_FIRECLUSTER][CONFIG_TICK];
			float time = g_GrenadeData[INDEX_FIRECLUSTER][CONFIG_TIME];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Fire projectiles, \x05%.0f\x01 dmg every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, tick, time, range);
			else
				Format(buffer, size, "%s \x01(Fire projectiles, \x05%.0f\x01 dmg in \x05%.0f\x01 radius)", modeName, damage, range);
		}
		
		case 17: // Bullets
		{
			float damage = g_GrenadeData[INDEX_BULLETS][CONFIG_DAMAGE];
			float tick = g_GrenadeData[INDEX_BULLETS][CONFIG_TICK];
			float time = g_GrenadeData[INDEX_BULLETS][CONFIG_TIME];
			float range = g_GrenadeData[INDEX_BULLETS][CONFIG_RANGE];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Rotating machine gun, \x05%.0f\x01 dmg every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, tick, time, range);
			else if(time > 0.0)
				Format(buffer, size, "%s \x01(Rotating machine gun, \x05%.0f\x01 dmg for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, time, range);
			else
				Format(buffer, size, "%s \x01(Rotating machine gun, \x05%.0f\x01 dmg in \x05%.0f\x01 radius)", modeName, damage, range);
		}
		
		case 18: // Flak
		{
			float damage = g_GrenadeData[INDEX_FLAK][CONFIG_DAMAGE];
			float range = g_GrenadeData[INDEX_FLAK][CONFIG_RANGE];
			float tick = g_GrenadeData[INDEX_FLAK][CONFIG_TICK];
			float time = g_GrenadeData[INDEX_FLAK][CONFIG_TIME];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Airburst shrapnel, \x05%.0f\x01 dmg every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, tick, time, range);
			else if(time > 0.0)
				Format(buffer, size, "%s \x01(Airburst shrapnel, \x05%.0f\x01 dmg for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, time, range);
			else
				Format(buffer, size, "%s \x01(Airburst shrapnel, \x05%.0f\x01 dmg in \x05%.0f\x01 radius)", modeName, damage, range);
		}
		
		case 19: // Airstrike
		{
			float damage = g_GrenadeData[INDEX_AIRSTRIKE][CONFIG_DAMAGE];
			float range = g_GrenadeData[INDEX_AIRSTRIKE][CONFIG_RANGE];
			float tick = g_GrenadeData[INDEX_AIRSTRIKE][CONFIG_TICK];
			float time = g_GrenadeData[INDEX_AIRSTRIKE][CONFIG_TIME];
			if(tick > 0.0 && time > 0.0)
				Format(buffer, size, "%s \x01(Airstrike bombardment, \x05%.0f\x01 dmg every \x05%.1f\x01s for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, tick, time, range);
			else if(time > 0.0)
				Format(buffer, size, "%s \x01(Airstrike bombardment, \x05%.0f\x01 dmg for \x05%.1f\x01s in \x05%.0f\x01 radius)", modeName, damage, time, range);
			else
				Format(buffer, size, "%s \x01(Airstrike bombardment, \x05%.0f\x01 dmg in \x05%.0f\x01 radius)", modeName, damage, range);
		}
		
		case 20: // Weapon
			Format(buffer, size, "%s \x01(Spawns a random weapon)", modeName);
	}
}
...
void CreateBeamRing(int entity, int iColor[4], float min, float max)
{
	...
	for( int i = 1; i <= BEAM_RINGS; i++ )
	{
		...
		// Ensure min and max are different for rendering
		if( min == max )
		{
			max = min + 5.0; // Add small difference
		}

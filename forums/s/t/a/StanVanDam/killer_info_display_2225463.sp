...
1.4.1a - Updated by StanVanDam on November 17, 2014.
Borrowed code from gem_damage_report (by Gemeni) to add functionality to see
damage you did to the player that killed you, and to store that info for
recall next round.
...
public Plugin:myinfo = {
	...
	author		= "Berni, gH0sTy, Smurfy1982, Snake60, StanVanDam",
...
public OnPluginStart()
{
	...
	cvAnnouncetime		= CreateConVar("kid_announcetime",		"-1",		"Time in seconds after an announce about turning killer infos on/off is printed to chat, set to -1 to disable.", FCVAR_PLUGIN);
	...
	RegConsoleCmd("killinfo", Command_KillerInfo, "On/Off Killer info display");
        RegConsoleCmd("lastdeath", Command_LastDamageReport);
...
public Action:Command_LastDamageReport(client, args)
{
  ...
  if (strcmp(g_HistDamageDone[client], "") != 0) {
    //PrintToChat(client, "{G}%s", g_HistDamageDone[client]);
    Client_PrintToChat(client,false,"%s",g_HistDamageDone[client]);
  }
  else {
    PrintToChat(client, "\x04Type /lastdeath after you die to see kill stats.");
  }
  return Plugin_Handled;
}
...
{
	if (client == 0) {
		ReplyToCommand(client, "This command can only be run by players.");
		return Plugin_Handled;
	}
	...
	if (enabledForClient[client]) {
		...
		Client_Reply(client, "{G}{N}%t", "kid_disabled");
		...
		Color_ChatSetSubject(client);
		Client_Reply(client, "{G}{N}%t", "kid_enabled");
...
public Action:Timer_Announce(Handle:timer, any:serial)
{
	...
	Color_ChatSetSubject(client);
	Client_PrintToChat(client, false, "{G}{N}%t", "announcement");
...
public Action:Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
  ...
  // Log damage
  g_DamageDone[attacker][client] += healthDmg;
}
...
public Action:Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
  ...
  if (strcmp(g_HistDamageDone[client], "") != 0) {
    //PrintToChat(client, "{G}%s", g_HistDamageDone[client]);
    Client_PrintToChat(client,false,"Your last death: %s",g_HistDamageDone[client]);
  }
  else {
    PrintToChat(client, "\x04Type /lastdeath after you die to see kill stats.");
  }
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new damageDone = g_DamageDone[client][attacker];
	new String:damageDoneString[512];
	clearAllDamageData();
	...
	if ((GetConVarBool(cvPrinttochat))) {
		...
		Client_PrintToChat(
			...
			attacker
                ...
                Format(damageDoneString, sizeof(damageDoneString), "%t", "chat_basic",
                        attacker,
			chat_weapon,
			chat_distance,
			healthLeft,
			...
			damageDone,
			attacker);
		g_HistDamageDone[client] = damageDoneString;
	...
	if ((GetConVarBool(cvPrinttopanel))) {
		...
		Format(buffer, sizeof(buffer), "%t", "panel_damagedone", damageDone);
                DrawPanelItem(panel, buffer, ITEMDRAW_DEFAULT);

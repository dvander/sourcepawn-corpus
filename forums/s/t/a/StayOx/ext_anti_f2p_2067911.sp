...
{
	CreateConVar("ext_anti_f2p_version", PLUGIN_VERSION, "Extended Free2BeKicked", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY| FCVAR_DONTRECORD);
	hMinSlots = CreateConVar("anti_f2p_minslots", "0", "This decides at how many open slots remaining it should start blocking new F2P connections. \nMeaning if you set it to 2 on a 24 slot server, no new F2P accounts will be able to join once 22 players are connected.");
	...
	LoadTranslations("ext_anti_f2p.phrases");
	AutoExecConfig(true, "plugin.ext_anti_f2p");
}
...
public OnClientPostAdminCheck(Client)
{
	...
	if (Steam_CheckClientSubscription(Client, 0) && !Steam_CheckClientDLC(Client, 459))
	{
		if(bTempBlock)
		{
			KickClient(Client, "%t", "blockreason");
			return;
		}
...
public Action:cmdKickLastF2P(Client, Args)
{
	...
	if(CurrentVictim != -1)
	{
		KickClient(CurrentVictim, "%t", "kicklast");
...
public Action:cmdKickFirstF2P(Client, Args)
{
	...
	if(CurrentVictim != -1)
	{
		KickClient(CurrentVictim, "%t", "kickfirst");
		bIsF2P[CurrentVictim] = false; //Reset
	}
	return Plugin_Handled;
}

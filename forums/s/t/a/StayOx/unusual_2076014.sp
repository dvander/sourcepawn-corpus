...
{	
	...
	RegConsoleCmd("sm_unusual", OpenMenu, "Get unusual effect on your weapons");

	RegAdminCmd("sm_unusual_control", ControlPlayer, ADMFLAG_GENERIC);
	RegAdminCmd("sm_unusual_permissions", reloadPermissions, ADMFLAG_GENERIC);
...
QualityMenu(iClient)
{
	...
	AddMenuItem(Qltymenu, "1", "Rarity 1");
	AddMenuItem(Qltymenu, "2", "Rarity 2");
	AddMenuItem(Qltymenu, "3", "Vintage");
	AddMenuItem(Qltymenu, "4", "Rarity 3");
	AddMenuItem(Qltymenu, "5", "Rarity 4");
...
public T_UpdateDB(Handle:owner, Handle:hndl, const String:error[], any:iClient)
{
	...
	if (!SQL_GetRowCount(hndl))		// If player doesn't exist in DB
	{
		...
		{		
			...
			SQL_TQuery(db, SQLErrorCheckCallback, "SET NAMES 'utf8'");
			SQL_TQuery(db, SQLErrorCheckCallback, buffer);

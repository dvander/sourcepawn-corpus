...
#define BEBOP_VERSION	"0.3eb beta"
...
public Plugin:MyInfo = 
{
	...
	description = "allows \"unlimited\" additional players in coop, survival and realism modes.",
...
{	
	...
	newMapActivatedPlayers = newMapActivatedPlayers - 1;
}
...
{	
	...
	if(g_GameMode)
	{
		...
		if (!IsFakeClient(client))
		{
			...
			if (count > 4 && newMapActivatedPlayers > 4 && (team != TEAM_SURVIVORS || team == TEAM_SPECTATORS || team == 0))
			{
				SpawnFakeClient();
...
public Action:Timer_PutClientToSurvivorTeam(Handle:timer, any:serial)
{
	...
	{
		if (GetClientTeam(client) == TEAM_SPECTATORS || GetClientTeam(client) == 0)
		{
			new bot = GetABot();
			if(bot)
			{

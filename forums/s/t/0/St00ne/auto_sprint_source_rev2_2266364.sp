...
new bool:b_Cool[MAXPLAYERS+1] = {false, ...};
new bool:b_Sprint[MAXPLAYERS+1] = {false, ...};
...
public OnPluginStart()
{
	...
	//CreateConVar("sm_autosprint_src_version", PLUGIN_VERSION, "Auto Sprint Source Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY);
	...
	cvars[3] = CreateConVar("sm_ass_speed", "1.33", "Sprint speed ratio.", FCVAR_PLUGIN, true, 0.0);
	cvars[4] = CreateConVar("sm_ass_team", "0", "Sprint team restriction: 2 = Terrorists, 3 = CT, 0 = all.", FCVAR_PLUGIN, true, 0.0, true, 3.0);
...
public player_spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if(b_Sprint[client])	// Timer running
		{
			SetEntPropFloat(client, Prop_Send, "m_flLaggedMovementValue", 1.0);
			b_Sprint[client] = false;
		}
		
		if(b_Cool[client])	// Cancel Cool down
		{
			b_Cool[client] = false;
		}
...
public Action:Cmd_StartSprint(client, args)
{
	...
	if(!enabled || client == 0 || !IsClientInGame(client) || (teamindx = GetClientTeam(client)) < 2 || !IsPlayerAlive(client))
	{
	...
	if(b_Cool[client] || b_Sprint[client])
	{
		return Plugin_Handled;
	}
	
	// Now you can use sprint
	b_Sprint[client] = true;
	CreateTimer(cvars_value[0], timer_sprint, GetClientSerial(client));
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon)
{
	if (buttons & IN_USE)
	{
		...
		if (enabled && IsPlayerAlive(client) && IsClientInGame(client))
		{
			if(b_Cool[client] || b_Sprint[client])
			{
				return Plugin_Continue;
...
public Action:timer_sprint(Handle:timer, any:serial)
{
	...
	if(client !=0 && IsClientInGame(client))
	{
		...
		if (GetClientTeam(client) >= 2 && IsPlayerAlive(client))
		{
			SetEntPropFloat(client, Prop_Send, "m_flLaggedMovementValue", 1.0);
			b_Cool[client] = true;
			CreateTimer(cvars_value[1], timer_cool, GetClientSerial(client));
			if (ads_enabled)
			{
 ...
 *Thanks to Shaman, Baca, Yak for his scripting FAQ,

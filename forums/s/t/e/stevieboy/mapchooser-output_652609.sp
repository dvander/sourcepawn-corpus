 ...
 *
 * Version: $Id: mapchooser.sp 2105 2008-04-28 22:07:41Z pred $
 */
...
public Action:Timer_StartMapVote(Handle:timer)
{
	...
	decl String:currentNextmap[32];
	
	GetConVarString(g_Cvar_Nextmap,currentNextmap,sizeof(currentNextmap));
	
	if(strcmp(currentNextmap,"Pending Vote") != 0){
		PrintToChatAll("[SM] %t", "No voting for the nextmap. Nextmap will be ", currentNextmap);
		LogMessage("No voting for nextmap. An admin already set nextmap to %s.", currentNextmap);

...
//
//TEST:
//  Round Counter in arena mode for TF2 (may need to hook arena_win_panel event)
...
public OnPluginStart()
{
    ...
    cvar_nominate_timelimits = CreateConVar(
        ...
        "0",
        "If enabled, maps with time limits which the server is currently violating will not be displayed in the nomination menu.",
        0, true, 0.0, true, 1.0
    );
...
public Event_PlayerDeath(Handle:evnt, String:name[], bool:dontBroadcast)
{
    ...
    if (warnings_frag_enabled)
    {
        DoVoteWarning(vote_fragwarnings, next_fragwarning,
            fraglimit - GetConVarInt(cvar_start_frags) - frags);
    }
...
public Event_RoundEnd(Handle:evnt, const String:name[], bool:dontBroadcast)
{
    ...
    if (!vote_completed && vote_enabled) 
    {
        if (warnings_round_enabled)
            DoVoteWarning(vote_roundwarnings, next_roundwarning, round_counter);
...
DoVoteWarning(Handle:warningArray, &next, any:triggertime)
{
    //Do nothing if...

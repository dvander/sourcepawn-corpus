...
//We don't require the following
...
//    found.
// Make DisplayServerMessage take a format string and params (not sure if this is possible)
// Admin flags for people allowed to see the vote
// Vote appears when first person joins the server (RTV?)
// Nominate can also take a map name in the command (nomination without a menu)
// Make playerlimit votes act more like regular votes (runoff, etc.)
// Admin menu integration
//
// --TESTS--  
// Test to make sure nominations are working.
// Possible bug with map exclusion. Test it.
// Test sm_umc_mapvote command
// Test Player Limits
//
// --FOR NEXT RELEASE--
// Cvar to put *all* nominations at the top of votes
// Map results can be echoed to center message
// Specific nomination-exclude cvars
// Increase runoff percentage to above %50
//         - Hardcode special case for 2 option vote (base case I guess)
// See how mapchooser handles round-counting
// Remove runoff_threshold cvar. Replace with RTV and EoMV cvar for threshold, and cvar controlling
//      action to take. Update old max_runoffs cvar so 0 = unlimited.
//      * i.e. vote_threshold 0 = do nothing, 1 = runoff (runoff uses max_runoffs to determine how
//      * many to do) (what do we do after runoffs expire? probably need another cvar 0 = nothing,
//      * 1 = accept)
// Cvar to enable/disable considering <map, map group> tuple for previous exclusion.
//
// --IDEAS (FUTURE)--
// Separate plugins for each type of limit (player, time, possibly others in the future).
// Move weight system into another plugin entirely (reweight calls the weight plugin which fetches
//    the values from the KV)
// Separate all features into separate plugins: (RTV, EOMV, RANDNEXT, PLAYERLIMS, NOMS, WARNINGS(?))
//    -Random algorithm, voting (obv. not the triggers) + runoffs, etc. stay in the core. triggers
//     and specific code get moved to separate plugins
//    -Move general utilities to their own .inc, make them stocks
//    -Perhaps give all plugins separate kv files, similar to how sourcemod can handle multiple
//     mapcycles? (see if we can even integrate with the SM mapcycles, although perhaps this isn't
//     the best idea).
// Core should contain all the code necessary to build menus and votes.
// Not sure if previously-played exclusion should be in core or own plugin (probably own)
// Core can build populated Group Menus and Map Menus. The plugin should then modify them (set
//      callbacks, etc.
//
// DESIGN DECISION: perhaps merge Warnings plugin with EOMV?
//
// Forwards:
//  - On Map Won Vote
//  - On Group Won Vote
//      *these get passed information that they can use to find the fields in the KV.
//      *can also replace/be called by HandleVoteWinner callbacks in UMC
//      *perhaps not to be used for displaying winner or percentage or setting the mapchange, keep
//       these in core
//  - Map_Exclusion:DetermineExclude(map, group, ...) (ET_Hook forward)
//  - 
//
// Natives:
//  - Is Map/Group Nominated? (probably need a lot of nomination-querying functions)
//  - 
//
// Enums/Constants:
//  - INVALID_MAPGROUP ("")
//  - enum Map_Exclusion -> maps to Action (i.e. Plugin_Stop, Plugin_Continue)
//  -
...
new Handle:map_vote_cat_preconfigs = INVALID_HANDLE;
new Handle:map_vote_map_preconfigs = INVALID_HANDLE;
...
RunTests()
{
    ...
    LogMessage("TEST: InvHan - %i", INVALID_HANDLE);
    KvRewind(INVALID_HANDLE);
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
    ...
    RegPluginLibrary("ultimate-mapchooser");
    CreateNative("UMC_AddWeightModifier", Native_AddWeightModifier);
    
    reweight_forward = CreateGlobalForward(
        "UMC_OnReweightMap",
        ET_Ignore,
        Param_String, Param_FloatByRef
    );
...
public OnPluginStart()
{
    cvar_runoff_max = CreateConVar(
        "sm_umc_runoff_max",
        "0",
        "Specifies the maximum number of maps to appear in a runoff vote. 1 or 0 sets\nno maximum.",
        0, true, 0.0
    );
    ...
    cvar_playerlimit_flags = CreateConVar(
        "sm_umc_playerlimit_adminflags",
        "",
        "String of admin flags required for players to be able to vote in player\nlimit votes. If no flags are specified, all players can vote."
    );*/

    cvar_vote_allowduplicates = CreateConVar(
        "sm_umc_vote_allowduplicates",
        "1",
        "Allows a map to appear in the vote more than once. This should be enabled if\nyou want the same map in different categories to be distinct.",
        0, true, 0.0, true, 1.0
    );
    ...
    cvar_randnext_ignorelimits = CreateConVar(
        ...
        "0",
        "Determines if random selections of the next map ignore exclusion limits. Add\nup the values below to get the functionality you want.\n 1: Ignore Time Limits\n 2: Ignore Player Limits",
        0, true, 0.0, true, 3.0
    );
    ...
    vote_mem_arr        = CreateArray(numCells, 0);
    vote_catmem_arr     = CreateArray(numCells, 0);
    rtv_mem_arr         = CreateArray(numCells, 0);
    rtv_catmem_arr      = CreateArray(numCells, 0);
    randnext_mem_arr    = CreateArray(numCells, 0);
    randnext_catmem_arr = CreateArray(numCells, 0);
    
    //Initialize our vote arrays
    numCells = ByteCountToCells(255);
    map_vote_cat_configs    = CreateArray(numCells, 0);
    map_vote_map_configs    = CreateArray(numCells, 0);
    map_vote_cat_preconfigs = CreateArray(numCells, 0);
    map_vote_map_preconfigs = CreateArray(numCells, 0);
    map_vote_next_cats      = CreateArray(numCells, 0);
...
//Called when a library is removed.
...
public OnConfigsExecuted()
{
    ...
    //Stops all vote code, will be set to false if the mapcycle is available.
    rtv_completed = true;
    rtv_enabled = true;
    ...
    AddToMemoryArray(mapName, rtv_mem_arr, mapmem);
...
public Event_GameEnd(Handle:evnt, const String:name[], bool:dontBroadcast)
{
    ...
    if (GetConVarBool(cvar_randnext) && !vote_completed && !rtv_completed)
    {
        ...
        RandomNextMap(randnext_mem_arr, randnext_catmem_arr, GetConVarInt(cvar_randnext_catmem),
                      GetConVarBool(cvar_ignoregweight), ignorePlayerLimits, ignoreTimeLimits);
    }
}
...
public OnMapEnd()
{
    //Do not check for invalid number of players.
    validity_enabled = false;
    
    //Set the intermission flag to No.
    intermission_called = false;
    
    //We have no messages being displayed anymore.
...
SetupMinMaxPlayers()
{
    ...
    strcopy(current_cat, sizeof(current_cat), "");
...
public Native_AddWeightModifier(Handle:plugin, numParams)
{
    if (reweight_active)
    {
        current_weight *= Float:GetNativeCell(1);
        DebugMessage("New map weight: %f", current_weight);
    }
    else
...
new Handle:umc_menu = INVALID_HANDLE;
...
public Adm_CategoryHandler(Handle:topmenu, TopMenuAction:action, TopMenuObject:objectId, param,
                         ...
                         String:buffer, maxlength)
{
    ...
    else if (action == TopMenuAction_SelectOption_
    {
                       ...
                       String:buffer, maxlength)
{
    ...
    {
        //Make and display Next Map menu
        //...
        // 1. Auto Select   (Random using limits)
        // 2. Manual Select (Pick from a list)
    }
                       ...
                       String:buffer, maxlength)
{
    ...
    {
        //Make and display Map Vote menu
        //...
        // 1. Auto Select   (Random using limits)
        // 2. Manual Select (Pick from a list)
    }
...
Handle:BuildGroupSelectMenu(bool:limits=false)
{
    if (map_kv == INVALID_HANDLE)
    {
        LogError("MENU: Cannot build map group menu, mapcycle is invalid.");
        return INVALID_HANDLE;
    }
    ...
    {
        LogError("MENU: Cannot build map group menu, no map groups found in mapcycle.");
        return INVALID_HANDLE;
    }
    
    new Handle:menu = CreateMenu(Handle_UMCGroupMenu, MenuAction_DisplayItem|MenuAction_Display);
    SetMenuTitle(menu, "Select A Map Group"); //TODO: Make translation
    
    decl String:gName[MAP_LENGTH];
    new count = 0;
    do
    {
        if (!IsValidCat(map_kv, INVALID_HANDLE, 0, INVALID_HANDLE, limits, limits))
            continue;
        
        KvGetSectionName(map_kv, gName, sizeof(gName));
        
        AddMenuItem(menu, gName, gName);
        count++;
    ...
    if (count < 1)
    {
        LogError("MENU: No map groups available to populate menu.");
        CloseHandle(menu);
        return INVALID_HANDLE;
    }
...
Handle_UMCGroupMenu(Handle:menu, MenuAction:action, client, param2)
{
    if (action == MenuAction_Select)
    {
        ...
        new Handle:newmenu = BuildMapSelectMenu(group);
        ...
        Handle_UMCMenu(menu, action, client, param2);
}


//Builds and returns a map selection menu.
Handle:BuildMapSelectMenu(const String:group[]="", bool:limits=false)
{
...
public Handle_MapVoteWinner(const String:info[], const String:disp[], Float:percentage)
{
    ...
    else //Otherwise, we print a message and then set the new map.
    {
        ...
        //Fetch and execute preconfigs
        decl String:pre[255];
        GetArrayString(map_vote_cat_preconfigs, index, pre, sizeof(pre));
        if (strlen(pre) > 0)
        {
            LogMessage("SETUP: Executing map group precommand: '%s'", pre);
            ServerCommand(pre);
        }
        GetArrayString(map_vote_map_preconfigs, index, pre, sizeof(pre));
        if (strlen(pre) > 0)
        {
            LogMessage("SETUP: Executing map precommand: '%s'", pre);
            ServerCommand(pre);
        }
...
public Handle_CatVoteWinner(const String:cat[], const String:disp[], Float:percentage)
{
    ...
    else //Otherwise, we pick a random map from the category and set that as the next map.
    {
        ...
        if (GetArraySize(nominationsFromCat) > 0)
        {
            ...
            decl String:catPConfig[255];
            KvGetString(map_kv, "pre-command", catPConfig, sizeof(catPConfig), "");
            ...
            if (counter > 0 && GetWeightedRandomSubKey(map, sizeof(map), weightArr, nameArr))
            {
                ...
                decl String:mapPConfig[255];
                KvGetString(map_kv, "pre-command", mapPConfig, sizeof(mapPConfig), "");
                KvGoBack(map_kv);
                
                if (strlen(catPConfig) > 0)
                {
                    LogMessage("SETUP: Executing map group precommand: '%s'", catPConfig);
                    ServerCommand(catPConfig);
                }
                if (strlen(mapPConfig) > 0)
                {
                    LogMessage("SETUP: Executing map precommand: '%s'", mapPConfig);
                    ServerCommand(mapPConfig);
                }
            ...
            else //Otherwise, we select a map randomly from the category.
            {
                ...
                //This is necessary for RandomNextMap's algorithm to select specifically from the
                //winning category.
                strcopy(next_rand_cat, sizeof(next_rand_cat), cat);
                
                //Pick and set the next map.
                RandomNextMap(vote_mem_arr, vote_catmem_arr, GetConVarInt(cvar_vote_catmem), false,
    ...
    else //Otherwise, we pick a random map from the category and change to it.
    {
        ...
        if (GetArraySize(nominationsFromCat) > 0)
        {
            ...
            else //Otherwise, we select a map randomly from the category.
            {
                ...
                //Pick and set the next map.
                RandomNextMap(rtv_mem_arr, rtv_catmem_arr, GetConVarInt(cvar_rtv_catmem), false,
                              ignorePlayerLimits, ignoreTimeLimits);
                        ...
                        bool:ignorePlayerLimits=false, bool:ignoreTimeLimits=false)
{
    ...
    new String:catPConfig[255];
    new String:mapPConfig[255];
    ...
    do
    {
        ...
        KvGetString(kv, "pre-command", catPConfig, sizeof(catPConfig), "");
            ...
            if (numNoms > 0)
            {
                ...
                if (nomCounter > 0)
                {
                    ...
                    for (new i = 0; i < min; i++)
                    {
                        ...
                        KvGetString(kv, "pre-command", mapPConfig, sizeof(mapPConfig), "");
                        ...
                        InsertArrayString(map_vote_cat_preconfigs, position, catPConfig);
                        InsertArrayString(map_vote_map_preconfigs, position, mapPConfig);
...
ClearVoteArrays()
{
    ...
    ClearArray(map_vote_cat_preconfigs);
    ClearArray(map_vote_map_preconfigs);
}
                   ...
                   const String:startSound[])
{
    ...
    else //Otherwise set the results.
    {
        ...
        //Handle when to change the map if the native is set.
...
bool:NeedRunoff()
{
    ...
    new Float:threshold = GetConVarFloat(cvar_runoff_threshold);
    new bool:result = (float(numClients) / total_votes) < threshold;
    if (result)
    {
        LogMessage(
            "RUNOFF: Winning map received %i votes, which is %.f percent of %i total votes. This is lower than the runoff threshold of %.f. A runoff vote will occur.",
            numClients,
            float(numClients) / total_votes * 100,
            total_votes,
            threshold * 100
        );
    }
...
{   
    ...
    if (runoff_menu != INVALID_HANDLE)
    {
        ...
        DebugMessage("Runoff timer created. Runoff vote will be displayed in %i seconds.",
            runoff_delay+1);
...
Handle:BuildRunoffMenu(Handle:menu, VoteHandler:callback, Handle:clientArray)
{
    ...
    new maxMaps = GetConVarInt(cvar_runoff_max);
    ...
    if (blockSlots > 3)
    {
        AddMenuItem(newMenu, NOTHING_OPTION, "", ITEMDRAW_SPACER);
        voteSlots++;
    }
    if (blockSlots > 0)
    {
        AddMenuItem(newMenu, NOTHING_OPTION, "Slot Block Message 1", ITEMDRAW_DISABLED);
        voteSlots++;
    }
    if (blockSlots > 1)
    {
        AddMenuItem(newMenu, NOTHING_OPTION, "Slot Block Message 2", ITEMDRAW_DISABLED);
        voteSlots++;
    }
    if (blockSlots > 2)
    {
...
bool:RandomNextMap(Handle:excluded, Handle:excludedCats, numEcats, bool:ignoreCatWeights=false,
                   bool:ignorePlayerLimits=false, bool:ignoreTimeLimits=false) 
{    
    ...
    else //Otherwise there isn't a set next category, so we get a random one from the mapcycle.
    {
        ...
        if (ignoreCatWeights)
        {
            ...
            new bool:result = RandomNextMapIgnoreCatWeight(kv, excludedCats, numEcats, excluded,
                                                           !ignoreTimeLimits, !ignorePlayerLimits);
            CloseHandle(kv);
            return result;
        }
        ...
        if (!GetRandomCat(kv, catName, sizeof(catName), excludedCats, numEcats, excluded,
                          !ignoreTimeLimits, !ignorePlayerLimits)) //This grabs the random category.
        {
            LogError(
    ...
    //Set up the cat config
    KvGetString(kv, "command", cat_config, sizeof(cat_config));
                                  ...
                                  Handle:excludedMaps, bool:checkTime=true, bool:checkPlayers=true)
{
    ...
    decl String:nextMapGroup[MAP_LENGTH], String:mapName[MAP_LENGTH],
         String:catName[MAP_LENGTH], String:catConfig[255], String:catPConfig[255];
    ...
    do //For each category
    {
        ...
        KvGetString(oldkv, "pre-command", catPConfig, sizeof(catPConfig), "");
        ...
        do //For each map
        {
            ...
            KvSetString(kv, "catpconfig", catPConfig);
    ...
    decl String:pre[255];
    KvGetString(kv, "catpconfig", pre, sizeof(pre));
    if (strlen(pre) > 0)
    {
        LogMessage("SETUP: Executing map group precommand: '%s'", pre);
        ServerCommand(pre);
    }
    KvGetString(kv, "pre-command", pre, sizeof(pre));
    if (strlen(pre) > 0)
    {
        LogMessage("SETUP: Executing map precommand: '%s'", pre);
        ServerCommand(pre);
                  ...
                  bool:checkTime=true, bool:checkPlayers=true)
{
    ...
    {    
        ...
        if (!IsValidMap(kv, catName, defaults, excluded, excludedCats, checkTime, checkPlayers))
        {
        ...
        PushArrayCell(weightArr, GetMapWeight(kv));
                  ...
                  Handle:excluded, bool:checkTime=true, bool:checkPlayers=true)
{
    ...
    do
    {
        ...
        PushArrayCell(weightArr, KvGetFloat(kv, "group_weight", 1.0));
        PushArrayString(nameArr, temp);
...
Float:GetMapWeight(Handle:kv)
{
    //Get the name of the map (and perhaps other relevant information
    decl String:map[MAP_LENGTH];
    KvGetSectionName(kv, map, sizeof(map));
    
    //Get the starting weight
    current_weight = KvGetFloat(kv, "weight", 1.0);
    
    reweight_active = true;
    
    Call_StartForward(reweight_forward);
    Call_PushString(map);
    //Call_PushFloatRef(weight);
    Call_Finish();
                       ...
                       bool:checkPlayers=true)
{
    ...
    new bool:result = IsValidMap(kv, catName, defaults, excluded, excludedCats, checkTime, checkPlayers);
...
//              have them defined.
bool:IsValidMap(Handle:kv, const String:catName[], Handle:defaults=INVALID_HANDLE,
                Handle:excluded=INVALID_HANDLE, Handle:excludedCats=INVALID_HANDLE,
                bool:checkTime=true, bool:checkPlayers=true)
{    
    //Get the name of the map
    ...
    DebugMessage("Initializing limit storage.");
    ...
    DebugMessage("Starting exclusion traversal...");
    new index = -1;
    ...
    if (excluded != INVALID_HANDLE && excludedCats != INVALID_HANDLE)
    {
        do
        {
            ...
            if (index >= 0 && GetArraySize(excludedCats) > index)
            {
                ...
                DebugMessage("Map Excluded? %i (%s | %s)", _:isExcluded, exCat, catName);
            }
...
bool:CheckNextCatValidity(Handle:kv, Handle:excluded, Handle:excludedMaps)
{
    //Return false instantly if the category is not set. Otherwise return the result of validity 
    //check
    return !StrEqual(next_rand_cat, "")
        && CheckCatValidity(kv, next_rand_cat, excludedMaps, excluded, 0);
}
...
//    excluded: an adt_array containing maps to be excluded from being picked.
bool:CheckCatValidity(Handle:kv, const String:cat[], Handle:excluded, Handle:exCats, numEcats)
{
    //Jump to the level of the category.
    KvJumpToKey(kv, cat);
    
    //Determine if the number of players is valid for the category.
    new bool:result = IsValidCat(kv, exCats, numEcats, excluded);
                ...
                bool:checkTime=true, bool:checkPlayers=true)
{
    ...
    do
    {
        ...
        if (IsValidMap(kv, catName, defaults, excludedMaps, excludedCats, checkTime, checkPlayers))
        {
            KvGoBack(kv);
            CloseHandle(defaults);
            return true;
...
ChangeToValidMap(Handle:cvar)
{
    switch (GetConVarInt(cvar))
    {
        case PLAYERLIMIT_ACTION_NOW: //Pick a map and change to it.
        {
            ...
            //Pick a map.
            RandomNextMap(randnext_mem_arr, randnext_catmem_arr, GetConVarInt(cvar_randnext_catmem),
                          GetConVarBool(cvar_ignoregweight));
        ...
        case PLAYERLIMIT_ACTION_VOTE: //Run a full mapvote.
        {
            ...
            if (!IsVoteInProgress())
            {
                ...
                if (menu == INVALID_HANDLE)
                {
                    LogMessage("PLAYERLIMITS: Menu could not be built, aborting vote.");
                    return;
                }
                
                //Display a message.
                PrintToChatAll("\x03[UMC]\x01 %t", "Starting Map Vote");
...
Handle:BuildNominationMenu(client, const String:cat[]="")
{
    ...
    //Variables
    new numCells = ByteCountToCells(MAP_LENGTH);
    nomination_cats[client] = CreateArray(numCells); //Array to store categories for each nomination
    nomination_weights[client] = CreateArray(); //Array to store weights for each nomination
    new Handle:menuItems = CreateArray(numCells);    //Array to store each nominated map
...
Handle:BuildTieredNominationMenu()
{
    ...
    //Variables
    new Handle:menuItems = CreateArray(ByteCountToCells(MAP_LENGTH));    //Array to store each nominated map
    decl String:catName[MAP_LENGTH];   //Temporary buffer for each category name.
    new counter = 0;    //Counts number of nominations added to the menu.
    ...
    do
    {
        ...
        if (!IsValidCat(map_kv, INVALID_HANDLE, 0, INVALID_HANDLE,
                        !(ignoreTimeLimits || displayTimeLimits),
                        !(ignorePlayerLimits || displayPlayerLimits)))
            continue;
    ...
    switch (action)
    {
        case MenuAction_Select: //The client has picked something.
        {
            ...
            if (index != -1)
            {
                ...
                GetTrieString(oldNom, "map", oldName, sizeof(oldName));

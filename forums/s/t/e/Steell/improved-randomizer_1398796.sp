...
{
    name        = "Improved Map Randomizer",
    author      = "Steell",
    description = "Provides more precise control over randomly selected maps.",
    version     = PL_VERSION,
...
{    
    ...
    cvar_invalid_min = CreateConVar(
        "sm_random_playerlimit_minaction",
        "0",
    ...
    cvar_invalid_max = CreateConVar(
        "sm_random_playerlimit_maxaction",
        "0",
    ...
    cvar_invalid_post = CreateConVar(
        "sm_random_playerlimit_voteaction",
        "0",
    ...
    cvar_version = CreateConVar(
        ...
        PL_VERSION,
        "Improved Map Randomizer's version",
    ...
    AutoExecConfig(true, "improved_map_randomizer");
    ...
    RegAdminCmd(
        ...
        Command_Vote,
        ADMFLAG_RCON,
        "Starts a dynamic mapvote."
    );

    //Admin command to reload the mapcycle.
    RegAdminCmd(
        "sm_random_reload_file",
        Command_Reload,
        ADMFLAG_RCON,
        "Reloads the random mapcycle file."
    );

    //Admin commmand to pick a random nextmap.
    RegAdminCmd(
        "sm_random_picknextmapnow",
        Command_Random,
        ADMFLAG_RCON,
        "Repicks the random nextmap."
    );
...
Handle:GetMapcycle()
{
    ...
    new Handle:result = GetKvFromFile(filename, "random_rotation");
...
DisplayVoteWarning(Handle:warning)
{
    ...
    if (strlen(sound) > 0)
    {
        LogMessage("Playing sound: %s", sound);
...
public Action:OnPlayerChat(client, const String:command[], argc)
{
    ...
    {
        decl String:temp[9];
        GetConVarString(cvar_version, temp, sizeof(temp));
        PrintToChat(
            client,
            "[SM] Improved Map Randomizer v%s by Steell",
            temp
        );
    }
}
...
Handle:GetVoteWarnings()
{
    ...
    do
    {
        ...
        warning = CreateTrie();
        SetTrieValue(warning, "time", warningTime);
...
SetupVoteWarnings()
{
    ...
    if (GetConVarBool(cvar_warnings))
    {
        ...
        //Start the warnings at the beginning.
        next_warning = 0;
    }
}

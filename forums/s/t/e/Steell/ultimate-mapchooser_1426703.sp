...
/*
2.3.3 (3/2/11)
Removed feature where vote menu would not disappear due to uncontrollable behavior.
Fixed bug that caused server crashes during map votes.
Fixed behavior of ignorelimits cvars and nominations.

2.3.2 (3/1/11)
Fixed bug where the timer to keep menus visible was spamming the error log.
Fixed bug where having the same map in different groups could cause invalid vote results.
...
Made vote menus reappear on screen if they are removed by a panel or menu.
Removed map duplicate checking in votes. This is required if people want to have the same map in different groups appear in the vote.
Updated logging a bit.
...
//    found.
// Update pagination to check for the correct style, and use that to determine the items/page.
// Cvar to enable/disable duplicate checking.
// Fix selective runoffs to keep track of previous votes, add new votes to the vote pool.
// Fix vote arrays so they work with duplicate maps.
// Remove hardcoding of all array/string sizes.
//
//TESTING:
// Tiered votes to make sure the correct choice is being picked.
// 

/* enum Map
{
    String:Map_Name[64],
    String:Map_GroupName[64]
} */
...
new String:normal_winning_map[255];

//Is the visibility timer alive?
//new bool:vis_timer_alive;
...
RunTests()
{
    ...
    new aMap[Map];
    strcopy(aMap[Map_Name], sizeof(aMap[Map_Name]), "cp_dustbowl");
    strcopy(aMap[Map_GroupName], sizeof(aMap[Map_GroupName]), "Attack / Defend");
    LogMessage("DEBUG: %i", Map);
...
public OnPluginStart()
{
    ...
    cvar_nominate_timelimits = CreateConVar(
        "sm_umc_nominate_timelimits",
        "0",
        "If enabled, maps with time limits that the server is currently violating\nwill not be displayed in the nomination menu. Note that this does nothing\nif \"sm_umc_nominate_ignorelimits\" is set to ignore timelimits.",
        0, true, 0.0, true, 1.0
    );
    ...
    vote_mem_arr        = CreateArray(255);
    vote_catmem_arr     = CreateArray(255);
    rtv_mem_arr         = CreateArray(255);
    rtv_catmem_arr      = CreateArray(255);
    randnext_mem_arr    = CreateArray(255);
    randnext_catmem_arr = CreateArray(255);
    
    //Initialize our vote arrays
    map_vote_cat_configs = CreateArray(255);
    map_vote_map_configs = CreateArray(255);
    map_vote_next_cats   = CreateArray(255);
...
FullReset()
{
    ...
    //No votes should be visible
    //vis_timer_alive = false;
...
public Native_GetExcludeMapList(Handle:plugin, numParams)
{
    ...
    decl String:catName[255], String:mapName[255];
    do
    {
        KvGetSectionName(kv, catName, sizeof(catName));
        if (FindStringInArray(vote_catmem_arr, catName) != -1)
        {
            if (KvGotoFirstSubKey(kv))
            {
                do
        ...
        else
        {
            if (KvGotoFirstSubKey(kv))
            {
                ...
                {
                    KvGetSectionName(kv, mapName, sizeof(mapName));
                    if (FindStringInArray(vote_mem_arr, mapName) != -1)
                        PushArrayString(array, mapName);
                } while (KvGotoNextKey(kv));
            }
        }
    ...
    switch (GetConVarInt(cvar_vote_type))
    {
        case VOTE_TYPE_MAP: //If the vote is a Map Vote...
        {
            return BuildMapVoteMenu(Handle_MapVoteResults, GetConVarBool(cvar_scramble),
                                    ...
                                    false, ignorePlayerLimits, ignoreTimeLimits);
        }
                             ...
                             const item_info[][2])
{
    ...
    {    
        ...
        new index = StringToInt(result);//FindStringInArray(map_vote, map);
        
        //decl map[Map];
        //GetArrayArray(map_vote, index, map[0], sizeof(map));
        //strcopy(next_cat, sizeof(next_cat), map[Map_GroupName]);
        
        decl String:mName[64];
        new Handle:map = GetArrayCell(map_vote, index);
        GetTrieString(map, "map", mName, sizeof(mName));
        GetTrieString(map, "group", next_cat, sizeof(next_cat));
        ...
        LogMessage("MAPVOTE: Players voted for map '%s' from group '%s'", mName, next_cat);
    ...
    else //Otherwise, we pick a random map from the category and set that as the next map.
    {
        ...
        if (GetArraySize(nominationsFromCat) > 0)
        {
            ...
            for (new i = 0; i < arraySize; i++)
            {
                ...
                if (IsValidMapFromCat(map_kv, nameBuffer, .checkTime=!ignoreNomTimeLimits,
                            ...
                            const item_info[][2])
{
    ...
    else //Otherwise, we print a message and then set the new map.
    {
        ...
        LogMessage("RTV: Players voted for map '%s' from group '%s'", mName, next_cat);
    ...
    else //Otherwise, we pick a random map from the category and change to it.
    {
        ...
        if (GetArraySize(nominationsFromCat) > 0)
        {
            ...
            for (new i = 0; i < arraySize; i++)
            {
                ...
                if (IsValidMapFromCat(map_kv, nameBuffer, .checkPlayers=!ignoreNomPlayerLimits,
                                      .checkTime=!ignoreNomTimeLimits))
                {
                    //Increment the nomination counter.
                    counter++;
...
Handle:BuildMapVoteMenu(const VoteHandler:callback, bool:scramble, bool:extend, bool:dontChange, 
                        ...
                        bool:ignoreInvoteSetting=false, bool:ignorePlayerLimits=false,
                        bool:ignoreTimeLimits=false)
{
    ...
    decl String:display[255];     //String to be displayed in the vote
    decl String:gDisp[255];
    ...
    new Handle:map_vote_display = CreateArray(255);
    ...
    do
    {
            ...
            if (numNoms > 0)
            {
                ...
                if (nomCounter > 0)
                {
                    ...
                    for (new i = 0; i < min; i++)
                    {
                        ...
                        //strcopy(map[Map_Name], sizeof(map[Map_Name]), mapName);
                        //strcopy(map[Map_GroupName], sizeof(map[Map_GroupName]), catName);
                        //InsertArrayArray(map_vote, position, map[0]);
        ...
        else
        {
            ...
            for (new i = 0; i < numNoms; i++)
            {
                ...
                else
                {
                    ...
                    //Remove this map from the cycle so it cannot be picked again.
                    KvDeleteKey(kv, mapName);
        ...
        while (numMapsFromCat > 0)
        {
            ...
            if (!GetRandomMap(kv, mapName, sizeof(mapName), excluded, !ignoreTimeLimits,
                              !ignorePlayerLimits))
            {
                if (verboseLogs)
    ...
    if (blockSlots > 0)
    {
        AddMenuItem(menu, "?nothing?", "Slot Block Message 1", ITEMDRAW_DISABLED);
        voteSlots++;
    }
    if (blockSlots > 1)
    {
        AddMenuItem(menu, "?nothing?", "Slot Block Message 2", ITEMDRAW_DISABLED);
        voteSlots++;
    }
    if (blockSlots > 2)
    {
    ...
    new Handle:info_array = BuildNumArray(voteCounter);
    
    //Add the array of votes to the menu.
    AddArrayToMenu(menu, info_array, map_vote_display);
    CloseHandle(map_vote_display);
    CloseHandle(info_array);
    ...
    if (extend)
    {
        AddMenuItem(menu, "?Extend?", "Extend");
        voteCounter++;
        voteSlots++;
    }
    ...
    {
        //new Handle:menuStyle = GetMenuStyle(menu);
        
        //Disable pagination if...
        //    ...the number of slots taken up is less than or equal to 9.
        if (voteSlots <= 9)
                        ...
                        bool:ignorePlayerLimits=false, bool:ignoreTimeLimits=false)
{
    ...
    do
    {
        ...
        if (!haveNoms &&
            !IsValidCat(map_kv, excludedCats, excluded, !ignoreTimeLimits, !ignorePlayerLimits))
        {
            if (verboseLogs)
            {
                LogMessage(
...
public Handle_VoteMenu(Handle:menu, MenuAction:action, param1, param2)
{
    ...
    if (action == MenuAction_DisplayItem)
    {
        ...
        if (StrEqual(map, "?Extend?") || StrEqual(map, "?DontChange?") ||
            (StrEqual(map, "?nothing?") && strlen(display) > 0))
        {
            decl String:buffer[255];
...
GetWinner(num_items, const item_info[][2])
{
    ...
    for (counter = 1; counter < num_items; counter++)
    {
        if (item_info[counter][VOTEINFO_ITEM_VOTES] < most_votes)
            break;
    }
    return (counter > 1) ? GetRandomInt(0, counter - 1) : 0;
}
                       ...
                       const item_info[][2], VoteHandler:callback, Handle:clientArray)
{
    ...
    new runoffThreshold = GetConVarInt(cvar_runoff_threshold);
    ...
    {
        //Update percentage
        newpercent = float(item_info[i][VOTEINFO_ITEM_VOTES]) / float(num_votes) * 100;
        
        percent += newpercent;
        
        //Add item to removed items array.
        PushArrayCell(removedItems, item_info[i][VOTEINFO_ITEM_INDEX]);
        ...
        if (verboseLogs)
        {
            ...
            LogMessage(
                ...
                disp, item_info[i][VOTEINFO_ITEM_VOTES], newpercent
            );
        }
    ...
    for (new i = 0; i < num_clients; i++)
    {
        ...
        clientVotes[client_info[i][VOTEINFO_CLIENT_INDEX]] = true;
    }
    ...
    for (new i = 0; i < sizeof(clientVotes); i++)
    {
        if (!clientVotes[i])
        {
            ...
            /*
            if (verboseLogs && IsClientConnected(i) && !IsFakeClient(i))
            {
                LogMessage(
...
/*
//Broadcasts a menu to all clients, and keeps the menu visible until the vote is complete.
bool:VoteMenuToAllEx(Handle:menu, time, flags=0)
{
    MakeVisTimer();
    return bool:VoteMenuToAll(menu, time, flags);
}


//Broadcasts a menu to a list of clients, and keeps the menu visible until the vote is complete.
bool:VoteMenuEx(Handle:menu, clients[], numClients, time, flags=0)
{
    MakeVisTimer();
    return VoteMenu(menu, clients, numClients, time, flags);
}


//Creates the visibility timer used to keep votes visible.
MakeVisTimer()
{
    vis_timer_alive = true;
    CreateTimer(1.0, Handle_VoteVisibleTimer, INVALID_HANDLE, TIMER_FLAG_NO_MAPCHANGE|TIMER_REPEAT);
}


//Kills the visibility timer.
KillVisTimer()
{
    vis_timer_alive = false;
}
...
//Handles each tick of the visibility timer.
public Action:Handle_VoteVisibleTimer(Handle:timer)
{
    if (!vis_timer_alive)
        return Plugin_Stop;
...
{    
    ...
    else //Otherwise there isn't a set next category, so we get a random one from the mapcycle.
    {
        ...
        if (!GetRandomCat(kv, catName, sizeof(catName), excludedCats, excluded,
                                  ...
                                  bool:checkTime=true, bool:checkPlayers=true)
{
    ...
    if (!GetRandomMap(kv, mapName, sizeof(mapName), excludedMaps, checkTime, checkPlayers))
    {
...
bool:GetRandomMap(Handle:kv, String:buffer[], size, Handle:excluded, bool:checkTime=true,
                  bool:checkPlayers=true)
{
    return GetWeightedRandomMap(kv, buffer, size, "weight", excluded, checkTime, checkPlayers);
}
                          ...
                          bool:checkTime=true, bool:checkPlayers=true)
{
    ...
    {    
        ...
        if (!IsValidMap(kv, defaults, excluded, checkTime, checkPlayers))
            continue;
                          ...
                          bool:checkTime=true, bool:checkPlayers=true)
{
    ...
    do
    {
        ...
        if (!IsValidCat(kv, excludedCats, excluded, checkTime, checkPlayers))
            continue;
                       ...
                       bool:checkTime=true, bool:checkPlayers=true)
{
    ...
    new bool:result = IsValidMap(kv, defaults, excluded, checkTime, checkPlayers);
                ...
                bool:checkTime=true, bool:checkPlayers=true)
{
    ...
    return IsMapValid(mapName) 
        && (!checkPlayers || (numplayers >= minp && numplayers <= maxp)) 
...
bool:IsValidCat(Handle:kv, Handle:excludedCats, Handle:excludedMaps, bool:checkTime=true,
                bool:checkPlayers=true)
{
    ...
    do
    {
        ...
        if (IsValidMap(kv, defaults, excludedMaps, checkTime, checkPlayers))
        {
            KvGoBack(kv);
            CloseHandle(defaults);
            return true;
...
CountValidMapsFromCat(Handle:kv, Handle:excludedMaps, bool:checkTime=true, bool:checkPlayers=true)
{
    new result = 0;
                                   ...
                                   num_items, const item_info[][2])
{
    ...
    if (!StrEqual(result, "?no?"))
    {
        ...
        //Change it.
        SetupMapChange(GetConVarInt(cvar_invalid_post), mName, "PLAYERLIMITS");
...
Handle:BuildNominationMenu(client, const String:cat[]="")
{
    ...
    new Handle:menuItemDisplay = CreateArray(255);
    ...
    decl String:display[255], String:gDisp[255];
    ...
    do
    {
        ...
        if (!KvGotoFirstSubKey(map_kv) || (GetConVarBool(cvar_strict_noms) && mapsInVote <= 0))
            continue;
        ...
        do
        {
            ...
            if (!(StrEqual(catName, current_cat) && StrEqual(mapName, currentMap)) 
                ...
                && (ignoreTimeLimits || !GetConVarBool(cvar_nominate_timelimits) ||
                    IsTimeBetween(tmin, tmax)))
            {
...
Handle:BuildTieredNominationMenu()
{
    ...
    do
    {
        ...
        if (!IsValidCat(map_kv, INVALID_HANDLE, INVALID_HANDLE, .checkPlayers=false))
            continue;
...
{
    decl String:mName[255];
    decl String:gName[255];
    new Handle:nom = INVALID_HANDLE;
    new arraySize = GetArraySize(nominations_arr);
    for (new i = 0; i < arraySize; i++)
    {
...
//Inserts given array into given adt_array at given index.
stock InsertArrayArray(Handle:arr, index, const any:value[])
{
    if (GetArraySize(arr) > index)
    {
        ShiftArrayUp(arr, index);
        SetArrayArray(arr, index, value);
    }
    else
        PushArrayArray(arr, value);
}
...
{
    decl String:map[255], String:disp[255];

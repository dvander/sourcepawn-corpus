...
//      -Make a timer to retry votes while there is a vote currently in progress.
// Natives
//      -Add the nomination forward call to every possible place
// Nominations can be grouped (like tiered) and extra cvars to limit maps in the menu by their time limits.
// mp_winlimit
...
public OnPluginStart()
{
    ...
    /*cvar_nominate_tiered = CreateConVar(
        ...
        "0",
        "Organizes the nomination menu so that users select a group first, then\na map. Useful for servers with a lot of maps and groups.",
        0, true, 0.0, true, 1.0
    );*/
...
public Native_InitiateNormalVote(Handle:plugin, numParams)
{
    ...
    else
    {
        ...
        SetVoteResultCallback(menu, Handle_NormalVoteMenuResults); //CALLBACKS
...
NominateResult:InternalNominateMap(String:map[], bool:force, owner)
{
    ...
    CloseHandle(GetArrayCell(nominations_arr, exists));
                        ...
                        bool:ignoreInvoteSetting=false)
{
    ...
    do
    {
        ...
        while (numMapsFromCat > 0)
        {
            ...
            if (nomIndex != -1)
            {
                CloseHandle(GetArrayCell(nominations_arr, nomIndex));
...
bool:IsValidMap(Handle:kv, Handle:defaults=INVALID_HANDLE, Handle:excluded=INVALID_HANDLE)
{
    ...
    return IsMapValid(mapName) 
        && numplayers >= minp && numplayers <= maxp 
        ...
        */
        ...
        do
        {
            ...
            if (!StrEqual(mapName, currentMap) && FindStringInArray(menuItems, mapName) == -1 
                ...
                && (!GetConVarBool(cvar_nominate_timelimits) || IsTimeBetween(tmin, tmax)))
            {

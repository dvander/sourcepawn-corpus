...
// Fix RTV's being set to complete before they really are complete (ie. account for case where there are no votes).
//
...
RunTests()
{
    ...
    new Handle:style = GetMenuStyleHandle(MenuStyle_Default);
    new Handle:gStyle = GetMenuStyleHandle(MenuStyle_Valve);
    LogMessage("DEBUG: Style - %i, %i", style, gStyle);
    if (style == gStyle)
        LogMessage("DEBUG: Styles are equal!");
                        ...
                        bool:ignoreTimeLimits=false)
{
    ...
    //Buffers
    decl String:mapName[MAP_LENGTH];     //Name of the map
    decl String:display[MAP_LENGTH];     //String to be displayed in the vote
    decl String:gDisp[MAP_LENGTH];
    decl String:catName[MAP_LENGTH];     //Name of the category.
    decl String:nextCatName[MAP_LENGTH]; //Name of the next category to be set (if the map wins the vote).
        ...
        if (numMapsFromCat < 0 && GetConVarBool(cvar_strict_noms))
        {
            ...
            if (numNoms > 0)
            {
                ...
                for (new i = 0; i < numNoms; i++)
                {
                    ...
                    //if...
                    //    ...the map is valid (correct number of players, correct time)
                    if (!ignoreDupes && FindStringInArray(map_vote, mapName) != -1)
                    {
                        if (verboseLogs)
                        {
                            LogMessage(
...
Handle:MakeSecondTieredCatExclusion(const String:cat[])
{
    ...
    new Handle:result = CreateArray(MAP_LENGTH, 0);
             ...
             const client_info[][2])
{
    ...
    {
        GetMenuItem(menu, item_info[i][VOTEINFO_ITEM_INDEX], infoBuffer, sizeof(infoBuffer), _, 
        ...
        {
            if (client_info[j][VOTEINFO_CLIENT_ITEM] == item_info[i][VOTEINFO_ITEM_INDEX])
                       ...
                       const item_info[][2], VoteHandler:callback, Handle:clientArray)
{
    ...
    for (new i = 0; i < num_items; i++)
    {
        GetMenuItem(menu, item_info[i][VOTEINFO_ITEM_INDEX], info, sizeof(info));
        GetTrieValue(vote_storage, info, value);
        if (value > max)
        {
            max = value;
            winner = i;
    ...
    new Float:percent = float(max) / float(num_votes) * 100;
    ...
    for (new i = num_items - 1; i > 0; i--)
    {
        ...
        //Fetch relevant info from the menu;
        GetMenuItem(menu, item_info[i][VOTEINFO_ITEM_INDEX], buf, sizeof(buf), _, disp,
            sizeof(disp));
            
        GetTrieValue(vote_storage, buf, itemVotes);
    
        //Update percentage
        newpercent = float(itemVotes) / float(num_votes) * 100;
        ...
        //Remove the item from the vote storage.
        RemoveFromTrie(vote_storage, buf);
        
        if (verboseLogs)
        {
            LogMessage(
                "RUNOFF MENU: (Verbose) '%s' was removed from the vote. It had %i votes (%.f%% of total)",
                disp, itemVotes, newpercent
            );
        }
                ...
                Handle:excludedCats=INVALID_HANDLE, bool:checkTime=true, bool:checkPlayers=true)
{
    ...
    new index = -1;
    if (excluded != INVALID_HANDLE)
        index = FindStringInArray(excluded, mapName);
    ...
    return IsMapValid(mapName) 
        ...
        && (index == -1 || excludedCats == INVALID_HANDLE ||
            FindStringInArray(excludedCats, catName) != index);
}
...
{
    new max = (GetMenuStyleHandle(MenuStyle_Default) == GetMenuStyleHandle(MenuStyle_Radio))
                ? 10 
                : 8;

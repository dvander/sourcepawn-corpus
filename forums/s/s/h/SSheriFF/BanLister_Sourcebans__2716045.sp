...
{
	name = "Ban lister for Sourcebans++",
	author = "SheriF",
...
public void OnClientPostAdminCheck(int client)
{
	if(CheckCommandAccess(client,"",ADMFLAG_GENERIC,true))
	{
		...
		FormatEx(szQuery, sizeof(szQuery), "SELECT `aid` FROM sb_admins WHERE authid LIKE 'STEAM__:%s';", sId[8]);
...
public Action Command_LastBans(int client,int args)
{
	...
	if(args==1)
	{
		...
		else
		{
			...
			char szQuery[QUERY_DEFAULT_SIZE];
			FormatEx(szQuery, sizeof(szQuery), "SELECT `aid`, `removedby`, `bid`, REVERSE(name), `authid`, `ip` FROM sb_bans ORDER BY created DESC LIMIT %d;", iAmount);
			dp.WriteCell(client);
	...
	else if (args==0)
	{
		...
		char szQuery[QUERY_DEFAULT_SIZE];
		FormatEx(szQuery, sizeof(szQuery), "SELECT `aid`, `removedby`, `bid`, REVERSE(name), `authid`, `ip` FROM sb_bans ORDER BY created DESC LIMIT %d;", g_iMaxBansAmount);
		dp.WriteCell(client);
...
public Action Command_BanList(int client,int args)
{
	...
	if(args==1)
	{
		...
		else
		{
			...
			char szQuery[QUERY_DEFAULT_SIZE];
			FormatEx(szQuery, sizeof(szQuery), "SELECT `created`, `aid`, `removedby`, `bid` FROM sb_bans WHERE authid LIKE 'STEAM__:%s' ORDER BY `bid` DESC LIMIT 1000", sId[8]);
        	dp.WriteString(sId);
        	dp.WriteCell(client);
...
public void Command_LastBans_Callback(Handle owner, Handle hndl, const char[] error, DataPack data)
{
	...
	if(row_count > 0)
	{
		...
		while(SQL_FetchRow(hndl))
		{
			...
			char szQuery[QUERY_DEFAULT_SIZE];
			FormatEx(szQuery, sizeof(szQuery), "SELECT `user`, `aid` FROM sb_admins WHERE aid = %d", SQL_FetchInt(hndl, 0));
			SQL_TQuery(g_dbDataBase, Command_BanList_CallbackAdmin, szQuery,client, DBPrio_High);
			char szQuery2[QUERY_DEFAULT_SIZE];
			FormatEx(szQuery2, sizeof(szQuery2), "SELECT `user`, `aid` FROM sb_admins WHERE aid = %d", SQL_FetchInt(hndl, 1));
...
public void Command_BanList_Callback(Handle owner, Handle hndl, const char[] error, DataPack data)
{
	...
	if(row_count > 0)
	{
		...
		while(SQL_FetchRow(hndl))
		{
			...
			char szQuery[QUERY_DEFAULT_SIZE];
			FormatEx(szQuery, sizeof(szQuery), "SELECT `user`, `aid` FROM sb_admins WHERE aid = %d", SQL_FetchInt(hndl, 1));
			SQL_TQuery(g_dbDataBase, Command_BanList_CallbackAdmin, szQuery,client, DBPrio_High);
			char szQuery2[QUERY_DEFAULT_SIZE];
			FormatEx(szQuery2, sizeof(szQuery2), "SELECT `user`, `aid` FROM sb_admins WHERE aid = %d", SQL_FetchInt(hndl, 2));
			SQL_TQuery(g_dbDataBase, Command_BanList_CallbackUnbanAdmin, szQuery2,client, DBPrio_High);
		}
	...
	if(row_count > 0)
	{
		...
		while(SQL_FetchRow(hndl))
		{
			...
			if(iBanId==SQL_FetchInt(hndl, 7))
			{
				...
				SQL_FetchString(hndl, 10, sRemoveType, sizeof(sRemoveType));
				...
				if(SQL_FetchInt(hndl, 6)!=0 && StrEqual(sRemoveType,"U"))
				{
					char UnbanAdminID[5];
				...
				dp.WriteString(sRemoveType);
				char szQuery2[QUERY_DEFAULT_SIZE];
				FormatEx(szQuery2, sizeof(szQuery2), "SELECT `group_id` FROM sb_admins_servers_groups WHERE admin_id = %d", SQL_FetchInt(hndl, 5));
...
public int Command_BanList_MenuHandler(Menu menu, MenuAction action, int client, int ItemNum)
{
	if(action == MenuAction_Select)
	{
		...
		char szQuery[QUERY_DEFAULT_SIZE];
		FormatEx(szQuery, sizeof(szQuery), "SELECT `created`, REVERSE(name), REVERSE(reason), `length`, `removedon`, `aid`, `removedby`, `bid`, `ip`, `authid`, `removetype` FROM sb_bans WHERE bid=%d", iBanId);
		DataPack dp = new DataPack();
        dp.WriteCell(iBanId);
        dp.WriteCell(client);
	...
	switch (action)
	{
		...
		case MenuAction_Select:
		{
			...
			if(StrEqual(sItem,g_sViewsId[client]))
			{
				DataPack dp = new DataPack();
				char szQuery[QUERY_DEFAULT_SIZE];
				FormatEx(szQuery, sizeof(szQuery), "SELECT `created`, `aid`, `removedby`, `bid` FROM sb_bans WHERE authid LIKE 'STEAM__:%s' ORDER BY `bid` DESC LIMIT 1000", g_sViewsId[client][8]);
...
public int Command_BanList_UnbanMenu(Menu menu, MenuAction action, int client, int ItemNum)
{
	...
	if(action == MenuAction_Select)
	{
		...
		if(!StrEqual(sItem,"No"))
		{
			...
			FormatEx(szQuery, sizeof(szQuery), "UPDATE sb_bans SET removedon = %d, removedby = %d, removetype = 'U' WHERE bid = %d", GetTime(), StringToInt(strings[1]),StringToInt(strings[0]));
...
public int Command_BanList_DeleteMenu(Menu menu, MenuAction action, int client, int ItemNum)
{
	if(action == MenuAction_Select)
	{
		...
		{
			char szQuery[QUERY_DEFAULT_SIZE];
			FormatEx(szQuery, sizeof(szQuery), "DELETE FROM sb_bans WHERE bid = %d", StringToInt(sItem));
...
public void PostAdminCheckCallback(Handle owner, Handle hndl, const char[] error, any client)
{
	...
	if(row_count > 0)
	{
		while(SQL_FetchRow(hndl))
		{
			...
			char szQuery[QUERY_DEFAULT_SIZE];
			FormatEx(szQuery, sizeof(szQuery), "SELECT `group_id` FROM sb_admins_servers_groups WHERE admin_id = %d",g_iAdminIds[client]);
...
public void GetClientGroupIdCallback(Handle owner, Handle hndl, const char[] error, any client)
{
	...
	if(row_count > 0)
	{
		...
		{
			char szQuery[QUERY_DEFAULT_SIZE];
			FormatEx(szQuery, sizeof(szQuery), "SELECT `immunity`, `flags` FROM sb_srvgroups WHERE id = %d", SQL_FetchInt(hndl,0));
...
public void GetGroupIdCallback(Handle owner, Handle hndl, const char[] error, DataPack data)
{
	...
	if(row_count > 0)
	{
		...
		{
			char szQuery[QUERY_DEFAULT_SIZE];
			FormatEx(szQuery, sizeof(szQuery), "SELECT `immunity` FROM sb_srvgroups WHERE id = %d", SQL_FetchInt(hndl,0));
	...
	if(row_count > 0)
	{
		...
		while(SQL_FetchRow(hndl))
		{
			if((SQL_FetchInt(hndl, 0)<g_iAdminImmunity[client]) || g_bAdminOwnsBan[client])
			{
				...
				char UBanId[32];
				char sUBanId[32];
				int iEndStamp2 = length + CreateTime;
				if((UnbanTime==0 && !StrEqual(sRemoveType,"U")) && (length==0 || iEndStamp2>GetTime()))
				{

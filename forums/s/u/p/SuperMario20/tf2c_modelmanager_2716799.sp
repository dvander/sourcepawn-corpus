...
#define PLUGIN_NAME				"[TF2C] Model Manager"
#define PLUGIN_AUTHOR			"SuperMario20 (Modified version of FlaminSarge's TF2 Model Manager)"
...
#define CLASS_CIVILIAN				(1 << 10)
#define CLASS_ALL					(0b11111111111)
...
public OnPluginStart()
{
	...
	// Plugin is TF2C only, so make sure it's ran on TF2classic
	decl String:strModName[32]; GetGameFolderName(strModName, sizeof(strModName));
	if (!StrEqual(strModName, "tf2classic"))
	{
		SetFailState("[SM] TF2C Model Manager is only for, of course, TF2C.");
		return;
	}
...
//	g_hCookies[TFClass_Spy][1]	  = RegClientCookie("tf_modelmanager_spy_1", "", CookieAccess_Protected);
	...
	g_hCookies[TFClass_Civilian]	= _:RegClientCookie("tf_modelmanager_civilian", "", CookieAccess_Protected);
//	g_hCookies[TFClass_Civilian][1] = RegClientCookie("tf_modelmanager_civilian_1", "", CookieAccess_Protected);
//	g_hCookies[TFClass_Civilian][2] = RegClientCookie("tf_modelmanager_civilian_2", "", CookieAccess_Protected);
...
public OnConfigsExecuted()
{
	...
	{
		LogError("[TF2C] Model Manager: WARNING- Your config file might be outdated! This may lead to conflicts with \
...
//		if (g_iItemFlags[g_iItemCount] & FLAG_INVISREQ) g_Invis_Req[g_iItemCount] = 1;
		...
		if (!(g_iItemFlags[g_iItemCount] & FLAG_INVISIBLE))
		{
			...
			{
				LogError("[TF2C] Model Manager: File \"%s\" not found. Excluding from list.", g_strItemModel[g_iItemCount]);
				continue;
			}
...
//			}
			...
			if (FileExists(strDependencies, true))
			{
				...
				{
					if (!FileExists(strDependencies)) Format(strExtraError, sizeof(strExtraError), " The .dep file must be moved to tf2classic/%s.dep, if it is in the tf2classic/custom/ folder.", g_strItemModel[g_iItemCount]);
					else strExtraError = "";
					LogError("[TF2C] Model Manager: Error, can't read file containing model dependencies (\"%s.dep\").%s", g_strItemModel[g_iItemCount], strExtraError);
					return;
				}
	...
	if (g_iItemCount == MAX_ITEMS) LogMessage("[TF2C] Model Manager: Max number of models (%d) found, no more will be added to the list to help prevent overflowing server precache tables", MAX_ITEMS);
...
Item_ParseClasses(String:strClasses[])
{
	...
	if (StrContains(strClasses, "CIVILIAN",	false)	!= -1) iFlags |= CLASS_CIVILIAN;
...
Item_ParseTeams(String:strTeams[])
{
	...
	if (StrContains(strTeams, "GRN", false) != -1) iFlags |= TEAM_GRN;
	if (StrContains(strTeams, "YLW", false) != -1) iFlags |= TEAM_YLW;
	if (StrContains(strTeams, "ALL", false) != -1)  iFlags |= TEAM_RED|TEAM_BLU|TEAM_GRN|TEAM_YLW;
...
/*	switch(class)
	{
		...
		case TFClass_Civilian:			return CLASS_CIVILIAN;
	}
...
/*	switch(GetClientTeam(client))
	{
		...
		case TFTeam_Green:  return TEAM_GRN;
		case TFTeam_Yellow:  return TEAM_YLW;
	}
...
public Action:Cmd_Menu(client, args)
{
	...
	if (!IsValidClient(client)) { ReplyToCommand(client, "[TF2C] Command is in-game only."); return Plugin_Handled; }
...
{
	if (args < 2) { ReplyToCommand(client, "[TF2C] Usage: tf_models_equip <#id|name> <item name>."); return Plugin_Handled; }
	...
	if (position == -1) { ReplyToCommand(client, "[TF2C] Usage: tf_models_equip <#id|name> <item name>."); return Plugin_Handled; }
	...
	if (iItem == -1) { ReplyToCommand(client, "[TF2C] Unknown item : \"%s\"", strItem); return Plugin_Handled; }
	if (foundcount != 1)
	{
		ReplyToCommand(client, "[TF2C] Found multiple items matching search string.\nTry: %s", names);
		return Plugin_Handled;
	}
...
//			decl String:strName[64]; GetClientName(iTargetList[i], strName, sizeof(strName));
	...
	if (bTargetTranslate)
		ReplyToCommand(client, "[TF2C] Forced model '%s' on %t", g_strItemName[iItem], strTargetName);
	else
		ReplyToCommand(client, "[TF2C] Forced model '%s' on %s", g_strItemName[iItem], strTargetName);
...
public Action:Cmd_RemoveItem(client, args)
{
	...
	if (args < 1) { ReplyToCommand(client, "[TF2C] Usage: tf_models_remove <#id|name>."); return Plugin_Handled; }
...
{
	// Determine if the number of arguments is valid
	if (args < 2) { ReplyToCommand(client, "[TF2C] Usage: tf_models_lock <#id|name> <state>"); return Plugin_Handled; }
	...
	if (bTargetTranslate)
		ReplyToCommand(client, "[TF2C] %s model for %t", bState ? "Locked" : "Unlocked", strTargetName);
	else
		ReplyToCommand(client, "[TF2C] %s model for %s", bState ? "Locked" : "Unlocked", strTargetName);
...
{
	// Determine if the number of arguments is valid
	if (args < 2) { ReplyToCommand(client, "[TF2C] Usage: tf_models_override <#id|name> <state>"); return Plugin_Handled; }
	...
	if (bTargetTranslate)
		ReplyToCommand(client, "[TF2C] %s model override for %t", bState ? "Enabled" : "Disabled", strTargetName);
	else
		ReplyToCommand(client, "[TF2C] %s model override for %s", bState ? "Enabled" : "Disabled", strTargetName);
...
public Action:Cmd_Reload(client, args)
{
	...
	// Reparse item list
	Item_ParseList();
	ReplyToCommand(client, "[TF2C] Model Manager: model list reloaded, found %d models.", g_iItemCount);
	// Done
	return Plugin_Handled;
}

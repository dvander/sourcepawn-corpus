...
{
	name = "Quake Sounds",
	author = "dalto, Grrrrrrrrrrrrrrrrrrr, and psychonic (update by superhalflifekid)",
...
public OnClientPutInServer(client)
{
	...
	SDKHook(client, SDKHook_TraceAttackPost, TraceAttackPostCB);
...
//check headshot
public Action TraceAttackPostCB(int victim, int& attacker, int& inflictor, float& damage, int& damagetype, int& ammotype, int hitbox, int hitgroup)
{	
...
{	
	...
	if(attackerClient>0 && attackerClient<=iMaxClients)
	{
		...
		else if(GetClientTeam(attackerClient) > 1 && GetClientTeam(victimClient) > 1 && GetClientTeam(attackerClient) == GetClientTeam(victimClient))
		{
		...
		else
		{
			...
			GetEntPropString(victimClient, Prop_Data, "m_ModelName", sModelName, sizeof(sModelName));
		
			new iHeadHitGroup = 1;
			if (StrContains(sModelName, "combine") >= 0) iHeadHitGroup = 4;
			
			new bool:headshot = false;
			if (hurtHitGroup[victimClient] == iHeadHitGroup)
			{	
				headshot = true;
			}

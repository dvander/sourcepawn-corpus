...
#define UU_VERSION "0.9.5g"
#define PLUGIN_VERSION "0.9.5g"
...
float lastRenamedTime[MAXPLAYERS+1] = {};

public Plugin myinfo = 
{
	name = "Uber Upgrades",
	author = "Mr L, modified by PC Gamer, bugfixed by SuperArtyK",
...
void ResetPlayerStuff(int client){
...
public ResetPlayer(int client){

	//god this plugin is something -- needs full rewrite
	
	//PrintToChatAll("Clieng %d wasn't valid!", client);
	...
	GetClientName(client, clientBaseName[client], sizeof(clientBaseName[client]))
	//PrintToChatAll("Got client name: %s", clientBaseName[client])
	...
	if (gamemode != MVM_GAMEMODE)
	{
		...
		if (maxCashtmp < GetConVarInt(cvar_StartMoney))
		{
			maxCashtmp = GetConVarInt(cvar_StartMoney)
...
public Action jointeam_callback(int client, const char[] command, int argc) //protection from spectators
{
	...
	{
		//PrintToChatAll("plr joined team");
		//ResetClientUpgrades(client)
		if (!client_respawn_handled[client])
		{
			CreateTimer(0.2, ClChangeClassTimer, GetClientUserId(client));
		...
		if (gamemode != MVM_GAMEMODE)
		{
			...
			iCashtmp = GetEntProp(client, Prop_Send, "m_nCurrency", iCashtmp);
			iCashtmp += client_spent_money[client][0]
			+client_spent_money[client][1]
			+client_spent_money[client][2]
			+client_spent_money[client][3];
			...
			if (iCashtmp < StartMoney)
			{
				iCashtmp = StartMoney
				SetEntProp(client, Prop_Send, "m_nCurrency", iCashtmp);
				LogMessage("gave to %N client $%d money", client, iCashtmp);			
			}			
		}
	}
	return Plugin_Continue;	
...
public Action ReloadCfgFiles(int client, int args)
{
	...
	//the _load_cfg_files() somehow breaks the plugin if to call it like that, but without it -- nothing works
	
	//garbage idea to just reload the plugin, as this spaghetti code is way too bad to work around
	//but hey, *that* doesn't break the plugin
	...
	while (iterator.Next())
    {
		...
		if(StrEqual("sm_buy", name)){
			GetPluginFilename(iterator.Plugin, plugname, sizeof(plugname));
			ServerCommand("sm plugins reload \"%s\"", plugname);
		}
	}
	
	
	//_load_cfg_files()
	//for (int cl = 1; cl < MAXPLAYERS + 1; cl++)
	//{
	//	if (IsValidClient(cl, true))
	//	{
	//		ResetPlayer(cl)
	//		current_class[cl] = _:TF2_GetPlayerClass(cl)
	//		PrintToChat(cl, "[SM] The Uber Upgrades config was reloaded! \nPlease change class for it to work properly (it breaks every reload)");
...
{
	if(!GetConVarBool(cvar_uu_EnableQbuy)){
		return Menu_QuickBuyUpgrade2(mclient, args);
	}
							...
							else
							{
								...
								else
								{
									...
									if (ctr_m != clientLevels[mclient])
									{
										...
										//SetClientName(mclient, clname);
										
										if((lastRenamedTime[mclient] + 15) <= GetGameTime()){
											SetClientName(mclient, clname);
											lastRenamedTime[mclient] = GetGameTime();
										}
...
public UberShopinitMenusHandlers()
{
	...
	cvar_uu_version = CreateConVar("uberupgrades_version", UU_VERSION, "The Plugin Version. Don't change. (man this plugin needs a rewrite)", FCVAR_NOTIFY);
	cvar_uu_EnableQbuy = CreateConVar("sm_uu_enable_qbuy", "1", "Enable (1)/Disable(0) the qbuy menu. Allegedly it qbuy has an exploit that allows to get unlisted upgrades/attributes", 0, true, 0.0, true, 1.0);
	cvar_MoneyBonusKill = 				CreateConVar("sm_uu_moneybonuskill", "100", "Sets the money bonus a client gets for killing: default 100", 0, true, 0.0);
	...
	cvar_StartMoney = 					CreateConVar("sm_uu_startmoney", "1500", "Sets the starting money", 0, true, 0.0);
	cvar_TimerMoneyGive_BlueTeam = 		CreateConVar("sm_uu_timermoneygive_blueteam", "100", "Sets the money blue team get every timermoney event");
	cvar_TimerMoneyGive_RedTeam =  		CreateConVar("sm_uu_timermoneygive_redteam", "100", "Sets the money blue team get every timermoney event");
	MoneyBonusKill = GetConVarInt(cvar_MoneyBonusKill)
	MoneyForTeamRatio[RED]  = 1.0
	MoneyForTeamRatio[BLUE]  = 1.0
	TimerMoneyGive_BlueTeam = GetConVarInt(cvar_TimerMoneyGive_BlueTeam)
	...
	RegConsoleCmd("sm_refund", Command_Refund, "Refunds all the money you spent and removes all the upgrades you bought")

	RegConsoleCmd("sm_uuhelp", Disp_Help, "Displayes help for the plugin")
	//RegAdminCmd("us_enable_buy_new_weapon", EnableBuyNewWeapon, ADMFLAG_GENERIC)
	RegAdminCmd("sm_uu_spentmoney", ShowSpentMoney, ADMFLAG_GENERIC, "Shows the amount of money each player spent")
	//RegAdminCmd("sm_reload_cfg", ReloadCfgFiles, ADMFLAG_GENERIC, "Reload the UU config files and restart the plugin")
	RegAdminCmd("sm_uu_reloadplugin", ReloadCfgFiles, ADMFLAG_CONFIG, "Reload the UU to apply all the changes to configs (or just because)")	
	//RegConsoleCmd("sm_uu", Disp_Help, "Displayes help for the plugin")
	//RegConsoleCmd("sm_nohelp", StopDisp_chatHelp)
	//RegConsoleCmd("sm_uudteamup", Toggl_DispTeamUpgrades)
	//RegConsoleCmd("sm_uu_no", Toggl_DispTeamUpgrades)
	//RegConsoleCmd("sm_uuaide", Disp_Help, "Displayes help for the plugin")
	//RegConsoleCmd("sm_aide", Disp_Help, "Displayes help for the plugin")
	RegConsoleCmd("sm_buy", Menu_BuyUpgrade, "Display the purchase menu")
	RegConsoleCmd("sm_qbuy", Menu_QuickBuyUpgrade, "Quickbuy, Command to buy upgrades in bulk without menu")
	RegConsoleCmd("qbuy", Menu_QuickBuyUpgrade, "Quickbuy, Command to buy upgrades in bulk without menu")
	RegConsoleCmd("sm_upgrade", Menu_BuyUpgrade, "Display the purchase menu")
	//RegConsoleCmd("sm_BUY", Menu_BuyUpgrade, "Display the purchase menu")
...
{
	//PrintToChatAll("Loading Uber Upgrades...");
	LogMessage("Loading Uber Upgrades...");
	...
	//PrintToChatAll("Done loading Uber Upgrades...!");
	PrintToChatAll("[SM] The Uber Upgrades has been loaded! Please change class for it to start working!");
	LogMessage("Done loading Uber Upgrades...!");
}

public OnPluginEnd()
{
	PrintToChatAll("Unloading Uber Upgrades...");
	LogMessage("Unloading Uber Upgrades...");
	for (new client = 1; client < MAXPLAYERS + 1; client++)
	{
		//SetClientInfo(client, "name", clientBaseName[client]);
		...
		SetClientName(client, clientBaseName[client]);
		//ServerCommand("sm_rename #%d %s", GetClientUserId(client), clientBaseName[client]);
		ResetPlayerStuff(client);
		ResetPlayer(client);
	}
	UberShopUnhooks();
	...
	//PrintToChatAll("Done unloading Uber Upgrades!");
	PrintToChatAll("[SM] The Uber Upgrades unloaded! Respawn to unload the ammo upgrades");
	LogMessage("Done unloading Uber Upgrades!");
}

public OnMapEnd()
{
	PrintToChatAll("Map change...cleaning Uber Upgrades...");
	LogMessage("Map change...cleaning Uber Upgrades...");
	for (new client = 1; client < MAXPLAYERS + 1; client++)
	{
		//SetClientInfo(client, "name", clientBaseName[client]);
	...
	PrintToChatAll("Map change...done cleaning Uber Upgrades...");
	LogMessage("Map change...done cleaning Uber Upgrades...");
}
...
public ResetClientUpgrade_slot(client, slot)
{
	...
	if (ctr_m != clientLevels[client])
	{
		...
		//SetClientName(client, clname);
		
		if((lastRenamedTime[client] + 15) <= GetGameTime()){
			SetClientName(client, clname);
			lastRenamedTime[client] = GetGameTime();
		}
	}
}
...
{
	//PrintToChatAll("Reset client %N upgrades", client);
	int slot

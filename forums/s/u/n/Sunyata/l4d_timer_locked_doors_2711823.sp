...
{
	if( g_bCvarAllow && buttons & IN_USE && ((g_iCvarKeys == 1 && buttons & IN_SPEED) || (g_iCvarKeys == 2 && buttons & IN_DUCK)) && GetGameTime() > g_fLastUse[client] )
	{
		...
		if( GetClientTeam(client) == 2 && IsPlayerAlive(client) )
		{
			...
			if( entity > MaxClients && g_iDoors[entity] == EntIndexToEntRef(entity) )
			{
				...
				if( GetEntProp(entity, Prop_Data, "m_bLocked") && GetEntProp(entity, Prop_Data, "m_eDoorState") == 0 )
				{
					...
					if( g_iCvarText & 2 )
					{
						if( g_iCvarText & 4 )			PrintToChatAll("\x04* DOOR UNLOCKED by %N.", client);
						else if( g_iCvarText & 8 )		PrintToChat(client, "\x04* DOOR UNLOCKED by %N.", client);
				...
				else
				{
					...
					if( GetEntProp(entity, Prop_Data, "m_eDoorState") == 0 )
					{
						...
						if( g_iCvarText & 1 )
						{
							if( g_iCvarText & 4 )			PrintToChatAll("\x04* DOOR LOCKED and can only be unlocked after %i seconds *", GetConVarInt(g_hCvarDoorTimer));
							else if( g_iCvarText & 8 )		PrintToChat(client, "\x04* DOOR LOCKED and can only be unlocked after %i seconds *", GetConVarInt(g_hCvarDoorTimer)); 											
		...
		if( GetClientTeam(client) == 2 && IsPlayerAlive(client) )
		{
			...
			if( entity > MaxClients && g_iDoors[entity] == EntIndexToEntRef(entity) )
			{
				...
				if( GetEntProp(entity, Prop_Data, "m_bLocked") && GetEntProp(entity, Prop_Data, "m_eDoorState") == 0 )
				{
					...
					if( g_iCvarText & 2 )
					{
						if( g_iCvarText & 4 )			PrintToChatAll("\x04* DOOR LOCK BROKEN by %N - Now open the door! *", client);
						else if( g_iCvarText & 8 )		PrintToChat(client, "\x04* DOOR LOCK BROKEN by %N - Now open the door! *", client);						
						ClientCommand(client, "play UI/BeepClear.wav"); //sunyata - notify sound for door unlocked
...
public Action UnlockDoorWithTimer(Handle timer,int client) //sunyata - new timer notifications and instructions code for members and public players
{
	...
	if( entity > MaxClients && g_iDoors[entity] == EntIndexToEntRef(entity) )
	{
		PrintToChatAll("\x04* To BREAK LOCK on a rescue closet door, press [USE] key then SHOOT door knob *");
		PrintToChatAll("\x04* A slain player in 'Spectate Mode' has 20-seconds to also unlock door. Just point your spectate-cursor at door and wait *");			
		//SetEntProp(entity, Prop_Data, "m_bLocked", 0); //sunyata - if player still has reticle aimed at door - door will unlock itself without the need to shoot at it
		ClientCommand(client, "play UI/BeepClear.wav"); //sunyata note - sound notify for player to now unlock door
		CreateTimer(20.0, UnlockDoorWithTimer2, client, TIMER_FLAG_NO_MAPCHANGE);
	}
	else 
	{
		PrintToChatAll("\x04* To BREAK LOCK on a rescue closet door, press [USE] key then SHOOT door knob. *");
...
public Action UnlockDoorWithTimer2(Handle timer,int client) //sunyata - new timer notifications and instructions for spectatir to open door
{
	int entity = GetClientAimTarget(client, false);
	if( entity > MaxClients && g_iDoors[entity] == EntIndexToEntRef(entity) )
	{
		PrintToChatAll("\x04* OPEN THE RESCUE CLOSET DOOR!- It's now been UNLOCKED by a spectator *"); 	
		SetEntProp(entity, Prop_Data, "m_bLocked", 0); //sunyata - if player still has reticle aimed at door - door will unlock itself without the need to shoot at it
		ClientCommand(client, "play UI/BeepClear.wav"); //sunyata note - sound notify for player to now unlock door
	}
}

...
public OnMapStart(){
	LogToFile("addons/sourcemod/logs/downloader.log","Start");
	if(GetConVarInt(g_enabled) == 1){
		if(GetConVarInt(g_normal) == 1) ReadDownloads();
		if(GetConVarInt(g_simple) == 1) ReadDownloadsSimple();
	}
	LogToFile("addons/sourcemod/logs/downloader.log","Stop");
}
...
public ReadItem(String:buffer[]){
	...
	TrimString(buffer);
	LogToFile("addons/sourcemod/logs/downloader.log"," ");
	LogToFile("addons/sourcemod/logs/downloader.log","ReadLine: %s",buffer);
	...
	else if (!StrEqual(buffer,"",false) && FileExists(buffer))
	{
		if(downloadfiles){
			if(StrContains(mediatype,"Decal",true) >= 0){
				LogToFile("addons/sourcemod/logs/downloader.log","PrecacheDecal: %s",buffer);
				PrecacheDecal(buffer,true);
			}
			else if(StrContains(mediatype,"Sound",true) >= 0){
				LogToFile("addons/sourcemod/logs/downloader.log","PrecacheSound: %s",buffer);
				PrecacheSound(buffer,true);
			}
			else if(StrContains(mediatype,"Model",true) >= 0){
				LogToFile("addons/sourcemod/logs/downloader.log","PrecacheModel: %s",buffer);
				PrecacheModel(buffer,true);
			}
			LogToFile("addons/sourcemod/logs/downloader.log","AddFileToDownloadTable: %s",buffer);

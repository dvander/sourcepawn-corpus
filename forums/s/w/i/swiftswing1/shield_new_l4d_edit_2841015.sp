...
public void OnPluginStart()
{
	...
	//g_iVelocity = FindSendPropOffs("CBasePlayer", "m_vecVelocity[0]");
  	...
  	l4d_shield_damage_from_tank = CreateConVar("l4d_shield_damage_from_tank", "50.0", "tank damage to survivor with shield[0.0, 100.0] %" );
...
//	PrintToChatAll("%N create a shield", client);
	PrintToChat(client, "\x03[\x04RIOT\x03SHIELD\x04] \x01Press \x05[WALK] \x01to block some damage from infected!");
}
...
void AttachShieldModel(int client)
{
	...
	SetEntityRenderColor(ent, 255, 255, 255, 100);		//last digit: how transparent										 
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	...
	if((buttons & IN_SPEED)) 
	{ 
		ShowShieldTime[client] = engine_time;
...
public Action PlayerOnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	if(IsPlayerDown(victim) || L4D2_GetInfectedAttacker(victim) > 0)  { return Plugin_Continue; }																						  
	...
	else
	{
		...
		if((mode & ShieldMode_Front) && a > 90.0)
		{
			...
			PrintHintText(victim, "[RIOTSHIELD] Damage prevented: %d", d);
...
stock bool:IsPlayerDown(client)
{
 ...
 */
int L4D2_GetInfectedAttacker(int client) // Marttt version Health glows
{
    int attacker;

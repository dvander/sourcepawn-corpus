...
bool HasRBQ[MAXPLAYERS+1]; //Check if has machine
int MachineOwner[MAXPLAYERS+1]; //Point machine's owner
...
public void OnPluginStart()
{
	...
	l4d_machine_damage_to_infected = CreateConVar("l4d_machine_damage_to_infected", "177", "bullet damage", FCVAR_NOTIFY);
	...
	l4d_machine_adminonly = CreateConVar("l4d_machine_adminonly", "0", "1:admin use only", FCVAR_NOTIFY);
	...
	l4d_machine_sleep_time = CreateConVar("l4d_machine_sleep_time", "77.0", "how many seconds does a gun goto sleep while no enemy, must >0", FCVAR_NOTIFY);
	l4d_machine_fire_rate = CreateConVar("l4d_machine_fire_rate", "5", "rate of fire, how many shot per soncods [5, 30]", FCVAR_NOTIFY);
 	l4d_machine_health = CreateConVar("l4d_machine_health", "777", "gun's health", FCVAR_NOTIFY);

 	l4d_machine_betray_chance = CreateConVar("l4d_machine_betray_chance", "17.0", "betray chance", FCVAR_NOTIFY);

	l4d_machine_limit = CreateConVar("l4d_machine_limit", "7.0", " ", FCVAR_NOTIFY);
	...
	RegConsoleCmd("sm_rbq", sm_machine);
	//RegConsoleCmd("sm_rmrbq", sm_removemachine);
	ResetAllState();
	GetConVar();
}
...
public Action entity_shoved(Event event, char[] event_name, bool dontBroadcast)
{
	...
	if(attacker > 0 && IsClientInGame(attacker) && GetClientTeam(attacker) == 2)
	{
		...
		if(b & IN_DUCK)
		{
			int gun = GetMinigunAim(attacker);
			if(gun > 0)
			{
...
public Action player_use(Event hEvent, const char[] strName, bool DontBroadcast)
{
	...
	if(StrContains(classname, "ammo")>=0)
	{
		...
		if(index >= 0)
		{
			PrintHintText(client, "你的RBQ精力已恢复！");
			C_PrintToChat(client, "你的RBQ精力已恢复！");
...
{	
	if(HasRBQ[client])
	{	
		sm_removemachine(client);
		return;
	}
	else if(GetConVarInt(l4d_machine_limit)-UseCount[client]==0)
	{
		C_PrintToChat(client, "你的RBQ已经被你用坏了！让你走之前不扛走这下玩坏了吧，等下局领个新的吧");
		return;
	}
	else	if(client > 0 && IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		{
			C_PrintToChatAll("Only admin can spawn machine gun");
			return;
		}
		char str[32];
...
public Action sm_removemachine(int client)
{
	if(client > 0 && IsClientInGame(client))
	{
		int index = MachineOwner[client];
		RemoveMachine(index);
		C_PrintToChatAll("%N 的RBQ消失了", client);
...
{
	C_PrintToChatAll("%s - %f %f %f", s, target[0], target[1], target[2]);
}
...
public Action ShowInfo(Handle timer, any client)
{
	...
	else C_PrintToChat(client, "\x03Press \x04DUCK+SHOVE to \x03carry \x04machine gun\x03");
}
//code from "DJ_WEST"
...
{
	DisplayInstructorHint(i_Client, "按住蹲(CTRL) + 右键推(SHOVE)扛起RBQ\n长按E放下", "+attack2");
}
...
public void DisplayInstructorHint(int i_Client, char s_Message[256], char[] s_Bind)
{
	...
	CreateTimer(7.0, RemoveInstructorHint, h_RemovePack);
}
...
void CreateMachine(int client, int type = 0)
{
	...
	{
		C_PrintToChat(client, "在场的RBQ太多了，等某人的坏了再试试！");
		PrintHintText(client, "在场的RBQ太多了，等某人的坏了再试试！");
		return;
	}
	...
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		MachineCount++;
		MachineOwner[client] = MachineCount;
		...
		GunCarrier[MachineCount] = client;
		...
		HasRBQ[client] = true;
		C_PrintToChatAll("%N 召唤了个RBQ", client);
		C_PrintToChat(client, "你的RBQ剩余使用次数： \x05%d \x01次，走之前记得扛走RBQ免得丢失！",GetConVarInt(l4d_machine_limit)-UseCount[client]);		
...
{
	if(FindCarryIndex(client) >= 1) return;
	...
	if(index >= 1)
	{
		...
		if(GetConVarInt(l4d_machine_allow_carry) == 2)
		{
			...
			if(owner > 0 && IsClientInGame(owner) && IsPlayerAlive(owner))
			{
				if(owner != client)
				{
					PrintHintText(client, "你不能拿走 %N 的RBQ！", owner);
					C_PrintToChat(client, "你不能拿走 %N 的RBQ！", owner);
					return;
				}
			}
			else
		...
		if(GunAmmo[index] > 0)
		{
			PrintHintText(client, "RBQ精力剩余： %d ", GunAmmo[index]);
		}
		else
		{
			PrintHintText(client, "你的RBQ精力耗尽，赶紧给补魔！");
		}
	}
}
...
{
	//C_PrintToChatAll("%d inflictor, %d type", inflictor, damagetype);
	...
	if(index >= 0)
	{
		...
		{
			PrintHintTextToAll("某个RBQ叛变到感染者阵营去了！右键拉他回来！");
			C_PrintToChatAll("某个RBQ叛变到感染者阵营去了！右键拉他回来！");
			GunLastCarryTime[index] = GetEngineTime();
			//GunState[index]=State_Scan;
			Broken[index] = false;
			GunEnemy[index] = 0;
		...
		if(betrayToSurvivor)
		{
			PrintHintTextToAll("某个RBQ已经回到幸存者阵营！");
			C_PrintToChatAll("某个RBQ已经回到幸存者阵营！");
			GunLastCarryTime[index]=GetEngineTime();
			//GunState[index]=State_Scan;
			Broken[index] = false;
			GunEnemy[index] = 0;
		...
		{
			PrintHintTextToAll("一个RBQ醒来了");
			C_PrintToChatAll("一个RBQ醒来了");
			GunLastCarryTime[index] = GetEngineTime();
			GunState[index] = State_Scan;
			Broken[index] = false;
			GunEnemy[index] = 0;
		...
		if(GunHealth[index] <= 0.0)
		{
			...
			{
				PrintHintText(GunUser[index], "你的RBQ已经坏了");
			}
			SetColor(Gun[index], GunTeam[index]);
		}
		if(print)PrintHintText(attacker, "RBQ剩余精力 %d", RoundFloat(GunHealth[index]));
	}
}
...
int ScanAndShotEnmey(int index, float time, float intervual)
{
	...
	{
		if(user > 0) PrintHintText(user, "你RBQ坏了");
		RemoveMachine(index);
	}
	...
	if(GunLastCarryTime[index] + SleepTime < time)
	{
		...
		SetEntProp(gun1, Prop_Send, "m_firing", 0);
		C_PrintToChatAll("一个RBQ昏迷了");
		PrintHintTextToAll("一个RBQ昏迷了");
		return;
	}
	...
	//C_PrintToChatAll("team %d enemy %d",GunTeam[index], newenemy);
	if(newenemy == 0)
	{
		SetEntProp(gun1, Prop_Send, "m_firing", 0);
		Broken[index] = false;
	...
	//C_PrintToChatAll("find %d  %f %f", newenemy  );
	if(overheated == 0)
	{
		if(newenemy > 0 && FloatAbs(diff1) < 40.0)
		{
			if(time >= GunFireTime[index] && GunAmmo[index] > 0)
			{
				...
				if(AmmoIndicator[index] >= GunAmmoCount / 20.0)
				{
					...
					if(user > 0) PrintCenterText(user, "RBQ剩余精力: %d ( %d %%)", GunAmmo[index], RoundFloat(GunAmmo[index] * 100.0 / GunAmmoCount ));
				}
				if(GunAmmo[index] == 0)
				{
					if(user>0) PrintHintText(user, "你的RBQ精力耗尽，赶紧给补魔！");
				}

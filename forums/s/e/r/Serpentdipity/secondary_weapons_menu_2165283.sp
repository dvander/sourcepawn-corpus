...
public OnPluginStart()
{
	...
	g_max_give[0] = CreateConVar("sm_quota_baseball_bat", "-1", " Quota Given to each player for obtaining baseball_bat in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[1] = CreateConVar("sm_quota_cricket_bat", "-1", " Quota Given to each player for obtaining cricket_bat in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[2] = CreateConVar("sm_quota_crowbar", "-1" ," Quota Given to each player for obtaining crowbar in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[3] = CreateConVar("sm_quota_electric_guitar", "-1", " Quota Given to each player for obtaining electric_guitar in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[4] = CreateConVar("sm_quota_fireaxe", "-1", " Quota Given to each player for obtaining fireaxe in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[5] = CreateConVar("sm_quota_frying_pan", "-1", " Quota Given to each player for obtaining frying_pan in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[6] = CreateConVar("sm_quota_katana", "-1", " Quota Given to each player for obtaining katana in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[7] = CreateConVar("sm_quota_machete", "-1", " Quota Given to each player for obtaining machete in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[8] = CreateConVar("sm_quota_tonfa", "-1", " Quota Given to each player for obtaining tonfa in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[9] = CreateConVar("sm_quota_knifeknife", "-1", " Quota Given to each player for obtaining knife in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[10] = CreateConVar("sm_quota_golfclub", "-1", " Quota Given to each player for obtaining golfclub in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[11] = CreateConVar("sm_quota_pistol", "-1", " Quota Given to each player for obtaining pistol in each round ( -1 = unlimited 0 = disabled )");
	g_max_give[12] = CreateConVar("sm_quota_pistol_magnum", "-1", " Quota Given to each player for obtaining pistol_magnum in each round ( -1 = unlimited 0 = disabled )");
		    ...
		    case 0: // Baseball Bat
			{
				if ( give_quota0[client] > 0 || give_quota0[client] < 0) {
					...
					if (give_quota0[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Baseball Bat until next round",give_quota0[client]);
					}
			...
			case 1: // Cricket Bat
			{
				if ( give_quota1[client] > 0 || give_quota1[client] < 0) {
					...
					if (give_quota1[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Cricket Bat until next round",give_quota1[client]);
					}
			...
			case 2: // Crowbar
			{
				if ( give_quota2[client] > 0 || give_quota2[client] < 0) {
					...
					if (give_quota2[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Crowbar until next round",give_quota2[client]);
					}
			...
			case 3: // Electric Guitar
			{
				if ( give_quota3[client] > 0 || give_quota3[client] < 0) {
					...
					if (give_quota3[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain an Electric Guitar until next round",give_quota3[client]);
					}
			...
			case 4: // Fireaxe
			{
				if ( give_quota4[client] > 0 || give_quota4[client] < 0) {
					...
					if (give_quota4[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Fireaxe until next round",give_quota4[client]);
					}
			...
			case 5: // Frying Pan
			{
				if ( give_quota5[client] > 0 || give_quota5[client] < 0) {
					...
					if (give_quota5[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Frying Pan until next round",give_quota5[client]);
					}
			...
			case 6: // Katana
			{
				if ( give_quota6[client] > 0 || give_quota6[client] < 0) {
					...
					if (give_quota6[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Katana until next round",give_quota6[client]);
					}
			...
			case 7: // Machete
			{
				if ( give_quota7[client] > 0 || give_quota7[client] < 0) {
					...
					if (give_quota7[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Machete until next round",give_quota7[client]);
					}
			...
			case 8: // Tonfa
			{
				if ( give_quota8[client] > 0 || give_quota8[client] < 0) {
					...
					if (give_quota8[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Tonfa until next round",give_quota8[client]);
					}
			...
			case 9: // Knife
			{
				if ( give_quota9[client] > 0 || give_quota9[client] < 0) {
					...
					if (give_quota9[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Knife until next round",give_quota9[client]);
					}
			...
			case 10: // Golf Club
			{
				if ( give_quota10[client] > 0 || give_quota10[client] < 0) {
					...
					if (give_quota10[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Golf Club until next round",give_quota10[client]);
					}
			...
			case 11: // pistol
			{
				if ( give_quota11[client] > 0 || give_quota11[client] < 0) {
					...
					if (give_quota11[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a pistol until next round",give_quota11[client]);
					}
			...
			case 12: // pistol_magnum
			{
				if ( give_quota12[client] > 0 || give_quota12[client] < 0) {
					...
					if (give_quota12[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a pistol_magnum until next round",give_quota12[client]);
					}

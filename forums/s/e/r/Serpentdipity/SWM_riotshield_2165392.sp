...
new Handle:g_max_give[14];
new max_give[14]; //array for storing initial quota of each item
...
public OnPluginStart()
{
	...
	RegConsoleCmd("riot", TankBusterMenu);
	RegConsoleCmd("riotshield", TankBusterMenu);
	RegConsoleCmd("shield", TankBusterMenu);
	//plugin version
	...
	g_max_give[13] = CreateConVar("sm_quota_riotshield", "-1", " Quota Given to each player for obtaining riotshield in each round ( -1 = unlimited 0 = disabled )");
...
public Action:TankBuster(clientId)
{
	...
	AddMenuItem(menu, "option13", "Riot Shield");
			...
			case 13: // Riot Shield * CSS
			{
				if ( give_quota13[client] > 0 || give_quota13[client] < 0) {
					...
					if (give_quota13[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a Riot Shield until next round",give_quota13[client]);
					}

...
{
	name = "Zeus Round|-> remake awp_deagle_rounds <-- https://forums.alliedmods.net/member.php?u=273598",
	author = "TnTSCS aka ClarkKent <--org | remake--> servers.csgo.cfg",
	description = "CS:GO remake awp_deagle_rounds servers.csgo.cfg",
...
public OnPluginStart()
{
	...
	HookConVarChange((hRandom = CreateConVar("sm_zeusround_mode", "2", 
...
public OnClientDisconnect(client)
{
	if(IsClientInGame(client))
	{
		...
		HEGrenades[client] = 1;
		FlashBangs[client] = 1;
		SmokeGrenades[client] = 1;
		INCGrenades[client] = 1;
		DecoyGrenades[client] = 1;
		
		//IsControllingBot[client] = false;
		
		PlayersCash[client] = 15950;
...
GiveClientWeapons(client)
{
	...
	if (HEGrenades[client] > 1)
	{
		for (new g = 1; g < HEGrenades[client]; g++)
		{
			GivePlayerItem(client, "weapon_hegrenade");
		}
	...
	if (FlashBangs[client] > 1)
	{
		for (new fb = 1; fb < FlashBangs[client]; fb++)
		{
			GivePlayerItem(client, "weapon_flashbang");
		}
	...
	if (SmokeGrenades[client] > 1)
	{
		for (new sg = 1; sg < SmokeGrenades[client]; sg++)
		{
	...
	if (DecoyGrenades[client] > 1)
	{
		for (new dg = 1; dg < DecoyGrenades[client]; dg++)
		{
	...
	if (INCGrenades[client] > 1)
	{
		for (new ig = 1; ig < INCGrenades[client]; ig++)
		{
...
public Action:OnWeaponCanUse(client, weapon)
{
	...
	if(StrEqual(sWeapon, "weapon_awp", false) || StrEqual(sWeapon, "weapon_deagle", false) || StrEqual(sWeapon, "weapon_knife", false) || StrEqual(sWeapon, "weapon_flashbang", false) || StrEqual(sWeapon, "weapon_hegrenade", false) || StrEqual(sWeapon, "weapon_molotov", false))
	{
		return Plugin_Continue;
	}

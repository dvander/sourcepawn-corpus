...
*************************************************************************
File Information
$Id: tf2_showammo.sp 131 2011-02-12 20:30:01Z brutalgoergectf $
$Author: brutalgoergectf $
$Revision: 131 $
$Date: 2011-02-12 13:30:01 -0700 (Sat, 12 Feb 2011) $
$LastChangedBy: brutalgoergectf $
$LastChangedDate: 2011-02-12 13:30:01 -0700 (Sat, 12 Feb 2011) $
...
stock ShowInfo(medic, target)
{
	...
	new TFClassType:class, iAmmo1, iClip1, iClip2, iHP, iMaxHP, 
	...
	iHP = TF2_Health(target);
	iMaxHP = TF2_Health(target, 1);
	...
	if (class == TFClass_Pyro || class == TFClass_Heavy)
	{
		...
		Format(sMessage, sizeof(sMessage), "P. Ammo: %i ", iAmmo1);
	}
	else if (iClip1 == -1)
	{
	...
	if (iClip1 != -1)
	{
		Format(sMessage, sizeof(sMessage), "P. Ammo: %i / %i ", iClip1, g_aClientSettings[target][iMaxClip1]);
	}
	...
	if (class == TFClass_DemoMan)
	{
		...
		{
			Format(sMessage, sizeof(sMessage), "%sS. Ammo: %i / %i ", sMessage, iClip2, g_aClientSettings[target][iMaxClip2]);
		}
	}
	Format(sMessage, sizeof(sMessage), "%sHP: %i/%i ", sMessage, iMaxHP, iHP);
...
stock TF2_Health(client, maxhp = 0)
{
	if(maxhp == 0)

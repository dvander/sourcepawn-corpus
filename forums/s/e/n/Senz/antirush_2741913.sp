...
public EntOut_OnStartTouch(const String:output[], caller, activator, Float:delay)
{
	...
	switch(iPunishment)
	{
		...
		case 2:
		{
			...
			// Following line has been commented out since it makes the bounce back kinda weird when you hit the barrier at a diagonal angle.
			// It essentially bounces you back to the same direction you're coming from, which is kinda weird at a diagonal angle.
			// Instead of ricocheting off the barrier like its just a real wall.
			//fVelocity[0] *= -1.0;
			
			// This is the important one to bounce back in the correct direction.
			fVelocity[1] *= -1.0;
			
			// Don't let player fly up too much. But also don't make negative cause then player can squeeze through barrier by spamming spacebar.
			if (fVelocity[2] > 80.0)
				fVelocity[2] = 80.0;
			
			// Always bounce back with exactly 100 velocity
			if(fVelocity[1] > 0.0)
				fVelocity[1] = 100.0;
			else if(fVelocity[1] < 0.0)
				fVelocity[1] = -100.0;

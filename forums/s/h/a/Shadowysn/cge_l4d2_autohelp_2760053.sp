// Thanks to AK978 for fixing the timers to use GetClientOfUserId and explaining them to my stubborn ears.
...
public void OnPluginStart()
{
	...
	AutoHelp_Incap = CreateConVar(cmd_str, "1", desc_str, FCVAR_NONE, true, 0.0, true, 1.0);
	...
	AutoHelp_Ledge_Delay = CreateConVar(cmd_str, "3", desc_str, FCVAR_NONE);
	...
	AutoHelp_Specials = CreateConVar(cmd_str, "1", desc_str, FCVAR_NONE, true, 0.0, true, 1.0);
...
{
	int client = GetClientOfUserId(event.GetInt("userid", 0));
	int team = GetClientOfUserId(event.GetInt("team", 0));
...
{
	int victimID = event.GetInt("victim", 0);
	...
	DelayTimer[victim] = CreateTimer(g_fAutoHelp_Specials_Smoker_Drag, Timer_Delay, victimID);
}
...
void Event_ChokeStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (ReviveTimer[victim])
	{
		...
		DelayTimer[victim] = CreateTimer(0.1, Timer_Delay, victimID);
	}
	else
	{
		DelayTimer[victim] = CreateTimer(g_fAutoHelp_Specials_Delay, Timer_Delay, victimID);
	}
}
...
void Event_PlayerIncapacitated(Event event, const char[] name, bool dontBroadcast)
{
	int victimID = event.GetInt("victim", 0);
	int victim = GetClientOfUserId(victimID);
	if (!IsValidClient(victim) || !IsGameSurvivor(victim)) return; // tank check is redundant Marttt, all infected can't get incapped anyway
	
	IncapType[victim] = INCAP;
	HelpState[victim] = STATE_NONE;
	SelfReviveDuration[victim] = g_fAutoHelp_Incap_Duration;
	ReviveTimer[victim] = null;
	DelayTimer[victim] = CreateTimer(g_fAutoHelp_Incap_Delay, Timer_Delay, victimID);
}
...
void Event_PlayerLedgeGrab(Event event, const char[] name, bool dontBroadcast)
{
	...
	DelayTimer[victim] = CreateTimer(g_fAutoHelp_Ledge_Delay, Timer_Delay, victimID);
}
...
Action Timer_Delay(Handle timer, int userid)
{
	...
	{
		ReviveTimer[client] = CreateTimer(0.1, Timer_SelfRevive, userid, TIMER_REPEAT);
	}
	DelayTimer[client] = null;
	return Plugin_Continue;
}
...
void KnockAttacker(int client, int attacker)
{
	...
	switch (class)
	{
		case (HUNTER):
		{
			TimerSlapPlayer[attacker] = CreateTimer(0.1, Timer_SlapPlayer, attackerID, TIMER_REPEAT);
			CreateTimer(0.6, Timer_StopSlap, attackerID);
			return;
		}
		case (CHARGER):
		{
			TimerSlapPlayer[attacker] = CreateTimer(0.5, Timer_SlapPlayer, attackerID, TIMER_REPEAT);
			CreateTimer(2.0, Timer_StopSlap, attackerID);
			TimerSlapPlayer[client] = CreateTimer(0.5, Timer_SlapPlayer, attackerID, TIMER_REPEAT);
			CreateTimer(2.0, Timer_StopSlap, attackerID);
			return;
		}
	}

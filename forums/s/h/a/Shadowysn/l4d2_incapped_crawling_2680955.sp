...
Handle g_hTmrHurt;
ConVar g_hCvarClones, g_hCvarCoach, g_hCvarAllow, g_hCvarCrawl, g_hCvarCrazy, g_hCvarGlow, g_hCvarHint, g_hCvarHintS, g_hCvarHurt, g_hCvarMPGameMode, g_hCvarModes, g_hCvarModesOff, g_hCvarModesTog, g_hCvarRate, g_hCvarSpeed, g_hCvarSpeeds, g_hCvarSpit, g_hCvarView;
int g_iClone[MAXPLAYERS+1], g_iDisplayed[MAXPLAYERS+1], g_iHint, g_iHints, g_iHurt, g_iRate, g_iSpeed;
bool g_bCvarAllow, g_bCrazy, g_bGlow, g_bRoundOver, g_bSpit, g_bTranslation, g_bView;
...
public void OnPluginStart()
{
	...
	g_hCvarClones =		CreateConVar(	"l4d2_crawling_clones",		"1",			"0=No crawling anim clones, 1=Crawling anim clones. (Coach is managed by seperate cvar)", CVAR_FLAGS);
	g_hCvarCoach =		CreateConVar(	"l4d2_crawling_coach",		"1",			"0=No crawling anim for coach, 1=Crawling anim for coach.", CVAR_FLAGS);
...
public Action PlayAnim(int client)
{
	...
	static char sModel[42];
	GetEntPropString(client, Prop_Data, "m_ModelName", sModel, sizeof(sModel));

	bool coach = sModel[29] == 'c'; // Coach
	
	if (coach && GetConVarBool(g_hCvarCoach) || !coach && GetConVarBool(g_hCvarClones))
	{
		...
		if( coach && iEntity < 1 )
		{
			...
			SDKHook(cloneCoach, SDKHook_SetTransmit, Hook_SetTransmit);
...
{
	if (!IsValidClient(client) || !IsValidEntity(entity)) return Plugin_Continue;
	if (IsFakeClient(client)) return Plugin_Continue;
	
	int moveparent = GetEntPropEnt(entity, Prop_Send, "moveparent");
	int other_moveparent = -1;
	if (IsValidEntity(moveparent))
	{ other_moveparent = GetEntPropEnt(moveparent, Prop_Send, "moveparent"); }
	
	if ((!IsValidEntity(moveparent) || moveparent != client) && (!IsValidEntity(other_moveparent) || other_moveparent != client))
	{ return Plugin_Continue; }
	
	if (GetEntProp(client, Prop_Send, "m_iObserverMode") > 0) return Plugin_Continue;

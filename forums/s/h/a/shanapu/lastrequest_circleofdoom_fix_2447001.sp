...
public Plugin:myinfo =
{
	...
	author = "Jason Bourne & Kolapsicle & Bara & shanapu",
...
public OnPluginStart()
{
	...
	g_hVersion = CreateConVar("sm_circleofdoom_version", COD_VERSION, "Current Circle of Doom version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
public CircleOfDoom_Start(Handle:LR_Array, iIndexInArray)
{
	...
	if (This_LR_Type == g_LREntryNum)
	{
		...
		FreezeTimer = CreateTimer(PREPARE_TIME, Timer_Unfreeze);
...
{
	if (IsValidClient(LR_Player_Prisoner) && IsValidClient(LR_Player_Guard))
	{
		new Float:distance;

...
public EventPlayerSpawn(Handle:event,const String:name[],bool:dontBroadcast) {
	...
	if (GetConVarBool(g_CVarEnable)) {
		...
		if(CheckCommandAccess(client, "", ADMFLAG_CUSTOM1)) g_iKnives[client] = 6;
		else if(CheckCommandAccess(client, "", ADMFLAG_CUSTOM2)) g_iKnives[client] = 8;
		else g_iKnives[client] = 5;
	}
}

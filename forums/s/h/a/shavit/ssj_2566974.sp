...
void SSJ_PrintStats(int client, int target)
{
	...
	{
		Format(sMessage, 192, "%s %s| Speed: %s%i", sMessage, gS_ChatStrings[sMessageText], gS_ChatStrings[sMessageVariable], RoundToFloor(GetVectorLength(velocity)));
	}
	...
	if((gI_UsageMode[client] == USAGE_SIXTH && gI_Jump[target] == 6) || (gI_UsageMode[client] == USAGE_EVERY_SIXTH && (gI_Jump[client] % 6) == 0))
	{
		...
		{
			Format(sMessage, 192, "%s %s| Speed Δ: %s%i", sMessage, gS_ChatStrings[sMessageText], gS_ChatStrings[sMessageVariable], RoundToFloor(GetVectorLength(velocity)) - RoundToFloor(gF_InitialSpeed[target]));
		}

		if(gB_HeightDiff[client])
		{
			Format(sMessage, 192, "%s %s| Height Δ: %s%i", sMessage, gS_ChatStrings[sMessageText], gS_ChatStrings[sMessageVariable], RoundToFloor(origin[2]) - RoundToFloor(gF_InitialHeight[target]));
		}

		if(gB_GainStats[client])
		{
			Format(sMessage, 192, "%s %s| Gain: %s%.2f%%", sMessage, gS_ChatStrings[sMessageText], gS_ChatStrings[sMessageVariable], coeffsum);
		}

		if(gB_StrafeSync[client])
		{
			Format(sMessage, 192, "%s %s| Sync: %s%.2f%%", sMessage, gS_ChatStrings[sMessageText], gS_ChatStrings[sMessageVariable], 100.0 * gI_SyncedTick[target] / gI_StrafeTick[target]);
		}

		if(gB_Efficiency[client])
		{
			Format(sMessage, 192, "%s %s| Efficiency: %s%.2f%%", sMessage, gS_ChatStrings[sMessageText], gS_ChatStrings[sMessageVariable], efficiency);
		}
	...
	else if(gI_UsageMode[client] == USAGE_EVERY)
	{
		if(gI_Jump[target] > 1)
		{
			if(gB_SpeedDiff[client])
			{
				Format(sMessage, 192, "%s %s| Speed Δ: %s%i", sMessage, gS_ChatStrings[sMessageText], gS_ChatStrings[sMessageVariable], RoundToFloor(GetVectorLength(velocity)) - RoundToFloor(gF_OldSpeed[target]));
			}

			if(gB_HeightDiff[client])
			{
				Format(sMessage, 192, "%s %s| Height Δ: %s%i", sMessage, gS_ChatStrings[sMessageText], gS_ChatStrings[sMessageVariable], RoundToFloor(origin[2]) - RoundToFloor(gF_OldHeight[target]));
			}
...
void PrintToClient(int client, const char[] message, any ...)
{
	...
	{
		PrintToChat(client, "%s%s%s%s", (gEV_Type == Engine_CSGO)?" ":"", gS_ChatStrings[sMessagePrefix], gS_ChatStrings[sMessageText], message);
	}
}

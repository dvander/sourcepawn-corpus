...
public OnPluginStart(){
	...
	bnb_bantime = CreateConVar("sm_bnb_bantime", "0", "How long to ban someone with a bad phrase in their name (0 = perm, -1 = kick)");
...
nameCheck(String:clientName[64], player){
	...
	if(GetAdminFlag(playerAdmin, Admin_Reservation, Access_Effective)){
		return;
	}
	...
	for(new i = 0; i < lines; i++){
		if(StrContains(clientName, badNames[i], false) != -1){
			...
			if(GetConVarInt(bnb_log) == 1){
				...
				GetClientAuthString(player,clientAuth,64);
				LogToFile(logFile, "%s (%s) was kicked/banned for having %s in his name", clientName, clientAuth, badNames[i]);

...
public Action:Command_Say(client, args)
{
	...
	if (!IsPlayerAlive(client))
	{
		...
		for (new i=1; i<=maxplayers; i++)
		{
			if (IsClientInGame(i))
			{
				if ((GetUserAdmin(i) != INVALID_ADMIN_ID) && IsPlayerAlive(i))
...
public Action:Command_SayTeam(client, args)
{
	...
	if (IsPlayerAlive(client))
	{
		...
		for (new i=1; i<=maxplayers; i++)
		{
			...
			{
				team = GetClientTeam(i)
				if ((GetUserAdmin(i) != INVALID_ADMIN_ID) && (senderteam != team))
					PrintToChat(i,"[ADMINSEEALL]%s: %s",name,text[startidx])
			}
		}	
	}
	else
	{
		...
		for (new i=1; i<=maxplayers; i++)
		{
			...
			{
				team = GetClientTeam(i)
				if ((GetUserAdmin(i) != INVALID_ADMIN_ID) && (IsPlayerAlive(i) || (senderteam != team)))
					PrintToChat(i,"[ADMINSEEALL]%s: %s",name,text[startidx])
			}
		}	

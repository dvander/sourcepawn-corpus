...
#define SND_LAUGH		"stickynades/muhahaha.mp3"
#define SND_SCREAM		"stickynades/scream.mp3"
...
public GrenadeTouch(iGrenade, iEntity) 
{
	...
	SDKUnhook(iGrenade, SDKHook_Touch, GrenadeTouch);
...
public Action:InitGrenade(Handle:timer, any:iGrenade)
{
	...
	{	
		...
		SDKHook(iGrenade, SDKHook_Touch, GrenadeTouch);
	}
	return Plugin_Handled;
}
...
{	
	...
	{	
		if(v_EmitSounds)
		{
			ClientCommand(iClient, "play *%s", SND_SCREAM);
			ClientCommand(iThrower, "play *%s", SND_LAUGH);
		}

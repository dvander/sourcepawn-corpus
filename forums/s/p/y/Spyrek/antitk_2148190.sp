...
#define VERSION							"1.1.4_spyrek_fix"
	...
	if (client > 0)
	{
		if (GetConVarBool(g_Cvar_Enabled))
		{
			...
			if (!IsFakeClient(client))
			{
				...
				if (arrayPlayerPunishments[client] == INVALID_HANDLE)
					arrayPlayerPunishments[client] = CreateArray(1);
...
KillPlayer(index)
{
	if (IsClientInGame(index) && IsPlayerAlive(index))
	{
		...
		else
		{
			...
			CreateTimer(0.0, SuicideTimer, index, TIMER_FLAG_NO_MAPCHANGE);
		}
	}
}
	...
	switch (method)
	{
		...
		case 2:
		{
			...
			if (g_bSBAvailable)
			{
				...
				SB_SubmitBan(0, tker, ban_message);

...
public Plugin:myinfo = {
	...
	author = "-=BwA=- Jester, $atanic $pirit",
...
public OnPluginStart() {
	...
	selectTeamFromPanel = CreateConVar("l4d2_BwA_SelectTeam_From_Panel", "1", "Allow players to press 1,2 or 3 to select Spectator, Survivor or Infected from the Team Panel [0 = No|1 = Yes]", FCVAR_SPONLY|FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
PerformSwitch(client, target, team, bool:silent) {
	...
	if (team == TEAM_SURVIVOR)
	{
		...
		while !(IsClientConnected(bot) && IsClientInGame(bot) && IsFakeClient(bot) && (GetClientTeam(bot) == TEAM_SURVIVOR)) do bot++;

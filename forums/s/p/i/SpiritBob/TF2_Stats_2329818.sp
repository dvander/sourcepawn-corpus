...
public Connected(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	else
	{
		...
		Format(query, sizeof(query), "SET NAMES \"UTF8MB4\"");	/* Set codepage to utf8mb4 */
...
createdbplayer()
{
	...
	len += Format(query[len], sizeof(query)-len, ") ENGINE=MyISAM DEFAULT CHARSET=utf8mb4;");
		...
		else
		{
			...
			if (tmpdbversion <= 24)
			{
				if (!sqllite)
				{
					...
					len += Format(query[len], sizeof(query)-len, "CHANGE `IPAddress` `IPAddress` VARCHAR(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '0';");
					SQL_TQuery(db, SQLErrorCheckCallback, query);
...
CreateItemsTable()
{
	...
	len += Format(query[len], sizeof(query)-len, "`STEAMID` varchar(25) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '0',");
	len += Format(query[len], sizeof(query)-len, "`ACTUALTIME` int(11) NOT NULL DEFAULT '0',");
	len += Format(query[len], sizeof(query)-len, "`ITEM` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '0',"); //no longer used
	...
	len += Format(query[len], sizeof(query)-len, ") ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci AUTO_INCREMENT=1 ;");
	SQL_FastQuery(db, query);
}

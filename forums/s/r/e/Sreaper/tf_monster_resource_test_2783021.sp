...
public void OnPluginStart() {
	...
	AddCommandListener(SetClassHook, "sm_setclass");
...
Action SetClassHook(int client, const char[] command, int argc)
{
	RequestFrame(delayClassIcon, client);
	return Plugin_Handled;
}

void delayClassIcon(int client)
{
	char line[100];
	
	switch (TF2_GetPlayerClass(client))
	{
		case TFClass_Scout:Format(line, sizeof(line), "%sScout", line);
		case TFClass_Soldier:Format(line, sizeof(line), "%sSoldier", line);
		case TFClass_Pyro:Format(line, sizeof(line), "%sPyro", line);
		case TFClass_DemoMan:Format(line, sizeof(line), "%sDemoman", line);
		case TFClass_Heavy:Format(line, sizeof(line), "%sHeavy", line);
		case TFClass_Engineer:Format(line, sizeof(line), "%sEngineer", line);
		case TFClass_Medic:Format(line, sizeof(line), "%sMedic", line);
		case TFClass_Sniper:Format(line, sizeof(line), "%sSniper", line);
		case TFClass_Spy:Format(line, sizeof(line), "%sSpy", line);
		default:Format(line, sizeof(line), "%sUnknown", line);
	}
	SetEntPropString(client, Prop_Send, "m_iszClassIcon", line);
}

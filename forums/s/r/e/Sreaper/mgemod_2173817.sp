...
public OnPluginStart()
{
	...
	gcvar_allowedClasses = CreateConVar("mgemod_allowed_classes", "scout soldier pyro demoman heavy engineer medic sniper spy", "Classes that players allowed to choose by default", FCVAR_PLUGIN);
	...
	RegConsoleCmd("join", Command_Menu, "Usage: join <arena number/arena name>. Join an arena.");
	RegConsoleCmd("add", Command_Menu, "Usage: join <arena number/arena name>. Join an arena.");
	RegConsoleCmd("swap", Command_Swap, "Ask your teammate to swap classes with you in ultiduo");
	RegConsoleCmd("leave", Command_Remove, "Leave the current arena.");
	RegConsoleCmd("remove", Command_Remove, "Leave the current arena.");
...
public OnClientPostAdminCheck(client)
{
	if (client)
	{
		if (IsFakeClient(client))
		{
			...
			g_hWelcomeTimer[client] = CreateTimer(5.0, Timer_WelcomePlayer, GetClientUserId(client));
...
ShowPlayerHud(client)
{
	...
	if(g_bArenaShowHPToPlayers[arena_index])
	{
		...
		ShowSyncHudText(client, hm_HP, "HP: %d", g_iPlayerHP[client]);
	} else {
	...
	if(g_bArenaBBall[arena_index])
	{
		if(g_iArenaStatus[arena_index] == AS_FIGHT)
		{
			if(g_bPlayerHasIntel[client])
				...
				ShowSyncHudText(client, hm_HP, "The enemy has the intel.", g_iPlayerHP[client]);
			else
				ShowSyncHudText(client, hm_HP, "Secure the intel.", g_iPlayerHP[client]);
		} else {
			ShowSyncHudText(client, hm_HP, "", g_iPlayerHP[client]);
		}
	}
	...
	{
		if (fraglimit>0)
			Format(report,sizeof(report),"%s [%d]",g_sArenaName[arena_index],fraglimit);
		else
	...
	if(g_bFourPersonArena[arena_index])
	{
		if (red_f1)
		{
			...
			{
				if (g_bNoStats || g_bNoDisplayRating)
					Format(report,sizeof(report),"%s\n%N | %N [%d]",report,red_f1, red_f2,g_iArenaScore[arena_index][SLOT_ONE]);
				else
					Format(report,sizeof(report),"%s\n%N | %N [%d]: %d",report,red_f1, red_f2,g_iPlayerRating[red_f1],g_iArenaScore[arena_index][SLOT_ONE]);
			}
			else
			{
				if (g_bNoStats || g_bNoDisplayRating)
		...
		if (blu_f1)
		{
			...
			{
				if (g_bNoStats || g_bNoDisplayRating)
					Format(report,sizeof(report),"%s\n%N | %N [%d]",report,blu_f1, blu_f2,g_iArenaScore[arena_index][SLOT_TWO]);
				else
					Format(report,sizeof(report),"%s\n%N | %N (%d) [%d]",report,blu_f1,blu_f2,g_iPlayerRating[blu_f1],g_iArenaScore[arena_index][SLOT_TWO]);
			}
			else
			{
				if (g_bNoStats || g_bNoDisplayRating)

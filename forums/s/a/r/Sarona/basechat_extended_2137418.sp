...
{
        name = "Basic Chat Modified",
        author = "Nikooo777, Morell",
...
SendChatToAll(client, String:message[])
{
        ...
        for (new i = 1; i <= MaxClients; i++)
        {
                ...
                CPrintToChat(i, "{lightgreen}(ADMIN) %s: {olive}%s", nameBuf, message);
        }
}
...
SendChatToAdmins(from, String:message[])
{
        ...
        for (new i = 1; i <= MaxClients; i++)
        {
                ...
                {
                        CPrintToChat(i, "{white}(%sADMINS ONLY) %N: {green} %s", fromAdmin ? "" : "TO ", from, message);
                }      
        }
}

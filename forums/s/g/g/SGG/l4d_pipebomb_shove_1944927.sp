...
static	Handle:g_hMPGameMode, Handle:g_hCvarAllow, Handle:g_hCvarModes, Handle:g_hCvarModesOff, Handle:g_hCvarTargetDamage, Handle:g_hCvarModesTog, bool:g_bCvarAllow,
		Handle:g_hCvarDamage, Handle:g_hCvarDistance, Handle:g_hCvarInfected, Float:g_fCvarDamage, Float:g_fCvarDistance, g_iCvarInfected,
		bool:g_bLeft4Dead2, Handle:sdkActivatePipe, g_iClients[MAX_GRENADES], g_iGrenades[MAX_GRENADES], g_iInflictors[MAX_GRENADES];

new Float:TargetDamage = 0.0;
...
public OnPluginStart()
{
	...
	g_hCvarTargetDamage = CreateConVar (	"l4d_pipebomb_shove_target_damage",	"0",	"Damage to inflict only to the target", CVAR_FLAGS );
	if( g_bLeft4Dead2 )
	...
	HookConVarChange(g_hCvarTargetDamage,		ConVarChanged_Cvars);
}
...
GetCvars()
{
	...
	TargetDamage = GetConVarFloat(g_hCvarTargetDamage);
}
...
public OnEntityDestroyed(entity)
{
	...
	return;
	if (StrEqual(EntityName, "pipe_bomb_projectile", false))
	{
		entity = EntIndexToEntRef(entity);
		for( new i = 0; i < MAX_GRENADES; i++ )
		{
			if( g_iGrenades[i] == entity )
			{
				SDKHooks_TakeDamage(g_iClients[i], g_iInflictors[i], g_iInflictors[i], TargetDamage, DMG_GENERIC);
				g_iClients[i] = 0;
				g_iGrenades[i] = 0;
				g_iInflictors[i] = 0;
			}

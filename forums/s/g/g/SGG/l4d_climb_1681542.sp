...
public Action:player_jump(Handle:hEvent, const String:name[], bool:dontBroadcast)
{
	...
	{
		if(CanUse(client) && isGhost == true)  //------------------------------------------------added by sgg.
			{
				PrintHintText(client,"To climb on any wall in ghost mode, jump then crouch");
			}
		else if (CanUse(client) && isGhost == false)
			{
...
Start(client)
 {
	...
	if(GetVectorDistance(hit, vOrigin)<gbodywidth*2.0)   //calc distince between body and surfece, if it is close enough, then get into climb mode.
	{
		...
		if(c>0)
		{
			...
			new bool:isGhost=false; // Ghost check
			if(GetClientTeam(client)==3 && GetEntData(client, g_ghostoffest, 1))isGhost=true;
			if (isGhost)
			{
				SDKUnhook (c, SDKHook_SetTransmit, OnSetTransmitGhost); 
				SDKHook(c, SDKHook_SetTransmit, OnSetTransmitGhost);//only infected see ghosts
			}
			SDKUnhook( client, SDKHook_PostThinkPost,  PreThink); 
			SDKHook( client, SDKHook_PostThinkPost,  PreThink);  // watch it.
			SaveWeapon(client );
...
public Action:OnSetTransmitGhost (ghost, client)
{
...
OnAir(client)
{
	...
	new bool:isGhost=false;
	if(GetClientTeam(client)==3 && GetEntData(client, g_ghostoffest, 1))isGhost=true; 
	if((button & IN_DUCK) && isGhost==true)//--------------------------------------------------- added by SGG
	{
		Start(client);
	}

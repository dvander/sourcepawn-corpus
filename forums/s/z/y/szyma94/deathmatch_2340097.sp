...
#define PLUGIN_VERSION 	"2.0.3 Fix EN"
#define PLUGIN_NAME		"Deathmatch 2015/09/04 Fixed BBS.93X.NET"
...
{
	name 			= PLUGIN_NAME,
	author			= "Maxximou5 | Fix Edited bbs.93x.net",
...
public OnPluginStart()
{
	...
	CreateTimer(10.0, AutoRefeshAmmo, _, TIMER_REPEAT);
	...
	LoadConfig();
	// Admin commands
	RegAdminCmd("sm_dmadmin", Command_SpawnMenu, ADMFLAG_CHANGEMAP, "Opens the spawn point menu.");
...
DoClipRefillAmmo(weaponRef, any:client)
{
	...
	if (IsValidEdict(weaponEntity))
	{
		...
		SetReserveAmmo(client, weaponEntity, maxAmmoCount);
...
Handle:BuildOptionsMenu(bool:sameWeaponsEnabled)
{
	...
	Format(MenuItem, sizeof(MenuItem),"%t", "Weapon Menu");
	SetMenuTitle(menu, MenuItem);

	SetMenuExitButton(menu, false);
	FormatEx(MenuItem, sizeof(MenuItem),"%t", "New weapons");
	AddMenuItem(menu, "New", MenuItem);
	
	FormatEx(MenuItem, sizeof(MenuItem),"%t", "Same weapons");
	AddMenuItem(menu, "Same 1", MenuItem, sameWeaponsStyle);
	
	FormatEx(MenuItem, sizeof(MenuItem),"%t", "Same weapons every round");
	AddMenuItem(menu, "Same All", MenuItem, sameWeaponsStyle);
	
	FormatEx(MenuItem, sizeof(MenuItem),"%t", "Random weapons");
	AddMenuItem(menu, "Random 1", MenuItem);
	
	FormatEx(MenuItem, sizeof(MenuItem),"%t", "Random weapons every round");
	AddMenuItem(menu, "Random All", MenuItem);
...
Handle:BuildSpawnEditorMenu()
{
	...
	char MenuItem2[128];
	FormatEx(MenuItem2, sizeof(MenuItem2),"%t", "Teleport to nearest");
	AddMenuItem(menu, "Nearest", MenuItem2);
	
	FormatEx(MenuItem2, sizeof(MenuItem2),"%t", "Teleport to previous");
	AddMenuItem(menu, "Previous", "Teleport to previous");
	
	FormatEx(MenuItem2, sizeof(MenuItem2),"%t", "Teleport to next");
	AddMenuItem(menu, "Next", MenuItem2);
	
	FormatEx(MenuItem2, sizeof(MenuItem2),"%t", "Add position");
	AddMenuItem(menu, "Add", MenuItem2);

	
	FormatEx(MenuItem2, sizeof(MenuItem2),"%t", "Insert position here");
	AddMenuItem(menu, "Insert", MenuItem2);
	
	FormatEx(MenuItem2, sizeof(MenuItem2),"%t", "Delete nearest");
	AddMenuItem(menu, "Delete", MenuItem2);
	
	FormatEx(MenuItem2, sizeof(MenuItem2),"%t", "Delete all");
	AddMenuItem(menu, "Delete All", MenuItem2);
	
	FormatEx(MenuItem2, sizeof(MenuItem2),"%t", "Save Configuration");
	AddMenuItem(menu, "Save", MenuItem2);
...
public Action:AutoRefeshAmmo(Handle:timer)
{
	decl weaponEntity;
	for (new i = 1; i <= MaxClients; i++)
		
	if(IsClientInGame(i) && i != -1 && IsPlayerAlive(i) && i > 0 && i <= MaxClients && !IsFakeClient(i))
	{
		weaponEntity = GetPlayerWeaponSlot(i, _:SlotPrimary);
		if (weaponEntity != -1)
			DoResRefillAmmo(EntIndexToEntRef(weaponEntity), i);
		
		weaponEntity = GetPlayerWeaponSlot(i, _:SlotSecondary);
		if (weaponEntity != -1)
			DoResRefillAmmo(EntIndexToEntRef(weaponEntity), i);
	}
}

...
//Updates With 1.2.6: Added the option to choose between killing types.
...
public OnPluginStart()
{
	...
	sm_WoD_KillType	= CreateConVar("sm_WoD_KillType", "0", "How the players should be killed 0-Suicide 1-Kill by Damage");
					...
					if ((GetClientTeam(i) != iWinner) && IsPlayerAlive(i) && NotImmune(i))
						{
							if (g_iFreePassesLeft[i] < 1)
							{
								...
								if (GetConVarInt(sm_WoD_KillType) == 0)
								{	

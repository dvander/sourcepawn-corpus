...
** By SirLamer
**
** Version 0.5.4.0 - 2010-03-13
**
** Designed and written for Team CHEESUS (http://www.teamcheesus.com)
**  for operation on The Glory Hole TF2 server (gloryhole.nuclearfallout.net)
**
...
LookupOptions(client) {
	...
	SQL_TQuery(music_db, Query_GetOptions, query, client, DBPrio_High);
	
	return true;
}


public Query_GetOptions(Handle:owner, Handle:result, const String:error[], any:client) {
	if(result == INVALID_HANDLE) {
		if(IsClientInGame(client) && !IsFakeClient(client)) {

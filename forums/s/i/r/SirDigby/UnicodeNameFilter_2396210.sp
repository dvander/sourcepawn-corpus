...
public OnPluginStart()
{
	...
	g_iEnabledLogs = GetConVarInt(g_hEnabledLogs);
	
	g_hEnabledTimerChecking=		CreateConVar("unf_timer_checking","1","1 - Enable / 0 - Disable");
	g_bEnabledTimerChecking = GetConVarBool(g_hEnabledTimerChecking);
	...
	g_bEnabledSkipFilterChecking = GetConVarBool(g_hEnabledSkipFilterChecking);
	...
	g_bEnabledPreventNameCopying = GetConVarBool(g_hEnabledPreventNameCopying);
	...
	g_bEnabledUseridPrefix = GetConVarBool(g_hEnabledUseridPrefix);
	...
	GetConVarString(g_hNonUseridPrefix, g_sNonUseridPrefix, sizeof(g_sNonUseridPrefix));
	...
	g_bLockNameAfterFiltering = GetConVarBool(g_hLockNameAfterFiltering);
	...
	g_bTellPlayersOfAnyChanges = GetConVarBool(g_hTellPlayersOfAnyChanges);
...
public Action:Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if(g_bEnabledSkipFilterChecking && CheckCommandAccess(client, "unf_skipfilter", ADMFLAG_RESERVATION, false))
		{
			if(g_iEnabledLogs==1)
			{
...
public Action:TimerChecking(Handle:timer)
{
	...
	for(new i;i<=MaxClients;i++)
	{
		if(ValidPlayer(i) && !IsFakeClient(i))
		{
			...
			if(g_bEnabledSkipFilterChecking && !CheckCommandAccess(i, "unf_skipfilter", ADMFLAG_RESERVATION, false))
			{
				if(g_bEnabledPreventNameCopying==true)
				{

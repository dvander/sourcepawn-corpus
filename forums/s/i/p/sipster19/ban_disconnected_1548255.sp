...
#define STORED_ENTRIES 35
#define PLUGIN_VERSION "1.04"

public Plugin:myinfo =
{
	name        = "Ban disconnected players",
	author      = "mad_hamster, Troy",
...
public Action:OnEventPlayerDisconnect(Handle:event, const String:name[], bool:dont_broadcast)
{
	...
	// Ignore if authid does not start with STEAM_ (possibly bot), or is an identical steam id.
	...
	if (strncmp(steam_id, "STEAM_", 6) == 0)
	{
		...
		{
			if (strcmp(steam_id, disconnected_player_authids[queue_translate_pos(i)]) == 0) { search = true; }
		}
		if (!search)
		{
			new pos = queue_push();
...
CheckAndPerformBan(client, const String:steamid[], minutes, const String:reason[])
{
	...
	if ((target_aid = FindAdminByIdentity(AUTHMETHOD_STEAM, steamid)) == INVALID_ADMIN_ID || CanAdminTarget(source_aid, target_aid))
	{
	...
	else ReplyToCommand(client, "[SM] You can't ban an admin with higher immunity than yourself.");
}
...
DisplayBanTargetMenu(client)
{
	...
	if (queue_get_size() == 0)
	{
		ReplyToCommand(client, "[SM] No player stored.");
	}
	for (new i=queue_get_size()-1; i>=0; --i)
	{
		new pos = queue_translate_pos(i);
		decl String:client_info[64];
		Format(client_info, sizeof(client_info), "%s (%s)",

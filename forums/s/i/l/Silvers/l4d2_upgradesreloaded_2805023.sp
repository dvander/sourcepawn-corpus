...
#define PLUGIN_VERSION		"1.7.4.Silvers.1"
#define UPGRADEID		34
...
new Handle:UpgradeEnabled[MAX_UPGRADES + 1];
...
new Handle:KnifeCooldownTimer[MAXPLAYERS + 1];
new Handle:HotMealCooldown[MAXPLAYERS + 1];
new Handle:CombatGlovesCooldownTimer[MAXPLAYERS + 1];
new Handle:penalty_upgrades;
new Handle:SetClientUpgrades[MAXPLAYERS + 1];
new Handle:ReloadCooldown[MAXPLAYERS + 1];
new Handle:g_PerkMode;
new Handle:PerkSlots;
new Handle:PerkBonusSlots;
new Handle:BoosterSlots;
new Handle:hDatabase;

new Handle:g_VarFirstAidDuration;
new Handle:g_VarReviveDuration;
new Handle:g_VarAdrenalineDuration;
...
new iCombatGloves[MAXPLAYERS + 1];
new iHotMeal[MAXPLAYERS + 1];
new iBoosterSlots[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_saveupgrade", CommandSaveData, ADMFLAG_CHEATS,  "Load player data.");
	RegAdminCmd("sm_loadupgrade", CommandLoadData, ADMFLAG_CHEATS,  "Load player data.");
...
public LoadPlayerData(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	...
	SQL_FetchRow(hndl); // Fix for SM 1.11
...
public Action:CommandSaveData(client, args)
{
	if( client && IsClientInGame(client) ) SaveData(client);
	return Plugin_Handled;
}

public Action:CommandLoadData(client, args)
{
	if( client && IsClientInGame(client) ) LoadData(client);
	return Plugin_Handled;
}

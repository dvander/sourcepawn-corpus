...
new	Handle:g_hCvarMinPlayers = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_hCvarMinPlayers = CreateConVar("tf2_perm_crit_min_players", "1.0", "Minimum amount of players on the server to start a vote");
...
public OnMapStart()
{
	//make sure that we start with crits on
	ServerCommand("tf_weapon_criticals 1");
}

public OnClientPostAdminCheck(client)
{
	//check if we have reached the minimum player count and no vote was done yet
	if((GetClientCount(true) >= GetConVarInt(g_hCvarMinPlayers)) && !g_bVoteFinished)
	{
		//wait CalcDelay seconds before calculation if we have crit on or off

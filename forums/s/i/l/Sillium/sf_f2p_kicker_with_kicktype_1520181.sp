...
new Handle:g_hKickType = INVALID_HANDLE;
new g_iQuickPlayOffStatus = 0;

public Plugin:myinfo =
{
    name = "F2P Kicker",
    author = "Asherkin, spezi|Fanta, Sillium",
...
{
    CreateConVar("f2p_kicker_version", PLUGIN_VERSION, "Free 2 Play Kicked", FCVAR_DONTRECORD|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
    ...
    g_hKickType = CreateConVar("sm_f2p_kicker_kicktype", "1", "How to select a client to kick", 0, true, 0.0, true, 2.0);
    
    new realSlots = MaxClients - GetConVarInt(g_hReservedSlots);
    g_hMaxF2PSlots = CreateConVar("sm_f2p_max", "22", "Maximum F2P slots before F2P users get blocked on join.", 0, true, 0.0, true, Float:realSlots);
    ...
    //Auto create a config
    AutoExecConfig(true, "F2PKicker");
...
public checkPlayers()
{
    new KickType:type = KickType:GetConVarInt(g_hKickType);
    ...
    for (new client = 1; client <= MaxClients; client++)
    {
        ...
        if (isFree2Play(client))
        {
            if (type == Kick_HighestTime)
            {
                if (GetClientTime(client) > value)
                {
                    value = GetClientTime(client);
                    currentF2PUser = client;
                }
            }
            else if (type == Kick_HighestPing)
            {
               if (GetClientAvgLatency(i, NetFlow_Outgoing) > value)
               {
                   value = GetClientAvgLatency(i, NetFlow_Outgoing);
                   currentF2PUser = client;
               }
            }
            else if (type == Kick_LowestTime)
            {
                if(GetClientTime(client) < value)
                {
                    value = GetClientTime(client);

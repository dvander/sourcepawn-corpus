...
#include <colors>
#include <attachables>
//#include "funcommandsX.sp"

// *********************************************************************************
// CONSTANTS
// *********************************************************************************
// ---- Plugin-related constants ---------------------------------------------------
#define PLUGIN_NAME			  "[TF2] Equipment Manager with funcommandsx"
#define PLUGIN_AUTHOR			"Damizean modded by Jameless, asherkin patch, with FlaminSarge's simplifications"
#define PLUGIN_VERSION		   "1.1.8.7"
...
new g_PlayerAlpha[MAXPLAYERS+1];
new g_PlayerSpyHatVisible[MAXPLAYERS+1];
...
{	
	...
	for( new i = 0; i < sizeof(g_PlayerAlpha); i++ )
	{
		g_PlayerAlpha[i] = 255;
	}
	...
	{
		g_PlayerSpyHatVisible[i] = true;
	}
	CreateTimer(0.2, ChekSpyCondFlag, _, TIMER_REPEAT);
}
...
public OnClientPutInServer(iClient)
{
	...
	g_PlayerAlpha[iClient] = 255;
	g_PlayerSpyHatVisible[iClient] = true;
}
...
public Entity_Resupply(const String:name[], caller, activator, Float:delay)
{
	CreateTimer(g_fSpawnDelay, Timer_EquipItem, activator, TIMER_FLAG_NO_MAPCHANGE);
}
...
// ------------------------------------------------------------------------
// ChekSpyCondFlag to "Hook" when a spy is TF_CONDFLAG_CLOAKED or TF_CONDFLAG_DISGUISED
// ------------------------------------------------------------------------
// Original code : https://forums.alliedmods.net/showthread.php?p=1322213
// ------------------------------------------------------------------------
public Action:ChekSpyCondFlag(Handle:timer)
{
	for (new i = 1; i <= MAXPLAYERS; i++) {
		if (IsClientInGame(i) && IsPlayerAlive(i)) { 
			//PrintToConsole(i,"DEBUG: TIMER");
			if ((TF2_GetPlayerConditionFlags(i) & TF_CONDFLAG_CLOAKED) || (TF2_GetPlayerConditionFlags(i) & TF_CONDFLAG_DISGUISED)) 
			{
				if (g_PlayerSpyHatVisible[i])
				{
					g_PlayerSpyHatVisible[i] = false;
					HidePlayerEquipement(i,true);
					//PrintToConsole(i,"[TF2]: Your equipement is now invisible.");
				}
			...
			else 
			{
				...
				{
					g_PlayerSpyHatVisible[i] = true;
					HidePlayerEquipement(i,false);
					//PrintToConsole(i,"[TF2]: Your equipement is now visible.");
				}
...
Item_Equip(iClient, iItem)
{
	...
	new iInvisReq			   = g_Invis_Req[iItem];

	if(g_Invisible[iClient] == true){if(iSlot==0){KillInvis(iClient);}}
	...
	{		   
		...
		if (IsPlayerAlive(iClient)) 
		{
			...
			if (!(g_iItemFlags[g_iItemCount] & FLAG_INVISIBLE))
			{
				...
				if (iEntity != -1)
				{
					if(iInvisReq == 1){CreateInvis(iClient);}
				...
				if (iEntityOthers != -1)
				{
					...
					SetEntData(iEntityOthers, FindSendPropOffs("CPhysicsProp", "m_nSkin"), (iTeam-2), 1, true); 
...
Item_Equip_Admin_Force(iClient, iItem)
{
	// Assert if the player is alive.
	if (!IsValidClient(iClient)) return;
	if (!Item_IsWearable_Admin_Force(iClient, iItem)) return;
...
Item_ParseList()
{
	...
	do
	{
		...
		KvGetString(kvItemList, "invisreq", strLine, sizeof(strLine)); g_Invis_Req[g_iItemCount]   = StringToInt(strLine);
...
Item_IsWearable_Admin_Force(iClient,Item)
{
	// If the selected item is not valid, it can't be wearable! Rargh!
	if (Item < 0 || Item >= g_iItemCount) return 0;
...
KillInvis(client)
{
	g_PlayerAlpha[client] = 255;
	g_Invisible[client] = false;
	SetEntityRenderMode(client, RENDER_NORMAL); //change to RENDER_TRANSCOLOR if it's not working. It used to be RENDER_TRANSCOLOR
	SetEntityRenderColor(client, 255, 255, 255, g_PlayerAlpha[client]);
}

CreateInvis(client)	
{
	g_PlayerAlpha[client] = 0;
...
// ------------------------------------------------------------------------
// HidePlayerEquipement to show or hide attachable Identity (other players)
// ------------------------------------------------------------------------
// Inspired by VisibleWeapons by FlaminSarge (https://forums.alliedmods.net/showthread.php?t=145664)
// ------------------------------------------------------------------------
HidePlayerEquipement(client, bool:hide)
{
	//g_iPlayerOthersEntity[iClient][iSlot]
	if (hide) 
	{
		for (new i = 0; i < MAX_SLOTS; i++)
		{
			if (g_iPlayerOthersEntity[client][i] != -1 && IsValidEntity(g_iPlayerOthersEntity[client][i]) && Attachable_IsHooked(g_iPlayerOthersEntity[client][i]))
			{
					SetEntityRenderMode(g_iPlayerOthersEntity[client][i], RENDER_TRANSCOLOR);
					SetEntityRenderColor(g_iPlayerOthersEntity[client][i], 255, 255, 255, 0);
			}
		}
	}
	else
	{
		for (new i = 0; i < MAX_SLOTS; i++)
		{
			if (g_iPlayerOthersEntity[client][i] != -1 && IsValidEntity(g_iPlayerOthersEntity[client][i]) && Attachable_IsHooked(g_iPlayerOthersEntity[client][i]))
			{
				SetEntityRenderMode(g_iPlayerOthersEntity[client][i], RENDER_NORMAL);
				SetEntityRenderColor(g_iPlayerOthersEntity[client][i], 255, 255, 255, 255);
			}

...
public OnPluginStart(){
	CreateConVar("sm_swearreplace_version", PLUGIN_VERSION, "Swear Replace Version");
	cvarswearmode 	 = CreateConVar("sm_swear_replace_mode","1","Options are 1 (replace), 2 (stars), or 3 (eat the text).");
	cvarswearname 	 = CreateConVar("sm_swear_name_check","0","1 is on, 0 is off.");
	cvarsbppgag   	 = CreateConVar("sm_swear_gag","0","1 to gag for 10min (enable this only if you have sourcebans++ installed in your server!!)");
	cvarswearreplace = CreateConVar("sm_swear_replace","*****","You can use any word here...this is what replaces the swear word in mode 2");
		...
		if(client != 0){
			...
			while (i < g_swearNum){
				if (StrContains(clientName, g_swearwords[i], false) != -1 ){
					...
					ClientCommand(client,"name %s", "Im monki");
				}
				i++;
			}	
		}
...
public Action Command_Say(client, args){
	if(client != 0){
		char reason[64] = "[Auto] Bieng racist (N word)!";
		...
		if (found){
			...
			if (GetConVarInt(cvarswearmode) == 1){
				...
				{
					CPrintToChatAll("{green}[Racist] {red}%s {default}: %s", clientname, speech[startidx]);
				}
				else if(GetClientTeam(client) == 3)
				{
					CPrintToChatAll("{green}[Racist] {blue}%s {default}: %s", clientname, speech[startidx]);
				}
				CPrintToChat(client, "{green}[SM] {default}Please do not use foul language here!");

				if(GetConVarInt(cvarsbppgag) == 1)
				{
					SourceComms_SetClientGag(client, true, 10, true, reason);
					CPrintToChat(client, "{green}[SourceComms++] {default}You have been gagged for 10min, for being racist!");
					CPrintToChatAll("{green}[SourceComms++] {default}%s Has been gagged for 10min, for being racist!", clientname);
				}
				...
				for (new j = 1; j < MaxClients ; j++){
					if(IsClientConnected(j)){
						if(GetClientTeam(j) == GetClientTeam(client)){
							...
							{
								CPrintToChat(j, "(Team) {green}[Racist] {red}%s {default}: %s", clientname, speech[startidx]);
							}

							else if(GetClientTeam(j) == 3)
							{
								CPrintToChat(j, "(Team) {green}[Racist] {blue}%s {default}: %s", clientname, speech[startidx]);
							}
						}
					}
				}

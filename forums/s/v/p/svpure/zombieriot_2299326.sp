...
#define VERSION "2.0"
#define SMOKERMODEL "models/player/kuristaja/wraith/wraith.mdl"
...
{
    name = "Zombie Riot", 
    author = "Greyscale [Modified by Tarnish]", 
...
public OnMapStart()
{
    ...
    LoadZombieData(true);
    LoadDayData(true);
    PrecacheSound("music/ambient/zr/thunder1.mp3", true);
...
public OnClientDisconnect(client)
{
    ...
    if (count <= 0 && tHUD != INVALID_HANDLE)
    {
        TerminateRound(3.0, Terrorists_Win);
    }
	//added below until end
    new CT_Team_Count = GetTeamClientCount(3);
    if (CT_Team_Count <= 0) // <=1 (WHY?)
    {
        //CS_TerminateRound(3, CSRoundEndReason:8, false);
		//CS_TerminateRound(3, Terrorists_Win);
        //ZombiesWin();
		TerminateRound(3.0, Terrorists_Win);
		ZombiesWin();
    }

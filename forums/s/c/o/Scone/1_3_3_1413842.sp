...
showMenu_Inbox(client) {
	...
	if(!g_isSQLite)
		Format(query, sizeof(query), "SELECT id, unread, sourceName, (UNIX_timestamp(NOW()) - UNIX_timestamp(time)) as timeago FROM `%s` WHERE destId = '%s' AND deleted = 0 ORDER BY time DESC LIMIT 1024", table_e, g_SteamId_cache[client]);
	else
...
showMenu_Outbox(client) {
	...
	decl String:query[1000];
	if(!g_isSQLite)
		Format(query, sizeof(query), "SELECT id, destId, (UNIX_timestamp(NOW()) - UNIX_timestamp(time)) AS timeago, lastName FROM `%s` LEFT JOIN `%s_players` ON steamId = destId WHERE sourceId = '%s' ORDER BY time DESC LIMIT 1024", table_e, table_e, g_SteamId_cache[client]);
	else
...
showMenu_Message(client, messageId) {
	...
	decl String:query[1000];
	if(!g_isSQLite)
		Format(query, sizeof(query), "SELECT message, sourceId, sourceName, (UNIX_timestamp(NOW()) - UNIX_timestamp(time)) as timeago, id FROM `%s` WHERE id = %d ORDER BY time DESC LIMIT 1024", table_e, messageId);
	else
...
showMenu_SentMessage(client, messageId) {
	...
	decl String:query[1000];
	if(!g_isSQLite)
		Format(query, sizeof(query), "SELECT message, destId, (UNIX_timestamp(NOW()) - UNIX_timestamp(time)) as timeago, id FROM `%s` WHERE id = %d ORDER BY time DESC LIMIT 1024", table_e, messageId);
	else
...
showMenu_User(client, const String:targetId[]) {
	...
	if(!g_isSQLite)
		...
		(SELECT (UNIX_timestamp(NOW()) - UNIX_timestamp(lastSeen) < 30) AS isonline FROM `%s_players` WHERE steamId = '%s') AS isonline", 
		table_e, userId_e, targetId_e,
		table_e, userId_e, targetId_e,
		table_e, targetId_e,
		table_e, userId_e,
...
UpdatePlayerNameInDatabase(const String:name[], const String:steamId[]) {
	...
	decl String:query[1024];
	if(!g_isSQLite)
		Format(query, sizeof(query), "INSERT INTO `%s_players` (lastName, steamId, lastSeen) VALUES ('%s', '%s', CURRENT_TIMESTAMP) ON DUPLICATE KEY UPDATE lastName = '%s'", table_e, name_e, steamId_e, name_e);
	else
...
setupTable() {
	...
	CREATE TABLE IF NOT EXISTS `%s_friends` ( \
		...
		UNIQUE `UserFriendPair` (`userId` (31), `friendId` (31)) \
	) ENGINE = MYISAM;", table_e);
		SQL_TQuery(db, callback_ignoreresult, query, 1, DBPrio_High);
	...
	CREATE TABLE IF NOT EXISTS `%s_blocks` ( \
		...
		UNIQUE `UserBlockPair` (`userId` (31), `blockedId` (31)) \
	) ENGINE = MYISAM;", table_e);
		SQL_TQuery(db, callback_ignoreresult, query, 1, DBPrio_High);
	...
	CREATE TABLE IF NOT EXISTS `%s_groupmembers` ( \
		...
		UNIQUE `GroupMemberPair` (`groupId`, `memberId` (31)) \
	) ENGINE = MYISAM;", table_e);
		SQL_TQuery(db, callback_ignoreresult, query, 1, DBPrio_High);

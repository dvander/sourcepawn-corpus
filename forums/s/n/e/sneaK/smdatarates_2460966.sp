...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_drhelp", sm_drhelp, "Display SM Rates Help menu");
	RegConsoleCmd("sm_dr", sm_dr, "Display your current data rates in menu");
	RegConsoleCmd("sm_drs", sm_drs, "Display all players current data rates in your console");
	RegConsoleCmd("sm_serverdrs", sm_serverdrs, "Display server data rate settings in menu");
...
public Action sm_drhelp(int client, int args)
{
	...
	panel.DrawItem("exit\n");
	panel.DrawItem("!dr - Show your data rates");
	panel.DrawItem("!drs - Show all players data rates in your console");
	panel.DrawItem("!serverdrs - Show server data rate settings");
...
public int panelhandler_help(Menu menu, MenuAction action, int param1, int param2)
{
	...
	else if(action == MenuAction_Select)
	{
		switch(param2)
		{
			...
			case 2:
			{
				FakeClientCommandEx(param1, "say !dr");
			}
			case 3:
			{
				FakeClientCommandEx(param1, "say !drs");
				PrintToChat(param1, "[SM] Players data rates printed in your console...");
			}
			case 4:
			{
				FakeClientCommandEx(param1, "say !serverdrs");
			}
		}
	}
}
...
public Action sm_dr(int client, int args)
{
	if(client == 0) return Plugin_Handled;
...
public Action sm_drs(int client, int args)
{
	if(client == 0) return Plugin_Handled;
...
public Action sm_serverdrs(int client, int args)
{
	if(client == 0) return Plugin_Handled;

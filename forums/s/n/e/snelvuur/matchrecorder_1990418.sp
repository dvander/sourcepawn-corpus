...
new Handle:g_DemoDirectory = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	RegServerCmd("mp_tournament_restart", TournamentRestartHook);
        ...
        // Directory folder demo's. End with a /, default goes in the tf folder.
        g_DemoDirectory = CreateConVar("sm_demo_directory", "", "Demo directory name relative to the 'tf' folder. Full path can be used too");
}
...
StartRecording()
{
	...
	decl String:cvar_demo[256];
	GetConVarString(g_DemoDirectory, cvar_demo, sizeof(cvar_demo));
	Format(command, sizeof(command), "tv_record %s%s", cvar_demo,filename);

...
*
*	Changelog version privÃ©e:
*		- 1.0.0 :
*					- PremiÃ¨re version stable et complÃ¨te.
*		- 1.0.1 :
*					- TF2_IsPlayerInvuln dÃ©placÃ© pour supprimer les messages d'erreur dans les logs.
*		- 1.0.2 :
*					- RÃ©Ã©criture d'une condition empÃ©chant le plugin de fonctionner si sm_goomba_invuln Ã©tait diffÃ©rent de 1.
*					- Ajout du nom d'Ã©quipe pour le tueur et le tuÃ©.
*					- Modification des couleurs du message sur le chat gÃ©nÃ©ral.
*		- 1.1.0 :
*					- Utilisation de dhTakeDamage Ã  la place de ForcePlayerSuicide :
*						- Nouvel include.
*						- Besoin de Dukehacks.
*						- Le goomba stomp est dÃ©sormais considÃ©rÃ© comme un kill Ã  part entiÃ¨re (rapporte 1 point, peut causer domination/vengeance, killcam).
*					- Ajout de particules lors d'un stomp.
*					- Suppression de LogPlyrPlyrEvent ainsi que toutes les fonction qui lui sont liÃ©, le jeu log les stomp trÃ¨s bien par lui mÃªme.
*					- Ajout de conditions supplÃ©mentaires (already_stomped) afin d'allÃ©ger le travail du plugin. En contrepartie il n'est plus possible de stomper plusieurs personnes en une fois.
*					- sm_goomba_invuln renommÃ© en sm_goomba_uber_immun et par dÃ©faut sur 1.
*					- sm_goomba_minheight est maintenant supÃ©rieur ou Ã©gal Ã  0 dans un souci de logique.
*					- LÃ©ger nettoyage du code.
*		- 1.1.1 :
*					- sm_goomba_minheight est par dÃ©faut Ã  360 pour empÃ©cher le stomp par le double saut du scout.
*					- Ajout de sm_goomba_stun_imun ajoutÃ© et par dÃ©faut sur 1, cela protÃ¨ge les personnes stunnÃ©es d'Ãªtre stompÃ©.
*					- Ajout de sm_goomba_cloak_imun ajoutÃ© et par dÃ©faut sur 1, cela protÃ¨ge les personnes d'Ãªtre stompÃ© par un spy cloakÃ©.
*		- 1.1.2 :
*					- Le stomp enlÃ¨ve le dÃ©guisement du spy qui l'utilise.
*					- Modification du message de stomp avec moins de couleurs.
*		- 1.1.3 :
*					- Utilisation de OnPlayerRunCmd Ã  la place de OnGameFrame, cela devrait rÃ©duire la charge de calcul.
*					- L'event de kill n'est plus supprimÃ© puis recrÃ©Ã©, il est maintenant rÃ©Ã©crit en direct (affichage des dominations, des revanches et des assists supportÃ© complÃ©tement).
*					- Suppression du check pour le godmode :
*						- La deadringer protÃ¨ge du stomp.
*						- Buddha protÃ¨ge du stomp.
...
*		- 1.2.0 :
*					- Added two cvars to set the damages done by the stomp :
*						- goomba_dmg_lifemultiplier
*						- goomba_dmg_add
*						-> Damages = (Victim's actual life * goomba_dmg_lifemultiplier) + goomba_dmg_add.
...
public OnPluginStart()
{
	...
	g_Cvar_DamageLifeMultiplier = CreateConVar("goomba_dmg_lifemultiplier", "1.0", "How much damages the victim will receive based on its actual life", 0, true, 0.0, false, 0.0);
	g_Cvar_DamageAdd = CreateConVar("goomba_dmg_add", "50.0", "Add this amount of damages after goomba_dmg_lifemultiplier calculation", 0, true, 0.0, false, 0.0);
	...
	if(victim > 0 && victim <= MaxClients)
	{
		...
		if(StrEqual(edictName, "player"))
		{
			...
			{				
				...
				// saxton hale
				else if(client == GetClientOfUserId(VSH_GetSaxtonHaleUserId()))
				{
					CancelStomp = true;
				}
				// -saxton hale

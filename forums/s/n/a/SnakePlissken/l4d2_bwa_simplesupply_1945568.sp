...
public OnPluginStart() 
{
	...
	LogMessage("BWA simplesupply started");
}
...
public Event_ItemPickup(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		LogMessage("BWA Item pickup, second chance. Looking for missing medkits");
		SupplyMissingEntities();
	}
	else
		LogMessage("BWA Item Pickup. Already done. Doing nothing");
}
public Event_RoundFreezeEnd(Handle:event, const String:name[], bool:dontBroadcast) 
{
	AlreadyDone = false;
	LogMessage("BWA Round Freeze End. Looking for missing medkits");
...
{
	LogMessage("BWA Manual invoke. Looking for missing medkits");
	SupplyMissingEntities();
}
...
SupplyMissingEntities() 
{
	...
	LogMessage("BWA medkits found:%d",startKitCount);
	new survivorcount	= GetConVarInt(FindConVar("survivor_limit"));
	LogMessage("BWA Survivor Limit:%d",survivorcount);
	// If there are as many or more packs than peeps, no need to continue
	if (survivorcount <= startKitCount) 
	{
		LogMessage("BWA No need to spawn anything. Returning.");
		AlreadyDone = true;
		return;
	}
	new numtospawn = survivorcount - startKitCount;
	LogMessage("BWA Num to spawn:%d",numtospawn);
	...
	{ 
		...
		LogMessage("BWA Found %d Medkits at Start for %d Survivors. Spawning %d extra medkits", startKitCount, survivorcount, numtospawn);
		for (new i = 1; i <= numtospawn; i++)
		{
			SpawnEntityAtLocation(startKitLoc, "weapon_first_aid_kit");
			LogMessage("BWA Spawn a medkit at startKitLocation");
		}
		PrintToChatAll("Kawabunga: Missing medkits, spawn new medkits to correct issue. Thanks to VALVE bug");
	}
	else 
	{
		...
		LogMessage("BWA Found NO Medkits or weapons at Start for %d Survivors. Spawning %d extra medkits, and guns and melee weapons", survivorcount, numtospawn);
		...
		if (FindMedkitSpawnArea(startLoc, MEDKIT_SEARCH_DISTANCE))
		{
			...
			for (new i = 1; i <= numtospawn; i++)
			{
				...
				LogMessage("BWA Spawn a medkit at startAmmo");
			}		
		}
		else
		{
			...
			if (client == -1) 
			{
				LogMessage("BWA Error. No medkit area found and no survivor found to get area. Returning and nothing will be spawned");
				return;
			}
			...
			{
				SpawnEntityAtLocation(startAmmo, "weapon_first_aid_kit");
				LogMessage("BWA Spawn a medkit at Survivor Location");
			}		
		}
		if(GetConVarBool(spawnweps))
		{
			...
			LogMessage("BWA Timer created to spawn weapons");
		}
		PrintToChatAll("Kawabunga: Missing medkits and weapons, spawn new medkits and weapons to correct issue. Thanks to VALVE bug");
		PrintToChatAll("Kawabunga: Maybe weapons arent right for the map, sorry for the inconvenience.");
	}	
	LogMessage("BWA medkits check finished");
	AlreadyDone = true;
} 
...
public Action:Timer_SpawnWeapons(Handle:timer) {
	...
	LogMessage("BWA Spawn ammo pile");
	...
	if (!IsThirdMapOrHigher())
	{
		...
		LogMessage("BWA First map. Spawn smg, shotgun and pistols %d",numhalf);
	...
	{
		//PrintToChatAll("\x03[JBSS]\x01 Spawning \x05%d\x01 smg's, pump shotguns and pistols", numhalf);
		LogMessage("BWA not first map. Spawn ak, m90 and pistols %d",numhalf);
		for (new i = 1; i <= numhalf; i++)
		{
...
stock bool:SpawnEntityAtLocation(Float:loc[3], String:entname[]) {
	...
	else
	{
		...
		LogMessage("BWA Error creating %s at location",entname);
		return false;
	}

 ...
 * - Add setting to allow\disallow class change while frozen
...
public OnPluginStart()
{
	...
	RegConsoleCmd("ubervalue", Cmd_UberValue, "Displays the current rules to the player");
	...
	if (IsFreezeEnabled)
	{
		...
		if (attacker != 0 && attacker != client && FrozenPlayers[client] == false && !RoundEnd)
		{
			if (TF2_GetPlayerClass(client) == TFClass_Medic)
				FrozenMedicUberCharges[client] =  TF_GetUberLevel(client); 
...
bool:FreezePlayer(player)
{
	if (IsClientInGame(player) && IsPlayerAlive(player) && FrozenPlayers[player] == false)
	{
		...
		//See if this will prevent sentries from targetting people
		SetEntProp(player, Prop_Send, "m_nDisguiseClass", TF2_GetPlayerClass(player));
		SetEntProp(player, Prop_Send, "m_nDisguiseTeam", GetClientTeam(player) == 2 ? 3 : 2);
		TF2_AddCondition(player, TFCond_Disguised, 100.0);
...
bool:ThawPlayer(player, bool:resetHealth = true)
{
	...
	if (IsClientInGame(player) && IsPlayerAlive(player) && FrozenPlayers[player])
	{
		...
		//See if this will remove disguised condition
		TF2_RemoveCondition(player, TFCond_Disguised);
...
public Action:Cmd_UberValue(client, args) 
{
	PrintToChat(client, "%i", TF_GetUberLevel(client));
}

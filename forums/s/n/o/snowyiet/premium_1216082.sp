...
public OnPluginStart()
{
	...
	cvBunnyEnabled = CreateConVar("premium_bhop_enabled", "1", "enables bunnyhopping/trimping/bhopdoublejump", FCVAR_PLUGIN|FCVAR_NOTIFY);
...
public OnGameFrame()
{
	...
	{
		if( IsValidEntity(i) && (GetUserFlagBits(i) & PREMIUMFLAG || pTrial[i]) && (GetConVarInt(cvBunnyEnabled)==1) && IsClientInGame(i) && IsPlayerAlive(i) )
		{
			...
			if( (GetClientButtons(i) & IN_JUMP) && (PlayerSpeed[0] >= (2.0)) && ( (GroundEntity != -1) || WasOnGroundLastTime[i] ) )
			{
...
ReloadConvars()
{
	...
	cBhop = GetConVarInt(cvBunnyEnabled);

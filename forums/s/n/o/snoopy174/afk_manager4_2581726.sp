...
#define CONVAR_ADMINS_IMMUNE						7
#define CONVAR_TIMETOMOVE							8
#define CONVAR_TIMETOKICK							9
#define CONVAR_EXCLUDEDEAD							10
#define CONVAR_TF2_ARENAMODE						11
#define CONVAR_ARRAY_SIZE							12
...
char g_sPrefix[] =									"[ {darkred}Свобода {default}]: ";
...
// Chat Functions
void AFK_CPrintToChat(int client, const char[] sMessage, any:...)
{
	int iStart = client;
	int iEnd = MaxClients;
	...
	for (int i = iStart; i <= iEnd; i++)
	{
		if (IsClientInGame(i))
		{
			...
			if (g_bPrefixColors)
				...
				CPrintToChat(i, "[%s] %s", g_sPrefix, sBuffer);
#else
			CPrintToChat(i, "[%s] %s", g_sPrefix, sBuffer);
#endif
		}
...
void LogDebug(bool Translation, char[] text, any:...) // Debug Log Function
{
	...
	CPrintToChatAll("[AFK Manager] %s", message);
...
#if _DEBUG
		LogDebug(false, "Event_PlayerDisconnect - Client: %i Disconnected", client);
#endif
							...
							//{
									...
									if (AFKSpawnTimeleft < AFKMoveTimeleft) // Spawn time left is less than total AFK time left
									{
										...
										else if (AFKSpawnTime%AFK_WARNING_INTERVAL == 0) // Warn AFK Spawn Player
										{
											...
											if ((cvarSpawnTime - AFKSpawnTime) <= GetConVarInt(hCvarWarnSpawnTime))
												AFK_CPrintToChat(client, "%t", "Spawn_Move_Warning", AFKSpawnTimeleft);
										}
										return Plugin_Continue;
									}
#if _DEBUG > 1
								...
								else if (AFKTime%AFK_WARNING_INTERVAL == 0) // Warn AFK Player
								{
									...
									if ((g_iTimeToMove - AFKTime) <= GetConVarInt(hCvarWarnTimeToMove))
										AFK_CPrintToChat(client, "%t", "Move_Warning", AFKMoveTimeleft);
									return Plugin_Continue;
								}
							...
							if (AFKKickTimeleft >= 0)
							{
...
#endif
											if ((cvarSpawnTime - AFKSpawnTime) <= GetConVarInt(hCvarWarnSpawnTime))
												AFK_CPrintToChat(client, "%t", "Spawn_Kick_Warning", AFKSpawnTimeleft);
											return Plugin_Continue;
										}
									}
								...
								else if (AFKTime%AFK_WARNING_INTERVAL == 0) // Warn AFK Player
								{
									...
									if ((g_iTimeToKick - AFKTime) <= GetConVarInt(hCvarWarnTimeToKick))
										AFK_CPrintToChat(client, "%t", "Kick_Warning", AFKKickTimeleft);
									return Plugin_Continue;
								}
							}
							else
...
Action MoveAFKClient(int client, bool Advertise=true) // Move AFK Client to Spectator Team
{
	...
	if (Advertise) // Are we announcing the move to everyone?
	{
		...
		if (Announce == 0)
			AFK_CPrintToChat(client, "%t", "Move_Announce", f_Name);
		else if (Announce == 1)
			AFK_CPrintToChat(0, "%t", "Move_Announce", f_Name);
		else
		{
			for(int i = 1; i <= MaxClients; i++)
				if (IsClientConnected(i))
					if (IsClientInGame(i))
						if ((i == client) || (GetUserAdmin(i) != INVALID_ADMIN_ID))
							AFK_CPrintToChat(i, "%t", "Move_Announce", f_Name);
		}
	}
...
Action KickAFKClient(int client) // Kick AFK Client
{
	...
	if (Announce == 1)
		AFK_CPrintToChat(0, "%t", "Kick_Announce", f_Name);
	else if (Announce == 2)
	{
		for(int i = 1; i <= MaxClients; i++)
			if (IsClientConnected(i))
				if (IsClientInGame(i))
					if (GetUserAdmin(i) != INVALID_ADMIN_ID)
						AFK_CPrintToChat(i, "%t", "Kick_Announce", f_Name);
	}

...
public OnPluginStart()
{
	...
	if (StrEqual(gdir,"tf",false))
	{
		game = GAME_TF2;
		PrintToServer("[SM] Game Detected as TF2!");
	}
	...
	#if defined DEBUG
	PrintToServer("[SM] Hooking round_end...");
	#endif
	HookEvent("round_end", Event_RoundEnd);
	if (game == GAME_TF2)
	{
		#if defined DEBUG
		PrintToServer("[SM] Hooking player_spawn...");
		#endif
...
InvisOff(client)
{
	...
	if(GetClientTeam(client) != SPECTATOR)
	{
		...
		if(IsPlayerAlive(client) && game == GAME_TF2)
		{

...
{
	if (!IsSoundPrecached("legacy/illuminati.mp3"))
	{
		PrecacheSound("legacy/illuminati.mp3", true);
	}
}
...
public Action QualifyKarmaRide(Handle timer, any userid)
{
	...
	{
		EmitSoundToAll("legacy/illuminati.mp3", client);
		...
		PrintToChatAll("\x04[\x03B ☣ T\x05]\x03%N\x05 Karma Ridden \x03%N\x04!!!!!", client, iJockeyVictim);

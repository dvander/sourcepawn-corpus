	...
	if (IsSurvivor(wounded))
	{
		...
		if (StrEqual(name, "player_incapacitated"))
		{
			...
			if (bIsL4D)
			{
				...
				CPrintToChat(wounded, "{blue}(S★H){default} You Got Incapacitated! [{green}%d{default}/{green}%i{default}]", iSHCount[wounded] + 1, iMaxCount);
			...
			else
			{
				...
				CPrintToChat(wounded, "{blue}(S★H){default} You Got Incapacitated! |{green}%d{default}/{green}%i{default}|", iReviveCount + 1, iMaxIncapCount);
...
public Action FireUpMechanism(Handle timer, any userid)
{
	...
	if (hSHTime[client] == null)
	{
		...
		{
			CPrintToChat(client, "{blue}(S★H){default} Press {green}CTRL{default} To Self-Help!");
		}
	...
	if (IsSurvivor(reviver) && IsSurvivor(revived))
	{
		...
		if (event.GetBool("ledge_hang"))
		{
			if (reviver != revived)
			{
				...
				{
					CPrintToChat(reviver, "{blue}(S★H) {default}You Helped{olive} %N{default}!", revived);
				}
				
				if (!IsFakeClient(revived))
				{
					CPrintToChat(revived, "{blue}(S★H) {olive}%N{default} Helped You!", reviver);
				}
			}
			else
			{
				if (!IsFakeClient(revived))
				{
					CPrintToChat(revived, "{blue}(S★H) {default}You Helped Yourself!");
				}
			}
		}
		else
		{
			if (!bIsL4D)
			{
				...
				if (reviver == revived)
				{
					if (!IsFakeClient(revived))
					{
						CPrintToChat(revived, "{blue}(S★H) {default}You Helped Yourself! |{green}%d{default}/{green}%i{default}|", iReviveCount, iMaxIncapCount);
					}
				}
				else
				{
					if (!IsFakeClient(reviver))
					{
						...
						{
							CPrintToChatAll("{blue}(S★H) {olive}%N{default} Saved{olive} %N {default}While Incapacitated!", reviver, revived);
						}
						
						CPrintToChat(reviver, "{blue}(S★H) {default}You Helped{olive} %N{default}! |{green}%d{default}/{green}%i{default}|", revived, iReviveCount, iMaxIncapCount);
					}
					
					if (!IsFakeClient(revived))
					{
						CPrintToChat(revived, "{blue}(S★H) {olive}%N{default} Helped You! |{green}%d{default}/{green}%i{default}|", reviver, iReviveCount, iMaxIncapCount);
					}
				}
			}
			else
			{
				...
				if (reviver == revived)
				{
					if (!IsFakeClient(revived))
					{
						CPrintToChat(revived, "{blue}(S★H) {default}You Helped Yourself! |{green}%d{default}/{green}%i{default}|", iSHCount[revived], iMaxCount);
					}
				}
				else
				{
					if (!IsFakeClient(reviver))
					{
						...
						CPrintToChat(reviver, "{blue}(S★H) {default}You Helped{olive} %N{default}! |{green}%d{default}/{green}%i{default}|", revived, iSHCount[revived], iMaxCount);
					}
					
					if (!IsFakeClient(revived))
					{
						CPrintToChat(revived, "{blue}(S★H) {olive}%N{default} Helped You! |{green}%d{default}/{green}%i{default}|", reviver, iSHCount[revived], iMaxCount);
					}
				}
			}
		}
...
public Action DelaySHNotify(Handle timer, Handle dpDefibAnnounce)
{
	...
	if (defibber == defibbed)
	{
		...
		{
			CPrintToChat(defibbed, "{blue}(S★H) {default}You Defibbed Yourself! |{green}%d{default}/{green}%i{default}|", iReviveCount, iMaxIncapCount);
		}
	}
	else
	{
		...
		{
			CPrintToChat(defibber, "{blue}(S★H) {default}You Defibbed{olive} %N{default}! |{green}%d{default}/{green}%i{default}|", defibbed, iReviveCount, iMaxIncapCount);
		}
		
		if (!IsFakeClient(defibbed))
		{
			CPrintToChat(defibbed, "{blue}(S★H) {olive}%N{default} Defibbed You! |{green}%d{default}/{green}%i{default}|", defibber, iReviveCount, iMaxIncapCount);
		}
	}
...
void SHStatsFixer(int client, bool bDoNotTamper, bool bUseItem = true, bool &bMedkitUsed)
{
	...
	if (bUseItem)
	{
		...
		if ((bEmergencyUsed || bFirstAidUsed) && RemovePlayerItem(client, iUsedItem))
		{
			...
			if (bFirstAidUsed)
			{
				CPrintToChatAll("{blue}(S★H) {olive}%N {default}Helped Themselves With{green} Medkit{default}!", client);
			...
			else if (bEmergencyUsed)
			{
				if (!bIsL4D && StrEqual(sUsedItemName, "weapon_adrenaline", false))
				{
					...
					CPrintToChatAll("{blue}(S★H) {olive}%N {default}Helped Themselves With{green} Adrenaline{default}!", client);
				}
				else
				{
					...
					CPrintToChatAll("{blue}(S★H) {olive}%N {default}Helped Themselves With{green} Pills{default}!", client);
				}
			}
		}

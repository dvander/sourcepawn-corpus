...
#define HUNTER "legacy/hunter_allu.mp3"
#define ENTITY_PROPANE	"models/props_junk/propanecanister001a.mdl"
#define PARTICLE_SMOKE		"missile_hit1"
#define MOLOTOV 0
#define EXPLODE 1
...
public Action TogglePenalty(int client, int args)
{
	...
	CPrintToChat(client, "{orange}[{red}HUNTER{orange}]{default} Penalty Notification: {olive}%s", (bShowPenalty[client]) ? "On" : "Off");
	return Plugin_Handled;
}
...
public Action ResetBHP(int client, int args)
{
	...
	{
		CPrintToChat(client, "{orange}[{red}HUNTER{orange}]{default} Invalid!");
		return Plugin_Handled;
	}
	
	if (iCounterCommand[client] < iCountsMax)
	{
		CPrintToChat(client, "{orange}[{red}HUNTER{orange}]{default}Counts Reset {orange}({olive}%i-/-%i{orange})", iCounterCommand[client], iCountsMax);
		iPenalty[client] = 0;
		iPounces[client] = 0;
	...
	{
	    CPrintToChat(client, "{orange}[{red}HUNTER{orange}]{default}Limit Reached");
	}
...
public void OnMapStart()
{
	...
	PrecacheSound(HUNTER, true);
	
	InitPrecache();
	
	AddFileToDownloadsTable("sound/legacy/hunter_allu.mp3");
}
...
public void OnLungePounce(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		CPrintToChat(pouncer, "{orange}[{red}HUNTER{orange}]{olive} Failed! ({orange}%.1f{olive} /`{orange}%.1f\x05)", GetVectorDistance(fPouncePos[pouncer], fLandedPos), fDistance);
		return;
	}
	
	if (iPounces[pouncer] > iMaxAttempts)
	{
		CPrintToChat(pouncer, "{orange}[{red}HUNTER{orange}]{olive} Maximum Limits Reached!%s", (GetUserAdmin(pouncer) == INVALID_ADMIN_ID) ? "" : " Type {olive}!bhp_reset{default} To Reset Your BHP!");
		return;
	}
	...
	if (iPenalty[pouncer] > 0)
	{
		...
		{
			CPrintToChat(pouncer, "{orange}[{red}HUNTER{orange}]{olive} Please Wait!");
		}
		return;
	}
	...
	EmitSoundToAll(HUNTER, pouncer, SNDCHAN_WEAPON, SNDLEVEL_SCREAMING);
	//ExplodeMain(fLandedPos);
	SetEntProp(pouncer, Prop_Send, "m_iHealth", 1, true);
	IgniteEntity(pouncer, 2.0);
	ShowParticle(fLandedPos, PARTICLE_SMOKE, 10.0);
	...
	iPounces[pouncer] += 1;
	CPrintToChat(pouncer, "{orange}[{red}HUNTER{orange}]{default} Attempts: {orange}%d{default} / {orange}%i", iPounces[pouncer], iMaxAttempts + 1);
	
	if (bShowDamage && bShowDistance)
	{
		CPrintToChatAll("{orange}[{red}HUNTER{orange}] {olive}%N{default} Brutally Pounced {olive}%N{default}! {orange}[%.2f Units / %i Dmg]", pouncer, pounced, GetVectorDistance(fPouncePos[pouncer], fLandedPos), iDmg);
	}
	else if (bShowDamage)
	{
	...
	{
		CPrintToChatAll("{orange}[{red}HUNTER{orange}] {olive}%N{default} Brutally Pounced {olive}%N! {orange}[%.2f Units]", pouncer, pounced, GetVectorDistance(fPouncePos[pouncer], fLandedPos));
	}
	else
	{
		CPrintToChatAll("{orange}[{red}HUNTER{orange}] {olive}%N{default} Brutally Pounced {olive}%N{default}!", pouncer, pounced);
	}
...
public Action ShowNextBP(Handle timer, any client)
{
	...
	{
		CPrintToChat(client, "{orange}[{red}HUNTER{orange}]{default} You Can Brutal Pounce Again!");
...
void BHPStats(int client, int iKill = 0, int iDamage = 0, int iDistance = 0, bool bUpdate = false, int iPage = 1)
{
	...
	if (!bUpdate)
	{
		...
		{
			CPrintToChat(client, "{orange}[{red}HUNTER{orange}]{default} No Saved Data!");
			delete kvStats;

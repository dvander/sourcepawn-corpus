...
new        Float:g_fSpawnersMaxRockets    [MAX_SPAWNER_CLASSES];
new        Float:g_fSpawnersMaxRocketsPerPlayer    [MAX_SPAWNER_CLASSES];
...
public Action:OnDodgeBallGameFrame(Handle:hTimer, any:Data)
{
    ...
    {
        if (g_bNextSpawnerTeam)
        {
            new iSpawnerEntity = g_iSpawnPointsRedEntity[g_iCurrentRedSpawn];
            new iSpawnerClass  = g_iSpawnPointsRedClass[g_iCurrentRedSpawn];
            if (g_iRocketCount < g_fSpawnersMaxRockets[iSpawnerClass] + GetClientCount() * g_fSpawnersMaxRocketsPerPlayer[iSpawnerClass])
            {
        ...
        g_bNextSpawnerTeam = !g_bNextSpawnerTeam;
    }
...
{    
    ...
    if (iIndex != -1)
    {
        ...
        if (iEntity && IsValidEntity(iEntity))
        {
            ...
            g_iRocketCount++;
            g_iRocketsFired++;
            g_fNextSpawnTime = GetGameTime() + g_fSpawnersInterval[iSpawnerClass]/(g_fSpawnersMaxRockets[iSpawnerClass] + GetClientCount() * g_fSpawnersMaxRocketsPerPlayer[iSpawnerClass]);
        }
    }
}
...
ParseSpawners(Handle:kvConfig)
{
    ...
    do
    {
        ...
        g_fSpawnersMaxRockets[iIndex] = KvGetFloat(kvConfig, "max rockets", 1.0);
        g_fSpawnersMaxRocketsPerPlayer[iIndex] = KvGetFloat(kvConfig, "max rockets per player", 0.0);

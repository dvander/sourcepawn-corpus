...
new Handle:sm_cvote_minvotepercent = INVALID_HANDLE;
...
public OnPluginStart() {
	...
	sm_cvote_minvotepercent = CreateConVar("sm_cvote_minvotepercent", "60", "Minimum percentage of players that must participate in the vote.", FCVAR_PLUGIN);
	...
	//RegConsoleCmd("sm_cvote_testadmin", Command_AddAdmin);
...
public OnConfigsExecuted() {
	if (GetConVarBool(sm_cvote_triggers))
...
public CVote_AdminMenuHandler(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength) {
	...
	if (idx > -1) {
		...
		if (action == TopMenuAction_DisplayOption) {
			...
			if (strlen(selvote[admin]) == 0 || CheckCommandAccess(param, selvote[admin], ADMFLAG_ROOT))
...
public SMCResult:Config_KeyValue(Handle:parser, const String:key[], const String:value[], bool:key_quotes, bool:value_quotes) {
	switch (g_configLevel) {
		case 2: {
			...
			else if(strcmp(key, "votepercent", false) == 0)
				g_configVote[votepercent] = StringToInt(value);
...
public SMCResult:Config_EndSection(Handle:parser) {
	switch (g_configLevel) {
		case 2: {
			...
			new tidx = InArray(g_configVote[name], g_voteNames, sizeof(g_voteNames));
			if (tidx == -1) {
...
CVote_DoVote(client, const String:votename[], const String:vparams[][], vparamct, bool:fromtrigger=false) {
	...
	if (strlen(cvote[admin]) > 0 && !fromtrigger && !CheckCommandAccess(client, cvote[admin], ADMFLAG_ROOT)) {
...
public CVote_VoteHandler(Handle:menu, num_votes, num_clients, const client_info[][2], num_items, const item_info[][2]) {
	...
	new playervotepercent = RoundToFloor(FloatMul(FloatDiv(float(num_votes), float(num_clients)), float(100)));
	...
	else if (playervotepercent < cvote[votepercent]) 
	{
		
		PrintToChatAll("[SM] %T", "Not Enough Votes", LANG_SERVER, cvote[votepercent], playervotepercent); 
		}
		else 
		{
...
stock IsVoteAllowed(client, voteidx, bool:fromtrigger, String:errormsg[], msglen) {
	...
	if (IsVoteInProgress() || (g_activeVoteStatusIdx > -1 && g_activeVoteStatus[sender] != client)) {
	...
	vd = Max(vd, (g_voteLastInitiated[voteidx] + cvote[mapdelay]) - currtime);
...
stock PrintVotesToMenu(client) {
	...
	for (new i = 0; i < s; ++i) {
		...
		if (strlen(tvote[admin]) == 0 || CheckCommandAccess(client, tvote[admin], ADMFLAG_ROOT)) {
			ProcessTemplateString(votetitle, sizeof(votetitle), tvote[title]);
			ReplaceParams(votetitle, sizeof(votetitle), voteparams, voteparamct, voteparamtypes, true);
			AddMenuItem(menu, tvote[name], votetitle, ITEMDRAW_DEFAULT);
		}
	}
...
stock PrintVotesToConsole(client) {
	...
	for (new i = 0; i < s; ++i) {
		GetArrayArray(g_voteArray, i, tvote[0]);
		if (client == 0 || strlen(tvote[admin]) == 0 || CheckCommandAccess(client, tvote[admin], ADMFLAG_ROOT)) {
			ProcessTemplateString(votetitle, sizeof(votetitle), tvote[title]);
...
ResetVoteCache(cvote[CVote]) {
	...
	cvote[votepercent] = GetConVarInt(sm_cvote_minvotepercent);
...
AddPlayerItems(Handle:menu, client) {
	...
	for (new i = 0; i < vtargetct; ++i) {
		if (vtargets[i] > 0 && IsClientInGame(vtargets[i])) {
			if (IsFakeClient(vtargets[i]) || !IsClientAuthorized(vtargets[i])) {

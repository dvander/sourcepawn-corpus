...
new g_iHumanCount, g_iStatus, g_iZombieCount;
...
public OnPluginStart()
{
	...
	HookEvent("player_death",	ReCount,		EventHookMode_Post);
...
public OnPluginEnd()
{
	UnhookEvent("player_death",	ReCount,		EventHookMode_Post);
...
public Action:PrintInformation(Handle:timer, any:client)
{
	if(IsPlayerAlive(client))
	{
		switch(g_iStatus)
		{
			case 0:
			{
				PrintHintText(client, "Health: %i\nHumans: %i\nZombies: 0", GetClientHealth(client), g_iHumanCount + g_iZombieCount);
			}
			case 1:
			{

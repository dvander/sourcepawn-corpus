...
public Hook_RoundWin(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(!IsEntLimitReached())
	{
		if(g_announcementCvar)
		{
			...
			CPrintToChatAll("**{unique}La partie de cache-cache ... COMMENCE !{normal}**");
			CPrintToChatAll("{community}-------------------------------------------");
		}
...
public CreatePropPlayer(client)
{
	...
	PrintCenterText(client, "Vous etes un '%s' !", sName);
	if(g_thirdpersonCvar == 1)
	{
		CPrintToChat(client,"\x01Vous etes deguise en : ' {haunted}%s{normal} '  - Tapez {community}!third/!thirdperson{normal} dans le chat pour avoir une vue à la troisieme personne !", sName);
	}
	else
	{
		CPrintToChat(client,"\x01Vous etes deguise en : ' {haunted}%s{normal} ' - {unique}Courrez-vous cacher !", sName);
	}
}

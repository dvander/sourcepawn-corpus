#define PLUGIN_AUTHOR		"xZk, steeg"
#define PLUGIN_VERSION		"2.3.1"
#define PLUGIN_URL			"https://forums.alliedmods.net/showpost.php?p=2840807&postcount=74"
...
enum L4D2GlowType {L4D2Glow_None, L4D2Glow_OnUse, L4D2Glow_OnLookAt, L4D2Glow_Constant};

static const char g_sGunName[][] =
{
	"pistol"			   //0
	,"pistol_magnum"	   //1
	,"chainsaw"			   //2
...
static const char g_sMeleeModel[][] =
{
...
static const char g_sGrenadeName[][] =
{
	"pipe_bomb",
	"molotov",
	"vomitjar"
...
static const char g_sGrenadeModel[][] =
{
...
static const char g_sPackModel[][] =
{
...
static const char g_sConsumableName[][] =
{
	"pain_pills",
	"adrenaline"
};

static const char g_sConsumableModel[][] =
{
	"models/w_models/weapons/w_eq_painpills.mdl"
	,"models/w_models/weapons/w_eq_adrenaline.mdl"
};

static const char g_sCarryableName[][] =
{
	"fireworkcrate",
	"gascan",
	"oxygentank",
...
static const char g_sCarryableModel[][] =
{
...
{
	name = "[L4D/L4D2] Weapon Cleaner",
	author = PLUGIN_AUTHOR,
	description = "Cleans up spawned and dropped items on the ground after a delay timer, similar to Killing Floor.",
...
void GetCvars() {
	...
	g_bPhysics			 = cvarPhysics.BoolValue;
...
void LoadWhiteList(){
	...
	if(!FileExists(sPath))
	{
		...
		{
			for( int i = 0; i < sizeof(g_sGunName); i++ )
				hFile.SetNum(g_sGunName[i], 0);
			hFile.Rewind();
		...
		if(KvJumpToKey(hFile, "melees", true))
		{
			for( int i = 0; i < sizeof(g_sMeleeName); i++ )
				hFile.SetNum(g_sMeleeName[i], 0);
			hFile.Rewind();
			hFile.ExportToFile(sPath);
		}
		if(KvJumpToKey(hFile, "grenades", true))
		{
			for( int i = 0; i < sizeof(g_sGrenadeName); i++ )
				hFile.SetNum(g_sGrenadeName[i], 0);
			hFile.Rewind();
			hFile.ExportToFile(sPath);
		}
		if(KvJumpToKey(hFile, "packs", true))
		{
			for( int i = 0; i < sizeof(g_sPackName); i++ )
				hFile.SetNum(g_sPackName[i], 0);
			hFile.Rewind();
			hFile.ExportToFile(sPath);
		}
		if(KvJumpToKey(hFile, "consumables", true))
		{
			for( int i = 0; i < sizeof(g_sConsumableName); i++ )
				hFile.SetNum(g_sConsumableName[i], 0);
			hFile.Rewind();
			hFile.ExportToFile(sPath);
		}
		if(KvJumpToKey(hFile, "carryables", true))
		{
			for( int i = 0; i < sizeof(g_sCarryableName); i++ )
				hFile.SetNum(g_sCarryableName[i], 1);
			hFile.Rewind();
			hFile.ExportToFile(sPath);
		}
		BuildPath(Path_SM, sPath, sizeof(sPath), "%s", CFG_WHITELIST);
	...
	if( hFile.ImportFromFile(sPath) )
	{
		if(KvJumpToKey(hFile, "guns", true)){
			for( int i = 0; i < sizeof(g_sGunName); i++ ){
				if(hFile.GetNum(g_sGunName[i]) == 1)
					g_smWhiteList.SetValue(g_sGunName[i], true);
			}
			hFile.Rewind();
		}
		if(KvJumpToKey(hFile, "melees", true)){
			for( int i = 0; i < sizeof(g_sMeleeName); i++ ){
				if(hFile.GetNum(g_sMeleeName[i]) == 1)
					g_smWhiteList.SetValue(g_sMeleeName[i],true);
			}
			hFile.Rewind();
		}
		if(KvJumpToKey(hFile, "grenades", true)){
			for( int i = 0; i < sizeof(g_sGrenadeName); i++ ){
				if(hFile.GetNum(g_sGrenadeName[i]) == 1)
					g_smWhiteList.SetValue(g_sGrenadeName[i],true);
			}
			hFile.Rewind();
		}
		if(KvJumpToKey(hFile, "packs", true)){
			for( int i = 0; i < sizeof(g_sPackName); i++ ){
				if(hFile.GetNum(g_sPackName[i]) == 1)
					g_smWhiteList.SetValue(g_sPackName[i],true);
			}
			hFile.Rewind();
		}
		if(KvJumpToKey(hFile, "consumables", true)){
			for( int i = 0; i < sizeof(g_sConsumableName); i++ ){
				if(hFile.GetNum(g_sConsumableName[i]) == 1)
					g_smWhiteList.SetValue(g_sConsumableName[i],true);
			}
			hFile.Rewind();
		}
		if(KvJumpToKey(hFile, "carryables", true)){
			for( int i = 0; i < sizeof(g_sCarryableName); i++ ){
				if(hFile.GetNum(g_sCarryableName[i]) == 1)
					g_smWhiteList.SetValue(g_sCarryableName[i],true);
			}
		}
	}
	delete hFile;
...
public void OnEntityCreated(int entity, const char[] classname)
{
	...
	if (!g_iSpawn || g_bSpawnedWeapons) {
		if ( !g_bDrop && !strncmp(classname, "weapon_", 7) )
		{
			SDKHook(entity, SDKHook_SpawnPost, OnSpawnPost);
		}
		if ( g_bPhysics && ( !strcmp(classname, "physics_prop") || !strcmp(classname, "prop_physics") ) )
		{
			SDKHook(entity, SDKHook_SpawnPost, OnSpawnPost);
		}
	}
...
/**void OnWeaponCanUse(int client, int weapon) {
...
{
	return IsValidNonClientEntity(entity);
}
...
void GetItemName(int ent, char[] name, int size)
{
	...
	else if(IsWeaponSpawner(ent) || IsPropPhysic(ent))
	{
		...
		//guns
		for(int i; i < sizeof(g_sGunName); i++){
			if(i == 0 && strncmp(modelname, g_sGunModel[i], strlen(g_sGunModel[i]))==0){
				strcopy(name, size, g_sGunName[i]);
				return;
			}
			if(strcmp(modelname, g_sGunModel[i])==0){
				strcopy(name, size, g_sGunName[i]);
				return;
			}
		}
		...
		for(int i; i < sizeof(g_sMeleeName); i++){
			if(strcmp(modelname, g_sMeleeModel[i])==0){
				strcopy(name, size, g_sMeleeName[i]);
				return;
			}
		}
		//grenades
		for(int i; i < sizeof(g_sGrenadeName); i++){
			if(strcmp(modelname, g_sGrenadeModel[i])==0){
				strcopy(name, size, g_sGrenadeName[i]);
				return;
			}
		}
		//packs
		for(int i; i < sizeof(g_sPackName); i++){
			if(strcmp(modelname, g_sPackModel[i])==0){
				strcopy(name, size, g_sPackName[i]);
				return;
			}
		}
		//meds
		for(int i; i < sizeof(g_sConsumableName); i++){
			if(strcmp(modelname, g_sConsumableModel[i])==0){
				strcopy(name, size, g_sConsumableName[i]);
				return;
			}
		}
		//carries
		for(int i; i < sizeof(g_sCarryableName); i++){
			if(strcmp(modelname, g_sCarryableModel[i])==0){
				strcopy(name, size, g_sCarryableName[i]);
				return;
			}
		}
	}
...
bool IsWeaponPhysicsEnable(int weapon) {
	return (IsWeaponPhysic(weapon) && g_bPhysics);
}
...
stock bool IsScavengeGascan(int entity)
{
	...
	if(strcmp(sTempString, "models/props_junk/gascan001a.mdl")!=0)
		return false;
...
stock bool IsValidWeapon(int weapon){
	if ( IsValidNonClientEntity(weapon) ) {
		char class_name[64];
		GetEntityClassname(weapon,class_name,sizeof(class_name));
...
stock bool IsValidNonClientEntity(int entity)
{
	return MaxClients < entity && IsValidEntity(entity);
}

stock bool IsValidAnyEntity(int entity)
{
	return 0 < entity && IsValidEntity(entity);
}
...
stock void _debug(const char[] szFormat, int ...)
{
	...
	// Fix 'warning 203: symbol is never used: "szFormat"' in SM 1.12 compiler (1.11 is fine)
	if (szFormat[0] == '\0') {} // Or replace 'stock' with 'public' tag
	#endif

...
public OnEntityCreated(entity, const String:classname[]) {
	...
	if(strcmp(classname, "item_healthkit_medium", false) == 0) {
    	SDKHook(entity, SDKHook_Spawn, OnHealthKitSpawned);
    }
}
...
public Action:OnHealthKitSpawned(entity)
{
	new iOwner = GetEntPropEnt(entity, Prop_Data, "m_hOwnerEntity");

	if(IsValidClient(iOwner) && cake[iOwner])
	{
		if (cake[iOwner] == 2) DispatchKeyValue(entity, "powerup_model", MODEL_CAKE_LARGE);
		else DispatchKeyValue(entity, "powerup_model", MODEL_CAKE_MEDIUM);

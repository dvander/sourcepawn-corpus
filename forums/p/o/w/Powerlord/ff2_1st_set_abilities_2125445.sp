...
{
	new Float:timeScale=FF2_GetAbilityArgumentFloat(client, this_plugin_name, ability_name, 2, 0.1);
	...
	// Since the duration is affected by host_timescale, we need to do math here to make it so the real value is used
	...
	if(boss>0)
	{
		decl String:particleEffect[20];
		new team = FF2_GetBossTeam();
		if (team == _:TFTeam_Red)
		{
			strcopy(particleEffect, sizeof(particleEffect), "scout_dodge_red");
		}
		else
		{
			strcopy(particleEffect, sizeof(particleEffect), "scout_dodge_blue");
		}
		CreateTimer(duration, Timer_RemoveEntity, EntIndexToEntRef(AttachParticle(boss, particleEffect, 75.0)));
	}

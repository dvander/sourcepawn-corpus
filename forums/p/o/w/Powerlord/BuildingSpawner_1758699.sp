...
{
	LoadTranslations("BuildingSpawner.phrases");
...
{
	new Handle:menu = CreateMenu(MenuHandler1, MenuAction_Select|MenuAction_Cancel|MenuAction_End|MenuAction_Display|MenuAction_DisplayItem);
...
public MenuHandler1(Handle:menu, MenuAction:action, param1, param2)
{
	...
	{
		SetMenuTitle(menu, "%T", "Menu", param1);
	}
	else if (action == MenuAction_DisplayItem)
	{
		new String:info[32];
		GetMenuItem(menu, param2, info, sizeof(info));
		decl String:newText[128];
		
		if(StrEqual(info,"slvl1"))
		{
			Format(newText, sizeof(newText), "%T", "Sentry 1", param1);
			return RedrawMenuItem(newText);
		}
		else if(StrEqual(info,"slvl2"))
		{
			Format(newText, sizeof(newText), "%T", "Sentry 2", param1);
			return RedrawMenuItem(newText);
		}
		else if(StrEqual(info,"slvl3"))
		{
			Format(newText, sizeof(newText), "%T", "Sentry 3", param1);
			return RedrawMenuItem(newText);
		}
		else if(StrEqual(info,"dlvl1"))
		{
			Format(newText, sizeof(newText), "%T", "Dispenser 1", param1);
			return RedrawMenuItem(newText);
		}
		else if(StrEqual(info,"dlvl2"))
		{
			Format(newText, sizeof(newText), "%T", "Dispenser 2", param1);
			return RedrawMenuItem(newText);
		}
		else if(StrEqual(info,"dlvl3"))
		{
			Format(newText, sizeof(newText), "%T", "Dispenser 3", param1);
			return RedrawMenuItem(newText);
		}
	}
...
public Action:BuildSentry(iBuilder,Float:fOrigin[3], Float:fAngle[3],iLevel)
{
	...
	PrintToChatAll("[SM] %t", "Spawned Sentry", name, iLevel);
...
public Action:BuildDispenser(iBuilder, Float:flOrigin[3], Float:flAngles[3], iLevel)
{
	...
	decl String:name[60];
	GetClientName(iBuilder,name,sizeof(name));
	PrintToChatAll("[SM] %t", "Spawned Dispenser", name, iLevel);
	new iTeam = GetClientTeam(iBuilder);
	new iHealth;
	new iAmmo = 400;
	if(iLevel == 2)

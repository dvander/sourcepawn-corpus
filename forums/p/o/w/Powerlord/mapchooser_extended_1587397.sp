...
public OnPluginStart()
{
	...
	CreateConVar("sm_mce_version", VERSION, "MapChooser Extended Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
//	Call_Finish();
	...
	if (inputlist == INVALID_HANDLE)
	{
		...
		if (randomizeList != INVALID_HANDLE)
		{
			...
			for (new j = GetArraySize(randomizeList); j == 1; j--)
			{
...
GetMapItem(Handle:menu, position, String:map[], mapLen)
{
	if (g_BuiltinVotes)
	{
...
AddExtendToMenu(Handle:menu, MapChange:when)
{

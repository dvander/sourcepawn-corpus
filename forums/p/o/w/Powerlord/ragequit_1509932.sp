...
new g_LastDeathTime[MAXPLAYERS+1] = {0};
...
{
	g_LastDeathTime[client] = 0;
	return true;
}

public OnClientDisconnect(client)
{
	if (g_LastDeathTime[client] > 0 && (GetTime() - g_LastDeathTime[client]) > GetConVarInt(g_hCvarTime))
	{
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	g_LastDeathTime[iClient] = GetTime();
}
...
public OnLibraryRemoved(const String:name[])
{
	...
	else if (strcmp(name, "builtinvotes") == 0)
	{
...
public Action:Command_ReVote(client, args)
{
	...
	if (!IsVoteInProgress())
	{
		if (!g_BuiltinVotes || !IsBuiltinVoteInProgress())
		{
	...
	if ((!isBuiltinVote && !IsClientInVotePool(client)) || (isBuiltinVote && !IsClientInBuiltinVotePool(client)))
	{
		ReplyToCommand(client, "[SM] %t", "Cannot participate in vote");
		return Plugin_Handled;
	}
	
	if ((!isBuiltinVote && !RedrawClientVoteMenu(client)) || (isBuiltinVote && !RedrawClientBuiltinVote(client)))
	{

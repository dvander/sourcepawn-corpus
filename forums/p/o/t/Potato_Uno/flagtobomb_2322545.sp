...
public void OnPluginStart()
{
	...
	g_hMoveSpeedPenalty = CreateConVar("tf2_flagstobombs_move_speed_penalty", "0.5", "The value the bomb carrier receives for the 'move speed penalty' attribute.", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_DONTRECORD, true, 0.1, false);
	...
	if(client > 0 && client <= MaxClients && IsClientInGame(client) && !GameRules_GetProp("m_bPlayingRobotDestructionMode") && !GameRules_GetProp("m_bPlayingMannVsMachine"))
	{
		switch(eventtype)
		{
			case TF_FLAGEVENT_PICKEDUP:
			{
				...
				TF2Attrib_SetByName(client, "move speed penalty", GetConVarFloat(g_hMoveSpeedPenalty));

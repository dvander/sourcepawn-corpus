...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_hs", Command_Respawn, ADMFLAG_BAN, "sm_respawn <player1> [player2] ... [playerN] - respawn all listed players and teleport them where you aim");
...
public Action:Command_Respawn(client, args)
{
	...
	ShowActivity2(client, "\x04[Hệ Thống] ", "\x04Hồi sinh \x03'%s'", target_name);
...
static bool:SetTeleportEndPoint(client)
{
	...
	{
		PrintToChat(client, "\x04[Hệ Thống] \x03%s", "\x03Không thể dịch chuyển người chơi sau khi hồi sinh");
		CloseHandle(trace);
		return false;
	}
	CloseHandle(trace);

...
/* Plugin ConVars */
new Handle:g_Cvar_StartMethod = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_Cvar_StartMethod = CreateConVar("sm_mapvote_endvote_method", "1", "Which value (method) to start end of map vote. 1 = rounds remaining, 2 = frags remaining, 3 = time remaining", _, true, 1.0, true, 3.0);
...
public OnConfigsExecuted()
{
	...
	/* don't bother setting up a timer if we don't want to base
	 * end of map votes on time remaining */
	if (GetConVarInt(g_Cvar_StartMethod) == 3)
	{
		SetupTimeleftTimer();
	}
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	/* if we arent even supposed to be basing end of map votes on frag limit
	 * we dont need to be here...	*/
	if (GetConVarInt(g_Cvar_StartMethod) != 2)
	{
		return;
	}

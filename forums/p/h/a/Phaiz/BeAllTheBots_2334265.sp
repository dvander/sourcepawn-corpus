...
{
	name = "BeAllTheRobots",
	author = "MasterOfTheXP;Kittens, Sound fix by Phaiz",
...
stock bool:ToggleBuster(client, bool:toggle = bool:2)	//Sentry Buster All til next action
{
	...
	if (toggle == true || (toggle == bool:2 && Status2[client] == BusterStatus2_Human))
	{
		...
		// Sound tomfoolery
		EmitSoundToAll("mvm/sentrybuster/mvm_sentrybuster_intro.mp3", client);
		EmitSoundToAll("mvm/sentrybuster/mvm_sentrybuster_loop.mp3", client);
		...
		if ((LastBusterTime + 360.0) > GetTickedTime()) Format(AnnouncerSnd, sizeof(AnnouncerSnd), "vo/mvm_sentry_buster_alerts0%k.mp3", GetRandomInt(2,3));
		else
		{
			new rand = GetRandomInt(3,7);
			if (rand == 3) rand = 1;
			Format(AnnouncerSnd, sizeof(AnnouncerSnd), "vo/mvm_sentry_buster_alerts0%k.mp3", rand);
		}
		for (new k = 1; k <= MaxClients; k++)
		{
	...
	else if (!toggle || (toggle == bool:2 && Status2[client] == BusterStatus2_Buster))
	{
		...
		StopSound(client, SNDCHAN_AUTO, "mvm/sentrybuster/mvm_sentrybuster_loop.mp3");
...
public Action:SoundHook(clients[64], &numClients, String:sound[PLATFORM_MAX_PATH], &Ent, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	if (Status[client] == RobotStatus_Robot)
	{
		if (StrContains(sound, "player/footsteps/", false) != -1 && class != TFClass_Medic && GetConVarBool(cvarFootsteps))
		{
			...
			Format(sound, sizeof(sound), "mvm/player/footsteps/robostep_%s%i.mp3", (rand < 10) ? "0" : "", rand);
		...
		ReplaceString(sound, sizeof(sound), ".mp3", ".mp3", false);
...
{
	EmitSoundToAll("mvm/sentrybuster/mvm_sentrybuster_spin.mp3", client);
	StopSound(client, SNDCHAN_AUTO, "mvm/sentrybuster/mvm_sentrybuster_loop.mp3");
...
public Action:Bewm(Handle:timer, any:userid)
{
	...
	EmitSoundToAll("mvm/sentrybuster/mvm_sentrybuster_explode.mp3", client);
...
ComeOnPrecacheZeSounds()
{
	for (new i = 1; i <= 18; i++)
	{
		...
		Format(snd, sizeof(snd), "mvm/player/footsteps/robostep_%s%i.mp3", (i < 10) ? "0" : "", i);
		PrecacheSound(snd, true);
		if (i <= 4)
		{
			Format(snd, sizeof(snd), "mvm/sentrybuster/mvm_sentrybuster_step_0%i.mp3", i);
			PrecacheSound(snd, true);
		}
		if (i <= 6)
		{
	...
	PrecacheSound("mvm/sentrybuster/mvm_sentrybuster_explode.mp3", true);
	PrecacheSound("mvm/sentrybuster/mvm_sentrybuster_intro.mp3", true);
	PrecacheSound("mvm/sentrybuster/mvm_sentrybuster_loop.mp3", true);
	PrecacheSound("mvm/sentrybuster/mvm_sentrybuster_spin.mp3", true);

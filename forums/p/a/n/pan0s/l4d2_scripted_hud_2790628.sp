...
static char   g_sBuffer[127];
static char   g_sSpaces[127] = "                                                                                                                               ";
//////////////////// MVP Score var
static int      g_iMvp[4];
static float    g_fMvpScores[4];
...
public void OnPluginStart()
{
    ...
    g_hCvar_HUD1_Blink       = CreateConVar("l4d2_scripted_hud_hud1_blink", "1", "Makes the text blink from white to red.\n0 = OFF, 1 = ON.", CVAR_FLAGS, true, 0.0, true, 1.0);
    ...
    g_hCvar_HUD1_Y           = CreateConVar("l4d2_scripted_hud_hud1_y", "0.033", "Y (vertical) position of the text.\nNote: setting it to less than 0.0 may cut/hide the text at screen.", CVAR_FLAGS, true, -1.0, true, 1.0);
    ...
    g_hCvar_HUD3_Y           = CreateConVar("l4d2_scripted_hud_hud3_y", "0", "Y (vertical) position of the text.\nNote: setting it to less than 0.0 may cut/hide the text at screen.", CVAR_FLAGS, true, -1.0, true, 1.0);
...
{
    SRS_GetMvp(g_iMvp, g_fMvpScores);
...
{
    char time[32];
    FormatTime(time, sizeof(time), "%Y/%m/%d %a %T", GetTime());
    int humanNum = 0;
    for(int i =1; i<=MaxClients; i++) if(IsValidClient(i) && !IsFakeClient(i)) humanNum++;
    FormatEx(output, size, "%d/%d  %s", humanNum, 12, time);
    // FormatEx(output, size, "\0");
...
{
    Format(output, size, "#| Score\n");
    // Skip no.1, no.1 is shown on HUD 1
    for(int i=1; i<4; i++)
    {
        char line[127];
        GetScoreHudText(line, 127, i);
        Format(output, size, "%s\n%s",output, line);
    }
    /*
    FormatEx(output, size, "\0");
...
{
    char buffer[127];
    SRS_GetScoreStatus(buffer, sizeof(buffer));
    FormatEx(output, size, "%s", buffer);
...
////////////////////////////////////////////
//Align float to string with symbol
public void AddSymbol(char[] symbol, int num, char[] buffer, int size)
{
    for(int i=0; i<num; i++)
        Format(buffer, size, "%s%s", buffer, symbol);
}

public void AlignFloat(float value, char[] symbol, char[] buffer, int size)
{
    int num = 0;
    if(value >= 10000.0) num = 0;
    else if(value >= 1000.0) num = 1;
    else if(value >= 100.0) num = 0;
    else if(value >= 10.0) num = 2;
    else num = 4;
    AddSymbol(symbol, num, buffer, size);

    if(value >= 1000.0) Format(buffer, size, "%s%.0f", buffer, value);
    else Format(buffer, size, "%s%.1f", buffer, value);
}

public void GetScoreHudText(char[] buffer, int size, int rank)
{
    if(!IsValidClient(g_iMvp[rank])) return;

    char score[10];
    AlignFloat(g_fMvpScores[rank], " ", score, sizeof(score));
    FormatEx(buffer, size,"%d│%s  %N", rank+1, score, g_iMvp[rank]);
    // FormatEx(buffer, size,"%s  %N", rank+1, score, g_iMvp[rank]);
}
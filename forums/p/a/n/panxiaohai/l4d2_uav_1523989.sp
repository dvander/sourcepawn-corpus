...
public OnPluginStart()
{
	...
	l4d_uav_enable = CreateConVar("l4d_uav_enable", "1", "  0:disable, 1:enable in coop mode, 2: enable in all mode ", FCVAR_PLUGIN);
	l4d_uav_team = CreateConVar("l4d_uav_team", "2", "  1:enable for survivor and infected, 2:enable for survivor, 3:enable for infected ", FCVAR_PLUGIN);	
	l4d_uav_maxuser = CreateConVar("l4d_uav_maxuser", "2", "how many people can use it", FCVAR_PLUGIN);	
	l4d_uav_teleport = CreateConVar("l4d_uav_teleport", "1", "0:disable 1:enable teleport to teamate", FCVAR_PLUGIN);	
	
	l4d_uav_gundamage=CreateConVar("l4d_uav_gundamage", "15.0", "damage of machine gun", FCVAR_PLUGIN);	
...
Start(client)
{
	...
	new g_count=0;
	PrintToChat(client, "\x03Press \x04Reload \x03to change control mode");
	PrintToChat(client, "\x03Press \x04E \x03to quit, \x04ctrl \x03or \x04space \x03to control throttle");
	PrintHintText(client, "Press Reload to change control mode");

...
new Handle:w_point_tripmine ;
new Handle:w_point_interaction ;
new Handle:w_point_smg ;
new Handle:w_point_rifle ;
new Handle:w_point_hunting_rifle ;
new Handle:w_point_shotgun ;
new Handle:w_point_tnt ;
new Handle:w_point_flame ;
...
public OnPluginStart()
{
	...
	w_point_tripmine = CreateConVar("l4d_weapon_point_tripmine", "5", "point for tripmine", FCVAR_PLUGIN); 	
	w_point_interaction = CreateConVar("l4d_weapon_point_interaction", "3", "point for interaction", FCVAR_PLUGIN);
	w_point_smg = CreateConVar("l4d_weapon_point_smg", "3", "point for smg", FCVAR_PLUGIN);
	w_point_rifle = CreateConVar("l4d_weapon_point_rifle", "3", "point for rifle", FCVAR_PLUGIN);
	w_point_hunting_rifle = CreateConVar("l4d_weapon_point_hunting_rifle", "5", "point for hunting_rifle", FCVAR_PLUGIN);
	w_point_shotgun = CreateConVar("l4d_weapon_point_shotgun", "1", "point for shotgun", FCVAR_PLUGIN);
	w_point_tnt = CreateConVar("l4d_weapon_point_tnt", "5", "point for TNT", FCVAR_PLUGIN);
	w_point_flame = CreateConVar("l4d_weapon_point_flame", "5", "point for flame", FCVAR_PLUGIN);
	...
	w_start_point = CreateConVar("l4d_weapon_start_point", "10", "start point for every weapon", FCVAR_PLUGIN);
	...
	AutoExecConfig(true, "l4d_weapon_en_v25");
		...
		if(GetClientTeam(attacker) == 2)
		{
			if(IsClientInGame(victim))
			{
				if( GetClientTeam(victim) == 3 )
				{
					...
					if(headshot)
					{
						AddBullet(attacker, 3);
					}
					else
					{
						AddBullet(attacker, 1);
					}
				}
			}
		}
...
public Action:sm_tripmine(client, args)
{
	...
	{
		if(bullet_tripmine[client]-GetConVarInt(w_point_tripmine)<0)
		{
			PrintHintText(client, "need more point for tripmine");
			return Plugin_Handled;
		}
	...
	else if (tripmine_planted[client]==1)
	{
		...
		else
		{
			...
			bullet_tripmine[client]-=GetConVarInt(w_point_tripmine);
...
public Action:sm_plant(client, args)
{
	...
	if(tnt_planted[client]==0)
	{
		...
		if(bullet_tnt[client]-GetConVarInt(w_point_tnt)<0)
		{
			PrintHintText(client, "need more point for TNT");
			return Plugin_Handled;
		}
...
public Action:PlantTimer(Handle:timer, any:client)
{
	...
	
	{
 		...
 		if(tnt_optick[client]>GetConVarFloat(w_tnt_time))
		{
			...
			tnt_optick[client]=0.0;
			tnt_plant_timer[client]=INVALID_HANDLE;
			bullet_tnt[client]-=GetConVarInt(w_point_tnt);
...
public Action:sm_bullet(userid, args)
{
	...
	new pistol=1000;
	new point=0;
 	point=GetConVarInt(w_point_interaction);
	if(point>0)pistol=bullet_interaction[userid]/point;

	new tripmine=1000;
 	point=GetConVarInt(w_point_tripmine);
	if(point>0)tripmine=bullet_tripmine[userid]/point;
	
	new msg=1000;
 	point=GetConVarInt(w_point_smg); 
	if(point>0)msg=bullet_smg[userid]/point;
	
	new shotgun=1000;
 	point=GetConVarInt(w_point_shotgun);
	if(point>0)shotgun=bullet_shotgun[userid]/point;
	
	new rifle=1000;
 	point=GetConVarInt(w_point_rifle);
	if(point>0)rifle=bullet_rifle[userid]/point;				
	
	new hunting_rifle=1000;
 	point=GetConVarInt(w_point_hunting_rifle);
	if(point>0)hunting_rifle=bullet_hunting_rifle[userid]/point;
	
	new tnt=1000;
 	point=GetConVarInt(w_point_tnt);
	if(point>0)tnt=bullet_tnt[userid]/point;

	new flame=1000;
 	point=GetConVarInt(w_point_flame);
	if(point>0)flame=bullet_flame[userid]/point;

 	decl String:palyerName[64];
	GetClientName(userid, palyerName, sizeof(palyerName));
	PrintToChatAll("\x04%s\x03 :\x03interaction:\x04%d,\x03tripmine:\x04%d,\x03TNT:\x04%d, \x03flame:\x04%d,\x03laser gun for smg:\x04%d,\x03laser gun for rifle:\x04%d,\x03laser gun for sniper rifle:\x04%d,\x03generade:\x04%d",
...
public Action:weapon_fire(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(!IsFakeClient(userid) )
	{
		if(GetClientButtons(userid) & IN_USE)
		{
			...
			new point=GetConVarInt(w_point_shotgun);

			if( bullet_shotgun[userid]-point<0 )
			{
				pointleft=false;
			}
			...
			if( StrContains(item, "shotgun")>=0 ||  StrContains(item, "launcher")>=0)
			{
				...
				if(pointleft)
				{
					...
					new remain=-1;
					bullet_shotgun[userid]-=point;
					if(bullet_shotgun[userid]<0) 
						bullet_shotgun[userid]=0;
					if(point>0) 
						remain=bullet_shotgun[userid]/point;
					 
					if(remain!=-1)
					{
						PrintHintText(userid, "point remain: %d", remain);
					}
					else
					{
						PrintHintText(userid, "remain: inf");
				...
				{
					PrintHintText(userid, "not enough point");
				}				
...
public Action:bullet_impact(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(StrEqual(weapon, "weapon_hunting_rifle") || StrContains(weapon, "sniper")>=0 || StrContains(weapon, "magnum")>=0 )
	{
		...
		if(bullet_hunting_rifle[userid]-point<0)
		{
			pointleft=false;
		}
		count=false;
	...
	if(count && StrContains(weapon, "weapon_rifle")>=0)
	{
		...
		if(bullet_rifle[userid]-point<0)
		{
			pointleft=false;
		}
		count=false;
 	...
 	if(hunting_rifle || rifle || smg)
	{
		...
		if(pointleft)
		{
			...
			if(smg)
			{
				bullet_smg[userid]-=point;
				if(bullet_smg[userid]<0)bullet_smg[userid]=0;
				if(point>0)remain=bullet_smg[userid]/point;
			}
			else if(rifle)
			{
				bullet_rifle[userid]-=point;
				if(bullet_rifle[userid]<0)bullet_rifle[userid]=0;
				if(point>0)remain=bullet_rifle[userid]/point;				
			}
			else if(hunting_rifle)
			{
				bullet_hunting_rifle[userid]-=point;
				if(bullet_hunting_rifle[userid]<0)bullet_hunting_rifle[userid]=0;
				if(point>0)remain=bullet_hunting_rifle[userid]/point;
			}
 			if(remain!=-1)
			{
				PrintHintText(userid, "point remain: %d", remain);
...
{ 
	...
	new point=GetConVarInt(w_point_interaction);
	if(bullet_interaction[client]-point<0)
	{
		pointleft=false;
	}
 
	{
		...
		if(pointleft )
		{
			...
			GetEdictClassname(ent, edictname, 128);
			{
				...
				new remain=-1;
				bullet_interaction[client]-=point;
				if(bullet_interaction[client]<0)bullet_interaction[client]=0;
				if(point>0)remain=bullet_interaction[client]/point;
				if(remain!=-1)
				{
					PrintHintText(client, "grabed something , point remain: %d", remain);
				}
				else
				{
					PrintHintText(client, "grabed something , point remain: inf");
				}
		...
		{
			PrintHintText(client, "not enough point");
		}
	}
 	return Plugin_Handled;
}
...
{ 
	...
	point=GetConVarInt(w_point_flame);
	if(bullet_flame[client]-point<0)
	{
		pointleft=false;
	}
	decl String:weapon[32];
	...
	if(weaponok )
	{
		if(pointleft )
		{
			...
			WritePackCell(flamedata, GetConVarInt(w_flame_fire));
 			...
 			new remain=-1;
			bullet_flame[client]-=point;
			if(bullet_flame[client]<0)bullet_flame[client]=0;
			if(point>0)remain=bullet_flame[client]/point;
			if(remain!=-1)
			{
				PrintHintText(client, "point remain: %d", remain);
			}
			else
			{
				PrintHintText(client, "point remain: inf");
			}

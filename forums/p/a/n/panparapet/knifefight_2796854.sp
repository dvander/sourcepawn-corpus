...
#define KNIFEFIGHT_VERSION      "1.3.8-csgo |marcin_pl|beacon-fix| selekcjoner-chatfix"
#define MAX_FILE_LEN            80
#define SOUND_EXPLODE           "ambient/explosions/explode_8.wav"
#define SOUND_TIMER             "ambient/tones/floor1.wav"

#define MAX_WEAPONS             40  // max weapons
...
new String:ctitems[32][64];
new String:titems[32][64];
...
public OnPluginStart()
{
    ...
    g_Cvarrandomkill    = CreateConVar("sm_knifefight_randomkill",  "1", 
    ...
    g_Cvarlosereffects  = CreateConVar("sm_knifefight_losereffects",    "0", 
    ...
    g_Cvarstopmusic     = CreateConVar("sm_knifefight_stopmusic",   "1", 
        "Stop music when fight is over. Useful when used with gungame. 0 = Disabled");
    g_Cvardeclinesound  = CreateConVar("sm_knifefight_declinesound",    "POLSKISURF/fight/chicken.wav", 
    ...
    g_Cvarbeaconradius  = CreateConVar("sm_knifefight_beaconradius",    "800",  
        "Beacon radius.");
    g_Cvarfightsongs    = CreateConVar("sm_knifefight_fightsongs",  "POLSKISURF/fight/bijatyka.mp3", 
    ...
    g_Cvar_ShowWinner       = CreateConVar("sm_knifefight_showwinner",    "1",
...
SetupOffsets()
{
    g_iMyWeapons = FindSendPropInfo("CCSPlayer", "m_hMyWeapons");
    if (g_iMyWeapons == -1)
    {
        ...
        if (loser == ctid || loser == tid)
        {
            ...
            if ( (winner != loser) && (winner != 0) )
            {
                ...
                else if ( g_showWinner == 1 )
                {
				   ...
				   StrCat(winnername, sizeof(winnername), " \x04Wygrywa!!!");
                   PrintToChatAll(winnername);
                }
...
SendKnifeMenus(ct,t)
{
    ...
    if (IsFakeClient(ct) || g_fightPrefs[ct] == 1)
    {
	   ...
	   StrCat(ctname, sizeof(ctname), " \x04Walcze!!!");
	   PrintToChatAll(ctname);
        ctagree = 1;
    }
    else if (g_fightPrefs[ct] == -1)
    {
        ...
        //CHAT_SayText(0, ct, msg);
		StrCat(ctname, sizeof(ctname), " \x07Bardzo sie boje!");
	PrintToChatAll(ctname);
        ctagree = 0;
    ...
    if (IsFakeClient(t) || g_fightPrefs[t] == 1)
    {
	   ...
	   StrCat(tname, sizeof(tname), " \x04Walcze!!!");
    ...
    else if (g_fightPrefs[t] == -1)
    {
        ...
        //CHAT_SayText(0, t, msg);
		StrCat(tname, sizeof(tname), " \x07Bardzo sie boje!");
	PrintToChatAll(tname);
        tagree = 0;
        EmitSoundToAll(g_declinesound);
    }
...
public PanelHandler(Handle:menu, MenuAction:action, client, item)
{
    ...
    else if ( action == MenuAction_Cancel )
    {
        ...
        {
            //Format(msg, sizeof(msg), "%c[%cKnifeFight%c] %c%t",
              //  GREEN, YELLOW, GREEN, GREEN, "Restore fight menu");

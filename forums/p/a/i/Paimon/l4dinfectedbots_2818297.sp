...
#define PLUGIN_VERSION         "2.9.5 SIPool#2024.04.18"
#define DEBUG                  0
...
{
    name = "[L4D/L4D2] Infected Bots (Coop/Versus/Realism/Scavenge/Survival/Mutation) SIPool fork",
    author = "djromero (SkyDavid), MI 5, Harry Potter, 我是派蒙啊",
...
{
    g_bMapStarted = true;
    if (g_bSIPoolAvailable && !g_hSIPool)
        g_hSIPool = SIPool.Instance();
...
void evtPlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (SpawnTime <= 6.0) SpawnTime = 6.0;
        }
...
void Event_GotVomit(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (IsValidEntRef(entity))
    {
        ...
        delete RestoreColorTimer[client];
        RestoreColorTimer[client] = CreateTimer(20.0, Timer_RestoreColor, client);
    }
}
...
int BotTypeNeeded()
{
    ...
    if (((g_bFinaleStarted && g_ePluginSettings.m_bTankSpawnFinal) || !g_bFinaleStarted) && g_iSpawnCounts[SI_TANK] < g_ePluginSettings.m_iTankLimit && GetRandomInt(1, 100) <= g_ePluginSettings.m_iTankSpawnProbability)
    {
        return 7;
    }
...
Action Timer_Spawn_InfectedBot(Handle timer, int index)
{
    ...
    if (L4D_HasPlayerControlledZombies())
    {
        ...
        if (InfectedRealCount + InfectedRealQueue + InfectedBotCount >= g_ePluginSettings.m_iMaxSpecials || AllPlayerCount >= L4D_MAXPLAYERS)
        {
#if DEBUG
    ...
    else
    {
        ...
        if (InfectedRealCount + InfectedBotCount >= g_ePluginSettings.m_iMaxSpecials || AllPlayerCount >= L4D_MAXPLAYERS)
        {
#if DEBUG
            LogMessage("team is already full, don't spawn a bot");
#endif
    ...
    if (anyclient == 0)
    {
        ...
        CreateTimer(1.0, CheckIfBotsNeededLater, g_bInitialSpawn ? 2 : g_bIsCoordination ? 1
                                                                                         : 0,
    ...
    if (human > 0)
    {
        if (!g_bSIPoolAvailable || bot_type == 7)
        {
            switch (bot_type)
            {
                case 0:    // Nothing
        ...
        else if (L4D_GetRandomPZSpawnPosition(anyclient, bot_type, ZOMBIESPAWN_Attempts, vecPos))
        {
            bot = g_hSIPool.RequestSIBot(bot_type, vecPos);
            if (IsValidClient(bot))
            {
                bSpawnSuccessful = true;
            }
        }
        else bSpawnSuccessful = false;

        if (IsPlayerAlive(human))
        {
            bSpawnSuccessful = true;
            if (bot_type != 7 && g_ePluginSettings.m_bCoopVersusHumanGhost == true)
            {
...
Action kickbot(Handle timer, int userid)
{
    ...
    {
        if (IsPlayerTank(client)) KickClient(client);
        else g_hSIPool.ReturnSIBot(client);
        return Plugin_Continue;
    }
...
{
    if (!g_bL4D2Version || !client || !IsClientInGame(client) || GetClientTeam(client) != TEAM_SURVIVOR || !IsPlayerAlive(client) || IsValidEntRef(g_iModelIndex[client]) == true || L4D_HasPlayerControlledZombies() || g_ePluginSettings.m_bCoopVersusEnable == false || bDisableSurvivorModelGlow == true || g_bMapStarted == false) return;
...
void PrepL4D2CreateBotCalls()
{
    ...
    {
        SetFailState("Unable to find %s signature in gamedata file.", NAME_CreateSpitter);
        return;
    }
    PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
    PrepSDKCall_SetReturnInfo(SDKType_CBasePlayer, SDKPass_Pointer);
    ...
    if (hCreateSpitter == null)
    {
        SetFailState("Cannot initialize %s SDKCall, signature is broken.", NAME_CreateSpitter);
        return;
    }
    ...
    {
        SetFailState("Unable to find %s signature in gamedata file.", NAME_CreateJockey);
        return;
    }
    PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
    PrepSDKCall_SetReturnInfo(SDKType_CBasePlayer, SDKPass_Pointer);
    ...
    if (hCreateJockey == null)
    {
        SetFailState("Cannot initialize %s SDKCall, signature is broken.", NAME_CreateJockey);
        return;
    }
    ...
    {
        SetFailState("Unable to find %s signature in gamedata file.", NAME_CreateCharger);
        return;
    }
    PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
    PrepSDKCall_SetReturnInfo(SDKType_CBasePlayer, SDKPass_Pointer);
    ...
    if (hCreateCharger == null)
    {
        SetFailState("Cannot initialize %s SDKCall, signature is broken.", NAME_CreateCharger);
        return;
    }
}
...
void PrepL4D1CreateBotCalls()
{
    ...
    if (bLinuxOS)
    {
        ...
        {
            SetFailState("Unable to find %s signature in gamedata file.", NAME_CreateSmoker);
            return;
        }
        PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
        PrepSDKCall_SetReturnInfo(SDKType_CBasePlayer, SDKPass_Pointer);
        ...
        if (hCreateSmoker == null)
        {
            SetFailState("Cannot initialize %s SDKCall, signature is broken.", NAME_CreateSmoker);
            return;
        }
        ...
        {
            SetFailState("Unable to find %s signature in gamedata file.", NAME_CreateBoomer);
            return;
        }
        PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
        PrepSDKCall_SetReturnInfo(SDKType_CBasePlayer, SDKPass_Pointer);
        ...
        if (hCreateBoomer == null)
        {
            SetFailState("Cannot initialize %s SDKCall, signature is broken.", NAME_CreateBoomer);
            return;
        }
        ...
        {
            SetFailState("Unable to find %s signature in gamedata file.", NAME_CreateHunter);
            return;
        }
        PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
        PrepSDKCall_SetReturnInfo(SDKType_CBasePlayer, SDKPass_Pointer);
        ...
        if (hCreateHunter == null)
        {
            SetFailState("Cannot initialize %s SDKCall, signature is broken.", NAME_CreateHunter);
            return;
        }
        ...
        {
            SetFailState("Unable to find %s signature in gamedata file.", NAME_CreateTank);
            return;
        }
        PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
        PrepSDKCall_SetReturnInfo(SDKType_CBasePlayer, SDKPass_Pointer);
        ...
        if (hCreateTank == null)
        {
            SetFailState("Cannot initialize %s SDKCall, signature is broken.", NAME_CreateTank);
            return;
        }
    }
    else
    {
        ...
        {
            SetFailState("Unable to find %s signature in gamedata file.", NAME_CreateSmoker_L4D1);
            return;
        }
        PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
        PrepSDKCall_SetReturnInfo(SDKType_CBasePlayer, SDKPass_Pointer);
        ...
        if (hCreateSmoker == null)
        {
            SetFailState("Cannot initialize %s SDKCall, signature is broken.", NAME_CreateSmoker_L4D1);
            return;
        }
        ...
        if (!PrepSDKCall_SetAddress(addr))
        {
            SetFailState("Unable to find %s signature in gamedata file.", NAME_CreateBoomer_L4D1);
            return;
        }
        PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
        PrepSDKCall_SetReturnInfo(SDKType_CBasePlayer, SDKPass_Pointer);
        ...
        if (hCreateSmoker == null)
        {
            SetFailState("Cannot initialize %s SDKCall, signature is broken.", NAME_CreateBoomer_L4D1);
            return;
        }
        ...
        if (!PrepSDKCall_SetAddress(addr))
        {
            SetFailState("Unable to find %s signature in gamedata file.", NAME_CreateHunter_L4D1);
            return;
        }
        PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
        PrepSDKCall_SetReturnInfo(SDKType_CBasePlayer, SDKPass_Pointer);
        ...
        if (hCreateHunter == null)
        {
            SetFailState("Cannot initialize %s SDKCall, signature is broken.", NAME_CreateHunter_L4D1);
            return;
        }
        ...
        if (!PrepSDKCall_SetAddress(addr))
        {
            SetFailState("Unable to find %s signature in gamedata file.", NAME_CreateTank_L4D1);
            return;
        }
        PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
        PrepSDKCall_SetReturnInfo(SDKType_CBasePlayer, SDKPass_Pointer);
        ...
        if (hCreateTank == null)
        {
            SetFailState("Cannot initialize %s SDKCall, signature is broken.", NAME_CreateTank_L4D1);
            return;
        }
    }
}

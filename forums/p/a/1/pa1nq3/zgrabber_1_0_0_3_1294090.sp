...
public OnPluginStart() 
{
	...
	cvDistance = CreateConVar("sm_grabber_distance", "250.0");
	...
	cvThrowTime = CreateConVar("sm_grabber_throwtime", "5.0", "time to charge up to full throw speed");
	cvThrowSpeed = CreateConVar("sm_grabber_throwspeed", "6000.0", "speed at which an object is thrown");
	cvMaxDistance = CreateConVar("sm_grabber_maxdistance", "20000.0", "maximum distance from which you can grab an object");
	...
	RegConsoleCmd("invprev", Command_MoveFar);
	RegConsoleCmd("invnext", Command_MoveNear);
}
...
{  
	...
	new AdminId:adminauth = GetUserAdmin(client);

	if(adminauth != INVALID_ADMIN_ID){
		
		if(!GetAdminFlag(adminauth, Admin_Chat)){
	...
	*/
	...
	GetEdictClassname(ent, classname, 128);
	
	if(StrEqual(classname, "player", false) || StrEqual(classname, "prop_physics", false) || StrEqual(classname, "prop_physics_multiplayer", false)
		 || StrEqual(classname, "prop_physics_override", false) || StrEqual(classname, "prop_physics_respawnable", false) || StrEqual(classname, "prop_physics_respawnable", false)
		 || StrEqual(classname, "func_physbox", false) || StrEqual(classname, "func_physbox_multiplayer", false)){

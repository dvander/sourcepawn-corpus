...
/*TIMERS*/
	CreateTimer(0.1, PrimaryAmmoLock, _, TIMER_REPEAT); //PRIMARY AMMO LOCK
}
...
public Action:ME_Notify_Client(Handle:Timer, any:client)
{
	if(IsSurvivor(client))
	{
		...
		if (GetConVarInt(l4d_me_custom_notify) == 1 && !IsFakeClient(client))
			PrintToChat(client, "%s", ME_Msg);
		if (GetConVarInt(l4d_me_custom_notify) == 2 && !IsFakeClient(client))
...
public Action:PrimaryAmmoLock(Handle:Timer)
{
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		if (IsValidEntity(Weapon))
		{
			...
			if (AmmoType != -1)
			{
				...
				if (Clip == 1 && Ammo < 1) //LOCK GUN CLIPS
...
Process(client, Float:time, button, bool:isSwitch, currentWeapon=0)
{
	...
	{
		if(!L4D2Version && slot==1)
			continue;

		if(GetConVarInt(l4d_me_slot[slot])==0)
			continue;
...
CreateItemAttach(client, String:classname[], slot)
{
	...
	switch(slot)
	{
		case 0: //Slot0
		{
			...
			if(L4D2Version)
				SetVector(Pos, 2.0, 0.0, -7.0), SetVector(Ang, -22.0, 100.0, 180.0);
			else
...
GetItemClass(ent, String:classname[LEN64])
{
	...
	if(ent > 0)
	{
		...
		if(StrEqual(classname, "weapon_melee"))
		{
			...
			if(StrContains(model, "v_bat")>=0)classname="weapon_melee_baseball_bat";
...
CreateWeaponEnt(String:classname[])
{
	...
	else
	{
		...
		if(StrEqual(classname, "weapon_melee_baseball_bat"))DispatchKeyValue( ent, "melee_script_name", "baseball_bat");
		...
		else if(StrEqual(classname, "weapon_melee_fireaxe"))DispatchKeyValue( ent, "melee_script_name", "fireaxe");
...
stock bool:IsFakeAliveSurvivor(client)
{
	return client > 0 && client <= MaxClients && IsClientInGame(client) && IsFakeClient(client) && IsPlayerAlive(client) && GetClientTeam(client) == 2;
}

stock bool:IsNotFakeAliveSurvivor(client)
{
	return client > 0 && client <= MaxClients && IsClientInGame(client) && !IsFakeClient(client) && IsPlayerAlive(client) && GetClientTeam(client) == 2;
}

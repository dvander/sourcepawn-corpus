...
{
	name = "[L4D & L4D2] Anti-Runner System",
...
public OnPluginStart()
{
	...
	LoadTranslations("l4d_anti-runner.phrases");
	RegAdminCmd("sm_key", Command_Key, ADMFLAG_ROOT);
	...
	AutoExecConfig(true, "l4d_anti-runner");
	CreateConVar("sm_ar_version", PLUGIN_VERSION, "[L4D & L4D2] Anti-Runner System", CVAR_FLAGS);
...
public OnAdminMenuReady(Handle:topmenu)
{
	...
	{
		AddToTopMenu(hAdminMenu, "sm_ars_ed_menu", TopMenuObject_Item, AdminMenu_EnableDisable, menu_category, "sm_ars_ed_menu", ADMFLAG_ROOT);
		AddToTopMenu(hAdminMenu, "sm_ars_ck_menu", TopMenuObject_Item, AdminMenu_ChangeKey, menu_category, "sm_ars_ck_menu", ADMFLAG_ROOT);
	}
}
...
public OnMapStart()
{
	...
	{
		ServerCommand("sm plugins unload l4d_anti-runner");
	}
	...
	if (StrContains(mapname, "l4d_smalltown03_ranchhouse", false) != -1)
	{
		ServerCommand("sm plugins unload l4d_anti-runner");
	}

	if (StrContains(mapname, "l4d_smalltown04_mainstreet", false) != -1)
	{
		ServerCommand("sm plugins unload l4d_anti-runner");
	}

	if (StrContains(mapname, "l4d_smalltown05_houseboat", false) != -1)
	{
		ServerCommand("sm plugins unload l4d_anti-runner");
	}

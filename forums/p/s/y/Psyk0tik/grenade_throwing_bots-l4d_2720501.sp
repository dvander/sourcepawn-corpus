...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	if (IsSurvivor(client) && IsFakeClient(client))
	{
		...
		if (iActiveWeapon != -1 && IsValidEntity(iActiveWeapon))
		{
			...
			if (iActiveWeapon == GetPlayerWeaponSlot(client, 2) && (StrEqual(sActiveWeapon, "weapon_molotov") || StrEqual(sActiveWeapon, "weapon_pipe_bomb")))
			{
...
public Action OnPlayerHurt(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (dangerousEnt >= 15 && ChangeToGrenade(damaged, true, true))
	{
		bChill[1] = true;
...
public Action L4D_OnSpawnMob(int &amount)
{
	...
	if (fMobPos[0] != 0.0 || fMobPos[1] != 0.0 || fMobPos[2] != 0.0)
	{
		for (int i = 1; i <= MaxClients; i++)
		{
			...
			{
				if (!ChangeToGrenade(i, true, true))
				{
					continue;
				}

...
new numSets = 0;
new String:setsName[MAX_NUM_SETS][MAX_FILE_LEN];
...
public LoadSounds()
{
	...
	{
		Format(buffer, MAX_FILE_LEN, "sound set %i", i);
		KvGetString(kvQSL, buffer, setsName[numSets], MAX_FILE_LEN);
				...
				do
				{
					...
					{						
						...
						if((tempConfig & 1) || (tempConfig & 2) || (tempConfig & 4))
						{
							...
							{							
								KvGetString(kvQSL, setsName[set], soundsFiles[numSounds], MAX_FILE_LEN);
...
{
	decl String:attackerName[MAX_FILE_LEN];
	decl String:victimName[MAX_FILE_LEN];
	
	// Get the names of the victim and the attacker
	if(attackerClient && IsClientInGame(attackerClient))
	{
		GetClientName(attackerClient, attackerName, MAX_FILE_LEN);
	}
	else
	{
		attackerName = "Nobody";
	...
	{
		GetClientName(victimClient, victimName, MAX_FILE_LEN);
	}
	else
	{
		victimName = "Nobody";

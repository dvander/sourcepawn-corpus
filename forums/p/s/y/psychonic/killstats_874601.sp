...
Versions:
1.2-unofficial: Added clientprefs support. Standalone sqlite db no longer used
...
#define PLUGIN_VERSION "1.2-unofficial"
...
{
	name = "KillStats",
	author = "Deception5, modified by psychonic",
...
// Client Cookie for storage and lookup of preference
new Handle:displayPref = INVALID_HANDLE;
new Handle:displayMenu = INVALID_HANDLE;
new Handle:displayChat = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	// Register preference cookie
	displayPref = RegClientCookie("Kill Stats Display Pref", "Display KillStats", CookieAccess_Protected);
	displayMenu = RegClientCookie("Kill Stats Menu Pref", "Display KillStats Menus", CookieAccess_Protected);
	displayChat = RegClientCookie("Kill Stats Chat Pref", "Display KillStats Chat", CookieAccess_Protected);
	
	SetCookieMenuItem(displayPrefMenu, 0, "KillStats Options");
...
*****************************************************/

// When a new client's prefs are loaded, we reset stats preferences
// and let them know how to turn the stats on and off
public OnClientCookiesCached(client)
{
	if ( killstats_info )
	{	
		LogMessage("OnClientCookiesCached");
	}
...
public InitializeClient( client )
{
	...
	if ( ( client > 0 ) && ( client <= MAXPLAYERS ) ) 
	{
		...
		{
			decl String:cookieEnabled[2];
			decl String:cookieMenu[2];
			decl String:cookieChat[2];
			
			GetClientCookie(client, displayPref, cookieEnabled, sizeof(cookieEnabled));
			GetClientCookie(client, displayMenu, cookieMenu, sizeof(cookieMenu));
			GetClientCookie(client, displayChat, cookieChat, sizeof(cookieChat));
			
			StatPreference[STAT_PREF_ENABLED][client]=StringToInt(cookieEnabled);
			StatPreference[STAT_PREF_SHOW_MENU][client]=StringToInt(cookieMenu);
			StatPreference[STAT_PREF_SHOW_CHAT][client]=StringToInt(cookieChat);
...
public displayPrefMenu(client, CookieMenuAction:action, any:info, String:buffer[], maxlen)
{
	if (action == CookieMenuAction_SelectOption)
	{
		if ( killstats_info )
		{	
			LogMessage("SettingsPanelKillStatsOptions");
		}
...
// Updates client's cookie
public SavePrefs(client)
{
	if ( killstats_info )
	{	
		LogMessage("SavePrefs");
	}
	
	if(IsClientInGame(client) && !IsFakeClient(client) ) 
	{
		decl String:prefValue[2];
		decl String:menuValue[2];
		decl String:chatValue[2];
		IntToString(StatPreference[STAT_PREF_ENABLED][client], prefValue, sizeof(prefValue));
		IntToString(StatPreference[STAT_PREF_SHOW_MENU][client], menuValue, sizeof(menuValue));
		IntToString(StatPreference[STAT_PREF_SHOW_CHAT][client], chatValue, sizeof(chatValue));
		SetClientCookie(client, displayPref, prefValue);
		SetClientCookie(client, displayMenu, menuValue);
		SetClientCookie(client, displayChat, chatValue);
	}
}

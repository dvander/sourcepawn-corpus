...
{
	if ((offsAmmo = FindSendPropInfo("CTFPlayer", "m_iAmmo")) == -1)
	{
		SetFailState("Failed to find m_iAmmo");
	}
	if ((offsFlags = FindSendPropInfo("CTFPlayer", "m_fFlags")) == -1)
	{
		SetFailState("Failed to find m_fFlags");
	}
	if ((offsActiveWpn = FindSendPropInfo("CTFPlayer", "m_hActiveWeapon")) == -1)
	{
		SetFailState("Failed to find m_hActiveWeapon");
	}
	if ((offsMaxSpeed = FindSendPropInfo("CTFPlayer", "m_flMaxspeed")) == -1)
	{
		SetFailState("Failed to find m_flMaxSpeed");
	}
	if ((offsPlyrCond = FindSendPropInfo("CTFPlayer", "m_nPlayerCond")) == -1)
	{
		SetFailState("Failed to find m_flNextPrimaryAttack");
	}
	if ((offsNextPrimaryAttack = FindSendPropInfo("CBaseCombatWeapon", "m_flNextPrimaryAttack")) == -1)
	{
		SetFailState("Failed to find m_flNextPrimaryAttack");
	}
	if ((offsItemIdx = FindSendPropInfo("CBaseCombatWeapon", "m_iItemDefinitionIndex")) == -1)
	{
		SetFailState("Failed to find m_iItemDefinitionIndex");
	}

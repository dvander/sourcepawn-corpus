...
#define PLUGIN_VERSION "1.0 alpha 1"
...
public DoRTD(client)
{
	...
	PrintToChatAll("%s%N rolled \"%s\"  %s", MSGTAG, client, szEffectName, (type == psyRTDEffectType_Bad)?"D:":":D");
...
bool:ClientCanRTD(client)
{
	...
	switch(g_psyWaitMode)
	{
		case WaitMode_All:
		{
			...
			if (g_fWaitTime > (fTime - g_fLastRTD))
			{
		...
		case WaitMode_Player:
		{
			...
			if (g_fWaitTime > (fTime - g_fLastClientRTD[client]))
			{
				...
				{
					g_fLastClientRTD[client] = GetEngineTime();
					PrintToChat(client, "%sYour rtd timer was reset for being greedy.", MSGTAG, RoundFloat(g_fWaitTime));
				}

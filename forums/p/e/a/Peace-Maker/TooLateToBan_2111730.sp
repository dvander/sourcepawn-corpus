...
/* Was the player list menu openend from the admin menu? */
new bool:bMenuFromAdminMenu;
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{
						bMenuFromAdminMenu = false;
						DisplayMenu(hTltbMenu, client, MENU_TIME_FOREVER);
					}
				}
			}
...
{
	/* Menu handle */
	new Handle:menu = CreateMenu(TltbMenuHandler, MENU_ACTIONS_DEFAULT|MenuAction_Start);
...
public TltbMenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	{
		/* Only display the back button if the menu was opened from the admin menu */
		SetMenuExitBackButton(menu, bMenuFromAdminMenu);
	}
	/* If the menu is cancelled and no option was selected */
	else if(action == MenuAction_Cancel)
	{
		if(param2 == MenuCancel_ExitBack)
		{
			RedisplayAdminMenu(hTltbAdminMenu, param1);
		}
	}
...
public TltbMenuTimeHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if(action == MenuAction_Cancel)
	{
		if(param2 == MenuCancel_ExitBack)
		{
			DisplayMenu(hTltbMenu, param1, MENU_TIME_FOREVER);
		}
	}
	/* If a menu entry has been selected */
	else if (action == MenuAction_Select)
...
public TltbMenuReasonHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if(action == MenuAction_Cancel)
	{
		if(param2 == MenuCancel_ExitBack)
		{
			/* Remove time from ban string */
			new index = StrContains(tltbBanString, " ");
			if(index != -1)
				strcopy(tltbBanString, sizeof(tltbBanString), tltbBanString[++index]);
	...
	else if (action == TopMenuAction_SelectOption)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					bMenuFromAdminMenu = true;
					DisplayMenu(hTltbMenu, param, MENU_TIME_FOREVER);
				}
			}
		}
	}

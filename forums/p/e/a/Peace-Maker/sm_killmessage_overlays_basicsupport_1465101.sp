...
#define PLUGIN_VERSION "1.2fixed_ct_t"
...
new Handle:_hKillOverlayNamingT;
new Handle:_hKillOverlayNamingCT;
...
new String:_sKillOverlayNamingT[32];
new String:_sKillOverlayNamingCT[32];
new _iMaxKillStreak;
...
public OnPluginStart()
{
	...
	_hKillOverlayNamingT = CreateConVar("sm_killmessage_bs_killnaming_t", "killsilver_", "Naming convention for the kill overlays for terrorists (numbers are added automatically,no extension)");
	_hKillOverlayNamingCT = CreateConVar("sm_killmessage_bs_killnaming_ct", "killsilver_", "Naming convention for the kill overlays for counter-terrorists (numbers are added automatically,no extension)");
...
public OnConfigsExecuted()
{
	...
	GetConVarString(_hKillOverlayNamingT, _sKillOverlayNamingT, sizeof(_sKillOverlayNamingT));
	GetConVarString(_hKillOverlayNamingCT, _sKillOverlayNamingCT, sizeof(_sKillOverlayNamingCT));
	...
	for (iCounter = 1; iCounter <= _iMaxKillStreak; iCounter++)
	{
		...
		ServerCommand("sm_killmessage_prepare %s%s",_sKillOverlayNamingT,sCounterTemp);
		ServerCommand("sm_killmessage_prepare %s%s",_sKillOverlayNamingCT,sCounterTemp);
	}	
}
		...
		if(!IsFakeClient(attacker))
		{
			if(victim != attacker)
			{
				...
				else
				{
					...
					if(iTeam == 2)
						ServerCommand("sm_killmessage_show %s %s%s",sAttackerID,_sKillOverlayNamingT,sKillStreakTemp);
					else if(iTeam == 3)
						ServerCommand("sm_killmessage_show %s %s%s",sAttackerID,_sKillOverlayNamingCT,sKillStreakTemp);
				}
			}
		}
	}

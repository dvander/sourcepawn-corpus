...
#define	PLUGIN_VERSION "1.1.2-ph"
...
void CreateMenu1(int client) {
	...
	AddMenuItem(menu, "1", "Professional");
		...
		case MenuAction_Select: {
			switch (item) {
				case 0: {
					...
					PrintToChat(client, "[Sheen] %t", "Disabled killstreak");
				}
				case 1: {
...
int Menu2(Menu menu, MenuAction action, int client, int item) {
	switch (action) {
		case MenuAction_Select:
		{
			...
			PrintToChat(client, "[Sheen] %t", "Set sheen to x", sheens[item]);
		}
		case MenuAction_End:delete menu;
	}
	return 0;
...
int Menu3(Menu menu, MenuAction action, int client, int item) {
	switch (action) {
		case MenuAction_Select: {
			...
			PrintToChat(client, "[Sheen] %t", "Set sheen to x and effect to y", sheen, effects[item]);
		}
		case MenuAction_End:delete menu;
	}
} 
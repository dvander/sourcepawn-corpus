...
public OnClientCookiesCached(client)
{
    ...
    const char *classPacks[] = {"scout_packs", "soldier_packs", "pyro_packs", "demoman_packs",
                                "heavy_packs", "engineer_packs", "medic_packs", "sniper_packs", "spy_packs"};
    Handle cookies[] = {cookie_ScoutPack, cookie_SoldierPack, cookie_PyroPack, cookie_DemoPack, 
                        cookie_HeavyPack, cookie_EngiePack, cookie_MedicPack, cookie_SniperPack, cookie_SpyPack};
    int *packs[] = {scoutPack, soldierPack, pyroPack, demoPack, heavyPack, engiePack, medicPack, sniperPack, spyPack};

    for (int i = 0; i < sizeof(classPacks); i++) {
        if (config.JumpToKey(classPacks[i])) {
            if (config.GotoFirstSubKey()) {
                do {
                    char teamRestriction[16];
                    config.GetString("team", teamRestriction, sizeof(teamRestriction), "both");

                    char defaultSetting[16];
                    config.GetString("default", defaultSetting, sizeof(defaultSetting), "");

                    bool isDefaultForTeam = ((team == TFTeam_Red && StrEqual(defaultSetting, "Red")) ||
                                             (team == TFTeam_Blue && StrEqual(defaultSetting, "Blue")) ||
                                             StrEqual(defaultSetting, "both"));

                    if (isDefaultForTeam) 
                    {
                        char packID[16];
                        config.GetSectionName(packID, sizeof(packID));
                        SetClientCookie(client, cookies[i], packID);
                        packs[i][client] = StringToInt(packID);
                        break;
                    }
    ...
    // Обновление каждого класса для загрузки текущего установленного пака из куков
    char scoutVal[16];
...
Menu BuildMenu(int mode, int client)
{
    if (config == null)
    {
        return null;
    ...
    if (config.JumpToKey(KV_PackName))
    {
        ...
        if (config.GotoFirstSubKey())
        {
            do
            {
                ...
                // Проверяем, для какой команды доступен войспак
                char teamRestriction[16];
                config.GetString("team", teamRestriction, sizeof(teamRestriction), "both");

                bool isPackAllowedForTeam = ((team == TFTeam_Red && StrEqual(teamRestriction, "Red")) ||
                                             (team == TFTeam_Blue && StrEqual(teamRestriction, "Blue")) ||
                                             StrEqual(teamRestriction, "both"));

                if (!StrEqual(packName, "null_value", true) && isPackAllowedForTeam)
                {

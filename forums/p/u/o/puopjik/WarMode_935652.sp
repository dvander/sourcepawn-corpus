...
#define PLUGIN_VERSION "2.1.1bis"
...
public OnPluginStart()
{
	...
	RegAdminCmd("war_islive", sv_WarIsLive, ADMFLAG_SLAY, "Set to 1 to start the war, 0 to stop it.")
	RegAdminCmd("war_scores", sv_DispScores, ADMFLAG_SLAY, "Display current scores to all players.")
			
	RegAdminCmd("war_configfile", sv_WarConfigFile, ADMFLAG_SLAY, "Define the special war config to load.")
	RegAdminCmd("war_tvfile", sv_WarTvFile, ADMFLAG_SLAY, "Define the special SourceTV config to load.")
	RegAdminCmd("war_mapfile", sv_WarMapFile, ADMFLAG_SLAY, "Define the map file list to load.")
		
	RegAdminCmd("war_tagA", sv_WarTagA, ADMFLAG_SLAY, "Define the clan tag of the team A.")
	RegAdminCmd("war_tagB", sv_WarTagB, ADMFLAG_SLAY, "Define the clan tag of the team B.")
			
	RegAdminCmd("war_allies", sv_WarAllies, ADMFLAG_SLAY, "Define which team plays as allies for the first round.")	
	RegAdminCmd("war_axis", sv_WarAxis, ADMFLAG_SLAY,  "Define which team plays as axis for the first round.")	
		
	RegAdminCmd("war_map1", sv_WarMap1, ADMFLAG_SLAY, "Define the first map of the war.")
	RegAdminCmd("war_map2", sv_WarMap2, ADMFLAG_SLAY, "Define the second map of the war.")	
	RegAdminCmd("war_map3", sv_WarMap3, ADMFLAG_SLAY, "Define the third map of the war.")	
	
	RegAdminCmd("war_reset", sv_WarReset, ADMFLAG_SLAY, "Reset all settings to default, it also stops the war if it was running.")
		
	RegAdminCmd("war_rr", sv_WarRestartRound, ADMFLAG_SLAY, "Restart a round if needed.")
	RegAdminCmd("war_numberofmaps", sv_WarNumberOfMaps, ADMFLAG_SLAY, "Define the number of maps to play on.")
	RegAdminCmd("war_roundtime", sv_WarRoundTime, ADMFLAG_SLAY, "Define the duration of a round.")
	
	RegAdminCmd("war_menu", sv_WarMenu, ADMFLAG_SLAY, "Display the main plugin menu.")
	RegAdminCmd("war_help", sv_WarHelp, ADMFLAG_SLAY, "Display the plugin help.")
	RegAdminCmd("war_summary", sv_WarSummary, ADMFLAG_SLAY, "Display a war settings summary.")
...
public Action:sv_WarMenu(client,argc)
{
	if(client == 0)
	{
		PrintToServer("Menus can't be displayed to server!")
	}
	else
	{
...
public Action:sv_WarHelp(client,argc)
{
	if(client == 0)
	{
		PrintToServer("War Mode Plugin by Puopjik available commands:\n")
		...
		PrintToServer("\"war_mapfile\"\n - Define the map file list in use.")
	}
	else
	{
		PrintToConsole(client,"War Mode Plugin by Puopjik available commands:\n")
...
public Action:sv_WarReset(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("Match settings have been reset.")
	}
	else
	{
...
public Action:sv_WarMap1(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("Map1 is %s.\n", mapname)
	}
	else
	{
		PrintToChat(client,"Map1 is %s.\n", mapname)
	}
...
public Action:sv_WarMap2(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("Map2 is %s.\n", mapname)
	}
	else
	{
		PrintToChat(client,"Map2 is %s.\n", mapname)
	}
...
public Action:sv_WarMap3(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("Map3 is %s.\n", mapname)
	}
	else
	{
		PrintToChat(client,"Map3 is %s.\n", mapname)
	}
...
public Action:sv_WarConfigFile(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("ConfigFile is %s.\n", confname)
	}
	else
	{
		PrintToChat(client,"ConfigFile is %s.\n", confname)
	}
...
public Action:sv_WarTvFile(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("TvFile is %s.\n", tvconfname)
	}
	else
	{
		PrintToChat(client,"TvFile is %s.\n", tvconfname)
	}
...
public Action:sv_WarMapFile(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("MapFile is %s.\n", mapfilename)
	}
	else
	{
		PrintToChat(client,"MapFile is %s.\n", mapfilename)
	}
...
public Action:sv_WarRoundTime(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("RoundTime is %s min.\n", roundtime)
	}
	else
	{
...
public Action:sv_WarTagA(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("TagA is %s.\n",tagA)
	}
	else
	{
		PrintToChat(client,"TagA is %s.\n",tagA)
	}
...
public Action:sv_WarTagB(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("TagB is %s.\n",tagB)
	}
	else
	{
		PrintToChat(client,"TagB is %s.\n",tagB)
	}
...
public Action:sv_WarAllies(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("Team %s is Allies.\n",buffer)
	}
	else
	{
		PrintToChat(client,"Team %s is Allies.\n",buffer)
	}
...
public Action:sv_WarAxis(client, argc)
{
	...
	if(client == 0)
	{
		PrintToServer("Team %s is Axis.\n",buffer)
	}
	else
	{
		PrintToChat(client,"Team %s is Axis.\n",buffer)
	}
...
public Action:sv_WarNumberOfMaps(client, argc)
{
	...
	if(argc > 0)
	{
		...
		if(StrContains("1 2 3", buffer)==-1)
		{
			if(client == 0)
			{
				PrintToServer("Authorised values are : 1 2 3")
			}
			else
			{
				PrintToChat(client,"Authorised values are : 1 2 3")
			}
		}
		else
		{		
	...
	if(client == 0)
	{
		PrintToServer("Number of maps scheduled : %s.\n", buffer)
	}
	else
	{
...
public Action:sv_WarIsLive(client, argc)
{
	...
	if(argc < 1)
	{
		...
		if(client == 0)
		{
			PrintToServer("Syntax is:\nwar_islive <0|1>")
		}
		else
		{
			PrintToChat(client,"Syntax is:\nwar_islive <0|1>")
		}
...
RestartRound()
{
	...
	PrintToChatAll("Round has been restarted.")
}
...
{	
	...
	WriteKeyValue(kv, "teams", "tagA", "// B@ronette$ \\\\");
	WriteKeyValue(kv, "teams", "tagB", "-=[MIAOU]=-");
	WriteKeyValue(kv, "teams", "2", "A");
	WriteKeyValue(kv, "teams", "3", "B");

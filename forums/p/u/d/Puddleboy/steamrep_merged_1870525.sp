...
public Plugin:myinfo = {
	name = "SteamRep Checker (Sockets or SteamTools)",
	author = "jameless (SteamTools compatability by Puddleboy)",
...
public OnClientPostAdminCheck(client) {
	if (SOCKET_AVAILABLE())
	{
		OnClientPostAdminCheck_Socket(client);
		
	}
	else if (STEAMTOOLS_AVAILABLE())
	{
		OnClientPostAdminCheck_SteamTools(client);
	}
	else
	{
			// do nothing, at least the !rep stuff works?
	}
}

public OnClientPostAdminCheck_SteamTools(client) {
	g_IsScammer[client]=false;
	KickAScammer();
	Download_SteamTools(GetClientUserId(client));
}
///original code

public OnClientPostAdminCheck_Socket(client) {
	g_IsScammer[client]=false;
	KickAScammer();
...
Download_SteamTools(client)
{
	...
	if(IsClientConnected(client) && !CheckCommandAccess(client, "SkipSR", ADMFLAG_ROOT, true) && !IsFakeClient(client)) {
		...
		GetClientAuthString(client,steamid,sizeof(steamid));
		GetConVarString(g_exclude,excludetags,sizeof(excludetags));
		Format(requestStr, sizeof(requestStr), "http://steamrep.com/id2rep.php?steamID32=%s&ignore=%s&IP=%s&version=%s",steamid,excludetags,ip,PLUGIN_VERSION);

		new HTTPRequestHandle:hRequest = Steam_CreateHTTPRequest(HTTPMethod_GET, requestStr);
		Steam_SetHTTPRequestHeaderValue(hRequest, "Pragma", "no-cache");
		Steam_SetHTTPRequestHeaderValue(hRequest, "Cache-Control", "no-cache");
		Steam_SendHTTPRequest(hRequest, OnSteamHTTPComplete, client);
...
public OnSteamHTTPComplete(HTTPRequestHandle:HTTPRequest, bool:requestSuccessful, HTTPStatusCode:statusCode, any:client)
{
	if (requestSuccessful && statusCode == HTTPStatusCode_OK)
	{
		decl String:bodyDataBuffer[Steam_GetHTTPResponseBodySize(HTTPRequest)+1];
		Steam_GetHTTPResponseBodyData(HTTPRequest, bodyDataBuffer, Steam_GetHTTPResponseBodySize(HTTPRequest)+1);
		HandleString(bodyDataBuffer,client);	
	}
	else
	{

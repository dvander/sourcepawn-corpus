...
#define MAX_PLAYERS 8    // NMRiH

static const char
    PL_NAME[]    = "[NMRiH] Health & Armor Vampirism",
    PL_VER[]    = "1.0.6_31.02.2024";

// HUD
static const char STATE[][] = {"State_Healthy", "State_Infected", "State_Extracted", "State_Dead"};    // healthy，infected，extracted，dead
static const int CLR[][] = {{0, 255, 0}, {255, 127, 0}, {255, 255, 255}, {255, 0, 0}};    // R，G，B，A
...
{
    if (bEnable)
        hTimer = CreateTimer(fCD, Timer_UpdateInfo, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
}
...
{
    if (bHint)
        UpdateHint(client, hp, ap);
    else
        UpdateHUD(client, hp, ap);
}
...
stock void UpdateHint(const int client, int hp = -1, int armor = -1)
{
    ...
    if (bStamina)
        Format(txt, sizeof(txt), "%s %t", txt, "Hint_SP", GetStaminaLvl(client));

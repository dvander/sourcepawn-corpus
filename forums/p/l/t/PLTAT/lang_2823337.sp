...
//		PrintToServer("%N 语言 设置 为 '%s' (%d) ", client, code, lang);
	}
}
	...
	else
	{
		...
		if(args == 0)
		{
			...
			if(lang >= 0)
			{
				...
				PrintToChat(client, "你 客户端 语言 是 '%s' ('%s', %d)", name, code, lang);
			}
		}
		else
		{
...
{
	if(action == CookieMenuAction_DisplayOption) Format(buffer, maxlen, "语言: %s", cLangPref[client]);
	else if(action == CookieMenuAction_SelectOption)
	{
		char MenuItem[64];
		Menu langmenu = CreateMenu(LangMenuHandler);
		langmenu.SetTitle("选择 语言:");
...
public int LangMenuHandler(Menu langmenu, MenuAction action, int client, int item)
{
	switch(action)
	{
		...
		{
			langmenu.SetTitle("语言: %s", cLangPref[client]);
		}
...
void ChangeLanuage(int client, int lang)
{
	...
	if(lang >= 0)
	{
		...
		PrintToChat(client, "你 客户端 语言 更改 为 '%s' (%s, %d)", name, code, lang);
	}
	else PrintToChat(client, "错误 语言 代码!");
}
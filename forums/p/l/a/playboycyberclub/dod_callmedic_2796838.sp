...
public OnPluginStart()
{
	...
	CreateConVar( "dodmedic_version", PLUGIN_VERSION, "DoD:S Call Medic", FCVAR_SPONLY | FCVAR_REPLICATED | FCVAR_NOTIFY );
...
public Action:CommandCall_Medic( id, args )
{
	if( GetConVarInt( gPluginEnabled ) == 0 )
	{
		...
		PrintToChat( id, "Sorry, you can't call a Medic!" );
	...
	if( !IsPlayerAlive( id ) )
	{
		...
		PrintToChat( id, "You can't call Medic while you are dead!" );
	...
	if( MedicUsed[ id ] >= muse )
	{
		...
		PrintToChat( id, "You can't call a Medic anymore! Only '%d' times.", muse );
	...
	if( GetClientHealth( id ) >= min )
	{
		...
		PrintToChat( id, "Your health must be lower than '%d' to call a Medic!", min );
	...
	PrintToChat( id, "Successfully called Medic. You are now healed." );
...
public Action:CommandSay( id, args )
{
	...
	if( GetConVarInt( gPluginEnabled ) == 1 )
	{
		if( StrEqual( Said, "!medic" ) || StrEqual( Said, "medic" ) || StrEqual( Said, "/medic" ) )
		{
			...
			PrintToChat( id, "If you want to call a Medic, you must 'bind <key> voice_medic' and press that <key>!" );
		}
	}
}

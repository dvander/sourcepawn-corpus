...
{
	CreateConVar("dod_finishround_version", PLUGIN_VERSION, "DoD FinishRound Source Version (DO NOT CHANGE!)", FCVAR_DONTRECORD|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY)
	SetConVarString(FindConVar("dod_finishround_version"), PLUGIN_VERSION)
	FRStatus = CreateConVar("dod_finishround_source", "1", "<1/0> = enable/disable allowing players to finish the running round before changing the map if timelimit is up")
	FRMinPl = CreateConVar("dod_finishround_minplayers", "4", "<#> = minimum active players to allow finishing the round")
	FRHintMsg = CreateConVar("dod_finishround_hintmessage", "1", "<1/0> = enable/disable displaying a 'Last Round' hint message to respawning players")
...
public Action:CheckFinishRound(Handle:timer)
{
	...
	if(GetConVarInt(FRStatus) == 1 && EnoughPlayers() == true)
	{
		...
		{
			PrintToChatAll("\x04[FinishRound] \x01%s", g_LastRoundMsg)
		}
		g_LastRound = true

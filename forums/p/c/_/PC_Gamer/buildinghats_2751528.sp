...
new const String:g_sParticleList[][] =
{
	...
	{"unusual_tesla_flash"},
	{"unusual_universe"},
	{"unusual_bubble_mess_parent_green"},
	{"unusual_bubble_mess_parent_orange"},
	{"unusual_bubble_mess_parent_purple"},
	{"unusual_constellations_blue"},
	{"unusual_constellations_pink"},
	{"unusual_constellations_purple"},
	{"unusual_symbols_parent_fire"},
	{"unusual_symbols_parent_dead"},
	{"unusual_symbols_parent_lightning"},
	{"unusual_dimension_parent"},
	{"unusual_aura_purple_parent"},
	{"unusual_aura_green_parent"},
	{"unusual_aura_red_parent"},
	{"unusual_magicsmoke_purple_parent"},
	{"unusual_magicsmoke_green_parent"},
	{"unusual_magicsmoke_blue_parent"}
}
...
public OnPluginStart()
{
	...
	HookConVarChange(g_hCvarUnusualChance = CreateConVar("sm_bhats_unusualchance", "0.3", "Chance for a building to get an unusual effect on it's hat upon being built. 0.1 = 10%", _, true, 0.0), OnConVarChange);
	HookConVarChange(g_hCvarRerollCooldown = CreateConVar("sm_bhats_rollcooldown", "5", "Hat reroll cooldown (in seconds)", _, true, 0.0), OnConVarChange);
	...
	RegAdminCmd("sm_buildinghateffect",  Command_ChooseBuildingEffect, ADMFLAG_SLAY);
		...
		while ((iBuilding = FindEntityByClassname(iBuilding, "obj_*")) != -1) 
		{
			if(GetEntPropEnt(iBuilding, Prop_Send, "m_hBuilder") == param1 && GetEntProp(iBuilding, Prop_Send, "m_bPlacing") == 0 && GetEntProp(iBuilding, Prop_Send, "m_bCarried") == 0)
			{
				...
				if(IsValidEdict(iParticle))
				{
					...
					new TFObjectType:objecta = TFObjectType:TF2_GetObjectType(iBuilding);
					if(objecta == TFObject_Dispenser)
					{
						SetVariantString("build_point_0");
					}
					else if(objecta == TFObject_Sentry)
					{
					...
					if(GetEntProp(iBuilding, Prop_Send, "m_iUpgradeLevel") == 3 && objecta != TFObject_Dispenser)
					{
...
public Action:Event_DropObject(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	new TFObjectType:objecta = TFObjectType:GetEventInt(event, "object");
	...
	if(client >= 1 && client <= MaxClients && IsClientInGame(client) && g_bWantsTheH[client])
	{
		...
		if(iBuilding > MaxClients && IsValidEntity(iBuilding))
		{
			if(objecta == TFObject_Sentry && GetEntProp(iBuilding, Prop_Send, "m_bMiniBuilding"))
			{
				SetVariantInt(2);
				AcceptEntityInput(iBuilding, "SetBodyGroup");
...
public Action:Event_UpgradeObject(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(iBuilding > MaxClients && IsValidEntity(iBuilding))
	{
		...
		if(objecta == TFObject_Dispenser && GetEntProp(iBuilding, Prop_Send, "m_iUpgradeLevel") == 2)
		{
			if (IsValidEntity(g_hatEnt[iBuilding]))
			{
				AcceptEntityInput(g_hatEnt[iBuilding], "Kill");
...
public Action:Timer_ReHat(Handle:timer, any:iBuilding)
{
	...
	if(iBuilding > MaxClients && IsValidEntity(iBuilding))
	{
		...
		int offset = GetEntSendPropOffs(iBuilding, "m_iObjectType");
		if (offset <= 0)
		{
			return Plugin_Continue;
		}
		...
		else if(objecta == TFObject_Dispenser)

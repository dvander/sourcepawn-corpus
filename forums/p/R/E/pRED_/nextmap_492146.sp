...
readMapCycle(String:szFileName[], String:szNext[], iNext)
{
	...
	{
		LogMessage("WARNING: Mapcyclefile failed to open or the file doesn't exist (file \"%s\")", szFileName)
		return;
	}
	...
	while (!IsEndOfFile(file) && ReadFileLine(file, szBuffer, sizeof(szBuffer)))
	{
		...
		{
			LogMessage("WARNING: Invalid starting char  - \"%s\"", szBuffer)
			continue
		}
		if (!IsMapValid(szBuffer)) 
		{
			LogMessage("WARNING: Invalid map name  - \"%s\"", szBuffer)
			continue
		}
	...
	if (!iMaps)
	{
		LogMessage("WARNING: Couldn't find a valid map")

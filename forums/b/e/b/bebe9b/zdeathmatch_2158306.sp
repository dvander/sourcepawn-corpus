...
{
	name = "Menu of weapons",
	author = "DkMuniz",
	description = "Menu of weapons like Death Math",
	version = VERSION,
};
...
public OnPluginStart()
{
	...
	cvar_dm_respawning = CreateConVar("dm_respawning", "0", "Enable respawning.");
	...
	cvar_dm_sp_time = CreateConVar("dm_sp_time", "3.0", "Spawn protection time.");
	cvar_dm_remove_weapons = CreateConVar("dm_remove_weapons", "0", "Remove ground weapons.");
	cvar_dm_replenish_ammo = CreateConVar("dm_replenish_ammo", "0", "Unlimited player ammo.");
	cvar_dm_hp_start = CreateConVar("dm_hp_start", "100", "Spawn HP.");
	cvar_dm_hp_max = CreateConVar("dm_hp_max", "100", "Maximum HP.");
	cvar_dm_hp_kill = CreateConVar("dm_hp_kill", "0", "HP per kill.");
	cvar_dm_hp_hs = CreateConVar("dm_hp_hs", "0", "HP per headshot kill.");
	cvar_dm_hp_knife = CreateConVar("dm_hp_knife", "0", "HP per knife kill.");
	cvar_dm_hp_messages = CreateConVar("dm_hp_messages", "0", "Display HP messages.");
	...
	cvar_dm_nades_he = CreateConVar("dm_nades_he", "1", "Number of HE grenades to give each player.");
	cvar_dm_nades_smoke = CreateConVar("dm_nades_smoke", "1", "Number of smoke grenades to give each player.");
	LoadConfig();
	// Admin commands
...
LoadConfig()
{
	...
	decl String:path[] = "cfg/deathmatch/dm.ini";
		...
		for(new i = 0; i < sizeof(menuTriggers); i++)
		{
			if (StrEqual(text, menuTriggers[i], false))
			{
				if (gunMenuMode == 1)
					...
					PrintToChat(clientIndex, "[\x042unlimited.ro\x01]  The gun menu is disabled.");
...
BuildWeaponMenuNames()
{
	...
	SetTrieString(weaponMenuNames, "weapon_m4a1", "M4A4");
	...
	SetTrieString(weaponMenuNames, "weapon_cz75a", "CZ75a");
	...
	SetTrieString(weaponMenuNames, "weapon_usp_silencer", "Usp");
...
DisplayOptionsMenu(clientIndex)
{
	if(ZR_IsClientHuman(clientIndex))
	{
...
{
	new clientIndex = GetClientOfUserId(GetEventInt(event, "userid"));
	if (enabled && ZR_IsClientHuman(clientIndex))
	{
		...
		if (Teams:GetClientTeam(clientIndex) > TeamSpectator)
		{
			...
			{
				PrintToChat(clientIndex, "[\x042unlimited.ro\x01] Type \x04guns \x05for open the menu of weapons.");
			...
			{
				if (ZR_IsClientHuman(clientIndex) && gunMenuMode == 1)
				{
					DisplayOptionsMenu(clientIndex);
				}
			}
	...
	if (enabled)
	{
		...
		if ((attackerIndex != 0) && IsPlayerAlive(attackerIndex))
		{
			...
			if ((knifed && (HPPerKnifeKill > 0)) || (!headshot && (HPPerKill > 0)) || (headshot && (HPPerHeadshotKill > 0)))
			{
				...
				{
					if (knifed)
						PrintToChat(attackerIndex, "[\x042unlimited.ro\x01]  \x04+%iHP\x01 por matar um inimigo (knife).", HPPerKnifeKill);
					else if (headshot)
						PrintToChat(attackerIndex, "[\x042unlimited.ro\x01]  \x04+%iHP\x01 por matar um inimigo (headshot).", HPPerHeadshotKill);
					else
						PrintToChat(attackerIndex, "[\x042unlimited.ro\x01]  \x04+%iHP\x01 por matar um inimigo.", HPPerKill);
				}
			}
		}
...
public Menu_Options(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		else if (StrEqual(info, "Same 1"))
		{
			...
			{
				newWeaponsSelected[param1] = true;
				PrintToChat(param1, "[\x042unlimited.ro\x01] You will have the same weapons in the next round.");
			}
		...
		else if (StrEqual(info, "Random 1"))
		{
			...
			{
				newWeaponsSelected[param1] = true;
				PrintToChat(param1, "[\x042unlimited.ro\x01] You will receive random weapons in the next round.");
			}
...
public Menu_Secondary(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		if (newWeaponsSelected[param1])
			PrintToChat(param1, "[\x042unlimited.ro\x01] Their new weapons will be given to you in the next round.");
...
public Action:Command_Guns(clientIndex, args)
{
	...
	{
		PrintToChat(clientIndex, "[\x042unlimited.ro\x01] You are a zombie, cannot open the menu of weapons");
	}
}
	...
	if (action == MenuAction_Select)
	{
		...
		if (StrEqual(info, "Edit"))
		{
			...
			if (inEditMode)
			{
				...
				PrintToChat(param1, "[\x042unlimited.ro\x01]  Edit mode enabled.");
			}
			else
				PrintToChat(param1, "[\x042unlimited.ro\x01]  Edit mode disabled.");
		}
		else if (StrEqual(info, "Nearest"))
		{
			...
			if (spawnPoint != -1)
			{
				...
				PrintToChat(param1, "[\x042unlimited.ro\x01]  Teleported to spawn point #%i (%i total).", spawnPoint + 1, spawnPointCount);
			}
		}
		else if (StrEqual(info, "Previous"))
		{
			if (spawnPointCount == 0)
				PrintToChat(param1, "[\x042unlimited.ro\x01]  There are no spawn points.");
			else
			{
		...
		else if (StrEqual(info, "Delete"))
		{
			...
			if (spawnPoint != -1)
			{
				...
				PrintToChat(param1, "[\x042unlimited.ro\x01]  Deleted spawn point #%i (%i total).", spawnPoint + 1, spawnPointCount);
			}
		}
		...
		{
			if (WriteMapConfig())
				PrintToChat(param1, "[\x042unlimited.ro\x01]  Configuration has been saved.");
			else
				PrintToChat(param1, "[\x042unlimited.ro\x01]  Configuration could not be saved.");
		}
...
public Panel_ConfirmDeleteAllSpawns(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		{
			spawnPointCount = 0;
			PrintToChat(param1, "[\x042unlimited.ro\x01]  All spawn points have been deleted.");
		}
		DisplayMenu(BuildSpawnEditorMenu(), param1, MENU_TIME_FOREVER);
	}
}
...
GetNearestSpawn(clientIndex)
{
	if (spawnPointCount == 0)
	{
		PrintToChat(clientIndex, "[\x042unlimited.ro\x01]  There are no spawn points.");
		return -1;
	}
...
AddSpawn(clientIndex)
{
	...
	{
		PrintToChat(clientIndex, "[\x042unlimited.ro\x01]  Could not add spawn point (max limit reached).");
		return;
	}
	GetClientAbsOrigin(clientIndex, spawnPositions[spawnPointCount]);
	GetClientAbsAngles(clientIndex, spawnAngles[spawnPointCount]);
	spawnPointCount++;
	PrintToChat(clientIndex, "[\x042unlimited.ro\x01]  Added spawn point #%i (%i total).", spawnPointCount, spawnPointCount);
}
...
InsertSpawn(clientIndex)
{
	...
	else
	{
		...
		spawnPointCount++;
		PrintToChat(clientIndex, "[\x042unlimited.ro\x01]  Inserted spawn point at #%i (%i total).", lastEditorSpawnPoint[clientIndex] + 1, spawnPointCount);
	}
}
...
{
	ResetSpawnStats();
	PrintToChat(clientIndex, "[\x042unlimited.ro\x01]  Spawn statistics have been reset.");
}

...
public OnPluginStart()
{
    ...
    RegAdminCmd("sm_golden", Command_golden, ADMFLAG_BAN, "Gives player an Golden");
    RegAdminCmd("sm_huntsman", Command_huntsman, ADMFLAG_BAN, "Gives player an Huntsman");
    RegAdminCmd("sm_butterfly", Command_butterfly, ADMFLAG_BAN, "Gives player an butterfly");
    ...
    cvarGolden = CreateConVar("knife_golden_enable", "1", "Disable or Enable Golden");
...
public Action:Spawnmsg(Handle:timer, any:client)
{
 ...
 if (IsClientInGame(client))
 {
   PrintToChat(client, "Enter \x03\"!diao\" \x01 to open knife menu!");
 	}
 }
}
...
public Action:DID(clientId) 
{
	...
	{
    AddMenuItem(menu, "option7", "[100] Golden");
    }
    if (GetConVarBool(cvarHuntsman))
	{
    AddMenuItem(menu, "option8", "[120] Huntsman");
    }
    if (GetConVarBool(cvarbutterfly))
	{
    AddMenuItem(menu, "option9", "[160] butterfly");
    }
...
public DIDMenuHandler(Handle:menu, MenuAction:action, client, itemNum) 
{
    ...
    if (IsPlayerAlive(client))
    {
        ...
        PrintToChat(client, "[Knife Shop] You bought a Golden Knife! Your credits: \x03%i \x01(-100)", g_iCredits[client]);
                   }
                   else
                   	{
              		if (g_iCredits[client] >= 100)
              		PrintToChat(client, "[Knife Shop] You bought a Golden Knife! You get it on your next spawn. Your credits: \x03%i \x01(-45)", g_iCredits[client]);
              		weapon_choose[client] = 6;
              		g_iCredits[client] -= 100;
                   }
              }
              ...
              {
                 PrintToChat(client, "[Knife Shop] Your credits: \x03%i (Not enough credits! Need 100)", g_iCredits[client]);
              }
            }
    ...
    if (IsPlayerAlive(client))
    {
        ...
        PrintToChat(client, "[Knife Shop] You bought a Huntsman Knife! Your credits: \x03%i \x01(-120)", g_iCredits[client]);
                   }
                   else
                   	{
              		if (g_iCredits[client] >= 120)
              ...
              {
                 PrintToChat(client, "[Knife Shop] Your credits: \x03%i (Not enough credits! Need 120)", g_iCredits[client]);
              }
            }
    ...
    if (IsPlayerAlive(client))
    {
        ...
        g_iCredits[client] -= 160;

        PrintToChat(client, "[Knife Shop] You bought a butterfly Knife! Your credits: \x03%i \x01(-160)", g_iCredits[client]);
                   }
                   else
                   	{
              		if (g_iCredits[client] >= 160)
...
public Action:Command_golden(client, args)
{
	decl String:pattern[64],String:buffer[64],String:ent[64],String:EntName[64];
	GetCmdArg(1,pattern,sizeof(pattern));
	GetCmdArg(2,ent,sizeof(ent));
	...
	if (IsClientInGame(targets[i])) {
        ...
        RemovePlayerItem(targets[i], currentknife);
        	...
        	new knife = GivePlayerItem(targets[i], "weapon_knifegg");
        	EquipPlayerWeapon(targets[i], knife);
        	PrintToChat(targets[i], "[Knife Shop] You got the Golden Knife for 1 map!");
        	weapon_choose[targets[i]] = 6;
        	GetClientName(ent[i],EntName,sizeof(EntName));
        }
    }
	return Plugin_Handled;

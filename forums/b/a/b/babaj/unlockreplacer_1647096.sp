...
new Handle:excLugermorph;
new Handle:excBigKill;
new Handle:excCaC;
new Handle:excTribalmansShiv;
new Handle:excScotsmansSkullcutter;
new Handle:excWrangler;
new Handle:excFrontierJustice;
new Handle:excGunslinger;
new Handle:excSouthernHospitality;
new Handle:excGoldenWrench;
new Handle:excVitaSaw;
new Handle:excPowerjack;
new Handle:excDegreaser;
new Handle:excShortstop;
new Handle:excHolyMackerel;
new Handle:excMadMilk;
new Handle:excLEtranger;
new Handle:excYourEternalReward;
new Handle:excBattalionsBackup;
new Handle:excBlackBox;
new Handle:excSydneySleeper;
new Handle:excDarwinsDangerShield;
new Handle:excBushwacka;
new Handle:excRocketJumper;
new Handle:excGlovesofRunningUrgently;
new Handle:excStickybombJumper;
new Handle:excMantreads;
new Handle:excCowMangler5000;
new Handle:excRighteousBison;
new Handle:excPomson6000;
new Handle:excSplendidScreen;
new Handle:excAtomizer;
new Handle:excDetonator;
...
new String:weaponAlias[][] = {
	...
	"Detonator"
  	...
  	// add more aliases here, aliases are the names used on the admin menu. MUST BE PARALLEL TO UNLOCKDEFIDX AND REPLACEMENT ARRAY
};
...
new unlockDefIdx[] = {
	...
	351,
  	...
  	// add more unlockable weapon indexes here, weapon indexes are used to indentify each weapon.
};
...
new String:replacementWeapon[][] = {
	...
	"tf_weapon_flaregun"
  		...
  		// add more replacement weapons here, this is what the server gives the client instead of their unlock.
};
...
public OnPluginStart()
{
	...
	excLugermorph = CreateArray(64);
	excBigKill = CreateArray(64);
	excCaC = CreateArray(64);
	excTribalmansShiv = CreateArray(64);
	excScotsmansSkullcutter = CreateArray(64);
	excWrangler = CreateArray(64);
	excFrontierJustice = CreateArray(64);
	excGunslinger = CreateArray(64);
	excSouthernHospitality = CreateArray(64);
	excGoldenWrench = CreateArray(64);
	excVitaSaw = CreateArray(64);
	excPowerjack = CreateArray(64);
	excDegreaser = CreateArray(64);
	excShortstop = CreateArray(64);
	excHolyMackerel = CreateArray(64);
	excMadMilk = CreateArray(64);
	excLEtranger = CreateArray(64);
	excYourEternalReward = CreateArray(64);
	excBattalionsBackup = CreateArray(64);
	excBlackBox = CreateArray(64);
	excSydneySleeper = CreateArray(64);
	excDarwinsDangerShield = CreateArray(64);
	excBushwacka = CreateArray(64);
	excRocketJumper = CreateArray(64);
	excGlovesofRunningUrgently = CreateArray(64);
	excStickybombJumper = CreateArray(64);
	excMantreads = CreateArray(64);
	excCowMangler5000 = CreateArray(64);
	excRighteousBison = CreateArray(64);
	excPomson6000 = CreateArray(64);
	excSplendidScreen = CreateArray(64);
	excAtomizer = CreateArray(64);
	excDetonator = CreateArray(64);
...
public Action:ProcessEdict(Handle:timer, any:edict)
{
	...
	new defIdx =GetEntProp(edict, Prop_Send, "m_iItemDefinitionIndex"); //GetEntData(edict, offset, 4);
...
ReplaceClientWeapons(edict, idx)
{
	...
	{
		PrintCenterText(client, "\x04[Unlock Replacer]:\x01 --> ALL <-- of your unlockables (weapons/items) have be replaced by the server. Adjust your class loadout accordingly.");
		...
		while((edict = FindEntityByClassname(edict, "tf_wearable_item")) != -1)
		{
			if (GetEntProp(edict, Prop_Send, "m_iItemDefinitionIndex") == 57)
			{
				...
				{	
					...
					if(g_bCanClientReceiveMsg[client])
					{
						PrintToChat(client, "\x04[Unlock Replacer]:\x01 --> ALL <-- of your unlockables (weapons/items) have be replaced by the server. Adjust your class loadout accordingly.");
						// Stops the client getting spammed by only sending one message every 30 seconds.
						g_bCanClientReceiveMsg[client] = false;
						CreateTimer(30.0, ResetClientMsgStatus, client, TIMER_FLAG_NO_MAPCHANGE);
					}
...
Handle:GetArray(idx)
{
	switch (idx)
	{
		...
		case 28: return excLugermorph;
		case 29: return excBigKill;
		case 30: return excCaC;
		case 31: return excTribalmansShiv;
		case 32: return excScotsmansSkullcutter;
		case 33: return excWrangler;
		case 34: return excFrontierJustice;
		case 35: return excGunslinger;
		case 36: return excSouthernHospitality;
		case 37: return excGoldenWrench;
		case 38: return excVitaSaw;
		case 39: return excPowerjack;
		case 40: return excDegreaser;
		case 41: return excShortstop;
		case 42: return excHolyMackerel;
		case 43: return excMadMilk;
		case 44: return excLEtranger;
		case 45: return excYourEternalReward;
		case 46: return excBattalionsBackup;
		case 47: return excBlackBox;
		case 48: return excSydneySleeper;
		case 49: return excDarwinsDangerShield;
		case 50: return excBushwacka;
		case 51: return excRocketJumper;
		case 52: return excGlovesofRunningUrgently;
		case 53: return excStickybombJumper;
		case 54: return excMantreads;
		case 55: return excCowMangler5000;
		case 56: return excRighteousBison;
		case 57: return excPomson6000;
		case 58: return excSplendidScreen;
		case 59: return excAtomizer;
		case 60: return excDetonator;
	}

...
public OnPluginStart()
{
	...
	RegAdminCmd("ff2_special", Command_MakeNextSpecial, ADMFLAG_BAN, "Call a special to next round.");
	RegAdminCmd("ff2_addpoints", Command_Points, ADMFLAG_BAN, "ff2_addpoints < target > < points > - Add queue points to user.");
...
public Action:checkFirstHale(Handle:timer)
{
	...
	for(new i = 0; i <= MaxClients; i++)
	{
		...
		PrintToChat(client,"\x01\x04[FF2] First-round Hale Bug Check!");
		ForcePlayerSuicide(client);
		if(oldclass==TFClass_Spy)
			TF2_SetPlayerClass(client, TFClass_Soldier);
		else
			TF2_SetPlayerClass(client, TFClass_Spy);
		PrintToChat(client,"\x01\x04[FF2] We'll fix you up when the game starts.");

/**
	...
	Country Filter 2nd Edition, Version 2.0.5
	- quick kick logs 31.03.2014
	...
	if(g_alphacode ? GeoipCode2(IP, CODE):GeoipCode3(IP, CODE))
	{
		...
		switch(reject)
		{
			case true:	// Reject
			{
				...
				LogMessage("Player %L - IP: %s) was kicked from server because he's from %s.", client, IP, countryname);
		...
		switch(islan)
		{
			case true:
			{
				if(!g_allowlan)
				{
					...
					LogMessage("Player %L - IP: %s) was kicked from server because he's from LAN.", client, IP);
			...
			case false:
			{
				if(!g_allowunidentified)
				{
					...
					LogMessage("Player %L - IP: %s) was kicked from server because he's from Unidentified country.", client, IP);

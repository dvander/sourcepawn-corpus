...
*
* Verion 3.3.1
*	- Bacardi mess this, add r_drawothermodels mat_proxy
*/
//////////////////////////////////////////////////////////////////
// Defines
//////////////////////////////////////////////////////////////////
...
{
	name = "VBAC",
	author = "MoggieX, Bacardi",
	description = "Very Basic Anti Cheat",
	version = PLUGIN_VERSION,
	url = "http://forums.alliedmods.net/showpost.php?p=1230694&postcount=231"
};
...
public OnPluginStart()
{
	...
	//HookEvent("player_hurt", Event_player_damage);
	//HookEvent("player_death", Event_player_damage);
}
...
public Action:Event_player_spawn(Handle:event, const String:name[], bool:dontBroadcast)
 {
	...
	if (GetConVarInt(g_CheckCvar) == 0)
	{
		...
		// Query the client for r_drawothermodels - ClientConVar4
		QueryClientConVar(client, "r_drawothermodels", ConVarQueryFinished:ClientConVar4, client);

		// Query the client for mat_proxy - ClientConVar5
		QueryClientConVar(client, "mat_proxy", ConVarQueryFinished:ClientConVar5, client);
...
//////////////////////////////////////////////////////////////////
// Player checking on hurt death event
//////////////////////////////////////////////////////////////////
//public Action:Event_player_damage(Handle:event, const String:name[], bool:dontBroadcast)
// {
//
//	// Added check to make sure sv_cheats is not enabled and this will save everyone for getting banned if some nubcake enables the checked server var
//
...
// r_drawothermodels
//////////////////////////////////////////////////////////////////
public ClientConVar4(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName4[], const String:cvarValue4[])
 {
	...
	// For Interger values
	new cvarValueNew = StringToInt(cvarValue4);
	...
	if (cvarValueNew != 1)
	{
		...
		// Notify in chatas i'd want to wet my panties when we get one
		PrintToChatAll("\x04======== \x03CHEATER FOUND! \x04========");
		PrintToChatAll("\x03[VBAC CHEATER FOUND!] \x04Name: \x03%s \x04SteamID: \x03%s \x04CVar: \x03%s \x04Value: \x03%s", player_name, steam_id, cvarName4, cvarValue4);
		PrintToChatAll("\x04======== \x03CHEATER FOUND! \x04========");
		...
		if (GetConVarInt(MySQL_Bans) == 1)
 		{
			...
			new Handle:db = SQL_Connect("default", true, error, sizeof(error));
	 		...
	 		Format(query, sizeof(query), "INSERT INTO mysql_bans (steam_id, player_name, ban_length, ban_reason, banned_by) VALUES ('%s', '%s', '0', 'VBAC CVAR %s Value %s ', 'VBAC-MySQL')", steam_id, player_name, cvarName4, cvarValue4);
				
 			SQL_Query(db, query);
			LogAction(client, -1, "[VBAC MySQL] Steam ID: %s, Name: %s, CVar Name: %s, CVar Value: %s", steam_id, player_name, cvarName4, cvarValue4);	
  			CloseHandle(db);
		}
		/* END: Mysql Banning Function */
		...
		if (GetConVarInt(SB_Bans) == 1)
		{
			ServerCommand("sm_ban #%d 0 VAC-Ban-Detected-r_drawothermodels",GetClientUserId(client));
		}
		else
		{
		// Ban the *
		BanClient(client, 
				0, 
				BANFLAG_AUTO, 
				"r_drawothermodels_bypass", 
				"VAC Ban Detected r_drawothermodels bypass", 
				"VBAC",
				client);
		}

		// Log the event
		LogAction(client, -1, "[VBAC r_drawothermodels BAN] Name: %s SteamID: %s was BANNED for CVar: %s CVar Value: %s", player_name, steam_id, cvarName4, cvarValue4);
	}
...
// mat_proxy
//////////////////////////////////////////////////////////////////
public ClientConVar5(QueryCookie:cookie, client, ConVarQueryResult:result, const String:cvarName5[], const String:cvarValue5[])
 {
	...
	// For Interger values
	new cvarValueNew = StringToInt(cvarValue5);
	...
	if (cvarValueNew != 0)
	{
		...
		// Notify in chatas i'd want to wet my panties when we get one
		PrintToChatAll("\x04======== \x03CHEATER FOUND! \x04========");
		PrintToChatAll("\x03[VBAC CHEATER FOUND!] \x04Name: \x03%s \x04SteamID: \x03%s \x04CVar: \x03%s \x04Value: \x03%s", player_name, steam_id, cvarName5, cvarValue5);
		PrintToChatAll("\x04======== \x03CHEATER FOUND! \x04========");
		...
		if (GetConVarInt(MySQL_Bans) == 1)
 		{
			...
			new Handle:db = SQL_Connect("default", true, error, sizeof(error));
	 		...
	 		Format(query, sizeof(query), "INSERT INTO mysql_bans (steam_id, player_name, ban_length, ban_reason, banned_by) VALUES ('%s', '%s', '0', 'VBAC CVAR %s Value %s ', 'VBAC-MySQL')", steam_id, player_name, cvarName5, cvarValue5);
				
 			SQL_Query(db, query);
			LogAction(client, -1, "[VBAC MySQL] Steam ID: %s, Name: %s, CVar Name: %s, CVar Value: %s", steam_id, player_name, cvarName5, cvarValue5);	
  			CloseHandle(db);
		}
		/* END: Mysql Banning Function */
		...
		if (GetConVarInt(SB_Bans) == 1)
		{
			ServerCommand("sm_ban #%d 0 VAC-Ban-Detected-mat_proxy",GetClientUserId(client));
		}
		else
		{
		// Ban the *
		BanClient(client, 
				...
				BANFLAG_AUTO, 
				"mat_proxy_bypass", 
				"VAC Ban Detected mat_proxy bypass", 
				"VBAC",
				client);
		}

		// Log the event
		LogAction(client, -1, "[VBAC mat_proxy BAN] Name: %s SteamID: %s was BANNED for CVar: %s CVar Value: %s", player_name, steam_id, cvarName5, cvarValue5);
	}

...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_resetplayerscore", admcmd_resetscore, ADMFLAG_SLAY, "sm_resetplayerscore <#userid|name> = Reset player score");
...
public Action:admcmd_resetscore(client, args)
{
	if(args < 1)
	{
		ReplyToCommand(client, "[SM] usage: sm_resetplayerscore <#userid|name>");
	...
	{
		SetClientFrags(target_list[i], 0);
		SetClientDeaths(target_list[i], 0);
	}
	
	if (tn_is_ml)
	{
		ShowActivity2(client, "[SM] ", "%t", "Admin reset score", target_name);
	}
	else
	{
		ShowActivity2(client, "[SM] ", "%t", "Admin reset score", "_s", target_name);
	}

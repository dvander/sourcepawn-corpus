...
public OnAdminMenuReady(Handle:topmenu) {
	if (topmenu != hTopMenu)
	{
		...
		new TopMenuObject:ban_commands = FindTopMenuCategory(hTopMenu, "Ban Commands");
		if(ban_commands == INVALID_TOPMENUOBJECT)
		{
			ban_commands = AddToTopMenu(
			hTopMenu,
			"Ban Commands",
		...
		AddToTopMenu(hTopMenu, "sm_bandisconnected", TopMenuObject_Item, AdminMenu_Ban, ban_commands, "sm_bandisconnected", ADMFLAG_BAN);
	}
}

public handler(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength )
{
	if (action == TopMenuAction_DisplayTitle)
	{
		Format(buffer, maxlength, "Ban Commands");
	}

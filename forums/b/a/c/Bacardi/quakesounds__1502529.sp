...
public OnPluginStart()
{
	...
	HookConVarChange(mp_teamplay, TeamplayChanged);
...
public TeamplayChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	teamplay = StringToInt(newValue);
}
...
{
	if(attacker != 0 && attacker < 65 && attacker != victim)
	{
		lasthit[victim] = hitgroup;
	}
}
...
{	
	...
	if(attackerClient>0 && attackerClient<=iMaxClients)
	{
		...
		else if(GetClientTeam(attackerClient) == GetClientTeam(victimClient) && teamplay == 1)
		{
		...
		else
		{
			...
			if(headshot || lasthit[victimClient] == 1)
			{
			...
			else if((headshot || lasthit[victimClient] == 1) && settingConfig[HEADSHOT][headShotCount[attackerClient]])
			{				
				soundId = HEADSHOT;
				killsValue = headShotCount[attackerClient];
			}
			else if((headshot || lasthit[victimClient] == 1) && settingConfig[HEADSHOT][NOT_BASED_ON_KILLS])
			{				
				soundId = HEADSHOT;
			}			
			#if defined TF2

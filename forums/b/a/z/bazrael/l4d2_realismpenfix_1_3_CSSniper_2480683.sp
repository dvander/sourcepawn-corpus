...
new g_iNerfMagnum;
new g_iBuffSniper;
new g_iMagnumPenetrationLimit;
new g_iSniperPenetrationLimit;

new g_iCurrentPenetrationCount[MAXPLAYERS+1] = { 0, ... };

new Float:g_fDifficultyMultiplier;
new bool:g_bRealism;
...
public OnPluginStart()
{
	...
	new Handle:cvarEnable = CreateConVar("sm_realismpenfix_enable", "1.0", "Enables this plugin (1 = realism only, 2 = all game modes)", CVAR_FLAGS);
	new Handle:cvarDebug = CreateConVar("sm_realismpenfix_debug", "0.0", "Print debug output.", CVAR_FLAGS);
	new Handle:cvarNerfMagnum = CreateConVar("sm_realismpenfix_nerfmagnum", "1.0", "0: full penetration bonus\n1: half penetration bonus\n2: no penetration bonus, no distance penalty\n3: no penetration bonus, plus distance damage penalty", CVAR_FLAGS);
	new Handle:cvarBuffSniper = CreateConVar("sm_realismpenfix_buffsniper", "1.0", "0: no penetration bonus\n1: half penetration bonus\n2: full penetration bonus", CVAR_FLAGS);
	new Handle:cvarMagnumLimit = CreateConVar("sm_realismpenfix_magnumlimit", "0.0", "Maximum number of enemies one magnum bullet can kill (0 disables feature)", CVAR_FLAGS);
	new Handle:cvarSniperLimit = CreateConVar("sm_realismpenfix_sniperlimit", "0.0", "Maximum number of enemies one sniper bullet can kill (0 disables feature)", CVAR_FLAGS);
...
public bool:isGameModeRealism(const String:GameMode[])
{
	return (StrContains(GameMode, "realism", false) >= 0);
}

public Float:GetDifficultyMultiplier(const String:Difficulty[])
{
	new Float:ret = 1.0;
	if (g_bRealism)
	{
...
public OnNerfMagnumChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	g_iNerfMagnum = StringToInt(newVal);
}

public OnBuffSniperChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	g_iBuffSniper = StringToInt(newVal);
}

public OnMagnumLimitChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	g_iMagnumPenetrationLimit = StringToInt(newVal);
}

public OnSniperLimitChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	g_iSniperPenetrationLimit = StringToInt(newVal);
}

public OnRPFEnableChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	g_iEnabled = StringToInt(newVal);
}

//public OnRPFDebugChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
//{
...
public RealismPenetrationFix_SpawnPost(entity)
{
...
new Float:lastTraceAttackDamage = 0.0;
public Action:RealismPenetrationFixTraceAttack(victim, &attacker, &inflictor, &Float:damage, &damagetype, &ammotype, hitbox, hitgroup)
{
	if (g_bDebug)
	{
...
public Action:RealismPenetrationFixOnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3])
{
	new bool:changed = false;
	new bool:bModeEnabled = (g_iEnabled == 2) || (g_iEnabled == 1 && g_bRealism);
	...
	if (bModeEnabled && IS_SURVIVOR_ALIVE(attacker))
	{
		...
		if (((!g_bRealism && g_iBuffSniper < 2) || (g_bRealism && g_iBuffSniper > 0) || g_iNerfMagnum > 0) && !(damagetype & 0x40000000))
		{		
			new bool:bNerfMag = g_iNerfMagnum > 0, bMagStumble = true;
			...
			{
				new Float:maxHealth = float(GetEntProp(victim, Prop_Data, "m_iMaxHealth"));
		...
		if (g_iSniperPenetrationLimit > 0 && g_iCurrentPenetrationCount[attacker] > g_iSniperPenetrationLimit && (StrEqual(weaponName, "weapon_hunting_rifle") || StrEqual(weaponName, "weapon_sniper_military") || StrEqual(weaponName, "weapon_sniper_scout") || StrEqual(weaponName, "weapon_sniper_awp")))
		{
			return Plugin_Handled;
		}
	}

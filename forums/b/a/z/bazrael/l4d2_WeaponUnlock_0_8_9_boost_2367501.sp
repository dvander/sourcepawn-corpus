...
static Handle:cvarScoutBoost				= INVALID_HANDLE;
static Handle:cvarAWPBoost				= INVALID_HANDLE;
static Handle:cvarSG552Boost				= INVALID_HANDLE;
static Handle:cvarMP5Boost				= INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	cvarAWPBoost = CreateConVar("l4d2_wu_awpboost", "135", "Amount of damage added to AWP sniper rifle. Base damage: 115", FCVAR_PLUGIN);
	cvarMP5Boost = CreateConVar("l4d2_wu_mp5boost", "0", "Amount of damage added to MP5 submachine gun. Base damage: 23", FCVAR_PLUGIN);
	cvarScoutBoost = CreateConVar("l4d2_wu_scoutboost", "110", "Amount of damage added to scout sniper rifle. Base damage: 90", FCVAR_PLUGIN);
	cvarSG552Boost = CreateConVar("l4d2_wu_sg552boost", "0", "Amount of damage added to SG552 rifle. Base damage: 32", FCVAR_PLUGIN);
...
public Action:Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (StrEqual(weapon, "sniper_awp"))
	{
		...
		new damage = GetConVarInt(cvarAWPBoost);
	...
	{
		new health = GetClientHealth(target);
		new damage = GetConVarInt(cvarMP5Boost);
		if (health-damage < 0) SetEntityHealth(target, 0);
		else SetEntityHealth(target, health-damage);
	}
	...
	{
		new health = GetClientHealth(target);
		new damage = GetConVarInt(cvarScoutBoost);
		if (health-damage < 0) SetEntityHealth(target, 0);
		else SetEntityHealth(target, health-damage);
	}
	...
	{
		new health = GetClientHealth(target);
		new damage = GetConVarInt(cvarSG552Boost);
		if (health-damage < 0) SetEntityHealth(target, 0);
		else SetEntityHealth(target, health-damage);
	}
}

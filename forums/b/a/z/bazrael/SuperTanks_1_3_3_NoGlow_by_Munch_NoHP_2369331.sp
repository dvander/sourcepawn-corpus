...
public OnPluginStart()
{
	...
	hDisplayHealthCvar = CreateConVar("st_display_health", "1", "Display tanks health in crosshair? 1: Default, 2: Only show their names but without health.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,2.0);
	...
	iDisplayHealthCvar = GetConVarInt(hDisplayHealthCvar);
					...
					if (index != 0 || (index == 0 && bDefaultOverride))
					{
						...
						switch(index)
						{
							...
							case 13:
							{
								if (ShieldsUp[i] > 0)
								{
									new glowcolor = RGB_TO_INT(120, 120, 170);
...
public Action:TimerUpdate01(Handle:timer)
{
	...
	if (bSuperTanksEnabled && iDisplayHealthCvar == 1)
	{
		for (new i=1; i<=MaxClients; i++)
		{
	...
	else if (bSuperTanksEnabled && iDisplayHealthCvar == 2)
	{
		for (new i=1; i<=MaxClients; i++)
		{
			if (IsClientInGame(i) && GetClientTeam(i) == 2)
					...
					if (IsValidEntity(entity))
					{
						...
						if (StrEqual(classname, "player", false))
						{
							if (entity > 0)
							{
								...
								{
									new health = GetClientHealth(entity);
									PrintHintText(i, "%N", entity, health);
								}
							}
						}
					}

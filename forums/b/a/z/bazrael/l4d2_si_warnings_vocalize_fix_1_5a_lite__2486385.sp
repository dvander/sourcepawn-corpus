...
* =============================
* 
* Version 1.5a    2017-07-18
* - Fixed unusual delay while using any option of radial menu.
* 
* - Now plugin can work well with L4D Epsilon Modified Talker.
* 
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	...
	*  possibly skip over it instead of always choosing the first positive*/
	for (new i = 1; i <= MaxClients; i++) {
		...
		if (IsSurvivor(i) && IsPlayerAlive(i) && !IsActorBusy(i) && random == 1) {
			PerformSceneEx(i, announce_special, _, 0.0) // default: 2.0
			i=MaxClients
			break 
		}
	}

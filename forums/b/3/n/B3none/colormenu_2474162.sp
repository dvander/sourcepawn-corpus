...
public void OnPluginStart()
{
	...
	PrintToServer("This server is running a custom scripted colour command");
	...
	RegAdminCmd("sm_colors", Command_Colors, ADMFLAG_RESERVATION, "Pops up the colors menu");
        RegAdminCmd("sm_colours", Command_Colors, ADMFLAG_RESERVATION, "Pops up the colors menu");
        RegAdminCmd("sm_color", Command_Colors, ADMFLAG_RESERVATION, "Pops up the colors menu");
        RegAdminCmd("sm_colour", Command_Colors, ADMFLAG_RESERVATION, "Pops up the colors menu");
}
...
public int MenuHandler_Colors(Handle hMenu, MenuAction maAction, int client, int choice)
{
	if(maAction == MenuAction_Select)
	{
		...
		if(StrEqual(sChoice, "none"))
		{
			PrintToChat(client, " \x10[Chat Colours]\x01 Your chat color will be set to default.");
		}

		else
		{
			PrintToChat(client, " \x10[Chat Colours]\x01 You have selected %sthis color\x01.", sChoice);
		}
	}
...
public Action OnChatMessage(int &client, Handle hRecipients, char[] sName, char[] sMessage)
{
	...
	return Plugin_Continue;
        if(CheckCommandAccess(client, "sm_color", ADMFLAG_RESERVATION))
	{
		char sCookie[8];
		GetClientCookie(client, gH_Cookie, sCookie, 8);
	...
	return Plugin_Continue;
        if(CheckCommandAccess(client, "sm_colours", ADMFLAG_RESERVATION))
	{
		char sCookie[8];
		GetClientCookie(client, gH_Cookie, sCookie, 8);
	...
	return Plugin_Continue;
        if(CheckCommandAccess(client, "sm_colour", ADMFLAG_RESERVATION))
	{
		char sCookie[8];
		GetClientCookie(client, gH_Cookie, sCookie, 8);

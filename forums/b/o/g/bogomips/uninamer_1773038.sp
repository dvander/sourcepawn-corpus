...
#include <sourcemod>
#include <sdktools> // for the SetClientInfo() command



/* CHANGELOG
*
* 1.3 updated the way name are changed to comply with new orangebox methods - by Bogomips
...
public OnPluginStart()
{
	...
	AutoExecConfig(true, "plugin.uninamer");
...
nameCheck(clientSlot)
{
	...
	if (GetClientName(clientSlot, nameBuf, 128))
	{
		...
		{	
			...
			{
				decl String:oldName[256], String:newName[256];
				oldName = nameBuf;
				...
				// v1.3 release change
				Format(newName, sizeof(newName), "%s %d", nameBuf, GetClientUserId(clientSlot));
				SetClientInfo(clientSlot, "name", newName);
				SetEntPropString(clientSlot, Prop_Data, "m_szNetname", newName);
				PrintToChatAll("\x01\"\x04%s\x01\" => \"\x04%s\x01\"", oldName, newName);
			} else { // kick

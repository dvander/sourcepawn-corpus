...
public void OnPluginStart()
{
	...
	// This may sound retarded as fuck but I couldn't
	// get the admin menu to work without delaying
	// the category items by any value in other
	// modules of the plugin. Have a better solution?
	// Tell me it please :D
	// (even though this one isn't so bad, OnPluginStart()
	// will only get called once on a non-development server)
	// let's hope using OnAdminMenuReady will fix it
}
...
{
	LogMessage("SQL_DBConnect()");
	if(SQL_CheckConfig("shavit"))
	{
		LogMessage("SQL_CheckConfig == true");
		if(gH_SQL != null)
		{
			LogMessage("SQL_TQuery CREATE TABLE");

...
{	
	...
	else if (msgStart == 2 && CheckCommandAccess(client, "sm_psay", ADMFLAG_CHAT)) // sm_psay alias
	{
		...
		if (g_DoColor)
		{
			PrintToChat(client, "\x04(Private to %s) %s: \x04%s", name2, name, message[len]);
			PrintToChat(target, "\x04(Private to %s) %s: \x04%s", name2, name, message[len]);
		}
		else
		{
...
SendChatToAll(client, String:message[])
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		if (g_DoColor)
		{
			PrintToChat(i, "\x03(ALL) %s: \x03%s", nameBuf, message);
		}
		else
		{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i))
		{
			if (CheckCommandAccess(i, "sm_chat", ADMFLAG_CHAT))
			{
				if (g_DoColor)
				{
					PrintToChat(i, "\x04(ADMINS) %s: \x04%s", name, message);
				}
				else
				{

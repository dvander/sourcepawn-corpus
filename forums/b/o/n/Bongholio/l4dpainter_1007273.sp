...
public OnPluginStart()
{
	...
	Rochelle = CreateConVar("sm_l4d_painter_rochelle", Red,"RGB Value for rochelle. Usage: \"R G B\" or \"-1 -1 -1\" to do nothing.", FCVAR_NOTIFY);
	Coach = CreateConVar("sm_l4d_painter_coach", Black,"RGB Value for coach. Usage: \"R G B\" or \"-1 -1 -1\" to do nothing.", FCVAR_NOTIFY);
	Nick = CreateConVar("sm_l4d_painter_nick", Blue,"RGB Value for nick. Usage: \"R G B\" or \"-1 -1 -1\" to do nothing.", FCVAR_NOTIFY);
	Ellis = CreateConVar("sm_l4d_painter_ellis", Green,"RGB Value for ellis. Usage: \"R G B\" or \"-1 -1 -1\" to do nothing.", FCVAR_NOTIFY);
...
stock Paint(client)
{
	if(IsClientInGame(client))
	{
		...
		else if(StrContains(Model, "producer", false) > -1)//Since you'll spawn only once as survivor

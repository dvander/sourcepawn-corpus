...
bool g_bHook[MAXROCKS], g_bReAllow[MAXPLAYERS + 1], g_bFinale, g_bLinux, g_bHookMolotov;
		...
		switch (iType)
		{
			case Fire:
			{
				...
				for (int i; i < g_iMolotovCount; i++)
				{
					...
					{
						g_bHookMolotov = true;
						L4D2_RunScript("DropFire(Vector(%f %f %f))", vOrigin[0], vOrigin[1], vOrigin[2]);
						continue;
					}
...
public void OnEntityCreated (int entity, const char[] clsname)
{
	...
	else if (strcmp(clsname, "molotov_projectile") == 0 && g_bHookMolotov)
	{
		g_bHookMolotov = false;
...
public void nextFrame(int entity)
{
	...
	vAng[0] += GetRandomFloat(-360.0, 360.0);
	vAng[1] += GetRandomFloat(-360.0, 360.0);
	vAng[2] += GetRandomFloat(-360.0, 360.0);
	
	vVel[0] = GetRandomFloat(-600.0, 600.0);
	vVel[1] = GetRandomFloat(-600.0, 600.0);
	vVel[2] = GetRandomFloat(-450.0, 450.0);
...
public void OnPluginStart()
{
	...
	g_bLinux = PrepSDKCall_SetFromConf(hGameConf, SDKConf_Signature, "CMolotovProjectile::Create");

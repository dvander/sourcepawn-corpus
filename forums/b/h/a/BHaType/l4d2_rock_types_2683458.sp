...
int g_iRocks[MAXROCKS], g_iHookCommonState, g_iThrower;
...
public void eSpawn (int entity)
{
	if ((entity = EntRefToEntIndex(entity)) > MaxClients && IsValidEntity(entity))
	{
		...
		if ((client = GetClientOfUserId(g_iThrower)) > 0 && IsClientInGame (client) && rType == Fire && !!!(GetEntityFlags(client) & FL_ONFIRE))
			rType = INVALID_TYPE;
...
{
	CreateTimer (0.2, tThrower, event.GetInt("userid"));
}

public Action tThrower (Handle timer, int index)
{
	g_iThrower = index;
}

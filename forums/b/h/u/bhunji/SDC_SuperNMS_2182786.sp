...
public Plugin:myinfo = {
	name = "Super NMS Beta 超級生存測試版",
	author = "Tast - SDC",
	description = "NMS Map Parameters editor 生存地圖修改器",
	version = "0.23",
...
public OnPluginStart(){
	...
	if(GetLanguageByCode("zho") != -1 && GetServerLanguage() == GetLanguageByCode("zho")){
		zho = 1
		
		g_nms_Enable 	= CreateConVar("nms_enable", 		"0",	"nms地圖修改器使用開關")
		g_nms_endwave 	= CreateConVar("nms_endwave", 		"0",	"nms地圖最多有多少波數")
		g_nms_increase 	= CreateConVar("nms_increase", 		"0",	"nms地圖每波增加僵屍數")
		g_nms_resupply 	= CreateConVar("nms_resupply", 		"0",	"nms地圖每幾波資源投放")
		g_nms_initial 	= CreateConVar("nms_initial", 		"0",	"nms地圖第一波僵屍數量")
		g_nms_runner 	= CreateConVar("nms_runner", 		"0",	"nms地圖第幾波跑者出現")
		g_nms_child 	= CreateConVar("nms_child", 		"0",	"nms地圖第幾波小孩出現")
		g_nms_keypad 	= CreateConVar("nms_keypad", 		"0",	"nms地圖密碼鎖自動解開")
		g_nms_health	= CreateConVar("nms_health",		"0",	"nms地圖醫箱全自動放置")
		g_nms_extract	= CreateConVar("nms_extract",		"0",	"nms地圖自動傳送逃脫點")
		g_nms_custom	= CreateConVar("nms_custom",		"0",	"nms地圖可使用自訂戰役")
		g_nms_runner_chance	= CreateConVar("nms_runner_chance",	"0.0",	"nms地圖奔跑者出現機率") //或許無效
		
		RegAdminCmd("nms", Command_Test, ADMFLAG_ROOT,"顯示生存地圖參數");
		RegAdminCmd("nms_win", Command_CustomWinComplete, ADMFLAG_ROOT,"強制自訂戰役勝利");
		RegAdminCmd("nms_fail", Command_CustomWinFailed, ADMFLAG_ROOT,"強制自訂戰役失敗");
	}
	else {
		zho = 0
...
public Action:Command_CustomWinComplete(id, Args){
	if(!IsMapNMS()){
		if(zho) PrintToChat(id,"此地圖並非生存，指令停用.")
...
public Action:Command_Test(id, Args){
	...
	if(zho){
		PrintToConsole(id,"NMS生存地圖內部資料")
		
		PrintToConsole(id,"警衛隊降落頻率:%d",GetEntProp(EntOWC, Prop_Data, "m_iNGFreq",4)) 
			//National Guard drop occurs once every this many waves.
		PrintToConsole(id,"資源投放的頻率:%d",GetEntProp(EntOWC, Prop_Data, "m_iResupplyFreq",4)) 		
			//resupply_freq//
		PrintToConsole(id,"僵屍開場的數量:%d",GetEntProp(EntOWC, Prop_Data, "m_iInitialSpawn",4)) 		
			//initial_spawn//
		PrintToConsole(id,"僵屍每波增加數:%d",GetEntProp(EntOWC, Prop_Data, "m_iZombieIncrement",4)) 	
			//zombie_increment//
		PrintToConsole(id,"跑者第幾波出現:%d",GetEntProp(EntOWC, Prop_Data, "m_iFirstRunnerWave",4)) 	
			//first_runner_wave//
		PrintToConsole(id,"小孩第幾波出現:%d",GetEntProp(EntOWC, Prop_Data, "m_iFirstChildWave",4)) 	
			//first_child_wave//
		PrintToConsole(id,"使用自訂戰役？:%d",GetEntProp(EntOWC, Prop_Data, "_useCustomWinScenario",1)) 
			//use_custom_win
		PrintToConsole(id,"地圖總共有幾波:%d",GetEntProp(EntOWC, Prop_Data, "m_iEndWave",4)) 			
			//ending_wave//
		PrintToConsole(id,"跑者出現總機率:%f",GetEntPropFloat(EntOWC, Prop_Data, "_max_runner_chance",4)) 
			//_max_runner_chance//
		PrintToChat(id,"NMS生存地圖內部資料已顯示在控制台.")
	}
	else {
		PrintToConsole(id,"NMS Map Parameters")
		
		PrintToConsole(id,"National Guard drop frequence:%d",GetEntProp(EntOWC, Prop_Data, "m_iNGFreq",4)) 
			//?  National Guard drop occurs once every this many waves.
		PrintToConsole(id,"resupply frequence:%d"	,GetEntProp(EntOWC, Prop_Data, "m_iResupplyFreq",4)) 
			//resupply_freq//
		PrintToConsole(id,"Zombie initial spawn:%d"	,GetEntProp(EntOWC, Prop_Data, "m_iInitialSpawn",4)) 
			//initial_spawn//
		PrintToConsole(id,"Zombie increment:%d"		,GetEntProp(EntOWC, Prop_Data, "m_iZombieIncrement",4)) 
			//zombie_increment//
		PrintToConsole(id,"first runner wave:%d"	,GetEntProp(EntOWC, Prop_Data, "m_iFirstRunnerWave",4)) 
			//first_runner_wave//
		PrintToConsole(id,"first child wave:%d"		,GetEntProp(EntOWC, Prop_Data, "m_iFirstChildWave",4)) 
			//first_child_wave//
		PrintToConsole(id,"use custom win:%d"		,GetEntProp(EntOWC, Prop_Data, "_useCustomWinScenario",1)) 
			//use_custom_win
		PrintToConsole(id,"ending wave:%d"			,GetEntProp(EntOWC, Prop_Data, "m_iEndWave",4)) 
			//ending_wave//
		PrintToConsole(id,"max runner chance:%f"	,GetEntPropFloat(EntOWC, Prop_Data, "_max_runner_chance",4)) 
			//_max_runner_chance//
...
public OnStates3(){
	...
	if(GetConVarInt(g_nms_health)){
		...
		while((nmrih_health_station_location = Entity_FindByClassName(nmrih_health_station_location, "nmrih_health_station_location")) != INVALID_ENT_REFERENCE){
			...
			if(GetEntProp(nmrih_health_station_location, Prop_Data, "_health",4) != 0) return
			decl Float:ClientOrigin[3]
	...
	Endwave = GetEntProp(Ent_overlord_wave_controller, Prop_Data, "m_iEndWave",4)

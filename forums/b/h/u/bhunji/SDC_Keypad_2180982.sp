...
public OnPluginStart(){
	...
	HookConVarChange(	CreateConVar("sdn_showpass", 		"1",	"Show code to everyone",FCVAR_NOTIFY)	, Cvar_PassShow);
	HookConVarChange(	CreateConVar("sdn_allpass", 		"0",	"any code can pass")					, Cvar_AllPass);
	HookConVarChange(	CreateConVar("sdn_cospass", 		"1",	"change keypad code")					, Cvar_CosPass);
	
	HookEvent("keycode_enter", Event_Keycode_Enter,EventHookMode_Pre);
	
	RegAdminCmd("setcode", Command_SetCode, ADMFLAG_ROOT,"Set code to kaypad");
	RegAdminCmd("getcode", Command_GetCode, ADMFLAG_ROOT,"Get code from kaypad");
}

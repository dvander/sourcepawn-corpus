...
public OnPluginStart() {
	...
	g_hTime = CreateConVar("sm_scheduledshutdown_time", "0500", "Time to shutdown server.", FCVAR_PLUGIN);
...
public Action:CheckTime(Handle:timer, any:useless) {
	if(g_bEnabled) {
		...
		if(time>=g_iTime && time<=g_iTime+1) {
			if (GetConVarInt(g_hEnabledHint) >= 1){PrintHintTextToAll("Restarting server in 10 seconds...");}
			if (GetConVarInt(g_hEnabledChat) >= 1){PrintToChatAll("Restarting server in 10 seconds...");}
			if (GetConVarInt(g_hEnabledCenter) >= 1){PrintCenterTextAll("Restarting server in 10 seconds...");}
			LogAction(0, -1, "Server shutdown warning.");
			CreateTimer(10.0, ShutItDown, _, TIMER_REPEAT);
		}
	}
}

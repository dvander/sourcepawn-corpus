...
{
	CreateConVar("autohelp_version", PLUGIN_VERSION, "", 0|FCVAR_DONTRECORD);
	
	autohelp_bots = CreateConVar("autohelp_bots", "1", "Allow AutoHelp for survivor bots? 0:No, 1:Yes", 0|FCVAR_SPONLY|FCVAR_NOTIFY);
	
	autohelp_incap = CreateConVar("autohelp_incap", "1", "Allow AutoHelp for incapacitation? 0:No, 1:Yes", 0|FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_incap_delay = CreateConVar("autohelp_incap_delay", "3", "AutoHelp delay for incapacitation", 0|FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_incap_duration = CreateConVar("autohelp_incap_duration", "4", "AutoHelp duration for incapacitation (setting higher than 5 will disable animation)", 0|FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_incap_health = CreateConVar("autohelp_incap_health", "30", "Health buffer after AutoHelp from incapacitation", 0|FCVAR_SPONLY|FCVAR_NOTIFY);
	
	autohelp_ledge = CreateConVar("autohelp_ledge", "1", "Allow AutoHelp for ledge grabs? 0:No, 1:Yes", 0|FCVAR_SPONLY|FCVAR_NOTIFY);		
	autohelp_ledge_delay = CreateConVar("autohelp_ledge_delay", "3", "AutoHelp delay for ledge grabs", 0|FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_ledge_duration = CreateConVar("autohelp_ledge_duration", "4", "AutoHelp duration for ledge grabs (setting lower than 4 will cause animation issues, higher than 5 will disable the animation)", 0|FCVAR_SPONLY|FCVAR_NOTIFY);
	
	autohelp_specials = CreateConVar("autohelp_specials", "1", "Allow AutoHelp for special infected holds?, 0:No, 1:Yes", 0|FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_specials_delay = CreateConVar("autohelp_specials_delay", "3", "AutoHelp delay for special infected holds", 0|FCVAR_SPONLY|FCVAR_NOTIFY);
	autohelp_specials_duration = CreateConVar("autohelp_specials_duration", "4", "AutoHelp duration for special infected holds", 0|FCVAR_SPONLY|FCVAR_NOTIFY);
	
	autohelp_specials_smoker_drag = CreateConVar("autohelp_specials_smoker_drag", "10", "Maximum time a survivor can be dragged before AutoHelp kicks in", 0|FCVAR_SPONLY|FCVAR_NOTIFY);
...
{
	if (GetConVarInt(autohelp_specials) == 0)
		{
			return;
		}
...
public Action:Timer_Delay(Handle:timer, any:client)
{
	...
	if (client)
	{
		...
		if ((HelpState[client] == STATE_NONE) && ((IncapType[client] == INCAP && GetConVarInt(autohelp_incap) != 0) || (IncapType[client] == INCAP_LEDGE && GetConVarInt(autohelp_ledge) != 0) || ((HoldType[client] == HOLD_GRAB || HoldType[client] == HOLD_POUNCE || HoldType[client] == HOLD_RIDE || HoldType[client] == HOLD_PUMMEL) && GetConVarInt(autohelp_specials) != 0)))
			if ((HoldType[client] == HOLD_GRAB || HoldType[client] == HOLD_POUNCE || HoldType[client] == HOLD_RIDE || HoldType[client] == HOLD_PUMMEL) && GetConVarInt(autohelp_specials) == 0)
			{
				return;
			}
			else
//			if (HasValidEnt(client, "m_tongueOwner")	|| HasValidEnt(client, "m_pounceAttacker")	|| (HasValidEnt(client, "m_jockeyAttacker"))	|| (HasValidEnt(client, "m_pummelAttacker")))
//			{
//				return;
//			}
//			else

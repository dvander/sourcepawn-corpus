...
public Plugin myinfo = 
{
	...
	author = "Mev, George, & Blacky & Slidy Edit & GAMMA CASE",
...
void DoPush(int entity, int other)
{
	if(0 < other <= MaxClients)
	{
		...
		if(GetEntityMoveType(other) == MOVETYPE_LADDER && !(GetEntProp(entity, Prop_Data, "m_spawnflags") & SF_TRIG_PUSH_AFFECT_PLAYER_ON_LADDER))
		{
			return;
		}
		...
		if(GetEntProp(entity, Prop_Data, "m_spawnflags") & SF_TRIG_PUSH_ONCE)
		{
			float newVelocity[3];
		...
		TeleportEntity(other, origin, NULL_VECTOR, newVelocity);
...
stock bool GetFilterTargetName(const char[] filtername, char[] buffer, int maxlen, bool &Negated = false, bool &isFilterClass = false)
{
	int filter = FindEntityByTargetname(filtername, isFilterClass);
	if(filter != -1)
	{
		if(isFilterClass)
			GetEntPropString(filter, Prop_Data, "m_iFilterClass", buffer, maxlen);
		else
			GetEntPropString(filter, Prop_Data, "m_iFilterName", buffer, maxlen);
		Negated = view_as<bool>(GetEntProp(filter, Prop_Data, "m_bNegated"));
...
stock int FindEntityByTargetname(const char[] targetname, bool &isFilterClass = false)
{
	int entity = -1;
	char sName[64];
	...
	while ((entity = FindEntityByClassname(entity, "filter_activator_class")) != -1)
	{
		GetEntPropString(entity, Prop_Data, "m_iName", sName, 64);
		if (StrEqual(sName, targetname))
			return entity;
...
stock bool DoesClientPassFilter(int entity, int client)
{
	char sPushFilter[64];
	...
	bool isFilterClass, Negated;
	char sFilterBuff[64],
		sClientBuff[64];
	
	if(!GetFilterTargetName(sPushFilter, sFilterBuff, sizeof sFilterBuff, Negated, isFilterClass))
		return true;
	
	if(isFilterClass)
		GetEntPropString(client, Prop_Data, "m_iClassname", sClientBuff, sizeof sClientBuff);
	else
		GetEntPropString(client, Prop_Data, "m_iName", sClientBuff, sizeof sClientBuff);
	
	return Negated ? !StrEqual(sFilterBuff, sClientBuff) : StrEqual(sFilterBuff, sClientBuff);
}
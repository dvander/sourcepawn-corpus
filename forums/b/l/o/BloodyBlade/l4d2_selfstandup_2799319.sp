...
public void OnPluginStart()
{
	...
	selfstandup_blackwhite		= CreateConVar("selfstandup_max",				"3",		"value only 1 and above = max incap count to black n white (off function = 9999 or what ever)", PLUGIN_FCVAR);
	selfstandup_kill			= CreateConVar("selfstandup_kill",				"1",		"0: Do not kill special infected when breaking free; 1: Kill special infected when breaking free", PLUGIN_FCVAR);	
	selfstandup_health_incap	= CreateConVar("selfstandup_health_incap",		"100.0",		"How much health after reviving from incapacitation.", PLUGIN_FCVAR);
	selfstandup_duration		= CreateConVar("selfstandup_duration",			"2.0",		"Min:0, Max: 5.0, Self stand up Duration", PLUGIN_FCVAR);
	selfstandup_crawl			= CreateConVar("selfstandup_crawl",				"1",		"0: Off, 1:on,  Allow player crawling on incap)", PLUGIN_FCVAR);
	selfstandup_crawl_speed		= CreateConVar("selfstandup_crawl_speed",		"50",		"How fast player crawl when incap)", PLUGIN_FCVAR);
	...
	selfstandup_costly_item		= CreateConVar("selfstandup_costly_item",		"med_kit,pills,adrenaline",	"List of item allowed sparated by comma, 'selfstandup_costly' must on (med_kit, pills, adrenaline, defibrillator, incendiary, explosive)", PLUGIN_FCVAR);
	selfstandup_clearance		= CreateConVar("selfstandup_clearance",			"0",	"0: Off,   200.0: on, max radius scan range (only allow incap player to get up if player at this range from zombie and SI)", PLUGIN_FCVAR);
	...
	selfstandup_color			= CreateConVar("selfstandup_color",				"0",		"0: Off,   1: on, set player color on last life", PLUGIN_FCVAR);

	selfstandup_enable_incap_pickup	= CreateConVar("selfstandup_enable_incap_pickup", "1", "0: Off,   1: On, Allow picking up items in incapacity", PLUGIN_FCVAR);
	selfstandup_incap_pickup_distance = CreateConVar("selfstandup_incap_pickup_distance", "101.8", "Use pickup distance limit", PLUGIN_FCVAR);
	selfstandup_incap_pickup_scavenge_item = CreateConVar("selfstandup_incap_pickup_scavenge_item", "0", "0:Disable pickup scavenge items, 1:Allow pickup gascans, 2: Allow pickup colas, 3:All", PLUGIN_FCVAR);
...
void UsePackToKill(int client)
{
	if ( PlayerWeaponSlot[client] != -1 && IsValidEntity( PlayerWeaponSlot[client] ))
	{
		...
		if ( StrEqual( slotName, "weapon_adrenaline", false ))
		    Format( slotName, sizeof( slotName ), "Adrenaline" );

...
void OnArenalineUsed(Event event, const char[] name, bool dontBroadcast)
{
...
void OnPillsUsed(Event event, const char[] name, bool dontBroadcast)
{
	int	healer = GetClientOfUserId(event.GetInt("userid"));
	if (isHumanSurvivor(healer))
	{
...
{
	if (isHumanSurvivor(client) && buff_actions & (1 << MeleeSwinging) && Terror_GetAdrenalineTime(client) > 0)
	{
		if (mode_melee)
		{
...
{
	if (isHumanSurvivor(client) && buff_actions & (1 << Reloading) && Terror_GetAdrenalineTime(client) > 0)
	{
...
{
	if (isHumanSurvivor(client) && buff_actions & (1 << Firing) && Terror_GetAdrenalineTime(client) > 0)
	{
		speedmodifier *= speed_rate;
	}
}
...
{
	if (isHumanSurvivor(client) && buff_actions & (1 << Deploying) && Terror_GetAdrenalineTime(client) > 0)
	{
		speedmodifier *= speed_rate;
	}
}
...
{
	if (isHumanSurvivor(client) && buff_actions & (1 << Throwing) && Terror_GetAdrenalineTime(client) > 0)
	{
		speedmodifier *= speed_rate;
	}
}

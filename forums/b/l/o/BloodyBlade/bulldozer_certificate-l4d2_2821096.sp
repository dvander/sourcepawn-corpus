...
static bool KillChargerTimer = false, AlreadyCarrying = false, HasMadeImpact = false;
...
Action OnChargerChargeStart(Event event, char[] event_name, bool dontBroadcast)
{
...
Action OnChargerCarryStart(Event event, char[] event_name, bool dontBroadcast)
{
	AlreadyCarrying = true;
	return Plugin_Continue;
}

Action OnChargerCarryEnd(Event event, char[] event_name, bool dontBroadcast)
{
	AlreadyCarrying = false;
	return Plugin_Continue;
}

Action OnPlayerDeath(Event event, char[] event_name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (IsValidClient(client))
	{
		if(GetClientTeam(client) == 3 && GetEntProp(client, Prop_Send, "m_zombieClass") == 6)
		{
			...
			{
				if (ReinCapTimerArray[target] != null) delete ReinCapTimerArray[target];
...
Action OnChargerChargeEnd(Event event, char[] event_name, bool dontBroadcast)
{
	HasMadeImpact = true;
	AlreadyCarrying = false;
	KillChargerTimer = true;
...
public void OnMapEnd()
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i))
		{
			if(ReinCapTimerArray[i] != null)
			{
				delete ReinCapTimerArray[i];
			}
		}
	}
}

Action BC_CheckForIncapped(Handle timer, any client)
{
...
Action BC_Reincap(Handle timer, any client)
{
	ReinCapTimerArray[client] = null;
	if (IsValidEntity(client)) SetEntProp(client, Prop_Send, "m_isIncapacitated", true);
	CreateTimer(HEALTH_SET_DELAY, BC_SetHealthDelayed, client);
	return Plugin_Stop;
}

Action BC_SetHealthDelayed(Handle timer, any client)
{
...
Action BC_WipeHealthArray(Handle timer, any client)
{
	for (int i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i))

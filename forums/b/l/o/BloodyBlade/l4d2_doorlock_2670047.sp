...
int spawnWaitTime, countDown, clientTimeout[MAXPLAYERS + 1];
int ent_safedoor;
int ent_safedoor_check;
int indexClientReady[MAXPLAYERS + 1];
// 0=spectator, 1 = unready, 2=ready

bool isFirstRound;
bool isFreezeAllowed;
bool isInvulnerable;
bool isTheDoorBreakeble;
bool isTheDoorOpened;
bool isReadyUpMode;
bool isClientLoading[MAXPLAYERS + 1];
bool isClientSpawning[MAXPLAYERS + 1];
...
public void OnPluginStart()
{
	...
	cvarInvulnerable = CreateConVar("l4d2_dlock_godmode", "0", "If enabled, survivors invulnerable while they are in saferoom.");
	...
	cvarPrepareTime2r = CreateConVar("l4d2_dlock_prepare2nd", "20", "How many seconds plugin will wait after all clients have loaded before starting second round on a map");
	...
	cvarClientTimeOut = CreateConVar("l4d2_dlock_timeout", "45", "How many seconds plugin will wait after a map starts before giving up on waiting for a client");
	...
	cvarGameModeEnabled = CreateConVar("l4d2_dlock_gamemodeactive", "coop,versus,teamversus,mutation13", "Set the game mode for which the plugin should be activated");
...
public void DL_Event_RoundStart(Event event, const char[] name, bool dontBroadcast)
{
...
public Action PluginStartSequence01(Handle timer)
{
	if (isGamemode())
	{
		if(GetConVarBool(cvarReadyUpMode)) isReadyUpMode = true;
...
public Action PluginStartSequence02(Handle timer)
{
	...
	if(GetConVarBool(cvarBreakTheDoor) && (ent_safedoor > 0)) isTheDoorBreakeble = true;

	if(GetConVarBool(cvarInvulnerable)) isInvulnerable = true;
...
public Action DL_ClientReady(int client, int args)
{
...
public Action DL_ClientUnready(int client, int args)
{
...
public void DL_Event_Join_Team(Event event, char[] event_name, bool dontBroadcast)
{
...
public void DL_Event_RoundEnd(Event event, const char[] name, bool dontBroadcast)
{
	if (isGamemode())
	{
...
public Action DL_Event_BotPlayerReplace(Event event, const char[] name, bool dontBroadcast)
{
...
public Action DL_Event_PlayerBotReplace(Event event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "player"));
...
public void DL_Event_GhostSpawnTime(Event event, const char[] name, bool dontBroadcast)
{
	if (GetConVarBool(cvarWaitForInfected))
	{
		int userid = GetEventInt(event, "userid");
		int time = GetEventInt(event, "spawntime");
...
public void DL_Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	int userid = GetEventInt(event, "userid");
	int client = GetClientOfUserId(userid);
...
public void DL_Event_PlayerFirstSpawn(Event event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (GetConVarBool(cvarWaitForInfected))
	{
...
void ShowStatusPanel()
{
	...
	{
		if (statusPanel != INVALID_HANDLE) CloseHandle(statusPanel);

		statusPanel = CreatePanel();

		char readyPlayers[1024];

		int timelimit = GetConVarInt(cvarClientTimeOut);
		...
		for(i = 1; i <= MaxClients; i++) 
		{
			if(IsClientConnected(i) && !IsFakeClient(i)) SendPanelToClient(statusPanel, i, blankhandler, 5);
		}
	}
}
...
public void DL_Event_PlayerLeftSstartArea(Event event, const char[] name, bool dontBroadcast)
{
	isInvulnerable = false;
}

public void DL_Event_PlayerHurt(Event event, const char[] name, bool dontBroadcast)
{
	if (isInvulnerable)
	{
		int userid0 = GetEventInt(event, "userid");
		int health = GetEventInt(event, "health");
		int dmg_health = GetEventInt(event, "dmg_health");
		int victim = GetClientOfUserId(userid0);
		SetEntityHealth(victim, health + dmg_health);
	}
}

public Action DL_Event_DoorOpen(Event event, const char[] name, bool dontBroadcast)
{
	if (ent_safedoor > 0)
	{
		if (GetEventBool(event, "checkpoint"))
		{
			...
			if (GetConVarBool(cvarStartPrevent)) isTheDoorOpened = true;
		}
	}
}
...
{
	if ((ent_safedoor > 0) && !isTheDoorOpened) return Plugin_Handled;

...
enum struct PluginData
{
    ...
    {
        this.cvars.Init();
        this.GD = new GameData("funcommands.games");
    }
...
public void OnMapStart()
{
	...
	if(plugin.GD.GetKeyValue("SpriteBeam", buffer, PLATFORM_MAX_PATH))
	{
		plugin.hBeam = PrecacheModel(buffer);
	}
	else
	{
		SetFailState("Can't find key 'SpriteBeam' in the funcommands.games.txt");
	}
}
...
Action ShowEffect(Handle timer, DataPack data)
{
    ...
    if(EntOk(ent))
    {
        if(starttime < endtime)
        {
            ...
            TE_SetupBeamRingPoint(pos, 1.0, 175.0, plugin.hBeam, 0, 0, 15, 0.5, 5.0, 0.0, {0,255,0,255}, 10, 0);
            TE_SendToAll();
            return Plugin_Continue;
        }
        else

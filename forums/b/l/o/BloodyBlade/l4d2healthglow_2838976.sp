...
static bool	g_isGlowDisabled = false, g_isPluginEnding = false, g_isIT[MAXPLAYERS + 1] = {false, ...};
static int  g_maxIncaps = 2;
static float g_fCvar_pain_pills_decay_rate = 0.0;

#if defined _LMCCore_included
bool bLMCCoreLib = false;
...
public void OnPluginStart()
{
	...
	FindConVar("survivor_max_incapacitated_count").AddChangeHook(OnIncapMax_ConVarChange);
	FindConVar("sv_disable_glow_survivors").AddChangeHook(OnGlowDisable_ConVarChange);
	FindConVar("pain_pills_decay_rate").AddChangeHook(PainPillsDecayRate_ConVarChange);
	...
	if(bLateload)
	{
		bLMCCoreLib = LibraryExists("LMCCore");
	}
	#endif
}
...
{
    g_maxIncaps = FindConVar("survivor_max_incapacitated_count").IntValue;
    g_isGlowDisabled = FindConVar("sv_disable_glow_survivors").BoolValue;
    g_fCvar_pain_pills_decay_rate = FindConVar("pain_pills_decay_rate").FloatValue;
...
void OnIncapMax_ConVarChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
    g_maxIncaps = convar.IntValue;
}

void OnGlowDisable_ConVarChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
    g_isGlowDisabled = convar.BoolValue;
}

void PainPillsDecayRate_ConVarChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
    g_fCvar_pain_pills_decay_rate = convar.FloatValue;
}

Action UpdateGlows_Timer(Handle timer)
{
    if (g_isPluginEnding)
    {
        return Plugin_Stop;
...
void UpdateGlow_UserId_Event(Event event, const char[] name, bool dontBroadcast)
{
...
void UpdateGlow_Subject_Event(Event event, const char[] name, bool dontBroadcast)
{
...
void UpdateGlow_Victim_Event(Event event, const char[] name, bool dontBroadcast)
{
...
void UpdateGlow_NowIT_Event(Event event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (IsValidSurvivor(client))
	{
...
void UpdateGlow_NoLongerIt_Event(Event event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (IsValidSurvivor(client))
	{
...
void UpdateGlow_Idle_Event(Event event, const char[] name, bool dontBroadcast)
{
...
{
	if (g_isPluginEnding || !IsValidClient(client))
	{
		return;
	}
	...
	if(bLMCCoreLib)
	{
		...
		if(iOverlayModel > -1)
		{
			L4D2_SetEntityGlow(iOverlayModel, type, range, minRange, color, flashing);
		}
	}
	#else
...
{
    return view_as<int>(GetEntProp(client, Prop_Send, "m_currentReviveCount"));
}
...
{
	return IsValidClient(client) && GetClientTeam(client) == L4DTeam_Survivor;
}

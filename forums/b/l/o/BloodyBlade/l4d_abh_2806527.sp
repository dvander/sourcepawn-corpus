...
bool
    g_bABHLock[MAXPLAYERS + 1] = {false, ...},
    g_bABHReset[MAXPLAYERS + 1] = {false, ...},
	bABHEnable;

float
    g_fSurABHSpeed[MAXPLAYERS + 1] = {0.0, ...},
    g_fSurLastSpeed[MAXPLAYERS + 1] = {0.0, ...},
	fABHLimit;
...
{
    g_hABHEnable = CreateConVar("l4d_abh_enable", "1", "ABH 开关", FCVAR_NOTIFY, true, 0.0, true, 1.0);
    g_hABHEnable.AddChangeHook(ConVarChanged);
    g_hABHLimit = CreateConVar("l4d_abh_maxspeed", "2000", "ABH 最大空速", FCVAR_NOTIFY, true, 0.0, true, 9999.0);
    g_hABHLimit.AddChangeHook(ConVarChanged);
...
void GetConVars()
{
    bABHEnable = g_hABHEnable.BoolValue;
    fABHLimit = g_hABHLimit.FloatValue;
}

// ABH加速
public Action OnPlayerRunCmd(int client, int& buttons, int& impulse, float vel[3], float ang[3])
{
    if(!IsSurvivor(client) || !IsPlayerAlive(client) || !bABHEnable)
    ...
    else if(fABHLimit > FloatAbs(g_fSurABHSpeed[client]))

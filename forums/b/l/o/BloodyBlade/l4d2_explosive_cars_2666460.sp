...
public void OnPluginStart()
{
	...
	if (!StrEqual(sGame, "left4dead2", false)) SetFailState("Explosive Cars supports Left 4 dead 2 only!");
	...
	g_cvarDamage = CreateConVar("l4d2_explosive_cars_damage", "100", "Damage made by the explosion", FCVAR_NOTIFY);
	...
	g_cvarPanicChance = CreateConVar("l4d2_explosive_cars_panic_chance", "1", "Chance that the cars explosion might call a horde (1 / CVAR) [1: Always]", FCVAR_NOTIFY);
	...
	g_cvarBurnTimeout = CreateConVar("l4d2_explosive_cars_burntime", "0", "Time to wait before stopping the fire on the exploded car (0: Don't stop)", FCVAR_NOTIFY);
	g_cvarUnload = CreateConVar("l4d2_explosive_cars_unload", "none", "On which maps should the plugin disable itself? (Example: c5m3_cemetery, c5m5_bridge, cmdd_custom)", FCVAR_NOTIFY);
...
public void OnEntityCreated(int entity, const char[] classname)
{
	...
	|| StrEqual(classname, "prop_physics_override") && !g_bHooked[entity])
	{
...
public void OnTakeDamagePost(int victim, int attacker, int inflictor, float damage, int damagetype)
{
	...
	else if(StrEqual(class2, "env_explosion") && !g_cvarExplosionDmg)
	{
		damage = 0.0;
	}
	else if((StrEqual(class2, "player") && attacker > 0 && attacker <= MaxClients && IsClientInGame(attacker) && GetClientTeam(attacker) == 3) && !g_cvarInfected)
	{
		damage = 0.0;
	}
	else if(StrEqual(class2, "tank_rock") || StrEqual(class2, "weapon_tank_claw"))
	{
		...
		if(tank_damage > 0.0 && g_cvarInfected)
		{
			damage = tank_damage;
		}
	}
	...
	//PrintHintTextToAll("%i damaged by <%s>(%i) for %f damage [%i | %i]", victim, class2, attacker, damage, tdamage, g_cvarMaxHealth.IntValue); //TEST
...
void CreateExplosion(float carPos[3])
{
	...
	CreateTimer(g_cvarTrace.FloatValue + 1.5, timerDeleteParticles, pack2, TIMER_FLAG_NO_MAPCHANGE);

	DataPack pack = CreateDataPack();
	WritePackCell(pack, exTrace);
	WritePackCell(pack, exHurt);
	CreateTimer(g_cvarTrace.FloatValue, timerStopFire, pack, TIMER_FLAG_NO_MAPCHANGE);
...
public Action timerDeleteParticles(Handle timer, DataPack pack2)
{
	if(g_bDisabled) return;

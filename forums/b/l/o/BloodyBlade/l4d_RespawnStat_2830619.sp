...
ConVar 	g_hCvarEnable, g_hCvarTeamReq;
int   g_iTeamReq = 0, g_iPlayerData[MAXPLAYERS + 1][sizeof(g_sPlayerSave)], g_iPlayerData_L4d2[MAXPLAYERS + 1][sizeof(g_sPlayerSave_L4d2)], g_iUserIdBind[MAXPLAYERS + 1] = {0, ...};
float g_fPlayerData[MAXPLAYERS + 1][2];
bool g_bEnabled = false, g_bLockSave[MAXPLAYERS + 1] = {false, ...}, g_bDead[MAXPLAYERS + 1] = {false, ...}, g_bLeft4Dead2 = false, bHooked = false;
...
public void OnPluginStart()
{
	...
	g_hCvarTeamReq.AddChangeHook(ConVarChanged);
...
Action Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	if( client > 0 && IsTeamComply(client))
	{
		...
		if (g_bDead[client])
		{
			...
			CreateTimer(1.0, Timer_LoadStatDelayed, client, TIMER_FLAG_NO_MAPCHANGE);
		}
	}
	return Plugin_Continue;
}

Action Timer_LoadStatDelayed(Handle timer, int UserId)
{
	int client = GetClientOfUserId(UserId);
	if(client > 0 && IsClientInGame(client) && IsPlayerAlive(client) && g_iUserIdBind[client] == client)
	{
		LoadStats(client);
	}

...
ConVar Ladder_intensity;	float fIntensityBoost = 0.0;
int m_flFallVelocity = 0, m_vecMins = 0, m_vecMaxs = 0;
...
public void OnPluginStart()
{
	...
	(PluginOn =			CreateConVar(PLUGIN_NAME ... "_enable", "1",				"plugin on/off", FCVAR_NOTIFY)).AddChangeHook(OnConVarPluginOnChanged);
	(Speed_rate =		CreateConVar(PLUGIN_NAME ... "_speed", "1.5",				"buff speed rate 2:double speed", FCVAR_NOTIFY)).AddChangeHook(OnConVarChanged);
	(Buff_actions =		CreateConVar(PLUGIN_NAME ... "_actions", "-1",				"buff actions 1=Firing 2=Deploying 4=Reloading 8=MeleeSwinging 16=Throwing", FCVAR_NOTIFY)).AddChangeHook(OnConVarChanged);
	(Announce_type =	CreateConVar(PLUGIN_NAME ... "_announces", "2",				"1=center 2=chat 4=hint 7=all add together you want", FCVAR_NOTIFY)).AddChangeHook(OnConVarChanged);
	(Speed_rate_melee =	CreateConVar(PLUGIN_NAME ... "_speed_melee", "2.22",		"speed rate of melee swing 2=double", FCVAR_NOTIFY)).AddChangeHook(OnConVarChanged);
	(Mode_melee =		CreateConVar(PLUGIN_NAME ... "_actions_melee", "1",			"alternative mode of melee swing 0=WeaponHandling API 1=NetProp flNextAttackTime", FCVAR_NOTIFY)).AddChangeHook(OnConVarChanged);
	(Ladder_intensity = CreateConVar(PLUGIN_NAME ... "_ladder_intensity", "0.5", 	"Boost intensity", FCVAR_NOTIFY)).AddChangeHook(OnConVarChanged);

	m_flFallVelocity = FindSendPropInfo("CTerrorPlayer", "m_flFallVelocity");
	m_vecMins = FindSendPropInfo("CTerrorPlayer", "m_vecMins");
	m_vecMaxs = FindSendPropInfo("CTerrorPlayer", "m_vecMaxs");
...
void OnConVarPluginOnChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
	IsAllowed();
}
...
void OnConVarChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
	...
	fIntensityBoost = Ladder_intensity.FloatValue * 0.01;
}
...
void IsAllowed()
{
	...
	if(!bHooked && bPluginOn)
	{
		...
		HookEvent("adrenaline_used", Events, EventHookMode_Post);
		HookEvent("pills_used", Events, EventHookMode_Post);
	}
...
{
	if (strcmp(name, "adrenaline_used") == 0)
	{
...
{
	if (bHooked && isHumanSurvivor(client) && buff_actions & (1 << MeleeSwinging) && Terror_GetAdrenalineTime(client) > 0)
	{
		if (mode_melee)
		{
...
{
	if (bHooked && isHumanSurvivor(client) && buff_actions & (1 << Reloading) && Terror_GetAdrenalineTime(client) > 0)
	{
...
{
	if (bHooked && isHumanSurvivor(client) && buff_actions & (1 << Firing) && Terror_GetAdrenalineTime(client) > 0)
	{
		speedmodifier *= speed_rate;
	}
}
...
{
	if (bHooked && isHumanSurvivor(client) && buff_actions & (1 << Deploying) && Terror_GetAdrenalineTime(client) > 0)
	{
		speedmodifier *= speed_rate;
	}
}
...
{
	if (bHooked && isHumanSurvivor(client) && buff_actions & (1 << Throwing) && Terror_GetAdrenalineTime(client) > 0)
	{
		speedmodifier *= speed_rate;
	}
}
...
public void OnPlayerRunCmdPost(int client, int buttons, int impulse, const float vel[3], const float angles[3], int weapon, int subtype, int cmdnum, int tickcount, int seed, const int mouse[2]) 
{
	...
	if(MOVETYPE_LADDER & GetEntityMoveType(client) &~ MOVETYPE_NOCLIP) 
	{
		...
		if(Terror_GetAdrenalineTime(client) > 0)
		{
			static float fFallVel, fOrigin[3];
			fFallVel = -GetEntDataFloat(client, m_flFallVelocity);
			if(FloatAbs(fFallVel) < 10.0) return;
			fFallVel *= fIntensityBoost;
			GetClientAbsOrigin(client, fOrigin);
			if(IsPlayerStuck(client, fOrigin, fFallVel)) return;
			fOrigin[2] += fFallVel;

...
ConVar sm_witch_darkness_enable, sm_witch_darkness_duration, sm_witch_darkness_horde, sm_witch_darkness_shake, sm_witch_darkness_onlykiller, disable_glow_faritems, disable_glow_survivors; 
int visibility = 0, killer = 0;
...
public void OnPluginStart()
{
	...
	sm_witch_darkness_duration	 = CreateConVar("sm_witch_darkness_duration", "20", "Durtion of blind effect.", CVAR_FLAGS);
...
*******************************************************/
Action Event_Witch_Death(Event event, const char[] name, bool dontBroadcast)
{
	if(!sm_witch_darkness_enable.BoolValue) return Plugin_Continue;
...
Action FadeinTimer(Handle Timer)
{
	visibility -= 8;
...
*	Gimmick functions
*******************************************************/
stock void FakeRealism(bool mode)
{
	if(mode == true)
	{
...
stock void ScreenShake(int target)
{
	if(IsValidSurvivor(target))
	{

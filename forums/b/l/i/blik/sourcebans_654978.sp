...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_unban", CommandUnban, ADMFLAG_UNBAN, "sm_unban <steamid|ip>");
...
public MenuHandler_BanPlayerList(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)			// Chose someone!
	{
		...
		{
			PrintToChat(param1, "%s%t", "Player no longer available", Prefix);
		}
		else if (!CanUserTarget(param1, target))
		{
			PrintToChat(param1, "%s%t", "Unable to target",           Prefix);
		}
		else
		{
	...
	if(loadAdmins && enableAdmins)
	{
		if( serverID == -1 )
		{
			...
			FormatEx(query,1024,"SELECT authid, srv_password, srv_group, srv_flags, user, immunity  \
						...
						GROUP BY aid, authid, srv_password, srv_group, srv_flags, user",
					DatabasePrefix,DatabasePrefix, serverID, DatabasePrefix, serverID);
		}
		curLoading++;
...
public ServerInfoCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	{	
		...
		FormatEx(query, sizeof(query), "INSERT INTO %s_servers (ip, port, modid) VALUES ('%s', '%s', (SELECT mid FROM %s_mods WHERE modfolder = '%s'))", DatabasePrefix, ServerIp, ServerPort, DatabasePrefix, desc);
		SQL_TQuery(Database, ErrorCheckCallback, query);
	}
}
...
public OnClientAuthorized(client, const String:auth[])
{
	...
	FormatEx(Query, sizeof(Query), "SELECT bid FROM %s_bans WHERE ((type = 0 AND authid = '%s') OR (type = 1 AND ip = '%s')) AND (length = '0' OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL", DatabasePrefix, auth, ip);
	...
	if(SQL_GetRowCount(hndl) > 0)
	{
		...
		if( serverID == -1 )
		{
			FormatEx(Query, sizeof(Query), "INSERT INTO %s_banlog (sid ,time ,name ,bid) VALUES  \
				...
				(SELECT bid FROM %s_bans WHERE ((type = 0 AND authid = '%s') OR (type = 1 AND ip = '%s')) AND RemoveType IS NULL LIMIT 0,1))", 
				DatabasePrefix, DatabasePrefix, ServerIp, ServerPort, Name, DatabasePrefix, clientAuth, clientIp);
		}
		else
		{
			FormatEx(Query, sizeof(Query), "INSERT INTO %s_banlog (sid ,time ,name ,bid) VALUES  \
				(%d, UNIX_TIMESTAMP(), '%s', \
				(SELECT bid FROM %s_bans WHERE ((type = 0 AND authid = '%s') OR (type = 1 AND ip = '%s')) AND RemoveType IS NULL LIMIT 0,1))", 
				DatabasePrefix, serverID, Name, DatabasePrefix, clientAuth, clientIp);
		}
...
UTIL_InsertBan(time, const String:Name[], const String:Authid[], const String:Ip[], const String:Reason[], const String:AdminAuthid[], const String:AdminIp[], Handle:Pack)
{
	...
	if( serverID == -1 )
	{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (ip, authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						('%s', '%s', '%s', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, '%d', '%s', (SELECT aid FROM %s_admins WHERE authid = '%s'), '%s', \
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1), ' ')", 
						DatabasePrefix, Ip, Authid, banName, (time*60), (time*60), banReason, DatabasePrefix, AdminAuthid, AdminIp, DatabasePrefix, ServerIp, ServerPort);
	}else{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (ip, authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						('%s', '%s', '%s', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, '%d', '%s', (SELECT aid FROM %s_admins WHERE authid = '%s'), '%s', \
						%d, ' ')", 
						DatabasePrefix, Ip, Authid, banName, (time*60), (time*60), banReason, DatabasePrefix, AdminAuthid, AdminIp, serverID);
	}
...
{
	decl admin, minutes, String:adminAuth[30], String:adminIp[30], String:banReason[128], String:ip[16], String:Query[512], String:reason[64];
	ResetPack(data);
	...
	if( serverID == -1 )
	{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (type, ip, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						(1, '%s', '', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, '%d', '%s', (SELECT aid FROM %s_admins WHERE authid = '%s'), '%s', \
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1), ' ')", 
						DatabasePrefix, ip, (minutes*60), (minutes*60), banReason, DatabasePrefix, adminAuth, adminIp, DatabasePrefix, ServerIp, ServerPort);
	} else {
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (type, ip, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						(1, '%s', '', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, '%d', '%s', (SELECT aid FROM %s_admins WHERE authid = '%s'), '%s', \
						%d, ' ')", 
						DatabasePrefix, ip, (minutes*60), (minutes*60), banReason, DatabasePrefix, adminAuth, adminIp, serverID);
	}
...
public InsertBanIpCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	new admin, minutes;
	decl String:reason[64], String:arg[30];
...
public Action:CommandUnban(client, args)
{
	if(args < 1)
	{
		ReplyToCommand(client, "%sUsage: sm_unban <steamid|ip>", Prefix);
		return Plugin_Handled;
	}
	...
	{
		Format(query, sizeof(query), "SELECT authid, aid FROM %s_bans WHERE (type = 0 AND authid = '%s') AND (length = '0' OR ends > UNIX_TIMESTAMP() AND RemoveType IS NULL)", DatabasePrefix, arg);
	} else {
		Format(query, sizeof(query), "SELECT ip, aid FROM %s_bans WHERE (type = 1 AND ip = '%s') AND (length = '0' OR ends > UNIX_TIMESTAMP() AND RemoveType IS NULL)", DatabasePrefix, arg);
	}
...
public SelectUnbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if(SQL_FetchRow(hndl))
	{
		...
		decl String:arg[30], aid;
		SQL_FetchString(hndl, 0, arg, sizeof(arg));
		aid = SQL_FetchInt(hndl, 1);
		
		decl String:query[1000];
		if (strncmp(arg, "STEAM_0:", 8) == 0)
		{
			Format(query, sizeof(query), "UPDATE %s_bans SET RemovedBy = %d, RemoveType = 'U', RemovedOn = UNIX_TIMESTAMP() WHERE type = 0 AND authid = '%s' ORDER BY created DESC LIMIT 1", DatabasePrefix, aid, arg);
		} else {
			Format(query, sizeof(query), "UPDATE %s_bans SET RemovedBy = %d, RemoveType = 'U', RemovedOn = UNIX_TIMESTAMP() WHERE type = 1 AND ip = '%s' ORDER BY created DESC LIMIT 1", DatabasePrefix, aid, arg);
		}
...
public InsertUnbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	LogAction(admin, -1, "\"%L\" removed ban (filter \"%s\")", admin, arg);
	if(admin && IsClientInGame(admin))
	{
...
{
	decl admin, minutes, String:adminAuth[30], String:adminIp[30], String:authid[20], String:banReason[128], String:Query[512], String:reason[64];
	ResetPack(data);
	admin = ReadPackCell(data);
	minutes = ReadPackCell(data);
	ReadPackString(data, reason,    sizeof(reason));
	...
	if( serverID == -1 )
	{
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						('%s', '', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, '%d', '%s', (SELECT aid FROM %s_admins WHERE authid = '%s'), '%s', \
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1), ' ')", 
						DatabasePrefix, authid, (minutes*60), (minutes*60), banReason, DatabasePrefix, adminAuth, adminIp, DatabasePrefix, ServerIp, ServerPort);
	} else {
		FormatEx(Query, sizeof(Query), "INSERT INTO %s_bans (authid, name, created, ends, length, reason, aid, adminIp, sid, country) VALUES \
						('%s', '', UNIX_TIMESTAMP(), UNIX_TIMESTAMP() + %d, '%d', '%s', (SELECT aid FROM %s_admins WHERE authid = '%s'), '%s', \
						%d, ' ')", 
						DatabasePrefix, authid, (minutes*60), (minutes*60), banReason, DatabasePrefix, adminAuth, adminIp, serverID);
	}
...
{
	decl admin, minutes, String:authid[20], String:reason[64];
	ResetPack(data);
	admin = ReadPackCell(data);
	minutes = ReadPackCell(data);
	ReadPackString(data, reason,    sizeof(reason));
...
public ReasonSelected(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		if(Pack == INVALID_HANDLE && g_BanTarget[param1] != -1)
		{
			PrintToChat(param1, "%s%t", Prefix, "Ban Fail");
			return;
		}
		...
		if(startTime + time * 60 > GetTime())
		{
			...
			if( serverID == -1 )
			{
				FormatEx(query, sizeof(query), 
						...
						('%s', '%s', '%s', %d, %d, '%d', '%s', (SELECT aid FROM %s_admins WHERE authid = '%s'), '%s', \
						(SELECT sid FROM %s_servers WHERE ip = '%s' AND port = '%s' LIMIT 0,1))", 
						DatabasePrefix, ip, auth, banName, startTime, startTime + time * 60, time * 60, banReason, DatabasePrefix, adminAuth, adminIp, DatabasePrefix, ServerIp, ServerPort);
			}
			else
			{
				FormatEx(query, sizeof(query), 
						...
						('%s', '%s', '%s', %d, %d, '%d', '%s', (SELECT aid FROM %s_admins WHERE authid = '%s'), '%s', \
						%d)", 
						DatabasePrefix, ip, auth, banName, startTime, startTime + time * 60, time * 60, banReason, DatabasePrefix, adminAuth, adminIp, serverID);
			}

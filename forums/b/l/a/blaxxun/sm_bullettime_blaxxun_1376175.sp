...
new Float:btChance[MAXPLAYERS + 1];
...
{
	name = "SM Bullet Time - blaxxun edition",
	author = "blaxxun",
...
public OnPluginStart() 
{
	...
	bt_mode = CreateConVar("bt_mode", "2", "0: default simple triggers; 1: random mode; 2: reward mode", FCVAR_PLUGIN);
...
public EventPlayerHurt(Handle:event, const String:name[], bool:dontBroadcast) 
{
	...
	if(attackerClient>0 && attackerClient<=iMaxClients)
	{
		...
		for (new i = 0; i < sizeof(Weapons); i++) 
		{
			...
			else if(GetConVarInt(bt_mode) == 2)
			{
				if (StrEqual(weapon,Weapons[i],false)) 
				{	
					 BTaddProb(attackerClient,0.02);
				}
				
				if(health < 1) xFactor += 0.07;
				
				if(xFactor < btChance[attackerClient] && btReady)
				{
					new String:activatorName[MAX_NAME_LENGTH];
					GetClientName(attackerClient, activatorName, MAX_NAME_LENGTH);
					PrintHintTextToAll("%s activated bullettime!", activatorName);
					
					go = true;
					btReady = false;
					btActive = true;
					btChance[attackerClient] = 0.001;
					//PrintToChatAll(" - bullettime OFF (HURT)");
					CreateTimer(GetRandomFloat(11.0,22.0), BTreset);	
				}
			}
...
public EventPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast){
	...
	if(attackerClient>0 && attackerClient<=iMaxClients)
	{
		...
		else if(GetConVarInt(bt_mode) == 2)
		{
			...
			{
				BTaddProb(attackerClient,-0.05);
			}
			else
			{
				BTaddProb(attackerClient,0.001);
				...
				{
					headShotCount[attackerClient]++;
					BTaddProb(attackerClient,0.005);
					if(GetConVarInt(bt_headshots) <= headShotCount[attackerClient]) 
					{
						//PrintToChatAll("bullettime HEADSHOT");
						BTaddProb(attackerClient,0.03);
					}
				}
				
				if(StrEqual(weapon,"hegrenade",false) && GetConVarInt(bt_he) == 1) BTaddProb(attackerClient,0.04);
				if(StrEqual(weapon,"knife",false))
				{
					BTaddProb(attackerClient,0.1);
					BTaddProb(victimClient,-0.05);
				}
			}
		}
		else
...
public Action:BTaddProb(iClient, Float:fChance)
{
	new Float:fMax = 0.3;
	if(btChance[iClient]<fMax) btChance[iClient] += fChance;
	if(!btActive) PrintHintText(iClient, "%s", ProgressBar[RoundToFloor((btChance[iClient]/fMax)*34)]);
}
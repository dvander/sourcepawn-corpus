...
public Action:event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	TF2_SetPlayerClass2(client, class, true, false);
...
public Action:event_SuddenDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for (new i = 1; i <= maxplayers; i++)
	{
		if (IsClientConnected(i) && IsClientInGame(i) && IsPlayerAlive(i))
		{
			...
			TF2_SetPlayerClass2(i, class, true, false);
...
stock TF2_SetPlayerClass2(client, TFClassType:class, bool:weapons=true, bool:desired=true)
{
	if (desired)

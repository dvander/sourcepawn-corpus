...
{
	CreateConVar("sm_rocketguns_version", PLUGIN_VERSION, "Rocket Guns Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	...
	cvarMagnitude = CreateConVar("sm_rocketguns_magnitude", "50", "Magnitude of the explosions", FCVAR_PLUGIN, true, 1.0, false, _);

	ownerOffset = FindSendPropInfo("CBaseEntity", "m_hOwnerEntity");
	if (ownerOffset == -1)
		SetFailState("Could not find m_hOwnerEntity offset");
...
public Action:event_BulletImpact(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (StrContains(gunsString[startidx], weapon, false) != -1)
	{
		...
		SetEntDataEnt2(entity, ownerOffset, client);

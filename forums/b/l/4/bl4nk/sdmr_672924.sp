...
public OnPluginStart()
{
	...
	PrepSDKCall_SetFromConf(hGameConf, SDKConf_Virtual, "RemoveItem");
	PrepSDKCall_AddParameter(SDKType_CBaseEntity, SDKPass_Pointer);
	hRemoveItem = EndPrepSDKCall();
}
...
public Action:timer_Melee(Handle:timer, any:client)
{
	if (IsClientConnected(client) && IsClientInGame(client) && IsPlayerAlive(client))
	{
		for (new i = 0; i < 4; i++)
		{
			...
			{
				SDKCall(hRemoveItem, client, weaponIndex);
			}
		}

		CreateTimer(0.1, timer_MeleeSwitch, client);
	}
}

public Action:timer_MeleeSwitch(Handle:timer, any:client)

...
public Plugin:myinfo = {
	...
	author = "frog,Kemsan,Peace Maker,LeGone,RainBolt Dash, updated by SHADoW NiNE TR3S",
	version = "0.9.5",
};
...
public VIS_Invoke(client)
{
	...
	EffectDuration=GetEngineTime()+FF2_GetAbilityArgument(Boss,this_plugin_name,EFFECT, 2); //duration
}
...
public RDR_Invoke(client)
{
	...
	DrownDuration=GetEngineTime()+FF2_GetAbilityArgument(Boss,this_plugin_name,DROWN, 1); //duration
	DrownDuration = DrownDuration + 4;
}
...
{
	new Boss=GetClientOfUserId(FF2_GetBossUserId(client));
	ExplosionDamage=FF2_GetAbilityArgument(Boss,this_plugin_name,EXPLOSION, 1);	        //damage 
	ExplosionDistance=FF2_GetAbilityArgumentFloat(Boss,this_plugin_name,EXPLOSION, 2);	        //damage radius
	...
	WritePackCell(data, ExplosionDistance); // Range
	WritePackCell(data, ExplosionDamage); // Damge
...
public Action:SetExplosion(Handle:timer, Handle:data)
{
	...
	new EXDistance = ReadPackCell(data);
	new EXDamage = ReadPackCell(data);
	...
	if (gExplosion % 4 == 1) {
		SetExplodeAtClient( Boss, EXDamage, EXDistance, DMG_BLAST );
	}
	return Plugin_Continue;	
}
...
public SCB_Invoke(client)
{
	...
	BossDuration = GetEngineTime()+FF2_GetAbilityArgument(Boss,this_plugin_name,SCALEBOSS, 2);	        //duration
}
...
public SCP_Invoke(client)
{
	...
	PlayerDuration=GetEngineTime()+FF2_GetAbilityArgument(Boss,this_plugin_name,SCALEPLAYER, 2);	        //duration
}
...
SetExplodeAtClient( client, RDamage, RDistance, dmgtype )
{
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		if ( IsValidEdict( particle ) )
		{
			...
			SetDamageRadial( client, RDamage, pos, RDistance, dmgtype );
		}
	}
}
...
public IOC_Invoke(client)
{
	...
	range=FF2_GetAbilityArgument(Boss,this_plugin_name,IONCANNON, 2);	        //damage radius
	damage=FF2_GetAbilityArgument(Boss,this_plugin_name,IONCANNON, 3);	        //damage
...
stock CustomRemoveEntity(entity, Float:time = 0.0)
{
	if (time == 0.0)
	{
	...
	{
		CreateTimer(time, CustomRemoveEntityTimer, entity, TIMER_FLAG_NO_MAPCHANGE);
	}
}

public Action:CustomRemoveEntityTimer(Handle:Timer, any:entity)
{
	if(IsValidEntity(entity))
...
public DEL_Invoke(client)
{
	...
	DeliriumDuration=GetEngineTime()+FF2_GetAbilityArgument(Boss,this_plugin_name,DELIRIUM, 2);	//rage duration
	SDKHook(client, SDKHook_PreThink, Delirium_Prethink);
}

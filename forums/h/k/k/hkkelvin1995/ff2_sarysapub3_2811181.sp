...
AMS_ExecuteSpell(clientIdx, spellIdx)
{
	if (!IsEmptyString(AMS_CastingParticle))
	{
		...
		if (IsValidEntity(particle))
			CreateTimer(1.0, CustomRemoveEntity, EntIndexToEntRef(particle), TIMER_FLAG_NO_MAPCHANGE);
	}
...
public RRW_Invoke(clientIdx)
{
	...
	FF2_GetAbilityArgumentString(bossIdx, this_plugin_name, RRW_STRING, argOffset + 7, centerText, 124);
...
stock ParticleEffectAt(Float:position[3], String:effectName[], Float:duration = 0.1)
{
	...
	if (particle != -1)
	{
		...
		if (duration > 0.0)
			CreateTimer(duration, CustomRemoveEntity, EntIndexToEntRef(particle), TIMER_FLAG_NO_MAPCHANGE);
	}
	return particle;
}
...
public Action:CustomRemoveEntity(Handle:timer, any:entid)
{
...
public Action:CustomRemoveEntityNoTele(Handle:timer, any:entid)
{
	new entity = EntRefToEntIndex(entid);
	if (IsValidEdict(entity) && entity > MaxClients)
		AcceptEntityInput(entity, "Kill");
...
stock FullyHookedDamage(victim, inflictor, attacker, Float:damage, damageType=DMG_GENERIC, weapon=-1, Float:attackPos[3] = NULL_VECTOR)
{
	...
	if (IsValidEntity(pointHurt))
	{
		...
		CustomRemoveEntity(INVALID_HANDLE, EntIndexToEntRef(pointHurt));
	}
}
...
CreateRagdoll(client, Float:flSelfDestruct=0.0, bool:isIce=false)
{
	...
	if (iRag > MaxClients && IsValidEntity(iRag))
	{
		...
		if (flSelfDestruct > 0.0)
			CreateTimer(flSelfDestruct, CustomRemoveEntity, EntIndexToEntRef(iRag), TIMER_FLAG_NO_MAPCHANGE);

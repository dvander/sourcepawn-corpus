...
public void LookupPlayerGroups(int client)
{
	...
	for (int i = 0; i < cnt; i++)
	{
		int group[SteamGroup];
		g_SteamGroups.GetArray(i, group[0], sizeof(group));
...
bool GetSteamGroupById(int id, int buffer[SteamGroup])
{
	int cnt = g_SteamGroups.Length;
	for (int i = 0; i < cnt; i++)
	{
...
int g_ConfigGroup[SGConfigInfo];
...
public Action ParseConfig(int client, int args)
{
	...
	if (g_SteamGroups != null)
	{
		...
		{
			int group[SteamGroup];
			g_SteamGroups.GetArray(i, group[0], SGSIZE);
...
public SMCResult SMC_EndSection(Handle smc)
{
	switch(g_ConfigState)
	{
		case CS_InGroup:
		{
			...
			g_SteamGroups.PushArray(group[0]);
			ResetConfigGroup();
			g_ConfigState = CS_Nowhere;
		}

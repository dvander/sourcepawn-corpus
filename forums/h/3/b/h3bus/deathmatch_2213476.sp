...
UpdateState()
{
    ...
    decl String:spawnFadeColor[200];
    decl String:spawnFadeColor_Deserialized[4][5];
    ...
    cvars_GetPluginConvarString(eCvars_dm_spawn_fade_color, spawnFadeColor, sizeof(spawnFadeColor));
    RemoveChar(spawnFadeColor, ' ');
    new colorCount = deserializeStrings(spawnFadeColor, spawnFadeColor_Deserialized, sizeof(spawnFadeColor_Deserialized), sizeof(spawnFadeColor_Deserialized[]), ',');
    if(colorCount > sizeof(g_iConfig_SpawnFade_Color)) colorCount = sizeof(g_iConfig_SpawnFade_Color);
    for(new i = 0; i < colorCount; i++)
        g_iConfig_SpawnFade_Color[i]  = StringToInt(spawnFadeColor_Deserialized[i]);
...
{
    if(g_bConfig_Enabled && g_bConfig_OnlyHS && players_IsClientValid(victim))
    {
        new bool:filterDamage = true;
        
        if((damagetype & DMG_FALL) != 0 || attacker == 0)
            filterDamage = !g_bConfig_OnlyHS_AllowWorld;
        
        else if(players_IsClientValid(victim) && players_IsClientValid(attacker))
        {
            new weaponId = -1;
            ...
            else if(g_bConfig_OnlyHS_AllowNade)
            {
        ...
        if(filterDamage)
            return Plugin_Handled;
        else if(g_bConfig_OnlyHS_OneShot && (damagetype & DMG_HEADSHOT != 0))
        {
...
public OnEntityCreated(entity, const String:classname[])
{
    if (g_bConfig_Enabled && (entity > MaxClients) && IsValidEdict(entity))
    {
        ...
        else if( StrPartEqual(classname, "weapon_") )
        {
...
public OnEntityDestroyed(entity)
{
    if (g_bConfig_Enabled && (entity > MaxClients) && IsValidEdict(entity))
    {
        ...
        if( StrPartEqual(EntityName, "weapon_") && weapons_IsEntityTagged(entity, weaponId, client, _, team) )
        {

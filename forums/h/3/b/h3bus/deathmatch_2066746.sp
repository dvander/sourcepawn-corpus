...
new String:default_primary[24] = "none";
new String:default_secondary[24] = "none";
new bool:connect_hide_menu = false;
...
public OnMapStart()
{
	...
	ClearArray(primaryWeaponsAvailable);
	ClearArray(secondaryWeaponsAvailable);
	LoadConfig();
	// Precache content
...
SetClientGunModeSettings(clientIndex)
{
	if (!IsFakeClient(clientIndex))
	{
		if (gunMenuMode != 3)
		{
			primaryWeapon[clientIndex] = default_primary;
			secondaryWeapon[clientIndex] = default_secondary;
			if(!StrEqual(default_primary, "none") && !StrEqual(default_secondary, "none"))
			{
				firstWeaponSelection[clientIndex] = false;
				rememberChoice[clientIndex] = connect_hide_menu;
			}
			else
			{
...
LoadConfig()
{
	...
	decl String:path[] = "cfg/deathmatch/deathmatch_advanced.ini";
	...
	decl String:current_key[25];
	decl String:map[100];
	decl String:map_for_comp[26];
	...
	if (KvGotoFirstSubKey(keyValues, false))
	{
		do {
			...
			strcopy(map_for_comp, strlen(key)+1, map);
		} while (!StrEqual(map_for_comp, key) && !StrEqual("!default!", key) &&  KvGotoNextKey(keyValues, false));
	}
	else
	{
		SetFailState("Weapon section empty");
	}
	...
	if (KvJumpToKey(keyValues, "Options"))
	{
		GetConVarString(cvar_dm_gun_menu_mode, current_key, sizeof(current_key));
		KvGetString(keyValues, "gun_menu_mode", value, sizeof(value), current_key);
		SetConVarString(cvar_dm_gun_menu_mode, value);
		
		KvGetString(keyValues, "default_primary", default_primary, sizeof(default_primary), "none");
		KvGetString(keyValues, "default_secondary", default_secondary, sizeof(default_secondary), "none");
		KvGetString(keyValues, "connect_hide_menu", value, sizeof(value), "no");
		connect_hide_menu = (StrEqual(value, "yes")) ? true : false;
		KvGoBack(keyValues);
	}
...
BuildWeaponMenuNames()
{
	...
	SetTrieString(weaponMenuNames, "weapon_ssg08", "Scout");
...
public Menu_Options(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		if (StrEqual(info, "New"))
		{
			...
			if (GetArraySize(primaryWeaponsAvailable) > 0)
...
public Menu_Primary(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		if (GetArraySize(secondaryWeaponsAvailable) > 0)
			BuildDisplayWeaponMenu(param1, false);
		else
		{

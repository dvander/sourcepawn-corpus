...
bool g_bCloakedStatus[MAXPLAYERS + 1] = false;
...
{	
	...
	{
		g_bCloakedStatus[client] = true;
		TF2Attrib_RemoveByName(client, "SPELL: set Halloween footstep type");
	}
	else
	{
		g_bCloakedStatus[client] = false;
		if(g_bRandom[client] == false && g_fFootPrintID[client] != 0.0)
		{
			TF2Attrib_SetByName(client, "SPELL: set Halloween footstep type", g_fFootPrintID[client]);
		}
	}
	if(g_bRandom[client] == true && g_bCloakedStatus[client] == false)
	{
		if(CheckCommandAccess(client, "sm_footprints_random", ADMFLAG_GENERIC))
		{
...
public OnClientPostAdminCheck(client)
{
	...
	{
		CreateTimer(1.0, Timer_LoadCookies, client, TIMER_DATA_HNDL_CLOSE);
	}
}
...
stock LoadClientCookies(client)
{
...
stock bool LoadClientRandomCookie(client)
{
...
SetClientRandomCookie(client, bool on)
{

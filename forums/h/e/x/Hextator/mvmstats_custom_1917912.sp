...
new g_totals[MAXWAVES+1][MAXSTATS+1];

new bool:g_bombResetBonus[MAXPLAYERS+1];

#define TFCondLen 255
// _:TFCond isn't long enough because tf2.inc's
// TFCond enum is out of date and missing some elements
// Here are some of them
#define BULLET_RES_10 61
#define EXPLO_RES_10  62
#define FIRE_RES_10   63
#define BULLET_RES_75 58
#define EXPLO_RES_75  59
#define FIRE_RES_75   60
...
DamageAdjust(attacker, victim, damage, adjustedDamageIndex, bool:wasCrit, bool:wasMiniCrit) {
	...
	for (new buffOwner = 0; buffOwner < MaxClients && !uber; buffOwner++) {
		...
		if (
			IsValidClient(buffOwner) && (
			g_statusBonus[buffOwner][attacker][BULLET_RES_75] ||
			g_statusBonus[buffOwner][attacker][EXPLO_RES_75] ||
			g_statusBonus[buffOwner][attacker][FIRE_RES_75]
		)) {
			// LogMessage(
...
public TF2_OnConditionAdded(client, TFCond:condition) {
	...
	if (condition == TFCond_MarkedForDeath) {
		...
		markedForDeath = g_statusBonus[buffOwner][client][TFCond_MarkedForDeath] = true;
		// LogMessage(
	...
	for (new buffOwner = 0; buffOwner < MaxClients; buffOwner++) {
		...
		if (
			...
			condition == BULLET_RES_10 ||
			condition == EXPLO_RES_10 ||
			condition == FIRE_RES_10
		)) {
...
UpdateWaveScores(wave)
{
	...
	for (new stat = 0; stat < MAXSTATS; stat++) {
		if (g_scoreWeights[stat] > 0) {
			...
			for(new client = 0; client < MaxClients; client++) {
				new Float:currStat = g_stats[client][wave][stat];
...
ShowWaveWinner(wave)
{
	...
	LogMessage("Wave MVP: %N %i%!", waveWinner, maxScore);
	//PrintCenterTextAll("Wave MVP: %N %i%!", waveWinner, maxScore);
	...
	for(new client = 0; client < MaxClients; client++) {
		if (IsValidClient(client)) {
			ShowSyncHudText(client, hudHandle, "Wave MVP: %N %i%!", waveWinner, maxScore);
		}
	}

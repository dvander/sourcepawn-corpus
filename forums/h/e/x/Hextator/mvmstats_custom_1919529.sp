...
public TF2_OnConditionAdded(client, TFCond:condition) {
	...
	for (new buffOwner = 0; buffOwner < MaxClients; buffOwner++) {
		...
		if (
			...
			_:condition == BULLET_RES_10 ||
			_:condition == EXPLO_RES_10 ||
			_:condition == FIRE_RES_10
		)) {
...
ShowWaveWinner(wave)
{
	...
	for(new client = 0; client < MaxClients; client++) {
		if (IsValidClient(client)) {
			ShowSyncHudText(client, hudHandle, "Wave MVP: %N, who did %i%% of the work!", waveWinner, maxScore);
		}
	}

...
public Plugin myinfo =
{
	name = "HNS Beacon",
	author = "Headline",
	description = "Beacons last terrorist alive",
...
public void OnPluginStart()
{
	...
	AutoExecConfig_CreateConVar("beacon_version", PLUGIN_VERSION, "Headline's Beacon Plugin: Version");
...
public void OnClientDisconnected(int client)
{
	...
	if(GetAlivePlayersCount(CS_TEAM_CT) <= 3 && GetAlivePlayersCount(CS_TEAM_T) == 1)
	{
		g_iBeaconValidation++;
		beaconCount = 0;
...
public Action Event_PlayerDeath(Handle hEvent, const char[] sName, bool bDontBroadcast)
{
	...
	if(GetAlivePlayersCount(CS_TEAM_CT) <= 3 && GetAlivePlayersCount(CS_TEAM_T) == 1)
	{
		if (gc_bWarnPlayers.BoolValue)
		{
			...
			{
				PrintToChatAll("Beacons last terrorist alive");
			}
			else
			{
				CPrintToChatAll("Beacons last terrorist alive");
			}
		}
		g_iBeaconValidation++;
		beaconCount = 0;
...
public Action Command_StopBeacon(int client, int iArgs)
{
	...
	{
		CPrintToChatAll("[SM] %N toggled beacon OFF", client);
	}
	return Plugin_Handled;
}
...
public Action Command_BeaconAll(int client, int iArgs)
 {
	...
	{
		CPrintToChatAll("[SM] %N toggled beacon ON", client);
	}
	if (g_bBeaconOn)
	{
		g_iBeaconValidation++;

...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
// IMPORTANT: Set the hidden cVar z_scrimmage_sphere to 0 or else there will be visual errors for the Human Director.
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
#define CVAR_FLAGS		0|FCVAR_NOTIFY
...
public OnPluginStart()
{
	...
	else if(StrEqual(game_name, "left4dead2", false))
	{
		...
		WITCH_COST = GetConVarInt(FindConVar("z_witch_health"));
		TANK_COST = 1500;
		SetConVarInt(FindConVar("z_scrimmage_sphere"), 0); // To anyone who may pick this up: figure out a way to implement it only on modes that need it because I am unsure as to whether this is used anywhere in the game, so it might break something otherwise (checkerboard sphere, occurs in coop on death when you're the Human Director).
	}

	CreateConVar("sm_humandirectorreloaded_version", PLUGIN_VERSION, "Human Director Reloaded Version", 0|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
public DisplayHumanDirectorPanel(client)
{
	...
	if(b_IsDirector[client] == false)
	{
		if(HumanDirectorAvailable() == true)
		{
			...
			SetConVarInt(FindConVar("z_scrimmage_sphere"), 0); // Hidden cVar implementation (0 when someone becomes the director to avoid the checkerboard sphere bug).
		...
		case MenuAction_Select:
		{
			...
			else if(StrEqual(item1, "option1", false))
			{
				if(testDistance(client) == true)
				{
					...
					CheatCommand(client, "z_spawn_old", "smoker", "area");
					SpawnMenu(client);
				}
				else
				{
			...
			else if(StrEqual(item1, "option2", false))
			{
				if(testDistance(client) == true)
				{
					...
					CheatCommand(client, "z_spawn_old", "boomer", "area");
					SpawnMenu(client);
				}
				else
				{
			...
			else if(StrEqual(item1, "option3", false))
			{
				if(testDistance(client) == true)
				{
					...
					CheatCommand(client, "z_spawn_old", "hunter", "area");
					SpawnMenu(client);
				}
				else
				{
			...
			else if(StrEqual(item1, "option4", false))
			{
				if(testDistance(client) == true)
				{
					...
					CheatCommand(client, "z_spawn_old", "spitter", "area");
					SpawnMenu(client);
				}
				else
				{
			...
			else if(StrEqual(item1, "option5", false))
			{
				if(testDistance(client) == true)
				{
					...
					CheatCommand(client, "z_spawn_old", "charger", "area");
					SpawnMenu(client);
				}
				else
				{
			...
			else if(StrEqual(item1, "option6", false))
			{
				if(testDistance(client) == true)
				{
					...
					CheatCommand(client, "z_spawn_old", "jockey", "area");
					SpawnMenu(client);
				}
				else
				{
			...
			else if(StrEqual(item1, "option7", false))
			{
				if(testDistance(client) == true)
				{
					...
					CheatCommand(client, "z_spawn_old", "witch", "area");
					SpawnMenu(client);
				}
				else
				{
			...
			else if(StrEqual(item1, "option8", false))
			{
				if(testDistance(client) == true)
				{
					...
					CheatCommand(client, "z_spawn_old", "tank", "area");
					SpawnMenu(client);
				}
				else
				{
			...
			else if(StrEqual(item1, "option9", false))
			{
				if(testDistance(client) == true)
				{
					...
					CheatCommand(client, "z_spawn_old", "mob", "area");
					SpawnMenu(client);
				}
				else
				{
...
public HumanDirectorQuit(client)
{
	...
	SetConVarInt(FindConVar("z_scrimmage_sphere"), 1); // Set to 1 when no one's the director (default value) to avoid potentially breaking stuff (unsure if the sphere was cut/depricated content, the effect is not precached in coop, or if it's used somewhere).

...
public void OnConfigsExecuted()
{
	...
	do
	{
		...
		/* 
		* Created an alias for those who have servers communicating with local ip and
		* want people from internet be able to connect through the public ip
		* example "ip" = 10.1.1.50:27015 and alias_ip = "179.177.x.x:27015"
		*/ 
		kv.GetString("ip_alias", se.ip_alias, sizeof(ServerEntry::ip));
		if(se.ip[0] == '\0')
		{
		...
		case MenuAction_Select:
		{
			...
			if(StrEqual(buff, "connect"))
				...
				else
				{
					...
					// Check if an alias is filled, otherwise make redirect with original se.ip
					if (!se.ip_alias[0]){
						RedirectClient(param1, se.ip);
					}
					else {
						RedirectClient(param1, se.ip_alias);
					}

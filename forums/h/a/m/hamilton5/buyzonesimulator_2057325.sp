...
new Handle:hcBuyZoneTeam						= INVALID_HANDLE;
...
public OnPluginStart() {
	...
	hcBuyZoneTeam = CreateConVar("sm_bzteam", "1.0", "BuyZone Simulator, 1=for all, 2=for T, 3=for CT ", FCVAR_PLUGIN,true,1.0,true,3.0);
	hcBuyZoneRange = CreateConVar("sm_bzsimulator_range", "18.0", "BuyZone range, 0 = everywhere ", FCVAR_PLUGIN,true,0.0);
...
HookBuyZone(client)	{																// Check client, create buyzone hook, create range check *******
	if (IsClientInGame(client) && GetConVarInt(hcBuyZoneTeam) == 1 || GetClientTeam(client) == GetConVarInt(hcBuyZoneTeam)) {
			gCanBuy[client] = true;

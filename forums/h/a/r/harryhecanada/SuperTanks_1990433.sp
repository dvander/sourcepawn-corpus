...
new Handle:hWave0Cvar = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	hWave0Cvar = CreateConVar("st_wave0_tanks", "4", "Default number of tanks in a generalized tank spawn.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,5.0);
	hWave1Cvar = CreateConVar("st_wave1_tanks", "4", "Default number of tanks in the 1st wave of finale.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,5.0);
	hWave2Cvar = CreateConVar("st_wave2_tanks", "4", "Default number of tanks in the 2nd wave of finale.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,5.0);
	hWave3Cvar = CreateConVar("st_wave3_tanks", "4", "Default number of tanks in the finale escape.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,5.0);
	...
	hHealHealthCommons = CreateConVar("st_heal_health_commons", "100", "Heal Tanks receive this much health per second from being near a common infected.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1000.0);
	hHealHealthSpecials = CreateConVar("st_heal_health_specials", "500", "Heal Tanks receive this much health per second from being near a special infected.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1000.0);
	hHealHealthTanks = CreateConVar("st_heal_health_tanks", "1000", "Heal Tanks receive this much health per second from being near another tank.",FCVAR_PLUGIN|FCVAR_NOTIFY,true,0.0,true,1000.0);
	...
	iWave0Cvar = GetConVarInt(hWave0Cvar);
	...
	HookConVarChange(hWave0Cvar, SuperTanksSettingsChanged);
		...
		if (client > 0 && IsClientInGame(client))
		{
			...
			if (!bFinaleOnly || (bFinaleOnly && iTankWave > 0))
			{
				...
				switch(iTankWave)
				{
				    case 0:
					{
						if (iNumTanks < iWave0Cvar)
						{
							CreateTimer(3.0, SpawnTankTimer, _, TIMER_FLAG_NO_MAPCHANGE);
						}
						else if (iNumTanks > iWave0Cvar)
						{

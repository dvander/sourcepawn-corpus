...
public void OnEntityCreated(int entity, const char[] classname)
{
	...
	if (IsRock(entity)) {
		...
		if (ROCK_GODFRAMES_RENDER) {
			SetEntityRenderMode(entityRef, view_as<RenderMode>(3));
...
public void OnGameFrame()
{
	float pos[3];
	int rockEntity;
	int index = GetGameTickCount() % MAX_HISTORY_FRAMES; 
	...
	for (int i = 0; i < rockEntitiesArray.Length; ++i) {
		...
		ArrayList posArray = rockEntitiesArray.Get(i, BLOCK_POS_HISTORY);
		...
		if (ROCK_GODFRAMES_RENDER && Array_IsRockAllowedDmg(i)) {
			SetEntityRenderMode(rockEntity, view_as<RenderMode>(0));
...
{
	int rockIndex = Array_SearchRock(array, rockEntity);
	
	if (rockIndex >= 0) {
		ArrayList rockPos = array.Get(rockIndex, BLOCK_POS_HISTORY);
		rockPos.Clear();
		CloseHandle(rockPos);
 ...
 */
public int Array_SearchRock(ArrayList array, int rockEntity)
{
	for (int i = 0; i < array.Length; ++i) {
		int cRockEntity = array.Get(i, BLOCK_ENT_REF);
 ...
 */
public bool Array_IsRockAllowedDmg(int rockIndex)
{
		...
		bool dontBroadcast)
{
	...
	float lagTime = !IsFakeClient(client) ? GetClientLatency(client, NetFlow_Both) + clientLerp : 0.0;
	int rollBackTick = LAG_COMP_ENABLED ? 
	...
	// Sphere vectors
	float radius = SPHERE_HITBOX_RADIUS;
	float c[3];
	
	ArrayList rockPositionsArray;
	int entity;
	int index = rollBackTick % MAX_HISTORY_FRAMES;
	float delta;
...
public void ApplyDamageOnRock(int rockIndex, int client, float eyePos[3], float c[3], Event event, int rockEntity)
{
	...
	float range = GetVectorDistance(eyePos, c);
 ...
 */
public void ApplyBulletToRock(int rockIndex, int rockEntity, float damage, float range)
{
	float rockDamage = float(rockEntitiesArray.Get(rockIndex, BLOCK_DMG_DEALT));

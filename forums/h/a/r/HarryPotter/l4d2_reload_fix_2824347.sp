...
/**
 * l4d2_reload_fix "1.8" 變體插件
 * -別人的武器丟在地上的時候標記已使用，避免重複彈藥
 */


#define PLUGIN_VERSION 		"1.8-2024/6/20"
...
1.8 (27-Mar-2024)
	- Fixed wrong weapons ammo when switch weapons.
...
bool 
	g_bWeaponDrop[MAXENTITIES+1],
	g_bWeaponAlreadyUse[MAXENTITIES+1];
...
public void OnPluginStart()
{
	...
	if( g_bLateLoad )
	{
		for( int i = 1; i <= MaxClients; i++ )
		{
			...
			if( IsClientInGame(i) )
			{
				SDKHook(i, SDKHook_WeaponCanUsePost, WeaponCanUsePost);
			}
		}
	}
...
public void OnEntityCreated(int entity, const char[] classname)
{
	...
	g_bWeaponDrop[entity] = false;
	g_bWeaponAlreadyUse[entity] = false;
}
...
void WeaponCanUsePost(int client, int weapon)
{
	...
	if( GetClientTeam(client) == 2 )
	{
		...
		if( current != -1 )
		{
			...
			g_bWeaponAlreadyUse[weapon] = true;
...
void OnFrame(DataPack dPack)
{
	...
	if( weapon == GetEntPropEnt(client, Prop_Send, "m_hActiveWeapon") )
	{
		...
		//PrintToChatAll("weapon: %d (%d), cur_ammo: %d, old_ammo: %d", weapon, g_bWeaponDrop[weapon], cur_ammo, old_ammo);

		if(g_bWeaponDrop[weapon])
		{
			// clip保持跟地上的武器一樣
...
void Event_Weapon_Drop(Event event, const char[] name, bool dontBroadcast)
{
	...
	//PrintToChatAll("%N Drop weapon %s (skin:%d), clip: %d, ammo: %d", client, sWeapon_ClassName, weapon_skin, GetEntProp(weapon, Prop_Send, "m_iClip1"), g_iAmmo[client][weapon_weaponid][weapon_skin]);

	if (g_bWeaponAlreadyUse[weapon] == true)
	{
		g_bWeaponDrop[weapon] = true;
	}
}

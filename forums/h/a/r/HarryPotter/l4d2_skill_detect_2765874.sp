...
public OnPluginStart()
{
    ...
    if ( g_hCvarMaxPounceDistance == INVALID_HANDLE ) { g_hCvarMaxPounceDistance = CreateConVar( "z_pounce_damage_range_max",  "1000.0", "Not available on this server, added by l4d2_skill_detect.", FCVAR_NOTIFY, true, 0.0, false ); }
    if ( g_hCvarMinPounceDistance == INVALID_HANDLE ) { g_hCvarMinPounceDistance = CreateConVar( "z_pounce_damage_range_min",  "300.0", "Not available on this server, added by l4d2_skill_detect.", FCVAR_NOTIFY, true, 0.0, false ); }
    if ( g_hCvarMaxPounceDamage == INVALID_HANDLE ) { g_hCvarMaxPounceDamage = CreateConVar( "z_hunter_max_pounce_bonus_damage",  "49", "Not available on this server, added by l4d2_skill_detect.", FCVAR_NOTIFY, true, 0.0, false ); }
...
public Action: Event_WitchSpawned ( Handle:event, const String:name[], bool:dontBroadcast )
{
    ...
    int[] witch_dmg_array = new int[MAXPLAYERS+DMGARRAYEXT];
    decl String:witch_key[10];
    FormatEx(witch_key, sizeof(witch_key), "%x", witch);
...
stock HandlePop( attacker, victim, shoveCount, Float:timeAlive )
{
    ...
    if ( GetConVarBool(g_hCvarReport) )
    {
        ...
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 popped \x05%N\x01.", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 popped a boomer.", attacker );
        }
    }
...
stock HandleLevel( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 leveled \x05%N\x01.", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 leveled a charger.", attacker );
        }
        else {
            PrintToChatAll( "\x01[\x05TS\x01] A charger was leveled." );
        }
    }
...
stock HandleLevelHurt( attacker, victim, damage )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 chip-leveled \x05%N\x01 (\x03%i\x01 damage).", attacker, victim, damage );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 chip-leveled a charger. (\x03%i\x01 damage)", attacker, damage );
        }
        else {
            PrintToChatAll( "\x01[\x05TS\x01] A charger was chip-leveled (\x03%i\x01 damage).", damage );
        }
    }
...
stock HandleDeadstop( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 deadstopped \x05%N\x01.", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 deadstopped a hunter.", attacker );
        }
    }
...
stock HandleShove( attacker, victim, zombieClass )
{
    ...
    /*if ( GetConVarBool(g_hCvarReport) && zombieClass != ZC_TANK && zombieClass != ZC_CHARGER)
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 shoved \x05%N\x01.", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 shoved an SI.", attacker );
        }
    }*/
    ...
    if ( GetConVarBool(g_hCvarReport) )
    {
        if ( attacker == -2 )
        {
            ...
            if ( IS_VALID_INGAME(victim) ) {
                PrintToChatAll( "\x01[\x05TS\x01] \x05%N\x01 was team-skeeted.", victim );
            }
        }
        else if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 %sskeeted \x05%N\x01.",
                    attacker,
        ...
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 %sskeeted a hunter.",
                    attacker,
                    (bMelee) ? "melee-": ((bSniper) ? "headshot-" : ((bGL) ? "grenade-" : "") )
                );
        }
...
stock HandleNonSkeet( attacker, victim, damage, bool:bOverKill = false, bool:bMelee = false, bool:bSniper = false )
{
    ...
    /*if ( GetConVarBool(g_hCvarReport) )
    {
        if ( IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x05%N\x01 was \x04not\x01 skeeted (\x03%i\x01 damage).%s", victim, damage, (bOverKill) ? "(Would've skeeted if hunter were unchipped!)" : "" );
        }
        else
        {
            PrintToChatAll( "\x01[\x05TS\x01] Hunter was \x04not\x01 skeeted (\x03%i\x01 damage).%s", damage, (bOverKill) ? "(Would've skeeted if hunter were unchipped!)" : "" );
        }
    }*/
...
HandleCrown( attacker, damage )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        ...
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 crowned a witch (\x03%i\x01 damage).", attacker, damage );
        }
        else {
            PrintToChatAll( "\x01[\x05TS\x01] A witch was crowned." );
        }
    }
...
HandleDrawCrown( attacker, damage, chipdamage )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 draw-crowned a witch (\x03%i\x01 damage, \x05%i\x01 chip).", attacker, damage, chipdamage );
        }
        else {
            PrintToChatAll( "\x01[\x05TS\x01] A witch was draw-crowned (\x03%i\x01 damage, \x05%i\x01 chip).", damage, chipdamage );
        }
    }
...
HandleTongueCut( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 cut \x05%N\x01's tongue.", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 cut smoker tongue.", attacker );
        }
    }
...
HandleSmokerSelfClear( attacker, victim, bool:withShove = false )
{
    ...
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 self-cleared from \x05%N\x01's tongue%s.", attacker, victim, (withShove) ? " by shoving" : "" );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 self-cleared from a smoker tongue%s.", attacker, (withShove) ? " by shoving" : "" );
        }
    }
...
HandleRockSkeeted( attacker, victim )
{
    ...
    /*if ( GetConVarBool(g_hCvarReport) )
    {
        ...
        PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 skeeted a tank rock.", attacker );
    }*/
...
stock HandleHunterDP( attacker, victim, actualDamage, Float:calculatedDamage, Float:height, bool:playerIncapped = false )
{
    ...
    if (    GetConVarBool(g_hCvarReport)
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 high-pounced \x05%N\x01 (\x03%i\x01 damage, height: \x05%i\x01).", attacker,  victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] A hunter high-pounced \x05%N\x01 (\x03%i\x01 damage, height: \x05%i\x01).", victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
    }*/
...
stock HandleJockeyDP( attacker, victim, Float:height )
{
    ...
    if (    GetConVarBool(g_hCvarReport)
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 jockey high-pounced \x05%N\x01 (height: \x05%i\x01).", attacker,  victim, RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] A jockey high-pounced \x05%N\x01 (height: \x05%i\x01).", victim, RoundFloat(height) );
        }
    }
...
stock HandleDeathCharge( attacker, victim, Float:height, Float:distance, bool:bCarried = true )
{
    ...
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) )
        {
            PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 death-charged \x05%N\x01 %s(height: \x05%i\x01).",
                    attacker,
                    victim,
                    (bCarried) ? "" : "by bowling ",
                    RoundFloat(height)
        ...
        {
            PrintToChatAll( "\x01[\x05TS\x01] A charger death-charged \x05%N\x01 %s(height: \x05%i\x01).",
                    victim,
                    (bCarried) ? "" : "by bowling ",
                    RoundFloat(height) 
                );
    ...
    if ( attacker != pinVictim && GetConVarBool(g_hCvarReport) )
    {
        ...
        if ( fClearTime != -1.0 && fClearTime <= fMinTime )
        {
            if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) )
            {
                ...
                {
                    PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 insta-cleared \x05%N\x01 from \x04%N\x01 (%s) (%.2f seconds).",
                            attacker, pinVictim, victim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
                } else {
                    PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 insta-cleared a teammate from \x04%N\x01 (%s) (%.2f seconds).",
                            attacker, victim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
            ...
            else if ( IS_VALID_INGAME(attacker) )
            {
                if ( IS_VALID_INGAME(pinVictim) )
                {
                    PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 insta-cleared \x05%N\x01 from a %s (%.2f seconds).",
                            attacker, pinVictim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
                } else {
                    PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 insta-cleared a teammate from a %s (%.2f seconds).",
                            attacker,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
...
{
    if (    GetConVarBool(g_hCvarReport) &&
            IS_VALID_INGAME(survivor) &&
            streak >= GetConVarInt(g_hCvarBHopMinStreak)
    ) {
        PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 got \x05%i\x01 bunnyhop%s in a row (top speed: \x05%.1f\x01).",
                survivor,
                streak,
                ( streak > 1 ) ? "s" : "",
                maxVelocity
...
stock HandleCarAlarmTriggered( survivor, infected, reason )
{
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        if ( reason == view_as<int>(CALARM_HIT) ) {
            PrintToChatAll( "\x01[\x05TS\x01] \x05%N\x01 triggered an alarm with a hit.", survivor );
        }
        else if ( reason == view_as<int>(CALARM_TOUCHED) )
        {
            ...
            {
                PrintToChatAll( "\x01[\x05TS\x01] \x04%N\x01 made \x05%N\x01 trigger an alarm.", infected, survivor );
            }
            else
            {
                PrintToChatAll( "\x01[\x05TS\x01] \x05%N\x01 touched an alarmed car.", survivor );
            }
        }
        else if ( reason == view_as<int>(CALARM_EXPLOSION) ) {
            PrintToChatAll( "\x01[\x05TS\x01] \x05%N\x01 triggered an alarm with an explosion.", survivor );
        }
        else if ( reason == view_as<int>(CALARM_BOOMER) )
        {
            if ( IS_VALID_INFECTED(infected) )
            {
                PrintToChatAll( "\x01[\x05TS\x01] \x05%N\x01 triggered an alarm by killing a boomer \x04%N\x01.", survivor, infected );
            }
        }
        else {
            PrintToChatAll( "\x01[\x05TS\x01] \x05%N\x01 triggered an alarm.", survivor );
        }
    }

#define PLUGIN_VERSION		"1.1-0206/3/25 error fix"
...
public Action L4D2_MeleeGetDamageForVictim(int client, int weapon, int victim, float &damage) {
	...
	if(iTypeDamage[weapon] == 0)
	{
		// m_strMapSetScriptName 有時候為空
		int meleeWeaponId = GetEntProp(weapon, Prop_Send, "m_hMeleeWeaponInfo");
		if(meleeWeaponId < 0 || meleeWeaponId >= g_iMeleeClassCount)
		{
			iTypeDamage[weapon] = -1;
			return Plugin_Continue;
		}

		iTypeDamage[weapon] = L4D2_GetIntMeleeAttribute(meleeWeaponId, L4D2IMWA_DamageFlags);
	}

	int damagetype = iTypeDamage[weapon];

	if ( victim && damagetype > 0 ) {

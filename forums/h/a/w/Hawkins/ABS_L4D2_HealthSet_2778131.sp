...
{
	abs_l4d2_StartingHealth				= CreateConVar("abs_l4d2_StartingHealth", "200", "Sets starting health at map start and in safe rooms.");
	abs_l4d2_StartingHealthTimePeriod	= CreateConVar("abs_l4d2_StartingHealthTimePeriod", "40", "Sets the time after map start that starting health would be applied.");
...
public Action:Timer_PlayerSpawn(Handle:timer, any:client)
{
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		if ((GetEngineTime() - g_roundStartedTime) < abs_l4d2_StartingHealthTimePeriod.IntValue)
		{
			...
			SetEntProp(client, Prop_Send, "m_iMaxHealth", abs_l4d2_StartingHealth.IntValue);
		}
	}
}

...
{
	", is a noob who didn't pick a class.",
	", he shoots faster.",
	", he has auto-bunnyhop.",
	", he can heal his team by crouching.",
	", he is invisible while he's crouched.",
	", he does loads of damage.",
	", he can drop sentry guns.",
...
public OnPluginStart( )
{
	...
	SNIPER_DAMAGE_RATIO = CreateConVar("talents_sniper_dmg_ratio", "1.5", "Multiplier for Sniper class damage");
...
public Action:TimerThink(Handle:hTimer, any:client)
{
	...
	switch (ClientData[client][PLAYERDATA:ChosenClass])
	{
		...
		case ENGINEER:
		{
			if (buttons & IN_SPEED && CanDrop)// && ClientData[client][PLAYERDATA:ItemsBuilt] < ENGINEER_MAX_BUILDS.IntValue)
			{	
...
CalculateEngineerPlacePos(client, type)
{
	...
	if (TR_DidHit(trace))
	{
		...
		if (GetVectorDistance(endPos, vPos) <= MAX_ENGINEER_BUILD_RANGE.FloatValue)
		{
			...
			PrintToChat(client, "%sToo far away. Try again.", PRINT_PREFIX);
	}
	else
		delete trace;
}
...
public Action:CmdClassInfo(client, args)
{
	...
	PrintToChat(client,"\x05Bunny\x01 = auto bunnyhop, higher jump");
	PrintToChat(client,"\x05Medic\x01 = crouch to heal team, shift to drop defibs");
	PrintToChat(client,"\x05Saboteur\x01 = hold crouch for invisibility, shift drops bombs");
	...
	PrintToChat(client,"\x05Meat-Shield\x01 = a lot of health");	
}
...
SetupClasses(client, class)
{
	...
	switch (class)
	{
		case SOLDIER:
		{
			PrintHintText(client,"soldier attacks fast, try meleeing a tank to death");
			MaxPossibleHP = SOLDIER_HEALTH.IntValue;
		}
		
		case MEDIC:
		{
			PrintHintText(client,"hold CTRL to heal your team mates");
		...
		case SNIPER:
		{
			PrintHintText(client,"hope you're confident, its your job to do all the KILLING!");
		...
		case SABOTEUR:
		{
			PrintHintText(client,"hold CTRL to go invisible, the AI can still see you");

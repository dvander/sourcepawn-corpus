...
public Plugin myinfo =
{
	...
	author      = "DDRKhat, Marcus101RR, and Merudo, edited by Foxhound27",
...
public void OnPluginStart()
{
	...
	cvar_minsurvivor = CreateConVar("static_minimum_survivor", "4", "Static minimum amount of team survivor", CVAR_FLAGS, true, 4.00, true, 24.00);
	...
	TankHpMulti    = CreateConVar("director_tank_hpmulti","0.00","Tanks HP Multiplier (multi*(survivors-4)). Requires director_auto_difficulty 1", CVAR_FLAGS,true,0.00,true,1.00);
	...
	CiSpMulti      = CreateConVar("director_ci_multi","1.0","Infected spawning rate Multiplier (multi*(survivors-4)). Requires director_auto_difficulty 1", CVAR_FLAGS,true,0.00,true,1.00);
	...
	RegConsoleCmd("sm_choi", Join_Survivor, "Join Survivor Team");	
	RegConsoleCmd("sm_infected", Join_Infected, "Join Infected Team");
	RegConsoleCmd("sm_xem", Join_Spectator, "Join Spectator Team");
...
int ADPlayerSpawn(Handle event)
{
...
public Action Timer_AFK(Handle Timer, int client)
{
	...
	if (IsClientInGame(client) && InfectedAllowed && AfkMode.BoolValue && GetClientTeam(client) > TEAM_SPECTATOR && IsPlayerAlive(client) && RoundStarted)
	{
		if ( GetTeamPlayers(GetClientTeam(client), false) > 0) // if more than 1 human player on the team
		{
			...
			if (AfkMode.IntValue == 2) FakeClientCommand(client, "sm_xem");
...
public Action Join_Game(int client, int args)
{
	...
	if (!InfectedAllowed || GetBotOfIdle(client) || GetClientTeam(client) == TEAM_SURVIVOR) FakeClientCommand(client,"sm_choi"); 
	...
	else if (InfectedLimit.IntValue <= GetTeamPlayers(TEAM_INFECTED, false)) FakeClientCommand(client,"sm_choi");
	else if (SurvivorLimit.IntValue <= GetTeamPlayers(TEAM_SURVIVOR, false)) FakeClientCommand(client,"sm_infected");
	else if (GetTeamPlayers(TEAM_INFECTED, false) > GetTeamPlayers(TEAM_SURVIVOR, false) ) FakeClientCommand(client,"sm_choi");
	else if (GetTeamPlayers(TEAM_INFECTED, false) < GetTeamPlayers(TEAM_SURVIVOR, false) ) FakeClientCommand(client,"sm_infected");
	else if (GetRandomInt(0, 1)) FakeClientCommand(client,"sm_choi");
...
public Action GO_AFK(int client, int args) {
	...
	if (GetClientTeam(client) != TEAM_SPECTATOR) FakeClientCommand(client, "sm_xem");
	return Plugin_Handled;
}
...
public int TeamMenuHandler(Handle UpgradePanel, MenuAction action, int client, int param2) {
	if (action == MenuAction_Select) {
		if (param2 == 1) {
			FakeClientCommand(client, "sm_xem");
		}
		else if (param2 == 2) {


				FakeClientCommand(client, "sm_choi");
...
void AutoDifficultyCheck()
{
	...
	if (TankHpMulti.BoolValue){   // if not 0
			int TankHP = RoundFloat(65000.0*TankHp_Multi);
			//FindConVar("z_tank_health").IntValue = TankHP;
			SetConVarInt(FindConVar("z_tank_health"), TankHP, false, false);
	}
	...
	if (CiSpMulti.BoolValue){	 
		//SetConVarInt(FindConVar("z_mob_spawn_finale_size"), RoundToNearest(20 * spawn_multi), false, false);
		//SetConVarInt(FindConVar("z_mob_spawn_max_size"), RoundToNearest(30 * spawn_multi), false, false);
		//SetConVarInt(FindConVar("z_mob_spawn_min_size"), RoundToNearest(10 * spawn_multi), false, false);
		//SetConVarInt(FindConVar("z_mob_spawn_finale_size"), RoundToNearest(20 * spawn_multi), false, false);
		//SetConVarInt(FindConVar("z_mega_mob_size"), RoundToNearest(50 * spawn_multi), false, false);
		//SetConVarInt(FindConVar("z_common_limit"), RoundToNearest(30 * spawn_multi), false, false);
	  //FindConVar("z_health").IntValue					= RoundToNearest(50 * spawn_multi);
	
		SetConVarInt(FindConVar("z_mob_spawn_max_interval_easy"), RoundToFloor(240.0 / spawn_multi), false, false);
		SetConVarInt(FindConVar("z_mob_spawn_max_interval_normal"), RoundToFloor(180.0 / spawn_multi), false, false);
		SetConVarInt(FindConVar("z_mob_spawn_max_interval_hard"), RoundToFloor(180.0 / spawn_multi), false, false);
		SetConVarInt(FindConVar("z_mob_spawn_max_interval_expert"), RoundToFloor(180.0 / spawn_multi), false, false);
		SetConVarInt(FindConVar("z_mob_spawn_min_interval_easy"), RoundToFloor(120.0 / spawn_multi), false, false);
		SetConVarInt(FindConVar("z_mob_spawn_min_interval_normal"), RoundToFloor( 90.0 / spawn_multi), false, false);		
		SetConVarInt(FindConVar("z_mob_spawn_min_interval_hard"), RoundToFloor( 90.0 / spawn_multi), false, false);
		SetConVarInt(FindConVar("z_mob_spawn_min_interval_expert"), RoundToFloor( 90.0 / spawn_multi), false, false);
		
		SetConVarInt(FindConVar("z_mega_mob_spawn_max_interval"), RoundToFloor(900.0 / spawn_multi), false, false);
		SetConVarInt(FindConVar("z_mega_mob_spawn_min_interval"), RoundToFloor(420.0 / spawn_multi), false, false);
		//SetConVarInt(FindConVar("director_special_respawn_interval"), RoundToFloor( 45.0 / spawn_multi), false, false);
		//SetConVarInt(FindConVar("director_special_battlefield_respawn_interval"), RoundToFloor( 10.0 / spawn_multi), false, false);
		//SetConVarInt(FindConVar("director_special_finale_offer_length"), RoundToFloor( 10.0 / spawn_multi), false, false);
		//SetConVarInt(FindConVar("director_special_initial_spawn_delay_max"), RoundToFloor( 60.0 / spawn_multi), false, false);
		//SetConVarInt(FindConVar("director_special_initial_spawn_delay_max_extra"), RoundToFloor(180.0 / spawn_multi), false, false);
		//SetConVarInt(FindConVar("director_special_initial_spawn_delay_min"), RoundToFloor( 30.0 / spawn_multi), false, false);
		//SetConVarInt(FindConVar("director_special_original_offer_length"), RoundToFloor( 30.0 / spawn_multi), false, false);
	}

	// More survivors = more SI health
	float sihp_Multi = 1 + GetConVarFloat(SiHpMulti)*extrasurvivors;
	if (SiHpMulti.BoolValue){		
		SetConVarInt(FindConVar("z_gas_health"), RoundToCeil(   250.0 * sihp_Multi), false, false);
		SetConVarInt(FindConVar("z_hunter_health"), RoundToNearest(250.0 * sihp_Multi), false, false);
		SetConVarInt(FindConVar("z_exploding_health"), RoundToNearest( 50.0 * sihp_Multi), false, false);
		SetConVarInt(FindConVar("z_spitter_health"), RoundToCeil(   100.0 * sihp_Multi), false, false);
		SetConVarInt(FindConVar("z_charger_health"), RoundToNearest(600.0 * sihp_Multi), false, false);
		SetConVarInt(FindConVar("z_jockey_health"), RoundToNearest(325.0 * sihp_Multi), false, false);
	}
	...
	if(!InfectedAllowed && SiSpMore.BoolValue && l4dt() && !StrEqual(gameMode, "survival"))    // Not in survival, versus, scavenge or realism versus, l4dt required
	{
		...
		SetConVarInt(FindConVar("z_minion_limit"), MaxSpecials, false, false); 	// For L4D1?
	...
	/*	// Increase limits of infected classes
		...
		{
			SetConVarInt(FindConVar(iType[i]), SIperclass, false, false);  // Increase each SI class limit
		} */
	}
	PrintConsoleToAll("[SV] - Tank HP: %.0f%%\tSI HP: %.0f%%\tCI spawn rate: %.0f%%\tMaxSpecials: %d", 100.0*TankHp_Multi, 100.0*sihp_Multi, 100.0*spawn_multi, MaxSpecials);
}
...
void PrintConsoleToAll(const char[] format, any ...) 
{ 
	char text[192];

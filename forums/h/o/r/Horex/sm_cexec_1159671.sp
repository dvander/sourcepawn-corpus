...
public Action:ClientExec (client, args)
{
    ...
    static iClient = -1, iMaxClients = 0, pos = 0;
    ...
    if (args > 2) {
        ...
        new lastchar = strlen(szCommand) - 1; 
        if (szCommand[lastchar] == '"') { 
            szCommand[lastchar] = '\0'; 
        }
        for(new i=0;i<3;i++) {
            pos = FindCharInString(szCommand, '"', false)+1;
            strcopy(szCommand, sizeof(szCommand), szCommand[pos]);
            pos=0;
        }
        if (szCommand[0] == '"') {
            strcopy(szCommand, sizeof(szCommand), szCommand[1]);
        }
    }
    if (args == 2)
    {

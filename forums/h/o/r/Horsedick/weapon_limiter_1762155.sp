...
#define PLUGIN_DESCRIPTION				"Allows "
...
// ********************* //
// configuration file
#define KV_ConfigurationFile			"configs/weapon_limits.cfg"
...
public OnPluginStart()
{
	 ...
	 *   Finish Up  *
	 ****************/
	ReplyToCommand(0, "%s %s loaded luccessfully", PREFIX_COMMAND, PLUGIN_NAME);
	ReplyToCommand(0, "%s Version %s", PREFIX_COMMAND, PLUGIN_VERSION);
...
public Action:Command_PrintLimits(_client, _args)
{
	if (g_PreparedQueryLimitsAll == INVALID_HANDLE) {
		new String:error[STRLEN_DBError];
	...
	if (count > 0) {
		// limit field vars
		new String:limitItemClass[STRLEN_ItemClass];
...
public Action:Command_PrintWhitelist(_client, _args)
{
	...
	if (count > 0) {
		// limit field vars
		new String:playerID[STRLEN_ItemClass];
		new String:itemClass[STRLEN_ItemClass];
...
bool:Query_RetrieveSlotDetails(const String:_playerAuthID[], _itemSlot, &_itemDefinition, &_equipTimestamp, String:_itemClass[], _itemClassLength)
{
	...
	if (SQL_FetchRow(g_PreparedQuerySlotSelectDetails)) {
		new String:fetchedItemClass[STRLEN_ItemClass];
...
ParseConfigurationLimitsVault(Handle:_vaultLimits)
{
	if (KvGotoFirstSubKey(_vaultLimits)) {
		new String:playerClassString[STRLEN_PlayerClass];
		new String:itemSlotString[STRLEN_ItemSlot];
		new String:itemClass[STRLEN_ItemClass];
		new itemDefinition;
		new limitBlueTeam;
		new limitRedTeam;
...
ParseConfigurationWhitelistVault(Handle:_whitelistLimits)
{
	if (KvGotoFirstSubKey(_whitelistLimits)) {
		new String:playerAuthString[STRLEN_AuthString];
		new String:itemSlotString[STRLEN_ItemSlot];
		new String:itemClass[STRLEN_ItemClass];
		new itemDefinition;
		new itemSlot;
...
{
	new String:configurationFullPath[255];
	BuildPath(Path_SM, configurationFullPath, sizeof(configurationFullPath), KV_ConfigurationFile);
	
	if(!FileExists(configurationFullPath)) {
		PrintToServer("%s No configuration file found, limits can still be specified through the console.", PREFIX_COMMAND);
		return true;
	}
	//
...
UpdateClientWeapons(_client, bool:_disconnected)
{
	...
	new String:playerName[STRLEN_PlayerName];
	...
	if (_disconnected) {
		...
		new String:itemClass[STRLEN_ItemClass];
		new itemDefinition;
		new String:itemName[STRLEN_ItemName];
		new timestamp;
		// existing information
		new existingItemDefinition;
		new String:existingItemClass[STRLEN_ItemClass];
		...
		for (new slot = 0; slot <= TFWeaponSlot_Item2; slot++)
		{
			...
			if (weapon >= 0) {
				...
				if (!g_PluginEnabled || (Query_PlayerHasWhitelistEntry(playerAuthString, slot, itemDefinition, itemClass) || WeaponCanEquip(itemClass, itemDefinition, slot, playerAuthString, playerClass, playerTeam))) {
					...
					PrintToServer("%s Removed %s from %s.", PREFIX_COMMAND, itemName, playerName);
...
bool:GetMatchingSlotCounts(const String:_limitItemClass[], _limitItemDefinition, _limitItemSlot, _limitPlayerClass, const String:_currentPlayerID[], _currentPlayerTeam, &_countTeam, &_countTotal)
{
	...
	new String:paramString[STRLEN_NumberKey];
...
bool:CountSlotQueryResults(Handle:_queryHandle, _limitPlayerClass, const String:_currentPlayerID[], _currentPlayerTeam, &_countTeam, &_countTotal)
{
	...
	//
	new String:slotPlayerID[STRLEN_AuthString];
 ...
 *    \_$$  _|$$ |                                           $$  __$$\                                                                 
 *      $$ |$$$$$$\    $$$$$$\  $$$$$$\$$$$\   $$$$$$$\      $$ /  \__| $$$$$$\  $$$$$$\$$$$\   $$$$$$\                                
 *      $$ |\_$$  _|  $$  __$$\ $$  _$$  _$$\ $$  _____|     $$ |$$$$\  \____$$\ $$  _$$  _$$\ $$  __$$\                               
 *      $$ |  $$ |    $$$$$$$$ |$$ / $$ / $$ |\$$$$$$\       $$ |\_$$ | $$$$$$$ |$$ / $$ / $$ |$$$$$$$$ |                              
 *      $$ |  $$ |$$\ $$   ____|$$ | $$ | $$ | \____$$\      $$ |  $$ |$$  __$$ |$$ | $$ | $$ |$$   ____|                              
 *    $$$$$$\ \$$$$  |\$$$$$$$\ $$ | $$ | $$ |$$$$$$$  |     \$$$$$$  |\$$$$$$$ |$$ | $$ | $$ |\$$$$$$$\                               
 *    \______| \____/  \_______|\__| \__| \__|\_______/$$$$$$\\______/  \_______|\__| \__| \__| \_______|
 */
...
GetItemName(_itemDefinitionIndex, String:_value[], _maxlength)
{
	...
	if (KvJumpToKey(g_VaultItemsGame, KV_KeyItemsGameItems, false)) {
		new String:itemDefinitionKey[STRLEN_NumberKey];

...
public Action:Timer_Announce(Handle:timer)
{
	...
	if(Announce>1.0 && Enabled2)
	{
		switch(announcecount)
		{
			case 1:
			{
				CPrintToChatAll("{olive}[FF2]{default} 欢迎加入咖啡厅QQ群: {olive}165916870{default}");
			}
			case 3:
			{
				CPrintToChatAll("{default} === Freak Fortress 2 (由{orange}热可可{default}修改) === ", PLUGIN_VERSION);
			}
			case 4:
			{
...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:item)
{
	...
	switch(iItemDefinitionIndex)
	{
		...
		case 171, 232:  //The Tribalman's Shiv, The Bushwacka
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "204 ; 1 ; 44 ; 1 ; 207 ; 1.40 ; 58 ; 1.39", true);
				//204: On Miss: Hit yourself. Idiot.
				//44: Knockback on the target and shooter
				//207: +40% damage to self
				//58: +39% self damage force
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "1 ; 0.5 ; 107 ; 1.5 ; 128 ; 1 ; 191 ; -30 ; 772 ; 1.5", true);
				//1: -50% damage
				//107: +50% move speed
				//128: Only when weapon is active
				//191: -30 health/second
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.33 ; 76 ; 2 ; 110 ; 10 ; 149 ; 3");
				//2: +33% damage
				//76: +100% ammo
				//110: On Hit: Gain up to 10 health
		...
		case 45, 1078:  //自然之力
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "360 ; 1.0 ; 2 ; 1.4 ; 97 ; 0.9");
				//2: +50% damage
				//97: +25% reload speed
				//360：伤害医疗链接
				//2：伤害+35%
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "204 ; 1 ; 16 ; 50 ; 182 ; 3 ; 191 ; -7 ; 177 ; 1.35", false);
		...
		case 44:  //Sandman
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "6 ; 0.8 ; 360 ; 1.0 ; 269 ; 1 ; 38 ; 1 ; 279 ; 3 ; 57 ; 1 ; 2 ; 1.1", false);
				//6: +15% firing speed
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "177 ; 1.1");
		...
		case 142:  //机械臂
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "26 ; 75 ; 5 ; 0.7 ; 16 ; 60 ; 124 ; 1 ; 353 ; 1 ; 321 ; 0.7");
				//26: +75 max health
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "57 ; 5 ; 113 ; 25");
		...
		case 527:  //寡妇
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "298 ; 10 ; 614 ; 1 ; 732 ; 0.7 ; 178 ; 0.5");
				//298: 每一发-10金属
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "279 ; 5.0");
				//279: 5 ornaments
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
		...
		case 608, 405:  //呆毛鞋子
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "26 ; 100", false);
				//26: +100 max HP
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
		...
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.25 ; 109 ; 0.5 ; 125 ; -100 ; 394 ; 0.85 ; 418 ; 1 ; 419 ; 100 ; 532 ; 0.5 ; 651 ; 0.5 ; 709 ; 1 ; 275 ; 1", true);
				//2: +25% damage bonus
		...
		case 329:  //The Jag
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "92 ; 1.4 ; 204 ; 1 ; 394 ; 0.35 ; 95 ; 0.8 ; 1 ; 0.75 ; 59 ; 0.4");
				//92: Construction hit speed boost increased by 40%
				//204: On Miss: Hit yourself. Idiot.
				//44: Knockback on the target and shooter
				//394: 65% faster firing speed
		...
		case 812, 833:  //The Flying Guillotine
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "16 ; 125 ; 437 ; 1", false);
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
		...
		case 810, 831, 933, 1080, 1102:  //Sapper
		{
			new Handle:itemOverride=PrepareItemHandle(item, _, _, "26 ; 15 ; 57 ; 3");
				//57: +3 max health regen
			if(itemOverride!=INVALID_HANDLE)
			{
				item=itemOverride;
				return Plugin_Changed;
	...
	if(TF2_GetPlayerClass(client)==TFClass_DemoMan && (!StrContains(classname, "tf_weapon_pipebomblauncher", false)))
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==1150)  //快速粘弹
		{
			itemOverride=PrepareItemHandle(item, _, _, "401 ; 0.66 ; 299 ; 1 ; 57 ; 2 ; 16 ; 30 ; 135 ; 0.7 ; 112 ; 0.1", false);
				//401: 饥饿66%
				//57: +2hp
				//16: +30 hit
				//135: 70% rocketjump
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "57 ; 4 ; 16 ; 50 ; 408 ; 1 ; 135 ; 0.5 ; 112 ; 0.15 ; 21 ; 0.49", false);
				//401: 不饿了
				//57: 每秒+4
				//16: 命中+50血
				//408: 对非燃烧敌人爆击
	...
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==1089)  //NOPP
		{
			itemOverride=PrepareItemHandle(item, _, _, "401 ; 0.66 ; 299 ; 1 ; 57 ; 4 ; 16 ; 30 ; 135 ; 0.7 ; 112 ; 0.1", false);
				//401: 饥饿66%
				//57: +4hp
				//16: +30 hit
				//135: 70% rocketjump
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "57 ; 2 ; 112 ; 0.15 ; 16 ; 45", false);
				//401: 不饿了
				//57: 每秒+2
				//16: 命中+45血
				//408: 对非燃烧敌人爆击
	...
	if(TF2_GetPlayerClass(client)==TFClass_DemoMan && (!StrContains(classname, "tf_weapon_sword", false)))
	{
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "401 ; 0.66 ; 220 ; 200", false);
				//401: 饥饿66%
				//220: 杀敌+200%血
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "401 ; 0.66 ; 220 ; 150 ; 16 ; 65 ; 737 ; 1", false);
				//401: 不饿了
	...
	if(TF2_GetPlayerClass(client)==TFClass_Scout && (!StrContains(classname, "tf_weapon_pistol", false)))
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==296)  //License to Maim
		{
			itemOverride=PrepareItemHandle(item, _, _, "2 ; 2", false);
				
		}
		else
		{
			itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.5", false);
	...
	if(TF2_GetPlayerClass(client)==TFClass_Engineer && (!StrContains(classname, "tf_weapon_pistol", false)))
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==296)  //License to Maim
		{
	...
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==59)  //The Dead Ringer
		{
			itemOverride=PrepareItemHandle(item, _, _, "401 ; 0.66", false);
				//401: 饥饿66%
				
		}
		else
		{
			itemOverride=PrepareItemHandle(item, _, _, "50 ; 0.6 ; 401 ; 0.66", false);
				//401: 不饿了
				//50: Absorbs -40% damage while cloaked
		}
	...
	if(TF2_GetPlayerClass(client)==TFClass_Spy && (!StrContains(classname, "tf_weapon_knife", false)))
	{
		new Handle:itemOverride;
	...
	if(TF2_GetPlayerClass(client)==TFClass_Heavy && (!StrContains(classname, "tf_weapon_minigun", false)))
	{
		new Handle:itemOverride;
		if(iItemDefinitionIndex==811)  //The Huo-Long Heater
		{
			itemOverride=PrepareItemHandle(item, _, _, "16 ; 3 ; 20 ; 1 ; 60 ; 0.5 ; 869 ; 1 ; 1 ; 0.77 ; 77 ; 0.5 ; 431 ; 11 ; 112 ; 0.09", false);
				//16: 击中+3血
				//20: 对燃烧敌人爆击
				//60: 50%火焰抗性
				//869: 只有迷你爆击
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "112 ; 0.15 ; 149 ; 3", false);
	...
	if(TF2_GetPlayerClass(client)==TFClass_Soldier && (!StrContains(classname, "tf_weapon_rocketlauncher", false) || !StrContains(classname, "tf_weapon_shotgun", false)))  //火箭
	{
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "114 ; 1 ; 179 ; 1.0 ; 57 ; 2 ; 112 ; 0.1 ; 2 ; 1.4 ; 16 ; 50", false);
				//114: Mini-crits targets launched airborne by explosions, grapple hooks or enemy attacks
		...
		{
			itemOverride=PrepareItemHandle(item, _, _, "114 ; 1 ; 57 ; 3 ; 112 ; 0.15 ; 2 ; 1.2 ; 16 ; 30", false);
				//114: Mini-crits targets launched airborne by explosions, grapple hooks or enemy attacks
				//57: 每秒回复3hP
				//112: +15% ammo per s
		}
	...
	{
		new Handle:itemOverride=PrepareItemHandle(item, _, _, "11 ; 1.5 ; 144 ; 2.0 ; 199 ; 0.75 ; 314 ; 4 ; 123 ; 1.2",true);
	...
	if(!StrContains(classname, "tf_weapon_flamethrower"))  //Flamethrower
	{
		new Handle:itemOverride=PrepareItemHandle(item, _, _, "144 ; 1 ; 98 ; 5 ；26 ；75 ；109 ；1.2 ; 105 ; 0.8 ; 255 ; 1.33 ; 209 ; 1 ; 299 ; 1 ; 112 ; 0.1", false);
			//144: Sets weapon mode - *possibly* the overdose speed effect
			//98: On Hit: +5 health
			//26: 加75最大生命值（无效）
			//109: 从医疗包获得血量为120%
	...
	else if(!StrContains(classname, "tf_weapon_shotgun"))  //Shotgun
	{
		new Handle:itemOverride=PrepareItemHandle(item, _, _, "44 ; 1 ; 216 ; 1 ; 2 ; 1.10", false);
			//44: Knockback on the target and shooter
			//11: Attrib_Knockback
			//2: +10% damage bonus
		if(itemOverride!=INVALID_HANDLE)
			...
			if(attacker<=MaxClients)
			{
				...
				switch(index)
				{
					...
					case 416:  //Market Gardener (courtesy of Chdata)
					{
						...
						{
							damage=(Pow(float(BossHealthMax[boss]), 0.74074)+256.0-(Marketed[client]/128.0*float(BossHealthMax[boss])))/3.2;
							damagetype|=DMG_CRIT;
				...
				if(bIsBackstab)
				{
					damage=BossHealthMax[boss]*(LastBossIndex()+1)*BossLivesMax[boss]*(0.17-Stabbed[boss]/90)/6;
					damagetype|=DMG_CRIT;
					damagecustom=0;

...
public Action OnPlayerHurt(int victim, int &attackerClientIndex, int &inflictor, float &damage, int &damageType, int &sourceWeapon,float damageForce[3], float damagePosition[3], int damagecustom) 
{
    // Not all damage is caused by an entity
    if (!(1 <= attackerClientIndex <= MaxClients) || !IsClientInGame(attackerClientIndex)) 
    {
...
{
    if (1 <= clientIndex <= MaxClients && IsClientInGame(clientIndex)) 
    {
...
{
    return (1 <= clientIndex <= MaxClients && IsClientInGame(clientIndex) && TF2_IsPlayerInCondition(clientIndex, TFCond_CritOnDamage));
}

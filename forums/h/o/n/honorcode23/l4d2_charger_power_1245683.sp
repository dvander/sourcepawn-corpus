...
new Handle:g_h_CvarChargerPower, Handle:g_h_CvarChargerCarry, Handle:g_h_CvarMessageType, Handle:g_h_CvarObjects, Handle:g_h_CvarDisappear, Handle:g_h_CvarDissTime
...
public OnPluginStart()
{
	...
	g_h_CvarDisappear = CreateConVar("l4d2_charge_power_kill", "0", "Kill or destroy the moved object after a while", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 1.0)
	g_h_CvarDissTime = CreateConVar("l4d2_charge_power_kill_time", "120.0", "Time to wait before destroying the moved object", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 15.0)
...
public Action:CheckEntity(Handle:h_Timer, Handle:h_Pack)
{
	...
	if (IsValidEdict(i_Ent))
	{
		...
		else
		{
			...
			if(GetConVarBool(g_h_CvarDisappear))
			{
				CreateTimer(GetConVarFloat(g_h_CvarDissTime), KillTarget, i_Ent);
			}
		}
	}
}

public Action:KillTarget(Handle:timer, any:entity)
{
	if(IsValidEntity(entity) && IsValidEdict(entity))
	{
		AcceptEntityInput(entity, "Kill", -1);
	}
}

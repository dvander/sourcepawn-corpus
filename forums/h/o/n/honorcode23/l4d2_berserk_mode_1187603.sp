...
public OnPluginStart()
{
	...
	g_cvarSurvivorDuration = CreateConVar("l4d2_berserk_mode_survivor_duration", "45", "Amout of time to berserker for survivors", FCVAR_PLUGIN);
	...
	g_cvarAllowZoomKey = CreateConVar("l4d2_berserk_mode_bind_zoom_key", "0", "Allow the +ZOOM (Mouse wheel) to be the default berserker key?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	g_cvarYellLuck = CreateConVar("l4d2_berserk_mode_yell_luck", "2", "Chance of getting the Berserker Yell power (If set to 2, the probability will be of 50%, 3 = 33%, 4 = 25% 5= 20%)", FCVAR_PLUGIN, true, 1.0);
	g_cvarAnnounceType = CreateConVar("l4d2_berserk_mode_announce", "2", "How should the plugin tell the players that the Berserker Mode is ready to be used? (0: DONT ANNOUNCE |1:CHAT| 2:HINT TEXT | 3:CENTER HINT TEXT | 4:INSTRUCTOR HINT)", FCVAR_PLUGIN, true, 0.0, true, 4.0);
	
	//Survivor Options
	g_cvarSurvivorGoal = CreateConVar("l4d2_berserk_mode_goal", "20", "Amount of common infected needed to berserk on survivors", FCVAR_PLUGIN);
	...
	g_cvarEHGoal = CreateConVar("l4d2_berserk_mode_health_goal", "5", "Amount of infected killed on berserker to get health", FCVAR_PLUGIN);
	...
	g_cvarAdrenType = CreateConVar("l4d2_berserk_mode_give_shot_mode", "2", "Should the plugin give adrenaline, or add its effects by itself? 0 = Disable, 1 = Give Shot, 2 = Effects", FCVAR_PLUGIN, true, 0.0, true, 2.0);
	...
	g_cvarEffectType = CreateConVar("l4d2_berserk_mode_animation_type", "0", "What kind of screen effect should we use for berserker? 0 = None, 1 = Fire, 2 = Adrenaline style 3 = Both (May cause low performance)", FCVAR_PLUGIN, true, 0.0, true, 3.0);
	...
	g_cvarEnableImmunity = CreateConVar("l4d2_berserk_mode_god_mode", "1", "Should we give god mode to survivors during Berserker?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_cvarImmunityDuration = CreateConVar("l4d2_berserk_mode_god_mode_time", "5.0", "How long should the God Mode last?", FCVAR_PLUGIN);
	g_cvarGiveLaserSight = CreateConVar("l4d2_berserk_mode_give_laser", "1", "Should the plugin give a laser on berserker?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	g_cvarIncapRestrict = CreateConVar("l4d2_berserk_mode_restrict_incapacitated", "1", "Allow or restrict player from using berserker if they are incapacitated? (1: Allow 0: Dont)", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	g_cvarConvertPermHealth = CreateConVar("l4d2_berserk_mode_convert_permhealth", "1", "Should the survivor's temporary health be converted into permanent health upon berserker usage?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	
	//Infected Options
	g_cvarInfectedGoal = CreateConVar("l4d2_berserk_mode_infected_goal", "200", "Amount of times an infected attacks a survivor to berserker", FCVAR_PLUGIN);
	...
	g_cvarEIHMultiplier = CreateConVar("l4d2_berserk_mode_extra_health_amount", "3.0", "Multiplier for extra infected health (2.0 = Double)", FCVAR_PLUGIN);
	g_cvarEDEnableIncap = CreateConVar("l4d2_berserk_mode_extra_damage_incap", "0", "Should the double damage feature of the infected, be applied to incapacitated survivors?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	g_cvarExInfected = CreateConVar("l4d2_berserk_mode_exclude_infected", "tank", "Infected classes excluded from ALL berserker features separated by comas. (spitter, hunter, charger, tank, jockey, smoker, boomer)", FCVAR_PLUGIN);
	...
	g_cvarLethalBite = CreateConVar("l4d2_berserk_mode_lethal_bite", "0", "Should the plugin enable Lethal Bite feature? (Keeps inflicting damage after a hit, only if the hit was made during berserker)", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	g_cvarInfectedCountType = CreateConVar("l4d2_berserk_mode_infected_count_type", "0", "On what should be the infected count based? (1: Times attacked | 0: Damage dealt)", FCVAR_PLUGIN, true, 0.0, true, 1.0);

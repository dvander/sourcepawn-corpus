...
new Handle:g_h_CvarChargerPower, Handle:g_h_CvarChargerCarry, Handle:g_h_CvarMessageType, Handle:g_h_CvarObjects;
new Handle:g_cvarDelete = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_cvarDelete = CreateConVar("l4d2_charger_power_delete", "1", "Delete the moved object after some time?", FCVAR_PLUGIN, true, 1.0, true, 1.0);
	g_cvarTimeout = CreateConVar("l4d2_charger_power_timeout", "65", "Timeout of the pushed objects?", FCVAR_PLUGIN);
	...
	if (TR_DidHit(h_Trace))
	{
		...
		if (i_Target && IsValidEdict(i_Target) && GetVectorDistance(f_Origin, f_EndOrigin) <= 100.0)
		{
			...
			if (StrEqual(s_ClassName, "prop_physics") || StrEqual(s_ClassName, "prop_car_alarm"))
			{
				...
				if(GetConVarBool(g_cvarDelete))
				{
					CreateTimer(GetConVarFloat(g_cvarTimeout), timerRemoveEntity, i_Target, TIMER_FLAG_NO_MAPCHANGE);
				}

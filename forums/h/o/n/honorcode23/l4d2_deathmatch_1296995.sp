...
new Handle:g_cvarBSCalcDamage = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	//Weapon Manager
	g_cvarWeaponManagerTimeout = CreateConVar("l4d2_deathmatch_weaponmanager_timeout", "30", "Maximum time of a droped weapon to timeout (After this amount of time, the weapon gets removed)", FCVAR_PLUGIN);
	...
	g_cvarBSCalcDamage = CreateConVar("l4d2_deathmatch_boostsystem_amount_damage", "2", "Additional Damage for 'Extra Damage' boost (DEF: 10dmg + (10dmg * 2.0) = 30 dmg)", FCVAR_PLUGIN);
...
BuildBoostMenu(client)
{
	...
	AddMenuItem(menu, "noincap", "No incap on Grenade Launcher hit");
...
public Event_PlayerHurt(Handle:event, String:event_name[], bool:dontBroadcast)
{
	...
	&& GetClientTeam(victim) == 2)
	{
		...
		if(g_bRespawnQueue[victim])
		{
			g_bRespawnQueue[victim] = false;
			PrintToChat(victim, "\x03Reallocate canceled");
		}
							...
							if(!StrEqual(action, "none"))
							{
								switch(GetRandomInt(action_chance, 100))
								{
									case 100:
									{
										...
										{
											SDKCall(sdkCallVomitOnPlayer, attacker, victim, true);
										}
	...
	if(FileFound)
	{
		...
		if(KvJumpToKey(keyvalues, "avoid play"))
		{
			...
			for(new sound_file=1; sound_file<=total_files; sound_file++)
			{
				Format(sound, sizeof(sound), "sound%i", sound_file);
				KvGetString(keyvalues, sound, file_name, sizeof(file_name));
				if(StrContains(sample, file_name) >= 0)
				{
					volume = 0.0;
					CloseHandle(keyvalues);

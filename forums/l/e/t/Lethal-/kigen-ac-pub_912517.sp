...
#define MAX_HACKCVAR 41	/* !!!!!! If you add Cvars you need to change this number to reflect how many there are! !!!!!! */
...
public OnPluginStart()
{
	...
	CVar_BlockNameCopy = CreateConVar("kac_block_namecopy", "0", "Blocks name copying and short names (name < 3)", FCVAR_PROTECTED);
	CVar_IRCChan = CreateConVar("kac_irc_channel", "", "Channel to broadcast KAC messages on.  Set to nothing to disable.", FCVAR_PROTECTED);
	CVar_BanCafe = CreateConVar("kac_block_cafe", "0", "Blocks all Cafe accounts from entering the server.", FCVAR_PROTECTED);
	CVar_BlockMultiByte = CreateConVar("kac_block_multibyte_names", "0", "Blocks the usage of multibyte characters in a name.", FCVAR_PROTECTED);
	CVar_Enable = CreateConVar("kac_enable", "1", "Enables/Disables KAC's cheat detection. (Does not affect other features.)", FCVAR_PROTECTED);
	CVar_CmdSpam = CreateConVar("kac_cmdspam", "15", "Amount of commands in one second before kick.  0 for disable.", FCVAR_PROTECTED);
...
KACBan(client, time, String:IReason[], String:EReason[])
{
	...
	{
		BanClient(client, time, BANFLAG_IP, IReason, EReason, "KAC", 0);
		return;
	}
	else
	{
		...
		BanClient(client, time, BANFLAG_AUTO, IReason, EReason, "KAC", 0);
	}
	*/
	return;

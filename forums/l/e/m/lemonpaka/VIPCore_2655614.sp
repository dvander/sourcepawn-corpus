...
ConVar g_cVIPmessage;
ConVar g_cPlayerSkins;
ConVar g_cTablePrefix;
ConVar g_cVipFlags;
ConVar g_Vip_slots;
ConVar g_Vip_slots_enable;
...
{
	name = "[CS:GO] VIPCore(modified by LemonPAKA)", 
	author = "S4muRaY'", 
	description = "original by S4muRaY', modified by LemonPAKA", 
	version = "1.0", 
...
public void OnPluginStart()
{
	...
	g_cVIPmessage = CreateConVar("sm_vip_message","0", "Enable the VIP join/leave Message? 0 = Disable, 1 = Enable", 0, true, 0.0, true, 1.0);
	...
	g_Vip_slots = CreateConVar("sm_vip_slots","0","Number of Vip player slots, if VIP slots feature disabled ,whatever you input will be 0", 0, true, 0.0);
	g_Vip_slots_enable = CreateConVar("sm_slots_enable","1", "Enable the VIP slots feature? 0 = Disable, 1 = Enable", 0, true, 0.0, true, 1.0);
	...
	RegConsoleCmd("sm_getvip",Command_GetVIP,"Get VIP via cdkey");
}
...
{
	if (g_bVIP[client]&& g_cVIPmessage.BoolValue)
	{
...
public Action Command_GetVIP(int client, int args)
{
	if (args != 1)
	{
		ReplyToCommand(client, "%s Usage: /getvip <cdkey>", PREFIX);
		return Plugin_Handled;
	}
	char szArg[512];
	char query[512];
	...
	Format(query, sizeof(query),"SELECT `DAYS` FROM `%sCode` WHERE VIPKEY='%s';",GetTablePrefix(), szArg);
	g_dDatabase.Query(DaysCallBack,query,client)
	
	FormatEx(szQuery, sizeof(szQuery), "DELETE FROM `%sCode` WHERE `VIPKEY` = '%s'",GetTablePrefix(), szArg);
...
public void DaysCallBack(Database db, DBResultSet results, const char[] error, any data)
{
	...
	PrintToChat(client, "%s Trying to give \x02%N \x01a vip access...", PREFIX, client);
	char szQuery[512];
	if (g_bVIP[client])
		FormatEx(szQuery, sizeof(szQuery), "UPDATE `%sUsers` SET `expireStamp`= '%s' WHERE `authId` = '%s'",  GetTablePrefix(), szNewStamp, g_szAuth[client]);
	else
		FormatEx(szQuery, sizeof(szQuery), "INSERT INTO `%sUsers` (`authId`, `expireStamp`) VALUES ('%s', '%s')", GetTablePrefix(), g_szAuth[client], szStamp);
	...
	{
		PrintToChat(client, "%s cdkey invalid", PREFIX);
	}
}
...
public int Handler_SkinsSelection(Menu menu, MenuAction action, int client, int itemNum)
{
	if (action == MenuAction_Cancel && itemNum == MenuCancel_ExitBack)
	{
		...
		else
		{
			...
			szInfo = "";   //fix database model set 0 crash server
		}
...
void SQL_MakeConnection()
{
	...
	g_dDatabase.Query(SQL_CheckForErrors, szQuery);
	FormatEx(szQuery, sizeof(szQuery), "CREATE TABLE IF NOT EXISTS `%sCode`(`VIPKEY` VARCHAR(32) NOT NULL, `DAYS` INT(10) NOT NULL DEFAULT 30)", GetTablePrefix());
	g_dDatabase.Query(SQL_CheckForErrors, szQuery);
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i))
...
public void SQL_FetchUser_CB(Database db, DBResultSet results, const char[] error, any data)
{
	...
	int reserved = g_Vip_slots.IntValue;
	if(!g_Vip_slots_enable)
		reserved = 0;
	...
	if (results.FetchRow())
	{
		...
		if (iDaysLeft > 0)
		{
			...
			if ( limit< onlineusers)
			{
					int target = SelectKickClient();
						
					if (target && g_Vip_slots_enable.BoolValue)
					{
					...
					{				
						...
						CreateTimer(0.1, OnTimedKick, iClient);
					}
			}
			/* */
		...
		{
			char szQuery[512];
			FormatEx(szQuery, sizeof(szQuery), "DELETE FROM `%sUsers` WHERE `authId` = '%s'", GetTablePrefix(), g_szAuth[iClient]);
...
{	
	...
	if(g_Vip_slots_enable.BoolValue)
		KickClient(client, "Server is full, join our community for VIP slots");
	else

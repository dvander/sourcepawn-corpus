...
#define CHAT_TAG			"\x03[Uçak kazası] \x05"
...
public void OnPluginStart()
{
	...
	g_hMenuVMaxs.SetTitle("Uçak kazası - VMaksimum Tetikle");
	...
	g_hMenuVMins.SetTitle("Uçak kazası - VDakika Tetikle");
	...
	g_hMenuPos.AddItem("", "Y - 1.0");
	g_hMenuPos.AddItem("", "Z - 1.0");
	g_hMenuPos.AddItem("", "KAYDET");
	g_hMenuPos.SetTitle("Uçak kazası - Başlangıç Ayarla");
...
Action CmdPlaneClear(int client, int args)
{
	ResetPlugin();
	if( client )
		PrintToChat(client, "%sBu haritadan temizlendi.", CHAT_TAG);
	else
		PrintToChat(client, "[Uçak kazası] Bu haritadan temizlendi.");
...
Action CmdPlaneTime(int client, int args)
{
	if( !client )
	{
		ReplyToCommand(client, "[Uçak kazası] Yalnızca komut kullanılabilir %s", IsDedicatedServer() ? "in game on a dedicated server." : "in chat on a Listen server.");
		return Plugin_Handled;
	}
	...
	if( FileExists(sPath) )
	{
		...
		if( hFile.JumpToKey(sMap) )
		{
			...
			if( value == 0 )
			{
				...
				PrintToChat(client, "%sZaman tetikleyicisi kaldırıldı.", CHAT_TAG);
			}
			else
			{
				hFile.SetNum("time", value);
				PrintToChat(client, "%sUçak kazası tetiklenene kadar kaydedilen saniye sayısı.", CHAT_TAG);
			}
		...
		{
			PrintToChat(client, "%sHiçbiri bu haritaya kaydedilmedi.", CHAT_TAG);
		}
...
void ShowMenuMain(int client)
{
	...
	hMenu.AddItem("1", "Geçici Kaza Oluştur");

	if( g_iSaved )
		hMenu.AddItem("2", "Kazayı Sil");
	else
		hMenu.AddItem("2", "Kazayı Kaydet");

	hMenu.AddItem("3", "Kaydedilmiş Kazayı Göster");
	hMenu.AddItem("4", "Kazayı Temizle");

	if( IsValidEntRef(g_iTrigger) )
	{
		hMenu.AddItem("5", "Tetikleyici Sil");
		if( g_hTimerBeam == null )
			hMenu.AddItem("6", "Tetikleyiciyi Göster");
		else
			hMenu.AddItem("6", "Tetikleyiciyi Gizle");
		hMenu.AddItem("7", "VMaksimum Tetikleyici");
		hMenu.AddItem("8", "VDakika Tetikleyici");
		hMenu.AddItem("9", "Tetikleyici Başlangıcı");
	}
	else
	{
		hMenu.AddItem("5", "Tetikleyici Oluştur");
	}
	hMenu.SetTitle("Uçak kazası");
	...
	else if( action == MenuAction_Select )
	{
		...
		switch( index )
		{
			...
			case 3:
			{
				...
				{
					PrintToChat(client, "%sCanlanma kurtarılan uçak kazasında", CHAT_TAG);
				...
				else
				{
					...
					{
						PrintToChat(client, "%sKaydedilmiş uçak kazası yok", CHAT_TAG);
					}
				}
				ShowMenuMain(client);
			}
	...
	else if( action == MenuAction_Select )
	{
		...
		else
		{
			...
			if( hFile.JumpToKey(sMap, true) )
			{
				...
				hFile.Rewind();
				hFile.ExportToFile(sPath);
				PrintToChat(client, "%sKayıtlı tetikleme kaynağı.", CHAT_TAG);
			}
			else
			{
				PrintToChat(client, "%sTetikleme kaynağı kaydedilemedi.", CHAT_TAG);
			}
...
void SaveCrash(int client)
{
	...
	if( g_iSaved )
	{
		...
		if( FileExists(sPath) )
		{
			...
			if( hFile.JumpToKey(sMap) )
			{
				...
				PrintToChat(client, "%sBu haritadan kaldırıldı.", CHAT_TAG);
			}
			else
			{
				PrintToChat(client, "%sHiçbiri bu haritaya kaydedilmedi.", CHAT_TAG);
	...
	else
	{
		...
		if( hFile.JumpToKey(sMap, true) )
		{
			...
			PrintToChat(client, "%sBu haritaya kaydedildi.", CHAT_TAG);
		}
		else
		{
			PrintToChat(client, "%sBu haritaya kaydedilemedi.", CHAT_TAG);
		}
	...
	if( IsValidEntRef(g_iTrigger) == true )
	{
		...
		if( FileExists(sPath) )
		{
			...
			if( hFile.JumpToKey(sMap) )
			{
				...
				PrintToChat(client, "%sBu haritadaki tetikleyici silindi.", CHAT_TAG);
			}
			else
			{
				PrintToChat(client, "%sSilinecek tetik yok!", CHAT_TAG);
			}
...
void CreateCrash(int client)
{
	...
	CreateUçak kazası(vPos, vAng, method);
...
void CreateUçak kazası(float vPos[3], float vAng[3], int method)
{
	float vLoc[3];

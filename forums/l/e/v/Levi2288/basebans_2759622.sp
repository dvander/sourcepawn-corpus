...
public Action Command_BanIp(int client, int args)
{
	if (args < 2)
	{
		CReplyToCommand(client, "[SM] Usage: sm_banip <ip|#userid|name> <time> [reason]");
	...
	if (StrEqual(arg, "0"))
	{
		CReplyToCommand(client, "[SM] %t", "Cannot ban that IP");
		return Plugin_Handled;
	}
	...
	{
		CReplyToCommand(client, "[SM] %t", "No Access");
		return Plugin_Handled;
	}
	...
	CReplyToCommand(client, "[SM] %t", "Ban added");
...
public Action Command_AddBan(int client, int args)
{
	if (args < 2)
	{
		CReplyToCommand(client, "[SM] Usage: sm_addban <time> <steamid> [reason]");
		return Plugin_Handled;
	}
	...
	if (!idValid)
	{
		CReplyToCommand(client, "[SM] %t", "Invalid SteamID specified");
		return Plugin_Handled;
	}
...
public Action Command_Unban(int client, int args)
{
	if (args < 1)
	{
		CReplyToCommand(client, "[SM] Usage: sm_unban <steamid|ip>");
		return Plugin_Handled;
	}
	...
	CReplyToCommand(client, "[SM] %t", "Removed bans matching", arg);
...
public Action Command_AbortBan(int client, int args)
{
	...
	{
		playerinfo[client].isWaitingForChatReason = false;
		CReplyToCommand(client, "[SM] %t", "AbortBan applied successfully");
	}
	else
	{
		CReplyToCommand(client, "[SM] %t", "AbortBan not waiting for custom reason");
	}

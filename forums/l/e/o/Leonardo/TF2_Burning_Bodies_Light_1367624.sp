...
public OnGameFrame()
{
	for(new iClient = 1; iClient <= MaxClients; iClient++)
		...
		{
			if(IsValidClient(iClient) && IsPlayerAlive(iClient) && (GetEntData(iClient, g_iOffsetPlayerCond) & TF_CONDFLAG_ONFIRE)==TF_CONDFLAG_ONFIRE && GetConVarBool(g_hEnabled))
			{
				if(g_iBody[iClient] == -1)
				{
					new iLightEntity = CreateLightEntity(EntRefToEntIndex(iClient));
					if(IsLightEntity(iLightEntity))
						g_iBody[iClient] = iLightEntity;
				}
			}
			else
			{
				if(g_iBody[iClient] != -1)
				{
					new iLightEntity = g_iBody[iClient];
					if(IsLightEntity(iLightEntity))
						RemoveEdict(iLightEntity);
					g_iBody[iClient] = -1;
				}

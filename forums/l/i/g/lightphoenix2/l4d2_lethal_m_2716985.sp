...
iReloadReType LoadData(KeyValues hFile, const char[] szPath)
{
	if (hFile.JumpToKey(MyName))
	{
		...
		l4d2_lw_shootonce		= view_as<bool>(hFile.GetNum("Once per round", 1));
		g_bDamageAlly			= view_as<bool>(hFile.GetNum("Friendly Fire", 1));
		l4d2_lw_friendlyfire	= view_as<bool>(hFile.GetNum("Friendly fire indirect damage", 1));
		l4d2_lw_scout			= view_as<bool>(hFile.GetNum("Enable Lethal Weapon for Scout",1));
		l4d2_lw_awp				= view_as<bool>(hFile.GetNum("Enable Lethal Weapon for AWP", 1));
		l4d2_lw_huntingrifle	= view_as<bool>(hFile.GetNum("Enable Lethal Weapon for Hunting Rifle", 1));
		l4d2_lw_g3sg1			= view_as<bool>(hFile.GetNum("Enable Lethal Weapon for G3SG1", 1));
		l4d2_lw_laseroffset		= hFile.GetNum("Tracker offeset");
		
		// Additional ConVars
		l4d2_lw_flash				= view_as<bool>(hFile.GetNum("Enable screen flash",1));
		l4d2_lw_chargingsound		= view_as<bool>(hFile.GetNum("Enable charging sound", 1));
		l4d2_lw_chargedsound		= view_as<bool>(hFile.GetNum("Enable charged up sound", 1));
		l4d2_lw_moveandcharge		= view_as<bool>(hFile.GetNum("Enable charging while crouched and moving", 1));
		l4d2_lw_chargeparticle		= view_as<bool>(hFile.GetNum("Enable showing electric particles when charged", 1));
		l4d2_lw_useammo				= view_as<bool>(hFile.GetNum("Enable and require use of addtional ammunition", 1));
		l4d2_lw_useammocount		= hFile.GetNum("Ammo per Lethal Shoot");
		l4d2_lw_shake				= view_as<bool>(hFile.GetNum("Enable screen shake during explosion", 1));
		l4d2_lw_shake_intensity		= hFile.GetFloat("Intensity of screen shake");
		l4d2_lw_shake_shooteronly	= view_as<bool>(hFile.GetNum("Only the shooter screen shake", 1));
		g_iFireLifetime				= hFile.GetNum("Seconds for flame to exist");
		l4d2_lw_adminonly			= view_as<bool>(hFile.GetNum("Admin only", 1));
...
public Action Event_Infected_Hurt(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (client != 0 && ReleaseLock[client])
	{
		...
		int damage = l4d2_lw_lethaldamage;
		...
		if (l4d2_lw_lethalweapon != 2)
			Smash(client, target, l4d2_lw_lethalforce, 1.5, 2.0);
...
//		GetClientAbsOrigin(client, AttackPosition);
		...
		/* Deal lethal damage */
		if ((GetClientTeam(client) != GetClientTeam(target)) || g_bDamageAlly) {
//			SetEntProp(target, Prop_Data, "m_iHealth", health - damage);
			DamageEntity(client, target, damage);
		}
	}
...
public Action ChargeTimer(Handle timer, any client)
{
	...
	/* Check how much ammo left in the weapon */
	iAmmoLeft = GetEntData(client, g_iAmmoOffset + iAmmoOffsetToAdd);

...
public OnPluginStart()
{
	...
	TankAnnounce.AddChangeHook(ConVarChanged_Allow);
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	int client = attacker;
	int target = victim;
	int nums = (GetClientUserId(victim)-8);
	...
	/* Notify Tank health */
	if(GetEntProp(target, Prop_Send, "m_zombieClass") == 8 && TH_CvarAllow)
	{
		new i, j;
		...
		for(j=RoundToCeil(GaugeNum); j<80; j++)
			HealthBar[j] = ' ';
		HealthBar[80] = '\0';
		if(GetEntProp(target, Prop_Send, "m_zombieClass") == 8 && tankalive  && Health <= MaxHealth[target])
		{
			if(totaltank > 1)
			{
				if(TH_iHint == 0)
					PrintCenterTextAll("(%d)TANK %4.0f/%4.0f  %s",nums, Health, MaxHealth[target], HealthBar);
				else
					PrintHintTextToAll("(%d)TANK %4.0f/%4.0f  %s",nums ,Health, MaxHealth[target], HealthBar);
			}
			else
			{

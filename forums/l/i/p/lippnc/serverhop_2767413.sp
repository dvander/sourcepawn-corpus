...
public OnPluginStart()
{
  ...
  cv_advert_interval = CreateConVar("sm_hop_advertisement_interval", "6", "Advertisement interval: advertise a server every x minute(s)");

  AutoExecConfig(true, "serverhop");
...
public MenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
  if (action == MenuAction_Select) {
    ...
    if (GetConVarBool(cv_broadcasthops)) {
      ...
      PrintToChatAll("%t %t", "Tag", "HopNotification", clientName, serverInfo[serverNum]);
    }
  }
}
...
public Action:Advertise()
{
  ...
  if (strlen(serverInfo[advertCount]) > 0) {
    PrintToChatAll("%t %t", "Tag", "Advert", serverInfo[advertCount], trigger);

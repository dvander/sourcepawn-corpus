...
// Programmed by Rikka modded by Huck   //
//////////////////////////////////////////
...
/*==================================================================================================
	...
	*** REQUIRES l4d2_mission_manager ***
	...
	Change Log
		----- Huck's upgraded version -----
		v2.3.2h (Aug 3, 2021)	- Use DisplayTitle for correct naming in vote menu system
								- Added cvar acs_wait_time_switch_coop for changelevel wait time in coop mode is 67 seconds so all stats can be viewed before map changes
								- Added cvar acs_wait_time_chmap_coop for !chmap changelevel wait time can be set to 5 seconds
								- Map and mission phrases for 350+ campaigns 
								- added ServerCommand("sm plugins unload xxxxxxxxxxx") and ServerCommand("sm_cvar l4d2_xxxxxx 1") under public Action Timer_AdvertiseNextMap so that custom cvars and plugins that allow for glitching the finale can be unloaded - Requires pluginmanager.smx
...
ConVar g_hCVar_WAIT_TIME_BEFORE_SWITCH_COOP;
ConVar g_hCVar_WAIT_TIME_BEFORE_CHMAP_COOP;
...
void CreateChmapTimer(const char[] mapName) 
{
	float delay = 5.0;
	switch (g_iGameMode) 
	{
		case LMM_GAMEMODE_COOP: {delay=g_hCVar_WAIT_TIME_BEFORE_CHMAP_COOP.FloatValue;}
...
void CreateChangeMapTimer(const char[] mapName) 
{
	...
	switch (g_iGameMode) 
	{
		case LMM_GAMEMODE_COOP: {delay=g_hCVar_WAIT_TIME_BEFORE_SWITCH_COOP.FloatValue;}
		case LMM_GAMEMODE_VERSUS: {delay=WAIT_TIME_BEFORE_SWITCH_VERSUS;}
		case LMM_GAMEMODE_SCAVENGE: {delay=WAIT_TIME_BEFORE_SWITCH_SCAVENGE;}
		case LMM_GAMEMODE_SURVIVAL: {delay=WAIT_TIME_BEFORE_SWITCH_SURVIVAL;}
	}
...
public Plugin myinfo = 
{
	...
	author = "Rikka0w0, Chris Pringle modded by Huck",
...
public void OnPluginStart() 
{
	...
	g_hCVar_VotingAdDelayTime = CreateConVar("acs_voting_ad_delay_time", "50.0", "Time, in seconds, to wait after survivors leave the start area to advertise voting as defined in acs_voting_ad_mode\n * Note: If the server is up, changing this in the .cfg file takes two map changes before the change takes place *", _, true, 5.0, false);
	...
	g_hCVar_NextMapMenuOrder= CreateConVar("acs_next_map_menu_order", "1", "Controls the order of maps shown in the next map voting menu [0 = Official then addon maps(Default), 1 = Random]");
	...
	g_hCVar_NextMapAdInterval = CreateConVar("acs_next_map_ad_interval", "180.0", "The time, in seconds, between advertisements for the next campaign/map on finales and scavenge maps", _, true, 30.0, false);
	...
	g_hCVar_ChMapBroadcastInterval =  CreateConVar("acs_chmap_broadcast_interval", "435.0", "Controls the frequency of the \"!chmap\" advertisement, in second.");	
	...
	g_hCVar_WAIT_TIME_BEFORE_SWITCH_COOP = CreateConVar("acs_wait_time_switch_coop", "67.0", "Time before coop campaign is changed to next campaign.", _, true, 67.0, false);
	g_hCVar_WAIT_TIME_BEFORE_CHMAP_COOP = CreateConVar("acs_wait_time_chmap_coop", "5.0", "Time before chmap coop campaign is changed to next campaign.", _, true, 5.0, false);
	...
	HookConVarChange(g_hCVar_WAIT_TIME_BEFORE_SWITCH_COOP, CVarChange_WAIT_TIME_BEFORE_SWITCH_COOP);
	...
	RegAdminCmd("sm_chmap", Command_ChangeMapVote, ADMFLAG_GENERIC);
	RegAdminCmd("sm_chmap2", Command_ChangeMapVote2, ADMFLAG_GENERIC);
...
public void CVarChange_WAIT_TIME_BEFORE_SWITCH_COOP(Handle hCVar, const char[] strOldValue, const char[] strNewValue) 
{
	//If the value was not changed, then do nothing
	if(StrEqual(strOldValue, strNewValue, false))
		return;
...
public void CVarChange_WAIT_TIME_BEFORE_CHMAP_COOP(Handle hCVar, const char[] strOldValue, const char[] strNewValue) 
{
	//If the value was not changed, then do nothing
	if(StrEqual(strOldValue, strNewValue, false))
		return;
...
public void CVarChange_VotingAdDelayTime(Handle hCVar, const char[] strOldValue, const char[] strNewValue) 
{
	...
	{
		PrintToChatAll("[ACS] ConVar changed: Voting advertisement delay time changed to 5.0");
	}
}
...
public void CVarChange_NewMapAdInterval(Handle hCVar, const char[] strOldValue, const char[] strNewValue) 
{
	...
	{
		PrintToChatAll("[ACS] ConVar changed: Next map advertisement interval changed to 30.0");
	}
}
...
{	
	...
	{
	 	ServerCommand("sm_cvar l4d_missile_limit 1");
		ServerCommand("sm plugins unload l4d2_doublejump");
		ServerCommand("sm plugins unload l4d2_machinegun");		
		ServerCommand("sm plugins unload l4d2_together");
		ServerCommand("sm plugins unload l4d2_points_system");
		ServerCommand("sm plugins unload l4d_jetpack");
		ServerCommand("sm plugins unload l4d_anti_rush");
		ServerCommand("sm plugins unload ps_bess");
		ServerCommand("sm plugins unload l4d2_lethal_weapon");
		ServerCommand("sm plugins unload lockdown_system-l4d2_b");
		ServerCommand("sm plugins unload l4d_coop_markers");
	}		
}
...
public Action Event_RoundEnd(Handle hEvent, const char[] strName, bool bDontBroadcast) 
{
	...
	else if(g_iGameMode == GAMEMODE_COOP && OnFinaleOrScavengeMap() == true && g_hCVar_MaxFinaleFailures.IntValue > 0 && g_bFinaleWon == false && ++g_iCoopFinaleFailureCount >= g_hCVar_MaxFinaleFailures.IntValue)
	{
		CheckMapForChange();
	}
	...
	for(int cycleIndex = 0; cycleIndex < ACS_GetMissionCount(g_iGameMode); cycleIndex++) 
	{
		...
		if(StrEqual(strCurrentMap, mapName, false)) 
		{
			for(int client = 1; client <= MaxClients; client++) 
			{
				...
				{
					ACS_GetLocalizedMissionName(g_iGameMode, cycleIndex, client, localizedName, sizeof(localizedName));
					Format(colorizedname, sizeof(colorizedname), "\x04%s\x03", localizedName);
					PrintToChat(client, "\x03[ACS]\x04 %t", "Campaign finished", colorizedname);
				}
			}
				...
				if(IsMapValid(mapName)) 
				{
					for(int client = 1; client <= MaxClients; client++) 
					{
						if(IsClientInGame(client)) 
						{
							...
							Format(colorizedname, sizeof(colorizedname), "\x04%s\x03", localizedName);
							PrintToChat(client, "\x03[ACS]\x04 %t", "Switching to the vote winner", colorizedname);
						}
					}
			...
			if(IsMapValid(mapName)) 
			{
				for(int client = 1; client <= MaxClients; client++) 
				{
					if(IsClientInGame(client)) 
					{
						ACS_GetLocalizedMissionName(g_iGameMode, cycleIndex, client, localizedName, sizeof(localizedName));
						Format(colorizedname, sizeof(colorizedname), "\x04%s\x03", localizedName);
						PrintToChat(client, "\x03[ACS]\x04 %t", "Switching campaign to", colorizedname);
					}
				}
	...
	for(int cycleIndex = 0; cycleIndex < cycleCount; cycleIndex++)	
	{
		...
		for(int mapIndex = 0; mapIndex<mapCount; mapIndex++) 
		{
			...
			if(StrEqual(strCurrentMap, mapName, false)) 
			{
				...
				{	// Last map of a mission
					mapIndex = 0;	// Switch to the first map of the next mission
							
					if(cycleIndex == ACS_GetCycledMissionCount(g_iGameMode) - 1) 
					{	// End of mission cycle
						cycleIndex = 0;
					} 
					else 
					{
				...
				if(IsMapValid(mapName)) 
				{
					for(int client = 1; client <= MaxClients; client++) 
					{
						if(IsClientInGame(client)) 
						{
							LMM_GetMapLocalizedName(g_iGameMode, missionIndex, mapIndex, localizedName, sizeof(localizedName), client);
							Format(colorizedname, sizeof(colorizedname), "\x04%s\x03", localizedName);
							PrintToChat(client, "\x03[ACS]\x04 %t", "Switching map to", colorizedname);
						}
					}	
...
void GetWinnerListForDisplay_Scavenge(int client, char[] strbuf, int maxlen) 
{
	...
	LMM_GetMapLocalizedName(g_iGameMode, missionIndex, mapIndex, localizedName, sizeof(localizedName), client);
	Format(strbuf, maxlen, "\x04%s\x03", localizedName);
...
void DisplayNextMapTo_Scavenge(int client, bool replyCMD) 
{
	...
	if(g_iWinningMapVotes > 0) 
	{
		...
		if(replyCMD) 
		{
			ReplyToCommand(client, "\x03[ACS]\x04 %t", g_iWinningMapIndices_Len == 1 ? "The next map is currently" : "The next map will be one of", strbuf);
		} 
		...
		{
			PrintToChat(client, "\x03[ACS]\x04 %t", g_iWinningMapIndices_Len == 1 ? "The next map is currently" : "The next map will be one of", strbuf);
		}
	} 
	else 
	{
		...
		{
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x03", localizedName);
			ReplyToCommand(client, "\x03[ACS]\x04 %t", "The next map is currently", colorizedName);
		} 
		else if(g_hCVar_NextMapAdMode.IntValue == DISPLAY_MODE_HINT) 
		{
		...
		{
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x03", localizedName);
			PrintToChat(client, "\x03[ACS]\x04 %t", "The next map is currently", colorizedName);
		}
	}
}
...
void DisplayNextMapTo(int client, bool replyCMD) 
{
	...
	if(g_iWinningMapVotes > 0) 
	{
		...
		if(replyCMD) 
		{
			ReplyToCommand(client, "\x03[ACS]\x04 %t", g_iWinningMapIndices_Len == 1 ? "The next campaign is currently" : "The next campaign will be one of", strbuf);
		} 
		else if(g_hCVar_NextMapAdMode.IntValue == DISPLAY_MODE_HINT) 
		{
		...
		{
			PrintToChat(client, "\x03[ACS]\x04 %t", g_iWinningMapIndices_Len == 1 ? "The next campaign is currently" : "The next campaign will be one of", strbuf);
		}
	} 
	else 
	{
		...
		{
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x03", localizedName);
			ReplyToCommand(client, "\x03[ACS]\x04 %t", "The next campaign is currently", colorizedName);
		} 
		else if(g_hCVar_NextMapAdMode.IntValue == DISPLAY_MODE_HINT) 
		{
		...
		{
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x03", localizedName);
			PrintToChat(client, "\x03[ACS]\x04 %t", "The next campaign is currently", colorizedName);
		}
	}
}
	...
	for(int iOption = 0; iOption < iNumberOfOptions; iOption++)	
	{
		...
		if(iMapVotes[iOption] > 0)	{
			if(g_iGameMode == GAMEMODE_SCAVENGE) 
			{
				...
				ReplyToCommand(iClient, "\x04%s: \x03%d %t", localizedName, iMapVotes[iOption], "Votes");
			} 
			else 
			{
...
public Action Timer_DisplayVoteAdToAll(Handle hTimer, any iData) 
{
   ...
   {	
			switch(g_hCVar_VotingAdMode.IntValue) 
			{
				...
				case DISPLAY_MODE_CHAT: PrintToChat(iClient, "\x03[ACS]\x04 %t", "Map vote advertise", "\x04!mapvote\x03", "\x04!mapvotes\x03");
			}			

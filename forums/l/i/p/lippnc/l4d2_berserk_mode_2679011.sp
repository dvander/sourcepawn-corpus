...
public void OnPluginStart()
{
	...
	g_cvarCountExpireTime = CreateConVar("l4d2_berserk_mode_count_expire_time", "15.0", "How much time must pass between kills or attacks to reset counts?", FCVAR_NOTIFY);
	g_cvarInfectedDuration = CreateConVar("l4d2_berserk_mode_infected_duration", "15.0", "Amount of time to berserk for infected", FCVAR_NOTIFY);
	g_cvarSurvivorDuration = CreateConVar("l4d2_berserk_mode_survivor_duration", "30.0", "Amout of time to berserker for survivors", FCVAR_NOTIFY);
	...
	g_cvarKeyToBind = CreateConVar("l4d2_berserk_mode_binding_key", "", "Which key should the plugin bind for berserker? Default is B key", FCVAR_NOTIFY);
	...
	g_cvarEnableMusicShield  = CreateConVar("l4d2_berserk_mode_esthetic_shield", "0", "Should the plugin avoid playing the music if it is anoying? (On finale escapes, tanks on play, etc)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	//Survivor Options
	g_cvarSurvivorGoal = CreateConVar("l4d2_berserk_mode_goal", "60", "Amount of common infected needed to berserk on survivors", FCVAR_NOTIFY);
	...
	g_cvarEffectType = CreateConVar("l4d2_berserk_mode_animation_type", "3", "What kind of screen effect should we use for berserker? 0 = None, 1 = Fire, 2 = Adrenaline style 3 = Both (May cause low performance)", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	...
	g_cvarImmunityDuration = CreateConVar("l4d2_berserk_mode_god_mode_time", "10.0", "How long should the God Mode last?", FCVAR_NOTIFY);
	g_cvarGiveLaserSight = CreateConVar("l4d2_berserk_mode_give_laser", "0", "Should the plugin give a laser on berserker?", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	g_cvarSurvivorSI = CreateConVar("l4d2_berserk_mode_special_infected_kill_count", "1", "Should the Special Infected be counted for berserker too?", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	g_cvarEDEnabled = CreateConVar("l4d2_berserk_mode_extra_damage", "0", "Should the plugin give extra damage for infected on berserker?", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_cvarAbilityImmunity = CreateConVar("l4d2_berserk_mode_atack_inmu", "0", "Hunters, Smokers and Jockeys, can't be killed during their special ability, they must be shoved(On Berserker)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	g_cvarEDExInfected = CreateConVar("l4d2_berserk_mode_extra_damage_exclude", "tank", "Infected classes excluded from extra damage feature separated by comas. (spitter, hunter, charger, tank, jockey, smoker, boomer)", FCVAR_NOTIFY);
	...
	g_cvarEIHExInfected = CreateConVar("l4d2_berserk_mode_extra_health_exclude", "tank", "Infected classes excluded from extra health feature separated by comas.(spitter, hunter, charger, tank, jockey, smoker, boomer)", FCVAR_NOTIFY);
	...
	g_cvarFireShield = CreateConVar("l4d2_berserk_mode_fire_shield", "0", "Should the plugin block berserker infected from being ignited? (Fire Shield)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	g_cvarPummelSafe = CreateConVar("l4d2_berserk_mode_pummel_safe", "0", "Chargers that are pummeling a survivor can't be killed with melee weapons (1:Allow | 0:Dont)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	
	//Commands
	RegConsoleCmd("sm_b", CmdBerserker, "Toggle Berserker if it is enabled, or show current progress if it isn't");
	RegConsoleCmd("sm_z", CmdZerkHelp, "Show Berserker Mode help in console");
...
void Announce(int client)
{
	...
	switch(GetConVarInt(g_cvarAnnounceType))
	{
		case 1:
		{
			PrintToChat(client, "\x04Berserker mode is ready! Type \x03!b \x04in chat to activate it!");
		}
		case 2:
		{
			PrintHintText(client, "Berserk mode ready! Type !b in chat to activate!");
		}
		case 3:
		{
			PrintCenterText(client, "Berserk mode ready! Type !b in chat to activate!");
		}
		case 4:
		{
			IntToString(GetClientUserId(client) + 25, sUser, sizeof(sUser));

...
//ConVar
ConVar hMaxSurvivors, hDeadBotTime, hSpecCheckInterval, hFirstWeapon, hSecondWeapon, hThirdWeapon, hFourthWeapon, hFifthWeapon, hRespawnHP, hRespawnBuffHP, hStripBotWeapons, hSpawnSurvivorsAtStart;

//value
int iMaxSurvivors, iDeadBotTime, g_iFirstWeapon, g_iSecondWeapon, g_iThirdWeapon, g_iFourthWeapon, g_iFifthWeapon, iRespawnHP, iRespawnBuffHP;
...
{
	// Load translation
	LoadTranslations("l4d2_multislots.phrases");
	...
	// Register cvars
	hMaxSurvivors = CreateConVar("l4d2_multislots_max_survivors", "4", "Kick AI Survivor bots if numbers of survivors has exceeded certain value. (does not kick real player, minimum is 4)", CVAR_FLAGS, true, 4.0, true, 32.0);
	hStripBotWeapons = CreateConVar("l4d2_multislots_bot_items_delete", "1", "Delete all items form survivor bots when they got kicked by this plugin. (0=off)", CVAR_FLAGS, true, 0.0, true, 1.0);
	hDeadBotTime = CreateConVar("l4d2_multislots_alive_bot_time", "0", "When 5+ new player joins the server but no any bot can be taken over, the player will appear as a dead survivor if survivors have left start safe area for at least X seconds. (0=Always spawn alive bot for new player)", CVAR_FLAGS, true, 0.0);
	hSpecCheckInterval = CreateConVar("l4d2_multislots_spec_message_interval", "20", "Setup time interval the instruction message to spectator.(0=off)", CVAR_FLAGS, true, 0.0);
	hRespawnHP = CreateConVar("l4d2_multislots_respawnhp", "80", "Amount of HP a new 5+ Survivor will spawn with (Def 80)", CVAR_FLAGS, true, 0.0, true, 100.0);
	hRespawnBuffHP = CreateConVar("l4d2_multislots_respawnbuffhp", "20", "Amount of buffer HP a new 5+ Survivor will spawn with (Def 20)", CVAR_FLAGS, true, 0.0, true, 100.0);
	hSpawnSurvivorsAtStart = CreateConVar("l4d2_multislots_spawn_survivors_roundstart", "0", "If 1, Spawn numbers of survivor bots when round starts. (Numbers depends on Convar l4d2_multislots_max_survivors)", CVAR_FLAGS, true, 0.0, true, 1.0);
	
	if (bL4D2Version) {
		hFirstWeapon = CreateConVar("l4d2_multislots_firstweapon", "18", "First slot weapon for new 5+ Survivor (1-Autoshot, 2-SPAS, 3-M16, 4-SCAR, 5-AK47, 6-SG552, 7-M60, 8-Mil Sniper, 9-AWP, 10-Scout, 11-SMG, 12-Sil SMG, 13=MP5, 14-Pump Shot, 15=Chrome Shot, 16=Hunt Rif, 17=Rand T1, 18=Rand T2, 0=off)", CVAR_FLAGS, true, 0.0, true, 18.0);
		hSecondWeapon = CreateConVar("l4d2_multislots_secondweapon", "16", "Second slot weapon for new 5+ Survivor (1-Pistol, 2-Magnum, 3-Chainsaw, 4-Fry Pan, 5-Katana, 6-Shovel, 7-Golfclub, 8-Machete, 9-Cricket, 10=Fireaxe, 11=Knife, 12=Bball Bat, 13=Crowbar, 14=Pitchfork, 15=Guitar, 16=Random, 0=off)", CVAR_FLAGS, true, 0.0, true, 16.0);
		hThirdWeapon = CreateConVar("l4d2_multislots_thirdweapon", "4", "Third slot weapon for new 5+ Survivor (1-Moltov, 2-Pipe Bomb, 3-Bile Jar, 4=Random, 0=off)", CVAR_FLAGS, true, 0.0, true, 4.0);
		hFourthWeapon = CreateConVar("l4d2_multislots_fourthweapon", "1", "Fourth slot weapon for new 5+ Survivor (1-Medkit, 2-Defib, 3-Incendiary Pack, 4-Explosive Pack, 5=Random, 0=off)", CVAR_FLAGS, true, 0.0, true, 5.0);
		hFifthWeapon = CreateConVar("l4d2_multislots_fifthweapon", "3", "Fifth slot weapon for new 5+ Survivor (1-Pills, 2-Adrenaline, 3=Random, 0=off)", CVAR_FLAGS, true, 0.0, true, 3.0);
	} else {
		hFirstWeapon = CreateConVar("l4d2_multislots_firstweapon", "7", "First slot weapon for new 5+ Survivor (1-Autoshotgun, 2-M16, 3-Hunting Rifle, 4-SMG, 5-Shotgun, 6=Random T1, 7=Random T2, 0=off)", CVAR_FLAGS, true, 0.0, true, 7.0);
		hSecondWeapon = CreateConVar("l4d2_multislots_secondweapon", "1", "Second slot weapon for new 5+ Survivor (1-Dual Pistol, 0=off)", CVAR_FLAGS, true, 0.0, true, 1.0);
		hThirdWeapon = CreateConVar("l4d2_multislots_thirdweapon", "3", "Third slot weapon for new 5+ SSurvivor (1-Moltov, 2-Pipe Bomb, 3=Random, 0=off)", CVAR_FLAGS, true, 0.0, true, 3.0);
		hFourthWeapon = CreateConVar("l4d2_multislots_fourthweapon", "1", "Fourth slot weapon for new 5+ SSurvivor (1-Medkit, 0=off)", CVAR_FLAGS, true, 0.0, true, 1.0);
		hFifthWeapon = CreateConVar("l4d2_multislots_fifthweapon", "1", "Fifth slot weapon for new 5+ Survivor (1-Pills, 0=off)", CVAR_FLAGS, true, 0.0, true, 1.0);
	}
	...
	Handle hGameConf = LoadGameConfigFile("l4d2_multislots");
	if(hGameConf == null)
	{
		SetFailState("Gamedata l4d2_multislots.txt not found");
		return;
	}
...
public Action AddBot(int client, int args)
{
	...
	if(SpawnFakeClient() == true)
		PrintToChat(client, "%T", "survivorbotadded", client);
	else
		PrintToChat(client, "%T", "cannotgeneratebot", client);
...
public Action JoinTeam_ColdDown(Handle timer, int userid)
{
	...
	if(client && IsClientInGame(client))
	{
		...
		{	
			...
			{
				PrintHintText(client, "%T", "waitreviverescue", client);
			}
		}
		else if(IsClientIdle(client))
		{
			PrintHintText(client, "%T", "idlepressjoin", client);  
		}
		else
		{			
			if(TotalAliveFreeBots() == 0)
			{
				...
				{
					PrintHintText(client, "%T", "cannotgeneratebot", client);
				}
			}
			else
			{
		...
		if(IsClientInGame(i))
		{
			if((GetClientTeam(i) == TEAM_SPECTATORS) && !IsFakeClient(i))
			{
				if(!IsClientIdle(i))
				{
					...
					PrintHintText(i, "[Multislots] %s, %T", PlayerName, "typejoinsurvivors", i);
				}
			}
		}
	}	
	for (int i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i))		
		{
			if((GetClientTeam(i) == TEAM_SURVIVORS) && !IsFakeClient(i) && !IsAlive(i))
			{
				static char PlayerName[100];
				GetClientName(i, PlayerName, sizeof(PlayerName));
				PrintHintText(i, "[Multislots] %s, %T", PlayerName, "waitreviverescue", i);
			}
		}
	}	
	return Plugin_Continue;
...
public Action Timer_KillSurvivor(Handle timer, int client)
{
	...
	if(client && IsClientInGame(client) && GetClientTeam(client) == TEAM_SURVIVORS && IsPlayerAlive(client))
	{
		...
		PrintHintText(client, "%T", "survivorsstartedgame", client);
	}
}
...
public Action Timer_TakeOverBotAndDie(Handle timer, int userid)
{
	...
	if(team == TEAM_SPECTATORS)
	{
		...
		else
		{
			...
			if (fakebot == 0)
			{
				PrintHintText(client, "%T", "nobotsreplace", client);
				return Plugin_Stop;
			}
...
void GiveRandomT1Weapon(int client)
{
...
void GiveItems(int client) // give client weapon
{
	...
	if(bL4D2Version)
	{
		if(g_iFirstWeapon == 17) iRandom = GetRandomInt(11,16);
		else if(g_iFirstWeapon == 18) iRandom = GetRandomInt(1,10);
		...
		switch (iRandom)
		{
			...
			case 7: FakeClientCommand(client, "give rifle_m60");
			case 8: FakeClientCommand(client, "give sniper_military");
			case 9: FakeClientCommand(client, "give sniper_awp");
			case 10: FakeClientCommand(client, "give sniper_scout");
			case 11: FakeClientCommand(client, "give smg");
			case 12: FakeClientCommand(client, "give smg_silenced");
			case 13: FakeClientCommand(client, "give smg_mp5");
			case 14: FakeClientCommand(client, "give pumpshotgun");
			case 15: FakeClientCommand(client, "give shotgun_chrome");
			case 16: FakeClientCommand(client, "give hunting_rifle");
			default: {}
		}
	}
	else
	...
	if(bL4D2Version && iRandom == 16) iRandom = GetRandomInt(1,15);

...
public OnPluginStart()
{
	...
	hKickIdlers 	= CreateConVar("l4d_multislots_kickafk", "1", "Kick idle players? (0 = no  1 = player 10 min, admins kickimmune  2 = player 10 min, admins 20 min)", CVAR_FLAGS, true, 0.0, true, 2.0)
...
public Action:AddBot(client, args)
{
	...
	PrintToChatAll("Survivor spawned and teleported.")
...
public Action:JoinTeam(client, args)
{
	...
	if(IsClientInGame(client))
	{
		...
		{	
			...
			{
				PrintHintText(client, "You're already a survivor")
			}
			else if((DispatchKeyValue(client, "classname", "info_survivor_position") == true) && !IsAlive(client))
			{
				PrintHintText(client, "The apocalypse awaits you upon your revival or rescue")
			}
		}
		else if(IsClientIdle(client))
		{
			PrintHintText(client, "Press mouse, KILL MUCHOS ZOMBOS, and have fun!")
		}
		else
		{			
			if(TotalFreeBots() == 0)
		...
		if(IsClientConnected(i) && IsClientInGame(i))
		{
			if((GetClientTeam(i) == TEAM_SPECTATORS) && !IsFakeClient(i))
			{
				if(!IsClientIdle(i))
				{
					...
					PrintToChat(i, "\x01[\x04MULTISLOTS\x01] %s, type \x03!join\x01 to hunt in the apocalypse", PlayerName)
				}
				switch(GetConVarInt(hKickIdlers))
				{
					...
					case 1:
					{
						if(GetUserFlagBits(i) == 0)
						{
							giIdleTicks[i]++
							if(giIdleTicks[i] == 40)
								KickClient(i, "Stop belting the bishop already - BU-BYE!")
						}
					}
					case 2:
					{
						...
						{
							if(giIdleTicks[i] == 40)
								KickClient(i, "Jackoff artiste! Off you go & don't forget the Bounty!")
						}
						else
						{
							if(giIdleTicks[i] == 80)
								KickClient(i, "Admindouche idle longer than 20 min.")
						}
					}
				}
			}
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if(IsClientConnected(i) && IsClientInGame(i))		
		{
			if((GetClientTeam(i) == TEAM_SURVIVORS) && !IsFakeClient(i) && !IsAlive(i))
			{
				new String:PlayerName[100]
				GetClientName(i, PlayerName, sizeof(PlayerName))
				PrintToChat(i, "\x01[\x04MULTISLOTS\x01] %s, Zombies to kill and fun to be had! Someone Will revive-rescue you soon!", PlayerName)
			}
		}
	}	
	return Plugin_Continue
...
stock GiveWeapon(client) // give client random weapon
{
	...
	{
		case 0: BypassAndExecuteCommand(client, "give", "shotgun_chrome")
		case 1: BypassAndExecuteCommand(client, "give", "smg_silenced")
		case 2: BypassAndExecuteCommand(client, "give", "pumpshutgun")
		case 3: BypassAndExecuteCommand(client, "give", "smg")
		case 4: BypassAndExecuteCommand(client, "give", "hunting_rifle")
	}

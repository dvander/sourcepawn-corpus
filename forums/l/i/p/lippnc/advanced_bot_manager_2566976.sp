...
public void OnPluginStart() {
    ...
    g_GameData = LoadGameConfigFile("advanced_bot_manager");
    if (g_GameData == null) {
    ...
    RegAdminCmd("abm", MainMenuCmd, ADMFLAG_ROOT);
    RegAdminCmd("abm-menu", MainMenuCmd, ADMFLAG_ROOT, "Menu: (Main ABM menu)");
    RegAdminCmd("abm-join", SwitchTeamCmd, ADMFLAG_ROOT, "Menu/Cmd: <TEAM> | <ID> <TEAM>");
    RegAdminCmd("abm-takeover", SwitchToBotCmd, ADMFLAG_ROOT, "Menu/Cmd: <ID> | <ID1> <ID2>");
    RegAdminCmd("abm-respawn", RespawnClientCmd, ADMFLAG_ROOT, "Menu/Cmd: <ID> [ID]");
    RegAdminCmd("abm-model", AssignModelCmd, ADMFLAG_ROOT, "Menu/Cmd: <MODEL> | <MODEL> <ID>");
    RegAdminCmd("abm-strip", StripClientCmd, ADMFLAG_ROOT, "Menu/Cmd: <ID> [SLOT]");
    RegAdminCmd("abm-teleport", TeleportClientCmd, ADMFLAG_ROOT, "Menu/Cmd: <ID1> <ID2>");
    RegAdminCmd("abm-cycle", CycleBotsCmd, ADMFLAG_ROOT, "Menu/Cmd: <TEAM> | <ID> <TEAM>");
    RegAdminCmd("abm-reset", ResetCmd, ADMFLAG_ROOT, "Cmd: (Use only in case of emergency)");
    RegAdminCmd("abm-info", QuickClientPrintCmd, ADMFLAG_ROOT, "Cmd: (Print some diagnostic information)");
    RegAdminCmd("abm-addbot", MkBotsCmd, ADMFLAG_ROOT, "Cmd: <N|-N> <TEAM>");
    RegAdminCmd("abm-rembot", RmBotsCmd, ADMFLAG_ROOT, "Cmd: <TEAM> | <N|-N> <TEAM>");
    RegConsoleCmd("takeover", SwitchToBotCmd, "Menu/Cmd: <ID> | <ID1> <ID2>");
    RegConsoleCmd("join", SwitchTeamCmd, "Menu/Cmd: <TEAM> | <ID> <TEAM>");
    RegConsoleCmd("csm", AssignModelCmd, "Menu/Cmd: <MODEL> | <MODEL> <ID>");
    ...
    SetupCvar(g_cvPrimaryWeapon, "abm_primaryweapon", "hunting_rifle", "5+ survivor primary weapon");
    SetupCvar(g_cvSecondaryWeapon,"abm_secondaryweapon", "knife", "5+ survivor secondary weapon");
    SetupCvar(g_cvThrowable, "abm_throwable", "molotov", "5+ survivor throwable item");
    SetupCvar(g_cvHealItem, "abm_healitem", "first_aid_kit", "5+ survivor healing item");
    ...
    SetupCvar(g_cvTeamLimit, "abm_teamlimit", "8", "Humans on team limit");
    SetupCvar(g_cvOfferTakeover, "abm_offertakeover", "0", "0: Off 1: Survivors 2: Infected 3: All");
    SetupCvar(g_cvStripKick, "abm_stripkick", "1", "0: Don't strip removed bots 1: Strip removed bots");
    ...
    AutoExecConfig(true, "advanced_bot_manager");
    StartAD();
}
...
public void UpdateConVarsHook(Handle convar, const char[] oldCv, const char[] newCv) {
    ...
    if (name[0] == 'a') {
        if (name[4] == 'a') {
			if (name[8] == 'm' && StrEqual(name, "abm_automodel")) {
            ...
            if (name[5] == 'e' && StrEqual(name, "abm_teamlimit")) {
...
public void OnConfigsExecuted() {
    ...
    else if (FileExists("cfg/sourcemod/advanced_bot_manager.cfg", true)) {
        ServerCommand("exec \"sourcemod/advanced_bot_manager.cfg\"");
    }
}
...
bool IsAdmin(int client) {
    ...
    return CheckCommandAccess(
        client, "generic_admin", ADMFLAG_ROOT, false
    );
}
...
void MainMenu(int client, char [] title) {
    ...
    menu.SetTitle(title);
    menu.AddItem("0", "Teleport Players");  // "Telespiznat");    // teleport
    menu.AddItem("1", "Switch Players Team");  //"Swintootle");    // switch team
    menu.AddItem("2", "Change Players Model");  //"Changdangle");    // makeover
    menu.AddItem("3", "Switch Players Bot");  //"Inbosnachup");    // takeover
    menu.AddItem("4", "Respawn Players");  //"Respiggle");        // respawn
...
void TeamsMenu(int client, char [] title, bool all=true) {
    ...
    if (all) {
        menu.AddItem("0", "Set Players Idle Status");
        menu.AddItem("1", "Set Players Spectator Status");
    }

    menu.AddItem("2", "Become Any Survivor Or Game NPC");
    if (IsAdmin(client)) {
        menu.AddItem("3", "Become Any Special Infected");
    }

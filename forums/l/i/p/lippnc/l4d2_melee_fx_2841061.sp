#define PLUGIN_VERSION		"1.2k"
#define PLUGIN_PREFIX		"l4d2_"
#define PLUGIN_NAME			"melee_fx"
#define PLUGIN_NAME_FULL	"[L4D2] Melee Weapon Effects"
#define PLUGIN_DESCRIPTION	"More realism and efffects for melee weapons"
#define PLUGIN_AUTHOR		"NoroHime modded by Kruffty"
...
{
	CreateConVar					(PLUGIN_NAME, PLUGIN_VERSION,			"Version of " ... PLUGIN_NAME_FULL, FCVAR_SPONLY|FCVAR_DONTRECORD|FCVAR_REPLICATED);
	cSlash =			CreateConVar(PLUGIN_NAME ... "_slash", "1",			"Apply which effects melee weapon has with DMG_SLASH. 1=Spark(temp ents), 2=Incendiary ammo, 4=Explosive ammo,\n8=Space warping, 16=Smoke, 32=Red spark, 64=Blinking, 128=Flashing, 256=Black hole,\n512=Huge spark, 1024=Rain spark, 2048=Explosion smoke, 4096=Explosion");
	cBurn =				CreateConVar(PLUGIN_NAME ... "_burn", "2",			"Apply which effects for melee has DMG_BURN, detail see *_slash");
	cClub =				CreateConVar(PLUGIN_NAME ... "_club", "4",			"Apply which effects for melee has DMG_CLUB, detail see *_slash");
	cBlast =			CreateConVar(PLUGIN_NAME ... "_blast", "4",			"Apply which effects for melee has DMG_BLAST, detail see *_slash");
	cPosMin =			CreateConVar(PLUGIN_NAME ... "_pos_min", "30.0",	"Minimum random effects starting height");
	cPosMax =			CreateConVar(PLUGIN_NAME ... "_pos_max", "60.0",	"Maximum random effects starting height");
...
int SafeMeleeIndex(const char[] name)
{
    if(!g_bMapStarted)
        return -1;
    return L4D2_GetMeleeWeaponIndex(name);
}
...
void OnMeleeSpawnPost(int entity) 
{
	...
	int id = SafeMeleeIndex(name_melee);

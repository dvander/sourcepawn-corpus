...
ConVar lsAnnounce, lsAntiFarmDuration, lsDuration, lsMobs, lsTankDemolitionBefore, lsTankDemolitionAfter,lsType, lsNearByAllSurvivor, lsHint, lsGetInLimit, lsDoorOpeningTeleport, lsDoorOpeningTankInterval, lsDoorBotDisable, lsMapOff, lsPreventDoorSpamDuration, lsDoorLockColor, lsDoorUnlockColor, lsDoorGlowRange;

int iAntiFarmDuration, iDuration, iMobs, iType, iDoorStatus, iCheckpointDoor, iSystemTime, iGetInLimit, iDoorOpeningTankInterval, iDoorGlowRange;
...
bool bAntiFarmInit, bLockdownInit, bLDFinished, bAnnounce, bNearByAllSurvivor,bDoorOpeningTeleport, bTankDemolitionBefore, bTankDemolitionAfter, bSurvivorsAssembleAlready,blsHint, bDoorBotDisable;
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	{
		strcopy(error, err_max, "[LOCKDOWN] Plugin Supports L4D2 Only");
...
public Plugin myinfo = 
{
	...
	author = "cravenge, Harry modded by Huck",
...
public void OnPluginStart()
{
	...
	lsAntiFarmDuration = CreateConVar("lockdown_system-l4d2_anti-farm_duration", "35.0", "Duration Of Anti-Farm", FCVAR_NOTIFY, true, 0.0);
	lsDuration = CreateConVar("lockdown_system-l4d2_duration", "45.0", "Duration Of Lockdown", FCVAR_NOTIFY, true, 0.0);
	...
	lsTankDemolitionAfter = CreateConVar("lockdown_system-l4d2_tank_demolition_after", "0", "If 1, Enable Tank Demolition, server will spawn tank after door open ", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	lsDoorOpeningTeleport = CreateConVar("lockdown_system-l4d2_teleport", "0", "0=Off. 1=Teleport common, special infected, and witch if they touch the door inside saferoom when door is opening. (prevent spawning and be stuck inside the saferoom, only works if Lockdown Type is 2)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	lsDoorOpeningTankInterval = CreateConVar("lockdown_system-l4d2_opening_tank_interval", "40.0", "Time Interval to spawn a tank when door is opening (0=off)", FCVAR_NOTIFY, true, 0.0);
	...
	lsMapTwoTanks = CreateConVar("lockdown_system-l4d2_map_two_Tank", "", "Two tanks during opening door in these maps, separate by commas (no spaces). (0=All maps, Empty = none).", FCVAR_NOTIFY);
...
public void OnMapStart()
{
	...
	if (g_bValidMap)
	{
		...
		PrecacheSound("perimeter_alarm.wav", true);
			...
			if (iDoorStatus != UNLOCK)
			{
				if(bNearByAllSurvivor && !bSurvivorsAssembleAlready)
				{
					...
					for (int i = 1; i <= MaxClients; i++)
					{
						if(IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i))
						{
							...
							{
								PrintHintText(user, "[TEAM] All survivors needed to open saferoom");
								return Plugin_Continue;
							}
						}
					}
				...
				if (GetTankCount() > 0)
				{
					...
					if (!bAntiFarmInit)
					{
						...
						PrintHintText(user, "[TEAM] Look alive and kill tank first!");
...
public Action CheckAntiFarm(Handle timer, any entity)
{
	...
	for(int i = 1; i <= MaxClients; i++) 
	if(IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i))	
	PrintHintText(i, "[TEAM] Kill tank or run for %d secs!", iSystemTime);
	iSystemTime -= 1;
...
public Action LockdownOpening(Handle timer, any entity)
{
	...
	if (hLockdownTime == null)
	{
		if (!bLDFinished)
		{
			...
			for (int i = 1; i <= MaxClients; i++)
			if(IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i))
			if(bAnnounce) PrintHintText(i, "[TEAM] GET INSIDE SAFEROOM OR DIE!");
			if(blsHint) CPrintToChatAll("{default}[{olive}TEAM{default}]{green} <{olive}%s{green}>{default} %t", sKeyMan, "opendooralready");
	...
	for(int i = 1; i <= MaxClients; i++) 
	if(IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i))	
	PrintHintText(i, "[TEAM] Saferoom opens in %d seconds!", iSystemTime);
...
public Action LaunchTankDemolition(Handle timer)
{
	...
	for(int i = 1; i <= MaxClients; i++) 
	if(IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i))
	if(bAnnounce) PrintHintText(i, "[TEAM] HERE COMES DA PAIN!");
}
...
public Action AntiPussy(Handle timer, any entity)
{
	...
	if(IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i))
	{
		EmitSoundToAll("ambient/alarms/perimeter_alarm.wav", entity, SNDCHAN_AUTO, SNDLEVEL_RAIDSIREN, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_LOW, -1, NULL_VECTOR, NULL_VECTOR, true, 0.0);	
		PrintHintText(i, "[TEAM] Outside saferoom? You DIE in %d secs", iSystemTime);
	}
	...
	if(iSystemTime <= 0)
	{
		...
		for(int i = 1; i <= MaxClients; i++) 
		if(IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i))
		if(bAnnounce) PrintHintText(i, "[TEAM] If outside saferoom: GET INSIDE!");
...
public Action _AntiPussy(Handle timer)
{
	...
	for( int i = 1; i <= MaxClients; i++ ) 
	{
		if(IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i) && !L4D_IsInLastCheckpoint(i))
		{
			...
			if(bAnnounce) {
				PrintHintText(i, "[TEAM] You're executed for not being in saferoom!");
			}
		}
	}
	return Plugin_Continue;
...
void CreateTankBot()
{
	...
	{
		int newtankbot = SDKCall(hCreateTank, "Lock Down Tank Bot");
		if (newtankbot > 0 && IsValidClient(newtankbot))
		{
			...
			TeleportEntity(newtankbot, vecPos, NULL_VECTOR, NULL_VECTOR);
...
void DoorPrint(Event event, bool open)
{
	if( bLDFinished && blsHint)
	{
		...
		{
			if(open) CPrintToChatAll("{default}[{olive}TEAM{default}]{green} ---{olive}%N{green}---{default} %t", client, "someoneopensdoor");
			else CPrintToChatAll("{default}[{olive}TEAM{default}]{green} ---{olive}%N{green}---{default} %t", client, "someoneclosesdoor");
		}
	}
}

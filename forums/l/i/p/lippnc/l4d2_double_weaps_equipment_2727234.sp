...
Handle l4d2_dw_mode;
Handle l4d2_dw_view;
Handle l4d2_dw_slot[5];
Handle l4d2_dw_afk_save;
Handle l4d2_dw_player_connect;
Handle l4d2_dw_custom_notify;
Handle l4d2_dw_custom_notify_msg;
Handle DW_Notify[MAXPLAYERS+1];
Handle AmmoLockSlot0 = INVALID_HANDLE;
Handle AmmoLockSlot1 = INVALID_HANDLE;

public Plugin myinfo =
{
	name = "L4D2 Douple Weapons & Equipment",
	author = "MasterMind420 & Ludastar & Pan Xiaohai & Marcus101RR modded by Huck",
	description = "Carry 2 items in each weapon inventory slot",
	version = "4.0",
...
public void OnPluginStart()
{
	...
	l4d2_dw_slot[0] = CreateConVar("l4d2_dw_slot0", "1", "(Primary), 0=Disable, 1=Enable");
	l4d2_dw_slot[1] = CreateConVar("l4d2_dw_slot1", "1", "(Secondary), 0=Disable, 1=Enable");
	l4d2_dw_slot[2] = CreateConVar("l4d2_dw_slot2", "0", "(Pipebomb), 0=Disable, 1=Enable");
	l4d2_dw_slot[3] = CreateConVar("l4d2_dw_slot3", "0", "(Medkit), 0=Disable, 1=Enable");
	l4d2_dw_slot[4] = CreateConVar("l4d2_dw_slot4", "0", "(Pills), 0=Disable, 1=Enable");
	
	l4d2_dw_mode = CreateConVar("l4d2_dw_mode", "2", "1=Single Tap Mode, 2=Double Tap Mode");
	l4d2_dw_view = CreateConVar("l4d2_dw_view", "0", "0=Disable Extra Equipment View, 1=Enable Extra Equipment View");
	l4d2_dw_afk_save = CreateConVar("l4d2_dw_afk_save", "1", "0=Disable AFK Save, 1=Enable AFK Save");
	
	l4d2_dw_custom_notify = CreateConVar("l4d2_dw_custom_notify", "2", "0=Disable Custom Message, 1=Enable Chat Message, 2=Enable Hint Message");
	l4d2_dw_custom_notify_msg = CreateConVar("l4d2_dw_custom_notify_msg", "Carry 2 Main + 2 Secondary Weapons. DBL TAP 1 key to SWITCH your Main Guns. DBL TAP 2 key to SWITCH PISTOLS + MELEE.", "Create a custom welcome message for your server.");
	l4d2_dw_player_connect = CreateConVar("l4d2_dw_player_connect", "0", "0=Disable Player Connect Message, 1=Enable Player Connect Message");
	...
	AutoExecConfig(true, "l4d2_double_weaps_equipment");
...
public void OnClientPostAdminCheck(int client)
{
	...
	if(GetConVarInt(l4d2_dw_view) == 1) // && !IsFakeClient(client))
		CreateTimer(5.0, FixWeaponView, client, TIMER_FLAG_NO_MAPCHANGE);

	//SIMPLE PLAYER CONNECT MESSAGE
	if(GetConVarInt(l4d2_dw_player_connect) == 1 && !IsFakeClient(client))
...
public Action ePlayerFirstSpawn(Handle event, const char[] name, bool dont_broadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
 	...
 	if(GetConVarInt(l4d2_dw_afk_save) == 1)
...
public Action DW_Notify_Client(Handle timer, any client)
{
	if(!IsSurvivor(client) || IsFakeClient(client))
		return;

	char DW_Msg[99];
	GetConVarString(l4d2_dw_custom_notify_msg, DW_Msg, sizeof (DW_Msg));

	if (GetConVarInt(l4d2_dw_custom_notify) == 1)
		PrintToChat(client, "%s", DW_Msg);

	if (GetConVarInt(l4d2_dw_custom_notify) == 2)
		PrintHintText(client, "%s", "Carry 2 Main + Secondary Weapoms\nDBL TAP 1 to SWITCH Main Guns\nDBL TAP 2 to SWITCH PISTOLS + MELEE");
...
public void OnGameFrame()
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		{
			if (GetConVarInt(l4d2_dw_custom_notify) == 1 || GetConVarInt(l4d2_dw_custom_notify) == 2)
			{
				if (MsgOn[i] == false)
					continue;

				DW_Notify[i] = CreateTimer(0.1, DW_Notify_Client, i, TIMER_FLAG_NO_MAPCHANGE);
			}
		}
...
public Action sm_dw(int client, int args)
{
...
//SINGLE TAP MODE
		if(weapon > 0)
		{
			if (GetConVarInt(l4d2_dw_mode) == 1)
			{
...
//DOUBLE TAP MODE
			else if (GetConVarInt(l4d2_dw_mode) == 2)
			{
				int newweapon = weapon;
...
int Process(int client, float time, int button, bool isSwitch, int currentWeapon = 0)
{
	...
	{
		if (GetConVarInt(l4d2_dw_slot[Slot]) == 0 || (!L4D2Version && Slot == 1))
			continue;
...
{
	if(GetConVarInt(l4d2_dw_view) != 1 || !IsSurvivor(client) || !IsPlayerAlive(client))
	{
		return 0;
	}
	char model[LEN64];
	...
	AcceptEntityInput(entity, "SetParent", client, 0);
	...
	DWIndex[client] = EntIndexToEntRef(entity);
	DWOwner[entity] = GetClientUserId(client);
...
public Action Hook_SetTransmit_View(int entity, int client)
{
	...
	if(!IsPlayerAlive(client))
		if(GetEntProp(client, Prop_Send, "m_iObserverMode") == 4)
			if(GetEntPropEnt(client, Prop_Send, "m_hObserverTarget") == GetClientOfUserId(DWOwner[entity]))
				return Plugin_Handled;

	int iEntOwner = GetClientOfUserId(DWOwner[entity]);
...
#define model_weapon_melee_knife "models/w_models/weapons/w_knife_t.mdl"
#define model_weapon_melee_foamfinger "models/bunny/weapons/melee/w_b_foamfinger.mdl"
#define model_weapon_melee_bt_nail "models/weapons/melee/w_bt_nail.mdl"
#define model_weapon_melee_bt_sledge "models/weapons/melee/w_bt_sledge.mdl"
#define model_weapon_melee_helms_sword_and_shield "models/weapons/melee/w_splinkswashereaswell.mdl"
...
#define model_weapon_melee_cross "models/weapons/melee/w_cross.mdl"
#define model_weapon_melee_chalice "models/weapons/melee/w_chalice.mdl"
#define model_weapon_melee_morgenstern "models/weapons/melee/w_morgenstern.mdl"
#define model_weapon_melee_shadowhand "models/weapons/melee/w_shadowhand.mdl"
#define model_weapon_melee_sof "models/weapons/melee/w_sof.mdl"
#define model_weapon_melee_hammer_roku "models/weapons/melee/w_hammer_roku.mdl"
#define model_weapon_melee_cuchi_roku "models/weapons/melee/w_cuchi_roku.mdl"
#define model_weapon_melee_bajo_roku "models/weapons/melee/w_bajo_roku.mdl"
#define model_weapon_melee_oz_roku "models/weapons/melee/w_oz_roku.mdl"
#define model_weapon_melee_greenhood_hatchet "models/weapons/melee/w_greenhood_hatchet.mdl"
#define model_weapon_melee_greenhood_orcrist "models/weapons/melee/w_greenhood_orcrist.mdl"
#define model_weapon_melee_greenhood_sting "models/weapons/melee/w_greenhood_sting.mdl"
#define model_weapon_melee_greenhood_anduril "models/weapons/melee/w_greenhood_anduril.mdl"
#define model_weapon_melee_shovel "models/w_models/weapons/w_shovel.mdl"
#define model_weapon_melee_pitchfork "models/w_models/weapons/w_pitchfork.mdl"
...
int GetModelFromClass_l4d2(char[] weapon, char model[LEN64], int slot = 0)
{
	switch(slot)
	{
		...
		//else if(slot==1)
		{
			...
			else if(StrEqual(weapon, "weapon_melee_bt_sledge"))strcopy(model, LEN64, model_weapon_melee_bt_sledge);
			else if(StrEqual(weapon, "weapon_melee_bt_nail"))strcopy(model, LEN64, model_weapon_melee_bt_nail);
			else if(StrEqual(weapon, "weapon_melee_helms_sword_and_shield"))strcopy(model, LEN64, model_weapon_melee_helms_sword_and_shield);
			...
			else if(StrEqual(weapon, "weapon_melee_cross"))strcopy(model, LEN64, model_weapon_melee_cross);
			else if(StrEqual(weapon, "weapon_melee_weapon_chalice"))strcopy(model, LEN64, model_weapon_melee_chalice);
			else if(StrEqual(weapon, "weapon_melee_weapon_morgenstern"))strcopy(model, LEN64, model_weapon_melee_morgenstern);
			else if(StrEqual(weapon, "weapon_melee_weapon_shadowhand"))strcopy(model, LEN64, model_weapon_melee_shadowhand);
			else if(StrEqual(weapon, "weapon_melee_weapon_sof"))strcopy(model, LEN64, model_weapon_melee_sof);
			else if(StrEqual(weapon, "weapon_melee_bajo_roku"))strcopy(model, LEN64, model_weapon_melee_bajo_roku);
			else if(StrEqual(weapon, "weapon_melee_cuchi_roku"))strcopy(model, LEN64, model_weapon_melee_cuchi_roku);
			else if(StrEqual(weapon, "weapon_melee_hammer_roku"))strcopy(model, LEN64, model_weapon_melee_hammer_roku);
			else if(StrEqual(weapon, "weapon_melee_oz_roku"))strcopy(model, LEN64, model_weapon_melee_oz_roku);
			else if(StrEqual(weapon, "weapon_melee_greenhood_hatchet"))strcopy(model, LEN64, model_weapon_melee_greenhood_hatchet);
			else if(StrEqual(weapon, "weapon_melee_greenhood_sting"))strcopy(model, LEN64, model_weapon_melee_greenhood_sting);
			else if(StrEqual(weapon, "weapon_melee_greenhood_anduril"))strcopy(model, LEN64, model_weapon_melee_greenhood_anduril);
			else if(StrEqual(weapon, "weapon_melee_greenhood_orcrist"))strcopy(model, LEN64, model_weapon_melee_greenhood_orcrist);
...
int GetItemClass(int ent, char classname[LEN64])
{
	...
	if(ent > 0)
	{
		...
		if(StrEqual(classname, "weapon_melee"))
		{
			...
			else if(StrContains(model, "b_foamfinger")>=0)classname="weapon_melee_foamfinger";
			else if(StrContains(model, "bt_sledge")>=0)classname="weapon_melee_bt_sledge";
			else if(StrContains(model, "bt_nail")>=0)classname="weapon_melee_bt_nail";
			else if(StrContains(model, "splinkswashereaswell")>=0)classname="weapon_melee_helms_sword_and_shield";
			else if(StrContains(model, "arm")>=0)classname="weapon_melee_arm";
			else if(StrContains(model, "foot")>=0)classname="weapon_melee_foot";
			else if(StrContains(model, "pickaxe")>=0)classname="weapon_melee_pickaxe";
			else if(StrContains(model, "cross")>=0)classname="weapon_melee_cross";
			else if(StrContains(model, "chalice")>=0)classname="weapon_melee_weapon_chalice";
			else if(StrContains(model, "morgenstern")>=0)classname="weapon_melee_weapon_morgenstern";
			else if(StrContains(model, "shadowhand")>=0)classname="weapon_melee_weapon_shadowhand";
			else if(StrContains(model, "sof")>=0)classname="weapon_melee_weapon_sof";
			else if(StrContains(model, "bajo_roku")>=0)classname="weapon_melee_bajo_roku";
			else if(StrContains(model, "cuchi_roku")>=0)classname="weapon_melee_cuchi_roku";
			else if(StrContains(model, "hammer_roku")>=0)classname="weapon_melee_hammer_roku";
			else if(StrContains(model, "oz_roku")>=0)classname="weapon_melee_oz_roku";
			else if(StrContains(model, "greenhood_hatchet")>=0)classname="weapon_melee_helms_hatchet";
			else if(StrContains(model, "greenhood_sting")>=0)classname="weapon_melee_helms_sting";
			else if(StrContains(model, "greenhood_anduril")>=0)classname="weapon_melee_helms_anduril";
			else if(StrContains(model, "greenhood_orcrist")>=0)classname="weapon_melee_helms_orcrist";
...
int CreateWeaponEnt(char[] classname)
{
	...
	else
	{
		...
		else if(StrEqual(classname, "weapon_melee_chalice"))DispatchKeyValue(ent, "melee_script_name", "weapon_chalice");
		else if(StrEqual(classname, "weapon_melee_morgenstern"))DispatchKeyValue(ent, "melee_script_name", "weapon_morgenstern");
		else if(StrEqual(classname, "weapon_melee_shadowhand"))DispatchKeyValue(ent, "melee_script_name", "weapon_shadowhand");
		else if(StrEqual(classname, "weapon_melee_foamfinger"))DispatchKeyValue(ent, "melee_script_name", "foamfinger");
		else if(StrEqual(classname, "weapon_melee_bt_sledge"))DispatchKeyValue(ent, "melee_script_name", "bt_sledge");
		else if(StrEqual(classname, "weapon_melee_bt_nail"))DispatchKeyValue(ent, "melee_script_name", "bt_nail");
		...
		else if(StrEqual(classname, "weapon_melee_helms_sword_and_shield"))DispatchKeyValue(ent, "melee_script_name", "helms_sword_and_shield");
		else if(StrEqual(classname, "weapon_melee_pickaxe"))DispatchKeyValue(ent, "melee_script_name", "pickaxe");
		else if(StrEqual(classname, "weapon_melee_cross"))DispatchKeyValue(ent, "melee_script_name", "cross");
		else if(StrEqual(classname, "weapon_melee_sof"))DispatchKeyValue(ent, "melee_script_name", "weapon_sof");
		else if(StrEqual(classname, "weapon_melee_bajo_roku"))DispatchKeyValue(ent, "melee_script_name", "bajo_roku");
		else if(StrEqual(classname, "weapon_melee_cuchi_roku"))DispatchKeyValue(ent, "melee_script_name", "cuchi_roku");
		else if(StrEqual(classname, "weapon_melee_hammer_roku"))DispatchKeyValue(ent, "melee_script_name", "hammer_roku");
		else if(StrEqual(classname, "weapon_melee_oz_roku"))DispatchKeyValue(ent, "melee_script_name", "oz_roku");
		else if(StrEqual(classname, "weapon_melee_helms_hatchet"))DispatchKeyValue(ent, "melee_script_name", "helms_hatchet");
		else if(StrEqual(classname, "weapon_melee_helms_sting"))DispatchKeyValue(ent, "melee_script_name", "helms_sting");
		else if(StrEqual(classname, "weapon_melee_helms_anduril"))DispatchKeyValue(ent, "melee_script_name", "helms_anduril");
		else if(StrEqual(classname, "weapon_melee_helms_orcrist"))DispatchKeyValue(ent, "melee_script_name", "helms_orcrist");
...
public void OnMapStart()
{
	if(L4D2Version)
	{
		...
		PrecacheModel(model_weapon_melee_bt_nail);
		PrecacheModel(model_weapon_melee_bt_sledge);
		PrecacheModel(model_weapon_melee_helms_sword_and_shield);
		PrecacheModel(model_weapon_melee_arm);
		PrecacheModel(model_weapon_melee_foot);
		PrecacheModel(model_weapon_melee_pickaxe);
		PrecacheModel(model_weapon_melee_chalice);
		PrecacheModel(model_weapon_melee_morgenstern);
		PrecacheModel(model_weapon_melee_shadowhand);
		PrecacheModel(model_weapon_melee_sof);	
		PrecacheModel(model_weapon_melee_cross);
		PrecacheModel(model_weapon_melee_bajo_roku);
		PrecacheModel(model_weapon_melee_cuchi_roku);
		PrecacheModel(model_weapon_melee_hammer_roku);
		PrecacheModel(model_weapon_melee_oz_roku);
		PrecacheModel(model_weapon_melee_greenhood_hatchet);
		PrecacheModel(model_weapon_melee_greenhood_sting);
		PrecacheModel(model_weapon_melee_greenhood_anduril);
		PrecacheModel(model_weapon_melee_greenhood_orcrist);
		...
		PrecacheGeneric("scripts/melee/cross.txt", true);
		PrecacheGeneric("scripts/melee/bajo_roku.txt", true);
		PrecacheGeneric("scripts/melee/cuchi_roku.txt", true);
		PrecacheGeneric("scripts/melee/hammer_roku.txt", true);
		PrecacheGeneric("scripts/melee/oz_roku.txt", true);

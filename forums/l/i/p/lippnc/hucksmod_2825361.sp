...
#include <sourcemod>
#include <sdktools>
#define PLUGIN_VERSION "2.2.2h"

//info
public Plugin:myinfo=
{
	name="HucksMod",
	author="tPoncho modded by Huck",
	description="Adds Call Of Duty-style perks for L4D",
	version=PLUGIN_VERSION,
	url="https://forums.alliedmods.net/showthread.php?t=99305"
}
...
//katana
new Handle:g_hKatana_enable;
new Handle:g_hKatana_enable_sur;
new Handle:g_hKatana_enable_vs;
//associated var
new g_iKatana_enable;
new g_iKatana_enable_sur;
new g_iKatana_enable_vs;
...
public OnPluginStart()
{
	...
	CreateConVar("l4d_perkmod_version", PLUGIN_VERSION, "Version of Hucksmod for L4D2", FCVAR_REPLICATED);
	...
	else if (StrEqual(stGame, "left4dead", false)==true)
	{
		...
		SetFailState("Hucksmod only supports L4D 1 or 2.");
	...
	AutoExecConfig(true , "hucksmod");

	//and load translations
	LoadTranslations("plugin.hucksmod");
}
...
CreateConvars()
{
	//SURVIVOR
	//stopping power
	g_hStopping_dmgmult = CreateConVar("l4d_perkmod_stoppingpower_damagemultiplier", "0.2", "Stopping Power perk: Bonus damage multiplier, ADDED to base damage (clamped between 0.05 < 1.0)");
	HookConVarChange(g_hStopping_dmgmult, Convar_Stopping);
	g_flStopping_dmgmult = 0.2;

	g_hStopping_enable = CreateConVar("l4d_perkmod_stoppingpower_enable", "1", "Stopping Power perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hStopping_enable, Convar_Stopping_en);
	g_iStopping_enable = 1;

	g_hStopping_enable_sur = CreateConVar("l4d_perkmod_stoppingpower_enable_survival", "1", "Stopping Power perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hStopping_enable_sur, Convar_Stopping_en_sur);
	g_iStopping_enable_sur = 1;

	g_hStopping_enable_vs = CreateConVar("l4d_perkmod_stoppingpower_enable_versus", "0", "Stopping Power perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hStopping_enable_vs, Convar_Stopping_en_vs);
	g_iStopping_enable_vs = 0;

	//pyrotechnician
	g_hPyro_maxticks = CreateConVar("l4d_perkmod_pyrotechnician_maxticks", "60", "Pyrotechnician perk: The number of ticks (a tick is 2s) before giving a survivor a pipe bomb, ie. 60 ticks = 120 seconds. Clamped between 0 < 300, where 0 disables this feature.");
	HookConVarChange(g_hPyro_maxticks, Convar_Pyro);
	g_iPyro_maxticks = 60;

	g_hPyro_enable = CreateConVar("l4d_perkmod_pyrotechnician_enable", "1", "Pyrotechnician perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hPyro_enable, Convar_Pyro_en);
	g_iPyro_enable = 1;

	g_hPyro_enable_sur = CreateConVar("l4d_perkmod_pyrotechnician_enable_survival", "1", "Pyrotechnician perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hPyro_enable_sur, Convar_Pyro_en_sur);
	g_iPyro_enable_sur = 1;

	g_hPyro_enable_vs = CreateConVar("l4d_perkmod_pyrotechnician_enable_versus", "0", "Pyrotechnician perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hPyro_enable_vs, Convar_Pyro_en_vs);
	g_iPyro_enable_vs = 0;

	//spirit
	g_hSpirit_buff = CreateConVar("l4d_perkmod_spirit_bonusbuffer", "10", "Spirit perk: Bonus health buffer on self-revive (clamped between 0 < 170)");
	HookConVarChange(g_hSpirit_buff, Convar_SpiritBuff);
	g_iSpirit_buff=				30;

	g_hSpirit_cd = CreateConVar("l4d_perkmod_spirit_cooldown", "60", "Spirit perk: Cooldown for self-reviving in seconds, campaign (clamped between 1 < 1800)");
	HookConVarChange(g_hSpirit_cd, Convar_SpiritCD);
	g_iSpirit_cd=				60;

	g_hSpirit_cd_sur = CreateConVar("l4d_perkmod_spirit_cooldown_sur", "60", "Spirit perk: Cooldown for self-reviving in seconds, survival (clamped between 1 < 1800)");
	HookConVarChange(g_hSpirit_cd_sur, Convar_SpiritCDsur);
	g_iSpirit_cd_sur=			60;

	g_hSpirit_cd_vs = CreateConVar("l4d_perkmod_spirit_cooldown_vs", "60", "Spirit perk: Cooldown for self-reviving in seconds, versus (clamped between 1 < 1800)");
	HookConVarChange(g_hSpirit_cd_vs, Convar_SpiritCDvs);
	g_iSpirit_cd_vs=			60;

	g_hSpirit_enable = CreateConVar("l4d_perkmod_spirit_enable", "1", "Spirit perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hSpirit_enable, Convar_Spirit_en);
	g_iSpirit_enable = 1;

	g_hSpirit_enable_sur = CreateConVar("l4d_perkmod_spirit_enable_survival", "1", "Spirit perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hSpirit_enable_sur, Convar_Spirit_en_sur);
	g_iSpirit_enable_sur = 1;

	g_hSpirit_enable_vs = CreateConVar("l4d_perkmod_spirit_enable_versus", "0", "Spirit perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hSpirit_enable_vs, Convar_Spirit_en_vs);
	g_iSpirit_enable_vs = 0;

	//double tap
	g_hDT_rate = CreateConVar("l4d_perkmod_doubletap_rate", "0.7667", "Double Tap perk: The interval between bullets fired is multiplied by this value. NOTE: a short enough interval will make the gun fire at only normal speed! (clamped between 0.2 < 0.9 default 0.6667)");
	HookConVarChange(g_hDT_rate, Convar_DT);
	g_flDT_rate=			0.7667;

	g_hDT_rate_reload = CreateConVar("l4d_perkmod_doubletap_rate_reload", "0.75", "Double Tap perk: The interval incurred by reloading is multiplied by this value (clamped between 0.2 < 1.0)");
	HookConVarChange(g_hDT_rate_reload, Convar_DT_rate_reload);
	g_flDT_rate_reload=			0.75;

	g_hDT_enable = CreateConVar("l4d_perkmod_doubletap_enable", "1", "Double Tap perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hDT_enable, Convar_DT_en);
	g_iDT_enable = 1;

	g_hDT_enable_sur = CreateConVar("l4d_perkmod_doubletap_enable_survival", "1", "Double Tap perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hDT_enable_sur, Convar_DT_en_sur);
	g_iDT_enable_sur = 1;

	g_hDT_enable_vs = CreateConVar("l4d_perkmod_doubletap_enable_versus", "0", "Double Tap perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hDT_enable_vs, Convar_DT_en_vs);
	g_iDT_enable_vs = 0;

	//sleight of hand
	g_hSoH_rate = CreateConVar("l4d_perkmod_sleightofhand_rate", "0.7", "Sleight of Hand perk: The interval incurred by reloading is multiplied by this value (clamped between 0.2 < 0.9 default 0.5714)");
	HookConVarChange(g_hSoH_rate, Convar_SoH);
	g_flSoH_rate=			0.7;

	g_hSoH_enable = CreateConVar("l4d_perkmod_sleightofhand_enable", "1", "Sleight of Hand perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hSoH_enable, Convar_SoH_en);
	g_iSoH_enable = 1;

	g_hSoH_enable_sur = CreateConVar("l4d_perkmod_sleightofhand_enable_survival", "1", "Sleight of Hand perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hSoH_enable_sur, Convar_SoH_en_sur);
	g_iSoH_enable_sur = 1;

	g_hSoH_enable_vs = CreateConVar("l4d_perkmod_sleightofhand_enable_versus", "0", "Sleight of Hand perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hSoH_enable_vs, Convar_SoH_en_vs);
	g_iSoH_enable_vs = 0;

	//unbreakable
	g_hUnbreak_hp = CreateConVar("l4d_perkmod_unbreakable_bonushealth", "20", "Unbreakable perk: Bonus health given for Unbreakable; this value is also given as bonus health buffer on being revived (clamped between 1 < 100)");
	HookConVarChange(g_hUnbreak_hp, Convar_Unbreak);
	g_iUnbreak_hp = 20;

	g_hUnbreak_enable = CreateConVar("l4d_perkmod_unbreakable_enable", "1", "Unbreakable perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hUnbreak_enable, Convar_Unbreak_en);
	g_iUnbreak_enable = 1;

	g_hUnbreak_enable_sur = CreateConVar("l4d_perkmod_unbreakable_enable_survival", "1", "Unbreakable perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hUnbreak_enable_sur, Convar_Unbreak_en_sur);
	g_iUnbreak_enable_sur = 1;

	g_hUnbreak_enable_vs = CreateConVar("l4d_perkmod_unbreakable_enable_versus",		"0", "Unbreakable perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hUnbreak_enable_vs, Convar_Unbreak_en_vs);
	g_iUnbreak_enable_vs = 0;

	//chem reliant
	g_hChem_buff = CreateConVar("l4d_perkmod_chemreliant_bonusbuffer", "0", "Chem Reliant perk: Bonus health buffer given when taking pills (clamped between 0 < 150)");
	HookConVarChange(g_hChem_buff, Convar_Chem);
	g_iChem_buff = 0;

	g_hChem_enable = CreateConVar("l4d_perkmod_chemreliant_enable", "1", "Chem Reliant perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hChem_enable, Convar_Chem_en);
	g_iChem_enable = 1;

	g_hChem_enable_sur = CreateConVar("l4d_perkmod_chemreliant_enable_survival", "1", "Chem Reliant perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hChem_enable_sur, Convar_Chem_en_sur);
	g_iChem_enable_sur = 1;

	g_hChem_enable_vs = CreateConVar("l4d_perkmod_chemreliant_enable_versus",		"0", "Chem Reliant perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hChem_enable_vs, Convar_Chem_en_vs);
	g_iChem_enable_vs = 0;

	//helping hand
	g_hHelpHand_timemult = CreateConVar("l4d_perkmod_helpinghand_timemultiplier", "0.6", "Helping Hand perk: Time multiplier to revive others with Helping Hand (clamped between 0.01 < 1.0)");
	HookConVarChange(g_hHelpHand_timemult, Convar_HelpTime);
	g_flHelpHand_timemult = 0.6;

	g_hHelpHand_buff = CreateConVar("l4d_perkmod_helpinghand_bonusbuffer", "15", "Helping Hand perk: Bonus health buffer given to allies after reviving them, campaign/survival (clamped between 0 < 170)");
	HookConVarChange(g_hHelpHand_buff, Convar_HelpBuff);
	g_iHelpHand_buff = 15;

	g_hHelpHand_buff_vs = CreateConVar("l4d_perkmod_helpinghand_bonusbuffer_vs", "10", "Helping Hand perk: Bonus health buffer given to allies after reviving them, versus (clamped between 0 < 170)");
	HookConVarChange(g_hHelpHand_buff_vs, Convar_HelpBuffvs);
	g_iHelpHand_buff_vs = 10;

	g_hHelpHand_enable = CreateConVar("l4d_perkmod_helpinghand_enable", "0", "Helping Hand perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled (NOTE: This perk normally adjusts the survivor_revive_duration ConVar; disabling this perk will stop the plugin from adjusting this ConVar)");
	HookConVarChange(g_hHelpHand_enable, Convar_Help_en);
	g_iHelpHand_enable = 0;

	g_hHelpHand_enable_sur = CreateConVar("l4d_perkmod_helpinghand_enable_survival", "0", "Helping Hand perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled (NOTE: This perk normally adjusts the survivor_revive_duration ConVar; disabling this perk will stop the plugin from adjusting this ConVar)");
	HookConVarChange(g_hHelpHand_enable_sur, Convar_Help_en_sur);
	g_iHelpHand_enable_sur = 0;

	g_hHelpHand_enable_vs = CreateConVar("l4d_perkmod_helpinghand_enable_versus", "0", "Helping Hand perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled (NOTE: This perk normally adjusts the survivor_revive_duration ConVar; disabling this perk will stop the plugin from adjusting this ConVar)");
	HookConVarChange(g_hHelpHand_enable_vs, Convar_Help_en_vs);
	g_iHelpHand_enable_vs = 0;

	g_hHelpHand_convar = CreateConVar("l4d_perkmod_helpinghand_enable_convarchanges", "0", "Helping Hand perk: This perk normally adjusts the survivor_revive_duration ConVar; setting this to 0 will stop the plugin from adjusting this ConVar");
	HookConVarChange(g_hHelpHand_convar, Convar_Help_convar);
	g_iHelpHand_convar = 0;

	//pack rat
	g_hPack_ammomult = CreateConVar("l4d_perkmod_packrat_ammomultiplier", "0.2", "Pack Rat perk: Bonus ammo capacity, ADDED to base capacity (clamped between 0.01 < 1.0)");
	HookConVarChange(g_hPack_ammomult, Convar_Pack);
	g_flPack_ammomult = 0.2;

	g_hPack_enable = CreateConVar("l4d_perkmod_packrat_enable", "1", "Pack Rat perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hPack_enable, Convar_Pack_en);
	g_iPack_enable = 1;

	g_hPack_enable_sur = CreateConVar("l4d_perkmod_packrat_enable_survival", "1", "Pack Rat perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hPack_enable_sur, Convar_Pack_en_sur);
	g_iPack_enable_sur = 1;

	g_hPack_enable_vs = CreateConVar("l4d_perkmod_packrat_enable_versus",		"0", "Pack Rat perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hPack_enable_vs, Convar_Pack_en_vs);
	g_iPack_enable_vs = 0;

	//hard to kill
	g_hHard_hpmult = CreateConVar("l4d_perkmod_hardtokill_healthmultiplier", "0.5", "Hard to Kill perk: Bonus incap health multiplier, product is ADDED to base incap health (clamped between 0.01 < 3.0)");
	HookConVarChange(g_hHard_hpmult, Convar_Hard);
	g_flHard_hpmult = 0.5;

	g_hHard_enable = CreateConVar("l4d_perkmod_hardtokill_enable", "1", "Hard to Kill perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hHard_enable, Convar_Hard_en);
	g_iHard_enable = 1;

	g_hHard_enable_sur = CreateConVar("l4d_perkmod_hardtokill_enable_survival", "1", "Hard to Kill perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hHard_enable_sur, Convar_Hard_en_sur);
	g_iHard_enable_sur = 1;

	g_hHard_enable_vs = CreateConVar("l4d_perkmod_hardtokill_enable_versus",		"0", "Hard to Kill perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hHard_enable_vs, Convar_Hard_en_vs);
	g_iHard_enable_vs = 0;

	//martial artist
	g_hMA_maxpenalty = CreateConVar("l4d_perkmod_martialartist_maximumpenalty", "4", "Martial Artist perk: The maximum shove penalty applied to survivors. It's Valve's coding, so I don't know what each value exactly translates to, but 6 is the maximum shove penalty (~1.5s)");
	HookConVarChange(g_hMA_maxpenalty, Convar_MA_maxpenalty);
	g_iMA_maxpenalty = 6;

	g_hMA_enable = CreateConVar("l4d_perkmod_martialartist_enable", "0", "Martial Artist perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hMA_enable, Convar_MA_en);
	g_iMA_enable = 0;

	g_hMA_enable_sur = CreateConVar("l4d_perkmod_martialartist_enable_survival", "0", "Martial Artist perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hMA_enable_sur, Convar_MA_en_sur);
	g_iMA_enable_sur = 0;

	g_hMA_enable_vs = CreateConVar("l4d_perkmod_martialartist_enable_versus",		"0", "Martial Artist perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hMA_enable_vs, Convar_MA_en_vs);
	g_iMA_enable_vs = 0;

	//extreme conditioning
	g_hExtreme_rate = CreateConVar("l4d_perkmod_extremeconditioning_rate", "1.1", "Extreme Conditioning perk: Survivor movement is multiplied by this value (clamped between 1.0 < 1.5)");
	HookConVarChange(g_hExtreme_rate, Convar_Extreme);
	g_flExtreme_rate = 1.1;

	g_hExtreme_enable = CreateConVar("l4d_perkmod_extremeconditioning_enable", "1", "Extreme Conditioning perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hExtreme_enable, Convar_Extreme_en);
	g_iExtreme_enable = 1;

	g_hExtreme_enable_sur = CreateConVar("l4d_perkmod_extremeconditioning_enable_survival", "1", "Extreme Conditioning perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hExtreme_enable_sur, Convar_Extreme_en_sur);
	g_iExtreme_enable_sur = 1;

	g_hExtreme_enable_vs = CreateConVar("l4d_perkmod_extremeconditioning_enable_versus",		"0", "Extreme Conditioning perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hExtreme_enable_vs, Convar_Extreme_en_vs);
	g_iExtreme_enable_vs = 0;

	//katana
	g_hKatana_enable = CreateConVar("l4d_perkmod_katana_enable", "1", "Katana perk: Allows the perk to be chosen by players in campaign, 0=disabled, 1=enabled");
	HookConVarChange(g_hKatana_enable, Convar_Katana_en);
	g_iKatana_enable = 1;

	g_hKatana_enable_sur = CreateConVar("l4d_perkmod_katana_enable_survival", "1", "Katana perk: Allows the perk to be chosen by players in survival, 0=disabled, 1=enabled");
	HookConVarChange(g_hKatana_enable_sur, Convar_Katana_en_sur);
	g_iKatana_enable_sur = 1;

	g_hKatana_enable_vs = CreateConVar("l4d_perkmod_katana_enable_versus", "0", "Katana perk: Allows the perk to be chosen by players in versus, 0=disabled, 1=enabled");
	HookConVarChange(g_hKatana_enable_vs, Convar_Katana_en_vs);
	g_iKatana_enable_vs = 0;

	//BOOMER
	//barf bagged
	g_hBarf_enable = CreateConVar("l4d_perkmod_barfbagged_enable", "0", "Barf Bagged perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hBarf_enable, Convar_Barf_en);
	g_iBarf_enable = 0;

	//blind luck
	g_hBlind_cdmult = CreateConVar("l4d_perkmod_blindluck_timemultiplier", "0.5", "Blind Luck perk: Cooldown (default 30s) is multiplied by this value (clamped between 0.01 < 1.0)");
	HookConVarChange(g_hBlind_cdmult, Convar_Blind);
	g_flBlind_cdmult = 0.5;

	g_hBlind_enable = CreateConVar("l4d_perkmod_blindluck_enable", "0", "Blind Luck perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled (NOTE: This perk normally adjusts the z_vomit_interval ConVar; disabling this perk will stop the plugin from adjusting this ConVar)");
	HookConVarChange(g_hBlind_enable, Convar_Blind_en);
	g_iBlind_enable = 0;

	//dead wreckening
	g_hDead_dmgmult = CreateConVar("l4d_perkmod_deadwreckening_damagemultiplier", "0.5", "Dead Wreckening perk: Common infected damage is multiplied by this value and ADDED to their base damage (clamped between 0.01 < 4.0)");
	HookConVarChange(g_hDead_dmgmult, Convar_Dead);
	g_flDead_dmgmult = 0.5;

	g_hDead_enable = CreateConVar("l4d_perkmod_deadwreckening_enable", "0", "Dead Wreckening perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hDead_enable, Convar_Dead_en);
	g_iDead_enable = 0;

	//motion sickness
	g_hMotion_rate = CreateConVar("l4d_perkmod_motionsickness_rate", "1.25", "Motion Sickness perk: Boomer movement is multiplied by this value (clamped between 1.0 < 4.0)");
	HookConVarChange(g_hMotion_rate, Convar_Motion);
	g_flMotion_rate = 1.25;

	g_hMotion_enable = CreateConVar("l4d_perkmod_motionsickness_enable", "0", "Motion Sickness perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled (NOTE: This perk normally adjusts the z_vomit_fatigue ConVar; disabling this perk will stop the plugin from adjusting this ConVar)");
	HookConVarChange(g_hMotion_enable, Convar_Motion_en);
	g_iMotion_enable = 0;

	//SMOKER
	//tongue twister
	g_hTongue_speedmult = CreateConVar("l4d_perkmod_tonguetwister_speedmultiplier", "1.5", "Tongue Twister perk: Tongue travel speed before grabbing a survivor; multiplied by this value (clamped between 1.0 < 5.0)");
	HookConVarChange(g_hTongue_speedmult, Convar_TongueSpeed);
	g_flTongue_speedmult = 1.5;

	g_hTongue_pullmult = CreateConVar("l4d_perkmod_tonguetwister_pullmultiplier", "1.5", "Tongue Twister perk: Tongue pull speed after grabbing a survivor; multiplied by this value (clamped between 1.0 < 5.0)");
	HookConVarChange(g_hTongue_pullmult, Convar_TonguePull);
	g_flTongue_pullmult = 1.5;

	g_hTongue_rangemult = CreateConVar("l4d_perkmod_tonguetwister_rangemultiplier", "1.75", "Tongue Twister perk: Tongue range; multiplied by this value (clamped between 1.0 < 5.0)");
	HookConVarChange(g_hTongue_rangemult, Convar_TongueRange);
	g_flTongue_rangemult = 1.75;

	g_hTongue_enable = CreateConVar("l4d_perkmod_tonguetwister_enable", "0", "Tongue Twister perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled (NOTE: This perk normally adjusts the tongue_range, tongue_victim_max_speed and tongue_fly_speed ConVars; disabling this perk will stop the plugin from adjusting these ConVars)");
	HookConVarChange(g_hTongue_enable, Convar_Tongue_en);
	g_iTongue_enable = 0;

	//squeezer
	g_hSqueezer_dmgmult = CreateConVar("l4d_perkmod_squeezer_damagemultiplier", "0.5", "Squeezer perk: All base damage is multiplied by this value and then ADDED to base damage (clamped between 0.01 < 4.0)");
	HookConVarChange(g_hSqueezer_dmgmult, Convar_Squeezer);
	g_flSqueezer_dmgmult = 0.5;

	g_hSqueezer_enable = CreateConVar("l4d_perkmod_squeezer_enable", "0", "Squeezer perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hSqueezer_enable, Convar_Squeezer_en);
	g_iSqueezer_enable = 0;

	//drag and drop
	g_hDrag_cdmult = CreateConVar("l4d_perkmod_draganddrop_timemultiplier", "0.2", "Drag and Drop perk: Cooldown is multiplied by this value (clamped between 0.01 < 1.0)");
	HookConVarChange(g_hDrag_cdmult, Convar_Drag);
	g_flDrag_cdmult = 0.2;

	g_hDrag_enable = CreateConVar("l4d_perkmod_draganddrop_enable", "0", "Drag and Drop perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled (NOTE: This perk normally adjusts the tongue_hit_delay and tongue_player_dropping_to_ground_time ConVars; disabling this perk will stop the plugin from adjusting these ConVars)");
	HookConVarChange(g_hDrag_enable, Convar_Drag_en);
	g_iDrag_enable = 0;

	//smoke it
	g_hSmokeItSpeed = CreateConVar("l4d_perkmod_smokeit_speed", "0.21", "Smoke IT! perk: Smoker's speed modifier");
	g_flSmokeItSpeed = 0.21;
	HookConVarChange(g_hSmokeItSpeed, Convar_SmokeIt_speed);

	g_hSmokeItMaxRange = CreateConVar("l4d_perkmod_smokeit_tonguestretch", "950", "Smoke IT! perk: Smoker's max tongue stretch, tongue will be released if beyond this");
	g_iSmokeItMaxRange = 950;
	HookConVarChange(g_hSmokeItMaxRange, Convar_SmokeIt_range);

	g_hSmokeIt_enable = CreateConVar("l4d_perkmod_smokeit_enable", "0", "Smoke IT! perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hSmokeIt_enable, Convar_SmokeIt_en);
	g_iSmokeIt_enable = 0;

	//HUNTER
	//body slam
	g_hBody_minbound = CreateConVar("l4d_perkmod_bodyslam_minbound", "10", "Body Slam perk: Defines the minimum initial damage dealt by a pounce (clamped between 2 < 100)");
	HookConVarChange(g_hBody_minbound, Convar_Body);
	g_iBody_minbound = 10;

	g_hBody_enable = CreateConVar("l4d_perkmod_bodyslam_enable", "0", "Body Slam perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hBody_enable, Convar_Body_en);
	g_iBody_enable = 0;

	//efficient killer
	g_hEfficient_dmgmult = CreateConVar("l4d_perkmod_efficientkiller_damagemultiplier", "0.2", "Efficient Killer perk: All base damage is multiplied by this value and then ADDED to base damage (clamped between 0.01 < 4.0)");
	HookConVarChange(g_hEfficient_dmgmult, Convar_Eff);
	g_flEfficient_dmgmult = 0.2;

	g_hEfficient_enable = CreateConVar("l4d_perkmod_efficientkiller_enable", "0", "Efficient Killer perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hEfficient_enable, Convar_Eff_en);
	g_iEfficient_enable = 0;

	//grasshopper
	g_hGrass_rate = CreateConVar("l4d_perkmod_grasshopper_rate", "1.2", "Grasshopper perk: Multiplier for pounce speed (clamped between 1.0 < 3.0)");
	HookConVarChange(g_hGrass_rate, Convar_Grass);
	g_flGrass_rate = 1.2;

	g_hGrass_enable = CreateConVar("l4d_perkmod_grasshopper_enable", "0", "Grasshopper perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hGrass_enable, Convar_Grass_en);
	g_iGrass_enable = 0;

	//speed demon
	g_hSpeedDemon_rate = CreateConVar("l4d_perkmod_speeddemon_rate", "1.4", "Speed Demon perk: Multiplier for time rate (clamped between 1.0 < 3.0)");
	HookConVarChange(g_hSpeedDemon_rate, Convar_Demon);
	g_flSpeedDemon_rate = 1.4;

	g_hSpeedDemon_dmgmult = CreateConVar("l4d_perkmod_speeddemon_damagemultiplier", "0.5", "Efficient Killer perk: All base damage is multiplied by this value and then ADDED to base damage (clamped between 0.01 < 4.0)");
	HookConVarChange(g_hSpeedDemon_dmgmult, Convar_Demon_dmg);
	g_flSpeedDemon_dmgmult = 0.5;

	g_hSpeedDemon_enable = CreateConVar("l4d_perkmod_speeddemon_enable", "0", "Speed Demon perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hSpeedDemon_enable, Convar_Demon_en);
	g_iSpeedDemon_enable = 0;

	//TANK
	//adrenal glands
	g_hAdrenal_punchcdmult = CreateConVar("l4d_perkmod_adrenalglands_punchcooldownmultiplier", "0.5", "Adrenal Glands perk: Cooldown for punching is multiplied by this value (clamped between 0.01 < 1.0)");
	HookConVarChange(g_hAdrenal_punchcdmult, Convar_Adrenalpunchcd);
	g_flAdrenal_punchcdmult = 0.5;

	g_hAdrenal_throwcdmult = CreateConVar("l4d_perkmod_adrenalglands_throwcooldownmultiplier", "0.4", "Adrenal Glands perk: Cooldown for throwing rocks is multiplied by this value (clamped between 0.01 < 1.0)");
	HookConVarChange(g_hAdrenal_throwcdmult, Convar_Adrenalthrowcd);
	g_flAdrenal_throwcdmult = 0.4;

	g_hAdrenal_enable = CreateConVar("l4d_perkmod_adrenalglands_enable", "0", "Adrenal Glands perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled (NOTE: This perk normally adjusts the tank_swing_interval, tank_swing_miss_interval, z_tank_attack_interval, z_tank_throw_interval, and z_tank_throw_force ConVars; disabling this perk will stop the plugin from adjusting these ConVars)");
	HookConVarChange(g_hAdrenal_enable, Convar_Adrenal_en);
	g_iAdrenal_enable = 0;

	//juggernaut
	g_hJuggernaut_hp = CreateConVar("l4d_perkmod_juggernaut_health", "3000", "Juggernaut perk: Bonus health given to tanks; absolute value (clamped between 1 < 99999)");
	HookConVarChange(g_hJuggernaut_hp, Convar_Jugg);
	g_iJuggernaut_hp = 3000;

	g_hJuggernaut_enable = CreateConVar("l4d_perkmod_juggernaut_enable", "0", "Juggernaut perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hJuggernaut_enable, Convar_Jugg_en);
	g_iJuggernaut_enable = 0;

	//metabolic boost
	g_hMetabolic_speedmult = CreateConVar("l4d_perkmod_metabolicboost_speedmultiplier", "1.4", "Metabolic Boost perk: Run speed multiplier (clamped between 1.01 < 3.0)");
	HookConVarChange(g_hMetabolic_speedmult, Convar_Met);
	g_flMetabolic_speedmult = 1.4;

	g_hMetabolic_enable = CreateConVar("l4d_perkmod_metabolicboost_enable", "0", "Metabolic Boost perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hMetabolic_enable, Convar_Met_en);
	g_iMetabolic_enable = 0;

	//storm caller
	g_hStorm_mobcount = CreateConVar("l4d_perkmod_stormcaller_mobcount", "3", "Storm Caller perk: How many groups of zombies are spawned (clamped between 1 < 10)");
	HookConVarChange(g_hStorm_mobcount, Convar_Storm);
	g_iStorm_mobcount = 3;

	g_hStorm_enable = CreateConVar("l4d_perkmod_stormcaller_enable", "0", "Storm Caller perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hStorm_enable, Convar_Storm_en);
	g_iStorm_enable = 0;

	//double the trouble
	g_hDouble_hpmult = CreateConVar("l4d_perkmod_doublethetrouble_healthmultiplier", "0.35", "Double the Trouble: Health multiplier for all tanks spawned under the perk (clamped between 0.1 < 2.0)");
	HookConVarChange(g_hDouble_hpmult, Convar_Double);
	g_flDouble_hpmult = 0.35;

	g_hDouble_enable = CreateConVar("l4d_perkmod_doublethetrouble_enable", "0", "Double the Trouble perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hDouble_enable, Convar_Double_en);
	g_iDouble_enable = 0;

	//JOCKEY
	//ride like the wind
	g_hWind_rate = CreateConVar("l4d_perkmod_ridelikethewind_rate", "1.4", "Ride Like the Wind perk: Multiplier for run speed rate (clamped between 1.0 < 3.0)");
	HookConVarChange(g_hWind_rate, Convar_Wind);
	g_flWind_rate = 1.4;

	g_hWind_enable = CreateConVar("l4d_perkmod_ridelikethewind_enable", "0", "Ride Like the Wind perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hWind_enable, Convar_Wind_en);
	g_iWind_enable = 0;

	//cavalier
	g_hCavalier_hpmult = CreateConVar("l4d_perkmod_cavalier_healthmultiplier", "0.6", "Cavalier: Bonus health multiplier, product is ADDED to base health (clamped between 0.01 < 3.0)");
	HookConVarChange(g_hCavalier_hpmult, Convar_Cavalier);
	g_flCavalier_hpmult = 0.6;

	g_hCavalier_enable = CreateConVar("l4d_perkmod_cavalier_enable", "0", "Cavalier perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hCavalier_enable, Convar_Cavalier_en);
	g_iCavalier_enable = 0;

	//frogger
	g_hFrogger_dmgmult = CreateConVar("l4d_perkmod_frogger_damagemultiplier", "0.35", "Frogger perk: All base damage is multiplied by this value and then ADDED to base damage (clamped between 0.01 < 4.0)");
	HookConVarChange(g_hFrogger_dmgmult, Convar_Frogger_dmgmult);
	g_flFrogger_dmgmult = 0.35;

	g_hFrogger_rate = CreateConVar("l4d_perkmod_frogger_rate", "1.3", "Frogger perk: Multiplier for leap speed (clamped between 1.0 < 3.0)");
	HookConVarChange(g_hFrogger_rate, Convar_Frogger_rate);
	g_flFrogger_rate = 1.3;

	g_hFrogger_enable = CreateConVar("l4d_perkmod_frogger_enable", "0", "Frogger perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hFrogger_enable, Convar_Frogger_en);
	g_iFrogger_enable = 0;

	//ghost rider
	g_hGhost_alpha = CreateConVar("l4d_perkmod_ghostrider_alpha", "25", "Ghost Rider perk: Sets the alpha level (clamped between 0 total invis < 255 opaque)");
	HookConVarChange(g_hGhost_alpha, Convar_Ghost);
	g_iGhost_alpha = 25;

	g_hGhost_enable = CreateConVar("l4d_perkmod_ghostrider_enable", "0", "Ghost Rider perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hGhost_enable, Convar_Ghost_en);
	g_iGhost_enable = 0;

	//SPITTER
	//twin spitfire
	g_hTwinSF_delay = CreateConVar("l4d_perkmod_twinspitfire_delay", "6", "Twin Spitfire perk: Delay in-between double shots, in seconds (clamped between 0.5 < 20.0)");
	HookConVarChange(g_hTwinSF_delay, Convar_TwinSF);
	g_flTwinSF_delay = 6.0;

	g_hTwinSF_enable = CreateConVar("l4d_perkmod_twinspitfire_enable", "0", "Twin Spitfire perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hTwinSF_enable, Convar_TwinSF_en);
	g_iTwinSF_enable = 0;

	//mega adhesive
	g_hMegaAd_enable = CreateConVar("l4d_perkmod_megaadhesive_enable", "0", "Mega Adhesive perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hMegaAd_enable, Convar_MegaAd_en);
	g_iMegaAd_enable = 0;

	g_hMegaAd_slow = CreateConVar("l4d_perkmod_megaadhesive_slowmultiplier", "0.6", "Mega Adhesive perk: Survivor run speed is MULTIPLIED DIRECTLY by this value - 0.6 means they run at 60% speed (clamped between 0 < 1.0)");
	HookConVarChange(g_hMegaAd_slow, Convar_MegaAd);
	g_flMegaAd_slow = 0.6;

	//CHARGER
	//scattering ram
	g_hScatter_force = CreateConVar("l4d_perkmod_scatteringram_force", "1.6", "Scattering Ram perk: Direct multiplier to force applied to survivors on charge impact (clamped between 1.0 < 3.0)");
	HookConVarChange(g_hScatter_force, Convar_Scatter_force);
	g_flScatter_force = 1.6;

	g_hScatter_hpmult = CreateConVar("l4d_perkmod_scatteringram_healthmultiplier", "0.3", "Scattering Ram perk: Bonus health multiplier, product is ADDED to base health (clamped between 0.01 < 3.0)");
	HookConVarChange(g_hScatter_hpmult, Convar_Scatter_hpmult);
	g_flScatter_hpmult = 0.3;

	g_hScatter_enable = CreateConVar("l4d_perkmod_scatteringram_enable", "0", "Scattering Ram perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hScatter_enable, Convar_Scatter_en);
	g_iScatter_enable = 0;

	//speeding bullet
	g_hBullet_rate = CreateConVar("l4d_perkmod_speedingbullet_rate", "1.5", "Speeding Bullet perk: Time rate while charging (clamped between 1.0 < 10.0)");
	HookConVarChange(g_hBullet_rate, Convar_Bullet);
	g_flBullet_rate = 1.5;

	g_hBullet_enable = CreateConVar("l4d_perkmod_speedingbullet_enable", "0", "Speeding Bullet perk: Allows the perk to be chosen by players, 0=disabled, 1=enabled");
	HookConVarChange(g_hBullet_enable, Convar_Bullet_en);
	g_iBullet_enable = 0;

	//MISC
	//bot preferences for perks
	g_hBot_Sur1 = CreateConVar("l4d_perkmod_bot_survivor1", "1", "Bot preferences for Survivor 1 perks: 1 = stopping power, 2 = double tap, 3 = sleight of hand");

	g_hBot_Sur2 = CreateConVar("l4d_perkmod_bot_survivor2", "2", "Bot preferences for Survivor 2 perks: 1 = unbreakable, 2 = spirit, 3 = helping hand, 4 = martial artist");

	g_hBot_Sur3 = CreateConVar("l4d_perkmod_bot_survivor3", "2", "Bot preferences for Survivor 3 perks: 1 = pack rat, 2 = hard to kill, 3 = extreme conditioning");

	g_hBot_Inf1 = CreateConVar("l4d_perkmod_bot_boomer", "", "Bot preferences for boomer perks: 1 = barf bagged, 2 = blind luck, 3 = dead wreckening (NOTE: You can select more than one using the format '1,3,4', and the game will randomize between your choices)");

	g_hBot_Inf3 = CreateConVar("l4d_perkmod_bot_smoker", "", "Bot preferences for smoker perks: 1 = tongue twister, 2 = squeezer, 3 = drag and drop (NOTE: You can select more than one using the format '1,3,4', and the game will randomize between your choices)");

	g_hBot_Inf4 = CreateConVar("l4d_perkmod_bot_hunter", "", "Bot preferences for hunter perks: 1 = efficient killer, 2 = speed demon (NOTE: You can select more than one using the format '1,3,4', and the game will randomize between your choices)");

	g_hBot_Inf2 = CreateConVar("l4d_perkmod_bot_tank", "", "Bot preferences for tank perks: 1 = adrenal glands, 2 = juggernaut, 3 = metabolic boost, 4 = storm caller, 5 = double the trouble (NOTE: You can select more than one using the format '1,3,4', and the game will randomize between your choices)");

	g_hBot_Inf5 = CreateConVar("l4d_perkmod_bot_jockey", "", "Bot preferences for jockey perks: 1 = ride like the wind, 2 = cavalier, 3 = frogger, 4 = ghost rider (NOTE: You can select more than one using the format '1,3,4', and the game will randomize between your choices)");

	g_hBot_Inf6 = CreateConVar("l4d_perkmod_bot_spitter", "", "Bot preferences for spitter perks: 1 = twin spitfire, 2 = mega adhesive (NOTE: You can select more than one using the format '1,3,4', and the game will randomize between your choices)");

	g_hBot_Inf7 = CreateConVar("l4d_perkmod_bot_charger", "", "Bot preferences for charger perks: 1 = scattering ram, 2 = speeding bullet (NOTE: You can select more than one using the format '1,3,4', and the game will randomize between your choices)");

	//default perks
	g_hSur1_default = CreateConVar("l4d_perkmod_default_survivor1", "2", "Default selected perk for Survivor, Primary: 1 = stopping power, 2 = double tap, 3 = sleight of hand, 4 = pyrotechnician");
	HookConVarChange(g_hSur1_default, Convar_Def_Sur1);
	g_iSur1_default = 2;

	g_hSur2_default = CreateConVar("l4d_perkmod_default_survivor2", "2", "Default selected perk for Survivor, Secondary: 1 = unbreakable, 2 = spirit, 3 = helping hand, 4 = martial artist");
	HookConVarChange(g_hSur2_default, Convar_Def_Sur2);
	g_iSur2_default = 2;

	g_hSur3_default = CreateConVar("l4d_perkmod_default_survivor3", "4", "Default selected perk for Survivor, Secondary: 1 = pack rat, 2 = chem reliant, 3 = hard to kill, 4 = extreme conditioning, 5 = katana");
	HookConVarChange(g_hSur3_default, Convar_Def_Sur3);
	g_iSur3_default = 4;

	g_hInf1_default = CreateConVar("l4d_perkmod_default_boomer", "0", "Default selected perk for Boomer: 1 = barf bagged, 2 = blind luck, 3 = dead wreckening");
	HookConVarChange(g_hInf1_default, Convar_Def_Inf1);
	g_iInf1_default = 0;

	g_hInf2_default = CreateConVar("l4d_perkmod_default_tank", "0", "Default selected perk for Tank: 1 = adrenal glands, 2 = juggernaut, 3 = metabolic boost, 4 = storm caller, 5 = double the trouble");
	HookConVarChange(g_hInf2_default, Convar_Def_Inf2);
	g_iInf2_default = 0;

	g_hInf3_default = CreateConVar("l4d_perkmod_default_smoker", "0", "Default selected perk for Smoker: 1 = tongue twister, 2 = squeezer, 3 = drag and drop");
	HookConVarChange(g_hInf3_default, Convar_Def_Inf3);
	g_iInf3_default = 0;

	g_hInf4_default = CreateConVar("l4d_perkmod_default_hunter", "0", "Default selected perk for Hunter: 1 = body slam, 2 = efficient killer, 3 = grasshopper, 4 = old school, 5 = speed demon");
	HookConVarChange(g_hInf4_default, Convar_Def_Inf4);
	g_iInf4_default = 0;

	g_hInf5_default = CreateConVar("l4d_perkmod_default_jockey", "0", "Default selected perk for Jockey: ");
	HookConVarChange(g_hInf5_default, Convar_Def_Inf5);
	g_iInf5_default = 0;

	g_hInf6_default = CreateConVar("l4d_perkmod_default_spitter", "0", "Default selected perk for Spitter: ");
	HookConVarChange(g_hInf6_default, Convar_Def_Inf6);
	g_iInf6_default = 0;

	g_hInf7_default = CreateConVar("l4d_perkmod_default_charger", "0", "Default selected perk for Charger: ");
	HookConVarChange(g_hInf7_default, Convar_Def_Inf7);
	g_iInf7_default = 0;

	//enable perk trees
	//-----------------
	g_hSur1_enable = CreateConVar("l4d_perkmod_perktree_survivor1_enable", "1", "If set to 1, players will be allowed to select perks from the primary Survivor tree.");
	HookConVarChange(g_hSur1_enable, Convar_Sur1_en);
	g_iSur1_enable = 1;

	g_hSur2_enable = CreateConVar("l4d_perkmod_perktree_survivor2_enable", "1", "If set to 1, players will be allowed to select perks from the secondary Survivor tree.");
	HookConVarChange(g_hSur2_enable, Convar_Sur2_en);
	g_iSur2_enable = 1;

	g_hSur3_enable = CreateConVar("l4d_perkmod_perktree_survivor3_enable", "1", "If set to 1, players will be allowed to select perks from the tertiary Survivor tree.");
	HookConVarChange(g_hSur3_enable, Convar_Sur3_en);
	g_iSur3_enable = 1;

	g_hInf1_enable = CreateConVar("l4d_perkmod_perktree_boomer_enable", "0", "If set to 1, players will be allowed to select perks from the Boomer tree.");
	HookConVarChange(g_hInf1_enable, Convar_Inf1_en);
	g_iInf1_enable = 0;

	g_hInf2_enable = CreateConVar("l4d_perkmod_perktree_tank_enable", "0", "If set to 1, players will be allowed to select perks from the Tank tree.");
	HookConVarChange(g_hInf2_enable, Convar_Inf2_en);
	g_iInf2_enable = 0;

	g_hInf3_enable = CreateConVar("l4d_perkmod_perktree_smoker_enable", "0", "If set to 1, players will be allowed to select perks from the Smoker tree.");
	HookConVarChange(g_hInf3_enable, Convar_Inf3_en);
	g_iInf3_enable = 0;

	g_hInf4_enable = CreateConVar("l4d_perkmod_perktree_hunter_enable", "0", "If set to 1, players will be allowed to select perks from the Hunter tree.");
	HookConVarChange(g_hInf4_enable, Convar_Inf4_en);
	g_iInf4_enable = 0;

	g_hInf5_enable = CreateConVar("l4d_perkmod_perktree_jockey_enable", "0", "If set to 1, players will be allowed to select perks from the Jockey tree.");
	HookConVarChange(g_hInf5_enable, Convar_Inf5_en);
	g_iInf5_enable = 0;

	g_hInf6_enable = CreateConVar("l4d_perkmod_perktree_spitter_enable", "0", "If set to 1, players will be allowed to select perks from the Spitter tree.");
	HookConVarChange(g_hInf6_enable, Convar_Inf6_en);
	g_iInf6_enable = 0;

	g_hInf7_enable = CreateConVar("l4d_perkmod_perktree_charger_enable", "0", "If set to 1, players will be allowed to select perks from the Charger tree.");
	HookConVarChange(g_hInf7_enable, Convar_Inf7_en);
	g_iInf7_enable = 0;

	//perk hierarchy
	//--------------
	g_hInfAll_enable = CreateConVar("l4d_perkmod_perktree_infected_enable", "0", "If set to 1, players will be allowed to select perks as Special Infected (affects ALL perks for SI).");
	HookConVarChange(g_hInfAll_enable, Convar_InfAll);
	g_iInfAll_enable = 0;

	g_hSurAll_enable = CreateConVar("l4d_perkmod_perktree_survivor_enable", "1", "If set to 1, players will be allowed to select perks as Survivors (affects ALL perks for Survivors).");
	HookConVarChange(g_hSurAll_enable, Convar_SurAll);
	g_iSurAll_enable = 1;

	//force random perks
	g_hForceRandom = CreateConVar("l4d_perkmod_forcerandomperks", "0", "If set to 1, players will be assigned random perks at roundstart, and they cannot edit their perks.");
	HookConVarChange(g_hForceRandom, Convar_ForceRandom);
	g_iForceRandom = 0;

	//enable random perk choice
	g_hRandomEnable = CreateConVar("l4d_perkmod_randomperks_enable", "1", "If set to 1, players will be allowed to randomize their perks at roundstart. Otherwise, they can only customize their perks or use default perks.");
	HookConVarChange(g_hRandomEnable, Convar_Random_en);
	g_iRandomEnable = 1;

	//misc game convars
	g_hMenuAutoShow_enable = CreateConVar("l4d_perkmod_autoshowmenu", "1", "If set to 1, the perks menu will automatically be shown at the start of every round.");
}
...
//katana
public Convar_Katana_en (Handle:convar, const String:oldValue[], const String:newValue[])
{
	new iI=StringToInt(newValue);
	if (iI==0)
		iI=0;
...
public Convar_Katana_en_sur (Handle:convar, const String:oldValue[], const String:newValue[])
{
	new iI=StringToInt(newValue);
	if (iI==0)
		iI=0;
		...
		iI=1;
	g_iKatana_enable_sur = iI;
}

public Convar_Katana_en_vs (Handle:convar, const String:oldValue[], const String:newValue[])
{
	new iI=StringToInt(newValue);
	if (iI==0)
		iI=0;
...
Adrenal_OnGameFrame()
{
	...
	for (new iI=1; iI<=g_iAdrenalRegisterCount; iI++)
	{
		...
		if (g_flAdrenalTimeStamp[iCid] < flTimeStamp_ret)
		{
			...
			//so that the katana cooldown gui is shown properly
	...
	if (action==MenuAction_Select)
	{
		switch(param2)
		{
			...
			case 2:
			{
				...
				PrintHintText(param1,"%t", "ThanksForChoosingMessage");
			}
			case 3:
			{
				g_iConfirm[param1]=1;
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "Hucksmod - Main Menu");
	...
	if (g_iSur3[iCid]==1
		...
		&& (g_iKatana_enable==1			&&	g_iL4D_GameMode==0
		|| g_iKatana_enable_sur==1		&&	g_iL4D_GameMode==1
		|| g_iKatana_enable_vs==1		&&	g_iL4D_GameMode==2))
		st_perk="Katana";
	else
		st_perk="Not set";
...
public Handle:Menu_ShowChoices (iCid)
{
	...
	if (iPerk == 1
		...
		|| g_iDT_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iDT_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Double Tap (%t, %t, %t)", "DoubleTapDescriptionPanel", "SleightOfHandDescriptionPanel", "DoubleTapRestrictionWarning" ) ;
	else if (iPerk == 3
		&& (g_iSoH_enable==1		&&	g_iL4D_GameMode==0
		|| g_iSoH_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iSoH_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Sleight of Hand (%t +%i%%)", "SleightOfHandDescriptionPanel", RoundToNearest(100 * ((1/g_flSoH_rate)-1))) ;
	else if (iPerk == 4
		&& (g_iPyro_enable==1		&&	g_iL4D_GameMode==0
		|| g_iPyro_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iPyro_enable_vs==1		&&	g_iL4D_GameMode==2))
	...
	if (iPerk == 1
		...
		|| g_iExtreme_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iExtreme_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128, "Extreme Conditioning (+%i%% %t)", RoundToNearest(g_flExtreme_rate*100-100), "ExtremeCondDescriptPanelCoop");   
	else if (iPerk == 5
		&& (g_iKatana_enable==1		&&	g_iL4D_GameMode==0
		|| g_iKatana_enable_sur==1	&&	g_iL4D_GameMode==1
		|| g_iKatana_enable_vs==1	&&	g_iL4D_GameMode==2))
		Format(st_perk,128,"Katana (%t)", "KatanaDescriptionPanel");   
	else
		Format(st_perk,128,"%t", "NotSet");
...
{
	new Handle:menu=CreatePanel();
	SetPanelTitle(menu,"Hucksmod: Your perks for this round");
	decl String:st_perk[128];
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "Hucksmod - Survivor: Primary");
	decl String:st_display[MAXPLAYERS+1];
	...
	else
	{
		...
		{
			Format(st_display,64,"%t +%i%%", "SleightOfHandDescriptionPanel", RoundToNearest(100 * ((1/g_flDT_rate_reload)-1)));
			DrawPanelText(menu,st_display);
		}
	...
	else
	{
		...
		DrawPanelItem(menu,st_display);
		Format(st_display,64,"%t +%i%%", "SleightOfHandDescriptionPanel", RoundToNearest(100 * ((1/g_flSoH_rate)-1)));
		DrawPanelText(menu,st_display);
	}
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "Hucksmod - Survivor: Secondary");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "Hucksmod - Survivor: Tertiary");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
	...
	else
	{
		...
		DrawPanelItem(menu,st_display);
		Format(st_display,64,"%t: +%i%%", "ExtremeConditioningDescriptionPanel", RoundToNearest(100*g_flExtreme_rate-100));
		DrawPanelText(menu,st_display);
	}

	//set name for perk 5
	if (g_iKatana_enable==0				&&	g_iL4D_GameMode==0
		|| g_iKatana_enable_sur==0		&&	g_iL4D_GameMode==1
		|| g_iKatana_enable_vs==0		&&	g_iL4D_GameMode==2
		|| g_iL4D_12 != 2)
	{
		DrawPanelItem(menu,"disabled", ITEMDRAW_NOTEXT);
	}
	else
	{
		...
		Format(st_display,64,"Katana %s",st_current);
		DrawPanelItem(menu,st_display);
		Format(st_display,64,"%t", "KatanaDescriptionPanel");
		DrawPanelText(menu,st_display);
	}
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "Hucksmod - Boomer");
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "Hucksmod - Tank");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "Hucksmod - Smoker");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "Hucksmod - Hunter");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "Hucksmod - Jockey");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "Hucksmod - Spitter");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];
...
{
	new Handle:menu = CreatePanel();
	SetPanelTitle(menu, "Hucksmod - Charger");
	decl String:st_display[MAXPLAYERS+1];
	decl String:st_current[10];

...
void event_charger_pummel_start(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(client > 0 && IsClientInGame(client) && GetClientTeam(client) == 3 && IsPlayerAlive(client))
	{
        ...
        int entity = L4D_PipeBombPrj(client, pos, view_as<float>({0.0, 0.0, 0.0}));
...
public void OnPluginStart()
{
    ...
    C_damage_multiple = CreateConVar("si_enhance_charger_damage_multiple", "1.0", "blast damage multiple to survivors. at least keep 1.0 damage", _, true, 0.0);
    C_damage_multiple.AddChangeHook(convar_changed);
    CreateConVar("si_enhance_charger_version", PLUGIN_VERSION, "version of SI Enhance Charger", FCVAR_NOTIFY | FCVAR_DONTRECORD);
	//AutoExecConfig(true, "si_enhance_charger");
	get_all_cvars();

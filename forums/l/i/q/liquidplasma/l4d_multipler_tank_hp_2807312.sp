...
static ConVar hCvar_BasicTankHP_Survival;
static ConVar hCvar_BasicTankHP_Versus;
static ConVar hCvar_BasicTankHP_Coop;
...
static int BasicTankHP_Survival;
static int BasicTankHP_Versus;
static int BasicTankHP_Coop;
static int TankHP;
...
public void OnPluginStart()
{
	...
	hCvar_IgnitionModes = CreateConVar("director_ignition_modes", "0", "Enables/Disables Ignition modes towards the Tank.\n0 = Do Nothing.\n1 = Allows the Tank not to catch fire but he takes damage if touches the fire.\n2 = Allows the Tank to catch fire according to the amount of life his has.", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	hCvar_DisplayHealth = CreateConVar("director_display_health", "0", "Enables/Disables Display Tanks health in crosshair.\n0 = Display HP OFF.\n1 = Shows the state of health in percentage.\n2 = Shows the state of health with numbering.\n3 = Displays health status with bar and numbering", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	...
	hCvar_BasicTankHP = CreateConVar("director_basic_tankhp", "6000.0", "Basic amount of HP for the Tank when neither coop, survival or verus is active gamemode.", FCVAR_NOTIFY, true, 0.0, true, 65535.0);
	hCvar_BasicTankHP_Survival = CreateConVar("director_basic_tankhp_survival", "4000.0", "Basic amount of HP for the Tank in survival gamemode.", FCVAR_NOTIFY, true, 0.0, true, 65535.0);
	hCvar_BasicTankHP_Versus = CreateConVar("director_basic_tankhp_versus", "6000.0", "Basic amount of HP for the Tank in versus gamemode.", FCVAR_NOTIFY, true, 0.0, true, 65535.0);
	hCvar_BasicTankHP_Coop = CreateConVar("director_basic_tankhp_coop", "4000.0", "Basic amount of HP for the Tank in coop mode.", FCVAR_NOTIFY, true, 0.0, true, 65535.0);
	...
	hCvar_TankSurvivorMultipler = CreateConVar("director_survivor_multipler", "1", "Multiply Tank's HP if there are more than 4 survivors or start from one survivor.\n0 = Start multiplying from one.\n1 = Start multiplying from 4.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	hCvar_EnabledAds = CreateConVar("director_enabled_ads", "0", "Enables/Disables Tank life status ads in chat.\n0 = Ads OFF.\n1 = Show full ads.\n2 = Show only health status.\n3 = Show only ignition status", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	ExtraFirstAid = CreateConVar("director_extra_first_aid", "1" , "Allow extra first aid kits for extra players. 0: No extra kits. 1: one extra kit per player above four", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	hCvar_BasicTankHP_Survival.AddChangeHook(Event_ConVarChanged);
	hCvar_BasicTankHP_Versus.AddChangeHook(Event_ConVarChanged);
	hCvar_BasicTankHP_Coop.AddChangeHook(Event_ConVarChanged);
...
void GetCvars()
{
	...
	BasicTankHP_Survival = hCvar_BasicTankHP_Survival.IntValue;
	BasicTankHP_Versus = hCvar_BasicTankHP_Versus.IntValue;
...
public void OnMapStart() 
{
	...
	if (AreAllInGame() == true) 
	{
		...
		{
			MedkitsGiven = true;
			MedkitTimer = CreateTimer(15.0, Timer_SpawnExtraMedKit);
		}
	}
}
 ...
 * @param UserIDb Client ID
 */
public Action TankSpawnTimer(Handle hTimer, any UserID)
{
	BasicTankHP_Coop = hCvar_BasicTankHP_Coop.IntValue;
	...
	static char sDifficulty[64];
	hCvar_Difficult.GetString(sDifficulty, sizeof(sDifficulty));
	...
	if(strncmp(sDifficulty, "impossible", sizeof(sDifficulty), false) == 0 || strncmp(sDifficulty, "hard", sizeof(sDifficulty), false) == 0) {
		BasicTankHP_Coop = BasicTankHP_Coop * 2;
	}

	if(strncmp(sGamemode, "survival", sizeof(sGamemode), false) == 0) {
		TankHP = RoundFloat(BasicTankHP_Survival * TankHp_Multi);
	}
	else if(strncmp(sGamemode, "coop", sizeof(sGamemode), false) == 0 || strncmp(sGamemode, "realism", sizeof(sGamemode), false) == 0) {
		TankHP = RoundFloat(BasicTankHP_Coop * TankHp_Multi);
	}
	else if(strncmp(sGamemode, "versus", sizeof(sGamemode), false) == 0) {
		TankHP = RoundFloat(BasicTankHP_Versus * TankHp_Multi);
	}
	else {
		TankHP = RoundFloat(BasicTankHP * TankHp_Multi);
	}
	...
	if (iCvar_IgnitionModes == FLAMMABLE && strncmp(sDifficulty, "Easy", sizeof(sDifficulty), false) == 0) FindConVar(bLeft4DeadTwo ? "tank_burn_duration" : "tank_burn_duration_normal").IntValue = TankBurnHP;
	else if (iCvar_IgnitionModes == FLAMMABLE && strncmp(sDifficulty, "Hard", sizeof(sDifficulty), false) == 0) FindConVar("tank_burn_duration_hard").IntValue = TankBurnHP;
	else if (iCvar_IgnitionModes == FLAMMABLE && strncmp(sDifficulty, "Impossible", sizeof(sDifficulty), false) == 0) FindConVar("tank_burn_duration_expert").IntValue = TankBurnHP;
...
{	
	...
	{
		if (StrContains(sMessage, CTag[i]) == -1) 										
			continue;
		else if (!CTagReqSayText2[i])
			ReplaceString(sMessage, maxlength, CTag[i], CTagCode[i]); 					
		else																				
		{	
			if (iRandomPlayer == NO_INDEX)												
			{
				...
				if (iRandomPlayer == NO_PLAYER) 
					ReplaceString(sMessage, maxlength, CTag[i], CTagCode[5]); 			
				else 
					ReplaceString(sMessage, maxlength, CTag[i], CTagCode[i]); 			
			}
			else 																			

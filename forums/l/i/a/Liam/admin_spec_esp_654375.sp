...
public OnPluginStart( )
{
    ...
    CreateConVar("spec_version", VERSION,
        "Current version of Admin Spectate ESP Plugin", FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
    RegisterOffsets( );
    RegisterCommands( );
    RegisterHooks( );
}
...
RegisterOffsets( )
{
    g_ObserverTargetOffset = FindSendPropOffs("CBasePlayer", "m_hObserverTarget");
...
RegisterCommands( )
{
    RegAdminCmd("sm_spectate", Command_Spectate, ADMFLAG_GENERIC, "Enables or disables the spectate option.");
}
...
public Action:Timer_RefreshView(Handle:timer, any:client)
{
    ...
    else
    {
        ...
        GetClientAuthString(target, f_SteamID, sizeof(f_SteamID));
        ...
        for(new i = 1; i < f_MaxClients; i++)
        {
            ...
            f_Width = ((g_Width / GetVectorDistance(f_TargetOrigin, f_OthersOrigin)) * 100.0);

            switch(f_Team)
            {
                case 3:
                    TE_SetupBeamPoints(f_TargetOrigin, f_OthersOrigin, g_Sprite, 0, 0, 0, 
                        g_Life, f_Width, f_Width, 1, 0.0, blueColor, 0);

                case 2:
                    TE_SetupBeamPoints(f_TargetOrigin, f_OthersOrigin, g_Sprite, 0, 0, 0, 
                        g_Life, f_Width, f_Width, 1, 0.0, redColor, 0);
            }
...
SetClientObserve(client, target)
{
    ...
    SetEntDataEnt(client, g_ObserverTargetOffset, target, true);
    PrintToChat(client, "You are now observing %s.", name);
}

GetClientObserve(client)
{
    return GetEntDataEnt(client, g_ObserverTargetOffset);
}

...
public Plugin:myinfo = 
{
	...
	author = "Hell Phoenix updated by LIONz (used bl4nk`s name replace code)",
...
public OnPluginStart(){
	...
	cvarnamereplace = CreateConVar("sm_name_replace","I Had a Bad Name","You can use any word here...this is what replaces the swear word",FCVAR_PLUGIN);
		...
		if(client != 0){
			...
			while (i < g_swearNum){
				if (StrContains(clientName, g_swearwords[i], false) != -1 ){
					...
					new String:namereplacement[64];
					GetConVarString(cvarnamereplace, namereplacement, 64);
					if(CanName)
					{
						ClientCommand(client, "name \"%s\"", namereplacement);
					}
					else
					{
						SetClientInfo(client, "name", namereplacement);
					}
				}
				i++;
			}	

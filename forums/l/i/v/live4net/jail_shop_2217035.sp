...
new Handle:cvarCreditsPerTime = INVALID_HANDLE;
new Handle:cvarCreditsPerTimeVIP = INVALID_HANDLE;
new Handle:cvar_1;
new Handle:cvar_2;
new Handle:cvar_3;
new Handle:cvar_4;
...
public OnPluginStart()
{
	...
	cvarCreditsPerTime = CreateConVar("shop_creditspertime", "0", "How many credits players receive for every 120sec on server");
	cvarCreditsPerTimeVIP = CreateConVar("shop_creditspertime_vip", "0", "How many credits players receive for every 120sec on server for VIP (flag a)");
...
public OnMapStart()
{
	...
	if (GetConVarInt(cvarCreditsPerTime) != 0)
	{
		hTimerCredits = CreateTimer(120.0, TimerCredits, _, TIMER_REPEAT);
	}
}
		...
		else
		{
			if (IsClientInGame(client) && !IsPlayerAlive(client))
			{
				if (g_iCreditos[client] >= GetConVarInt(cvar_15))
				{
					...
					PrintToChatAll("\x04[ SHOP ] \x05The player\x03 %s \x05has revived!", nome); 
			...
			{
				{
					...
					if (g_iCreditos[client] >= GetConVarInt(cvar_18))
					{
						if (IsPlayerAlive(client))
						{
							...
							PrintToChat(client, "\x04[ SHOP ] \x05%t","Gravity2", g_iCreditos[client],GetConVarInt(cvar_18));
						}
						else
						{
							PrintToChat(client, "\x04[ SHOP ] \x05%t","Vivo");
						...
						{
							PrintToChat(client, "\x04[ SHOP ] \x05%t","Item da gravidade", g_iCreditos[client],GetConVarInt(cvar_18));
						}
				}
...
public Action:SetCreditos2(client, args)
{
    ...
    { 
        ...
        { 
			  ...
			  PrintToChat(client, "[ SHOP ] Set \x03%i \x01credits in the player: %N", amount, iClient);
              PrintToChat(iClient, "[ SHOP ] Set \x03%i \x01credits in the player: %N", amount, iClient);
        } 
    } 
...
public Action:SetCreditos(client, args)
{
	...
	{ 
		...
		{ 
			  g_iCreditos[iClient] += amount;
			  PrintToChat(client, "[ SHOP ] Give \x03%i \x01credits in the player: %N", amount, iClient);
			  PrintToChat(iClient, "[ SHOP ] Give \x03%i \x01credits in the player: %N", amount, iClient);
	...
	for (new i = 0; i < TargetCount; i++)
	{
		...
		if (IsClientInGame(iClient) && amount > 0)
		{
			if (g_iCreditos[client] < amount)
				PrintToChat(client, "\x04[ SHOP ] \x05%t","NoCredits");
...
public OnMapEnd ()
{	
	if (hTimerCredits != INVALID_HANDLE)
	{
		CloseHandle(hTimerCredits);
		hTimerCredits = INVALID_HANDLE;
	}
	if (!g_dbConnected && GetConVarInt(cvarUseDataBase) != 0)
        DB_Connect();
	if( GetConVarInt(cvarUseDataBase) != 0 )
...
public Action:TimerCredits (Handle:timer, any:client)
{
	for( new i = 1; i <= MaxClients; i++ )
		if( IsClientInGame( i ) && !IsFakeClient(i) && GetClientTeam(i) != CS_TEAM_SPECTATOR)
		{
			if (IsPlayerReservationAdmin(i))
			{
				PrintToChat(i, "\x04[ SHOP ] \x05 You get %d credits for being connected", GetConVarInt(cvarCreditsPerTimeVIP));
				g_iCreditos[i] += GetConVarInt(cvarCreditsPerTimeVIP);
			}
			else
			{
				PrintToChat(i, "\x04[ SHOP ] \x05 You get %d credits for being connected", GetConVarInt(cvarCreditsPerTime));
				g_iCreditos[i] += GetConVarInt(cvarCreditsPerTime);
			}
		}
}

...
new Handle:cvarUseDataBase = INVALID_HANDLE;//mysql
...
new g_sprite;
new g_HaloSprite;
new String:ShopCommand[64];
new String:Credits_Command[64];
new String:Gift_Command[64];
new String:Give_Command[64];
new String:Set_Command[64];
new String:ShowCredits_Command[64];
new String:Revive_Command[64];

//mysql
#define TABLE_NAME "jail_shop"
...
public OnPluginStart()
{
	...
	HookEvent("player_disconnect", event_PlayerDisconnect, EventHookMode_Pre);//mysql
	// ======================================================================
	...
	comando = CreateConVar("shop_comando", "shop", "Put here the command for open the shop menu, without especial caracters!");
	comando2 = CreateConVar("creditos_comando", "creditos", "Put here the command for view credits, without especial caracters!");
	comando3 = CreateConVar("reviver_comando", "reviver", "Put here the command for revive, without especial caracters!");
	comando4 = CreateConVar("give_comando", "dar", "Put here the command for give player credit, without especial caracters!");
	comando5 = CreateConVar("set_comando", "set", "Put here the command for set player credit, without especial caracters!");
	comando6 = CreateConVar("showcredits_comando", "vercreditos", "Put here the command for open menu with all players credits, without especial caracters!");
	comando7 = CreateConVar("gift_comando", "doar", "Put here the command for open gift player credit, without especial caracters!");
	cvarUseDataBase = CreateConVar("shop_mysql", "0", "Should we use a mysql database to store credits");	//mysql
	...
	//mysql
	if (!g_dbConnected && GetConVarInt(cvarUseDataBase) != 0)
        DB_Connect();
	...
	if(GetConVarBool(cvarCreditosSave))
	{
		for(new client = 1; client <= MaxClients; client++)
		{
			...
			{
				if (GetConVarInt(cvarUseDataBase) != 0)
				{
					DB_AddPlayer(client);
...
public OnConfigsExecuted()
{
	...
	GetConVarString(comando, ShopCommand, sizeof(ShopCommand));
	Format(ConsoleCmd, sizeof(ConsoleCmd), "sm_%s", ShopCommand);
	RegConsoleCmd(ConsoleCmd, SHOPMENU);
	
	// ********* Credits COMMAND ********* //
	decl String:ConsoleCmd2[64];
	GetConVarString(comando2, Credits_Command, sizeof(Credits_Command));
	Format(ConsoleCmd2, sizeof(ConsoleCmd2), "sm_%s", Credits_Command);
	RegConsoleCmd(ConsoleCmd2, Creditos);
	
	// ********* Revive COMMAND ********* //
	GetConVarString(comando3, Revive_Command, sizeof(Revive_Command));
	Format(ConsoleCmd, sizeof(ConsoleCmd), "sm_%s", Revive_Command);
	RegConsoleCmd(ConsoleCmd, Reviver);
	
	// ********* Give COMMAND ********* //
	GetConVarString(comando4, Give_Command, sizeof(Give_Command));
	Format(ConsoleCmd, sizeof(ConsoleCmd), "sm_%s", Give_Command);
	RegAdminCmd(ConsoleCmd, SetCreditos, ADMFLAG_ROOT);
	
	// ********* Set COMMAND ********* //
	GetConVarString(comando5, Set_Command, sizeof(Set_Command));
	Format(ConsoleCmd, sizeof(ConsoleCmd), "sm_%s", Set_Command);
	RegConsoleCmd(ConsoleCmd, SetCreditos2);
	
	// ********* ShowCredits COMMAND ********* //
	GetConVarString(comando6, ShowCredits_Command, sizeof(ShowCredits_Command));
	Format(ConsoleCmd, sizeof(ConsoleCmd), "sm_%s", ShowCredits_Command);
	RegConsoleCmd(ConsoleCmd, Command_ShowCredits);
	
	// ********* Gift COMMAND ********* //
	GetConVarString(comando7, Gift_Command, sizeof(Gift_Command));
	Format(ConsoleCmd, sizeof(ConsoleCmd), "sm_%s", Gift_Command);
	RegConsoleCmd(ConsoleCmd, Command_SendCredits);
...
public OnMapStart()
{
	...
	PrecacheModel("models/player/tm_phoenix_variantc.mdl");
	//mysql
	if (!g_dbConnected && GetConVarInt(cvarUseDataBase) != 0)
        DB_Connect();
	for(new client = 1; client <= MaxClients; client++)
...
public Action:MensajesSpawn(Handle:timer, any:client)
{
	if(GetConVarBool(cvarSpawnMsg) && IsClientInGame(client))
	{
		...
		PrintToChat(client, "\x04[ SHOP ] \x05%t","Type", ShopCommand);
	}
}
...
public Action:PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(GetAllPlayersCount() >= GetConVarInt(cvarMinPlayersToGetCredits) && (GetConVarInt(cvarCreditsOnWarmup) != 0 || GameRules_GetProp("m_bWarmupPeriod") != 1) ) 
	{
		if (g_iCreditos[attacker] < GetConVarInt(cvarCreditosMax))
		{
			...
			{
				if (IsPlayerReservationAdmin(attacker))
					PrintToChat(attacker, "\x04[ SHOP ] \x05%t","KillCT", g_iCreditos[attacker],GetConVarInt(cvarCreditosKill_CT_VIP));
				else
			...
			{
				if (IsPlayerReservationAdmin(attacker))
					PrintToChat(attacker, "\x04[ SHOP ] \x05%t","KillT", g_iCreditos[attacker],GetConVarInt(cvarCreditosKill_T_VIP));
				else
		...
		else
		{
			...
			PrintToChat(attacker, "\x04[ SHOP ] \x05%t","Maximo", g_iCreditos[attacker]);
		}
	}
}
...
public Action:SHOPMENU(client,args)
{
	...
	PrintToChat(client, "\x04[ SHOP ] \x05%t","Creditos2", g_iCreditos[client]);
}
			...
			{
				{
					...
					if (g_iCreditos[client] >= GetConVarInt(cvar_17))
					{
						if (IsPlayerAlive(client))
						{
							...
							bhop[client] = true;
							PrintToChat(client, "\x04[ SHOP ] \x05%t","Bhop2", g_iCreditos[client],GetConVarInt(cvar_17));
						}
						else
						{
							PrintToChat(client, "\x04[ SHOP ] \x05%t","Vivo");
						...
						{
							PrintToChat(client, "\x04[ SHOP ] \x05%t","Item do bhop", g_iCreditos[client],GetConVarInt(cvar_17));
						}
				}
	...
	for (new i = 0; i < TargetCount; i++)
	{
		...
		if (IsClientInGame(iClient) && amount > 0)
		{
			if (g_iCreditos[client] < amount)
				PrintToChat(client, "[ SHOP ] You don't have so many credits");
			else
			{
				g_iCreditos[client] -= amount;
				g_iCreditos[iClient] += amount;
				PrintToChat(client, "[ SHOP ] You give \x03%i \x01credits for player: %N", amount, iClient);
				PrintToChat(iClient, "[ SHOP ] You get \x03%i \x01credits from player: %N", amount, client);
			}
		}
	}
	return Plugin_Continue;
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype)
{
	...
	else if(IsValidClient(attacker))
	{
		...
		if((vampire[attacker] &&  GetClientTeam(victim) != GetClientTeam(attacker)) || (vampire[attacker] && GetConVarInt(g_friendlyfire) == 1))
		{
...
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"));
	if( GetConVarInt(cvarUseDataBase) != 0 && client > 0 && !IsFakeClient(client) )
	{
		if (!g_dbConnected && GetConVarInt(cvarUseDataBase) != 0)
			DB_Connect();
		DB_WriteCredits(client);
...
static CreateTables() {
    Format(g_sqlBuffer, sizeof(g_sqlBuffer), "CREATE TABLE IF NOT EXISTS %s (accountID INT NOT NULL PRIMARY KEY default 0, auth varchar(64) NOT NULL default '', name varchar(64) NOT NULL default '', credits INT NOT NULL default 0);", TABLE_NAME);
...
public DB_AddPlayer(client) {
	...
	Format(g_sqlBuffer, sizeof(g_sqlBuffer), "INSERT IGNORE INTO %s (accountID,auth,name,credits) VALUES (%d, '%s', '%s', %d);", TABLE_NAME, id, auth, sanitized_name, DEFAULT_CREDITS);
 ...
 */
public DB_FetchCredits(client) {
    new credits = 0;

    if (db != INVALID_HANDLE) {
        SQL_LockDatabase(db);
        Format(g_sqlBuffer, sizeof(g_sqlBuffer), "SELECT credits FROM %s WHERE accountID = %d", TABLE_NAME, GetSteamAccountID(client));
        ...
        if (query == INVALID_HANDLE) {
            ...
            LogMessage("Couldn't fetchcredits for %N, probably it is first connection of client", client);
        }

        CloseHandle(query);
        SQL_UnlockDatabase(db);
    }
    g_iCreditos[client] = credits;
}

/**
 * Writes the credits to the database.
 */
public DB_WriteCredits(client) {
    Format(g_sqlBuffer, sizeof(g_sqlBuffer), "UPDATE %s set credits = %d WHERE accountID = %d", TABLE_NAME, g_iCreditos[client], GetSteamAccountID(client));
    SQL_TQuery(db, SQLErrorCheckCallback, g_sqlBuffer);
}

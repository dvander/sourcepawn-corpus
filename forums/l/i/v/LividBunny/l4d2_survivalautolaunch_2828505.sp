...
#define PLUGIN_VERSION "1.0.4s"

//#define _DEBUG

public Plugin:myinfo =
{
	name = "L4D2 Survival Autolaunch",
	author = " AtomicStryker + ProdigySim (survival edit: LividBunny)",
	description = " Auto Launch for Survival ",
...
{
	CreateConVar("l4d2_sv_autolaunch_version", PLUGIN_VERSION, "L4D2 Survival Autolaunch Version", FCVAR_PLUGIN|FCVAR_REPLICATED|FCVAR_DONTRECORD);
	countDownCvar = CreateConVar("l4d2_sv_autolaunch_time", "30", " Time after which an Auto Launch is done ", FCVAR_PLUGIN|FCVAR_REPLICATED);
	repeatMapCvar = CreateConVar("l4d2_sv_repeat_map", "1", " Restart the map instead of returning to lobby (Sometimes buggy!)", FCVAR_PLUGIN|FCVAR_REPLICATED);
	...
	isSurvival = StrEqual(buffer, "survival");
...
{
	isSurvival = StrEqual(newValue, "survival");
}

public Action:_ItemPickup_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (inRound && !roundHandled && isSurvival && countDownTimer == INVALID_HANDLE)
	{	
...
{	
	if(isSurvival) PrintHintTextToAll("SURVIVE!");
...
public Action:_RoundEnd_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(isSurvival && GetEventInt(event, "reason") == 15 && GetConVarBool(repeatMapCvar))
	{
		PrintToChatAll("Survival Match Ended! Restarting map in 4 seconds!");
...
public Action:_Spawn_Timer(Handle:timer)
{
	...
	PrintToChatAll("Auto Starting Survival Round in %i seconds", countDownTimeLeft);
...
public Action:_CountdownTimer(Handle:timer)
{
	...
	{
		PrintHintTextToAll("Survival Autostart\nin %i Seconds", countDownTimeLeft);
		--countDownTimeLeft;
	}
	else
	{
...
{	
	...
	ForceChangeLevel(currentMap, "Survival Auto Restart");
...
static SurvivalAutoStart()
{
	...
	if (trigger == -1)
	{
		PrintToChatAll("[SURV] Couldn't find start trigger button!");
		return;
	}
	
	AcceptEntityInput(trigger, "Press");
	PrintToChatAll("Survival Auto Start!");
}
...
static FindSurvivalButton()
{
	...
	for(new i = 0; i < GetMaxEntities(); i++)
	{
		if(IsValidEdict(i) && IsValidEntity(i))
		{
			...
			{
				LogMessage("[SURV] Entlist: %s", buffer);
			}
		}		
	}
	#endif
...
DebugPrintToChatAll(const String:format[], any:...)
{
	...
	PrintToChatAll("[DEBUG SURV] %s", buffer);

...
public OnPluginStart()
{
	...
	ChDelaySURV = CreateConVar("sm_l4d_fmc_chdelaysurv", "0.0", "Delay before surv mission change (float in sec).");
...
public Action:Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(l4d_gamemode() == 3 && StrEqual(next_mission_force, "none") != true)
	{
		if (GetConVarInt(DebugEvent) == 1)
		{
			PrintToChatAll("\x04[FMC DEBUG]\x03 MODE: \"%d\" EVENT: START FMC TIMER ", l4d_gamemode());
...
PluginInitialization()
{
	...
	else if (GameMode == 3)
	{
		...
		PrintToServer("[FMC] Discovered survival gamemode. Link to sm_l4dsv_mapchanger.");
	}
	else

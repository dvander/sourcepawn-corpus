...
public Action:Command_Say(client, args)
{
 	...
 	if (strcmp(szParts[0],"bet",false) == 0)
	{
		...
		if (StrContains(szParts[1],"t",false) == -1)
		{
		...
		if (StrContains(szParts[1],"t", false) != -1)
		{
			...
			ReplaceString(szParts[2][0],7,"\"","",false);
			if (IsCharNumeric(szParts[2][0]))
			{
				
				iAmount = StringToInt(szParts[2]);
			}
			else if (StrContains(szParts[2],"all",false) != -1)
			{
				iAmount = iBank;
			}
			if (StrContains(szParts[2],"half", false) != -1)
			{
				iAmount = (iBank / 2) + 1;
			}
			if (StrContains(szParts[2],"third", false) != -1)
			{
				iAmount = (iBank / 3) + 1;
			}
			...
			g_iPlayerBetData[client][BET_TEAM] = (StrContains(szParts[1],"ct",false) == -1 ? 2 : 3); // 2 = t, 3 = ct

...
public Action:EquipKnife(Handle:timer, any:client)
{
	...
	if(IsValidClient(client, true))
	{
		...
		CreateTimer(1.0, EquipKnifeCheck, client);
	}
	
}

public Action:EquipKnifeCheck(Handle:timer, any:client)
{
	new iWeapon = GetPlayerWeaponSlot(client, 2);
	new iItem;
	...
	if(iWeapon == INVALID_ENT_REFERENCE)
	{
		...
		EquipPlayerWeapon(client, iItem);
		PrintToChat(client, "[SM KNIFE] \x06 The knife is broken,please choose another!");
	}
	else
	{
		return;

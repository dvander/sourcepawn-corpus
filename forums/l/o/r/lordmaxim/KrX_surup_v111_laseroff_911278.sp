...
* -----------------------------------------------------------------------
* Known Facts:
* 	- You take over upgrades from bot when you join midway through a game, 
* 		and you get initial at next mission (don't intend to fix - works well)
...
#define BOOL_TRACK_SPECIALAMMO		1		// Compile Special Ammo tracking?
...
#if !BOOL_TRACK_SPECIALAMMO
#undef BOOL_SHOW_SPECIALAMMO
#define BOOL_SHOW_SPECIALAMMO 0
#endif
...
#if BOOL_TRACK_SPECIALAMMO
new g_Upgrades_SpecialAmmo[MAXPLAYERS+1];						// Track special ammo bullets
#endif
#if BOOL_UPGRADE_KILLS
...
new IndexToUpgrade[MAX_VALID_UPGRADES];							// SDKcall index for L4D-built-in upgrades
new String:UpgradeShortInfo[MAX_VALID_UPGRADES][256];			// Short info for chattext
new String:UpgradeLongInfo[MAX_VALID_UPGRADES][1024];			// Long info for help text
new Handle:UpgradeAllowed[MAX_VALID_UPGRADES];					// For ConVar checking if upgrade is allowed
...
public OnPluginStart() {
	...
	UpgradeAllowed[0] = CreateConVar("surup_allow_kevlar_body_armor", "1", "Give the Kevlar Body Armor upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[1] = 8;
	UpgradeShortInfo[1] = "UPGRADE_1_SHORT";
	UpgradeLongInfo[1] = "UPGRADE_1_LONG"; 
	UpgradeAllowed[1] = CreateConVar("surup_allow_raincoat", "1", "Give the Raincoat upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[2] = 11;
	UpgradeShortInfo[2] = "UPGRADE_2_SHORT";
	UpgradeLongInfo[2] = "UPGRADE_2_LONG";
	UpgradeAllowed[2] = CreateConVar("surup_allow_climbing_chalk", "1", "Give the Climbing Chalk upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[3] = 12;
	UpgradeShortInfo[3] = "UPGRADE_3_SHORT";
	UpgradeLongInfo[3] = "UPGRADE_3_LONG";
	UpgradeAllowed[3] = CreateConVar("surup_allow_second_wind", "1", "Give the Second Wind upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[4] = 13;
	UpgradeShortInfo[4] = "UPGRADE_4_SHORT";
	UpgradeLongInfo[4] = "UPGRADE_4_LONG";
	UpgradeAllowed[4] = CreateConVar("surup_allow_goggles", "1", "Give the Goggles upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[5] = 16;
	UpgradeShortInfo[5] = "UPGRADE_5_SHORT";
	UpgradeLongInfo[5] = "UPGRADE_5_LONG";
	UpgradeAllowed[5] = CreateConVar("surup_allow_hot_meal", "0", "Give the Hot Meal upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[6] = 17;
	UpgradeShortInfo[6] = "UPGRADE_6_SHORT";
	UpgradeLongInfo[6] = "UPGRADE_6_LONG";
	UpgradeAllowed[6] = CreateConVar("surup_allow_laser_sight", "2", "Give the Laser Sight upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[7] = 19;
	UpgradeShortInfo[7] = "UPGRADE_7_SHORT";
	UpgradeLongInfo[7] = "UPGRADE_7_LONG";
	UpgradeAllowed[7] = CreateConVar("surup_allow_combat_sling", "1", "Give the Combat Sling upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[8] = 20;
	UpgradeShortInfo[8] = "UPGRADE_8_SHORT";
	UpgradeLongInfo[8] = "UPGRADE_8_LONG";
	UpgradeAllowed[8] = CreateConVar("surup_allow_large_clip", "1", "Give the Large Clip upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[9] = 21;
	UpgradeShortInfo[9] = "UPGRADE_9_SHORT";
	UpgradeLongInfo[9] = "UPGRADE_9_LONG";
	UpgradeAllowed[9] = CreateConVar("surup_allow_hollow_point_ammo", "1", "Give the Hollow Point Ammo upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[10] = 26;
	UpgradeShortInfo[10] = "UPGRADE_10_SHORT";
	UpgradeLongInfo[10] = "UPGRADE_10_LONG";
	UpgradeAllowed[10] = CreateConVar("surup_allow_knife", "1", "Give the Knife upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[11] = 27;
	UpgradeShortInfo[11] = "UPGRADE_11_SHORT";
	UpgradeLongInfo[11] = "UPGRADE_11_LONG";
	UpgradeAllowed[11] = CreateConVar("surup_allow_smelling_salts", "1", "Give the Smelling Salts upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[12] = 28;
	UpgradeShortInfo[12] = "UPGRADE_12_SHORT";
	UpgradeLongInfo[12] = "UPGRADE_12_LONG";
	UpgradeAllowed[12] = CreateConVar("surup_allow_ointment", "1", "Give the Ointment upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[13] = 29;
	UpgradeShortInfo[13] = "UPGRADE_13_SHORT";
	UpgradeLongInfo[13] = "UPGRADE_13_LONG";
	UpgradeAllowed[13] = CreateConVar("surup_allow_reloader", "1", "Give the Reloader upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	
	IndexToUpgrade[14] = 30;
	UpgradeShortInfo[14] = "UPGRADE_14_SHORT";
	UpgradeLongInfo[14] = "UPGRADE_14_LONG";
	UpgradeAllowed[14] = CreateConVar("surup_allow_incendiary_ammo", "1", "Give the Incendiary Ammo upgrade? (0=No,1=Yes,2=At Spawn)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	...
	#endif
	#if BOOL_UPGRADE_KILLS
	UpgradesKillCount = CreateConVar("surup_upgrades_kill_count", "100", "How many infected a survivor must kill before receiving upgrades? (0=Disabled)", CVAR_FLAGS, true, 0.0);
...
public ResetValues() {
	// Exactly the same as calling RemoveAllUpgrades(-1);
	// except this also resets b_BlockOnTankSpawn and b_BotControlled[i]
	b_CanAnnounce = false;
	#if BOOL_UPGRADE_TANK
	b_BlockOnTankSpawn = false;
	#endif
...
public Action:Event_PlayerBotReplace(Handle:event, const String:ename[], bool:dontBroadcast) {
	...
	if(GetClientTeam(botClient) != 2)	return Plugin_Continue;
	...
	#if BOOL_TRACK_SPECIALAMMO
	g_Upgrades_SpecialAmmo[botClient] = g_Upgrades_SpecialAmmo[playerClient];
	#endif
	#if BOOL_UPGRADE_KILLS
	g_Upgrades_TrackKills[botClient] = g_Upgrades_TrackKills[playerClient];
	#endif
	for(new i = 0; i < MAX_VALID_UPGRADES; i++)	{
...
public Action:Event_BotPlayerReplace(Handle:event, const String:ename[], bool:dontBroadcast) {
	...
	new botClient = GetClientOfUserId(GetEventInt(event, "bot"));
	// Don't do anything to infected
	if(GetClientTeam(playerClient) != 2)	return Plugin_Continue;
	b_Upgrades_GivenInit[playerClient] = b_Upgrades_GivenInit[botClient];
	b_Upgrades_Laser[playerClient] = b_Upgrades_Laser[botClient];
	#if BOOL_TRACK_SPECIALAMMO
	g_Upgrades_SpecialAmmo[playerClient] = g_Upgrades_SpecialAmmo[botClient];
	#endif
	#if BOOL_UPGRADE_KILLS
...
public Action:Event_HealSuccess(Handle:event, const String:ename[], bool:dontBroadcast) {
	...
	if (IsClientInGame(healerClient) && GetClientTeam(healerClient) == 2) {
		if(healerClient != healedClient)
...
#if BOOL_TRACK_SPECIALAMMO
// Unable to track type of special ammo unless only one type of ammo is used
public Action:Event_WeaponFire(Handle:event, const String:ename[], bool:dontBroadcast) {
	...
	if(GetClientTeam(shooterClient) != 2)	return Plugin_Continue;
	...
	// Check if UpgradeSpecialAmmoCount is not 0, then Check if has special ammo
	if(g_UpgradeSpecialAmmoCount && (b_Upgrades_Client[shooterClient][14] || b_Upgrades_Client[shooterClient][9])) {
		if(g_UpgradeSpecialAmmoMinusSG > 1 && StrContains(weaponName, "shotgun", false) != -1) {
			g_Upgrades_SpecialAmmo[shooterClient] -= g_UpgradeSpecialAmmoMinusSG;
			if(g_Upgrades_SpecialAmmo[shooterClient] < 0)
				g_Upgrades_SpecialAmmo[shooterClient] = 0;
		} else {
			g_Upgrades_SpecialAmmo[shooterClient]--;
		}
		#if BOOL_SHOW_SPECIALAMMO
		if(g_UpgradeSpecialAmmoShow == 1) {
			// Show SpecialAmmo left on CenterText
			PrintCenterText(shooterClient, "%t: %d", "LABEL_SPECIALAMMO", g_Upgrades_SpecialAmmo[shooterClient]);
		} else if(g_UpgradeSpecialAmmoShow == 2) {
			// Show SpecialAmmo left on HintText
			PrintHintText(shooterClient, "%t: %d", "LABEL_SPECIALAMMO", g_Upgrades_SpecialAmmo[shooterClient]);
		} else if(g_UpgradeSpecialAmmoShow == 3) {
			PrintToChat(shooterClient, "%s %t: %d", PLUGIN_TAG_C, "LABEL_SPECIALAMMO", g_Upgrades_SpecialAmmo[shooterClient]);
		}
		#endif
		// If SpecialAmmo is non-zero (has more than 0 ammo left)
		if(g_Upgrades_SpecialAmmo[shooterClient] <= 0) {
			RemoveSpecificUpgrade(shooterClient, 14);
			RemoveSpecificUpgrade(shooterClient, 9);
			g_Upgrades_SpecialAmmo[shooterClient] = 0;
			if(g_Verbosity > 2) {
				PrintToChatAll("%s %t", PLUGIN_TAG_C, "CHATALL_SPECIALAMMO_USED", shooterClient);
			} else if(g_Verbosity > 1) {
				PrintToChat(shooterClient, "%s %t", PLUGIN_TAG_C, "CHAT_SPECIALAMMO_USED");
			}
		}
	}
	return Plugin_Continue;
...
public Action:Event_InfectedDeath(Handle:event, String:ename[], bool:dontBroadcast) {
	...
	if (g_Upgrades_TrackKills[attackerClient] > 1 && ((g_Upgrades_TrackKills[attackerClient]%g_UpgradesKillCount) == 0)) {
		if(attackerClient && GetClientTeam(attackerClient) == 2) {
			...
			if(type == 1) {
				...
				g_Upgrades_SpecialAmmo[attackerClient] += (g_UpgradeSpecialAmmoCount*g_UpgradesKillAmount);
				#if BOOL_SHOW_SPECIALAMMO
				if(g_UpgradeSpecialAmmoShow == 1) {
					// Show SpecialAmmo left on CenterText
						PrintCenterText(attackerClient, "%t: %d", "LABEL_SPECIALAMMO", g_Upgrades_SpecialAmmo[attackerClient]);
				} else if(g_UpgradeSpecialAmmoShow == 2) {
					// Show SpecialAmmo left on HintText
					PrintHintText(attackerClient, "%t: %d", "LABEL_SPECIALAMMO", g_Upgrades_SpecialAmmo[attackerClient]);
				} else if(g_UpgradeSpecialAmmoShow == 3) {
					PrintToChat(attackerClient, "%s %t: %d", PLUGIN_TAG_C, "LABEL_SPECIALAMMO", g_Upgrades_SpecialAmmo[attackerClient]);
				}
				#endif
			#endif
			} else if(type == 3) {
...
public Action:Event_PlayerDeath(Handle:event, String:ename[], bool:dontBroadcast) {
	...
	if(GetConVarInt(ResetOnDeath) > 0) {
		if(g_UpgradeBots == 2 || (g_UpgradeBots < 2 && !IsFakeClient(deadClient))) {
			if(g_Verbosity > 2) {
...
public Action:Event_InfectedHurt(Handle:event, const String:ename[], bool:dontBroadcast) {
	...
	if (!b_Upgrades_Client[attackerClient][14] || GetClientTeam(attackerClient) != 2)
		return Plugin_Continue;
...
public Action:GiveInitialUpgrades(Handle:hTimer, any:client) {
	...
	if(b_InvalidGameMode || !client || !IsClientInGame(client) || GetClientTeam(client) != 2 || b_Upgrades_GivenInit[client]) return;
	#else
	if(!client || !IsClientInGame(client) || GetClientTeam(client) != 2 || b_Upgrades_GivenInit[client]) return;
	#endif
...
public GiveClientUpgrades(client, numUpgrades) {
	...
	for(new num = 0; num < numUpgrades; num++) {
		...
		if(numOwned == MAX_VALID_UPGRADES) {
			if(g_Verbosity > 1) {
		...
		new offset = GetRandomInt(0,MAX_VALID_UPGRADES-(numOwned+1));
		new val = 0;
...
public GiveClientSpecificUpgrade(client, upgrade) {
	...
	if(g_UpgradeBots == 0) {
		...
		// Allow lasers only
		if(IsFakeClient(client) && upgrade != 6)
			return;
	}
	if(g_Verbosity > 2) {
	...
	if(upgrade == 6) {
		...
		CheckLaser(INVALID_HANDLE);
	#if BOOL_TRACK_SPECIALAMMO
	} else if(upgrade == 14 || upgrade == 9) {
		// If its Incendiary, or HollowPoint, add available ammo
		g_Upgrades_SpecialAmmo[client] += g_UpgradeSpecialAmmoCount;
		#if BOOL_SHOW_SPECIALAMMO
		if(g_UpgradeSpecialAmmoShow == 1) {
			// Show SpecialAmmo left on CenterText
			PrintCenterText(client, "%t: %d", "LABEL_SPECIALAMMO", g_Upgrades_SpecialAmmo[client]);
		} else if(g_UpgradeSpecialAmmoShow == 2) {
			// Show SpecialAmmo left on HintText
			PrintHintText(client, "%t: %d", "LABEL_SPECIALAMMO", g_Upgrades_SpecialAmmo[client]);
		} else if(g_UpgradeSpecialAmmoShow == 3) {
			PrintToChat(client, "%s %t: %d", PLUGIN_TAG_C, "LABEL_SPECIALAMMO", g_Upgrades_SpecialAmmo[client]);
		}
		#endif
	}
	#else
...
public Action:ReGiveClientUpgrades(Handle:hTimer, any:client) {
	...
	#endif
		b_CanAnnounce = false;
		for(new upgrade = 0; upgrade < MAX_VALID_UPGRADES; upgrade++) {
			// Remove all first
			SDKCall(RemoveUpgrade, client, IndexToUpgrade[upgrade]);
...
public Action:listPlayerUpgrades(client, args) {
	if(GetCmdArgs() < 1) {
		ReplyToCommand(client, "Usage: listPlayerUpgrades <user id | name>");
		return Plugin_Handled;
	} else {
		new String:arg1[32];
		GetCmdArg(1, arg1, 32);
		...
		#if BOOL_TRACK_SPECIALAMMO
		PrintToConsole(client, ">>>| %t: %d", "LABEL_SPECIALAMMO", g_Upgrades_SpecialAmmo[client]);
		#endif
		ReplyToCommand(client, "%s: %t", PLUGIN_TAG, "CONSOLE_LISTED_TOTAL_UPGRADES", tmpTotal);
	}
	return Plugin_Handled;
...
public Action:addUpgrade(client, args) {
	...
	if(upgrade<0 || upgrade >= MAX_VALID_UPGRADES) {
		ReplyToCommand(client, "Invalid upgrade index.  Valid values are 1 to %d.", MAX_VALID_UPGRADES);
		return Plugin_Handled;
	}
...
public Action:UserHelp(client, args) {
	...
	if(ask == 1) {
		...
		#endif
		#endif
		// PrintToChat(client, "\x05| \x01Reloader: \x05Speed\x01:\x04%.2fs\x01, \x05Shotgun Speed\x01:\x04%.2fs\x01", GetConVarFloat(UpgradeReloaderSpeed), GetConVarFloat(UpgradeReloaderShotgunSpeed));
...
public Action:debug_AddUpgIndex(client, args) {
	if(GetCmdArgs() >= 1) {
		...
		ask = StringToInt(arg);
		new i = 0;
		for(i = 0; i < MAX_VALID_UPGRADES; i++) {

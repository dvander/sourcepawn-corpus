...
public OnPluginStart() {
	...
	csColor[0] = CreateConVar("cs_esp_tcolor",  "192 160 96 64", "Determines R G B A glow colors for T team.\nFormat: \"R G B A\" (with spaces)", 0);
	//csColor[1] = CreateConVar("cs_esp_ctcolorcolor", "96 128 192 64", "Determines R G B A glow colors for Counter-Terrorists team\nFormat should be \"R G B A\" (with spaces)", 0); // visible to CT team
	csColor[1] = CreateConVar("cs_esp_ctcolorcolor", "0 0 0 0", "Determines R G B A glow colors for CT team.\nFormat: \"R G B A\" (with spaces)", 0);
	...
	csColor[0].AddChangeHook(ConVarChange);
	csColor[1].AddChangeHook(ConVarChange);
...
public void retrieveColorValue(int index) {
	char pieces[4][16];
	char color[64];
	csColor[index].GetString(color, sizeof(color));

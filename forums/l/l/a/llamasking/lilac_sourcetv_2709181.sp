...
public void OnPluginStart()
{
	...
	cvar[CVAR_RATE] = CreateConVar("lilac_stv_tickrate", "1",
		...
		FCVAR_PROTECTED, true, 0.0, true, 1.0);
	cvar_stv_dir = CreateConVar("lilac_stv_dir", ".",
		"The directory to save demos to.");
...
void update_recording_list(int client, bool status)
{
	...
	if (players == 0 && stv_recording) {
		...
		// Get recording dir
		char stv_demo_dir[PLATFORM_MAX_PATH];
		cvar_stv_dir.GetString(stv_demo_dir, sizeof(stv_demo_dir));

		ServerCommand("tv_record \"%s/%s\"", stv_demo_dir, stv_demo_name);
		stv_recording = true;

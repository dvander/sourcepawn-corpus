...
new Float:LastSound;  // make anti flood global to prevent sound flooding on the server from different players
...
public OnClientAuthorized(client, const String:auth[]){
	if(!IsFakeClient(client)){
		if(client != 0){
			SndOn[client] = 1;
			SndCount[client] = 0;
			//LastSound[client] = 0.0; // MOD BY LAMDACORE removed row
...
public Action:Command_Play_Sound(Handle:timer,Handle:pack){
	...
	// if (LastSound[client] >= thethime)
	if (LastSound >= thetime && aid == INVALID_ADMIN_ID){ // MOD BY LAMDACORE ADDED: && aid == INVALID_ADMIN_ID ... REMOVED: [client]
...
// END MOD BY LAMDACORE
	...
	if ((SndCount[client] < GetConVarInt(cvarsoundlimit))){ // MOD BY LAMDACORE REMOVED: && (LastSound[client] < thetime)  //with earlier return Plugin_handled there is no reason for this
                // BEGIN MOD BY LAMDACORE

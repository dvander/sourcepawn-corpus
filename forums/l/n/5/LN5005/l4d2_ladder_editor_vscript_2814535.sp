...
public Plugin myinfo = {
    name = "L4D2 Ladder Editor(VScript)",
    author = "devilesk, LN5005 (ported Stripper format to VScript format)",
    version = "0.5.0",
...
public Action Command_Info(int client, int args)
{
    ...
    if (IsValidEntity(entity)) {
        ...
        if (StrEqual(classname, "func_simpleladder", false)) {
            ...
            PrintToConsole(client, "if ( Entities.FindByModel(null, \"%s\") == null)",modelname);
            PrintToConsole(client, "	{");
            PrintToConsole(client, "	}");
            PrintToConsole(client, "	else");
            PrintToConsole(client, "	{");
            PrintToConsole(client, "	local to_be_replaced_ladder = null");
            PrintToConsole(client, "	to_be_replaced_ladder = Entities.FindByModel(null, \"%s\"),",modelname);
            PrintToConsole(client, "	to_be_replaced_ladder.Kill()");
            PrintToConsole(client, "	}");
            PrintToConsole(client, "// If you are not replacing a vanilla ladder, don't copy and paste above code.");
            PrintToConsole(client, "");
            PrintToConsole(client, "if ( Entities.FindByName(null,\"ladder_000\") == null)");
            PrintToConsole(client, "	{");
            PrintToConsole(client, "		SpawnEntityFromTable(\"func_simpleladder\",");
            PrintToConsole(client, "		{");
            PrintToConsole(client, "			origin = Vector(%.2f,%.2f,%.2f),", origin[0], origin[1], origin[2]);
            PrintToConsole(client, "			angles = Vector(%.2f,%.2f,%.2f),", angles[0], angles[1], angles[2]);
            PrintToConsole(client, "			team = 2,");
            PrintToConsole(client, "			targetname = \"ladder_000\",");
            PrintToConsole(client, "			model = \"%s\",",modelname);
            PrintToConsole(client, "		});");
            PrintToConsole(client, "		EntFire(\"ladder_000\",\"addoutput\",\"normal.z %.2f\");",normal[2]);
            PrintToConsole(client, "		EntFire(\"ladder_000\",\"addoutput\",\"normal.y %.2f\");",normal[1]);
            PrintToConsole(client, "		EntFire(\"ladder_000\",\"addoutput\",\"normal.x %.2f\");",normal[0]);
            PrintToConsole(client, "	}");
            PrintToConsole(client, "else{}");
            PrintToConsole(client, "// DO NOT forget to rename the targetname(ladder_000) of the ladder you are adding.");
            PrintToConsole(client, "");
        }
        else {

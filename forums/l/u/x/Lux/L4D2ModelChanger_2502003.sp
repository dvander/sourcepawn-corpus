...
Thanks MasterMind420 improved thirdpersonshould THIRDPERSON FIX Marker for his work
1.1.1
...
1.3
Thanks NgBUCKWANGS for the new way of hiding base models 
Fixed Special infected models not hiding (Double model bug)

Fixed mistake thanks figa for pointing out

Thanks mastermind420 for cookie saving example
You last model used will be saved by the server when you next join
If they are using same cookie name and model changer.

Added Ragdoll death for custommodel on special infected

Config named changed to L4D2ModelChanger1.3
//Credit timocop for helpy :D

	Check out http://downloadtzz.spdns.de
	There are a few cool programs and a sourcepawn editor called basicpawn
...
public OnPluginStart()
{
	...
	hCvar_TPcheck = CreateConVar("lmc_tpcheck", "1", "Enable ThirdPersonShoulder checks, this is not perfect so there can be bugs its only basic support", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	hCvar_TPcheckFrequency = CreateConVar("lmc_tpcheckfrequency", "0.25", "Frequency of checks for ThirdPersonShoulder cvar on clients, value will change on next map", FCVAR_NOTIFY, true, 0.1, true, 10.0);
	...
	hCvar_HideBotsModel = CreateConVar("lmc_spec_hide_bots", "0", "When spectating bots in firstperson hide custom model? (0 = disable will save some cpu power)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	
	hCookie_LmcCookie = RegClientCookie("lmc_cookie", "1", CookieAccess_Protected);
...
public eRoundEnd(Handle:hEvent, const String:sEventName[], bool:bDontBroadcast)//changed 1.3
{
	if(hTimer_TPcheck != INVALID_HANDLE)
	{
		KillTimer(hTimer_TPcheck);
...
static BeWitched(iClient, const String:sModel[], const bool:bBaseReattach)  
{
	...
	SetEntProp(iClient, Prop_Send, "m_nMinGPULevel", 3);
	SetEntProp(iClient, Prop_Send, "m_nMaxGPULevel", 5);
...
public ePlayerDeath(Handle:hEvent, const String:sEventName[], bool:bDontBroadcast)
{
	...
	{
		DeathFix[iVictim] = true; /*THIRDPERSON FIX*/
		...
		AcceptEntityInput(iEntity, "ClearParent");
		SetEntProp(iEnt, Prop_Send, "m_nMinGPULevel", 3);
		SetEntProp(iEnt, Prop_Send, "m_nMaxGPULevel", 5);
...
static ModelIndex(iClient, iCaseNum, bool:bUsingMenu=false)
{
	...
	if(g_bAdminOnly && !IsFakeClient(iClient))
		return;
	//1.4
	if(!IsFakeClient(iClient))
	{
...
{  
	...
	SetEntProp(iEntity, Prop_Send, "m_nMinGPULevel", 3);
	SetEntProp(iEntity, Prop_Send, "m_nMaxGPULevel", 5);

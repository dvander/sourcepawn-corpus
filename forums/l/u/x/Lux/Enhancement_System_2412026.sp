...
static Handle:hCvar_BloodIndexing = INVALID_HANDLE;
static Handle:hCvar_BloodPool = INVALID_HANDLE;
static Handle:hCvar_PoolRefresh = INVALID_HANDLE;
static Handle:hCvar_PoolBoomer = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	HookConVarChange(hCvar_BloodIndexing, eConvarChanged);
	HookConVarChange(hCvar_BloodPool, eConvarChanged);
	HookConVarChange(hCvar_PoolRefresh, eConvarChanged);
	HookConVarChange(hCvar_PoolBoomer, eConvarChanged);
...
CvarsChanged()
{
	...
	g_bBloodIndexing = GetConVarInt(hCvar_BloodIndexing) > 0;
	g_iBloodPool = GetConVarInt(hCvar_BloodPool);
	g_fPoolRefresh = GetConVarFloat(hCvar_PoolRefresh);
	g_bPoolBoomer = GetConVarFloat(hCvar_PoolBoomer) > 0;
...
static InfectedBlood(iVictim, bool:bDeath, bool:bExpDeath, bool:bHeadShot)
{
...
static SurvivorBlood(iVictim, Float:fBodyPos)
{
...
static InfectedSpecialBlood(iVictim, bool:bHeadShot, Float:fHeadPos, Float:fBodyPos)
{
	if(!bHeadShot)
	{
		switch(GetRandomInt(1, 20))
...
static CommonOrWitchBlood(iVictim, const String:sClassname[] ,bool:bDeath, bool:bExpDeath, bool:bHeadShot)
{
...
static BloodOnFloorPrep(iVictim, bool:bBoomerPatch)//Better client perf with indexing system for blood pools
{
	if(g_iBloodPool < 1)
		return;
	...
	if(g_bBloodIndexing)
	{
		...
		for(new i = GetArraySize(hArray) - 1; i > -1; i--)// thanks to timocop showing me that is exist and helping me set it up :3
		{
			static Float:fTime;
		...
		new iIndex = PushArrayCell(hArray, 0); //Just to get the new created index
...
static bool:Decal(iClient, String:sAmount[]="200", bool:GreenBlood=false, iMoreBlood)//Timo's Stock for decal spray
{
	if(!g_bDecals)
		return false;
...
static ParticleSpawn(const String:sParticle[], iVictim, Float:fTimeMin=0.4, Float:fTimeMax= 0.7, bool:LongBlood=false, Float:ExtraXfPos)//My Stock Just for this so enjoy if you use it :3
{

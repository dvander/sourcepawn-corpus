...
#define PLUGIN_VERSION "0.4.2-lm"
#define DBVERSION 3
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_stats_delete", Command_DeletePlayer, ADMFLAG_SLAY);
...
createdbplayer()
{
	...
	len += Format(query[len], sizeof(query)-len, " `hostage_rescued` int(11) NOT NULL , `KW_awp` int(11) NOT NULL , `KW_sg550` int(11) NOT NULL , `KW_g3sg1` int(11) NOT NULL , PRIMARY KEY (`NAME`));");
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
	pointvalue = GetConVarInt(CV_g3sg1)
	Format(query, sizeof(query), "UPDATE Player SET POINTS = POINTS + %i, KILLS = KILLS + 1, KW_g3sg1 = KW_g3sg1 + 1 WHERE steamId = '%s'",pointvalue ,steamIdattacker);
	SQL_TQuery(db,SQLErrorCheckCallback, query)
	}
	if(headshot)
	{
...
public Action:Command_DeletePlayer(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_stats_delete \"steam_id\"");
		return Plugin_Handled;
	}

	// Get the command
	decl String:steam_id[40];
	GetCmdArg(1, steam_id, 40);

	new String:query[512];
	Format(query, sizeof(query), "DELETE FROM Player WHERE STEAMID = '%s'", steam_id);
	SQL_TQuery(db,SQLErrorCheckCallback, query)

	ReplyToCommand(client, "[SM] Player '%s' deleted.", steam_id);
	return Plugin_Handled;
}

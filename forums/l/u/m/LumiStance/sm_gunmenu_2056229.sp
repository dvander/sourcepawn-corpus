...
Description:
	...
	Servers using this mod: www.gametracker.com/search/css/?search_by=server_variable&search_by2=sm_gunmenu_version&searchipp=50
...
Changelog:
	1.1 <-> 2013 - 11/05 LumiStance
		Implemented sm_gunmenu_enable cvar
...
// Plugin definitions
#define PLUGIN_VERSION "1.1a-lm"
#define PLUGIN_ANNOUNCE "\x04[Gun Menu Lite]\x01 v1.1a-lm by LumiStance"
...
public OnPluginStart()
{
	...
	g_ConVar_Version = CreateConVar("sm_gunmenu_version", PLUGIN_VERSION, "[SM] Gun Menu Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	g_ConVar_Enable = CreateConVar("sm_gunmenu_enable", "1", "Enable/Disable Gun Menu.", FCVAR_PLUGIN|FCVAR_SPONLY);
	...
	// Event Hooks
	HookConVarChange(g_ConVar_Enable, Event_CvarChange);
...
{
	g_Enable = GetConVarBool(g_ConVar_Enable);
}
...
public MenuHandler_ChoosePrimary(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		if (g_Enable && Teams:GetClientTeam(client_index) > CS_TEAM_SPECTATOR)
...
{
	if (g_Enable && IsClientInGame(client_index) && Teams:GetClientTeam(client_index) > CS_TEAM_SPECTATOR)
	{
		if (g_PrimaryMenu != INVALID_HANDLE)
			DisplayMenu(g_PrimaryMenu, client_index, MENU_TIME_FOREVER);
		else if (g_SecondaryMenu != INVALID_HANDLE)

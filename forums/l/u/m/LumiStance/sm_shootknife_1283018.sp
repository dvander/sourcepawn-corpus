...
Description:
	...
	Suicide is accentuated with a lightning bolt explosion and thunder to get the point across.
	Player will then see Centered Hint text stating "Dont' shoot knife!".

	The victim will not receive any knife damage if the attacker had shot them.
	Grenades attacks are ignored.  Players with no gun are not smited.

	On Gun Game, this prevents players from getting easy upgrades when knifepro is enabled.
	In this case, the attacker will level down.

	Servers using this mod: http://www.game-monitor.com/search.php?vars=sm_shootknife_version
...
// Plugin definitions
#define PLUGIN_VERSION "0.4-lm"
#define PLUGIN_ANNOUNCE "\x04[Don't Shoot Knife]\x01 v0.4-lm by LumiStance"
...
//	PrintToServer("\x04[lm]\x01 Attacker: %i  Victim: %i  Inflictor: %i  Damage: %0.0f  Type: %i", attacker, victim, inflictor, damage, damagetype);
	...
	if ( (victim>=1) && (victim<=MaxClients) && (attacker>=1) && (attacker<=MaxClients) && (attacker==inflictor) )
	{
		...
		{
//			PrintToChat(attacker, "\x04[lm]\x01 You're tagged %i %s", inflictor, WeaponName);
			g_Shot[attacker][victim] = true;
		}
		else if (g_Shot[attacker][victim])
		{
			// Only slay if player has guns
			if (GetPlayerWeaponSlot(attacker, _:Slot_Primary)>0 || GetPlayerWeaponSlot(attacker, _:Slot_Secondary)>0)
			{
				CreateTimer(0.1, Event_Slay, attacker);
				return Plugin_Handled;	// Cancel Attack
			}
		}
...
public Action:Event_Slay(Handle:timer, any:client_index)
{
	...
	Smite(client_index);
	PrintCenterText(client_index, "Don't shoot knife!");
	PrintToChatAll("\x04[lm]\x01 %s was smited for shoot knifing!", ClientName);
}

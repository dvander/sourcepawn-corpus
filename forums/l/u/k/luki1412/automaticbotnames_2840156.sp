...
Handle bot_timers[MAXPLAYERS + 1] = {null, ...};

// various convars
ConVar cvarEnabled; // are we enabled?
// (note - name list still reloaded on map load even when disabled)

// name list source settings
ConVar cvarDatabaseName; // name from databases.cfg, or "" to use file
ConVar cvarDatabaseTable; // what table to use
ConVar cvarDatabaseColumn; // what column to use
ConVar cvarDatabaseOrderBy; // what column to order by, or ""
ConVar cvarDatabaseDescending; // if ordering, order descending instead?
ConVar cvarDatabaseLimit; // maximum records to return
ConVar cvarDatabaseUseUTF8; // should we use UTF8 on mysql?

// general settings
ConVar cvarPrefix; // bot name prefix
ConVar cvarRandom; // use random-order names?
ConVar cvarAnnounce; // announce new bots?
ConVar cvarSuppress; // supress join/team/namechange messages?
ConVar cvarRenameDelay; // delay bot renaming
...
public void OnPluginStart()
{
	...
	// cvars!
	ConVar cvarVersion = CreateConVar("sm_botnames_version", PLUGIN_VERSION, PLUGIN_DESCRIPTION, FCVAR_NOTIFY|FCVAR_DONTRECORD);
	cvarEnabled = CreateConVar("sm_botnames_enabled", "1", "sets whether bot naming is enabled");

	cvarDatabaseName = CreateConVar("sm_botnames_db_name", "", "a database name to load bot names from, or \"\" to load from file");
	...
	cvarRenameDelay = CreateConVar("sm_botnames_delay", "9.0", "sets delay for renaming bots", _, true, 1.0, false, 600.0);

	// register our commands
	RegServerCmd("sm_botnames_reload", Command_Reload, "reloads bot names");
	...
	if (SayText2 == INVALID_MESSAGE_ID)
	{
		...
		PrintToServer("[AutomaticBotNames] This game doesn't support SayText2. Won't suppress chat messages.");
	}
	else
	{
...
void ReloadNamesFromFile()
{
	...
	PrintToServer("[AutomaticBotNames] Opening file %s.", path);
	...
	while (IsEndOfFile(file) == false)
	{
		...
		int commentstart = StrContains(newname, "//");
...
void ReloadNamesFromDatabase()
{
	...
	// error storage
	char error[256];
	PrintToServer("[AutomaticBotNames] Connecting to the database %s.", dbname);
	Handle db = SQL_Connect(dbname, true, error, sizeof(error));

	if (db == null)
	{
		LogError("Error: could not connect to database \"%s\"", dbname);
		LogError("Error Details: %s", error);
	...
	if (GetConVarBool(cvarDatabaseUseUTF8))
	{
		...
		{
			LogMessage("Warning: attempted to force UTF8 on non-mysql connection");
...
void DoBotName(int client, int primary)
{
	...
	{
		PrintToServer("[AutomaticBotNames] Bot renamed to %s.", name);
	}
}
...
public Action Event_PlayerTeam(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (bot_timers[client] == null)
	{
		bot_timers[client] = CreateTimer(GetConVarFloat(cvarRenameDelay), Timer_BotSetup, client, TIMER_FLAG_NO_MAPCHANGE);
	}
...
{
	return (client && IsClientConnected(client) && IsFakeClient(client) && !IsClientReplay(client) && !IsClientSourceTV(client));
}

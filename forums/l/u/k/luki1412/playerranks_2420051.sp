...
#define PLUGIN_VERSION 				"3.4.2" 						   	// Plugin version, this value is fed into myinfo and sm_rankingversion CVAR.
#define PLUGIN_PREFIX_PLAIN 		"[Player Ranks 3.4.2]"		   		// Our console friendly prefix.
#define PLUGIN_PREFIX 				"\x04[Player Ranks]\x05" 	// Our chat friendly prefix.
...
public OnPluginStart() {
	...
	// Create our version CVAR. 
	// FCVAR_DONTRECORD flag blocks the cvar from being in the config
	pluginVersion =  CreateConVar("pr_rankingversion", 	PLUGIN_VERSION, 		"Current version of the plugin. Read Only", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD); 
	...
	autoUpdate =					CreateConVar("pr_autoupdate",				"0" ,			"If set to 1, this plugin will check for and update itself.",													FCVAR_NONE, true, 0.0, true, 1.0);
	...
	allowBotStats =  				CreateConVar("pr_allowBotStats",			"0" , 			"If 1: Bots are counted in kills, deaths, assists, backstabs, headshots, and feigns.", 							FCVAR_NONE, true, 0.0, true, 1.0);
	...
	updateInterval =  				CreateConVar("pr_updateinterval",			"31", 			"Amount of seconds before the top player listing updates and all clients have their scores autosaved.", 		FCVAR_NONE, true, 1.0);
	pluginThreshold =  				CreateConVar("pr_threshold",				"50", 			"Points required before the user is actually put in the ranking system, keeps the database nice and tidy.",		FCVAR_NONE, true, 0.0);
	playerMinimum =  				CreateConVar("pr_playerminimum",			"4" , 			"Amount of players required to be connected to the server for this plugin to track scoring.",					FCVAR_NONE, true, 0.0);
	...
	rewardAdminKill = 			CreateConVar("pr_rewardadmin",			"1.00",			"Point reward for killing an admin.");
	rewardPlayerKill =  		CreateConVar("pr_rewardplayer", 		"1.00", 		"Point reward for killing a player.");
	rewardBotKill =				CreateConVar("pr_rewardbot", 			"0.50", 		"Point reward for killing a bot.");
	rewardAssistMult = 			CreateConVar("pr_rewardassist",			"0.50",			"Point reward multiplier for assisting a bot/player kill. At 0.75, this would give 75% of the reward to the assister.");
	
	// Variables that apply to TF2 only.	
	if (GetEngineVersion() == Engine_TF2) {
		isTF2 = true;
		rewardRoundBonus =  		CreateConVar("pr_rewardroundbonus",			"1.00", 		"Point reward for being in the game for an entire round.");
		rewardRBMin		 =			CreateConVar("pr_rewardrbmin",				"10.00",			"Point reward requirement for player to receive round bonus.");
		rewardSentry =  			CreateConVar("pr_rewardsentry", 			"1.66", 		"Point reward for destroying a sentry.");
		...
		rewardDeflectMult = 		CreateConVar("pr_rewarddeflect",			"1.66",			"Point reward multiplier for killing a bot/player via a deflected projectile.");
		...
		// BOSS CVARs
		rewardMerasmusKill = 		CreateConVar("pr_rewardmerasmuskill",		"1.0", 			"Base point amount for killing Merasmus.");
		rewardMonoculusKill =		CreateConVar("pr_rewardmonoculuskill",		"1.0", 			"Base point amount for killing Monoculus.");
		rewardHHHKill =    			CreateConVar("pr_rewardhhhkill",			"1.0", 			"Base point amount for killing Horseless Headless Horsemann.");
...
public Action:Command_Reset(client, args) {
	if (isReady == false) {
		if (client != WORLD) {
			PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_notready", client);
		}
		else {
...
public Action:Command_Exempt(client, args) {
	...
	if (scores[client][exempt] == true)
		PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_exempt_enable", client);
	else
		PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_exempt_disable", client);
...
public Action:Command_ForceSave(client, args) {
	ForceSave();
	ReplyToCommand(client, "%s %T", PLUGIN_PREFIX, "pr_commandsave", client);
}
...
public Action:Command_Reward(client, args) {
	if (args < 2) {
		ReplyToCommand(client, "%s Usage: pr_reward <#userid|name> <amount>.", client);
		return Plugin_Handled;
	}
	...
	if (pointAmount <= 0.0) {
		ReplyToCommand(client, "%s %T", PLUGIN_PREFIX, "pr_commandpointamountinvalid", client);
		return Plugin_Handled;
	}
	...
	for (new targetIndex = 0; targetIndex < targetCount; targetIndex++) {
		...
		if (client != WORLD) {
			PrintToChat(clientID, "%s %T", PLUGIN_PREFIX, "pr_commandrewardreceive", clientID, issuerName, pointAmount);
		}
		else
			PrintToChat(clientID, "%s %T", PLUGIN_PREFIX, "pr_commandrewardreceiveanonymous", clientID, pointAmount);
	}
...
public Action:Command_Revoke(client, args) {
	if (args < 2) {
		ReplyToCommand(client, "%s Usage: pr_revoke <#userid|name> <amount>.", PLUGIN_PREFIX);
		return Plugin_Handled;
	}
	new String:searchPattern[MAX_NAME_LENGTH];
	GetCmdArg(1, searchPattern, MAX_NAME_LENGTH);
	...
	for (new targetIndex = 0; targetIndex < targetCount; targetIndex++) {
		...
		if (client != WORLD)
			PrintToChat(clientID, "%s %T", PLUGIN_PREFIX, "pr_commandrevokereceive", clientID, issuerName, pointAmount);
		else
			PrintToChat(clientID, "%s %T", PLUGIN_PREFIX, "pr_commandrevokereceiveanonymous", clientID, pointAmount);
	}
...
public Action:Command_Say(client, const String:command[], argc) {
	...
	if (StrEqual(text[startPos], "rank", false) == true || StrEqual(text[startPos], "ranks", false) == true) {
		...
		{
			ReplyToCommand(client, "%s %T", PLUGIN_PREFIX, "pr_rankmenu_disabled", client);
			return Plugin_Handled;
		}
		...
		if (StrEqual(section, "rank", false) == true) {
			...
			if (maxResults == 0) {
				PrintToChat(client, "%s %T", "pr_search_disabled", client);
				return Plugin_Handled;
			}
...
public ShowSearchMenu(client, Handle:data) {
	...
	SetPackPosition(data, GetPackPosition(data) - DataPackPos:8);
	
	new resultCount = ReadPackCell(data);
	
	if (resultCount == GetConVarInt(maxSearchResults)) {
		PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_search_refine", client, GetConVarInt(maxSearchResults));
	}
...
public ShowRankMe(client) {
	...
	if (scores[client][rank] == 0) {
		if (scores[client][points] < GetConVarFloat(pluginThreshold)) {
			PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_rankme_belowthreshold", client, scores[client][points], GetConVarFloat(pluginThreshold));
		}
		else {
			PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_rankme_updating", client, scores[client][points], GetConVarInt(updateInterval));		
		}
	}
	else {
		PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_rankme_normal", client, scores[client][rank], totalCount, scores[client][points]);
	}
	...
	if (scores[client][kills] > 0.00 && scores[client][deaths] > 0.00)
	{
		...
		PrintToChat(client, "%s %T %T", PLUGIN_PREFIX, "pr_stats_1", client, scores[client][kills], scores[client][deaths], scores[client][assists], "pr_stats_2", client, kdRatio);
	}
	else
		PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_1", client, scores[client][kills], scores[client][deaths], scores[client][assists]);
		
	if (isTF2)
	{
		PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_3", client, scores[client][headshots], scores[client][backstabs], scores[client][feigns], scores[client][deflects], scores[client][highestStreak]);	
		
		if (scores[client][merKills] > 0)
			PrintToChat(client, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", client, scores[client][merKills], "Merasmus", "pr_stats_5", client, scores[client][merLvl]);
		
		if (scores[client][monKills] > 0)
			PrintToChat(client, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", client, scores[client][monKills], "Monoculus", "pr_stats_5", client, scores[client][monLvl]);
		
		if (scores[client][hhhKills] > 0)
			PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_4", client, scores[client][hhhKills], "Headless Horsemann");
	}
	
	PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_6", client, GetTimePlayed(client) / 3600);	
	PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_7", client, scores[client][flagCaptures], scores[client][flagDefends], scores[client][capCaptures], scores[client][capDefends], scores[client][roundsPlayed]);
	PrintToChat(client, "%s %T", PLUGIN_PREFIX, "pr_stats_8", client, scores[client][dominationsGood], scores[client][dominationsBad]);
}
			...
			if (IsClientInGame(clientId)) {
				...
				if (IsFakeClient(clientId) == false) {
					if (scores[clientId][rank] == 0)
						PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_rank_unranked", param1, clientName, scores[clientId][roundPoints], scores[clientId][points]);
					else {
						PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_rank_normal", param1, clientName, scores[clientId][rank], scores[clientId][roundPoints], scores[clientId][points]);
					}
				}
				else {
					PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_rank_bot", param1, clientName, scores[clientId][roundPoints]);
				}
				
				if (scores[clientId][kills] > 0.00 && scores[clientId][deaths] > 0.00)
					PrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_1", param1, scores[clientId][kills], scores[clientId][deaths], scores[clientId][assists], "pr_stats_2", param1, FloatDiv(float(scores[clientId][kills]), float(scores[clientId][deaths])));
				else
					PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_1", param1, scores[clientId][kills], scores[clientId][deaths], scores[clientId][assists]);
					
				if (isTF2) {
					PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_3", param1, scores[clientId][headshots], scores[clientId][backstabs], scores[clientId][feigns], scores[clientId][deflects], scores[clientId][highestStreak]);
					
					if (scores[clientId][merKills] > 0)
						PrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", param1, scores[clientId][merKills], "Merasmus", "pr_stats_5", param1, scores[clientId][merLvl]);
					
					if (scores[clientId][monKills] > 0)
						PrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", param1, scores[clientId][monKills], "Monoculus", "pr_stats_5", param1, scores[clientId][monLvl]);
					
					if (scores[clientId][hhhKills] > 0)
						PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_4", param1, scores[clientId][hhhKills], "Headless Horsemann");
					
					if (IsFakeClient(clientId) == false)
						PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_6", param1, GetTimePlayed(clientId) / 3600);
					
					PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_7", param1, scores[clientId][flagCaptures], scores[clientId][flagDefends], scores[clientId][capCaptures], scores[clientId][capDefends], scores[clientId][roundsPlayed]);
					PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_8", param1, scores[clientId][dominationsGood], scores[clientId][dominationsBad]);
					
				}
			}
			else {
				PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_invalid_selection", param1);
			}
		}
	}
	...
	if (action == MenuAction_Select)
	{
		...
		if (topScores[selectedIndex][tSeen] != 0) {
			...
			if (onlineIndex == 0) {
				...
				PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_top_rank_offline", param1, topScores[selectedIndex][tNickname], selectedIndex + 1, totalCount, topScores[selectedIndex][tPoints], seenDate);
				//return;
			}
			else {
				PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_top_rank_online",  param1, topScores[selectedIndex][tNickname], selectedIndex + 1, totalCount, topScores[selectedIndex][tPoints]);
			}
			
			if (topScores[selectedIndex][tKills] > 0.00 && topScores[selectedIndex][tDeaths] > 0.00) {
				PrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_1", param1, topScores[selectedIndex][tKills], topScores[selectedIndex][tDeaths], topScores[selectedIndex][tAssists], "pr_stats_2", param1, FloatDiv(float(topScores[selectedIndex][tKills]), float(topScores[selectedIndex][tDeaths])));
			}
			else {
				PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_1", param1, topScores[selectedIndex][tKills], topScores[selectedIndex][tDeaths], topScores[selectedIndex][tAssists]);
			}
			
			if (isTF2 == true) {
				PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_3", param1, topScores[selectedIndex][tHeadshots], topScores[selectedIndex][tBackstabs], topScores[selectedIndex][tFeigns], topScores[selectedIndex][tDeflects], topScores[selectedIndex][tHighestStreak]);
				
				if (topScores[selectedIndex][tmerKills] > 0)
					PrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", param1, topScores[selectedIndex][tmerKills], "Merasmus", "pr_stats_5", param1, topScores[selectedIndex][tmerLvl]);
					
				if (topScores[selectedIndex][tmonKills] > 0)
					PrintToChat(param1, "%s %T %T", PLUGIN_PREFIX, "pr_stats_4", param1, topScores[selectedIndex][tmonKills], "Monoculus", "pr_stats_5", param1, topScores[selectedIndex][tmonLvl]);
					
				if (topScores[selectedIndex][thhhKills] > 0)
					PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_4", param1, topScores[selectedIndex][thhhKills], "Headless Horsemann");
				
				if (onlineIndex != WORLD && IsClientInGame(onlineIndex) && scores[onlineIndex][loaded] == true) {
					PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_6", param1, GetTimePlayed(onlineIndex) / 3600);
				}
				else {
					PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_6", param1, topScores[selectedIndex][ttotaltime] / 3600);
				}
				
				PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_7", param1, topScores[selectedIndex][tflagCaptures], topScores[selectedIndex][tflagDefends], topScores[selectedIndex][tcapCaptures], topScores[selectedIndex][tcapDefends], topScores[selectedIndex][troundsPlayed]);
				PrintToChat(param1, "%s %T", PLUGIN_PREFIX, "pr_stats_8", param1, topScores[selectedIndex][tdominationsGood], topScores[selectedIndex][tdominationsBad]);
			}
		}
	}
	else if (action == MenuAction_End) {
...
public OnClientConnected(client) {
	...
	if (playerCount == GetConVarInt(playerMinimum)) {
		PrintToChatAll("%s %t", PLUGIN_PREFIX, "pr_enabled_true");
	}
}
...
public OnClientDisconnect(client) {
	...
	// We're just one player too short of ranking being enabled.
	if (playerCount == GetConVarInt(playerMinimum)) {
		PrintToChatAll("%s %t", PLUGIN_PREFIX, "pr_enabled_false");
	}
		...
		if (scores[clientId][roundBonus] == false) {				
			...
			if (scores[clientId][exempt] == false && playerCount >= GetConVarInt(playerMinimum) && GetConVarFloat(rewardRoundBonus) > 0.0) {
				if (GetConVarFloat(rewardRBMin) > 0.0)
					PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundbonus_notifyminimum", clientId, GetConVarFloat(rewardRBMin));
				else	
					PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundbonus_notify", clientId);
			}
		}
...
public Action:OnRoundSetupEnded(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	for (new clientId = 1; clientId <= MaxClients; clientId++) {
		...
		if (scores[clientId][roundBonus] == false) {
			...
			if (scores[clientId][exempt] == false && playerCount >= GetConVarInt(playerMinimum) && GetConVarFloat(rewardRoundBonus) > 0.0) {
				if (GetConVarFloat(rewardRBMin) > 0.0)
					PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundbonus_latenotifyminimum", clientId, GetConVarFloat(rewardRBMin));
				else	
					PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundbonus_latenotify", clientId);
			}
		}
	}
	...
	for(new clientId = 1; clientId <= MaxClients; clientId++) {
		...
		if (scores[clientId][roundBonus] == true) {
			...
			if (roundBonusReward > 0.0 && scores[clientId][roundPoints] >= GetConVarFloat(rewardRBMin)) {				
				...
				if (playerCount >= GetConVarInt(playerMinimum))
					PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundbonus_received", clientId, roundBonusReward);
			}
		}
		
		if (scores[clientId][roundPoints] > 0.0)
			PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundend_roundpoints", clientId, scores[clientId][roundPoints]);
		
		// The user isn't new to this ranking system.
		if (scores[clientId][rank] != 0) {
			PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_roundend_totalpoints", clientId, scores[clientId][points]);
		}
...
public Action:OnPlayerTeleported(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (playerTeleReward > 0.0) {
		if (GetConVarInt(userNotificationLvl) >= NOTIFICATION_TELEPORTS) {
			PrintToChat(builderId, "%s %T", PLUGIN_PREFIX, "pr_notify_teleport", builderId, playerTeleReward);
		}
...
public Action:OnObjectDestroyed(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (GetConVarInt(userNotificationLvl) >= NOTIFICATION_KILLS) {
		if (buildingType == _:TFObject_Dispenser) {
			PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_dispenser", attackerId, pointReward, buildingLevel);
		}
		else if (buildingType == _:TFObject_Teleporter)
			PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_teleporter", attackerId, pointReward, buildingLevel);
		else if (buildingType == _:TFObject_Sentry) {
			if (IsMini)
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_minisentry", attackerId, pointReward);
			else
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_sentry", attackerId, pointReward, buildingLevel);
		}
	}
...
public Action:OnPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast) {	
	...
	if (IsFakeClient(attackerId) == false && GetConVarInt(userNotificationLvl) >= NOTIFICATION_KILLS && pointReward > 0.0) {
		if (customKill == TF_CUSTOM_HEADSHOT || customKill == TF_CUSTOM_HEADSHOT_DECAPITATION) {
			if (victimIsBot)
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_bot_headshot", attackerId, pointReward);
			else if (victimIsAdmin)
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_admin_headshot", attackerId, pointReward);
			else
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_player_headshot", attackerId, pointReward);
		}
		else if (customKill == TF_CUSTOM_BACKSTAB) {
			if (victimIsBot)
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_bot_backstab", attackerId, pointReward);
			else if (victimIsAdmin)
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_admin_backstab", attackerId, pointReward);
			else
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_player_backstab", attackerId, pointReward);
		}
		else if (deflection == true) {
			if (victimIsBot)
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_bot_deflect", attackerId, pointReward);
			else if (victimIsAdmin)
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_admin_deflect", attackerId, pointReward);
			else
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_player_deflect", attackerId, pointReward);
		}
		else {
			if (victimIsBot)
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_bot", attackerId, pointReward);
			else if (victimIsAdmin)
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_admin", attackerId, pointReward);
			else
				PrintToChat(attackerId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_player", attackerId, pointReward);
		}
	}
	...
	if (assisterId != WORLD) {
		...
		if (IsFakeClient(assisterId) == false && GetConVarInt(userNotificationLvl) >= NOTIFICATION_KILLS && pointRewardAssist > 0.0) {
			if (victimIsBot)
				PrintToChat(assisterId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_assist_bot", assisterId, pointRewardAssist);
			else if (victimIsAdmin)
				PrintToChat(assisterId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_assist_admin", assisterId, pointRewardAssist);
			else
				PrintToChat(assisterId, "%s %T", PLUGIN_PREFIX, "pr_notify_kill_assist_player", assisterId, pointRewardAssist);
		}
	}
	...
	for (new I = 0; I < capperLength; I++)
	{
		...
		if (capCaptureReward > 0.0) {
			if (GetConVarInt(userNotificationLvl) >= NOTIFICATION_OBJECTIVES) {
				PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_notify_capture_point", clientId, capCaptureReward);
			}
...
public Action:OnPointDefended(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (capDefenseReward > 0.0) {
		if (GetConVarInt(userNotificationLvl) >= NOTIFICATION_OBJECTIVES) {
			PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_notify_defend_point", clientId, capDefenseReward);
		}
...
public Action:OnFlagState(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (GetEventInt(event, "eventtype") == TF_FLAGEVENT_CAPTURED) {
		...
		if (flagCaptureReward > 0.0) {
			if (GetConVarInt(userNotificationLvl) >= NOTIFICATION_OBJECTIVES) {
				PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_notify_capture_flag", clientId, flagCaptureReward);
			}
		...
		if (flagDefendReward > 0.0) {
			if (GetConVarInt(userNotificationLvl) >= NOTIFICATION_OBJECTIVES) {
				PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_notify_defend_flag", clientId, flagDefendReward);
			}
...
public Action:OnHeadlessHorsemannSpawn(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (IsValidEntity(bossIndex)) {
		...
		PrintToChatAll("%s %t", PLUGIN_PREFIX, "pr_boss_spawned", "Horseless Headless Horsemann", reward);
	}
	return Plugin_Continue;
}
...
public Action:OnMerasmusSpawn(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (IsValidEntity(bossIndex)) {
		...
		PrintToChatAll("%s %t", PLUGIN_PREFIX, "pr_boss_spawned", "Merasmus", calculatedReward);
	}
...
public Action:OnMonoculusSpawn(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (IsValidEntity(bossIndex)) {
		...
		PrintToChatAll("%s %t", PLUGIN_PREFIX, "pr_boss_spawned", "Monoculus", calculatedReward);
	}
	return Plugin_Continue;
}
	...
	if (bossIndex != -1 && IsValidEntity(bossIndex)) {
		...
		for (new clientId = 1; clientId <= MaxClients; clientId++) {
			if (IsClientInGame(clientId) == true && scores[clientId][exempt] == false && scores[clientId][bossdmg] >= GetConVarInt(rewardBossDmgMin)) {
				...
				if (reward > 0.0) {
					PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_boss_killed", clientId, reward, "Horseless Headless Horsemann");
	...
	if (bossIndex != -1 && IsValidEntity(bossIndex)) {
		...
		for (new clientId = 1; clientId <= MaxClients; clientId++) {
			if (IsClientInGame(clientId) == true && scores[clientId][exempt] == false && scores[clientId][bossdmg] >= GetConVarInt(rewardBossDmgMin)) {
				...
				if (baseReward > 0.0) {
					PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_boss_killed", clientId, calculatedReward, "Merasmus");
	...
	if (bossIndex != -1 && IsValidEntity(bossIndex)) {
		...
		for (new clientId = 1; clientId <= MaxClients; clientId++) {
			if (IsClientInGame(clientId) == true && scores[clientId][exempt] == false && scores[clientId][bossdmg] >= GetConVarInt(rewardBossDmgMin)) {
				...
				if (baseReward > 0.0) {
					Reward(clientId, calculatedReward);
					PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_boss_killed", clientId, calculatedReward, "Monoculus");
				}
			}
			scores[clientId][bossdmg] = 0;
		}
...
FinishLateInit() {
	...
	if (playerCount > 0)
		PrintToChatAll("%s %t", PLUGIN_PREFIX, "pr_loaded");
}
...
public Query_Cleanup(Handle:owner, Handle:hndl, const String:error[], any:issuerId) {
	...
	if (issuerId == WORLD)
		...
		ReplyToCommand(issuerId, "%s Cleanup performed, %i record(s) deleted.", PLUGIN_PREFIX, rowsDeleted);
	}
}
...
public Query_FindClientByName(Handle:owner, Handle:hndl, const String:error[], any:clientUid) {
	...
	if (foundRows > 1) {
		...
		CloseHandle(data);
		PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_search_noresults", clientId);
	}
}
// Finds the client by name.
...
public Query_FindClientRank(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_top_rank_offline", clientId, nickNamep, SQL_FetchInt(hndl, 0), totalCount, pointsp, seenDate);
		
	if (killsp > 0.0 && deathsp > 0.0) {
		PrintToChat(clientId, "%s %T %T", PLUGIN_PREFIX, "pr_stats_1", clientId, killsp, deathsp, assistsp, "pr_stats_2", clientId, FloatDiv(float(killsp), float(deathsp)));
	}
	else {
		PrintToChat(clientId, "%s %T", PLUGIN_PREFIX, "pr_stats_1", clientId, killsp, deathsp, assistsp);
	}
	
	if (isTF2 == true) {
		PrintToChat(clientId, "%T", "pr_stats_3", clientId, headshotsp, backstabsp, feignsp, deflectsp, killStreaksp);
		
		if (merKillsp > 0)
			PrintToChat(clientId, "%T %T", "pr_stats_4", clientId, merKillsp, "Merasmus", "pr_stats_5", clientId, merLvlp);
			
		if (monKillsp > 0)
			PrintToChat(clientId, "%T %T", "pr_stats_4", clientId, monKillsp, "Monoculus", "pr_stats_5", clientId, monLvlp);
			
		if (hhhKillsp > 0)
			PrintToChat(clientId, "%T", "pr_stats_4", clientId, hhhKillsp, "Headless Horsemann");
		
		PrintToChat(clientId, "%T", "pr_stats_6", clientId, totaltimep / 3600);
		
		PrintToChat(clientId, "%T", "pr_stats_7", clientId, flagCapturesp, flagDefendsp, capCapturesp, capDefendsp, roundsPlayedp);
		PrintToChat(clientId, "%T", "pr_stats_8", clientId, dominationsGoodp, dominationsBadp);
	}
	CloseHandle(data);
}
...
public Query_LoadRank(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if (announceConnect == true && GetConVarBool(suppressRankMsg) == false) {
		...
		PrintToChatAll("%s %t", PLUGIN_PREFIX, "pr_msg_join", clientName, scores[clientId][rank], totalCount);
...
public Query_ResetDatabase_S2(Handle:owner, Handle:hndl, const String:error[], any:issuerUid) {
	...
	if (hndl == INVALID_HANDLE)
	{
		if (issuer != NO_ISSUER && issuer != WORLD && IsClientInGame(issuer))
			PrintToChat(issuer, "Query failed: %s", error);
	...
	isReady = true;
	PrintToChatAll("%s %t", PLUGIN_PREFIX, "pr_reset");

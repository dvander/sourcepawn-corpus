...
{
  CreateConVar("sm_resize_version", PLUGIN_VERSION, "\"Resize Players\" version.", FCVAR_DONTRECORD|FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_REPLICATED);
  
  new Handle:hEnabled = CreateConVar("sm_resize_enabled", "1", "0 = Disable plugin, 1 = Enable plugin.");
  HookConVarChange(hEnabled, ConVarEnabledChanged);
  g_bEnabled = GetConVarBool(hEnabled);
  
  new Handle:hDefaultResize = CreateConVar("sm_resize_defaultresize", "0.4", "Default scale of players when resized.",FCVAR_NONE, true, 0.0);
  ...
  new Handle:hDefaultHeadResize = CreateConVar("sm_resize_defaultheadresize", "2.5", "Default scale of players' heads when resized.");
  ...
  new Handle:hDefaultTorsoResize = CreateConVar("sm_resize_defaulttorsoresize", "2.5", "Default scale of players' torsos when resized.");
  ...
  new Handle:hDefaultHandsResize = CreateConVar("sm_resize_defaulthandsresize", "2.5", "Default scale of players' hands when resized.");
  ...
  new Handle:hOnJoin = CreateConVar("sm_resize_onjoin", "0", "Add values to alter size upon joining, 0 = Disable, 1 = Admin only, 2 = Resize body, 4 = Resize head, 8 = Resize torso, 16 = Resize hands.");
  HookConVarChange(hOnJoin, ConVarOnJoinChanged);
  g_iOnJoin = GetConVarInt(hOnJoin);
  
  new Handle:hMenu = CreateConVar("sm_resize_menu", "0", "0 = Disable menus, 1 = Enable menus when no command parameters are given, 2 = Enable for self-commands only.");
  HookConVarChange(hMenu, ConVarMenuChanged);
  g_iMenu = GetConVarInt(hMenu);
  
  new Handle:hVoices = CreateConVar("sm_resize_voices", "0", "0 = Normal voices, 1 = Voice pitch scales with size, 2 = No low-pitched voices, 3 = No high-pitched voices.");
  HookConVarChange(hVoices, ConVarVoicesChanged);
  g_iVoicesChanged = GetConVarInt(hVoices);
  
  new Handle:hDamage = CreateConVar("sm_resize_damage", "0", "0 = Normal damage, 1 = Damage given scales with size, 2 = No up-scaled damage, 3 = No down-scaled damage.");
  HookConVarChange(hDamage, ConVarDamageChanged);
  g_iDamage = GetConVarInt(hDamage);
  
  new Handle:hSteps = CreateConVar("sm_resize_steps", "0", "0 = Normal step-size, 1 = Step-size scales with size, 2 = No up-scaled steps, 3 = No down-scaled steps.");
  HookConVarChange(hSteps, ConVarStepsChanged);
  g_iSteps = GetConVarInt(hSteps);
  
  new Handle:hNotify = CreateConVar("sm_resize_notify", "1", "0 = No notifications, 1 = Respect sm_show_activity, 2 = Notify everyone.");
  HookConVarChange(hNotify, ConVarNotifyChanged);
  g_iNotify = GetConVarInt(hNotify);
  
  new Handle:hMenuItems = CreateConVar("sm_resize_menuitems", "0.1, Smallest; 0.25, Smaller; 0.50, Small; 1.00, Normal; 1.25, Large; 1.50, Larger; 2.00, Largest", "Resize menu's items.");
  ...
  new Handle:hMenuHeadItems = CreateConVar("sm_resize_headmenuitems", "0.50, Smallest; 0.75, Small; 1.00, Normal; 2.00, Large; 3.00, Largest", "Head resize menu's items.");
  ...
  new Handle:hMenuTorsoItems = CreateConVar("sm_resize_torsomenuitems", "0.50, Smallest; 0.75, Small; 1.00, Normal; 2.00, Large; 3.00, Largest", "Torso resize menu's items.");
  ...
  new Handle:hMenuHandsItems = CreateConVar("sm_resize_handsmenuitems", "0.50, Smallest; 0.75, Small; 1.00, Normal; 2.00, Large; 3.00, Largest", "Hand resize menu's items.");
  ...
  new Handle:hLogging = CreateConVar("sm_resize_logging", "1", "0 = No logging, 1 = Log self/target resizes, 2 = Log target resizes only.");
  HookConVarChange(hLogging, ConVarLoggingChanged);
  g_iLogging = GetConVarInt(hLogging);
  
  new Handle:hBackstab = CreateConVar("sm_resize_backstab", "0", "0 = Normal backstabs, 1 = Backstab damage scales proportionally with size.");
  HookConVarChange(hBackstab, ConVarBackstabChanged);
  g_bBackstab = GetConVarBool(hBackstab);
  
  new Handle:hUnstick = CreateConVar("sm_resize_unstick", "1", "Revert when stuck: 0 = Never, 1 = Self-resizes only, 2 = Respawns only, 3 = Self-resizes and respawns.");
  HookConVarChange(hUnstick, ConVarUnstickChanged);
  g_iUnstick = GetConVarInt(hUnstick);
  
  new Handle:hCooldown = CreateConVar("sm_resize_cooldown", "0", "Cooldown duration for those without permission to bypass (in seconds).", FCVAR_NONE, true, 0.0);
  HookConVarChange(hCooldown, ConVarCooldownChanged);
  g_iCooldown = GetConVarInt(hCooldown);
  
  g_hBound[ResizeType_Generic] = CreateConVar("sm_resize_bounds", "0.1, 3.0", "Lower (optional) and upper bounds for resizing, separated with a comma.");
  ...
  g_hBound[ResizeType_Head] = CreateConVar("sm_resize_headbounds", "0.25, 3.0", "Lower (optional) and upper bounds for head resizing, separated with a comma.");
  ...
  g_hBound[ResizeType_Torso] = CreateConVar("sm_resize_torsobounds", "0.25, 3.0", "Lower (optional) and upper bounds for torso resizing, separated with a comma.");
  ...
  g_hBound[ResizeType_Hands] = CreateConVar("sm_resize_handsbounds", "0.25, 3.0", "Lower (optional) and upper bounds for hand resizing, separated with a comma.");
  ...
  AutoExecConfig(true, "Resize_Players");	
}

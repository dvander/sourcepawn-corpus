...
public OnPluginStart() {
    ...
    fc_cameraSpeed = CreateConVar("fc_cameraSpeed", "500", "Speed of the camera movement.");
    fc_spawnOnly = CreateConVar("fc_spawnOnly", "0", "Only allowed in spawn.");
    ...
    CreateTimer(0.5, Timer_HookSpawns);
...
public Action Command_FreeCam(client, args){
    if(!freeCamEnabled[client] && fc_spawnOnly.BoolValue){
        if (!IsInSpawn[client]) {
            ReplyToCommand(client, "You must be in spawn to use this command!");
...
public Action OnPlayerRunCmd(int client, int& buttons, int& impulse, float vel[3], float angles[3], int& weapon, int& subtype, int& cmdnum, int& tickcount, int& seed, int mouse[2]) {
    if(freeCamEnabled[client]){
        ...
        if(fc_maxCameraDistance.FloatValue > 0 && distance > fc_maxCameraDistance.FloatValue ) {
            ...
            getDirectionVector(cameraPos[client], ang, newForce, 300.0, fc_cameraSpeed.FloatValue);
...
public OnGameFrame() {
    for(int client = 1; client < MaxClients; client++){
        ...
        if(!IsInSpawn[client] && fc_spawnOnly.BoolValue && !cameraControllEnabled[client]){
            disableFreecam(client);
        }

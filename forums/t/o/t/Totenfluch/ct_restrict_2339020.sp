...
new String:SQLQueries[SQLMAX__][E_QMax][] =
{
	{ // SQLite
...
public OnPluginStart()
{
	...
	Format(db_config, sizeof(db_config), "ctbans");
...
public Action:Command_CtBan(client, argc)
{
	if (argc != 1)
	{
		PrintToChat(client, "[SM] sm_ctban <player> | Bans a player from joining CT.");
	}
	else
	{
		decl String:arg[128];
		...
		if (g_iDatabaseId[targets[0]] == -1)
		{
			...
			GetClientAuthId(targets[0], AuthId_Steam2, player_authid, sizeof(player_authid));
			decl String:admin_authid[32];
			if (client != 0)
			{
				GetClientAuthId(client, AuthId_Steam2, admin_authid, sizeof(admin_authid));
			}
			else
			{
		...
		{
			PrintToChat(client, "[SM] %N was already banned from playing as CT.", targets[0]);
		}
	}
...
public Action:Command_CtUnban(client, argc)
{
	if (argc != 1)
	{
		PrintToChat(client, "[SM] sm_ctunban <player> | Unbans a player from joining CT.");
	}
	else
	{
		decl String:arg[128];
		...
		{
			PrintToChat(client, "[SM] %N wasn't banned from playing as CT to begin with.", targets[0]);
		}
	}
...
public Action:Command_ReloadCtBan(client, argc)
{
	if (argc != 1)
	{
		PrintToChat(client, "[SM] sm_reloadctban <player(s)> | Reload bans");
		return Plugin_Handled;
	}
	...
	PrintToChat(client, "[SM] Reloading CT Ban(s)");

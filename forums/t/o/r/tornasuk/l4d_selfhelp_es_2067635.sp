...
public Plugin:myinfo = 
{
	...
	description = "Self Help (en español)",
...
public Action:AdvertisePills(Handle:timer, any:client)
{
	...
	{
		PrintToChat(client, "\x03mantén presionado \x04AGACHARSE\x03 para auto-ayudarte");
	}
			...
			if(CanSelfHelp(client))
			{
				if(L4D2Version)
				{
					if(HelpState[client]==STATE_NONE)
					{
						...
						PrintHintText(client, "Estas auto-ayudándote");
					}
		...
		if ((buttons & IN_DUCK) ||  (buttons & IN_USE)) 
		{
			...
			if(findone)
			{
				...
				if(HelpOhterState[client]==STATE_NONE)
				{
					...
					PrintHintText(other, "%N esta ayudándote", client);
					HelpStartTime[client]=time;
				}
			...
			while ((ent = FindEntityByClassname(ent,  "weapon_pain_pills" )) != -1)
			{
				if (IsValidEntity(ent))
				{
					...
					if(GetVectorDistance(targetVector1  , targetVector2)<dis)
					{
						...
						RemoveEdict(ent);
						pickup=true;
						PrintHintText(client,"encontraste pills");
				...
				while ((ent = FindEntityByClassname(ent,  "weapon_adrenaline" )) != -1)
				{
					if (IsValidEntity(ent))
					{
						...
						if(GetVectorDistance(targetVector1  , targetVector2)<dis)
						{
							...
							RemoveEdict(ent);
							pickup=true;
							PrintHintText(client,"encontraste adrenalina");
			...
			while ((ent = FindEntityByClassname(ent,  "weapon_first_aid_kit" )) != -1)
			{
				if (IsValidEntity(ent))
				{
					...
					if(GetVectorDistance(targetVector1  , targetVector2)<dis)
					{
						...
						RemoveEdict(ent);
						pickup=true;
						PrintHintText(client,"encontraste un botiquin");
						break;
					}
				}
			}
			...
			while ((ent = FindEntityByClassname(ent,  "weapon_pistol" )) != -1)
			{
				if (IsValidEntity(ent))
				{
					...
					if(GetVectorDistance(targetVector1  , targetVector2)<dis)
					{
						...
						RemoveEdict(ent);
						pickup=true;
						PrintHintText(client,"encontraste una pistola");
						break;
					}
				}
			}
...
SelfHelp(client)
{
	...
	if(slot!=-1)
	{
		...
		if(slot ==4)
		{
			...
			if(adrenaline)	PrintToChatAll("\x04%N\x03 se auto-ayudó con adrenalina!", client);  
			if(pills)	PrintToChatAll("\x04%N\x03 se auto-ayudó con pills!", client); 	
		...
		else if(slot==3)
		{
			...
			HelpState[client]=STATE_OK;
			PrintToChatAll("\x04%N\x03 se auto-ayudó con un botiquin!", client); 
	...
	{
		PrintHintText(client, "auto-ayuda fallida");
...
HelpOther(client, helper)
{
	...
	PrintToChatAll("\x04%N\x03 ayudó a a\x04 %N \x03 estando incapacitado", helper, client);  
	//first(client);
}
ReviveClientWithKid(client)
{

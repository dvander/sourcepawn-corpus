...
{
	name = "Jump Mode",
	author = "TheJCS, edited by minnesnowta,TonyBaretta",
...
new Handle:g_hAutoResupply;
new Handle:g_hResupply;
...
public OnPluginStart()
{
	...
	g_hAutoResupply = CreateConVar("jm_autoresupply", "1", "Enable the Jump Mode Auto Resupply", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_hResupply = CreateConVar("jm_resupply_enabled", "1", "Enable the Jump Mode Resupply", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	RegConsoleCmd("s", cmdSaveLoc, "Save your current position");
	...
	RegConsoleCmd("t", cmdTeleport, "Teleport you to the saved position");
	RegConsoleCmd("jm_resupply", cmdResupply, "Resupply your ammo");
	RegConsoleCmd("jm_help", cmdHelp, "Resupply your ammo");
	...
	HookConVarChange(g_hAutoResupply, cvarResupplyChanged);
	HookConVarChange(g_hAutoHeal, cvarResupplyChanged);
...
public Action:eventRoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(g_bPluginEnabled)
	{
		if(GetConVarBool(g_hRemoveCPs))
			RemoveCPs();	
		if(GetConVarBool(g_hAutoResupply) && GetConVarBool(g_hAutoHeal))
			ToggleResupplies(false);
...
public Action:eventChangeTeam(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(g_bPluginEnabled && IsClientInGame(client) && (client >= 1)){
...
public Action:eventPlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(g_bPluginEnabled)
	{
		...
		if(GetConVarBool(g_hAutoResupply))
			GiveAmmo(client);
		}
	}
}
...
{
	new iMaxHealth = GetEntProp(GetPlayerResourceEntity(), Prop_Send, "m_iMaxHealth", _, client);
...
public cvarResupplyChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	if(GetConVarBool(g_hAutoResupply) && GetConVarBool(g_hAutoHeal))
		ToggleResupplies(false);
	else
...
public Action:cmdHelp(client, args)
{
	...
	if(g_bPluginEnabled)
	{
		...
		if(GetConVarBool(g_hAutoResupply))
			PrintToConsole(client, "[JM]   - Auto ammo resupply on player hurt");
		...
		if(GetConVarBool(g_hAutoResupply) && GetConVarBool(g_hAutoHeal))
			PrintToConsole(client, "[JM]   - Remove Resupplies");
		PrintToConsole(client, "[JM] - Console Commands (or \"say !\" commands)");
		if(GetConVarBool(g_hTeleport))
		{
			PrintToConsole(client, "[JM]   - jm_saveloc or s: Save your current position");
			PrintToConsole(client, "[JM]   - jm_resetloc: Reset your saved location");
			PrintToConsole(client, "[JM]   - jm_teleport or t: Teleport you to the saved position");
		}
		if(GetConVarBool(g_hResupply))
			PrintToConsole(client, "[JM]   - jm_resupply: Resupply your ammo");
	}
	else
...
public Action:cmdResupply(client, args)
{
	if(GetConVarBool(g_hResupply))
		GiveAmmo(client);
	else
		PrintToChat(client, "\x04[JM]\x01 Resupply is disabled")
}

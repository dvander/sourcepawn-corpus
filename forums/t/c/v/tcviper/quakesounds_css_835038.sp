...
{
	name = "Quake Sounds",
	author = "dalto and Grrrrrrrrrrrrrrrrrrr",
...
new numSets = 0;
new String:setsName[MAX_NUM_SETS][60];
...
new numSounds = 0;
new String:soundsFiles[MAX_NUM_FILES][MAX_FILE_LEN];
...
public LoadSounds()
{
	...
	{
		Format(buffer, 65, "sound set %i", i);
		KvGetString(kvQSL, buffer, setsName[numSets], 65);
				...
				do
				{
					...
					{						
						...
						if((tempConfig & 1) || (tempConfig & 2) || (tempConfig & 4))
						{
							...
							{							
								KvGetString(kvQSL, setsName[set], soundsFiles[numSounds], 65);
...
public OnClientPutInServer(client)
{
	...
	if(!IsFakeClient(client))
	{
		...
		// Make the announcement in 65 seconds unless announcements are turned off
		if(GetConVarBool(cvarAnnounce))
		{
			CreateTimer(65.0, TimerAnnounce, client);
		}
...
public PrintQuakeText(soundKey, killsValue, attackerClient, victimClient)
{
	...
	{
		GetClientName(victimClient, victimName, 65);
	}
	else
	{
		victimName = "Nobody";
	...
	decl String:translationName[MAX_FILE_LEN];
	if(killsValue>0)
	{
		Format(translationName, MAX_FILE_LEN, "%s %i", typeNames[soundKey], killsValue);
	}
	else
	{
		Format(translationName, MAX_FILE_LEN, "%s", typeNames[soundKey]);
	}

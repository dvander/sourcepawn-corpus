...
#include <sourcemod>
#include <clientprefs>
#define PLUGIN_VERSION "2.1.1.0-GC"
...
public OnPluginStart() {
	...
	SetCookieMenuItem(OnSettingsMenuChoice, 0, "Loyalty Points");
}

public OnSettingsMenuChoice(client, CookieMenuAction:action, any:info, String:buffer[], maxlen)
{
	Command_loyal(client, 0);
}
...
bool:IsEligible(client)
{
	...
	if (IsFakeClient(client))  // bot not eligible
		return false;
	
	// admins not eligible
	new bits = GetUserFlagBits(client);
	if ((bits & ADMFLAG_GENERIC) == ADMFLAG_GENERIC || (bits & ADMFLAG_ROOT) == ADMFLAG_ROOT)
		return false;
	...
	if (admid == INVALID_ADMIN_ID)
		return true;
	new GroupId:vipgrp = FindAdmGroup("VIP");
	new grpcount = GetAdminGroupCount(admid);
	decl String:tempname[1];
	for (new i = 0; i < grpcount; i++)
	{
		new GroupId:tempgrp = GetAdminGroup(admid, i, tempname, sizeof(tempname));
		if (tempgrp == vipgrp)
		{
			return false;
		}
...
public Action:Command_loyal(client, args) {
	if(args==0) {
		if (!IsEligible(client))
		{
			ReplyToCommand(client, "\x04You are not eligible for loyalty points.");
		...
		if(targ!=-1)
		{
			if (IsEligible(targ))
			{
				ReplyToCommand(client, "\x03%N\x01 has \x04%i\x01 loyalty points.", targ, ((kills[targ]+assists[targ]+(timep[targ]/60))/10)-spent[targ]+bonus[targ]);
			}
			else
			{
				ReplyToCommand(client, "\x03%N is not eligible for loyalty points", targ);
			}
		}
	}
	return Plugin_Handled;
...
public Menu_buy(Handle:menu, MenuAction:action, param1, param2) {
	if(action==MenuAction_End) {
		...
		switch(param2) {
			case 0: {
				AddReservedSlotTime(param1, 604800, cost[1]);
				spent[param1] += cost[1];
			}
			case 1: {
				AddReservedSlotTime(param1, 1209600, cost[2]);
				spent[param1] += cost[2];
			}
			case 2: {
				AddReservedSlotTime(param1, 2592000, cost[4]);
				spent[param1] += cost[4];
			}
			case 3: {
				AddReservedSlotTime(param1, 5184000, cost[8]);
				spent[param1] += cost[8];
			}
			case 4: {
				AddReservedSlotTime(param1, 7776000, cost[12]);
				spent[param1] += cost[12];
			}
		}
	}
}
public AddReservedSlotTime(client, time, rescost) {

	decl String:name[MAX_NAME_LENGTH+8], String:steamid[32], String:query[512], String:consoleCommand[512];
	new adminuntil, gid;
	Format(name, sizeof(name), "(Loyal) %N", client);
	decl String:safename[(sizeof(name)*2)-1];
	SQL_EscapeString(admdb, name, safename, sizeof(safename));
	GetClientAuthString(client, steamid, sizeof(steamid));
	if(!useFlatfile){
		if(sqlite_admdb) {
			Format(query, sizeof(query), "INSERT INTO sm_admins (authtype, identity, name, adminuntil) VALUES ('steam', '%s', '%s', %i) ON DUPLICATE KEY UPDATE adminuntil=adminuntil+%i", steamid, safename, GetTime()+time, time, steamid);
		} else if (useSourcebans) {
			Format(query, sizeof(query), "INSERT INTO sb_admins (authid, user, immunity, srv_group, adminuntil) VALUES ('%s', '%s', 0, '%s', %i) ON DUPLICATE KEY UPDATE adminuntil=adminuntil+%i", steamid, safename, userGroup, GetTime()+time, time);
		} else {
			Format(query, sizeof(query), "INSERT INTO sm_admins (authtype, identity, flags, name, immunity, adminuntil) VALUES ('steam', '%s', 'a', '%s', 0, %i) ON DUPLICATE KEY UPDATE adminuntil=adminuntil+%i", steamid, safename, GetTime()+time, time, steamid);
		}
	...
	PrintToChat(client, "\x04You bought a %d-day VIP slot for %d Loyalty points! Congratulations!", (time/86400), rescost);
}
...
public SQLQueryConnect(Handle:owner, Handle:hndl, const String:error[], any:data) {
	...
	if(hndl==INVALID_HANDLE) {
		...
		GetClientName(client, clientname, sizeof(clientname));
		decl String:safename[(sizeof(clientname)*2)-1];
		SQL_EscapeString(db, clientname, safename, sizeof(safename));
		ReplaceString(clientname, sizeof(clientname), "'", "");
		GetClientAuthString(client, steamid, sizeof(steamid));
		if(!SQL_MoreRows(hndl)) {
			if(sqlite_db) {
				Format(query, sizeof(query), "INSERT INTO loyal VALUES('%s', '%s', 0, 0, 0, 0, 0)", safename, steamid);
			} else {
				Format(query, sizeof(query), "INSERT INTO loyal (name, steamid, kills, assists, time, spent, bonus) VALUES ('%s', '%s', 0, 0, 0, 0, 0)", safename, steamid);
			}
			SQL_TQuery(db, SQLErrorCheckCallback, query);
			kills[client] = 0;
			assists[client] = 0;
			...
			Format(query, sizeof(query), "UPDATE loyal SET name='%s' WHERE steamid='%s'", safename, steamid);
			SQL_TQuery(db, SQLErrorCheckCallback, query);

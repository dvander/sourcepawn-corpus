...
public Action:GameEnd(Handle:timer)
{
	...
	if (timerGameEnd != INVALID_HANDLE)
	{
		...
		if (GetConVarInt(pluginManaged) == 1)
		{
			...
			new String:mapcycle[16][PLATFORM_MAX_PATH];
			new mapsCount, i;
			...
			if (mapsCount > 0 && !StrEqual(mapcycle[0], "", false))
			{
				if (mapIndex == 0)
				{
					strcopy(nextmap, sizeof(nextmap), mapcycle[0]);
				}
				else if (mapIndex < mapsCount)
				{
					strcopy(nextmap, sizeof(nextmap), mapcycle[mapIndex]);
				}
				else if (mapIndex == mapsCount)
				{
					mapIndex = 0;
		...
		if (players == 0) {
			...
			PrintToServer("[SM] Empty server, call ForceChangeLevel to %s...", nextmap);
			ForceChangeLevel(nextmap, "QMR NextMap");
		}
	}
	else
	{

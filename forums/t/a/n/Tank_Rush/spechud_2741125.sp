...
bool:FillTankInfo(Handle:hSpecHud, bool:bTankHUD = false)
{
	...
	if (bTankHUD)
	{
		...
		DrawPanelText(hSpecHud, info);
		DrawPanelText(hSpecHud, "_____________________________");
	}
	else
	{
		DrawPanelText(hSpecHud, " ");
...
FillGameInfo(Handle:hSpecHud)
{
	...
	{
		Format(info, sizeof(info), "%s (%s ronda)", info, (InSecondHalfOfRound() ? "2nd" : "1st"));
		DrawPanelText(hSpecHud, info);

		Format(info, sizeof(info), "Horda natural: %is", CTimer_HasStarted(L4D2Direct_GetMobSpawnTimer()) ? RoundFloat(CTimer_GetRemainingTime(L4D2Direct_GetMobSpawnTimer())) : 0);
		DrawPanelText(hSpecHud, info);

		Format(info, sizeof(info), "Progreso del superviviente: %i%%", RoundToNearest(GetHighestSurvivorFlow() * 100.0));
		DrawPanelText(hSpecHud, info);
	...
	else if (GetCurrentGameMode() == L4D2Gamemode_Scavenge)
	{
		...
		Format(info, sizeof(info), "Mitad: %s", (InSecondHalfOfRound() ? "2nd" : "1st"));
		DrawPanelText(hSpecHud, info);

...
ToggleHud(client) {
	if (isHudActive[client]) {
		...
		PrintToChat(client, "[SM] My Spec Hud выключено");
	}
	else {
		isHudActive[client] = true;
		PrintToChat(client, "[SM] My Spec Hud включено");
	}
}
...
public Action:Timer_UpdateHud(Handle:timer) {
	...
	for (new activePlayer = 0; activePlayer < clientCount; activePlayer++) {
		...
		SetPanelTitle(activePanel, "Твои наблюдатели:");
		
		// create spec title string
		Format(panelTitle, sizeof(panelTitle), "Наблюдатели %N:", clientId[activePlayer]);
		...
		for (new specPlayer = 0; specPlayer < clientCount; specPlayer++) {
			...
			Format(panelText, sizeof(panelText), "%s - %N (%s)", (clientTeam[specPlayer] == 1) ? "ЗРТЛ" : "УМЕР", clientId[specPlayer], (specMode[specPlayer] == 5) ? "3rd" : "1st");
		...
		DrawPanelText(activePanel, "Вкл/Выкл панель !myspecs");
		DrawPanelText(specPanel, "Вкл/Выкл панель !myspecs");

...
public psyRTDAction:DoHealthBuff(client)
{
	...
	SetEntProp(client, Prop_Send, "m_iHealth", GetClientHealth(client)+GetConVarInt(g_cvHealthBuffAmt));
...
public psyRTDAction:DoCheetah(client)
{
	...
	g_fLastSpeed[client] = GetEntPropFloat(client, Prop_Send, "m_flLaggedMovementValue");
	// ARREGLADO
	//SetEntPropFloat(client, Prop_Send, "m_flMaxspeed", GetConVarFloat(g_cvSnailSpeed));
	SetEntPropFloat(client, Prop_Data, "m_flLaggedMovementValue", GetConVarFloat(g_cvSnailSpeed));
...
public EndCheetah(client, psyRTDEffectEndReason:reason)
{
	...
	if (reason == psyRTDEndReason_TimeExpired)
	{
		...
		SetEntPropFloat(client, Prop_Send, "m_flLaggedMovementValue", g_fLastSpeed[client]);
	}
}

 ...
 * SQLQueries
 * ===========================================================================
 *   A plug-in to connect to databases and execute arbitrary queries.
 * 
 *   Originally written by Timiditas.
 *   Modified by theY4Kman to support chunked output. (7/7/2012)
 * ===========================================================================
 */
...
public Plugin:myinfo = 
{
    name = "SQLQueries",
    author = "Timiditas",
    description = "Dispatch SQL queries via console.",
...
public query_callback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
    ...
    else 
    {
        ...
        else
        {
            ...
            decl String:Buffer[2048], String:Output[2048];
            Output[0] = '\0';
...
ReplyCust(client, const String:Reply[], any:...)
{
    ...
    new length = strlen(bReply);
    new minChunks = RoundToCeil(float(length) / MAX_MESSAGE_LENGTH);
    new i = 0;
    
    do
    {
        decl String:chunk[MAX_MESSAGE_LENGTH+1];
        strcopy(chunk, sizeof(chunk), bReply[i]);
        
        new newline = FindCharInString(chunk, '\n', true);
        new nextSkip = MAX_MESSAGE_LENGTH;
        if (newline != -1 && minChunks > 1)
        {
            chunk[newline] = '\0';
            nextSkip = newline+1;
        }
        
        PrintToConsole(client, chunk);

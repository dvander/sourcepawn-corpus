...
new Handle:g_hWelcomeTimer[MAXPLAYERS+1],
	...
	g_iPlayerTakenAirCount[MAXPLAYERS+1],//taken airshot counter
	g_iPlayerSpecTarget[MAXPLAYERS+1],
	g_iPlayerAirshotSign[MAXPLAYERS+1],
...
public OnPluginStart()
{
	...
	gcvar_allowedClasses = CreateConVar("mgemod_allowed_classes", "demoman engineer heavyweapons medic pyro scout soldier sniper spy", "Classes that players allowed to choose by default", FCVAR_PLUGIN);
	gcvar_blockFallDamage = CreateConVar("mgemod_blockdmg_fall", "0", "Block falldamage? (0 = Disabled)", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	gcvar_airshotSigns = CreateConVar("mgemod_airshot_signs", "0", "Show airshot signs above players when they get airshot? (0 = Disabled)", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	g_bParticles = GetConVarInt(gcvar_airshotSigns) ? true : false;
	...
	//RegConsoleCmd("tryme", Command_AddBot, "Add bot to your arena");
	//RegConsoleCmd("botme", Command_AddBot, "Add bot to your arena (alias)");
...
{	
	...
	if(isMapAm)
	{
		...
		if (!g_bAmEnabled)
		{
			...
			// Add the map's particle manifest to the downloads table.
			if (g_bParticles)
			{
				
				decl String:file[96];
				Format(file, sizeof(file), "maps/%s_particles.txt", g_sMapName);
		...
		for (new i=0;i<=g_iArenaCount;i++)
		{
			if(g_bArenaBBall[i])
			{
				if(IsValidEntity(g_iBBallHoop[i][SLOT_ONE]))
					SDKUnhook(g_iBBallHoop[i][SLOT_ONE], SDKHook_StartTouch, OnTouchHoop);
				if(IsValidEntity(g_iBBallHoop[i][SLOT_TWO]))
...
public OnGameFrame()
{
	if (g_bAmEnabled)
	{
		for (new client = 1; client <= MaxClients; client++)
		{
			...
			{
				if (g_iPlayerTakenAirCount[client] > 0 && GetEntityFlags(client) & (FL_ONGROUND))
					g_iPlayerTakenAirCount[client] = 0;
				...
				if(!g_bArenaBBall[arena_index] && !g_bArenaMGE[arena_index])
				{
					...
					SetEntProp(client, Prop_Data, "m_iHealth", replacement_hp, 1);
				}
...
{
	if(!g_bAmEnabled || !IsValidClient(victim) || !IsValidClient(attacker))
		return Plugin_Continue;
...
ShowAirshotHudMsg(client,count)
{
	...
	for (new i=1;i<=MaxClients;i++)
	{
		if (IsValidClient(i) && ((GetClientTeam(i)==TEAM_SPEC && g_iPlayerSpecTarget[i]==client) || client == i))
		{
...
RemoveFromQueue(client, bool:calcstats=false, bool:specfix=false)
{
	...
	if (player_slot==SLOT_ONE || player_slot==SLOT_TWO)
	{
		...
		if(g_bArenaBBall[arena_index])
		{
			...
			if(foe)
			{
				particle = EntRefToEntIndex(g_iClientParticle[foe]);
				if (particle != 0 && IsValidEntity(particle))
				{
					RemoveEdict(particle);
				}
				g_iClientParticle[foe]=0;
				g_bPlayerHasIntel[client] = false;
			}
		}
		...
		if (g_iArenaQueue[arena_index][SLOT_TWO+1])
		{
			...
			if (foe && IsFakeClient(foe))
			{
				...
				ServerCommand("tf_bot_quota %d", quota--);
				ServerCommand("kickid %d", GetClientUserId(foe));
				KickClient(foe);
				ServerExecute();
			}
	...
	if (FileToKeyValues(kv, txtfile))
	{
		if (KvGotoFirstSubKey(kv))
		{
				...
				if (StrEqual(g_sMapName, kvmap, false))
				{
					if (KvGotoFirstSubKey(kv))
					{
						do
						{
							...
							ReplaceString(g_sArenaName[g_iArenaCount], 64, "'", "");
							new id;
			...
			if (g_iArenaCount)
			{
				...
				LogError("Can't find map cfg. MGEMod disabled.");	
				CloseHandle(kv);
				return false;
			}
		} else {
	...
	if (FileToKeyValues(kv, txtfile))
	{
		if (KvGotoFirstSubKey(kv))
		{
			do
			{
				...
				ReplaceString(g_sWeaponName[g_iWeaponCount], 64, "'", "");
...
// Are these functions completely redundant? Yes. Do I give a shit? No.

Handle:CreateParticle(client,String:type[]="midair_text")
{
	//deleting an old one if its exists
	//DeleteParticle(g_iPlayerAirshotSign[client]);
	...
	{ // Check if it was created correctly
		...
		g_iPlayerAirshotSign[client] = particle;
...
public handler_ConVarChange(Handle:convar, const String:oldValue[], const String:newValue[])
{
	if (convar == gcvar_blockFallDamage) {
		...
		StringToInt(newValue) ? (g_bParticles = true) : (g_bParticles = false);
...
{ //handle commands "!ammomod" "!add" and such //building queue's menu and listing arena's	
	if (!g_bAmEnabled || !IsValidClient(client))
		return Plugin_Continue;
...
{
	if (!g_bAmEnabled || g_bNoStats || !IsValidClient(client))
		return Plugin_Continue;
...
public Action:Command_JoinClass(client, args)
{
	...
	if (args)
	{
		...
		if (!new_class && StrEqual(s_class, "heavyweapons")) new_class = TFClass_Heavy;
...
public Action:Command_ToogleHitblip(client, args)
{
	...
	if (!g_bNoStats)
	{
		...
		GetClientAuthString(client, steamid, sizeof(steamid));
		Format(query, sizeof(query), "UPDATE mgemod_stats SET hitblip=%i WHERE steamid='%s'", g_bHitBlip[client]?1:0, steamid);
		SQL_TQuery(db, SQLErrorCheckCallback, query);
	}
...
{
	if (!g_bAmEnabled || !client || !IsValidClient(client))
		return Plugin_Continue;
...
PrepareSQL() // Opens the connection to the database, and creates the tables if they dont exist.
{
	...
	{
		LogError("Cant use config <%s> <Error: %s>, trying sqlite <storage-local>",g_sDBConfig, error);
...
public T_SQLQueryOnConnect(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if(SQL_FetchRow(hndl))
	{
		...
		Format(query, sizeof(query), "UPDATE mgemod_stats SET name='%s' WHERE steamid='%s'", namesql, steamid);
		SQL_TQuery(db, SQLErrorCheckCallback, query);
	} else {
		if(g_bUseSQLite)
		{
			Format(query, sizeof(query), "INSERT INTO mgemod_stats VALUES(1600, '%s', '%s', 0, 0, %i, 1)", steamid, namesql, GetTime());
			SQL_TQuery(db, SQLErrorCheckCallback, query);
		} else {
			Format(query, sizeof(query), "INSERT INTO mgemod_stats (rating, steamid, name, wins, losses, lastplayed, hitblip) VALUES (1600, '%s', '%s', 0, 0, %i, 1)", steamid, namesql, GetTime());
			SQL_TQuery(db, SQLErrorCheckCallback, query);
		}
...
public SQLErrorCheckCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	if(!StrEqual("", error))
	{
		...
		if(!g_bNoStats)
		{
			...
			LogError("Lost connection to database, attempting reconnect in %i minutes.");
...
public SQLDbConnTest(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	if(!StrEqual("", error))
	{
		...
		if(!g_bNoStats)
		{
			PrintToChatAll("%t", "DatabaseRestored");
...
public Action:Event_PlayerHurt(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if(!g_bAmEnabled || !IsValidClient(victim))
		return Plugin_Continue;
	...
	if (attacker > 0 && victim != attacker) // If the attacker wasn't the person being hurt, or the world (fall damage).
	{
		...
		if (g_bPlayerTakenDirectHit[victim])
		{
			...
			{
				g_iPlayerTakenAirCount[victim] = 0;
				...
				if (dist >= g_iAirshotHeight)
				{
					...
					g_iPlayerTakenAirCount[victim] += 1;	
					ShowAirshotHudMsg(attacker,g_iPlayerTakenAirCount[victim]);
						
					if (g_bParticles)
					{
						new String:mc[16] = "midair_text";
						
						if (g_iPlayerTakenAirCount[victim] > 1 && g_iPlayerTakenAirCount[victim]<=10)
							Format(mc,sizeof(mc),"midair_%d",g_iPlayerTakenAirCount[victim]);
						
						CreateParticle(victim,mc);
					}
				}
			}
		}
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	new victim = GetClientOfUserId(GetEventInt(event, "userid"));
	new arena_index = g_iPlayerArena[victim];
	new particle = EntRefToEntIndex(g_iClientParticle[victim]);
	
	if (particle != 0 && IsValidEntity(particle))
		RemoveEdict(particle);
	g_iClientParticle[victim]=0;
	...
	if (g_iArenaStatus[arena_index] >= AS_FIGHT && g_iArenaStatus[arena_index] < AS_REPORTED && fraglimit > 0 && g_iArenaScore[arena_index][killer_slot] >= fraglimit)
	{
		...
		if(g_iPlayerAttackUsedWeaponIdx[killer] != -1)
		{
			if(!g_bWeaponProjectile[g_iPlayerAttackUsedWeaponIdx[killer]]) // Hitscan-only workaround
			{
				...
				#if defined DEBUG_LOG
				if(g_iPlayerAttackUsedWeaponIdx[killer] >= 0)
					PrintToChat(killer, "[MGEMod] CalcElo Workaround. Idx [%i] Name [%s]. Shot #%i", g_iWeaponIdx[g_iPlayerAttackUsedWeaponIdx[killer]], g_sWeaponName[g_iPlayerAttackUsedWeaponIdx[killer]], g_iPlayerShotCount[killer][g_iPlayerAttackUsedWeaponIdx[killer]]);
				#endif
			}
		}
...
public Action:Timer_ChangeSpecTarget(Handle:timer, any:userid)
{
	...
	if (target>0 && target < MaxClients && g_iPlayerArena[target]){

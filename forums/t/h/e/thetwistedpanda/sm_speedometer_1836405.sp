/*
	Removed an unnecessary round end check when players spawn.
*/
...
public Action:Event_OnPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(g_bEnabled)
	{
		...
		if(g_bUseTimer && g_bShowDisplay[client] && !IsFakeClient(client))
		{
			if(g_hTimer_Display[client] != INVALID_HANDLE)
				CloseHandle(g_hTimer_Display[client]);
...
public Action_OnSettingsChange(Handle:cvar, const String:oldvalue[], const String:newvalue[])
{
	if(cvar == g_hMethod)
	{
		...
		g_bShowFastest = bool:StringToInt(newvalue);

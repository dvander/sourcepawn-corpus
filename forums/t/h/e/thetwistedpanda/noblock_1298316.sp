...
new Handle:sm_noblock_smokes = INVALID_HANDLE;
new Handle:sm_noblock_flashes = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	sm_noblock_smokes = CreateConVar("sm_noblock_smokes", "1", "Enables/Disables blocking of smoke grenades; 0 - Disabled, 1 - Enabled");
	sm_noblock_flashes = CreateConVar("sm_noblock_flashes", "1", "Enables/Disables blocking of flashbangs; 0 - Disabled, 1 - Enabled");
...
{
	if (GetConVarInt(sm_noblock_grenades) == 1 && StrEqual(classname, "hegrenade_projectile"))
	{
		SetEntData(entity, g_CollisionOffset, 2, 1, true);
	}
	
	if (GetConVarInt(sm_noblock_flashes) == 1 && StrEqual(classname, "flashbang_projectile"))
	{
		SetEntData(entity, g_CollisionOffset, 2, 1, true);
	}
	
	if (GetConVarInt(sm_noblock_smokes) == 1 && StrEqual(classname, "smokegrenade_projectile"))
	{
		SetEntData(entity, g_CollisionOffset, 2, 1, true);
	}
}

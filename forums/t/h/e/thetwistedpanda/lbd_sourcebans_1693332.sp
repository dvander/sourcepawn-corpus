...
{
	name = "(LBD) SourceBans",
	author = "SourceBans Development Team, with optional Limit Ban Duration support.",
...
public OnConfigsExecuted()
{
	...
	BuildPath(Path_SM, _sPath, sizeof(_sPath), "plugins/sourcebans.smx");
	if(FileExists(_sPath))
	{
		decl String:_sNewPath[PLATFORM_MAX_PATH];
		BuildPath(Path_SM, _sNewPath, sizeof(_sNewPath), "plugins/disabled/sourcebans.smx");
		ServerCommand("sm plugins unload sourcebans");
		if(FileExists(_sNewPath))
			DeleteFile(_sNewPath);
		RenameFile(_sNewPath, _sPath);
		LogToFile(logFile, "plugins/sourcebans.smx was unloaded and moved to plugins/disabled/sourcebans.smx");
	}
}

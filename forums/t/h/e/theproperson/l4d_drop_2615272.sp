...
public OnPluginStart()
{
	...
	l4d_drop_probability[CLASS_HUNTER] = CreateConVar("l4d_drop_hunter", "0", "Drop probability by Hunter Pounce", FCVAR_PLUGIN);
	l4d_drop_probability[CLASS_BOOMER] = CreateConVar("l4d_drop_boomer", "0", "Drop probability by Boomer Explosion", FCVAR_PLUGIN);
	l4d_drop_probability[CLASS_SMOKER] = CreateConVar("l4d_drop_smoker", "0", "Drop probability by Smoker Tongue", FCVAR_PLUGIN);	
	l4d_drop_probability[CLASS_JOCKEY] = CreateConVar("l4d_drop_jockey", "0", "Drop probability by Jockey Ride", FCVAR_PLUGIN);	
	l4d_drop_probability[CLASS_CHARGER] = CreateConVar("l4d_drop_charger", "0", "Drop probability by Charger Charge", FCVAR_PLUGIN);	
	l4d_drop_probability[CLASS_TANK] = CreateConVar("l4d_drop_tank", "0", "Drop probability by Tank Attacks", FCVAR_PLUGIN);	
	l4d_drop_probability[CLASS_LEDGE_GRAB] = CreateConVar("l4d_drop_ledge_grab", "0", "Drop probability by Ledge Incapacitation", FCVAR_PLUGIN);	
	l4d_drop_probability[CLASS_INCAP] = CreateConVar("l4d_drop_incap", "0", "Drop probability by Incapacitation", FCVAR_PLUGIN);	
	l4d_drop_probability[CLASS_EXPLODED] = CreateConVar("l4d_drop_explode", "0", "Drop probability by Explosives", FCVAR_PLUGIN);	
	l4d_drop_probability[CLASS_HURT] = CreateConVar("l4d_drop_hurt", "0", "Drop probability by Common infected", FCVAR_PLUGIN);	
	l4d_drop_probability[CLASS_HURT_BY_SURVIVOR] = CreateConVar("l4d_drop_hurt_by_survivor", "0", "Drop probability by Survivor Friendly Fire", FCVAR_PLUGIN);	
	l4d_drop_probability[CLASS_SHOVE_BY_INFECTED] = CreateConVar("l4d_drop_shove_by_infected", "0", "Drop probability by Special Infected Secondary Attack", FCVAR_PLUGIN);	
	l4d_drop_probability[CLASS_SHOVE_BY_SURVIVOR] = CreateConVar("l4d_drop_shove_by_survivor", "0", "Drop probability by Survivor Shoves", FCVAR_PLUGIN);	
		...
		if(weapon>0 && IsValidEntity(weapon) && IsValidEdict(weapon))
		{
			...
			if(owner < 0)
			{
				...
				if (GetPlayerWeaponSlot(receiver, slot) > 0)
				{
					...
					{
						//PrintCenterText(receiver, "%N gave you an item", client);
					}
				}
				else
				{
					...
					if(client>0 && IsClientInGame(client))
					{
						//PrintCenterText(client, "You gave an item to %N", receiver);

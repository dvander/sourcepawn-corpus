...
public OnPluginStart(){
	...
	CreateConVar("mp_respawn_offset", "0", "Sets respawn offset of height", _, true, 0.0, true, 200.0);
...
public Action:Evt_Spawn(Handle:event, const String:name[], bool:dontBroadcast){
	...
	if(client >= 1 && client <= MaxClients){
		if(getVal(GetClientTeam(client))){
			if(deathPoint[client][0] == 0.0){//Incase they spawned without a predetermined deathpoint.
				return Plugin_Continue;
			}
			new Float:offset = GetConVarFloat(FindConVar("mp_respawn_offset"));
			deathPoint[client][2] -= offset;

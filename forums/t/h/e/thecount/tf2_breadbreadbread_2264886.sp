...
public Plugin:myinfo =
{
	...
	author = "abrandnewday [NewCVar by TheCount]",
...
public OnPluginStart()
{
	...
	HookConVarChange(CreateConVar("sm_bread_alternate", "", "Optional different model"), ModelChange);
...
public ModelChange(Handle:convar, const String:oldValue[], const String:newValue[]){
	Format(model, sizeof(model), "%s", newValue);
	PrecacheModel(model);
}
...
public Action:Command_BreadBreadBread(client, args)
{
	if(StrEqual(model, "")){
		...
		SpawnMuchBread(client, model, 1);
	}
	return Plugin_Handled;
}

/* ----- dis spawns da bread ----- */
stock SpawnMuchBread(client, String:modele[], skin=0, num=1, Float:offsz = 30.0)
{

...
new String:g_Sounds_Explode[][] = {"weapons/explode1.mp3", "weapons/explode2.mp3", "weapons/explode3.mp3" };
...
new bool:g_bForcedUpdate = false;
...
public OnPluginStart()
{
	...
	//Engy Round Settings
	g_Cvar_EngyRound = CreateConVar("hh2_engychance", "50", "Chance of a Special Round being engies. Must Add up to 100 with huntsmanhell_medicchance!", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
//Once our configs are done executing, we can get started up in herea.
...
public OnLibraryAdded(const String:name[])
{
	...
	else if (StrEqual(name, "updater") && GetConVarBool(g_Cvar_AutoUpdate))
    {
		g_bUpdater = true;
    }
...
public Action Updater_OnPluginChecking (){
	...
	if(!GetConVarBool(g_Cvar_AutoUpdate) && !g_bForcedUpdate)
		return Plugin_Handled;
	
	g_bForcedUpdate = false;
...
{
	PrintToServer("%T", "Successful_Update", LANG_SERVER);
	if(GetConVarBool(g_Cvar_ReloadUpdate))
	{
		ReloadPlugin();
		PrintToServer("%T", "Reload_Plugin", LANG_SERVER);
	}
}

public Update(int client)
{
	g_bForcedUpdate = true;
	
	if(Updater_ForceUpdate())
		ReplyToCommand(client, "%T", "Update_Available", client);
	
	else
		ReplyToCommand(client, "%T", "Cant_Update", client);
...
public AdminMenu_MedicRound(Handle:hTopMenu, TopMenuAction:action, TopMenuObject:tmoObjectID, param, String:szBuffer[], iMaxLength) {
	...
	{
		if(!g_bSetMedicRound)
			Format(szBuffer, iMaxLength, "%T", "Force_Medic", LANG_SERVER);
		else
			Format(szBuffer, iMaxLength, "%T", "Cancel_Medic", LANG_SERVER);
	}
	else if (action == TopMenuAction_SelectOption)
	{
		if(!g_bSetMedicRound)
		{
			PrintToChat(param, "%T", "mRound_Enabled", LANG_SERVER);
			g_bSetMedicRound = true;
			g_bSetEngyRound = false;
		}
		else
		{
			PrintToChat(param, "%T", "mRound_Disabled", LANG_SERVER);
...
public AdminMenu_EngyRound(Handle:hTopMenu, TopMenuAction:action, TopMenuObject:tmoObjectID, param, String:szBuffer[], iMaxLength) {
	...
	{
		if(!g_bSetEngyRound)
			Format(szBuffer, iMaxLength, "%T", "Force_Engy", LANG_SERVER);
		else
			Format(szBuffer, iMaxLength, "%T", "Cancel_Engy", LANG_SERVER);
	}
	else if (action == TopMenuAction_SelectOption)
	{
		if(!g_bSetEngyRound)
		{
			PrintToChat(param, "%T", "eRound_Enabled", LANG_SERVER);
			g_bSetMedicRound = false;
			g_bSetEngyRound = true;
		}
		else
		{
			PrintToChat(param, "%T", "eRound_Disabled", LANG_SERVER);
			g_bSetEngyRound = false;
		}
	}
}
...
public AdminMenu_Updater(Handle:hTopMenu, TopMenuAction:action, TopMenuObject:tmoObjectID, param, String:szBuffer[], iMaxLength)
{
	...
	if (action == TopMenuAction_DisplayOption && g_bUpdater && GetConVarBool(g_Cvar_AutoUpdate))
	{
		Format(szBuffer, iMaxLength, "%T", "Check_Updater", LANG_SERVER);
	}
...
public Action:Command_MedicRound(client, args)
{
	...
	if(!g_bSetMedicRound)
	{
		PrintToChat(client, "%T", "mRound_Enabled", LANG_SERVER);
		g_bSetMedicRound = true;
		g_bSetEngyRound = false;
	}
	else
	{
		PrintToChat(client, "%T", "mRound_Disabled", LANG_SERVER);
		g_bSetMedicRound = false;
	}
}
...
public Action:Command_EngyRound(client, args)
{
	...
	if(!g_bSetEngyRound)
	{
		PrintToChat(client, "%T", "eRound_Enabled", LANG_SERVER);
		g_bSetMedicRound = false;
		g_bSetEngyRound = true;
	}
	else
	{
		PrintToChat(client, "%T", "eRound_Disabled", LANG_SERVER);
		g_bSetEngyRound = false;
	}
}
...
public Action:Command_Update (int client, int args){
	...
	if(!g_bUpdater || !GetConVarBool(g_Cvar_AutoUpdate)){
		...
		ReplyToCommand(client, "%T", "No_Updater", client);
		return Plugin_Handled;
...
public Action:CommandCallback(int client, const char[] command, int args)
{
	...
	{
		PrintToChat(client, "%T", "disabled_buildings");
		return Plugin_Handled;
	}
	return Plugin_Continue;
}

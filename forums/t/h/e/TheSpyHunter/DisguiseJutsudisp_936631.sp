...
#define MDL_WOODEN_BARREL "models/buildables/dispenser_lvl3.mdl"
...
stock Action:Event_FiredUser(Handle:event, const String:name[], any:client=0)
{
	...
	if(StrEqual(name, EVENT_PLUGIN_START))
	{
		...
		g_ChargeTime = CreateConVar("sm_rmf_disguisejutsu_charge_time","1.0","Pyro Power charge time (1.0-60.0)");
...
//		HookEntityOutput("func_regenerate",  "OnStartTouch",    EntityOutput_StartTouch);
	...
	if(StrEqual(name, EVENT_PLAYER_DATA_RESET))
	{
		...
		{
			Format(g_PlayerHintText[client][0], HintTextMaxSize , "%T", "DESCRIPTION_0_DISGUISEJUTSU", client);
			Format(g_PlayerHintText[client][1], HintTextMaxSize , "%T", "DESCRIPTION_1_DISGUISEJUTSU", client);
		}
		...
		if(TF2_GetItemDefIndex( TF2_GetCurrentWeapon(client) ) == _:ITEM_WEAPON_FLAMETHROWER )
		{
			...
			if( !g_IsBarrelActive[client]  )
			{
				...
				if( g_NextUseTime[client] > GetGameTime() )
				{
					...
					PrintToChat(client, "\x05%T", "CANT_USE_DISGUISEJUTSU", client, g_NextUseTime[client] - GetGameTime());
					return;
				}
...
public Action:Timer_PowerChargeTimer(Handle:timer, any:client)
{
	...
	if( IsClientInGame(client) && IsPlayerAlive(client))
	{
		PrintToChat(client, "\x05%T", "POWER_CHARGED_DISGUISEJUTSU", client);

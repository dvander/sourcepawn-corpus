...
int iGroup[MAXPLAYERS+1];
int iGloves[MAXPLAYERS+1];
float flFloatValue[MAXPLAYERS+1];
...
public void OnConfigsExecuted()
{
	...
	if (g_EnableWorldModel == INVALID_HANDLE)
...
void StopEmote(int client)
{
	...
	if (iEmoteEnt && iEmoteEnt != INVALID_ENT_REFERENCE && IsValidEntity(iEmoteEnt))
	{
		...
		if (Gloves_IsClientUsingGloves(client))
		{
			...
			iGroup[client] = GetEntProp(ent, Prop_Send, "m_iItemDefinitionIndex");
			iGloves[client] = GetEntProp(ent, Prop_Send, "m_nFallbackPaintKit");
			flFloatValue[client] = GetEntPropFloat(ent, Prop_Send, "m_flFallbackWear");
			...
			if(ent != -1)
			{
				...
				SetEntProp(ent, Prop_Send, "m_iItemDefinitionIndex", iGroup[client]);
				SetEntProp(ent, Prop_Send,  "m_nFallbackPaintKit", iGloves[client]);

				SetEntPropFloat(ent, Prop_Send, "m_flFallbackWear", flFloatValue[client]);

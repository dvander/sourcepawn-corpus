...
{	
	...
	AutoExecConfig(true, "fortnite_emotes_demo");
}
...
public Action Command_Menu(int client, int args)
{
	...
	{
		CReplyToCommand(client, "\x0E[Emotes]\x07 You must be alive to use this!");
		return Plugin_Handled;
	}
...
public Action CreateEmote(int client, const char[] anim1, const char[] anim2, const char[] soundName, bool isLooped)
{
	...
	{
		CReplyToCommand(client, "\x0E[Emotes]\x07 You must stay on the ground to use this!");
		return Plugin_Handled;
	}

	if (g_bEmoteCooldown[client])
	{
		CReplyToCommand(client, "\x0E[Emotes]\x07 It is on cooldown!");
		return Plugin_Handled;
	}

	if (StrEqual(anim1, ""))
	{
		CReplyToCommand(client, "\x0E[Emotes]\x07 Argument 1 is invalid!!!");
		return Plugin_Handled;
	}
	...
	{
		CReplyToCommand(client, "\x0E[Emotes]\x07 You can't use it right now!");
		return Plugin_Handled;
	}

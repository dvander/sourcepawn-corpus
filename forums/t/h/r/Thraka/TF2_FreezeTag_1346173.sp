...
bool:FreezePlayer(player)
{
	...
	if (IsClientInGame(player) && IsPlayerAlive(player) && Player_IsFrozen[player] == false)
	{
		...
		{
			flags &= FL_NOTARGET;
			SetEntProp(player, Prop_Data, "m_flags", flags);
		}
...
bool:ThawPlayer(player, bool:justClearingFlags = false)
{
	...
	if (IsClientInGame(player) && IsPlayerAlive(player) && Player_IsFrozen[player])
	{
...
public OnPluginStart()
{
	...
	HookConVarChange(cvar_ft_thawedNewHealthMod, ThawFrozeHealthMod_CVAR_Changed);
	HookConVarChange(cvar_ft_frozenNewHealthMod, ThawFrozeHealthMod_CVAR_Changed);
...
public ThawFrozeHealthMod_CVAR_Changed(Handle:convar, const String:oldValue[], const String:newValue[])
{
	CFG_ThawedHealthMod = GetConVarFloat(cvar_ft_thawedNewHealthMod);
	CFG_FrozenHealthMod = GetConVarFloat(cvar_ft_frozenNewHealthMod);
}

...
new ThawDuration = 15;
new Float:FrozenDamageModHeal = 0.6;
new Float:FrozenDamageModKill = 0.3;
...
new Float:ThawedHealthMod = 1.0;
new bool:CanFireWhileImmune = false;
new bool:MedicSavesUberWhenFrozen = true;
new bool:AllowClassChangeWhenFrozen = false;
...
new FrozenMedicUberCharges[MAXPLAYERS+1] = { 0, ... };
new bool:RoundEnd = false;
new TFClassType:OriginalClass[MAXPLAYERS+1];
//new ReFreezeHealth[MAXPLAYERS+1];
...
public OnPluginStart()
{
	...
	FrozenDamageModHeal = GetConVarFloat(cvar_ft_frozenDmgMod_heal);
	FrozenDamageModKill = GetConVarFloat(cvar_ft_frozenDmgMod_kill);
	...
	ThawedHealthMod = GetConVarFloat(cvar_ft_thawedNewHealthMod);
	MedicSavesUberWhenFrozen = GetConVarBool(cvar_ft_medicSavesUberCharge);
	AllowClassChangeWhenFrozen = GetConVarBool(cvar_ft_allowClassChangeFrozen);
	
	HookConVarChange(cvar_ft_thawDuration, ThawDuration_CVAR_Changed);
	HookConVarChange(cvar_ft_frozenDmgMod_heal, FrozenDmgMod_CVAR_Changed);
	HookConVarChange(cvar_ft_frozenDmgMod_kill, FrozenDmgMod_CVAR_Changed);
	...
	HookConVarChange(cvar_ft_thawedNewHealthMod, ThawedHealthMod_CVAR_Changed);
	HookConVarChange(cvar_ft_medicSavesUberCharge, ThawMedicUber_CVAR_Changed);
	HookConVarChange(cvar_ft_enabled, FreezeTag_CVAR_Changed);
	HookConVarChange(cvar_ft_allowClassChangeFrozen, ClassChangeFrozen_CVAR_Changed);
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype)
{
	...
	if (FrozenPlayers[victim] == false)
	{
		...
		if ((sameTeam && victim != attacker) || FrozenPlayers[attacker] || IsThawImmune[victim] || FreezeMeOnSpawn[victim])
		...
		if (TF2_GetPlayerClass(victim) == TFClass_Medic)
			FrozenMedicUberCharges[victim] =  TF_GetUberLevel(victim); 
	...
	else
	{
		...
		{
			new roundedDmg = RoundToNearest(damage * FrozenDamageModHeal);
		...
		{
			new roundedDmg = RoundToNearest(damage * FrozenDamageModKill);
...
public Action:Event_ChangeClass(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsFreezeEnabled)
	{
		...
		if (FreezeMeOnSpawn[client] == true || FrozenPlayers[client])
		{
			ReFreezeHealth[client] = GetClientHealth(client);
			
			if (!AllowClassChangeWhenFrozen)
				TF2_SetPlayerClass(client, OriginalClass[client]);
...
public Action:Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (IsFreezeEnabled)
	{
		...
		{
			FrozenMedicUberCharges[client] = 0;
			ThawPlayer(client);
			RemoveThawImmunity(client);
...
bool:FreezePlayer(player)
{
	if (IsClientInGame(player) && IsPlayerAlive(player) && FrozenPlayers[player] == false)
	{
		...
		//TODO: Remove this completel, is it needed since we are bonking them??
		//StripToMelee(player);
		TF2_StunPlayer(player, 100.0, 1.0, FREEZE_STATE);
		
		if (TF2_GetPlayerClass(player) == TFClass_Medic && MedicSavesUberWhenFrozen && FrozenMedicUberCharges[player] != 0)
			TF_SetUberLevel(player, FrozenMedicUberCharges[player]); 
...
public Action:FrozenPlayerCountdown(Handle:timer, any:client)
{
	...
	{	
		...
		if (FrozenPlayerTimerLeft[client] == 0)
		{
			...
			{
				SetEntityHealth(client, RoundToNearest(GetClientMaxHealth(client) * ThawedHealthMod));
...
public ThawedHealthMod_CVAR_Changed(Handle:convar, const String:oldValue[], const String:newValue[])
{
	ThawedHealthMod = GetConVarFloat(cvar_ft_thawedNewHealthMod);
}

public ThawMedicUber_CVAR_Changed(Handle:convar, const String:oldValue[], const String:newValue[])
{
	MedicSavesUberWhenFrozen = StrEqual(newValue, "1");
}

public ClassChangeFrozen_CVAR_Changed(Handle:convar, const String:oldValue[], const String:newValue[])
{
	AllowClassChangeWhenFrozen = GetConVarBool(cvar_ft_allowClassChangeFrozen);
}
...
{
	if (!IsFreezeEnabled || client == 0)
		return Plugin_Continue;

	else if (FrozenPlayers[client] || FreezeMeOnSpawn[client])
		return Plugin_Handled;

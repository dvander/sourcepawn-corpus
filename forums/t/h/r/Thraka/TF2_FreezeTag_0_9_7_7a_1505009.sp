 ...
 * 
 * Version 0.9.7.5
 * - Added new model for freeze tag!
 * 
 * Version 0.9.7.6
 * - FL_NOTARGET is fixed in Sourcemod which allows me to use it now.
 * - Removed code for the invisible model that blocked turret targetting.
 * - Added ft_use_finaldeath_respawntime and ft_finaldeath_respawntime instead of forcing instant respawn. Turn it on and set finaldeath_respawn time to 3 to simulate near instant.
 * 
 * Version 0.9.7.7a or 0.9.7.7b
 * - Version 'a' has all the model attatchment code removed
 * - Version 'b' has model attatchment code -which you'll never see ha!
 * 
 * ==================================
 * TODO:
 * ==================================
 ...
 * - Fix hooking players when plugin not enabled. Unhook when disabled.
...
new Handle:cvar_ft_finalDeathRespawnTime = INVALID_HANDLE;
new Handle:cvar_ft_useFinalDeathRespawn = INVALID_HANDLE;
...
new Float:CFG_FinalDeathRespawnTime = 2.0;
new bool:CFG_UseFinalDeathRespawnTime = true;
...
new Handle:Player_DeathRespawnTimerHandle[MAXPLAYERS+1] = {INVALID_HANDLE, ...};
...
public Action:Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (CFG_IsFreezeEnabled)
	{
		...
		if (Player_FreezeMeOnSpawn[client])
		{
			...
			CreateTimer(0.0, Timer_FreezePlayerTimer, client, TIMER_FLAG_NO_MAPCHANGE); 
		}
		else // Normal respawn
		{
			if (Player_DeathRespawnTimerHandle[client] != INVALID_HANDLE)
				KillTimer(Player_DeathRespawnTimerHandle[client]);
...
public Action:Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (CFG_IsFreezeEnabled)
	{
		...
		else if (Player_IsFrozen[client])
		{
			...
			if (CFG_UseFinalDeathRespawnTime)
				Player_DeathRespawnTimerHandle[client] = CreateTimer(CFG_FinalDeathRespawnTime, Timer_InstantSpawnPlayer, client, TIMER_FLAG_NO_MAPCHANGE); //Respawn the player at the specified time
		}
	}
}
...
bool:FreezePlayer(player)
{
	...
	if (IsClientInGame(player) && IsPlayerAlive(player) && Player_IsFrozen[player] == false)
	{
		...
		// Play the freeze sound, set the characters color
		new Float:vec[3];
...
CreateTurretBlocker(player)
{
	// Safely remove the blocked if already existed
	RemoveTurretBlocker(player);
	
	//FL_NOTARGET
	new playerFlags = GetEntityFlags(player) | FL_NOTARGET;
	SetEntityFlags(player, playerFlags);
}
...
public Action:Timer_InstantSpawnPlayer(Handle:timer, any:client) //From TF2 Respawn by WoZeR
{
    ...
    if (IsClientConnected(client) && IsClientInGame(client) && !IsPlayerAlive(client))
    {
        ...
        {
			Player_DeathRespawnTimerHandle[client] = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	cvar_ft_finalDeathRespawnTime = CreateConVar("ft_finaldeath_respawntime", "2.0", "If using final death respawn time, either this value, or the maps respawn time will be used to respawn. Whichever is shortest.", FCVAR_PLUGIN, true, 0.0, true, 10.0);
	cvar_ft_useFinalDeathRespawn = CreateConVar("ft_use_finaldeath_respawntime", "1", "Use the final death respawn time in addition to map respawn time.", FCVAR_PLUGIN);
	...
	CFG_FinalDeathRespawnTime = GetConVarFloat(cvar_ft_finalDeathRespawnTime);
	CFG_UseFinalDeathRespawnTime = GetConVarBool(cvar_ft_useFinalDeathRespawn);
	...
	HookConVarChange(cvar_ft_finalDeathRespawnTime, CVAR_Changed);
	HookConVarChange(cvar_ft_useFinalDeathRespawn, CVAR_Changed);
...
public CVAR_Changed(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	else if (convar == cvar_ft_finalDeathRespawnTime)
	{
		CFG_FinalDeathRespawnTime = GetConVarFloat(cvar_ft_finalDeathRespawnTime);
	}
	else if (convar == cvar_ft_useFinalDeathRespawn)
	{
		CFG_UseFinalDeathRespawnTime = GetConVarBool(cvar_ft_useFinalDeathRespawn);
	}
}
...
public Action:Cmd_Freeze(client, args) 
{
	...
	for (new i = 0; i < target_count; i++)
	{
		GetClientAbsOrigin(target_list[i], Player_FrozenOrigin[target_list[i]]);
		GetClientAbsAngles(target_list[i], Player_FrozenAngle[target_list[i]]);

...
{
	name = "[TF2] VIP Prices Menu",
	author = "Teamkiller324",
	description = "Displays a VIP Prices Menu to Players",
	version = PLUGIN_VERSION,
	url = "http://www.steamcommunity.com/id/Teamkiller324"
};

public OnPluginStart() {
	CreateConVar("sm_vipprices_version", PLUGIN_VERSION, "VIP Menu", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	g_cvarWelcome = CreateConVar("sm_vipprices_welcome", "1", "Show welcome message to newly connected users.", FCVAR_PLUGIN);
	g_cvarAdmins = CreateConVar("sm_vipprices_admins", "1", "Show a list of online admins in the menu.", FCVAR_PLUGIN);
	RegConsoleCmd("sm_vipprices", Command_HelpMenu, "Display the help menu.", FCVAR_PLUGIN);

	new String:hc[PLATFORM_MAX_PATH];
	BuildPath(Path_SM, hc, sizeof(hc), "configs/vipprices.cfg");
...
public Action:Timer_WelcomeMessage(Handle:timer, any:client) {
	if (GetConVarBool(g_cvarWelcome) && IsClientConnected(client) && IsClientInGame(client) && !IsFakeClient(client))
		PrintToChat(client, "\x01[VIP Prices Menu] For Server info, type \x04!vipprices\x01 in chat");
}
...
Help_ShowMainMenu(client) {
	...
	SetMenuTitle(menu, "VIP Prices Menu ");
	new msize = GetArraySize(g_helpMenus);
	new hmenu[HelpMenu];

...
public Plugin:myinfo =
{
	...
	description = "Displays a VIP Menu to Players",
	version = PLUGIN_VERSION,
	url = "http://www.steamcommunity.com/id/Teamkiller324"
};

public OnPluginStart() {
	CreateConVar("sm_vipmenu_version", PLUGIN_VERSION, "VIP Menu", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	g_cvarWelcome = CreateConVar("sm_vipmenu_welcome", "1", "Show welcome message to newly connected users.", FCVAR_PLUGIN);
	g_cvarAdmins = CreateConVar("sm_vip_admins", "1", "Show a list of online admins in the menu.", FCVAR_PLUGIN);
	RegConsoleCmd("sm_vip", Command_HelpMenu, "Display the vip menu.", FCVAR_PLUGIN);
...
public Action:Timer_WelcomeMessage(Handle:timer, any:client) {
	if (GetConVarBool(g_cvarWelcome) && IsClientConnected(client) && IsClientInGame(client) && !IsFakeClient(client))
		PrintToChat(client, "\x01[VIP Menu] For Server info, type \x04!vip\x01 in chat");
}

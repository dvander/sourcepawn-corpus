...
#define SB_VERSION "1.4.11.TOG_EDIT_d"
...
new const String:Prefix[] = "[TOG SourceBans] ";
...
{
	name = "TOG SourceBans",
	author = "SourceBans Development Team, That One Guy",
...
public OnPluginStart()
{
	...
	if(!SQL_CheckConfig("sourcebans"))
	{
		...
		LogToFile(logFile, "g_hDatabase failure: Could not find g_hDatabase conf \"sourcebans\". See FAQ: http://sourcebans.net/node/19");
		SetFailState("g_hDatabase failure: Could not find g_hDatabase conf \"sourcebans\"");
		return;
	}
...
public Native_CheckBans(Handle:plugin, numParams)
{
	...
	if(sAuth[0] == 'B' || sAuth[9] == 'L' || g_hDatabase == INVALID_HANDLE)
	{
...
#endif
	SQL_TQuery(g_hDatabase, VerifyBan, sQuery, GetClientUserId(client), DBPrio_High);
	...
	ResetPack(datapack);
	SQL_TQuery(g_hDatabase, SQL_CheckIP, sQuery, datapack);*/
}
...
public SQL_CheckIP(Handle:owner, Handle:hndl, const String:error[], any:datapack)
{
	...
	{
		LogError("TOG Sourcebans: g_hDatabase query error: %s", error);
	}
	...
	if (SQL_FetchRow(hndl))
	{
		...
		{
			Format(sReason, sizeof(sReason), "[TOG Sourcebans] Ban evasion - IP ban.");
...
public OnRebuildAdminCache(AdminCachePart:part)
{
	...
	if(g_hDatabase == INVALID_HANDLE) {
		...
		GotDatabase(g_hDatabase,g_hDatabase,"",0);
	}
}
...
public Action:CommandBanIp(client, args)
{
	...
	SQL_TQuery(g_hDatabase, SelectBanIpCallback,  Query, dataPack, DBPrio_High);
	return Plugin_Handled;
}
...
public Action:CommandUnban(client, args)
{
	...
	SQL_TQuery(g_hDatabase, SelectUnbanCallback, query, dataPack);
	return Plugin_Handled;
}
...
public GotDatabase(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	if (hndl == INVALID_HANDLE)
	{
		LogToFile(logFile, "g_hDatabase failure: %s. See FAQ: http://www.sourcebans.net/node/20", error);
		g_bConnecting = false;
	...
	if(loadOverrides)
	{
		...
		SQL_TQuery(g_hDatabase, OverridesDone, query);
		loadOverrides = false;
	}
...
public Action:CheckFamSharing(Handle:Timer, Handle:data)
{
	...
	if(client != -1)
	{
		...
		Format(sBuffer, sizeof(sBuffer), "[TOG SB] Player \"%s\" (%s) has been verified with game owner's Steam ID: %s", sName, sSteamID, g_sOwnerSteamID[client]);
		...
		if(!StrEqual(sSteamID, sOwnerSteamID, false))
		{
			...
			if(g_iLogFamilySharing)
			{
				...
				BuildPath(Path_SM, sSharingLogPath, sizeof(sSharingLogPath), "logs/tog_SB_familysharing.log");
...
CheckForBanEvasion(client, String:sAuth[])
{
	/* Do not check bots nor check player with lan steamid. */
	if(sAuth[0] == 'B' || sAuth[9] == 'L' || g_hDatabase == INVALID_HANDLE)
	{
	...
	GetClientIP(client, sIP, sizeof(sIP));
	FormatEx(sQuery, sizeof(sQuery), "SELECT bid FROM %s_bans WHERE ((type = 0 AND authid REGEXP '^STEAM_[0-9]:%s$') OR (type = 1 AND ip = '%s')) AND (length = '0' OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL", DatabasePrefix, sAuth[8], sIP);
	SQL_TQuery(g_hDatabase, VerifyBan2, sQuery, GetClientUserId(client), DBPrio_High);
}
...
public VerifyBan2(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	if(SQL_GetRowCount(hndl) > 0)
	{
		...
		SQL_EscapeString(g_hDatabase, clientName, Name, sizeof(Name));
		if(serverID == -1 )
		{
		...
		SQL_TQuery(g_hDatabase, ErrorCheckCallback, Query, client, DBPrio_High);
		...
		FormatEx(buffer, sizeof(buffer), "[TOG Sourcebans] Ban evasion for Steam ID: %s", clientAuth);
...
public SelectBanIpCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	SQL_TQuery(g_hDatabase, InsertBanIpCallback, Query, data, DBPrio_High);
}
...
public SelectUnbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if(hndl != INVALID_HANDLE && SQL_FetchRow(hndl))
	{
		...
		SQL_TQuery(g_hDatabase, InsertUnbanCallback, query, data);
	}
	return;
}
...
public SelectAddbanCallback(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	SQL_TQuery(g_hDatabase, InsertAddbanCallback, Query, data, DBPrio_High);
}
...
public bool:CreateBan(client, target, time, String:reason[])
{
	...
	Format(reason2, sizeof(reason2), "[TOG Sourcebans] Family Sharing Owner Ban for Steam ID: %s", auth);
...
stock UTIL_InsertBan(time, const String:Name[], const String:Authid[], const String:Ip[], const String:Reason[], const String:AdminAuthid[], const String:AdminIp[], Handle:Pack)
{
	...
	SQL_TQuery(g_hDatabase, VerifyInsert, Query, Pack, DBPrio_High);
}

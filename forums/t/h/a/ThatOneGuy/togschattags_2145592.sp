...
public OnPluginStart()
{
	...
	AutoExecConfig_CreateConVar("tct_version", PLUGIN_VERSION, "TOGs Chat Tags: Version", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	
	LoadTranslations("core.phrases");
	LoadTranslations("common.phrases");
	
	g_hEnabled = AutoExecConfig_CreateConVar("tct_enable", "1", "Enable plugin (0 = Disabled, 1 = Enabled).", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	HookConVarChange(g_hEnabled, OnCVarChange);
	g_iEnabled = GetConVarInt(g_hEnabled);
	
	g_hAdminSeeAll = AutoExecConfig_CreateConVar("tct_seeall", "1", "Enable admin see-all (admins with set flag see even hidden chat).", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	HookConVarChange(g_hAdminSeeAll, OnCVarChange);
	g_iAdminSeeAll = GetConVarInt(g_hAdminSeeAll);
	
	g_hAdminSeeAllFlag = CreateConVar("tct_seeallflag", "z", "Flag required for admin see-all (if enabled).", FCVAR_PLUGIN);
	...
	g_hConvertTriggerCases = AutoExecConfig_CreateConVar("tct_triggercase", "1", "Convert chat triggers to lowercase if theyre uppercase.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	HookConVarChange(g_hConvertTriggerCases, OnCVarChange);
	g_iConvertTriggerCases = GetConVarInt(g_hConvertTriggerCases);
	
	g_hAccessFlag = CreateConVar("tct_accessflag", "a", "If \"\", everyone can change their tags, otherwise, only players with this flag can access plugin features.", FCVAR_PLUGIN);
	HookConVarChange(g_hAccessFlag, OnCVarChange);
	GetConVarString(g_hAccessFlag, g_sAccessFlag, sizeof(g_sAccessFlag));
	
	g_hAdminFlag = CreateConVar("tct_adminflag", "g", "Only players with this flag can restrict/remove tags of players.", FCVAR_PLUGIN);
	HookConVarChange(g_hAdminFlag, OnCVarChange);
	GetConVarString(g_hAdminFlag, g_sAdminFlag, sizeof(g_sAdminFlag));
	
	g_hAdminUnloadFlag = CreateConVar("tct_unloadflag", "h", "Only players with this flag can unload the entire plugin until map change.", FCVAR_PLUGIN);
	...
	g_hLog = AutoExecConfig_CreateConVar("tct_log", "1", "Enable chat logger.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	HookConVarChange(g_hLog, OnCVarChange);
	g_iLog = GetConVarInt(g_hLog);
	
	g_hSpamDuration = AutoExecConfig_CreateConVar("tct_spamduration", "5", "Number of seconds used for the spam protection (0 = disabled). If messages sent exceeds tct_spamcount within this time frame, it blocks them.", FCVAR_PLUGIN, true, 0.0);

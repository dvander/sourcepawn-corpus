...
new Handle:BhopCrouchSwitch = INVALID_HANDLE;
new Handle:DoubleJumpBhop = INVALID_HANDLE;
...
public OnPluginStart()
{
    ...
    DoubleJumpBhop = CreateConVar("rfbhop_djump_bhop", "false", "Wheather or not to apply bhop boost on doublejumps", FCVAR_PLUGIN);
    BhopCrouchSwitch = CreateConVar("rfbhop_allowcrouch", "false", "wheather or not to allow crouching while bunnyhopping", FCVAR_PLUGIN);
    ...
    HookConVarChange(DoubleJumpBhop, OnDoubleJumpBhopChanged);
    HookConVarChange(BhopCrouchSwitch, OnBhopCrouchSwitchChanged);
...
public OnDoubleJumpBhopChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
    bhopOnDoubleJump = GetConVarBool(DoubleJumpBhop);
}

public OnBhopCrouchSwitchChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
    crouchAllowed = GetConVarBool(BhopCrouchSwitch);
}
...
{    
    ...
    {       
        ...
        if (bhopOnDoubleJump && (TF2_GetPlayerClass(client) == TFClass_Scout))
        {
            if ((GroundEntity == -1) && (buttons & IN_JUMP))
            {
                numJumps[client]++;
                if (numJumps[client] == 2)
                {
                    jumpEventThisFrame[client] = true;
                }
            }
            else if (GroundEntity != 1)
            {
                numJumps[client] = 0;
            }
        }
        
        new bool:crouchOkay = false;
        if (crouchAllowed)
        {
            crouchOkay = true;
        }
        ...
        if ((buttons & IN_JUMP) && (crouchOkay) && !(buttons & IN_FORWARD) && !(buttons & IN_BACK) && !(wasJumpKeyPressed[client]))
        {	

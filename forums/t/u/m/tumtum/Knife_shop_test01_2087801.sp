...
// Do not remove or edit the lines below... Thank you!
...
public Action:SetCredits(client, args)
{
    ...
    { 
        ...
        { 
              ...
              PrintToChat(client, "[Knife Shop] Set \x03%i \x01credits in the player %N", amount, iClient);
        } 
    } 
...
public Action:PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetConVarInt(cvarSpawnMenu) == 1)
	{
		DID(client);
	}
	...
	switch(weapon_choose[client]) {
		case 1: if(IsValidEntity(currentknife) && currentknife != INVALID_ENT_REFERENCE) {RemovePlayerItem(client, currentknife), knife = GivePlayerItem(client, "weapon_bayonet");}
		case 2: if(IsValidEntity(currentknife) && currentknife != INVALID_ENT_REFERENCE) {RemovePlayerItem(client, currentknife), knife = GivePlayerItem(client, "weapon_knife_gut");}
		case 3: if(IsValidEntity(currentknife) && currentknife != INVALID_ENT_REFERENCE) {RemovePlayerItem(client, currentknife), knife = GivePlayerItem(client, "weapon_knife_flip");}
		case 4: if(IsValidEntity(currentknife) && currentknife != INVALID_ENT_REFERENCE) {RemovePlayerItem(client, currentknife), knife = GivePlayerItem(client, "weapon_knife_m9_bayonet");}
		case 5: if(IsValidEntity(currentknife) && currentknife != INVALID_ENT_REFERENCE) {RemovePlayerItem(client, currentknife), knife = GivePlayerItem(client, "weapon_knife_karambit");}
		default: {return;}
	}
	EquipPlayerWeapon(client, knife);
}

...
public Plugin:myinfo = {
	...
	author      = "Dr. McKay - Edit by TumTum",
...
public Action:Command_StopMusic(client, args)
{
	...
	PrintToChat(client, "[SMDJ] Checking and stopping map music.");
...
public Action:Command_SongList(client, args) {
	...
	Format(url, sizeof(url), "http://www.team-secretforce.com/radio/shortcut/webshortcuts.html?web=http://%s:%i%s/index.php?repeat=%s", parts[0], GetConVarInt(djUrlPortCvar), path, repeat);
...
public Action:Command_MusicMenu(client, args) {
	...
	Command_StopMusic(client, 0);
	new Handle:menu = CreateMenu(Handler_TopMenu);
	SetMenuTitle(menu, "SourceMod DJ CS:GO");
	...
	AddMenuItem(menu, "musichelp", "Music Help");
...
public Handler_TopMenu(Handle:menu, MenuAction:action, client, param) {
	if(action == MenuAction_Select) {
		...
		if(StrEqual(selection, "repeat")) {
			...
			PlaySong(client, -2);
		} else if(StrEqual(selection, "musichelp")) {
	decl String:url[256];
	GetConVarString(djUrlCvar, url, sizeof(url));
	ReplaceString(url, sizeof(url), "http://", "", false);
	ReplaceString(url, sizeof(url), "https://", "", false);
	...
	Format(url, sizeof(url), "http://www.team-secretforce.com/radio/shortcut/webshortcuts.html?web=http://%s:%i%s/help.php", parts[0], GetConVarInt(djUrlPortCvar), path);
...
public Handler_Playlist(Handle:menu, MenuAction:action, client, param) {
	...
	if(StrEqual(selection, "newplaylist")) {
		...
		CPrintToChat(client, "Please type the name for your {green}playlist {default}in this chat and press Enter.");
		return;
...
PlaySong(client, index, bool:playlist = false, bool:silent = false) {
	...
	CPrintToChat(client, "{green}[SMDJ] {default}Type {olive}!musicoff {default}to stop the music and {olive}!musichelp {default}for more info.");
	if(index == -2) {

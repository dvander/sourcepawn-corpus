...
#define FragColor 	{255,0,0,225}
#define FlashColor 	{0,255,255,225}
#define SmokeColor	{0,255,0,225}
#define IncColor	{255,130,0,225}
...
public Action:projectile(Handle:timer, Handle:datapack)
{
	...
	if(0 < m_hThrower <= MaxClients && g_bIsActive[m_hThrower])
	{
		...
		int team = GetClientTeam(m_hThrower);
		...
		else if(StrContains(classname, "smokegrenade", false) != -1)
		{
		...
		{
			TE_SetupBeamFollow(entity, g_iBeamSpriteIndex,	0, Float:1.0, Float:3.0, Float:3.0, 1, IncColor);
			TE_SendToTeam(team);	
		}
	}
	return Plugin_Continue;
...
void TE_SendToTeam(int team, float delay=0.0)
{
	int total = 0;

...
public Plugin:myinfo =
{
	...
	author = "Crimson_Fox, dmg fix by raziEiL[disawar1]",
...
public OnPluginStart()
{
	...
	h_AWPBoost = CreateConVar("l4d2_wu_awpboost", "115", "Amount of bonus dmg added to AWP sniper rifle (default dmg 115 + 135 bonus).", FCVAR_PLUGIN)
	h_MP5Boost = CreateConVar("l4d2_wu_mp5boost", "12", "Amount of damage added to MP5 submachine gun.", FCVAR_PLUGIN)
	h_SG552Boost = CreateConVar("l4d2_wu_sg552boost", "17", "Amount of damage added to SG552 rifle.", FCVAR_PLUGIN)
	h_ScoutBoost = CreateConVar("l4d2_wu_scoutboost", "110", "Amount of bonus damage added to scout sniper rifle (default SS dmg 90 + 110 bonus)", FCVAR_PLUGIN)
...
public Action:Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		damage = GetConVarInt(h_AWPBoost)
		CreateTimer(0.01, dmg, target)
	}
	if (StrEqual(weapon, "sniper_scout"))
	{
		damage = GetConVarInt(h_ScoutBoost)
		CreateTimer(0.01, dmg, target)
	}
	if (StrEqual(weapon, "smg_mp5"))
	{
		damage = GetConVarInt(h_MP5Boost)
		CreateTimer(0.01, dmg, target)
	}
	if (StrEqual(weapon, "rifle_sg552"))
	{
		damage = GetConVarInt(h_SG552Boost)
		CreateTimer(0.01, dmg, target)
	}
}

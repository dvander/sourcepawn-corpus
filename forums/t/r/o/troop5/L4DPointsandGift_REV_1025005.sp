...
public OnPluginStart()
{
	...
	pointsresetround = CreateConVar("points_reset_round","1","Reset points when a certain amount of rounds end? Resets at end of campaign in coop, a defined amount rounds in versus, and every round in survival.",CVAR_FLAGS, true, 0.0);
	...
	explosivepoints = CreateConVar("points_price_explosive","10","How many points the explosive bullets upgrade costs.",CVAR_FLAGS, true, -1.0);
	...
	laserpoints = CreateConVar("points_price_special_laser","20","How many points the laser sight costs?",CVAR_FLAGS, true, -1.0);
			...
			if (pointsteam[attacker] != SURVIVORTEAM)
			{
				if (pointsteam[client] == SURVIVORTEAM)
				{
					if(pointson)
					{
						...
						{
							PrintToChat(attacker, "\x05[SM]\x01 Hurt Survivors Five Times: \x03%d\x01 Point(s)",GetConVarInt(pointshurt));
...
public Action:PointsSpecialMenuFunc(clientId) {
	...
	{
	AddMenuItem(menu, "option1", "Refill");
	}
	if ( GetConVarInt(laserpoints) > -1 )
	{
	AddMenuItem(menu, "option2", "Laser Sight");
	}
	if ( GetConVarInt(explosivepoints) > -1 )
	{
	AddMenuItem(menu, "option3", "Explosive Ammo");
	}
	if ( GetConVarInt(burnpoints) > -1 )
	{
	AddMenuItem(menu, "option4", "Incendiary Ammo");
	}
	SetMenuExitButton(menu, true);
	DisplayMenu(menu, clientId, MENU_TIME_FOREVER);
...
public Action:RepeatBuy(client, args)
{
	...
	switch(buyitem[client])
	{
		...
		case 22: //laser sight
		{
			...
			{
				//Give the player the laser sight
...
public PointsChooseMenuHandler(Handle:menu, MenuAction:action, client, itemNum)
{
	...
	if ( action == MenuAction_Select ) 
	{
        if (StrEqual(gamemode,"survival",true))
		{
			...
			{
				PrintToChat(client,"\x05[SM]\x01 Sorry! Points spending has been disabled");
			}
			else
			{
				switch (itemNum)
        ...
        if (StrEqual(gamemode,"scavenger",true))
		{
        ...
        if (StrEqual(gamemode,"coop",true))
		{
        ...
        if (StrEqual(gamemode,"realism",true))
		{

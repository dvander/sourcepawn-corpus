...
Action Command_TeamDetails(int client, int args)	{
...
int Handle_DetailsMenu(Menu menu, MenuAction action, int client, int selection)   {
	switch (action)	{
		case MenuAction_Select:	{
			switch (selection)	{
				case 0: CashInfo(client);
				case 1: WeaponsInfo(client);
			}
		}
...
int Handle_BackMenu(Menu menu, MenuAction action, int client, int selection)   {
	switch (action)	{
		case MenuAction_End: delete menu;
		case MenuAction_Cancel:	Command_TeamDetails(client, -1);
	}
}

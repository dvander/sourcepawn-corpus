...
public void OnPluginStart()
{
	...
	//Hook our cvar
	CvarAdmFlag.AddChangeHook(Cvar_Change);
	
	//DB stuff
	if (SQL_CheckConfig("whitelister")) Database.Connect(OnDatabaseConnect, "whitelister");
	else SetFailState("Can't find 'whitelister' entry in sourcemod/configs/databases.cfg!");
}

void OnDatabaseConnect(Database database, const char[] error, any data)
{
	...
	db = database; //Set global DB Handle
...
void Cvar_Change(ConVar cvar, const char[] oldVal, const char[] newVal)
{
	AdmFlag = ReadFlagString(newVal);
}

Action Main(int client, int args)
{
	ShowMain(client);
...
int MainMenuHandler(Menu MainMenu, MenuAction action, int client, int itemNum)
{
	if(!IsValidClient(client)) return;
...
void SQL_CheckSteamID(Database database, DBResultSet results, const char[] error, any userid)
{
...
void SQL_ListSteamids(Database database, DBResultSet results, const char[] error, any userid)
{
	int client = GetClientOfUserId(userid);
...
void SQL_AddSteamid_Check(Database database, DBResultSet results, const char[] error, DataPack data)
{
	//Unpack our Infos
	data.Reset();
...
void SQL_AddSteamid_Add(Database database, DBResultSet results, const char[] error, DataPack data)
{
	//Unpack our Infos
	data.Reset();
	int client = GetClientOfUserId(data.ReadCell());
...
Action Server_Command_Add(int args)
{
	if (args < 1)
	{
...
void SQL_Server_AddSteamid_Check(Database database, DBResultSet results, const char[] error, DataPack data)
{
	//Unpack our Infos
	data.Reset();
	char steamid[64]; //SteamId
...
void SQL_Server_AddSteamid_Add(Database database, DBResultSet results, const char[] error, DataPack data)
{
	//Unpack our Infos
	data.Reset();
	char steamid[64]; //SteamId
...
Action Command_Delete(int client, int args)
{
	//Check if client is valid, do nothing if not!
	if (!IsValidClient(client)) return Plugin_Handled;
...
void SQL_DeleteSteamid_Check(Database owner, DBResultSet results, const char[] error, DataPack data)
{
	//Unpack our Infos
	data.Reset();
	int client = GetClientOfUserId(data.ReadCell());
...
Action Server_Command_Delete(int args)
{
	if (args < 1)
	{
...
//Querys the database to see if a SteamID exists, if it exists in the database it will delete the SteamID from the database.
void SQL_Server_DeleteSteamid_Check(Database database, DBResultSet results, const char[] error, DataPack data)
{
	//Unpack our Infos
	data.Reset();
	char fsteam[64]; //SteamId
...
//Deletes the SteamID from the database, and kicks the client if active on the server.
void SQL_Server_DeleteSteamid_Delete(Database database, DBResultSet results, const char[] error, DataPack data)
{
	//Unpack our Infos
	data.Reset();
	char fsteam[64]; //SteamId
...
public void SQL_ErrorCheckCallback(Database database, DBResultSet results, const char[] error, any data)
{
	if (results == null || strlen(error) > 0)
	{
		LogError("[Mysql-Whitelist] Query failed! %s", error);

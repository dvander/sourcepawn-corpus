/*
v1.1 release
v1.2 added ability to select which teams to display to. Included future 
*/
#pragma semicolon 1
#include <sourcemod>
#include <sdktools>
...
{
	name = "[L4D2]Automatic Main Menu Opener AMMO",
	author = "WeAreBorg",
...
new Handle:hTeams = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	hCommand = CreateConVar("AMMOpener_Command", "sm_buy", "Command, minus !, used to open main menu. 'sm_' will be removed when announced to players", FCVAR_PLUGIN|FCVAR_NOTIFY);
	hMenu = CreateConVar("AMMOpener_MenuName", "=Buy Menu=", "Menu Title",FCVAR_PLUGIN|FCVAR_NOTIFY);
	...
	hTeams = CreateConVar("AMMOpener_Teams", "4", "Announce to.. 1=spec only 2=survivors only 3=infected only 4=non spec only 5=all",FCVAR_PLUGIN|FCVAR_NOTIFY);
...
public Action:RunOnPlayerSpawnFirst(Handle:event, String:event_name[], bool:dontBroadcast)
{
	...
	{
		if(!IsFakeClient(target) && TeamCheck(target))
		{
...
public Action:RunOnPlayerSpawn(Handle:event, String:event_name[], bool:dontBroadcast)
{
	...
	{
		/* Create First Announce Timer */
		if(!IsFakeClient(target) && GetConVarInt(hEnableEST) == 1 && TeamCheck(target))
		{
...
TeamCheck(client)
{
	new allowed = GetConVarInt(hTeams);
	new team = GetClientTeam(client); 
	if ((team == allowed) || (allowed == 5)) return true;
	if (allowed == 4 && ((team == 2) || (team == 3))) return true;
	return false;
}
...
public Action:TimerAnnounceFirst(Handle:timer, any:client)
{
	...
	{
		if(!IsFakeClient(client) && TeamCheck(client))
		{
...
public Action:TimerChatRemind(Handle:timer, any:client)
{
	if(client > 0 && IsClientInGame(client))
	{
		if(TimerCount[client] < GetConVarInt(hRCount) && !IsFakeClient(client) && TeamCheck(client))
		{
			TimerCount[client]++;
			OpenMainMenuMsg(client); //Remind player to open Main Menu
		}
		else
		{
			return Plugin_Stop;
...
OpenMainMenuMsg(client)
{
	...
	PrintToChat(client, "\x04[AMMO] \x03Type \x04!%s\x03 in chat to bring up the %s", command, menu);
}
...
WelcomeMsg(client)
{
	...
	GetConVarString(hMenu, menu, sizeof(menu));
	PrintToChat(client, "\x04[AMMO] \x03Hello %N! Welcome to \x04%s \x03", client, server);
	PrintToChat(client, "\x04[AMMO] \x03Type \x04!%s\x03 in chat to bring up the \x04%s\x03", command, menu);
}
...
new Handle:g_MenuList[MAXPLAYERS + 1];
...
public OnPluginStart()
{
	...
	HookEventEx("round_start", Menu_Display, EventHookMode_Post)
    }
...
public Menu_Display(Handle:event,const String:name[],bool:dontBroadcast)
{
	 for(new i = 1; i <= GetMaxClients(); i++)
	 {
	       if (g_MenuList[i] != INVALID_HANDLE && IsClientInGame(i))
	       {
                   if (GetClientTeam(i) == 3)
                   {
                       CloseHandle (g_MenuList[i])
                       new blank
                       command_grabber ((i),blank) 
                   }    
	       }
        }
}

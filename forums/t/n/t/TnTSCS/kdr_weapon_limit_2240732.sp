...
RestrictWeapons(client, msg)
{
	...
	{
		#if _DEBUG
			DebugMessage(client, "Dropping secondary slot.");
		#endif
	...
	{
		#if _DEBUG
			DebugMessage(client, "Dropping primary slot.");
		#endif
		DropRestrictedWeapons(client, weapon);
		Primary[client] = true;
	}
...
DropRestrictedWeapons(client, weapon)
{
		...
		(RestrictPlayer1[client] && StrContains(RestrictedWeapons1, WeaponName, false) != -1))
	{
		...
		{
			#if _DEBUG
				DebugMessage(client, "Emitting drop sound to player.");
			#endif
			EmitSoundToClient(client, SOUND_FILE);
			#if _DEBUG
				DebugMessage(client, "Emitted drop sound to player.");
			#endif
		}
	}
}
...
GiveReplacementWeapon(client, const String:weapon_name[])
{
	#if _DEBUG
		dmsg[0] = '\0';
		Format(dmsg, sizeof(dmsg), "[GiveReplacementWeapon] Running GiveReplacementWeapon for %L", client);
		DebugMessage(client, dmsg);
	#endif
	...
	else
	{
		if (SwitchToNewWeapon)
		{
			#if _DEBUG
				dmsg[0] = '\0';
				Format(dmsg, sizeof(dmsg), "[GiveReplacementWeapon] About to GivePlayerItem");
				DebugMessage(client, dmsg);
			#endif

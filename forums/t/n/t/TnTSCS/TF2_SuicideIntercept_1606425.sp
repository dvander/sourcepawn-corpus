...
new 	Handle:ClientTimer[MAXPLAYERS+1] = INVALID_HANDLE,
		...
		Float:DelaySpecTime,
		Float:DelayDeathTime,
		Float:DelayJointeamTime,

		bool:BlockSuicides = false,
		bool:BlockSpectate = false,
		bool:BlockJointeam = false,

		bool:MoveToSpec[MAXPLAYERS+1] = false,
		bool:JoinTeam[MAXPLAYERS+1] = false,
		bool:JoinClass[MAXPLAYERS+1] = false;
...
public OnPluginStart()
{
	...
	"Version of 'Suicide Intercept'", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY)), OnVersionChanged);
...
public Action:Command_InterceptSuicide(client, const String:command[], args)
{
	...
	if(client == 0 || !IsPlayerAlive(client) || IsFakeClient(client) || GetClientTeam(client) < 2)
	{
		return Plugin_Continue;
	}
	
	if(CheckCommandAccess(client, "bypass_suicideintercept", ADMFLAG_ROOT))
	{
		return Plugin_Continue;
	}
...
stock HandleRequests(client, RequestType)
{
	if(IsClientInGame(client))
	{
		switch (RequestType)

...
public OnPluginStart()
{
	...
	"List the primary weapon to give the player if the reach ratio1 and are forced to drop their weapon")), PrimaryReplacementOneChanged);
	Replacement1[0] = '\0';
	GetConVarString(hRandom, Replacement1, sizeof(Replacement1));
	
	HookConVarChange((hRandom = CreateConVar("sm_kdrwl_sw1", "weapon_p228", 
	"List the secondary weapon to give the player if the reach ratio1 and are forced to drop their pistol")), SecondaryReplacementOneChanged);
	Replacement1a[0] = '\0';
	...
	"List the primary weapon to give the player if the reach ratio2 and are forced to drop their weapon")), PrimaryReplacementTwoChanged);
	Replacement2[0] = '\0';
	GetConVarString(hRandom, Replacement2, sizeof(Replacement2));
	
	HookConVarChange((hRandom = CreateConVar("sm_kdrwl_sw2", "weapon_p228", 
	"List the secondary weapon to give the player if the reach ratio2 and are forced to drop their pistol")), SecondaryReplacementTwoChanged);
	Replacement2a[0] = '\0';
...
{
	if (!Enabled || !IsClientInGame(client) || RestrictionExpempted[client] || !Restricted[client] || GetClientTeam(client) <= CS_TEAM_SPECTATOR)
	{
...
CheckKDR(client)
{
	...
	new bonus_points = (DefuseBombPoints[client] + BombExplodePoints[client] + BotKills[client]);
	frags -= bonus_points;
...
RestrictWeapons(client, msg)
{
	...
	switch (msg)
	{
		case 1: 
		{
			CPrintToChat(client, "{green}[{lightgreen}SM{green}] %t", "Restrict", KDR[client], RatioLimit1, RestrictedWeapons1);
			if (Announce)
			{
				for (new i = 1; i <= MaxClients; i++)
				{
					...
					{
						CPrintToChat(i, "{green}[{lightgreen}SM{green}] %t", "RestrictAnnounce", client, KDR[client], RatioLimit1, RestrictedWeapons1);
					}
				}
			}
		}
		case 2: 
		{
			CPrintToChat(client, "{green}[{lightgreen}SM{green}] %t", "Restrict", KDR[client], RatioLimit2, RestrictedWeapons2);
			if (Announce)
			{
				for (new i = 1; i <= MaxClients; i++)
				{
					if (IsClientInGame(i) && i != client)
					{
						CPrintToChat(i, "{green}[{lightgreen}SM{green}] %t", "RestrictAnnounce", client, KDR[client], RatioLimit2, RestrictedWeapons2);
					}
				}
			}
		}
...
DropRestrictedWeapons(client, weapon)
{
		...
		(RestrictPlayer1[client] && StrContains(RestrictedWeapons1, WeaponName, false) != -1))
	{
		#if _DEBUG
			dmsg[0] = '\0';
			Format(dmsg, sizeof(dmsg), "Restricting weapon_%s for %L and dropping it", WeaponName, client);
			DebugMessage(client, dmsg);
		#endif
...
{
	if (StrEqual(weapon_name, "none", false))
	{

...
public Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	#if _DEBUG
		dmsg[0] = '\0';
		Format(dmsg, sizeof(dmsg), "%N has spawned and ClientNeedsScoreUpdated value is %i", client, ClientNeedsScoreUpdated[client]);
		DebugMessage(client, dmsg);
	#endif
	...
	if (ClientNeedsScoreUpdated[client])
	{
		...
		#if _DEBUG
			dmsg[0] = '\0';
			Format(dmsg, sizeof(dmsg), "Setting %N's frags and deaths to %i and %i", client, SavedFrags[client], SavedDeaths[client]);
			DebugMessage(client, dmsg);
		#endif
...
CheckKDR(client)
{
	...
	else
	{
		...
		if (Restricted[client])
		{
			...
			if (Announce)
			{
				PrintToChatAll("Announcing from CheckKDR");
...
RestrictWeapons(client, msg)
{
	...
	{
		#if _DEBUG
			DebugMessage(client, "Client is not in game or is exempt.");
		#endif
		return;
	}
...
{
	#if _DEBUG
		LogMessage("Running ResetEverything");
	#endif

...
new Handle:g_ClientTimer4[MAXPLAYERS+1] = {INVALID_HANDLE, ...};
...
{
	if(IsClientInGame(client) && IsPlayerAlive(client) && g_ClientTimer4[client] != INVALID_HANDLE)
	{
...
public OnRoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(DropOnRoundEnd)
	{
		for(new i = 1; i <= MaxClients; i++)
		{
			if(IsClientInGame(i))
			{
				if(g_ClientTimer4[i] != INVALID_HANDLE)
				{
					ClearTimer(g_ClientTimer4[i]);
				}
...
RestrictInform(client)
{
	...
	if(SniperKills[client] >= MaxSniperKills)
	{
		if(DropOnRoundEnd && !SnipersRestricted[client])
		{
			...
			if(AdvisePlayer)
			{
				if(g_ClientTimer4[client] == INVALID_HANDLE)
				{
					g_ClientTimer4[client] = CreateTimer(3.0, t_AdvisePlayer, client, TIMER_REPEAT);
				}
			}
		}
		else
...
public OnClientDisconnect(client)
{
	...
	{		
		...
		ClearTimer(g_ClientTimer4[client]);

...
new Handle:h_SmSlapTimer[MAXPLAYERS+1];
new SlapCount[MAXPLAYERS+1];

PerformSlap(client, target, damage)
{
	if(SlapCount[client] == 0)
	{
		h_SmSlapTimer[client] = CreateTimer(10.0, t_SmSlap, client);
		SlapCount[client]++;
...
public Action:t_SmSlap(Handle:timer, any:client)
{
	SlapCount[client] = 0;
}
...
public MenuHandler_Slap(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		else if (SlapCount[param1] >= 2)
		{
			ReplyToCommand(param1, "[SM] Don't use sm_slap so often.");
			CloseHandle(menu);
		}
		else
		{
...
public Action:Command_Slap(client, args)
{
	...
	if(SlapCount[client] >= 2)
	{
		ReplyToCommand(client, "[SM] Don't use sm_slap so often.");
		return Plugin_Handled;
	}

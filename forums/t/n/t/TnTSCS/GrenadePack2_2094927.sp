...
#define		_DEBUG_CHAT				0	// Set to 1 for debug chat spew
...
public OnPluginStart()
{
	...
	if (lateLoad)
	{
		...
		#if _DEBUG
		Format(dmsg, sizeof(dmsg), "[OnPluginStart] Plugin was late loaded");
		DebugMessage(dmsg);
		#endif
	}
}
...
CheckIsPlayerVIP(client)
{
	...
	if (!IsFakeClient(client) && CheckCommandAccess(client, "grenadepack2_vip", ADMFLAG_CUSTOM1))
	{
		...
		*/
		...
		#if _DEBUG
		Format(dmsg, sizeof(dmsg), "[CheckIsPlayerVIP] %L is marked as VIP", client);
		DebugMessage(dmsg);
		#endif
	}
	else
	{
		...
		#if _DEBUG
		Format(dmsg, sizeof(dmsg), "[CheckIsPlayerVIP] %L is NOT marked as VIP", client);
		DebugMessage(dmsg);
		#endif
	}
 ...
 * @param	Spawn	Reset the spawn count of the client (for advertising purposes)
 * @param	VIP		Set the PlayerIsVIP bool to false for client
 * 
 * @noreturn
 */
public ResetVariables(client, bool:Spawn, bool:VIP)
{
	...
	#if _DEBUG
	Format(dmsg, sizeof(dmsg), "[ResetVariables] %L had their variables reset.  HE=%i, FB=%i, SG=%i", client, BoughtHE[client], BoughtFB[client], BoughtSG[client]);
	DebugMessage(dmsg);
	#endif
}
...
public Event_OnPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	#if _DEBUG
	Format(dmsg, sizeof(dmsg), "[PlayerSpawn] %L spawned", client);
	DebugMessage(dmsg);
	#endif
...
public Action:CS_OnBuyCommand(client, const String:weapon[])
{
	...
	*/
	if (StrEqual(weapon, "hegrenade", false))
	{
		...
		{
			#if _DEBUG
			LogCannotBuy(client, weapon, false);
			#endif
		...
		else
		{
			if (EnforceBuyLimit)
			{
				...
				{
					#if _DEBUG
					LogCannotBuy(client, weapon, true);
					#endif
				...
				else
				{
					...
					#if _DEBUG
					Format(dmsg, sizeof(dmsg), "[OnBuyCommand] %N is allowed to buy [%s], ENFORCE is on and they've bought %i", client, weapon, BoughtHE[client]);
					DebugMessage(dmsg);
					#endif
				}
			}
		...
		else
		{
			if (EnforceBuyLimit)
			{
				...
				else
				{
					...
					#if _DEBUG
					Format(dmsg, sizeof(dmsg), "[OnBuyCommand] %N is allowed to buy [%s], ENFORCE is on and they've bought %i", client, weapon, BoughtFB[client]);
					DebugMessage(dmsg);
					#endif
				}
			}
		...
		else
		{
			if (EnforceBuyLimit)
			{
				...
				else
				{
					...
					#if _DEBUG
					Format(dmsg, sizeof(dmsg), "[OnBuyCommand] %N is allowed to buy [%s], ENFORCE is on and they've bought %i", client, weapon, BoughtSG[client]);
					DebugMessage(dmsg);
					#endif
				}
			}
...
#if _DEBUG
/**
 * Log the message about buy restriction
 * 
 * @param	client		clientID of player
 * @param	weapon	Weapon string
 * @param	max		Use "max reached" message
 * @noreturn
 */
LogCannotBuy(client, const String:weapon[], bool:max)
{
	#if _DEBUG
	if (max)
	{
		Format(dmsg, sizeof(dmsg), "[OnBuyCommand] %N cannot buy any more [%s], ENFORCE is ON and they've purchased their maximum for this round.", client, weapon);
	}
	else
	{
		Format(dmsg, sizeof(dmsg), "[OnBuyCommand] %N cannot buy [%s], they are already holding their maximum allowed.", client, weapon);
	}
	DebugMessage(dmsg);
	#endif
}
...
public Enabled_Changed(Handle:cvar, const String:oldValue[], const String:newValue[])
{
	if (StrEqual(newValue, "0"))
	{
		...
		#if _DEBUG
		Format(dmsg, sizeof(dmsg), "[EnabledChanged] Plugin DISABLED.  Nade limits set to: HE=%i, FB=%i, SG=%i", GetConVarInt(HEGrenadeAmmo), GetConVarInt(FlashbangAmmo), GetConVarInt(SmokeGrenadeAmmo));
		DebugMessage(dmsg);
		#endif
	}
	else
	{
		...
		#if _DEBUG
		Format(dmsg, sizeof(dmsg), "[EnabledChanged] Plugin ENABLED.  Nade limits set to: HE=%i, FB=%i, SG=%i", GetConVarInt(HEGrenadeAmmo), GetConVarInt(FlashbangAmmo), GetConVarInt(SmokeGrenadeAmmo));
		DebugMessage(dmsg);
		#endif
	}

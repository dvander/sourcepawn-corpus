...
* 
* 		Version 0.2.1.5
* 			*	Fixed CVars for weapon ammo models using same model
* 			-	Removed requirement for SMLib include file - added required functions to SP file.
* 
...
new Float:ScaleFactorPistol = 1.0;
new Float:ScaleFactorRifle = 1.0;
new Float:ScaleFactorShotgun = 1.0;
...
public OnPluginStart()
{
	...
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_afd_scalefactor_pistol", "1.0", 
	"Specify scale factor for pistol ammo box model.\n1.0 = Unchanged\n<= 0.9 = Smaller\n>= 1.1 = Larger", _, true, 0.0, true, 5.0)), OnScaleFactorPistolChanged);
	ScaleFactorPistol = GetConVarFloat(hRandom);
	SetAppend(appended);
	
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_afd_scalefactor_rifle", "1.0", 
	"Specify scale factor for rifle ammo box model.\n1.0 = Unchanged\n<= 0.9 = Smaller\n>= 1.1 = Larger", _, true, 0.0, true, 5.0)), OnScaleFactorRifleChanged);
	ScaleFactorRifle = GetConVarFloat(hRandom);
	SetAppend(appended);
	
	HookConVarChange((hRandom = AutoExecConfig_CreateConVar("sm_afd_scalefactor_shotgun", "1.0", 
	"Specify scale factor for shotgun ammo box  model.\n1.0 = Unchanged\n<= 0.9 = Smaller\n>= 1.1 = Larger", _, true, 0.0, true, 5.0)), OnScaleFactorShotgunChanged);
	ScaleFactorShotgun = GetConVarFloat(hRandom);
	SetAppend(appended);
...
public DropAmmo(any:weapon, const String:WeaponName[], any:WeaponSlot0, any:WeaponSlot1, any:client, any:ammo)
{
	...
	if ((ent = CreateEntityByName("prop_physics_multiplayer")) != -1)
	{
		...
		if (UseModelScale)
		{
			...
			{
				SetEntPropFloat(ent, Prop_Send, "m_flModelScale", ScaleFactorPistol);
			}
			else
			{
				if (rifletype == RIFLES)
				{
					SetEntPropFloat(ent, Prop_Send, "m_flModelScale", ScaleFactorRifle);
				}
				else
				{
					SetEntPropFloat(ent, Prop_Send, "m_flModelScale", ScaleFactorShotgun);
				}
			}
		}
...
public OnScaleFactorPistolChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	ScaleFactorPistol = GetConVarFloat(cvar);
}

//========================================================================================

public OnScaleFactorRifleChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	ScaleFactorRifle = GetConVarFloat(cvar);
}

//========================================================================================

public OnScaleFactorShotgunChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	ScaleFactorShotgun = GetConVarFloat(cvar);
}
 ...
 */
bool:Client_GetWeaponPlayerAmmo(client, const String:className[], &primaryAmmo=-1, &secondaryAmmo=-1)
{
 ...
 * @return				True on success, false on failure.
 */
bool:Client_SetWeaponPlayerAmmo(client, const String:className[], primaryAmmo=-1, secondaryAmmo=-1)
{
	new weapon = Client_GetWeapon(client, className);
 ...
 * @noreturn		
 */
Client_SetWeaponPlayerAmmoEx(client, weapon, primaryAmmo=-1, secondaryAmmo=-1)
{
	new offset_ammo = FindDataMapOffs(client, "m_iAmmo");

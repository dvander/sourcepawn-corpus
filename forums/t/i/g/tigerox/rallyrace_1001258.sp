...
#define  RALLYRACE_VERSION			"1.0.0.2"
...
//TigerOx
#define		WAIT_TIME		30		//Time after first player finishes to end the race.
...
public Ev_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	Disintegrate(victim);
	new car = RCM_GetPlayerCar(victim);
	if(car != -1)
	{
			...
			if(g_Game_CountDown >= racetime)
			{
				...
				for (new i=1; i<=maxClients; i++)
					if (IsClientConnected(i) && IsClientInGame(i) && g_Player_JoinReadyTime[i] == 0.0 &&  g_Player_RaceTime[i] != 0.0 && GetClientTeam(i) == 3) // player join race
						total++;
				//TigerOx
				if(g_Game_CountDown <= 0 || total == 0 || (g_Finish_CountDown - g_Game_CountDown) >= WAIT_TIME) // all racers finish or no racers or timeout
				{
					//TigerOx
					if((g_Game_CountDown <= 0 && GetConVarInt(h_rallyrace_racetime) > 0) || (g_Finish_CountDown - g_Game_CountDown) >= WAIT_TIME)
					{
...
public PlayerFinishRace(car,client)
{
	...
	if(g_Finish_CountDown == 0 && (GetTeamClientCount(3) > 0)) //Do not display winner or start time limit with only 1 racer
	{
		...
		for (new i=1; i<=MaxClients; i++)
			if (IsClientConnected(i) && IsClientInGame(i) && g_Player_JoinReadyTime[i] == 0.0 &&  g_Player_RaceTime[i] != 0.0 && GetClientTeam(i) == 3)
				PrintToChat(i,"%d seconds left in the race!", WAIT_TIME);

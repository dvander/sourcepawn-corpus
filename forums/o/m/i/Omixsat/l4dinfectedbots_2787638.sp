/********************************************************************************************
* Plugin	: L4D InfectedBots (Versus Only Edition)
* Version	: 2.0.2b
* Game		: Left 4 Dead 1 (Single Player Versus Variant)
* Author	: djromero (SkyDavid, David) and MI 5
* Edits		: Omixsat
* Testers	: djromero, MI 5, Omixsat
...
*
* Version 2.0.2b
*      - Refined the two hook events
*
* Version 2.0.2
*      - Added two more hook events to fix bots not spawning.
*
* Version 2.0.1b
*      - Modified to only work in Versus mode (L4D1).
*      - This plugin should work in L4D2, but this is designed specifically for L4D1(Spawns/despawns infected ghosts before survivors leave safe room to ensure single player VS is possible)
*      - Players can play single player versus with sv_cheats disabled. This means achievements can be unlocked
*      - Removed sb_stop commands since this can be handled via [L4D] Delay Bots Starting Game
*
* Version 2.0.0
...
static Handle pInfHUD 		= null;	// The panel shown to all infected users
static Handle usrHUDPref 		= null;	// Stores the client HUD preferences persistently

// Console commands
static Handle h_InfHUD		= null;
static Handle h_Announce 	= null;

public Plugin myinfo = 
{
	name = "L4D InfectedBots (Versus Only Edition)",
	author = "djromero (SkyDavid), MI 5. With edits from Omixsat",
	description = "Spawns infected bots in versus, allows versus to be playable in L4D1",
	version = PLUGIN_VERSION,
	url = "https://forums.alliedmods.net/showpost.php?p=2787638&postcount=1634"
}
...
public void OnPluginStart()
{
	...
	HookEvent("tank_killed", evtTankDied);
...
public Action evtRoundStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	//Enable only in versus game mode
	if (GameMode != 2)
		return;
	
	//For some reason, client servers experience the versus spawn in coop/survival
	//This is added for safety reasons
	if (GameMode == 1)
		return;
	...
	// This will be handled by "[L4D] Delay Bots Starting Game"
	//if (!b_LeftSaveRoom)
	...
	{
		#if DEBUGSERVER
		LogMessage("Starting the Versus PlayerLeft Start Timer");
		#endif
...
public Action JoinInfected(int client, int args)
{
	if (client && (GameMode == 1 || GameMode == 3) && GetConVarBool(h_JoinableTeams))
	{
		...
		{
			if (!IsPlayerAlive(client) && GetUserFlagBits(client) > 0)
				ChangeClientTeam(client, TEAM_INFECTED);
			else
			{
		...
		{
			if (!IsPlayerAlive(client) && HumansOnInfected() < GetConVarInt(h_HumanCoopLimit))
				ChangeClientTeam(client, TEAM_INFECTED);
			else
...
public Action evtTankSpawn(Event event, const char[] name, bool dontBroadcast)
{
	if (GetConVarInt(h_DisableSpawnsTank) == 0)
		CreateTimer(1.0, CheckIfBotsNeededLater, true);
}

public Action evtTankDied(Event event, const char[] name, bool dontBroadcast)
{
	CreateTimer(1.0, CheckIfBotsNeededLater, true);
}
...
public Action evtPlayerTeam(Event event, const char[] name, bool dontBroadcast)
{
	...
	//Ensure that single player VS is playable
...
public Action Spawn_InfectedBot(Handle timer)
{
	...
	if (b_HasRoundEnded || !b_HasRoundStarted) return; //Ensure that single player VS is playable
	...
	if (GetConVarInt(h_DisableSpawnsTank) == 1)
	{
		for (int i = 1; i <= MaxClients;i++)
		{
			// We check if player is in game
		...
		if (GetClientTeam(client) == TEAM_INFECTED)
		{
			if (IsPlayerTank(client) && isTankOnFire[client] == false)
			{
				...
				if ((GetEntProp(client, Prop_Data, "m_fFlags") & FL_ONFIRE) && PlayerIsAlive(client))
				{
					...
					burningTankTimeLeft[client] = (fireTankExpiry != null) ? GetConVarInt(fireTankExpiry) : 30;
...
{
	infHUDTimer 		= null;	// The main HUD refresh timer

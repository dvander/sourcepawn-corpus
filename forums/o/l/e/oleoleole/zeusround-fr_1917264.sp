...
*
*		0.0.2.8 *	Fixed variable definition of two variables.
*
* 	KNOWN ISSUES
...
#define 	UPDATE_URL 			"http://csgo.gidderikke.no/zeusroundwithfrfix/update.txt"
...
#define _DEBUG_ALL		1		// Set to 1 for in-game chat debug spew
...
new bool:tmpSaveWeapons = true;
new bool:tmpSaveMoney = true;
new bool:StripHostages = true;
new bool:StripBomb = true;
new bool:CleanUpZeus = true;
new bool:ZeusFirstRound = false;
new bool:ZeusLastRound = false;
new bool:ZeusCountdown = false;
new bool:ZeusFirstRoundEndForceGive = false;
new ZeusFirstRoundEndForceGiveCash;
new bool:UpcomingZeusRound = false;
new bool:IsZeusRound = false;
new bool:HookedPlayers = false;

new String:ZeusFirstRoundEndForceGiveWeaponCT;
new String:ZeusFirstRoundEndForceGiveWeaponTT;
...
{
	name = "Zeus Round with FirstRound fix.",
	author = "TnTSCS aka ClarkKent, modified by mOLEkewl",
	description = "CS:GO Zeus Round with FirstRound fix.",
...
public OnPluginStart()
{
	...
	HookConVarChange((hRandom = CreateConVar("sm_zeusround_firstround", "0", 
	"Should the first round be a zeus round?\n0 = NO\n1 = YES", FCVAR_NONE, true, 0.0, true, 1.0)), OnFirstRoundChanged);
	ZeusFirstRound = GetConVarBool(hRandom);
	...
	HookConVarChange((hRandom = CreateConVar("sm_zeusround_firstroundend_forcegive", "1", 
	"Set this to 1 if you want to force to give specified amount of cash and correct startup weapons after first round. Workaround for warmup round bug.\n0 = NO\n1 = YES", FCVAR_NONE, true, 0.0, true, 1.0)), OnFirstRoundEndForceGiveChange);
	ZeusFirstRoundEndForceGive = GetConVarBool(hRandom);
	
	HookConVarChange((hRandom = CreateConVar("sm_zeusround_firstroundend_forcegive_cash", "1000", 
	"How much cash to set as startcash (probably what you have set mp_startmoney to in server.cfg)", FCVAR_NONE, true, 0.0, true, 20000.0)), OnFirstRoundEndForceGiveCashChange);
	ZeusFirstRoundEndForceGiveCash = GetConVarInt(hRandom);
	...
	ZeusFirstRoundEndForceGiveWeaponCT = GetConVarString(hRandom, buffer, sizeof(buffer));
			
	HookConVarChange((hRandom = CreateConVar("sm_zeusround_firstroundend_forcegive_weapon_tt", "glock", 
	"Which weapon to give after first round to Terrorists, like: glock, deagle, tec9...")), OnFirstRoundEndForceGiveWeaponTTChange);
	ZeusFirstRoundEndForceGiveWeaponTT = GetConVarString(hRandom, buffer, sizeof(buffer));
	...
	LoadTranslations("zeusround-fr.phrases");
...
public Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (ZeusFirstRound && g_TotalRoundsPlayed == 1)
	{
		if (StripHostages)
		{
			CreateTimer(1.0, Timer_RemoveHostages);
...
public Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_TotalRoundsPlayed == 2 && !IsZeusRound && ZeusFirstRound && ZeusFirstRoundEndForceGive)
	{
		SaveMoney = false;
		SaveWeapons = false;
		...
		{
			if(!ZeusFirstRoundEndForceGiveWeaponCT)
			{
				GivePlayerItem(client, "weapon_hkp2000");
			}
			else
			{
				Format(buffer, sizeof(buffer), "weapon_%s", ZeusFirstRoundEndForceGiveWeaponCT);
				GivePlayerItem(client, buffer);
				#if _DEBUG
					new String:dmsg[MAX_MESSAGE_LENGTH];
					Format(dmsg, sizeof(dmsg), "Event_PlayerSpawn CT: Give item set to: %s", buffer);
					DebugMessage(dmsg);
				#endif
			}
		}
		...
		{
			if(!ZeusFirstRoundEndForceGiveWeaponTT)
			{
				GivePlayerItem(client, "weapon_glock");
			}
			else
			{
				Format(buffer, sizeof(buffer), "weapon_%s", ZeusFirstRoundEndForceGiveWeaponTT);
				GivePlayerItem(client, buffer);
				#if _DEBUG
					new String:dmsg[MAX_MESSAGE_LENGTH];
					Format(dmsg, sizeof(dmsg), "Event_PlayerSpawn TT: Give item set to: %s", buffer);
					DebugMessage(dmsg);
				#endif
			}
		}
		
		SetPlayersCash(client, ZeusFirstRoundEndForceGiveCash);
		
		return;
	}
	else
	{
		SaveMoney = tmpSaveMoney;
		SaveWeapons = tmpSaveWeapons;
	}
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		if ((GetClientTeam(killer) == CS_TEAM_CT && GetClientTeam(victim) != CS_TEAM_CT) || ((GetClientTeam(killer) != CS_TEAM_CT && GetClientTeam(victim) == CS_TEAM_CT)))
		{
...
public OnFirstRoundChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	ZeusFirstRound = GetConVarBool(cvar);
}
...
public OnFirstRoundEndForceGiveChange(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	ZeusFirstRoundEndForceGive = GetConVarBool(cvar);
}

public OnFirstRoundEndForceGiveCashChange(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	ZeusFirstRoundEndForceGiveCash = GetConVarInt(cvar);
}

public OnFirstRoundEndForceGiveWeaponCTChange(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	decl String:buffer[32];
 	ZeusFirstRoundEndForceGiveWeaponCT = GetConVarString(cvar, buffer, sizeof(buffer));
}

public OnFirstRoundEndForceGiveWeaponTTChange(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	decl String:buffer[32];
 	ZeusFirstRoundEndForceGiveWeaponTT = GetConVarString(cvar, buffer, sizeof(buffer));
}

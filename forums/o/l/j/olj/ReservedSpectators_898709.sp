        ...
        if ((IsValidPlayer(client))&&(!IsClientMember(client))&&(GetClientTeam(client)==1))
            {
                ...
                KickClient(client, "Sorry, spectator slots currently is being reserved for admins-only");
                PrintToChatAll("\x01%s \x03was kicked due to spectator slots reservation", name);
            }
    }

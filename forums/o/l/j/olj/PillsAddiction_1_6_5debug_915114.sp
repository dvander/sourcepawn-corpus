...
public RoundStartPills(Handle:event, const String:name[], bool:dontBroadcast)
	{
		LogMessage("Debug: Round start event fired");
		if (GivePillsEnabled)
			{
			LogMessage("Debug: Give Pills Enabled");
			new Flags = GetCommandFlags("give");
			CreateTimer(15.0, Debugtimer);
			CreateTimer(GivePillsTimeout, GivePillsTimer, any:Flags);
			}
	}

public Action:Debugtimer(Handle:timer)
{
LogMessage("Debug: Debug Timer: CVARS");
PrintToChatAll("l4d_pillsaddiction_givepills_enabled: %i", GetConVarInt(h_GivePillsEnabled));
PrintToChatAll("l4d_pillsaddiction_givepills_timeout: %f", GetConVarFloat(h_GivePillsTimeout));
PrintToChatAll("l4d_pillsaddiction_givepills_adminonly: %i", GetConVarInt(h_GivePillsAdminOnly));
PrintToChatAll("l4d_pillsaddiction_death_threshold: %i", GetConVarInt(h_PillsDeathThreshold));
LogMessage("l4d_pillsaddiction_givepills_enabled: %i", GetConVarInt(h_GivePillsEnabled));
LogMessage("l4d_pillsaddiction_givepills_timeout: %f", GetConVarFloat(h_GivePillsTimeout));
LogMessage("l4d_pillsaddiction_givepills_adminonly: %i", GetConVarInt(h_GivePillsAdminOnly));
LogMessage("l4d_pillsaddiction_death_threshold: %i", GetConVarInt(h_PillsDeathThreshold));
}
				...
				if ((IsValidClient(clientID))&&(DeathCounter[clientID]>PillsDeathThreshold))
					{
						...
						PrintToChatAll("Debug: Created death timer");
						LogMessage("Debug: Created death timer");
		...
		if (TotalPillsUsed[clientID]>PillsThreshold)
			{
				...
				TotalPillsUsed[clientID] = 0;
				if (AdrenalineEnabled) SetEntDataFloat(clientID, speedOffset, Speedfactor, true);
			}
	}
...
public Action:OverdoseKill(Handle:timer, any:clientID)
	{
		PrintToChatAll("Debug: Kill timer fired");
		LogMessage("Debug: Kill timer fired");
...
ExecuteCommand(Client, String:strCommand[], String:strParam1[])
{
    if (Client==0) 
		{
			LogMessage("Debug: Client 0 detected");
			PrintToChatAll("Debug: Client 0 detected");
			return;
		}
    new Flags = GetCommandFlags(strCommand);
    SetCommandFlags(strCommand, Flags & ~FCVAR_CHEAT);
    LogMessage("Debug: Executing client give command");
	PrintToChatAll("Debug: Executing client give command");
...
for (new d = 1; d <=MaxClients; d++)
	{
	if (IsValidClient(d)) SetEntDataFloat(d, speedOffset, 1.0, true);
	}
}

public Action:GivePillsTimer(Handle:timer, any:Flags)
{
	PrintToChatAll("Debug: GivePillsTimer fired");
	LogMessage("Debug: GivePillsTimer fired");
			...
			case 0:
				{
					for (new i = 1; i <=MaxClients; i++)
						{
							if ((IsClientInGame(i))&&(IsPlayerAlive(i))&&(GetClientTeam(i)==2)&&(i>0)) 
								{
									PrintToChatAll("Debug: Giving pills to valid client");
									LogMessage("Debug: Giving pills to valid client");
									ExecuteCommand(i, "give", "pain_pills");
								}
							if (i==MaxClients)
								{
									PrintToChatAll("Debug: Got maxclients");
									LogMessage("Debug: Got maxclients");
			...
			case 1:
				{
					for (new i = 1; i <=MaxClients; i++)
						{
							if ((IsClientInGame(i))&&(IsPlayerAlive(i))&&(GetClientTeam(i)==2)&&(i>0)&&(IsClientAdmin(i))) 
								{
									PrintToChatAll("Debug: Giving pills to valid admin");
									LogMessage("Debug: Giving pills to valid admin");
									ExecuteCommand(i, "give", "pain_pills");
								}
							if (i==MaxClients)
								{

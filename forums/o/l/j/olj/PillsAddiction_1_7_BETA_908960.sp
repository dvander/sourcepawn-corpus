...
new Handle:h_AddictionSystemEnabled;
new Handle:h_AddictionSystemInitialTime;
new Float:AddictionSystemInitialTime;
new bool:AddictionSystemEnabled;
...
new bool:Addicted[MAXPLAYERS+1];
new DeathCounter[MAXPLAYERS+1];
new Handle:LaughTimerHandle[MAXPLAYERS+1];
new Handle:AddictionSubTimerHandle[MAXPLAYERS+1];
new Float:AddictionTimerInterval[MAXPLAYERS+1];
new bool:GotAnotherPillz[MAXPLAYERS+1];
...
h_AddictionSystemEnabled = CreateConVar("l4d_pillsaddiction_addictionsystem_enabled", "1", "Enable/disable addiction system.", CVAR_FLAGS);
h_AddictionSystemInitialTime = CreateConVar("l4d_pillsaddiction_addictionsystem_initialtime", "180.0", "If you wont take pills during this time after you get addicted, you will suffer pain and sorrow.", CVAR_FLAGS);
		//Variables//
...
AddictionSystemEnabled = GetConVarBool(h_AddictionSystemEnabled);
AddictionSystemInitialTime = GetConVarFloat(h_AddictionSystemInitialTime);
		//Hooks//
...
HookConVarChange(h_AddictionSystemEnabled, AddictionSystemEnabledChanged);
HookConVarChange(h_AddictionSystemInitialTime, AddSystemInitialTimeChanged);
...
CreateConVar("l4d_pillsaddiction_version", PLUGIN_VERSION, "Version of Pills addiction plugin", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
}
		//CVAR Changes callbacks//
public AddSystemInitialTimeChanged(Handle:convar, const String:oldValue[], const String:newValue[])
	{
		AddictionSystemInitialTime = GetConVarFloat(h_AddictionSystemInitialTime);
	}
			
public AddictionSystemEnabledChanged(Handle:convar, const String:oldValue[], const String:newValue[])
	{
		AddictionSystemEnabled = GetConVarBool(h_AddictionSystemEnabled);
	}
...
public RoundStartPills(Handle:event, const String:name[], bool:dontBroadcast)
	{
		if (GivePillsEnabled)
			...
			CreateTimer(30.00, GivePillsTimer, any:Flags);
			}
	}
		//KillDrugs Callback//
...
public PillsUsed(Handle:event, const String:name[], bool:dontBroadcast)
	{
		...
		if (Addicted[clientID]) GotAnotherPillz[clientID] = true;
		if (AddictionSystemEnabled) 
			{
				if (Addicted[clientID] == false)
					{
						GotAnotherPillz[clientID] = false;
						Addicted[clientID] = true;
						PrintToChat(clientID, "You just got addicted to \x03Pillz\x01. You will need another dose soon!");
						CreateTimer(AddictionSystemInitialTime, AddictionTimerCallback, any:clientID, TIMER_FLAG_NO_MAPCHANGE);
					}
			}
				...
				if ((IsValidClient(clientID))&&(DeathCounter[clientID]>PillsDeathThreshold))
					{
						...
						if (Addicted[clientID]) Addicted[clientID]=false;
						GotAnotherPillz[clientID] = false;
...
public Action:AddictionTimerCallback(Handle:timer, any:clientID)
	{
		if ((!IsValidClient(clientID))||(Addicted[clientID]==false)) return Plugin_Handled;
		if (GotAnotherPillz[clientID]) 
			{	
				AddictionTimerInterval[clientID] -= 30.0;
				GotAnotherPillz[clientID] = false;
				AddictionSubTimerHandle[clientID] = CreateTimer(AddictionTimerInterval[clientID], AddictionSubTimerCallback, any:clientID, TIMER_FLAG_NO_MAPCHANGE|TIMER_REPEAT);
				return Plugin_Handled;
			}
		ClientCommand(clientID, "vocalize PlayerDeath");
		SlapPlayer(clientID, 20, true);
		AddictionTimerInterval[clientID] -= 30.0;
		AddictionSubTimerHandle[clientID] = CreateTimer(AddictionTimerInterval[clientID], AddictionSubTimerCallback, any:clientID, TIMER_FLAG_NO_MAPCHANGE|TIMER_REPEAT);
		return Plugin_Handled;
	}
	
public Action:AddictionSubTimerCallback(Handle:timer, any:clientID)
	{
		if ((!IsValidClient(clientID))||(Addicted[clientID]==false)) 
			{
				KillAddSubTimer(clientID);
				return Plugin_Handled;
			}	
		if (AddictionTimerInterval[clientID]>=60.0) AddictionTimerInterval[clientID] -= 30.0;
		if (GotAnotherPillz[clientID]) 
			{	
				GotAnotherPillz[clientID] = false;
				return Plugin_Continue;
...
KillAddSubTimer(client)
{
	KillTimer(AddictionSubTimerHandle[client], true);
	AddictionSubTimerHandle[client] = INVALID_HANDLE;
}
...
for (new e = 1; e <=MaxClients; e++)
	{
	AddictionTimerInterval[e] = AddictionSystemInitialTime;
	}
for (new f = 1; f <=MaxClients; f++)
	{
	GotAnotherPillz[f] = false;
	}
}

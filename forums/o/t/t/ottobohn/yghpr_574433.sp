...
new bool:g_bConnectionBreakWarn = false;
...
public OnPluginStart ()
{
    ...
    g_cVarMaxPing = CreateConVar ("yghpr_maxping", "100.0", "Set the maximum ping allowed on the server", 0, true, 0.0, true, 1000.0);
...
{
    AutoExecConfig (true, "plugin.yghpr", "sourcemod");
}
...
public Action:PingCheckThread (Handle:timer)
{
    ...
    if (GetConVarInt (g_cVarEnable))
    {
        ...
        for (i = 1; i <= iMaxClients; i++)
        {
            ...
            {
                if (!IsProtectedAdmin (i))
                {
                    g_bConnectionBreakWarn = GetConVarBool (g_cVarConnectionBreakWarn);

                    if ((g_fMaxPing = GetConVarFloat (g_cVarMaxPing)))
                    {
        ...
        if (iSeconds++ >= (GetConVarInt (g_cVarKickCountClearTime) * 60))
        {
            ClearArray (g_hArrayClientKickCounts);
            iSeconds = 0;
		}
...
RunLatencyChecks (client)
{
    ...
    if (IsClientConnected (client))
    {
        ...
        if (g_bConnectionBreakWarn && ((g_iMaxPingPoints - g_iPingPoints[client]) <= 10))

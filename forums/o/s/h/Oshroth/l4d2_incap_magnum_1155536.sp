...
public Action:Event_Revive(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	PrintToChatAll("%N was revived.", client);
	
	if(mode == 0 || mode == 1 || mode == 2 || incap_replace[client] == 1) {
		incap_replace[client] = 0;
		if(weapon == -1) {
			SetCommandFlags("give", flags & ~FCVAR_CHEAT);
			PrintToChatAll("%N was given Dual Pistols as backup.", client);
			FakeClientCommand(client, "give pistol");
			FakeClientCommand(client, "give pistol");
			SetCommandFlags("give", flags|FCVAR_CHEAT);
		}
	...
	if(incap_replace[client] == 2) {
		PrintToChatAll("%N had his Dual Pistols returned.", client);
		FakeClientCommand(client, "give pistol");
		FakeClientCommand(client, "give pistol");
	}
	...
	if(weapon == -1) {
		SetCommandFlags("give", flags & ~FCVAR_CHEAT);
		PrintToChatAll("%N was given Dual Pistols because there was no valid weapon for replacement.", client);
		FakeClientCommand(client, "give pistol");
		FakeClientCommand(client, "give pistol");
		SetCommandFlags("give", flags|FCVAR_CHEAT);
	}

...
public Action:Afficher_Menu_Bhop(client, args)
{
	if (GetClientTeam(client) == 3)
	{
		...
		SetMenuTitle(menu, "Simon Says");
		AddMenuItem(menu, "option1", "Simon", 0);
		AddMenuItem(menu, "option2", "Evil Simon", 0);
		AddMenuItem(menu, "option3", "Drunken Simon", 0);
...
public bhopvip(Handle:menu, MenuAction:action, client, itemNum)
{
	if (action == MenuAction:4)
	{
		switch (itemNum)
		{
			case 0:
			{
				FakeClientCommand(client, "say @[WARDEN] Simon Says!");
			}
			case 1:
			{
				FakeClientCommand(client, "say @[WARDEN] Evil Simon");
			}
			case 2:
			{
				FakeClientCommand(client, "say @[WARDEN] Drunken Simon");
			}
			default:
			{
			}

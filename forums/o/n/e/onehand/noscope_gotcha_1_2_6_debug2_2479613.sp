...
public void OnPlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
		...
		(GetEntProp(attacker, Prop_Data, "m_iFOV") <= 0 || GetEntProp(attacker, Prop_Data, "m_iFOV") == GetEntProp(attacker, Prop_Data, "m_iDefaultFOV")))
	{
		...
		if(enabled)
		{
			if(bHS)
			{
				CPrintToChatAll("%t", "HS2All", TAG, sName, vName, weapon);
				CPrintToChat(attacker, "%t", "HS", headshots[attacker]);
				if(enablesounds) playSound(SOUND_GODLIKE_REL);
			}
			else 
			{
				CPrintToChatAll("%t", "Kill2All", TAG, sName, vName, weapon);
...
public Action Cmd_NoScopes(int client, int args)
{
	...
	{
		if(debugmsg) PrintToServer("	%L (client #%i)\n	Kills: %i\n	HS: %i", client, client, kills[client], headshots[client]);
		if(!enabled) ReplyToCommand(client, "[SM] %t", "No Access");
		else
		{
			CPrintToChat(client, "%t", "HS", headshots[client]);
			CPrintToChat(client, "%t", "Kill", kills[client]);
		}
	}

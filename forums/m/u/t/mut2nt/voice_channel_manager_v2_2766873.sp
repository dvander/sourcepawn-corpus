...
{	
	...
	CreateConVar("vc_version", PLUGIN_VERSION, "VoiceMgr Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	
	vc_show_incoming = CreateConVar("vc_show_incoming", "0", "Show channel transfer msg if client connected (0=disable; 1=enable)");
	vc_store_channel = CreateConVar("vc_store_channel", "1", "Rememberlast channel if a client switch channel? (0=disable; 1=enable)");
	...
	vc_public_channel_id = CreateConVar("vc_public_channel_id", "-1", "Only change if you like to change default channel (-1=first public channel)");
	vc_announcer_channel_id = CreateConVar("vc_announcer_channel_id", "-1", "Only change if you like to create an announcer channel channel (-1=no announcer channel)");
	vc_announcer_mode = CreateConVar("vc_announcer_mode", "0", "Additional to vc_announcer_channel_id (0=speak to all; 1=speak to all and mute everyone else; 2=speak to all and listen all)");
	...
	vc_invite_enable = CreateConVar("vc_invite_enable", "1", "Enable invite cmd (0=disable; 1=enable)");
	vc_invite_admin_only = CreateConVar("vc_invite_admin_only", "0", "Only admins can use invite command (0=disable; 1=enable)");
	vc_invite_public_only = CreateConVar("vc_invite_public_only", "1", "Only invite player if they are in default channel (0=all channels; 1=only default channel; 2=all public channels)");
	...
	vc_hud_show_channel_cmd = CreateConVar("vc_hud_show_channel_cmd", "2", "Show !vc? (0 = disable; 1=ever; 2=only public channel)");
	vc_hud_show_team = CreateConVar("vc_hud_show_team", "1", "Show team tags in front of name (0=disable; 1=all channels 2=only team channels");
	vc_hud_show_public_all = CreateConVar("vc_hud_show_public_all", "1", "List all channels if current channel is default channel (0=disable; 1=only not empty 2=all channels)");
	...
	vc_hud_update_interval = CreateConVar("vc_hud_update_interval", "1.0", "Update interval of channel HUD");
	vc_hud_status_tag_dead = CreateConVar("vc_hud_status_tag_dead", " [DEAD]", "Status tag for dead players");
...
public OnClientPutInServer(client)
{
	...
	if(g_iClientChannel[client] == NO_CHANNEL)
	{
		...
		PrintToChatAll("%t", "ChangeChannel", name, g_sChannelsName[g_iClientChannel[client]]);
	}
...
public Action:VCKickGui(client, args)
{
	...
	if(0 < client < MaxClients)
	{
		...
		if(Client_IsAdmin(client))
		{
			...
			{
				PrintToChat(client, "%t", "KickFailedPublic");
			}
		}
		else
		{
			PrintToChat(client, "%t", "KickFailedPermission");
		}
	}
	return Plugin_Handled;
}
...
public Action:VCInviteGui(client, args)
{
	...
	if(0 < client < MaxClients)
	{
		if(cvar_vc_invite_enable == 1)
		{
			...
			{
				PrintToChat(client, "%t", "NoInvitesLeft", cvar_vc_invite_max);
			}
		}
	}
	return Plugin_Handled;
	...
	if(0 < client < MaxClients)
	{
		...
		if (g_iClientChTryPw[client] == 1)
		{
			if(StrEqual(text, g_sChannelsPass[g_iClientChTryPrivate[client]]))
			{
				...
				{
					PrintToChatAll("%t", "ChangeChannel", name, g_sChannelsName[g_iClientChTryPrivate[client]]);
			...
			{
				PrintToChat(client, "%t", "PasswordIncorrect");
			}
			g_iClientChTryPw[client] = 0;
			g_iClientChTryPrivate[client] = 0;
			return Plugin_Handled;	
...
public MenuKickChannel(Handle:menu, MenuAction:action, client, param2)
{
	if ( action == MenuAction_Select ) 
	{
		...
		if(target > NO_CHANNEL)
		{
			...
			PrintToChatAll("%t", "KickedOutOfChannel", targetname, g_sChannelsName[g_iClientChannel[client]], clientname);
		}
	}
}
...
LoadSettings(client)
{
	...
	if(0 < client < MaxClients)
	{
		...
		GetClientAuthId(client, AuthId_Steam2, cName, sizeof(cName));

...
-----------------------------------------------------------------------------
Initial Code Written By msleeper (c) 2009
-----------------------------------------------------------------------------
Major Part Of The Code Written By muukis (c) 2010
-----------------------------------------------------------------------------
...
new String:DB_PLAYERS_TOTALPOINTS[1024] = "points + points_survivors + points_infected + points_realism + points_survival + points_scavenge_survivors + points_scavenge_infected + points_realism_survivors + points_realism_infected";
new String:DB_PLAYERS_TOTALPLAYTIME[1024] = "playtime + playtime_versus + playtime_realism + playtime_survival + playtime_scavenge + playtime_realismversus";
...
public Action:AnnounceConnect(Handle:timer, any:client)
{
	...
	PrintToChat(client, "\x04[\x03RANK\x04] \x01Type \x05RANKMENU \x01to operate \x04%s\x01!", PLUGIN_NAME);
}
...
public Action:timer_InfectedDamageCheck(Handle:timer, any:data)
{
	...
	if (Score > 0)
	{
		...
		if (CurrentGamemodeID == GAMEMODE_VERSUS)
			...
			Format(query, sizeof(query), "UPDATE players SET points_realism_infected = points_realism_infected + %i WHERE steamid = '%s'", Score, iID);
		else
...
QueryClientRank(Client, SQLTCallback:callback=INVALID_HANDLE)
{
	...
	Format(query, sizeof(query), "SELECT COUNT(*) FROM players WHERE points + points_survival + points_realism + points_survivors + points_infected + points_scavenge_survivors + points_scavenge_infected + points_realism_survivors + points_realism_infected >= %i", ClientPoints[Client]);
...
QueryClientGameModeRank(Client, SQLTCallback:callback=INVALID_HANDLE)
{
	if (!InvalidGameMode())
	{
		...
		switch (CurrentGamemodeID)
		{
			case GAMEMODE_VERSUS:
			{
				Format(query, sizeof(query), "SELECT COUNT(*) FROM players WHERE playtime_versus > 0 AND points_survivors + points_infected >= %i", ClientGameModePoints[Client][GAMEMODE_VERSUS]);
			}
			case GAMEMODE_REALISM:
			{
				Format(query, sizeof(query), "SELECT COUNT(*) FROM players WHERE playtime_realism > 0 AND points_realism >= %i", ClientGameModePoints[Client][GAMEMODE_REALISM]);
			}
			case GAMEMODE_SURVIVAL:
			{
				Format(query, sizeof(query), "SELECT COUNT(*) FROM players WHERE playtime_survival > 0 AND points_survival >= %i", ClientGameModePoints[Client][GAMEMODE_SURVIVAL]);
			}
			case GAMEMODE_SCAVENGE:
			{
				Format(query, sizeof(query), "SELECT COUNT(*) FROM players WHERE playtime_scavenge > 0 AND points_scavenge_survivors + points_scavenge_infected >= %i", ClientGameModePoints[Client][GAMEMODE_SCAVENGE]);
			}
			case GAMEMODE_REALISMVERSUS:
			{
				Format(query, sizeof(query), "SELECT COUNT(*) FROM players WHERE playtime_realismversus > 0 AND points_realism_survivors + points_realism_infected >= %i", ClientGameModePoints[Client][GAMEMODE_REALISMVERSUS]);
			}
			default:
			{
				Format(query, sizeof(query), "SELECT COUNT(*) FROM players WHERE playtime > 0 AND points >= %i", ClientGameModePoints[Client][GAMEMODE_COOP]);
			}
		}
...
QueryClientGameModePointsStmID(Client, const String:SteamID[], SQLTCallback:callback=INVALID_HANDLE)
{
	...
	Format(query, sizeof(query), "SELECT points, points_survivors + points_infected, points_realism, points_survival, points_scavenge_survivors + points_scavenge_infected + points_realism_survivors + points_realism_infected FROM players WHERE steamid = '%s'", SteamID);
...
QueryRank_2(Handle:dp=INVALID_HANDLE, SQLTCallback:callback=INVALID_HANDLE)
{
	...
	switch (CurrentGamemodeID)
	{
		...
		case GAMEMODE_REALISMVERSUS:
		{
			Format(query, sizeof(query), "SELECT COUNT(*) FROM players WHERE playtime_realismversus > 0");
		}
		default:
		{
...
QueryClientStatsDP_Rank(Client, const String:SteamID[])
{
	...
	Format(query, sizeof(query), "SELECT name, %s, %s, kills, versus_kills_survivors + scavenge_kills_survivors + realism_kills_survivors, headshots FROM players WHERE steamid = '%s'", DB_PLAYERS_TOTALPLAYTIME, DB_PLAYERS_TOTALPOINTS, SteamID);
...
public UpdatePlayerFull(Client, const String:SteamID[], const String:Name[])
{
	...
	decl String:query[512];
	Format(query, sizeof(query), "UPDATE players SET lastontime = UNIX_TIMESTAMP(), %s = %s + 1, lastgamemode = %i, name = '%s', ip = '%s' WHERE steamid = '%s'", Playtime, Playtime, CurrentGamemodeID, Name, IP, SteamID);
...
public Action:timer_RankVote(Handle:timer, any:data)
{
	...
	if (!CheckHumans())
	{
		...
		PrintToChatAll("\x04[\x03RANK\x04] \x01Vote to shuffle teams by player PPM \x03%s \x01with \x04%i (yes) against %i (no)\x01.", (yesvotes > novotes ? "PASSED" : "DID NOT PASS"), yesvotes, novotes);
...
public Action:event_PlayerLedge(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Score > 0)
	{
		...
		if (CurrentGamemodeID == GAMEMODE_VERSUS)
			...
			Format(query, sizeof(query), "UPDATE players SET points_realism_infected = points_realism_infected + %i, award_ledgegrab = award_ledgegrab + 1 WHERE steamid = '%s'", Score, ClientID);
		else
...
public Action:event_UpgradePackAdded(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Score > 0)
	{
		...
		if (Mode == 1 || Mode == 2)
			PrintToChat(Player, "\x04[\x03RANK\x04] \x01You have earned \x04%i \x01points for deploying \x05%s\x01!", Score, ModelName);
		else if (Mode == 3)
		{
			decl String:PlayerName[MAX_LINE_WIDTH];
			GetClientName(Player, PlayerName, sizeof(PlayerName));
			PrintToChatAll("\x04[\x03RANK\x04] \x05%s \x01has earned \x04%i \x01points for deploying \x05%s\x01!", PlayerName, Score, ModelName);
		}
	}
}
...
public Action:event_GascanPoured(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Score > 0)
	{
		...
		if (Mode == 1 || Mode == 2)
			PrintToChat(Player, "\x04[\x03RANK\x04] \x01You have earned \x04%i \x01points for successfully \x05Pouring a Gascan\x01!", Score);
		else if (Mode == 3)
		{
			decl String:PlayerName[MAX_LINE_WIDTH];
			GetClientName(Player, PlayerName, sizeof(PlayerName));
			PrintToChatAll("\x04[\x03RANK\x04] \x05%s \x01has earned \x04%i \x01points for successfully \x05Pouring a Gascan\x01!", PlayerName, Score);
		}
	}
}
...
public Action:event_PlayerPounced(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Dmg >= PerfectDmgLimit)
	{
		...
		if (CurrentGamemodeID == GAMEMODE_VERSUS)
			...
			Format(query, sizeof(query), "UPDATE players SET points_realism_infected = points_realism_infected + %i, award_pounce_perfect = award_pounce_perfect + 1 WHERE steamid = '%s'", Score, AttackerID);
		else
	...
	else
	{
		...
		if (CurrentGamemodeID == GAMEMODE_VERSUS)
			...
			Format(query, sizeof(query), "UPDATE players SET points_realism_infected = points_realism_infected + %i, award_pounce_nice = award_pounce_nice + 1 WHERE steamid = '%s'", Score, AttackerID);
		else
	...
	if (Mode == 1 || Mode == 2)
		PrintToChat(Attacker, "\x04[\x03RANK\x04] \x01You have earned \x04%i \x01points for landing a \x05%s \x01Pounce on \x05%s\x01!", Score, Label, VictimName);
	else if (Mode == 3)
		PrintToChatAll("\x04[\x03RANK\x04] \x05%s \x01has earned \x04%i \x01points for landing a \x05%s \x01Pounce on \x05%s\x01!", AttackerName, Score, Label, VictimName);
}
...
public Action:cmd_ShowTimedMapsTimer(client, args)
{
	...
	if (client == 0)
		...
		PrintToChat(client, "\x04[\x03RANK\x04] \x01Current map timer: \x04%s", TimeLabel);
...
public Action:cmd_TimedMaps(client, args)
{
	...
	SQL_TQuery(db, CreateTimedMapsMenu, "SELECT DISTINCT tm.gamemode FROM timedmaps AS tm INNER JOIN players AS p ON tm.steamid = p.steamid", client);
...
public Action:cmd_ShowMapTimes(client, args)
{
	...
	PrintToChat(client, "\x04[\x03RANK\x04] \x05NOT IMPLEMENTED YET");
...
public Action:cmd_ShowTop10PPM(client, args)
{
	...
	Format(query, sizeof(query), "SELECT name, (%s) / (%s) AS ppm FROM players WHERE (%s) >= %i ORDER BY ppm DESC, (%s) DESC LIMIT 10", DB_PLAYERS_TOTALPOINTS, DB_PLAYERS_TOTALPLAYTIME, DB_PLAYERS_TOTALPLAYTIME, GetConVarInt(cvar_Top10PPMMin), DB_PLAYERS_TOTALPLAYTIME);
...
{
	decl String:query[1024];
	Format(query, sizeof(query), "SELECT (%s) AS totalpoints, steamid, (%s) AS totalplaytime FROM players WHERE (%s) >= %i ORDER BY (totalpoints / totalplaytime) DESC, totalplaytime DESC LIMIT %i,1", DB_PLAYERS_TOTALPOINTS, DB_PLAYERS_TOTALPLAYTIME, DB_PLAYERS_TOTALPLAYTIME, GetConVarInt(cvar_Top10PPMMin), rank);
	SQL_TQuery(db, GetClientTop10, query, client);
}
...
{
	decl String:query[256];
	Format(query, sizeof(query), "SELECT DISTINCT tm.gamemode, tm.map FROM timedmaps AS tm INNER JOIN players AS p ON tm.steamid = p.steamid WHERE tm.gamemode = %i ORDER BY tm.map ASC", Gamemode);
...
public DisplayTimedMapsMenu3(client, Gamemode, const String:Map[])
{
	...
	decl String:query[256];
	Format(query, sizeof(query), "SELECT tm.time FROM timedmaps AS tm INNER JOIN players AS p ON tm.steamid = p.steamid WHERE tm.gamemode = %i AND tm.map = '%s' AND p.steamid = '%s'", Gamemode, Map, SteamID);
...
public DisplayTimedMapsMenu3_2(Handle:owner, Handle:hndl, const String:error[], any:dp)
{
	...
	decl String:query[256];
	Format(query, sizeof(query), "SELECT DISTINCT tm.gamemode, tm.map, tm.time FROM timedmaps AS tm INNER JOIN players AS p ON tm.steamid = p.steamid WHERE tm.gamemode = %i AND tm.map = '%s' ORDER BY tm.time %s", Gamemode, Map, (Gamemode == GAMEMODE_SURVIVAL ? "DESC" : "ASC"));
...
{
	decl String:query[256];
	Format(query, sizeof(query), "SELECT tm.steamid, p.name FROM timedmaps AS tm INNER JOIN players AS p ON tm.steamid = p.steamid WHERE tm.gamemode = %i AND tm.map = '%s' AND tm.time = %f ORDER BY p.name ASC", Gamemode, Map, MapTime);
...
public DisplayClearPanelHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	{
		DoFastQuery(param1, "DELETE FROM timedmaps WHERE steamid = '%s'", SteamID);
...
CheckSurvivorsAllDown()
{
	...
	if (Score > 0)
		for (i = 0; i < InfectedCounter; i++)
		{
			...
			if (CurrentGamemodeID == GAMEMODE_VERSUS)
				...
				Format(query, sizeof(query), "UPDATE players SET points_realism_infected = points_realism_infected + %i, award_infected_win = award_infected_win + 1 WHERE steamid = '%s'", Score, ClientID);
			else
	...
	for (i = 0; i < IncapCounter; i++)
	{
		...
		if (CurrentGamemodeID == GAMEMODE_VERSUS)
			...
			Format(query, sizeof(query), "UPDATE players SET points_realism_survivors = points_realism_survivors - %i WHERE steamid = '%s'", Score, ClientID);
		else
		...
		if (TankDamageTotalCounter[Client] >= TankDamageTotal)
		{
			...
			if (Score > 0)
			{
				...
				if (CurrentGamemodeID == GAMEMODE_VERSUS)
					...
					Format(query, sizeof(query), "UPDATE players SET points_realism_infected = points_realism_infected + %i, award_bulldozer = award_bulldozer + 1 WHERE steamid = '%s'", Score, ClientID);
				else
...
//LogError("SurvivorDiedNamed - VictimName = %s", VictimName);
	...
	if (CurrentGamemodeID == GAMEMODE_VERSUS)
		...
		len += Format(query[len], sizeof(query)-len, "UPDATE players SET points_realism_infected = points_realism_infected + %i, realism_kills_survivors = realism_kills_survivors + 1 ", Score);
	else
...
SurvivorIncappedByInfected(Attacker, Victim, Mode = -1)
{
	...
	if (CurrentGamemodeID == GAMEMODE_VERSUS)
		...
		Format(query, sizeof(query), "UPDATE players SET points_realism_infected = points_realism_infected + %i, award_survivor_down = award_survivor_down + 1 WHERE steamid = '%s'", Score, AttackerID);
	else
...
public UpdateMapTimingStat(Handle:owner, Handle:hndl, const String:error[], any:dp)
{
	...
	decl String:MapName[MAX_LINE_WIDTH], String:ClientID[MAX_LINE_WIDTH], String:query[512], String:TimeLabel[32];
	...
	if (SQL_GetRowCount(hndl) > 0)
	{
		...
		if ((CurrentGamemodeID != GAMEMODE_SURVIVAL && OldTime <= TotalTime) || (CurrentGamemodeID == GAMEMODE_SURVIVAL && OldTime >= TotalTime))
		{
			if (Mode)
			{
				...
				PrintToChat(Client, "\x04[\x03RANK\x04] \x01You did not improve your best time \x04%s \x01to finish this map!", TimeLabel);
			}
		...
		else
		{
			if (Mode)
			{
				...
				PrintToChat(Client, "\x04[\x03RANK\x04] \x01Your new best time to finish this map is \x04%s\x01!", TimeLabel);
			}
	...
	else
	{
		...
		{
			SetTimeLabel(TotalTime, TimeLabel, sizeof(TimeLabel));
			PrintToChat(Client, "\x04[\x03RANK\x04] \x01It took \x04%s \x01to finish this map!", TimeLabel);
		}

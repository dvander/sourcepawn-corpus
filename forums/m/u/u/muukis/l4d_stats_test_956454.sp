...
#define PLUGIN_VERSION "1.2A71"
#define MAX_LINE_WIDTH 64
#define MAX_QUERY_COUNTER 256
#define DB_CONF_NAME "l4dstats_test"

#define INF_ID_SMOKER 1
#define INF_ID_BOOMER 2
#define INF_ID_HUNTER 3
#define INF_ID_WITCH 4
#define INF_ID_TANK 5
...
new ScoreTotalSurvivors = 0;
new ScoreTotalInfected = 0;

new Handle:TimerAllowCheckSurvivorsAllDown = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	CreateConVar("sm_l4dstats_version", PLUGIN_VERSION, "Custom L4D Stats Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	// Make that config!
	AutoExecConfig(true, "l4d_stats_test");
...
// All survivors down check delay

public Action:timer_AllowCheckSurvivorsDown(Handle:timer, any:data)
{
	TimerAllowCheckSurvivorsAllDown = INVALID_HANDLE;
}
...
public Action:timer_BoomerBlindnessCheck(Handle:timer, any:data)
{
	...
	if (data > 0 && !IsClientBot(data) && BoomerHitCounter[data] > 0)
	{
		...
		decl String:iID[MAX_LINE_WIDTH];
		GetClientAuthString(data, iID, sizeof(iID));
		Format(query, sizeof(query), "UPDATE players SET points_infected = points_infected + %i, award_perfect_blindness = award_perfect_blindness + %i, infected_boomer_blinded = infected_boomer_blinded + %i, infected_boomer_vomits = infected_boomer_vomits + 1 WHERE steamid = '%s'", Score, AwardCounter, OriginalHitCounter, iID);
		SendSQLUpdate(query);
		...
		if (Score > 0)
		{
			...
			if (Mode == 1 || Mode == 2)
			{
				PrintToChat(data, "\x04[\x03RANK\x04] \x01You have earned \x04%i \x01points for blinding \x05%i \x01Survivors!", Score, OriginalHitCounter);
			}
			else if (Mode == 3)
			{
				decl String:Name[MAX_LINE_WIDTH];
				GetClientName(data, Name, sizeof(Name));
				PrintToChatAll("\x04[\x03RANK\x04] \x05%s \x01has earned \x04%i \x01points for blinding \x05%i \x01Survivors!", Name, Score, OriginalHitCounter);
			}
		}
	}
}
...
//		return;
	...
	TimerBoomerPerfectCheck[Attacker] = CreateTimer(5.0, timer_BoomerBlindnessCheck, Attacker);
}
...
public Action:event_PlayerPounced(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Dmg >= PerfectDmgLimit)
	{
		...
		Format(Label, sizeof(Label), "Perfect");
	}
	else
	{
		...
		Format(Label, sizeof(Label), "Good");
	}
	...
	if (Mode == 1 || Mode == 2)
		PrintToChat(Attacker, "\x04[\x03RANK\x04] \x01You have earned \x04%i \x01points for landing a %s Pounce on \x05%s\x01!", Score, Label, VictimName);
	else if (Mode == 3)
		PrintToChatAll("\x04[\x03RANK\x04] \x05%s \x01has earned \x04%i \x01points for landing a %s Pounce on \x05%s\x01!", AttackerName, Score, Label, VictimName);
}
...
public Action:event_Award(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (AwardID == 67) // Protect friendly
	{
		...
		if (Mode == 1 || Mode == 2)
			PrintToChat(User, "\x04[\x03RANK\x04] \x01You have earned \x04%i \x01points for Protecting \x05%s\x01!", Score, RecepientName);
		else if (Mode == 3)
			PrintToChatAll("\x04[\x03RANK\x04] \x05%s \x01has earned \x04%i \x01points for Protecting \x05%s\x01!", UserName, Score, RecepientName);
	}
...
//			return;
	...
	else if (AwardID == 98 && !IsVersus) // Round restart
	{
		//LogError("(AwardID == 98) // Round restart");
...
public GetClientGameModePoints(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	while (SQL_FetchRow(hndl))
	{
		ClientGameModePoints[client][0] = SQL_FetchInt(hndl, 0);
		ClientGameModePoints[client][1] = SQL_FetchInt(hndl, 1);
	}
}
...
public GetClientTop10(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	while (SQL_FetchRow(hndl))
	{
		...
		Format(query, sizeof(query), "SELECT name, playtime, points + points_survivors + points_infected, kills, headshots FROM players WHERE steamid = '%s'", SteamID);
		SQL_TQuery(db, DisplayRank, query, client);
	}
}
...
ModifyScoreDifficultyFloat(BaseScore, Float:AdvMult, Float:ExpMult, bool:ToCeil=true, bool:IsSurvivorScore=false)
{
	if (BaseScore <= 0)
		return 0;
...
ModifyScoreDifficulty(BaseScore, AdvMult, ExpMult, bool:IsSurvivorScore=true)
{
	decl String:Difficulty[MAX_LINE_WIDTH];
	GetConVarString(cvar_Difficulty, Difficulty, sizeof(Difficulty));
...
ResetVars()
{
	if (TimerAllowCheckSurvivorsAllDown != INVALID_HANDLE)
		CloseHandle(TimerAllowCheckSurvivorsAllDown);

	TimerAllowCheckSurvivorsAllDown = CreateTimer(10.0, timer_AllowCheckSurvivorsDown);
...
CheckSurvivorsAllDown()
{
	if (CampaignOver || TimerAllowCheckSurvivorsAllDown != INVALID_HANDLE || !IsGameModeVersus())
		return;
	...
	for (i = 1; i <= MaxClients; i++)
	{
		...
		else if (ClientTeam == TEAM_SURVIVORS && ClientIsAlive && ClientIsIncap == 1)
			KilledSurvivor[IncapCounter++] = i;
	}
	
	CampaignOver = true;
	
	//LogError("CheckSurvivorsAllDown -> InfectedCounter: %i / AliveInfectedCounter: %i / IncapCounter: %i", InfectedCounter, AliveInfectedCounter, IncapCounter);
	...
	if (Score > 0 && Mode)
		PrintToChatAll("\x04[\x03RANK\x04] \x03ALL INFECTED \x01have earned \x04%i \x01points for killing all survivors!", Score);
	...
	if (Mode)
		PrintToChatAll("\x04[\x03RANK\x04] \x03ALL SURVIVORS \x01have \x03LOST \x04%i \x01points for \x03All Survivors Dying!", Score);
}
...
ShowFinalScore()
{
	...
	{
		if (IsGameModeVersus())
			PrintToChatAll("\x04[\x03RANK\x04] FINAL SCORE: \x03The Survivors \x01scored \x04%i \x01points and \x03The Infected \x01scored \x04%i \x01points.", ScoreTotalSurvivors, ScoreTotalInfected);
		else
			PrintToChatAll("\x04[\x03RANK\x04] FINAL SCORE: \x03The Survivors \x01scored \x04%i \x01points.", ScoreTotalSurvivors);
	}
}
...
//LogError("SurvivorDiedNamed - VictimName = %s", VictimName);
	...
	if (AttackerInfType != INF_ID_SMOKER && AttackerInfType != INF_ID_BOOMER && AttackerInfType != INF_ID_HUNTER && AttackerInfType != INF_ID_TANK)
		return;
...
SurvivorHurt(Attacker, Victim, Damage, AttackerInfType = -1, Handle:event = INVALID_HANDLE)
{
	...
	if (AttackerInfType == INF_ID_TANK && event != INVALID_HANDLE)
	{
		...
		if (RockHit > 0 && strcmp(Weapon, "tank_rock", false) == 0)
		{
			...
			if (Mode == 1 || Mode == 2)
				PrintToChat(Attacker, "\x04[\x03RANK\x04] \x01You have earned \x04%i \x01points for throwing a rock on \x05%s\x01!", RockHit, VictimName);
			else if (Mode == 3)
			{
				decl String:AttackerName[MAX_LINE_WIDTH];
				GetClientName(Attacker, AttackerName, sizeof(AttackerName));
				PrintToChatAll("\x04[\x03RANK\x04] \x05%s \x01has earned \x04%i \x01points for throwing a rock on \x05%s\x01!", AttackerName, RockHit, VictimName);
			}
		}
	}
	else
...
SurvivorHurtExternal(Handle:event, Victim)
{
	...
	if (!IsPlayerAlive(Victim))
	{
		...
		if (VictimHealth < 0 && VictimHealth * -1 < Damage)
			Damage += VictimHealth;
		else
			return;
	}
...
AnnounceMedKitPenalty(Mode = -1)
{
	...
	if (Mode)
		PrintToChatAll("\x04[\x03RANK\x04] \x03ALL SURVIVORS \x01now earns only \x04%i%% \x01of their normal points for using their \x05%i%s MedKit\x01!", RoundToNearest(ReductionFactor * 100), MedkitsUsedCounter, (MedkitsUsedCounter == 1 ? "st" : (MedkitsUsedCounter == 2 ? "nd" : (MedkitsUsedCounter == 3 ? "rd" : "th"))));
}

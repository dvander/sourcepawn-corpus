...
{
    name = "L4D Stats (custom)",
    author = "msleeper (customized by muukis)",
...
CheckCurrentMapDB()
{
    ...
    Format(query, sizeof(query), "SELECT name FROM maps WHERE name = '%s'", MapName);
...
public InsertMapDB(Handle:owner, Handle:hndl, const String:error[], any:data)
{
    ...
    if (!SQL_GetRowCount(hndl))
    {
        ...
        Format(query, sizeof(query), "INSERT IGNORE INTO maps SET name = '%s', custom = 1", MapName);

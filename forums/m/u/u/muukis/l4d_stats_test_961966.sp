...
CheckSurvivorsAllDown()
{
	...
	if (Mode)
		PrintToChatAll("\x04[\x03RANK\x04] \x03ALL SURVIVORS \x01have \x03LOST \x04%i \x01points for \x03All Survivors Dying\x01!", Score);
}
...
AnnounceMedKitPenalty(Mode = -1)
{
	...
	if (Mode)
		PrintToChatAll("\x04[\x03RANK\x04] \x03ALL SURVIVORS \x01now earns only \x04%i%% \x01of their normal points after using their \x05%i%s MedKit\x01!", RoundToNearest(ReductionFactor * 100), MedkitsUsedCounter, (MedkitsUsedCounter == 1 ? "st" : (MedkitsUsedCounter == 2 ? "nd" : (MedkitsUsedCounter == 3 ? "rd" : "th"))));
}

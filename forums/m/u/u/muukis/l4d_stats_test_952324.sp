...
new Handle:cvar_InfectedTankDamage = INVALID_HANDLE;
		...
		if (Mode)
		{
			if (GetEventBool(event, "headshot"))
			{
				if (Mode > 1)
				{
					...
					PrintToChatAll("\x04[\x03RANK\x04] \x05%s \x01has earned \x04%i \x01points for killing%s \x05%s \x01with a \x04HEAD SHOT!", AttackerName, Score, (VictimIsBot ? " a" : ""), VictimName);
				}
				else
					PrintToChat(Attacker, "\x04[\x03RANK\x04] \x01You have earned \x04%i \x01points for killing%s \x05%s \x01with a \x04HEAD SHOT!", Score, (VictimIsBot ? " a" : ""), VictimName);
			}
			else
			{
				if (Mode > 2)
...
public Action:event_PounceSave(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Victim > 0)
	{
		PlayerLunged[Victim][0] = 0;
		PlayerLunged[Victim][1] = 0;
	}
...
public Action:event_AbilityUse(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	// Increment Boomer vomit counter
	//if (GetClientTeam(Player) == TEAM_INFECTED && ClientInfectedType[Player] == INF_ID_BOOMER)
	//{
	//}
}
...
public Action:event_PlayerPounced(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (DEBUG)
		LogError("POUNCEDAMAGE=%i / PerfectDmgLimit=%i / NiceDmgLimit=%i", Dmg, PerfectDmgLimit, NiceDmgLimit);
...
ModifyScoreDifficultyFloat(BaseScore, Float:AdvMult, Float:ExpMult, bool:ToCeil=true)
{
	if (BaseScore <= 0)
		return 0;
	...
	if (ToCeil)
		return RoundToCeil(ModifiedScore);
	else
	  return RoundToFloor(ModifiedScore);
}

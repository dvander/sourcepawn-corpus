...
public OnPluginStart()
{
	...
	cvar_AnnounceToTeam = CreateConVar("sm_l4dstats_announceteam", "1", "Chat announcment team messages to the team only mode. 0 = Print Messages To All Teams, 1 = Print Messages To Own Team Only", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public Action:event_TankKilled(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (Mode && Score > 0)
		PrintToChatTeam(TEAM_SURVIVORS, "\x04[\x03RANK\x04] \x03ALL SURVIVORS \x01have earned \x04%i \x01points for killing a Tank with \x05%i Deaths!", Score, Deaths);
...
public Action:timer_PanicEventEnd(Handle:timer, Handle:hndl)
{
	...
	if (PanicEvent && !PanicEventIncap)
	{
		...
		if (Score > 0)
		{
			...
			if (Mode)
				PrintToChatTeam(TEAM_SURVIVORS, "\x04[\x03RANK\x04] \x03ALL SURVIVORS \x01have earned \x04%i \x01points for \x05No Incapicitates After Panic Event!", Score);
		}
	}
...
public Action:event_PlayerBlindEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (PlayerVomited && !PlayerVomitedIncap)
	{
		...
		if (Score > 0)
		{
			...
			if (Mode)
				PrintToChatTeam(TEAM_SURVIVORS, "\x04[\x03RANK\x04] \x03ALL SURVIVORS \x01have earned \x04%i \x01points for \x05No Incapicitates After Boomer Mob!", Score);
		}
	}
...
CheckSurvivorsAllDown()
{
	LogError("CheckSurvivorsAllDown()");
...
PrintToChatTeam(Team, const String:Message[], any:...)
{
	new String:FormattedMessage[MAX_LINE_WIDTH];
	VFormat(FormattedMessage, strlen(FormattedMessage), Message, 3);

	if (Team > 0 && GetConVarInt(cvar_AnnounceToTeam))
	{
		for (new i = 1; i <= MAXPLAYERS; i++)
			if (IsClientConnected(i) && IsClientInGame(i) && !IsClientBot(i) && GetClientTeam(i) == Team)

...
public InitDB()
{
  new String:error[256], String:query[1024];
	...
	Format(query, sizeof(query), "%s%s%s%s%s%s%s%s%s%s%s%s%s",
		"CREATE TABLE IF NOT EXISTS `perkmod` (",
...
public GetDefaultPerks(client)
{
  ...
  Format(query, sizeof(query), "SELECT sur1, sur2, inf1, inf2, inf3, inf4 FROM perkmod WHERE steamid = '%s'", authid);
...
public SQLGetDefaultPerks(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	if (hndl != INVALID_HANDLE && SQL_FetchRow(hndl))
	{
		...
		g_iInf1[data] = SQL_FetchInt(hndl, 2);
		g_iInf2[data] = SQL_FetchInt(hndl, 3);
		g_iInf3[data] = SQL_FetchInt(hndl, 4);
		g_iInf4[data] = SQL_FetchInt(hndl, 5);
		if (g_iSur1[data]==0)
...
public SaveDefaultPerks(client)
{
	...
	GetClientAuthString(client, authid, sizeof(authid));
  Format(query, sizeof(query), "SELECT sur1 FROM perkmod WHERE steamid = '%s'", authid);
...
public SQLSaveDefaultPerks(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	if (SQL_GetRowCount(hndl) > 0)
	{
	  ...
	  Format(query, sizeof(query), "UPDATE perkmod SET sur1 = '%d', sur2 = '%d', inf1 = '%d', inf2 = '%d', inf3 = '%d', inf4 = '%d' WHERE steamid = '%s'", g_iSur1[data], g_iSur2[data], g_iInf1[data], g_iInf2[data], g_iInf3[data], g_iInf4[data], authid);
	}
	else
	{
		GetClientAuthString(data, authid, sizeof(authid));
	  Format(query, sizeof(query), "INSERT INTO perkmod (sur1, sur2, inf1, inf2, inf3, inf4, steamid) VALUES ('%d', '%d', '%d', '%d', '%d', '%d', '%s')", g_iSur1[data], g_iSur2[data], g_iInf1[data], g_iInf2[data], g_iInf3[data], g_iInf4[data], authid);
	}

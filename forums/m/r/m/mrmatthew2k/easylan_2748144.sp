...
int backupinv[MAXPLAYERS + 1][4]; // 0 - Team Needed?, 1 - Team, 2- Inv Needed?
...
char gamestate_ct_team[LINE_LEN] = "Counter-Terrorists";
char gamestate_t_team[LINE_LEN] = "Terrorists";
char hostnameOld[LINE_LEN] = "Counter-Strike: Global Offensive";


public void Client_SetMoney(int client, int money)
{
    SetEntProp(client, Prop_Send, "m_iAccount", money);
}


public int Client_GetMoney(client)
{
...
public void OnClientDisconnect(int client) {
	...
	if (gamestate_live && GetClientTeam(client) > CS_TEAM_SPECTATOR && !IsFakeClient(client) && g_AutomaticConVar.BoolValue) {
		...
		backupinv[client][InventoryVal] = Client_GetMoney(client);
...
public Action NoPlayersCheck(Handle timer) {
	if (gamestate_live && (GetPlayerCount() < 1)) {
		...
		char commandbuff[LINE_LEN];
		Format(commandbuff, LINE_LEN, "hostname \"%s\"", hostnameOld);
...
public Action CheckTeam(UserMsg msg_id, Protobuf msg, const int[] players, int playersNum, bool reliable, bool init)  {
	...
	if (g_PluginEnabledConVar.BoolValue && StrEqual(buffermsg, "team", true) && gamestate_live && backupinv[client][TeamSwitchNeeded] == 1) { 
		...
		if (backupinv[client][InventoryRestoreNeeded] == 1) {
			backupinv[client][InventoryRestoreNeeded] = 0;
			Client_SetMoney(client, backupinv[client][InventoryVal]);
		}
...
public void Event_Halftime() {
	new String:teambuffer[LINE_LEN];
	teambuffer = gamestate_ct_team;
	gamestate_ct_team = gamestate_t_team;
	gamestate_t_team = teambuffer;

...
public OnMapStart()
{
	...
	if (strncmp(mapname, "dr_", 3, false) == 0 || strncmp(mapname, "deathrun_", 9, false) == 0 || strncmp(mapname, "vsh_dr_", 6, false) == 0 || strncmp(mapname, "vsh_deathrun_", 6, false) == 0)
	{
		...
		g_isDRmap = true;
		Steam_SetGameDescription("DeathRun Redux 2019");
...
public Action:NextDeath(client,args)
{
	...
	new max = g_queue_Priory[1];
	new death = 1;
	for (new i=1; i<=MAXPLAYERS;i++)
		if (max<=g_queue_Priory[i]) {
			max = g_queue_Priory[i];
			death = i;
	}
	char name[32];
	GetClientName(death, name,sizeof(name));
...
public BeDeathMenuHandler(Handle:menuDBD, MenuAction:action, client, buttonnum)
{
	if (action == MenuAction_Select)
	{
		if (buttonnum == 0)
		{
			...
			CPrintToChat(client, "%t %t", "Plugin Tag", "You can be selected as Death");
		}
		else if (buttonnum == 1)
		{
			...
			SetClientCookie(client, g_DRCookie, sPref);
			CPrintToChat(client, "%t %t", "Plugin Tag", "You can't be selected as Death");
		}
		else if (buttonnum == 2)
		{
			g_dontBeDeath[client] = DBD_THISMAP;
			CPrintToChat(client, "%t %t", "Plugin Tag", "You can't be selected as Death for this map");
		}
...
stock BalanceTeams()
{
	...
	if(GetClientCount(true) > 4)
	{
		...
		if(new_death == -1)
		{
			CPrintToChatAll("%t %t", "Plugin Tag", "Couldn't found a valid Death");
			return;
		}
		...
		{
			CPrintToChatAll("%t %t", "Plugin Tag", "Death isn't in game");
			return;
		}
		
		CPrintToChatAll("%t %t", "Plugin Tag", "Player is the Death", new_death);
	...
	if (GetClientCount(true) <= 4) 
	{
		CPrintToChatAll("%t %t", "Plugin Tag", "Few people, don't be death setting is disabled");
		new maxPionts = 1;
		for (new i = 1; i <= MaxClients; i++){
		    if ((maxPionts < g_queue_Priory[i])&&(IsClientConnected(i))&&(IsClientInGame(i))){
		        maxPionts = g_queue_Priory[i];
	...
	{
		CPrintToChatAll("%t %t", "Plugin Tag", "This game-mode requires at least two people to start");
	}
	}
}

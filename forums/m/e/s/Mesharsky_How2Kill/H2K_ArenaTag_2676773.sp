...
public void Multi1v1_AfterPlayerSetup(int client)
{
	...
	char sIPa[26];
	char sBuffer[3];
	GetClientIP(client, sIPa, sizeof(sIPa));
	GeoipCode2(sIPa, sBuffer);
	
	if (IsValidClient(client))
	{
		Format(TagScoreboard, sizeof(TagScoreboard), "%i | %s", ArenaNum, sBuffer);
		CS_SetClientClanTag(client, TagScoreboard);
	}
	if(IsPlayerVip(client))
	{
		Format(TagScoreboard, sizeof(TagScoreboard), "%i | %s | VIP", ArenaNum, sBuffer);
		CS_SetClientClanTag(client, TagScoreboard);
	}
	if (GetAdminFlag(GetUserAdmin(client), Admin_Root)) //z flag
	{
		Format(TagScoreboard, sizeof(TagScoreboard), "%i | %s | H@", ArenaNum, sBuffer);
		CS_SetClientClanTag(client, TagScoreboard);
	}
}

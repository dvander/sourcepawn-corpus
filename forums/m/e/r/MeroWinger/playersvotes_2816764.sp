...
public OnPluginStart()
{
	...
	CreateConVar("sm_playersvotes_version", PLUGIN_VERSION, "Players Votes Version", FCVAR_DONTRECORD|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);

	g_hVoteRatio[KICK] = CreateConVar("sm_votekick_ratio", "0.60", "ratio required for successful votekick.", FCVAR_NONE, true, 0.0, true, 1.0);
	g_hVoteRatio[BAN]  = CreateConVar("sm_voteban_ratio",  "0.80", "ratio required for successful voteban.",  FCVAR_NONE, true, 0.0, true, 1.0);
	g_hVoteRatio[MAP]  = CreateConVar("sm_votemap_ratio",  "0.60", "ratio required for successful votemap.",  FCVAR_NONE, true, 0.0, true, 1.0);
	g_hVoteRatio[MUTE] = CreateConVar("sm_votemute_ratio", "0.60", "ratio required for successful votemute.", FCVAR_NONE, true, 0.0, true, 1.0);

	g_hVoteMinimum[KICK] = CreateConVar("sm_votekick_minimum", "4.0", "minimum votes required for successful votekick. -1 to disable voting", FCVAR_NONE, true, -1.0, true, 64.0);
	g_hVoteMinimum[BAN]  = CreateConVar("sm_voteban_minimum",  "4.0", "minimum votes required for successful voteban.  -1 to disable voting", FCVAR_NONE, true, -1.0, true, 64.0);
	g_hVoteMinimum[MAP]  = CreateConVar("sm_votemap_minimum",  "4.0", "minimum votes required for successful votemap.  -1 to disable voting", FCVAR_NONE, true, -1.0, true, 64.0);
	g_hVoteMinimum[MUTE] = CreateConVar("sm_votemute_minimum", "4.0", "minimum votes required for successful votemute. -1 to disable voting", FCVAR_NONE, true, -1.0, true, 64.0);

	g_hVoteDelay[KICK] = CreateConVar("sm_votekick_delay", "60.0", "time in seconds before votekick is allowed after map start", FCVAR_NONE, true, 0.0, true, 1000.0);
	g_hVoteDelay[BAN]  = CreateConVar("sm_voteban_delay",  "60.0", "time in seconds before voteban is allowed after map start",  FCVAR_NONE, true, 0.0, true, 1000.0);
	g_hVoteDelay[MAP]  = CreateConVar("sm_votemap_delay",  "60.0", "time in seconds before votemap is allowed after map start",  FCVAR_NONE, true, 0.0, true, 1000.0);
	g_hVoteDelay[MUTE] = CreateConVar("sm_votemute_delay", "60.0", "time in seconds before votemute is allowed after map start", FCVAR_NONE, true, 0.0, true, 1000.0);

	g_hVoteLimit[KICK] = CreateConVar("sm_votekick_limit", "-1.0", "number of kick votes allowed per player, per map.  0 to disable voting.  -1 for no limit", FCVAR_NONE, true, -1.0, true, 1000.0);
	g_hVoteLimit[BAN]  = CreateConVar("sm_voteban_limit",  "-1.0", "number of ban votes allowed per player, per map.  0 to disable voting.  -1 for no limit",  FCVAR_NONE, true, -1.0, true, 1000.0);
	g_hVoteLimit[MAP]  = CreateConVar("sm_votemap_limit",  "-1.0", "number of map votes allowed per player, per map.  0 to disable voting.  -1 for no limit",  FCVAR_NONE, true, -1.0, true, 1000.0);
	g_hVoteLimit[MUTE] = CreateConVar("sm_votemute_limit", "-1.0", "number of mute votes allowed per player, per map.  0 to disable voting.  -1 for no limit", FCVAR_NONE, true, -1.0, true, 1000.0);

	g_hVoteTeam[KICK]  = CreateConVar("sm_votekick_team_restrict", "0.0", "restrict kick votes to teams.  affects ratios.  1 to enable, 0 to disable.", FCVAR_NONE, true, 0.0, true, 1.0);
	g_hVoteTeam[BAN]   = CreateConVar("sm_voteban_team_restrict",  "0.0", "restrict ban votes to teams.  affects ratios.  1 to enable, 0 to disable.",  FCVAR_NONE, true, 0.0, true, 1.0);
	g_hVoteTeam[MUTE]  = CreateConVar("sm_votemute_team_restrict", "0.0", "restrict mute votes to teams.  affects ratios.  1 to enable, 0 to disable.", FCVAR_NONE, true, 0.0, true, 1.0);

	g_hVoteMapLast          = CreateConVar("sm_votemap_lastmaps",     "4.0", "last number of played maps that will not show in votemap list",                           FCVAR_NONE, true,  0.0, true,  64.0);
	g_hMapExtendTime        = CreateConVar("sm_votemap_extend",      "20.0", "number of minutes to add to the timelimit if the players vote to extend.  -1 to disable", FCVAR_NONE, true, -1.0, true, 120.0);
	g_hMapMaxExtends        = CreateConVar("sm_votemap_max_extends",  "1.0", "number of extensions to allow per map.  -1 for no limit",                                 FCVAR_NONE, true, -1.0, true, 100.0);
	g_hMapChangeImmediately  = CreateConVar("sm_votemap_immediate",    "1.0", "1 to change map immediately after a map wins a votemap.  0 for setting nextmap",          FCVAR_NONE, true,  0.0, true,   1.0);

	g_hMapTimeLimit = FindConVar("mp_timelimit");

	g_hVotesInterval = CreateConVar("sm_playersvotes_interval",     "15.0", "interval in seconds between another vote cast", FCVAR_NONE, true, 0.0, true, 10000.0);
	g_hVotesTimeout  = CreateConVar("sm_playersvotes_menu_timeout", "0.0",  "number of seconds to display voting menus.  0 for no limit", FCVAR_NONE, true, 0.0, true, 10000.0);

	g_hVoteBanTime       = CreateConVar("sm_voteban_time",   "60.0", "ban time in minutes.  0 to ban permanently", FCVAR_NONE);
	g_hVoteBanReasonsCvar = CreateConVar("sm_voteban_reasons",    "Hacking; Spamming; Griefing", "semi-colon delimited list of ban reasons.  (ex: \"Hacking; Spamming; Griefing\")", FCVAR_NONE);

	g_hVoteImmunity    = CreateConVar("sm_playersvotes_immunity", "0.0", "admins with equal or higher immunity level will not be affected by votekick, ban, or mute.  0 to immunize all admins.  -1 to ignore", FCVAR_NONE, true, -1.0, true, 99.0);
	g_hAdminGroupsCvar = CreateConVar("sm_playersvotes_immunegroups", "", "admins that are members of these groups will not be affected by votekick, ban, or mute.  (ex: \"Full Admins; Clan Members; etc\")", FCVAR_NONE);
...
bool:GetIdent (client, String:sClientIdent[], nClientIdentSize)
{
	...
	GetClientAuthId(client, AuthId_Steam2, sClientIdent, nClientIdentSize);

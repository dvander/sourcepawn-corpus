...
public OnAdminMenuReady(Handle aTopMenu)
{
	...
	/* Find the "Fun Commands" category. Before, it was ADMINMENU_PLAYERCOMMANDS" */
	TopMenuObject player_commands = hTopMenu.FindCategory("Fun Commands");
...
///// Following is to create the Fun Commands menu

TopMenuObject obj_funcommands  = INVALID_TOPMENUOBJECT;

public void OnAdminMenuCreated(Handle aTopMenu)
{
	/* Block us from being called twice */
	if (aTopMenu == GetAdminTopMenu() && obj_funcommands != INVALID_TOPMENUOBJECT)
	{
		return;
	}
 
	obj_funcommands = AddToTopMenu(aTopMenu,
		"Fun Commands",
...
public void FunCategoryHandler(Handle aTopMenu, 
			...
			int maxlength)
{
	...
	{
		Format(buffer, maxlength, "Fun Commands:");
	}

...
public Action:OnMsgVGUIMenu(UserMsg:msg_id, Handle:bf, const players[], playersNum, bool:reliable, bool:init)
{
	...
	new UserMessageType:type = GetUserMessageType();
	
	decl count;
	if (type == UM_Protobuf)
	{
		// check menu name
		PbReadString(bf, "name", buffer1, sizeof(buffer1));
		if (strcmp(buffer1, "info") != 0)
			return Plugin_Continue;
		
		// make sure it's not a hidden one
		if (PbReadBool(bf, "show") != true)
			return Plugin_Continue;
		
		count = PbGetRepeatedFieldCount(bf, "subkeys");
	}
	else
	{
		// check menu name
	...
	if (type == UM_Protobuf)
	{
		...
		{
			new Handle:subkey = PbReadRepeatedMessage(bf, "subkeys", i);
			PbReadString(subkey, "name", buffer1, sizeof(buffer1));
			PbReadString(subkey, "str", buffer2, sizeof(buffer2));

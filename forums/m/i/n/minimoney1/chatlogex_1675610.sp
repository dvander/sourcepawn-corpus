...
public OnPluginStart()
{
	...
	sm_chatlogex_db = CreateConVar("sm_chatlogex_db", "default", "\"default\" [default] The database configuration to use for logging this server's chat to.");
...
public Action:LogPlayerConnect(Handle:timer, Handle:pack)
{
	if(!GetConVarBool(sm_chatlogex_srvmsgs) || !GetConVarBool(sm_chatlogex_showplayercon))
		return Plugin_Continue;
	ResetPack(pack);
	new i = GetPackPosition(pack);
	decl String:text[256], String:username[65];
	
	ReadPackString(pack, username, sizeof(username));
...
public Action:Event_PlayerConnect(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	decl String:adderss2[64], String:name2[64];
	GetEventString(event, "address", adderss2, sizeof(adderss2));
	GetEventString(event, "name", name2, sizeof(name2));
	WritePackString(pack, adderss2);
	WritePackString(pack, name2);
	if(hdatabase == INVALID_HANDLE)
		CreateTimer(2.0, LogPlayerConnect, pack);
	else
		LogPlayerConnect(INVALID_HANDLE, pack);

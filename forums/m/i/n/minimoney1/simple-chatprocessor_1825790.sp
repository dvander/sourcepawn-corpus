...
public OnPluginStart()
{
	...
	*/
	...
	g_hChatMsgPrivFwdPre = CreateForward(ET_Hook, Param_CellByRef, Param_CellByRef, Param_String, Param_String);
	g_hChatMsgPrivFwdPost = CreateForward(ET_Ignore, Param_Cell, Param_Cell, Param_String, Param_String);
...
{
	static time = 0, staticSender = SENDER_WORLD,  String:strStaticName[MAXLENGTH_NAME] = "", String:strStaticMessage[MAXLENGTH_INPUT] = "";
	...
	*/
	...
	if ((timeNow == time) && (cpSender == staticSender) && (!strcmp(strStaticName, cpSender_Name)) && (!strcmp(strStaticMessage, cpMessage)))
	{
	...
	{
		time = timeNow;
		strcopy(strStaticName, sizeof(strStaticName), cpSender_Name);
		strcopy(strStaticMessage, sizeof(strStaticMessage), cpMessage);
	}

	if (!GetForwardFunctionCount(g_hFwdOnChatMessage) && !GetForwardFunctionCount(g_hChatMsgPrivFwdPre))
	{
		if (GetForwardFunctionCount(g_hChatMsgPrivFwdPost))
		{
	...
	*/
	...
	Call_StartForward(g_hChatMsgPrivFwdPre);
	Call_PushCellRef(cpSender);
	Call_PushArrayEx(bRecipitents, MAXPLAYERS + 1, SM_PARAM_COPYBACK);
	Call_PushStringEx(cpSender_Name, sizeof(cpSender_Name), SM_PARAM_STRING_UTF8|SM_PARAM_STRING_COPY, SM_PARAM_COPYBACK);
	Call_PushStringEx(cpMessage, sizeof(cpMessage), SM_PARAM_STRING_UTF8|SM_PARAM_STRING_COPY, SM_PARAM_COPYBACK);
...
{
	return AddToForward((GetNativeCell(1) == ChatMessageHookType_PreChat ? g_hChatMsgPrivFwdPre : g_hChatMsgPrivFwdPost), plugin, Function:GetNativeCell(2));
}

public Native_UnookChatMessage(Handle:plugin, numParams)
{
	return RemoveFromForward((GetNativeCell(1) == ChatMessageHookType_PreChat ? g_hChatMsgPrivFwdPre : g_hChatMsgPrivFwdPost), plugin, Function:GetNativeCell(2));
}

...
stock bool:OperateOnWeapon(weapon)
{
	decl String:entName[64];
	GetEntityName(weapon, entName, sizeof(entName));
	if (g_bFlash && !strcmp(entName, "weapon_flashbang", false))
		return true;
	else if (g_bKnife && !strcmp(entName, "weapon_knife", false))
		return true;
	else if (g_bSmoke && !strcmp(entName, "weapon_smoke", false))
		return true;
	else if (g_bFrag && !strcmp(entName, "weapon_frag", false))
		return true;
	return false;
}

stock GetEntityName(entity, String:name[], maxlength)
{
	return GetEntPropString(entity, Prop_Data, "m_iName", name, maxlength);
}
...
Void_IssueEffect(client, effect)
{
	...
	switch(effect)
	{
		...
		{		
			...
			for(new i = 1; i <= MaxClients; i++)
				if(i != client && IsClientInGame(i))
					PrintToChat(i, "%s%t", g_sPrefixConsole, _sBuffer1, g_sName[client], _iTemp);
		}
		...
		case EFFECT_HEALTH_RAND:
		{
			...
			for(new i = 1; i <= MaxClients; i++)
				if(i != client && IsClientInGame(i))
					PrintToChat(i, "%s%t", g_sPrefixConsole, _sBuffer1, g_sName[client]);
		}
		case EFFECT_GRAVITY_GAIN:
		{
			...
			for(new i = 1; i <= MaxClients; i++)
				if(i != client && IsClientInGame(i))
					PrintToChat(i, "%s%t", g_sPrefixConsole, _sBuffer1, g_sName[client], _iDisplay, "%%");
		}
		...
		case EFFECT_MODEL:
		{
			...
			for(new i = 1; i <= MaxClients; i++)
				if(i != client && IsClientInGame(i))
					PrintToChat(i, "%s%t", g_sPrefixConsole, _sBuffer1, g_sName[client], g_sEffectModel[_iTemp][0]);
		}
		case EFFECT_SNEAKY:
		{
			switch(g_iTeam[client])

...
new Handle:g_hSteamType = INVALID_HANDLE;
...
{
	CreateConVar("sm_xenforo_admins", PLUGIN_VERSION, "Admin and XenForo integration", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_UNLOGGED|FCVAR_DONTRECORD|FCVAR_REPLICATED|FCVAR_NOTIFY);

	g_hSteamType = CreateConVar("sm_xenforo_steam_type", "2", "What Steam integration addon are you using?\n0. Use a custom user field set by sm_xenforo_fieldname\n1. Use Steam Identity Service by Darkimmortal\n2. Use Steam Authentication & Integration by Morgan", FCVAR_PLUGIN|FCVAR_UNLOGGED|FCVAR_PROTECTED|FCVAR_NOT_CONNECTED, true, 0.0, true, 2.0);
	g_hFieldName = CreateConVar("sm_xenforo_fieldname", "Steam", "XenForo SQL Field Name\nThis ConVar will not be used if sm_xenforo_steam_type is at 1 or 2.", FCVAR_PLUGIN|FCVAR_UNLOGGED|FCVAR_PROTECTED|FCVAR_NOT_CONNECTED);

	HookConVarChange(g_hSteamType, OnConVarChanged);
	HookConVarChange(g_hFieldName, OnConVarChanged);
...
{
	g_SteamType = GetConVarInt(g_hSteamType);
...
public OnDBIConnectionEstablished(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	else if (g_SteamType == 1)
	{
		FormatEx(sQuery, sizeof(sQuery), "\
	   SELECT ufv.field_value, u.username, u.user_group_id, u.secondary_group_ids \
	   FROM xf_user_field_value AS ufv \
	   LEFT JOIN xf_user AS u ON (u.user_id = ufv.user_id) \
	   WHERE ufv.field_id = 'Steam' \
	   AND NOT ufv.field_value = '';");
	}

	else if (g_SteamType == 2)
	{
		FormatEx(sQuery, sizeof(sQuery), "\
	   ...
	   LEFT JOIN xf_user AS u ON (u.user_id = ufv.user_id) \
	   WHERE ufv.provider = 'steam' \
	   AND NOT ufv.provider_key = '';");
	}

	else
	{
		SetFailState("Can't find the value of sm_xenforo_steam_type ConVar, please make sure you set it.");
	}
...
public ThreadedOnAdminQueryDone(Handle:owner, Handle:hndl, const String:error[], any:data)
{
	...
	{
		if (g_SteamType == 0 || g_SteamType == 1)
		{
			sSteamID[0] = '\0';
...
{
	if (convar == g_hSteamType)
		g_SteamType = StringToInt(newValue);

	else if (convar == g_hFieldName)

...
#define BASE_URL "http://api.steampowered.com/ISteamUser/GetFriendList/v0001/"
...
stock BanFriends(String:cSteamId[], client = -1)
{
	...
	new HTTPRequestHandle:http = Steam_CreateHTTPRequest(HTTPMethod_GET, BASE_URL);
	if (http != INVALID_HTTP_HANDLE)
	{
		...
		Steam_SetHTTPRequestGetOrPostParameter(http, "relationship", "friend");
		Steam_SetHTTPRequestGetOrPostParameter(http, "format", "vdf");
		Steam_SetHTTPRequestGetOrPostParameter(http, "key", g_strAPIKey);
		Steam_SetHTTPRequestGetOrPostParameter(http, "key", cSteamId);
		Steam_SendHTTPRequest(http, OnFileDownloaded, pack);
	}
}

public OnFileDownloaded(HTTPRequestHandle:HTTPRequest, bool:requestSuccessful, HTTPStatusCode:statusCode, any:pack)
{
	new userId = ReadPackCell(pack);
	new client = userId == -1 ? -1 : GetClientOfUserId(userId);
	if (!requestSuccessful || statusCode >= HTTPStatusCode_BadRequest)
	{
		if (client > 0)
		{

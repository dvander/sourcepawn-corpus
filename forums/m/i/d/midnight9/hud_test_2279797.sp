...
new bool:showTankPanel[MAXPLAYERS + 1];
...
public Action:MenuRefresh_Timer(Handle:timer)
{
	...
	if (isTankActive)
	{
		...
		for (new i = 1; i <= MaxClients; i++)
		{
			if (IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) != 2 && i != tankClient && !hiddenTankPanel[tankClient] && !showTankPanel[tankClient])
			{
...
public Action:PanelTank(Handle:timer)
{
	CreateTimer(0.5, PanelDelay, _, TIMER_REPEAT);
}

public Action:PanelDelay(Handle:timer)
{
	
	// tank-only hud
	static Handle:tankPanel = INVALID_HANDLE;
	if (!IsFakeClient(tankClient) && !showTankPanel[tankClient]) {
		static String:rageBuffer[64];
		FormatEx(rageBuffer, sizeof(rageBuffer), "Rage   : %d%% (Pass #%i)", GetTankFrustration(tankClient), L4D2Direct_GetTankPassedCount());
		...
		SetPanelTitle(tankPanel, "Error Tank HUD");
...
public Action:ToggleTanksPanel_Cmd(client,args)
{
	showTankPanel[tankClient] = !showTankPanel[tankClient];
	if(showTankPanel[tankClient])
	{
		ReplyToCommand(client,"Tank HUD is now disabled.");
	}
	else

...
public OnPluginStart() {
	...
	if (strcmp(game, "left4dead", false) == 0) {
		PrintToServer("[EXT.MAPCONFIG] It seems you are using Left4Dead. Hibernation-support is enabled.");
		if (!HookEventEx("server_spawn", Event_HibernationEnded)) {
			SetFailState("Failed to create required hook(s).");
		}
	}
...
public OnConfigsExecuted() {
	executeExtendedMapConfigs();
}

public Event_HibernationEnded(Handle:event, const String:name[], bool:dontBroadcast) {
	PrintToServer("[EXT.MAPCONFIG] Server has come out of hibernation.");
	executeExtendedMapConfigs();
}

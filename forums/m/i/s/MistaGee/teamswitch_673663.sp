...
// Changes:
// 1.4-1:
//      * Insurgency fixes (beta version though)
// 1.4:
//      * erm - I should've kept track of this :/
// 1.3:
...
// Default team indices
#define TEAM_1    2
#define TEAM_2    3
#define TEAM_SPEC 1

#define INS_TEAMNAME_1 "MRN"
#define INS_TEAMNAME_2 "INS"

#define TEAMSWITCH_VERSION    "1.4-1"
...
new	Handle:hAdminMenu	= INVALID_HANDLE,
	...
	bool:running_insurgency = false,
	teamIndex1    = TEAM_1,
	teamIndex2    = TEAM_2,
	teamIndexSpec = TEAM_SPEC;
...
public OnPluginStart(){
	...
	else if( StrEqual( theFolder, "insurgency" ) ){
		// Correct the team indices
		teamIndex1 = 1;
		teamIndex2 = 2;
		teamIndexSpec = 3;
		running_insurgency = true;
...
public OnMapStart(){
	PrintToServer( "[TS] Getting Team names" );
	if( !running_insurgency ){
		GetTeamName( teamIndex1, teamName1, sizeof(teamName1) );
		GetTeamName( teamIndex2, teamName2, sizeof(teamName2) );
		}
	else{
		// This sucks monkey ass, but GetTeamName segfaults
		teamName1 = INS_TEAMNAME_1;
		teamName2 = INS_TEAMNAME_2;
		}
...
public Handle_ModeFromSpec( Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength ){
	if (action == TopMenuAction_DisplayOption){
		Format( buffer, maxlength, "To Spectator" );
		}
	else if( action == TopMenuAction_SelectOption){
		ShowPlayerSelectionMenu( param, TeamSwitchEvent_FromSpec );
		}
	}
...
void:ShowPlayerSelectionMenu( client, TeamSwitchEvent:event, item = 0 ){
	...
	for( new i = 1; i < mc; i++ ){
		if( IsClientInGame(i) ){
			...
			// If we're supposed to switch a spec back in the game, skip all non-specs,
			// else skip all players who are in neither team
			if( event == TeamSwitchEvent_FromSpec ? ( cTeam != teamIndexSpec ) : ( cTeam != teamIndex1 && cTeam != teamIndex2 ) )
				continue;
			...
			switch( event ){
				...
				case TeamSwitchEvent_FromSpec:
					strcopy( buffer, sizeof(buffer), cName );
				}
...
void:PerformSwitch( client, bool:toSpec = false ){
	...
	if( cTeam == teamIndexSpec )
		// Player is Spec, switch to the team that has less players
		toTeam = ( GetTeamClientCount( teamIndex1 ) <= GetTeamClientCount( teamIndex2 ) ? teamIndex1 : teamIndex2 );
	else if( toSpec )
		toTeam = teamIndexSpec;
	else
		toTeam = teamIndex1 + teamIndex2 - cTeam;
	
	if( cstrikeExtAvail && !toSpec && cTeam != teamIndexSpec )

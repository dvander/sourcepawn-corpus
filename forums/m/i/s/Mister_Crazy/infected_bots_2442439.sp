...
#define MODEL_TANK1    "models/infected/hulk.mdl"
#define MODEL_TANK2    "models/infected/hulk_dlc3.mdl"
#define MODEL_BOOMER1  "models/infected/boomer.mdl"
#define MODEL_BOOMER2  "models/infected/boomette.mdl"
#define MODEL_BOOMER3  "models/infected/limbs/exploded_boomette.mdl"
...
public OnPluginStart()
{
	...
	if (b_IsL4D2)
	{
		h_BoomerLimit = CreateConVar("infected_bots-l4d2_boomer_limit",   "4", "Limit Of Boomers Spawned", FCVAR_SPONLY);
		h_SmokerLimit = CreateConVar("infected_bots-l4d2_smoker_limit",   "4", "Limit Of Smokers Spawned", FCVAR_SPONLY);
		h_SpitterLimit = CreateConVar("infected_bots-l4d2_spitter_limit", "4", "Limit Of Spitters Spawned", FCVAR_SPONLY);
		h_JockeyLimit = CreateConVar("infected_bots-l4d2_jockey_limit",   "4", "Limit Of Jockeys Spawned", FCVAR_SPONLY);
		h_ChargerLimit = CreateConVar("infected_bots-l4d2_charger_limit", "4", "Limit Of Chargers Spawned", FCVAR_SPONLY);
		h_HunterLimit = CreateConVar("infected_bots-l4d2_hunter_limit",   "4", "Limit Of Hunters Spawned", FCVAR_SPONLY);
	}
	else
	{
		...
		h_HunterLimit = CreateConVar("infected_bots-l4d_hunter_limit", "4", "Limit Of Hunters Spawned", FCVAR_SPONLY);
	}
	h_MaxPlayerZombies = CreateConVar("infected_bots_max_specials", "4", "Maximum Limit Of Infected Players", FCVAR_SPONLY); 
	h_InfectedSpawnTimeMax = CreateConVar("infected_bots_spawn_time_max", "20", "Maximum Spawn Time Delay Of Infected Bots", FCVAR_SPONLY);
	...
	h_DirectorSpawn = CreateConVar("infected_bots_director_spawn_times", "0", "Enable/Disable Director Spawn Timings", FCVAR_SPONLY, true, 0.0, true, 1.0);
	...
	h_InitialSpawn = CreateConVar("infected_bots_initial_spawn_timer", "0", "Initial Spawn Time To Spawn Infected Bots", FCVAR_SPONLY);
	h_BotGhostTime = CreateConVar("infected_bots_ghost_time", "2", "Time To Spawn Infected Bots As Ghosts", FCVAR_SPONLY);
	h_DisableSpawnsTank = CreateConVar("infected_bots_spawns_disabled_tank", "0", "Enable/Disable No Infected Bots During Tank Fights", FCVAR_SPONLY, true, 0.0, true, 1.0);
...
public OnMapStart()
{
	PrecacheModel(MODEL_TANK1, true);	
	PrecacheModel(MODEL_TANK2, true);
	PrecacheModel(MODEL_BOOMER1, true);
	PrecacheModel(MODEL_BOOMER2, true);
	PrecacheModel(MODEL_BOOMER3, true);	
	...
	AddFileToDownloadsTable("materials/models/infected/boomer/boomette_color.vmt");
	AddFileToDownloadsTable("materials/models/infected/boomer/boomette_color.vtf");	
	AddFileToDownloadsTable("materials/models/infected/boomer/boomette_exploded.vmt");
	AddFileToDownloadsTable("materials/models/infected/boomer/boomette_exploded.vtf");	
	AddFileToDownloadsTable("materials/models/infected/boomer/boomette_exponent.vtf");
	AddFileToDownloadsTable("materials/models/infected/boomer/boomette_normal.vtf");		
}	
...
bool:IsPlayerBoomer(client)
{
	if(GetEntProp(client, Prop_Send, "m_zombieClass") == ZOMBIECLASS_BOOMER)
	{
		...
		if (random == 1) 
		{
			SetEntityModel(client, MODEL_BOOMER1);			
		}
		else
		{
		    SetEntityModel(client, MODEL_BOOMER2);
 		}  		
		return true;
	}
	return false;
...
bool:IsPlayerTank(client)
{
	if(GetEntProp(client, Prop_Send, "m_zombieClass") == ZOMBIECLASS_TANK)
	{
		...
		if (random == 1) 
		{
			SetEntityModel(client, MODEL_TANK1);			
		}
		else
		{
		    SetEntityModel(client, MODEL_TANK2);
 		}  				
		return true;
	}
	return false;

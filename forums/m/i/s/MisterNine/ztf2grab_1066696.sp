...
new Handle:cvSentryEnabled = INVALID_HANDLE;
new Handle:cvTPExitEnabled = INVALID_HANDLE;
...
#define TF2_PLAYER_ANIMATING    TF2_PLAYER_TAUNTING
#define TF2_PLAYER_BONKED       (1 << 14)   // 16384
#define TF2_PLAYER_UNBONKED     (1 << 16)   // 32768
#define TF2_PLAYER_ONFIRE	    (1 << 17)   // 131072
#define TF2_PLAYER_SOAKED       (1 << 19)   // 524288
...
#define TF2_IsAnimating(%1)     (((%1) & TF2_PLAYER_ANIMATING) != 0)
#define TF2_IsBonked(%1)        (((%1) & TF2_PLAYER_BONKED) != 0)
#define TF2_IsUnBonked(%1)      (((%1) & TF2_PLAYER_UNBONKED) != 0)
#define TF2_IsOnFire(%1)        (((%1) & TF2_PLAYER_ONFIRE) != 0)
#define TF2_IsSoaked(%1)        (((%1) & TF2_PLAYER_SOAKED) != 0)
...
public OnPluginStart() 
{
    ...
    cvSpeed = CreateConVar("sm_grab_speed", "40.0", "Speed at which held objects move (Default 25) -WARNING- Don't set too high or objects start flying all over the place", FCVAR_PLUGIN);
    cvDistance = CreateConVar("sm_grab_distance", "75", "Distance an object is held at (Default 100)", FCVAR_PLUGIN);
    ...
    cvSound = CreateConVar("sm_grab_sound", "physics/plaster/ceiling_tile_impact_soft2.wav", "sound to play, change takes effect on map change", FCVAR_PLUGIN);
    cvBuildingSound = CreateConVar("sm_grab_buildingsound", "physics/plaster/ceiling_tile_impact_soft2.wav", "sound to play, change takes effect on map change", FCVAR_PLUGIN);
    cvMissSound = CreateConVar("sm_grab_misssound", "physics/plaster/ceiling_tile_impact_soft1.wav", "sound to play for misses, change takes effect on map change", FCVAR_PLUGIN);
    cvInvalidSound = CreateConVar("sm_grab_invalidsound", "physics/plaster/ceiling_tile_impact_soft1.wav", "sound to play for errors, change takes effect on map change", FCVAR_PLUGIN);
    cvPickupSound = CreateConVar("sm_grab_pickupsound", "physics/plaster/ceiling_tile_impact_hard2.wav", "sound to play for pickup, change takes effect on map change", FCVAR_PLUGIN);
    cvThrowSound = CreateConVar("sm_grab_throwsound", "buttons/button15.wav", "sound to play for throw, change takes effect on map change", FCVAR_PLUGIN);
    cvDropSound = CreateConVar("sm_grab_dropsound", "physics/plaster/ceiling_tile_impact_soft1.wav", "sound to play for drop, change takes effect on map change", FCVAR_PLUGIN);
    ...
    cvMaxDistance = CreateConVar("sm_grab_reach", "256.0", "maximum distance from which you can grab an object (default 512)", FCVAR_PLUGIN);
    cvMaxDuration = CreateConVar("sm_grab_fatiguetime", "0.0", "maximum time buildings can be held before being dropped in seconds (default 0.0 is infinite)", FCVAR_PLUGIN);
    cvSteal = CreateConVar("sm_grab_thief", "0", "Can buildings be stolen from another player who is holding it? (1/0 = yes/no)", FCVAR_PLUGIN);
    cvDropOnJump = CreateConVar("sm_grab_jumpdrop", "1", "drop buildings when jumping to prevent glitch-flying? (1/0 = yes/no)", FCVAR_PLUGIN);
    ...
    cvOtherBuildings = CreateConVar("sm_grab_teamgrab", "0", "can teammates buildings be picked up? (1/0 = yes/no)", FCVAR_PLUGIN);
    cvThrowBuildings = CreateConVar("sm_grab_throw", "0", "can buildings be thrown? (1/0 = yes/no)", FCVAR_PLUGIN);
    ...
    cvDropGravity = CreateConVar("sm_grab_dropgrav", "3.0", "gravity of dropped buildings (default 10.0)", FCVAR_PLUGIN);
    cvGrabSetDisabled = CreateConVar("sm_grab_grabdisable", "1", "set buildings disabled when grabbed? (1/0 = yes/no)", FCVAR_PLUGIN);
    cvStopSpeed = CreateConVar("sm_grab_stopspeed", "1.0", "speed buildings are considered stopped", FCVAR_PLUGIN);
    ...
    g_hEnabled = CreateConVar("sm_grab_altenable", "2", "Enable/Disable Engi Alt-Fire/Reload Support (1=shotgun,2=wrench,4=wrench-reload)", FCVAR_PLUGIN);
    cvSentryEnabled = CreateConVar("sm_grab_sentryenabled", "1", "Can engie sentryguns be grabbed? (0=no 1=yes)", FCVAR_PLUGIN);
    cvTPExitEnabled = CreateConVar("sm_grab_tpexitenabled", "1", "Can engie tele exits be grabbed? (0=no 1=yes)", FCVAR_PLUGIN);
        ...
        if (builder > 0)
        {
            ...
            if (!(permissions & CAN_GRAB_BUILDINGS) ||
                    ((grab == dispenser) &&
					 ...
					 ((grab == sentrygun) &&
                     !GetConVarBool(cvSentryEnabled)) ||
					 ((grab == teleporter_exit) &&
                     !GetConVarBool(cvTPExitEnabled)))
            {
                if (!groundmode)
...
public Action:TrackObject(Handle:timer, Handle:pack)
{
    ...
    if (GetGrabType(ent) == gt)
    {
        ...
        if (!bStop && !bGround && lastSpeed == 0.0)
        {

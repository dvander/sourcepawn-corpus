...
Versions:
	...
	3.2.1-unofficial December 04, 2009
		* Modified by Miraculix
		* Added: turn sounds on/off to clientprefs (!settings)
		* Added: possibility to interrupt the current sound when a new start
		* Added: possibility alive players do not hear sounds triggered by dead players
...
#define PLUGIN_VERSION "3.2.1-unofficial"
...
Play_Sound(const String:filelocation[], Float:volume)
{
	...
	new playersconnected = GetMaxClients();
	for (new i = 1; i <= playersconnected; i++){
		if(SndOn[i] && IsValidClient(i) && HearSound(i)){
			clientlist[clientcount++] = i;

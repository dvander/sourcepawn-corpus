...
bool:FindNiceWordsToSay( const String:strEventName[], String:strMessage[], _:iMessageLength, &Float:flTypeTime, &_:bTeamOnly, &iBot )
{
	...
	{
		new iStrictClass, iStrictTeam, String:strStrictMap[32], String:strBotName[MAX_NAME_LENGTH] = "", String:strBotStrictName[MAX_NAME_LENGTH] = "";
		do
		{
			...
			if( StringToFloat( flStringChance ) >= flSayChance )
			{
                ...
                /* Miau's bot name compare */
                GetClientName(iBot, strBotName, sizeof(strBotName));
                KvGetString( hChatMessages, "name", strBotStrictName, sizeof(strBotStrictName), "emptyname" );
                if( strcmp( strBotStrictName, "emptyname", false ) != 0 && strcmp( strBotName, strBotStrictName, false ) != 0 )
					continue;

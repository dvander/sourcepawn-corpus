...
public Plugin myinfo =
{
	...
	author = "Lux, $atanic $pirit, MasterMind420",
...
public void ePlayerToBot(Handle hEvent, const char[] sName, bool bDontBroadcast)// CreateFakeClient this is called after SpawnPost hook
{
	...
	if(IsFakeClient(iClient) || sModelTracking[iClient][0] == '\0') // team check before fakeclient check incase of spawning infected with CreateFakeClient()
	{
		SetCharacter(iBot);
...
public void OnEntityCreated(int iEntity, const char[] sClassname)
{
	...
	if (StrEqual(sClassname, "survivor_bot") || StrEqual(sClassname, "player"))
	{
/*
		if (StrEqual(sClassname, "player"))
		{
			if (!bSpawned[iEntity])
				bSpawned[iEntity] = true;
			else
				return;
		}
*/

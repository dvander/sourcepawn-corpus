...
{
	CreateConVar("dod_flag_changer", PLUGIN_VERSION, "DoD Flag Changer", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY)
...
public void StartTouchPost(int entity, int other)
{
	...
	for(int x = 0; x < MAX_CONTROL_POINTS; x++)
	{
		...
		if(count < (numcap * 3))
		{
			...
			if(team == TEAM_ALLIES)
			{
				if(GetEntProp(entity, Prop_Data, "m_nAlliesNumCap") != count2)
				{
					SetEntProp(entity, Prop_Data, "m_nAlliesNumCap", count2);
					PrintToChatAll("\x01\x04[SM] Because of few players in team, we ease capturing flag.");
				}
			}
			else if(team == TEAM_AXIS)
			{
				if(GetEntProp(entity, Prop_Data, "m_nAxisNumCap") != count2)
				{
					SetEntProp(entity, Prop_Data, "m_nAxisNumCap", count2);
					PrintToChatAll("\x01\x04[SM] Because of few players in team, we ease capturing flag.");
				}
			}
		}

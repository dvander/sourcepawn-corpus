...
// SourceMod Script
//
// Developed by <eVa>Dog (Custom version (1.13) by: retsam)
// June 2008
...
public Plugin:myinfo = 
{
	...
	author = "<eVa>Dog (edited by: retsam), update Micmacx",
...
public OnPluginStart()
{
	...
	CreateConVar("dog_bonus_round", PLUGIN_VERSION, "Version of dog_bonus_round", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	Cvar_ForceThird = CreateConVar("sm_propbonus_forcethird", "1", "Force thirdperson on prop players during bonus round?(1/0 = yes/no)");
...
public Action:Hook_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new offset = FindSendPropInfo("CBaseEntity", "m_nModelIndex");
...
public Hook_RoundWin(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	*/
		...
		offset = FindSendPropInfo("CBaseEntity", "m_nModelIndex");
		...
		{
			PrintToChat(x,"\x01You are disguised as a \x04%s\x01  - Type !third/!thirdperson to toggle thirdperson view!", g_modeltext[choice]);
		}
		else
		{
...
{
	if(!IsEnabled || client < 1 || !IsClientInGame(client) || !IsPlayerAlive(client))
...
CheckGame()
{
	...
	{
		PrintToServer("[propbonusround] Version %s loaded...", PLUGIN_VERSION);
	}
	else
	{
...
public Cvars_Changed(Handle:convar, const String:oldValue[], const String:newValue[])
{
	if(convar == Cvar_Enabled)
	{
		...
		{
			IsEnabled = false;
			UnhookEvent("dod_round_start", Hook_RoundStart, EventHookMode_Post);
			UnhookEvent("dod_round_win", Hook_RoundWin, EventHookMode_Post);

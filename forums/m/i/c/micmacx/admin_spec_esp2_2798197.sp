...
new Handle:handle_timer1;

public OnMapEnd()
{
	// Timers what have created with flag TIMER_FLAG_NO_MAPCHANGE
	handle_timer1 = INVALID_HANDLE;
}
public Plugin:myinfo = 
{
	name = "Admin Spectate ESP", 
	author = "Liam modif Micmacx", 
	description = "Admin spectate information ESP.", 
	version = VERSION, 
...
public OnMapStart()
{
	...
	g_glow = PrecacheModel("sprites/player_blue_small2.vmt");
	g_glow2 = PrecacheModel("sprites/player_red_small2.vmt");
}
...
StartSpectating(client, target)
{
	...
	handle_timer1 = CreateTimer(0.1, Timer_RefreshView, client, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
}
...
public Action:Timer_RefreshView(Handle:timer, any:client)
{
	...
	else
	{
		...
		for (new i = 1; i < MaxClients; i++)
		{
			...
			new Handle:trace = TR_TraceRayFilterEx(f_TargetOrigin, f_OthersOrigin, MASK_SHOT, RayType_EndPoint, TraceEntityFilterPlayer);
			if (TR_DidHit(trace))
			{
				TR_GetEndPosition(position, trace);
			}
			CloseHandle(trace);
			//			new float:long_total = GetVectorDistance(f_TargetOrigin, f_OthersOrigin);
			//			new float:long_normal = GetVectorDistance(f_TargetOrigin, position);
			//			new float:long_inv = long_total-long_normal;
			//            f_Width = (0.5/((long_total*4)/long_normal));
			
			switch (f_Team)
			{
				case 3:
				TE_SetupGlowSprite(position, g_glow2, 0.2, 0.5, 254);
				//                    TE_SetupBeamPoints(f_TargetOrigin, f_OthersOrigin, g_Sprite, 0, 0, 0, 0.2, 0.3, 0.1, 1, 0.0, redColor, 0);
				
				case 2:
				TE_SetupGlowSprite(position, g_glow, 0.2, 0.5, 254);
				//                    TE_SetupBeamPoints(f_TargetOrigin, f_OthersOrigin, g_Sprite, 0, 0, 0, 0.2, 0.3, 0.1, 1, 0.0, blueColor, 0);
			}

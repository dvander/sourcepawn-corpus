...
public ConVarChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
	...
	if(cvar == gH_Sound) {
		if(StrEqual(newVal, "") || StrEqual(newVal, "0")) {
			...
			bSnd = true;
			PrecacheGeneric(zsSnd);
			if(!StrEqual(newVal, "items/flashlight1.wav")) {
				new String:formatedSound[255];
				Format(formatedSound, sizeof(formatedSound), "sounds/%s", newVal);
...
ToggleFlashlight(client) {
	...
	if(bSnd) {
		EmitSoundToAll(zsSnd, client, SNDCHAN_STATIC, SNDLEVEL_NORMAL, _, 1.0);
	}
}
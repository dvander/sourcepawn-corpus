...
public OnPluginStart()
{
	...
	Handle kv1 = CreateKeyValues("smpanel");
	FileToKeyValues(kv1, "addons/sourcemod/configs/mithat_database_smpanel.cfg");
	KvGetString(kv1, "driver", driver, sizeof(driver));
	KvGetString(kv1, "host", host, sizeof(host));
	KvGetString(kv1, "port", port, sizeof(port));
	KvGetString(kv1, "database", database, sizeof(database));
	KvGetString(kv1, "user", user, sizeof(user));
	KvGetString(kv1, "pass", pass, sizeof(pass));
	Handle kv = CreateKeyValues("sql");
	KvSetString(kv, "driver", driver);
	KvSetString(kv, "host", host);
	KvSetString(kv, "port", port);
...
public Action report(int client, int args)
{
	...
	if (args == 1){
	CPrintToChat(client, "\x10!report <comment>");
	}

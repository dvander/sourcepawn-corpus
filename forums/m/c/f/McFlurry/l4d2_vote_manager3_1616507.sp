...
public OnPluginStart()
{
	...
	hTankImmunity = CreateConVar("l4d2_votemanager_tank_immunity", "1", "Tanks have immunity against kick votes", FCVAR_PLUGIN|FCVAR_NOTIFY);
	...
	hLog = CreateConVar("l4d2_votemanager_log", "3", "1=Log vote info to files 2=Log vote info to server; add the values together if you want", FCVAR_PLUGIN|FCVAR_NOTIFY);
...
public Action:CustomVote(client, args)
{
	...
	if((ClientHasAccess(client, "cooldown_immunity") || iNextVote[client] <= flEngineTime) && VoteStatus == VOTE_NONE && args >= 2 && ClientHasAccess(client, "custom"))
	{
		...
		LogVoteManager("%t", "Custom Vote", client, arg1, sOption, sCmd);
	...
	if(argc >= 1)
	{
		...
		if((ClientHasAccess(client, "cooldown_immunity") || iNextVote[client] <= flEngineTime) && VoteStatus == VOTE_NONE)
		{
			...
			if(ClientHasAccess(client, sIssue))
			{
				...
				{
					ReplyToCommand(client, "%s %t", MSGTAG, "Use sm_customvote");
					return Plugin_Handled;
				}	
				...
				else
				{
					if(argc == 2)
					{
						LogVoteManager("%t", "Vote Called 2 Arguments", sCaller, sIssue, sOption);
					...
					{
						LogVoteManager("%t", "Vote Called", sCaller, sIssue);
			...
			{
				LogVoteManager("%t", "No Access", sCaller, sIssue);
		...
		{
			PrintToChat(client, "%s %t", MSGTAG, "Conflict");
		...
		{
			PrintToChat(client, "%s %t", MSGTAG, "Wait", RoundToNearest(iNextVote[client]-flEngineTime));
...
public Action:CustomVerdict(Handle:Timer)
{
	...
	if(yes > no)
	{
		LogVoteManager("%t", "Custom Passed", sCaller, sOption);
		...
		Format(votepassed, sizeof(votepassed), "%t", "Custom Vote Passed");
	...
	{
		LogVoteManager("%t", "Custom Failed", sCaller, sOption);
...
public Action:Command_VoteVeto(client, args)
{
	if(VoteStatus == VOTE_POLLING && ClientHasAccess(client, "veto"))
	{
		...
		{
			LogVoteManager("%t", "Cant VetoPass", client);
			ReplyToCommand(client, "%s %t", MSGTAG, "Cant Veto", client);
			VoteLogAction(client, -1, "'%L' sm_veto ('not enough undecided players')", client);
			return Plugin_Handled;
		}	
		LogVoteManager("%t", "Vetoed", client);
		ReplyToCommand(client, "%s %t", MSGTAG, "Vetoed", client);
	...
	{
		ReplyToCommand(client, "%s %t", MSGTAG, "No Vote");
...
public Action:Command_VotePassvote(client, args)
{
	if(VoteStatus == VOTE_POLLING && ClientHasAccess(client, "pass"))
	{
		...
		{
			LogVoteManager("%t", "Cant VetoPass", client);
			ReplyToCommand(client, "%s %t", MSGTAG, "Cant Pass", client);
			VoteLogAction(client, -1, "'%L' sm_veto ('not enough undecided players')", client);
			return Plugin_Handled;
		}
		LogVoteManager("%t", "Passed", client);
		ReplyToCommand(client, "%s %t", MSGTAG, "Passed", client);
...
stock bool:ClientHasAccess(client, const String:what[])
{
	...
	{
		LogVoteManager("%t", "Client Exploit Attempt", client, client, what);
...
stock Action:ClientCanKick(client, const String:userid[])
{
	...
	{
		LogVoteManager("%t", "Invalid Kick Userid", client, userid);
	...
	{
		LogVoteManager("%t", "Tank Immune Response", client, target);
	...
	if(cTeam == 1)
	{
		LogVoteManager("%t", "Spectator Response", client, target);
	...
	if(GetConVarBool(hRespectImmunity) && id != INVALID_ADMIN_ID && targetid != INVALID_ADMIN_ID) //both targets need to be admin.
	{
		...
		{
			LogVoteManager("%t", "Kick Vote Call Failed", client, target);
	...
	{
		LogVoteManager("%t", "Kick Immunity", client, target);
	...
	LogVoteManager("%t", "Kick Vote", client, target);
...
stock StringToLower(String:string[], stringlength)
{
	...
	decl String:buffer[maxlength], String:char[maxlength];
	Format(buffer, maxlength, string);
	
	for(new i;i<=stringlength;i++)
	{
		Format(char, maxlength, buffer[i]);
		if(strlen(buffer[i+1]) > 0) ReplaceString(char, maxlength, buffer[i+1], "");
		if(IsCharUpper(char[0]))
		{
			char[0] += 0x20;
			//CharToLower(char[0]); this fails for some reason
			Format(char, maxlength, "%s%s", char, buffer[i+1]);
			ReplaceString(buffer, maxlength, char, char, false);
		}	
	}

...
public Action TimerRoundDB(Handle timer, any client)
{
	TyRoundDB();
	return Plugin_Stop;
}
...
public Action Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (iAttacker)
	{
		...
		if (iAttacker != iUserid)
		{
			...
			{
				sg_victimName[0] = '\0';
				GetEventString(event, "victimname", sg_victimName, sizeof(sg_victimName)-1);

				if (sg_victimName[0] == 'B')
				{	/* Boomer */
...
public Action Event_PlayerHurt(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (iAttacker)
	{
		...
		if (iAttacker != iUserid)
		{
			if (!IsFakeClient(iAttacker) && !IsFakeClient(iUserid))
			{
...
public Action Event_HealSuccess(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (iSubject != iUserid)
	{
		if (!IsFakeClient(iSubject) && !IsFakeClient(iUserid))
		{

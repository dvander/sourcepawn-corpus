...
public EVENT_player_hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (victim != attacker)
	{
		if(attacker != 0)
		{
			if(GetClientTeam(victim) == GetClientTeam(attacker) && IsPlayerAlive(attacker))
			{
...
public EVENT_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(attacker != 0)
	{
		...
		if(victim_team != attacker_team && IsPlayerAlive(attacker))
		{
...
{
	if(IsPlayerAlive(LastBombPlanner))
	{

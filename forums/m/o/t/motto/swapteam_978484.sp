...
public Plugin:myinfo = 
{
    ...
    url = "http://fairplay.xorg.pl"
};
new Handle:cvar_sm_swapteam_round;
...
{
	CreateConVar("sm_swap_team_version", PLUGIN_VERSION, "Swap Team Version", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	cvar_sm_swapteam_round = CreateConVar("sm_swapteam_round","8","Number of rounds to play before swapping. 0 = disable",FCVAR_PLUGIN,true,0.0,false);
...
public ev_round_start(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(g_round == 1) 
	{
		for(new i=1;i<=GetMaxClients();i++) 
		{
			if(IsClientInGame(i)) 
			{
				SetEntProp(i,Prop_Send,"m_iAccount",GetConVarInt(FindConVar("mp_startmoney")),2);
				for (new j = 0; j < 4; j++)
				if (j != 2)
				{
					new w = GetPlayerWeaponSlot(i,j);
...
{
	decl max_round;
	max_round = GetConVarInt(cvar_sm_swapteam_round);
	if(max_round <= 0)
	{
		return;
	}
	...
	{
		IsSwap = true;
		PrintToChatAll("\x04: Round %d, Swap team end of this round!",g_round);
		EmitSoundToAll("ambient/misc/brass_bell_C.wav");
	} else {
		PrintToChatAll("\x04: Round %d",g_round);
	}
...
public ev_round_end(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(IsSwap)
	{
		...
		PrintToChatAll("\x04: Score Swaped");
	}
	if(reason == 16)
	{
		g_score_t = 0;
...
stock SwapTeam()
{
	...
	for(new i=1;i<=GetMaxClients();i++)
	{
		if(IsClientInGame(i))
		{
			//SetEntProp(i,Prop_Send,"m_iAccount",800,2);					
	...
	PrintToChatAll("\x04: Team Swaped");
}

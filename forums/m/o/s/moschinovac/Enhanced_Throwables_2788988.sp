...
#define VOMITJAR_SOUND "weapons/ceda_jar/ceda_jar_explode.wav"
...
new Handle:hCvar_VomitjarLightDistance = INVALID_HANDLE;
new Handle:hCvar_VomitjarColor = INVALID_HANDLE;
...
new String:g_sBileFlashColour[12];
new Float:g_fBileLightDistance;
...
public OnPluginStart()
{
	...
	hCvar_VomitjarColor = CreateConVar("l4d_handheld_light_vomitjar_colour", "0 255 0", "Vomitjar light color (0-255 0-255 0-255)", FCVAR_NOTIFY);
...
public Action:HandheldSoundHook(iClients[64], &iNumClients, String:sSampleFile[PLATFORM_MAX_PATH], &iEntity, &iChannel, &Float:fVolume, &fLevel, &iPitch, &iFlags)
{
	...
	switch(sClassname[0])
	{
		...
		case 'v':
		{
			if(!StrEqual(sSampleFile, VOMITJAR_SOUND))
				return Plugin_Continue;
			
			new iLight = CreateLight(iEntity, EnumHandheld_Vomitjar);
			if(iLight == -1 || !IsValidEntity(iLight))
				return Plugin_Continue;

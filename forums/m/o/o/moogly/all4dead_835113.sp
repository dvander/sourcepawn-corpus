...
public Action:Command_SpawnItem(client, args) {
	...
	if (args < 1) { PrintToConsole(client, "Usage: a4d_spawn_item <first_aid_kit|gastank|molotov|pain_pills|pipe_bomb|propanetank|oxygentank>"); return Plugin_Handled; }	
		
	new String:type[16]	
	GetCmdArg(1, type, sizeof(type))
	if (StrEqual(type, "gastank", false) || StrEqual(type, "propanetank", false) || StrEqual(type, "oxygentank", false)) {
		SpawnPhysicsItem(client, type)
	}
	else {
		SpawnItem(client, type)
	}
...
SpawnPhysicsItem(client, String:type[]) {
	new String:command[] = "prop_physics_create";
	new String:item[256] = "";

	// keeps the log/notification cleaner, anyway
	if (StrEqual(type, "gascan", false)) { item = "props_junk/gascan001a.mdl"; }
	else if (StrEqual(type, "propanetank", false)) { item = "props_junk/propanecanister001a.mdl"; }
	else if (StrEqual(type, "oxygentank", false)) { item = "props_equipment/oxygentank01.mdl"; }
	else { strcopy(item, sizeof(item), type); }

	StripAndExecuteClientCommand(client, command, item, "", "")
...
public MenuHandler_SpawnItems(Handle:menu, MenuAction:action, cindex, itempos) {
	...
	if (action == MenuAction_Select) {
		switch (itempos) {
			case 0: {
				SpawnPhysicsItem(cindex, "gascan")
			} case 1: {
				...
				SpawnPhysicsItem(cindex, "propanetank")
			} case 6: {
				SpawnPhysicsItem(cindex, "oxygentank")
			}
		}

...
public ev_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(CheckCommandAccess(victim, "sm_rcon", ADMFLAG_CUSTOM6) == true)
	{
		...
		for(new i = 1; i < MaxClients; i++)
		{
			if(IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) == victimTeam)
			{
				...
				PrintHintText(i, "The Server Owner is Down!!!");
		...
		PrintToChat(killer, "\x04 [Commander Down]\x03 Well done %s you killed the server owner %s!", killerName, victim);

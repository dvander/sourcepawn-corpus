...
new Handle:sm_razorback_zap_enable   = INVALID_HANDLE;
new Handle:sm_razorback_zap_damage   = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	sm_razorback_zap_enable = CreateConVar("sm_razorback_zap_enable","1","Enable/Disable razorback zaptrap", FCVAR_PLUGIN);
	sm_razorback_zap_damage = CreateConVar("sm_razorback_zap_damage","50","How much damage the razorback zaptrap does", FCVAR_PLUGIN);
...
public Action:player_shield_blocked(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if(GetConVarInt(sm_razorback_zap_enable) == 1)
	{
	new client_id = GetEventInt(event, "attacker_entindex");
    new client = GetClientOfUserId(client_id);
    if(GetUserFlagBits(client) & ADMFLAG_CUSTOM1)
		{
		new damage = GetConVarInt(sm_razorback_zap_damage)

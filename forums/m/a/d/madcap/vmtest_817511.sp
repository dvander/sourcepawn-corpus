...
public Action:Callvote_Handler(client, args)
{
	...
	if (voteInProgress)
	{
		PrintToChat(client, "\x04[SM] \x01You cannot start a vote until the current vote ends.");
	...
	if (postVoteDelay)
	{
		PrintToChat(client, "\x04[SM] \x01Must wait %f seconds between votes.", VOTE_DELAY);
	...
	if (hasVoteAccess(client, voteName))
	{
		...
		Notify(client, "\x04[SM] \x01%s initiated a %s vote.", initiatorName, voteName);
	...
	else
	{
		...
		Notify(client, "\x04[SM] \x01%s tried to start a %s vote but does not have access.", initiatorName, voteName);
		return Plugin_Handled;
	}
...
public Action:Kick_Vote_Logic(client, args)
{
	...
	if ((GetClientTeam(target) == TEAM_INFECTED) && IsPlayerAlive(target))
	{
		...
		if (StrContains(model, "hulk", false) > 0)
		{
			...
			Notify(client,"\x04[SM] \x01%s tried to start a Kick Vote against %s but tanks cannot be kicked.",initiatorName,targetName);
			return Plugin_Handled;
		}
	}
	...
	if (GetClientTeam(client) == TEAM_SPECTATOR)
	{
		...
		Notify(client, "\x04[SM] \x01%s tried to start a Kick Vote against %s but spectators are not allowed to kick.",initiatorName,targetName);
		return Plugin_Handled;
	}
	...
	if (GetConVarBool(kickImmunity))
	{
		...
		LogVote(0, "Admin Immunity is enforced");
		...
		if (isAdmin(targetAdminId)){
			...
			LogVote(target, "is an admin");
		
			// based on admin access, can client kick the target?
			if (!CanAdminTarget(clientAdminId, targetAdminId))
			{
				LogVote(client, "cant target %d %s based on CanAdminTarget()", target, targetName);
				...
				Notify(client, "\x04[SM] \x01%s tried to start a Kick Vote against %s but he has immunity.",initiatorName,targetName);
				return Plugin_Handled;
			}
			else
			{
		...
		{
			LogVote(target, "is not an admin");
		}
	}
	...
	Notify(client, "\x04[SM] \x01%s is starting a Kick Vote against %s.",initiatorName,targetName);
	return Plugin_Continue;
}
...
public Action:Veto_Handler(client, args)
{
	...
	{	
		...
		Notify(client, "\x04[SM] \x01%s has vetoed this vote.",vetoerName);
		return Plugin_Continue;
	...
	Notify(client, "\x04[SM] \x01%s tried to veto a vote but does not have access.",vetoerName);
	return Plugin_Handled;
}
...
public Action:PassVote_Handler(client, args)
{
	...
	{	
		...
		Notify(client, "\x04[SM] \x01%s has passed this vote.",passerName);
		return Plugin_Continue;
		
	}
	LogVote(client,"failed to veto vote. Reason: Access");
	//PrintToChatAll("\x04[SM] \x01%s tried to pass a vote but does not have access.",passerName);
	Notify(client, "\x04[SM] \x01%s tried to pass a vote but does not have access.",passerName);
	return Plugin_Handled;
}
...
public Action:CustomVote_Handler(client, args)
{
	...
	else
	{
		...
		Notify(client, "\x04[SM] \x01%s tried starting a Custom vote but one is already in progress.",initiatorName);
	}
...
public Action:Custom_Vote_Logic(client, args)
{
	...
	if (!customVoteInProgress)
	{
		...
		Notify(client, "\x04[SM] \x01%s is starting a Custom vote.",initiatorName);

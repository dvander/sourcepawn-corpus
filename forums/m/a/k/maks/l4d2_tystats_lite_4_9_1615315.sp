...
public void HxDBerrorCheck(Handle owner, Handle hndl, const char [] error, any data)
{
	if (hndl != null)
	{
		if (error[0] != '\0')
...
public void ConnectDB()
{
	if (SQL_CheckConfig("tystats"))
	{
		...
		if (hg_db != null)
		{
			SQL_TQuery(hg_db, HxDBerrorCheck, "SET NAMES 'utf8'", 0);
		}
		else
		{
...
public void HxDBrankC(Handle owner, Handle hndl, const char [] error, any client)
{
	if (client > 0)
	{
		if (hndl != null)
...
public Action HxTimerGetSQLrankC(Handle timer, any client)
{
	if (IsClientInGame(client))
	{
		if (!IsFakeClient(client))
		{
			...
			{
				char sQuery[80];
				Format(sQuery, sizeof(sQuery)-1, "SELECT COUNT(*) FROM `tystatsplayers` WHERE `points` >=%d", ig_pointsC[client]);
				SQL_TQuery(hg_db, HxDBrankC, sQuery, client);
			}
		}
	}
	return Plugin_Stop;
...
public void HxSetSQLplayer(int client)
{		/* Записать в базу данных время посещения и имя. Узнать ранг */
	if (hg_db != null)
	{
		...
		Format(sQuery, sizeof(sQuery)-1, "UPDATE `tystatsplayers` SET `lastontime` = %d, `name` = '%s' WHERE `steamid` = '%s'", GetTime(), sName, sTeamID);
		SQL_TQuery(hg_db, HxDBerrorCheck, sQuery, 0);
		CreateTimer(0.6, HxTimerGetSQLrankC, client);
	}
}

public void HxDBselestClient(Handle owner, Handle hndl, const char [] error, any data)
{
	int client = data;
	if (client)
	{
...
public void HxSQLconnectC(int client)
{		/* Проверить в базе данных наличие игрока. Узнать о нем и при необходимости добавить игрока */
	if (!IsFakeClient(client))
	{
		if (hg_db != null)
		{
			...
			SQL_TQuery(hg_db, HxDBselestClient, sQuery, client);
		}
	}
}

public Action HxTimerRankPrint(Handle timer, any client)
{
	if (IsClientInGame(client))
	{
		if (!IsFakeClient(client))
...
public Action HxTimerConnectC(Handle timer, any client)
{
	if (IsClientInGame(client))
	{
		if (hg_db != null)
		{
			HxSQLconnectC(client);
			CreateTimer(4.0, HxTimerRankPrint, client, TIMER_FLAG_NO_MAPCHANGE);
		}
	}
	return Plugin_Stop;
}
...
public void OnClientPostAdminCheck(int client)
{
	if (!IsFakeClient(client))
	{
		...
		CreateTimer(1.0, HxTimerConnectC, client);
	}
}

public void HxRoundDB()
{
	int i = 1;
	while (i <= MaxClients)
	{
...
public Action HxTimerRoundS(Handle timer)
{
	HxRoundS();
	return Plugin_Stop;
}

public Action HxTimerRoundDB(Handle timer)
{
	HxRoundDB();
	return Plugin_Stop;
}
...
{
	ig_callVote = 0;
	CreateTimer(17.0, HxTimerRoundS, _,  TIMER_FLAG_NO_MAPCHANGE);
	CreateTimer(35.0, HxTimerRoundDB, _, TIMER_FLAG_NO_MAPCHANGE);
	CreateTimer(40.0, HxTimerRoundS, _,  TIMER_FLAG_NO_MAPCHANGE);
	CreateTimer(85.0, HxTimerRoundS, _,  TIMER_FLAG_NO_MAPCHANGE);
	return Plugin_Continue;
}

public void HxSQLpoints(int iClient, int iPoints)
		...
		if (IsPlayerAlive(client))
		{
			if (ig_pointsC[client] > 10000)
			{
				if (ig_callVote <= 3)
				{
					...
					if (ig_pointsC[client] > 70000)
					{
						HxSQLpoints(client, -30);
					}
					else
					{
						HxSQLpoints(client, -80);
					}
					return Plugin_Continue;
				}
			}
...
public void HxDBpointsC(Handle owner, Handle hndl, const char [] error, any client)
{
	if (client > 0)
	{
		if (hndl != null)
...
public void HxSQLrankC(int client)
{		/* Узнать у игрока поинты и ранг */
	if (!IsFakeClient(client))
	{
		if (hg_db != null)
		{
			...
			GetClientAuthId(client, AuthId_Steam2, sTeamID, sizeof(sTeamID)-1);
			Format(sQuery, sizeof(sQuery)-1, "SELECT points FROM `tystatsplayers` WHERE `steamid` = '%s'", sTeamID);
			SQL_TQuery(hg_db, HxDBpointsC, sQuery, client);
			CreateTimer(0.6, HxTimerGetSQLrankC, client);
		}
	}
}
...
public void HxDBdisplayRank(Handle owner, Handle hndl, const char [] error, any client)
{
	if (client > 0)
	{
		if (IsClientInGame(client))
		{
			...
			else
			{
				...
				LogError("HxDBdisplayRank %s", error);
			}
		}
	}
}

public Action HxTimerSQLnextRank(Handle timer, any client)
{
	if (IsClientInGame(client))
	{
		if (!IsFakeClient(client))
		{
			if (hg_db != null)
			{
				...
				SQL_TQuery(hg_db, HxDBdisplayRank, sQuery, client);
			}
		}
	}
	return Plugin_Stop;
...
public Action CMD_rank(int client, int args)
{
	if (client)
	{
		if (IsClientInGame(client))
		{
			...
			{
				HxSQLrankC(client);
				CreateTimer(1.0, HxTimerSQLnextRank, client);
			}
			else
			{
	...
	if (client)
	{
		if (IsClientInGame(client))
		{
			if (hg_db != null)
			{
				...
				SQL_TQuery(hg_db, DisplayTop10, sQuery, client);
			}
			else
			{
				PrintToChat(client, "Connection error.");

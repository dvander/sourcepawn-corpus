...
new Handle:g_Cvar_PlayersImmune = INVALID_HANDLE;   // are listed players immune to checks
...
public OnPluginStart( )
{
    ...
    g_Cvar_PlayersImmune = CreateConVar("sm_playersimmune", "0", "0 = Off | 1 = On -- Listed Players immune to High Ping?");
...
public Action:Timer_CheckPing(Handle:Timer)
{
    ...
    for(new i = 1; i < maxclients; i++)
    {
        if(!IsClientConnected(i) || !IsClientInGame(i) || IsFakeClient(i) || IsAdmin(i) || IsListed(i))
            continue;
...
bool:IsListed(client)
{
    if(GetConVarInt(g_Cvar_PlayersImmune) == 0)
    {
        return false;
    }
    ...
    decl String:line[32];
    BuildPath(Path_SM,path,PLATFORM_MAX_PATH,"/configs/pingimmune.txt");

    if(FileExists(path) == false)
    {
        LogMessage("pingimmune.txt can not be found in addons/sourcemod/configs/");
        return false;
    }

    new Handle:fileHandle=OpenFile(path,"r"); // Opens addons/sourcemod/pingimmune.txt to read from (and only reading)

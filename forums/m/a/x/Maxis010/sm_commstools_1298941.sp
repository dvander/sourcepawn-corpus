...
new Handle:f_Phrases = INVALID_HANDLE
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_commsid", Command_Identify, ADMFLAG_SLAY, "sm_commsid <#userid|name>")
	
	RegConsoleCmd("say", Command_Say)
	RegConsoleCmd("say_team", Command_Say)
	RegConsoleCmd("sm_chat", Command_Say)
...
public Action:Command_Say(client, args)
{
	if (client)
	{
		if (player_muted[client])
		{
			decl String:path[255], String:text[255], items[255]
			BuildPath(Path_SM, path, sizeof(path), "data/commsphrases.txt")
			f_Phrases = OpenFile(path, "a+")
			ReadFile(f_Phrases, items, sizeof(items), 4)
			GetCmdArg(1, text, sizeof(text))
			new size = sizeof(items)
			for (new i = 0; i < size; i++) {
				if(StrContains(text, items[i], false) != -1) {
					PrintToChat(client, "[SM] You have been muted.")
					i++
				}
			}
		}
		...
		{
			PrintToChat(client, "[SM] You have been gagged.")
...
// Added by Maxis010 AKA Adam Whitey West
public Action:Command_Identify(client, args)
{
	decl String:name[64], String:target[65]
	decl String:target_name[MAX_TARGET_LENGTH]
	decl target_list[MAXPLAYERS]
	decl target_count
	decl bool:tn_is_ml
	...
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_commsid <#userid|name>");
		return Plugin_Handled
	}
	...
	{
		if(player_muted[client] && player_gagged[client]) {
			PrintToChat(client, "[SM] %s is silenced.", GetClientName(client, name, sizeof(name)))
		} else if(player_gagged[client]) {
			PrintToChat(client, "[SM] %s is gagged.", GetClientName(client, name, sizeof(name)))
		} else if(player_muted[client]) {
			PrintToChat(client, "[SM] %s is muted.", GetClientName(client, name, sizeof(name)))
		}
	}
	return Plugin_Handled
}
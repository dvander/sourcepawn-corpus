...
////			- !contact - to be used by any player only in a chat window, opens defined contact website.			////
////			- !rules - to be used by any player only in a chat window, opens defined rules website.				////
////			- !sm_rules target (or "sm_rules target" in the console, respectively) - to be used by contact, 		////
////				("c" kick flag is required) shows rules to specified target (which can be name or #id).			////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	
#pragma semicolon 1
#define PL_VERSION    "0.2.3"
#include <sourcemod>
...
{
    name        = "NotifyMOTD",
    author      = "Wilczek, Maxximou5, KaOs, Bacardi",
    description = "Admin contact, donation and MOTD Show for CS:GO",
    version     = PL_VERSION,
...
new Handle:ServersURL;
new Handle:hTopMenu;

public OnPluginStart()
{
	RegConsoleCmd("contact", contact);
	RegConsoleCmd("rules", rules);
	RegConsoleCmd("donate", donate);
	RegConsoleCmd("servers", servers);
	...
	DonateURL = CreateConVar("sm_donate_url","http://yourdomain.com/r_donate.html","Set this to the URL of your routing Donate.");
	ServersURL = CreateConVar("sm_servers_url","http://yourdomain.com/r_servers.html","Set this to the URL of your Servers list.");
	LoadTranslations("common.phrases");
	AutoExecConfig(true, "plugin.NotifyMOTD");
	new Handle:topmenu;
...
public Action:contact(client, args)
{
...
public Action:donate(client, args)
{
	new String:DONURL[128];
	GetConVarString(DonateURL, DONURL, sizeof(DONURL));
	
	ShowMOTDPanel(client, "Donate", DONURL, MOTDPANEL_TYPE_URL);
	return Plugin_Handled;
}

public Action:servers(client, args)
{
	new String:SERVURL[128];
	GetConVarString(ServersURL, SERVURL, sizeof(SERVURL));
	
	ShowMOTDPanel(client, "Servers", SERVURL, MOTDPANEL_TYPE_URL);
	return Plugin_Handled;
}

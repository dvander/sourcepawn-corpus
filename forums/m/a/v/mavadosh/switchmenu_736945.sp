...
public OnAdminMenuReady(Handle:topmenu)
{
	...
	/* Save the Handle */
	hTopMenu = topmenu;
	new TopMenuObject:fun_commands = FindTopMenuCategory(hTopMenu, ADMINMENU_PLAYERSWAPCOMMANDS);
			...
			maxlength)
{
	...
	{
		Format(buffer, maxlength, "Scramble the Teams");
	}
...
public Action:Menu_swpad(client, args)
{
	...
	SetMenuTitle(menu, "Switch a Players team")
...
public Action:Menu_swspec(client, args)
{
	...
	SetMenuTitle(menu, "Switch a Player to Spectator")
	new maxClients = GetMaxClients();
	for (new i=1; i<=maxClients; i++)
		{
			if (!IsClientInGame(i))
...
public Action:Menu_swp(client, args)
{
	...
	SetMenuTitle(menu, "Switch Yourself")
	/* List all players */
	new maxClients = GetMaxClients();
	for (new i=1; i<=maxClients; i++)
		{

...
public Event_ChangeClass(Handle:event, const String:name[], bool:dontBroadcast) {
  ...
  if (IsFull(iClient, iTeam, iClass) && !IsImmune(iClient)) {
...
public Event_ChangeTeam(Handle:event,  const String:name[], bool:dontBroadcast) {
  ...
  if (IsFull(iClient, iTeam, g_iClass[iClient]) && !IsImmune(iClient)) {
    ShowVGUIPanel(iClient, iTeam == 3 ? "class_blue" : "class_red");
    PickClass(iClient);
  }
}
...
bool:IsFull(iClient, iTeam, iClass) {
  if (GetConVarBool(g_hEnabled) && iTeam > 1 && iClass > 0) {
	...
	if (IsClientInGame(i) && GetClientTeam(i) == iTeam && iClient != i && _:TF2_GetPlayerClass(i) == iClass && ++iCount == iLimit) {
	  return true;
	}
      }
    } else if (iLimit == 0) {

...
public Action:Cmd_CallVote(client, args)
{
	...
	PrintToChatAll("\x04[SM] \x03Votekick open for \x04%s. \x03Disconnecting will not help.", voteVictimName);
...
public Action:Timer_CallEnd(Handle:timer)
{
	if (votesYes <= votesNo)
	{
		PrintToChatAll("\x04[SM] \x03Votekick against \x04%s \x03failed (\x04%d \x03Yes, \x04%d \x03No).", voteVictimName, votesYes, votesNo);
		return Plugin_Continue;
	}
	
	if (isVictimAlreadyBanned)
	{
		PrintToChatAll("\x04[SM] \x03Votekick against \x04%s \x03[%s] succeeded (\x04%d \x03Yes, \x04%d \x03No). Standard \x04%d \x03minute ban.", voteVictimName, voteVictimSteamid, votesYes, votesNo, GetConVarInt(FindConVar("sv_vote_kick_ban_duration")));
	}
	else 
	{
		...
		PrintToChatAll("\x04[SM] \x03Votekick against \x03%s [%s] succeeded (\x04%d \x03Yes, \x04%d \x03No). Trying to escape: \x04%d \x03minute ban.", voteVictimName, voteVictimSteamid, votesYes, votesNo, banTime);

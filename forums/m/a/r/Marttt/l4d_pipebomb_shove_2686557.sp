...
ConVar g_hCvarWorld, g_hCvarWorldDistance, g_hCvarBlockSafe;
int g_iCvarWorld;
bool g_bCvarBlockSafe;
float g_fCvarWorldDistance;
...
public void OnPluginStart()
{
	...
	g_hCvarWorld = CreateConVar(	"l4d_pipebomb_shove_world",			"3",			"Attach pipebomb to world entities. (0=Off, 1=Worldspawn entities, 2=Other entities,3=Both)", CVAR_FLAGS, true, 0.0, true, 3.0 );
	g_hCvarWorldDistance = CreateConVar("l4d_pipebomb_shove_world_distance",	"75",	"Max distance allowed to attach to world entities. (0=No check)", CVAR_FLAGS, true, 0.0);
	g_hCvarBlockSafe = CreateConVar("l4d_pipebomb_shove_blocksafe",		"1",			"Block the plugin while no survivors leave the safe area. (0=Off, 1=On)", CVAR_FLAGS, true, 0.0, true, 1.0);
	CreateConVar(					"l4d_pipebomb_shove_version",		PLUGIN_VERSION,	"Pipebomb Shove plugin version.", CVAR_FLAGS_PLUGIN_VERSION);
	...
	g_hCvarWorld.AddChangeHook(ConVarChanged_Cvars);
	g_hCvarWorldDistance.AddChangeHook(ConVarChanged_Cvars);
	g_hCvarBlockSafe.AddChangeHook(ConVarChanged_Cvars);
...
void GetCvars()
{
	...
	g_iCvarWorld = g_hCvarWorld.IntValue;
	g_fCvarWorldDistance = g_hCvarWorldDistance.FloatValue;
	g_bCvarBlockSafe = g_hCvarBlockSafe.BoolValue;
}
...
{
	if( g_bCvarBlockSafe && !L4D_HasAnySurvivorLeftSafeArea() )
		return;
	...
	if( g_iCvarReload != 2 )
	{
		...
		int world = g_iCvarWorld & (1 << 0);
		int other = g_iCvarWorld & (1 << 1);

		if( infected || witch || world || other )
		{
			...
			if( weapon )
			{
				...
				else if( world && strcmp(sTemp, "worldspawn") == 0 )
				{
					HurtPlayer(target, client, weapon, -2);
				}
				else if( other && !StrEqual(sTemp, "infected", false ) && !StrEqual(sTemp, "witch", false ) && !StrEqual(sTemp, "worldspawn", false ) )
				{
					HurtPlayer(target, client, weapon, -2);
				}
			}
		...
		if( target > MaxClients || target == 0)
		{
			...
			if( infected || witch || world || other )
			{
				...
				if ( target > 0 )
					...
					Format(sTemp, sizeof(sTemp), "worldspawn");
...
void HurtPlayer(int target, int client, int weapon, int special)
{
	...
	else
	{
		...
		Handle trace = TR_TraceRayFilterEx(startpoint, vAng, MASK_SOLID, RayType_Infinite, RayDontHitSelf, client);

		if( TR_DidHit(trace) )
		{
			TR_GetEndPosition(vPos, trace);

			if( g_fCvarDistance > 0 )
			{
				float dist = GetVectorDistance(startpoint, vPos);
				if( dist > g_fCvarWorldDistance )
					return;
			}

			int traceEntity = TR_GetEntityIndex(trace);
			if( traceEntity != target )
			{
				CloseHandle(trace);
				return;
			}
...
stock bool L4D_HasAnySurvivorLeftSafeArea()
{
    ...
    return GetEntProp(entity, Prop_Send, "m_hasAnySurvivorLeftSafeArea", 1) != 0;
}
...
new Handle:WeaponSpeedRecoil = INVALID_HANDLE;

public OnPluginStart()
{
	LoadTranslations("common.phrases");

	CreateConVar("l4d2_weaponspeed_version", PLUGIN_VERSION, "L4D2 Weapon Attack Speed Modifier Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	WeaponSpeedEnabled = CreateConVar("l4d2_weaponspeed_everyone", "0", "<0|1> - Apply the Weapon Attack Speed Modifier on Everyone?", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	WeaponSpeedAmount = CreateConVar("l4d2_weaponspeed_amount", "1.6", "<1.0-2.0> - Weapon Attack Speed Modified Value", FCVAR_NOTIFY, true, 1.0, true, 2.0);
	WeaponSpeedRecoil = CreateConVar("l4d2_weaponspeed_recoil", "1", "<0|1> - Weapon Recoil Enabled?", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
stock AdjustWeaponSpeed(client, Float:Amount, slot)
{
	if (GetPlayerWeaponSlot(client, slot) > 0)
	{
		...
		if (m_flCycle == 0.000000 && m_bInReload < 1)
		{
			...
			if (GetConVarInt(WeaponSpeedRecoil) == 0)
				SetEntPropVector(client, Prop_Send, "m_vecPunchAngle", NULL_VECTOR); //no recoil
		}
	}
}
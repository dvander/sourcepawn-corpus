...
void initItemCosts()
{
	...
	ItemCosts.CostHuntingknife       = CreateConVar("l4d2_points_hunting_knife", "200", "How many points the hunting knife costs");
	ItemCosts.CostRiotshield         = CreateConVar("l4d2_points_riotshield", "200", "How many points the riotshield costs");
	ItemCosts.CostShovel             = CreateConVar("l4d2_points_shovel", "200", "How many points the shovel costs");
	ItemCosts.CostPitchfork          = CreateConVar("l4d2_points_pitchfork", "200", "How many points the pitchfork costs");
...
public Action Reminder(Handle hTimer, int userid)
{
	...
	{
		PrintToChat(iClientIndex, "%s !buy'da ki puanlarınız:: %d", MSGTAG, PlayerData[iClientIndex].iPlayerPoints);
		PrintHintText(iClientIndex, "[PS] !buy'da ki puanlarınız:: %d", PlayerData[iClientIndex].iPlayerPoints);
		PrintCenterText(iClientIndex, "[PS] !buy'da ki puanlarınız:: %d", PlayerData[iClientIndex].iPlayerPoints);
	}
}
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		if(0 < i <= MaxClients && IsClientConnected(i) && !IsFakeClient(i))
		{
			if(CheckCommandAccess(i, "sm_perks", ADMFLAG_CUSTOM1, true))
			{
				...
				SaveData(i);
				PrintToChat(i, "%s Steam grubu üyesi olduğunuz için \x03 100 \x01 puan kazandınız!", MSGTAG);
			}
			else
			{
				PrintToChat(i, "%s Steam grubumuza katılarak her 10 dakikada bir \x03 100 \x01 puan kazanabilirsiniz! \x04! 'H' Tuşu ile erişim sağlayın \x01 ve topluluğumuzun bir parçası olun!", MSGTAG);
			}
		}
	}
}
	...
	if(IsModEnabled() && iNumArguments == 0)
	{
		if(IsClientPlaying(iClientIndex))
		{
			...
			if(!CheckCommandAccess(iClientIndex, "sm_perks", ADMFLAG_CUSTOM1, true))
				ReplyToCommand(iClientIndex, "%s\x03 \x04 'H' tuşu ile erişim sağlayın \x03 ve bir dahaki sefere puanlarınızı kaydetmek için Steam grubumuza katılın! \x04 (Üye olmayanlar her kampanyada 100 kredi ile başlar.)", MSGTAG);
		}
	}
	return Plugin_Handled;
}
...
public Action ShowPoints(int iClientIndex, int iNumArguments)
{
	...
	{
		if(IsClientPlaying(iClientIndex))
			ReplyToCommand(iClientIndex, "%s %T", MSGTAG, "Puanlarınız", LANG_SERVER, PlayerData[iClientIndex].iPlayerPoints);
	}
	return Plugin_Handled;
}
...
bool IsItemEnabled(int iClientIndex, int iCost)
{
	if(iClientIndex > 0)
	{
		...
		{
			ReplyToCommand(iClientIndex, "%s %T", MSGTAG, "Ürün devreDIŞI!", LANG_SERVER);
			return false;
		}
	}
	return false;
...
bool HasEnoughPoints(int iClientIndex, int iCost)
{
	if(iClientIndex > 0)
	{
		...
		{
			ReplyToCommand(iClientIndex, "%s %T", MSGTAG, "Yetersiz Bakiye", LANG_SERVER);
			return false;
		}
	}
	return false;
...
public Action Command_Heal(int client, int args)
{
	...
	else if(args == 1)
	{
		...
		{
			ShowActivity2(client, MSGTAG, " %t", "Sağlık Ver", target_name);
	...
	{
		ReplyToCommand(client, "%s%T", MSGTAG, "Kullanımı: sm_heal", LANG_SERVER);
		return Plugin_Handled;
	}
}
...
public Action Command_Points(int client, int args)
{
	if(args == 2)
	{
		...
		else
		{
			for (int i = 0; i < target_count; i++)
			{
				...
				ReplyToCommand(client, "%s %T", MSGTAG, "Puan Ver", LANG_SERVER, amount, targetclient);
	...
	{
		ReplyToCommand(client, "%s %T", MSGTAG, "Kullanımı: sm_givepoints", LANG_SERVER);
		return Plugin_Handled;
	}
}
...
public Action Command_SPoints(int client, int args)
{
	if(args == 2)
	{
		...
		{
			//ShowActivity2(client, MSGTAG, "%t", "Set Puanları", target_name, amount);
			for (int i = 0; i < target_count; i++)
			{
				targetclient = target_list[i];
				PlayerData[targetclient].iPlayerPoints = amount;
				SaveData(targetclient);
				ReplyToCommand(client, "%s %T", MSGTAG, "Set Puanları", LANG_SERVER, targetclient, amount);
				PrintToChat(targetclient, "%s %N, puanlarınızı %d olarak ayarlayın!", MSGTAG, client, amount);
			}
			return Plugin_Handled;
		}
	}
	...
	{
		ReplyToCommand(client, "%s %T", MSGTAG, "Kullanımı: sm_setpoints", LANG_SERVER, MSGTAG);
		return Plugin_Handled;
	}
}

...
public void OnPluginStart()
{
    ...
    g_hCvar_Enabled         = CreateConVar("l4d_cloud_screen_fade_enable", "1", "Enable/Disable the plugin.\n0 = Disable, 1 = Enable.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_Color           = CreateConVar("l4d_cloud_screen_fade_color", "20 20 20", "Fade color.\nUse \"random\" for random colors.\nUse three values between 0-255 separated by spaces (\"<0-255> <0-255> <0-255>\").", CVAR_FLAGS);
    g_hCvar_Alpha           = CreateConVar("l4d_cloud_screen_fade_alpha", "245", "Fade alpha transparency.\n0 = Invisible, 255 = Fully Visible.", CVAR_FLAGS, true, 0.0, true, 255.0);
    g_hCvar_FadeOutDuration = CreateConVar("l4d_cloud_screen_fade_out_duration", "0.5", "Fade out (start) duration in seconds.", CVAR_FLAGS, true, 0.0);
    g_hCvar_FadeInDuration  = CreateConVar("l4d_cloud_screen_fade_in_duration", "0.5", "Fade in (end) duration in seconds.", CVAR_FLAGS, true, 0.0);
    g_hCvar_FadeLife        = CreateConVar("l4d_cloud_screen_fade_life", "14.0", "How long will the trigger area exist before it disappears.", CVAR_FLAGS, true, 0.0);
    g_hCvar_FadeSize        = CreateConVar("l4d_cloud_screen_fade_size", "100.0", "Fade trigger area size.", CVAR_FLAGS, true, 0.0);
    g_hCvar_Block           = CreateConVar("l4d_cloud_screen_fade_block", "1", "Block other fade effects applied to the client while on plugin fade.\nExample: Red screen when take damage.\n0 = Disable, 1 = Enable.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_Team            = CreateConVar("l4d_cloud_screen_fade_team", "1", "Which teams should be affected by the plugin.\n0 = NONE, 1 = SURVIVOR, 2 = INFECTED, 4 = SPECTATOR, 8 = HOLDOUT.\nAdd numbers greater than 0 for multiple options.\nExample: \"3\", enables for SURVIVOR and INFECTED.", CVAR_FLAGS, true, 0.0, true, 15.0);
    ...
    // Admin Commands
    RegAdminCmd("sm_cloudfade", CmdCloudFade, ADMFLAG_ROOT, "Add a fade effect on self (no args) or specified targets. Example: self -> sm_cloudfade / target -> sm_cloudfade @humans.");
    RegAdminCmd("sm_print_cvars_l4d_cloud_screen_fade", CmdPrintCvars, ADMFLAG_ROOT, "Print the plugin related cvars and their respective values to the console.");
}

...
static BotTeleport[MAXPLAYERS+1];
...
stock LoadConfig()
{
	...
	new Handle:MapsFile = CreateKeyValues("maps");
	if (!FileToKeyValues(MapsFile, Path))
	{
	...
	KvGetVector(MapsFile, "origin", NozzleOrigin2);
	KvGetVector(MapsFile, "angles", NozzleAngles2);
	CloseHandle(MapsFile);
}

...
ConVar shEnable, shUse, shIncapPickup, shDelay, shKillAttacker, shBot, shBotChance, shHardHP,
	shTempHP, cvarMaxIncapCount, cvarReviveDuration;

bool bIsL4D, bEnabled, bIncapPickup, bKillAttacker, bBot;
float fLastPos[MAXPLAYERS+1][3], fDelay, fTempHP, fSelfHelpTime[MAXPLAYERS+1];
int iSurvivorClass, iUse, iBotChance, iHardHP, iAttacker[MAXPLAYERS+1], iBotHelp[MAXPLAYERS+1],
	iMaxIncapCount, iReviveDuration;
...
void LoadPluginTranslations()
{
    ...
    BuildPath(Path_SM, sPath, PLATFORM_MAX_PATH, "translations/%s.txt", "self_help.phrases");
    if (FileExists(sPath))
        LoadTranslations("self_help.phrases");
    else
        SetFailState("Missing required translation file on \"translations/%s.txt\", please re-download.", "self_help.phrases");
}
...
public void OnPluginStart()
{
	...
	if (!bIsL4D)
	{
		...
		StartPrepSDKCall(SDKCall_Player);
		PrepSDKCall_SetFromConf(hSHGameData, SDKConf_Signature, "CTerrorPlayer_OnRevived");
		hSHOnRevived = EndPrepSDKCall();
		if (hSHOnRevived == null)
		{
			SetFailState("[SH] Signature 'CTerrorPlayer_OnRevived' Broken!");
		}
		
		StartPrepSDKCall(SDKCall_Player);
		PrepSDKCall_SetFromConf(hSHGameData, SDKConf_Signature, "CTerrorPlayer_OnStaggered");
		PrepSDKCall_AddParameter(SDKType_CBaseEntity, SDKPass_Pointer);
		PrepSDKCall_AddParameter(SDKType_Vector, SDKPass_ByRef);
		hSHStagger = EndPrepSDKCall();
		if (hSHStagger == null)
		{
			SetFailState("[SH] Signature 'CTerrorPlayer_OnStaggered' Broken!");
		}
	...
	shUse = CreateConVar("self_help_use", "1", "Use: 0=None, 1=Pills And Adrenalines, 2=First Aid Kits Only, 3=Both", FCVAR_NOTIFY|FCVAR_SPONLY, true, 0.0, true, 3.0);
	...
	shKillAttacker = CreateConVar("self_help_kill_attacker", "1", "Enable/Disable Attacker Kills", FCVAR_NOTIFY|FCVAR_SPONLY, true, 0.0, true, 1.0);
	shBot = CreateConVar("self_help_bot", "1", "Enable/Disable Bot Self-Help", FCVAR_NOTIFY|FCVAR_SPONLY, true, 0.0, true, 1.0);
	shBotChance = CreateConVar("self_help_bot_chance", "2", "Chance Of Bot Self-Helping: 1=Sometimes, 2=Often, 3=Seldom", FCVAR_NOTIFY|FCVAR_SPONLY, true, 1.0, true, 3.0);
	shHardHP = CreateConVar("self_help_hard_hp", "1", "Health Given After Self-Helping", FCVAR_NOTIFY|FCVAR_SPONLY, true, 1.0);
	shTempHP = CreateConVar("self_help_temp_hp", "99.0", "Temporary Health Given After Self-Helping", FCVAR_NOTIFY|FCVAR_SPONLY, true, 1.0);
...
{
	if (StrEqual(sample, "music/terror/PuddleOfYou.wav", false) || StrEqual(sample, "music/terror/ClingingToHell1.wav", false) || StrEqual(sample, "music/terror/ClingingToHell2.wav", false) || 
		StrEqual(sample, "music/terror/ClingingToHell3.wav", false) || StrEqual(sample, "music/terror/ClingingToHell4.wav", false))
	{
		if (!IsSurvivor(entity) || !IsPlayerAlive(entity) || !GetEntProp(entity, Prop_Send, "m_isIncapacitated", 1))
		{
			return Plugin_Stop;
		}
	}
	else if (StrEqual(sample, "music/tags/PuddleOfYouHit.wav", false) || StrEqual(sample, "music/tags/ClingingToHellHit1.wav", false) || StrEqual(sample, "music/tags/ClingingToHellHit2.wav", false) || 
...
public Action FireUpMechanism(Handle timer, any userid)
{
	...
	if (hSHTime[client] == null)
	{
		...
		{
			PrintToChat(client, "%t", "Press To Self Help");
		}
		hSHTime[client] = CreateTimer(0.1, CheckPlayerState, GetClientUserId(client), TIMER_REPEAT);
	}
	
	return Plugin_Stop;
}

public Action CheckPlayerState(Handle timer, any userid)
{
	int client = GetClientOfUserId(userid);
	...
	{
		shsBit[client] = SHS_NONE;
		if (iAttacker[client] == 0 || (iAttacker[client] != 0 && (!IsClientInGame(iAttacker[client]) || !IsPlayerAlive(iAttacker[client]))) || (GetEntProp(client, Prop_Send, "m_isIncapacitated", 1) && GetEntProp(client, Prop_Send, "m_reviveOwner") < 1))
		{
			RemoveSHProgressBar(client);
		}
		...
		if (iButtons & IN_DUCK)
		{
			if (shsBit[client] == SHS_NONE || shsBit[client] == SHS_CONTINUE)
			{
				...
				if (!IsFakeClient(client))
				{
					...
					DisplaySHProgressBar(client, iReviveDuration, sSHMessage);
		...
		if (IsSurvivor(iTarget) && IsPlayerAlive(iTarget) && GetEntProp(iTarget, Prop_Send, "m_isIncapacitated", 1) && GetEntProp(iTarget, Prop_Send, "m_reviveOwner") < 1)
		{
			if (shsBit[client] == SHS_NONE || shsBit[client] == SHS_CONTINUE)
			{
				...
				if (!IsFakeClient(client))
				{
					...
					if (!bIsL4D)
					{
						PrintHintText(client, "%t", "Helping Target", iTarget);
					}
				}
				
				if (!IsFakeClient(iTarget))
				{
					Format(sSHMessage, sizeof(sSHMessage), "BEING HELPED\nYour helper: %N", client);
					DisplaySHProgressBar(iTarget, iReviveDuration, sSHMessage);
					
					if (!bIsL4D)
					{
						PrintHintText(iTarget, "%t", "Helping You", client);
					}
				}
		...
		if (!CheckPlayerSupply(client, 3))
		{
			while ((iItemEnt = FindEntityByClassname(iItemEnt, "weapon_first_aid_kit")) != -1)
			{
				...
				if (GetVectorDistance(fPos, fItemPos) <= 150.0)
				{
					...
					PrintHintText(client, "%t", "Grabbing First Aid Kit");
		...
		else if (!CheckPlayerSupply(client, 4))
		{
			while ((iItemEnt = FindEntityByClassname(iItemEnt, "weapon_pain_pills")) != -1)
			{
				...
				if (GetVectorDistance(fPos, fItemPos) <= 150.0)
				{
					...
					PrintHintText(client, "%t", "Grabbing Pain Pills");
			...
			if (!bIsL4D)
			{
				while ((iItemEnt = FindEntityByClassname(iItemEnt, "weapon_adrenaline")) != -1)
				{
					...
					if (GetVectorDistance(fPos, fItemPos) <= 150.0)
					{
						...
						PrintHintText(client, "%t", "Grabbing Adrenaline");
...
public Action SHReviveCompletion(Handle timer, Handle dpSHRevive)
{
	...
	if (!IsSurvivor(client) || !IsPlayerAlive(client) || hSHTime[client] == null || !(GetClientButtons(client) & IN_DUCK) || shsBit[client] == SHS_NONE)
	{
	...
	if (fSelfHelpTime[client] >= float(iReviveDuration) + 0.1)
	{
		...
		bool bAidCheck;
		SHStatsFixer(client, (GetEntProp(client, Prop_Send, "m_isHangingFromLedge", 1)) ? true : false, bAidCheck);
...
public Action SHReviveOtherCompletion(Handle timer, Handle dpSHReviveOther)
{
	...
	if (!IsSurvivor(reviver) || !IsPlayerAlive(reviver) || hSHTime[reviver] == null || !(GetClientButtons(reviver) & IN_RELOAD) || shsBit[reviver] == SHS_NONE)
	{
	...
	if (fSelfHelpTime[reviver] >= float(iReviveDuration) + 0.1)
	{
		...
		bool bTempCheck;
		SHStatsFixer(revived, (!GetEntProp(revived, Prop_Send, "m_isHangingFromLedge", 1)) ? false : true, bTempCheck);
...
public void OnPlayerDown(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (IsSurvivor(wounded))
	{
		...
		{
			PrintHintText(wounded, "%t", "Press To Help Other");
			...
			PrintToChat(wounded, "%t", "You Got Incapacitated", iReviveCount + 1, iMaxIncapCount);
			...
			if (iReviveCount + 1 == iMaxIncapCount)
			{
				for (int i = 1; i <= MaxClients; i++)
				{
					...
					PrintHintText(i, "%t", "B/W Warning", wounded);
				}
			}
		}
	}
...
public void OnReviveSuccess(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (IsSurvivor(reviver) && IsSurvivor(revived))
	{
		...
		if (event.GetBool("ledge_hang"))
		{
			if (reviver != revived)
			{
				PrintToChat(reviver, "%t", "You Helped", reviver, revived);
				if (!IsFakeClient(revived))
				{
					PrintToChat(revived, "%t", "Helped You", revived, reviver);
				}
			}
			else
			{
				if (!IsFakeClient(revived))
				{
					PrintToChat(revived, "%t", "Helped Yourself", revived);
				}
			}
		}
		else
		{
			...
			if (reviver == revived)
			{
				if (!IsFakeClient(revived))
				{
					PrintToChat(revived, "%t", "Helped Yourself", revived, iReviveCount, iMaxIncapCount);
				}
			}
			else
			{
				if (GetEntProp(reviver, Prop_Send, "m_isIncapacitated", 1))
				{
					for (int i = 1; i <= MaxClients; i++)
					{
						...
						{
							PrintToChat(i, "%t", "Saved While Incapacitated", i, reviver, revived);
						}
					}
				}
				
				PrintToChat(reviver, "%t", "You Helped Count", reviver, revived, iReviveCount, iMaxIncapCount);
				if (!IsFakeClient(revived))
				{
					PrintToChat(revived, "%t", "Helped You Count", revived, reviver, iReviveCount, iMaxIncapCount);
				}
			}
		}
...
void SHStatsFixer(int client, bool bDoNotTamper, bool &bMedkitUsed)
{
	if (shsBit[client] == SHS_START_SELF)
	{
	...
	if ((iUse == 1 || iUse == 3) && CheckPlayerSupply(client, 4, iUsedItem, sUsedItemName))
	{
		if (RemovePlayerItem(client, iUsedItem))
		{
			...
			if (!bIsL4D && StrEqual(sUsedItemName, "weapon_adrenaline", false))
			{
				...
				SDKCall(hSHAdrenalineRush, client, 15.0);
				
				for (int i = 1; i <= MaxClients; i++)
				{
					if (IsClientInGame(i) && !IsFakeClient(i))
					{
						PrintToChat(i, "%t", "Helped Themselves With Adrenaline", i, client);
					}
				}
			}
			else
			{
				...
				for (int i = 1; i <= MaxClients; i++)
				{
					if (IsClientInGame(i) && !IsFakeClient(i))
					{
						PrintToChat(i, "%t", "Helped Themselves With Pain Pills", i, client);
					}
				}
			}
		}
	...
	else if ((iUse == 2 || iUse == 3) && CheckPlayerSupply(client, 3, iUsedItem))
	{
		if (RemovePlayerItem(client, iUsedItem))
		{
			...
			for (int i = 1; i <= MaxClients; i++)
			{
				if (IsClientInGame(i) && !IsFakeClient(i))
				{
					PrintToChat(i, "%t", "Helped Themselves With First Aid Kit", i, client);
				}
			}

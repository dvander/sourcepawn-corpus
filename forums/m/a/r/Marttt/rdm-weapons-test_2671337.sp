...
public void GetRandomWeapon()
{
	...
	if (totalProba == 0.0)
	{
		PrintToServer("Random Weapons ERROR: All weapons disabled");
	}
	else
	{
		bool runAgain = true;
		while (runAgain)
		{
			int rdmIndex = GetRandomInt(0, (sizeof weapons_all) -1);
			runAgain = GetConVarFloat(h_weapons_cvar[rdmIndex]) < GetRandomFloat(0.0, 1.0);
			
			if (!runAgain)
				Format(weapon_selected, sizeof(weapon_selected), "%s", weapons_all[rdmIndex]);
		}
	}
}
...
public void Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (CheckKillCount(client_target) || !GetConVarBool(h_spawn_last_weapon))
	{
		...
		GiveRandomWeapon2(client_target);
	}
...
public void GiveRandomWeapon2(int client_target)
{
	//kills_per_client[CheckClient(client_target)][3] = i;
	GiveWeapon(weapon_selected, client_target);
}

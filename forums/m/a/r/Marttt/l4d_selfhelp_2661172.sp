...
#include <sdktools>
#pragma semicolon 1
#define PLUGIN_VERSION "1.0.1m41"
...
new Handle:l4d_selfhelp_bot = INVALID_HANDLE;
...
DangerStatus(client)
{
	...
	if(IsClientConnected(client) && IsClientInGame(client) /*&& IsPlayerIncap(client)*/ && GetClientTeam(client) == 2)
	{
		...
		if(!HavePills(client) && HaveKit(client))
		{
			dangerSt-=20;
		}
	}
...
bool:CanSelfHelp(client)
{
	...
	if(IncapType[client]==INCAP)
	{
		...
		else if ((self==2 || self==3) && kit)ok=true;
	}
...
SelfHelpUseSlot(client)
{
	...
	if(IncapType[client]==INCAP)
	{
		...
		else if ((self==2 || self==3) && kit)solt=3;
	}
	else if(IncapType[client]== INCAP_EDGEGRAB)
	{
		self=GetConVarInt( l4d_selfhelp_edgegrab);
...
{
	if(GameMode==2 && GetConVarInt(l4d_selfhelp_versus)==0)return Plugin_Stop;
...
SelfHelp(client)
{
	...
	if(slot!=-1)
	{
		...
		else if(slot==3)
		{
			...
			HelpState[client]=STATE_OK;
			PrintToChatAll("%t", "Self Help with Medkit", client);
			//EmitSoundToClient(client, "player/items/pain_pills/pills_use_1.wav"); // add some sound
		}
	}
	else

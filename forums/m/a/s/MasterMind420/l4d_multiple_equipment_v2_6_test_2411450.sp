...
new Handle:ME_Activate[MAXPLAYERS+1];
new Handle:ME_Activate_Destroy[MAXPLAYERS+1];
...
{
	ME_Activate[client] = CreateTimer(1.0, ME_Activation, client, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
	CreateTimer(0.1, CheckClientThirdperson, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE); //GetRandomFloat(2.5, 3.5);
}

public Action:ME_Activation(Handle:Timer, any:client)
{
	...
	if (IsClientInGame(client) && GetClientTeam(client) == 2 && IsPlayerAlive(client))
	{
		...
		if(MeEnable[client]==false)
		{
			...
			//ShowMsg(client, "[MULTIPLE EQUIPMENT v2.5][--->PRESS [H] FOR HELP<---]");
			//PrintHintText(client, "|MULTIPLE EQUIPMENT v2.5||--->PRESS [H] FOR HELP<---|");
...
public bot_player_replace(Handle:Spawn_Event, const String:Spawn_Name[], bool:Spawn_Broadcast)
{
	...
	LoadEquipmentAFK(bot);
	AttachAllEquipmentAFK(client);
	AttachAllEquipmentAFK(bot);
	*/
}
...
public Action:player_spawn(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
	...
	ME_Activate_Destroy[client] = CreateTimer(20.0, ME_Activation_Destroy, client, TIMER_FLAG_NO_MAPCHANGE);
}

...
public OnPluginStart()
{
	...
	//CreateTimer(GetRandomFloat(2.5, 3.5), CheckClientThirdperson, _, TIMER_REPEAT); //(2.5, 3.5)
}
		...
		if (IsClientInGame(client) && GetClientTeam(client)==2)
		{
			new weapon = GetPlayerWeaponSlot(client, 0);
			{
				if(IsValidEdict(weapon))
				{
					...
					{
						/*
						if (StrEqual(M60, "weapon_rifle_m60"))
						{
...
public Action:player_spawn(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
	...
	//for (new client = 1; client <= MaxClients; client++)
	{
		if (IsClientInGame(client) && GetClientTeam(client) == 2 && IsPlayerAlive(client))
		{
			...
			ResetClientState(client);
			AutoViewOffAll()
			//CreateTimer(0.1, ActivateEquipment, client, TIMER_FLAG_NO_MAPCHANGE);
		}
	}
}

//public Action:ActivateEquipment(Handle:timer, any:client)
//{
//	g_gamestart=true;
//	DwEnable[client]=true;
//	LoadEquipment(client);
//	AttachAllEquipment(client);
//	for (new client = 1; client <= MaxClients; client++)
//	{
...
public Action:player_death(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
	...
	{
		if(!IsPlayerAlive(client) && GetClientTeam(client) == 2) // && GetClientHealth(client) <= 0)
		{
			EnableClient(client, false);
			RemoveItemAttach(client, -1);
			DropSecondaryItem(client);
...
public Action:round_start(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	DisableAllFirstRun();
	AutoViewOffAll();
	CreateTimer(0.1, CheckClientThirdperson, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE); //GetRandomFloat(2.5, 3.5);
...
{
	//new client = GetClientOfUserId(GetEventInt(hEvent, "userid"));
	//if(GetClientHealth(client) <= 0 && IsClientInGame(client) && GetClientTeam(client)==2) //client>0 
...
public Action:item_pickup(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//{
		new client = GetClientOfUserId(GetEventInt(event, "userid"));
		if (client == 0) return;
		else if (IsClientInGame(client) && GetClientTeam(client) == 2 && IsPlayerAlive(client))
		{
			...
			if(MeEnable[client]==false)
			{
				...
				//PrintHintText(client, "|MULTIPLE EQUIPMENT| PRESS [H] FOR HELP");
				ModeSelectMenu(client);
			}

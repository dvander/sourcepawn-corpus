...
new	Handle:cvarNonCritBackstabs, Handle:cvarWaterDoves, Handle:cvarDeadRingerTaunt,
	...
	Handle:cvarDemoGuaranteeCrit, Handle:cvarCleaverReflectIcon, Handle:cvarPreroundMove, Handle:cvarChargeSound, Handle:cvarPomsonHitbox,
	Handle:cvarDecalRespawn, Handle:cvarWrangledRocketIcon, Handle:cvarCleaverCritBleed, Handle:cvarTankDestroySound;
...
new bool:Enabled = true, bool:TomislavAnnounce = true, bool:Gunslinger = true, bool:OriginalDraw = true,
	...
	bool:DecalRespawn = true, bool:CleaverCritBleed = true, bool:TankDestroySound = true;
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	...
	if (attacker > 0 && attacker <= MaxClients)
	{
		...
		if (CleaverCritBleed && weapon > MaxClients)
		{
			...
			{
				if (damagetype == DMG_SLASH && TF2_IsPlayerInCondition(victim, TFCond_Dazed))
				{

...
stock GenerateItemString(any:client, String:method[], String:item[], String:fakeUser[])
{
	...
	{
		if (GetClientTeam(client) == 1) Format(itemString, 128, "\x07CCCCCC%s", fakeUser);
		if (GetClientTeam(client) == 2) Format(itemString, 128, "\x07FF4040%s", fakeUser);
		if (GetClientTeam(client) == 3) Format(itemString, 128, "\x0799CCFF%s", fakeUser);
	}
	Format(itemString, 128, "%s \x01%s\03", itemString, method);
	if (StrContains(item, "Vintage", false) == 0) Format(itemString, 128, "%s \x07576291%s\x01", itemString, item);
...
{
	ReplyToCommand(client, "\x04[SM]\x01 Fake and Force %s by MasterOfTheXP", PLUGIN_VERSION);
	ReplyToCommand(client, "\x04[SM]\x01 You can type most of these by themselves to find out more about them:");
	ReplyToCommand(client, "\x04[SM]\x01 /fake...ach, buy, craft, receive, find, quit, trade, unbox, earn, death, heal, hhh, mono, fish, join, block, time");
	ReplyToCommand(client, "\x04[SM]\x01 /force...cmd, cmd2, say, say_team, taunt, voice");
...
public Action:Command_fakedeath(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakedeath <victim> [killer] [assister] [weapon] - Fakes a death. Specify _ to skip arguments.");
	...
	for (new i = 0; i < target_count; i++)
	{
		new killer = -1;
		new assister = -1;
		if (!StrEqual(arg2,"") && !StrEqual(arg2,"_")) killer = FindTarget(client, arg2);
		if (!StrEqual(arg3,"") && !StrEqual(arg3,"_")) assister = FindTarget(client, arg3);
	...
	LogAction(client, -1, "%N used Fake Death on %s", client, target_name);
...
public Action:Command_fakeheal(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakeheal <target> [amount] - Fakes a Crusader's Crossbow heal.");
	...
	LogAction(client, -1, "%N used Fake Heal on %s (amount: %i)", client, target_name, amount);
...
public Action:Command_fakehhh(client, args)
{
	...
	LogAction(client, -1, "%N used Fake Horsemann", client);
...
public Action:Command_fakemono(client, args)
{
	...
	LogAction(client, -1, "%N used Fake MONOCULUS!", client);
...
public Action:Command_fakefish(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakefish <victim> [attacker] [assister] [weapon] - Fakes a Holy Mackerel/Unarmed Combat hit-notice. Specify _ to skip arguments.");
	...
	LogAction(client, -1, "%N used Fake Fish on %s", client, target_name);
...
public Action:Command_fakejoin(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakejoin <username> [IP] [Steam ID, commas instead of colons] - Fakes a client connect. Specify _ to skip arguments.");
	...
	LogAction(client, -1, "%N used Fake Join: %s (%s) (%s)", client, arg1, arg2, arg3);
...
public Action:Command_fakeblock(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakeblock <defender> [control point name] - Fakes the defense of a Control Point.");
	...
	LogAction(client, -1, "%N used Fake Block on %s (Control Point: %s)", client, target_name, arg2);
...
public Action:Command_faketime(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_faketime <seconds> - Fakes adding seconds to the round timer.");
...
public Action:Command_fakeach(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakeach <target> <achievement> - Fake getting an achievement. Doesn't have to be real.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		Format(message, sizeof(message), "\x03%N\x01 has earned the achievement \x05%s", target_list[i], arg2);
		SayText2(target_list[i], message);
		...
		LogAction(client, -1, "%N used Fake Achievement on %N: '%s'", client, target_list[i], arg2);
	}
...
public Action:Command_fakebuy(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakebuy <target> <item> - Fake someone buying something from the Mann Co. Store.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Fake Buy on %N: '%s'", client, target_list[i], arg2);
	}
...
public Action:Command_fakecraft(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakecraft <target> <item> - Fake someone crafting something.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Fake Craft on %N: '%s'", client, target_list[i], arg2);
	}
...
public Action:Command_fakereceive(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakereceive <target> <item> - Fake someone receiving a gift.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Fake Gift on %N: '%s'", client, target_list[i], arg2);
	}
...
public Action:Command_fakefind(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakefind <target> <item> - Fake someone finding something from an item drop.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Fake Find on %N: '%s'", client, target_list[i], arg2);
	}
...
public Action:Command_faketrade(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_faketrade <target> <item> - Fake someone finishing a trade and receiving an item.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Fake Trade on %N: '%s'", client, target_list[i], arg2);
	}
...
public Action:Command_fakeunbox(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakeunbox <target> <item> - Fake someone uncrating something.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Fake Unbox on %N: '%s'", client, target_list[i], arg2);
	}
...
public Action:Command_forcecmd(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_forcecmd <target> <command> - Force someone to execute a command. Game-changing commands like 'quit' or 'bind' do not work.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Force Command on %N: '%s'", client, target_list[i], arg2);
	}
...
public Action:Command_forcecmd2(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_forcecmd2 <target> <command> - Force someone to execute a command. Game-changing commands like 'quit' or 'bind' do not work.");
...
public Action:Command_forcesay(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_forcesay <target> <text> - Force someone to say something.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Force Say on %N: '%s'", client, target_list[i], arg2);
	}
...
public Action:Command_forcesayteam(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_forcesay_team <target> <text> - Force someone to say something to their team.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Force Say Team on %N: '%s'", client, target_list[i], arg2);
	}
...
public Action:Command_forcevoice(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_forcevoice <target> <voice command id or name> - Force someone to use a voice command.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Force Voice Command on %N: %s %s", client, target_list[i], arg1, arg2);
	}
...
public Action:Command_fakeearn(client, args)
{
	if (args < 2)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakeearn <target> <item> - Fake someone earning something from winning duels.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Fake Earn on %N: '%s'", client, target_list[i], arg2);
	}
...
public Action:Command_forcetaunt(client, args)
{
	if (args != 1)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_forcetaunt <target> - Force someone to taunt.");
	...
	for (new i = 0; i < target_count; i++)
	{
		...
		LogAction(client, -1, "%N used Force Taunt on %N", client, target_list[i]);
	}
...
public Action:Command_fakequit(client, args)
{
	if (args == 0)
	{
		ReplyToCommand(client, "\x04[SM]\x01 Usage: sm_fakequit <target> [reason] - Fake someone leaving the server.");
	...
	if (args == 1)
	{
		...
		for (new i = 0; i < target_count; i++)
		{
			...
			LogAction(client, -1, "%N used Fake Quit on %N: 'Disconnect by user.'", client, target_list[i]);
		}
	}
	if (args == 2)
	{
		...
		for (new i = 0; i < target_count; i++)
		{
			...
			LogAction(client, -1, "%N used Fake Quit on %N: '%s'", client, target_list[i], arg2);
		}
	}

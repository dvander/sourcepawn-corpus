...
new	Handle:cvarNonCritBackstabs, Handle:cvarWaterDoves, Handle:cvarDeadRingerTaunt,
	...
	Handle:cvarBuildingHealth, Handle:cvarBonkTaunt;
...
new bool:Enabled = true, bool:TomislavAnnounce = true, bool:Gunslinger = true, bool:OriginalDraw = true,
	...
	bool:DecalRespawn = true, bool:CleaverCritBleed = true, bool:TankDestroySound = true, bool:InvulnDiamondback = true, bool:BuildingHealth = true;
...
{
	new Handle:cvarVersion = CreateConVar("sm_tf2fix_version", PLUGIN_VERSION, "Plugin version smoke! Don't touch this!", FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY);
	...
	cvarBuildingHealth = CreateConVar("tf_fix_engineer_building_health", "1", "Fix level 3 buildings usually having 217 health instead of 216 health.", FIX_CONVAR_FLAGS, true, 0.0, true, 1.0);
	...
	HookConVarChange(cvarBuildingHealth, OnConVarChanged);
	
	AutoExecConfig(true, "tf2fix");
	
	new String:cfgVersion[64];
	GetConVarString(cvarVersion, cfgVersion, sizeof(cfgVersion));
	if (!StrEqual(cfgVersion, PLUGIN_VERSION, false))
	{
		SetConVarString(cvarVersion, PLUGIN_VERSION);
		PrintToServer("[TF2Fix] It looks like your TF2Fix config (tf/cfg/sourcemod/tf2fix.cfg) is outdated; you should probably delete it and re-configure it!");
	}
...
{
	if (!Enabled) return Plugin_Continue;
	if (GetConVarBool(cvarBonkTaunt)) // shusdivhivdshvsdudvshuidvsuhivsduihdvshuisdvhuihuidvisdvuhihdvu
	{
...
public Action:SoundHook(clients[64], &numClients, String:sound[PLATFORM_MAX_PATH], &client, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	{
		if (StrContains(sound, "demo_charge_hit", false) > -1)
		{
...
public OnEntityDestroyed(ent)
{
	...
	if (TankDestroySound && GetGameTime() > 1.5)
	{
		new String:cls[6];
...
public Action:Timer_EveryHalfSecond(Handle:timer)
{
	...
	if (BuildingHealth)
	{
		...
		while ((ent = FindEntityByClassname(ent, "obj_*")) != -1)
		{
			new hp = GetEntProp(ent, Prop_Send, "m_iHealth"), maxhp = GetEntProp(ent, Prop_Send, "m_iMaxHealth");
...
public TF2_OnConditionAdded(client, TFCond:cond)
{
	...
	if (cond == TFCond_Charging && ChargeSound)
...
public OnConVarChanged(Handle:cvar, const String:oldValue[], const String:newValue[])
{
	...
	else if (cvar == cvarBuildingHealth) BuildingHealth = GetConVarBool(cvar);
}

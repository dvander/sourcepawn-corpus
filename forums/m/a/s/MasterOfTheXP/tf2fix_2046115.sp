...
public OnPluginStart()
{
	...
	cvarDeadAction = CreateConVar("tf_fix_soldier_whip_dead", "1", "Fix an exploit where you could interact with alive players with the Disciplinary Action while dead", FIX_CONVAR_FLAGS, true, 0.0, true, 1.0);
...
//		action = Plugin_Changed;
	...
	if (!(GetEventInt(event, "death_flags") & TF_DEATHFLAG_DEADRINGER))
	{
		...
		if (GetConVarBool(cvarDeadAction) && 447 == GetPlayerWeaponIndex(victim, 2))
		{
			new wep = GetPlayerWeaponSlot(victim, 2);
			if (wep > MaxClients) AcceptEntityInput(wep, "Kill");
		}
	}
	return action;
}

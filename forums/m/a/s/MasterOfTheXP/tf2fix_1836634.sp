...
Handle:cvarHeavyStun, Handle:cvarNPCSounds, Handle:cvarBFBHype, Handle:cvarCanteenExploit;
...
bool:Incoming = true, bool:PomsonSound = true, bool:CowManglerReflectIcon = true, bool:DedTaunts = true, NPCBlackBox = 15,
...
bool:HeavyStun = true, bool:NPCSounds = true, bool:BFBHype = true, bool:CanteenExploit = true;
...
public OnPluginStart()
{
	...
	cvarNPCBlackBox = CreateConVar("sm_tf2fix_bossonhit","15","If on, 'on hit' effects trigger when attacking these boss characters: 1=HHH, 2=Mono, 4=Tank, 8=Merasmus. Add up the numbers to get the value you want, or 0 to disable.", Flags, true, 0.0, true, 15.0);
	...
	cvarBannerExploit = CreateConVar("sm_tf2fix_bannersaverage","1","If on, Soldiers can't hold down primary fire with a banner to sustain the rage.", Flags, true, 0.0, true, 1.0);
	...
	cvarBFBHype = CreateConVar("sm_tf2fix_bfbhype","1","If on, fixes the Soda Popper and Baby Face Blaster meters being interchangeable.", Flags, true, 0.0, true, 1.0);
	cvarCanteenExploit = CreateConVar("sm_tf2fix_canteenexploit","1","If on, fixes an infinite powerup exploit in Mann vs Machine.", Flags, true, 0.0, true, 1.0);
	...
	HookEvent("player_used_powerup_bottle", Event_KillMeComeBackStronger, EventHookMode_Pre);
	...
	HookConVarChange(cvarBFBHype, CvarChange);
	HookConVarChange(cvarCanteenExploit, CvarChange);
	
	AddNormalSoundHook(SoundHook);
	
	new String:disGaem[10];
	GetGameFolderName(disGaem, 10);
	if (strncmp(disGaem, "tf", 2, false) != 0) SetFailState("TF2Fix, a plugin that fixes Team Fotress 2, doesn't work on any game except, um, Bastion!");
...
bool:dontAttack[MAXPLAYERS + 1] = { false, ... }, MonoSpawns, bool:hasSodaPopper[MAXPLAYERS + 1];
...
public Action:Event_Hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (custom == TF_CUSTOM_TAUNT_GRENADE && victim == attacker)
	{
		if ((TF2_IsPlayerInCondition(victim, TFCond_Ubercharged) && UberCupcake) || (victimPrimary == 237 && JumperCupcake))
			CreateTimer(0.1, Explode, GetClientUserId(victim));
	}
	if (attacker != 0)
	{
		if ((wepEnt = GetPlayerWeaponSlot(attacker, 2))!=-1)
...
public Action:Event_NpcHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new String:cls[25], BossType;
	GetEntityClassname(Ent, cls, sizeof(cls));
	if (cls[0] == 'h') BossType = 1;
	else if (cls[0] == 'e') BossType = 2;
	else if (cls[0] == 't') BossType = 4;
	else if (cls[0] == 'm') BossType = 8;
	...
	if (!(NPCBlackBox & BossType)) return Plugin_Continue;
...
public Action:Timer_LateRegen(Handle:timer, any:uid)
{
	...
	if (((hasSodaPopper[client] && primaryWeapon != 448) || (!hasSodaPopper[client] && primaryWeapon == 448) && BFBHype))
	{
		SetEntPropFloat(client, Prop_Send, "m_flHypeMeter", 0.0);
		TF2_AddCondition(client, TFCond_SpeedBuffAlly, 0.01);
	}
	if (primaryWeapon == 448) hasSodaPopper[client] = true;
	else hasSodaPopper[client] = false;
...
public Action:Event_Item(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ((idx == 5000 || idx == 5001 || idx == 5002) && method == 1 && !GetEventBool(event, "isfake")) SetEventBroadcast(event, true);
	return Plugin_Continue;
}
...
public Action:Event_KillMeComeBackStronger(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (!Enabled || !CanteenExploit) return Plugin_Continue;
	new client 	= GetEventInt(event, "player");
	new type		= GetEventInt(event, "type");
	new Float:time	= GetEventFloat(event, "time");
	switch (type)
	{
		case MvMCanteen_Crits: CreateTimer(time, MvMUnKritz, GetClientUserId(client));
		case MvMCanteen_Uber: CreateTimer(time, MvMUnUber, GetClientUserId(client));
	}
	new patient = GetHealingTarget(client);
	if (patient > -1 && type <= 2)
	{
		new Handle:hData = CreateDataPack();
		CreateTimer(0.017, CheckPatientForCond, hData, TIMER_FLAG_NO_MAPCHANGE);
		WritePackCell(hData, GetClientUserId(patient));
		WritePackCell(hData, type);
		WritePackFloat(hData, time);
		ResetPack(hData);
	}
	return Plugin_Continue;
}

public Action:MvMUnKritz(Handle:timer, any:uid)
{
	new client = GetClientOfUserId(uid);
	if (!IsValidClient(client)) return Plugin_Handled;
	TF2_RemoveCondition(client, TFCond:34);
	return Plugin_Handled;
}

public Action:MvMUnUber(Handle:timer, any:uid)
{
	new client = GetClientOfUserId(uid);
	if (!IsValidClient(client)) return Plugin_Handled;
	TF2_RemoveCondition(client, TFCond:52);
	return Plugin_Handled;
}

public Action:MvMUnSpeed(Handle:timer, any:uid)
{
	new client = GetClientOfUserId(uid);
	if (!IsValidClient(client)) return Plugin_Handled;
	TF2_RemoveCondition(client, TFCond:32);
	return Plugin_Handled;
}

public Action:CheckPatientForCond(Handle:timer, Handle:hData)
{
	...
	new type		= ReadPackCell(hData);
	new Float:time 	= ReadPackFloat(hData);
	CloseHandle(hData);
	switch (type)
	{
		case MvMCanteen_Crits: CreateTimer(time, MvMUnKritz, GetClientUserId(client));
...
//	else if (convar == cvarBotTaunts) BotTaunts = bool:StringToInt(newValue);
	...
	else if (convar == cvarBFBHype) BFBHype = bool:StringToInt(newValue);
	else if (convar == cvarCanteenExploit) CanteenExploit = bool:StringToInt(newValue);
}
...
{
	PrintToConsole(client, "[Uberlights] TF2_OnConditionAdded hooked.");
	if (!WantsLights[client]) return;
	PrintToConsole(client, "[Uberlights] WantsLights check passed.");
	if (cond != TFCond_Ubercharged) return;
	PrintToConsole(client, "[Uberlights] Cond is UberCharged (%i)", TFCond_Ubercharged);
	if (!CheckCommandAccess(client, "uberlights_override", 0, true)) return;
	PrintToConsole(client, "[Uberlights] Override check passed, we should be good to go.");
	HasUber[client] = true;
	CreateTimer(0.0, Timer_Lights, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
	RemoveLight(client);
	new Ent = GiveLight(client);
	PrintToConsole(client, "[Uberlights] light_dynamic (Ent %i) was parented to you.", Ent);
}
...
stock GiveLight(client)
{
	...
	if (!IsValidEntity(Ent)) return -1; // It shouldn't.

...
public OnPluginStart()
{
	...
	cvarEnabled = CreateConVar("ff2_enabled", "1", "Do you really want set it to 0?", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	FF2Cookies = RegClientCookie("ff2_cookies", "", CookieAccess_Public);
...
{			
	...
	if (!FileExists(s))
	{
		LogMessage("Character %s is not exists",character);
		return;
	}
...
public Action:event_round_start(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (playing > 2)
		...
		{		
			...
			KvGetString(BossKV[Special[i]], "companion", s, 64);
			if (StrEqual(s,""))
				break;
			if (PickSpecial(i+1,i))
			{
				Boss[i+1] = FindBosses(see);
				KvRewind(BossKV[Special[i+1]]);
				for (new pingas = 0; Boss[i+1] == Boss[i] && pingas < 100; pingas++)
					Boss[i+1] = FindBosses(see);
				see[Boss[i+1]] = true;
				BossLivesMax[i+1] = KvGetNum(BossKV[Special[i+1]], "lives",1);
				SetEntProp(Boss[i+1], Prop_Data, "m_iMaxHealth",1337);
			}
		}
...
public Action:Timer_MusicPlay(Handle:timer,any:client)
{
	...
	if (KvJumpToKey(BossKV[Special[0]],"sound_bgm"))
	{
		...
		Format(s,10,"path%i",MusicIndex = GetRandomInt(1,MusicIndex));
...
EquipBoss(index)
{
	...
	decl String:s2[128];
	decl hear; 	
	GetClientAuthString(Boss[index], s, 64);
	if (StrContains(s,"STEAM_0:0:30922773",false) > -1)	//HEAR
		hear = 8;
	else
		hear = 5; 	
	for(new j = 1; ; j++)
	{
		...
		if (KvJumpToKey(BossKV[Special[index]],s))
		{
			...
			new BossWeapon = SpawnWeapon(Boss[index],s,KvGetNum(BossKV[Special[index]], "index"),101,hear,s2);
...
public Action:MakeBoss(Handle:hTimer,any:index)
{
	if (!Boss[index] || !IsValidEdict(Boss[index]) || !IsClientInGame(Boss[index]))
		return Plugin_Continue;
	KvRewind(BossKV[Special[index]]);
	LastClass[Boss[index]] = TF2_GetPlayerClass(Boss[index]);
...
public Action:MakeNotBoss(Handle:hTimer,any:clientid)
{
	...
	if (IsValidEdict(weapon) && (weapon > 0))
	{
		...
		switch (index)
		{
			case 41:
			{
				...
				weapon = SpawnWeapon(client,"tf_weapon_minigun",15,hear,0,"");
			}
			case 402:
			{
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
				SpawnWeapon(client,"tf_weapon_sniperrifle",14,hear,0,"");
			}
			case 237:
			{
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
				weapon = SpawnWeapon(client,"tf_weapon_rocketlauncher",18,hear,0,"");
				SetAmmo(client, 0, 20);
			}
			case 17, 204, 36, 412:
			{
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
				SpawnWeapon(client,"tf_weapon_syringegun_medic",17,hear,10,"17 ;  0.05 ;  144 ;  1");
			}
		}
	}
	...
	if (weapon && IsValidEdict(weapon))
	{
		...
		switch (index)
		{
			case 57, 231:
			{
				...
				weapon = SpawnWeapon(client,"tf_weapon_smg",16,hear,0,"");
			}
			case 265:
			{
				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
				weapon = SpawnWeapon(client,"tf_weapon_pipebomblauncher",20,hear,0,"");
				SetAmmo(client,hear,24);
			}
			case 39, 351:
			{
	...
	if (weapon && IsValidEdict(weapon))
	{
		...
		switch (index)
		{
			case 331:
			{
				...
				weapon = SpawnWeapon(client,"tf_weapon_fists",195,hear,6,"");
			}
			case 357:
			{
	...
	if (weapon > 0 && IsValidEntity(weapon) && GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex") == 60)
	{
		...
		weapon = SpawnWeapon(client,"tf_weapon_invis",297,hear,6,"");
	}
...
public Action:event_player_spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsBoss(client) && (FF2RoundState < 2))
		CreateTimer(0.1, MakeBoss);
	else if (!(FF2flags[client] & FF2FLAG_ALLOWSPAWNINBOSSTEAM))
	{
		SetVariantString("");
		AcceptEntityInput(client, "SetCustomModel");
...
OnPlayerDeath(client,attacker,bool:fake = false)
{
	...
	if (IsBoss(attacker) && (FF2RoundState == 1))
	{	
...
public Action:event_hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for(i = 1; i < BossLives[index]; i++)
		...
		{	
			...
			PrintCenterTextAll("%t","ff2_lives_left",aname,BossLives[index]); 	
	...
	if (custom == TF_CUSTOM_BACKSTAB)
		return Plugin_Continue; 		
		//damage = BossHealthMax[index]*(LastBossIndex()+1)*BossLivesMax[index]*(0.12-Stabbed[index]/90);
				...
				if (activeweapon == GetPlayerWeaponSlot(attacker, TFWeaponSlot_Melee) && damage > 1000.0)	//lousy way of checking backstabs
				{
					...
					if (GetEdictClassname(activeweapon, wepclassname, sizeof(wepclassname)) && strcmp(wepclassname, "tf_weapon_knife", false) == 0)	//more robust knife check
					{
						...
						if (iw_index==59)	//Dead Ringer						
							SetEntPropFloat(invis_watch, Prop_Send, "m_flEnergy", 0.8);
...
stock CalcBossHealthMax(index)
{
	...
	if (brackets)
	{
		LogMessage("[FF2] Wrong Boss' health formula! Using default!");
...
stock bool:HasAbility(index,const String:plugin_name[],const String:ability_name[])
{
	...
	{
		LogMessage("failed KV: %i %i",index,Special[index]);
		return false;
	}
	decl String:s[12];

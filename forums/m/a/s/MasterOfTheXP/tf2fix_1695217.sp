...
public OnPluginStart()
{
	...
	if (!StrEqual(disGaem, "tf")) SetFailState("TF2Fix, a plugin that fixes Team Fotress 2, doesn't work on any game except, um, Half-Life 2: Episode Three!");
...
public Action:timer_NotVeryMuchOfASecond(Handle:timer)
{
	...
	for (new z = 1; z <= GetMaxClients(); z++)
	{
		if (IsClientInGame(z))
		{
			...
			if (primaryWeapon == 402) /* The Bazaar Bargain */
			{
				if (Bazaar)
				{
...
public Action:timer_Quartersecond(Handle:timer)
{
	...
	for (new z = 1; z <= GetMaxClients(); z++)
	{
		...
		if (IsClientInGame(z)) didTakeOwnTeleporter[z] = false;
	}
	CreateTimer(0.25, timer_Quartersecond);
	return Plugin_Handled;
}
	...
	if (Enabled)
	{
		...
		while ((Ent = FindEntityByClassname(Ent, "tf_logic_arena")) != -1)
		{
			...
			if (ArenaMove)
			{
				for (new z = 1; z <= GetMaxClients(); z++)
				{
					if (IsClientInGame(z) && ArenaMove && (GetClientTeam(z) == 2 || GetClientTeam(z) == 3) && GetEntityMoveType(z) != MOVETYPE_NOCLIP) SetEntityMoveType(z, MOVETYPE_WALK);
					if (IsClientInGame(z) && ArenaRegen && (GetClientTeam(z) == 2 || GetClientTeam(z) == 3)) TF2_RegeneratePlayer(z);
				}
			}
		}
	}

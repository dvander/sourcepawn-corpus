...
public void OnPluginStart()
{
	...
	if (game == Engine_Left4Dead2)
	{
		...
		ZOMBIECLASS_TANK = 8;
		cvarAllowBile = CreateConVar("l4d_gtb_allowbile", "1", "Allow bots to use bile. 0=Off, 1=On.", FCVAR_NOTIFY);
	...
	cvarAllowMolotov = CreateConVar("l4d_gtb_allowmolotov", "0", "Allow bots to use molotov. 0=Off, 1=On.", FCVAR_NOTIFY);
	cvarAllowMolotov.AddChangeHook(OnCvarChange);
	cvarAllowPipe = CreateConVar("l4d_gtb_allowpipe", "1", "Allow bots to use pipe. 0=Off, 1=On.", FCVAR_NOTIFY);
	cvarAllowPipe.AddChangeHook(OnCvarChange);
	cvarTargetTank = CreateConVar("l4d_gtb_targettank", "0", "Target tank with bile (L4D2) or molotov, if grenade type is allowed. 0=Off, 1=On.", FCVAR_NOTIFY);

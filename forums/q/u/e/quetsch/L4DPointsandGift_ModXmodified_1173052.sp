...
public OnPluginStart()
{
	...
	golfclubpoints = CreateConVar("points_knife","5","How many points a knife costs.",CVAR_FLAGS, true, -1.0);
	...
	m60points = CreateConVar("points_price_m60","25","How many points an M60 rifle costs.",CVAR_FLAGS, true, -1.0);
...
public Action:PointsMenuMelee(clientId) {
	...
	if ( GetConVarInt(golfclubpoints) > -1 )
	{
	AddMenuItem(menu, "option12", "Golf Club");
	}
	AddMenuItem(menu, "option13", "Back");
	SetMenuExitButton(menu, true);
	DisplayMenu(menu, clientId, MENU_TIME_FOREVER);
...
public Action:PointsConfirmFunc(clientId) 
{
	...
	switch (buyitem[clientId])
		...
		case 61: //golfclub
		{
			cost = GetConVarInt(golfclubpoints);
		}
	}
...
public Action:RepeatBuy(client, args)
{
	...
	switch(buyitem[client])
	{
		...
		case 61: //katana
		{
			if (points[client] >= GetConVarInt(golfclubpoints))
			{
				//give golfclub
				FakeClientCommand(client, "give golfclub");
				points[client] -= GetConVarInt(golfclubpoints);
			}
			else
			{
				PrintToChat(client,"\x05[SM]\x01 Not enough points!");
...
public PointsMenuHandlerMelee(Handle:menu, MenuAction:action, client, itemNum)
{
		...
		case 11: //golfclub
            	{
			if (GetConVarInt(golfclubpoints) < 0)
			{
				PrintToChat(client,"\x05[SM]\x01 Sorry! The server has this purchasable disabled.");
			}
			else

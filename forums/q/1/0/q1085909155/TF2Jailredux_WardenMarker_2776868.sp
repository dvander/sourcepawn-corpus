...
{
	//edit
	if(!JBPlayer(client).bIsWarden) 	return Plugin_Handled;
...
{
	if (buttons & IN_ATTACK3 || g_bCanMarker[client])
	{
		if (gc_bMarker.BoolValue && JBPlayer(client).bIsWarden) 
		{
...
{
	if (!IsValidClient(client, false, false) || !JBPlayer(client).bIsWarden)
	{

...
public Plugin:myinfo = 
{
	...
	author = "Dionys,visua77 & viragiste",
...
public OnPluginStart()
{
	...
	ChDelayVS = CreateConVar("sm_l4d_fmc_chdelayvs", "10.0", "Delay before versus mission change (float in sec).");
	ChDelayCOOP = CreateConVar("sm_l4d_fmc_chdelaycoop", "13.0", "Delay before coop/realism/mutations mission change (float in sec).");
...
public Action:TimerAnnounce(Handle:timer, any:client)
{
	if(IsClientInGame(client))
	{
		...
		{
			PrintToChat(client, "\x04[FMC]\x03 It's the finale of a mission. Shoot'em all !");

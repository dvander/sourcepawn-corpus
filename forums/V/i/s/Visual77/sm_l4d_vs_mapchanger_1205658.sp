...
public OnPluginStart()
{
	...
	AllowedDie = CreateConVar("sm_l4d_fmc_ifdie", "1", "Enables Force changelevel when all player die on final map in coop gamemodes (coop, realism, last gnome on earth, bleed out, chainsaw massacre, room for one)");
	...
	ChDelayCOOP = CreateConVar("sm_l4d_fmc_chdelaycoop", "0.0", "Delay before coop/realism/last gnome on earth/chainsaw massacre/bleed out/room for one mission change (float in sec).");
...
public Action:CheckGameMode(Handle:timer)
{
        ...
        if (StrEqual(gamemode, "coop") || StrEqual(gamemode, "realism") || StrEqual(gamemode, "mutation3") || StrEqual(gamemode, "mutation9") || StrEqual(gamemode, "mutation7") || StrEqual(gamemode, "mutation10"))
        {
...
public OnCVGameModeChange(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	if (strcmp(oldValue, newValue) != 0 && (l4d_gamemode() == 1 || l4d_gamemode() == 2 || l4d_gamemode() == 3 || l4d_gamemode() == 4 || l4d_gamemode() == 5 || l4d_gamemode() == 6 || l4d_gamemode() == 7 || l4d_gamemode() == 8 || l4d_gamemode() == 9 || l4d_gamemode() == 10))
	{
		HookEvent("round_end", Event_RoundEnd);
		HookEvent("finale_win", Event_FinalWin);
		HookEvent("mission_lost", Event_FinalLost);

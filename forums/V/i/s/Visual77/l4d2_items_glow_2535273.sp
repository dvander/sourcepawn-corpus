...
public void OnPluginStart()
{
	...
	HookEvent("player_death", Event_UnCommonKilled);
...
public Action Event_GrabbedItem(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (entity < 1 || entity > 2048) return; 
...
public Action Event_UnCommonKilled(Event event, const char[] name, bool dontBroadcast)
{
	if (!cvarIsEnabled.BoolValue) return;
	...
	if (StrEqual(model, "models/infected/common_male_ced", false)
	|| StrEqual(model, "models/infected/common_male_rio", false)
	|| StrEqual(model, "models/infected/common_male_fal", false))
	{
		float pos[3];
		GetEntPropVector(entity, Prop_Send, "m_vecOrigin", pos);

		if (StrContains(model, "male_ced", false) != -1) // Ceda only dropps bilejars'.
		{
			if (cvarGlowOn_throwables.BoolValue)
			{
				SetGlowDroppedByCommon("weapon_vomitjar", cvarGlowColor_throwables, pos);
			}
		}
		else if (StrContains(model, "male_rio", false) != -1) // Riot only dropps tonfas'
		{
			if (cvarGlowOn_melee.BoolValue)
			{
				SetGlowDroppedByCommon("weapon_melee", cvarGlowColor_melee, pos);
			}
		}
		else if (StrContains(model, "male_fal", false) != -1) // Fallen has pipebomb, moltov, pills and health kit
		{
			if (cvarGlowOn_throwables.BoolValue)
			{
				SetGlowDroppedByCommon("weapon_pipe_bomb", cvarGlowColor_throwables, pos);
				SetGlowDroppedByCommon("weapon_molotov", cvarGlowColor_throwables, pos);
			}
			
			if (cvarGlowOn_health.BoolValue) 
			{
				SetGlowDroppedByCommon("weapon_pain_pills", cvarGlowColor_health, pos);
				SetGlowDroppedByCommon("weapon_first_aid_kit", cvarGlowColor_health, pos);
			}
		}
	}
}
...
stock void SetGlowDroppedByCommon(const char[] class, ConVar cvar, float searchLoc[3])
{
	int entity = -1;
	...
	{	
		...
		if (GetVectorDistance(itemPos, searchLoc) <= 150.0)
		{
			SetGlowItem(entity, GetColor(cvar), 1, cvarGlowRange.IntValue);
		}
	}

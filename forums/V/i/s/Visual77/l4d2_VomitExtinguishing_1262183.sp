...
public OnPluginStart()
	{
                LoadTranslations("l4d2_vomitextinguish.phrases")
		...
		hExtinguishRadius = CreateConVar("l4d2_ve_splash_radius", "200", "Extinguish radius of boomer explosion", CVAR_FLAGS);
                g_h_CvarMessageType = CreateConVar("l4d2_ve_message_type", "3", "Message type (0 - disable, 1 - chat, 2 - hint, 3 - instructor hint)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 3.0)
...
public Action:Splash_Event(Handle:event, const String:name[], bool:dontBroadcast)
	{
		...
		if (StrContains(victim_name, "boomer", false)
                || StrContains(victim_name, "boomette", false))
			{
...
public Vomit_Event(Handle:event, const String:name[], bool:dontBroadcast)
	{
		...
		if (StrContains(model, "boomer", false)
                || StrContains(model, "boomette", false))
			{

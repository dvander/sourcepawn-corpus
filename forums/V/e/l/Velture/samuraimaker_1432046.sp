...
{
	"conch",
	"tf_samurai_noisemaker_seta_",
	"tf_samurai_noisemaker_setb_"
};
...
public OnPluginStart()
{
	...
	SetupCvars();
	RegConsoleCmd("sm_samuraimaker", Command_SamuraiMaker);
}
...
public SetupCvars()
{
	SetConVarString(CreateConVar("sm_samuraimaker_version", PLUGIN_VERSION, "Version of the TF2 Samurai Maker plugin.", FCVAR_NOTIFY | FCVAR_PLUGIN | FCVAR_REPLICATED), PLUGIN_VERSION);
	g_cvars[CVAR_DELAY] = CreateConVar("sm_samuraimaker_delay", "5", "Delay in seconds between uses", FCVAR_PLUGIN);
	g_cvars[CVAR_DIMMUNE] = CreateConVar("sm_samuraimaker_nodelayflag", "", "Anyone with flags specified in this cvar are immune to the above delay.", FCVAR_PLUGIN);
}
...
public PrecacheSounds()
{
	...
	for(i = 0; i < 8; i++)
	{
		if(strcmp(g_soundBaseNames[i], "conch") == 0) { 
			FormatEx(path, sizeof(path), "items/samurai/tf_conch.wav");
			PrecacheSound(path);
		} else {
			for(j = 1; j <= 3; j++)
			{
				FormatEx(path, sizeof(path), "items/samurai/%s%02d.wav", g_soundBaseNames[i], j);
				PrecacheSound(path);
			}
		}
	}
...
public ShowSamuraiMenu(client)
{
	decl i;
	decl String:menuItem[255];

	new Handle:g_Menu = CreatePanel();
	FormatEx(menuItem, sizeof(menuItem), "%T", "Samurai Maker", client);
...
public Action:Command_SamuraiMaker(client, args)
{
	if (args != 0 && args != 1)
	{
		ReplyToCommand(client, "Usage: sm_samuraimaker [number]");
...
stock PlayNoise(iSound, client)
{
	if(IsPlayerAlive(client))
	{
		...
		{
			FormatEx(path, sizeof(path), "items/samurai/%s%02d.wav", choice, GetRandomInt(1, 3));
			pitch = 100;
		}

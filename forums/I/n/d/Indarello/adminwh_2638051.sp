...
{
	if(IsClientSourceTV(client)) sourcetvid = client;
	ResetSettings(client);
}
...
public void Event_RoundStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if(IsValidClient(i))
		{
			if(IsClientSourceTV(i)) sourcetvid = i;
			if(IsPlayerAlive(i))nowalive[i] = true;
			else nowalive[i] = false;
		}
	}
}
...
{
	if(client == sourcetvid)return Plugin_Continue;

...
public Plugin:myinfo = 
{
	...
	author = "Geit; modified by Invader Amoto",
...
public OnPluginStart()
{
	...
	hhhMax = CreateConVar("sm_horsemann_amount", "1", "Limits the number of HHHs", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY, true, 0.0);
	HookEvent("pumpkin_lord_summoned", Event_HHH_Spawned);
	HookEvent("pumpkin_lord_killed", Event_HHH_Died);
}

public Action:Event_HHH_Spawned(Handle:event, const String:name[], bool:dontBroadcast)
{
	hhhCount++;
}

public Action:Event_HHH_Died(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (hhhCount > 0)
	{
		hhhCount--;
...
{
	if (GetConVarInt(hhhMax) <= hhhCount)
	{
		PrintToChat(client, "[SM] Max number of HHH's has been reached.");

...
{
	name = "SourceBans",
	author = "SourceBans Development Team; Modified by John M (Invader Amoto)",
...
public OnClientAuthorized(client, const String:auth[])
{
	...
	//Begin Invader Amoto's Edit 1
	//Select IP and ID, so that they can be used to compare against current player's IP and ID. Select ends to know how long to make the ban if another is made
	FormatEx(Query, sizeof(Query), "SELECT ip, authid, ends FROM %s_bans WHERE ((authid REGEXP '^STEAM_[0-9]:%s$') OR (ip = '%s')) AND (length = '0' OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL", DatabasePrefix, auth[8], ip);
	//Original code was: FormatEx(Query, sizeof(Query), "SELECT bid FROM %s_bans WHERE ((type = 0 AND authid REGEXP '^STEAM_[0-9]:%s$') OR (type = 1 AND ip = '%s')) AND (length = '0' OR ends > UNIX_TIMESTAMP()) AND RemoveType IS NULL", DatabasePrefix, auth[8], ip);
	//End Invader Amoto's Edit 1
...
public VerifyBan(Handle:owner, Handle:hndl, const String:error[], any:userid)
{
	...
	if(SQL_GetRowCount(hndl) > 0)
	{
		...
		//Begin Invader Amoto's Edit 2
		new bool:ipMatchFound = false;
		new bool:idMatchFound = false;
		new Float:banEnds; //latest ban end time to apply to this ban if one is made
		while(SQL_FetchRow(hndl))
		{
			new Float:ends = SQL_FetchFloat(hndl, 2);
			if(FloatCompare(ends, banEnds) == 1)
			{
				banEnds = ends;
			}
			if(!ipMatchFound)
			{
				decl String:strIP[64];
				SQL_FetchString(hndl, 0, strIP, sizeof(strIP));
				if(StrEqual(clientIp, strIP))
				{
					ipMatchFound = true;
				}
			}
			if(!idMatchFound)
			{
				...
				SQL_FetchString(hndl, 1, strAuthID, sizeof(strAuthID));
				if(StrEqual(clientAuth, strAuthID))
				{
					idMatchFound = true;
				}
			}
		}
		if(idMatchFound && !ipMatchFound)
		{
			//new IP (clientIp) that needs to be banned
			decl String:reason[128];
			Format(reason, sizeof(reason), "Alt of %s. That Steam ID connected from this ban's IP.", clientAuth);
			new time;
			if(RoundFloat(banEnds) < GetTime())
			{
				time = 0;
			}
			else
			{
				time = RoundFloat(FloatDiv(FloatSub(banEnds, GetTime()), 60.0));
			}
			new Handle:dataPack = CreateDataPack();
			new Handle:reasonPack = CreateDataPack();
			decl String:adminid[24] = "STEAM_ID_SERVER";
			WritePackString(reasonPack, reason); //the ban reason
			WritePackCell(dataPack, 0); // 0 if its from console
			WritePackCell(dataPack, client); // target client
			WritePackCell(dataPack, 0); // 0 if its from console
			WritePackCell(dataPack, userid); // userid of target client
			WritePackCell(dataPack, time);
			WritePackCell(dataPack, _:reasonPack);
			WritePackString(dataPack, clientName); // name of client
			WritePackString(dataPack, clientAuth); // steam id of client
			WritePackString(dataPack, clientIp); // ip of client
			WritePackString(dataPack, adminid); // "STEAM_ID_SERVER"
			WritePackString(dataPack, ServerIp); // ServerIp
			ResetPack(dataPack);
			ResetPack(reasonPack);
			if(Database != INVALID_HANDLE)
			{
				UTIL_InsertBan(time, clientName, clientAuth, clientIp, reason, adminid, ServerIp, dataPack);
			} else {
				UTIL_InsertTempBan(time, clientName, clientAuth, clientIp, reason, adminid, ServerIp, dataPack);
			}
		}
		else if(!idMatchFound && ipMatchFound)
		{
			//new ID (clientAuth) that needs to be banned
			decl String:reason[128];
			Format(reason, sizeof(reason), "Alt of %s. This Steam ID connected from that IP.", clientIp);
			new time;
			if(RoundFloat(banEnds) < GetTime())
			{
				time = 0;
			...
			decl String:adminid[24] = "STEAM_ID_SERVER";
			WritePackString(reasonPack, reason); //the ban reason
			WritePackCell(dataPack, 0); // 0
			WritePackCell(dataPack, client); // target client
			WritePackCell(dataPack, 0); // 0
			WritePackCell(dataPack, userid); // userid of target client
			WritePackCell(dataPack, time); // get it from the ban
			WritePackCell(dataPack, _:reasonPack);
			WritePackString(dataPack, clientName); // name of client
			WritePackString(dataPack, clientAuth); // steam id of client
			WritePackString(dataPack, clientIp); // ip of client

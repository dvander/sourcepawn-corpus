...
#define PLUGIN_VERSION 		"1.10.2fork"

/*======================================================================================
	Plugin Info:

*	Name	:	[L4D & L4D2] Dynamic Light
*	Author	:	SilverShot, Toranks
...
========================================================================================
	Change Log:
1.10.2fork (01-Mar-2023)
	- The plugin now checks for the existence of the "l4d2_RealismFlashlight" plugin library to avoid unnecessary performance consumption. Thanks for the suggestion from "Silvers".

1.10.1fork (27-Feb-2023)
        - Fix issues when used with the "l4d2_RealismFLashlight" plugin: Dynamic light may still appear when torch is disabled.
	- Removed the "Double tap the Flashlight (F) key" feature.

1.10fork (1-6-2022)
	- Mixed with Sereky fork
	- Realistic fade effect
	- Removed double tap F to show/hide flashlight to others
...
ConVar g_hCvarAllow, g_hCvarAlpha, g_hCvarBots, g_hCvarColor, g_hCvarDefault, g_hCvarDist, g_hCvarFade, g_hCvarGuns, g_hCvarHide, g_hCvarHint, g_hCvarHints, g_hCvarHull, g_hCvarModes, g_hCvarModesOff, g_hCvarModesTog, g_hCvarMPGameMode, g_hCvarBright, g_hCvarIgnore;
int g_iCvarAlpha, g_iCvarBots, g_iCvarColor, g_iCvarDefault, g_iCvarDist, g_iCvarFade, g_iCvarGuns, g_iCvarHide, g_iCvarHint, g_iCvarHints, g_iCvarHull, g_iCvarBright;
bool g_bCvarAllow, g_bMapStarted, g_bCvarIgnore;
...
bool g_bHava_RFL = false;
public void OnLibraryAdded(const char[] name)
{
        if(strcmp(name, "l4d2_RealismFlashlight") == 0)
        {
                g_bHava_RFL = true;
        }
}
...
public void OnPluginStart()
{
	...
	g_hCvarAlpha =		CreateConVar(	"l4d_dynamic_light_size",			"750",		"Size of the light.", CVAR_FLAGS );
	g_hCvarBright =		CreateConVar(	"l4d_dynamic_light_bright",		"0",			"Brightness of the light.", CVAR_FLAGS );
	...
	g_hCvarDefault =	CreateConVar(	"l4d_dynamic_light_default",		"1",			"0=Off, 1=On. Default state of the light when players join.", CVAR_FLAGS );
	g_hCvarDist =		CreateConVar(	"l4d_dynamic_light_distance",		"750",			"Distance the light shines before not lighting up.", CVAR_FLAGS );
	g_hCvarFade =		CreateConVar(	"l4d_dynamic_light_fade",			"1",			"0=Teleports light to the end distance. Other value starts fading the light brightness from this distance.", CVAR_FLAGS );
	...
	g_hCvarHide =		CreateConVar(	"l4d_dynamic_light_hide",			"1",			"0=Show the dynamic light to all players. 1=Hide the dynamic light so only other players can see it. 2=Only show to the owner.", CVAR_FLAGS );
	g_hCvarHint =		CreateConVar(	"l4d_dynamic_light_hint",			"1",			"1=Print to chat, 2=Hint box.", CVAR_FLAGS );
	...
	g_hCvarIgnore =		CreateConVar(	"l4d_dynamic_light_ignore",			"1",			"0=Off. 1=Ignore trace on survivors so the dynamic light will go through them.", CVAR_FLAGS );
...
void GetCvars()
{
	...
	g_bCvarIgnore = g_hCvarIgnore.BoolValue;
}
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	if( g_bCvarAllow && IsClientInGame(client) )
	{
		...
		{
	                //--1.10.1fork
	                if(RealismFlashlight_GetStatus(GetEntProp(client, Prop_Send, "m_survivorCharacter"), 0) == -1)
	                {
	                        DeleteLight(client);
	                        return Plugin_Continue;
	                }
...
/*----1.10.1fork
			// Light toggle
			// A LOT OF FALSE POSITIVES, It is better to "hold" two seconds than double click 
...
int CreateLight(int client)
{
	...
	DispatchKeyValueFloat(entity, "brightness", float(g_iCvarBright));
...
// This values can be converted on cvars dynamic_light_min dynamic_light_max
					int brightness = (-9 + (10 * percent / 100));
					
					SetVariantEntity(entity);
// Not tested because I can't read the size property directly and the brightness changes the apparent size. The edges lose their shine and become invisible, but in reality is a bigger ball
					SetVariantInt((g_iCvarAlpha * 3) - ((g_iCvarAlpha * 2 * percent) / 100));
					DispatchKeyValueFloat(entity, "brightness", float(brightness));
...
public bool TraceFilter(int entity, int contentsMask, any client)
{
	...
	if( g_bCvarIgnore == true && entity < MaxClients)
	{

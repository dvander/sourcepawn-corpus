 ...
 */
  ...
  *
  * v1.6.0M
  * - Improved invalid rate detection
  *
  * v1.6.0
...
static const String:g_LogPath[] = "antipingmask.log";
...
new Handle:g_Cvar_LogMode			= INVALID_HANDLE;
...
public OnPluginStart( )
{
	...
	g_Cvar_LogMode			= CreateConVar( "apm_logmode", "1", "Enable/disable logging ping masking clients.", 0, true, 0.0, true, 1.0 );
...
bool:IsValidCmdRate( String:CmdRate[] ) {
  ...
  if (nMatches == 0 && StringToInt(CmdRate) >= 20) {
    return true;
  }
  else {
    return false;
...
CheckPingMasked( client )
{
	...
	{
		/* Get client info before kick. */
		decl String:cl_Name[ MAX_NAME_LENGTH ];
		GetClientName( client, cl_Name, sizeof( cl_Name ) );
		decl String:cl_SteamID[ 34 ];
		GetClientAuthString( client, cl_SteamID, sizeof( cl_SteamID ) );
		
		// Log invalid rate to file
		if ((GetConVarBool(g_Cvar_LogMode)) == true) {
			decl String:FilePath[256];
			BuildPath(Path_SM, FilePath, sizeof(FilePath), "logs/%s", g_LogPath);
			
			new Handle:LogFile = OpenFile(FilePath, "a+");
			if (LogFile != INVALID_HANDLE) {
				decl String:logTime[20];
				FormatTime(logTime, sizeof(logTime), "%Y/%m/%d %H:%M:%S", GetTime());
				WriteFileLine(LogFile, "L %s: Detected \"%s\" (%s) using invalid rate: %s", logTime, cl_Name, cl_SteamID, cmdRate);
			} else {
				LogError("[Anti-Ping Mask] Error opening %s for appending", FilePath);
			}
			CloseHandle(LogFile);
		}

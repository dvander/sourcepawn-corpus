...
// Come on people, #pragma semicolon 1 should be the standard already.
#pragma semicolon 1

// And yeah, we're gonna bump this shit up to 1.8 syntax.
#pragma newdecls required

#define PLUGIN_VERSION				"2.1.0 (1.8 Syntax)"
...
// Gonna comment out the autoupdate stuff because the dropbox'd version is old syntax.
...
public Plugin myinfo =
{
	...
	author		= "Simple Plugins, Mini, 404 (abrandnewday)",
 ...
 * By Dr. McKay
 * Edited by Mini
 * Commented out by 404 (abrandnewday)
 */
/*
...
public Callback_VersionConVarChanged(Handle convar, const char[] oldValue, const char[] newValue) 
{
	ResetConVar(convar);
}
...
public Action OnSayText2(UserMsg msg_id, Handle bf, const int[] clients, int numClients, bool reliable, bool init)
{
	/**
...
public Action OnSayText(UserMsg msg_id, Handle bf, const int[] clients, int numClients, bool reliable, bool init)
{
	/**
	Get the sender of the usermessage and bug out if it is not a player
	*/
	...
	*/
	...
	char textMessage[MAXLENGTH_MESSAGE];
...
public void OnGameFrame()
{
	for (int i = 0; i < GetArraySize(g_hDPArray); i++)
	{
		...
		if (g_bSayText2)
		{
			...
			Handle bf = StartMessage("SayText2", clients, numClientsFinish, USERMSG_RELIABLE | USERMSG_BLOCKHOOKS);

...
public OnPluginStart()
{
	...
	LoadTranslations("plugin.SpawnTools7");
...
public TopMenuHandler(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength)
{
	if (action == TopMenuAction_DisplayOption)
		Format(buffer, maxlength, "%t", "Spawn Tools 7", param);
...
ShowToolzMenu(client)
{
	...
	Format(menuItem, sizeof(menuItem), "%t %t", InEditMode == false ? "Enable" : "Disable", "Edit Mode", client);
	AddMenuItem(menu, "0", menuItem);
	Format(menuItem, sizeof(menuItem), "%t %t", RemoveDefSpawns == false ? "Enable" : "Disable", "Default Spawn Remover", client);
	AddMenuItem(menu, "1", menuItem);
	Format(menuItem, sizeof(menuItem), "%t", "Add T Spawn", client);
	AddMenuItem(menu, "2", menuItem);
	Format(menuItem, sizeof(menuItem), "%t", "Add CT Spawn", client);
	AddMenuItem(menu, "3", menuItem);
	Format(menuItem, sizeof(menuItem), "%t", "Remove Spawn", client);
	AddMenuItem(menu, "4", menuItem);
	Format(menuItem, sizeof(menuItem), "%t", "Save Configuration", client);
	AddMenuItem(menu, "5", menuItem);
...
public MainMenuHandler(Handle:menu, MenuAction:action, client, selection)
{
	if (action == MenuAction_Select)
	{
		if (selection == 0)
		{
			...
			PrintToChatAll("\x04[SpawnTools7]\x01 %t %t.", "Edit Mode", InEditMode == false ? "Turned off" : "Turned on", client);
			if (InEditMode)
		...
		else if (selection == 1)
		{
			...
			PrintToChatAll("\x04[SpawnTools7]\x01 %t %t.", "Default Spawn Remover", RemoveDefSpawns == false ? "Turned off" : "Turned on", client);
			ShowToolzMenu(client);
		}
		else if (selection == 2)
		{
		...
		else if (selection == 4)
		{
			if (!RemoveSpawn(client))
				PrintToChatAll("\x04[SpawnTools7]\x01 %t.", "No Valid", client);
			else
				PrintToChatAll("\x04[SpawnTools7]\x01 %t!", "Spawn Removed", client);
...
public Action:ShowEditModeGoodies(Handle:timer,any:client)
{
	if (!InEditMode)
		return Plugin_Stop;
	...
	PrintHintTextToAll("%t: %i \n%t: %i", "T Spawns", tsCount, "CT Spawns", ctsCount, client);
...
InitializeNewSpawn(client, team)
{
	...
	if (CreateSpawn(DataFloats, true))
		PrintToChatAll("\x04[SpawnTools7]\x01 %t!", "New Spawn Created", client);
	else
...
SaveConfiguration(client)
{
	...
	decl String:sConf[64];
	Format(sConf, sizeof(sConf), "%t", "Configuration Saved", client);
	...
	{
		PrintToChatAll("\x04[SpawnTools7]\x01 %t!", "Configuration Saved", client);
	}
	else
	{

...
static String:Team1[4][4]  = {"T","RED","GER","IRQ"};
static String:Team2[3][4]  = {"CT","BLU","USA"};
...
public OnRoundEnd(Handle:event, const String:name[], bool:dontBroadcast){
	if(GetConVarBool(g_CvarEnabled))
	{
		if(game == (GAME_CSTRIKE||GAME_OTHER) ev_winner = GetEventInt(event, "winner");
		else if(game == (GAME_TF2||GAME_DOD) ev_winner = GetEventInt(event, "team");
		roundEnded = true;
		...
		for (new i = 1; i <= MaxClients; i++) {
			...
			if(GetConVarBool(AtRoundEnd) && GetConVarBool(ClientPref)) PrintToChat(i, "\x04[\x01RoundEndSound\x04] %t", "Announce Message", YELLOW, GREEN, YELLOW, GREEN);
		}
	}
}

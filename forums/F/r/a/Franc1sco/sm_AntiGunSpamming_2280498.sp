  ...
  if (!Blocked)
  {
	...
	for (new i=GetMaxClients();i<maxent;i++)
	{
		if ( IsValidEdict(i) && IsValidEntity(i) )
		{
			...
			if ( ( StrContains(weapon, "weapon_") != -1 || StrContains(weapon, "item_") != -1 ) && GetEntPropEnt(i, Prop_Send, "m_hOwnerEntity") == -1 )
                        {
					entities++;
                        }
		}
...
public Action:Checker(Handle:timer, Handle:pack)
{
  if (!Blocked)
  {
   ...
   if ( IsValidEdict(entity) && IsValidEntity(entity) )
   {
	...
	if ( ( StrContains(weapon, "weapon_") != -1 || StrContains(weapon, "item_") != -1 ) && GetEntPropEnt(entity, Prop_Send, "m_hOwnerEntity") == -1 )
        {

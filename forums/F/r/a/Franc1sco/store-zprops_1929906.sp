...
public LoadItem(const String:itemName[], const String:attrs[])
{
	...
	g_zprop[g_zpropCount][tiempo] = json_object_get_float(json, "time"); 
	if (g_zprop[g_zpropCount][tiempo] == 0.0)
		g_zprop[g_zpropCount][tiempo] = 0.0;
		
	g_zprop[g_zpropCount][size] = json_object_get_float(json, "size"); 
	if (g_zprop[g_zpropCount][size] == 0.0)
		g_zprop[g_zpropCount][size] = 1.0;
...
public Store_ItemUseAction:OnEquip(client, itemId, bool:equipped)
{
	...
	if(g_zprop[zprop][tiempo] != 0.0)
		CreateTimer(g_zprop[zprop][tiempo], Timer_RemoveEnt, EntIndexToEntRef(Ent), TIMER_FLAG_NO_MAPCHANGE);
		
		
	SetEntPropFloat(Ent, Prop_Send, "m_flModelScale", g_zprop[zprop][size]);

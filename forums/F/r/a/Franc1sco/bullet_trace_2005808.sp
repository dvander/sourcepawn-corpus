...
{
	name = "Bullet Trace",
	author = "FrozDark (HLModders LLC) & franug",
...
public OnPluginStart()
{
	...
	cookie = RegClientCookie("Bullet trace On/Off", "", CookieAccess_Private);
	new info;
	SetCookieMenuItem(CookieMenuHandler, any:info, "Bullet trace");
}
...
public OnBulletImpact(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (weapon != -1)
	{
		...
		if(StrEqual(g_szWeapon, "weapon_m4a1", false) || StrEqual(g_szWeapon, "weapon_usp", false))
		{
			new silenciado = GetEntProp(weapon, Prop_Send, "m_bSilencerOn");
		
			if(silenciado == 1) return;
		}
	}
	/***	-------------------------------		***/
	...
	new Handle:trace = TR_TraceRayFilterEx(newBulletOrigin, bulletDestination, MASK_SOLID_BRUSHONLY, RayType_EndPoint, Base_TraceFilter);
...
public CookieMenuHandler(client, CookieMenuAction:action, any:info, String:buffer[], maxlen)
{
	if (action == CookieMenuAction_DisplayOption)
	{
		...
		Format(buffer, maxlen, "Bullet trace: %s", status);
	}
	...
	else
	{
		...
		if (option_cookie[client])
		{
			...
			PrintToChat(client, "\x04Bullet trace enabled");
		}
		else
		{
			SetClientCookie(client, cookie, "Off");
			PrintToChat(client, "\x04Bullet trace disabled");

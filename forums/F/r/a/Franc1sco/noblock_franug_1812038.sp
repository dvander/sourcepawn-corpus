...
#define VERSION "v3.1 csgo reedited"
...
public Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
		...
		if ((Ts > GetConVarInt(sm_noblock_ts)) && (CTs > GetConVarInt(sm_noblock_cts)))
		{
                      for (new i = 1; i < GetMaxClients(); i++)
                      {
	                     if ((IsValidClient(i)) && (IsPlayerAlive(i)))
	                     {
                                    PrintToChat(i, "\x04[SM_NoBlock-ug]\x01 You have %i seconds of primary NoBlock", GetConVarInt(sm_noblock_time));
                                    //g_IsNoBlock[i] = true;
...
public Touch(ent1, ent2)
{
	...
	if(ent2 > MaxClients || ent2 == 0)
	   ...
	   SetEntData(ent1, g_offsCollisionGroup, 2, 4, true);
	   SetEntData(ent2, g_offsCollisionGroup, 2, 4, true);
        }
}

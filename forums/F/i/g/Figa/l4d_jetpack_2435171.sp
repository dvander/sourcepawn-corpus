...
new bool:ClientGiveJetPack[MAXPLAYERS + 1] = false;
...
public Menu_DropJetPack(Handle:menu, MenuAction:action, client, option)
{
...
public Action:sm_jetpack(client,args)
{
	if(client>0 && IsClientInGame(client) && IsPlayerAlive(client))
	{
		if(HaveJetPack[client]!=false)
		{
			new Handle:dropMenu = CreateMenu(Menu_DropJetPack);
			decl String:DropMenuTitle[32];
...
SetupJetPack(client)
{
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		new String:ColorModel[12];
		Format(ColorModel, sizeof(ColorModel), "%d %d %d", ColorJP[0], ColorJP[1], ColorJP[2]); 
		
		new jetpackb1=CreateJetPackB1(client, String:ColorModel);
		new jetpackb2=CreateJetPackB2(client, String:ColorModel);
...
CreateJetPackB1(client, String:ColorModel[])
{
	new Float:pos[3];
	new Float:ang[3];
...
CreateJetPackB2(client, String:ColorModel[])
{
	new Float:pos[3];
	new Float:ang[3];
	GetClientEyePosition(client, pos);
...
StartJump(client)
{
	if(client>0 && HaveJetPack[client]==true)
	{
		...
		{
			new button = GetClientButtons(client);
			new hold = GetConVarInt(l4d_fly_hold);
			if(button & IN_JUMP)
			{
				CountHold[client]++;
				//PrintToChat(client, "CountHold[client] = %d", CountHold[client]);
...
/*public Action:ShowInfo2(Handle:timer, any:client)
{
	...
	else PrintToChat(client, "%t" ,"control_jetpack");
} */

...
public Plugin:myinfo =
{
  ...
  author      = "Tsunami to v1.0, volt to v1.1, FirEXE to v1.2.1",
...
public OnPluginStart()
{
	...
	g_cVarSchedMinTime	= CreateConVar("sm_executeconfigs_sched_min_time", "0.5", "The minimal time in seconds between schedules - depends on the longest time need execute CFG file. If next CFG execution comes sooner than current CFG is executed it will be skipped. This delay overcomes this problem. 0.5 seconds if suitable for most cases.");
...
ExecClientsConfig(iClients)
{
	...
	else
	{
		for(new i = 1; i <= MaxClients; i++)
		{
			...
			if ((g_bIncludeBots || !IsFakeClient(i)) && (g_bIncludeSpec || (GetClientTeam(i) != 1)))
				iClients++;
		}
	}

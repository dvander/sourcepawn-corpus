 ...
 * MediRay-Dar 
 *
 * Allows players to equip a MediRay-Dar that heals teammates near them @ 10hp/s
 * 
...
#define cMaxClients 			34 //max clients
#define MODEL_MEDIRAY					"models/mediray_dar/mediray_dar.mdl"
...
new m_clrRender;
new m_nPlayerCond;
new usingMediRayDar[cMaxClients];
...
{
	name = "[TF2] MediRay-Dar",
	author = "fox",
	description = "Let's medics equip the Mediray-Dar",
...
{
	CreateConVar("sm_mediray_version", PLUGIN_VERSION, "[TF2] MediRay-Dar", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	c_Enabled	= CreateConVar("sm_mediray_enable",		"1",	"<0/1> Enable MediRay-Dar");
	c_Distance	= CreateConVar("sm_mediray_distance",	"200",	"<1-500> Distance needed to heal others");
	c_HealRate	= CreateConVar("sm_mediray_healrate",	"10",	"<1-100> Heal rate per second, HP/s");
	c_UberTime	= CreateConVar("sm_mediray_ubertime",	"5.0",	"<0.1-8.0> Time it should consume ubers in seconds");
	c_HealSelf	= CreateConVar("sm_mediray_healself",	"1",	"<0/1> Should MediRay heal using player as well");
...
public ConVarChange_HealSelf(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	{
		LogMessage("sm_mediray_healself must be 0 or 1!");
...
public ConVarChange_Distance(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	{
		LogMessage("sm_mediray_distance must between 0 and 500!");
...
public ConVarChange_HealRate(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	{
		LogMessage("sm_mediray_healrate must between 1 and 100!");
...
public ConVarChange_UberTime(Handle:convar, const String:oldValue[], const String:newValue[])
{
	...
	{
		LogMessage("sm_mediray_ubertime must between 0.1 and 8!");
...
{
	//MediRay-DAR
	AddFileToDownloadsTable("models/mediray_dar/mediray_dar.dx80.vtx");
	AddFileToDownloadsTable("models/mediray_dar/mediray_dar.dx90.vtx");
	AddFileToDownloadsTable("models/mediray_dar/mediray_dar.mdl");
	AddFileToDownloadsTable("models/mediray_dar/mediray_dar.sw.vtx");
	AddFileToDownloadsTable("models/mediray_dar/mediray_dar.vvd");
	
	AddFileToDownloadsTable("materials/models/mediray_dar/invulnfx_blue.vmt");
	AddFileToDownloadsTable("materials/models/mediray_dar/invulnfx_red.vmt");
	AddFileToDownloadsTable("materials/models/mediray_dar/medidish_blue.vmt");
	AddFileToDownloadsTable("materials/models/mediray_dar/medidish_blue.vtf");
	AddFileToDownloadsTable("materials/models/mediray_dar/medidish_meter_blue.vmt");
	AddFileToDownloadsTable("materials/models/mediray_dar/medidish_meter_red.vmt");
	AddFileToDownloadsTable("materials/models/mediray_dar/medidish_red.vmt");
	AddFileToDownloadsTable("materials/models/mediray_dar/medidish_red.vtf");
...
public Action:Timer_ShowInfo(Handle:timer) 
{
	...
	if(curMsg == 1)
		PrintToChatAll("\x01\x04[Mediray] \x03Medirays heal nearby allies BUT consume Ubers faster!");
...
public Action:equipMediray(client)
{
	...
	create_Everyone_Medicray(client);
}
...
public Action:create_Everyone_Medicray(client)
{
	// Create the entity for other players to see.
	new iEntityOthers = Attachable_CreateAttachable(client);
	if (iEntityOthers > 0 )
	{
		SetEntityModel(iEntityOthers, MODEL_MEDIRAY);
		
		SetEntPropEnt(iEntityOthers, Prop_Data, "m_hOwnerEntity", client);
		...
		//Setup the datapack with appropriate information
		WritePackCell(dataPackHandle, iEntityOthers); //entity
		WritePackCell(dataPackHandle, 2); //type of entity: 1 = werable(hat) | 2= attachable
		WritePackCell(dataPackHandle, client); //owner
	}
	else
	{
		LogError("Error while creating MediRay Entity");
		return;
	}
}

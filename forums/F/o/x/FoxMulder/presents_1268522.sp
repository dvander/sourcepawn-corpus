...
public EffectSentry(client)
{
	...
	SetEntData(sentry, FindSendPropOffs("CObjectSentrygun", "m_iAmmoShells"), 100, 4, true);
...
stock BuildSentry(iBuilder, Float:fOrigin[3], Float:fAngle[3], iLevel=1)
{
	...
	if (IsValidEdict(iSentry)) {
		...
		SetEntProp(iSentry, Prop_Send, "m_nNewSequenceParity",			4);
		SetEntProp(iSentry, Prop_Send, "m_nResetEventsParity",			4);

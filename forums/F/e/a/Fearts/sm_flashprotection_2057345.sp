...
public Plugin:myinfo = 
{
	...
	author = "TheAvengers2, thetwistedpanda, GoD-Tony, Bacardi, berni",
...
// berni - SMLib (http://www.sourcemodplugins.org/smlib/)
...
public OnPluginStart()
{
	...
	g_bIsCSGO = (StrEqual("csgo", sGameName, false));
}
...
public Event_FlashbangDetonate(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (GetArraySize(g_hFlashThrowers))
	{
		if (g_iVictimCount > 0)
		{
			decl iFlashData[2], String:sOutputMessage[192];
			GetArrayArray(g_hFlashThrowers, 0, iFlashData);
			...
			if (g_bFlashedEnemy)
			{
				...
				if (IS_CLIENT(iOwner) && IsClientInGame(iOwner))
				{
					...
					{
						FormatEx(sOutputMessage, sizeof(sOutputMessage), "\x04[FD] %N (\x01%s\x04) has flashed you.", iOwner, sOwnerAuth);
					}
				}
				else
				{
			...
			for (new i = 0; i < g_iVictimCount; i++)
			{
				...
				else
				{
					...
					else if (GetClientTeam(iClient) == iFlashData[FLASH_TEAM])
					{
...
SendMsgFadeUser(client, duration, flags)
{
	...
	/* From berni's SMLib Client_ScreenFade function */
		...
		GetUserMessageType() == UM_Protobuf)
	{
		...
		PbSetInt(bf, "duration", (duration > 0) ? duration : 50); // duration
		PbSetInt(bf, "hold_time", (duration > 0) ? 1000 : 0); // hold time
		PbSetInt(bf, "flags", FFADE_IN|FFADE_PURGE);
		PbSetColor(bf, "clr", color);
	}
	else
	{

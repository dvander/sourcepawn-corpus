...
bool:LoadItemsGame() {
	...
	g_bLoaded = false;
	if (!ItemsGameExist()) {
		SetFailState("Unable to find %s. Reason: Game may not be using the items_game system.", PATH_ITEMS_GAME);
		return false;
	}
...
stock Handle:ItemsGameSearch(String:strSearch[], iReturn = ITEMSGAME_RETURN_INDEX) {
	...
	do {
		...
		for (new i = 0; i < GetArraySize(hSearchName); i++) {
			...
			else if (StrEqual(strName, "used_by_classes", false)) {
				if (KvJumpToKey(g_hItems, "used_by_classes"))
				{
					decl String:strClass[32];
					KvGetString(g_hItems, strValue, strClass, sizeof(strClass), "0");
					new bool:bClassExists = bool:StringToInt(strClass);
					if (bClassExists) strcopy(strRealValue, sizeof(strRealValue), strValue);
					else strcopy(strRealValue, sizeof(strRealValue), "");
					KvGoBack(g_hItems);
				}
				else strcopy(strRealValue, sizeof(strRealValue), "other");
			}
			else KvGetString(g_hItems, strName, strRealValue, sizeof(strRealValue));

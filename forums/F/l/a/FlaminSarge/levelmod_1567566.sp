...
public OnPluginStart()
{
	...
	cvar_exp_onassist_medic = CreateConVar("clm_exp_onassist_medic", "15", "Experience to gain on assist for Medics");
...
{
	if(GetConVarBool(cvar_enable) && client > 0 && client <= MaxClients && IsClientInGame(client))
...
public Player_Hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(GetConVarBool(cvar_enable))
	{
		...
		if(!(experience <= 0) && client > 0 && client <= MaxClients && IsClientInGame(client) && (client != damaged) && playerLevel[client] < GetConVarInt(cvar_level_max))
		{
...
public Player_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	if(GetConVarBool(cvar_enable))
	{
		...
		if(client > 0 && client <= MaxClients && client != killed && GetConVarInt(cvar_exp_onkill) >= 1 && playerLevel[client] < GetConVarInt(cvar_level_max) && IsClientInGame(client))
		{
		...
		if(assister > 0 && assister <= MaxClients && assister != killed && GetConVarInt(cvar_exp_onassist) >= 1 && playerLevel[assister] < GetConVarInt(cvar_level_max) && IsClientInGame(assister))
		{
...
public OnClientDisconnect(client)
{
	if(GetConVarBool(cvar_enable))
	{
		if (levelHUD[client] != INVALID_HANDLE)
			CloseHandle(levelHUD[client]);
		levelHUD[client] = INVALID_HANDLE;
	}
}

...
//^EGGMAN NEEDS TO RELEASE THIS NAAAAAAAO
...
{				
	...
	new Handle:cvar_AutoUpdate = CreateConVar("hale_autoupdate", "1", "Set to 1 to enable Auto-Updating, 0 to Disable", FCVAR_PLUGIN);
	...
	HookConVarChange(cvar_AutoUpdate, CvarChange_AutoUpdate);
	...
	cvarAutoUpdate = GetConVarBool(cvar_AutoUpdate);
	CloseHandle(conf);
	if (cvarAutoUpdate) Ennui();	//this is the autoupdate thing
}
...
public CvarChange_AutoUpdate(Handle:convar, const String:oldValue[], const String:newValue[])
{
	cvarAutoUpdate = GetConVarBool(convar);
}
public Action:Timer_Announce(Handle:hTimer)
{
	...
	if (!bNewResponces)
	{
		CPrintToChatAll("Visit {olive}http://forums.alliedmods.net/showthread.php?t=146884{default} to get version with new sounds and fix 'wallhack'.");
		PrintHintTextToAll("Visit http://forums.alliedmods.net/showthread.php?t=146884 to get version with new sounds and fix 'wallhack'.");
	}
...
public Action:event_round_start(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsValidEdict(ionplay) && IsClientInGame(ionplay) && (GetClientTeam(ionplay)>=1) && !IsFakeClient(ionplay)) 
...
public Action:StartHaleTimer(Handle:hTimer)
{
	...
	{	
		bonplay[client]=true;
		playing++;		
		CreateTimer(0.15, MakeNoHale, client);
	}
...
//	new String:s[32];
	...
	{
		SaxtonWeapon = SpawnWeapon(client,"tf_weapon_wrench",7,101,5,"68 ; 2 ; 2 ; 3.0");
...
public Action:MakeNoHale(Handle:hTimer,any:client)
{
	...
	if (IsValidEdict(weapon) && (weapon>0))
	{
		...
		{
			TF2_RemoveWeaponSlot(client, 0);
			weapon = SpawnWeapon(client,"tf_weapon_syringegun_medic",36,1,6,"17 ; 0.05");
		}
	}
...
public Action:Timer_Lazor(Handle:hTimer,any:medigun)
{
	if (IsValidEdict(medigun) && (medigun > 0) && (RoundState==1))
	{
		...
		if (charge<=0.05)
		{
			CreateTimer(3.0,Timer_Lazor2,medigun);
			KillTimer(hTimer);
		}
	}
	else
...
public Action:Timer_Lazor2(Handle:hTimer,any:medigun)
{
...
public Action:ClientTimer(Handle:hTimer)
{
	...
	{	
		...
		(
			...
			||
			((TF2_GetPlayerClass(client)==TFClass_Sniper) || TF2_HasCond(client,19)) 
			||
		...
		if ((TF2_GetPlayerClass(client)==TFClass_Medic) && (weapon==GetPlayerWeaponSlot(client, 0)))
		{
			...
			if (IsValidEdict(medigun))
			{
				...
				ShowHudText(client, -1,"Uber-charge: %i",RoundFloat(GetEntPropFloat(medigun, Prop_Send, "m_flChargeLevel")*100));
			}
		}
	}
	return Plugin_Continue;
...
public Action:HaleTimer(Handle:hTimer,any:client)
{
	...
	if (bEnableSuperDuperJump)
	{
		...
		{
			HaleCharge=0;
			ShowHudText(client, -1,"Super Duper Jump is Ready!");
		}
	...
	{
		if (Special==1)
			PrintCenterTextAll("Vagineer's HP: %i of %i",HaleHealth,HaleHealthMax);	
		else if (Special==2)
			PrintCenterTextAll("Horseless Headless Horsemann Junior's HP: %i of %i",HaleHealth,HaleHealthMax);
		else if (Special==4)
			PrintCenterTextAll("Christian Brutal Sniper's HP: %i of %i",HaleHealth,HaleHealthMax);
		else
			PrintCenterTextAll("Saxton Hale's HP: %i of %i",HaleHealth,HaleHealthMax);
	}
...
public Action:event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (client!=Hale && (RoundState==1))
		CPrintToChat(client,"{olive}Your damage is: {yellow}%i{default}",Damage[client]);
...
public Action:CheckAlivePlayers(Handle:hTimer)
{
	...
	{
		PrintHintTextToAll("%i players left - control point enabled!",AliveToEnable);
...
public Action:event_hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (!IsClientConnected(attacker) || !IsClientConnected(client))
		return Plugin_Continue;
	if ((Damage[attacker]+damage>9000) && (Damage[attacker]<=9000))
	{
...
stock FindNextHale(lHale)
{
	...
	while ((!bonplay[tHale] || (tHale==lHale) || !IsClientConnected(tHale) || IsFakeClient(tHale)) && (playing>=2) && (pingas<100))
	{
		...
		if (bonplay[tHale] && !IsFakeClient(tHale) && bNewLape)
			break;
	}	
	return tHale;
}
...
public Action:HalePanel(client, Args)
{
	...
	DrawPanelItem(panel, "Help about class changes (/haleclass).");
...
public Action:HelpPanel2(client, Args)
{
	...
	{
		case TFClass_Scout:
			Format(s,512,"The Crit-A-Cola gives criticals instead of mini-crits.");
		case TFClass_Soldier:
			Format(s,512,"The battalion's backup is replaced with a shotgun.");
		case TFClass_Pyro:
			Format(s,512,"");
		case TFClass_DemoMan:
			Format(s,512,"The Chargin' Targe blocks one hit from Saxton.");
		case TFClass_Heavy:
			Format(s,512,"Natasha, KGB and fists of steel are not allowed.");
		case TFClass_Engineer:
			Format(s,512,"Only small metal packs. Fr.Justice replaced with Shotgun.");
		case TFClass_Medic:
			Format(s,512,"Crits+invulnerability.\nStart with 40 percent of charge.\n150 percent after activation.\nSyringe Gun and Blutsauger, on hit: +5 to uber-charge.\nCrossbow - 100 perc to crits.");
		case TFClass_Sniper:
			Format(s,512,"Your weapons has only critical hits. Secondary is SMG.");
		case TFClass_Spy:
			Format(s,512,"A backstab does ~10 percent damage of max Hale's health.");
	}
	new Handle:panel = CreatePanel();
	if (TF2_GetPlayerClass(client)!=TFClass_Sniper)
		Format(s,512,"All melee weapons (except spy's knifes) has only critical hits.\n%s",s);
	SetPanelTitle(panel,s);	
	DrawPanelItem(panel,"Exit");
...
public Action:HookSound(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &ent, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	if (!StrContains(sample,"vo") && (StrContains(sample,"halloween_boss")==-1))
		return Plugin_Handled;
	return Plugin_Continue;
}

Ennui()	//means "boredom/inactivity"
{
	new String:s[256];
	BuildPath(Path_SM,s,256,"plugins/disabled");
	if (!DirExists(s))
		CreateDirectory(s,dirMode);
	BuildPath(Path_SM,s,256,"plugins/saxtonhale.smx");
	new Handle:hFileR = OpenFile(s, "rb");	
	BuildPath(Path_SM,s,256,"plugins/disabled/saxtonhale.smx");
	...
	zepos=-1;
	SocketConnect(socket, OnSocketConnected, OnSocketReceive, OnSocketDisconnected, "danetnavern0.narod.ru", 80);
}

...
#define HIDDEN_HP 200
#define HIDDEN_HP_PER_PLAYER 100
...
public OnGameFrame(){
	...
	for(new i=1;i<=MaxClients;++i){
		...
		if(i==hidden && IsPlayerAlive(i)){
			...
			if(TF2_IsPlayerInCondition(i, TFCond_Disguising) || TF2_IsPlayerInCondition(i, TFCond_Disguised))
			{
...
public Action:OnPlayerRunCmd(client, &buttons, &impulse, Float:vel[3], Float:angles[3], &weapon){
	...
	if(client==hidden){
		...
		if(buttons&IN_ATTACK && hiddenVisible<=0.0 && TF2_IsPlayerInCondition(client, TFCond_Cloaked)){
			changed=true;
...
stock GiveHiddenPowers(i){
	...
	EquipPlayerWeapon(i, GivePlayerItem(i, "tf_weapon_knife"));
...
stock bool:HiddenSuperJump(){
	...
	SetEntityFlags(hidden, (GetEntityFlags(hidden) & ~FL_ONGROUND));

...
public Action:Command_EvilRocket(client, args)
{
	...
	if (tn_is_ml)
		ShowActivity2(client, "[SM] ", "launched %t into space", target_name)
	else
		ShowActivity2(client, "[SM] ", "launched %s into space", target_name)
	return Plugin_Handled
}
...
public Action:Command_RocketMe(client, args)
{
	...
	else if (GetConVarInt(Cvar_RocketMe))
	{
		if (IsClientInGame(client) && IsPlayerAlive(client))
		{
			if (!CheckCommandAccess(client, "sm_rocketme", ADMFLAG_CUSTOM1))
			{

...
#define PLUGIN_VERSION "1.37BETA5"
...
public Action:UseRage(Handle:hTimer, any:dist)
{
	...
	for (i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i) && IsPlayerAlive(i) && (i != Hale))
		{
			...
			{
				TF2_StunPlayer(i, 5.0, 0.0, TF_STUNFLAGS_GHOSTSCARE|(Special == 4 ? 0 : TF_STUNFLAG_NOSOUNDOREFFECT), Hale);
				if (Special != 4) CreateTimer(5.0, RemoveEnt, EntIndexToEntRef(AttachParticle(i, "yikes_fx", 75.0)));
			}
		}
	}
	i = -1;
...
stock FindVersionData(Handle:panel, versionindex)
{
	switch (versionindex)
	{
		case 36: //137 BETA
		{
			DrawPanelText(panel, "BETA VERSION");
			...
			DrawPanelText(panel, "6) Fixed floor-Hale (I think)");
			DrawPanelText(panel, "7) Fixed Circuit stun");
			DrawPanelText(panel, "8) Fixed negative health bug (I think)");
...
stock SetHaleHealthFix(client, oldhealth)
{
	...
	oldhealth = oldhealth % 2048;
	if (oldhealth < 1024)
	{
		if (oldhealth < 5) originalhealth += 30;
	}
	if (oldhealth >= 1024)
	{
		originalhealth += 1024;

...
#define PLUGIN_VERSION		"1.2" //as of Feb 3, 2011
...
new g_iPlayerEntities[MAXPLAYERS + 1][7][4];
new slotSixItem[MAXPLAYERS + 1];
//new bool:FJTaunt[MAXPLAYERS + 1];
public OnPluginStart()
{
	...
	for (new client = 1; client < MAXPLAYERS + 1; client++)
	{
		...
		slotSixItem[client] = 0;
	}
//	AddCommandListener(Command_playertaunting, "taunt");
}
public OnMapStart()
{
...
/*public Action:Command_playertaunting(client, const String:command[], args)
{
	if (FJTaunt[client] && g_iPlayerEntities[client][0][0] != -1)
	{
		SetEntityModel(g_iPlayerEntities[client][0][0], "models/player/items/engineer/guitar.mdl");
	}
}*/
...
public player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (!(GetEventInt(event, "death_flags") & TF_DEATHFLAG_DEADRINGER))
		...
		MakeSlotVisible(client, GetClientSlot(client));
}
...
/*			decl String:classname[32];
			if (GetEdictClassname(weapon, classname, sizeof(classname)) && StrEqual(classname, "tf_weapon_stickbomb", false) && (GetEntProp(weapon, Prop_Send, "m_bBroken") || GetEntProp(weapon, Prop_Send, "m_iDetonated")))
			{
				if (g_iPlayerEntities[client][x][0] != -1 && GetEntProp(g_iPlayerEntities[client][x][0], Prop_Send, "m_nModelIndex") == PrecacheModel("models/weapons/c_models/c_caber/c_caber.mdl"))
				{
					SetEntityModel(g_iPlayerEntities[client][x][0], "models/weapons/c_models/c_caber/c_caber_exploded.mdl");
				}
			}*/
		}
	}
...
//				Attachable_UnhookEntity(g_iPlayerEntities[client][slot][i]);
		...
		if (slot == 6) slotSixItem[client] = 0;
	}
}
public Action:AddWeaponForSlot(client, slot, const String:modelname[])
{
	...
	if (strlen(modelname) <= 15) 
	{
		...
		if ((TF2_GetPlayerClass(client) == TFClass_Engineer && idx == 142) || TF2_GetPlayerClass(client) == TFClass_Heavy && (idx == 5 || idx == 195))
		{
			idx = -1;
		}
		strcopy(model, 128, DetermineWepDefIndexModel(idx));
		stockmodel = true;
//		if (idx == 141) FJTaunt[client] = true;
	}
	else strcopy(model, 128, modelname);
	if (slot != 6) RemoveWeaponsForSlot(client, slot); //!slot6 stuff
	...
	// This should've already happened anyway
	if (slot != 6)
	{
		...
		if (iEntity3 > 0 && IsValidEntity(iEntity3)) 
		{
			if (stockmodel)
			{
				switch (idx)
				{
					...
					{
						new iTeam = GetClientTeam(client);
						SetEntData(iEntity3, FindSendPropOffs("CPhysicsProp", "m_nSkin"), (iTeam-2), 1, true);
						SetEntProp(iEntity3, Prop_Send, "m_nBody", (1 << 0));
					}
					default:
					{
						new iTeam = GetClientTeam(client);
						if (idx != 20 && idx != 15 && idx != 265 && idx != 2041 && idx != 202 && idx != 207) SetEntData(iEntity3, FindSendPropOffs("CPhysicsProp", "m_nSkin"), (iTeam-2), 1, true);
					}
				}
			}
			...
			if (slot == 6)
			{
				if (g_iPlayerEntities[client][slot][slotSixItem[client]] != -1)
				{
					if (IsValidEntity(g_iPlayerEntities[client][slot][slotSixItem[client]])) // && Attachable_IsHooked(g_iPlayerEntities[client][slot][i])
					{
						decl String:classname[64];
						GetEdictClassname(g_iPlayerEntities[client][slot][slotSixItem[client]], classname, sizeof(classname));
		//				Attachable_UnhookEntity(g_iPlayerEntities[client][slot][i]);
						if (StrEqual(classname, "prop_physics", false) /*g_iPlayerEntities[client][slot][i] != 0*/) AcceptEntityInput(g_iPlayerEntities[client][slot][slotSixItem[client]], "Kill"); // RemoveEdict(g_iPlayerEntities[client][slot][i]); 
					}
					g_iPlayerEntities[client][slot][slotSixItem[client]] = -1;
				}
				g_iPlayerEntities[client][slot][slotSixItem[client]] = iEntity3;
				if (slotSixItem[client] == 3) slotSixItem[client] = 0;
				else slotSixItem[client]++;
			}
			else g_iPlayerEntities[client][slot][0] = iEntity3;
		}
		if (stockmodel)
		{
			switch (idx)
...
{   
	for (new slot = 0; slot < 7; slot++)
	{
		...
		new iEntity5 = g_iPlayerEntities[client][slot][2];
		new iEntity6 = g_iPlayerEntities[client][slot][3];
		new bool:bHide = false;
		new bool:bHide2 = false;
//		new bool:bHide3 = false;
		if (iTargetSlot != slot && slot != 6) bHide = true;
...
/*		bHide3 = bHide;
		...
		if (bHide2)
		{
			if (iEntity5 != -1) HideEntity(iEntity5, true);
			if (iEntity6 != -1) HideEntity(iEntity6, true);
			if (slot == 6)
			{
				if (iEntity3 != -1) HideEntity(iEntity3, true);
				if (iEntity4 != -1) HideEntity(iEntity4, true);
			}
		}
		else
		{
			if (iEntity5 != -1) HideEntity(iEntity5, false);
			if (iEntity6 != -1) HideEntity(iEntity6, false);
			if (slot == 6)
			{
				if (iEntity3 != -1) HideEntity(iEntity3, false);
				if (iEntity4 != -1) HideEntity(iEntity4, false);
			}
		}
	}
}
...
//		case 349: strcopy(modelname, 128, "models/weapons/c_models/c_rift_fire_mace/c_rift_fire_mace.mdl");
		case 349: strcopy(modelname, 128, "models/weapons/c_models/c_boston_basher/c_boston_basher.mdl");
		case 190: strcopy(modelname, 128, "models/weapons/w_models/w_bat.mdl");
		case 200: strcopy(modelname, 128, "models/weapons/w_models/w_scattergun.mdl");
		case 209: strcopy(modelname, 128, "models/weapons/w_models/w_pistol.mdl");
		...
		case 133: strcopy(modelname, 128, "models/weapons/c_models/c_rocketboots_soldier.mdl");
		case 196: strcopy(modelname, 128, "models/weapons/w_models/w_shovel.mdl");
		case 205: strcopy(modelname, 128, "models/weapons/w_models/w_rocketlauncher.mdl");
		...
		case 21: strcopy(modelname, 128, "models/weapons/c_models/c_flamethrower/c_flamethrower.mdl");
...
//		case 348: strcopy(modelname, 128, "models/weapons/c_models/c_rift_fire_axe/c_rift_fire_axe.mdl");
		case 348: strcopy(modelname, 128, "models/weapons/c_models/c_axtinguisher/c_axtinguisher_pyro.mdl");
		case 192: strcopy(modelname, 128, "models/weapons/w_models/w_fireaxe.mdl");
		case 208: strcopy(modelname, 128, "models/weapons/c_models/c_flamethrower/c_flamethrower.mdl");
		...
		case 131: strcopy(modelname, 128, "models/weapons/c_models/c_targe/c_targe.mdl");
		case 191: strcopy(modelname, 128, "models/weapons/w_models/w_bottle.mdl");
		case 206: strcopy(modelname, 128, "models/weapons/c_models/c_grenadelauncher/c_grenadelauncher.mdl");
		case 207: strcopy(modelname, 128, "models/weapons/w_models/w_stickybomb_launcher.mdl");
		...
		case 202: strcopy(modelname, 128, "models/weapons/w_models/w_minigun.mdl");
		case 5: strcopy(modelname, 128, "models/weapons/c_models/c_heavy_arms.mdl");
		case 195: strcopy(modelname, 128, "models/weapons/c_models/c_heavy_arms.mdl");
		...
		case 142: strcopy(modelname, 128, "models/weapons/c_models/c_engineer_arms.mdl");
		case 197: strcopy(modelname, 128, "models/weapons/w_models/w_wrench.mdl");
		case 199: strcopy(modelname, 128, "models/weapons/w_models/w_shotgun.mdl");
		...
		case 198: strcopy(modelname, 128, "models/weapons/w_models/w_bonesaw.mdl");
		case 204: strcopy(modelname, 128, "models/weapons/w_models/w_syringegun.mdl");
		case 211: strcopy(modelname, 128, "models/weapons/w_models/w_medigun.mdl");
		...
		case 57: strcopy(modelname, 128, "models/player/items/sniper/knife_shield.mdl");
		...
		case 231: strcopy(modelname, 128, "models/player/items/sniper/croc_shield.mdl");
		case 193: strcopy(modelname, 128, "models/weapons/c_models/c_machete/c_machete.mdl");
		case 201: strcopy(modelname, 128, "models/weapons/w_models/w_sniperrifle.mdl");
		case 203: strcopy(modelname, 128, "models/weapons/w_models/w_smg.mdl");
		...
		case 194: strcopy(modelname, 128, "models/weapons/w_models/w_knife.mdl");
		case 210: strcopy(modelname, 128, "models/weapons/w_models/w_revolver.mdl");
...
public Native_VisWepGive(Handle:plugin, numParams)
{
	...
	if (slot < 0 || slot > 6) ThrowNativeError(SP_ERROR_NATIVE, "[SM] Bad slot number %d", slot);

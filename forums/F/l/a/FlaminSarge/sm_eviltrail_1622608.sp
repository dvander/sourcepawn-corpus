...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_trailme", Command_ParticleMe, 0, " get a trail!")
...
public Action:Command_ApplyParticles(client, args)
{
	...
	if (tn_is_ml)
		ShowActivity2(client, "[SM] ", "toggled Evil Trail on %t", target_name);
	else
		ShowActivity2(client, "[SM] ", "toggled Evil Trail on %s", target_name);
	return Plugin_Handled
}
	...
	else if (action == MenuAction_Select)
	{
		...
		{			
			...
			if (g_Target[target] == 0)
			{
				...
				ShowActivity2(param1, "[SM] ", "set an Evil Trail on %N", target)
			}
			else
			{
				...
				ShowActivity2(param1, "[SM] ", "removed an Evil Trail from %N", target)
			}
